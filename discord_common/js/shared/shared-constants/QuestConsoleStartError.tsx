// discord_common/js/shared/shared-constants/QuestConsoleStartError.tsx
import size from "../../../../_runtime/metro/00002__.js";

const result = size.fileFinishedImporting("../discord_common/js/shared/shared-constants/QuestConsoleStartError.tsx");

export const QuestConsoleStartError = {
  NO_GAME_ONLINE: "no_game_online",
  NO_GAME_OFFLINE: "no_game_offline",
  NO_GAME_UNKNOWN: "no_game_unknown",
  NO_GAME_CLOAKED: "no_game_cloacked",
  LOCKED_ACCOUNT: "locked_account",
  CHILD_ACCOUNT: "child_account",
  EXPIRED_CREDENTIAL: "expired_credential",
  PROVIDER_UNAVAILABLE: "provider_unavailable",
};
