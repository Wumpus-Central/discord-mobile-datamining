...
prototype["trackExperimentExposure"] = function trackExperimentExposure(
  ...
  if ("user" === unit_type) {
    self.withExposureTracking(tmp3, () =>
      self.track(
        WebAnalyticsEvents.EXPERIMENT_USER_EVALUATION_EXPOSED,
        { evaluation_id, experiment, exposure_location, unit_type, tracked_variation_id },
        { flush: true },
      ),
    );
    ...
    self.withExposureTracking(tmp3, () =>
      self.track(
        WebAnalyticsEvents.EXPERIMENT_INSTALLATION_EVALUATION_EXPOSED,
        { evaluation_id, installation_id, experiment, exposure_location, unit_type, tracked_variation_id },
        { flush: true },
      ),
    );
    ...
    self.withExposureTracking(tmp3, () =>
      self.track(
        WebAnalyticsEvents.EXPERIMENT_GUILD_EVALUATION_EXPOSED,
        { evaluation_id, guild_id, experiment, exposure_location, unit_type, tracked_variation_id, revision },
        { flush: true },
      ),
    );
  }
...
prototype["trackCommonTriggerPointExposures"] = function trackCommonTriggerPointExposures(location) {
  ...
  function _loop(evaluationId) {
    ...
    self.withExposureTracking(tmp3, () =>
      self.track(
        WebAnalyticsEvents.EXPERIMENT_USER_EVALUATION_EXPOSED,
        { evaluation_id: evaluationId, exposure_location: evaluationId, unit_type: "user" },
        { flush: true },
      ),
    );
  }
  ...
  function _loop2(evaluationId, unitId) {
    ...
    _self.withExposureTracking(tmp3, () =>
      self.track(
        WebAnalyticsEvents.EXPERIMENT_INSTALLATION_EVALUATION_EXPOSED,
        { evaluation_id: evaluationId, exposure_location: evaluationId, unit_type: "installation", installation_id },
        { flush: true },
      ),
    );
  }
    ...
    if ("user" === tmp.kind) {
      ...
      if (null != first) {
        const obj2 = {
          experiment: name,
          unit_type: tmp.kind,
          suppression_source: client_override,
