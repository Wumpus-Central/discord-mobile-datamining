...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f77924(unit.unit), items.findIndex((max) => {
    max = max.max;
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    const tmp12 = f77924(4560)(items, (unit) => f77924(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === closure_1_10.NONE;
      if (tmp) {
        tmp = closure_0 === max;
