...
import reduceDefault from "../../../../../_runtime/04950_reduce.js";
import _modDef4953 from "../../../../../_runtime/metro/04953__.js";
import _slicedToArray from "../../../../../_runtime/metro/00032__.js";
import BaseConnection from "../BaseConnection.tsx";

require = fn;
let Constants = fn(4854);
({
...
class Connection extends tmp4 {
  constructor(arg0, arg1, arg2) {
    ...
    tmp1.handleSpeakingFlags = function handleSpeakingFlags(id, flag, arg2) {
      ...
      const experimentFlags = closure_0.experimentFlags;
      if (!experimentFlags.has(constants.SWALLOW_VOLUME_ONLY_SPEAKING_EVENTS)) {
        ...
        if (id === closure_0.userId) {
          let audioSSRC = closure_0.audioSSRC;
        } else {
            ...
            if (0 === diff) {
              ...
              if (diff1 >= 0) {
                closure_0.emit(
                  ...
                  100 * _modDef4953(diff1 / (diff + diff1), 0, 1),
                );
                const tmp6 = _modDef4953(diff1 / (diff + diff1), 0, 1);
              }
            }
            const outbound = rtp.rtp.outbound;
            const first = outbound.filter((type) => "audio" === type.type)[0];
            const outbound1 = closure_0.stats.rtp.outbound;
                ...
                if (null != first.framesCaptured) {
                  if (null != first1.framesCaptured) {
                    ...
                    if (!obj2.supportsFeature(constants8.KRISP_NATIVE_ERROR)) {
                      ...
                      if (closure_0.inputMode === constants3.VOICE_ACTIVITY) {
  ...
  const replayConnection = voiceEngine.createReplayConnection(
    ...
    () => {
      ...
      const codecCapabilities = voiceEngine.getCodecCapabilities((arg0) => {
        ...
        const conn = videoSupported.conn;
        conn.startReplay();
...
prototype["initialize"] = function initialize(address) {
      ...
      if (!self.destroyed) {
        ...
        if (null == transportInfo) {
          ...
          const codecCapabilities = voiceEngine.getCodecCapabilities((arg0) => {
            ...
            const experimentCodecs = createVoiceConnection(fnResult[7]).getExperimentCodecs(
              connectionTransportOptions.experimentFlags,
            );
            ...
            const codecs1 = connectionTransportOptions.codecs;
            ...
            const encryptionModes = port.getEncryptionModes((arg0) => {
              ...
              if (!selfMute) {
                selfMute = connectionTransportOptions.context === constants3.STREAM;
              }
              port.setSelfMute(selfMute);
              port.setSelfDeafen(connectionTransportOptions.selfDeaf);
              const result = port.setOnSpeakingCallback(connectionTransportOptions.handleSpeakingNative);
              if (port.setOnNativeMuteChangedCallback != null) {
                const result1 = setOnNativeMuteChangedCallback(connectionTransportOptions.handleNativeMuteChanged);
              }
              if (port.setOnSpeakingWhileMutedCallback != null) {
                const result2 = setOnSpeakingWhileMutedCallback(connectionTransportOptions.handleSpeakingWhileMuted);
              }
              const setPingInterval = port.setPingInterval;
              ...
              if (port.setPingTimeoutCallback != null) {
                const result3 = setPingTimeoutCallback(connectionTransportOptions.handlePingTimeout);
              }
              if (port.setOnVideoEncoderFallbackCallback != null) {
                const result4 = setOnVideoEncoderFallbackCallback(
                  connectionTransportOptions.handleVideoEncoderFallback,
                );
              }
              if (port.setOnVideoDecoderFallbackCallback != null) {
                const result5 = setOnVideoDecoderFallbackCallback(
                  connectionTransportOptions.handleVideoDecoderFallback,
                );
              }
              if (port.setVideoCodecErrorCallback != null) {
                const result6 = setVideoCodecErrorCallback(connectionTransportOptions.handleVideoCodecError);
              }
              voiceEngine.setTransportOptions({
                builtInEchoCancellation: true,
                echoCancellation: connectionTransportOptions.echoCancellation,
                noiseSuppression: connectionTransportOptions.noiseSuppression,
                automaticGainControl: connectionTransportOptions.automaticGainControl.enabled,
                automaticGainControlConfig: connectionTransportOptions.automaticGainControl,
                noiseCancellation: connectionTransportOptions.noiseCancellation,
                noiseCancellationDuringProcessing: connectionTransportOptions.noiseCancellationDuringProcessing,
              });
              voiceEngine.setNoInputThreshold(-100);
              voiceEngine.setNoInputCallback(connectionTransportOptions.handleNoInput);
              if (connectionTransportOptions.videoSupported) {
                port.setOnVideoCallback(connectionTransportOptions.handleVideo);
                if (port.setOnFirstFrameCallback != null) {
                  const result7 = setOnFirstFrameCallback(connectionTransportOptions.handleFirstFrame);
                }
                if (port.setOnFirstFrameDeliveredStatsCallback != null) {
                  const result8 = setOnFirstFrameDeliveredStatsCallback(
                    connectionTransportOptions.handleFirstFrameStats,
                  );
                }
                if (port.setOnFirstFrameEncryptedStatsCallback != null) {
                  const result9 = setOnFirstFrameEncryptedStatsCallback(
                    connectionTransportOptions.handleFirstFrameEncryptedStats,
                  );
                }
                const setOnDesktopSourceEnded = port.setOnDesktopSourceEnded;
                if (setOnDesktopSourceEnded != null) {
                  const result10 = setOnDesktopSourceEnded(connectionTransportOptions.handleDesktopSourceEnded);
                }
                const setOnSoundshare = port.setOnSoundshare;
                if (setOnSoundshare != null) {
                  setOnSoundshare(connectionTransportOptions.handleSoundshare);
                }
                const setOnSoundshareEnded = port.setOnSoundshareEnded;
                if (setOnSoundshareEnded != null) {
                  setOnSoundshareEnded(connectionTransportOptions.handleSoundshareEnded);
                }
                const setOnSoundshareFailed = port.setOnSoundshareFailed;
                if (setOnSoundshareFailed != null) {
                  const result11 = setOnSoundshareFailed(connectionTransportOptions.handleSoundshareFailed);
                }
              }
              if (port.setOnMLSFailureCallback != null) {
                const result12 = setOnMLSFailureCallback(connectionTransportOptions.handleMLSFailure);
              }
              connectionTransportOptions.setConnectionState(constants.CONNECTED);
              connectionTransportOptions.emit(createVoiceConnection(fnResult[6]).BaseConnectionEvent.Connected, map, {
                address,
                port,
                mode: connectionTransportOptions.chooseEncryptionMode(address.modes, arg0),
                codecs: connectionTransportOptions.codecs,
              });
              connectionTransportOptions.on(
                createVoiceConnection(fnResult[6]).BaseConnectionEvent.Stats,
                connectionTransportOptions.handleStats,
              );
              const userOptions = connectionTransportOptions.getUserOptions();
              ...
              connectionTransportOptions.emit(
                createVoiceConnection(fnResult[6]).BaseConnectionEvent.RemoteStreamsReady,
                userOptions.length,
              );
              const keys = Object.keys(connectionTransportOptions.localSpeakingFlags);
              for (const item10172 of keys) {
                if (item10172 !== connectionTransportOptions.userId) {
                  let setSpeakingFlagsResult = connectionTransportOptions.setSpeakingFlags(
                    item10172,
                    connectionTransportOptions.localSpeakingFlags[item10172],
                  );
                }
                continue;
              }
  ...
  let obj2 = createVoiceConnection(1994);
};
...
prototype["getStats"] = function getStats() {
  ...
  if (this.connectionState === constants4.DISCONNECTED) {
    ...
    let obj = self(4945);
    resolved = self(4945)
      .timeout(promise, self(4894).STATS_INTERVAL)
      .catch((error) => {
        if (!(error instanceof self(4945).TimeoutError)) {
          throw error;
        }
      });
    const timeoutResult = self(4945).timeout(promise, self(4894).STATS_INTERVAL);
  }
  return resolved;
};
...
prototype["setForceAudioInput"] = function setForceAudioInput(arg0) {
  if (flag === undefined) {
    flag = false;
  }
  let flag2 = arg2;
  ...
  if (someResult) {
    if (self.initialCodecs.length > 0) {
      if (self.lastOverrideCodecDenylist.length > 0) {
        ...
        new Set(parts.map((item) => item.trim().toUpperCase()));
      }
  ...
  return new Promise((fn, arg1) => {
    ...
    function _loop(iter) {
      ...
      if (-1 === findIndexResult) {
        ...
        const items = [];
        if (!_modDef4948(self.videoStreamParameters[findIndexResult], closure_1[findIndexResult])) {
          const obj = {};
          const merged = Object.assign(closure_1[findIndexResult]);
          self.videoStreamParameters[findIndexResult] = obj;
          const obj2 = {};
          const merged1 = Object.assign(closure_1[findIndexResult]);
          items.push(obj2);
        }
        const conn = self.conn;
...
prototype["getUserOptions"] = function getUserOptions() {
  ...
  return keys.map((id) => {
    ...
    const obj2 = {
      id,
      ssrc: self.remoteAudioSSRCs[id],
      videoSsrc: num,
      videoSsrcs: self.remoteVideoSSRCs[id],
      rtxSsrc: null,
      mute: null,
      volume: null,
    };
