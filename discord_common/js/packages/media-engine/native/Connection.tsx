...
import destroyDefault from "destroy" /* 4557 */;
import WantsVideoQuality from "WantsVideoQuality" /* 4558 */;
import VADAggressiveness2 from "VADAggressiveness" /* 4610 */;
...
prototype["getStats"] = function getStats() {
  ...
  if (this.connectionState === constants4.DISCONNECTED) {
    ...
    let obj = self(4606);
    resolved = self(4606).timeout(promise, self(4555).STATS_INTERVAL).catch((arg0) => {
      if (!(arg0 instanceof self(table[8]).TimeoutError)) {
        throw arg0;
      }
    });
    const timeoutResult = self(4606).timeout(promise, self(4555).STATS_INTERVAL);
  }
  return resolved;
};
...
prototype["setAudioVideoOverridesTransport"] = function setAudioVideoOverridesTransport(overrideDeniedVideoCodecs) {
  ...
  if (someResult) {
    if (self.initialCodecs.length > 0) {
      ...
      self.emit(set(4545).BaseConnectionEvent.VideoEncoderFallback, self.codecs);
    }
  }
};
    ...
    if (null != calcMaxBitrateFuncResult) {
      ...
      if (tmp12) {
        ...
        if (self.videoStreamParameters.length <= num5) {
          const Video = tmp9(4545).BaseConnectionEvent.Video;
