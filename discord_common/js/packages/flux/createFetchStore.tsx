...
export const createFetchStore = function createFetchStore(ApplicationStore, initialize) {
  ({ getQueryId: importDefault, get: dependencyMap, load: asyncGeneratorStep, getIsLoading: _slicedToArray, getError: closure_5, retryConfig } = initialize);
  ...
  getUseStoreState = function getUseStoreState(queryId) {
    if (null == queryId) {
      ...
      if (null == value) {
        obj = _mod560.create(() => ({ isLoading: false, error: null, backoff: closure_1_7(), lastSuccessAt: null, failureLockedUntil: null }));
  ...
  function useCfsHook() {
    ...
    if (null == tmp9) {
      ...
      value = map.get(tmp9);
      if (null == value) {
        obj = ApplicationStore(560).create(() => ({ isLoading: false, error: null, backoff: closure_1_7(), lastSuccessAt: null, failureLockedUntil: null }));
    ...
    const iter = (async () => {
      await Promise.all(closure_129_0.map((args) => {
        ...
        if (null == tmp) {
          ...
          if (null == value) {
            obj = closure_0(dependencyMap[5]).create(() => ({ isLoading: false, error: null, backoff: closure_1_7(), lastSuccessAt: null, failureLockedUntil: null }));
