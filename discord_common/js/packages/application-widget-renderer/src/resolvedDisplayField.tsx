...
export const resolveTextComponentValues = function resolveTextComponentValues(subtitle_1, resolveFieldValue, numberFormat, arg3) {
  ...
  if (null == subtitle_1) {
    ...
    const items = [resolveFieldValue.ResolvedValueType.STRING, resolveFieldValue.ResolvedValueType.NUMBER];
    let iter = resolveFieldValue(subtitle_1.fields.text, items);
    let str = null;
    if (!flag2) {
      const items1 = [tmp9(12423).ResolvedValueType.STRING, tmp9(12423).ResolvedValueType.NUMBER];
    ...
    const items2 = [resolveFieldValue.ResolvedValueType.MEDIA];
...
export const resolveStatComponentValues = function resolveStatComponentValues(fields, closure_1, closure_2, formatDurationNarrow, arg4) {
  ...
  if (null == fields) {
    ...
    const items1 = [resolveFieldValue.ResolvedValueType.STRING];
    ...
    if (null == iter) {
      ...
      if (iter.type === tmp6(12423).ResolvedValueType.STRING) {
        let formatResult = iter.value;
      } else if (iter.presentationType === tmp6(12424).ApplicationWidgetFieldPresentationType.DURATION) {
...
export const resolveSingleStringOrSkeleton = function resolveSingleStringOrSkeleton(componentConfig, description, resolveFieldValue) {
  ...
  const items = [resolveFieldValue.ResolvedValueType.STRING];
