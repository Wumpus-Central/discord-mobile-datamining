// === Module 12638: resolveTextComponentValues ===

// Module 12638 (resolveTextComponentValues)
import set from "set" /* 2 */;
import resolveFieldValue2 from "resolveFieldValue" /* 12639 */;
...
export const resolveTextComponentValues = function resolveTextComponentValues(subtitle_1, resolveFieldValue, numberFormat, arg3) {
  ...
  if (null == subtitle_1) {
    ...
    let str = null;
    if (!flag2) {
      const items1 = [tmp9(12639).ResolvedValueType.STRING, tmp9(12639).ResolvedValueType.NUMBER];
...
export const resolveStatComponentValues = function resolveStatComponentValues(fields, closure_1, closure_2, formatDurationNarrow, arg4) {
  ...
  if (null == fields) {
    ...
    if (null == iter) {
      ...
      if (iter.type === tmp6(12639).ResolvedValueType.STRING) {
        let formatResult = iter.value;
      } else if (iter.presentationType === tmp6(12640).ApplicationWidgetFieldPresentationType.DURATION) {
