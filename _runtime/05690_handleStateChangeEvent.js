// _runtime/05690_handleStateChangeEvent.js
import _mod5610 from "metro/05610__.js";
import _mod5612 from "metro/05612__.js";
import _isNativeReflectConstruct from "05625__isNativeReflectConstruct.js";
import isGestureEnabled from "05672_isGestureEnabled.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod5610.State.UNDETERMINED) {
    if (state === tmp(5610).State.BEGAN) {
      let tmpResult = tmp(5672);
      tmpResult.runCallback(tmp(5625).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod5610.State.BEGAN) {
    if (state === tmp(5610).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5672);
      tmpResult.runCallback(tmp(5625).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5610).State.FAILED || state === tmp(5610).State.CANCELLED;
    if (oldState === tmp(5610).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5672).runCallback(tmp(5625).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5672);
    }
    const tmp7 = state === tmp(5610).State.FAILED || state === tmp(5610).State.CANCELLED;
    tmp(5672).runCallback(tmp(5625).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
...
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("metro/05610__.js").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod5612.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5672);
    tmpResult = tmp(5672);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("metro/05612__.js").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5612).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5672);
          tmpResult1.runCallback(tmp(5672).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5672);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5672).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5672);
        tmp(5672).runCallback(tmp(5625).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(5672);
      }
    }
  }
}
