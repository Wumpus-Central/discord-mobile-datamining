...
export const addFetchEndInstrumentationHandler = function addFetchEndInstrumentationHandler(arg0) {
  ...
  addHandler.maybeInstrument("fetch-body-resolved", () => {
    ...
    {
      callback(7744).fill(callback(7739).GLOBAL_OBJ, "fetch", (arg0) => {
        closure_0 = arg0;
        return () => {
          ...
          return callback.apply(closure_2_0(closure_2_1[5]).GLOBAL_OBJ, items).then(function(result) {
            ...
            if (tmp5) {
              ...
              const result = lib(closure_2_1[4]).addNonEnumerableProperty(error, "framesToPop", 1);
              const tmpResult = lib(closure_2_1[4]);
            }
            throw error;
          });
        };
      });
      const obj = callback(7744);
    }
  });
};
export const addFetchInstrumentationHandler = function addFetchInstrumentationHandler(arg0, arg1) {
  const _require = arg1;
  _require(7736).addHandler("fetch", arg0);
  let obj = _require(7736);
  _require(7736).maybeInstrument("fetch", () => {
    let flag = callback;
