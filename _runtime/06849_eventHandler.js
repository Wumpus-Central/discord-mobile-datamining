// _runtime/06849_eventHandler.js
import _mod6769 from "metro/06769__.js";
import TouchEventType from "06771_TouchEventType.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6769.State.UNDETERMINED) {
    if (state === _mod6769.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6769.State.BEGAN) {
    if (state === _mod6769.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6769.State.FAILED || state === _mod6769.State.CANCELLED;
    if (oldState === _mod6769.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6769.State.FAILED || state === _mod6769.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(6831).flattenAndFilterEvent,
  State: fn(6769).State,
  runCallback: fn(6831).runCallback,
  CALLBACK_TYPE: fn(6784).CALLBACK_TYPE,
};
...
handleTouchEvent.__closure = {
  TouchEventType: fn(6771).TouchEventType,
  runCallback: fn(6831).runCallback,
  touchEventTypeToCallbackType: fn(6831).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6784.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(6831).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(6831).isEventForHandlerWithTag,
  isStateChangeEvent: fn(6833).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(6833).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
