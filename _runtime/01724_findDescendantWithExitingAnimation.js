// _runtime/01724_findDescendantWithExitingAnimation.js
  ...
  if (arr instanceof globalThis.HTMLElement) {
    ...
    if (isDummy) {
      ...
      if (value) {
        ...
        _mod1723.setElementPosition(arr, value);
...
export const configureWebLayoutAnimations = function configureWebLayoutAnimations() {
  if (obj.isWindowAvailable()) {
    ...
    if (null === document.getElementById(ReanimatedPredefinedWebAnimationsStyle)) {
      ...
      element.onload = () => {
        if (element.sheet) {
          for (const key10016 in tmp(1693).Animations) {
  ...
  if (obj.isWindowAvailable()) {
    ...
    if (element.sheet) {
      ...
      if (1 < closure_5.length) {
        ...
        const reanimatedError = new _mod1648.ReanimatedError("Failed to obtain animation index.");
      ...
      if (obj.isWindowAvailable()) {
        ...
        if (undefined === sum) {
          ...
          if (sum < closure_5.length) {
            ...
            const reanimatedError1 = new _mod1648.ReanimatedError("Failed to obtain animation index.");
...
export const addHTMLMutationObserver = function addHTMLMutationObserver() {
  ...
  if (!c6) {
    isWindowAvailableResult = _mod1640.isWindowAvailable();
  }
