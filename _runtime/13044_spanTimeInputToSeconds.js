// _runtime/13044_spanTimeInputToSeconds.js
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod13056.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== _mod13056.SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(parentSpan, sentrySpan) {
  ...
  const result = _mod13045.addNonEnumerableProperty(sentrySpan, _sentryRootSpan, tmp2);
  if (parentSpan[_sentryChildSpans]) {
    ...
    const result1 = _mod13045.addNonEnumerableProperty(parentSpan, _sentryChildSpans, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(_mod13066.getCurrentScope());
