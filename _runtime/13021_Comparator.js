...
class Comparator {
  constructor(arg0, arg1) {
    ...
    tmp2 = Comparator;
    tmp3 = closure_1;
    tmp4 = require("module_12994")(require);
    str = global;
    ...
    parts = str2.split(/\s+/);
    joined = parts.join(" ");
    tmp6 = require("module_12996")("comparator", joined, tmp4);
    self.options = tmp4;
    ...
    tmp8 = require("module_12996")("comp", self);
    return;
  }
}
let obj = {
  ...
  value: function parse(str) {
    const self = this;
    const safeRe = Comparator(12997).safeRe;
    const t = Comparator(12997).t;
    ...
    if (match) {
      ...
      if (match[2]) {
        const tmp15 = new tmp5(12993)(match[2], self.options.loose);
        self.semver = tmp15;
      } else {
...
const items = [
  ...
  },
  {
    ...
    value: function test(arg0) {
      const self = this;
      let tmp = arg0;
      Comparator(12996)("Comparator.test", arg0, this.options.loose);
      if (this.semver !== closure_3) {
        if (tmp !== tmp5) {
          if (typeof tmp === "string") {
            try {
              tmp = new tmp2(12993)(tmp, self.options);
            } catch (err) {
              return false;
            }
          }
          return tmp2(13019)(tmp, self.operator, self.semver, self.options);
        }
      }
      return true;
    }
    ...
    value: function intersects(value) {
      if (value instanceof Comparator) {
        ...
        if ("" === this.operator) {
          ...
          if (!isMatch) {
            const obj2 = new Comparator(13022)(value.value, arg1);
          ...
          if (!isMatch1) {
            const obj = new Comparator(13022)(self.value, arg1);
            isMatch1 = obj.test(value.semver);
          }
          tmp8 = isMatch1;
        } else {
          const tmp45 = Comparator(12994)(arg1);
            ...
            if (!tmp9) {
              ...
              if (tmp13) {
                ...
                if (tmp16) {
                  ...
                  let tmp19 = !tmp18;
                  if (tmp18) {
                    let startsWithResult3 = Comparator(13019)(self.semver, "<", value.semver, tmp45);
                    ...
                    if (!tmp24) {
                      let startsWithResult4 = Comparator(13019)(self.semver, ">", value.semver, tmp45);
