...
let c4 = 1.618;
fn(6941).NativeProxy.updateGestureHandlerConfig;
const __initData = {
...
maybeUnpackValue.__closure = { Reanimated: fn(6940).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6940.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = _mod6940.Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("metro/06940__.js").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      [tmp6, tmp7] = tmp5;
      let tmp10 = _require;
      let Reanimated = require("metro/06940__.js").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6940).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6940.Reanimated;
          const fn = function t() {
