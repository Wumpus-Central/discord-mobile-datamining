// _runtime/05655_isSupportedGesture.js
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5658).SingleGestureName.Tap !== type) {
      if (tmp(5658).SingleGestureName.LongPress !== type) {
        if (tmp(5658).SingleGestureName.Fling !== type) {
          if (tmp(5658).SingleGestureName.Native !== type) {
            if (tmp(5658).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(require("05671_context.js").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5642).Reanimated;
    let obj = gesture(5656);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5667).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5671).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5671);
    }
    return false;
  }, items2);
  if (null == tmp) {
