// _runtime/05664_onGestureHandlerEvent.js
import get_ActivityIndicator from "00017_get_ActivityIndicator.js";
import map2 from "05665_map.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5667).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5667).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5667).State.BEGAN) {
        if (handlerTag.state === tmp(5667).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5667).State.END) {
          if (handlerTag.oldState === tmp(5667).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5667).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5667).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5668).GestureStateManager;
      ...
      if (tmp(5669).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5669).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5669).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5669).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5665).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
