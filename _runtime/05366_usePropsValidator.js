// _runtime/05366_usePropsValidator.js
...
export const usePropsValidator = (index) => {
  ...
  topInset(() => {
    ...
    const mapped = items.map((str) => {
      ...
      callback2(table[1])(tmp4, "Snap point '" + str + "' is invalid. if you want to allow user to close the sheet, Please use 'enablePanDownToClose' prop.");
    });
    ...
    if (!enableDynamicSizing) {
      tmp16 = typeof tmp12 === "__REMOTEDEV__";
    }
    if (!tmp16) {
      let tmp17 = tmp12 >= -1;
      if (tmp17) {
    ...
    let tmp20 = typeof topInset === "Object";
    const tmp2Result1 = snapPoints(enableDynamicSizing[1]);
    if (typeof topInset !== "Object") {
    ...
    let tmp23 = typeof bottomInset === "Object";
    const tmp2Result2 = snapPoints(enableDynamicSizing[1]);
    if (typeof bottomInset !== "Object") {
