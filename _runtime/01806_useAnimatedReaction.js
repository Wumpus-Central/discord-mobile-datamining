// _runtime/01806_useAnimatedReaction.js
...
export const useAnimatedReaction = function useAnimatedReaction(fn, fn2, items1) {
  const _require = fn;
  dependencyMap = fn2;
  useEffect = require("01805_useSharedValue.js").useSharedValue(null);
  ...
  let tmp = _require;
  let tmp3 = require("01656_isJest.js").shouldBeUseWeb() && !values.length;
  arr2 = items1;
  if (tmp3) {
    let length;
    if (arr2 != null) {
  ...
  useEffect(() => {
    ...
    return () => {
      callback(1697).stopMapper(callback);
    };
  }, arr2);
};