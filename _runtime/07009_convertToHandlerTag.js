// === Module 7009: convertToHandlerTag ===
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod7006.BaseGesture) {
...
let items = [...fn(7010).baseGestureHandlerWithDetectorProps, ...fn(7011).tapGestureHandlerProps, ...fn(7021).panGestureHandlerProps, ...fn(7021).panGestureHandlerCustomNativeProps, ...fn(7022).longPressGestureHandlerProps, ...fn(7023).forceTouchGestureHandlerProps, ...fn(7025).flingGestureHandlerProps, ...fn(7026).hoverGestureHandlerProps, ...fn(7027).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6988.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
