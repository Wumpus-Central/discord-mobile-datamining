// _runtime/05716_isSupportedGesture.js
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5719).SingleGestureName.Tap !== type) {
      if (tmp(5719).SingleGestureName.LongPress !== type) {
        if (tmp(5719).SingleGestureName.Fling !== type) {
          if (tmp(5719).SingleGestureName.Native !== type) {
            if (tmp(5719).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(require("05732_context.js").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5703).Reanimated;
    let obj = gesture(5717);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5728).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5732).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5732);
    }
    return false;
  }, items2);
  if (null == tmp) {
