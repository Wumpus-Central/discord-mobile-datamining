...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(5645).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5638);
  ...
  const fn = function f() {
    return { enabled: iter.value === scrollableGesture(5634).SCROLLABLE_STATE.UNLOCKED };
  };
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5634).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  const tmp7 = callback(() => {
    let result;
    if (dependencyMap) {
      const Gesture = scrollableGesture(5661).Gesture;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5661).GestureDetector, obj);
  } else {
    const obj2 = {};
