...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9271).bytes(arr2);
...
export const randomBytes = function randomBytes() {
  ...
  if (Hash(9272).crypto) {
    if (typeof Hash(9272).crypto.getRandomValues === "function") {
      const _crypto2 = Hash(9272).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9272).crypto) {
    if (typeof Hash(9272).crypto.randomBytes === "function") {
      const _crypto = Hash(9272).crypto;
