...
import { JEST_WORKER_ID } from "01764_JEST_WORKER_ID.js";
const require = arg1;
...
function processColor(arr, target) {
  ...
  if (tmp5) {
    ...
    let tmp28 = target !== tmp(1765).ValueProcessorTarget.CSS;
    if (!tmp28) {
    ...
    if (tmp7) {
      ...
      if (isDynamicColorObjectIOS(arr)) {
        if (tmp(1764).IS_IOS) {
          ...
          const reanimatedError = new tmp(1664).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
          throw reanimatedError;
        }
      }
      if (null === tmp12) {
        const reanimatedError1 = new tmp(1664).ReanimatedError(obj.invalidColor(arr));
...
processColor.__closure = { processColorNumber, ValueProcessorTarget: require("module_1765").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
  ...
  if (typeof arr === "number") {
    ...
    if (tmp2) {
      ...
      if (tmp7(1764).IS_IOS) {
        ...
        const reanimatedError = new tmp7(1664).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
        throw reanimatedError;
      }
    } else {
      const reanimatedError1 = new tmp7(1664).ReanimatedError(obj.invalidProcessedColor(arr));
      throw reanimatedError1;
    }
  }
}
const obj1 = { processColorNumber, ValueProcessorTarget: require("module_1765").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
