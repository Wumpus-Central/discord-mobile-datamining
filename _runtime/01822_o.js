...
const fn2 = function t(arr) {
  closure_0 = arg1;
  return arr.map((item, index) => Math.pow(item / 255, num));
};
fn2.__closure = {};
...
const fn4 = function a(arg0, arg1, arg2, gamma) {
  ...
  if (1 !== num) {
    ...
    const mapped = r.map((item, index) => Math.pow(item / 255, num));
    if (typeof fn2 !== "function") {
      HermesBuiltin.throwTypeError();
    }
    const mapped1 = g.map((item, index) => Math.pow(item / 255, num));
    if (typeof fn2 !== "function") {
      HermesBuiltin.throwTypeError();
    }
    mapped2 = b.map((item, index) => Math.pow(item / 255, num));
    tmp2 = mapped1;
    tmp3 = mapped;
  }
...
function interpolateColor(closure_1_1, keys, arg2, arg3, length) {
  let str = arg3;
  if (arg3 === undefined) {
    str = "RGB";
  }
  ...
  if ("HSV" === str) {
    ...
    return fn(importDefault, keys, obj, obj);
  } else if ("RGB" === str) {
    ...
    return fn4(importDefault, keys, obj, obj);
  } else if ("LAB" === str) {
    ...
    return fn5(importDefault, keys, obj1, obj);
  } else {
