...
export default function SearchBar(ref) {
  ...
  const imperativeHandle = noop.useImperativeHandle(ref.ref, () => ({
    ...
    toggleCancelButton(arg0) {
      closure_0 = arg0;
      closure_1_1((arg0) => {
        const Commands = ref1(5236).Commands;
  ...
  if (ref1(5228).isSearchBarAvailableForCurrentPlatform) {
    ...
    obj.obscureBackground = tmp3(5228).parseBooleanToOptionalBooleanNativeProp(obscureBackground);
    const tmp3Result = tmp3(5228);
    obj.hideNavigationBar = tmp3(5228).parseBooleanToOptionalBooleanNativeProp(hideNavigationBar);
