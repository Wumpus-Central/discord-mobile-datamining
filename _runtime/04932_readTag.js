...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  sum = typeSize + 04929__.getTypeSize("SHORT");
  const obj2 = 04929__;
  const sum1 = sum + 04929__.getTypeSize("LONG");
  const obj3 = 04929__;
  const shortAt = 04929__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 04929__;
  const shortAt1 = 04929__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 04929__;
  const longAt = 04929__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 04929__.typeSizes[shortAt1]) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 04929__.typeSizes[shortAt1];
    BYTE = 04929__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 04929__.typeSizes[BYTE];
  }
  if (BYTE === 04929__.tagTypes.ASCII) {
    let asciiValue = 04929__.getAsciiValue(items);
    const tmp9Result = 04929__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 04929__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 04929__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 04929__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };