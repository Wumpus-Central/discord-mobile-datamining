// _runtime/05321_attachHandlers.js
  ...
  preparedGesture(gesturesToAttach[1]).ghQueueMicrotask(() => {
    if (preparedGesture.isMounted) {
      for (const item10007 of gesturesToAttach) {
        ...
        let tmp9 = preparedGesture;
        let tmp10 = gesturesToAttach;
        let obj3 = preparedGesture(gesturesToAttach[2]);
        let updateGestureHandlerResult = obj.updateGestureHandler(item10007.handlerTag, obj2.filterConfig(item10007.config, preparedGesture(gesturesToAttach[2]).ALLOWED_PROPS, obj3.extractGestureRelations(item10007)));
        continue;
      }
