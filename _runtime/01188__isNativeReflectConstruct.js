// _runtime/01188__isNativeReflectConstruct.js
...
let items = [
  ...
  },
  {
    ...
    value: function _reportAppStart() {
      ...
      const client = ReactNativeProfiler(1132).getClient();
      if (client) {
        const addIntegration = client.addIntegration;
        if (!tmp4) {
          const self = this;
          const call = addIntegration.call;
          const integration = _captureAppStart(1189).createIntegration(this.name);
          if (typeof call === "unknown") {
            addIntegration(integration);
          } else {
            call(client, integration);
          }
          const _captureAppStartResult = _captureAppStart(1189);
        }
        const appRegistryIntegration = _captureAppStart(1150).getAppRegistryIntegration(client);
        if (!appRegistryIntegration) {
          const debug = _captureAppStart(814).debug;
          debug.warn("AppRegistryIntegration.onRunApplication not found or invalid.");
          _captureAppStart = _captureAppStart(1149)._captureAppStart;
          _captureAppStart({ isManual: false });
          const _captureAppStartResult2 = _captureAppStart(1149);
        }
        appRegistryIntegration.onRunApplication(closure_7.onRunApplicationHook);
        const _captureAppStartResult1 = _captureAppStart(1150);
