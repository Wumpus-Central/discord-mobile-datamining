...
arg5.logSpanStart = function logSpanStart(spanContext) {
  ...
  if (require("metro/07403___SENTRY_DEBUG__.js") /* __SENTRY_DEBUG__ */.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(7380).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(7380).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(7380).spanToJSON(rootSpan);
    }
    const logger = tmp(7375).logger;
