// _runtime/07395_withScope.js
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7393).getMainCarrier();
  const obj = AsyncContextStack(7393);
  const sentryCarrier = AsyncContextStack(7393).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7393).getMainCarrier();
      let obj = callback(7393);
      let sentryCarrier = callback(7393).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7401);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7393).getMainCarrier();
      const obj = callback(7393);
      const sentryCarrier = callback(7393).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7401);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
