...
arg5.addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((item, index) => callback(7747).snipLine(item, 0));
    const _Math5 = Math;
    ...
    lineno.post_context = substr1.map((item, index) => callback(7747).snipLine(item, 0));
  }
};
