// _runtime/05172_constructTag.js
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod5132.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5125).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5125);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod5132.TYPE_ITXT) {
    stringFromDataView = module(5158).decode("UTF-8", decompressResult);
    const obj5 = module(5158);
  }
