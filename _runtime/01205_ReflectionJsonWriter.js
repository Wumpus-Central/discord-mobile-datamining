// _runtime/01205_ReflectionJsonWriter.js
      ...
      if ("map" == kind.kind) {
        ...
        if ("scalar" === kind3) {
          ...
          while (tmp118 !== undefined) {
            ...
            let tmp59Result = ReflectionJsonWriter(1195);
        ...
        if ("scalar" === kind2) {
          ...
          for (let num2 = 0; num2 < obj.length; num2 = num2 + 1) {
            ...
            let tmp24Result = ReflectionJsonWriter(1195);
    ...
    value: function scalar(arg0, NumberResult, arg2, arg3, arg4) {
      ...
      if (undefined !== NumberResult) {
        ...
        if (ReflectionJsonWriter(1199).ScalarType.INT32 !== arg0) {
          if (ReflectionJsonWriter(1199).ScalarType.SFIXED32 !== arg0) {
            if (ReflectionJsonWriter(1199).ScalarType.SINT32 !== arg0) {
              if (ReflectionJsonWriter(1199).ScalarType.FIXED32 !== arg0) {
                if (ReflectionJsonWriter(1199).ScalarType.UINT32 !== arg0) {
                  if (ReflectionJsonWriter(1199).ScalarType.FLOAT === arg0) {
                    ReflectionJsonWriter(1195).assertFloat32(tmp);
                    const tmp7Result = ReflectionJsonWriter(1195);
                  } else if (ReflectionJsonWriter(1199).ScalarType.DOUBLE !== arg0) {
                    if (ReflectionJsonWriter(1199).ScalarType.STRING === arg0) {
                      if ("" === tmp) {
                        let str5;
                        if (tmp5) {
                          str5 = "";
                        ...
                        ReflectionJsonWriter(1195).assert(typeof tmp === "string");
                        tmp19 = tmp;
                        const tmp7Result10 = ReflectionJsonWriter(1195);
                      }
                      return tmp19;
                    } else if (ReflectionJsonWriter(1199).ScalarType.BOOL === arg0) {
                      if (false === tmp) {
                        let tmp16 = !tmp5;
                        const tmp17 = !tmp5;
                      } else {
                        ReflectionJsonWriter(1195).assert(typeof tmp === "boolean");
                        tmp16 = tmp;
                        const tmp7Result11 = ReflectionJsonWriter(1195);
                      }
                      return tmp16;
                    } else {
                      if (ReflectionJsonWriter(1199).ScalarType.UINT64 !== arg0) {
                        if (ReflectionJsonWriter(1199).ScalarType.FIXED64 !== arg0) {
                          if (ReflectionJsonWriter(1199).ScalarType.INT64 !== arg0) {
                            if (ReflectionJsonWriter(1199).ScalarType.SFIXED64 !== arg0) {
                              if (ReflectionJsonWriter(1199).ScalarType.SINT64 !== arg0) {
                                if (ReflectionJsonWriter(1199).ScalarType.BYTES === arg0) {
                                  const _Uint8Array = Uint8Array;
                                  ReflectionJsonWriter(1195).assert(tmp instanceof Uint8Array);
                                  if (tmp.byteLength) {
                                    let str = ReflectionJsonWriter(1188).base64encode(tmp);
                                    const tmp7Result13 = ReflectionJsonWriter(1188);
                                  } else if (tmp5) {
                                    str = "";
                                  }
                                  return str;
                          ...
                          ReflectionJsonWriter(1195).assert(tmp11);
                          const PbLong = ReflectionJsonWriter(1193).PbLong;
                          const tmp7Result14 = ReflectionJsonWriter(1195);
                      ...
                      ReflectionJsonWriter(1195).assert(tmp13);
                      const PbULong = ReflectionJsonWriter(1193).PbULong;
                      const tmp7Result15 = ReflectionJsonWriter(1195);
                  ...
                  if (0 === tmp) {
                    ...
                    ReflectionJsonWriter(1195).assert(typeof tmp === "number");
                    const _Number = Number;
                    str6 = "NaN";
                    if (!Number.isNaN(tmp)) {
                    ...
                    const tmp7Result16 = ReflectionJsonWriter(1195);
                  }
                  return str6;
                }
              }
              if (0 === tmp) {
                ...
                ReflectionJsonWriter(1195).assertUInt32(tmp);
                tmp24 = tmp;
                const tmp7Result17 = ReflectionJsonWriter(1195);
              }
              return tmp24;
            }
          }
        ...
        if (0 === tmp) {
          ...
          ReflectionJsonWriter(1195).assertInt32(tmp);
          const tmp7Result18 = ReflectionJsonWriter(1195);
        }
        return tmp;
      } else {
        ReflectionJsonWriter(1195).assert(arg3);
