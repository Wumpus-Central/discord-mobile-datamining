// _runtime/08067_extractProps.js
import pickNotNil from "08068_pickNotNil.js";
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8059)(obj, markerMid, arg1);
  module(8069)(obj, markerMid, items);
  module(8072)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8060)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8058)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
