...
function maybeUnpackValue(enabled) {
  const Reanimated = 05510__.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5510").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === 05510__.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5510).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, _slicedToArray) {
  ...
  if (undefined !== _05510__.Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _05510__.Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5510).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, _slicedToArray) {
  if (undefined !== _05510__.Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5510).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5510).Reanimated;
        const fn = function t() {
