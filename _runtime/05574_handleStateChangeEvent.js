// _runtime/05574_handleStateChangeEvent.js
import { _isNativeReflectConstruct } from "05509__isNativeReflectConstruct.js";
import { isGestureEnabled } from "05556_isGestureEnabled.js";
import { 05494__ } from "metro/05494__.js";
import { 05496__ } from "metro/05496__.js";
const require = arg1;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === 05494__.State.UNDETERMINED) {
    if (state === tmp(5494).State.BEGAN) {
      let tmpResult = tmp(5556);
      tmpResult.runCallback(tmp(5509).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === 05494__.State.BEGAN) {
    if (state === tmp(5494).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5556);
      tmpResult.runCallback(tmp(5509).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5494).State.FAILED || state === tmp(5494).State.CANCELLED;
    if (oldState === tmp(5494).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5556).runCallback(tmp(5509).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5556);
    }
    const tmp7 = state === tmp(5494).State.FAILED || state === tmp(5494).State.CANCELLED;
    tmp(5556).runCallback(tmp(5509).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(5556);
  }
}
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("module_5494").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== 05496__.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5556);
    tmpResult = tmp(5556);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("module_5496").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5496).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5556);
          tmpResult1.runCallback(tmp(5556).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5556);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5556).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5556);
        tmp(5556).runCallback(tmp(5509).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(5556);
      }
    }
  }
}
