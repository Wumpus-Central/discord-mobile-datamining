// _runtime/13822___exportStarResult25.js
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13794).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13820).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13820).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13819).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13818).FormatApproximately(internalSlots, result);
    ...
    return tmp(13807).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13819).FormatNumeric(internalSlots, isNaN);
};