// _runtime/01526_createNavigationContainerRef.js
const require = arg1;
...
arg5.createNavigationContainerRef = function createNavigationContainerRef() {
      ...
      if (null != arg0) {
        ...
        let item = entries.forEach((item, index) => {
          ...
          item = arr.forEach((item, index) => {
            closure_1_0.addListener(closure_0, item);
          });
        });
      }
    }
  ...
  const merged = Object.assign(items.reduce((acc, item, index) => {
    closure_0 = item;
    acc[item] = () => {
      ...
      if ("removeListener" === first) {
        ...
        first = tmp18;
        if (item[tmp17]) {
          item[tmp17] = item[tmp17].filter((item, index) => item !== closure_0);
        }
        if (closure_1_1 != null) {
          closure_1_1.removeListener(tmp17, tmp18);
        }
        ...
        return () => {
          ...
          closure_0 = closure_1;
          if (first[closure_0]) {
            first[tmp] = first[tmp].filter((item, index) => item !== closure_0);
          }
          if (closure_1 != null) {
            closure_1.removeListener(tmp, closure_1);
          }
        };
      } else {
