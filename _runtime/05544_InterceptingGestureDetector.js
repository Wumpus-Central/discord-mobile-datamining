// _runtime/05544_InterceptingGestureDetector.js
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5545).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5545).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5545).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5545).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5481).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5540).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5482);
  }
  ...
  c5(() => {
    ...
    if (prop) {
      _undefined.setMode(gesture(5545).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        _undefined.setMode(gesture(5545).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5543).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5481).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = c8(ReanimatedNativeDetector, obj1);
    return c8(tmp(5545).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    let error = new Error(tmp(5472).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
  const tmp20 = c5;
