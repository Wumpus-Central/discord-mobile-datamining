// _runtime/05143_getTagName.js
import getDataView from "05104_getDataView.js";
import _modDef5121 from "metro/05121__.js";
...
export default {
  read(arg0, arg1) {
    ...
    if (0 < arg0.length) {
      ...
      const shortAt = _modDef5121.getShortAt(dataView, sum);
      const sum1 = sum + c5;
      const tmp15 = getTagName(dataView, sum1);
      let name = tmp15.tagName;
      const sum2 = sum1 + tmp15.tagNameSize;
      const obj4 = _modDef5121;
      const longAt = _modDef5121.getLongAt(dataView, sum2);
      const sum3 = sum2 + c6;
      if (stringFromDataView === c4) {
        let tmp5Result = tmp5(5104);
        const dataView1 = tmp5Result.getDataView(dataView.buffer, sum3, longAt);
        obj = { id: null, value: null };
        obj[0] = shortAt;
        tmp5Result = tmp5(5104);
        obj[1] = tmp5Result.getStringFromDataView(dataView1, 0, longAt);
        if (tmp10(5144)[shortAt]) {
          try {
            obj.description = tmp10(5144)[shortAt].description(dataView1);
            if (!name) {
              name = tmp10(5144)[shortAt].name;
            }
            obj[name] = obj;
            const obj6 = tmp10(5144)[shortAt];
          } catch (err) {
