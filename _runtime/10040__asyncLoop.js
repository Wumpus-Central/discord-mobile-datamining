...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(10041).bytes(arr2);
...
export const randomBytes = function randomBytes() {
  ...
  if (Hash(10042).crypto) {
    if (typeof Hash(10042).crypto.getRandomValues === "function") {
      const _crypto2 = Hash(10042).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(10042).crypto) {
    if (typeof Hash(10042).crypto.randomBytes === "function") {
      const _crypto = Hash(10042).crypto;
