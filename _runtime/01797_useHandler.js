...
export const useHandler = function useHandler(memoizedGestureCallbacks, items10) {
  ...
  for (const key10024 in arg0) {
    ...
    if (obj8.isWorkletFunction(arg0[key10024])) {
      ...
      let tmp6 = new.target;
      let reanimatedError = new tmp14(1663).ReanimatedError(
  ...
  const dependencies = require("01798_isAnimated.js").buildDependencies(items10, memoizedGestureCallbacks);
  const obj3 = isAnimated;
  const tmp9 = _require;
  tmp.current.savedDependencies = dependencies;
  obj = {
    context,
    doDependenciesDiffer: !require("01798_isAnimated.js").areDependenciesEqual(dependencies, savedDependencies),
    useWeb: null,
  };
  const obj4 = isAnimated;
  let isWebResult = require("01655_isJest.js").isWeb();
  if (!isWebResult) {
