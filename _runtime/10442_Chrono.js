// _runtime/10442_Chrono.js
import ENDefaultConfiguration from "10443_ENDefaultConfiguration.js";
...
obj = {
  ...
  value: function executeParser(closure_0, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof index(10449).ParsingResult) {
        index = parsingResult.index;
        text = parsingResult.text;
        dependencyMap = text;
        closure_0.debug(() =>
          console.log("" + index.constructor.name + " extracted (at index=" + index + ") '" + closure_1 + "'"),
        );
        items.push(parsingResult);
        const substr1 = text.substring(index + text.length);
        const match2 = patternResult.exec(substr1);
      } else if (!(extractResult instanceof tmp6(10449).ParsingComponents)) {
...
obj = {
  ...
  value: function createParsingComponents(extractResult) {
    ...
    if (!(extractResult instanceof ParsingContext(10449).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(10449).ParsingComponents(this.reference, extractResult);
    }
...
const items2 = [
  obj,
  {
    ...
    value: function createParsingResult(index, length2, extractResult, extractResult) {
      ...
      return new ParsingContext(10449).ParsingResult(
        self.reference,
        index,
        substr,
        parsingComponents,
