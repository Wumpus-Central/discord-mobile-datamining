// _runtime/04991_readTag.js
import { importDefaultResult1 } from "04992_importDefaultResult1.js";
import { 04974__ } from "metro/04974__.js";
import { 04988__ } from "metro/04988__.js";
const require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  let obj = 04988__;
  const typeSize = obj.getTypeSize("SHORT");
  sum = typeSize + 04988__.getTypeSize("SHORT");
  const obj2 = 04988__;
  const sum1 = sum + 04988__.getTypeSize("LONG");
  const obj3 = 04988__;
  const shortAt = 04988__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 04988__;
  const shortAt1 = 04988__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 04988__;
  const longAt = 04988__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 04988__.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = callback(4988).typeSizes[shortAt1] * longAt;
      return result <= callback(4988).getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(4988).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + callback(4988).typeSizes[shortAt1] * longAt <= byteLength.byteLength;
    ...
    let tmp34 = str;
    if (shortAt1 === tmp4(4988).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(4992)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(4992)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(4992)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(4992)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(4992)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(4988).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(4988).tagTypes.SRATIONAL) {
          combined = tmp4(4992)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 04988__.typeSizes[shortAt1];
    BYTE = 04988__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 04988__.typeSizes[BYTE];
  }
  if (BYTE === 04988__.tagTypes.ASCII) {
    let asciiValue = 04988__.getAsciiValue(items);
    const tmp9Result = 04988__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: require("module_4988").getByteAt, 3: require("module_4988").getAsciiAt, 4: require("module_4988").getShortAt, 5: require("module_4988").getLongAt, 7: require("module_4988").getRationalAt, 9: null, 10: require("module_4988").getUndefinedAt, 13: null };
obj[9] = require("module_4988").getSlongAt;
obj[10] = require("module_4988").getSrationalAt;
obj[13] = require("module_4988").getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  let obj = 04988__;
  const typeSize = obj.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 04988__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 04988__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
  ...
  if (04974__.USE_THUMBNAIL) {
    let tmp23Result = tmp23(4988);
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      tmp23Result = tmp23(4988);
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 04988__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };