// _runtime/12336_createChildOrRootSpan.js
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12343).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(12335).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12307).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(12307);
        const client = tmp(12329).getClient();
        if (client) {
      ...
      tmp(12340).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(12340);
      const result = tmp(12331).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12338).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(12338);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(12307);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(12307).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12307).spanIsSampled(parentSpan);
      tmp(12338).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(12338);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12338).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(12338);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(12335).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(12341).sampleSpan(options, obj);
    const tmpResult = tmp(12341);
  }
  ...
  obj1.attributes = { [_mod12317.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(12343).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12317).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = require("12320_getMainCarrier.js").getMainCarrier();
  const obj = getMainCarrier;
  const tmp = _require;
  const asyncContextStrategy = require("12321_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12329).withScope((setPropagationContext) => {
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
            _null(12324)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return require("12329_getClient.js").withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = getMainCarrier;
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = require("12321_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(12329).withScope((setSDKProcessingMetadata) => {
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(12329).withScope((arg0) => {
      _null(12324)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(12329);
