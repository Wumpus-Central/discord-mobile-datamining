...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7813).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(7790).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(7790);
    const currentScope = monitorSlug(7803).getCurrentScope();
    const obj3 = monitorSlug(7803);
    const client = monitorSlug(7803).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7803);
    monitorSlug(7787).uuid4();
  }
  let currentScope = require("07803_getClient.js").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = getClient;
  let client = require("07803_getClient.js").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7804).DEBUG_BUILD) {
      let logger2 = tmp(7776).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7776).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = getClient;
  captureCheckInResult = require("07787_addContextToFrame.js").uuid4();
};