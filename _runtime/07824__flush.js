...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  let currentScope = _require(7803).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7803);
  let client = _require(7803).getClient();
  if (client) {
    if (client.captureCheckIn) {
  ...
  let obj3 = _require(7803);
  captureCheckInResult = _require(7787).uuid4();
};