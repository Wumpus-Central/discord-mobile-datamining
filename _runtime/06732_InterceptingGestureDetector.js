...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(6733).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(6733).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(6669).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = closure_8(ReanimatedNativeDetector, obj1);
    return closure_8(tmp(6733).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    error = new Error(tmp(6660).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
