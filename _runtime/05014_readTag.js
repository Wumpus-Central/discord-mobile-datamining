...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  sum = typeSize + 05011__.getTypeSize("SHORT");
  const obj2 = 05011__;
  const sum1 = sum + 05011__.getTypeSize("LONG");
  const obj3 = 05011__;
  const shortAt = 05011__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 05011__;
  const shortAt1 = 05011__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 05011__;
  const longAt = 05011__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 05011__.typeSizes[shortAt1]) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 05011__.typeSizes[shortAt1];
    BYTE = 05011__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 05011__.typeSizes[BYTE];
  }
  if (BYTE === 05011__.tagTypes.ASCII) {
    let asciiValue = 05011__.getAsciiValue(items);
    const tmp9Result = 05011__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 05011__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 05011__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 05011__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };