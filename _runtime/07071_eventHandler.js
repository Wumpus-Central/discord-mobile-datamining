// _runtime/07071_eventHandler.js
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6991.State.UNDETERMINED) {
    if (state === _mod6991.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7006.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6991.State.BEGAN) {
    if (state === _mod6991.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7006.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6991.State.FAILED || state === _mod6991.State.CANCELLED;
    if (oldState === _mod6991.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7006.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6991.State.FAILED || state === _mod6991.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7006.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(7053).flattenAndFilterEvent,
  State: fn(6991).State,
  runCallback: fn(7053).runCallback,
...
handleTouchEvent.__closure = {
  ...
  runCallback: fn(7053).runCallback,
  touchEventTypeToCallbackType: fn(7053).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7006.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(7053).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(7053).isEventForHandlerWithTag,
  isStateChangeEvent: fn(7055).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(7055).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
