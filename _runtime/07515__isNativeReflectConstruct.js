// _runtime/07515__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7516).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7516);
      return ServerRuntimeClient(7480).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7480);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7516).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7483).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7483);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7509).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7484).DEBUG_BUILD) {
        const logger = tmp2(7456).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7517);
          ...
          if (ServerRuntimeClient(7484).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7456).logger;
          ...
          if (ServerRuntimeClient(7484).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7456).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7467);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7478)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7492);
        if (_getSpanForScopeResult) {
