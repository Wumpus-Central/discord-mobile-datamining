// _runtime/05404_FILE_TYPES_REQUIRED_ADDITIONAL_CHECK.js
import AudioTypes from "05411_AudioTypes.js";
import ImageTypes from "05412_ImageTypes.js";
import VideoTypes from "05413_VideoTypes.js";
import CompressedTypes from "05414_CompressedTypes.js";
import OtherTypes from "05415_OtherTypes.js";
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5402).fetchFromObject(FileTypes, key10027.toUpperCase());
  },
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5402).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    },
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5405).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (FileTypes(5405).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!FileTypes(5405).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5402).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (FileTypes(5405).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (
          ...
          if (FileTypes(5402).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
