// _runtime/16023__isNativeReflectConstruct.js
import noopDefault from "00019_noop.js";
import _inheritsDefault from "00098__inherits.js";
import get_ActivityIndicatorDefault from "16024_get_ActivityIndicator.js";
...
function captureRef(obj, options) {
  if (ViewShot(16024)) {
    ...
    return tmp(16024).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16024).releaseCapture(str);
    const obj = ViewShot(16024);
  }
}
noopDefault;
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16024)) {
    ...
    return tmp(16024).captureScreen(options);
  } else {
    const _Error = Error;
    error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
