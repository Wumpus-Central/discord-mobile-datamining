// === Module 7631: createChildOrRootSpan ===
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = _require(7615).getMainCarrier();
  const obj = _require(7615);
  const tmp = _require;
  const asyncContextStrategy = _require(7616).getAsyncContextStrategy(mainCarrier);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(7624).withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = _require(7615);
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = _require(7616).getAsyncContextStrategy(mainCarrier);
