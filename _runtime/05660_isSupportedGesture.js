// _runtime/05660_isSupportedGesture.js
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5663).SingleGestureName.Tap !== type) {
      if (tmp(5663).SingleGestureName.LongPress !== type) {
        if (tmp(5663).SingleGestureName.Fling !== type) {
          if (tmp(5663).SingleGestureName.Native !== type) {
            if (tmp(5663).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(require("05676_context.js").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5647).Reanimated;
    let obj = gesture(5661);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5672).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5676).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5676);
    }
    return false;
  }, items2);
  if (null == tmp) {
