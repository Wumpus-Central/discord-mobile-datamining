...
let items = [
  entry,
  {
    ...
    value: function assert(arg0, arg1, PbLong) {
      if (!arg0) {
        const typeofJsonValueResult = ReflectionJsonReader(1187).typeofJsonValue(PbLong);
          ...
          if ("map" == obj.kind) {
            if (null === tmp9) {
              ...
              for (const item10165 of entries1) {
                ...
                let str3 = self.scalar(first, obj.K, ReflectionJsonReader(1199).LongType.STRING, obj.name);
  ...
  },
  {
    ...
    value: function _enum(arg0, str, arg2, arg3) {
      ...
      if ("google.protobuf.NullValue" == arg0[0]) {
        const _HermesInternal = HermesInternal;
        ReflectionJsonReader(1195).assert(null === " only accepts null.", "Unable to parse field " + self.info.typeName + "#" + arg2 + ", enum " + arg0[0] + " only accepts null.");
      ...
      if (null === str) {
        ...
        const obj2 = ReflectionJsonReader(1195);
      }
    }
  },
  {
    ...
    value: function scalar(flag, arg1, STRING, arg3) {
      try {
        if (ReflectionJsonReader(1199).ScalarType.DOUBLE !== arg1) {
          if (ReflectionJsonReader(1199).ScalarType.FLOAT !== arg1) {
            if (ReflectionJsonReader(1199).ScalarType.INT32 !== arg1) {
              if (ReflectionJsonReader(1199).ScalarType.FIXED32 !== arg1) {
                if (ReflectionJsonReader(1199).ScalarType.SFIXED32 !== arg1) {
                  if (ReflectionJsonReader(1199).ScalarType.SINT32 !== arg1) {
                    if (ReflectionJsonReader(1199).ScalarType.UINT32 !== arg1) {
                      if (ReflectionJsonReader(1199).ScalarType.INT64 !== arg1) {
                        if (ReflectionJsonReader(1199).ScalarType.SFIXED64 !== arg1) {
                          if (ReflectionJsonReader(1199).ScalarType.SINT64 !== arg1) {
                            if (ReflectionJsonReader(1199).ScalarType.FIXED64 !== arg1) {
                              if (ReflectionJsonReader(1199).ScalarType.UINT64 !== arg1) {
                                if (ReflectionJsonReader(1199).ScalarType.BOOL === arg1) {
                                  if (null === flag) {
                                    return false;
                                  } else if (typeof flag === "boolean") {
                                    return flag;
                                  }
                                } else if (ReflectionJsonReader(1199).ScalarType.STRING === arg1) {
                                  if (null === flag) {
                                    return "";
                                ...
                                } else if (ReflectionJsonReader(1199).ScalarType.BYTES === arg1) {
                                  if (null !== flag) {
                                    if ("" !== flag) {
                                      if (typeof flag === "string") {
                                        return ReflectionJsonReader(1188).base64decode(flag);
                                      }
                                    }
                                  }
                            ...
                            if (null === flag) {
                              return ReflectionJsonReader(1204).reflectionLongConvert(ReflectionJsonReader(1193).PbULong.ZERO, STRING);
                            } else {
                              const PbULong = ReflectionJsonReader(1193).PbULong;
                              return ReflectionJsonReader(1204).reflectionLongConvert(PbULong.from(flag), STRING);
                            }
                          }
                        }
                      }
                      if (null === flag) {
                        return ReflectionJsonReader(1204).reflectionLongConvert(ReflectionJsonReader(1193).PbLong.ZERO, STRING);
                      } else {
                        const PbLong = ReflectionJsonReader(1193).PbLong;
                        return ReflectionJsonReader(1204).reflectionLongConvert(PbLong.from(flag), STRING);
                      }
                    }
                  }
                }
            ...
            if (null === flag) {
              ...
              if (undefined !== NumberResult) {
                if (arg1 == ReflectionJsonReader(1199).ScalarType.UINT32) {
                  ReflectionJsonReader(1195).assertUInt32(NumberResult);
                  const tmp3Result12 = ReflectionJsonReader(1195);
                } else {
                  ReflectionJsonReader(1195).assertInt32(NumberResult);
                  const tmp3Result13 = ReflectionJsonReader(1195);
                }
                return NumberResult;
              }
            }
          ...
          if (typeof flag === "string") {
            ...
            if (Number.isNaN(NumberResult1)) {
              ...
              if (Number.isFinite(tmp26)) {
                if (arg1 == ReflectionJsonReader(1199).ScalarType.FLOAT) {
                  ReflectionJsonReader(1195).assertFloat32(tmp26);
                  const tmp3Result14 = ReflectionJsonReader(1195);
                }
