// _runtime/06154_eventHandler.js
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6074.State.UNDETERMINED) {
    if (state === _mod6074.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6074.State.BEGAN) {
    if (state === _mod6074.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6074.State.FAILED || state === _mod6074.State.CANCELLED;
    if (oldState === _mod6074.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6074.State.FAILED || state === _mod6074.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(6136).flattenAndFilterEvent,
  State: fn(6074).State,
  runCallback: fn(6136).runCallback,
...
handleTouchEvent.__closure = {
  ...
  runCallback: fn(6136).runCallback,
  touchEventTypeToCallbackType: fn(6136).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(6136).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(6136).isEventForHandlerWithTag,
  isStateChangeEvent: fn(6138).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(6138).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
