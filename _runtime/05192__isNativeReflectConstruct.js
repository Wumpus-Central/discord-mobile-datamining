// _runtime/05192__isNativeReflectConstruct.js
import _inheritsDefault from "00098__inherits.js";
import _wrapNativeSuperDefault from "metro/00158__wrapNativeSuper.js";
import calculateGPSValueDefault from "05195_calculateGPSValue.js";
...
function readTags(_raw, dataView, arg2) {
  try {
    ...
    ParseError(5157).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5194).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5194);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5195)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5195);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5157).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5157).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5157);
    obj = {};
    ...
    ParseError(5157).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5157);
  }
  ParseError(5157).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
