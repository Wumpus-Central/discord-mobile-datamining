// _runtime/06332_JSFPSMonitor.js
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6267).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(6333).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(6333);
      obj.maxFPS = JSFPSMonitor(6333).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(6333);
      obj.averageFPS = JSFPSMonitor(6333).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    },
  },
];

export const JSFPSMonitor = _modDef6280(JSFPSMonitor, items);
