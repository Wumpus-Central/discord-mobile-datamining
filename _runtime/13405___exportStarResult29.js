...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = require(13387) /* getMultiInternalSlots */.repeat("0", maximumSignificantDigits);
    let roundedNumber = require(13389) /* TEN */.ZERO;
    ...
    result = require(13399) /* __exportStarResult11 */.ApplyUnsignedRoundingMode(ZERO, timesResult, roundedNumber, result);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13387).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13387).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13387).repeat("0", -num3 - 1)}${arr}`;
