// _runtime/05012_FILE_TYPES_REQUIRED_ADDITIONAL_CHECK.js
import AudioTypes from "05019_AudioTypes.js";
import ImageTypes from "05020_ImageTypes.js";
import VideoTypes from "05021_VideoTypes.js";
import CompressedTypes from "05022_CompressedTypes.js";
import OtherTypes from "05023_OtherTypes.js";
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(5010).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(5010).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5013).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5013).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5013).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5010).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5013).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(5010).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
