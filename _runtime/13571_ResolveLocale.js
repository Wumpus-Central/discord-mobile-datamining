// _runtime/13571_ResolveLocale.js
import { LookupMatcher } from "13572_LookupMatcher.js";
import { isMatched } from "13573_isMatched.js";
import { BestFitMatcher } from "13577_BestFitMatcher.js";
let require = arg1;
...
arg5.ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(13578).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult2 = tmp14(13573).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(13579).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(13580).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj.locale = result;
  return obj;
};