// _runtime/06421_applyScopeDataToEvent.js
const require = arg1;
...
arg5.applyScopeDataToEvent = function applyScopeDataToEvent(extra) {
  ...
  const dropUndefinedKeysResult3 = require(6377) /* addNonEnumerableProperty */.dropUndefinedKeys(contexts);
  ...
  if (span) {
    ...
    obj3[0] = tmp3(6376).spanToTraceContext(span);
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj3;
    const obj4 = { dynamicSamplingContext: null };
    const tmp3Result2 = tmp3(6376);
    obj4[0] = tmp3(6407).getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj4;
    const tmp3Result3 = tmp3(6407);
    const rootSpan = tmp3(6376).getRootSpan(span);
    const tmp3Result4 = tmp3(6376);
    const description = tmp3(6376).spanToJSON(rootSpan).description;
...
arg5.mergeAndOverwriteScopeData = function mergeAndOverwriteScopeData(arg0, arg1, arg2) {
  arg0[arg1] = require(6396) /* merge */.merge(arg0[arg1], arg2, 1);
};
arg5.mergeScopeData = function mergeScopeData(extra) {
  ...
  let obj = require(6396) /* merge */;
  extra.extra = obj.merge(extra.extra, extra, 1);
  extra.tags = require(6396) /* merge */.merge(extra.tags, tags, 1);
  const obj2 = require(6396) /* merge */;
  extra.user = require(6396) /* merge */.merge(extra.user, user, 1);
  const obj3 = require(6396) /* merge */;
  extra.contexts = require(6396) /* merge */.merge(extra.contexts, contexts, 1);
  const obj4 = require(6396) /* merge */;
  extra.sdkProcessingMetadata = require(6396) /* merge */.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
