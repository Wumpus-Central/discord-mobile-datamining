// _runtime/01687__isNativeReflectConstruct.js
import _modDef38 from "metro/00038__.js";
import isJest2 from "01655_isJest.js";
import JSPropsUpdaterPaperDefault from "01747_JSPropsUpdaterPaper.js";
import isInlineStyleTransform from "01748_isInlineStyleTransform.js";
import dummyListener from "01756_dummyListener.js";
import setAndForwardRefDefault from "01758_setAndForwardRef.js";
import updateLayoutAnimations from "01766_updateLayoutAnimations.js";
...
import startObservingProgress from "01688_startObservingProgress.js";
...
export const createAnimatedComponent = function createAnimatedComponent(fn) {
  ...
  let items = [
    {
      ...
      value: function componentDidMount() {
        ...
        const self = this;
        if (!closure_1_13) {
          const nativeEventsManager = new AnimatedComponent(1759).NativeEventsManager(self, closure_1);
        ...
        let isFabricResult = AnimatedComponent(1655).isFabric();
        if (isFabricResult) {
          isFabricResult = -1 !== componentViewTag;
        }
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp17(1761).PropsRegistryGarbageCollector;
          PropsRegistryGarbageCollector.registerView(componentViewTag, self);
        }
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp17(1755).ComponentRegistry;
          ComponentRegistry.register(componentViewTag, self);
        }
        const result2 = self._configureLayoutAnimation(tmp17(1677).LayoutAnimationType.LAYOUT, self.props.layout);
        const result3 = self._configureLayoutAnimation(tmp17(1677).LayoutAnimationType.EXITING, self.props.exiting);
        let tmp25 = tmp17;
        if (closure_1_13) {
          tmp25 = tmp17;
          if (self._componentDOMRef) {
            ...
            if (self.props.exiting) {
              AnimatedComponent(1706).saveSnapshot(_componentDOMRef);
              const obj2 = AnimatedComponent(1706);
            }
            if (self.props.entering) {
              if (obj3.getReducedMotionFromConfig(self.props.entering)) {
                ...
                if (current) {
                  ...
                  const result4 = tmp32(1706).startWebLayoutAnimation(self.props, _componentDOMRef, tmp32(1677).LayoutAnimationType.ENTERING);
                  tmp25 = tmp32;
                  const tmp32Result = tmp32(1706);
                }
              }
              obj3 = AnimatedComponent(1706);
            } else {
              self._isFirstRender = false;
            }
          }
    ...
    },
    {
      ...
      value: function componentWillUnmount() {
        ...
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp4(1761).PropsRegistryGarbageCollector;
        ...
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp4(1755).ComponentRegistry;
          ComponentRegistry.unregister(componentViewTag);
        }
        if (closure_13) {
          if (self._componentDOMRef) {
            if (exiting) {
              let tmp4Result = tmp4(1706);
              if (tmp4Result.getReducedMotionFromConfig(exiting)) {
                ...
                tmp4Result = tmp4(1739);
                const result2 = tmp4Result.addHTMLMutationObserver();
                ({ props, _componentDOMRef } = self);
                const result3 = tmp4(1706).startWebLayoutAnimation(props, _componentDOMRef, tmp4(1677).LayoutAnimationType.EXITING);
                const tmp4Result1 = tmp4(1706);
              }
            }
            const _viewInfo = self._viewInfo;
            let isFabricResult1 = !closure_16;
            if (!closure_16) {
              isFabricResult1 = tmp4(1655).isFabric();
              const tmp4Result2 = tmp4(1655);
            }
            if (isFabricResult1) {
              isFabricResult1 = shadowNodeWrapper;
            }
            if (isFabricResult1) {
              tmp4(1696).markNodeAsRemovable(shadowNodeWrapper);
              const tmp4Result3 = tmp4(1696);
            }
            ...
            if (!tmp4Result4.isFabric()) {
              if ("getReduceMotion" in exiting) {
                if (typeof exiting.getReduceMotion === "function") {
                  let reduceMotionFromConfig = tmp4(1692).getReduceMotionFromConfig(exiting.getReduceMotion());
                  const tmp4Result5 = tmp4(1692);
                }
                if (!reduceMotionFromConfig) {
                  const result4 = self._configureLayoutAnimation(tmp4(1677).LayoutAnimationType.EXITING, exiting);
                }
              }
              reduceMotionFromConfig = tmp4(1692).getReduceMotionFromConfig();
              const tmp4Result6 = tmp4(1692);
            }
            tmp4Result4 = tmp4(1655);
          }
        }
      }
    },
          ...
          if (closure_16) {
            ...
            if (findHostInstanceResult) {
              ...
              if (tmpResult.isFabric()) {
                shadowNodeWrapperFromRef = tmp(1680).getShadowNodeWrapperFromRef(self, findHostInstanceResult);
                const tmpResult1 = tmp(1680);
              }
              _componentDOMRef = null;
            } else {
              const reanimatedError = new tmp(1663).ReanimatedError("Cannot find host instance for this component. Maybe it renders nothing?");
    ...
    },
    {
      ...
      value: function componentDidUpdate(layout) {
        const self = this;
        const result = this._configureLayoutAnimation(AnimatedComponent(1677).LayoutAnimationType.LAYOUT, this.props.layout, layout.layout);
        const result1 = this._configureLayoutAnimation(AnimatedComponent(1677).LayoutAnimationType.EXITING, this.props.exiting, layout.exiting);
        if (!tmp5) {
          ...
          if (arg2) {
            if (self.props.layout) {
              ...
              if (tmpResult.getReducedMotionFromConfig(self.props.layout)) {
                ...
                const result4 = tmp(1706).tryActivateLayoutTransition(self.props, self._componentDOMRef, arg2);
                const tmpResult1 = tmp(1706);
              }
            }
          }
        }
      ...
      value: function _configureLayoutAnimation(EXITING, exiting, exiting2) {
        if (!isJest) {
          if (exiting !== exiting2) {
            ...
            if (obj2.isFabric()) {
              if (EXITING === tmp5(1677).LayoutAnimationType.ENTERING) {
                let reanimatedID = self.reanimatedID;
              }
              let maybeBuildResult = tmp3;
              if (tmp3) {
                let tmp8;
                if (EXITING !== tmp5(1677).LayoutAnimationType.LAYOUT) {
                ...
                maybeBuildResult = tmp5(1767).maybeBuild(tmp3, tmp8, AnimatedComponent.displayName);
                const tmp5Result = tmp5(1767);
              }
      ...
      value: function _configureSharedTransition(flag) {
        ...
        if (!closure_13) {
          ...
          if (sharedTransitionTag) {
            ...
            if (_sharedElementTransition2 == null) {
              _sharedElementTransition2 = new AnimatedComponent(1768).SharedTransition();
            }
        ...
        if (getReduceMotion) {
          if ("getReduceMotion" in getReduceMotion) {
            if (typeof getReduceMotion.getReduceMotion === "function") {
              let reduceMotionFromConfig = AnimatedComponent(1692).getReduceMotionFromConfig(getReduceMotion.getReduceMotion());
              const obj2 = AnimatedComponent(1692);
            }
            return reduceMotionFromConfig;
          }
        }
        reduceMotionFromConfig = AnimatedComponent(1692).getReduceMotionFromConfig();
      }
    },
    {
    ...
    },
    {
      ...
      value: function render() {
        ...
        if (tmp3) {
          let obj = AnimatedComponent(1706);
        ...
        if (!tmp3) {
          ...
          if (!current) {
            let obj3 = AnimatedComponent(1655);
            if (obj3.isFabric()) {
  ...
  dependencyMap = tmp9;
  tmp9.contextType = AnimatedComponent(1790).SkipEnteringContext;
  tmp9.displayName = "AnimatedComponent(" + fn.displayName || fn.name || "Component" + ")";
  const componentWithRefResult = AnimatedComponent(1791).componentWithRef((arg0, arg1) => {
    let obj = {};
