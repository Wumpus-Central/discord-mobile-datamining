// _runtime/01703_setElementAnimation.js
import { isWorkletFunction } from "01659_isWorkletFunction.js";
import { isReducedMotionEnabledInSystem } from "01675_isReducedMotionEnabledInSystem.js";
import { TransitionType } from "01690_TransitionType.js";
import { BaseAnimationBuilder } from "01704_BaseAnimationBuilder.js";
import { weakMap } from "01720_weakMap.js";
const require = arg1;
...
function setElementAnimation(dummy, dummyAnimationConfig, arg2) {
  ...
  if (!(animationName in _TransitionType.Animations)) {
    let result = tmp2(1721).scheduleAnimationCleanup(animationName, duration + delay, () => {
      if (flag) {
        const snapshots = dummy(dummyAnimationConfig[7]).snapshots;
...
arg5.getReducedMotionFromConfig = function getReducedMotionFromConfig(entering) {
  if (entering.reduceMotionV) {
    ...
    if (isWorkletFunction.ReduceMotion.Never === reduceMotionV) {
      return false;
    } else if (tmp3(1659).ReduceMotion.Always === reduceMotionV) {
      return true;
    } else {
      return tmp3(1675).ReducedMotionManager.jsValue;
    }
  } else {
...
arg5.getProcessedConfig = function getProcessedConfig(animationWithInitialValues, animationType, entering) {
  ...
  if (animationWithInitialValues in TransitionType.Animations) {
    num = tmp(1690).Animations[animationWithInitialValues].duration;
  }
  ...
  if (!entering.delayV) {
    obj[3] = num3;
    if (entering.easingV) {
      const tmp8 = entering.easingV[tmp(undefined, 1686).EasingNameSymbol];
      let tmpResult = tmp(1693);
      if (tmp9) {
      ...
      tmp9 = tmp8 in tmp(1693).WebEasings;
    } else {
      easingByName = tmp(1693).getEasingByName("linear");
      const tmpResult1 = tmp(1693);
    }
    obj[4] = easingByName;
    let callbackV = null;
...
arg5.handleLayoutTransition = function handleLayoutTransition(_componentDOMRef, processedConfig, easingY) {
  ...
  if (ENTRY_EXIT === tmp4(1690).TransitionType.CURVED) {
    tmp4Result = tmp4(1727);
...
arg5.handleExitingAnimation = function handleExitingAnimation(offsetParent, processedConfig) {
  ...
  const snapshots2 = tmp6(1720).snapshots;
