...
maybeUnpackValue.__closure = { Reanimated: require("module_6698").Reanimated };
...
export const bindSharedValues = function bindSharedValues(closure_1, closure_2) {
  ...
  if (undefined !== _require(6698).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _require(6698).Reanimated;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, closure_2) {
  if (undefined !== _require(6698).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(6698).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(6698).Reanimated;
        const fn = function t() {
