...
function extractProps(markerMid) {
  ...
  if (markerStart) {
    let tmp7;
    if (markerStart) {
      const match = markerStart.match(pickNotNil /* pickNotNil */.idPattern);
      let tmp10;
      if (match) {
        tmp10 = match[1];
      }
  ...
  if (str) {
    let tmp11;
    if (str) {
      const match1 = str.match(pickNotNil /* pickNotNil */.idPattern);
      let tmp14;
      if (match1) {
        tmp14 = match1[1];
      }
  ...
  if (str2) {
    let tmp15;
    if (str2) {
      const match2 = str2.match(pickNotNil /* pickNotNil */.idPattern);
      let tmp18;
      if (match2) {
        tmp18 = match2[1];
      }
  ...
  if (clipPath) {
    const match3 = clipPath.match(pickNotNil /* pickNotNil */.idPattern);
    if (match3) {
  ...
  if (mask) {
    const match4 = mask.match(pickNotNil /* pickNotNil */.idPattern);
    if (match4) {
      obj.mask = match4[1];
    } else {
  ...
  if (filter) {
    const match5 = filter.match(pickNotNil /* pickNotNil */.idPattern);
    if (match5) {
      obj.filter = match5[1];
    } else {
