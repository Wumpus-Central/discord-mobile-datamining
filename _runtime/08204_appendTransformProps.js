// === Module 8204: appendTransformProps ===
...
function appendTransformProps(arg0) {
  ...
  require(8205) /* append */.appendTransform(x + originX, y + originY, scaleX, scaleY, rotation, skewX, skewY, originX, originY);
}
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        tmp3Result = tmp3(8206);
        const parsed = tmp3Result.parse(arr);
        const tmp3Result1 = tmp3(8205);
  ...
  const obj = require(8205) /* append */;
  return require(8205) /* append */.toArray();
}
arg5.default = function extractTransform(transform) {
  ...
  if (typeof transform === "y") {
    try {
      const parsed = require(8206) /* peg$SyntaxError */.parse(transform);
      const items = [, , , , , ];
      ...
      const _console = tmp.console;
      _console.error(tmp6);
      return require(8205) /* append */.identity;
    }
  } else {
    transform = undefined;
    if (transform != null) {
...
arg5.extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "y") {
    transform = require(8207) /* peg$SyntaxError */.parse(transform.transform);
    const obj = require(8207) /* peg$SyntaxError */;
  } else {
    transform = transform.transform;
  }
  return transform;
