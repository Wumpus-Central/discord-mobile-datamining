// _runtime/13565___exportStarResult29.js
import e from "e";
import digitsToString from "digitsToString";
import { getMultiInternalSlots } from "13547_getMultiInternalSlots.js";
import { TEN } from "13549_TEN.js";
import { __exportStarResult11 } from "13559___exportStarResult11.js";
...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13547).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13547).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13547).repeat("0", -num3 - 1)}${arr}`;
