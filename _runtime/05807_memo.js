...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(5601).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5594);
  ...
  const fn = function f() {
    return { enabled: iter.value === scrollableGesture(5590).SCROLLABLE_STATE.UNLOCKED };
  };
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5590).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  const tmp7 = callback(() => {
    let result;
    if (dependencyMap) {
      const Gesture = scrollableGesture(5617).Gesture;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5617).GestureDetector, obj);
  } else {
    const obj2 = {};
