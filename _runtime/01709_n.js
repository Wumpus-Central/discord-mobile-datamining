...
let fn = function n(config, _animationToClamp) {
  ...
  const fn = function u() {
    ...
    if (undefined !== config.max) {
      strippedValue1 = config(1676).recognizePrefixSuffix(tmp3.max).strippedValue;
    ...
    obj = {
      ...
      reduceMotion: config(1676).getReduceMotionForAnimation(config.reduceMotion)
    };
    return obj;
  };
  obj = { _animationToClamp, config, recognizePrefixSuffix: _require(1676).recognizePrefixSuffix, logger: _require(1640).logger, getReduceMotionForAnimation: _require(1676).getReduceMotionForAnimation };
  fn.__closure = obj;
