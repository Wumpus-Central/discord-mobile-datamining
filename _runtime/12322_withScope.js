// _runtime/12322_withScope.js
import closure_2 from "metro/00041__classCallCheck.js";
import _createClass from "metro/00042__createClass.js";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12320).getMainCarrier();
  const obj = AsyncContextStack(12320);
  const sentryCarrier = AsyncContextStack(12320).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(12328);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12320).getMainCarrier();
      let obj = callback(12320);
      let sentryCarrier = callback(12320).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12328);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12320).getMainCarrier();
      const obj = callback(12320);
      const sentryCarrier = callback(12320).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12328);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
