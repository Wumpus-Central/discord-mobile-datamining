...
arg5.logSpanStart = function logSpanStart(spanContext) {
  ...
  if (require(6399) /* __SENTRY_DEBUG__ */.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(6376).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(6376).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(6376).spanToJSON(rootSpan);
    }
    const logger = tmp(6371).logger;
