// _runtime/01185_TimeToInitialDisplay.js
import registerSpanErrorInstrumentation from "00814_registerSpanErrorInstrumentation.js";
import _isNativeReflectConstruct from "01186__isNativeReflectConstruct.js";
    ...
    if (found) {
      ...
      if (startInactiveSpanResult) {
        ...
        if (isAutoInstrumented) {
          const attr = startInactiveSpanResult.setAttribute(tmp(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, tmp(1155).SPAN_ORIGIN_AUTO_UI_TIME_TO_DISPLAY);
        } else {
          const result = weakMap.set(activeSpan, true);
          const attr1 = startInactiveSpanResult.setAttribute(tmp(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, tmp(1155).SPAN_ORIGIN_MANUAL_UI_TIME_TO_DISPLAY);
        }
...
function startTimeToFullDisplaySpan(arg0) {
  ...
  let timeout;
  const activeSpan = spanId(814).getActiveSpan();
  let obj3 = spanId(814);
  if (activeSpan) {
    ...
    if (found) {
      ...
      if (found1) {
        ...
        if (startInactiveSpanResult) {
          ...
          tmp(814).fill(startInactiveSpanResult, "end", (arg0) => {
            closure_0 = arg0;
            return (arg0) => {
          ...
          const SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN = tmp(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN;
          const tmpResult2 = tmp(1155);
          if (isAutoInstrumented) {
