// _runtime/01745_NativeEventsManager.js
...
const entry = {
  ...
  value: function attachEvents() {
    ...
    for (const key10010 in props) {
      ...
      if (hasItem) {
        hasItem = tmp6.workletEventHandler instanceof NativeEventsManager(1743).WorkletEventHandler;
      }
      if (!hasItem) {
        continue;
      } else {
    ...
    value: function updateEvents(current) {
      ...
      if (_classPrivateFieldBase(this, closure_6)[closure_6] !== eventViewTag) {
        for (const key10071 in arg0) {
          ...
          if (hasItem) {
            hasItem = tmp37.workletEventHandler instanceof NativeEventsManager(1743).WorkletEventHandler;
          }
          if (!hasItem) {
            continue;
          } else {
        ...
        for (const key10010 in arg0) {
          ...
          if (hasItem1) {
            hasItem1 = tmp29.workletEventHandler instanceof NativeEventsManager(1743).WorkletEventHandler;
          }
          if (!hasItem1) {
            continue;
          } else {
            ...
            if (tmp5) {
              let tmp30Result = NativeEventsManager(1735);
              let hasItem2 = tmp30Result.has("workletEventHandler", tmp5);
              if (hasItem2) {
                hasItem2 = tmp5.workletEventHandler instanceof NativeEventsManager(1743).WorkletEventHandler;
              }
              if (hasItem2) {
        ...
        for (const key10054 in props) {
          ...
          if (hasItem3) {
            hasItem3 = tmp33.workletEventHandler instanceof NativeEventsManager(1743).WorkletEventHandler;
          }
          if (!hasItem3) {
            continue;
          } else {
  ...
  },
  {
    ...
    value: function getEventViewTag(arg0) {
      ...
      if (getScrollableNode) {
        ...
        if (setNativeProps) {
          let num3 = NativeEventsManager(1746).findNodeHandle(_classPrivateFieldBase(self, closure_4)[closure_4]);
          if (num3 == null) {
            num3 = -1;
          }
