// _runtime/12593_createChildOrRootSpan.js
import _mod12574 from "metro/12574__.js";
import getClient from "12586_getClient.js";
import hasTracingEnabled from "12591_hasTracingEnabled.js";
import closure_2 from "metro/00032__slicedToArray.js";
import { getMainCarrier } from "12577_getMainCarrier.js";
import { getAsyncContextStrategy } from "12578_getAsyncContextStrategy.js";
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12600).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(12592).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12564).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(12564);
        const client = tmp(12586).getClient();
        if (client) {
        ...
        const tmpResult2 = tmp(12586);
        tmp(12564).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult3 = tmp(12564);
      }
      tmp(12597).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(12597);
      const result = tmp(12588).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12595).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(12595);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(12564);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(12564).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12564).spanIsSampled(parentSpan);
      tmp(12595).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(12595);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12595).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(12595);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(12592).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(12598).sampleSpan(options, obj);
    const tmpResult = tmp(12598);
  }
  ...
  obj1.attributes = { [_mod12574.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(12600).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12574).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = require("12577_getMainCarrier.js").getMainCarrier();
  const obj = getMainCarrier;
  const tmp = _require;
  const asyncContextStrategy = require("12578_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12586).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(
        callback(closure_1_1[9]).propagationContextFromHeaders(closure_1, closure_2),
      );
      return callback();
    });
  }
          ...
          ? (arg0) => {
              ...
              if (asyncContextStrategy.withActiveSpan) {
                ...
                withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
                  _null(12581)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return require("12586_getClient.js").withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = getMainCarrier;
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = require("12578_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
      ...
      return undefined !== closure_4
        ...
        : ((arg0) => arg0())(() => {
            ...
            if (sentryNonRecordingSpan.onlyIfParent) {
              ...
              const tmpResult1 = tmp(tmp2[3]);
              return tmp(tmp2[4]).handleCallbackErrors(
                () => finishAndSetSpan(sentryNonRecordingSpan, finishAndSetSpan),
                () => {
                  obj = sentryNonRecordingSpan(closure_2_1[5]);
                  const status = obj.spanToJSON(sentryNonRecordingSpan).status;
                  const isRecordingResult = sentryNonRecordingSpan.isRecording();
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(12586).withScope((setSDKProcessingMetadata) => {
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(12586).withScope((arg0) => {
      _null(12581)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(12586);
