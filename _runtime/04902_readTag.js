...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  sum = typeSize + 04899__.getTypeSize("SHORT");
  const obj2 = 04899__;
  const sum1 = sum + 04899__.getTypeSize("LONG");
  const obj3 = 04899__;
  const shortAt = 04899__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 04899__;
  const shortAt1 = 04899__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 04899__;
  const longAt = 04899__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 04899__.typeSizes[shortAt1]) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 04899__.typeSizes[shortAt1];
    BYTE = 04899__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 04899__.typeSizes[BYTE];
  }
  if (BYTE === 04899__.tagTypes.ASCII) {
    let asciiValue = 04899__.getAsciiValue(items);
    const tmp9Result = 04899__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 04899__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 04899__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 04899__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };