...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_836").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(816).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(836).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(816).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(816);
        SentrySpan(858).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(858);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      const obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 836).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(816).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 836).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], profile_id: this._attributes[SentrySpan(undefined, 836).SEMANTIC_ATTRIBUTE_PROFILE_ID], exclusive_time: this._attributes[SentrySpan(undefined, 836).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME], measurements: null, is_segment: null, segment_id: null, links: null };
      ({ _endTime: obj[7], _traceId: obj[8], _attributes } = this);
      const obj2 = SentrySpan(816);
      obj[12] = SentrySpan(859).timedEventsToMeasurements(this._events);
      ...
      obj[14] = spanId;
      const obj3 = SentrySpan(859);
      obj[15] = SentrySpan(816).convertSpanLinksForEnvelope(self._links);
      return obj;
    }
  },
  {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(845).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(817).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(845).getCurrentScope();
              const tmpResult2 = tmp(845);
            }
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        if (!self._name) {
          if (tmp3(820).DEBUG_BUILD) {
            const debug = tmp3(821).debug;
        ...
        if (true === self._sampled) {
          ...
          const mapped = found.map((arg0) => self(816).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp11 = self._attributes[tmp3(undefined, 836).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((arg0) => {
            const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(836).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
            delete tmp2[tmp];
          });
          ...
          obj = { trace: null };
          obj[0] = tmp3(816).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          obj1 = { capturedSpanScope: null, capturedSpanIsolationScope: null, dynamicSamplingContext: null };
          obj1[0] = scope;
          obj1[1] = capturedScopesOnSpan.isolationScope;
          const tmp3Result1 = tmp3(816);
          obj1[2] = tmp3(854).getDynamicSamplingContextFromSpan(self);
          obj[6] = obj1;
          ...
          const tmp3Result2 = tmp3(854);
          const result = tmp3(859).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
