// _runtime/14112___exportStarResult25.js
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(14084).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(14110).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(14110).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(14109).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(14108).FormatApproximately(internalSlots, result);
    ...
    return tmp(14097).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(14109).FormatNumeric(internalSlots, isNaN);
};
