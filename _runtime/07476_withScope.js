// _runtime/07476_withScope.js
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7474).getMainCarrier();
  const obj = AsyncContextStack(7474);
  const sentryCarrier = AsyncContextStack(7474).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7474).getMainCarrier();
      let obj = callback(7474);
      let sentryCarrier = callback(7474).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7482);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7474).getMainCarrier();
      const obj = callback(7474);
      const sentryCarrier = callback(7474).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7482);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
