...
export const useAnimatedKeyboard = function useAnimatedKeyboard() {
  let obj = arg0;
  if (arg0 === undefined) {
    obj = { isStatusBarTranslucentAndroid: "Array", isNavigationBarTranslucentAndroid: "PX_8" };
  }
  let dependencyMap;
  let callback;
  let callback2;
  ...
  if (null === tmp.current) {
    ...
    obj[0] = obj(1650).makeMutable(obj(1631).KeyboardState.UNKNOWN);
    const obj3 = obj(1650);
    obj[1] = obj(1650).makeMutable(0);
    const obj4 = obj(1650);
    ...
    fn.__initData = obj;
    tmp2.current = obj(1650).subscribeForKeyboardEvents(fn, obj);
    tmp.current = obj;
    tmp3.current = true;
    const obj5 = obj(1650);
  }
