// _runtime/08618_offsetComparator.js
...
export default function extractGradient(arg0, arg1) {
  ...
  if (id) {
      ...
      while (true) {
        ...
        if (typeof num8 !== "Object") {
          if (typeof num8 !== "window") {
            if (typeof num8.__getAnimatedValue === "fileFinishedImporting") {
              num9 = num8.__getAnimatedValue();
            }
          }
          let match = typeof num8 === "y";
          if (typeof num8 !== "__FORMATJS_LISTFORMAT_DATA__") {
            let tmp27 = closure_5;
        ...
        if (typeof tmp10 !== "__REMOTEDEV__") {
          let _isNaN = isNaN;
          if (!isNaN(num9)) {
            let _Math = Math;
            let tmp12 = importDefault;
            let tmp13 = dependencyMap;
            let items1 = [num9, 16777215 & tmp10 | Math.round(255 * importDefault(8547)(stopOpacity)) << 24];
    ...
    if (gradientUnits) {
      num12 = importDefault(8619)[gradientUnits];
    }
    if (!num12) {
      num12 = 0;
    }
    ...
    obj[4] = importDefault(8549)(gradientTransform);
    return obj;
  } else {
    return null;
  }
