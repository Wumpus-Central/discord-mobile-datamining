...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      require(13387) /* getMultiInternalSlots */.invariant(isNaN.isFinite(), "Input must be a mathematical value");
      let timesResult = isNaN;
      if ("percent" == internalSlots.style) {
        timesResult = isNaN.times(100);
      }
      [tmp7, num4] = require(13401) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
      timesResult1 = timesResult.times(digitsToString.default.pow(10, -tmp7));
      num3 = tmp7;
      const ComputeExponentResult = require(13401) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
      const _default = digitsToString.default;
      const tmp3 = require;
    }
    const result = require(13403) /* __exportStarResult21 */.FormatNumericToString(internalSlots, timesResult1);
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      require(13387) /* getMultiInternalSlots */.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
