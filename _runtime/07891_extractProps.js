// _runtime/07891_extractProps.js
import { pickNotNil } from "07892_pickNotNil.js";
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7883)(obj, markerMid, arg1);
  module(7893)(obj, markerMid, items);
  module(7896)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7884)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7882)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
