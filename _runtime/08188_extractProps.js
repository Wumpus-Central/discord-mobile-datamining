// _runtime/08188_extractProps.js
import pickNotNil from "08189_pickNotNil.js";
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8180)(obj, markerMid, arg1);
  module(8190)(obj, markerMid, items);
  module(8193)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8181)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8179)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
