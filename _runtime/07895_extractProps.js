// _runtime/07895_extractProps.js
import { pickNotNil } from "07896_pickNotNil.js";
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7887)(obj, markerMid, arg1);
  module(7897)(obj, markerMid, items);
  module(7900)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7888)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7886)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
