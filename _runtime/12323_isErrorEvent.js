// _runtime/12323_isErrorEvent.js
...
let items = [
  ...
  },
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      self = this;
      dependencyMap = arg3;
      obj = { event_id: obj(12279).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = obj(12279);
      const tmp = obj;
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(12296).DEBUG_BUILD) {
          const logger = tmp(12268).logger;
        ...
        BaseClient(12291).updateSession(release, { init: false });
        const obj = BaseClient(12291);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(12324).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(12324).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(12324);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(12311).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
    ...
    value: function recordDroppedEvent(arg0, arg1, num) {
      ...
      if (this._options.sendClientReports) {
        ...
        if (BaseClient(12296).DEBUG_BUILD) {
          const logger = tmp6(12268).logger;
          let str3 = "";
          if (num2 > 1) {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12296).DEBUG_BUILD) {
        let logger = tmp2(12268).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12292).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(12324).setupIntegrations(this, integrations);
      const obj = BaseClient(12324);
      const result = BaseClient(12324).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(12308).parseSampleRate(sampleRate);
        let obj = str(12308);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12325).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(12292).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(12296).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(12268).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(12326);
