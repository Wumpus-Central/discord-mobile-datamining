  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_13172").getClient();
        if (client) {
          client.flush(closure_0);
        } else {
          if (require("module_13173").DEBUG_BUILD) {
            const logger = require("module_13145").logger;
            logger.warn("Cannot flush events. No client defined.");
          }
          const resolved = Promise.resolve(false);
        }
        c1 = 3;
        const obj3 = require("module_13172");
      }
    } catch (tmp8) {
      c1 = tmp;
      throw tmp8;
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod13159.timestampInSeconds() - closure_3 };
    const currentScope = _mod13172.getCurrentScope();
    const client = _mod13172.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_13172").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_13172");
  let client = require("module_13172").getClient();
  if (client) {
    if (client.captureCheckIn) {
  ...
  let obj3 = require("module_13172");
  captureCheckInResult = require("module_13156").uuid4();
};