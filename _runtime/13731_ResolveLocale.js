// _runtime/13731_ResolveLocale.js
import LookupMatcher from "13732_LookupMatcher.js";
import _mod13733 from "metro/13733__.js";
import BestFitMatcher from "13737_BestFitMatcher.js";
import CanonicalizeUValue from "13739_CanonicalizeUValue.js";
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(13738).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod13733.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod13733.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = _mod13733.invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(13740).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
