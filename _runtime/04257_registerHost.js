...
function reducer(arg0, type) {
  ...
  if (portalName(4253).ACTIONS.REGISTER_HOST === type) {
    ...
    return obj;
  } else if (tmp4(4253).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4253).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4253).ACTIONS.REMOVE_PORTAL === type) {
