...
import allowedNativeProps from "allowedNativeProps" /* 7041 */;
...
let c4 = 1.618;
fn(7027).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(7026).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod7026.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = _mod7026.Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_7026").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      [tmp6, tmp7] = tmp5;
      let tmp10 = _require;
      let Reanimated = require("module_7026").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(7026).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod7026.Reanimated;
          const fn = function t() {
