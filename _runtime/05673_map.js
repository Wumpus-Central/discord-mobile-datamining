// _runtime/05673_map.js
import _mod5647 from "metro/05647__.js";
import hash from "05661_hash.js";
import items2 from "05662_items2.js";
import isNativeEvent from "05674_isNativeEvent.js";
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!disableReanimated) {
    result = undefined !== _mod5647.Reanimated;
  }
  if (result) {
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      ...
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(5662).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(5647).Reanimated;
