// _runtime/01736_checkIfConfigIsValid.js
...
function checkIfConfigIsValid(duration) {
  ...
  if (min) {
    ...
    dependencyMap =
      dependencyMap +
      ", clamp.min should be lower than clamp.max, got clamp: {min: " +
      duration.clamp.min +
      ", max: " +
      duration.clamp.max +
      "} ";
  ...
  if ("" !== dependencyMap) {
    const logger = require("01657_addLogBoxLog.js").logger;
...
initialCalculations.__initData = {
  ...
  if (NumberResult === startValue) {
    ...
    items3[
      HermesBuiltin.arraySpread(
        items2.filter((arg0) => undefined !== arg0),
        0,
      )
    ] = zeta;
    const _Math10 = Math;
...
scaleZetaToMatchClamps.__initData = {
...
function underDampedSpringCalculations(toValue, arg1) {
  ...
  return {
    position: toValue - result,
    velocity:
      zeta * omega0 * result -
      expResult * (cosResult * (-toValue.velocity + zeta * omega0 * diff) - omega1 * diff * sinResult),
  };
}
