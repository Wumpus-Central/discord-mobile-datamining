// _runtime/12614_isErrorEvent.js
...
let items = [
  ...
  },
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      self = this;
      dependencyMap = arg3;
      obj = { event_id: obj(12570).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = obj(12570);
      const tmp = obj;
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(12587).DEBUG_BUILD) {
          const logger = tmp(12559).logger;
        ...
        BaseClient(12582).updateSession(release, { init: false });
        const obj = BaseClient(12582);
      }
    },
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(12615).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(12615).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(12615);
      }
    },
  },
  {
    ...
    value: function sendEvent(arg0) {
      ...
      let eventEnvelope = BaseClient(self[12]).createEventEnvelope(
        arg0,
        self._dsn,
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(
        BaseClient(12602).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel),
      );
    },
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12587).DEBUG_BUILD) {
        let logger = tmp2(12559).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12583).resolvedSyncPromise({});
    },
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(12615).setupIntegrations(this, integrations);
      const obj = BaseClient(12615);
      const result = BaseClient(12615).afterSetupIntegrations(this, integrations);
    },
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(12599).parseSampleRate(sampleRate);
        let obj = str(12599);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12616).SentryError(
              "Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")",
              "log",
            );
            return str(12583).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
      ...
      let nextPromise = self
        ...
        .then((spans) => {
          if (null === spans) {
            ...
            let sentryError = new str(type[17]).SentryError(
      ...
      return self
        ...
        .then((sdkProcessingMetadata) => {
          if (null === sdkProcessingMetadata) {
            ...
            const _HermesInternal = HermesInternal;
            const sentryError = new str(type[17]).SentryError(
        ...
        .then(null, (arg0) => {
          if (arg0 instanceof str(type[17]).SentryError) {
            ...
            const _HermesInternal = HermesInternal;
            const sentryError = new tmp(tmp2[17]).SentryError(
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(12587).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(12559).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(12617);
