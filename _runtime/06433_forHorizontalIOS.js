// _runtime/06433_forHorizontalIOS.js
import conditional from "06434_conditional.js";
...
export const forModalPresentationIOS = function forModalPresentationIOS(next) {
  ...
  return {
    cardStyle: obj5,
    overlayStyle: { opacity: tmp3Result.interpolate({ inputRange: [0, 1, 1.0001, 2], outputRange: [0, 0.3, 1, 1] }) },
  };
};
export const forFadeFromBottomAndroid = function forFadeFromBottomAndroid(current) {
  ...
  obj3.opacity = conditional.conditional(
    closing,
    current.progress,
...
export const forFadeFromCenter = function forFadeFromCenter(current) {
  const progress = current.current.progress;
  const obj = {
    cardStyle: { opacity: progress.interpolate({ inputRange: [0, 0.5, 0.9, 1], outputRange: [0, 0.25, 0.7, 1] }) },
    overlayStyle: null,
  };
