// _runtime/09757__isNativeReflectConstruct.js
import _asyncLoop from "09751__asyncLoop.js";
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9752).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9751).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9752).exists(this);
      HashMD(9752).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9751).createView(content);
