  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(4934).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(4934);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(4897).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(4897).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(4897);
    obj = {};
    ...
    ParseError(4897).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(4897);
  }
  ParseError(4897).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
