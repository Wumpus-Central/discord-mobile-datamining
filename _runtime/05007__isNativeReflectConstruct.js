// _runtime/05007__isNativeReflectConstruct.js
import _slicedToArray from "_slicedToArray";
import _classCallCheck from "_classCallCheck";
import _possibleConstructorReturn from "_possibleConstructorReturn";
import _getPrototypeOf from "_getPrototypeOf";
import importDefaultResult from "_createClass";
import { calculateGPSValue } from "05010_calculateGPSValue.js";
...
function readTags(_raw, dataView, arg2) {
  ...
  try {
    ...
    ParseError(4972).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5009).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5009);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValue[tmp2] === "function") {
        tmp12 = tmp13(5010)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5010);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(4972).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(4972).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(4972);
    obj = {};
    ...
    ParseError(4972).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(4972);
  }
  ParseError(4972).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
