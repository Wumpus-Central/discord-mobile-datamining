...
arg5.parseNumberSkeletonFromString = function parseNumberSkeletonFromString(closure_2) {
  if (0 === closure_2.length) {
    ...
    const items = [];
    const parts = closure_2.split(_mod1284.WHITE_SPACE_REGEX);
    const found = parts.filter((item, index) => item.length > 0);
    let num2 = 0;
    if (0 < found.length) {
