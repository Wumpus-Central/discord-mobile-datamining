...
// Module 10446 (Chrono)
import ENDefaultConfiguration from "ENDefaultConfiguration" /* 10447 */;
...
obj = {
  ...
  value: function executeParser(closure_0, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof index(10453).ParsingResult) {
      ...
      } else if (!(extractResult instanceof tmp6(10453).ParsingComponents)) {
...
obj = {
  ...
  value: function createParsingComponents(extractResult) {
    ...
    if (!(extractResult instanceof ParsingContext(10453).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(10453).ParsingComponents(this.reference, extractResult);
    }
...
const items2 = [
  obj,
  {
    ...
    value: function createParsingResult(index, length2, extractResult, extractResult) {
      ...
      return new ParsingContext(10453).ParsingResult(self.reference, index, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
