// _runtime/07490_getDynamicSamplingContextFromSpan.js
import { addNonEnumerableProperty } from "07460_addNonEnumerableProperty.js";
import { baggageHeaderToDynamicSamplingContext } from "07467_baggageHeaderToDynamicSamplingContext.js";
import { getClient } from "07481_getClient.js";
import { 07491__ } from "metro/07491__.js";
const require = arg1;
    ...
    if (rootSpan[_frozenDsc]) {
      ...
      if (result) {
        ...
        if (!DEFAULT_ENVIRONMENT) {
          DEFAULT_ENVIRONMENT = tmp(7491).DEFAULT_ENVIRONMENT;
        }
        ...
        const dropUndefinedKeysResult = tmp(7460).dropUndefinedKeys(obj);
        client.emit("createDsc", dropUndefinedKeysResult);
        const tmpResult1 = tmp(7460);
        const spanToJSONResult = tmp(7459).spanToJSON(rootSpan);
        const tmp13 = spanToJSONResult.data || {};
        const tmp14 = tmp13[tmp(undefined, 7469).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
        if (null != tmp14) {
        ...
        tmp17 = "url" !== tmp13[tmp(undefined, 7469).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
        if (tmpResult3.hasTracingEnabled()) {
          const _String = String;
          dropUndefinedKeysResult.sampled = String(tmp(7459).spanIsSampled(rootSpan));
          const tmpResult4 = tmp(7459);
        }
...
arg5.getDynamicSamplingContextFromClient = function getDynamicSamplingContextFromClient(trace_id, getOptions) {
  ...
  let DEFAULT_ENVIRONMENT = options.environment;
  if (!DEFAULT_ENVIRONMENT) {
    DEFAULT_ENVIRONMENT = 07491__.DEFAULT_ENVIRONMENT;
  }
...
arg5.getDynamicSamplingContextFromScope = function getDynamicSamplingContextFromScope(getOptions, getPropagationContext) {
  ...
  if (!dsc) {
    ...
    let DEFAULT_ENVIRONMENT = options.environment;
    if (!DEFAULT_ENVIRONMENT) {
      DEFAULT_ENVIRONMENT = tmp5(7491).DEFAULT_ENVIRONMENT;
    }
    obj = { environment: null, release: null, public_key: null, trace_id: null };
    obj[0] = DEFAULT_ENVIRONMENT;
    obj[1] = options.release;
