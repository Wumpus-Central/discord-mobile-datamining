// _runtime/05539_getHandler.js
import { tagMessage } from "05493_tagMessage.js";
...
function getHandler(arg0, onBegin) {
  ...
  } else if (tmp(5509).CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (tmp(5509).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(5509).CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (tmp(5509).CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (tmp(5509).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(5509).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(5509).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(5509).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(5509).CALLBACK_TYPE.TOUCHES_CANCEL === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (05496__.TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(5509).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(5496).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(5509).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(5496).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(5509).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(5496).TouchEventType.TOUCHES_CANCEL === arg0) {
    return tmp(5509).CALLBACK_TYPE.TOUCHES_CANCEL;
  } else {
    return tmp(5509).CALLBACK_TYPE.UNDEFINED;
  }
}
obj = { TouchEventType: require("module_5496").TouchEventType, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
