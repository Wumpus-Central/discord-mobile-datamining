...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (
    ...
    const _Error2 = Error;
    error = new Error(
      require("05665_tagMessage.js").tagMessage(
        'The "disableReanimated" property must not be changed after the handler is created.',
      ),
    );
    throw error;
  } else {
    const gestureCallbacks = require("05744_useGestureCallbacks.js").useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        ...
        const error1 = new Error(
          require("05665_tagMessage.js").tagMessage("Failed to create reanimated event handlers."),
        );
        throw error1;
      }
    }
