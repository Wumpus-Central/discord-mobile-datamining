// _runtime/01311_binaryReadOptions.js
...
let items = [
  ...
  },
  {
    ...
    value: function skip(arg0) {
      ...
      let tmp14;
      const self = this;
      if (BinaryReader(1310).WireType.Varint === arg0) {
        self.pos = +self.pos + 1;
        ...
        if (tmp(1310).WireType.Bit64 === arg0) {
          self.pos = self.pos + 4;
        } else if (tmp(1310).WireType.Bit32 !== arg0) {
          if (tmp(1310).WireType.LengthDelimited === arg0) {
            self.pos = self.pos + self.uint32();
          } else if (tmp(1310).WireType.StartGroup === arg0) {
            const tmp8 = self.tag()[1];
            if (tmp8 !== tmp(1310).WireType.EndGroup) {
              do {
                let skipResult = self.skip(self.tag()[1]);
                let tmp10 = BinaryReader;
                let tmp11 = dependencyMap;
                EndGroup = BinaryReader(1310).WireType.EndGroup;
              } while (tmp8 !== EndGroup);
            }
          } else {
            const _Error = Error;
  ...
  },
  {
    ...
    value: function int64() {
      return BinaryReader(1313).PbLong(...this.varint64());
    },
  },
  {
    key: "uint64",
    value: function uint64() {
      return BinaryReader(1313).PbULong(...this.varint64());
    },
  },
  {
    key: "sint64",
    value: function sint64() {
      ...
      const pbLong = new BinaryReader(1313).PbLong(((tmp2 >>> 1) | ((1 & tmp3) << 31)) ^ tmp4, (tmp3 >>> 1) ^ tmp4);
      return pbLong;
    },
  },
  {
  ...
  },
  {
    ...
    value: function fixed64() {
      const pbULong = new BinaryReader(1313).PbULong(this.sfixed32(), this.sfixed32());
      return pbULong;
    },
  },
  {
    ...
    value: function sfixed64() {
      const pbLong = new BinaryReader(1313).PbLong(this.sfixed32(), this.sfixed32());
      return pbLong;
    },
  },
  {
