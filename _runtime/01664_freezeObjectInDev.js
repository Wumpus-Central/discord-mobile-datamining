// _runtime/01664_freezeObjectInDev.js
import _slicedToArray from "_slicedToArray";
import isJest from "isJest";
import { weakMap } from "01665_weakMap.js";
...
let fn = function u() {
  const proxy = new Proxy({}, {
    get(arg0, arg1) {
      if ("_isReanimatedSharedValue" !== arg1) {
        if ("__remoteFunction" !== arg1) {
          ...
          const reanimatedError = new callback(1645).ReanimatedError("Trying to access property `" + String(arg1) + "` of an object which cannot be sent to the UI runtime.");
    ...
    set() {
      const reanimatedError = new callback(1645).ReanimatedError("Trying to write to an object which cannot be sent to the UI runtime.");
      throw reanimatedError;
    }
  });
  return proxy;
...
export const makeShareable = isJest ? (function makeShareableJS(arg0) {
  ...
  if (shareableMappingCache.get(arg0)) {
    ...
    obj[0] = fn;
    const shareableMappingCache2 = tmp(1665).shareableMappingCache;
