...
export const applyScopeDataToEvent = function applyScopeDataToEvent(extra, arg1) {
  ...
  const dropUndefinedKeysResult = _mod13151.dropUndefinedKeys(extra);
  ...
  const dropUndefinedKeysResult1 = _mod13151.dropUndefinedKeys(tags);
  ...
  const dropUndefinedKeysResult2 = _mod13151.dropUndefinedKeys(user);
  ...
  const tmp3Result7 = _mod13151;
  const dropUndefinedKeysResult3 = _mod13151.dropUndefinedKeys(contexts);
  ...
  if (span) {
    const obj6 = { trace: spanTimeInputToSeconds.spanToTraceContext(span) };
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj6;
    const obj7 = { dynamicSamplingContext: null };
    const tmp3Result9 = spanTimeInputToSeconds;
    obj7.dynamicSamplingContext = _mod13181.getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj7;
    const tmp3Result10 = _mod13181;
    const rootSpan = spanTimeInputToSeconds.getRootSpan(span);
    const tmp3Result11 = spanTimeInputToSeconds;
    ...
    const tmp3Result12 = spanTimeInputToSeconds;
  }
...
export const mergeScopeData = function mergeScopeData(extra, arg1) {
  ...
  extra.extra = _mod13170.merge(extra.extra, extra, 1);
  extra.tags = _mod13170.merge(extra.tags, tags, 1);
  extra.user = _mod13170.merge(extra.user, user, 1);
  extra.contexts = _mod13170.merge(extra.contexts, contexts, 1);
  extra.sdkProcessingMetadata = _mod13170.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
