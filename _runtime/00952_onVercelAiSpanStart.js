// === Module 952: onVercelAiSpanStart ===
...
function onVercelAiSpanStart(setAttribute) {
  ...
  if (description) {
    if (data[tmp3(undefined, 953).AI_TOOL_CALL_NAME_ATTRIBUTE]) {
      if (data[tmp3(undefined, 953).AI_TOOL_CALL_ID_ATTRIBUTE]) {
        if ("ai.toolCall" === description) {
          const attr = setAttribute.setAttribute(tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, "auto.vercelai.otel");
          const attr1 = setAttribute.setAttribute(tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_OP, "gen_ai.execute_tool");
          const AI_TOOL_CALL_NAME_ATTRIBUTE = tmp3(953).AI_TOOL_CALL_NAME_ATTRIBUTE;
          if (null != data[AI_TOOL_CALL_NAME_ATTRIBUTE]) {
            data["gen_ai.tool.name"] = data[AI_TOOL_CALL_NAME_ATTRIBUTE];
            delete tmp[tmp2];
          }
          const AI_TOOL_CALL_ID_ATTRIBUTE = tmp3(953).AI_TOOL_CALL_ID_ATTRIBUTE;
          ...
          if (typeof gen_ai_tool_call_id === "string") {
            const toolCallSpanMap = tmp3(956).toolCallSpanMap;
    ...
    let startsWithResult = data[tmp3(undefined, 953).AI_OPERATION_ID_ATTRIBUTE];
    if (!startsWithResult) {
      startsWithResult = description.startsWith("ai.");
    }
    if (startsWithResult) {
      const attr3 = setAttribute.setAttribute(tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, "auto.vercelai.otel");
      ...
      const tmp11 = data[tmp3(undefined, 953).AI_TELEMETRY_FUNCTION_ID_ATTRIBUTE];
      let tmp12 = tmp11;
      if (tmp11) {
      ...
      let tmp3Result = tmp3(954);
      const messagesFromPrompt = tmp3Result.requestMessagesFromPrompt(setAttribute, data);
      if (tmp17) {
        const attr6 = setAttribute.setAttribute(tmp3(955).GEN_AI_RESPONSE_MODEL_ATTRIBUTE, data[tmp3(undefined, 953).AI_MODEL_ID_ATTRIBUTE]);
      }
      ...
      if (spanOpFromName) {
        const attr8 = setAttribute.setAttribute(tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_OP, spanOpFromName);
      }
      const tmp22 = data[tmp3(undefined, 953).AI_MODEL_ID_ATTRIBUTE];
      if (tmp22) {
      ...
      tmp17 = data[tmp3(undefined, 953).AI_MODEL_ID_ATTRIBUTE] && !data[tmp3(undefined, 955).GEN_AI_RESPONSE_MODEL_ATTRIBUTE];
    }
  }
}
...
function processEndedVercelAiSpan(item10015) {
  ...
  if ("auto.vercelai.otel" === item10015.origin) {
    renameAttributeKey(data, _mod953.AI_USAGE_COMPLETION_TOKENS_ATTRIBUTE, _mod955.GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE);
    renameAttributeKey(data, _mod953.AI_USAGE_PROMPT_TOKENS_ATTRIBUTE, _mod955.GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE);
    renameAttributeKey(data, _mod953.AI_USAGE_CACHED_INPUT_TOKENS_ATTRIBUTE, _mod955.GEN_AI_USAGE_INPUT_TOKENS_CACHED_ATTRIBUTE);
    const tmp34 = data[_mod955.GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE];
    let tmp35 = typeof tmp34 === "number";
    if (typeof tmp34 === "number") {
      tmp35 = typeof data[tmp29(undefined, 955).GEN_AI_USAGE_INPUT_TOKENS_CACHED_ATTRIBUTE] === "number";
    }
    if (tmp35) {
      data[tmp29(955).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] = data[tmp29(undefined, 955).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] + data[tmp29(undefined, 955).GEN_AI_USAGE_INPUT_TOKENS_CACHED_ATTRIBUTE];
    }
    const tmp = data[_mod955.GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE];
    let tmp2 = typeof tmp === "number";
    if (typeof tmp === "number") {
      tmp2 = typeof data[tmp29(undefined, 955).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE] === "number";
    }
    if (tmp2) {
      data["gen_ai.usage.total_tokens"] = data[tmp29(undefined, 955).GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE] + data[tmp29(undefined, 955).GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE];
    }
    let isArray = data[tmp29(undefined, 953).AI_PROMPT_TOOLS_ATTRIBUTE];
    if (isArray) {
      const _Array = Array;
      isArray = Array.isArray(data[tmp29(undefined, 953).AI_PROMPT_TOOLS_ATTRIBUTE]);
    }
    if (isArray) {
      data[tmp29(953).AI_PROMPT_TOOLS_ATTRIBUTE] = tmp29(954).convertAvailableToolsToJsonString(data[tmp29(undefined, 953).AI_PROMPT_TOOLS_ATTRIBUTE]);
      const tmp29Result = tmp29(954);
    }
    renameAttributeKey(data, _mod953.OPERATION_NAME_ATTRIBUTE, _mod955.GEN_AI_OPERATION_NAME_ATTRIBUTE);
    renameAttributeKey(data, _mod953.AI_PROMPT_MESSAGES_ATTRIBUTE, _mod955.GEN_AI_REQUEST_MESSAGES_ATTRIBUTE);
    renameAttributeKey(data, _mod953.AI_RESPONSE_TEXT_ATTRIBUTE, "gen_ai.response.text");
    renameAttributeKey(data, _mod953.AI_RESPONSE_TOOL_CALLS_ATTRIBUTE, "gen_ai.response.tool_calls");
    renameAttributeKey(data, _mod953.AI_RESPONSE_OBJECT_ATTRIBUTE, "gen_ai.response.object");
    renameAttributeKey(data, _mod953.AI_PROMPT_TOOLS_ATTRIBUTE, "gen_ai.request.available_tools");
    renameAttributeKey(data, _mod953.AI_TOOL_CALL_ARGS_ATTRIBUTE, "gen_ai.tool.input");
    renameAttributeKey(data, _mod953.AI_TOOL_CALL_RESULT_ATTRIBUTE, "gen_ai.tool.output");
    renameAttributeKey(data, _mod953.AI_SCHEMA_ATTRIBUTE, "gen_ai.request.schema");
    renameAttributeKey(data, _mod953.AI_MODEL_ID_ATTRIBUTE, _mod955.GEN_AI_REQUEST_MODEL_ATTRIBUTE);
