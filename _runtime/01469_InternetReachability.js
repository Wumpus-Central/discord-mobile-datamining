// _runtime/01469_InternetReachability.js
...
class InternetReachability {
  constructor(arg0, arg1) {
    ...
    this._checkInternetReachability = () => {
      ...
      const nextPromise1 = Promise.race(items)
        .then((result) => {
          const _configuration = closure_0._configuration;
          return _configuration.reachabilityTest(result);
        })
        .then((result) => {
          ...
          const _configuration = closure_0._configuration;
          closure_0._currentTimeoutHandle = setTimeout(
            closure_0._checkInternetReachability,
            closure_0._isInternetReachable
              ? _configuration.reachabilityLongTimeout
      ...
      obj2.promise = Promise.race(items)
        ...
        .catch((error) => {
          if ("canceled" === error) {
            ...
            self._currentTimeoutHandle = setTimeout(
              self._checkInternetReachability,
              self._configuration.reachabilityShortTimeout,
            );
          }
        })
        .then(
