// === Module 7693: addToMetricsAggregator ===

// Module 7693 (addToMetricsAggregator)
import errorCallback from "errorCallback" /* 7593 */;
import _mod7694 from "module_7694" /* 7694 */;
...
function addToMetricsAggregator(arg0, SET_METRIC_TYPE, arg2, arg3, arg4) {
  ...
  if (!client) {
    client = _require(7624).getClient();
    const obj2 = _require(7624);
  }
  if (client) {
    const activeSpan = _require(7602).getActiveSpan();
    let rootSpan;
    if (activeSpan) {
    ...
    if (_require(7625).DEBUG_BUILD) {
      const logger = tmp3(7597).logger;
      const _HermesInternal = HermesInternal;
      logger.log("Adding value of " + arg3 + " to " + SET_METRIC_TYPE + " metric " + arg2);
    }
    const obj3 = _require(7602);
    const globalSingleton = _require(7598).getGlobalSingleton("globalMetricsAggregators", () => {
    ...
    const tmp3Result1 = _require(7598);
  }
}
errorCallback;
...
export const metrics = {
  ...
  timing(arg0, arg1, fn) {
    ...
    if (typeof fn === "function") {
      ...
      return _require(7631).startSpanManual(obj, (arg0) => {
        const callback = arg0;
      ...
      obj.unit = str;
      const DISTRIBUTION_METRIC_TYPE = _require(7694).DISTRIBUTION_METRIC_TYPE;
      let parsed = fn;
      if (typeof fn === "string") {
