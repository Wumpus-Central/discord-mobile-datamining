...
let items = [
  {
    ...
    set(arg0) {
      ...
      if (null === basicURLParseResult) {
        ...
        if (null !== query) {
          self._query._list = URLImpl(14454).parseUrlencoded(query);
          const tmpResult = URLImpl(14454);
        }
      }
  ...
  },
  {
    ...
    set(arg0) {
      const self = this;
      let obj = URLImpl(14451);
      if (!obj.cannotHaveAUsernamePasswordPort(this._url)) {
        if ("" === arg0) {
          self._url.port = null;
        } else {
          obj = { url: self._url, stateOverride: "port" };
          URLImpl(14451).basicURLParse(arg0, obj);
          const tmpResult = URLImpl(14451);
        }
      }
    },
