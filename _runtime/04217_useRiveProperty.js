// _runtime/04217_useRiveProperty.js
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29489) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f29489;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = require("04218_useDisposableMemo.js").useDisposableMemo(() => {
    if (closure_0) {
