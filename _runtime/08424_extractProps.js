// _runtime/08424_extractProps.js
import { pickNotNil } from "08425_pickNotNil.js";
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8416)(obj, markerMid, arg1);
  module(8426)(obj, markerMid, items);
  module(8429)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8417)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8415)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
