// === Module 12413: MetricsAggregator ===
...
const entry = {
  ...
  value: function add(metricType, arg1, diff) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12327).timestampInSeconds();
      const obj2 = MetricsAggregator(12327);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(12414).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(12414);
    const sanitizeTagsResult = MetricsAggregator(12414).sanitizeTags(obj);
    const obj4 = MetricsAggregator(12414);
    const sanitizeUnitResult = MetricsAggregator(12414).sanitizeUnit(str);
    const obj5 = MetricsAggregator(12414);
    const bucketKey = MetricsAggregator(12414).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    const _buckets = this._buckets;
    value = _buckets.get(bucketKey);
    let num = 0;
    if (value) {
      num = 0;
      if (metricType === MetricsAggregator(12410).SET_METRIC_TYPE) {
    ...
    if (value) {
      ...
      const tmp15 = new MetricsAggregator(12415).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(12414);
    const result1 = MetricsAggregator(12318).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(12410).MAX_WEIGHT) {
      self.flush();
    }
    const tmp5Result = MetricsAggregator(12318);
  }
};
const items = [
  entry,
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12327).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12410).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
        ...
        const obj = MetricsAggregator(12327);
      }
    }
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(12416).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(12416);
      }
    }
  }
];
