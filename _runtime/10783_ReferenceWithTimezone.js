// _runtime/10783_ReferenceWithTimezone.js
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10784).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10782).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10782).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10782).addDuration(
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10786).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10786).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10786).assignSimilarDate(obj, addDurationResult);
