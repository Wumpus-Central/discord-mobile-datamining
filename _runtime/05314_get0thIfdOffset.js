// _runtime/05314_get0thIfdOffset.js
import _modDef5297 from "metro/05297__.js";
import _modDef5311 from "metro/05311__.js";
import IFD_TYPE_0TH2 from "05315_IFD_TYPE_0TH.js";
...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  const shortAt1 = _modDef5311.getShortAt(byteLength, sum + typeSize, byteOrder);
  const longAt = _modDef5311.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5311.typeSizes[shortAt1]) {
    if (
      (function tagValueFitsInOffsetSlot(shortAt1, longAt) {
        const result = _modDef5311.typeSizes[shortAt1] * longAt;
        return result <= _modDef5311.getTypeSize("LONG");
      })(shortAt1, longAt)
    ) {
      const sum2 = sum + sum1;
      ...
      const longAt1 = _modDef5311.getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if (
        (function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
          sum = sum + longAt1;
          return sum + _modDef5311.typeSizes[shortAt1] * longAt <= byteLength.byteLength;
        })(byteLength, sum, longAt1, shortAt1, longAt)
      ) {
        str = getTagValue(byteLength, sum + longAt1, shortAt1, longAt, byteOrder, 33723 === shortAt);
        tmp21 = longAt1;
      }
      const tmp4Result = _modDef5311;
    }
    let tmp34 = str;
    if (shortAt1 === _modDef5311.tagTypes.ASCII) {
    ...
    if (undefined !== IFD_TYPE_0THDefault[IFD_TYPE_0TH][shortAt]) {
      ...
      if (shortAt1 !== _modDef5311.tagTypes.RATIONAL) {
        if (shortAt1 !== _modDef5311.tagTypes.SRATIONAL) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5311.typeSizes[shortAt1];
    BYTE = _modDef5311.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    let arr = items.push(obj[BYTE](byteLength, sum, byteOrder));
    sum = sum + _modDef5311.typeSizes[BYTE];
  }
  if (BYTE === _modDef5311.tagTypes.ASCII) {
    let asciiValue = _modDef5311.getAsciiValue(items);
    const tmp9Result = _modDef5311;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = {
  1: null,
  2: _modDef5311.getByteAt,
  3: _modDef5311.getAsciiAt,
  4: _modDef5311.getShortAt,
  5: _modDef5311.getLongAt,
  7: _modDef5311.getRationalAt,
  9: null,
  10: _modDef5311.getUndefinedAt,
  13: null,
};
obj[9] = _modDef5311.getSlongAt;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, c5, byteOrder) {
  return c5 + _modDef5311.getLongAt(buffer, c5 + 4, byteOrder);
};
export { readIfd };
