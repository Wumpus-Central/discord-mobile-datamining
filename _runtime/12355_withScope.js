// _runtime/12355_withScope.js
import closure_2 from "metro/00041__classCallCheck.js";
import _createClass from "metro/00042__createClass.js";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12353).getMainCarrier();
  const obj = AsyncContextStack(12353);
  const sentryCarrier = AsyncContextStack(12353).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(12361);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12353).getMainCarrier();
      let obj = callback(12353);
      let sentryCarrier = callback(12353).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12361);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12353).getMainCarrier();
      const obj = callback(12353);
      const sentryCarrier = callback(12353).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12361);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
