// _runtime/13501___exportStarResult11.js
  ...
  if (eq.eq(timesResult)) {
    ...
    if ("zero" === result) {
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        tmp2(13489).invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === result) {
          return timesResult;
        } else if ("half-infinity" === result) {
          return roundedNumber;
        } else {
          tmp2(13489).invariant("half-even" === result, "unsignedRoundingMode should be half-even");
