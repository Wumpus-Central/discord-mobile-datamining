// _runtime/14328_ResolveLocale.js
import LookupMatcher from "14329_LookupMatcher.js";
import _mod14330 from "metro/14330__.js";
import BestFitMatcher from "14334_BestFitMatcher.js";
import CanonicalizeUValue from "14336_CanonicalizeUValue.js";
...
export const ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher, arg3, arg4, fn) {
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14335).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = _mod14330.invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      let tmp19 = undefined === first;
      if (!tmp19) {
        tmp19 = typeof first === "string";
      }
      let invariantResult1 = _mod14330.invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = _mod14330.invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14337).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj2.locale = result;
  return obj2;
};
