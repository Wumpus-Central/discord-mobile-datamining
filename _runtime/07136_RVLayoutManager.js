// _runtime/07136_RVLayoutManager.js
import _modDef7111 from "metro/07111__.js";
import _classCallCheck from "metro/07110__.js";
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7137).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7137);
      const findLastVisibleIndexResult = RVLayoutManager(7137).findLastVisibleIndex(
        this.layouts,
        arg1,
        this.horizontal,
      );
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new RVLayoutManager(7124).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = RVLayoutManager(7124).ConsecutiveNumbers.EMPTY;
      const obj2 = RVLayoutManager(7137);
    },
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(7098).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef7111(RVLayoutManager, items);
