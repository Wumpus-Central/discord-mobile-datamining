// _runtime/07249_JSFPSMonitor.js
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7184).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7250).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7250);
      obj.maxFPS = JSFPSMonitor(7250).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7250);
      obj.averageFPS = JSFPSMonitor(7250).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    },
  },
];

export const JSFPSMonitor = _modDef7197(JSFPSMonitor, items);
