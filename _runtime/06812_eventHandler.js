// _runtime/06812_eventHandler.js
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6732.State.UNDETERMINED) {
    if (state === _mod6732.State.BEGAN) {
      let tmpResult = DEFAULT_PROPS_TRANSFORMER;
      tmpResult.runCallback(_mod6747.CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod6732.State.BEGAN) {
    if (state === _mod6732.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmpResult = DEFAULT_PROPS_TRANSFORMER;
      tmpResult.runCallback(_mod6747.CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === _mod6732.State.FAILED || state === _mod6732.State.CANCELLED;
    if (oldState === _mod6732.State.ACTIVE) {
      if (fn != null) {
        fn(obj);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6747.CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6732.State.FAILED || state === _mod6732.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6747.CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
...
let obj = {
  flattenAndFilterEvent: fn(6794).flattenAndFilterEvent,
  State: fn(6732).State,
  runCallback: fn(6794).runCallback,
  CALLBACK_TYPE: fn(6747).CALLBACK_TYPE,
};
...
obj = {
  TouchEventType: fn(6734).TouchEventType,
  runCallback: fn(6794).runCallback,
  touchEventTypeToCallbackType: fn(6794).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6747.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(6794).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(6794).isEventForHandlerWithTag,
  isStateChangeEvent: fn(6796).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(6796).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
