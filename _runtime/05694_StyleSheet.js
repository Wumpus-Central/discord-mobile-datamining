...
export default importAllResult.memo(function CardContainer(active) {
  ...
  let num = importAllResult.useContext(active(5645).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp4 = importDefault(5695);
  ({ onPageChangeStart: c9, onPageChangeCancel: c10, onPageChangeConfirm: c11 } = importDefault(5695)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  let str = "box-none";
  const tmp4Result = importDefault(5695)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (previousScene) {
    ...
    headerTitle = tmp(5645).getHeaderTitle(options2, route.name);
    tmp10 = headerTitle;
    const tmpResult = tmp(5645);
  }
  const items2 = [tmp10];
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj7[1] = tmp13(tmp(5645).HeaderHeightContext.Provider, obj8);
      obj6[1] = tmp13(tmp(5645).HeaderShownContext.Provider, obj7);
      obj5[1] = tmp13(tmp(5645).HeaderBackContext.Provider, obj6);
      ...
      obj4[1] = items5;
      obj3[1] = scene(tmp3(5671).Provider, obj4);
