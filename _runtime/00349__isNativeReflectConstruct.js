...
class ScrollView {
  constructor(arg0) {
    self = this;
    tmp = _isNativeReflectConstruct(this, f102251);
    items = [];
    items[0] = global;
    tmp2 = _isNativeReflectConstruct;
    obj = _isNativeReflectConstruct(f102251);
    ...
    tmp3Result.getScrollResponder = () => f102251;
    tmp3Result.getScrollableNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f102251.getNativeScrollRef());
    };
    tmp3Result.getInnerViewNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f102251._innerView.nativeInstance);
    };
    tmp3Result.getInnerViewRef = () => f102251._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f102251._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num) => {
      ...
      const nativeScrollRef = f102251.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (num, arg1, _preventNegativeScrollOffset) => {
      ...
      f102251._additionalScrollOffset = num;
      f102251._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f102251._innerView.nativeInstance) {
        if (typeof num === "number") {
    ...
    tmp3Result.scrollResponderZoomTo = (animated) => {
      ...
      if ("animated" in animated) {
        f102251._animated = animated.animated;
        delete tmp[tmp2];
    ...
    tmp3Result._handleScroll = (arg0) => {
      f102251._observedScrollSinceBecomingResponder = true;
      if (f102251.props.onScroll) {
    ...
    tmp3Result._handleLayout = (nativeEvent) => {
      let obj = f102251;
      if (true === f102251.props.invertStickyHeaders) {
    ...
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f102251.props.onContentSizeChange) {
        const props = f102251.props;
    ...
    f102251 = (arg0) => Object.assign(arg0, { getScrollResponder: f102251.getScrollResponder, getScrollableNode: f102251.getScrollableNode, getInnerViewNode: f102251.getInnerViewNode, getInnerViewRef: f102251.getInnerViewRef, getNativeScrollRef: f102251.getNativeScrollRef, scrollTo: f102251.scrollTo, scrollToEnd: f102251.scrollToEnd, flashScrollIndicators: f102251.flashScrollIndicators, scrollResponderZoomTo: f102251.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f102251.scrollResponderScrollNativeHandleToKeyboard });
    f102251 = undefined;
    obj1 = {
      getForwardingRef: require("areInputsEqual")((arg0) => {
            let closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f102251._keyboardMetrics = endCoordinates.endCoordinates;
      if (f102251.props.onKeyboardWillShow) {
        const props = f102251.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f102251._keyboardMetrics = null;
      if (f102251.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f102251._keyboardMetrics = endCoordinates.endCoordinates;
      if (f102251.props.onKeyboardDidShow) {
        const props = f102251.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f102251._keyboardMetrics = null;
      if (f102251.props.onKeyboardDidHide) {
        const props = tmp.props;
        props.onKeyboardDidHide(arg0);
      }
    };
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      const _performance = f102251.performance;
      f102251._lastMomentumScrollBeginTime = _performance.now();
      if (f102251.props.onMomentumScrollBegin) {
        const props = f102251.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = f102251.performance;
      f102251._lastMomentumScrollEndTime = _performance.now();
      if (f102251.props.onMomentumScrollEnd) {
        const props = f102251.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f102251.props.keyboardDismissMode) {
        outer1_2(outer1_4[15])();
      }
      if (f102251.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f102251._isAnimating();
      ...
      if (f102251.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = f102251.performance;
      return _performance.now() - f102251._lastMomentumScrollEndTime < 16 || f102251._lastMomentumScrollEndTime < f102251._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f102251._observedScrollSinceBecomingResponder = false;
      if (f102251.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f102251._becameResponderWhileAnimating = f102251._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f102251._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f102251.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f102251._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f102251.props.disableScrollViewPanResponder && f102251._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f102251.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f102251._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f102251._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f102251._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f102251.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
