...
class ScrollView {
  constructor(arg0) {
    self = this;
    tmp = closure_7(this, f103904);
    items = [];
    items[0] = global;
    tmp2 = closure_9;
    obj = closure_9(f103904);
    tmp3 = closure_8;
    ...
    tmp3Result.getScrollResponder = () => f103904;
    tmp3Result.getScrollableNode = () => {
      obj = obj(closure_1_4[8]);
      return obj.findNodeHandle(f103904.getNativeScrollRef());
    };
    tmp3Result.getInnerViewNode = () => {
      obj = obj(closure_1_4[8]);
      return obj.findNodeHandle(f103904._innerView.nativeInstance);
    };
    tmp3Result.getInnerViewRef = () => f103904._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f103904._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num) => {
      ...
      const nativeScrollRef = f103904.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (num, arg1, _preventNegativeScrollOffset) => {
      ...
      f103904._additionalScrollOffset = num;
      f103904._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f103904._innerView.nativeInstance) {
        if (typeof num === "number") {
    ...
    tmp3Result.scrollResponderZoomTo = (animated) => {
      ...
      if ("animated" in animated) {
        f103904._animated = animated.animated;
        delete tmp[tmp2];
    ...
    tmp3Result._handleScroll = (arg0) => {
      f103904._observedScrollSinceBecomingResponder = true;
      if (f103904.props.onScroll) {
    ...
    tmp3Result._handleLayout = (nativeEvent) => {
      obj = f103904;
      if (true === f103904.props.invertStickyHeaders) {
    ...
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f103904.props.onContentSizeChange) {
        const props = f103904.props;
    ...
    f103904 = (arg0) => Object.assign(arg0, { getScrollResponder: f103904.getScrollResponder, getScrollableNode: f103904.getScrollableNode, getInnerViewNode: f103904.getInnerViewNode, getInnerViewRef: f103904.getInnerViewRef, getNativeScrollRef: f103904.getNativeScrollRef, scrollTo: f103904.scrollTo, scrollToEnd: f103904.scrollToEnd, flashScrollIndicators: f103904.flashScrollIndicators, scrollResponderZoomTo: f103904.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f103904.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj1 = {
      getForwardingRef: require("areInputsEqual")((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f103904._keyboardMetrics = endCoordinates.endCoordinates;
      if (f103904.props.onKeyboardWillShow) {
        const props = f103904.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f103904._keyboardMetrics = null;
      if (f103904.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f103904._keyboardMetrics = endCoordinates.endCoordinates;
      if (f103904.props.onKeyboardDidShow) {
        const props = f103904.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f103904._keyboardMetrics = null;
      if (f103904.props.onKeyboardDidHide) {
        const props = tmp.props;
        props.onKeyboardDidHide(arg0);
      }
    };
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      const _performance = f103904.performance;
      f103904._lastMomentumScrollBeginTime = _performance.now();
      if (f103904.props.onMomentumScrollBegin) {
        const props = f103904.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = f103904.performance;
      f103904._lastMomentumScrollEndTime = _performance.now();
      if (f103904.props.onMomentumScrollEnd) {
        const props = f103904.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f103904.props.keyboardDismissMode) {
        closure_1_2(closure_1_4[15])();
      }
      if (f103904.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f103904._isAnimating();
      ...
      if (f103904.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = f103904.performance;
      return _performance.now() - f103904._lastMomentumScrollEndTime < 16 || f103904._lastMomentumScrollEndTime < f103904._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f103904._observedScrollSinceBecomingResponder = false;
      if (f103904.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f103904._becameResponderWhileAnimating = f103904._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f103904._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f103904.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f103904._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f103904.props.disableScrollViewPanResponder && f103904._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f103904.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f103904._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f103904._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f103904._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f103904.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
