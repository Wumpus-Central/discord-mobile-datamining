...
class ScrollView {
  constructor(arg0) {
    self = this;
    tmp = _isNativeReflectConstruct(this, f102647);
    items = [];
    items[0] = global;
    tmp2 = _isNativeReflectConstruct;
    obj = _isNativeReflectConstruct(f102647);
    ...
    tmp3Result.getScrollResponder = () => f102647;
    tmp3Result.getScrollableNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f102647.getNativeScrollRef());
    };
    tmp3Result.getInnerViewNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f102647._innerView.nativeInstance);
    };
    tmp3Result.getInnerViewRef = () => f102647._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f102647._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num) => {
      ...
      const nativeScrollRef = f102647.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (num, arg1, _preventNegativeScrollOffset) => {
      ...
      f102647._additionalScrollOffset = num;
      f102647._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f102647._innerView.nativeInstance) {
        if (typeof num === "number") {
    ...
    tmp3Result.scrollResponderZoomTo = (animated) => {
      ...
      if ("animated" in animated) {
        f102647._animated = animated.animated;
        delete tmp[tmp2];
    ...
    tmp3Result._handleScroll = (arg0) => {
      f102647._observedScrollSinceBecomingResponder = true;
      if (f102647.props.onScroll) {
    ...
    tmp3Result._handleLayout = (nativeEvent) => {
      let obj = f102647;
      if (true === f102647.props.invertStickyHeaders) {
    ...
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f102647.props.onContentSizeChange) {
        const props = f102647.props;
    ...
    f102647 = (arg0) => Object.assign(arg0, { getScrollResponder: f102647.getScrollResponder, getScrollableNode: f102647.getScrollableNode, getInnerViewNode: f102647.getInnerViewNode, getInnerViewRef: f102647.getInnerViewRef, getNativeScrollRef: f102647.getNativeScrollRef, scrollTo: f102647.scrollTo, scrollToEnd: f102647.scrollToEnd, flashScrollIndicators: f102647.flashScrollIndicators, scrollResponderZoomTo: f102647.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f102647.scrollResponderScrollNativeHandleToKeyboard });
    f102647 = undefined;
    obj1 = {
      getForwardingRef: require("areInputsEqual")((arg0) => {
            let closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f102647._keyboardMetrics = endCoordinates.endCoordinates;
      if (f102647.props.onKeyboardWillShow) {
        const props = f102647.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f102647._keyboardMetrics = null;
      if (f102647.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f102647._keyboardMetrics = endCoordinates.endCoordinates;
      if (f102647.props.onKeyboardDidShow) {
        const props = f102647.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f102647._keyboardMetrics = null;
      if (f102647.props.onKeyboardDidHide) {
        const props = tmp.props;
        props.onKeyboardDidHide(arg0);
      }
    };
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      const _performance = f102647.performance;
      f102647._lastMomentumScrollBeginTime = _performance.now();
      if (f102647.props.onMomentumScrollBegin) {
        const props = f102647.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = f102647.performance;
      f102647._lastMomentumScrollEndTime = _performance.now();
      if (f102647.props.onMomentumScrollEnd) {
        const props = f102647.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f102647.props.keyboardDismissMode) {
        outer1_2(outer1_4[15])();
      }
      if (f102647.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f102647._isAnimating();
      ...
      if (f102647.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = f102647.performance;
      return _performance.now() - f102647._lastMomentumScrollEndTime < 16 || f102647._lastMomentumScrollEndTime < f102647._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f102647._observedScrollSinceBecomingResponder = false;
      if (f102647.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f102647._becameResponderWhileAnimating = f102647._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f102647._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f102647.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f102647._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f102647.props.disableScrollViewPanResponder && f102647._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f102647.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f102647._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f102647._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f102647._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f102647.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
