...
class ScrollView {
  constructor(arg0) {
    self = this;
    tmp = _isNativeReflectConstruct(this, f101934);
    items = [];
    items[0] = global;
    tmp2 = _isNativeReflectConstruct;
    obj = _isNativeReflectConstruct(f101934);
    ...
    tmp3Result.getScrollResponder = () => f101934;
    tmp3Result.getScrollableNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f101934.getNativeScrollRef());
    };
    tmp3Result.getInnerViewNode = () => {
      obj = obj(outer1_4[8]);
      return obj.findNodeHandle(f101934._innerView.nativeInstance);
    };
    tmp3Result.getInnerViewRef = () => f101934._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f101934._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num) => {
      ...
      const nativeScrollRef = f101934.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (num, arg1, _preventNegativeScrollOffset) => {
      ...
      f101934._additionalScrollOffset = num;
      f101934._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f101934._innerView.nativeInstance) {
        if (typeof num === "number") {
    ...
    tmp3Result.scrollResponderZoomTo = (animated) => {
      ...
      if ("animated" in animated) {
        f101934._animated = animated.animated;
        delete tmp[tmp2];
    ...
    tmp3Result._handleScroll = (arg0) => {
      f101934._observedScrollSinceBecomingResponder = true;
      if (f101934.props.onScroll) {
    ...
    tmp3Result._handleLayout = (nativeEvent) => {
      let obj = f101934;
      if (true === f101934.props.invertStickyHeaders) {
    ...
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f101934.props.onContentSizeChange) {
        const props = f101934.props;
    ...
    f101934 = (arg0) => Object.assign(arg0, { getScrollResponder: f101934.getScrollResponder, getScrollableNode: f101934.getScrollableNode, getInnerViewNode: f101934.getInnerViewNode, getInnerViewRef: f101934.getInnerViewRef, getNativeScrollRef: f101934.getNativeScrollRef, scrollTo: f101934.scrollTo, scrollToEnd: f101934.scrollToEnd, flashScrollIndicators: f101934.flashScrollIndicators, scrollResponderZoomTo: f101934.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f101934.scrollResponderScrollNativeHandleToKeyboard });
    f101934 = undefined;
    obj1 = {
      getForwardingRef: require("areInputsEqual")((arg0) => {
            let closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f101934._keyboardMetrics = endCoordinates.endCoordinates;
      if (f101934.props.onKeyboardWillShow) {
        const props = f101934.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f101934._keyboardMetrics = null;
      if (f101934.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f101934._keyboardMetrics = endCoordinates.endCoordinates;
      if (f101934.props.onKeyboardDidShow) {
        const props = f101934.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f101934._keyboardMetrics = null;
      if (f101934.props.onKeyboardDidHide) {
        const props = tmp.props;
        props.onKeyboardDidHide(arg0);
      }
    };
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      const _performance = f101934.performance;
      f101934._lastMomentumScrollBeginTime = _performance.now();
      if (f101934.props.onMomentumScrollBegin) {
        const props = f101934.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = f101934.performance;
      f101934._lastMomentumScrollEndTime = _performance.now();
      if (f101934.props.onMomentumScrollEnd) {
        const props = f101934.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f101934.props.keyboardDismissMode) {
        outer1_2(outer1_4[15])();
      }
      if (f101934.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f101934._isAnimating();
      ...
      if (f101934.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = f101934.performance;
      return _performance.now() - f101934._lastMomentumScrollEndTime < 16 || f101934._lastMomentumScrollEndTime < f101934._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f101934._observedScrollSinceBecomingResponder = false;
      if (f101934.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f101934._becameResponderWhileAnimating = f101934._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f101934._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f101934.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f101934._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f101934.props.disableScrollViewPanResponder && f101934._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f101934.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f101934._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f101934._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f101934._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f101934.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
