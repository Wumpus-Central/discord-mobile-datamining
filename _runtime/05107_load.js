// _runtime/05107_load.js
import errorDefault from "05108_error.js";
  ...
  if (typeof response === "string") {
    ...
    if (obj1.test(response)) {
      ...
      if (obj2.test(response)) {
        let resolved = Promise.resolve(response(5109).dataUriToBuffer(response));
        const obj5 = response(5109);
      } else {
        obj2 = then;
        if (then === undefined) {
          obj2 = {};
...
function loadView(byteLength, then) {
  let obj = then;
  if (then === undefined) {
    obj = { expanded: false, async: false, includeUnknown: false, domParser: "id" };
  }
