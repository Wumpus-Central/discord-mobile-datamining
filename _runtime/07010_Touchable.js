...
export const Touchable = (hitSlop) => {
  ...
  if (undefined === animationDuration) {
    ...
    const obj2 = {
      tapAnimationInDuration: num12,
      tapAnimationOutDuration: num12,
      longPressAnimationOutDuration: num12,
      hoverAnimationInDuration: num12,
      hoverAnimationOutDuration: num12,
    };
    obj = obj2;
  } else {
    let num5 = 0;
  ...
  const tmp20 = ref2(() => {
    if (closure_9.current == null) {
      tmp.current = _mod6971.isKeyboardDismissingTap(closure_8);
    }
  }, items);
  constants = tmp20;
  const tmp21 = ref2(() => {
  ...
  const nativeGesture = onLongPress(onPressIn[5]).useNativeGesture({
    onBegin: tmp24,
    onActivate: tmp25,
    onFinalize: tmp26,
    onUpdate: tmp27,
