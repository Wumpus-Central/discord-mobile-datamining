...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f103021, result) {
  f103021.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f103021[_sentry_idleSpan]) {
    f103021.emit("afterStartPageLoadSpan", tmp3);
  }
  return f103021[_sentry_idleSpan];
};