...
export const browserTracingIntegration = () => {
  ...
  let closure_3 = { name: "r", source: "accessibilityRole" };
            ...
            on.on("startNavigationSpan", (arg0, isRedirect) => {
              ...
              if (obj.getClient() === closure_0) {
                ...
                if (isRedirect) {
                  ...
                  const result2 = currentScope.setSDKProcessingMetadata({ normalizedRequest: "title" });
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f103417, result) {
  f103417.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f103417[_sentry_idleSpan]) {
    f103417.emit("afterStartPageLoadSpan", tmp3);
  }
  return f103417[_sentry_idleSpan];
};