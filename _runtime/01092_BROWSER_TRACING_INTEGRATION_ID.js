...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f102336, result) {
  f102336.emit("startPageLoadSpan", result, arg2);
  const currentScope = require(817) /* registerSpanErrorInstrumentation */.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f102336[_sentry_idleSpan]) {
    f102336.emit("afterStartPageLoadSpan", tmp3);
  }
  return f102336[_sentry_idleSpan];
};