...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f102434, result) {
  f102434.emit("startPageLoadSpan", result, arg2);
  const currentScope = require(817) /* registerSpanErrorInstrumentation */.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f102434[_sentry_idleSpan]) {
    f102434.emit("afterStartPageLoadSpan", tmp3);
  }
  return f102434[_sentry_idleSpan];
};