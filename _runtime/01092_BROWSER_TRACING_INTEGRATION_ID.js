...
export const browserTracingIntegration = () => {
  ...
  let closure_3 = { name: "Array", source: "ct" };
            ...
            on.on("startNavigationSpan", (arg0, isRedirect) => {
              ...
              if (obj.getClient() === closure_0) {
                ...
                if (isRedirect) {
                  ...
                  const result2 = currentScope.setSDKProcessingMetadata({ normalizedRequest: "Promise" });
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f103056, result) {
  f103056.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f103056[_sentry_idleSpan]) {
    f103056.emit("afterStartPageLoadSpan", tmp3);
  }
  return f103056[_sentry_idleSpan];
};