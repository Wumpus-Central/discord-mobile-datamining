  ...
  tmp3Result = tmp3(() => {
    ...
    if (-1 !== currentStickyIndex) {
      ...
      if (currentStickyIndex < data.length) {
        obj = { index: currentStickyIndex, item: tmp6[currentStickyIndex], renderItem, layout: { x: 0, y: 0, width: 0, height: 0 }, refHolder: current, extraData, trailingItem: "accessible", target: "f8wNDl", hidden: "AVUnderlyingError", inverted };
        tmpResult = jsx(ViewHolder.ViewHolder, { index: currentStickyIndex, item: tmp6[currentStickyIndex], renderItem, layout: { x: 0, y: 0, width: 0, height: 0 }, refHolder: current, extraData, trailingItem: "accessible", target: "f8wNDl", hidden: "AVUnderlyingError", inverted });
      }
    }
