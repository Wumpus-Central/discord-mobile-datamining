...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = MetricsAggregator(7752).timestampInSeconds();
        const obj2 = MetricsAggregator(7752);
      }
      const self = this;
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = MetricsAggregator(7839).sanitizeMetricKey(arg1);
      const obj3 = MetricsAggregator(7839);
      const sanitizeTagsResult = MetricsAggregator(7839).sanitizeTags(obj);
      const obj4 = MetricsAggregator(7839);
      const sanitizeUnitResult = MetricsAggregator(7839).sanitizeUnit(str);
      const obj5 = MetricsAggregator(7839);
      const bucketKey = MetricsAggregator(7839).getBucketKey(arg0, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
      ...
      const obj6 = MetricsAggregator(7839);
      const result1 = MetricsAggregator(7743).updateMetricSummaryOnActiveSpan(arg0, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(7752).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(7835).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(7841).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(7841);
      }
    }
  }
];
