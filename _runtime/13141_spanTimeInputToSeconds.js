// _runtime/13141_spanTimeInputToSeconds.js
...
import _mod13148 from "metro/13148__.js";
...
import _mod13158 from "metro/13158__.js";
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod13153.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== _mod13153.SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(parentSpan, sentrySpan) {
  ...
  const result = _mod13142.addNonEnumerableProperty(sentrySpan, _sentryRootSpan, tmp2);
  if (parentSpan[_sentryChildSpans]) {
    ...
    const result1 = _mod13142.addNonEnumerableProperty(parentSpan, _sentryChildSpans, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const tmpResult = _mod13158;
    activeSpan = tmpResult._getSpanForScope(_mod13163.getCurrentScope());
...
export const updateMetricSummaryOnActiveSpan = function updateMetricSummaryOnActiveSpan(
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const tmpResult = _mod13158;
    activeSpan = tmpResult._getSpanForScope(_mod13163.getCurrentScope());
    const tmpResult3 = _mod13163;
  }
  if (activeSpan) {
    const tmpResult4 = _mod13152;
