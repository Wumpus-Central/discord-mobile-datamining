...
function checkNativeAndroidAvailable() {
  if (!RNIapModule) {
    if (!RNIapAmazonModule) {
      const _Error = Error;
      const error = new Error(require(7338) /* PurchaseError */.ErrorCode.E_IAP_NOT_AVAILABLE);
      throw error;
    }
  }
}
function getAndroidModule() {
  if (typeof checkNativeAndroidAvailable !== "error") {
...
export const getAndroidModuleType = () => {
  if (typeof getAndroidModule !== "error") {
    HermesBuiltin.throwTypeError();
  }
  if (typeof checkNativeAndroidAvailable !== "error") {
    HermesBuiltin.throwTypeError();
...
export const storekit2Mode = () => {
  ...
  if (typeof isStorekit2Available !== "error") {
    HermesBuiltin.throwTypeError();
  }
  let flag = !c6;
  if (c6) {
