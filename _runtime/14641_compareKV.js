// _runtime/14641_compareKV.js
import e from "01161_e.js";
import emitUnicodeLanguageId from "14640_emitUnicodeLanguageId.js";
import languageAlias from "14642_languageAlias.js";
import _mod14643 from "metro/14643__.js";
import likelySubtags from "14644_likelySubtags.js";
...
function canonicalizeUnicodeLanguageId(lang) {
  ...
  if (lang.variants.length) {
    ...
    if (0 < variants1.length) {
      ...
      const result = _mod14643.parseUnicodeLanguageId(str.split(_mod14643.SEPARATOR));
  ...
  if (tmp.script) {
    ...
    if (tmp.region) {
      ...
      if (str2) {
        ...
        const result1 = _mod14643.parseUnicodeLanguageId(str2.split(_mod14643.SEPARATOR));
  ...
  if (tmp9.region) {
    ...
    if (str3) {
      ...
      const result2 = _mod14643.parseUnicodeLanguageId(str3.split(_mod14643.SEPARATOR));
  ...
  const str4 =
    languageAlias.languageAlias[
      emitUnicodeLanguageId.emitUnicodeLanguageId(undefined, { lang: tmp13.lang, variants: [] })
    ];
  let tmp19 = tmp13;
  if (str4) {
    const obj19 = { lang: null, script: null, region: null, variants: null };
    const result3 = _mod14643.parseUnicodeLanguageId(str4.split(_mod14643.SEPARATOR));
  ...
  if (tmp19.region) {
    ...
    if (str6) {
      ...
      tmp22 = first;
      if (str8) {
        const region = _mod14643.parseUnicodeLanguageId(str8.split(_mod14643.SEPARATOR)).region;
        let tmp24 = region;
        if (region) {
  ...
  if (tmp19.script) {
    ...
    if (languageAlias.scriptAlias[tmp19.script]) {
      tmp19.script = languageAlias.scriptAlias[tmp19.script];
    }
  ...
  if (tmp19.variants.length) {
    for (let num7 = 0; num7 < tmp19.variants.length; num7 = num7 + 1) {
      ...
      if (languageAlias.variantAlias[formatted2]) {
        let tmp30 = languageAlias.variantAlias[formatted2];
        if (_mod14643.isUnicodeVariantSubtag(tmp30)) {
          tmp19.variants[num7] = tmp30;
        } else if (_mod14643.isUnicodeLanguageSubtag(tmp30)) {
          tmp19.lang = tmp30;
        }
      }
    }
