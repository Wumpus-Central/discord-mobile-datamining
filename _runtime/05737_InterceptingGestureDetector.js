// === Module 5737: InterceptingGestureDetector ===
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5738).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5738).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5738).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5738).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5674).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5733).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5675);
  }
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(5738).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(5738).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5736).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5674).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = closure_8(ReanimatedNativeDetector, obj1);
    return closure_8(tmp(5738).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    error = new Error(tmp(5665).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
