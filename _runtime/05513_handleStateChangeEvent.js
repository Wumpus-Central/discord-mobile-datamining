// _runtime/05513_handleStateChangeEvent.js
import { _isNativeReflectConstruct } from "05448__isNativeReflectConstruct.js";
import { isGestureEnabled } from "05495_isGestureEnabled.js";
import { 05433__ } from "metro/05433__.js";
import { 05435__ } from "metro/05435__.js";
const require = arg1;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === 05433__.State.UNDETERMINED) {
    if (state === tmp(5433).State.BEGAN) {
      let tmpResult = tmp(5495);
      tmpResult.runCallback(tmp(5448).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === 05433__.State.BEGAN) {
    if (state === tmp(5433).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5495);
      tmpResult.runCallback(tmp(5448).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5433).State.FAILED || state === tmp(5433).State.CANCELLED;
    if (oldState === tmp(5433).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5495).runCallback(tmp(5448).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5495);
    }
    const tmp7 = state === tmp(5433).State.FAILED || state === tmp(5433).State.CANCELLED;
    tmp(5495).runCallback(tmp(5448).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(5495);
  }
}
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("module_5433").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== 05435__.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5495);
    tmpResult = tmp(5495);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("module_5435").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5435).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5495);
          tmpResult1.runCallback(tmp(5495).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5495);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5495).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5495);
        tmp(5495).runCallback(tmp(5448).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(5495);
      }
    }
  }
}
