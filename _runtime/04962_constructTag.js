// _runtime/04962_constructTag.js
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod4922.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4915).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4915);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod4922.TYPE_ITXT) {
    stringFromDataView = module(4948).decode("UTF-8", decompressResult);
    const obj5 = module(4948);
  }
