// _runtime/06430__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(6431).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(6431);
      return ServerRuntimeClient(6395).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      return obj.resolvedSyncPromise(ServerRuntimeClient(6431).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(6398).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(6398);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(6424).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(6399).DEBUG_BUILD) {
        const logger = tmp2(6371).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(6432);
          ...
          if (ServerRuntimeClient(6399).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(6371).logger;
          ...
          if (ServerRuntimeClient(6399).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(6371).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(6382);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(6393)._getSpanForScope(arg0);
        if (_getSpanForScopeResult) {
          let tmpResult = tmp(6376);
        ...
        const tmpResult1 = ServerRuntimeClient(6407);
        if (_getSpanForScopeResult) {
