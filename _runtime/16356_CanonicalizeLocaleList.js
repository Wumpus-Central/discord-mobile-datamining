// _runtime/16356_CanonicalizeLocaleList.js
import { RegExpResult1 } from "16357_RegExpResult1.js";
import { createDateTimeFormat } from "16358_createDateTimeFormat.js";
let require = arg1;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch) {
                  let expVariantDupes = tmp5(16357).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(16357).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(16357).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f63781(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch) {
      const expVariantDupes = tmp(16357).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(16357).expSingletonDupes;
