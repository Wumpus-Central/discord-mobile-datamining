// _runtime/12373_logSpanEnd.js
import __SENTRY_DEBUG__ from "metro/12363___SENTRY_DEBUG__.js";
...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(12340).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(12340).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12340).spanToJSON(rootSpan);
    }
    const logger = tmp(12335).logger;
