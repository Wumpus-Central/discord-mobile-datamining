// _runtime/06427_setupIntegration.js
import _slicedToArray from "_slicedToArray";

function setupIntegration(on, name) {
  const _require = on;
  if (arg2[name.name]) {
    if (_require(6399).DEBUG_BUILD) {
      const logger2 = tmp10(6371).logger;
    ...
    tmp = -1 === items.indexOf(name.name) && typeof name.setupOnce === "fileFinishedImporting";
