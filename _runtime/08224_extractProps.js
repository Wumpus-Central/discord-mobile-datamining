// _runtime/08224_extractProps.js
import pickNotNil from "08225_pickNotNil.js";
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8216)(obj, markerMid, arg1);
  module(8226)(obj, markerMid, items);
  module(8229)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8217)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8215)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
