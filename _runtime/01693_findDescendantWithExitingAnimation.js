  ...
  if (isDummy instanceof globalThis.HTMLElement) {
    ...
    if (isDummy) {
      ...
      if (value) {
        ...
        tmp2(1692).setElementPosition(isDummy, value);
        ...
        const tmp2Result = tmp2(1692);
      } else {
        const logger = tmp2(1610).logger;
...
arg5.scheduleAnimationCleanup = function scheduleAnimationCleanup(animationName, arg1, arg2) {
  ...
  const timerId = setTimeout(() => {
    if (obj.isWindowAvailable()) {
      ...
      if (undefined === sum) {
        const reanimatedError = new animationName(1617).ReanimatedError("Failed to obtain animation index.");
        throw reanimatedError;
      } else {
        dependencyMap();
        const sheet = element.sheet;
        ...
        if (sum < outer1_5.length) {
          ...
          const reanimatedError1 = new animationName(1617).ReanimatedError("Failed to obtain animation index.");
