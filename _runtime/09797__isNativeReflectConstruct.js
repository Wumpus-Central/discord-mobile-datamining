// _runtime/09797__isNativeReflectConstruct.js
import _asyncLoop from "09791__asyncLoop.js";
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9792).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9791).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9792).exists(this);
      HashMD(9792).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9791).createView(content);
