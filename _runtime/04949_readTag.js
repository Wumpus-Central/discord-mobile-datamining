// _runtime/04949_readTag.js
import { importDefaultResult1 } from "04950_importDefaultResult1.js";
import { 04932__ } from "metro/04932__.js";
import { 04946__ } from "metro/04946__.js";
const require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  let obj = 04946__;
  const typeSize = obj.getTypeSize("SHORT");
  sum = typeSize + 04946__.getTypeSize("SHORT");
  const obj2 = 04946__;
  const sum1 = sum + 04946__.getTypeSize("LONG");
  const obj3 = 04946__;
  const shortAt = 04946__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 04946__;
  const shortAt1 = 04946__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 04946__;
  const longAt = 04946__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 04946__.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = callback(4946).typeSizes[shortAt1] * longAt;
      return result <= callback(4946).getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(4946).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + callback(4946).typeSizes[shortAt1] * longAt <= byteLength.byteLength;
    ...
    let tmp34 = str;
    if (shortAt1 === tmp4(4946).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(4950)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(4950)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(4950)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(4950)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(4950)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(4946).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(4946).tagTypes.SRATIONAL) {
          combined = tmp4(4950)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 04946__.typeSizes[shortAt1];
    BYTE = 04946__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 04946__.typeSizes[BYTE];
  }
  if (BYTE === 04946__.tagTypes.ASCII) {
    let asciiValue = 04946__.getAsciiValue(items);
    const tmp9Result = 04946__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: require("module_4946").getByteAt, 3: require("module_4946").getAsciiAt, 4: require("module_4946").getShortAt, 5: require("module_4946").getLongAt, 7: require("module_4946").getRationalAt, 9: null, 10: require("module_4946").getUndefinedAt, 13: null };
obj[9] = require("module_4946").getSlongAt;
obj[10] = require("module_4946").getSrationalAt;
obj[13] = require("module_4946").getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  let obj = 04946__;
  const typeSize = obj.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 04946__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 04946__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
  ...
  if (04932__.USE_THUMBNAIL) {
    let tmp23Result = tmp23(4946);
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      tmp23Result = tmp23(4946);
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 04946__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };