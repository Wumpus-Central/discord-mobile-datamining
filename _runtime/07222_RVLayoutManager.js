// _runtime/07222_RVLayoutManager.js
import _modDef7197 from "metro/07197__.js";
import _classCallCheck from "metro/07196__.js";
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7223).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7223);
      const findLastVisibleIndexResult = RVLayoutManager(7223).findLastVisibleIndex(
        this.layouts,
        arg1,
        this.horizontal,
      );
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new RVLayoutManager(7210).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = RVLayoutManager(7210).ConsecutiveNumbers.EMPTY;
      const obj2 = RVLayoutManager(7223);
    },
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(7184).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef7197(RVLayoutManager, items);
