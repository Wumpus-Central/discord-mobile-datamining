// _runtime/01802_o.js
import { t } from "01645_t.js";
import { call } from "01677_call.js";
import { isReanimated3 } from "01678_isReanimated3.js";
import { useSharedValue } from "01786_useSharedValue.js";
import { getVal } from "01803_getVal.js";
import { convertLrgbToOklab } from "01804_convertLrgbToOklab.js";
const fn = function o(arg0, arg1, h, useCorrectedHSVInterpolation) {
  ...
  if (undefined !== useCorrectedHSVInterpolation) {
    ...
    let tmpResult = tmp(1803);
    const interpolateResult1 = tmpResult.interpolate(arg0, arg1, h.s, tmp(1803).Extrapolation.CLAMP);
    tmpResult = tmp(1803);
    const interpolateResult2 = tmpResult.interpolate(arg0, arg1, h.v, tmp(1803).Extrapolation.CLAMP);
    const tmpResult1 = tmp(1803);
    const tmpResult2 = tmp(1677);
    return tmpResult2.hsvToColor(interpolateResult, interpolateResult1, interpolateResult2, tmpResult1.interpolate(arg0, arg1, h.a, tmp(1803).Extrapolation.CLAMP));
  }
  const items = [arg1[0]];
  h = h.h;
  const items1 = [h[0]];
...
const fn4 = function a(arg0, arg1, arg2, gamma) {
  ...
  const interpolateResult = num(1803).interpolate(arg0, arg1, tmp3, num(1803).Extrapolation.CLAMP);
  const obj = num(1803);
  const interpolateResult1 = num(1803).interpolate(arg0, arg1, tmp2, num(1803).Extrapolation.CLAMP);
  const obj2 = num(1803);
  const interpolateResult2 = num(1803).interpolate(arg0, arg1, mapped2, num(1803).Extrapolation.CLAMP);
  const obj3 = num(1803);
  const interpolateResult3 = num(1803).interpolate(arg0, arg1, arg2.a, num(1803).Extrapolation.CLAMP);
  if (1 === num) {
    let tmp6Result = tmp6(1677);
    let rgbaColorResult = tmp6Result.rgbaColor(interpolateResult, interpolateResult1, interpolateResult2, interpolateResult3);
  } else {
    tmp6Result = tmp6(1677);
...
const fn9 = function i(arg0) {
  const tmp = fn6(arg0, (arg0) => {
    const convert = callback2(1804).oklab.convert;
