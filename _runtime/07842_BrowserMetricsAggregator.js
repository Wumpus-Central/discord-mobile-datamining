...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = BrowserMetricsAggregator(7752).timestampInSeconds();
        const obj2 = BrowserMetricsAggregator(7752);
      }
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = BrowserMetricsAggregator(7839).sanitizeMetricKey(arg1);
      const obj3 = BrowserMetricsAggregator(7839);
      const sanitizeTagsResult = BrowserMetricsAggregator(7839).sanitizeTags(obj);
      const obj4 = BrowserMetricsAggregator(7839);
      const sanitizeUnitResult = BrowserMetricsAggregator(7839).sanitizeUnit(str);
      const obj5 = BrowserMetricsAggregator(7839);
      const bucketKey = BrowserMetricsAggregator(7839).getBucketKey(arg0, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
  ...
  },
  {
    ...
    value: function flush() {
      ...
      if (0 !== this._buckets.size) {
        ...
        const result = BrowserMetricsAggregator(7841).captureAggregateMetrics(self._client, arr);
        const _buckets2 = self._buckets;
        _buckets2.clear();
        const obj = BrowserMetricsAggregator(7841);
      }
    }
  },
  {
