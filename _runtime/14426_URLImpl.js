...
let items = [
  {
    ...
    set(arg0) {
      ...
      if (null === basicURLParseResult) {
        ...
        if (null !== query) {
          self._query._list = URLImpl(14430).parseUrlencoded(query);
          const tmpResult = URLImpl(14430);
        }
      }
  ...
  },
  {
    ...
    set(arg0) {
      const self = this;
      let obj = URLImpl(14427);
      if (!obj.cannotHaveAUsernamePasswordPort(this._url)) {
        if ("" === arg0) {
          self._url.port = null;
        } else {
          obj = { url: self._url, stateOverride: "port" };
          URLImpl(14427).basicURLParse(arg0, obj);
          const tmpResult = URLImpl(14427);
        }
      }
    },
