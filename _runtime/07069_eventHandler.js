// _runtime/07069_eventHandler.js
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6989.State.UNDETERMINED) {
    if (state === _mod6989.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6989.State.BEGAN) {
    if (state === _mod6989.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6989.State.FAILED || state === _mod6989.State.CANCELLED;
    if (oldState === _mod6989.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6989.State.FAILED || state === _mod6989.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(7051).flattenAndFilterEvent,
  State: fn(6989).State,
  runCallback: fn(7051).runCallback,
  CALLBACK_TYPE: fn(7004).CALLBACK_TYPE,
};
...
handleTouchEvent.__closure = {
  TouchEventType: fn(6991).TouchEventType,
  runCallback: fn(7051).runCallback,
  touchEventTypeToCallbackType: fn(7051).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(7051).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(7051).isEventForHandlerWithTag,
  isStateChangeEvent: fn(7053).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(7053).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
