// === Module 6145: InterceptingGestureDetector ===
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6086).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6086);
  ...
  if (prop) {
    let REANIMATED = tmp(6146).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6146).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6146).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6146).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6082).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6141).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6083);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6146.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (gesture != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6146.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6142).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6142);
    const gestureRelationsUpdater = tmp(6143).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6143);
    const detectorAttachmentGuard = tmp(6144).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6082).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6146).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6073).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
