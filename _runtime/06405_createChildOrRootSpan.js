// _runtime/06405_createChildOrRootSpan.js
...
function createChildOrRootSpan(forceTransaction) {
  ...
  let obj = require(6403) /* hasTracingEnabled */;
  if (obj.hasTracingEnabled()) {
    let tmpResult = tmp(6398);
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(6412).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(6404).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(6376).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(6376);
        const client = tmp(6398).getClient();
        if (client) {
        ...
        const tmpResult2 = tmp(6398);
        tmp(6376).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult3 = tmp(6376);
      }
      tmp(6409).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(6409);
      const result = tmp(6400).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(6407).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(6407);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(6376);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(6376).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(6376).spanIsSampled(parentSpan);
      tmp(6407).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(6407);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(6407).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(6407);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(6404).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  let tmp6;
  let tmp7;
  let obj = require(6398) /* getClient */;
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(6410).sampleSpan(options, obj);
    const tmpResult = tmp(6410);
  }
  ...
  obj1.attributes = { [require(6386).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(6412).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(6386).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _require(6389).getMainCarrier();
  const obj = _require(6389);
  const tmp = _require;
  const asyncContextStrategy = _require(6390).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(6398).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.startTime) {
      ...
      obj.startTimestamp = tmp3(6376).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj;
      const tmp3Result = tmp3(6376);
    }
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = tmp2(tmp3[1]).withScope((arg0) => {
            _null(6393)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(6398).withScope((setPropagationContext) => {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(6398).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [outer1_3]: true });
      return callback();
    });
    const tmpResult = tmp(6398);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(6398).withScope((arg0) => {
      _null(6393)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(6398);
