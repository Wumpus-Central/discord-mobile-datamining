const fn = function o(arg0, arg1, h, useCorrectedHSVInterpolation) {
  ...
  if (undefined !== useCorrectedHSVInterpolation) {
    ...
    tmpResult = tmp(1775);
    const interpolateResult2 = tmpResult.interpolate(arg0, arg1, h.v, tmp(1775).Extrapolation.CLAMP);
    const tmpResult1 = tmp(1775);
    const tmpResult2 = tmp(1649);
    return tmpResult2.hsvToColor(interpolateResult, interpolateResult1, interpolateResult2, tmpResult1.interpolate(arg0, arg1, h.a, tmp(1775).Extrapolation.CLAMP));
  }
  const items = [arg1[0]];
  h = h.h;
  const items1 = [h[0]];
...
const fn4 = function a(arg0, arg1, arg2, gamma) {
  ...
  const interpolateResult = num(1775).interpolate(arg0, arg1, tmp3, num(1775).Extrapolation.CLAMP);
  const obj = num(1775);
  const interpolateResult1 = num(1775).interpolate(arg0, arg1, tmp2, num(1775).Extrapolation.CLAMP);
  const obj2 = num(1775);
  const interpolateResult2 = num(1775).interpolate(arg0, arg1, mapped2, num(1775).Extrapolation.CLAMP);
  const obj3 = num(1775);
  const interpolateResult3 = num(1775).interpolate(arg0, arg1, arg2.a, num(1775).Extrapolation.CLAMP);
  if (1 === num) {
    let tmp6Result = tmp6(1649);
...
const fn6 = function l(arg0, arg1) {
  ...
  for (let num = 0; num < arg0.length; num = num + 1) {
    ...
    let tmp4 = num;
    if (typeof processColorResult !== "V") {
...
const fn9 = function i(arg0) {
  const tmp = fn6(arg0, (arg0) => {
    const convert = callback2(1776).oklab.convert;
