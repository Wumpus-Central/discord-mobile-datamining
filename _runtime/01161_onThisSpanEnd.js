...
export const ignoreEmptyBackNavigation = (on, c4) => {
  let closure_0 = c4;
  const f68548 = (arg0) => {
    const data = c4(f68548[2]).spanToJSON(arg0).data;
    let prop;
    if (null !== data) {
      if (undefined !== data) {
        ...
        if (tmpResult.isSentrySpan(c4)) {
          on.on("spanEnd", (arg0) => {
            if (arg0 === c4) {
              if (f68550(tmp)) {
                c4 = tmp;
                const spanDescendants = c4(f68550[2]).getSpanDescendants(tmp);
...
export const ignoreEmptyRouteChangeTransactions = (on, c4) => {
  ...
  closure_0 = c4;
  f68550 = (arg0) => {
    const spanToJSONResult = c4(f68550[2]).spanToJSON(arg0);
    let tmp2 = spanToJSONResult.description === f68550;
    if (tmp2) {
  ...
  f68551 = (arg0) => {
    const debug = c4(f68550[2]).debug;
    debug.log("Discarding empty \"" + f68550 + "\" transaction that never received route information.");
    if (null != c4) {
