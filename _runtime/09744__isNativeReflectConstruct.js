// _runtime/09744__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9739).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9738).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9739).exists(this);
      HashMD(9739).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9738).createView(content);
