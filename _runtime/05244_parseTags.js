    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          let tmp15 = obj;
          let tmp16 = encoding;
          obj[3] = getTagDescription(tmp8(5245).iptc[uint16], items, obj, encoding);
...
function getTagDescription(description, items, arg2, encoding) {
  if (
    ...
    if (
      ...
      decodeResult = module(5246).decode(encoding, items);
      const obj = module(5246);
    }
    return decodeResult;
  } else {
    try {
