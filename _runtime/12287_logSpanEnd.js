// _runtime/12287_logSpanEnd.js
import __SENTRY_DEBUG__ from "metro/12277___SENTRY_DEBUG__.js";
...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(12254).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(12254).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12254).spanToJSON(rootSpan);
    }
    const logger = tmp(12249).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult1 = tmp(12254);
  }
};