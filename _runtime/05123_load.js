// _runtime/05123_load.js
import errorDefault from "05124_error.js";
  ...
  if (typeof response === "string") {
    ...
    if (obj1.test(response)) {
      ...
      if (obj2.test(response)) {
        let resolved = Promise.resolve(response(5125).dataUriToBuffer(response));
        const obj5 = response(5125);
      } else {
        obj2 = then;
        if (then === undefined) {
          obj2 = {};
...
function loadView(byteLength, then) {
  let obj = then;
  if (then === undefined) {
    obj = { expanded: false, async: false, includeUnknown: false, domParser: "bottom" };
  }
