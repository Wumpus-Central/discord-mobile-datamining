// _runtime/01636_overrideLogFunctionImplementation.js
import isJest from "isJest";
import isJest from "isJest";
import isJest from "isJest";
import addLogBoxLog from "addLogBoxLog";
import addLogBoxLog from "addLogBoxLog";
import { isJest } from "01637_isJest.js";
import { addLogBoxLog } from "01638_addLogBoxLog.js";
import { runWorkletOnJS } from "01641_runWorkletOnJS.js";
...
function overrideLogFunctionImplementation() {
  ...
  const fn = function e(arg0) {
    callback(1641).runOnJS(callback(1638).logToLogBoxAndConsole)(arg0);
  };
...
function setupCallGuard() {
  ...
  global.__ErrorUtils = {
    reportFatalError(message) {
      ...
      obj.runOnJS(arr(1645).reportFatalErrorOnJS)(obj);
    }
  };
}
  ...
  if (!obj.isWeb()) {
    if (ReanimatedModule) {
      if (isJest) {
        ...
        globalThis.requestAnimationFrame = tmp(1649).mockedRequestAnimationFrame;
      }
      const fn = function o() {
