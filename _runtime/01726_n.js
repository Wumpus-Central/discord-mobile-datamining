...
let fn = function n(config, _animationToClamp) {
  ...
  const fn = function u() {
    ...
    if (undefined !== config.max) {
      strippedValue1 = config(1693).recognizePrefixSuffix(tmp3.max).strippedValue;
      const obj2 = config(1693);
    }
    obj = {
      isHigherOrder: true,
      ...
      reduceMotion: config(1693).getReduceMotionForAnimation(config.reduceMotion),
    };
    return obj;
  };
  obj = {
    _animationToClamp,
    config,
    recognizePrefixSuffix: require("01693_isValidLayoutAnimationProp.js").recognizePrefixSuffix,
    logger: require("01657_addLogBoxLog.js").logger,
...
fn.__closure = {
  ...
  recognizePrefixSuffix: require("isValidLayoutAnimationProp").recognizePrefixSuffix,
