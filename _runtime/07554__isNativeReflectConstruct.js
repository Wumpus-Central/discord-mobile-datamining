// _runtime/07554__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7555).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7555);
      return ServerRuntimeClient(7519).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7519);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7555).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7522).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7522);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7548).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7523).DEBUG_BUILD) {
        const logger = tmp2(7495).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7556);
          ...
          if (ServerRuntimeClient(7523).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7495).logger;
          ...
          if (ServerRuntimeClient(7523).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7495).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7506);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7517)._getSpanForScope(arg0);
        if (_getSpanForScopeResult) {
          let tmpResult = tmp(7500);
        ...
        const tmpResult1 = ServerRuntimeClient(7531);
        if (_getSpanForScopeResult) {
