// _runtime/13186_applyScopeDataToEvent.js
...
export const applyScopeDataToEvent = function applyScopeDataToEvent(extra, arg1) {
  ...
  const dropUndefinedKeysResult = _mod13142.dropUndefinedKeys(extra);
  ...
  const dropUndefinedKeysResult1 = _mod13142.dropUndefinedKeys(tags);
  ...
  const dropUndefinedKeysResult2 = _mod13142.dropUndefinedKeys(user);
  ...
  const tmp3Result7 = _mod13142;
  const dropUndefinedKeysResult3 = _mod13142.dropUndefinedKeys(contexts);
  ...
  if (span) {
    ...
    obj7.dynamicSamplingContext = _mod13172.getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj7;
    const tmp3Result10 = _mod13172;
...
export const mergeScopeData = function mergeScopeData(extra, arg1) {
  ...
  extra.extra = _mod13161.merge(extra.extra, extra, 1);
  extra.tags = _mod13161.merge(extra.tags, tags, 1);
  extra.user = _mod13161.merge(extra.user, user, 1);
  extra.contexts = _mod13161.merge(extra.contexts, contexts, 1);
  extra.sdkProcessingMetadata = _mod13161.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
