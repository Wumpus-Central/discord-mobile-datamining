...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(816).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      const tmpResult2 = tmp(816);
      ({ op: op2, description: description2 } = tmp(816).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(816).spanToJSON(rootSpan);
    }
    const debug = tmp(821).debug;
