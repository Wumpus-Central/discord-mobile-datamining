...
function parseTags(buffer) {
  ...
  if (buffer.byteLength !== buffer.getUint32()) {
    ...
    const keys = Object.keys(require(4939).iccProfile);
    for (let num = 0; num < keys.length; num = num + 1) {
      let tmp = keys[num];
      let tmp2 = require;
      let tmp3 = dependencyMap;
      let iter = require(4939).iccProfile[tmp];
      ...
      if (buffer.length < 132) {
        ...
        if (0 < uint32) {
          while (buffer.length >= num6 + c8) {
            ...
            let obj17 = require(4897) /* getDataView */;
            ...
            if (uint321 > buffer.length) {
              ...
              if (stringFromDataView1 === desc) {
                ...
                if (uint323 > uint322) {
                  ...
                  if (tmp55(4939).iccTags[stringFromDataView]) {
                    obj = { value: null, description: null };
                    obj[0] = applyResult;
                    obj[1] = applyResult;
                    obj[tmp55(4939).iccTags[stringFromDataView].name] = obj;
                  } else {
                ...
                if (stringFromDataView1 === mluc) {
                  ...
                  if (1 === uint324) {
                    ...
                    if (tmp25(4939).iccTags[stringFromDataView]) {
                      let obj3 = { value: null, description: null };
                      obj3[0] = text;
                      obj3[1] = text;
                      obj[tmp25(4939).iccTags[stringFromDataView].name] = obj3;
                    } else {
                    ...
                    if (require(4939).iccTags[stringFromDataView]) {
                      obj6 = { value: null, description: null };
                      obj6[0] = obj5;
                      obj6[1] = obj5;
                      obj[tmp33(4939).iccTags[stringFromDataView].name] = obj6;
                    } else {
                  ...
                  if (stringFromDataView1 === text) {
                    ...
                    if (tmp55(4939).iccTags[stringFromDataView]) {
                      ...
                      obj8[0] = applyResult1;
                      obj8[1] = applyResult1;
                      obj[tmp55(4939).iccTags[stringFromDataView].name] = obj8;
                    } else {
                    ...
                    if (stringFromDataView1 === c7) {
                      ...
                      if (tmp55(4939).iccTags[stringFromDataView]) {
                        ...
                        obj[tmp55(4939).iccTags[stringFromDataView].name] = obj10;
                      } else {
...
arg5.default = {
  read(buffer, arr) {
    if (arg2) {
      if (arr[0].compressionMethod !== require(4897) /* getDataView */.COMPRESSION_METHOD_NONE) {
