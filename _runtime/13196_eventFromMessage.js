// _runtime/13196_eventFromMessage.js
    ...
    if (event_id) {
      if (event_id.syntheticException) {
        ...
        if (arr.length) {
          ...
          obj2.values = items;
          obj.exception = obj2;
          const result = _mod13147.addExceptionMechanism(obj, { synthetic: true });
        }
...
export const eventFromUnknownInput = function eventFromUnknownInput(getOptions, fn, name, data) {
  ...
  if (obj2.isError(name)) {
    ...
    if (tmp2Result.isPlainObject(name)) {
      ...
      const obj = { __serialized__: _mod13181.normalizeToSize(name, normalizeDepth) };
      let tmp12;
      ...
      if (tmp12) {
        ...
        const result = _mod13142.extractExceptionKeysForMessage(name);
  ...
  const result1 = _mod13147.addExceptionTypeValue(obj5, undefined, undefined);
  const result2 = _mod13147.addExceptionMechanism(obj5, data);
  const obj9 = {};
