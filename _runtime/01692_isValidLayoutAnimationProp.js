// === Module 1692: isValidLayoutAnimationProp ===

// Module 1692 (isValidLayoutAnimationProp)
import runWorkletOnJS from "runWorkletOnJS" /* 1659 */;
import t from "t" /* 1663 */;
import isWorkletFunction from "isWorkletFunction" /* 1677 */;
import isReducedMotionEnabledInSystem from "isReducedMotionEnabledInSystem" /* 1693 */;
import isAffineMatrixFlat from "isAffineMatrixFlat" /* 1694 */;
    ...
    if (!isJest) {
      ...
      if (!factory) {
        if (!obj.isWorkletFunction(factory)) {
          const reanimatedError = new tmp5(1663).ReanimatedError("The easing function is not a worklet. Please make sure you import `Easing` from react-native-reanimated.");
  ...
  if (isHigherOrder.isHigherOrder) {
    ...
    function transformationMatrixOnFrame(arg0, arg1) {
      ...
      const item = items1.forEach((arg0, arg1) => {
        ...
        const obj2 = lib(1694);
        return items.push(obj.addMatrices(lib.startMatrices[arg0], obj2.scaleMatrix(lib(1694).subtractMatrices(lib.stopMatrices[arg0], lib.startMatrices[arg0]), tmp2)));
      });
      const items2 = [];
      const items3 = ["x", "y", "z"];
      ...
      arg0.current = obj4.flatten(obj5.multiplyMatrices(obj6.multiplyMatrices(tmp4, callback(1694).multiplyMatrices(tmp5, multiplyMatricesResult)), tmp3));
