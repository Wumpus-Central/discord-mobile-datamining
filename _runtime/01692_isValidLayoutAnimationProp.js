// _runtime/01692_isValidLayoutAnimationProp.js
import runWorkletOnJS from "01659_runWorkletOnJS.js";
import t from "01663_t.js";
import isWorkletFunction from "01677_isWorkletFunction.js";
import isReducedMotionEnabledInSystem from "01693_isReducedMotionEnabledInSystem.js";
import isAffineMatrixFlat from "01694_isAffineMatrixFlat.js";
import call from "01695_call.js";
    ...
    if (!isJest) {
      ...
      if (!factory) {
        if (!obj.isWorkletFunction(factory)) {
          const reanimatedError = new tmp5(1663).ReanimatedError(
...
applyProgressToMatrix.__closure = {
  addMatrices: isAffineMatrixFlat.addMatrices,
  scaleMatrix: isAffineMatrixFlat.scaleMatrix,
  subtractMatrices: isAffineMatrixFlat.subtractMatrices,
};
  ...
  if (isHigherOrder.isHigherOrder) {
    ...
    function transformationMatrixOnFrame(arg0, arg1) {
      ...
      const item = items1.forEach((arg0, arg1) => {
        ...
        return items.push(
          obj.addMatrices(
            lib.startMatrices[arg0],
            obj2.scaleMatrix(lib(1694).subtractMatrices(lib.stopMatrices[arg0], lib.startMatrices[arg0]), tmp2),
          ),
        );
      });
      const items2 = [];
      ...
      const item1 = items3.forEach((arg0, arg1) => {
        ...
        items2.push(lib(1694).getRotationMatrix(sum, arg0));
      });
      [tmp7, tmp8, tmp9] = items2;
      const onFrameResult = first.onFrame(arg0[0], arg1);
      let obj2 = callback(1694);
      const obj3 = callback(1694);
      const multiplyMatricesResult = obj2.multiplyMatrices(tmp7, callback(1694).multiplyMatrices(tmp8, tmp9));
      const obj4 = callback(1694);
      const obj5 = callback(1694);
      const obj6 = callback(1694);
      arg0.current = obj4.flatten(
        obj5.multiplyMatrices(
          obj6.multiplyMatrices(tmp4, callback(1694).multiplyMatrices(tmp5, multiplyMatricesResult)),
          tmp3,
        ),
      );
...
decorateAnimation.__closure = {
  ...
  isColor: call.isColor,
  toLinearSpace: call.toLinearSpace,
  convertToRGBA: call.convertToRGBA,
  clampRGBA: call.clampRGBA,
  rgbaArrayToRGBAColor: call.rgbaArrayToRGBAColor,
  toGammaSpace: call.toGammaSpace,
  decomposeMatrixIntoMatricesAndAngles: isAffineMatrixFlat.decomposeMatrixIntoMatricesAndAngles,
  applyProgressToMatrix,
  applyProgressToNumber,
  getRotationMatrix: isAffineMatrixFlat.getRotationMatrix,
  multiplyMatrices: isAffineMatrixFlat.multiplyMatrices,
  flatten: isAffineMatrixFlat.flatten,
  isAffineMatrixFlat: isAffineMatrixFlat.isAffineMatrixFlat,
};
