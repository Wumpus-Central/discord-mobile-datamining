// _runtime/06494_handleStateChangeEvent.js
import _mod6414 from "metro/06414__.js";
import _mod6416 from "metro/06416__.js";
import _isNativeReflectConstruct from "06429__isNativeReflectConstruct.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod6414.State.UNDETERMINED) {
    if (state === tmp(6414).State.BEGAN) {
      let tmpResult = tmp(6476);
      tmpResult.runCallback(tmp(6429).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod6414.State.BEGAN) {
    if (state === tmp(6414).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(6476);
      tmpResult.runCallback(tmp(6429).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(6414).State.FAILED || state === tmp(6414).State.CANCELLED;
    if (oldState === tmp(6414).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(6476).runCallback(tmp(6429).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(6476);
    }
    const tmp7 = state === tmp(6414).State.FAILED || state === tmp(6414).State.CANCELLED;
    tmp(6476).runCallback(tmp(6429).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(6476);
  }
}
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("metro/06414__.js").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod6416.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(6476);
    tmpResult = tmp(6476);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("metro/06416__.js").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(6416).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(6476);
          tmpResult1.runCallback(tmp(6476).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(6476);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(6476).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(6476);
        tmp(6476).runCallback(tmp(6429).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(6476);
      }
    }
  }
}
