...
if (!require("all")) {
  if (!require("__core-js_shared__").state) {
    ...
    let closure_6 = tmp;
    require("module_13625")[tmp] = true;
    let fn = function t(facade) {
      if (call(facade, closure_6)) {
        const typeError = new tmp(13580).TypeError("Object already initialized");
        throw typeError;
      } else {
        arg1.facade = facade;
        tmp(13636)(facade, tmp3, arg1);
        return arg1;
      }
      tmp3 = closure_6;
    };
...
if (!state) {
  ...
  const weakMap = new require("module_13580").WeakMap();
...
fn4 = function t(facade) {
  if (state.has(facade)) {
    const typeError = new 13580__.TypeError("Object already initialized");
    throw typeError;
  } else {
    arg1.facade = facade;
