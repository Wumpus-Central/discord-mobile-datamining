// _runtime/05544_isSupportedGesture.js
import _slicedToArray from "_slicedToArray";
import noop from "noop";
import { isGestureEnabled } from "05556_isGestureEnabled.js";
import { context } from "05560_context.js";
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5547).SingleGestureName.Tap !== type) {
      if (tmp(5547).SingleGestureName.LongPress !== type) {
        if (tmp(5547).SingleGestureName.Fling !== type) {
          if (tmp(5547).SingleGestureName.Native !== type) {
            if (tmp(5547).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
