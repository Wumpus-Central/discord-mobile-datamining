// _runtime/05602_onGestureHandlerEvent.js
import get_ActivityIndicator from "00017_get_ActivityIndicator.js";
import map2 from "05603_map.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5605).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5605).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5605).State.BEGAN) {
        if (handlerTag.state === tmp(5605).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5605).State.END) {
          if (handlerTag.oldState === tmp(5605).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5605).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5605).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5606).GestureStateManager;
      ...
      if (tmp(5607).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5607).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5607).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5607).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5603).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
