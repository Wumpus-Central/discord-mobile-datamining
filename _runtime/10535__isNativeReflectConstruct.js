// _runtime/10535__isNativeReflectConstruct.js
...
import parseYear from "10531_parseYear.js";
...
const regExp = new RegExp(
  "([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(parseYear.MONTH_DICTIONARY) +
    ")(?:\\s*(?:de|,)?\\s*(" +
    parseYear.YEAR_PATTERN +
    "))?(?=\\W|$)",
  "i",
);
...
const items = [
  ...
  },
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = PTMonthNameLittleEndianParser(10531).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10531).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply("year", tmp2(10451).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4));
        }
        if (index[2]) {
