...
export function createNativeFlagGetter(cdpInteractionMetricsEnabled, arg1) {
  const f69946 = () => {
    ...
    if (!hasItem) {
      ...
      console.error("Could not access feature flag '" + f69946 + "' because native module method was not available");
    }
    const tmp2Result = NativeReactNativeFeatureFlagsCxxDefault;
    let tmp13Result;
    if (tmp2Result != null) {
      if (tmp2Result[f69946] != null) {
        tmp13Result = tmp13();
      }
    }
    return tmp13Result;
