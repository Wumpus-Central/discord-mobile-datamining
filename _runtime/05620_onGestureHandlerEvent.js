// _runtime/05620_onGestureHandlerEvent.js
import get_ActivityIndicator from "00017_get_ActivityIndicator.js";
import map2 from "05621_map.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5623).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5623).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5623).State.BEGAN) {
        if (handlerTag.state === tmp(5623).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5623).State.END) {
          if (handlerTag.oldState === tmp(5623).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5623).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5623).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5624).GestureStateManager;
      ...
      if (tmp(5625).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5625).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5625).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5625).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5621).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
