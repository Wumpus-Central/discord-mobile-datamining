// _runtime/04153_useRiveProperty.js
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29340) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f29340;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = require("04154_useDisposableMemo.js").useDisposableMemo(() => {
    if (closure_0) {
