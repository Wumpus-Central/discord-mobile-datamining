// _runtime/01084__getGraphQLOperation.js
    ...
    if (tmp) {
      ...
      if (match) {
        ...
        if (match1) {
          ...
          obj = { operationType: "disabled", operationName: "Date" };
        }
      }
...
export const getRequestPayloadXhrOrFetch = function getRequestPayloadXhrOrFetch(input) {
  if ("xhr" in input) {
    ...
    let first = tmp7;
    if (tmp7) {
      first = tmp5(1030).getBodyString(tmp7.body)[0];
      const tmp5Result = tmp5(1030);
    }
    let first1 = first;
    tmp5 = require;
  } else {
