// _runtime/10914_CarouselLayout.js
import cancelAnimation from "01637_cancelAnimation.js";
import SINGLE_ITEM from "10906_SINGLE_ITEM.js";
...
export const CarouselLayout = noop.forwardRef((arg0, ref) => {
  ...
  const onProgressChange1 = loop(autoFillData[7]).useOnProgressChange({
    autoFillData,
    loop,
    size,
  ...
  const callback = dataLength.useCallback(() => {
    ...
    const result = SINGLE_ITEM.computedRealIndexWithAutoFillData({
      index: rounded,
  ...
  const autoPlay1 = loop(autoFillData[10]).useAutoPlay({
    autoPlay,
    autoPlayInterval,
    autoPlayReverse,
    carouselController,
  ...
  const obj12 = {
    ...
    children: rawDataLength(tmp(tmp2[13]).ItemRenderer, {
      data,
      dataLength,
      rawDataLength,
      loop,
