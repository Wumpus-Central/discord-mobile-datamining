// _runtime/07114_RVLayoutManager.js
import _createClassDefault from "metro/07089__createClass.js";
import closure_2 from "metro/07088__classCallCheck.js";
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7115).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7115);
      const findLastVisibleIndexResult = RVLayoutManager(7115).findLastVisibleIndex(this.layouts, arg1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new tmp(7102).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = tmp(7102).ConsecutiveNumbers.EMPTY;
    }
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        error = new Error(RVLayoutManager(7076).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
