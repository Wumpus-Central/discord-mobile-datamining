// _runtime/01584__toPropertyKey.js
...
export const useDescriptors = function useDescriptors(state) {
  ...
  closure_11 = React.useContext(state(1535).ThemeContext);
  [c12, tmp2] = callback(React.useState({}), 2);
  c13 = tmp2;
  const context = React.useContext(state(1528).NavigationBuilderContext);
  ...
  const routeCache = state(1550).useRouteCache(state.routes);
