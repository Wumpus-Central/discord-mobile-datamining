...
export default function SearchBar(ref) {
  ...
  const imperativeHandle = noop.useImperativeHandle(ref.ref, () => ({
    ...
    toggleCancelButton(arg0) {
      closure_0 = arg0;
      closure_1_1((arg0) => {
        const Commands = ref1(5149).Commands;
  ...
  if (ref1(5141).isSearchBarAvailableForCurrentPlatform) {
    ...
    obj.obscureBackground = tmp3(5141).parseBooleanToOptionalBooleanNativeProp(obscureBackground);
    const tmp3Result = tmp3(5141);
    obj.hideNavigationBar = tmp3(5141).parseBooleanToOptionalBooleanNativeProp(hideNavigationBar);
