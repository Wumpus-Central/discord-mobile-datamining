// _runtime/07220_RVLayoutManager.js
...
let items = [
  ...
  },
  {
    ...
    value: function getVisibleLayouts(bound1, arg1) {
      const result = RVLayoutManager(7221).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(7221);
      const findLastVisibleIndexResult = RVLayoutManager(7221).findLastVisibleIndex(
        this.layouts,
        arg1,
        this.horizontal,
      );
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          let EMPTY = new RVLayoutManager(7208).ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = RVLayoutManager(7208).ConsecutiveNumbers.EMPTY;
      const obj2 = RVLayoutManager(7221);
    },
  },
  {
    key: "deleteLayout",
  ...
  },
  {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(7182).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
...
export const RVLayoutManager = _modDef7195(RVLayoutManager, items);
