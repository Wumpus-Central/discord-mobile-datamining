...
import parseUnicodeLanguageId from "parseUnicodeLanguageId" /* 13644 */;
...
function canonicalizeUnicodeLanguageId(lang) {
  ...
  if (lang.variants.length) {
    ...
    if (0 < variants.length) {
      ...
      const result = parseUnicodeLanguageId.parseUnicodeLanguageId(str.split(parseUnicodeLanguageId.SEPARATOR));
      obj[0] = result.lang;
  ...
  if (tmp.script) {
    ...
    if (tmp.region) {
      ...
      if (str2) {
        ...
        const result1 = parseUnicodeLanguageId.parseUnicodeLanguageId(str2.split(parseUnicodeLanguageId.SEPARATOR));
  ...
  if (tmp9.region) {
    ...
    if (str3) {
      ...
      const result2 = parseUnicodeLanguageId.parseUnicodeLanguageId(str3.split(parseUnicodeLanguageId.SEPARATOR));
      obj3[0] = result2.lang;
  ...
  if (str4) {
    ...
    const result3 = parseUnicodeLanguageId.parseUnicodeLanguageId(str4.split(parseUnicodeLanguageId.SEPARATOR));
    obj5[0] = result3.lang;
  ...
  if (tmp19.region) {
    ...
    const str6 = _mod13643.territoryAlias[formatted];
    let tmp22;
    if (str6) {
      ...
      obj6[2] = [];
      const str8 = __exportStarResult2.likelySubtags[emitUnicodeLanguageId.emitUnicodeLanguageId(undefined, obj6)];
      tmp22 = first;
      if (str8) {
        const region = parseUnicodeLanguageId.parseUnicodeLanguageId(str8.split(parseUnicodeLanguageId.SEPARATOR)).region;
        let tmp24 = region;
        if (region) {
  ...
  if (tmp19.script) {
    ...
    if (_mod13643.scriptAlias[tmp19.script]) {
      tmp19.script = _mod13643.scriptAlias[tmp19.script];
    }
  ...
  if (tmp19.variants.length) {
    for (let num7 = 0; num7 < tmp19.variants.length; num7 = num7 + 1) {
      ...
      if (_mod13643.variantAlias[formatted2]) {
        let tmp30 = _mod13643.variantAlias[formatted2];
        if (parseUnicodeLanguageId.isUnicodeVariantSubtag(tmp30)) {
          tmp19.variants[num7] = tmp30;
        } else if (parseUnicodeLanguageId.isUnicodeLanguageSubtag(tmp30)) {
          tmp19.lang = tmp30;
        }
      }
    }
    ...
    for (let num3 = 0; num3 < extensions.length; num3 = num3 + 1) {
      ...
      if ("u" === type) {
        ...
        if (iter.attributes) {
          ...
          let _Object = Object;
          let keys = Object.keys(attributes.reduce((acc, item, index) => {
