// _runtime/05450_get0thIfdOffset.js
import _modDef5433 from "metro/05433__.js";
import _modDef5447 from "metro/05447__.js";
import IFD_TYPE_0TH2 from "05451_IFD_TYPE_0TH.js";

const IFD_TYPE_0THDefault = IFD_TYPE_0TH2;

require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5447.getTypeSize("SHORT");
  sum = typeSize + _modDef5447.getTypeSize("SHORT");
  const sum1 = sum + _modDef5447.getTypeSize("LONG");
  const shortAt = _modDef5447.getShortAt(byteLength, sum, byteOrder);
  const shortAt1 = _modDef5447.getShortAt(byteLength, sum + typeSize, byteOrder);
  const longAt = _modDef5447.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5447.typeSizes[shortAt1]) {
    if (
      (function tagValueFitsInOffsetSlot(shortAt1, longAt) {
        const result = _modDef5447.typeSizes[shortAt1] * longAt;
        return result <= _modDef5447.getTypeSize("LONG");
      })(shortAt1, longAt)
    ) {
      const sum2 = sum + sum1;
      ...
      const longAt1 = _modDef5447.getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if (
        (function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
          sum = sum + longAt1;
          return sum + _modDef5447.typeSizes[shortAt1] * longAt <= byteLength.byteLength;
        })(byteLength, sum, longAt1, shortAt1, longAt)
      ) {
        str = getTagValue(byteLength, sum + longAt1, shortAt1, longAt, byteOrder, 33723 === shortAt);
        tmp21 = longAt1;
      }
      const tmp4Result = _modDef5447;
    }
    let tmp34 = str;
    if (shortAt1 === _modDef5447.tagTypes.ASCII) {
    ...
    if (undefined !== IFD_TYPE_0THDefault[IFD_TYPE_0TH][shortAt]) {
      ...
      if (shortAt1 !== _modDef5447.tagTypes.RATIONAL) {
        if (shortAt1 !== _modDef5447.tagTypes.SRATIONAL) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5447.typeSizes[shortAt1];
    BYTE = _modDef5447.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    let arr = items.push(obj[BYTE](byteLength, sum, byteOrder));
    sum = sum + _modDef5447.typeSizes[BYTE];
  }
  if (BYTE === _modDef5447.tagTypes.ASCII) {
    let asciiValue = _modDef5447.getAsciiValue(items);
    const tmp9Result = _modDef5447;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = {
  1: null,
  2: _modDef5447.getByteAt,
  3: _modDef5447.getAsciiAt,
  4: _modDef5447.getShortAt,
  5: _modDef5447.getLongAt,
  7: _modDef5447.getRationalAt,
  9: null,
  10: _modDef5447.getUndefinedAt,
  13: null,
};
obj[9] = _modDef5447.getSlongAt;
obj[10] = _modDef5447.getSrationalAt;
obj[13] = _modDef5447.getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  const typeSize = _modDef5447.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = _modDef5447.getShortAt(byteLength, sum, byteOrder);
  ...
  if (_modDef5433.USE_THUMBNAIL) {
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      const longAt = _modDef5447.getLongAt(byteLength, tmp5, byteOrder);
      let tmp26 = 0 !== longAt;
      if (tmp26) {
      ...
      const tmp23Result2 = _modDef5447;
    }
    tmp23Result = _modDef5447;
  }
  return obj3;
}

export const get0thIfdOffset = function get0thIfdOffset(buffer, c5, byteOrder) {
  return c5 + _modDef5447.getLongAt(buffer, c5 + 4, byteOrder);
};
export { readIfd };
