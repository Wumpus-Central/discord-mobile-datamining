// _runtime/07554_createChildOrRootSpan.js
import _slicedToArray from "_slicedToArray";
import { getMainCarrier } from "07538_getMainCarrier.js";
import { getAsyncContextStrategy } from "07539_getAsyncContextStrategy.js";
import { getClient } from "07547_getClient.js";
import { hasTracingEnabled } from "07552_hasTracingEnabled.js";
import { 07535__ } from "metro/07535__.js";
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(7561).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(7553).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(7525).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(7525);
        const client = tmp(7547).getClient();
        if (client) {
      ...
      tmp(7558).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(7558);
      const result = tmp(7549).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(7556).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(7556);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(7525);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(7525).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(7525).spanIsSampled(parentSpan);
      tmp(7556).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(7556);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(7556).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(7556);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(7553).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(7559).sampleSpan(options, obj);
    const tmpResult = tmp(7559);
  }
  ...
  obj1.attributes = { [07535__.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(7561).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(7535).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _getMainCarrier.getMainCarrier();
  const obj = _getMainCarrier;
  const tmp = _require;
  ...
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(7547).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.startTime) {
      ...
      obj.startTimestamp = tmp3(7525).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj;
      const tmp3Result = tmp3(7525);
    }
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = tmp2(tmp3[1]).withScope((arg0) => {
            _null(7542)._setSpanForScope(arg0, _null);
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(7547).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [outer1_3]: true });
      return callback();
    });
    const tmpResult = tmp(7547);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(7547).withScope((arg0) => {
      _null(7542)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(7547);
