// _runtime/01669__isNativeReflectConstruct.js
...
import { updateLayoutAnimations } from "01748_updateLayoutAnimations.js";
...
export const createAnimatedComponent = function createAnimatedComponent(fn) {
      ...
      value: function _configureLayoutAnimation(EXITING, exiting, exiting2) {
        if (!isJest) {
          if (exiting !== exiting2) {
            ...
            if (obj2.isFabric()) {
              if (EXITING === tmp5(1659).LayoutAnimationType.ENTERING) {
                let reanimatedID = self.reanimatedID;
              }
              let maybeBuildResult = tmp3;
              if (tmp3) {
                let tmp8;
                if (EXITING !== tmp5(1659).LayoutAnimationType.LAYOUT) {
                ...
                maybeBuildResult = tmp5(1749).maybeBuild(tmp3, tmp8, AnimatedComponent.displayName);
                const tmp5Result = tmp5(1749);
              }
  ...
  tmp9.contextType = AnimatedComponent(1772).SkipEnteringContext;
  tmp9.displayName = "AnimatedComponent(" + fn.displayName || fn.name || "Component" + ")";
  const componentWithRefResult = AnimatedComponent(1773).componentWithRef((arg0, arg1) => {
    let obj = {};
