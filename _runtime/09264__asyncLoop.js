...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9265).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9266).crypto) {
    if (typeof tmp(9266).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9266).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9266).crypto) {
    if (typeof tmp(9266).crypto.randomBytes === "function") {
      const _crypto = tmp(9266).crypto;
