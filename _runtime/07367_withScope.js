// _runtime/07367_withScope.js
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7365).getMainCarrier();
  const obj = AsyncContextStack(7365);
  const sentryCarrier = AsyncContextStack(7365).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(7373);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7365).getMainCarrier();
      let obj = callback(7365);
      let sentryCarrier = callback(7365).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7373);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7365).getMainCarrier();
      const obj = callback(7365);
      const sentryCarrier = callback(7365).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7373);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
