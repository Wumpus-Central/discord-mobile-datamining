...
function checkNativeAndroidAvailable() {
  if (!RNIapModule) {
    if (!RNIapAmazonModule) {
      const _Error = Error;
      const error = new Error(require(6343) /* PurchaseError */.ErrorCode.E_IAP_NOT_AVAILABLE);
      throw error;
    }
  }
}
function getAndroidModule() {
  if (typeof checkNativeAndroidAvailable !== "fileFinishedImporting") {
...
export const getAndroidModuleType = () => {
  if (typeof getAndroidModule !== "fileFinishedImporting") {
    HermesBuiltin.throwTypeError();
  }
  if (typeof checkNativeAndroidAvailable !== "fileFinishedImporting") {
    HermesBuiltin.throwTypeError();
...
export const storekit2Mode = () => {
  ...
  if (typeof isStorekit2Available !== "fileFinishedImporting") {
    HermesBuiltin.throwTypeError();
  }
  let flag = !c6;
  if (c6) {
