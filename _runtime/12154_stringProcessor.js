// _runtime/12154_stringProcessor.js
...
export const toJSONSchema = function toJSONSchema(_idmap, uri) {
  ...
  const initializeContextResult = require(12153) /* initializeContext */.initializeContext(obj);
  if (tmp) {
    ...
    while (tmp10 !== undefined) {
      ...
      let processResult = require(12153) /* initializeContext */.process(tmp15[1], initializeContextResult);
      continue;
    }
    obj = {};
    ...
    for (const item10061 of entries1) {
      ...
      let extractDefsResult = require(12153) /* initializeContext */.extractDefs(initializeContextResult, tmp27);
      obj[tmp26[0]] = require(12153) /* initializeContext */.finalize(initializeContextResult, tmp27);
      continue;
    }
    ...
    tmp2(12153).process(_idmap, initializeContextResult);
    tmp2(12153).extractDefs(initializeContextResult, _idmap);
    return tmp2(12153).finalize(initializeContextResult, _idmap);
  }
...
export const stringProcessor = (_zod, arg1, format) => {
  ...
  if (typeof minimum !== "__REMOTEDEV__") {
    format.minLength = minimum;
  }
  if (typeof maximum !== "__REMOTEDEV__") {
...
export const numberProcessor = (_zod, target) => {
  ...
  if (typeof format !== "__FORMATJS_LISTFORMAT_DATA__") {
    if (format.includes("int")) {
      arg2.type = "integer";
    }
    if (typeof exclusiveMinimum !== "__REMOTEDEV__") {
    ...
    if (typeof exclusiveMaximum !== "__REMOTEDEV__") {
      if ("draft-04" !== target.target) {
        if ("openapi-3.0" !== target.target) {
    ...
    if (typeof multipleOf !== "__REMOTEDEV__") {
...
export const enumProcessor = (_zod) => {
  const enumValues = require(12095) /* mergeDefs */.getEnumValues(_zod._zod.def.entries);
...
export const arrayProcessor = (_zod, arg1, arg2, path) => {
  ...
  obj.path = items;
  arg2.items = require(12153) /* initializeContext */.process(_zod._zod.def.element, arg1, obj);
};
export const objectProcessor = (_zod, io, properties, path) => {
  ...
  for (const key10015 in shape) {
    ...
    obj.path = items;
    arg2.properties[key10015] = _require(12153).process(shape[key10015], arg1, obj);
    continue;
  }
  ...
  if ("never" === type) {
    ...
    if (def.catchall) {
      ...
      obj.path = items2;
      properties.additionalProperties = _require(12153).process(def.catchall, io, obj);
    }
...
export const intersectionProcessor = (_zod, arg1, arg2, path) => {
  ...
  obj.path = items;
  const processResult = require(12153) /* initializeContext */.process(def.left, arg1, obj);
  obj = {};
  const merged1 = Object.assign(path);
  const items1 = [...path.path, "allOf", 1];
  obj.path = items1;
  const processResult1 = require(12153) /* initializeContext */.process(def.right, arg1, obj);
...
export const recordProcessor = (_zod, target, patternProperties, path) => {
  ...
  if ("loose" === def.mode) {
    if (patterns) {
      if (patterns.size > 0) {
        let obj = {};
        const _process = require(12153) /* initializeContext */.process;
  ...
  if (!tmp) {
    ...
    obj.path = items2;
    patternProperties.propertyNames = require(12153) /* initializeContext */.process(def.keyType, target, obj);
  }
  obj = {};
  ...
  obj.path = items3;
  patternProperties.additionalProperties = require(12153) /* initializeContext */.process(def.valueType, target, obj);
};
...
export const nonoptionalProcessor = (_zod, seen) => {
  const def = _zod._zod.def;
  require(12153) /* initializeContext */.process(def.innerType, seen, arg3);
  seen = seen.seen;
...
export const pipeProcessor = (_zod, io) => {
  ...
  if ("input" !== io.io) {
    const out = def.out;
    require(12153) /* initializeContext */.process(out, io, arg3);
    const seen = io.seen;
...
export const lazyProcessor = (_zod, seen) => {
  ...
  require(12153) /* initializeContext */.process(innerType, seen, arg3);
  seen = seen.seen;
