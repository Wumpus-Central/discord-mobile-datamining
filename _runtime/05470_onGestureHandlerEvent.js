// _runtime/05470_onGestureHandlerEvent.js
import { DeviceEventEmitter } from "get ActivityIndicator";
import { map } from "05471_map.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5473).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5473).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5473).State.BEGAN) {
        if (handlerTag.state === tmp(5473).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5473).State.END) {
          if (handlerTag.oldState === tmp(5473).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5473).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5473).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5474).GestureStateManager;
      ...
      if (tmp(5475).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5475).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5475).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5475).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5471).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
