// === Module 1708: startWebLayoutAnimation ===

// Module 1708 (startWebLayoutAnimation)
import isWorkletFunction from "isWorkletFunction" /* 1678 */;
import TransitionType from "TransitionType" /* 1709 */;
import setElementAnimation from "setElementAnimation" /* 1722 */;
import findDescendantWithExitingAnimation from "findDescendantWithExitingAnimation" /* 1740 */;
...
function startWebLayoutAnimation(props, _componentDOMRef, ENTERING, easingY) {
  ...
  } else if (ENTERING === tmp(1678).LayoutAnimationType.EXITING) {
    entering = props.exiting;
  } else {
    entering = null;
    if (ENTERING === tmp(1678).LayoutAnimationType.LAYOUT) {
      entering = props.layout;
    }
  }
  let processedConfig = null;
  if (entering) {
    const tmp5 = entering instanceof tmp(1723).Keyframe;
    if (tmp5) {
      let tmpResult = tmp(1741);
    ...
    if (!flag) {
      ...
      const tmpResult1 = tmp(1722);
    }
    const tmp8 = ENTERING === tmp(1678).LayoutAnimationType.LAYOUT || tmp5 || undefined !== entering.initialValues;
  }
  ...
  if (processedConfig) {
    if (tmp(1678).LayoutAnimationType.ENTERING === ENTERING) {
      tmp(1722).setElementAnimation(_componentDOMRef, processedConfig, true);
      const tmpResult3 = tmp(1722);
    } else if (tmp(1678).LayoutAnimationType.LAYOUT === ENTERING) {
      easingY.reversed = processedConfig.reversed;
      const result1 = tmp(1722).handleLayoutTransition(_componentDOMRef, processedConfig, easingY);
      const tmpResult4 = tmp(1722);
    } else if (tmp(1678).LayoutAnimationType.EXITING === ENTERING) {
      const result2 = tmp(1722).handleExitingAnimation(_componentDOMRef, processedConfig);
      const tmpResult5 = tmp(1722);
    }
  } else {
    const elementVisible = tmp(1739).makeElementVisible(_componentDOMRef, 0);
    const tmpResult6 = tmp(1739);
  }
}
...
arg5.tryActivateLayoutTransition = function tryActivateLayoutTransition(props, _componentDOMRef, arg2) {
  if (props.layout) {
    ...
    if (!obj.areDOMRectsEqual(size, arg2)) {
      ...
      let str;
      if (easingXV != null) {
        str = easingXV[tmp3(undefined, 1705).EasingNameSymbol];
      }
      if (str == null) {
        str = "ease";
      }
      ...
      let str2;
      if (easingYV != null) {
        str2 = easingYV[tmp3(undefined, 1705).EasingNameSymbol];
      }
      if (str2 == null) {
        str2 = "ease";
      }
      ...
      obj[7] = presetName;
      obj[8] = presetName1;
      startWebLayoutAnimation(props, _componentDOMRef, tmp3(1678).LayoutAnimationType.LAYOUT, obj);
    }
  }
};