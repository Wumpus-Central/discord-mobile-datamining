// _runtime/13135_extractRequestData.js
  ...
  let item = include.forEach((item) => {
    if ("headers" === item) {
      ...
      if (!include.includes("ip")) {
        const ipHeaderNames = _mod13137.ipHeaderNames;
      ...
      if (!cookies) {
        ...
        if (cookie) {
          cookie = _mod13138.parseCookie(tmp21.cookie);
        }
        cookies = cookie;
      }
      if (!cookies) {
...
export const addNormalizedRequestDataToEvent = function addNormalizedRequestDataToEvent(
  ...
  if (obj.request) {
    ...
    if (obj2.includes("cookies")) {
      ...
      if (!cookies) {
        ...
        if (obj4.cookie) {
          parseCookieResult = _mod13138.parseCookie(obj4.cookie);
        }
  ...
  if (obj.ip) {
    ...
    if (ipAddress) {
      ipAddress = _mod13137.getClientIPAddress(normalizedRequest.headers);
    }
    if (!ipAddress) {
...
export const addRequestDataToEvent = function addRequestDataToEvent(request, request, include) {
  ...
  if (obj.ip) {
    ...
    if (ip) {
      ip = _mod13137.getClientIPAddress(request.headers);
    }
    if (!ip) {
      ip = request.ip;
    }
...
export const httpRequestToRequestData = function httpRequestToRequestData(headers) {
  ...
  return _mod13060.dropUndefinedKeys(request);
};
