...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  let currentScope = _require(7690).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7690);
  let client = _require(7690).getClient();
  if (client) {
    if (client.captureCheckIn) {
  ...
  let obj3 = _require(7690);
  captureCheckInResult = _require(7674).uuid4();
};