...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7700).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(7677).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(7677);
    const currentScope = monitorSlug(7690).getCurrentScope();
    const obj3 = monitorSlug(7690);
    const client = monitorSlug(7690).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7690);
    monitorSlug(7674).uuid4();
  }
  let currentScope = require("07690_getClient.js").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = getClient;
  let client = require("07690_getClient.js").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7691).DEBUG_BUILD) {
      let logger2 = tmp(7663).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7663).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = getClient;
  captureCheckInResult = require("07674_addContextToFrame.js").uuid4();
};