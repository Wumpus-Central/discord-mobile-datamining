// _runtime/00867_applyClientOptions.js
const require = arg1;
...
function applyClientOptions(environment, environment2) {
  ...
  if (!DEFAULT_ENVIRONMENT) {
    DEFAULT_ENVIRONMENT = maxValueLength(855).DEFAULT_ENVIRONMENT;
  }
  ...
  if (url) {
    request.url = maxValueLength(829).truncate(request.url, maxValueLength);
    let obj = maxValueLength(829);
  }
  if (maxValueLength) {
...
arg5.applyDebugIds = function applyDebugIds(exception) {
  _require = require("00871_getFilenameToDebugIdMap.js").getFilenameToDebugIdMap(arg1);
