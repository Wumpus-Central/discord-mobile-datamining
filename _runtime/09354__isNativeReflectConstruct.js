// _runtime/09354__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9349).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9348).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9349).exists(this);
      HashMD(9349).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9348).createView(content);
