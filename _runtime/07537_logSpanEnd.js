...
arg5.logSpanStart = function logSpanStart(spanContext) {
  ...
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(7504).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(7504).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(7504).spanToJSON(rootSpan);
    }
    const logger = tmp(7499).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult1 = tmp(7504);
  }
};