// _runtime/07876_MetricsAggregator.js
...
class MetricsAggregator {
  constructor(arg0) {
    ...
    tmp4 = closure_1;
    this._interval = setInterval(() => self._flush(), require("metro/07873__.js").DEFAULT_FLUSH_INTERVAL);
    if (this._interval.unref) {
      _interval = self._interval;
      unrefResult = _interval.unref();
    }
    random = Math.random();
    self._flushShift = Math.floor(random * require("metro/07873__.js").DEFAULT_FLUSH_INTERVAL / 1000);
...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = MetricsAggregator(7790).timestampInSeconds();
        const obj2 = MetricsAggregator(7790);
      }
      const self = this;
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = MetricsAggregator(7877).sanitizeMetricKey(arg1);
      const obj3 = MetricsAggregator(7877);
      const sanitizeTagsResult = MetricsAggregator(7877).sanitizeTags(obj);
      const obj4 = MetricsAggregator(7877);
      const sanitizeUnitResult = MetricsAggregator(7877).sanitizeUnit(str);
      const obj5 = MetricsAggregator(7877);
      const bucketKey = MetricsAggregator(7877).getBucketKey(arg0, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
      const _buckets = this._buckets;
      const value = _buckets.get(bucketKey);
      let num = 0;
      if (value) {
        num = 0;
        if (arg0 === tmp5(7873).SET_METRIC_TYPE) {
      ...
      if (value) {
        ...
        const tmp15 = new tmp5(7878).METRIC_MAP[arg0](diff);
        obj[0] = tmp15;
        obj[1] = rounded;
        obj[2] = arg0;
      ...
      const obj6 = MetricsAggregator(7877);
      const result1 = MetricsAggregator(7781).updateMetricSummaryOnActiveSpan(arg0, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
      self._bucketsTotalWeight = self._bucketsTotalWeight + obj.metric.weight;
      if (self._bucketsTotalWeight >= MetricsAggregator(7873).MAX_WEIGHT) {
        self.flush();
      }
    }
  },
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(7790).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(7873).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(7879).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(7879);
      }
    }
  }
];
