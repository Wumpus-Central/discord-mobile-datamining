// === Module 1162: userInteractionIntegration ===
      ...
      if (client.getOptions().enableUserInteractionTracing) {
        if (elementId) {
          ...
          if (currentReactNativeTracingIntegration.state.currentRoute) {
            ...
            if (activeSpan) {
              tmp18 = !tmp(1157).isSentryInteractionSpan(activeSpan);
              const tmpResult1 = tmp(1157);
            }
            if (activeSpan) {
              if (tmp18) {
                const debug7 = tmp(814).debug;
                const _HermesInternal8 = HermesInternal;
                debug7.warn("[" + UserInteraction + "] Did not create " + op + " transaction because active transaction " + tmp(814).spanToJSON(activeSpan).description + " exists on the scope.");
            ...
            if (activeSpan) {
              if (tmpResult3.spanToJSON(activeSpan).description === combined) {
                if (tmpResult4.spanToJSON(activeSpan).op === op) {
                  const debug5 = tmp(814).debug;
                  const _HermesInternal6 = HermesInternal;
                  debug5.warn("[" + UserInteraction + "] Did not create " + op + " transaction because it the same transaction " + tmp(814).spanToJSON(activeSpan).description + " already exists on the scope.");
                  const tmpResult5 = tmp(814);
                }
                tmpResult4 = tmp(814);
              }
              tmpResult3 = tmp(814);
            ...
            obj[1] = op;
            obj[2] = currentScope;
            const tmpResult6 = tmp(814);
            const result = tmp(1157).clearActiveSpanFromScope(currentScope);
            const tmpResult7 = tmp(1157);
            ...
            const startIdleSpanResult = tmp(1157).startIdleSpan(obj, obj);
            const attr = startIdleSpanResult.setAttribute(tmp(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, tmp(1155).SPAN_ORIGIN_MANUAL_INTERACTION);
            const tmpResult8 = tmp(1157);
            const result1 = tmp(1158).onlySampleIfChildSpans(client, startIdleSpanResult);
            const debug6 = tmp(814).debug;
