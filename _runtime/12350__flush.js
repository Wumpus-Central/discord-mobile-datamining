...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12339).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(12316).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(12316);
    const currentScope = monitorSlug(12329).getCurrentScope();
    const obj3 = monitorSlug(12329);
    const client = monitorSlug(12329).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12329);
    monitorSlug(12313).uuid4();
  }
  let currentScope = require("12329_getClient.js").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = getClient;
  let client = require("12329_getClient.js").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12330).DEBUG_BUILD) {
      let logger2 = tmp(12302).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(12302).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = getClient;
  captureCheckInResult = require("12313_addContextToFrame.js").uuid4();
};