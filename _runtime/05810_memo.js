...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(5604).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5597);
  ...
  const fn = function f() {
    return { enabled: iter.value === scrollableGesture(5593).SCROLLABLE_STATE.UNLOCKED };
  };
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5593).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  const tmp7 = callback(() => {
    let result;
    if (dependencyMap) {
      const Gesture = scrollableGesture(5620).Gesture;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5620).GestureDetector, obj);
  } else {
    const obj2 = {};
