...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7431).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7408).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7408);
    const currentScope = monitorSlug(7421).getCurrentScope();
    const obj3 = monitorSlug(7421);
    const client = monitorSlug(7421).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7421);
    monitorSlug(7405).uuid4();
  }
  ...
  if (client) {
    if (client.captureCheckIn) {
      ...
      let logger2 = tmp(7394).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    const asyncGeneratorStep = captureCheckInResult;
    let tmpResult = tmp(7408);
