// _runtime/05033_getTagName.js
import _slicedToArray from "_slicedToArray";
import { getDataView } from "04994_getDataView.js";
import { 05011__ } from "metro/05011__.js";
...
export default {
  read(arg0, arg1) {
    ...
    if (0 < arg0.length) {
      ...
      const shortAt = 05011__.getShortAt(dataView, sum);
      const sum1 = sum + c5;
      const tmp15 = getTagName(dataView, sum1);
      let name = tmp15.tagName;
      const sum2 = sum1 + tmp15.tagNameSize;
      const obj4 = 05011__;
      const longAt = 05011__.getLongAt(dataView, sum2);
      const sum3 = sum2 + c6;
      if (stringFromDataView === c4) {
        let tmp5Result = tmp5(4994);
        const dataView1 = tmp5Result.getDataView(dataView.buffer, sum3, longAt);
        obj = { id: null, value: null };
        obj[0] = shortAt;
        tmp5Result = tmp5(4994);
        obj[1] = tmp5Result.getStringFromDataView(dataView1, 0, longAt);
        if (tmp10(5034)[shortAt]) {
          try {
            obj.description = tmp10(5034)[shortAt].description(dataView1);
            if (!name) {
              name = tmp10(5034)[shortAt].name;
            }
            obj[name] = obj;
            const obj6 = tmp10(5034)[shortAt];
          } catch (err) {
