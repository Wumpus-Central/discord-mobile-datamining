...
import JEST_WORKER_ID from "JEST_WORKER_ID" /* 1763 */;
...
function processColor(arr, target) {
  ...
  if (tmp5) {
    ...
    let tmp28 = target !== tmp(1764).ValueProcessorTarget.CSS;
    if (!tmp28) {
    ...
    if (tmp7) {
      ...
      if (isDynamicColorObjectIOS(arr)) {
        if (tmp(1763).IS_IOS) {
          ...
          const reanimatedError = new tmp(1663).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
          throw reanimatedError;
        }
      }
      if (null === tmp12) {
        const reanimatedError1 = new tmp(1663).ReanimatedError(obj.invalidColor(arr));
...
processColor.__closure = { processColorNumber, ValueProcessorTarget: require("module_1764").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
  ...
  if (typeof arr === "number") {
    ...
    if (tmp2) {
      ...
      if (tmp7(1763).IS_IOS) {
        ...
        const reanimatedError = new tmp7(1663).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
        throw reanimatedError;
      }
    } else {
      const reanimatedError1 = new tmp7(1663).ReanimatedError(obj.invalidProcessedColor(arr));
      throw reanimatedError1;
    }
  }
}
const obj1 = { processColorNumber, ValueProcessorTarget: require("module_1764").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
