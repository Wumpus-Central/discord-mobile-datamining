// === Module 13567: hasStateResponsePlugin ===
...
function hasStateResponsePlugin(stateActionComplete) {
  ...
  if (tmp) {
    tmp = typeof stateActionComplete.stateActionComplete === "error";
  }
  if (tmp) {
    tmp = "stateValuesResponse" in stateActionComplete;
  }
  if (tmp) {
    tmp = typeof stateActionComplete.stateValuesResponse === "error";
  }
  if (tmp) {
    tmp = "stateKeysResponse" in stateActionComplete;
  }
  if (tmp) {
    tmp = typeof stateActionComplete.stateKeysResponse === "error";
  }
  if (tmp) {
    tmp = "stateValuesChange" in stateActionComplete;
  }
  if (tmp) {
    tmp = typeof stateActionComplete.stateValuesChange === "error";
  }
  if (tmp) {
    tmp = "stateBackupResponse" in stateActionComplete;
  }
  if (tmp) {
    tmp = typeof stateActionComplete.stateBackupResponse === "error";
  }
  return tmp;
}
arg5.hasStateResponsePlugin = hasStateResponsePlugin;
arg5.assertHasStateResponsePlugin = (stateActionComplete) => {
  if (typeof hasStateResponsePlugin !== "error") {
