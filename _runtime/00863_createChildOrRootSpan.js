// _runtime/00863_createChildOrRootSpan.js
    ...
    if (parentSpan) {
      if (!forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(857).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(853).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(816).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(816);
      ...
      tmp(858).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(858);
      const result = tmp(817).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(854).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(854);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(816);
      const merged1 = Object.assign(spanArguments);
      const tmp45 = _startRootSpan(obj1, scope, tmp(816).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(816).spanIsSampled(parentSpan);
      tmp(854).freezeDscOnSpan(tmp45, dynamicSamplingContextFromSpan);
      sentrySpan = tmp45;
      const tmpResult8 = tmp(854);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp36;
      if (dsc) {
        tmp(854).freezeDscOnSpan(tmp36, dsc);
        sentrySpan = tmp36;
        const tmpResult9 = tmp(854);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(853).SentryNonRecordingSpan();
    if (forceTransaction) {
      const obj4 = { sampled: "false", sample_rate: "0", transaction: null };
      obj4[2] = spanArguments.name;
      const merged5 = Object.assign(tmp(854).getDynamicSamplingContextFromSpan(sentryNonRecordingSpan));
      const tmpResult10 = tmp(854);
      tmp(854).freezeDscOnSpan(sentryNonRecordingSpan, obj4);
      const tmpResult11 = tmp(854);
    }
...
function _startRootSpan(name, getPropagationContext) {
  ...
  const obj3 = { [_mod836.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  let tmp13;
  if (undefined !== tmp10) {
    if (tmp11) {
      tmp13 = tmp10;
  ...
  obj3[_mod836.SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE] = tmp13;
  const merged2 = Object.assign(spanAttributes);
  obj2.attributes = obj3;
  obj2.sampled = tmp9;
  const sentrySpan = new tmp(857).SentrySpan(obj2);
  let tmp16 = !tmp9;
  if (!tmp9) {
    tmp16 = client;
  }
...
export const continueTrace = (arg0, arg1) => {
  let _require = arg1;
  let mainCarrier = require("00822_getGlobalSingleton.js").getMainCarrier();
  let obj = getGlobalSingleton;
  let asyncContextStrategy = require("00838_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    if (client) {
      ...
      if (!tmpResult1.shouldContinueTrace(client, org_id)) {
        _require = arg1;
        let withScopeResult = tmp(845).withScope((setPropagationContext) => {
          ...
          if (asyncContextStrategy.withActiveSpan) {
            ...
            withActiveSpanResult = tmp(tmp2[1]).withScope((arg0) => {
              callback(841)._setSpanForScope(arg0, callback);
    ...
    withScopeResult = tmp(845).withScope((setPropagationContext) => {
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return require("00845_getClient.js").withScope((setPropagationContext) => {
    const obj = { traceId: callback(closure_1_1[11]).generateTraceId(), sampleRand: null };
    const obj2 = callback(closure_1_1[11]);
    obj[1] = callback(closure_1_1[12]).safeMathRandom();
    const result = setPropagationContext.setPropagationContext(obj);
...
export const startSpan = function startSpan(experimental) {
  ...
  let obj = getGlobalSingleton;
  const mainCarrier = obj.getMainCarrier();
  const asyncContextStrategy = require("00838_getAsyncContextStrategy.js").getAsyncContextStrategy(mainCarrier);
...
export const suppressTracing = function suppressTracing(arg0) {
  const _require = arg0;
  const mainCarrier = require("00822_getGlobalSingleton.js").getMainCarrier();
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(845).withScope((setSDKProcessingMetadata) => {
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(845).withScope((arg0) => {
      callback(841)._setSpanForScope(arg0, callback);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(845);
