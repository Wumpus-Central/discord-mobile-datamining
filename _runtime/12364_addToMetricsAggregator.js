// === Module 12364: addToMetricsAggregator ===
...
function addToMetricsAggregator(arg0, SET_METRIC_TYPE, arg2, arg3, arg4) {
  ...
  if (!client) {
    client = _require(12295).getClient();
    const obj2 = _require(12295);
  }
  if (client) {
    const activeSpan = _require(12273).getActiveSpan();
    let rootSpan;
    if (activeSpan) {
    ...
    if (_require(12296).DEBUG_BUILD) {
      const logger = tmp3(12268).logger;
      const _HermesInternal = HermesInternal;
      logger.log("Adding value of " + arg3 + " to " + SET_METRIC_TYPE + " metric " + arg2);
    }
    const obj3 = _require(12273);
    const globalSingleton = _require(12269).getGlobalSingleton("globalMetricsAggregators", () => {
    ...
    const tmp3Result1 = _require(12269);
  }
}
errorCallback;
...
export const metrics = {
  ...
  timing(arg0, arg1, fn) {
    ...
    if (typeof fn === "function") {
      ...
      return _require(12302).startSpanManual(obj, (arg0) => {
        const callback = arg0;
      ...
      obj.unit = str;
      const DISTRIBUTION_METRIC_TYPE = _require(12365).DISTRIBUTION_METRIC_TYPE;
      let parsed = fn;
      if (typeof fn === "string") {
