// _runtime/06477_map.js
import _mod6451 from "metro/06451__.js";
import hash from "06465_hash.js";
import items2 from "06466_items2.js";
import isNativeEvent from "06478_isNativeEvent.js";
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6451.Reanimated;
  }
  if (result) {
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      ...
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(6466).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(6451).Reanimated;
