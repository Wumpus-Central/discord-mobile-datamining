...
export const useHandler = function useHandler(memoizedGestureCallbacks, items10) {
  ...
  const dependencies = require("01782_isAnimated.js").buildDependencies(items10, memoizedGestureCallbacks);
  const obj3 = isAnimated;
  const tmp9 = _require;
  tmp.current.savedDependencies = dependencies;
  obj = {
    context,
    doDependenciesDiffer: !require("01782_isAnimated.js").areDependenciesEqual(dependencies, savedDependencies),
    useWeb: null,
  };
  const obj4 = isAnimated;
  let isWebResult = require("01639_isJest.js").isWeb();
  if (!isWebResult) {
