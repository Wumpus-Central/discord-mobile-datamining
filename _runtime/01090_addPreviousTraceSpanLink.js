// === Module 1090: addPreviousTraceSpanLink ===

// Module 1090 (addPreviousTraceSpanLink)
import ignoreNextOnError from "ignoreNextOnError" /* 1025 */;
import addClsInstrumentationHandler from "addClsInstrumentationHandler" /* 1030 */;
import __SENTRY_DEBUG__ from "__SENTRY_DEBUG__" /* 1069 */;
import items from "items" /* 1070 */;
  ...
  if (spanContext) {
    ...
    if (spanContext.traceId !== spanToJSONResult.trace_id) {
      ...
      if (Date.now() / 1000 - spanContext.startTimestamp <= 3600) {
        ...
        obj1[0] = spanContext;
        const obj2 = {};
        obj2[tmp(814).SEMANTIC_LINK_ATTRIBUTE_LINK_TYPE] = "previous_trace";
        obj1[1] = obj2;
