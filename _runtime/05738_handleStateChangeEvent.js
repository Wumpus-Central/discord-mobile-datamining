// _runtime/05738_handleStateChangeEvent.js
import _mod5658 from "metro/05658__.js";
import _mod5660 from "metro/05660__.js";
import _isNativeReflectConstruct from "05673__isNativeReflectConstruct.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod5658.State.UNDETERMINED) {
    if (state === tmp(5658).State.BEGAN) {
      let tmpResult = tmp(5720);
      tmpResult.runCallback(tmp(5673).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod5658.State.BEGAN) {
    if (state === tmp(5658).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5720);
      tmpResult.runCallback(tmp(5673).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5658).State.FAILED || state === tmp(5658).State.CANCELLED;
    if (oldState === tmp(5658).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5720).runCallback(tmp(5673).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5720);
    }
    const tmp7 = state === tmp(5658).State.FAILED || state === tmp(5658).State.CANCELLED;
    tmp(5720).runCallback(tmp(5673).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(5720);
  }
}
let obj = {
  flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent,
  State: require("metro/05658__.js").State,
  runCallback: require("isGestureEnabled").runCallback,
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod5660.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5720);
    tmpResult = tmp(5720);
...
obj = {
  ...
  runCallback: require("isGestureEnabled").runCallback,
  touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType,
};
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5660).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5720);
          tmpResult1.runCallback(tmp(5720).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5720);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5720).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5720);
        tmp(5720).runCallback(tmp(5673).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: require("isGestureEnabled").maybeExtractNativeEvent,
  isEventForHandlerWithTag: require("isGestureEnabled").isEventForHandlerWithTag,
  isStateChangeEvent: require("isNativeEvent").isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: require("isNativeEvent").isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
