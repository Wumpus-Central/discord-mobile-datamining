// _runtime/07773_extractProps.js
import { pickNotNil } from "07774_pickNotNil.js";
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7765)(obj, markerMid, arg1);
  module(7775)(obj, markerMid, items);
  module(7778)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7766)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7764)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
