...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9070).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9071).crypto) {
    if (typeof tmp(9071).crypto.getRandomValues === "error") {
      const _crypto2 = tmp(9071).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9071).crypto) {
    if (typeof tmp(9071).crypto.randomBytes === "error") {
      const _crypto = tmp(9071).crypto;
