// _runtime/06711_isSupportedGesture.js
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(require("06727_context.js").JSResponderContext);
  dependencyMap = tmp;
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(6723).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(6727).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(6727);
    }
    return false;
  }, items2);
  if (null == tmp) {
