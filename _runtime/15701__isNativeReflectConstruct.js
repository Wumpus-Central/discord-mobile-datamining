// _runtime/15701__isNativeReflectConstruct.js
...
function captureRef(obj, options) {
  ...
  if (ViewShot(15702)) {
    ...
    return tmp(15702).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(15702).releaseCapture(str);
    const obj = ViewShot(15702);
  }
}
...
export const captureScreen = function captureScreen(options) {
  ...
  if (ViewShot(15702)) {
    ...
    return tmp(15702).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
