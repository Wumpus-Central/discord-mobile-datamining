...
export const useDescriptors = function useDescriptors(state) {
  ...
  const theme = noop.useContext(state(1521).ThemeContext);
  [c12, tmp2] = noop.useState({});
  c13 = tmp2;
  const context = noop.useContext(state(1514).NavigationBuilderContext);
  ...
  const navigationCache = state(1571).useNavigationCache({ state, getState, navigation, setOptions: tmp2, router, emitter });
  ({ base: c22, navigations: c23 } = navigationCache);
  let obj = state(1571);
  const routeCache = state(1536).useRouteCache(state.routes);
  ...
  function render(route, navigation, options, routeState) {
    ...
    const tmp5 = onAction(state(1572).SceneView, {
      navigation,
      route,
      screen: props,
      routeState,
    ...
    const obj3 = { value, children: onAction(state(1525).NavigationProvider, { route, navigation, children: layoutResult }) };
    return onAction(state(1514).NavigationBuilderContext.Provider, obj3, route.key);
  }
