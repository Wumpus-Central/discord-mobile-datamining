...
export const SceneView = function SceneView(getState) {
  ...
  const addOptionsGetter = route(routeState[2]).useOptionsGetters({
    key: route.key,
  ...
  obj4.children = setState(route(routeState[6]).EnsureSingleNavigator, {
    children: setState(route(routeState[7]).StaticContainer, obj5),
  });
