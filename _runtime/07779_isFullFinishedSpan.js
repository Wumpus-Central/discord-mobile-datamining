// === Module 7779: isFullFinishedSpan ===
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(7743).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(7753).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(7743).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(7743);
        SentrySpan(7776).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(7776);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      obj[12] = this._attributes[SentrySpan(undefined, 7753).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(7754);
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(7765).getClient();
      if (client) {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (!client) {
          client = tmp3(7765).getClient();
          const tmp3Result1 = tmp3(7765);
        }
        if (true !== self._sampled) {
          if (tmp3(7766).DEBUG_BUILD) {
            const logger3 = tmp3(7738).logger;
          ...
          const tmp3Result4 = tmp3(7744);
          obj2[0] = tmp3(7774).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(7774);
          obj[7] = tmp3(7754).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(7754);
          const result = tmp3(7780).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(7766).DEBUG_BUILD) {
              const logger2 = tmp3(7738).logger;
              const _JSON = JSON;
