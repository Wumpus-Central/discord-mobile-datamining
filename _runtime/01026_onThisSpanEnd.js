// _runtime/01026_onThisSpanEnd.js
...
export const adjustTransactionDuration = (on, activeSpan) => {
  ...
  if (obj.isRootSpan(activeSpan)) {
    ...
    const debug = require("00682_registerSpanErrorInstrumentation.js").debug;
        ...
        if (tmpResult.isSentrySpan(c4)) {
          on.on("spanEnd", (arg0) => {
            if (arg0 === c4) {
              if (f71979(tmp)) {
                c4 = tmp;
                const spanDescendants = c4(f71979[2]).getSpanDescendants(tmp);
                if (
...
export const ignoreEmptyRouteChangeTransactions = (on, c4) => {
  ...
  closure_0 = c4;
  f71979 = (arg0) => {
    const spanToJSONResult = c4(f71979[2]).spanToJSON(arg0);
    let tmp2 = spanToJSONResult.description === f71979;
    if (tmp2) {
  ...
  f71980 = (arg0) => {
    const debug = c4(f71979[2]).debug;
    debug.log('Discarding empty "' + f71979 + '" transaction that never received route information.');
    if (null != c4) {
