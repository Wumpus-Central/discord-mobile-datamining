// _runtime/06983_eventHandler.js
import _mod6903 from "metro/06903__.js";
import TouchEventType from "06905_TouchEventType.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6903.State.UNDETERMINED) {
    if (state === _mod6903.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6903.State.BEGAN) {
    if (state === _mod6903.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6903.State.FAILED || state === _mod6903.State.CANCELLED;
    if (oldState === _mod6903.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6903.State.FAILED || state === _mod6903.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(6965).flattenAndFilterEvent,
  State: fn(6903).State,
  runCallback: fn(6965).runCallback,
  CALLBACK_TYPE: fn(6918).CALLBACK_TYPE,
};
...
handleTouchEvent.__closure = {
  TouchEventType: fn(6905).TouchEventType,
  runCallback: fn(6965).runCallback,
  touchEventTypeToCallbackType: fn(6965).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(6965).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(6965).isEventForHandlerWithTag,
  isStateChangeEvent: fn(6967).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(6967).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
