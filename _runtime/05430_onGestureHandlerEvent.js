// _runtime/05430_onGestureHandlerEvent.js
import { DeviceEventEmitter } from "get ActivityIndicator";
import { map } from "05431_map.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5433).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5433).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5433).State.BEGAN) {
        if (handlerTag.state === tmp(5433).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5433).State.END) {
          if (handlerTag.oldState === tmp(5433).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5433).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5433).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5434).GestureStateManager;
      ...
      if (tmp(5435).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5435).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5435).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5435).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5431).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
