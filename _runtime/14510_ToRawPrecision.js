// _runtime/14510_ToRawPrecision.js
import digitsToString2 from "14491_digitsToString.js";
...
export const ToRawPrecision = function ToRawPrecision(
  ...
  let timesResult;
  if (ZERO.isZero()) {
    let repeatResult = _mod14492.repeat("0", maximumSignificantDigits);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(14492).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(14492).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(14492).repeat("0", -num3 - 1)}${arr}`;
