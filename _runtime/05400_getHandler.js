// _runtime/05400_getHandler.js
import { toArray } from "05357_toArray.js";
import { _isNativeReflectConstruct } from "05392__isNativeReflectConstruct.js";
import { 05361__ } from "metro/05361__.js";
const require = arg1;
const dependencyMap = arg6;
function getHandler(arg0, onBegin) {
  if (_isNativeReflectConstruct.CALLBACK_TYPE.BEGAN === arg0) {
    return onBegin.onBegin;
  } else if (tmp(5392).CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (tmp(5392).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(5392).CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (tmp(5392).CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (tmp(5392).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(5392).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(5392).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(5392).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(5392).CALLBACK_TYPE.TOUCHES_CANCELLED === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (05361__.TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(5392).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(5361).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(5392).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(5361).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(5392).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(5361).TouchEventType.TOUCHES_CANCELLED === arg0) {
    return tmp(5392).CALLBACK_TYPE.TOUCHES_CANCELLED;
  } else {
    return tmp(5392).CALLBACK_TYPE.UNDEFINED;
  }
}
obj = { TouchEventType: require("module_5361").TouchEventType, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
