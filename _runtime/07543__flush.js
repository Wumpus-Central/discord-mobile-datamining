...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7532).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7509).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7509);
    const currentScope = monitorSlug(7522).getCurrentScope();
    const obj3 = monitorSlug(7522);
    const client = monitorSlug(7522).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7522);
    monitorSlug(7506).uuid4();
  }
