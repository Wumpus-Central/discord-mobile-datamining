...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = require(13386) /* getMultiInternalSlots */.repeat("0", maximumSignificantDigits);
    let roundedNumber = require(13388) /* TEN */.ZERO;
    ...
    result = require(13398) /* __exportStarResult11 */.ApplyUnsignedRoundingMode(ZERO, timesResult, roundedNumber, result);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13386).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13386).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13386).repeat("0", -num3 - 1)}${arr}`;
