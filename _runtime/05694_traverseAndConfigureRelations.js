// _runtime/05694_traverseAndConfigureRelations.js
import tagMessage from "05625_tagMessage.js";
import { isComposedGesture } from "05687_isComposedGesture.js";
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(5687).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(5679).ComposedGestureName.Simultaneous) {
