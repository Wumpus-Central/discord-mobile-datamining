// _runtime/14584_ApplyUnsignedRoundingMode.js
...
export const ApplyUnsignedRoundingMode = function ApplyUnsignedRoundingMode(
  ...
  if (eq.eq(timesResult)) {
    ...
    _mod14572.invariant(timesResult.lessThan(eq) && eq.lessThan(roundedNumber), combined1.concat(roundedNumber));
    if ("zero" === formattedString) {
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        _mod14572.invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === formattedString) {
          return timesResult;
        } else if ("half-infinity" === formattedString) {
          return roundedNumber;
        } else {
          _mod14572.invariant("half-even" === formattedString, "unsignedRoundingMode should be half-even");
