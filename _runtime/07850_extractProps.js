// _runtime/07850_extractProps.js
import { pickNotNil } from "07851_pickNotNil.js";
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7842)(obj, markerMid, arg1);
  module(7852)(obj, markerMid, items);
  module(7855)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7843)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7841)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
