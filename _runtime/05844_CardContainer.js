// _runtime/05844_CardContainer.js
...
export const CardContainer = importAllResult.memo(function CardContainerInner(active) {
  ...
  let num = importAllResult.useContext(active(5316).HeaderHeightContext);
  let tmp4 = focused;
  if (focused) {
    tmp4 = false !== scene.descriptor.options.keyboardHandlingEnabled;
  }
  let tmp2Result = tmp2(5845);
  ...
  if (previousScene) {
    ...
    headerTitle = tmp2(5316).getHeaderTitle(options, route.name);
    href = tmp2Result1.useLinkBuilder().buildHref(route.name, route.params);
    tmp8 = href;
    tmp9 = headerTitle;
    const tmp2Result2 = tmp2(5316);
  }
  c15 = tmp12;
  ...
  obj8[1] = closure_4(active(5316).HeaderHeightContext.Provider, { value: headerHeight, children: descriptor.render() });
  obj7[1] = closure_4(active(5316).HeaderShownContext.Provider, obj8);
  obj6[1] = closure_4(active(5316).HeaderBackContext.Provider, obj7);
  items4[1] = closure_4(closure_3, obj6);
  obj4[1] = items4;
  obj3[1] = tmp22(active(5834).ModalPresentationContext.Provider, obj4);
  obj1[29] = closure_4(closure_3, obj3);
  obj[6] = closure_4(active(5847).Card, obj1);
  return closure_4(active(5846).CardA11yWrapper, obj);
});