// _runtime/07835__isNativeReflectConstruct.js
import isErrorEvent from "07831_isErrorEvent.js";
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7836).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7836);
      return ServerRuntimeClient(7800).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7800);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7836).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7803).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7803);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7829).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7804).DEBUG_BUILD) {
        const logger = tmp2(7776).logger;
    ...
    value: function captureCheckIn(checkInId) {
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7837);
          ...
          if (ServerRuntimeClient(7804).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7776).logger;
          ...
          if (ServerRuntimeClient(7804).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7776).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7787);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7798)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7812);
        if (_getSpanForScopeResult) {
