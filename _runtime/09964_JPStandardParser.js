...
const items = [
  entry,
  {
    ...
    value: function extract(createParsingComponents, match) {
      const parsed = parseInt(JPStandardParser(9965).toHankaku(match[5]));
      const parsed1 = parseInt(JPStandardParser(9965).toHankaku(match[6]));
      ...
      if (match[2]) {
        ...
        if ("\u5143" != match[4]) {
          ...
          num = parseInt(JPStandardParser(9965).toHankaku(tmp8));
        }
        if ("\u4EE4\u548C" == match[3]) {
          let sum = num + 2018;
        ...
        parsingComponents.imply("year", JPStandardParser(9896).findYearClosestToRef(createParsingComponents.refDate, parsed1, parsed));
