  ...
  const observer = NativePerformanceCxx.createObserver(() => {
    ...
    if (takeRecordsResult) {
      if (0 !== takeRecordsResult.length) {
        const tmp11 = new importDefaultResultResult(
          takeRecordsResult.map(PerformanceObserver(170).rawToPerformanceEntry),
        );
        let num2 = 0;
...
let items = [
  entry,
  {
    ...
    value: function getEntriesByType(arg0) {
      closure_0 = arg0;
      return _classPrivateFieldBase(this, closure_6)[closure_6].filter(
        (entryType) => entryType.entryType === closure_0,
      );
    },
  },
  {
...
importDefaultResultResult1.supportedEntryTypes = Object.freeze(
  NativePerformanceCxx.map(fn(170).rawToPerformanceEntryType),
);
