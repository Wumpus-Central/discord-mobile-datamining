// _runtime/07017_forHorizontalIOS.js
import conditional from "07018_conditional.js";
...
function forHorizontalIOS(layouts) {
  ...
  let obj = {
    translateX: callback(
      progress.interpolate({ inputRange: [0, 1], outputRange: items, extrapolate: "clamp" }),
      inverted,
    ),
  };
  ...
  obj = {
    cardStyle: { transform: items1 },
    overlayStyle: {
      opacity: progress3.interpolate({ inputRange: [0, 1], outputRange: [0, 0.07], extrapolate: "clamp" }),
    },
    shadowStyle: null,
  };
  ...
  obj[2] = {
    shadowOpacity: progress4.interpolate({ inputRange: [0, 1], outputRange: [0, 0.3], extrapolate: "clamp" }),
  };
  return obj;
}
...
export const forModalPresentationIOS = function forModalPresentationIOS(next) {
  ...
  const obj2 = {
    ...
    borderTopLeftRadius: num9,
    borderTopRightRadius: num9,
...
export const forFadeFromBottomAndroid = function forFadeFromBottomAndroid(current) {
  ...
  obj[0] = conditional.conditional(
    closing,
    current.progress,
    progress2.interpolate({ inputRange: [0, 0.5, 0.9, 1], outputRange: [0, 0.25, 0.7, 1], extrapolate: "clamp" }),
  );
...
export const forRevealFromBottomAndroid = function forRevealFromBottomAndroid(layouts) {
  ...
  const items3 = [
    {
      translateY: callback(
        progress2.interpolate({ inputRange: [0, 1], outputRange: items2, extrapolate: "clamp" }),
        inverted,
      ),
    },
  ];
  ...
  const progress4 = current.progress;
  obj[2] = {
    opacity: progress4.interpolate({ inputRange: [0, 0.36, 1], outputRange: [0, 0.1, 0.1], extrapolate: "clamp" }),
  };
  return obj;
};
export const forScaleFromCenterAndroid = function forScaleFromCenterAndroid(closing) {
  ...
  const tmpResult = closure_3(
    progress.interpolate({ inputRange: [0, 1], outputRange: [0, 1], extrapolate: "clamp" }),
    num,
  );
  obj = { cardStyle: null };
  obj = {
    opacity: tmpResult.interpolate({
  ...
  obj1[0] = obj6.conditional(
    closing.closing,
    progress3.interpolate({ inputRange: [0, 1], outputRange: [0.925, 1], extrapolate: "clamp" }),
    tmpResult.interpolate({ inputRange: [0, 1, 2], outputRange: [0.85, 1, 1.075] }),
  );
  const items = [obj1];
  obj[1] = items;
  obj[0] = obj;
...
export const forFadeFromRightAndroid = function forFadeFromRightAndroid(closing) {
  ...
  tmp = callback;
  const tmp2 = callback(
    progress.interpolate({ inputRange: [0, 1], outputRange: [96, 0], extrapolate: "clamp" }),
    inverted,
  );
  const progress3 = current.progress;
  obj[0] = conditional.conditional(
    closing.closing,
    progress3.interpolate({ inputRange: [0, 1], outputRange: [0, 1], extrapolate: "clamp" }),
    current.progress,
  );
...
export const forBottomSheetAndroid = function forBottomSheetAndroid(current) {
  ...
  obj[0] = conditional.conditional(
    closing,
    current.progress,
    progress2.interpolate({ inputRange: [0, 1], outputRange: [0, 1], extrapolate: "clamp" }),
  );
  const items1 = [{ translateY: tmp }];
  obj[1] = items1;
  obj[0] = obj;
