// _runtime/01772__isNativeReflectConstruct.js
  ...
  isJest,
  {
    ...
    value: function setShouldAnimateExiting() {
      ...
      if (1 === closure_5.count(this.props.children)) {
        const findNodeHandleResult = LayoutAnimationConfig(1742).findNodeHandle(self);
        if (findNodeHandleResult) {
          const result = tmp(1678).setShouldAnimateExitingForTag(findNodeHandleResult, !self.props.skipExiting);
          const tmpResult = tmp(1678);
        }
        const obj = LayoutAnimationConfig(1742);
