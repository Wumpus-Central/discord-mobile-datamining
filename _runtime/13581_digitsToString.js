// _runtime/13581_digitsToString.js
let self = this;
self = this;
      ...
      if (d[0]) {
        ...
        if (Math.abs(e) < 1500000000000000) {
          if (charAtResult >= 7) {
            ...
            e2 = f110970(minusResult, constructor.plus(1), sum, 1);
  ...
  if (isFinite.isFinite()) {
    ...
    if (arr3[0]) {
      if (index < 0) {
        ...
        tmp35 = f110970(constructor1, tmp11, diff, rounding, 0, num4);
        ({ d, e } = tmp35);
        let tmp36 = closure_2;
      }
      if (!tmp36) {
          ...
          if (s.d) {
            if (!obj.isZero()) {
              if (obj.s < 0) {
                self.precision = sum;
                self.rounding = 1;
                const atanResult = self.atan(f110970(s, tmp3, sum, 1));
                if (sum > closure_21) {
                  const _Error2 = Error;
                  throw Error(c10);
                } else {
                ...
                atanResult1 = self.atan(f110970(s, tmp3, sum, 1));
              }
            }
          }
          if (sum > closure_21) {
...
const fn7 = function(arg0) {
  const constructor = new this.constructor(arg0);
  return f110970(this, constructor);
};
obj.div = fn7;
obj.dividedBy = fn7;
const fn8 = function(arg0) {
  let constructor = this.constructor;
  constructor = new constructor(arg0);
  const tmp2 = f110970(this, constructor, 0, 1, 1);
  ...
  if (this.isFinite()) {
    if (self.e < 0) {
      ...
      if (Math.max(sdResult, precision) < 2 * -self.e - 1) {
        ...
        constructor.rounding = 1;
        const obj3 = f110970(plusResult, constructor1.minus(self), diff + precision, 1);
        constructor.precision = precision;
        constructor.rounding = rounding;
        timesResult = f110970(plusResult, constructor1.minus(self), diff + precision, 1).ln().times(0.5);
        const lnResult = f110970(plusResult, constructor1.minus(self), diff + precision, 1).ln();
      }
    }
    if (absResult.eq(1)) {
      let num4 = self.s / 0;
    ...
    if (d) {
      if (d[0]) {
        if (!self.eq(1)) {
          ...
          let tmp28 = f110970(tmp19, tmp17Result, sum, 1);
          let tmp29 = sum;
          let tmp30 = precision;
  ...
  if (this.d) {
    if (constructor.s) {
      ...
      if (9 == constructor.modulo) {
        ...
        obj2 = f110970(self, tmp, 0, constructor.modulo, 1);
      }
      c8 = true;
...
const fn36 = function() {
  ...
  if (this.isFinite()) {
    if (self.isZero()) {
      ...
      sinResult.s = 1;
      constructor = new constructor(1);
      const obj4 = f110970(sinResult, constructor.minus(sinResult.times(sinResult)).sqrt(), precision + 10, 0);
      constructor.precision = precision;
      constructor.rounding = rounding;
      if (2 == closure_4) {
        negResult = obj4.neg();
...
obj.toFraction = function(arg0) {
  ...
  if (d) {
    ...
    let tmp47 = f110970;
    const obj4 = f110970(constructor4, constructor2, 0, 1, 1);
