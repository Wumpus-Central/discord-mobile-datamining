...
function createProfilePayload(arg0, arg1, resources, type) {
  if ("transaction" !== type.type) {
    ...
    if (DEBUG_BUILD) {
      DEBUG_BUILD = __SENTRY_DEBUG__ /* __SENTRY_DEBUG__ */.DEBUG_BUILD;
    }
    if (DEBUG_BUILD) {
    ...
    if (arg1) {
      if (typeof type.timestamp === "number") {
        ...
        result = 1000 * registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.timestampInSeconds();
        const obj14 = registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */;
      }
      ...
      if (!DEFAULT_ENVIRONMENT) {
        DEFAULT_ENVIRONMENT = registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.DEFAULT_ENVIRONMENT;
      }
      obj[5] = DEFAULT_ENVIRONMENT;
      obj = { name: "javascript", version: null };
      obj[1] = ignoreNextOnError /* ignoreNextOnError */.WINDOW.navigator.userAgent;
      obj[6] = obj;
      ...
      result2 = 1000 * registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.timestampInSeconds();
...
function applyDebugMetadata(arg0) {
  ...
  if (stackParser) {
    let debugImagesForResources = registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.getDebugImagesForResources(stackParser, arg0);
...
export const createProfileChunkPayload = function createProfileChunkPayload(closure_0, _client, _profilerId) {
  if (null == closure_0) {
    ...
    obj3[0] = registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.uuid4();
    str = undefined;
    if (sdk != null) {
      str = sdk.name;
    }
...
export const isAutomatedPageLoadSpan = function isAutomatedPageLoadSpan(rootSpan) {
  return "pageload" === registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.spanToJSON(rootSpan).op;
};
...
export const shouldProfileSpanLegacy = function shouldProfileSpanLegacy(rootSpan) {
  if (c14) {
    if (__SENTRY_DEBUG__ /* __SENTRY_DEBUG__ */.DEBUG_BUILD) {
      const debug6 = registerSpanErrorInstrumentation /* registerSpanErrorInstrumentation */.debug;
...
export const startJSSelfProfile = function startJSSelfProfile() {
  const Profiler = ignoreNextOnError /* ignoreNextOnError */.WINDOW.Profiler;
