...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod5660.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("metro/05660__.js").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod5660.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5660).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, closure_2) {
  ...
  if (undefined !== require("metro/05660__.js").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = require("metro/05660__.js").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5660).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, closure_2) {
  if (undefined !== require("metro/05660__.js").Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5660).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5660).Reanimated;
        const fn = function t() {
