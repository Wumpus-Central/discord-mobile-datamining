// _runtime/05688_getHandler.js
import noop from "00019_noop.js";
import _mod5625 from "metro/05625__.js";
import _isNativeReflectConstruct from "05638__isNativeReflectConstruct.js";

const useMemo = noop.useMemo;
function getHandler(arg0, onBegin) {
  if (_isNativeReflectConstruct.CALLBACK_TYPE.BEGAN === arg0) {
    return onBegin.onBegin;
  } else if (tmp(5638).CALLBACK_TYPE.START === arg0) {
    return onBegin.onActivate;
  } else if (tmp(5638).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(5638).CALLBACK_TYPE.END === arg0) {
    return onBegin.onDeactivate;
  } else if (tmp(5638).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(5638).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(5638).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(5638).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(5638).CALLBACK_TYPE.TOUCHES_CANCEL === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (_mod5625.TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(5638).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(5625).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(5638).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(5625).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(5638).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(5625).TouchEventType.TOUCHES_CANCEL === arg0) {
    return tmp(5638).CALLBACK_TYPE.TOUCHES_CANCEL;
  } else {
    return tmp(5638).CALLBACK_TYPE.UNDEFINED;
  }
}
obj = { TouchEventType: _mod5625.TouchEventType, CALLBACK_TYPE: _isNativeReflectConstruct.CALLBACK_TYPE };
