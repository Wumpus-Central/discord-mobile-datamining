          ...
          if (item10012.route.path) {
            if ("*" !== item10012.route.path) {
              ...
              if (pathname.pathname === arr2) {
                ...
                obj = { pathname: arr2 };
                const tmp11 = rebuildRoutePathFromAllRoutes(
                  item10012.filter((item) => item !== item10012.route),
                  obj,
                );
                let combined = tmp11;
                if ("/" !== tmp11[0]) {
...
export const resolveRouteNameAndSource = function resolveRouteNameAndSource(
  _location,
  routes,
  routes2,
  items,
