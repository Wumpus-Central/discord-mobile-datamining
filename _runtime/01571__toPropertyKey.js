...
export const useDescriptors = function useDescriptors(state) {
  ...
  const theme = noop.useContext(state(1522).ThemeContext);
  [c12, tmp2] = noop.useState({});
  c13 = tmp2;
  const context = noop.useContext(state(1515).NavigationBuilderContext);
  ...
  const navigationCache = state(1572).useNavigationCache({ state, getState, navigation, setOptions: tmp2, router, emitter });
  ({ base: c22, navigations: c23 } = navigationCache);
  let obj = state(1572);
  const routeCache = state(1537).useRouteCache(state.routes);
  ...
  function render(route, navigation, options, routeState) {
    ...
    const tmp5 = onAction(state(1573).SceneView, {
      navigation,
      route,
      screen: props,
      routeState,
    ...
    const obj3 = { value, children: onAction(state(1526).NavigationProvider, { route, navigation, children: layoutResult }) };
    return onAction(state(1515).NavigationBuilderContext.Provider, obj3, route.key);
  }
