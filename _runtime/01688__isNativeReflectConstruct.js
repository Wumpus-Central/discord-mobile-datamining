// _runtime/01688__isNativeReflectConstruct.js
import _modDef38 from "metro/00038__.js";
import isJest2 from "01656_isJest.js";
import JSPropsUpdaterPaperDefault from "01748_JSPropsUpdaterPaper.js";
import isInlineStyleTransform from "01749_isInlineStyleTransform.js";
import dummyListener from "01757_dummyListener.js";
import setAndForwardRefDefault from "01759_setAndForwardRef.js";
import updateLayoutAnimations from "01767_updateLayoutAnimations.js";
...
import startObservingProgress from "01689_startObservingProgress.js";
...
export const createAnimatedComponent = function createAnimatedComponent(fn) {
  ...
  let items = [
    {
      ...
      value: function componentDidMount() {
        ...
        const self = this;
        if (!closure_1_13) {
          const nativeEventsManager = new AnimatedComponent(1760).NativeEventsManager(self, closure_1);
        ...
        let isFabricResult = AnimatedComponent(1656).isFabric();
        if (isFabricResult) {
          isFabricResult = -1 !== componentViewTag;
        }
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp17(1762).PropsRegistryGarbageCollector;
          PropsRegistryGarbageCollector.registerView(componentViewTag, self);
        }
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp17(1756).ComponentRegistry;
          ComponentRegistry.register(componentViewTag, self);
        }
        const result2 = self._configureLayoutAnimation(tmp17(1678).LayoutAnimationType.LAYOUT, self.props.layout);
        const result3 = self._configureLayoutAnimation(tmp17(1678).LayoutAnimationType.EXITING, self.props.exiting);
        let tmp25 = tmp17;
        if (closure_1_13) {
          tmp25 = tmp17;
          if (self._componentDOMRef) {
            ...
            if (self.props.exiting) {
              AnimatedComponent(1707).saveSnapshot(_componentDOMRef);
              const obj2 = AnimatedComponent(1707);
            }
            if (self.props.entering) {
              if (obj3.getReducedMotionFromConfig(self.props.entering)) {
                ...
                if (current) {
                  ...
                  const result4 = tmp32(1707).startWebLayoutAnimation(self.props, _componentDOMRef, tmp32(1678).LayoutAnimationType.ENTERING);
                  tmp25 = tmp32;
                  const tmp32Result = tmp32(1707);
                }
              }
              obj3 = AnimatedComponent(1707);
            } else {
              self._isFirstRender = false;
            }
          }
    ...
    },
    {
      ...
      value: function componentWillUnmount() {
        ...
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp4(1762).PropsRegistryGarbageCollector;
        ...
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp4(1756).ComponentRegistry;
          ComponentRegistry.unregister(componentViewTag);
        }
        if (closure_13) {
          if (self._componentDOMRef) {
            if (exiting) {
              let tmp4Result = tmp4(1707);
              if (tmp4Result.getReducedMotionFromConfig(exiting)) {
                ...
                tmp4Result = tmp4(1740);
                const result2 = tmp4Result.addHTMLMutationObserver();
                ({ props, _componentDOMRef } = self);
                const result3 = tmp4(1707).startWebLayoutAnimation(props, _componentDOMRef, tmp4(1678).LayoutAnimationType.EXITING);
                const tmp4Result1 = tmp4(1707);
              }
            }
            const _viewInfo = self._viewInfo;
            let isFabricResult1 = !closure_16;
            if (!closure_16) {
              isFabricResult1 = tmp4(1656).isFabric();
              const tmp4Result2 = tmp4(1656);
            }
            if (isFabricResult1) {
              isFabricResult1 = shadowNodeWrapper;
            }
            if (isFabricResult1) {
              tmp4(1697).markNodeAsRemovable(shadowNodeWrapper);
              const tmp4Result3 = tmp4(1697);
            }
            ...
            if (!tmp4Result4.isFabric()) {
              if ("getReduceMotion" in exiting) {
                if (typeof exiting.getReduceMotion === "function") {
                  let reduceMotionFromConfig = tmp4(1693).getReduceMotionFromConfig(exiting.getReduceMotion());
                  const tmp4Result5 = tmp4(1693);
                }
                if (!reduceMotionFromConfig) {
                  const result4 = self._configureLayoutAnimation(tmp4(1678).LayoutAnimationType.EXITING, exiting);
                }
              }
              reduceMotionFromConfig = tmp4(1693).getReduceMotionFromConfig();
              const tmp4Result6 = tmp4(1693);
            }
            tmp4Result4 = tmp4(1656);
          }
        }
      }
    },
          ...
          if (closure_16) {
            ...
            if (findHostInstanceResult) {
              ...
              if (tmpResult.isFabric()) {
                shadowNodeWrapperFromRef = tmp(1681).getShadowNodeWrapperFromRef(self, findHostInstanceResult);
                const tmpResult1 = tmp(1681);
              }
              _componentDOMRef = null;
            } else {
              const reanimatedError = new tmp(1664).ReanimatedError("Cannot find host instance for this component. Maybe it renders nothing?");
    ...
    },
    {
      ...
      value: function componentDidUpdate(layout) {
        const self = this;
        const result = this._configureLayoutAnimation(AnimatedComponent(1678).LayoutAnimationType.LAYOUT, this.props.layout, layout.layout);
        const result1 = this._configureLayoutAnimation(AnimatedComponent(1678).LayoutAnimationType.EXITING, this.props.exiting, layout.exiting);
        if (!tmp5) {
          ...
          if (arg2) {
            if (self.props.layout) {
              ...
              if (tmpResult.getReducedMotionFromConfig(self.props.layout)) {
                ...
                const result4 = tmp(1707).tryActivateLayoutTransition(self.props, self._componentDOMRef, arg2);
                const tmpResult1 = tmp(1707);
              }
            }
          }
        }
      ...
      value: function _configureLayoutAnimation(EXITING, exiting, exiting2) {
        if (!isJest) {
          if (exiting !== exiting2) {
            ...
            if (obj2.isFabric()) {
              if (EXITING === tmp5(1678).LayoutAnimationType.ENTERING) {
                let reanimatedID = self.reanimatedID;
              }
              let maybeBuildResult = tmp3;
              if (tmp3) {
                let tmp8;
                if (EXITING !== tmp5(1678).LayoutAnimationType.LAYOUT) {
                ...
                maybeBuildResult = tmp5(1768).maybeBuild(tmp3, tmp8, AnimatedComponent.displayName);
                const tmp5Result = tmp5(1768);
              }
      ...
      value: function _configureSharedTransition(flag) {
        ...
        if (!closure_13) {
          ...
          if (sharedTransitionTag) {
            ...
            if (_sharedElementTransition2 == null) {
              _sharedElementTransition2 = new AnimatedComponent(1769).SharedTransition();
            }
        ...
        if (getReduceMotion) {
          if ("getReduceMotion" in getReduceMotion) {
            if (typeof getReduceMotion.getReduceMotion === "function") {
              let reduceMotionFromConfig = AnimatedComponent(1693).getReduceMotionFromConfig(getReduceMotion.getReduceMotion());
              const obj2 = AnimatedComponent(1693);
            }
            return reduceMotionFromConfig;
          }
        }
        reduceMotionFromConfig = AnimatedComponent(1693).getReduceMotionFromConfig();
      }
    },
    {
    ...
    },
    {
      ...
      value: function render() {
        ...
        if (tmp3) {
          let obj = AnimatedComponent(1707);
        ...
        if (!tmp3) {
          ...
          if (!current) {
            let obj3 = AnimatedComponent(1656);
            if (obj3.isFabric()) {
  ...
  dependencyMap = tmp9;
  tmp9.contextType = AnimatedComponent(1791).SkipEnteringContext;
  tmp9.displayName = "AnimatedComponent(" + fn.displayName || fn.name || "Component" + ")";
  const componentWithRefResult = AnimatedComponent(1792).componentWithRef((arg0, arg1) => {
    let obj = {};
