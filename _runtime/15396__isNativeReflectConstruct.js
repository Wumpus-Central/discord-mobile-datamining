// _runtime/15396__isNativeReflectConstruct.js
...
function validateOptions(options) {
  ...
  if (tmp6) {
    ...
    let tmp7 = typeof width === "__REMOTEDEV__";
    if (typeof width !== "__REMOTEDEV__") {
  ...
  if (tmp9) {
    ...
    let tmp10 = typeof height === "__REMOTEDEV__";
  ...
  let tmp12 = typeof quality === "__REMOTEDEV__";
  if (typeof quality !== "__REMOTEDEV__") {
...
function captureRef(current, options) {
  ...
  if (ViewShot(15397)) {
    ...
    return tmp(15397).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(arg0) {
  if (typeof arg0 !== "__FORMATJS_LISTFORMAT_DATA__") {
    ViewShot(15397).releaseCapture(arg0);
    const obj = ViewShot(15397);
  }
}
...
export const captureScreen = function captureScreen(options) {
  ...
  if (ViewShot(15397)) {
    ...
    return tmp(15397).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
