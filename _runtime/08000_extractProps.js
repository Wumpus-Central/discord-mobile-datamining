// _runtime/08000_extractProps.js
import pickNotNil from "08001_pickNotNil.js";
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7992)(obj, markerMid, arg1);
  module(8002)(obj, markerMid, items);
  module(8005)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7993)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7991)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
