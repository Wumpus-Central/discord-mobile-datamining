// _runtime/01739_findDescendantWithExitingAnimation.js
import isJest from "01655_isJest.js";
import addLogBoxLog from "01656_addLogBoxLog.js";
  ...
  if (isDummy instanceof globalThis.HTMLElement) {
    ...
    if (isDummy) {
      ...
      if (value) {
        ...
        const tmp2Result = tmp2(1738);
      } else {
        const logger = tmp2(1656).logger;
  ...
  const timerId = setTimeout(
    () => {
      if (obj.isWindowAvailable()) {
        ...
        if (undefined === sum) {
          const reanimatedError = new animationName(1663).ReanimatedError("Failed to obtain animation index.");
          throw reanimatedError;
        } else {
          dependencyMap();
          const sheet = element.sheet;
          ...
          if (sum < closure_1_5.length) {
            ...
            const reanimatedError1 = new animationName(1663).ReanimatedError("Failed to obtain animation index.");
