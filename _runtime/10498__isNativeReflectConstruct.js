// _runtime/10498__isNativeReflectConstruct.js
...
import parseNumberPattern from "10492_parseNumberPattern.js";
...
const regExp = new RegExp(
  "(?:am\\s*?)?(?:den\\s*?)?([0-9]{1,2})\\.(?:\\s*(?:bis(?:\\s*(?:am|zum))?|\\-|\\\u2013|\\s)\\s*([0-9]{1,2})\\.?)?\\s*(" +
    repeatedTimeunitPattern.matchAnyPattern(parseNumberPattern.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)(" +
    parseNumberPattern.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
...
const items = [
  ...
  },
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = DEMonthNameLittleEndianParser(10492).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10492).parseYear(index[4]));
        } else {
