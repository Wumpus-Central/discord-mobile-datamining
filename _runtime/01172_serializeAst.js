// _runtime/01172_serializeAst.js
...
function serializeAst(ast, value) {
  ...
  while (iter !== undefined) {
    ...
    if (typeof nextResult !== "string") {
      let first = tmp2[0];
      if (InternalIntlMessage(1158).FormatJsNodeType.Argument === first) {
        value.value = `${value.value}{${tmp2[1]}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Date === first) {
        value.value = `${value.value}{${tmp2[1]}, date`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Time === first) {
        value.value = `${value.value}{${tmp2[1]}, time`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Number === first) {
        value.value = `${value.value}{${tmp2[1]}, number`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Plural === first) {
        let str = "plural";
        if ("ordinal" == tmp2[4]) {
          str = "selectordinal";
        }
        ...
        value.value = `${value.value}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Pound === first) {
        value.value = `${value.value}#`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Select === first) {
        ...
        value.value = `${value.value}}`;
      } else if (InternalIntlMessage(1158).FormatJsNodeType.Tag === first) {
