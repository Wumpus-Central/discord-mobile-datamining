// _runtime/01722_addPxToTransform.js
import _objectWithoutProperties from "_objectWithoutProperties";
import _slicedToArray from "_slicedToArray";
import { TransitionType } from "01690_TransitionType.js";
import { convertAnimationObjectToKeyframes } from "01692_convertAnimationObjectToKeyframes.js";
import { findDescendantWithExitingAnimation } from "01721_findDescendantWithExitingAnimation.js";
...
export const TransitionGenerator = function TransitionGenerator(ENTRY_EXIT, easingY) {
  ...
  if (TransitionType.TransitionType.LINEAR === ENTRY_EXIT) {
    let tmp3Result = tmp3(1723);
    firstKeyframeObj = tmp3Result.LinearTransition(`REA${tmp}`, easingY);
  } else if (tmp3(1690).TransitionType.SEQUENCED === ENTRY_EXIT) {
    tmp3Result = tmp3(1724);
    firstKeyframeObj = tmp3Result.SequencedTransition(`REA${tmp}`, easingY);
  } else if (tmp3(1690).TransitionType.FADING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1725).FadingTransition(`REA${tmp}`, easingY);
    const tmp3Result1 = tmp3(1725);
  } else if (tmp3(1690).TransitionType.JUMPING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1726).JumpingTransition(`REA${tmp}`, easingY);
    const tmp3Result2 = tmp3(1726);
  } else if (tmp3(1690).TransitionType.CURVED === ENTRY_EXIT) {
    closure_6 = tmp7 + 1;
    const text1 = `REA${tmp7}`;
    const tmp3Result3 = tmp3(1727);
    ({ firstKeyframeObj, secondKeyframeObj } = tmp3(1727).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY));
    const CurvedTransitionResult = tmp3(1727).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY);
    const result = tmp3(1692).convertAnimationObjectToKeyframes(secondKeyframeObj);
    const tmp3Result4 = tmp3(1692);
    tmp3(1721).insertWebAnimation(`REA${+closure_6}`, result);
    const dummyTransitionKeyframeName = text1;
    const tmp3Result5 = tmp3(1721);
  } else if (tmp3(1690).TransitionType.ENTRY_EXIT === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1728).EntryExitTransition(`REA${tmp}`, easingY);
    const tmp3Result6 = tmp3(1728);
  }
