// _runtime/12932_spanTimeInputToSeconds.js
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod12944.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== _mod12944.SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(parentSpan, sentrySpan) {
  ...
  const result = _mod12933.addNonEnumerableProperty(sentrySpan, _sentryRootSpan, tmp2);
  if (parentSpan[_sentryChildSpans]) {
    ...
    const result1 = _mod12933.addNonEnumerableProperty(parentSpan, _sentryChildSpans, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(_mod12954.getCurrentScope());
...
export const updateMetricSummaryOnActiveSpan = function updateMetricSummaryOnActiveSpan(
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const tmpResult = _mod12949;
    activeSpan = tmpResult._getSpanForScope(_mod12954.getCurrentScope());
    const tmpResult3 = _mod12954;
  }
  if (activeSpan) {
    const tmpResult4 = _mod12943;
