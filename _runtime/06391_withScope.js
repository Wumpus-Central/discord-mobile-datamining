// _runtime/06391_withScope.js
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(6389).getMainCarrier();
  const obj = AsyncContextStack(6389);
  const sentryCarrier = AsyncContextStack(6389).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(6397);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(6389).getMainCarrier();
      let obj = callback(6389);
      let sentryCarrier = callback(6389).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6397);
    ...
    getCurrentScope() {
      const mainCarrier = callback(6389).getMainCarrier();
      const obj = callback(6389);
      const sentryCarrier = callback(6389).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6397);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
