// === Module 6092: convertToHandlerTag ===
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod6089.BaseGesture) {
...
let items = [...fn(6093).baseGestureHandlerWithDetectorProps, ...fn(6094).tapGestureHandlerProps, ...fn(6104).panGestureHandlerProps, ...fn(6104).panGestureHandlerCustomNativeProps, ...fn(6105).longPressGestureHandlerProps, ...fn(6106).forceTouchGestureHandlerProps, ...fn(6108).flingGestureHandlerProps, ...fn(6109).hoverGestureHandlerProps, ...fn(6110).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6071.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
