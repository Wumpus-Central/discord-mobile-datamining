...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9201).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9202).crypto) {
    if (typeof tmp(9202).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9202).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9202).crypto) {
    if (typeof tmp(9202).crypto.randomBytes === "function") {
      const _crypto = tmp(9202).crypto;
