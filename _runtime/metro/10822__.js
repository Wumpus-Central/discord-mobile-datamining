...
const entry = {
  ...
  value: function innerPatternString(arg0) {
    return (
      "(?:\u0441)?\\s*(" +
      RUMonthNameLittleEndianParser(10821).ORDINAL_NUMBER_PATTERN +
      ")(?:\\s{0,3}(?:\u043F\u043E|-|\u2013|\u0434\u043E)?\\s{0,3}(" +
      RUMonthNameLittleEndianParser(10821).ORDINAL_NUMBER_PATTERN +
      "))?(?:-|\\/|\\s{0,3}(?:of)?\\s{0,3})(" +
      RUMonthNameLittleEndianParser(10692).matchAnyPattern(RUMonthNameLittleEndianParser(10821).MONTH_DICTIONARY) +
      ")(?:(?:-|\\/|,?\\s{0,3})(" +
      RUMonthNameLittleEndianParser(10821).YEAR_PATTERN +
      "(?![^\\s]\\d)))?"
    );
  },
};
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", RUMonthNameLittleEndianParser(10821).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            RUMonthNameLittleEndianParser(10693).findYearClosestToRef(createParsingResult.refDate, result, tmp4),
          );
        }
        if (index[2]) {
          const start3 = parsingResult.start;
          const result1 = RUMonthNameLittleEndianParser(10821).parseOrdinalNumberPattern(index[2]);
