...
import PanGestureHandler from "../06421_PanGestureHandler.js";
import _mod6435 from "06435__.js";
import _mod6436 from "06436__.js";
...
export const Card = function Card(shadowEnabled) {
  ...
  const first1 = interpolationIndex(
    opening.useState(() => {
      value = new RN.Value(_mod6436.getInvertedMultiplier(gestureDirection, "rtl" === direction));
      return value;
    }),
    1,
  )[0];
  ...
  closure_32 = num(1506)(() => {
    if (null == ref3.current) {
      const InteractionManager = _mod6442.InteractionManager;
  ...
  const layoutEffect = opening.useLayoutEffect(() => {
    ...
    first1.setValue(_mod6436.getInvertedMultiplier(gestureDirection, "rtl" === direction));
  }, items);
  opening.useRef(null);
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (ref7.current.closing) {
            let num2 = _mod6435.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (closing) {
          let num3 = _mod6435.getDistanceForDirection(layout, gestureDirection, "rtl" === direction);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod6435.getDistanceForDirection(layout, gestureDirection, "rtl" === direction));
              const obj4 = { closing };
              closure_34(obj4);
            }
          }
  ...
  if (typeof backgroundColor === "string") {
    tmp16 = 0 === tmp5(5947)(backgroundColor).alpha();
    const obj14 = tmp5(5947)(backgroundColor);
  }
  ...
  let merged = Object.assign(
    flag2(6444).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }),
  );
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(6445).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(6421).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(6443).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
  const tmp18Result = flag2(6444);
};
