...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(6883).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  const bottomSheetInternal = scrollableGesture(6876).useBottomSheetInternal();
  ...
  const obj = scrollableGesture(6876);
  const fn = function f() {
    return { enabled: iter.value === value2.SCROLLABLE_STATE.UNLOCKED };
  };
  const tmp2Result = scrollableGesture(1637);
  fn.__closure = { animatedScrollableState: iter, SCROLLABLE_STATE: scrollableGesture(6872).SCROLLABLE_STATE };
  ...
  if (tmp7) {
    ...
    let tmp8Result = jsx(scrollableGesture(6899).GestureDetector, { gesture: tmp7, children: null });
  } else {
    const obj5 = {};
