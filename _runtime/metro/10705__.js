...
const entry = {
  ...
  value: function innerPatternString(arg0) {
    return (
      "((?:\u0432)\\s*)?(" +
      RUMonthNameParser(10573).matchAnyPattern(RUMonthNameParser(10702).MONTH_DICTIONARY) +
      ")\\s*(?:[,-]?\\s*(" +
      RUMonthNameParser(10702).YEAR_PATTERN +
      ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)"
    );
  },
};
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start4.assign("year", RUMonthNameParser(10702).parseYear(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply("year", RUMonthNameParser(10574).findYearClosestToRef(createParsingResult.refDate, 1, tmp9));
      }
