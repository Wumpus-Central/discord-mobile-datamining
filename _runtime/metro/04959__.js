...
arg5.default = {
  read(getUint8, sum) {
    ...
    const sum2 = sum1 + 65536 * _modDef4927.getByteAt(getUint8, sum + 2) + 1;
    obj.ImageWidth = { value: sum2, description: `${tmp9}px` };
    const sum3 = sum + c3;
    const tmpResult1 = _modDef4927;
    const byteAt2 = _modDef4927.getByteAt(getUint8, sum3);
    const tmpResult2 = _modDef4927;
    const sum4 = byteAt2 + 256 * _modDef4927.getByteAt(getUint8, sum3 + 1);
    const tmpResult3 = _modDef4927;
    const sum5 = sum4 + 65536 * _modDef4927.getByteAt(getUint8, sum3 + 2) + 1;
