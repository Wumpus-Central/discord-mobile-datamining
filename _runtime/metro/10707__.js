  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = ENMergeRelativeFollowByDateRefiner(10677).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeFollowByDateRefiner(10678).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeFollowByDateRefiner(10678).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10678).ParsingResult(
        start.reference,
        text.index,
