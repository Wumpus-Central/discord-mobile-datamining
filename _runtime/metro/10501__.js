...
const regExp = new RegExp(
  "(?:on\\s*?)?(" +
    _mod10499.ORDINAL_NUMBER_PATTERN +
    ")(?:\\s*(?:au|\\-|\\\u2013|jusqu'au?|\\s)\\s*(" +
    _mod10499.ORDINAL_NUMBER_PATTERN +
    "))?(?:-|/|\\s*(?:de)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10499.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)(" +
    _mod10499.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start.imply(
            "year",
            FRMonthNameLittleEndianParser(10438).findYearClosestToRef(createParsingResult.refDate, result, tmp4),
          );
        }
        if (index[2]) {
