...
import _mod10949 from "10949__.js";
...
const regExp = new RegExp(
  "((?:in)\\s*)?(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10949.MONTH_DICTIONARY) +
    ")\\s*(?:[,-]?\\s*(" +
    _mod10949.YEAR_PATTERN +
    ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)",
  "i",
);
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start4.assign("year", ENMonthNameParser(10949).parseYear(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply("year", ENMonthNameParser(10781).findYearClosestToRef(createParsingResult.refDate, 1, tmp9));
      }
