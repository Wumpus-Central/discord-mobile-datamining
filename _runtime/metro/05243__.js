...
import styleDefault from "style" /* 5245 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, arg1) {
  ...
  dependencyMap = flag5.useContext(screenId(5241).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5240(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5231);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = screenId(5244).isIOS26OrHigher;
  }
  _modDef5240(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5244).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(screenId(5248).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5231).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5230).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(screenId(5250).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5221, obj8);
});