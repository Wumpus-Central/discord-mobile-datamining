...
function SceneView(arg0) {
  ...
  const context = noop.useContext(num6(5848).HeaderShownContext);
  let num5 = noop.useContext(num6(5848).HeaderHeightContext);
  const context1 = noop.useContext(num6(5848).HeaderBackContext);
  const obj2 = num6(1615);
  const frameSize = num6(5848).useFrameSize((width) => width.width > width.height);
  num6 = 0;
  if (!context) {
    let top;
  ...
  const obj4 = num6(5848);
  const frameSize1 = num6(5848).useFrameSize((arg0) => 56 + num6);
  const tmp8Result = num6(5848);
  let num7 = 2;
  const tmp8Result5 = num6(1485);
  [tmp17, tmp18] = noop.useState(frameSize1);
  dependencyMap = tmp18;
  const tmp16 = _slicedToArray(noop.useState(frameSize1), 2);
  const callback = obj3.useCallback(num6(8160).debounce(tmp18, 100), []);
  ...
  if (previousDescriptor) {
    title = tmp8(5848).getHeaderTitle(previousDescriptor.options, previousDescriptor.route.name);
    const tmp8Result7 = tmp8(5848);
  ...
  let eventResult;
  const headerConfigProps = num6(8161).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  const tmp8Result8 = num6(8161);
  obj20.children = closure_10(num6(5848).HeaderBackContext.Provider, { value: memo1, children: descriptor.render() });
  items5[2] = closure_10(num6(5848).HeaderShownContext.Provider, obj20);
  obj13.children = items5;
  obj12.children = closure_11(num6(5848).HeaderHeightContext.Provider, obj13);
  obj10.children = closure_10(num6(8163).AnimatedHeaderHeightContext.Provider, obj12);
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_8164").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_8164");
  const invalidPreventRemoveError = require("module_8165").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_8165");
  const modalRouteKeys = require("module_8166").getModalRouteKeys(state.routes, descriptors);
  ...
  return closure_10(require("module_5848").SafeAreaProviderCompat, obj4);
};