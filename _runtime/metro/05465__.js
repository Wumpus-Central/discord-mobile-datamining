...
import _modDef5466 from "05466__.js";
import calculateGPSValueDefault from "../05468_calculateGPSValue.js";
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5430).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5430).objectAssign(tmp3, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5430);
        }
        tmp3 = obj;
      });
      continue;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5430).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5430);
  return {
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5430).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5430);
    const obj6 = {};
    ...
    ParseError(5430).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5430);
  }
  ParseError(5430).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
