...
const items = [
  entry,
  {
    ...
    value: function mergeResults(arg0, start, text, arg3) {
      ...
      if (null != str.match(/^-/i)) {
        reverseDurationResult = ENMergeRelativeAfterDateRefiner(10523).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeAfterDateRefiner(10524).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeAfterDateRefiner(10524).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeAfterDateRefiner(10524).ParsingResult(
        reference,
        index,
