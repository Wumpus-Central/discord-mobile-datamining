// _runtime/metro/07881__addTracingHeadersToFetchRequest.js
import errorCallback from "../07772_errorCallback.js";
import SyncPromise from "../07800_SyncPromise.js";
import hasTracingEnabled from "../07808_hasTracingEnabled.js";
import getTraceData from "../07847_getTraceData.js";
import closure_2 from "../00853__toArray.js";
import __SENTRY_DEBUG__ from "07775___SENTRY_DEBUG__.js";
import consoleSandbox from "../07776_consoleSandbox.js";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7793).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7793);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7793).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7858).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7858);
    }
    const activeSpan = setHttpStatus(7781).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7791).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7791).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7810).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7810);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7808);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7809).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7781);
  }
};