...
import _modDef8907 from "08907__.js";
...
class TSpan {
  constructor() {
    ...
    tmp3Result.setNativeProps = (matrix) => {
      ...
      const propsAndStylesResult = TSpan(8822).propsAndStyles(matrix);
      const obj = TSpan(8822);
      const merged = Object.assign(
        propsAndStylesResult,
        TSpan(8823).pickNotNil(extractFontDefault(propsAndStylesResult, false)),
      );
      if (closure_0.root) {
        const root = closure_0.root;
        root.setNativeProps(propsAndStylesResult);
      }
      const obj2 = TSpan(8823);
    };
    return tmp3Result;
  }
}
_inherits(TSpan, _modDef8831);
const entry = {
  key: "render",
  value: function render() {
    const propsAndStylesResult = TSpan(8822).propsAndStyles(this.props);
    const obj2 = {};
    const obj = TSpan(8822);
