...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[0].length <= 3) {
        if (!ENMonthNameParser(10861).FULL_MONTH_NAME_DICTIONARY[formatted]) {
          return null;
        }
      }
      let str2 = index[1];
      ...
      start.imply("day", 1);
      const tmp9 = ENMonthNameParser(10861).MONTH_DICTIONARY[formatted];
      const start2 = parsingResult.start;
      start2.assign("month", tmp9);
      if (index[3]) {
        const start4 = parsingResult.start;
        start4.assign("year", ENMonthNameParser(10861).parseYear(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply("year", ENMonthNameParser(10693).findYearClosestToRef(createParsingResult.refDate, 1, tmp9));
      }
