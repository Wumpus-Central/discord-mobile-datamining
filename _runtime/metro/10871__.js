...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod10868.MONTH_DICTIONARY) + ")\\s*(?:[,-]?\\s*(" + _mod10868.YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, arg1) {
      ...
      const tmp4 = NLMonthNameParser(10868).MONTH_DICTIONARY[arg1[1].toLowerCase(arg1[1])];
      parsingComponents.assign("month", tmp4);
      if (arg1[2]) {
        parsingComponents.assign("year", NLMonthNameParser(10868).parseYear(arg1[2]));
      } else {
        parsingComponents.imply("year", NLMonthNameParser(10775).findYearClosestToRef(createParsingComponents.refDate, 1, tmp4));
      }
