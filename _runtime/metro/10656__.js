...
const regExp = new RegExp(
  "(?:(?:\\,|\\(|\\\uFF08)\\s*)?(?:op\\s*?)?(?:(deze|vorige|volgende)\\s*(?:week\\s*)?)?(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10655.WEEKDAY_DICTIONARY) +
    ")(?=\\W|$)",
  "i",
);
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      ...
      return NLWeekdayParser(10588).createParsingComponentsAtWeekday(
        reference.reference,
        NLWeekdayParser(10655).WEEKDAY_DICTIONARY[formatted],
        str3,
      );
    },
  },
