...
import iccTags from "../05336_iccTags.js";
...
export default {
  read(buffer, arr, arg2) {
    if (arg2) {
      if (arr[0].compressionMethod !== _mod5294.COMPRESSION_METHOD_NONE) {
        if (arr[0].compressionMethod === _mod5294.COMPRESSION_METHOD_DEFLATE) {
          let _DataView = DataView;
          buffer = buffer.buffer;
          let dataView = new DataView(buffer.slice(arr[0].offset, arr[0].offset + arr[0].length));
          const tmpResult = _mod5294;
