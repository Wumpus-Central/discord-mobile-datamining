          ...
          if (hasItem) {
            ...
            tmpResult.withScope((addEventProcessor) => {
              ...
              if ("assert" !== level) {
                ...
                if (found) {
                  let tmp14Result = args(12875);
                  tmp14Result.captureException(found, obj);
                } else {
                  tmp14Result = args(12836);
                  const safeJoinResult = tmp14Result.safeJoin(args, " ");
                  args(12875).captureMessage(safeJoinResult, obj);
                ...
                args(12875).captureMessage(combined, obj);
                const obj2 = args(12875);
              }
            });
          }
        });
