...
function handleIntersectionResults(nextPromise, runResult, runResult1) {
  ...
  for (const item10054 of tmp14) {
    if ("unrecognized_keys" === item10054.code) {
      ...
      let arr2 = issues1.push(item10054);
    }
    continue;
  }
  const items = [...map];
...
function handleMapResult(issues, issues2, issues3, key, value, inst, arg6) {
  ...
  if (issues.issues.length) {
    ...
    if (propertyKeyTypes.has(typeof key)) {
      const items = [];
      HermesBuiltin.arraySpread(closure_10.prefixIssues(key, issues.issues), 0);
      HermesBuiltin.apply(items, issues);
    } else {
...
if (self2) {
  ...
  if (__setModuleDefault) {
    ...
    let closure_7 = fn(_mod9295);
    let fnResult = fn(_mod9290);
    let closure_9 = fn(_mod9296);
    let closure_10 = fn(require("09293__.js"));
      ...
      closure_10.defineLazy(tmp, "~standard", () => ({
        validate(arg0) {
          try {
            ...
            return obj(runChecks[7])
              .safeParseAsync(closure_1_0, tmp2)
              .then((success) => {
                if (success.success) {
        ...
        if (closure_2) {
          ...
          if (flag) {
            let nextPromise = Promise.all(items).then((result) =>
              handleUnionResults(result, closure_0, closure_0, closure_1),
            );
            const allPromises = Promise.all(items);
          } else {
        ...
        if (object.isObject(value)) {
          ...
          if (value4) {
            ...
            const obj2 = {
              ...
              discriminator: closure_1.discriminator,
              input: value,
              path: null,
              inst: null,
            };
                ...
                if (runResult instanceof Promise) {
                  ...
                  if (iter.issues.length) {
                    if ("loose" === closure_1.mode) {
                      ...
                      const issues1 = iter.issues;
                      obj3.issues = issues1.map((item) =>
                        closure_3_10.finalizeIssue(item, runResult1, closure_3_8.config()),
                      );
                      obj3.input = iter;
                      items = [iter];
                      obj3.path = items;
      ...
      _zod._zod.parse = (value, arg1) => {
        if (typeof value.value !== "string") {
          ...
          if (!pattern.test(value.value)) {
            const issues1 = value.issues;
            const obj = { input: value.value, inst, code: "invalid_format", format: null, pattern: null };
      ...
      _zod.implement = (fn) => {
        ...
        if (typeof fn !== "function") {
          ...
          return function () {
            ...
            let parsed = items;
            if (_def._def.input) {
              parsed = _mod9291.parse(_def._def.input, items);
            }
            const applyResult = Reflect.apply(_def, this, parsed);
            let parsed1 = applyResult;
            if (_def._def.output) {
              parsed1 = _mod9291.parse(_def._def.output, applyResult);
            }
            return parsed1;
          };
        }
            ...
            const iter = (async (arg0) => {
              if (1 === tmp5) {
                if (arg0 === 1) {
                  ...
                  c5 = 2;
                  c6 = 1;
                  return { value: _self(9291).parseAsync(self._def.input, closure_130_0), done: false };
                } else {
                ...
                if (3 === tmp5) {
                  if (arg0 === 1) {
                    ...
                    if (self._def.output) {
                      c5 = 4;
                      c6 = 1;
                      return { value: _self(9291).parseAsync(self._def.output, closure_130_2), done: false };
                    } else {
