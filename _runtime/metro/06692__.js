...
export const GestureDetector = (gesture) => {
  if (gesture.gesture) {
    ...
    const isomorphicLayoutEffect = gesture(webEventHandlers[7]).useIsomorphicLayoutEffect(() => {
      ...
      gesturesToAttach = { preparedGesture: current2, gestureConfig: gesture, gesturesToAttach, webEventHandlersRef: webEventHandlers, viewTag: _modDef6704(current.viewRef) };
    ...
    if (someResult) {
      ...
      let tmp28Result = <tmp9Result3.AnimatedWrap ref={viewRefHandler} onGestureHandlerEvent={current2.animatedEventHandler}>{gesture.children}</tmp9Result3.AnimatedWrap>;
    } else {
