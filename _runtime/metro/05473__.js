...
import iccTags from "../05474_iccTags.js";
          ...
          while (buffer.length >= num6 + c8) {
            ...
            if (uint321 > buffer.length) {
              ...
              if (stringFromDataView1 === desc) {
                ...
                if (0 < uint324) {
                  do {
                    ...
                    let obj5 = {
                      ...
                      text: obj8.getUnicodeStringFromDataView(buffer, uint321 + uint326, uint325),
                    };
                ...
                if (1 === uint324) {
                  text = items[0].text;
                  if (tmp25(5474).iccTags[stringFromDataView]) {
                    let obj9 = { value: text, description: text };
                    obj2[tmp25(5474).iccTags[stringFromDataView].name] = obj9;
                  } else {
...
export default {
  read(buffer, arr, arg2) {
    if (arg2) {
      if (arr[0].compressionMethod !== _mod5432.COMPRESSION_METHOD_NONE) {
        if (arr[0].compressionMethod === _mod5432.COMPRESSION_METHOD_DEFLATE) {
          let _DataView = DataView;
          buffer = buffer.buffer;
          let dataView = new DataView(buffer.slice(arr[0].offset, arr[0].offset + arr[0].length));
          const tmpResult = _mod5432;
