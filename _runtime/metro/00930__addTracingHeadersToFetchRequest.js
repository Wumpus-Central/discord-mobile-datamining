// _runtime/metro/00930__addTracingHeadersToFetchRequest.js
import getClient from "../00845_getClient.js";
import hasSpansEnabled from "../00852_hasSpansEnabled.js";
import getTraceData from "../00901_getTraceData.js";
...
arg5.instrumentFetchRequest = function instrumentFetchRequest(fetchData, arg1, arg2, arg3, obj) {
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (fetchData.response) {
              ...
              obj[0] = tmp3(837).SPAN_STATUS_ERROR;
              obj20.setStatus(obj);
            }
            obj20.end();
            let onRequestSpanEnd;
    ...
    if (hasSpansEnabledResult) {
      if (activeSpan) {
        const tmp3Result1 = tmp3(863);
        let tmp3Result2 = tmp3(897);
        if (startsWithResult) {
          ...
          tmp3Result2 = tmp3(897).stripDataUrlContent(url);
          obj3[0] = tmp3Result2;
          obj3[2] = method;
          obj3[tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
          str = tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_OP;
          method = "http.client";
          obj3[str] = "http.client";
          obj2[1] = obj3;
          let obj4 = obj2;
          const tmp3Result3 = tmp3(897);
        } else {
          const result = tmp3Result2.parseStringToURLObject(url);
          let sanitizedUrlStringFromUrlObject = url;
          if (result) {
            sanitizedUrlStringFromUrlObject = tmp3(897).getSanitizedUrlStringFromUrlObject(result);
            const tmp3Result4 = tmp3(897);
          }
          ...
          obj5[0] = tmp3(897).stripDataUrlContent(url);
          obj5[2] = method;
          obj5[tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
          obj5[tmp3(836).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
          if (result) {
            if (!tmp3Result6.isURLObjectRelative(result)) {
              obj5["http.url"] = tmp3(897).stripDataUrlContent(result.href);
              obj5["server.address"] = result.host;
              const tmp3Result7 = tmp3(897);
            }
            if (result.search) {
            ...
            tmp3Result6 = tmp3(897);
          }
          obj4[1] = obj5;
          const tmp3Result5 = tmp3(897);
        }
        tmp3Result1.startInactiveSpan(obj4);
        startsWithResult = url.startsWith("data:");
      }
    }
    const sentryNonRecordingSpan = new tmp3(853).SentryNonRecordingSpan();
