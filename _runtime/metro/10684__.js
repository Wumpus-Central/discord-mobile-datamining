...
const regExp = new RegExp(
  "(dit|deze|(?:aan)?komend|volgend|afgelopen|vorig)e?\\s*(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10675.TIME_UNIT_DICTIONARY) +
    ")(?=\\s*)(?=\\W|$)",
  "i",
);
      ...
      if ("volgend" != formatted) {
        if ("komend" != formatted) {
          if ("aankomend" != formatted) {
            ...
            const obj = {};
            obj[tmp4] = -1;
            const ParsingComponents = NLRelativeDateFormatParser(10584).ParsingComponents;
            return ParsingComponents.createRelativeFromReference(createParsingComponents.reference, obj);
          }
        }
      }
      const ParsingComponents2 = NLRelativeDateFormatParser(10584).ParsingComponents;
