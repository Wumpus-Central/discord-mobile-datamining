...
const regExp = new RegExp(
  "(" +
    ...
    ")(?:-|/|\\s*,?\\s*)(" +
    _mod10674.ORDINAL_NUMBER_PATTERN +
    ")(?!\\s*(?:am|pm))\\s*(?:(?:to|\\-)\\s*(" +
    _mod10674.ORDINAL_NUMBER_PATTERN +
    ")\\s*)?(?:(?:-|/|\\s*,\\s*|\\s+)(" +
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          addTagResult.assign("year", ENMonthNameMiddleEndianParser(10674).parseYear(index[4]));
        } else {
          addTagResult.imply(
            "year",
            ENMonthNameMiddleEndianParser(10676).findYearClosestToRef(createParsingComponents.refDate, result, tmp3),
          );
        }
        if (index[3]) {
          const result1 = ENMonthNameMiddleEndianParser(10674).parseOrdinalNumberPattern(index[3]);
