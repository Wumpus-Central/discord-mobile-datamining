...
import _modDef5295 from "05295__.js";
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          obj.name = combined;
          obj.value = items;
          obj.description = getTagDescription(_modDef5295.iptc[uint16], items, obj, encoding);
          let tmp17 = _modDef5295.iptc[uint16] && _modDef5295.iptc[uint16].repeatable;
          if (tmp17) {
            obj.repeatable = true;
          }
          let tmp18 = _modDef5295.iptc[uint16] && undefined !== _modDef5295.iptc[uint16].encoding_name;
          if (tmp18) {
            let obj4 = _modDef5295.iptc[uint16];
