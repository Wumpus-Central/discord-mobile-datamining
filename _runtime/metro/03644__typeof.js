...
let items = [
  {
    ...
    value: function parse(arg0, arg1, ordinalNumber) {
      ...
      if ("Y" === arg1) {
        return _createSuperInternal(3642).mapValue(_createSuperInternal(3642).parseNDigits(4, arg0), valueCallback);
      } else if ("Yo" === arg1) {
        return _createSuperInternal(3642).mapValue(ordinalNumber.ordinalNumber(arg0, { unit: "year" }), valueCallback);
      } else {
        return _createSuperInternal(3642).mapValue(_createSuperInternal(3642).parseNDigits(arg1.length, arg0), valueCallback);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function set(setUTCFullYear, era, isTwoDigitYear, firstWeekContainsDate) {
      if (isTwoDigitYear.isTwoDigitYear) {
        setUTCFullYear.setUTCFullYear(_createSuperInternal(3642).normalizeTwoDigitYear(isTwoDigitYear.year, tmp), 0, firstWeekContainsDate.firstWeekContainsDate);
        setUTCFullYear.setUTCHours(0, 0, 0, 0);
        return tmp5.default(setUTCFullYear, firstWeekContainsDate);
      } else {
        if ("era" in era) {
          if (1 !== era.era) {
