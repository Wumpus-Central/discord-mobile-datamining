...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "string") {
    ...
    if (!_mod13051.GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const encoder = new TextEncoder();
      let encodeResult = encoder.encode(data);
    }
    __SENTRY__ = __SENTRY__(13051).GLOBAL_OBJ.__SENTRY__;
    ...
    const items = [_mod13056.dropUndefinedKeys(obj3), data1];
    return items;
  }
};
...
export const createEventEnvelopeHeaders = function createEventEnvelopeHeaders(event_id, sdk, arg2, url) {
  ...
  let tmp4 = arg2 && url;
  if (tmp4) {
    const obj3 = { dsn: _mod13097.dsnToString(url) };
    tmp4 = obj3;
  }
  ...
  if (event_id.sdkProcessingMetadata && event_id.sdkProcessingMetadata.dynamicSamplingContext) {
    ...
    obj4.trace = _mod13056.dropUndefinedKeys(obj6);
    tmp8 = obj4;
  }
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    function readJson() {
      ...
      if (_mod13051.GLOBAL_OBJ.__SENTRY__) {
        if (_mod13051.GLOBAL_OBJ.__SENTRY__.decodePolyfill) {
          const __SENTRY__ = _mod13051.GLOBAL_OBJ.__SENTRY__;
    ...
    let encodePolyfill = dependencyMap;
    if (!require("13051__.js").GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const encoder = new TextEncoder();
  ...
  function append(json) {
    if (typeof sum === "string") {
      if (typeof json === "string") {
        ...
        if (_mod13051.GLOBAL_OBJ.__SENTRY__) {
          if (_mod13051.GLOBAL_OBJ.__SENTRY__.encodePolyfill) {
            const __SENTRY__2 = _mod13051.GLOBAL_OBJ.__SENTRY__;
