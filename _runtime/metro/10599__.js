...
import _mod10572 from "10572__.js";
...
const regExp = new RegExp(
  "(?:(?:\\,|\\(|\\\uFF08)\\s*)?(?:on\\s*?)?(?:(this|last|past|next)\\s*)?(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10572.WEEKDAY_DICTIONARY) +
  ...
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      ...
      if (undefined !== ENWeekdayParser(10572).WEEKDAY_DICTIONARY[formatted1]) {
        let sum = ENWeekdayParser(10572).WEEKDAY_DICTIONARY[formatted1];
      } else if ("weekend" == formatted1) {
        if ("last" == str2) {
          let SATURDAY = ENWeekdayParser(10578).Weekday.SUNDAY;
        } else {
          SATURDAY = ENWeekdayParser(10578).Weekday.SATURDAY;
        }
        sum = SATURDAY;
        ...
        if (day != ENWeekdayParser(10578).Weekday.SUNDAY) {
          if (day != ENWeekdayParser(10578).Weekday.SATURDAY) {
            const diff = day - 1;
            sum = (("last" == str2 ? diff - 1 : diff + 1) % 5) + 1;
          }
        }
        if ("last" == str2) {
          let MONDAY = ENWeekdayParser(10578).Weekday.FRIDAY;
        } else {
          MONDAY = ENWeekdayParser(10578).Weekday.MONDAY;
        }
        sum = MONDAY;
      }
