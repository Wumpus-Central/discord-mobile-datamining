...
export default {
  read(buffer, sum) {
    ...
    let tmp12;
    if (9 <= shortAt) {
      const byteAt2 = _modDef5244.getByteAt(buffer, sum + 7);
      const tmp3Result1 = _modDef5244;
      const byteAt3 = _modDef5244.getByteAt(buffer, sum + 7 + 1);
      const obj1 = { value: 256 * byteAt2 + byteAt3, description: `${tmp13}.${tmp14}` };
      tmp12 = obj1;
      const tmp3Result2 = _modDef5244;
    }
    const obj2 = {
      "JFIF Version": tmp12,
    ...
    let tmp15;
    if (10 <= shortAt) {
      const byteAt4 = _modDef5244.getByteAt(buffer, sum + 9);
      ...
      tmp15 = obj3;
      const tmp3Result3 = _modDef5244;
    }
    obj2["Resolution Unit"] = tmp15;
    let tmp17;
    if (12 <= shortAt) {
      const shortAt1 = _modDef5244.getShortAt(buffer, sum + 10);
      const obj4 = { value: shortAt1, description: "" + shortAt1 };
      tmp17 = obj4;
      const tmp3Result4 = _modDef5244;
    }
    obj2.XResolution = tmp17;
    let tmp19;
    if (14 <= shortAt) {
      const shortAt2 = _modDef5244.getShortAt(buffer, sum + 12);
      const obj5 = { value: shortAt2, description: "" + shortAt2 };
      tmp19 = obj5;
      const tmp3Result5 = _modDef5244;
    }
