  ...
  if (client) {
    ...
    if (rootSpan[_frozenDsc]) {
      ...
      if (result) {
        ...
        const dropUndefinedKeysResult = _mod12807.dropUndefinedKeys(obj);
        client.emit("createDsc", dropUndefinedKeysResult);
        const tmpResult1 = _mod12807;
        const spanToJSONResult = spanTimeInputToSeconds.spanToJSON(rootSpan);
        const tmp13 = spanToJSONResult.data || {};
        const tmp14 = tmp13[_mod12816.SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
        if (null != tmp14) {
        ...
        tmp17 = "url" !== tmp13[_mod12816.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
...
export const freezeDscOnSpan = function freezeDscOnSpan(arg0, dsc) {
  const result = _mod12807.addNonEnumerableProperty(arg0, _frozenDsc, dsc);
};
