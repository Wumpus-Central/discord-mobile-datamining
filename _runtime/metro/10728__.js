  ...
  value: function innerPattern() {
    const regExp = new RegExp(
      "(?:\\s*((?:n\u00E4chste|kommende|folgende|letzte|vergangene|vorige|vor(?:her|an)gegangene)(?:s|n|m|r)?|vor|in)\\s*)?(" +
        DETimeUnitAgoFormatParser(10721).NUMBER_PATTERN +
        ")?(?:\\s*(n\u00E4chste|kommende|folgende|letzte|vergangene|vorige|vor(?:her|an)gegangene)(?:s|n|m|r)?)?\\s*(" +
        DETimeUnitAgoFormatParser(10675).matchAnyPattern(DETimeUnitAgoFormatParser(10721).TIME_UNIT_DICTIONARY) +
        ")",
      "i",
    );
    return regExp;
  ...
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      ...
      if (formatted) {
        ...
        if (isMatch) {
          reverseDurationResult = DETimeUnitAgoFormatParser(10677).reverseDuration(obj);
        }
        const ParsingComponents = DETimeUnitAgoFormatParser(10678).ParsingComponents;
