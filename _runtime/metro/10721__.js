...
const regExp = new RegExp(
  "([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10717.MONTH_DICTIONARY) +
    ")(?:\\s*(?:de|,)?\\s*(" +
    _mod10717.YEAR_PATTERN +
    "))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", ESMonthNameLittleEndianParser(10717).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            ESMonthNameLittleEndianParser(10574).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
