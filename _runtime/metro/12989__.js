  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 12963).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(12953).getRootSpan(self) === self;
        const tmpResult = SentrySpan(12953);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(12953).getRootSpan(self);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12976).DEBUG_BUILD) {
        const logger = SentrySpan(12948).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(12962).timestampInSeconds();
          const tmpResult = SentrySpan(12962);
        }
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(12991).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(12991);
            const client1 = SentrySpan(12975).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(12975);
          } else {
            if (SentrySpan(12976).DEBUG_BUILD) {
              const logger = SentrySpan(12948).logger;
          ...
          if (result) {
            let scope = SentrySpan(12977).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(12975).getCurrentScope();
              const tmpResult7 = SentrySpan(12975);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(12977);
          }
        }
      } else {
        SentrySpan(12953);
      }
      const obj = SentrySpan(12975);
    },
  },
  {
