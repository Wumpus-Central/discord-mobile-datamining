  ...
  if (client) {
    ...
    if (rootSpan[_frozenDsc]) {
      ...
      if (result) {
        ...
        tmp17 = "url" !== tmp13[_mod12955.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
...
export const freezeDscOnSpan = function freezeDscOnSpan(arg0, dsc) {
  const result = _mod12946.addNonEnumerableProperty(arg0, _frozenDsc, dsc);
};
export const getDynamicSamplingContextFromClient = function getDynamicSamplingContextFromClient(trace_id, getOptions) {
  ...
  const dropUndefinedKeysResult = _mod12946.dropUndefinedKeys({
    environment: DEFAULT_ENVIRONMENT,
    release: options.release,
