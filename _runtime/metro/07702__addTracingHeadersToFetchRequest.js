// _runtime/metro/07702__addTracingHeadersToFetchRequest.js
...
import __SENTRY_DEBUG__ from "07596___SENTRY_DEBUG__.js";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7614).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7614);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7614).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7679).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7679);
    }
    const activeSpan = setHttpStatus(7602).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7612).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7612).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7631).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7631);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7629);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7630).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7602);
  }
};