...
import _mod6781 from "06781__.js";
...
function getHandler(arg0, onBegin) {
  ...
  } else if (_mod6781.CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (_mod6781.CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (_mod6781.CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (_mod6781.CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (_mod6781.CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (_mod6781.CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (_mod6781.CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (_mod6781.CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (_mod6781.CALLBACK_TYPE.TOUCHES_CANCEL === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (TouchEventType.TouchEventType.TOUCHES_DOWN === arg0) {
    return _mod6781.CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (TouchEventType.TouchEventType.TOUCHES_MOVE === arg0) {
    return _mod6781.CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (TouchEventType.TouchEventType.TOUCHES_UP === arg0) {
    return _mod6781.CALLBACK_TYPE.TOUCHES_UP;
  } else if (TouchEventType.TouchEventType.TOUCHES_CANCEL === arg0) {
    return _mod6781.CALLBACK_TYPE.TOUCHES_CANCEL;
  } else {
    return _mod6781.CALLBACK_TYPE.UNDEFINED;
  }
}
              ...
              if (null != handlerTag.oldState) {
                if (handlerTag.oldState === _mod6766.State.UNDETERMINED) {
                  if (handlerTag.state === _mod6766.State.BEGAN) {
                    let tmp38 = runWorklet(_mod6781.CALLBACK_TYPE.BEGAN, tmp2, handlerTag);
                  }
                }
                if (handlerTag.oldState === _mod6766.State.BEGAN) {
                  if (handlerTag.state === _mod6766.State.ACTIVE) {
                    let tmp18 = runWorklet(_mod6781.CALLBACK_TYPE.START, tmp2, handlerTag);
                ...
                if (handlerTag.oldState !== handlerTag.state) {
                  if (handlerTag.state === _mod6766.State.END) {
                    if (handlerTag.oldState === _mod6766.State.ACTIVE) {
                      let flag3 = true;
                      let tmp32 = runWorklet(_mod6781.CALLBACK_TYPE.END, tmp2, handlerTag, true);
                    }
                    let flag4 = true;
                    let tmp36 = runWorklet(_mod6781.CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, true);
                  }
                }
                let tmp20 = handlerTag.state !== _mod6766.State.FAILED;
                if (tmp20) {
                  tmp20 = handlerTag.state !== _mod6766.State.CANCELLED;
                }
                if (!tmp20) {
                ...
                if (!tmp20) {
                  if (handlerTag.oldState === _mod6766.State.ACTIVE) {
                    let flag = false;
                    let tmp24 = runWorklet(_mod6781.CALLBACK_TYPE.END, tmp2, handlerTag, false);
                  }
                  let flag2 = false;
                  let tmp28 = runWorklet(_mod6781.CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, false);
                }
                ...
                if (null != handlerTag.eventType) {
                  ...
                  if (tmp4) {
                    ...
                    let tmp41Result = runWorklet(_mod6781.CALLBACK_TYPE.CHANGE, tmp2, result);
