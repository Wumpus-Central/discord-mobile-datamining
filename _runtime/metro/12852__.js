...
let items = [
  ...
  },
  {
    ...
    value: function sendEvent(arg0) {
      ...
      let eventEnvelope = BaseClient(12840).createEventEnvelope(arg0, self._dsn, self._options._metadata, self._options.tunnel);
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(12840).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
    ...
    value: function _processEvent(type, arg1, arg2) {
      ...
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12854).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
      ...
      let nextPromise = self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        if (null === result) {
          ...
          let sentryError = new BaseClient(12854).SentryError("An event processor returned `null`, will not send event.", "log");
          throw sentryError;
        } else {
          if (data.data) {
      ...
      return self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        ...
        if (null === sdkProcessingMetadata) {
          ...
          const _HermesInternal = HermesInternal;
          const sentryError = new BaseClient(12854).SentryError("" + closure_6 + " returned `null`, will not send event.", "log");
          throw sentryError;
        } else {
          if (session) {
