...
import _asyncLoop from "../09961__asyncLoop.js";
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9962).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9961).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9962).exists(this);
      HashMD(9962).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9961).createView(content);
