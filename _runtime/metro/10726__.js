...
const regExp = new RegExp("(this|last|past|next|after\\s*this)\\s*(" + repeatedTimeunitPattern.matchAnyPattern(_mod10697.TIME_UNIT_DICTIONARY) + ")(?=\\s*)(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, arg1) {
      ...
      const tmp3 = ENRelativeDateFormatParser(10697).TIME_UNIT_DICTIONARY[str3];
      if ("next" != formatted) {
        if (!formatted.startsWith("after")) {
          ...
          const obj4 = {};
          obj4[tmp3] = -1;
          const ParsingComponents = ENRelativeDateFormatParser(10701).ParsingComponents;
          return ParsingComponents.createRelativeFromReference(createParsingComponents.reference, obj4);
        }
      }
      const ParsingComponents2 = ENRelativeDateFormatParser(10701).ParsingComponents;
