...
const entry = {
  ...
  value: function render() {
    ...
    ({ dx, dy, result } = props);
    const items = [
      timestampProducer(FeDropShadow(7951), { in: str, stdDeviation: props.stdDeviation }),
      timestampProducer(FeDropShadow(7953), { dx, dy, result: "offsetblur" }),
      timestampProducer(FeDropShadow(7955), {
        floodColor: self.props.floodColor,
        floodOpacity: self.props.floodOpacity,
      }),
      timestampProducer(FeDropShadow(7944), { in2: "offsetblur", operator: "in" }),
    ];
