...
import "module_12796";
import consoleSandbox from "module_12797" /* 12797 */;
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData, fn, fn2, arg3) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12814).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12814);
            } else if (endResult.error) {
              obj = { code: setHttpStatus(12814).SPAN_STATUS_ERROR, message: "internal_error" };
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(12879).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(12879);
    }
    const activeSpan = setHttpStatus(12802).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        obj = { name: null, attributes: null };
        const _HermesInternal = HermesInternal;
        obj.name = "" + method + " " + url;
        const obj1 = { url, type: "fetch", "http.method": method, "http.url": tmp7, "server.address": host };
        obj1[setHttpStatus(12812).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(12812).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj.attributes = obj1;
        let startInactiveSpanResult = setHttpStatus(12831).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(12831);
      }
      ...
      if (fn2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12829);
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12830).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12802);
  }
};