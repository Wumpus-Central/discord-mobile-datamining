// _runtime/metro/08480__.js
import extractBrushDefault from "../08474_extractBrush.js";
import _modDef8483 from "08483__.js";
  ...
  constructor(arg0) {
    ...
    tmp3Result.setNativeProps = (obj) => {
      for (const key10004 in arg0) {
        let BrushProperties = Shape(8481).BrushProperties;
    ...
    tmp3Result.getBBox = (arg0) => {
      ...
      return Shape(8482).default.getBBox(tmp5, { fill, stroke, markers, clipped });
    };
    tmp3Result.getCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8482).default;
      return new importDefaultResult1Result(Shape(8482).default.getCTM(tmp));
    };
    tmp3Result.getScreenCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8482).default;
      return new importDefaultResult1Result(Shape(8482).default.getScreenCTM(tmp));
    };
    tmp3Result.isPointInFill = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8482).default.isPointInFill(tmp, arg0);
    };
    tmp3Result.isPointInStroke = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8482).default.isPointInStroke(tmp, arg0);
    };
    tmp3Result.getTotalLength = () => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8482).default.getTotalLength(tmp);
    };
    tmp3Result.getPointAtLength = (length) => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8482).default;
      const obj = { length };
      return new importDefaultResult1Result1(Shape(8482).default.getPointAtLength(tmp, { length }));
    };
