...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(13141).spanTimeInputToSeconds(arg0);
    },
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(13151).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    },
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(13141).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(13141);
        SentrySpan(13174).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(13174);
      }
    },
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 13151).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      const obj = SentrySpan(13142);
      obj6.status = SentrySpan(13141).getStatusMessage(this._status);
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      obj6.origin = _attributes[SentrySpan(undefined, 13151).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(13141);
      obj6._metrics_summary = SentrySpan(13152).getMetricSummaryJsonForSpan(this);
      obj6.profile_id = this._attributes[SentrySpan(undefined, 13151).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj6.exclusive_time = this._attributes[SentrySpan(undefined, 13151).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(13152);
      obj6.measurements = SentrySpan(13178).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(13141).getRootSpan(self) === self;
        const tmpResult = SentrySpan(13141);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(13141).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = SentrySpan(13141);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(13164).DEBUG_BUILD) {
        const logger = SentrySpan(13136).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(13150).timestampInSeconds();
          const tmpResult = SentrySpan(13150);
        }
      ...
      const obj2 = { name, time: SentrySpan(13141).spanTimeInputToSeconds(tmp7), attributes: obj };
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(13163).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(13179).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(13179);
            const client1 = SentrySpan(13163).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(13163);
          } else {
            if (SentrySpan(13164).DEBUG_BUILD) {
              const logger = SentrySpan(13136).logger;
          ...
          if (result) {
            let scope = SentrySpan(13165).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(13163).getCurrentScope();
              const tmpResult7 = SentrySpan(13163);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(13165);
          }
        }
      } else {
        SentrySpan(13141);
      }
      const obj = SentrySpan(13163);
    },
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      const self = this;
      const spanToJSONResult = self(13141).spanToJSON(this);
      if (tmp6) {
        if (!self._name) {
          if (tmp3(13164).DEBUG_BUILD) {
            const logger = tmp3(13136).logger;
            logger.warn("Transaction has no name, falling back to `<unlabeled transaction>`.");
          }
          self._name = "<unlabeled transaction>";
        }
        const capturedScopesOnSpan = tmp3(13165).getCapturedScopesOnSpan(self);
        const scope = capturedScopesOnSpan.scope;
        let currentScope = scope;
        if (!scope) {
          currentScope = tmp3(13163).getCurrentScope();
          const tmp3Result9 = tmp3(13163);
        }
        let client = currentScope.getClient();
        if (!client) {
          client = tmp3(13163).getClient();
          const tmp3Result10 = tmp3(13163);
        }
        if (true !== self._sampled) {
          if (tmp3(13164).DEBUG_BUILD) {
            const logger3 = tmp3(13136).logger;
          ...
          const spanDescendants = tmp3(13141).getSpanDescendants(self);
          ...
          const mapped = found.map((item) => self(13141).spanToJSON(item));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 13151).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(13151).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(13151).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          const obj2 = {
          ...
          const tmp3Result11 = tmp3(13141);
          obj3.trace = tmp3(13141).spanToTransactionTraceContext(self);
          obj2.contexts = obj3;
          let substr = found1;
          ...
          const tmp3Result12 = tmp3(13141);
          const obj5 = { dynamicSamplingContext: null };
          const tmp3Result13 = tmp3(13142);
          obj5.dynamicSamplingContext = tmp3(13172).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result13.dropUndefinedKeys(obj5));
          obj2.sdkProcessingMetadata = obj4;
          const tmp3Result14 = tmp3(13172);
          obj2._metrics_summary = tmp3(13152).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result15 = tmp3(13152);
          const result = tmp3(13178).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(13164).DEBUG_BUILD) {
              const logger2 = tmp3(13136).logger;
              const _JSON = JSON;
              logger2.log(
      ...
      const obj = self(13141);
      tmp6 =
        spanToJSONResult.start_timestamp &&
        spanToJSONResult.timestamp &&
        spanToJSONResult.span_id &&
        spanToJSONResult.trace_id;
    },
  },
];
