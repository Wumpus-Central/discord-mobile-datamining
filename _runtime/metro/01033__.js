...
import asyncExpiringMap from "asyncExpiringMap" /* 1034 */;
    ...
    if (current) {
      ...
      if (currentRoute) {
        if (_undefined) {
          ...
          const result = asyncExpiringMap.addTimeToInitialDisplayFallback(_undefined.spanContext().spanId, NATIVE.getNewScreenTimeToDisplay());
          if (tmp5) {
