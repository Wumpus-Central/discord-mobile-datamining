...
if (!module_1641.isJest()) {
  ...
  const entry1 = {
    ...
    value: function createWorkletRuntime(arg0, arg1) {
      const reanimatedError = new JSReanimated(1649).ReanimatedError(
  ...
  const entry5 = {
    ...
    value: function enableLayoutAnimations() {
      if (obj.isWeb()) {
        const logger3 = JSReanimated(1642).logger;
        logger3.warn("Layout Animations are not supported on web yet.");
      } else {
        if (tmpResult.isJest()) {
          const logger2 = JSReanimated(1642).logger;
          logger2.warn("Layout Animations are no-ops when using Jest.");
        } else {
          const tmpResult2 = JSReanimated(1641);
          const logger = JSReanimated(1642).logger;
          ...
          isChromeDebuggerResult = JSReanimated(1641).isChromeDebugger();
        }
        tmpResult = JSReanimated(1641);
      }
  ...
  const entry12 = {
    ...
    value: function initializeSensor(arg0, arg1) {
      ...
      } else if (JSReanimated(1663).SensorType.GYROSCOPE === arg0) {
        const _window4 = window;
        const gyroscope = new window.Gyroscope(obj);
        return gyroscope;
      } else if (JSReanimated(1663).SensorType.GRAVITY === arg0) {
        const _window3 = window;
        const gravitySensor = new window.GravitySensor(obj);
        return gravitySensor;
      } else if (JSReanimated(1663).SensorType.MAGNETIC_FIELD === arg0) {
        const _window2 = window;
        const magnetometer = new window.Magnetometer(obj);
        return magnetometer;
      } else if (JSReanimated(1663).SensorType.ROTATION === arg0) {
        const _window = window;
