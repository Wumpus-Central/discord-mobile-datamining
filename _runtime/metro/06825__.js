...
import allowedNativeProps2 from "../06814_allowedNativeProps.js";
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6826.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6799.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6826.shouldHandleTouchEvents(useAnimated);
};
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6826.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6826.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
