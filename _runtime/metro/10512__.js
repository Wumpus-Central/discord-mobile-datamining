...
function createTimeComponents(createParsingComponents, match, match2, match3, match4) {
  ...
  let parsed = parseInt(JPTimeExpressionParser(10507).toHankaku(match));
  if (isNaN(parsed)) {
    parsed = JPTimeExpressionParser(10507).jaStringToNumber(match);
  }
  if (parsed > 24) {
    return null;
  } else {
    if (match2) {
      ...
      if ("\u534A" !== match2) {
        ...
        const parsed1 = parseInt(JPTimeExpressionParser(10507).toHankaku(match2));
        const _isNaN = isNaN;
        num = parsed1;
        if (isNaN(parsed1)) {
          num = JPTimeExpressionParser(10507).jaStringToNumber(match2);
        }
      }
      if (num >= 60) {
        return null;
    ...
    if (match3) {
      ...
      let parsed2 = parseInt(JPTimeExpressionParser(10507).toHankaku(match3));
      const _isNaN2 = isNaN;
      if (isNaN(parsed2)) {
        parsed2 = JPTimeExpressionParser(10507).jaStringToNumber(match3);
      }
      if (parsed2 >= 60) {
        return null;
      } else {
        ...
        if ("\u5348\u524D" !== match4) {
          if ("a" !== str2.toLowerCase()) {
            ...
            if (!tmp8) {
              ...
              num6 = sum;
              num5 = JPTimeExpressionParser(10442).Meridiem.PM;
            }
          }
          str2 = match4[0];
        }
        const AM = JPTimeExpressionParser(10442).Meridiem.AM;
        num5 = AM;
        num6 = parsed;
        if (12 === parsed) {
          num6 = 0;
    ...
    if (num5 >= 0) {
      ...
      parsingComponents.imply("meridiem", JPTimeExpressionParser(10442).Meridiem.AM);
    } else {
      parsingComponents.imply("meridiem", JPTimeExpressionParser(10442).Meridiem.PM);
    }
            ...
            if (isCertainResult1) {
              ...
              if (value === JPTimeExpressionParser(10442).Meridiem.PM) {
                ...
                if (diff > end10.get("hour")) {
                  ...
                  end6.imply("meridiem", JPTimeExpressionParser(10442).Meridiem.AM);
                } else {
