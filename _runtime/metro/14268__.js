...
function parseUnicodeLanguageId(locale) {
  ...
  if (arr) {
    if ("root" === arr) {
      ...
      obj = { lang: arr, script: arr, region: arr1, variants: null };
...
export const parseUnicodeLocaleId = function parseUnicodeLocaleId(arr3) {
  ...
  if (parts.length) {
    ...
    if (tmp4) {
      ...
      const items8 = [];
      if (items3.length) {
        const obj3 = { type: "u", keywords: items3, attributes: items8 };
        let obj4 = obj3;
      } else {
        if (parts.length) {
          if (re5.test(parts[0])) {
        ...
        obj4 = { type: "u", attributes: items8, keywords: items3 };
      }
      items.push(obj4);
    }
  } else {
    obj5 = { extensions: items };
  }
