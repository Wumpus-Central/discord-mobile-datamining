...
const regExp = new RegExp(
  "(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10436.MONTH_DICTIONARY) +
    ")(?:-|/|\\s*,?\\s*)(" +
    _mod10436.ORDINAL_NUMBER_PATTERN +
    ")(?!\\s*(?:am|pm))\\s*(?:(?:to|\\-)\\s*(" +
    _mod10436.ORDINAL_NUMBER_PATTERN +
    ")\\s*)?(?:(?:-|/|\\s*,\\s*|\\s+)(" +
    _mod10436.YEAR_PATTERN +
    "))?(?=\\W|$)(?!\\:\\d)",
  "i",
);
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          addTagResult.assign("year", ENMonthNameMiddleEndianParser(10436).parseYear(index[4]));
        } else {
          addTagResult.imply(
            "year",
        ...
        if (index[3]) {
          const result1 = ENMonthNameMiddleEndianParser(10436).parseOrdinalNumberPattern(index[3]);
