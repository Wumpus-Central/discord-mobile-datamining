...
const regExp = new RegExp(
  "(?:on\\s{0,3})?(" +
    _mod10501.ORDINAL_NUMBER_PATTERN +
    ")(?:\\s{0,3}(?:to|\\-|\\\u2013|until|through|till)?\\s{0,3}(" +
    _mod10501.ORDINAL_NUMBER_PATTERN +
    "))?(?:-|/|\\s{0,3}(?:of)?\\s{0,3})(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10501.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s{0,3})(" +
    _mod10501.YEAR_PATTERN +
    "(?!\\w)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", ENMonthNameLittleEndianParser(10501).parseYear(index[4]));
        } else {
        ...
        if (index[2]) {
          ...
          const result1 = ENMonthNameLittleEndianParser(10501).parseOrdinalNumberPattern(index[2]);
