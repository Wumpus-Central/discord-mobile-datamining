  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 13063).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(13053).getRootSpan(self) === self;
        const tmpResult = SentrySpan(13053);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(13053).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = SentrySpan(13053);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(13076).DEBUG_BUILD) {
        const logger = SentrySpan(13048).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(13062).timestampInSeconds();
          const tmpResult = SentrySpan(13062);
        }
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(13091).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(13091);
            const client1 = SentrySpan(13075).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(13075);
          } else {
            if (SentrySpan(13076).DEBUG_BUILD) {
              const logger = SentrySpan(13048).logger;
          ...
          if (result) {
            let scope = SentrySpan(13077).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(13075).getCurrentScope();
              const tmpResult7 = SentrySpan(13075);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(13077);
          }
        }
      } else {
        SentrySpan(13053);
      }
      const obj = SentrySpan(13075);
    },
  },
  {
