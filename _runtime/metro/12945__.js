...
let items = [
  ...
  },
  {
    ...
    value: function sendEvent(arg0) {
      ...
      let eventEnvelope = BaseClient(12933).createEventEnvelope(
        arg0,
        self._dsn,
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(
        BaseClient(12933).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel),
      );
    },
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12918).DEBUG_BUILD) {
        let logger = BaseClient(12890).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12914).resolvedSyncPromise({});
      const tmp2Result = BaseClient(12914);
    },
  },
  {
      ...
      return currentScope(self[15])
        ...
        .then((contexts) => {
          if (null === contexts) {
            ...
            let obj = { trace: BaseClient(12917).getTraceContextFromScope(currentScope) };
            const merged = Object.assign(contexts.contexts);
            contexts.contexts = obj;
            const obj2 = BaseClient(12917);
            obj = { dynamicSamplingContext: BaseClient(12926).getDynamicSamplingContextFromScope(self, currentScope) };
      ...
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12947).SentryError(
      ...
      let nextPromise = self
        ...
        .then((result) => {
          if (null === result) {
            ...
            let sentryError = new BaseClient(12947).SentryError(
      ...
      return self
        ...
        .then((sdkProcessingMetadata) => {
          if (null === sdkProcessingMetadata) {
            ...
            const _HermesInternal = HermesInternal;
            const sentryError = new BaseClient(12947).SentryError(
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = BaseClient(12918).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = BaseClient(12890).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = BaseClient(12948);
          ...
          const logger3 = BaseClient(12890).logger;
          logger3.log("No dsn provided, will not send outcomes");
        }
      } else if (BaseClient(12918).DEBUG_BUILD) {
        const logger2 = BaseClient(12890).logger;
