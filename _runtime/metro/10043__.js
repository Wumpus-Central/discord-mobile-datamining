// _runtime/metro/10043__.js
import repeatedTimeunitPattern from "../09895_repeatedTimeunitPattern.js";
import AbstractParserWithWordBoundaryChecking from "../09902_AbstractParserWithWordBoundaryChecking.js";
import _mod10039 from "10039__.js";
...
const regExp = new RegExp(
  "([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?(?:\\s*(?:desde|de|\\-|\\\u2013|ao?|\\s)\\s*([0-9]{1,2})(?:\u00BA|\u00AA|\u00B0)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10039.MONTH_DICTIONARY) +
    ")(?:\\s*(?:de|,)?\\s*(" +
    _mod10039.YEAR_PATTERN +
    "))?(?=\\W|$)",
  "i",
);
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = ESMonthNameLittleEndianParser(10039).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", ESMonthNameLittleEndianParser(10039).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            ESMonthNameLittleEndianParser(9896).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
