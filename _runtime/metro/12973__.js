  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 12947).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(12937).getRootSpan(self) === self;
        const tmpResult = SentrySpan(12937);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(12937).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = SentrySpan(12937);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12960).DEBUG_BUILD) {
        const logger = SentrySpan(12932).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(12946).timestampInSeconds();
          const tmpResult = SentrySpan(12946);
        }
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(12975).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(12975);
            const client1 = SentrySpan(12959).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(12959);
          } else {
            if (SentrySpan(12960).DEBUG_BUILD) {
              const logger = SentrySpan(12932).logger;
          ...
          if (result) {
            let scope = SentrySpan(12961).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(12959).getCurrentScope();
              const tmpResult7 = SentrySpan(12959);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(12961);
          }
        }
      } else {
        SentrySpan(12937);
      }
      const obj = SentrySpan(12959);
    },
  },
  {
