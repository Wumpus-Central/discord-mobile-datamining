  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = ENMergeRelativeFollowByDateRefiner(10583).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeFollowByDateRefiner(10584).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeFollowByDateRefiner(10584).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10584).ParsingResult(
        start.reference,
        text.index,
