    ...
    if (tmp2Result.isEvent(type)) {
      ...
      if (typeof type === "object") {
        ...
        if (typeof globalThis.CustomEvent !== "undefined") {
          isInstanceOfResult = _mod13143.isInstanceOf(type, globalThis.CustomEvent);
          const tmp2Result2 = _mod13143;
        }
...
function serializeEventTarget(arg0) {
  try {
    if (obj.isElement(arg0)) {
      let htmlTreeAsStringResult = _mod13144.htmlTreeAsString(arg0);
      const tmp2Result = _mod13144;
    } else {
      const _Object = Object;
...
export const extractExceptionKeysForMessage = function extractExceptionKeysForMessage(name) {
  ...
  if (first) {
    if (first.length >= num) {
      return _mod13145.truncate(first, num);
    } else {
      ...
      if (length > 0) {
        ...
        if (length !== keys.length) {
          truncateResult = _mod13145.truncate(joined, num);
        }
