...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  const traceData = _mod13125.getTraceData({ span });
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      if (typeof Request !== "undefined") {
        ...
        isInstanceOfResult = _mod13061.isInstanceOf(headers, Request);
        const tmpResult = _mod13061;
      }
      let headers1;
    ...
    if (headers) {
      ...
      if (typeof Headers !== "undefined") {
        ...
        isInstanceOfResult1 = _mod13061.isInstanceOf(headers, Headers);
        const tmpResult2 = _mod13061;
      }
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData, fn, fn2, arg3) {
  ...
  if (fetchData.fetchData) {
    ...
    let hasTracingEnabledResult = _mod13086.hasTracingEnabled();
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(13071).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(13071);
            } else if (endResult.error) {
              const obj2 = { code: setHttpStatus(13071).SPAN_STATUS_ERROR, message: "internal_error" };
    ...
    if (tmp7) {
      const host = setHttpStatus(13136).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(13136);
    }
    const activeSpan = setHttpStatus(13059).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj4[setHttpStatus(13069).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj4[setHttpStatus(13069).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj3.attributes = obj4;
        let startInactiveSpanResult = setHttpStatus(13088).startInactiveSpan(obj3);
        const setHttpStatusResult4 = setHttpStatus(13088);
      }
      ...
      if (fn2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(13086);
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(13087).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(13059);
  }
};