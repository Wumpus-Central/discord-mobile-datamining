      ...
      if (typeof value === "object") {
        if (null !== value) {
          if (undefined !== value.value) {
            ...
            const reanimatedError = new _mod1649.ReanimatedError("Invalid value passed to `" + nextResult + "`, maybe you forgot to use `.value`?");
  ...
  if (flag5) {
    frame = function frame(timestamp) {
      ...
      if (isAnimationCancelled.isAnimationCancelled) {
        ...
        _mod1738.updateProps(dependencyMap, obj);
        if (flag3) {
    ...
    if (flag4) {
      require("module_1738").updateProps(shareableViewDescriptors, obj2);
      const obj8 = require("module_1738");
    }
  } else {
    styleUpdater.isAnimationCancelled = true;
    styleUpdater.animations = [];
    let shallowEqualResult = require("module_1784").shallowEqual(last, obj);
    if (shallowEqualResult) {
      shallowEqualResult = !arg5;
    }
    if (!shallowEqualResult) {
      tmp19(1738).updateProps(shareableViewDescriptors, obj, flag);
      const tmp19Result = tmp19(1738);
    }
...
styleUpdater.__closure = { SHOULD_BE_USE_WEB: module_1641, processBoxShadow: o.processBoxShadow, isAnimated: _mod1784.isAnimated, prepareAnimation, runAnimations, updateProps: _mod1738.updateProps, shallowEqual: _mod1784.shallowEqual };
...
let obj = { SHOULD_BE_USE_WEB: module_1641, processBoxShadow: o.processBoxShadow, isAnimated: _mod1784.isAnimated, prepareAnimation, runAnimations, updateProps: _mod1738.updateProps, shallowEqual: _mod1784.shallowEqual };
jestStyleUpdater.__closure = { isAnimated: _mod1784.isAnimated, prepareAnimation, runAnimations, updatePropsJestWrapper: _mod1738.updatePropsJestWrapper, shallowEqual: _mod1784.shallowEqual };
...
export const useAnimatedStyle = function useAnimatedStyle(fn, items, arg2, arg3) {
  ...
  if (arg2) {
    workletsHash = require("module_1784").buildWorkletsHash(items1);
    let obj2 = require("module_1784");
  }
  sharedValue = require("module_1790").useSharedValue(true);
  ...
  if (!tmp2.current) {
    ...
    obj.styleUpdaterContainer = { current: "__initData" };
    tmp2.current = obj;
