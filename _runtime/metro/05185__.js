...
import readTag from "../05186_readTag.js";
  ...
  read(getUint16, sum) {
    ...
    if (undefined !== ifd[c3]) {
      ...
      objectAssignResult = tmp3Result.objectAssign(
        ifd,
        tmp3Result.readIfd(getUint16, tmp3(5187).IFD_TYPE_EXIF, sum, sum + ifd[tmp5].value, byteOrder, arg2),
      );
    }
    if (undefined !== objectAssignResult[c4]) {
      const tmp3Result2 = tmp3(5186);
      objectAssignResult = tmp3(5166).objectAssign(
        objectAssignResult,
        tmp3Result2.readIfd(
          getUint16,
          tmp3(5187).IFD_TYPE_GPS,
          sum,
    ...
    if (undefined !== objectAssignResult[c5]) {
      ...
      objectAssignResult1 = tmp3(5166).objectAssign(
        objectAssignResult,
        tmp3Result4.readIfd(
          getUint16,
          tmp3(5187).IFD_TYPE_INTEROPERABILITY,
          sum,
