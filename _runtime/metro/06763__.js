...
import handlerIDToTag from "../06764_handlerIDToTag.js";
import _mod6766 from "06766__.js";
import _mod6767 from "06767__.js";
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === _mod6766.State.UNDETERMINED) {
        if (handlerTag.state === _mod6766.State.BEGAN) {
      ...
      if (handlerTag.oldState === _mod6766.State.BEGAN) {
        if (handlerTag.state === _mod6766.State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === _mod6766.State.END) {
          if (handlerTag.oldState === _mod6766.State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== _mod6766.State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== _mod6766.State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = _mod6767.GestureStateManager;
