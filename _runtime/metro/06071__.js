// _runtime/metro/06071__.js
import _mod17 from "00017__.js";
import handlerIDToTag from "../06072_handlerIDToTag.js";
import _mod6074 from "06074__.js";
import _mod6075 from "06075__.js";
import TouchEventType from "../06076_TouchEventType.js";
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === _mod6074.State.UNDETERMINED) {
        if (handlerTag.state === _mod6074.State.BEGAN) {
      ...
      if (handlerTag.oldState === _mod6074.State.BEGAN) {
        if (handlerTag.state === _mod6074.State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === _mod6074.State.END) {
          if (handlerTag.oldState === _mod6074.State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== _mod6074.State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== _mod6074.State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = _mod6075.GestureStateManager;
