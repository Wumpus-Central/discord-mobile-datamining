...
let items = [
  ...
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      SentrySpan(12873);
      const obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12882).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(12872).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 12882).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      const obj3 = SentrySpan(12872);
      obj._metrics_summary = SentrySpan(12883).getMetricSummaryJsonForSpan(this);
      obj.profile_id = this._attributes[SentrySpan(undefined, 12882).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj.exclusive_time = this._attributes[SentrySpan(undefined, 12882).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12883);
      obj.measurements = SentrySpan(12909).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        let tmpResult = SentrySpan(12872);
        _isStandaloneSpan = tmpResult.getRootSpan(self) === self;
      }
      obj.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        tmpResult = SentrySpan(12872);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12895).DEBUG_BUILD) {
        const logger = SentrySpan(12867).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          let tmpResult = SentrySpan(12881);
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = SentrySpan(12896).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(12894).getCurrentScope();
              const tmpResult2 = SentrySpan(12894);
            }
            scope.captureEvent(result);
            const tmpResult1 = SentrySpan(12896);
          }
        }
      } else {
        SentrySpan(12872);
      }
      const obj = SentrySpan(12894);
    }
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(12872);
          obj.trace = tmp3(12872).spanToTransactionTraceContext(self);
          obj.contexts = obj;
          let substr = found1;
          ...
          const tmp3Result4 = tmp3(12873);
          obj2.dynamicSamplingContext = tmp3(12903).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj.sdkProcessingMetadata = obj1;
          const tmp3Result5 = tmp3(12903);
          obj._metrics_summary = tmp3(12883).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
