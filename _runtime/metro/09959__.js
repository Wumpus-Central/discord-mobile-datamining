...
const regExp = new RegExp(
  "(?:on\\s*?)?(" +
    _mod9957.ORDINAL_NUMBER_PATTERN +
    ")(?:\\s*(?:au|\\-|\\\u2013|jusqu'au?|\\s)\\s*(" +
    _mod9957.ORDINAL_NUMBER_PATTERN +
    "))?(?:-|/|\\s*(?:de)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod9957.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)(" +
    _mod9957.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", FRMonthNameLittleEndianParser(9957).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            FRMonthNameLittleEndianParser(9896).findYearClosestToRef(createParsingResult.refDate, result, tmp4),
          );
        }
        if (index[2]) {
          const start3 = parsingResult.start;
          const result1 = FRMonthNameLittleEndianParser(9957).parseOrdinalNumberPattern(index[2]);
