...
import PanGestureHandler from "../07078_PanGestureHandler.js";
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1505)(() => {
    ...
    if (ref.current) {
      ...
      if (typeof getAnimateToValue === "function") {
        if (closing) {
          let num3 = _mod7092.getDistanceForDirection(layout, gestureDirection, "rtl" === direction);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7092.getDistanceForDirection(layout, gestureDirection, "rtl" === direction));
              obj = { closing };
              closure_34(obj);
            }
          }
  ...
  let merged = Object.assign(
    flag2(7101).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }),
  );
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7102).CardContent, obj9);
    obj6.children = items13;
    obj5.children = tmp17(tmp25, obj6);
    obj4.children = tmp19(tmp18(7078).PanGestureHandler, obj5);
    items5[2] = tmp19(tmp20.View, obj4);
    obj.children = items5;
    return tmp17(flag2(7100).CardAnimationContext.Provider, obj);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
