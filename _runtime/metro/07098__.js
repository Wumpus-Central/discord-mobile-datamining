...
import ButtonComponentDefault from "../07081_ButtonComponent.js";
...
import 07099__ from "07099__.js";
...
const ButtonComponent = module_7099(ButtonComponentDefault, { shouldCancelWhenOutside: false, shouldActivateOnStart: false });
...
class InnerBaseButton {
  constructor(arg0) {
    ...
    tmp3Result.handleEvent = (nativeEvent) => {
      ...
      if (pointerInside) {
        let tmp4 = state === InnerBorderlessButton(6991).State.BEGAN;
        if (!tmp4) {
          tmp4 = state === InnerBorderlessButton(6991).State.ACTIVE;
        }
        tmp = tmp4;
      }
      if (tmp8) {
      ...
      if (!longPressDetected) {
        onPress = nativeEvent.oldState === InnerBorderlessButton(6991).State.ACTIVE;
      }
      if (onPress) {
        onPress = state !== InnerBorderlessButton(6991).State.CANCELLED;
      }
      if (onPress) {
      ...
      if (!closure_0.lastIsPressed) {
        if (state === InnerBorderlessButton(6991).State.BEGAN) {
          if (pointerInside) {
      ...
      let tmp18 = state !== InnerBorderlessButton(6991).State.ACTIVE || pointerInside || undefined === closure_0.longPressTimeout;
      if (tmp18) {
        let tmp19 = undefined === closure_0.longPressTimeout;
        if (!tmp19) {
          let tmp22 = state !== InnerBorderlessButton(6991).State.END;
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6991).State.CANCELLED;
          }
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6991).State.FAILED;
          }
          tmp19 = tmp22;
        }
        tmp18 = tmp19;
