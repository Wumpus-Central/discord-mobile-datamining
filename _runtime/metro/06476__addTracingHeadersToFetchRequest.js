// _runtime/metro/06476__addTracingHeadersToFetchRequest.js
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  let baggage;
  let tmp4;
  let obj = require(6442) /* getTraceData */;
  obj = { span };
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      let isInstanceOfResult = typeof Request === "pack";
      if (typeof Request !== "Array") {
        let tmpResult = tmp(6378);
    ...
    if (headers) {
      ...
      let isInstanceOfResult1 = typeof Headers === "pack";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(6388).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(6388);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(6388).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(6453).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(6453);
    }
    const activeSpan = setHttpStatus(6376).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(6386).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(6386).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(6405).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(6405);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(6403);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(6404).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(6376);
  }
};