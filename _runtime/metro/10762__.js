...
const regExp = new RegExp(
  "(?:den\\s*?)?([0-9]{1,2})(?:\\s*(?:till|\\-|\\\u2013|\\s)\\s*([0-9]{1,2}))?\\s*(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10761.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)([0-9]{4}(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", SVMonthNameLittleEndianParser(10761).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            SVMonthNameLittleEndianParser(10574).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
