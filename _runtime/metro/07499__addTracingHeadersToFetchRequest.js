// _runtime/metro/07499__addTracingHeadersToFetchRequest.js
...
import "SyncPromise";
import __SENTRY_DEBUG__ from "__SENTRY_DEBUG__";
import { hasTracingEnabled } from "../07426_hasTracingEnabled.js";
import { getTraceData } from "../07465_getTraceData.js";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7411).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7411);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7411).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7476).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7476);
    }
    const activeSpan = setHttpStatus(7399).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7409).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7409).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7428).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7428);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7426);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7427).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7399);
  }
};