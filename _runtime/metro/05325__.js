...
import _modDef5326 from "05326__.js";
import _modDef5327 from "05327__.js";
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          obj.name = combined;
          obj.value = items;
          obj.description = getTagDescription(_modDef5326.iptc[uint16], items, obj, encoding);
          let tmp17 = _modDef5326.iptc[uint16] && _modDef5326.iptc[uint16].repeatable;
          if (tmp17) {
            obj.repeatable = true;
          }
          let tmp18 = _modDef5326.iptc[uint16] && undefined !== _modDef5326.iptc[uint16].encoding_name;
          if (tmp18) {
            let obj4 = _modDef5326.iptc[uint16];
