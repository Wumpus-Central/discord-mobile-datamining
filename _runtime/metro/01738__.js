...
import validateTransformOrigin from "validateTransformOrigin" /* 1739 */;
import ComponentRegistry2 from "ComponentRegistry" /* 1740 */;
...
if (module_1640.shouldBeUseWeb()) {
  ...
  fn = function s(value, transformOrigin) {
    ...
    _mod1680.processColorsInProps(transformOrigin);
  ...
  const obj3 = { processColorsInProps: _mod1680.processColorsInProps, processTransformOrigin: validateTransformOrigin.processTransformOrigin };
  fn.__closure = obj3;
...
if (module_1640.shouldBeUseWeb()) {
  function maybeThrowError() {
    if (!obj.isJest()) {
      const reanimatedError = new require("module_1648").ReanimatedError("`UpdatePropsManager` is not available on non-native platform.");
