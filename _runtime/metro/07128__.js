...
import PanGestureHandler from "../07108_PanGestureHandler.js";
import _mod7122 from "07122__.js";
import _mod7123 from "07123__.js";
...
export const Card = function Card(shadowEnabled) {
  ...
  const first1 = interpolationIndex(
    opening.useState(() => {
      value = new RN.Value(_mod7123.getInvertedMultiplier(gestureDirection, "rtl" === direction));
      return value;
    }),
    1,
  )[0];
  ...
  closure_32 = num(1505)(() => {
    if (null == ref3.current) {
      const InteractionManager = _mod7129.InteractionManager;
  ...
  const layoutEffect = opening.useLayoutEffect(() => {
    ...
    first1.setValue(_mod7123.getInvertedMultiplier(gestureDirection, "rtl" === direction));
  }, items);
  opening.useRef(null);
  ...
  let tmp11 = num(1505)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (ref7.current.closing) {
            let num2 = _mod7122.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (closing) {
          let num3 = _mod7122.getDistanceForDirection(layout, gestureDirection, "rtl" === direction);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7122.getDistanceForDirection(layout, gestureDirection, "rtl" === direction));
              const obj4 = { closing };
              closure_34(obj4);
            }
          }
  ...
  if (typeof backgroundColor === "string") {
    tmp16 = 0 === tmp5(5721)(backgroundColor).alpha();
    const obj14 = tmp5(5721)(backgroundColor);
  }
  ...
  let merged = Object.assign(
    flag2(7131).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }),
  );
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7132).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(7108).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(7130).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
  const tmp18Result = flag2(7131);
};
