...
import _modDef8673 from "08673__.js";
...
class TextPath {
  constructor() {
    ...
    tmp3Result.setNativeProps = (matrix) => {
      ...
      const merged = Object.assign(matrix, TextPath(8590).pickNotNil(extractFontDefault(matrix, true)));
      if (closure_0.root) {
        const root = closure_0.root;
        root.setNativeProps(matrix);
      }
      const obj = TextPath(8590);
    };
    return tmp3Result;
  }
}
_inherits(TextPath, _modDef8598);
const entry = {
  key: "render",
  value: function render() {
    ...
    let match = href;
    if (href) {
      match = href.match(TextPath(8590).idPattern);
    }
    let tmp5 = match;
    if (match) {
      tmp5 = match[1];
