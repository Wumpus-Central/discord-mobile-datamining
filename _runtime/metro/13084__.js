    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new _mod13091.SentrySpan(obj2);
        } else {
          const obj3 = { traceId };
          sentrySpan = new _mod13083.SentryNonRecordingSpan(obj3);
        }
        ...
        const tmpResult13 = _mod13077;
        spanTimeInputToSeconds.addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult14 = spanTimeInputToSeconds;
      }
      _mod13088.logSpanStart(sentrySpan);
      const tmpResult15 = _mod13088;
      const result = _mod13079.setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = _mod13086.getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult17 = _mod13086;
      ...
      _mod13086.freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult19 = _mod13086;
    } else {
      const obj5 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        _mod13086.freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult20 = _mod13086;
      }
    }
    const tmpResult = _mod13077;
  } else {
    const sentryNonRecordingSpan = new _mod13083.SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = _mod13089.sampleSpan(options, obj2);
    const tmpResult = _mod13089;
  }
  ...
  obj4.sampled = tmp6;
  const sentrySpan = new _mod13091.SentrySpan(obj4);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(_mod13065.SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  _require = arg1;
  const mainCarrier = require("13068__.js").getMainCarrier();
  const obj = require("13068__.js");
  const tmp = _require;
  const asyncContextStrategy = require("13069__.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(13077).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(
        _mod13062.propagationContextFromHeaders(dependencyMap, _slicedToArray),
      );
      return closure_0();
    });
  }
  const obj2 = require("13069__.js");
};
export const startInactiveSpan = function startInactiveSpan(experimental) {
  _require = experimental;
  let mainCarrier = require("13068__.js").getMainCarrier();
  let obj = require("13068__.js");
  const tmp3 = _require;
  const tmp4 = obj4;
  let asyncContextStrategy = require("13069__.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    return fn(() => {
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new _mod13083.SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
  ...
  let obj2 = require("13069__.js");
};
export const startNewTrace = function startNewTrace(arg0) {
  _require = arg0;
  return require("13077__.js").withScope((setPropagationContext) => {
    ...
    return tmp3(13077).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan
        ...
        : ((fn) => fn())(() => {
            ...
            let obj = sentryNonRecordingSpan(13077);
            ...
            if (sentryNonRecordingSpan.onlyIfParent) {
              ...
              return tmp(13085).handleCallbackErrors(
            ...
            const obj2 = sentryNonRecordingSpan(13072);
