...
let items = [
  ...
  },
  {
    ...
    value: function render() {
      ...
      size = { position: "absolute", left: bound + padding / 2, top: bound + padding / 2, width: diff1, height: diff1, borderRadius: diff1 / 2, alignItems: "center", justifyContent: "center", overflow: "hidden" };
      ...
      if (backgroundColor) {
        const obj1 = { d: circlePathResult, stroke: backgroundColor, strokeWidth: null, strokeLinecap: null, strokeDasharray: null, fill: "transparent" };
        if (!backgroundWidth) {
        ...
        tmp23Result = timestampProducer(CircularProgress(8484).Path, obj1);
      }
      const items = [tmp23Result, , ];
      tmp23Result = fill > 0;
      if (tmp23Result) {
        const obj2 = { d: circlePathResult1, stroke: tintColor, strokeWidth: width, strokeLinecap: fillLineCap, strokeDasharray: mapped, fill: "transparent" };
        tmp23Result = timestampProducer(CircularProgress(8484).Path, obj2);
      }
      items[1] = tmp23Result;
      items[2] = renderCapResult;
      obj.children = items;
      size1.children = React5(CircularProgress(8484).G, obj);
      const items1 = [timestampProducer(CircularProgress(8484).Svg, size1), ];
