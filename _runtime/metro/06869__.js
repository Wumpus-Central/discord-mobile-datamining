...
import ButtonComponentDefault from "../06852_ButtonComponent.js";
...
import 06870__ from "06870__.js";
...
const ButtonComponent = module_6870(ButtonComponentDefault, { shouldCancelWhenOutside: false, shouldActivateOnStart: false });
...
class InnerBaseButton {
  constructor(arg0) {
    ...
    tmp3Result.handleEvent = (nativeEvent) => {
      ...
      if (pointerInside) {
        let tmp4 = state === InnerBorderlessButton(6762).State.BEGAN;
        if (!tmp4) {
          tmp4 = state === InnerBorderlessButton(6762).State.ACTIVE;
        }
        tmp = tmp4;
      }
      if (tmp8) {
      ...
      if (!longPressDetected) {
        onPress = nativeEvent.oldState === InnerBorderlessButton(6762).State.ACTIVE;
      }
      if (onPress) {
        onPress = state !== InnerBorderlessButton(6762).State.CANCELLED;
      }
      if (onPress) {
      ...
      if (!closure_0.lastIsPressed) {
        if (state === InnerBorderlessButton(6762).State.BEGAN) {
          if (pointerInside) {
      ...
      let tmp18 = state !== InnerBorderlessButton(6762).State.ACTIVE || pointerInside || undefined === closure_0.longPressTimeout;
      if (tmp18) {
        let tmp19 = undefined === closure_0.longPressTimeout;
        if (!tmp19) {
          let tmp22 = state !== InnerBorderlessButton(6762).State.END;
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6762).State.CANCELLED;
          }
          if (tmp22) {
            tmp22 = state !== InnerBorderlessButton(6762).State.FAILED;
          }
          tmp19 = tmp22;
        }
        tmp18 = tmp19;
