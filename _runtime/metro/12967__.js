...
let items = [
  ...
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      SentrySpan(12932);
      const obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12941).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(12931).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 12941).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      const obj3 = SentrySpan(12931);
      obj._metrics_summary = SentrySpan(12942).getMetricSummaryJsonForSpan(this);
      obj.profile_id = this._attributes[SentrySpan(undefined, 12941).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj.exclusive_time = this._attributes[SentrySpan(undefined, 12941).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12942);
      obj.measurements = SentrySpan(12968).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        let tmpResult = SentrySpan(12931);
        _isStandaloneSpan = tmpResult.getRootSpan(self) === self;
      }
      obj.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        tmpResult = SentrySpan(12931);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12954).DEBUG_BUILD) {
        const logger = SentrySpan(12926).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          let tmpResult = SentrySpan(12940);
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = SentrySpan(12955).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(12953).getCurrentScope();
              const tmpResult2 = SentrySpan(12953);
            }
            scope.captureEvent(result);
            const tmpResult1 = SentrySpan(12955);
          }
        }
      } else {
        SentrySpan(12931);
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(12931);
          obj.trace = tmp3(12931).spanToTransactionTraceContext(self);
          obj.contexts = obj;
          let substr = found1;
          ...
          const tmp3Result4 = tmp3(12932);
          obj2.dynamicSamplingContext = tmp3(12962).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj.sdkProcessingMetadata = obj1;
          const tmp3Result5 = tmp3(12962);
          obj._metrics_summary = tmp3(12942).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
