...
const entry = {
  ...
  value: function innerPatternString(arg0) {
    return (
      "((?:\u0432|\u0443)\\s*)?(" +
      UkMonthNameParser(10698).matchAnyPattern(UkMonthNameParser(10852).MONTH_DICTIONARY) +
      ")\\s*(?:[,-]?\\s*(" +
      UkMonthNameParser(10852).YEAR_PATTERN +
      ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)"
    );
  },
};
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start4.assign("year", UkMonthNameParser(10852).parseYearPattern(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply(
          "year",
          UkMonthNameParser(10699).findYearClosestToRef(createParsingResult.reference.instant, 1, tmp9),
        );
      }
