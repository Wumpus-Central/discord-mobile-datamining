...
import S_UNICODE_REGEX from "../14228_S_UNICODE_REGEX.js";
...
export default function formatToParts(magnitude, numbers, arg2, currencyDisplay) {
  ...
  if (0 < parts4.length) {
    while (true) {
      ...
      if (str50) {
        if ("{0}" === str50) {
          ...
          obj = { type: "plusSign", value: tmp40.plusSign };
          arr = items.push(obj);
        } else if ("%" === str50) {
          let obj1 = { type: "percentSign", value: tmp40.percentSign };
          ...
          let push = items.push;
          if (obj4.test(str50)) {
            let obj3 = { type: "compact", value: str50.substring(3, str50.length - 1) };
            arr3 = push(obj3);
          } else {
            obj4 = { type: "literal", value: str50 };
            arr4 = push(obj4);
          }
        }
      }
    ...
    if (roundedNumber2.isNaN()) {
      obj5 = { type: "nan", value: formattedString2 };
      formattedString2 = [obj5];
      let items1 = formattedString2;
      push2.apply(items, items1);
    } else if (!roundedNumber2.isFinite()) {
      const obj6 = { type: "infinity", value: formattedString2 };
      items1 = [obj6];
    }
    value = value(14229).digitMapping[numberingSystem];
    ...
    if (flag) {
      if (tmp17) {
        if (null != tmp40.currencyGroup) {
          ...
          if (items2.length > 0) {
            do {
              let obj7 = { type: "integer", value: items2.pop() };
              let arr9 = obj5.push(obj7);
              if (items2.length > 0) {
                let obj8 = { type: "group", value };
      ...
      const obj9 = { type: "integer", value: substr };
      obj5.push(obj9);
    }
    if (undefined === substr1) {
      if ("scientific" === notation) {
        ...
        if (roundedNumber2.isFinite()) {
          const obj10 = { type: "exponentSeparator", value: tmp40.exponential };
          obj5.push(obj10);
          value = exponent;
          if (exponent < 0) {
            const obj11 = { type: "exponentMinusSign", value: tmp40.minusSign };
            obj5.push(obj11);
            value = -exponent;
          }
          items2 = new.target;
          let _default = new digitsToString.default(value);
          const obj12 = { type: "exponentInteger", value: null };
          formattedString = ToRawFixed.ToRawFixed(_default, 0, 0, roundingIncrement, formattedString).formattedString;
          obj12.value = formattedString;
          obj5.push(obj12);
          items1 = obj5;
        }
      } else {
      ...
      if (!tmp17) {
        const obj13 = { type: "decimal", value: tmp40.decimal };
        const obj14 = { type: "fraction", value: substr1 };
  ...
  if ("currency" === style) {
    if ("name" === currencyDisplay.currencyDisplay) {
      ...
      const items3 = [];
      const parts6 =
        numbers.numbers.currency[numberingSystem] || numbers.numbers.currency[first].unitPattern.split(/(\{[01]\})/g);
      for (let num18 = 0; num18 < parts6.length; num18 = num18 + 1) {
        ...
        if ("{0}" === tmp102) {
          ...
          let obj15 = { type: "currency", value: currency3 };
          let arr16 = items3.push(obj15);
        } else if (tmp102) {
          let obj16 = { type: "literal", value: tmp102 };
    ...
    if (numbers.units.simple[unit]) {
      ...
      let str59 =
        numbers.units.simple[unit][unitDisplay][arg2.select(arg2, timesResult1.toNumber(timesResult1))] ||
        numbers.units.simple[unit][unitDisplay].other;
      ...
      const tmp86 =
        numbers.units.simple[tmp81][unitDisplay][arg2.select(arg2, timesResult2.toNumber(timesResult2))] ||
        numbers.units.simple[tmp81][unitDisplay].other;
      const str53 = numbers.units.simple[tmp82].perUnit[unitDisplay];
      if (str53) {
        str59 = str53.replace("{0}", tmp86);
      } else {
        const str55 =
          numbers.units.simple[tmp82][unitDisplay][arg2.select(arg2, 1)] ||
          numbers.units.simple[tmp82][unitDisplay].other;
