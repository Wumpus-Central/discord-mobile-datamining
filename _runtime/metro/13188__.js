...
import consoleSandbox from "13145__.js";

export const createEventEnvelope = function createEventEnvelope(type, url, sdk, arg3) {
  const sdkMetadataForEnvelopeHeader = _mod13189.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
  ...
  const eventEnvelopeHeaders = _mod13189.createEventEnvelopeHeaders(type, sdkMetadataForEnvelopeHeader, arg3, url);
  delete tmp[tmp2];
  const items2 = [{ type: str }, type];
  const tmp3Result = _mod13189;
  const items3 = [items2];
  return _mod13189.createEnvelope(eventEnvelopeHeaders, items3);
};
export const createSessionEnvelope = function createSessionEnvelope(toJSON, url, sdk, arg3) {
  ...
  let tmp6 = arg3 && url;
  if (tmp6) {
    const obj4 = { dsn: _mod13192.dsnToString(url) };
    tmp6 = obj4;
    const tmpResult = _mod13192;
  }
  ...
  const date = new Date();
  const items2 = [items1];
  return _mod13189.createEnvelope(obj2, items2);
};
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  const dynamicSamplingContextFromSpan = beforeSendSpan(13181).getDynamicSamplingContextFromSpan(arg0[0]);
  let dsn = getDsn;
  if (getDsn) {
    dsn = getDsn.getDsn();
  }
  ...
  const obj = beforeSendSpan(13181);
  ...
  let tmp9 = tunnel && dsn;
  if (tmp9) {
    const obj4 = { dsn: tmp2(13192).dsnToString(dsn) };
    tmp9 = obj4;
    const tmp2Result = tmp2(13192);
  }
