...
export const startIdleSpan = function startIdleSpan(arg0) {
    ...
    if (spanToJSONResult.start_timestamp) {
      ...
      const item1 = found.forEach((isRecording) => {
        if (isRecording.isRecording()) {
          const obj = { code: _mod13162.SPAN_STATUS_ERROR, message: "cancelled" };
  ...
  const client = require("13172__.js").getClient();
  if (client) {
    if (tmp5Result.hasTracingEnabled()) {
      ...
      let obj3 = {
        apply(arg0, arg1, current) {
          ...
          if (!first) {
            first = _mod13159.timestampInSeconds();
          }
      ...
      const timerId = setTimeout(() => {
        if (!c2) {
          const obj = { code: _mod13162.SPAN_STATUS_ERROR, message: "deadline_exceeded" };
