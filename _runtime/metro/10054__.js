...
const regExp = new RegExp(
  "(?:on\\s{0,3})?(" +
    _mod10053.ORDINAL_NUMBER_PATTERN +
    ")(?:\\s{0,3}(?:al|\\-|\\\u2013|fino|alle|allo)?\\s{0,3}(" +
    _mod10053.ORDINAL_NUMBER_PATTERN +
    "))?(?:-|/|\\s{0,3}(?:dal)?\\s{0,3})(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10053.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s{0,3})(" +
    _mod10053.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", ENMonthNameLittleEndianParser(10053).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            ENMonthNameLittleEndianParser(9885).findYearClosestToRef(createParsingResult.refDate, result, tmp4),
          );
        }
        if (index[2]) {
          const start3 = parsingResult.start;
          const result1 = ENMonthNameLittleEndianParser(10053).parseOrdinalNumberPattern(index[2]);
