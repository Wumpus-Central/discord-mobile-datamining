...
export const useAnimatedGestureHandler = function useAnimatedGestureHandler(handlers, items10) {
  _require = handlers;
  const handler = require("module_1782").useHandler(handlers, items10);
  ...
  let event = fn;
  if (!useWeb) {
    event = require("module_1784").useEvent(fn, ["onGestureHandlerStateChange", "onGestureHandlerEvent"], tmp4);
    const tmpResult = require("module_1784");
  }
  return event;
};