...
const entry = {
  ...
  value: function render() {
    ...
    ({ dx, dy, result } = props);
    const items = [
      timestampProducer(FeDropShadow(7959), { in: str, stdDeviation: props.stdDeviation }),
      timestampProducer(FeDropShadow(7961), { dx, dy, result: "offsetblur" }),
      timestampProducer(FeDropShadow(7963), {
        floodColor: self.props.floodColor,
        floodOpacity: self.props.floodOpacity,
      }),
      timestampProducer(FeDropShadow(7952), { in2: "offsetblur", operator: "in" }),
    ];
