...
obj[524] = {
  ...
  repeatable: true,
  description(value) {
    const parts = _mod5319.getStringValue(value).split(":");
    let str2 = "";
    let str3 = "";
    if (parts[3]) {
      str3 = `/${tmp[3]}`;
...
obj[713] = {
  ...
  description(value, ObjectData_Preview_File_Format) {
    ...
    if (ObjectData_Preview_File_Format["ObjectData Preview File Format"]) {
      const stringValue1 = _mod5319.getStringValue(ObjectData_Preview_File_Format["ObjectData Preview File Format"].value);
      if (obj[stringValue1]) {
        if (obj[stringValue1][stringValue]) {
          return obj[stringValue1][stringValue];
        }
      }
      const tmpResult = _mod5319;
    }
    return stringValue;
  }
};
