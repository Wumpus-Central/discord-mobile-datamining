// _runtime/metro/10671__.js
import ENDefaultConfiguration2 from "../10672_ENDefaultConfiguration.js";
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(10678).ParsingResult) {
        ...
        debug.debug(() =>
          console.log("" + ParsingContext.constructor.name + " extracted (at index=" + index + ") '" + text1 + "'"),
        );
        items.push(parsingResult);
        const substr1 = text.substring(index + text1.length);
        const match2 = patternResult.exec(substr1);
      } else if (!(extractResult instanceof ParsingContext(10678).ParsingComponents)) {
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(10678).ParsingResult(
        self.reference,
        sum,
        substr,
        parsingComponents,
