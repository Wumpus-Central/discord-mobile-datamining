  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(prima|dal)$/i)) {
        reverseDurationResult = ENMergeRelativeDateRefiner(10562).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeDateRefiner(10563).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeDateRefiner(10563).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeDateRefiner(10563).ParsingResult(
        start.reference,
        text.index,
