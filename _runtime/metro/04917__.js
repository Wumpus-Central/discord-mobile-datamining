...
arg5.default = {
  parseAppMarkers(byteLength, flag2) {
    ...
    if (module(4918).USE_JPEG) {
      if (tmpResult1.isJpegFile(byteLength)) {
        const tmpResult2 = tmp(4921);
        const findJpegOffsetsResult = tmp(4921).findJpegOffsets(byteLength);
        obj = { fileType: null };
    ...
    if (module(4918).USE_PNG) {
      if (tmpResult3.isPngFile(byteLength)) {
        const tmpResult4 = tmp(4922);
        const findPngOffsetsResult = tmp(4922).findPngOffsets(byteLength, flag2);
        obj1 = { fileType: null };
    ...
    if (module(4918).USE_HEIC) {
      if (tmpResult5.isHeicFile(byteLength)) {
        const tmpResult6 = tmp(4923);
        const findHeicOffsetsResult = tmp(4923).findHeicOffsets(byteLength);
    ...
    if (module(4918).USE_AVIF) {
      if (tmpResult7.isAvifFile(byteLength)) {
        const tmpResult8 = tmp(4927);
        const findAvifOffsetsResult = tmp(4927).findAvifOffsets(byteLength);
    ...
    if (module(4918).USE_WEBP) {
      if (tmpResult9.isWebpFile(byteLength)) {
        const tmpResult10 = tmp(4928);
        const findOffsetsResult = tmp(4928).findOffsets(byteLength);
