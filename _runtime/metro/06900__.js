...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === _mod6903.State.UNDETERMINED) {
        if (handlerTag.state === _mod6903.State.BEGAN) {
      ...
      if (handlerTag.oldState === _mod6903.State.BEGAN) {
        if (handlerTag.state === _mod6903.State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === _mod6903.State.END) {
          if (handlerTag.oldState === _mod6903.State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== _mod6903.State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== _mod6903.State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = _mod6904.GestureStateManager;
