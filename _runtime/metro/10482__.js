      ...
      if (dateResult > start6.date()) {
        ...
        if (start20.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(10466).addDuration(dateResult2, { day: 7 }) > dateResult1) {
            const addDurationResult = AbstractMergeDateRangeRefiner(10466).addDuration(dateResult2, { day: 7 });
        ...
        if (start7.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(10466).addDuration(dateResult1, { day: -7 }) < dateResult2) {
            const addDurationResult1 = AbstractMergeDateRangeRefiner(10466).addDuration(dateResult1, { day: -7 });
        ...
        if (start8.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(10466).addDuration(dateResult2, { year: 1 }) > dateResult1) {
            ...
            start11.imply("year", AbstractMergeDateRangeRefiner(10466).addDuration(dateResult2, { year: 1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult2 = AbstractMergeDateRangeRefiner(10466).addDuration(dateResult2, { year: 1 });
          }
        }
        ...
        if (start9.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(10466).addDuration(dateResult1, { year: -1 }) < dateResult2) {
            ...
            start10.imply("year", AbstractMergeDateRangeRefiner(10466).addDuration(dateResult1, { year: -1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult3 = AbstractMergeDateRangeRefiner(10466).addDuration(dateResult1, { year: -1 });
          }
        }
