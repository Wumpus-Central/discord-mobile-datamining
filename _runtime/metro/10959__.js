...
const items = [
  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      const parseDurationResult = ENMergeRelativeDateRefiner(10943).parseDuration(text.text);
      let reverseDurationResult = parseDurationResult;
      if (null != str.match(/\s+(prima|dal)$/i)) {
        reverseDurationResult = ENMergeRelativeDateRefiner(10776).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeDateRefiner(10777).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeDateRefiner(10777).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeDateRefiner(10777).ParsingResult(start.reference, text.index, "" + text.text + arg0 + start.text, relativeFromReference);
    }
  }
];
