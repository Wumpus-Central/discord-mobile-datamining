...
const regExp = new RegExp("(?:on\\s{0,3})?(" + _mod10431.ORDINAL_NUMBER_PATTERN + ")(?:\\s{0,3}(?:to|\\-|\\\u2013|until|through|till)?\\s{0,3}(" + _mod10431.ORDINAL_NUMBER_PATTERN + "))?(?:-|/|\\s{0,3}(?:of)?\\s{0,3})(" + repeatedTimeunitPattern.matchAnyPattern(_mod10431.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s{0,3})(" + _mod10431.YEAR_PATTERN + "(?!\\w)))?(?=\\W|$)", "i");
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start.imply("year", ENMonthNameLittleEndianParser(10433).findYearClosestToRef(createParsingResult.refDate, result, tmp4));
        }
        if (index[2]) {
