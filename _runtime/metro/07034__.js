...
function getHandler(arg0, onBegin) {
  ...
  } else if (_mod7004.CALLBACK_TYPE.CHANGE === arg0) {
            ...
            if (typeof isStateChangeEvent === "function") {
              if (null != handlerTag.oldState) {
                if (handlerTag.oldState === _mod6989.State.UNDETERMINED) {
                  if (handlerTag.state === _mod6989.State.BEGAN) {
                    let tmp38 = runWorklet(_mod7004.CALLBACK_TYPE.BEGAN, tmp2, handlerTag);
                  }
                }
                if (handlerTag.oldState === _mod6989.State.BEGAN) {
                  if (handlerTag.state === _mod6989.State.ACTIVE) {
                    let tmp18 = runWorklet(_mod7004.CALLBACK_TYPE.START, tmp2, handlerTag);
                    sharedValue1.value[tmp2.handlerTag] = undefined;
                  }
                }
                if (handlerTag.oldState !== handlerTag.state) {
                  if (handlerTag.state === _mod6989.State.END) {
                    if (handlerTag.oldState === _mod6989.State.ACTIVE) {
                      let flag3 = true;
                      let tmp32 = runWorklet(_mod7004.CALLBACK_TYPE.END, tmp2, handlerTag, true);
                    }
                    let flag4 = true;
                    let tmp36 = runWorklet(_mod7004.CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, true);
                  }
                }
                let tmp20 = handlerTag.state !== _mod6989.State.FAILED;
                if (tmp20) {
                  tmp20 = handlerTag.state !== _mod6989.State.CANCELLED;
                }
                if (!tmp20) {
                ...
                if (!tmp20) {
                  if (handlerTag.oldState === _mod6989.State.ACTIVE) {
                    let flag = false;
                    let tmp24 = runWorklet(_mod7004.CALLBACK_TYPE.END, tmp2, handlerTag, false);
                  }
                  let flag2 = false;
                  let tmp28 = runWorklet(_mod7004.CALLBACK_TYPE.FINALIZE, tmp2, handlerTag, false);
                }
                ...
                if (null != handlerTag.eventType) {
                  ...
                  if (tmp4) {
                    ...
                    let tmp41Result = runWorklet(_mod7004.CALLBACK_TYPE.CHANGE, tmp2, result);
