...
import allowedNativeProps2 from "../07043_allowedNativeProps.js";
import _mod7055 from "07055__.js";
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod7055.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod7028.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod7055.shouldHandleTouchEvents(useAnimated);
};
export const prepareConfigForNativeSide = function prepareConfigForNativeSide(arg0, shouldUseReanimatedDetector) {
  ...
  while (tmp12 !== undefined) {
    ...
    let Reanimated = _mod7028.Reanimated;
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod7055.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod7055.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
