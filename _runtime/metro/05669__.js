...
export default importAllResult.memo(function Header(navigation) {
  ...
  const callback = importAllResult.useCallback(navigation(5670)(() => {
    let obj = navigation;
    if (tmp) {
      obj = {};
  ...
  const context = importAllResult.useContext(navigation(5671));
  ...
  const tmp4Result = navigation(5672);
  const tmp8 = jsx;
  obj.title = route(5645).getHeaderTitle(options, route.name);
