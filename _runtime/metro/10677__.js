...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (isNaN(num)) {
        num = ZHHantDeadlineFormatParser(10676).zhStringToNumber(index[1]);
      }
      if (isNaN(num)) {
        num = 3;
        if ("\u5E7E" !== index[1]) {
      ...
      if (index[2][0].match(/[日天星禮月年]/)) {
        if ("\u65E5" != str3) {
          ...
          const addDurationResult = ZHHantDeadlineFormatParser(10563).addDuration(createParsingResult.refDate, obj);
        ...
        const addDurationResult1 = ZHHantDeadlineFormatParser(10563).addDuration(createParsingResult.refDate, obj);
