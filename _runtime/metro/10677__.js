...
const combined =
  "(?:on\\s*?)?(" +
  _mod10675.ORDINAL_NUMBER_PATTERN +
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          parsingComponents.assign("year", NLMonthNameMiddleEndianParser(10675).parseYear(index[4]));
        } else {
          parsingComponents.imply(
            "year",
            NLMonthNameMiddleEndianParser(10582).findYearClosestToRef(createParsingComponents.refDate, result, tmp3),
          );
        }
        if (index[2]) {
          const result1 = NLMonthNameMiddleEndianParser(10675).parseOrdinalNumberPattern(index[2]);
