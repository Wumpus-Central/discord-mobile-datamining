...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = _mod6967.isNativeAnimatedEvent(useAnimated.onUpdate);
  }
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6940.Reanimated;
  }
  if (result) {
  ...
  useAnimated.needsPointerData = _mod6967.shouldHandleTouchEvents(useAnimated);
};
...
export const useClonedAndRemappedConfig = function useClonedAndRemappedConfig(gestureHandlerProps) {
  ...
  return useMemo(() => {
    ...
    if (!useAnimated) {
      useAnimated = _mod6967.isNativeAnimatedEvent(tmp3.onUpdate);
    }
    ...
    tmp3.needsPointerData = _mod6967.shouldHandleTouchEvents(tmp3);
    return tmp3;
  }, items);
};
