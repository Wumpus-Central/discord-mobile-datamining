...
class GenericTouchable {
  constructor() {
    ...
    tmp3Result.onHandlerStateChange = (nativeEvent) => {
      ...
      if (state !== GenericTouchable(6079).State.CANCELLED) {
        if (state !== GenericTouchable(6079).State.FAILED) {
          if (state === GenericTouchable(6079).State.BEGAN) {
            if (closure_0.STATE === closure_0.UNDETERMINED) {
              closure_0.handlePressIn();
            }
          }
          if (state === GenericTouchable(6079).State.END) {
...
let items = [
  ...
  },
  {
    ...
    value: function render() {
      ...
      return jsx(GenericTouchable(6186).LegacyBaseButton, { style: self.props.containerStyle, onHandlerStateChange: null, onGestureEvent: null, hitSlop: null, userSelect: null, shouldActivateOnStart: null, disallowInterruption: null, testID: null, touchSoundDisabled: null, enabled: null });
    }
  }
];
