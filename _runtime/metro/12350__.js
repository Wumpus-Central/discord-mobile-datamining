  ...
  value: function captureException(arg0, arg1, arg2) {
    ...
    if (obj2.checkOrSetAlreadyCaught(arg0)) {
      if (BaseClient(12323).DEBUG_BUILD) {
        const logger = BaseClient(12295).logger;
        logger.log(c4);
      }
      return uuid4Result;
    } else {
...
let items = [
  ...
  },
  {
    ...
    value: function sendEvent(arg0) {
      ...
      let eventEnvelope = BaseClient(12338).createEventEnvelope(
        arg0,
        self._dsn,
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(
        BaseClient(12338).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel),
      );
    },
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12323).DEBUG_BUILD) {
        let logger = BaseClient(12295).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12319).resolvedSyncPromise({});
      const tmp2Result = BaseClient(12319);
    },
  },
  {
      ...
      return currentScope(self[15])
        ...
        .then((contexts) => {
          if (null === contexts) {
            ...
            const obj4 = {
              dynamicSamplingContext: BaseClient(12331).getDynamicSamplingContextFromScope(self, currentScope),
            };
      ...
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12352).SentryError(
      ...
      let nextPromise = self
        ...
        .then((result) => {
          if (null === result) {
            ...
            let sentryError = new BaseClient(12352).SentryError(
      ...
      return self
        ...
        .then((sdkProcessingMetadata) => {
          if (null === sdkProcessingMetadata) {
            ...
            const _HermesInternal = HermesInternal;
            const sentryError = new BaseClient(12352).SentryError(
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = BaseClient(12323).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = BaseClient(12295).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tunnel = self._options.tunnel;
          if (tunnel) {
            tunnel = BaseClient(12342).dsnToString(self._dsn);
            const tmpResult2 = BaseClient(12342);
          }
          self.sendEnvelope(BaseClient(12353).createClientReportEnvelope(_clearOutcomesResult, tunnel));
          const tmpResult = BaseClient(12353);
        } else if (DEBUG_BUILD) {
          const logger3 = BaseClient(12295).logger;
          logger3.log("No dsn provided, will not send outcomes");
        }
      } else if (BaseClient(12323).DEBUG_BUILD) {
        const logger2 = BaseClient(12295).logger;
