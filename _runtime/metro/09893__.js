  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          addTagResult.assign("year", ENMonthNameMiddleEndianParser(9883).parseYear(index[4]));
        } else {
          addTagResult.imply("year", ENMonthNameMiddleEndianParser(9885).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = ENMonthNameMiddleEndianParser(9883).parseOrdinalNumberPattern(index[3]);
