...
import _modDef5559 from "05559__.js";
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          obj3.name = combined;
          obj3.value = items;
          obj3.description = getTagDescription(_modDef5558.iptc[uint16], items, obj, encoding);
          let tmp17 = _modDef5558.iptc[uint16] && _modDef5558.iptc[uint16].repeatable;
          if (tmp17) {
            obj3.repeatable = true;
          }
          let tmp18 = _modDef5558.iptc[uint16] && undefined !== _modDef5558.iptc[uint16].encoding_name;
          if (tmp18) {
            let obj5 = _modDef5558.iptc[uint16];
