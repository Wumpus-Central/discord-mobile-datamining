...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      ...
      let tmp18 = handlerTag.state !== _mod6079.State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== _mod6079.State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = _mod6080.GestureStateManager;
