...
import PanGestureHandler from "../07338_PanGestureHandler.js";
import _mod7352 from "07352__.js";
import _mod7353 from "07353__.js";
import _mod7359 from "07359__.js";
...
export const Card = function Card(shadowEnabled) {
  ...
  const first1 = interpolationIndex(
    opening.useState(() => {
      value = new RN.Value(_mod7353.getInvertedMultiplier(gestureDirection, "rtl" === direction));
      return value;
    }),
    1,
  )[0];
  ...
  closure_32 = num(1506)(() => {
    if (null == ref3.current) {
      const InteractionManager = _mod7359.InteractionManager;
  ...
  const layoutEffect = opening.useLayoutEffect(() => {
    ...
    first1.setValue(_mod7353.getInvertedMultiplier(gestureDirection, "rtl" === direction));
  }, items);
  opening.useRef(null);
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (ref7.current) {
        if (typeof getAnimateToValue === "function") {
          if (ref7.current.closing) {
            let num2 = _mod7352.getDistanceForDirection(tmp9, tmp10, "rtl" === tmp11);
          } else {
            num2 = 0;
          }
          tmp7 = num2;
      ...
      if (typeof getAnimateToValue === "function") {
        if (closing) {
          let num3 = _mod7352.getDistanceForDirection(layout, gestureDirection, "rtl" === direction);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7352.getDistanceForDirection(layout, gestureDirection, "rtl" === direction));
              const obj4 = { closing };
              closure_34(obj4);
            }
          }
  ...
  if (typeof backgroundColor === "string") {
    tmp16 = 0 === tmp5(5945)(backgroundColor).alpha();
    const obj14 = tmp5(5945)(backgroundColor);
  }
  ...
  let merged = Object.assign(
    flag2(7361).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }),
  );
  if (typeof direction === "function") {
    ...
    items13[1] = tmp19(tmp18(7362).CardContent, obj12);
    obj9.children = items13;
    obj8.children = tmp17(tmp25, obj9);
    obj7.children = tmp19(tmp18(7338).PanGestureHandler, obj8);
    items5[2] = tmp19(tmp20.View, obj7);
    obj3.children = items5;
    return tmp17(flag2(7360).CardAnimationContext.Provider, obj3);
  } else {
    throw new TypeError("Trying to call a non-function");
  }
  const tmp18Result = flag2(7361);
};
