// _runtime/metro/12440__addTracingHeadersToFetchRequest.js
import errorCallback from "../12331_errorCallback.js";
import SyncPromise from "../12359_SyncPromise.js";
import hasTracingEnabled from "../12367_hasTracingEnabled.js";
import getTraceData from "../12406_getTraceData.js";
import closure_2 from "../00853__toArray.js";
import __SENTRY_DEBUG__ from "12334___SENTRY_DEBUG__.js";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12352).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12352);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(12352).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(12417).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(12417);
    }
    const activeSpan = setHttpStatus(12340).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(12350).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(12350).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(12369).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(12369);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12367);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12368).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12340);
  }
};