  ...
  value: function innerPattern() {
    const regExp = new RegExp(
      "(?:\\s*((?:n\u00E4chste|kommende|folgende|letzte|vergangene|vorige|vor(?:her|an)gegangene)(?:s|n|m|r)?|vor|in)\\s*)?(" +
        DETimeUnitAgoFormatParser(10567).NUMBER_PATTERN +
        ")?(?:\\s*(n\u00E4chste|kommende|folgende|letzte|vergangene|vorige|vor(?:her|an)gegangene)(?:s|n|m|r)?)?\\s*(" +
        DETimeUnitAgoFormatParser(10521).matchAnyPattern(DETimeUnitAgoFormatParser(10567).TIME_UNIT_DICTIONARY) +
        ")",
      "i",
    );
    return regExp;
  ...
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      ...
      if (formatted) {
        ...
        if (isMatch) {
          reverseDurationResult = DETimeUnitAgoFormatParser(10523).reverseDuration(obj);
        }
        const ParsingComponents = DETimeUnitAgoFormatParser(10524).ParsingComponents;
