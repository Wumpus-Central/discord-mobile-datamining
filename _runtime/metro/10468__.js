...
const items = [
  entry,
  {
    ...
    value: function mergeResults(arg0, start, text, arg3) {
      ...
      if (null != str.match(/^-/i)) {
        reverseDurationResult = ENMergeRelativeAfterDateRefiner(10439).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeAfterDateRefiner(10440).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeAfterDateRefiner(10440).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeAfterDateRefiner(10440).ParsingResult(
        reference,
        index,
