...
import PanGestureHandler from "../07112_PanGestureHandler.js";
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (typeof getAnimateToValue === "function") {
        if (closing) {
          let num3 = _mod7126.getDistanceForDirection(layout, gestureDirection, "rtl" === direction);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7126.getDistanceForDirection(layout, gestureDirection, "rtl" === direction));
              const obj4 = { closing };
              closure_34(obj4);
            }
          }
  ...
  let merged = Object.assign(
    flag2(7135).gestureActivationCriteria({ layout, direction, gestureDirection, gestureResponseDistance }),
  );
