  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          parsingComponents.assign("year", ENMonthNameMiddleEndianParser(10671).parseYear(index[4]));
        } else {
          parsingComponents.imply("year", ENMonthNameMiddleEndianParser(10503).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[3]) {
          const result1 = ENMonthNameMiddleEndianParser(10671).parseOrdinalNumberPattern(index[3]);
