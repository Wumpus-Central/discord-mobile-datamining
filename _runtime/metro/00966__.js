...
function convertJSSelfProfileToSampledFormat(samples) {
  ...
  if (first) {
    ...
    let result = require("00682__.js").browserPerformanceTimeOrigin();
...
export const createProfileChunkPayload = function createProfileChunkPayload(frames, _client, _profilerId) {
  if (null == frames) {
    ...
    const typeError = new TypeError(
      "Cannot construct profiling event envelope without a valid profile. Got " + frames + " instead.",
    );
    throw typeError;
  } else {
    const items = [];
