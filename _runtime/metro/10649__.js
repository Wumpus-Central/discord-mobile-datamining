  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(prima|dal)$/i)) {
        reverseDurationResult = ENMergeRelativeDateRefiner(10466).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeDateRefiner(10467).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeDateRefiner(10467).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeDateRefiner(10467).ParsingResult(
        start.reference,
        text.index,
