...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod13059.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod13059.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod13059.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod13102.getFilenameToDebugIdMap(arg1);
  try {
