...
const regExp = new RegExp(
  "((?:in)\\s*)?(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10633.MONTH_DICTIONARY) +
    ")\\s*(?:[,-]?\\s*(" +
    _mod10633.YEAR_PATTERN +
    ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)",
  "i",
);
  ...
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (index[3]) {
        ...
        start4.assign("year", ENMonthNameParser(10633).parseYear(index[3]));
      } else {
        const start3 = parsingResult.start;
        start3.imply("year", ENMonthNameParser(10465).findYearClosestToRef(createParsingResult.refDate, 1, tmp9));
      }
