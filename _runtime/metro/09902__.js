    ...
    value: function mergeResults(arg0, start, start2) {
      ...
      if (dateResult > start6.date()) {
        ...
        if (start20.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(9886).addDuration(dateResult2, { day: 7 }) > dateResult1) {
            const addDurationResult = AbstractMergeDateRangeRefiner(9886).addDuration(dateResult2, { day: 7 });
        ...
        if (start7.isOnlyWeekdayComponent()) {
          if (AbstractMergeDateRangeRefiner(9886).addDuration(dateResult1, { day: -7 }) < dateResult2) {
            const addDurationResult1 = AbstractMergeDateRangeRefiner(9886).addDuration(dateResult1, { day: -7 });
        ...
        if (start8.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(9886).addDuration(dateResult2, { year: 1 }) > dateResult1) {
            const start11 = start2.start;
            start11.imply("year", AbstractMergeDateRangeRefiner(9886).addDuration(dateResult2, { year: 1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult2 = AbstractMergeDateRangeRefiner(9886).addDuration(dateResult2, { year: 1 });
          }
        }
        const start9 = start.start;
        if (start9.isDateWithUnknownYear()) {
          if (AbstractMergeDateRangeRefiner(9886).addDuration(dateResult1, { year: -1 }) < dateResult2) {
            const start10 = start.start;
            start10.imply("year", AbstractMergeDateRangeRefiner(9886).addDuration(dateResult1, { year: -1 }).getFullYear());
            tmp5 = start2;
            obj = start;
            const addDurationResult3 = AbstractMergeDateRangeRefiner(9886).addDuration(dateResult1, { year: -1 });
          }
        }
