// _runtime/metro/05553__.js
import _wrapNativeSuperDefault from "00158__wrapNativeSuper.js";
import _modDef5554 from "05554__.js";
import calculateGPSValueDefault from "../05556_calculateGPSValue.js";
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5518).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5518).objectAssign(tmp3, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5518);
        }
        tmp3 = obj;
      });
      continue;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5518).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5518);
  return {
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5518).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5518);
    const obj6 = {};
    ...
    ParseError(5518).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5518);
  }
  ParseError(5518).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
