...
import rubberBandDecay from "rubberBandDecay" /* 1715 */;
import rigidDecay from "rigidDecay" /* 1716 */;
  ...
  if (clamp.clamp) {
    ...
    if (Array.isArray(clamp.clamp)) {
      if (2 !== clamp.clamp.length) {
        ...
        const reanimatedError = new _mod1649.ReanimatedError("`clamp array` must contain 2 items but is given " + clamp.clamp.length + ".");
        throw reanimatedError;
      }
    } else {
      const _HermesInternal = HermesInternal;
      const reanimatedError1 = new _mod1649.ReanimatedError("`config.clamp` must be an array but is " + typeof clamp.clamp + ".");
      throw reanimatedError1;
    }
  }
  if (clamp.velocityFactor <= 0) {
    const _HermesInternal2 = HermesInternal;
    const reanimatedError2 = new _mod1649.ReanimatedError("`config.velocityFactor` must be greater then 0 but is " + clamp.velocityFactor + ".");
    throw reanimatedError2;
  } else if (clamp.rubberBandEffect) {
    if (!clamp.clamp) {
      const reanimatedError3 = new _mod1649.ReanimatedError("You need to set `clamp` property when using `rubberBandEffect`.");
...
fn = function n(userConfig, fn) {
  ...
  fn.__closure = { userConfig, isValidRubberBandConfig: require("module_1714").isValidRubberBandConfig, rubberBandDecay: require("rubberBandDecay").rubberBandDecay, rigidDecay: require("rigidDecay").rigidDecay, validateConfig, callback: fn, getReduceMotionForAnimation: require("module_1678").getReduceMotionForAnimation };
  fn.__workletHash = 17099614658252;
  fn.__initData = __initData;
  return obj.defineAnimation(0, fn);
};
fn.__closure = { defineAnimation: fn(1678).defineAnimation, isValidRubberBandConfig: fn(1714).isValidRubberBandConfig, rubberBandDecay: fn(1715).rubberBandDecay, rigidDecay: fn(1716).rigidDecay, validateConfig, getReduceMotionForAnimation: fn(1678).getReduceMotionForAnimation };
