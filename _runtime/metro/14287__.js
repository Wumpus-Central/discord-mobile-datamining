...
function addLikelySubtags(arr3) {
  ...
  if (script) {
    if (region) {
      ...
      const tmp4 = _mod14353.likelySubtags[_mod14353.emitUnicodeLanguageId(undefined, obj)];
      if (tmp4) {
        const result = _mod14353.parseUnicodeLanguageId(tmp4);
        ...
        return _mod14353.emitUnicodeLocaleId(parseUnicodeLocaleIdResult);
      }
    }
  }
  if (script) {
    const obj4 = { lang, script, variants: [] };
    const tmp5 = _mod14353.likelySubtags[_mod14353.emitUnicodeLanguageId(undefined, obj4)];
    if (tmp5) {
      const result1 = _mod14353.parseUnicodeLanguageId(tmp5);
      let items = variants;
      if (undefined === variants) {
        items = [];
      }
  ...
  if (region) {
    ...
    const tmp6 = _mod14353.likelySubtags[_mod14353.emitUnicodeLanguageId(undefined, obj7)];
    if (tmp6) {
      const result2 = _mod14353.parseUnicodeLanguageId(tmp6);
      let items1 = variants;
      if (undefined === variants) {
        items1 = [];
      }
  ...
  let tmp7 = _mod14353.likelySubtags[lang];
  if (!tmp7) {
    const obj10 = { lang: "und", script, variants: [] };
    tmp7 = _mod14353.likelySubtags[_mod14353.emitUnicodeLanguageId(undefined, obj10)];
  }
  if (tmp7) {
    const result3 = _mod14353.parseUnicodeLanguageId(tmp7);
    let items2 = variants;
    if (undefined === variants) {
      items2 = [];
    }
...
let tmp2 = (() => {
      ...
      try {
        ...
        tmp4 = (function removeLikelySubtags(arr3) {
          ...
          if (tmp2) {
            ...
            const obj3 = { variants: [] };
            const result = Locale(14353).emitUnicodeLanguageId(
              obj.__assign(Locale(1160).__assign({}, Locale(14353).parseUnicodeLanguageId(tmp2)), obj3),
            );
            const parseUnicodeLocaleIdResult = Locale(14353).parseUnicodeLocaleId(arr3);
            ({ lang, script, region, variants } = parseUnicodeLocaleIdResult.lang);
            const obj4 = { lang, variants: [] };
            if (closure_1_6(Locale(14353).emitUnicodeLanguageId(obj4)) === result) {
              const tmp3Result = Locale(1160);
              const tmp3Result6 = Locale(1160);
              if (undefined === variants) {
                variants = [];
              }
              if (!lang) {
              ...
              obj5.lang = obj6;
              let emitUnicodeLocaleIdResult = Locale(14353).emitUnicodeLocaleId(
                tmp3Result.__assign(Locale(1160).__assign({}, parseUnicodeLocaleIdResult), obj5),
              );
              const __assignResult = Locale(1160).__assign({}, parseUnicodeLocaleIdResult);
            } else {
              if (region) {
                const obj7 = { lang, region, variants: [] };
                if (closure_1_6(Locale(14353).emitUnicodeLanguageId(obj7)) === result) {
                  const tmp3Result7 = Locale(1160);
                  let items = variants;
                  const tmp3Result8 = Locale(1160);
                  if (undefined === variants) {
                    items = [];
                  }
                  let str2 = lang;
                  ...
                  obj8.lang = obj9;
                  emitUnicodeLocaleIdResult = Locale(14353).emitUnicodeLocaleId(
                    tmp3Result7.__assign(Locale(1160).__assign({}, parseUnicodeLocaleIdResult), obj8),
                  );
                  const __assignResult1 = Locale(1160).__assign({}, parseUnicodeLocaleIdResult);
                }
              }
              ...
              if (script) {
                ...
                emitUnicodeLocaleIdResult = arr3;
                if (closure_1_6(Locale(14353).emitUnicodeLanguageId(obj10)) === result) {
                  const tmp3Result9 = Locale(1160);
                  let items1 = variants;
                  const tmp3Result10 = Locale(1160);
                  if (undefined === variants) {
                    items1 = [];
                  }
                  let str = lang;
                  ...
                  obj11.lang = obj12;
                  emitUnicodeLocaleIdResult = Locale(14353).emitUnicodeLocaleId(
                    tmp3Result9.__assign(Locale(1160).__assign({}, parseUnicodeLocaleIdResult), obj11),
                  );
                  const __assignResult2 = Locale(1160).__assign({}, parseUnicodeLocaleIdResult);
                }
              }
            }
  ...
  Object.defineProperty(Locale.prototype, "baseName", {
    get() {
      return Locale(14353).emitUnicodeLanguageId(
        Locale(14353).parseUnicodeLanguageId(module_14288.default(this).locale),
      );
    },
    enumerable: false,
    configurable: true,
