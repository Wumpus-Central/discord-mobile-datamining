    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", PTMonthNameLittleEndianParser(10654).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            PTMonthNameLittleEndianParser(10574).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
