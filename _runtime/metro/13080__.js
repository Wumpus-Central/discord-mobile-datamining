  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 13054).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(13044).getRootSpan(self) === self;
        const tmpResult = SentrySpan(13044);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(13044).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = SentrySpan(13044);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(13067).DEBUG_BUILD) {
        const logger = SentrySpan(13039).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(13053).timestampInSeconds();
          const tmpResult = SentrySpan(13053);
        }
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(13082).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(13082);
            const client1 = SentrySpan(13066).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(13066);
          } else {
            if (SentrySpan(13067).DEBUG_BUILD) {
              const logger = SentrySpan(13039).logger;
          ...
          if (result) {
            let scope = SentrySpan(13068).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(13066).getCurrentScope();
              const tmpResult7 = SentrySpan(13066);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(13068);
          }
        }
      } else {
        SentrySpan(13044);
      }
      const obj = SentrySpan(13066);
    },
  },
  {
