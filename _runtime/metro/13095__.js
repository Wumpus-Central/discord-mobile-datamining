  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      const obj6 = {
        ...
        op: this._attributes[SentrySpan(undefined, 13069).SEMANTIC_ATTRIBUTE_SENTRY_OP],
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = SentrySpan(13059).getRootSpan(self) === self;
        const tmpResult = SentrySpan(13059);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = SentrySpan(13059).getRootSpan(self);
        spanId = rootSpan.spanContext().spanId;
        const tmpResult2 = SentrySpan(13059);
      }
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(13082).DEBUG_BUILD) {
        const logger = SentrySpan(13054).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = SentrySpan(13068).timestampInSeconds();
          const tmpResult = SentrySpan(13068);
        }
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = SentrySpan(13097).createSpanEnvelope(items, client);
            const tmpResult = SentrySpan(13097);
            const client1 = SentrySpan(13081).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
            ...
            const tmpResult5 = SentrySpan(13081);
          } else {
            if (SentrySpan(13082).DEBUG_BUILD) {
              const logger = SentrySpan(13054).logger;
          ...
          if (result) {
            let scope = SentrySpan(13083).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(13081).getCurrentScope();
              const tmpResult7 = SentrySpan(13081);
            }
            scope.captureEvent(result);
            const tmpResult6 = SentrySpan(13083);
          }
        }
      } else {
        SentrySpan(13059);
      }
      const obj = SentrySpan(13081);
    },
  },
  {
