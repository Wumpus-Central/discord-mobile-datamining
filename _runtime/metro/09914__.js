...
const regExp = new RegExp("(this|last|past|next|after|\\+|-)\\s*(" + _mod9883.TIME_UNITS_PATTERN + ")(?=\\W|$)", "i");
const regExp1 = new RegExp(
  "(this|last|past|next|after|\\+|-)\\s*(" + _mod9883.TIME_UNITS_NO_ABBR_PATTERN + ")(?=\\W|$)",
  "i",
);
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      ...
      const parseDurationResult = ENTimeUnitCasualRelativeFormatParser(9883).parseDuration(arg1[2]);
      if (parseDurationResult) {
        if ("last" !== formatted) {
          if ("past" !== formatted) {
            let reverseDurationResult = parseDurationResult;
          }
          const ParsingComponents = ENTimeUnitCasualRelativeFormatParser(9887).ParsingComponents;
          return ParsingComponents.createRelativeFromReference(reference.reference, reverseDurationResult);
        }
        reverseDurationResult = ENTimeUnitCasualRelativeFormatParser(9886).reverseDuration(parseDurationResult);
      } else {
        return null;
      }
    },
