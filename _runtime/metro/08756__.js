// _runtime/metro/08756__.js
import extractBrushDefault from "../08750_extractBrush.js";
import _modDef8759 from "08759__.js";
  ...
  constructor(arg0) {
    ...
    tmp3Result.setNativeProps = (obj) => {
      for (const key10004 in arg0) {
        let BrushProperties = Shape(8757).BrushProperties;
    ...
    tmp3Result.getBBox = (arg0) => {
      ...
      return Shape(8758).default.getBBox(tmp5, { fill: fill2, stroke: stroke2, markers: markers2, clipped: clipped2 });
    };
    tmp3Result.getCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8758).default;
      return new importDefaultResult1Result(Shape(8758).default.getCTM(tmp));
    };
    tmp3Result.getScreenCTM = () => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8758).default;
      return new importDefaultResult1Result(Shape(8758).default.getScreenCTM(tmp));
    };
    tmp3Result.isPointInFill = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8758).default.isPointInFill(tmp, arg0);
    };
    tmp3Result.isPointInStroke = (arg0) => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8758).default.isPointInStroke(tmp, arg0);
    };
    tmp3Result.getTotalLength = () => {
      const tmp = findNodeHandle(closure_0.root);
      return Shape(8758).default.getTotalLength(tmp);
    };
    tmp3Result.getPointAtLength = (length) => {
      const tmp = findNodeHandle(closure_0.root);
      const _default = Shape(8758).default;
      const obj = { length };
      return new importDefaultResult1Result1(Shape(8758).default.getPointAtLength(tmp, { length }));
    };
