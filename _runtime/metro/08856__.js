...
const entry = {
  ...
  value: function render() {
    ...
    ({ dx, dy, result } = props);
    const items = [
      timestampProducer(FeDropShadow(8857), { in: str, stdDeviation: props.stdDeviation }),
      timestampProducer(FeDropShadow(8859), { dx, dy, result: "offsetblur" }),
      timestampProducer(FeDropShadow(8861), {
        floodColor: self.props.floodColor,
        floodOpacity: self.props.floodOpacity,
      }),
      timestampProducer(FeDropShadow(8850), { in2: "offsetblur", operator: "in" }),
    ];
