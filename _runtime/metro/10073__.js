...
import _mod10072 from "10072__.js";
...
const regExp = new RegExp(
  "(?:den\\s*?)?([0-9]{1,2})(?:\\s*(?:till|\\-|\\\u2013|\\s)\\s*([0-9]{1,2}))?\\s*(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10072.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)([0-9]{4}(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = SVMonthNameLittleEndianParser(10072).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", SVMonthNameLittleEndianParser(10072).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            SVMonthNameLittleEndianParser(9885).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
