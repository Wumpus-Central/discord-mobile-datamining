  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = ENMergeRelativeFollowByDateRefiner(10776).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeFollowByDateRefiner(10777).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeFollowByDateRefiner(10777).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10777).ParsingResult(
        start.reference,
        text.index,
