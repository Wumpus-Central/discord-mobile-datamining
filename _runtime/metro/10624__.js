...
const regExp = new RegExp(
  "(?:on\\s*?)?(" +
    _mod10622.ORDINAL_NUMBER_PATTERN +
    ")(?:\\s*(?:au|\\-|\\\u2013|jusqu'au?|\\s)\\s*(" +
    _mod10622.ORDINAL_NUMBER_PATTERN +
    "))?(?:-|/|\\s*(?:de)?\\s*)(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10622.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)(" +
    _mod10622.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", FRMonthNameLittleEndianParser(10622).parseYear(index[4]));
        } else {
          const start = parsingResult.start;
          start.imply(
            "year",
            FRMonthNameLittleEndianParser(10561).findYearClosestToRef(createParsingResult.refDate, result, tmp4),
          );
        }
        if (index[2]) {
          const start3 = parsingResult.start;
          const result1 = FRMonthNameLittleEndianParser(10622).parseOrdinalNumberPattern(index[2]);
