...
const regExp = new RegExp(
  "(?:am\\s*?)?(?:den\\s*?)?([0-9]{1,2})\\.(?:\\s*(?:bis(?:\\s*(?:am|zum))?|\\-|\\\u2013|\\s)\\s*([0-9]{1,2})\\.?)?\\s*(" +
    repeatedTimeunitPattern.matchAnyPattern(_mod10478.MONTH_DICTIONARY) +
    ")(?:(?:-|/|,?\\s*)(" +
    _mod10478.YEAR_PATTERN +
    "(?![^\\s]\\d)))?(?=\\W|$)",
  "i",
);
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      if (parsed > 31) {
        ...
        if (index[4]) {
          ...
          start.imply(
            "year",
            DEMonthNameLittleEndianParser(10433).findYearClosestToRef(createParsingResult.refDate, parsed, tmp4),
          );
        }
        if (index[2]) {
