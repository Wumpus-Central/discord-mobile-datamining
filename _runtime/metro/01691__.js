...
import maybeModifyStyleForKeyframe from "../01705_maybeModifyStyleForKeyframe.js";
...
function startWebLayoutAnimation(props, _componentDOMRef, ENTERING, easingY) {
  ...
  if (entering) {
    ...
    let flag =
      !(animationWithInitialValues in tmp(1692).Animations) &&
      !(ENTERING === tmp(1661).LayoutAnimationType.LAYOUT || tmp5 || undefined !== entering.initialValues);
    if (flag) {
...
export const tryActivateLayoutTransition = function tryActivateLayoutTransition(props, _componentDOMRef, arg2) {
  if (props.layout) {
    ...
    if (!obj.areDOMRectsEqual(size, arg2)) {
      ...
      startWebLayoutAnimation(props, _componentDOMRef, tmp3(1661).LayoutAnimationType.LAYOUT, obj);
    }
  }
};
