...
let items = [
  ...
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      SentrySpan(12896);
      const obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12905).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(12895).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 12905).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2.timestamp, _traceId: obj2.trace_id, _attributes } = this);
      const obj3 = SentrySpan(12895);
      obj._metrics_summary = SentrySpan(12906).getMetricSummaryJsonForSpan(this);
      obj.profile_id = this._attributes[SentrySpan(undefined, 12905).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj.exclusive_time = this._attributes[SentrySpan(undefined, 12905).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12906);
      obj.measurements = SentrySpan(12932).timedEventsToMeasurements(this._events);
      let _isStandaloneSpan = this._isStandaloneSpan;
      if (_isStandaloneSpan) {
        let tmpResult = SentrySpan(12895);
        _isStandaloneSpan = tmpResult.getRootSpan(self) === self;
      }
      obj.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        tmpResult = SentrySpan(12895);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      if (SentrySpan(12918).DEBUG_BUILD) {
        const logger = SentrySpan(12890).logger;
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          let tmpResult = SentrySpan(12904);
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = SentrySpan(12919).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = SentrySpan(12917).getCurrentScope();
              const tmpResult2 = SentrySpan(12917);
            }
            scope.captureEvent(result);
            const tmpResult1 = SentrySpan(12919);
          }
        }
      } else {
        SentrySpan(12895);
      }
      const obj = SentrySpan(12917);
    }
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(12895);
          obj.trace = tmp3(12895).spanToTransactionTraceContext(self);
          obj.contexts = obj;
          let substr = found1;
          ...
          const tmp3Result4 = tmp3(12896);
          obj2.dynamicSamplingContext = tmp3(12926).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj.sdkProcessingMetadata = obj1;
          const tmp3Result5 = tmp3(12926);
          obj._metrics_summary = tmp3(12906).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
