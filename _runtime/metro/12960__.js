    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new _mod12967.SentrySpan(obj);
        } else {
          obj = { traceId };
          sentrySpan = new _mod12959.SentryNonRecordingSpan(obj);
        }
      ...
      _mod12964.logSpanStart(sentrySpan);
      const tmpResult4 = _mod12964;
      const result = _mod12955.setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = _mod12962.getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = _mod12962;
      ...
      _mod12962.freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = _mod12962;
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        _mod12962.freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = _mod12962;
      }
    }
  } else {
    const sentryNonRecordingSpan = new _mod12959.SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = _mod12965.sampleSpan(options, obj);
    const tmpResult = _mod12965;
  }
  ...
  obj1.attributes = { [_mod12941.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new _mod12967.SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(_mod12941.SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  _require = arg1;
  const mainCarrier = require("12944__.js").getMainCarrier();
  const obj = require("12944__.js");
  const tmp = _require;
  const asyncContextStrategy = require("12945__.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12953).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(
        _mod12938.propagationContextFromHeaders(dependencyMap, _slicedToArray),
      );
      return closure_0();
    });
  }
  const obj2 = require("12945__.js");
};
export const startInactiveSpan = function startInactiveSpan(experimental) {
  _require = experimental;
  spanArguments = require("12944__.js");
  let mainCarrier = spanArguments.getMainCarrier();
  let asyncContextStrategy = require("12945__.js").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    return fn(() => {
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new _mod12959.SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
  ...
  let obj2 = require("12945__.js");
  tmp3 = _require;
  tmp4 = spanArguments;
};
export const startNewTrace = function startNewTrace(arg0) {
  _require = arg0;
  return require("12953__.js").withScope((setPropagationContext) => {
      ...
      return undefined !== createChildOrRootSpan
        ...
        : ((fn) => fn())(() => {
            ...
            if (sentryNonRecordingSpan.onlyIfParent) {
              ...
              const tmpResult1 = tmp(12948);
              return tmp(12961).handleCallbackErrors(
            ...
            const obj2 = sentryNonRecordingSpan(12948);
          });
    });
  }
  let obj2 = require("12945__.js");
