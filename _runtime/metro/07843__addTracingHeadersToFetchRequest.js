...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7755).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7755);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7755).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7820).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7820);
    }
    const activeSpan = setHttpStatus(7743).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7753).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7753).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7772).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7772);
      }
    ...
    startInactiveSpanResult = new setHttpStatus(7771).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7743);
  }
};