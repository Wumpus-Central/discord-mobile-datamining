...
import _mod1721 from "module_1721" /* 1721 */;
...
fn = function n(toValue, userConfig, callback) {
  ...
  __initData = callback;
  const fn = function u() {
    let obj = { damping: 10, mass: 1, stiffness: 100, overshootClamping: false, restDisplacementThreshold: 0.01, restSpeedThreshold: 2, velocity: 0, duration: 2000, dampingRatio: 0.5, reduceMotion: "channel", clamp: "ix" };
    ...
    obj.skipAnimation = !_mod1721.checkIfConfigIsValid(obj);
    if (0 === obj.duration) {
    ...
    let obj3 = {
      onFrame: function springOnFrame(toValue, lastTimestamp) {
        ...
        if (obj.skipAnimation) {
          ...
          if (zeta < 1) {
            ...
            let result1 = closure_0(1721).underDampedSpringCalculations(toValue, obj2);
            const obj3 = closure_0(1721);
          } else {
            obj = closure_0(1721);
          ...
          const result2 = closure_0(1721).isAnimationTerminatingCalculation(toValue, tmp2);
      ...
      onStart(toValue, current, arg2, lastTimestamp) {
        ...
        if (lastTimestamp) {
          ...
          const obj2 = closure_0(1721);
          ({ zeta: toValue.zeta, omega0: toValue.omega0, omega1: toValue.omega1 } = closure_0(1721).initialCalculations(mass, tmp));
          if (undefined !== tmp.clamp) {
            toValue.zeta = closure_0(1721).scaleZetaToMatchClamps(toValue, tmp.clamp);
            const obj3 = closure_0(1721);
          }
          const initialCalculationsResult = closure_0(1721).initialCalculations(mass, tmp);
        }
        let lastTimestamp1;
        if (lastTimestamp != null) {
    ...
    obj3.reduceMotion = _mod1678.getReduceMotionForAnimation(obj.reduceMotion);
    return obj3;
  };
  let obj = require("module_1678");
  fn.__closure = { userConfig, checkIfConfigIsValid: require("module_1721").checkIfConfigIsValid, underDampedSpringCalculations: require("module_1721").underDampedSpringCalculations, criticallyDampedSpringCalculations: require("module_1721").criticallyDampedSpringCalculations, isAnimationTerminatingCalculation: require("module_1721").isAnimationTerminatingCalculation, calculateNewMassToMatchDuration: require("module_1721").calculateNewMassToMatchDuration, initialCalculations: require("module_1721").initialCalculations, scaleZetaToMatchClamps: require("module_1721").scaleZetaToMatchClamps, toValue, callback, getReduceMotionForAnimation: require("module_1678").getReduceMotionForAnimation };
  fn.__workletHash = 3229069592929;
  fn.__initData = __initData;
  return obj.defineAnimation(toValue, fn);
};
fn.__closure = { defineAnimation: fn(1678).defineAnimation, checkIfConfigIsValid: fn(1721).checkIfConfigIsValid, underDampedSpringCalculations: fn(1721).underDampedSpringCalculations, criticallyDampedSpringCalculations: fn(1721).criticallyDampedSpringCalculations, isAnimationTerminatingCalculation: fn(1721).isAnimationTerminatingCalculation, calculateNewMassToMatchDuration: fn(1721).calculateNewMassToMatchDuration, initialCalculations: fn(1721).initialCalculations, scaleZetaToMatchClamps: fn(1721).scaleZetaToMatchClamps, getReduceMotionForAnimation: fn(1678).getReduceMotionForAnimation };
