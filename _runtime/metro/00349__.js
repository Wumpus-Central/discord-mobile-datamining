...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f110324._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f110324)),
            f110324._textInputFocusError,
            f110324._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f110324._innerView.nativeInstance,
            f110324._inputMeasureAndScrollToKeyboard,
            f110324._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f110324._keyboardMetrics) {
              screenY = f110324._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f110324._additionalScrollOffset;
            let bound = sum;
    ...
    f110324 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f110324.getScrollResponder,
        getScrollableNode: f110324.getScrollableNode,
        getInnerViewNode: f110324.getInnerViewNode,
        getInnerViewRef: f110324.getInnerViewRef,
        getNativeScrollRef: f110324.getNativeScrollRef,
        scrollTo: f110324.scrollTo,
        scrollToEnd: f110324.scrollToEnd,
        flashScrollIndicators: f110324.flashScrollIndicators,
        scrollResponderZoomTo: f110324.scrollResponderZoomTo,
        scrollResponderScrollNativeHandleToKeyboard: f110324.scrollResponderScrollNativeHandleToKeyboard,
      });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f110324._lastMomentumScrollEndTime < 16 ||
        f110324._lastMomentumScrollEndTime < f110324._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f110324.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f110324.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f110324._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f110324._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f110324._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f110324.props.disableScrollViewPanResponder && f110324._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f110324.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f110324.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f110324._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f110324._isAnimating()) {
        return true;
      } else if (true === f110324.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f110324._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f110324._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f110324._softKeyboardIsDetached() &&
        f110324._keyboardIsDismissible() &&
        tmp;
    };
