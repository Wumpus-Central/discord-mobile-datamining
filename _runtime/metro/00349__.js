...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f107619._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f107619)),
            f107619._textInputFocusError,
            f107619._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f107619._innerView.nativeInstance,
            f107619._inputMeasureAndScrollToKeyboard,
            f107619._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f107619._keyboardMetrics) {
              screenY = f107619._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f107619._additionalScrollOffset;
            let bound = sum;
    ...
    f107619 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f107619.getScrollResponder,
        getScrollableNode: f107619.getScrollableNode,
        getInnerViewNode: f107619.getInnerViewNode,
        getInnerViewRef: f107619.getInnerViewRef,
        getNativeScrollRef: f107619.getNativeScrollRef,
        scrollTo: f107619.scrollTo,
        scrollToEnd: f107619.scrollToEnd,
        flashScrollIndicators: f107619.flashScrollIndicators,
        scrollResponderZoomTo: f107619.scrollResponderZoomTo,
        scrollResponderScrollNativeHandleToKeyboard: f107619.scrollResponderScrollNativeHandleToKeyboard,
      });
    closure_1 = undefined;
    obj1 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f107619._lastMomentumScrollEndTime < 16 ||
        f107619._lastMomentumScrollEndTime < f107619._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f107619.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f107619.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f107619._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f107619._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f107619._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f107619.props.disableScrollViewPanResponder && f107619._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f107619.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f107619.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f107619._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f107619._isAnimating()) {
        return true;
      } else if (true === f107619.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f107619._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f107619._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f107619._softKeyboardIsDetached() &&
        f107619._keyboardIsDismissible() &&
        tmp;
    };
