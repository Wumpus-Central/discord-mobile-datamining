...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f109312._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f109312)),
            f109312._textInputFocusError,
            f109312._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f109312._innerView.nativeInstance,
            f109312._inputMeasureAndScrollToKeyboard,
            f109312._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f109312._keyboardMetrics) {
              screenY = f109312._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f109312._additionalScrollOffset;
            let bound = sum;
    ...
    f109312 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f109312.getScrollResponder,
        getScrollableNode: f109312.getScrollableNode,
        getInnerViewNode: f109312.getInnerViewNode,
        getInnerViewRef: f109312.getInnerViewRef,
        getNativeScrollRef: f109312.getNativeScrollRef,
        scrollTo: f109312.scrollTo,
        scrollToEnd: f109312.scrollToEnd,
        flashScrollIndicators: f109312.flashScrollIndicators,
        scrollResponderZoomTo: f109312.scrollResponderZoomTo,
        scrollResponderScrollNativeHandleToKeyboard: f109312.scrollResponderScrollNativeHandleToKeyboard,
      });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f109312._lastMomentumScrollEndTime < 16 ||
        f109312._lastMomentumScrollEndTime < f109312._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f109312.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f109312.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f109312._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109312._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109312._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f109312.props.disableScrollViewPanResponder && f109312._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f109312.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f109312.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f109312._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f109312._isAnimating()) {
        return true;
      } else if (true === f109312.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f109312._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f109312._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f109312._softKeyboardIsDetached() &&
        f109312._keyboardIsDismissible() &&
        tmp;
    };
