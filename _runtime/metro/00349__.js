...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f110209._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f110209)),
            f110209._textInputFocusError,
            f110209._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f110209._innerView.nativeInstance,
            f110209._inputMeasureAndScrollToKeyboard,
            f110209._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f110209._keyboardMetrics) {
              screenY = f110209._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f110209._additionalScrollOffset;
            let bound = sum;
    ...
    f110209 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f110209.getScrollResponder,
        getScrollableNode: f110209.getScrollableNode,
        getInnerViewNode: f110209.getInnerViewNode,
        getInnerViewRef: f110209.getInnerViewRef,
        getNativeScrollRef: f110209.getNativeScrollRef,
        scrollTo: f110209.scrollTo,
        scrollToEnd: f110209.scrollToEnd,
        flashScrollIndicators: f110209.flashScrollIndicators,
        scrollResponderZoomTo: f110209.scrollResponderZoomTo,
        scrollResponderScrollNativeHandleToKeyboard: f110209.scrollResponderScrollNativeHandleToKeyboard,
      });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f110209._lastMomentumScrollEndTime < 16 ||
        f110209._lastMomentumScrollEndTime < f110209._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f110209.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f110209.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f110209._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f110209._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f110209._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f110209.props.disableScrollViewPanResponder && f110209._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f110209.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f110209.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f110209._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f110209._isAnimating()) {
        return true;
      } else if (true === f110209.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f110209._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f110209._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f110209._softKeyboardIsDetached() &&
        f110209._keyboardIsDismissible() &&
        tmp;
    };
