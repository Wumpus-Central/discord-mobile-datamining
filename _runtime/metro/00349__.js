...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f109454._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f109454)),
            f109454._textInputFocusError,
            f109454._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f109454._innerView.nativeInstance,
            f109454._inputMeasureAndScrollToKeyboard,
            f109454._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f109454._keyboardMetrics) {
              screenY = f109454._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f109454._additionalScrollOffset;
            let bound = sum;
    ...
    f109454 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f109454.getScrollResponder,
        getScrollableNode: f109454.getScrollableNode,
        getInnerViewNode: f109454.getInnerViewNode,
        getInnerViewRef: f109454.getInnerViewRef,
        getNativeScrollRef: f109454.getNativeScrollRef,
        scrollTo: f109454.scrollTo,
        scrollToEnd: f109454.scrollToEnd,
        flashScrollIndicators: f109454.flashScrollIndicators,
        scrollResponderZoomTo: f109454.scrollResponderZoomTo,
        scrollResponderScrollNativeHandleToKeyboard: f109454.scrollResponderScrollNativeHandleToKeyboard,
      });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f109454._lastMomentumScrollEndTime < 16 ||
        f109454._lastMomentumScrollEndTime < f109454._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f109454.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f109454.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f109454._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109454._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109454._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f109454.props.disableScrollViewPanResponder && f109454._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f109454.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f109454.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f109454._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f109454._isAnimating()) {
        return true;
      } else if (true === f109454.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f109454._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f109454._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f109454._softKeyboardIsDetached() &&
        f109454._keyboardIsDismissible() &&
        tmp;
    };
