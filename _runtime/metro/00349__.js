...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      if (null != f109906._innerView.nativeInstance) {
        if (typeof measureLayout === "number") {
          ...
          obj.measureLayout(
            measureLayout,
            tmp4(renderElement.findNodeHandle(f109906)),
            f109906._textInputFocusError,
            f109906._inputMeasureAndScrollToKeyboard,
          );
        } else {
          measureLayout.measureLayout(
            f109906._innerView.nativeInstance,
            f109906._inputMeasureAndScrollToKeyboard,
            f109906._textInputFocusError,
          );
        }
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f109906._keyboardMetrics) {
              screenY = f109906._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + f109906._additionalScrollOffset;
            let bound = sum;
    ...
    f109906 = (arg0) =>
      Object.assign(arg0, {
        getScrollResponder: f109906.getScrollResponder,
        getScrollableNode: f109906.getScrollableNode,
        getInnerViewNode: f109906.getInnerViewNode,
        getInnerViewRef: f109906.getInnerViewRef,
        getNativeScrollRef: f109906.getNativeScrollRef,
        scrollTo: f109906.scrollTo,
        scrollToEnd: f109906.scrollToEnd,
        flashScrollIndicators: f109906.flashScrollIndicators,
        scrollResponderZoomTo: f109906.scrollResponderZoomTo,
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return (
        _performance.now() - f109906._lastMomentumScrollEndTime < 16 ||
        f109906._lastMomentumScrollEndTime < f109906._lastMomentumScrollBeginTime
      );
    };
    ...
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      ...
      if (typeof nativeEvent.target !== "number") {
        ...
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = true === f109906.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = "always" === f109906.props.keyboardShouldPersistTaps;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = !f109906._keyboardIsDismissible();
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = nativeEvent.target === result;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109906._observedScrollSinceBecomingResponder;
        }
        if (!_becameResponderWhileAnimating) {
          _becameResponderWhileAnimating = f109906._becameResponderWhileAnimating;
        }
        if (!_becameResponderWhileAnimating) {
    ...
    tmp3Result._handleScrollShouldSetResponder = () =>
      true !== f109906.props.disableScrollViewPanResponder && f109906._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f109906.props.disableScrollViewPanResponder) {
        return false;
      } else {
        let tmp4 = "handled" !== f109906.props.keyboardShouldPersistTaps;
        const result = _modDef144.currentlyFocusedInput();
        if (!tmp4) {
          tmp4 = !f109906._keyboardIsDismissible();
        }
        if (!tmp4) {
    ...
    tmp3Result._handleStartShouldSetResponderCapture = (target) => {
      if (f109906._isAnimating()) {
        return true;
      } else if (true === f109906.props.disableScrollViewPanResponder) {
        return false;
      } else {
        ...
        if (typeof target !== "number") {
          const result = f109906._softKeyboardIsDetached();
          let tmp8 = !result;
          if (!result) {
            let isTextInputResult = !tmp;
            if (tmp) {
              isTextInputResult = !f109906._keyboardIsDismissible();
            }
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      ...
      tmp5 =
        ...
        f109906._softKeyboardIsDetached() &&
        f109906._keyboardIsDismissible() &&
        tmp;
    };
