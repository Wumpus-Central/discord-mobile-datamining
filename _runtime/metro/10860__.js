  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      ...
      if (null != str.match(/\s+(prima|dal)$/i)) {
        reverseDurationResult = ENMergeRelativeDateRefiner(10677).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = ENMergeRelativeDateRefiner(10678).ParsingComponents;
      const ReferenceWithTimezone = ENMergeRelativeDateRefiner(10678).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeDateRefiner(10678).ParsingResult(
        start.reference,
        text.index,
