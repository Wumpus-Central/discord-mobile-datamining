...
class Comparator {
  constructor(arg0, arg1) {
    ...
    parts = str2.split(/\s+/);
    joined = parts.join(" ");
    tmp6 = require("module_13128")("comparator", joined, tmp4);
    self.options = tmp4;
    ...
    tmp8 = require("module_13128")("comp", self);
    return;
  }
}
let obj = {
  ...
  value: function parse(str) {
    const self = this;
    const safeRe = Comparator(13129).safeRe;
    const t = Comparator(13129).t;
    ...
    if (match) {
      ...
      if (match[2]) {
        const tmp15 = new tmp5(13125)(match[2], self.options.loose);
        self.semver = tmp15;
      } else {
...
const items = [
  ...
  },
  {
    ...
    value: function test(arg0) {
      const self = this;
      let tmp = arg0;
      Comparator(13128)("Comparator.test", arg0, this.options.loose);
      if (this.semver !== closure_3) {
        if (tmp !== tmp5) {
          if (typeof tmp === "string") {
            try {
              tmp = new tmp2(13125)(tmp, self.options);
            } catch (err) {
              return false;
            }
          }
          return tmp2(13151)(tmp, self.operator, self.semver, self.options);
        }
      }
      return true;
    }
    ...
    value: function intersects(value) {
      if (value instanceof Comparator) {
        ...
        if ("" === this.operator) {
          ...
          if (!isMatch) {
            const obj2 = new Comparator(13154)(value.value, arg1);
          ...
          if (!isMatch1) {
            const obj = new Comparator(13154)(self.value, arg1);
            isMatch1 = obj.test(value.semver);
          }
          tmp8 = isMatch1;
        } else {
          const tmp45 = Comparator(13126)(arg1);
            ...
            if (!tmp9) {
              ...
              if (tmp13) {
                ...
                if (tmp16) {
                  ...
                  let tmp19 = !tmp18;
                  if (tmp18) {
                    let startsWithResult3 = Comparator(13151)(self.semver, "<", value.semver, tmp45);
                    ...
                    if (!tmp24) {
                      let startsWithResult4 = Comparator(13151)(self.semver, ">", value.semver, tmp45);
