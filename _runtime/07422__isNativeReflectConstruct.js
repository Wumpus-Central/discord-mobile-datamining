// === Module 7422: _isNativeReflectConstruct ===
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7423).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7423);
      return ServerRuntimeClient(7387).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7387);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7423).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7390).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7390);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7416).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7391).DEBUG_BUILD) {
        const logger = tmp2(7363).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7424);
          ...
          if (ServerRuntimeClient(7391).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7363).logger;
          ...
          if (ServerRuntimeClient(7391).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7363).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7374);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7385)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7399);
        if (_getSpanForScopeResult) {
