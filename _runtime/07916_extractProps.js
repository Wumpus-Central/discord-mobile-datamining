...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7908)(obj, markerMid, arg1);
  module(7918)(obj, markerMid, items);
  module(7921)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7909)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7907)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
