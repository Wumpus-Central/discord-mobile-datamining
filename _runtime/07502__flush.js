...
import { applyClientOptions } from "07503_applyClientOptions.js";
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7491).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7468).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7468);
    const currentScope = monitorSlug(7481).getCurrentScope();
    const obj3 = monitorSlug(7481);
    const client = monitorSlug(7481).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7481);
    monitorSlug(7465).uuid4();
  }
