// _runtime/00839_withScope.js
import closure_2 from "metro/00041__classCallCheck.js";
import _createClass from "metro/00042__createClass.js";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(822).getMainCarrier();
  const obj = AsyncContextStack(822);
  const sentryCarrier = AsyncContextStack(822).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(822).getMainCarrier();
      let obj = callback(822);
      let sentryCarrier = callback(822).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(844);
    ...
    getCurrentScope() {
      const mainCarrier = callback(822).getMainCarrier();
      const obj = callback(822);
      const sentryCarrier = callback(822).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(844);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
