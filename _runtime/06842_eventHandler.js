...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6762.State.UNDETERMINED) {
    if (state === _mod6762.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6762.State.BEGAN) {
    if (state === _mod6762.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6762.State.FAILED || state === _mod6762.State.CANCELLED;
    if (oldState === _mod6762.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6762.State.FAILED || state === _mod6762.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6824).flattenAndFilterEvent, State: fn(6762).State, runCallback: fn(6824).runCallback, CALLBACK_TYPE: fn(6777).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6824).flattenAndFilterEvent, State: fn(6762).State, runCallback: fn(6824).runCallback, CALLBACK_TYPE: fn(6777).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6824).flattenAndFilterEvent, runCallback: fn(6824).runCallback, CALLBACK_TYPE: fn(6777).CALLBACK_TYPE };
...
let obj2 = { flattenAndFilterEvent: fn(6824).flattenAndFilterEvent, runCallback: fn(6824).runCallback, CALLBACK_TYPE: fn(6777).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6764).TouchEventType, runCallback: fn(6824).runCallback, touchEventTypeToCallbackType: fn(6824).touchEventTypeToCallbackType };
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
            ...
            tmpResult7.runCallback(DEFAULT_PROPS_TRANSFORMER.touchEventTypeToCallbackType(result.eventType), arg2, result);
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
const obj3 = { TouchEventType: fn(6764).TouchEventType, runCallback: fn(6824).runCallback, touchEventTypeToCallbackType: fn(6824).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6824).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6824).isEventForHandlerWithTag, isStateChangeEvent: fn(6826).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6826).isTouchEvent, handleTouchEvent, handleUpdateEvent };
