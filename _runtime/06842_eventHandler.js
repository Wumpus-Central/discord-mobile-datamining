...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6762.State.UNDETERMINED) {
    if (state === _mod6762.State.BEGAN) {
      ...
      tmpResult.runCallback(_mod6777.CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod6762.State.BEGAN) {
    if (state === _mod6762.State.ACTIVE) {
      ...
      tmpResult.runCallback(_mod6777.CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === _mod6762.State.FAILED || state === _mod6762.State.CANCELLED;
    if (oldState === _mod6762.State.ACTIVE) {
      if (fn != null) {
        fn(obj);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6762.State.FAILED || state === _mod6762.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6777.CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
...
obj = { flattenAndFilterEvent: fn(6824).flattenAndFilterEvent, runCallback: fn(6824).runCallback, CALLBACK_TYPE: fn(6777).CALLBACK_TYPE };
...
obj = { TouchEventType: fn(6764).TouchEventType, runCallback: fn(6824).runCallback, touchEventTypeToCallbackType: fn(6824).touchEventTypeToCallbackType };
      ...
      if (tmpResult.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
            const tmpResult1 = DEFAULT_PROPS_TRANSFORMER;
            tmpResult1.runCallback(DEFAULT_PROPS_TRANSFORMER.touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult2 = DEFAULT_PROPS_TRANSFORMER;
          }
        } else {
