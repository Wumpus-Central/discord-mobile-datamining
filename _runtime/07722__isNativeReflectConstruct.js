// _runtime/07722__isNativeReflectConstruct.js
...
import __SENTRY_DEBUG__ from "metro/07662___SENTRY_DEBUG__.js";
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7723).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7723);
      return ServerRuntimeClient(7687).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7687);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7723).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7690).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7690);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7716).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7691).DEBUG_BUILD) {
        const logger = tmp2(7663).logger;
    ...
    value: function captureCheckIn(checkInId) {
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7724);
          ...
          if (ServerRuntimeClient(7691).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7663).logger;
          ...
          if (ServerRuntimeClient(7691).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7663).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7674);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7685)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7699);
        if (_getSpanForScopeResult) {
