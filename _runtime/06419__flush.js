...
function endSession() {
  ...
  const tmpResult1 = require(6398) /* getClient */;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(6398) /* getClient */.getCurrentScope();
  const obj = require(6398) /* getClient */;
  return currentScope.captureException(arg0, require(6420) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(6398) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(6398) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(6408).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(6398) /* getClient */;
  const userAgent = require(6372) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(6394);
  obj = { release, environment, user: null };
  const tmp3 = require(6372) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(6385).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(6385);
    const currentScope = monitorSlug(6398).getCurrentScope();
    const obj3 = monitorSlug(6398);
    const client = monitorSlug(6398).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(6398);
    monitorSlug(6382).uuid4();
  }
  let currentScope = _require(6398).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(6398);
  let client = _require(6398).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(6399).DEBUG_BUILD) {
      let logger2 = tmp(6371).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(6371).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(6398);
  captureCheckInResult = _require(6382).uuid4();
};