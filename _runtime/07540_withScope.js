// _runtime/07540_withScope.js
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7538).getMainCarrier();
  const obj = AsyncContextStack(7538);
  const sentryCarrier = AsyncContextStack(7538).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(7546);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7538).getMainCarrier();
      let obj = callback(7538);
      let sentryCarrier = callback(7538).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7546);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7538).getMainCarrier();
      const obj = callback(7538);
      const sentryCarrier = callback(7538).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7546);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
