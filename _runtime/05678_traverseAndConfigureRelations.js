// _runtime/05678_traverseAndConfigureRelations.js
import tagMessage from "05609_tagMessage.js";
import { isComposedGesture } from "05671_isComposedGesture.js";
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(5671).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(5663).ComposedGestureName.Simultaneous) {
