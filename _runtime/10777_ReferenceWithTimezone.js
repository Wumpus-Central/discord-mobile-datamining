// _runtime/10777_ReferenceWithTimezone.js
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10778).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10776).addDuration(result, EmptyDuration);
  ...
  },
  {
    ...
    value: function date() {
      const result = this.dateWithoutTimezoneAdjustment();
      const reference = this.reference;
      const systemTimezoneAdjustmentMinute = reference.getSystemTimezoneAdjustmentMinute(
        result,
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(10776).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(10776).addDuration(
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(10780).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(10780).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(10780).assignSimilarDate(obj, addDurationResult);
