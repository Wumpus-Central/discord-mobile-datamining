// _runtime/05169_constructTag.js
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod5129.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5122).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5122);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod5129.TYPE_ITXT) {
    stringFromDataView = module(5155).decode("UTF-8", decompressResult);
    const obj5 = module(5155);
  }
