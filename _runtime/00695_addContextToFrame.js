// _runtime/00695_addContextToFrame.js
import addNonEnumerableProperty from "00687_addNonEnumerableProperty.js";
import isMatchingPattern from "00697_isMatchingPattern.js";
...
arg5.addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((arg0) => callback(697).snipLine(arg0, 0));
    const _Math5 = Math;
    ...
    lineno.post_context = substr1.map((arg0) => callback(697).snipLine(arg0, 0));
...
arg5.uuid4 = function uuid4() {
  ...
  if (arg0 === undefined) {
    tmp = (function getCrypto() {
      const GLOBAL_OBJ = lib(686).GLOBAL_OBJ;
  ...
  try {
    ...
    if (randomUUID) {
      let obj = safeDateNow;
      return require("00696_safeDateNow.js").withRandomSafeContext(() => lib.randomUUID()).replace(/-/g, "");
    } else {
      let str3 = c2;
      if (!c2) {
      ...
      return str3.replace(/[018]/g, (arg0) => {
        const obj = lib(696);
        return arg0 ^ (15 & 16 * lib(696).safeMathRandom()) >> arg0 / 4.toString(16);
      });
    }
  } catch (err) {
  }
