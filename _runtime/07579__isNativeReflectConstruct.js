// _runtime/07579__isNativeReflectConstruct.js
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7580).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7580);
      return ServerRuntimeClient(7544).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7544);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7580).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7547).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7547);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7573).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7548).DEBUG_BUILD) {
        const logger = tmp2(7520).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7581);
          ...
          if (ServerRuntimeClient(7548).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7520).logger;
          ...
          if (ServerRuntimeClient(7548).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7520).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7531);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7542)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7556);
        if (_getSpanForScopeResult) {
