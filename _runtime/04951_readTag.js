// _runtime/04951_readTag.js
import { importDefaultResult1 } from "04952_importDefaultResult1.js";
import { 04934__ } from "metro/04934__.js";
import { 04948__ } from "metro/04948__.js";
const require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  let obj = 04948__;
  const typeSize = obj.getTypeSize("SHORT");
  sum = typeSize + 04948__.getTypeSize("SHORT");
  const obj2 = 04948__;
  const sum1 = sum + 04948__.getTypeSize("LONG");
  const obj3 = 04948__;
  const shortAt = 04948__.getShortAt(byteLength, sum, byteOrder);
  const obj4 = 04948__;
  const shortAt1 = 04948__.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = 04948__;
  const longAt = 04948__.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== 04948__.typeSizes[shortAt1]) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * 04948__.typeSizes[shortAt1];
    BYTE = 04948__.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + 04948__.typeSizes[BYTE];
  }
  if (BYTE === 04948__.tagTypes.ASCII) {
    let asciiValue = 04948__.getAsciiValue(items);
    const tmp9Result = 04948__;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = 04948__.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = 04948__;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + 04948__.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };