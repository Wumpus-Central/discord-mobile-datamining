// _runtime/06934_eventHandler.js
import _mod6854 from "metro/06854__.js";
import TouchEventType from "06856_TouchEventType.js";
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6854.State.UNDETERMINED) {
    if (state === _mod6854.State.BEGAN) {
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = DEFAULT_PROPS_TRANSFORMER;
    }
  }
  if (oldState === _mod6854.State.BEGAN) {
    if (state === _mod6854.State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === _mod6854.State.FAILED || state === _mod6854.State.CANCELLED;
    if (oldState === _mod6854.State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = DEFAULT_PROPS_TRANSFORMER;
    }
    const tmp7 = state === _mod6854.State.FAILED || state === _mod6854.State.CANCELLED;
    DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
...
handleStateChangeEvent.__closure = {
  flattenAndFilterEvent: fn(6916).flattenAndFilterEvent,
  State: fn(6854).State,
  runCallback: fn(6916).runCallback,
  CALLBACK_TYPE: fn(6869).CALLBACK_TYPE,
};
...
handleTouchEvent.__closure = {
  TouchEventType: fn(6856).TouchEventType,
  runCallback: fn(6916).runCallback,
  touchEventTypeToCallbackType: fn(6916).touchEventTypeToCallbackType,
};
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6869.CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
eventHandler.__closure = {
  maybeExtractNativeEvent: fn(6916).maybeExtractNativeEvent,
  isEventForHandlerWithTag: fn(6916).isEventForHandlerWithTag,
  isStateChangeEvent: fn(6918).isStateChangeEvent,
  handleStateChangeEvent,
  isTouchEvent: fn(6918).isTouchEvent,
  handleTouchEvent,
  handleUpdateEvent,
};
