// _runtime/13386___exportStarResult25.js
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13358).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13384).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13384).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13383).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13382).FormatApproximately(internalSlots, result);
    ...
    return tmp(13371).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13383).FormatNumeric(internalSlots, isNaN);
};