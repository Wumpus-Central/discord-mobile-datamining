// _runtime/05857_Color.js
import _mod5858 from "metro/05858__.js";
...
let obj = {};
let keys1 = Object.keys(_mod5858);
let iter = keys1[Symbol.iterator]();
const nextResult = iter.next();
while (iter !== undefined) {
  let items = [];
  let arraySpreadResult = HermesBuiltin.arraySpread(_mod5858[nextResult].labels, 0);
  class Color {
    constructor(arg0, arg1) {
      self = this;
      str = require;
...
const point = {
  ...
  object() {
    const self = this;
    obj = {};
    const channels = _mod5858[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[_mod5858[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  keyword(color) {
    if (undefined !== color) {
      ...
      const self = this;
      keywordResult = _mod5858[this.model].keyword(this.color);
    }
...
function _loop(item10136) {
  ...
  if (closure_3.includes(item10136)) {
    ...
    channels = require("metro/05858__.js")[item10136].channels;
    Color.prototype[item10136] = function () {
      ...
      if (this.model === closure_0) {
        ...
        const rawResult = _mod5858[self.model][closure_0].raw(self.color);
        const _Array = Array;
        let tmp2 = rawResult;
