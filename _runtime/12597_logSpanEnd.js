...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(12564).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(12564).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12564).spanToJSON(rootSpan);
    }
    const logger = tmp(12559).logger;
