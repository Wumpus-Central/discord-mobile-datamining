...
prototype2["canHaveMentions"] = function canHaveMentions() {
  ...
  if (tmp) {
    ...
    if (!(self._isThread && !self._isJoinedThread)) {
      ...
      let tmp9 = !result;
      if (!result) {
        let result1 = tmp4(6915).isOptInEnabledForGuild(self._guildId);
        if (result1) {
  ...
  if (null != outgoingAck) {
    ...
    _networkAwareRetry(() => {
      ...
      if (null != body) {
        ...
        if (dependencyMap) {
          tmp4(1988)(13104, tmp5.paths).then((arg0) => {
            let obj = closure_1;
            if (closure_1 == null) {
              obj = {};
            }
          ...
          const promise = tmp4(1988)(13104, tmp5.paths);
        }
...
obj = {
  ...
  BULK_ACK: function handleBulkAck(arg0) {
    ...
    const item = found.forEach((messageId) => {
      ...
      value.ack({ messageId: messageId.messageId, local: true, immediate: "T", force: "PX_8", isExplicitUserAction: true, trackAnalytics: false });
    });
