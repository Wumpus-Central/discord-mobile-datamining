...
import isChangelogUserDefault from "../modules/changelog/utils/isChangelogUser.tsx";
import _slicedToArray from "../../_runtime/metro/00032__.js";
import asyncGeneratorStep from "../../_runtime/00005_asyncGeneratorStep.js";
import BasicChannelCacheStore from "../modules/app_database/stores/BasicChannelCacheStore.tsx";
      ...
      if (!set.has(guild_id)) {
        if (Full !== closure_35.Basic) {
          ...
          if (null != databaseResult) {
            ...
            const result = require("TryLoad").tryLoadOrResetCacheGateway(
              "ensureGuildLoaded(" + guild_id + ")",
              () => ChannelReaderDefault.getSync(databaseResult, closure_0),
              "ensureGuildLoaded",
            );
            if (null == result) {
              set.add(guild_id);
...
prototype["loadGuildIds"] = function loadGuildIds(items) {
  ...
  if (0 === found.length) {
    ...
    if (null == databaseResult) {
      ...
      return tmp(2094).tryLoadOrResetCacheGatewayAsync(
        "loadChannels",
            ...
            while (true) {
              ...
              if (0 === c6) {
                ...
                let errorResult = closure_1_17.error(
                  `Failed to load channels from disk for ${closure_130_0.map((guildId) => guildId.guildId)}`,
                  closure_130_4,
                );
                ...
                if (arg0 === 1) {
                  ...
                  if (closure_1_31 !== closure_131_2) {
                    let fileOnlyResult = closure_1_17.fileOnly(
                      `lastResetTime has changed, skipping loads for ${closure_130_0.map((guildId) => guildId.guildId)}`,
                    );
                    c5 = 0;
                    c7 = 3;
...
prototype2["getDebugInfo"] = function getDebugInfo() {
  const obj = {
    loadedGuildIds: Array.from(set).sort(SnowflakeUtilsDefault.compare),
