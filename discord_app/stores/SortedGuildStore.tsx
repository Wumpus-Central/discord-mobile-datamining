...
function convertNodeToGuildFolder(type) {
  let children;
  type = type.type;
  if (areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER === type) {
    ...
    return obj;
  } else if (tmp(5219).GuildsNodeType.GUILD === type) {
...
function rebuildTree(arg0, arg1) {
  let guildsTree = new areArraysShallowlyEqual(5219).GuildsTree();
  if (0 === arg0.length) {
    if (arg1.length > 0) {
      ...
      while (tmp18 !== undefined) {
        ...
        let obj3 = areArraysShallowlyEqual(5219);
    ...
    while (iter2 !== undefined) {
      ...
      let tmp36 = nextResult.type === areArraysShallowlyEqual(5219).GuildsNodeType.GUILD;
      if (tmp36) {
    ...
    for (const item10123 of values) {
      ...
      let tmp58 = item10123.type === areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER;
      if (tmp58) {
        let tmp59 = item10123;
  ...
  while (iter !== undefined) {
    ...
    if (0 !== nextResult1.guildIds.length) {
      ...
      if (null == tmp5.folderId) {
        ...
        let obj2 = areArraysShallowlyEqual(5219);
        ...
        let obj4 = areArraysShallowlyEqual(5219);
        ...
        for (const item10030 of guildIds) {
          ...
          let obj = areArraysShallowlyEqual(5219);
...
function handleMoveById(sourceId) {
  ...
  if (null != node) {
    if (null != node1) {
      let tmp4 = combine;
      if (combine) {
        tmp4 = node.type === areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER;
      }
      ...
      tmp22Result(!(node.type === areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER && null != node1.parentId), "[SORTED GUILDS] Can't move a folder " + node.id + " to inside of another folder " + node1.parentId);
      if (combine) {
        let convertToFolderResult = node1;
        if (node1.type !== tmp13(5219).GuildsNodeType.FOLDER) {
...
function handleGuildFolderDeleteLocal(targetId) {
  ...
  if (tmp) {
    const tmp4 = element.type === element(5219).GuildsNodeType.FOLDER;
    if (tmp4) {
...
function handleGuildFolderExpand(folderId) {
  ...
  if (null != node) {
    if (node.type === areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER) {
...
function handleCollapseAll() {
  ...
  while (iter !== undefined) {
    ...
    let expanded = nextResult.type === areArraysShallowlyEqual(5219).GuildsNodeType.FOLDER;
    if (expanded) {
      let tmp6 = nextResult;
