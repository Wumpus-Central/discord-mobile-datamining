...
function convertNodeToGuildFolder(type) {
  type = type.type;
  if (obj42(5747).GuildsNodeType.FOLDER === type) {
    ...
    return obj3;
  } else if (obj42(5747).GuildsNodeType.GUILD === type) {
...
function rebuildTree(guildFolders1, guildPositions) {
  guildsTree = new obj42(5747).GuildsTree();
  if (0 === guildFolders1.length) {
    ...
    while (iter2 !== undefined) {
      ...
      let tmp36 = nextResult.type === obj42(5747).GuildsNodeType.GUILD;
      if (tmp36) {
    ...
    for (const item10123 of values) {
      let tmp58 = item10123.type === obj42(5747).GuildsNodeType.FOLDER;
      if (tmp58) {
...
function handleMoveById(sourceId) {
  ...
  if (null != node) {
    if (null != node1) {
      let tmp4 = combine;
      if (combine) {
        tmp4 = node.type === obj42(5747).GuildsNodeType.FOLDER;
      }
      ...
      tmp22Result2(!(node.type === obj42(5747).GuildsNodeType.FOLDER && null != node1.parentId), "[SORTED GUILDS] Can't move a folder " + node.id + " to inside of another folder " + node1.parentId);
      if (combine) {
        let convertToFolderResult = node1;
        if (node1.type !== obj42(5747).GuildsNodeType.FOLDER) {
      ...
      const tmp14 = node.type === obj42(5747).GuildsNodeType.FOLDER && null != node1.parentId;
    }
  }
  return false;
}
...
function handleGuildFolderDeleteLocal(targetId) {
  ...
  if (tmp) {
    const tmp4 = element.type === element(5747).GuildsNodeType.FOLDER;
    if (tmp4) {
...
function handleGuildFolderExpand(folderId) {
  ...
  if (null != node) {
    if (node.type === obj42(5747).GuildsNodeType.FOLDER) {
...
function handleCollapseAll() {
  ...
  while (iter !== undefined) {
    let tmp3 = nextResult;
    let expanded = nextResult.type === obj42(5747).GuildsNodeType.FOLDER;
    if (expanded) {
...
let guildsTree = new fn(5747).GuildsTree();
    ...
    tmp1.loadCache = function loadCache() {
      ...
      if (null != tree) {
        ...
        for (const item10011 of allNodesResult) {
          if (item10011.type === obj42(5747).GuildsNodeType.FOLDER) {
...
export const GuildsNodeType = fn(5747).GuildsNodeType;
