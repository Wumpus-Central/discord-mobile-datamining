...
import obj132Default from "../modules/tracking/TrackingConsentUtils.tsx";
import initialize from "BrowserHandoffStore.native.tsx";
import clearAll from "MobileCacheSnapshotStore.tsx";
    ...
    if (tmpResult.isValidFingerprintRoute()) {
      if (flag) {
        if (!handoffAvailable.isHandoffAvailable()) {
          ...
          obj[1] = obj;
          obj1 = { location: null };
          obj1[0] = transitionTo.getFingerprintLocation();
          obj[2] = obj1;
          const experiments = tmpResult.fetchExperiments(obj);
          nextPromise = experiments.then((result) => {
            ...
            const obj6 = callback(13555);
          }, () => {
            c33 = null;
            callback2(709).dispatch({ type: "EXPERIMENTS_FETCH_FAILURE" });
          });
          const tmpResult1 = transitionTo;
        }
        return nextPromise;
      }
    }
...
function handleLogout(isSwitchingAccount) {
  ...
  closure_25 = isSwitchingAccount1 ? LoginStates.LOGGING_IN : LoginStates.NONE;
  c28 = "";
  c30 = null;
  c29 = false;
  c35 = false;
...
const authenticationStore = new AuthenticationStore(importDefaultResult, {
  CONNECTION_OPEN: function handleConnectionOpen(arg0) {
    ...
    if (Storage4.get(constants.APP_FIRST_LOGIN, true)) {
      tmp6Result = expandEventPropertiesDefault;
      obj = { platform: null };
    ...
    const tmpResult = setSecondaryTokenAll;
  },
  OVERLAY_INITIALIZE: function handleOverlayInitialize(arg0) {
    ...
    if (!tmp8) {
      let tmp4Result = setSecondaryTokenAll;
      tmp4Result.removeAnalyticsToken();
    }
    tmp4Result = setSecondaryTokenAll;
    tmp4Result.setToken(token, id2);
    if (null != analyticsToken) {
      setSecondaryTokenAll.setAnalyticsToken(analyticsToken);
      const tmp4Result1 = setSecondaryTokenAll;
    }
    closure_22 = c21;
    c21 = null;
    const Storage2 = Storage6.Storage;
  ...
  UPDATE_TOKEN: function handleUpdateToken(userId) {
    ...
    if (!tmp8) {
      let tmpResult = setSecondaryTokenAll;
      tmpResult.removeAnalyticsToken();
    }
    tmpResult = setSecondaryTokenAll;
    tmpResult.setToken(userId.token, userId);
    closure_22 = c21;
    c21 = null;
    const Storage3 = Storage6.Storage;
    Storage3.remove(fingerprint);
    const tmp6 = null != setSecondaryTokenAll.getToken();
  },
  EXPERIMENTS_FETCH(withGuildExperiments) {
    ...
    closure_33 = experiments.then((result) => {
      const body = result.body;
      ({ fingerprint, installation } = body);
      let tmp = null != installation;
      ({ assignments, guild_experiments } = body);
...
result = require("obj132").fileFinishedImporting("stores/AuthenticationStore.tsx");
