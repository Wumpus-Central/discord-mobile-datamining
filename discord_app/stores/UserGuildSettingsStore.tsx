...
function updateUserGuildSettingsInternal(guild_id, channel_overrides) {
  ...
  closure_0 = guild_id;
  if (true === obj.muted) {
    if (navigation.setTimer(guild_id, obj.mute_config, () => {
  ...
  if (null != userGuildSettings[guild_id].channel_overrides) {
    const found = _modDef12(tmp19.channel_overrides).filter((item) => closure_1_0(closure_1_3[10]).computeIsMuted(item));
    const arr3 = _modDef12(tmp19.channel_overrides);
  ...
  delete tmp[tmp2];
  set1 = new Set(found1.map((channel_id) => channel_id.channel_id));
}
...
prototype["isOptInEnabled"] = function isOptInEnabled(id) {
  if (null == id) {
    ...
    let isOptInEnabledResult = ImpersonateStore.isOptInEnabled(id);
  } else {
    const self = this;
...
prototype["getGuildFavorites"] = function getGuildFavorites(id) {
  ...
  if (ImpersonateStore.isFullServerPreview(id)) {
    ...
    if (null == dependencyMap[id]) {
      ...
      dependencyMap[id] = found.map((channel_id) => channel_id.channel_id);
    }
...
prototype["getGuildUnreadSetting"] = function getGuildUnreadSetting(guild_id) {
  if (closure_18) {
    ...
    if (obj.hasFlag(guildFlags, constants2.UNREADS_ALL_MESSAGES)) {
      ...
      ALL_MESSAGES = FlagUtilsAll.hasFlag(guildFlags, constants2.UNREADS_ONLY_MENTIONS) ? UnreadSetting.ONLY_MENTIONS : UnreadSetting.UNSET;
...
prototype["getChannelUnreadSetting"] = function getChannelUnreadSetting(guild_id, id) {
  ...
  if (obj.hasFlag(channelIdFlags, constants.UNREADS_ALL_MESSAGES)) {
    ...
    ALL_MESSAGES = FlagUtilsAll.hasFlag(channelIdFlags, constants.UNREADS_ONLY_MENTIONS) ? UnreadSetting.ONLY_MENTIONS : UnreadSetting.UNSET;
