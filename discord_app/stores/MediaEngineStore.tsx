// === Module 4302: _detectH265HardwareDecode ===
...
function getSettings() {
  ...
  if (null == obj1.vadDuringPreProcess) {
    obj1.vadDuringPreProcess = require(12973) /* getVADBeforeProcessingExperimentConfig */.getVADBeforeProcessingExperimentConfig({ location: "getSettings" }).enabled;
    const obj7 = require(12973) /* getVADBeforeProcessingExperimentConfig */;
  }
  ...
  const aGC2ExperimentConfig = require(12974) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig({ location: "getSettings" });
...
function setInputMode(context) {
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    let obj = require(10565) /* setPushToTalkState */;
    obj.setPushToTalkState(false, false);
  }
  let showPTTSpeakingIndicator = importDefault(12975).getConfig({ location: "setInputMode" }).showPTTSpeakingIndicator;
...
function updateConnectionMuteDeaf(context) {
  ...
  if (MediaEngineContextTypes.DEFAULT === context) {
    ...
    if (!tmp3) {
      tmp3 = !importDefault(4798).didHavePermission(NativePermissionTypes.AUDIO);
      const obj = importDefault(4798);
    }
    let flag = tmp3;
  } else {
    flag = true;
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    importDefault(12956).updateNativeMute();
    const obj2 = importDefault(12956);
  }
}
      ...
      if (!tmp) {
        ...
        if (null != tmp2) {
          ...
          if (null != tmp2.desktopSource) {
            ...
            const obj20 = require(12972) /* HdrCaptureMode */;
            let isWindowsResult = require(500) /* set */.isWindows();
            if (isWindowsResult) {
              const tmp41 = importDefault(3866);
              let release;
              if (tmp41 != null) {
                release = tmp41.os.release;
              }
              isWindowsResult = importDefault(12986).satisfies(release, closure_29);
              const obj6 = importDefault(12986);
            }
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (videoHook2) {
                  const DisableHookFramePacerExperiment = tmp70(12979).DisableHookFramePacerExperiment;
                  tmp55 = !DisableHookFramePacerExperiment.getConfig({ location: "updateVideo" }).enabled;
                }
                let enabled2 = videoHook2;
                if (videoHook2) {
                  const VideoHookDX12Experiment = tmp70(12980).VideoHookDX12Experiment;
                  enabled2 = VideoHookDX12Experiment.getConfig({ location: "updateVideo" }).enabled;
                }
                const UpscaleSmallCapturedFramesExperiment = tmp70(12981).UpscaleSmallCapturedFramesExperiment;
                ...
                if (isMacResult) {
                  const tmp61 = importDefault(3866);
                  let release1;
                  if (tmp61 != null) {
                    release1 = tmp61.os.release;
                  }
                  isMacResult = importDefault(12986).satisfies(release1, closure_24);
                  const obj14 = importDefault(12986);
                }
                ...
                obj[13] = tmp70(12982).getGlobalFramePoolLockExperimentConfig({ location: "updateVideo" }).enabled;
                obj[14] = false;
                obj[15] = enabled2;
                ...
                const tmp70Result2 = tmp70(12982);
              } else if (true === c131) {
                const WGCDirtyRegionsExperiment = tmp70(12977).WGCDirtyRegionsExperiment;
                let enabled = WGCDirtyRegionsExperiment.getConfig({ location: "updateVideo" }).enabled;
              } else {
                const WGCDirtyRegionsAllExperiment = tmp70(12978).WGCDirtyRegionsAllExperiment;
                enabled = WGCDirtyRegionsAllExperiment.getConfig({ location: "updateVideo" }).enabled;
              }
            } else {
              let isWindowsResult2 = tmp70(500).isWindows();
              if (isWindowsResult2) {
                const tmp47 = importDefault(3866);
                let release2;
                if (tmp47 != null) {
                  release2 = tmp47.os.release;
                }
                isWindowsResult2 = importDefault(12986).satisfies(release2, closure_27);
                const obj8 = importDefault(12986);
              }
...
function noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) {
  if (NoiseCancellerError.KRISP_CPU_OVERUSE === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispCpuOveruse;
  } else if (tmp.KRISP_FAILED === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispFailed;
  } else if (tmp.KRISP_VAD_CPU_OVERUSE === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispVadCpuOveruse;
  } else if (tmp.KRISP_INIT_ERROR === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitError;
  } else if (tmp.KRISP_INIT_ERROR_NATIVE === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorNative;
  } else if (tmp.KRISP_INIT_ERROR_SSE4_NOT_SUPPORTED === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorSse4NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_AVX2_NOT_SUPPORTED === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorAvx2NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_UNSIGNED === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorUnsigned;
  } else if (tmp.KRISP_INIT_ERROR_GLOBAL_INIT === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorGlobalInit;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_8K === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorWeight8k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_16K === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorWeight16k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_32K === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorWeight32k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_VAD === noise_canceller_error) {
    return require(9038) /* mapped */.AVUnderlyingError.KrispInitErrorWeightVad;
  }
}
function handleVoiceProcessingError(noise_canceller_error) {
  obj.warn("Voice processing error: " + noise_canceller_error);
  obj = require(9038) /* mapped */;
  obj = { type: require(9038) /* mapped */.AVError.NOISE_CANCELLER_ERROR, underlyingError: noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) };
...
function handleVideoFilterError(arg0, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    obj[0] = require(9038) /* mapped */.AVError.VIDEO_BACKGROUND_UNAVAILABLE;
    if (constants9.UNSUPPORTED === arg0) {
      AVUnderlyingError = tmp4(9038).AVUnderlyingError;
      let VideoBackgroundInitFailed = AVUnderlyingError.VideoBackgroundUnsupported;
    } else if (tmp5.INIT_FAILED === arg0) {
      VideoBackgroundInitFailed = tmp4(9038).AVUnderlyingError.VideoBackgroundInitFailed;
    }
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  const merged = Object.assign(require(12974) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig(obj).noiseCancellationConfig);
  const result = setEchoCancellation.setAutomaticGainControl(obj);
  const noiseCancellation = tmp2.noiseCancellation;
  const obj3 = require(12974) /* getAGC2ExperimentConfig */;
  const tmp13Result = importDefault(10903)(noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  if (isWindowsResult) {
    ...
    isWindowsResult = tmp12(12986).satisfies(release, c152);
    const tmp12Result1 = tmp12(12986);
  }
  if (isWindowsResult) {
    if (setEchoCancellation.context === MediaEngineContextTypes.DEFAULT) {
      const systemwideEchoCancellationExperimentConfig = tmp8(12984).getSystemwideEchoCancellationExperimentConfig({ location: "updateConnectionVoiceProcessing" });
...
function applySettings() {
  ...
  if (obj2.isWindows()) {
    if (obj3.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
      ...
      if (null != guid) {
        tmp5(13042)(tmp7.guid, inputDeviceId, obj);
      }
    }
    obj3 = importDefault(13041);
    tmp5 = importDefault;
  }
...
function maybeTryHookProcess(pidFromDesktopSource, sound) {
  ...
  if (sound) {
    ...
    if (null != mediaEngineStore) {
      ...
      if (!obj.getExperimentalSoundshare()) {
        let obj2 = importDefault(4249);
        audioPid = obj2.getAudioPid(pidFromDesktopSource);
      }
      let str2 = "";
      if (null != audioPid) {
        str2 = importDefault(4249).generateSessionFromPid(audioPid);
        const obj4 = importDefault(4249);
      }
    ...
    if (null != soundshareId) {
      ...
      if (isWindowsResult) {
        ...
        const obj8 = importAll(12976);
        importAll(12976).attachToProcess(soundshareId, obj1).then((closure_0) => {
        ...
        const attachToProcessResult = importAll(12976).attachToProcess(soundshareId, obj1);
      }
  ...
  if (videoHook) {
    importAll(12976).attachToProcess(pidFromDesktopSource);
    const obj6 = importAll(12976);
  }
...
function trackVoiceProcessing(location) {
  ...
  const tmp11 = importDefault(10903);
  const tmp11Result = importDefault(10903)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
function setLoopback() {
  ...
  const tmp15 = importDefault(10903);
  const tmp15Result = importDefault(10903)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  const voiceFidelityCaps = tmp8(12983).getVoiceFidelityCaps({ location: "updateVoiceFidelityCaps" }, { krispEnabled: tmp15Result, noiseSuppressionEnabled: tmp14, echoCancellationEnabled: tmp13 });
        ...
        if (0 === c5) {
          ...
          if (2 === tmp8) {
            ...
            if (arg0 === 1) {
              ...
              lib = lib2(5263).requireModule("discord_krisp");
              let c109 = true;
...
let c85 = null;
let closure_86 = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Input Devices", disabled: true, guid: "accessible", hardwareId: "np", containerId: "o" } };
let closure_87 = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Output Devices", disabled: true, guid: "add", hardwareId: "description", containerId: "fileFinishedImporting" } };
let closure_88 = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Video Devices", disabled: true, guid: "borderRadius", hardwareId: "user", containerId: "isArray" } };
...
prototype["initialize"] = function initialize() {
  let self = this;
  if (navigation == null) {
    const tmp7 = new _pollMetrics(13048)(store3, self);
    navigation = tmp7;
  }
  ...
  let obj = store3;
  store3.on(pollMetrics(4352).MediaEngineEvent.Connection, (setAttenuation) => {
    ...
    if (enabled) {
      enabled = tmp9(13033).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result10 = tmp9(13033);
    }
    if (enabled) {
    ...
    if (enabled2) {
      enabled2 = tmp9(13034).getWmfGpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result12 = tmp9(13034);
    }
    if (enabled2) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    ...
    if (enabled3) {
      enabled3 = tmp9(13035).getWmfCpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result14 = tmp9(13035);
    }
    if (enabled3) {
      setAttenuation.setExperimentFlag(tmp22.INTEL_GPU_DISABLE, true);
    }
    ...
    if (enabled4) {
      enabled4 = tmp9(13033).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result16 = tmp9(13033);
    }
    if (enabled4) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    }
    const result2 = store3.setHasFullbandPerformance(tmp25(13036)());
    const result3 = setAttenuation.setRemoteAudioHistory(1000);
    if (_pollMetrics(4315)(closure_69)) {
    ...
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.Speaking, (userId, speakingFlags, arg2, voiceDb) => {
      let obj = outer1_1(outer1_3[44]);
    ...
    if (setAttenuation.context === constants4.DEFAULT) {
      let c98 = false;
      setAttenuation.on(tmp86(4352).BaseConnectionEvent.SpeakingWhileMuted, () => {
        let c98 = true;
        if (!c98) {
          closure_69.emitChange();
        }
    ...
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.DesktopSourceEnd, (endReason, errorCode) => {
      let obj = outer1_1(outer1_3[44]);
      obj = { type: "MEDIA_ENGINE_SET_GO_LIVE_SOURCE", settings: obj, endReason, errorCode };
      obj = { context: setAttenuation.context };
      obj.dispatch(obj);
    });
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.InteractionRequired, (required) => {
      let obj = callback(709);
      obj = { type: "MEDIA_ENGINE_INTERACTION_REQUIRED", required };
      obj.dispatch(obj);
    });
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.VideoHookInitialize, (arg0, arg1, arg2, arg3, arg4, arg5) => {
      let desktopSource;
      if (closure_74 != null) {
      ...
      if (null != desktopSource) {
        ...
        const merged = Object.assign(callback(4431)(desktopSource1));
        obj.track(constants.VIDEOHOOK_INITIALIZED, obj);
        const tmp15 = callback(4431);
      }
    });
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.NoiseCancellationError, closure_149);
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.VoiceActivityDetectorError, closure_149);
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.SdpError, (operation, error, type, sdp) => {
      let obj = callback(698);
      obj = { operation, error, type, sdp };
      obj.track(constants.SDP_ERROR, obj);
    });
    setAttenuation.on(pollMetrics(4352).BaseConnectionEvent.VideoState, (videoState) => {
      let obj = outer1_1(outer1_3[44]);
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.DeviceChange, (inputDevices, outputDevices, videoDevices) => {
    closure_89.stop();
    let obj = _pollMetrics(709);
    obj = { type: "MEDIA_ENGINE_DEVICES", inputDevices, outputDevices, videoDevices };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.VolumeChange, (inputVolume, outputVolume) => {
    let obj = _pollMetrics(709);
    obj = { type: "AUDIO_VOLUME_CHANGE", inputVolume, outputVolume };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.DesktopSourceEnd, (endReason, errorCode) => {
    let obj = _pollMetrics(709);
    obj = { type: "MEDIA_ENGINE_SET_GO_LIVE_SOURCE", settings: null, endReason, errorCode };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.AudioPermission, (granted) => {
    let c124 = true;
    let obj = _pollMetrics(709);
    obj = { type: "MEDIA_ENGINE_PERMISSION", kind: "audio", granted };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.VideoPermission, (granted) => {
    let obj = _pollMetrics(709);
    obj = { type: "MEDIA_ENGINE_PERMISSION", kind: "video", granted };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.WatchdogTimeout, callback2(function*() {
    if (c6 === 2) {
      c6 = 3;
      HermesBuiltin.throwTypeError();
    } else if (tmp6 === 3) {
      ...
      try {
        ...
        if (0 === c5) {
          ...
          if (typeof closure_2.status !== "SENTRY_RELEASE") {
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.VideoInputInitialized, (description) => {
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.AudioInputInitialized, (description) => {
    let obj = _pollMetrics(698);
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsRecordingRestartNeeded, () => {
    _pollMetrics(709).dispatch({ type: "CLIPS_RESTART" });
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsInitFailure, (arg0, arg1) => {
    let closure_0 = arg0;
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsRecordingHealthy, () => {
    if (0 !== c82) {
      const _HermesInternal = HermesInternal;
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsRecordingReadyChanged, (arg0) => {
    if (closure_83 !== arg0) {
      const _HermesInternal = HermesInternal;
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsBridgeIdleShutdown, () => {
    logger.info("Clips bridge idle shutdown");
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.ClipsRecordingEnded, (arg0, soundshareId) => {
    let id;
    ...
    if (id === arg0) {
      ...
      if (tmp3) {
        const result = callback(12976).cancelAttachToProcess(soundshareId);
        const obj = callback(12976);
      }
      desktopSource2 = null;
    }
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.NativeScreenSharePickerUpdate, (existing, content) => {
    let obj = _pollMetrics(709);
    obj = { type: "NATIVE_SCREEN_SHARE_PICKER_UPDATE", existing, content };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.NativeScreenSharePickerCancel, (existing) => {
    let obj = _pollMetrics(709);
    obj = { type: "NATIVE_SCREEN_SHARE_PICKER_CANCEL", existing };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.NativeScreenSharePickerError, (error) => {
    let obj = _pollMetrics(709);
    obj = { type: "NATIVE_SCREEN_SHARE_PICKER_ERROR", error };
    obj.dispatch(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.AudioDeviceModuleError, (audio_device_module, code, device_name) => {
    let obj = _pollMetrics(698);
    obj = { audio_device_module, code, device_name };
    obj.track(constants.AUDIO_DEVICE_MODULE_ERROR, obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.VideoCodecError, (mode) => {
    if ("encode" === mode.mode) {
      let VIDEO_DECODE_ERROR = pollMetrics(9038).AVError.VIDEO_ENCODE_ERROR;
      let tmp3 = pollMetrics;
    } else {
      VIDEO_DECODE_ERROR = pollMetrics(9038).AVError.VIDEO_DECODE_ERROR;
      tmp3 = pollMetrics;
    }
    let obj = { videoCodec: mode.codecStandard, errorMessage: mode.message };
    if (VIDEO_DECODE_ERROR === tmp3(9038).AVError.VIDEO_ENCODE_ERROR) {
      obj = { type: null };
    ...
    tmp3(9038).reportAVError(obj);
  });
  store3.on(pollMetrics(4352).MediaEngineEvent.ConnectionStats, (arr) => {
    let obj = _pollMetrics(709);
    obj = {
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.VoiceProcessingError, handleVoiceProcessingError);
  store3.on(pollMetrics(4352).MediaEngineEvent.VideoFilterError, handleVideoFilterError);
  store3.on(pollMetrics(4352).MediaEngineEvent.VoiceQueueMetrics, (arg0) => {
  ...
  store3.on(pollMetrics(4352).MediaEngineEvent.SystemMicrophoneModeChange, (arg0) => {
...
prototype["isSelfMute"] = function isSelfMute(DEFAULT) {
  ...
  if (!mute) {
    mute = !importDefault(4798).didHavePermission(NativePermissionTypes.AUDIO);
    const obj = importDefault(4798);
  }
  if (!mute) {
...
prototype["supportsExperimentalSoundshare"] = function supportsExperimentalSoundshare() {
  ...
  if (supportsResult) {
    ...
    supportsResult = importDefault(12986).satisfies(release, closure_31);
    const obj = importDefault(12986);
  }
  return supportsResult;
};
prototype["supportsHookSoundshare"] = function supportsHookSoundshare() {
  ...
  if (isWindowsResult) {
    const tmp6 = importDefault(3866);
    let release;
    if (tmp6 != null) {
      release = tmp6.os.release;
    }
    isWindowsResult = importDefault(12986).satisfies(release, closure_30);
    const obj2 = importDefault(12986);
  }
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = tmp6(13050).VideoCaptureDeviceNoReuseExperiment;
...
prototype["supportsScreenSoundshare"] = function supportsScreenSoundshare() {
  if (obj.isMac()) {
    ...
    if (supportsResult) {
      const tmp12 = importDefault(3866);
      let release;
      if (tmp12 != null) {
        release = tmp12.os.release;
      }
      supportsResult = importDefault(12986).satisfies(release, closure_25);
      const obj5 = importDefault(12986);
    }
    if (supportsResult) {
      let tmpResult = tmp(500);
      let satisfiesResult = tmpResult.isMac() && obj4.supports(tmp9.SCREEN_CAPTURE_KIT);
      if (satisfiesResult) {
        const tmp18 = importDefault(3866);
        let release1;
        if (tmp18 != null) {
          release1 = tmp18.os.release;
        }
        satisfiesResult = importDefault(12986).satisfies(release1, closure_24);
        const obj7 = importDefault(12986);
      }
...
prototype["getVideoStreamParameters"] = function getVideoStreamParameters(context) {
  ...
  if (this.supports(Features.VIDEO)) {
    ...
    if (enableSimulcast) {
      const DisableCameraSimulcastExperiment = require(13051) /* apexExperiment */.DisableCameraSimulcastExperiment;
...
const mediaEngineStore = new MediaEngineStore(require("dispatcher"), {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (tmp17) {
      const obj7 = importDefault(5263);
      const setMklZenOverrideEnabled = importDefault(5263).requireModule("discord_krisp").setMklZenOverrideEnabled;
      if (setMklZenOverrideEnabled != null) {
        const result1 = setMklZenOverrideEnabled(tmp2(13045).getMklZenOverrideExperimentConfig("handleConnectionOpen").enabled);
        const tmp2Result2 = tmp2(13045);
      }
      const requireModuleResult = importDefault(5263).requireModule("discord_krisp");
    }
    tmp17 = require(500) /* set */.isWindows() && c109;
    if (tmp2Result3.isIOS()) {
      const setNcModels = obj3.setNcModels;
      if (setNcModels != null) {
        setNcModels(tmp2(13046).getKrispV9iOSModels());
        const tmp2Result4 = tmp2(13046);
      }
      mediaEngineStore.emitChange();
    }
    const inputDeviceId = tmp.inputDeviceId;
    ...
    if (tmp2Result5.isWindows()) {
      if (obj12.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp26(13042)(tmp28.guid, inputDeviceId, obj3);
        }
      }
      obj12 = importDefault(13041);
      tmp26 = importDefault;
    }
    applyRemoteSettings();
  },
  ...
  AUDIO_SET_LOCAL_VIDEO_DISABLED: function handleSetLocalVideoDisabled(arg0) {
    ...
    if (!tmp22) {
      ...
      if (isAutomatic) {
        tmp3(12968)(userId, tmp12 ? constants12.AUTO_DISABLE : constants12.AUTO_ENABLE, tmp11);
        if (tmp12) {
          obj.add(userId);
        } else {
          obj.delete(userId);
        ...
        if (hasItem) {
          if (!tmp12) {
            ...
            tmp3(12968)(userId, constants12.MANUAL_REENABLE, tmp11);
          }
        }
        tmp3(12968)(userId, tmp12 ? constants12.MANUAL_DISABLE : constants12.MANUAL_ENABLE, tmp11);
      }
      if (tmp19) {
        tmp19 = !tmp12;
      }
  ...
  AUDIO_SET_AUDIO_MIXER_SETTINGS: function handleSetAudioMixerSettings(arg0) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    let supportsResult = true === settings.enabled && importDefault(12967).getConfig({ location: "MediaEngineStore" }).enabled;
    if (supportsResult) {
      supportsResult = store3.supports(Features.SPATIAL_AUDIO);
    }
    let c120 = supportsResult;
  ...
  AUDIO_SET_INPUT_DEVICE: function handleSetInputDevice(id) {
    ...
    if (obj6.isWindows()) {
      if (obj7.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp20(13042)(tmp22.guid, id, obj5);
        }
      }
      obj7 = importDefault(13041);
      tmp20 = importDefault;
    }
  ...
  AUDIO_SET_KRISP_MODEL_OVERRIDE: function handleSetKrispModelOverride(model) {
    const result = importDefault(12951).setKrispModelOverride(model.model);
    model = model.model;
    setLoopback();
  },
  ...
  AUDIO_SET_KRISP_SUPPRESSION_LEVEL: function handleSetKrispSuppressionLevel(level) {
    level = level.level;
    const result = importDefault(12951).setKrispSuppressionLevel(level);
  },
  ...
  MEDIA_ENGINE_DEVICES: function handleDevices(videoDevices) {
    ...
    if (0 === arr.length) {
      let obj = { id: null, index: 0, name: null, disabled: true, guid: "borderRadius", hardwareId: "end", containerId: "end" };
    ...
    if (0 === arr3.length) {
      const obj1 = { id: null, index: 0, name: null, disabled: true, guid: "borderRadius", hardwareId: "end", containerId: "end" };
    ...
    if (0 === arr5.length) {
      obj3 = { id: null, index: 0, name: null, disabled: true, guid: "borderRadius", hardwareId: "end", containerId: "end" };
    ...
    if (c94) {
      if (!tmp9Result1.isEqual(valueResult1, tmp40)) {
        ...
        let tmp52 = "Firefox" === tmp9(4574).name;
        if (tmp52) {
  ...
  AUDIO_INPUT_DETECTED: function handleInputDetected(inputDetected) {
    ...
    if (null == inputDetected) {
      ...
      if (inputDetected) {
        ...
        if (c102) {
          const config = importDefault(13047).getConfig({ location: "MediaEngineStore.handleInputDetected" });
  ...
  AUDIO_SET_BYPASS_SYSTEM_INPUT_PROCESSING: function handleBypassSystemInputProcessing(bypassEnabled) {
    ...
    const tmp19 = importDefault(10903);
    const tmp19Result = importDefault(10903)(tmp16.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
    const voiceFidelityCaps = tmp12(12983).getVoiceFidelityCaps({ location: "updateVoiceFidelityCaps" }, { krispEnabled: tmp19Result, noiseSuppressionEnabled: tmp18, echoCancellationEnabled: tmp17 });
  ...
  MEDIA_ENGINE_SET_GO_LIVE_SOURCE: function handleSetGoLiveSource(settings) {
    ...
    if (null != desktopSettings) {
      ...
      const pidFromDesktopSource = importDefault(4249).getPidFromDesktopSource(sourceId);
  ...
  MEDIA_ENGINE_SET_OPENH264_ENABLED: function handleSetOpenH264Enabled(enabled) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    const tmp13 = importDefault(5263);
    if (tmp13 != null) {
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    let obj = require(4313) /* isClipsEnabled */;
    if (obj.isDecoupledClipsEnabled()) {
      if (null != importDefault(3866)) {
        let id;
        if (obj != null) {
        ...
        if (videoHook2) {
          const DisableHookFramePacerExperiment = tmp(12979).DisableHookFramePacerExperiment;
          tmp21 = !DisableHookFramePacerExperiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        let enabled = videoHook2;
        if (videoHook2) {
          const VideoHookDX12Experiment = tmp(12980).VideoHookDX12Experiment;
          enabled = VideoHookDX12Experiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        const UpscaleSmallCapturedFramesExperiment = tmp(12981).UpscaleSmallCapturedFramesExperiment;
        ...
        if (isMacResult) {
          ...
          isMacResult = tmp3(12986).satisfies(release1, closure_24);
          const tmp3Result2 = tmp3(12986);
        }
  ...
  MEDIA_ENGINE_SET_DEVICE_AUDIO_EFFECTS: function handleSetDeviceAudioEffects(active) {
    ...
    const tmp4 = importDefault(10903);
    const tmp4Result = importDefault(10903)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
    const voiceFidelityCaps = require(12983) /* getAudioFidelityExperimentConfig */.getVoiceFidelityCaps({ location: "updateVoiceFidelityCaps" }, { krispEnabled: tmp4Result, noiseSuppressionEnabled: tmp3, echoCancellationEnabled: tmp2 });
