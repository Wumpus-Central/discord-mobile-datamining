...
import VideoGuardExperiment2 from "VideoGuardExperiment" /* 13260 */;
import isClipsEnabled from "isClipsEnabled" /* 13674 */;
import KrispUtilsDefault from "KrispUtils" /* 13810 */;
import NativeMuteManagerDefault from "NativeMuteManager" /* 13815 */;
import SpatialAudioForVoiceExperimentDefault from "SpatialAudioForVoiceExperiment" /* 13826 */;
import GoLiveHdrExperiment from "GoLiveHdrExperiment" /* 13999 */;
import StreamZeroVadLeadingExperiment2 from "StreamZeroVadLeadingExperiment" /* 14000 */;
import PTTEducationExperimentDefault from "PTTEducationExperiment" /* 14001 */;
import MuteAwareNoiseCancellationExperiment from "MuteAwareNoiseCancellationExperiment" /* 14002 */;
import HookAll from "Hook" /* 14003 */;
import AudioFidelityExperiment from "AudioFidelityExperiment" /* 14010 */;
import _modDef14014 from "module_14014" /* 14014 */;
import AudioEffectsExperimentDefault from "AudioEffectsExperiment" /* 14069 */;
import SilenceWarningExperimentDefault from "SilenceWarningExperiment" /* 14074 */;
import InputWatcherDefault from "InputWatcher" /* 14075 */;
import DisableCameraSimulcastExperiment2 from "DisableCameraSimulcastExperiment" /* 14078 */;
          ...
          if (null != tmp2.desktopSource) {
            ...
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (videoHook2) {
                  const DisableHookFramePacerExperiment = tmp70(14006).DisableHookFramePacerExperiment;
                  tmp55 = !DisableHookFramePacerExperiment.getConfig({ location: "updateVideo" }).enabled;
                }
                let enabled2 = videoHook2;
                if (videoHook2) {
                  const VideoHookDX12Experiment = tmp70(14007).VideoHookDX12Experiment;
                  enabled2 = VideoHookDX12Experiment.getConfig({ location: "updateVideo" }).enabled;
                }
                const UpscaleSmallCapturedFramesExperiment = tmp70(14008).UpscaleSmallCapturedFramesExperiment;
                ...
                obj.enableGlobalFramePoolLock = tmp70(14009).getGlobalFramePoolLockExperimentConfig({ location: "updateVideo" }).enabled;
                ...
                const tmp70Result2 = tmp70(14009);
              } else if (true === c132) {
                const WGCDirtyRegionsExperiment = tmp70(14004).WGCDirtyRegionsExperiment;
                enabled = WGCDirtyRegionsExperiment.getConfig({ location: "updateVideo" }).enabled;
              } else {
                const WGCDirtyRegionsAllExperiment = tmp70(14005).WGCDirtyRegionsAllExperiment;
...
function handleVideoFilterError(code, arg1) {
  ...
  if (arg1 !== constants11.LIVE) {
    ...
    if (constants10.UNSUPPORTED === code) {
      AVUnderlyingError = tmp4(9110).AVUnderlyingError;
      let VideoBackgroundInitFailed = AVUnderlyingError.VideoBackgroundUnsupported;
    } else if (tmp5.INIT_FAILED === code) {
      VideoBackgroundInitFailed = tmp4(9110).AVUnderlyingError.VideoBackgroundInitFailed;
    }
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  if (isWindowsResult) {
    ...
    isWindowsResult = tmp10(14014).satisfies(release, c154);
    const tmp10Result1 = tmp10(14014);
  }
  if (isWindowsResult) {
    if (setEchoCancellation.context === MediaEngineContextTypes.DEFAULT) {
      let tmp26Result = tmp26(14011);
  ...
  if (tmp26Result.isDesktop()) {
    const result5 = setEchoCancellation.setNoiseCancellationCpuDisablement(tmp26(14012).getKrispCpuDisablementConfig({ location: "updateConnectionVoiceProcessing" }).consecutiveFailures);
    const tmp26Result1 = tmp26(14012);
  }
...
function applySettings() {
  ...
  if (obj2.isWindows()) {
    if (obj3.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
      ...
      if (null != guid) {
        tmp5(14070)(tmp7.guid, inputDeviceId, obj);
      }
    }
      ...
      if (0 === c5) {
        ...
        if (2 === tmp8) {
          ...
          if (!message.includes(": ")) {
            ...
            let obj2 = { type: closure_130_0(closure_130_3[43]).AVError.NOISE_CANCELLER_ERROR, underlyingError: closure_130_148(KRISP_INIT_ERROR2) };
...
let c85 = null;
const inputDevices = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Input Devices", disabled: true, guid: "Boolean", hardwareId: "ip", containerId: "isArray" } };
const outputDevices = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Output Devices", disabled: true, guid: "call", hardwareId: "location", containerId: "format" } };
let dependencyMap = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, index: 0, name: "No Video Devices", disabled: true, guid: "call", hardwareId: "httpServerLocation", containerId: "fill" } };
...
prototype["initialize"] = function initialize() {
  ...
  closure_72.on(pollMetrics(4615).MediaEngineEvent.Connection, (setAttenuation) => {
    ...
    const result2 = closure_72.setHasFullbandPerformance(tmp25(14064)());
  ...
  if (tmp10Result6.isWeb()) {
    ...
    if (null == tmp56) {
      ...
      let obj2 = { threshold: -60, autoThreshold: tmp10(1115).isPlatformEmbedded || false, vadUseKrisp: true, vadKrispActivationThreshold: 0.5, vadLeading: 5, vadTrailing: 25, delay: 20, shortcut: [], updatedAt: "PX_16" };
      obj1.modeOptions = obj2;
      obj1.localMutes = {};
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = tmp6(14077).VideoCaptureDeviceNoReuseExperiment;
...
const mediaEngineStore = new MediaEngineStore(DispatcherDefault, {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (tmp2Result1.isIOS()) {
      ...
      if (setNcModels != null) {
        setNcModels(tmp2(14073).getKrispV9iOSModels());
        const tmp2Result2 = tmp2(14073);
      }
      mediaEngineStore.emitChange();
    }
    const inputDeviceId = tmp.inputDeviceId;
    ...
    if (tmp2Result3.isWindows()) {
      if (obj9.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp21(14070)(tmp23.guid, inputDeviceId, obj3);
        }
      }
  ...
  RTC_CONNECTION_STATE: function handleRTCConnectionStateUpdate(state) {
    ...
    if (constants3.CONNECTING === state) {
      ...
      if (0 !== set1.size) {
        ...
        if (null == tmp3) {
          obj = { mode: InputModes.VOICE_ACTIVITY, modeOptions: null, vadUseKrispSettingVersion: 0, ncUseKrispSettingVersion: 0, ncUseKrispjsSettingVersion: 0, mute: false, deaf: false, echoCancellation: true, noiseSuppression: false, automaticGainControl: true, noiseCancellation: true, bypassSystemInputProcessing: true, hardwareEnabledVersion: 0, silenceWarning: true, attenuation: 0, attenuateWhileSpeakingSelf: false, attenuateWhileSpeakingOthers: true, localMutes: null, disabledLocalVideos: null, videoToggleStateMap: null, localVolumes: null, audioMixerSettings: null, audioMixerSettingsVersion: 0, localPans: null, inputVolume: null, outputVolume: null, inputDeviceId: null, outputDeviceId: null, videoDeviceId: null, qos: false, qosMigrated: false, videoHook: null, experimentalSoundshare2: null, useSystemScreensharePicker: null, h265Enabled: true, vadThrehsoldMigrated: false, aecDumpEnabled: false, openH264Enabled: true, sidechainCompression: true, sidechainCompressionSettingVersion: 1, sidechainCompressionStrength: 50, automaticAudioSubsystem: true, activeInputProfile: null };
          obj = { threshold: -60, autoThreshold: DEFAULT2(1115).isPlatformEmbedded || false, vadUseKrisp: true, vadKrispActivationThreshold: 0.5, vadLeading: 5, vadTrailing: 25, delay: 20, shortcut: [], updatedAt: "PX_16" };
          obj.modeOptions = obj;
          obj.localMutes = {};
          obj.disabledLocalVideos = {};
          obj.videoToggleStateMap = {};
  ...
  AUDIO_SET_LOCAL_VIDEO_DISABLED: function handleSetLocalVideoDisabled(arg0) {
    ...
    if (!tmp22) {
      ...
      if (isAutomatic) {
        tmp3(13827)(userId, tmp12 ? constants13.AUTO_DISABLE : constants13.AUTO_ENABLE, tmp11);
        if (tmp12) {
          obj.add(userId);
        } else {
          obj.delete(userId);
        ...
        if (hasItem) {
          if (!tmp12) {
            ...
            tmp3(13827)(userId, constants13.MANUAL_REENABLE, tmp11);
          }
        }
        tmp3(13827)(userId, tmp12 ? constants13.MANUAL_DISABLE : constants13.MANUAL_ENABLE, tmp11);
      }
      if (tmp19) {
        tmp19 = !tmp12;
      }
  ...
  AUDIO_SET_INPUT_DEVICE: function handleSetInputDevice(id) {
    ...
    if (obj6.isWindows()) {
      if (obj7.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp20(14070)(tmp22.guid, id, obj5);
        }
      }
  ...
  MEDIA_ENGINE_DEVICES: function handleDevices(videoDevices) {
    ...
    if (0 === arr.length) {
      obj = { id: DEFAULT_DEVICE_ID, index: 0, name: tmp5, disabled: true, guid: "call", hardwareId: "justifyContent", containerId: "color" };
      obj = {};
      obj[obj.id] = obj;
      let valueResult = obj;
    } else {
    ...
    if (0 === arr3.length) {
      const obj1 = { id: DEFAULT_DEVICE_ID, index: 0, name: tmp24, disabled: true, guid: "call", hardwareId: "justifyContent", containerId: "color" };
      let obj2 = {};
      obj2[obj1.id] = obj1;
      valueResult = obj2;
    } else {
    ...
    if (0 === arr5.length) {
      obj3 = { id: DEFAULT_DEVICE_ID, index: 0, name: tmp41, disabled: true, guid: "call", hardwareId: "justifyContent", containerId: "color" };
      obj4 = {};
      obj4[obj3.id] = obj3;
        ...
        if (c102) {
          ...
          if (config.enableHardwareSilenceWarning) {
            timeout3.start(MINUTE, () => {
              ...
              if (firstResult == null) {
                const values = _modDef12(tmp3).values();
                firstResult = values.first();
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    if (obj.isClipsEnabled()) {
      if (null != DiscordNativeDefault) {
        ...
        if (videoHook2) {
          const DisableHookFramePacerExperiment = tmp(14006).DisableHookFramePacerExperiment;
          tmp21 = !DisableHookFramePacerExperiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        enabled = videoHook2;
        if (videoHook2) {
          const VideoHookDX12Experiment = tmp(14007).VideoHookDX12Experiment;
          enabled = VideoHookDX12Experiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        const UpscaleSmallCapturedFramesExperiment = tmp(14008).UpscaleSmallCapturedFramesExperiment;
        ...
        if (isMacResult) {
          ...
          isMacResult = tmp3(14014).satisfies(release1, __initData4);
          const tmp3Result2 = tmp3(14014);
        }
        ...
        obj2.videoEncoderExperiments = obj12.getVideoEncoderExperiments(MediaEngineContextTypes.STREAM, "streamer");
