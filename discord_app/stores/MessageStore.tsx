// === Module 4473: reinjectEphemerals ===
...
function handleConnectionOpen() {
  const item = importDefault(4926).forEach((mutate) => {
        ...
        try {
          ...
          if (0 === c6) {
            if (arg0 === 1) {
              ...
              let orCreate = tmp28;
              orCreate = undefined;
              const databaseResult = lib(1907).database();
              ...
              if (null != databaseResult) {
                if (null != basicChannel) {
                  ...
                  obj1[0] = lib(1907).messages(databaseResult).get(basicChannel.guild_id, tmp35, tmp36.id);
                  return obj1;
                }
              }
              const obj9 = lib(1907);
          ...
          lib(4926).commit(orCreate.receivePushNotification(lib, dependencyMap));
...
function receiveMediaMentionMessage(item10037) {
  ...
  if (null != message_id) {
    ...
    const orCreate = importDefault(4926).getOrCreate(attachment_id);
    let obj = {};
    ...
    tmp5(4926).commit(mutation);
    const tmp5Result = tmp5(4926);
  }
}
function handleCleanup() {
  const item = importDefault(4926).forEach((channelId) => {
...
function handleRelationshipUpdate() {
  let c0 = false;
  const item = importDefault(4926).forEach((reset) => {
...
function handleReaction(optimistic) {
  ...
  const value = importDefault(4926).get(channelId);
  if (null == value) {
    return false;
  } else {
    if (obj3.shouldApplyReaction(optimistic)) {
      let fetchFingerprint = id.getId() === userId;
      const updateResult = value.update(messageId, (addReaction) => {
        if ("MESSAGE_REACTION_ADD" === closure_1) {
          let addReactionResult = addReaction.addReaction(closure_2, fetchFingerprint, optimistic.colors, canEditMessage);
        } else {
          addReactionResult = addReaction.removeReaction(closure_2, fetchFingerprint, canEditMessage);
        }
        return addReactionResult;
      });
      tmp(4926).commit(updateResult);
    } else {
      return false;
    }
    obj3 = _require(3894);
  }
  const obj = importDefault(4926);
  tmp = importDefault;
}
function handleMessageSendFailedAutomod(arg0) {
  ...
  const failedMessageId = require(7201) /* items */.getFailedMessageId(messageData);
  const obj = require(7201) /* items */;
  const tmp3 = importDefault;
  const orCreate = importDefault(4926).getOrCreate(messageData.message.channelId);
  if (orCreate.has(failedMessageId)) {
    const updateResult = orCreate.update(failedMessageId, (embeds) => {
      ...
      if ("MESSAGE_SEND_FAILED_AUTOMOD" === closure_0) {
        result1 = result.set("flags", outer1_0(outer1_2[24]).addFlag(result.flags, outer1_19.EPHEMERAL));
        const obj2 = outer1_0(outer1_2[24]);
      }
      return result1;
    });
    tmp3(4926).commit(updateResult);
  } else {
    return false;
  }
  let obj2 = importDefault(4926);
}
...
let set = new Set();
let closure_25 = new require("dropChannelIfEmpty")("MessageStore");
let c26 = false;
  ...
  if (initialize.hasViewingRoles()) {
    ...
    if (initialize.isViewingRoles(guildId)) {
      if (!getUncachedChannelPermissions.can(constants4.VIEW_CHANNEL, channel)) {
        const tmp11 = new importDefault(4926)(arg0);
        return tmp11;
      }
    }
  }
  return importDefault(4926).getOrCreate(arg0);
};
prototype["getMessage"] = function getMessage(arg0, arg1) {
  const orCreate = importDefault(4926).getOrCreate(arg0);
...
prototype["jumpedMessageId"] = function jumpedMessageId(arg0) {
  const value = importDefault(4926).get(arg0);
  let jumpTargetId;
  if (value != null) {
...
prototype["isReady"] = function isReady(arg0) {
  return importDefault(4926).getOrCreate(arg0).ready;
};
...
prototype["isLoadingMessages"] = function isLoadingMessages(channelId) {
  return importDefault(4926).getOrCreate(channelId).loadingMore;
};
...
prototype["hasCurrentUserSentWaveBlockingMessage"] = function hasCurrentUserSentWaveBlockingMessage(id) {
  ...
  return null != messages.findNewest((type) => {
    let tmp = type.type !== outer1_22.FRIEND_REQUEST_ACCEPTED;
    if (tmp) {
      let id;
      if (id != null) {
        id = id.id;
...
const messageStore = new MessageStore(require("dispatcher"), {
  BACKGROUND_SYNC_CHANNEL_MESSAGES: function handleBackgroundSyncChannelMessages(changesByChannelId) {
    ...
    for (const key10012 in changesByChannelId) {
      ...
      let obj = importDefault(4926);
      let value = obj.get(key10012);
      if (null == value) {
        continue;
      } else {
        let tmp = require;
        let _default = require(4931) /* _handleConnectionOpen */.default;
  ...
  LOAD_MESSAGES_SUCCESS: function handleLoadMessagesSuccess(arg0) {
    ...
    const orCreate = importDefault(4926).getOrCreate(channelId);
  ...
  LOAD_MESSAGES_FAILURE: function handleLoadMessagesFailure(channelId) {
    const orCreate = importDefault(4926).getOrCreate(channelId.channelId);
    const obj = importDefault(4926);
    importDefault(4926).commit(orCreate.mutate({ loadingMore: false, error: true }));
  },
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(channelId) {
    ...
    importDefault(4926).commit(mutation);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    const obj = importDefault(4926);
    const tmp3 = require;
    const isConnectedResult = require(4931) /* _handleConnectionOpen */.default.isConnected();
      ...
      if (orCreate.has(messageId)) {
        ...
        if (true === isPollResult) {
          ...
          removeResult = orCreate.update(messageId, (set) => {
            const result = set.set("state", outer1_21.SEND_FAILED);
            if (result.isCommandType()) {
              ...
              let result2 = result1.set("flags", outer1_0(outer1_2[24]).addFlag(result1.flags, outer1_19.EPHEMERAL));
        ...
        importDefault(4926).commit(removeResult);
      }
    }
    return false;
  },
  ...
  MESSAGE_UPDATE: function handleMessageUpdate(message) {
    ...
    const orCreate = importDefault(4926).getOrCreate(message.message.channel_id);
    if (null != orCreate) {
      if (orCreate.has(id)) {
        let tmpResult = tmp(4926);
        ...
        if (null != message_id) {
          if ("content" in message) {
            ...
            tmp(4926).commit(updateResult1);
            const tmpResult1 = tmp(4926);
          }
        }
      }
    }
  ...
  MESSAGE_EXPLICIT_CONTENT_SCAN_TIMEOUT: function handleMessageExplicitContentScanTimeout(messageId) {
    ...
    if (null != value) {
      if (value.has(messageId)) {
        const updateResult = value.update(messageId, require(6956) /* redactionSettingToRenderedString */.handleExplicitMediaScanTimeoutForMessage);
        importDefault(4926).commit(updateResult);
      }
    }
    return false;
  ...
  MESSAGE_REVEAL: function handleMessageReveal(arg0) {
    ...
    const orCreate = importDefault(4926).getOrCreate(channelId);
    const obj = importDefault(4926);
    importDefault(4926).commit(orCreate.mutate({ revealedMessageId: messageId }));
  },
  THREAD_CREATE_LOCAL: function handleThreadCreateLocal(channelId) {
    ...
    const obj = importDefault(4926);
    importDefault(4926).commit(complete);
  },
  ...
  THREAD_MEMBER_LIST_UPDATE: function handleThreadMemberListUpdate(guildId) {
    ...
    let item = importDefault(4926).forEach((channelId) => {
      const channel = outer1_9.getChannel(channelId.channelId);
      let guild_id;
      if (channel != null) {
        guild_id = channel.guild_id;
  ...
  UPLOAD_FAIL: function handleUploadFail(messageId) {
    ...
    if (null == messageId) {
      ...
      if (set.has(messageId)) {
        const orCreate = importDefault(4926).getOrCreate(tmp);
