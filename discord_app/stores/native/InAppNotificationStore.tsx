...
import playInAppMessageSound from "playInAppMessageSound" /* 10252 */;
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = emoji(4794).tryParseChannelPath(tmp);
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = emoji(10235).shouldIncludeSelectedChannel();
                        const tmp19Result9 = emoji(10235);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = emoji(4863).createMessageRecord(message);
                          const tmp19Result11 = emoji(4863);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: emoji(10244).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = emoji(10244);
                            obj3.inAppNotificationId = emoji(10244).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                            const tmp19Result14 = emoji(10244);
                          } else {
                            return false;
                          }
                          tmp19Result12 = emoji(10243);
                        } else {
                          return false;
                        }
                        tmp19Result10 = emoji(10235);
                      }
                    }
                    return false;
                  }
