// === Module 10123: handleAlertMessage ===
...
import DATE_CONFIG from "DATE_CONFIG" /* 7892 */;
import GuildIncidentActionSources from "GuildIncidentActionSources" /* 7894 */;
import NativeEventEmitterDefault from "NativeEventEmitter" /* 9540 */;
import isOnlyDayLoss from "isOnlyDayLoss" /* 10126 */;
import canViewPotentiallyNSFWChannel from "canViewPotentiallyNSFWChannel" /* 10128 */;
import shouldNotifyBase from "shouldNotifyBase" /* 10129 */;
import isReactionMilestoneNotification from "isReactionMilestoneNotification" /* 10138 */;
import apexExperimentDefault from "apexExperiment" /* 10141 */;
import playInAppMessageSound from "playInAppMessageSound" /* 10147 */;
import closure_3 from "participantFromServer" /* 1386 */;
import closure_4 from "getParticipants" /* 4464 */;
import closure_5 from "computeAlertSettings" /* 10124 */;
import { trackMessageNotificationTimestamps as closure_6 } from "initialize" /* 5451 */;
import importDefaultResult from "freshTeenActivityWithMap" /* 7250 */;
...
import closure_11 from "DesktopNotificationTypes" /* 10125 */;
import closure_12 from "generateOldThreadCutoff" /* 4463 */;
import closure_13 from "handleConnectionOpen" /* 1981 */;
import closure_14 from "handleRequiredAction" /* 1381 */;
import closure_15 from "mergeGuildAvatar" /* 1922 */;
import ME from "ME" /* 676 */;
import { EMPTY_SCHEDULE_SNAPSHOT } from "isOnlyDayLoss" /* 10126 */;

require = arg1;
function handleAlertMessage() {
  obj = guild(10129);
  if (obj.allowInAppNotifications()) {
    let tmpResult = tmp(10142);
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = store2.getChannel(channel(10146)(guild));
          if (null == channel) {
            return false;
            ...
            if (obj.wasRecentlyDismissed(obj)) {
              ...
              obj1[4] = channel;
              obj1[5] = guild;
              obj1[6] = tmp(10138).generateInAppNotificationId();
              obj[0] = obj1;
...
const inAppNotificationStore = new InAppNotificationStore(dispatcherDefault, {
          ...
          if (tmpResult.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!uiStore.getChatOpen(channel_id)) {
                const result = tmp(10129).shouldIncludeSelectedChannel();
                const tmpResult1 = tmp(10129);
                if (tmpResult2.shouldNotify(message, channel_id, result)) {
                  const channel = store2.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4705).createMessageRecord(message);
                    const tmpResult3 = tmp(4705);
                    if (tmpResult4.isMessageContentPreviewable(messageRecord)) {
                      ...
                      obj[0] = MESSAGE;
                      const notificationDuration = tmp(10138).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult5 = tmp(10138);
                      obj[8] = tmp(10138).generateInAppNotificationId();
                    ...
                    tmpResult4 = tmp(10137);
                  }
                  obj7 = store2;
                } else {
                  return false;
                }
                tmpResult2 = tmp(10129);
              }
            }
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      const result = channelId(10138).extractMetadataFromNotification(_null);
      ({ guildId, channelId: channelId2, messageId: messageId2 } = result);
      const obj2 = channelId(10138);
      ...
      messageId(4668).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj3 = messageId(4668);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            ...
            if (null != channelId) {
              if (null != guildId) {
                const ReactionNotifications = tmp19(4136).ReactionNotifications;
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!uiStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10129).shouldIncludeSelectedChannel();
                        const tmp19Result1 = tmp19(10129);
                        ...
                        if (tmp19Result2.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4705).createMessageRecord(message);
                          const tmp19Result3 = tmp19(4705);
                          if (tmp19Result4.isMessageContentPreviewable(messageRecord)) {
                            ...
                            obj1[0] = REACTION;
                            obj1[1] = channelId;
                            obj1[2] = tmp19(10138).getNotificationDuration(REACTION);
                            obj1[3] = function onDismiss() {
                              callback(table[27]).clearNotification();
                            };
                            obj1[4] = channel;
                            ...
                            obj1[9] = found;
                            const tmp19Result5 = tmp19(10138);
                            obj1[10] = tmp19(10138).generateInAppNotificationId();
                            obj[0] = obj1;
                            handleEnqueueNotification(obj);
                          } else {
                            return false;
                          }
                          tmp19Result4 = tmp19(10137);
                        } else {
                          return false;
                        }
                        tmp19Result2 = tmp19(10129);
                      }
                    }
                    return false;
                  }
            ...
            if (tmpResult.shouldNotifyForForumThreadCreation(channel, channel, !tmpResult.isChannelFocused())) {
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  obj[4] = guild;
                  obj[5] = channel.id;
                  obj[6] = tmp(10138).getNotificationDuration(FORUM_THREAD_CREATED);
                  obj[7] = function onDismiss() {
                    callback(table[27]).clearNotification();
                  };
                  const tmpResult1 = tmp(10138);
                  obj[8] = tmp(10138).generateInAppNotificationId();
                  obj[0] = obj;
                  handleEnqueueNotification(obj);
                }
  ...
  CHANNEL_RTC_UPDATE_CHAT_OPEN: function handleVoiceChatOpen(channelId) {
    ...
    if (tmp) {
      ...
      obj[3] = messageId;
      obj[5] = _null.inAppNotificationId;
      chatOpen(4668).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj2 = chatOpen(4668);
    }
    obj1.removeAll(function predicate(type) {
      let tmp = type.type === closure_1_18.MESSAGE;
      if (tmp) {
    ...
    if (c24) {
      ...
      if (obj.getIsFamilyCenterV3Enabled({ location: "InAppNotificationStore" })) {
        ...
        if (null != diffSchedulesResult) {
          const EnableScreenDowntimeScheduleNotifications = tmp(4136).EnableScreenDowntimeScheduleNotifications;
          if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
            if (tmpResult1.allowInAppNotifications()) {
              ...
              const notificationDuration = tmp(10138).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
              const tmpResult2 = tmp(10138);
              obj[1] = tmp(10126).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
              obj[2] = notificationDuration;
              obj[3] = function onDismiss() {
                callback(table[27]).clearNotification();
              };
              const tmpResult3 = tmp(10126);
              obj[4] = tmp(10126).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
              const tmpResult4 = tmp(10126);
              obj[5] = tmp(10126).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
              const tmpResult5 = tmp(10126);
              obj[6] = tmp(10138).generateInAppNotificationId();
              obj[0] = obj;
              handleEnqueueNotification(obj);
              const tmpResult6 = tmp(10138);
            }
            tmpResult1 = tmp(10129);
          }
        }
      } else {
        return false;
