// === Module 10100: handleAlertMessage ===
...
import DATE_CONFIG from "DATE_CONFIG" /* 7870 */;
import GuildIncidentActionSources from "GuildIncidentActionSources" /* 7872 */;
import NativeEventEmitterDefault from "NativeEventEmitter" /* 9518 */;
import isOnlyDayLoss from "isOnlyDayLoss" /* 10103 */;
import canViewPotentiallyNSFWChannel from "canViewPotentiallyNSFWChannel" /* 10105 */;
import shouldNotifyBase from "shouldNotifyBase" /* 10106 */;
import isReactionMilestoneNotification from "isReactionMilestoneNotification" /* 10115 */;
import apexExperimentDefault from "apexExperiment" /* 10118 */;
import playInAppMessageSound from "playInAppMessageSound" /* 10124 */;
import closure_3 from "participantFromServer" /* 1390 */;
import closure_4 from "getParticipants" /* 4462 */;
import closure_5 from "computeAlertSettings" /* 10101 */;
import { trackMessageNotificationTimestamps as closure_6 } from "initialize" /* 5448 */;
import importDefaultResult from "freshTeenActivityWithMap" /* 7229 */;
import closure_8 from "handleConnectionClosedOrResumed" /* 1340 */;
import closure_9 from "ensureGuildLoaded" /* 1391 */;
import closure_10 from "createGuildRecordFromRust" /* 1910 */;
import closure_11 from "DesktopNotificationTypes" /* 10102 */;
...
import { EMPTY_SCHEDULE_SNAPSHOT } from "isOnlyDayLoss" /* 10103 */;
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = store2.getChannel(channel(10123)(guild));
          if (null == channel) {
            return false;
            ...
            if (obj.wasRecentlyDismissed(obj)) {
              ...
              obj1[4] = channel;
              obj1[5] = guild;
              obj1[6] = tmp(10115).generateInAppNotificationId();
              obj[0] = obj1;
...
const inAppNotificationStore = new InAppNotificationStore(dispatcherDefault, {
          ...
          if (tmpResult.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!uiStore.getChatOpen(channel_id)) {
                const result = tmp(10106).shouldIncludeSelectedChannel();
                const tmpResult1 = tmp(10106);
                if (tmpResult2.shouldNotify(message, channel_id, result)) {
                  const channel = store2.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4703).createMessageRecord(message);
                    const tmpResult3 = tmp(4703);
                    if (tmpResult4.isMessageContentPreviewable(messageRecord)) {
                      ...
                      obj[0] = MESSAGE;
                      const notificationDuration = tmp(10115).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult5 = tmp(10115);
                      obj[8] = tmp(10115).generateInAppNotificationId();
                    ...
                    tmpResult4 = tmp(10114);
                  }
                  obj7 = store2;
                } else {
                  return false;
                }
                tmpResult2 = tmp(10106);
              }
            }
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      const result = channelId(10115).extractMetadataFromNotification(_null);
      ({ guildId, channelId: channelId2, messageId: messageId2 } = result);
      const obj2 = channelId(10115);
      ...
      messageId(4666).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj3 = messageId(4666);
    }
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            ...
            if (null != channelId) {
              if (null != guildId) {
                const ReactionNotifications = tmp19(4135).ReactionNotifications;
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!uiStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10106).shouldIncludeSelectedChannel();
                        const tmp19Result1 = tmp19(10106);
                        ...
                        if (tmp19Result2.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4703).createMessageRecord(message);
                          const tmp19Result3 = tmp19(4703);
                          if (tmp19Result4.isMessageContentPreviewable(messageRecord)) {
                            ...
                            obj1[0] = REACTION;
                            obj1[1] = channelId;
                            obj1[2] = tmp19(10115).getNotificationDuration(REACTION);
                            obj1[3] = function onDismiss() {
                              callback(table[27]).clearNotification();
                            };
                            obj1[4] = channel;
                            ...
                            obj1[9] = found;
                            const tmp19Result5 = tmp19(10115);
                            obj1[10] = tmp19(10115).generateInAppNotificationId();
                            obj[0] = obj1;
                            handleEnqueueNotification(obj);
                          } else {
                            return false;
                          }
                          tmp19Result4 = tmp19(10114);
                        } else {
                          return false;
                        }
                        tmp19Result2 = tmp19(10106);
                      }
                    }
                    return false;
                  }
            ...
            if (tmpResult.shouldNotifyForForumThreadCreation(channel, channel, !tmpResult.isChannelFocused())) {
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  obj[4] = guild;
                  obj[5] = channel.id;
                  obj[6] = tmp(10115).getNotificationDuration(FORUM_THREAD_CREATED);
                  obj[7] = function onDismiss() {
                    callback(table[27]).clearNotification();
                  };
                  const tmpResult1 = tmp(10115);
                  obj[8] = tmp(10115).generateInAppNotificationId();
                  obj[0] = obj;
                  handleEnqueueNotification(obj);
                }
  ...
  CHANNEL_RTC_UPDATE_CHAT_OPEN: function handleVoiceChatOpen(channelId) {
    ...
    if (tmp) {
      ...
      obj[3] = messageId;
      obj[5] = _null.inAppNotificationId;
      chatOpen(4666).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj2 = chatOpen(4666);
    }
    obj1.removeAll(function predicate(type) {
      let tmp = type.type === closure_1_18.MESSAGE;
      if (tmp) {
    ...
    if (c24) {
      ...
      if (obj.getIsFamilyCenterV3Enabled({ location: "InAppNotificationStore" })) {
        ...
        if (null != diffSchedulesResult) {
          const EnableScreenDowntimeScheduleNotifications = tmp(4135).EnableScreenDowntimeScheduleNotifications;
          if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
            if (tmpResult1.allowInAppNotifications()) {
              ...
              const notificationDuration = tmp(10115).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
              const tmpResult2 = tmp(10115);
              obj[1] = tmp(10103).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
              obj[2] = notificationDuration;
              obj[3] = function onDismiss() {
                callback(table[27]).clearNotification();
              };
              const tmpResult3 = tmp(10103);
              obj[4] = tmp(10103).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
              const tmpResult4 = tmp(10103);
              obj[5] = tmp(10103).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
              const tmpResult5 = tmp(10103);
              obj[6] = tmp(10115).generateInAppNotificationId();
              obj[0] = obj;
              handleEnqueueNotification(obj);
              const tmpResult6 = tmp(10115);
            }
            tmpResult1 = tmp(10106);
          }
        }
      } else {
        return false;
