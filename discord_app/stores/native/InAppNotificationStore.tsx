...
import playInAppMessageSound from "playInAppMessageSound" /* 10346 */;
      ...
      if (null != message.reactions) {
        if (null != emoji) {
          if (obj14.allowInAppNotifications()) {
            let tryParseChannelPathResult = emoji(4876).tryParseChannelPath(tmp);
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = emoji(10329).shouldIncludeSelectedChannel();
                        const tmp19Result9 = emoji(10329);
                        const obj = { message, channel, reactor: user, includeSelectedChannel: result1 };
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          const messageRecord = emoji(4945).createMessageRecord(message);
                          const tmp19Result11 = emoji(4945);
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: emoji(10338).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = emoji(10338);
                            obj3.inAppNotificationId = emoji(10338).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                            const tmp19Result14 = emoji(10338);
                          } else {
                            return false;
                          }
                          tmp19Result12 = emoji(10337);
                        } else {
                          return false;
                        }
                        tmp19Result10 = emoji(10329);
                      }
                    }
                    return false;
                  }
