    ...
    const item = set.forEach((userId) => {
      if (null == outer1_7.getVoiceStateForChannel(channelId, userId)) {
        ...
        if (null != user) {
          ...
          obj[2] = user.id;
          obj[4] = tmp23;
          obj[10] = guildId(4445).getName(guildId, tmp, user);
          const obj2 = guildId(4445);
          const tmp24 = outer1_8;
          obj[11] = channelId(8678).getAvatarDecoration(user, guildId);
          const result1 = outer1_13.set(user.id, obj);
          const obj3 = channelId(8678);
        }
      } else {
        outer1_14.delete(userId);
      }
...
const rTCConnectionDesyncStore = new RTCConnectionDesyncStore(require("dispatcher"), {
  ...
  RTC_CONNECTION_STATE: function handleRTCConnectionState(arg0) {
    ...
    let state;
    ({ state, context } = arg0);
    let tmp = context === require(4352) /* BaseConnectionEvent */.MediaEngineContextTypes.DEFAULT;
    if (tmp) {
  ...
  RTC_CONNECTION_CLIENT_CONNECT: function handleRTCConnectionClientConnect(context) {
    ...
    let reduced = context.context === require(4352) /* BaseConnectionEvent */.MediaEngineContextTypes.DEFAULT;
    if (reduced) {
  ...
  RTC_CONNECTION_CLIENT_DISCONNECT: function handleRTCConnectionClientDisconnect(userId) {
    userId = userId.userId;
    let tmp = userId.context === require(4352) /* BaseConnectionEvent */.MediaEngineContextTypes.DEFAULT;
    if (tmp) {
