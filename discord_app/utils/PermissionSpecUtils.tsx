// === Module 16374: getPermissionOptions ===
...
import isGuildEligibleForStageChannels from "isGuildEligibleForStageChannels" /* 16375 */;
import experiment from "experiment" /* 16376 */;
import getGuildPermissionSpecAll from "getGuildPermissionSpec" /* 16378 */;
...
export default {
  generateChannelPermissionSpec(guild_id, closure_5, arg2) {
    ...
    const VoiceInThreadsExperiment = set(6084).VoiceInThreadsExperiment;
    ...
    if (constants2.GUILD_CATEGORY === type) {
      ...
      if (tmp.showStageChannelPermissions) {
        ...
        const items1 = [obj33(16378).generateChannelStageSection(tmp3, intl28.string(tmp4(1236).t.yniauk))];
        let items2 = items1;
        const obj33Result = obj33(16378);
      } else {
        items2 = [];
      }
      ...
      if (tmp16.GUILD_VOICE === type) {
        ...
        items4[6] = tmp27(16378).generateChannelAppsSection(tmp3, intl15.string(tmp4(1236).t["rrh/W6"]), { showActivities: false });
        items5 = items4;
        const tmp27Result = tmp27(16378);
      } else {
        if (tmp16.GUILD_FORUM !== type) {
          if (tmp16.GUILD_MEDIA !== type) {
            ...
            if (enabled) {
              ...
              const items6 = [tmp38(16378).generateChannelVoiceSection(tmp3, intl.string(tmp4(1236).t["46Ra1b"]))];
              let items7 = items6;
              const tmp38Result = tmp38(16378);
            } else {
              items7 = [];
            }
