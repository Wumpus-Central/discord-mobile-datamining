...
import deserializer from "fromString" /* 506 */;
...
function applyOverwrites(id, member, closure_23, overwrites) {
  ...
  if (null != overwrites[id]) {
    const removeResult = deserializer.remove(closure_23, tmp.deny);
    addResult = deserializer.add(removeResult, tmp.allow);
  }
  ...
  if (null != member) {
    ...
    if (0 < member.roles.length) {
      do {
        ...
        if (null != tmp7) {
          let obj3 = deserializer;
          addResult1 = obj3.add(tmp16, tmp7.allow);
          let obj4 = deserializer;
    ...
    const removeResult1 = deserializer.remove(addResult, tmp17);
    const addResult3 = deserializer.add(removeResult1, tmp18);
    let addResult4 = addResult3;
    if (null != overwrites[member.userId]) {
      const tmp19Result = deserializer;
    ...
    const hasItem = deserializer.has(addResult4, Permissions.ADMINISTRATOR);
    const tmp19Result1 = deserializer;
    ...
    if (tmp31) {
      found = deserializer.filter(addResult4, closure_29);
      const tmp19Result2 = deserializer;
    }
    const tmp28Result = isCommunicationDisabled;
    found1 = found;
    if (tmp34) {
      found1 = deserializer.filter(found, closure_28);
      const tmp19Result3 = deserializer;
    }
    tmp34 = isCommunicationDisabled.isMemberCommunicationDisabled(member) && !hasItem;
  }
  return found1;
}
function computePermissionsForMember(excludeGuildPermissions) {
  ...
  if (flag) {
    ...
    if (checkElevated) {
      ...
      if (!mfaEnabled) {
        removeResult = deserializer.remove(found1, closure_19);
        const tmp21Result1 = deserializer;
      }
      tmp39 = removeResult;
    }
    return tmp39;
    ...
    if (null == tmp4) {
      ...
      if (id !== id1) {
        if (callback2(tmp4, id)) {
          ...
          tmp21 = closure_24;
          if (flag2) {
            const currentUser2 = authStore2.getCurrentUser();
            let mfaEnabled;
            ...
            if (!mfaEnabled) {
              removeResult = deserializer.remove(closure_24, closure_19);
            }
            tmp21 = removeResult;
          }
        }
...
function applyThreadPermissions(context, permissions, hasJoinedResult, closure_14) {
  if (context.type === constants2.MEDIA_THREAD) {
    combineResult = deserializer.combine(Permissions.READ_MESSAGE_HISTORY, Permissions.VIEW_CHANNEL);
  } else {
    ...
    if (context.isLockedThread()) {
      ...
      if (!tmp8Result.has(permissions, SEND_MESSAGES.MANAGE_THREADS)) {
        let removeResult1 = tmp8(506).remove(permissions, SEND_MESSAGES.SEND_MESSAGES);
        ...
        if (guild_id === channel2.guild_id) {
          ...
          if (tmp10) {
            ...
            tmp10 = !keys.some((item, index) => {
              ...
              if (!tmp3) {
                tmp3 = !deserializer.equals(tmp2.allow, tmp.allow);
              }
              return tmp3;
            });
          }
...
export const getHighestRole = function getHighestRole(id) {
  ...
  if (null != member) {
    ...
    return sortedRoles.find((item, index) => {
...
export const canManageACategory = function canManageACategory(currentUser, guild, _categories) {
  ...
  let someResult = deserializer.has(computePermissions({ user, context, overwrites, roles, checkElevated: true, excludeGuildPermissions }), permission);
  if (!someResult) {
    someResult = _categories.some((item, index) => {
      const channel = item.channel;
      let hasItem = "null" !== channel.id;
      if (hasItem) {
        let obj = { permission: null, user: null, context: null };
        obj[0] = Permissions.MANAGE_CHANNELS;
        obj[1] = closure_0;
        obj[2] = channel;
        ...
        obj[3] = roles;
        obj[5] = excludeGuildPermissions;
        hasItem = deserializer.has(computePermissions(obj), permission);
      }
      return hasItem;
    });
  }
...
export const can = function can(arg0) {
  ...
  return deserializer.has(computePermissions({ user, context, overwrites, roles, checkElevated: true, excludeGuildPermissions }), permission);
};
...
export const canEveryoneRole = function canEveryoneRole(VIEW_CHANNEL, channel) {
  ...
  if (null == tmp) {
    ...
    if (null != tmp17) {
      const removeResult = deserializer.remove(permissions, tmp17.deny);
      addResult = deserializer.add(removeResult, tmp17.allow);
    }
    return deserializer.has(addResult, VIEW_CHANNEL);
  }
};
export const canEveryone = function canEveryone(VIEW_CHANNEL, channel) {
  ...
  if (null == tmp) {
    ...
    if (tmp10) {
      tmp10 = !importDefaultResult.some({}, (deny) => deserializer.has(deny.deny, closure_0));
    }
    return tmp10;
  }
};