// === Module 4147: getGuildTierFromGuild ===
  ...
  if (null != guild) {
    ...
    if (features.has(closure_11.MORE_STICKERS)) {
      if (premiumTier === BoostedGuildTiers.TIER_3) {
        let MAX_STICKER_SLOTS = require(4149) /* PremiumGuildOverrides */.PremiumGuildOverrides.MAX_STICKER_SLOTS;
      }
...
export const getAppliedGuildBoostMonths = function getAppliedGuildBoostMonths(arg0) {
  let num = importDefault(3837)().diff(importDefault(3837)(arg0), "months");
  if (num == null) {
    num = 1;
  }
  return num;
...
export const generateBlockGuildSubscriptionPurchasesNode = function generateBlockGuildSubscriptionPurchasesNode(fractionalState) {
  ...
  if (!tmp3) {
    ...
    const obj2 = require(4151) /* _fetchAppliedGuildBoostsForGuild */;
  }
...
export const getNumberOfAppliedBoostsNeededForTier = function getNumberOfAppliedBoostsNeededForTier(id) {
  return Math.max(0, AppliedGuildBoostsRequiredForBoostedGuildTier[arg1] - require(4162) /* useGuildAppliedBoostCount */.getGuildPowerupsBoostCount(id.id).available);
};
