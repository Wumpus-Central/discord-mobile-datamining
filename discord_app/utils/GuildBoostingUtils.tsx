    ...
    if (null != premiumTypeSubscription) {
      if (reduced.numAvailableGuildBoostSlots <= 0) {
        if (premiumTypeSubscription.status === constants2.PAST_DUE) {
          ...
          const numPremiumGuildSubscriptions = PremiumUtilsAll.getNumPremiumGuildSubscriptions(
...
export const getNumberOfAppliedBoostsNeededForTier = function getNumberOfAppliedBoostsNeededForTier(id, arg1) {
  return Math.max(
    ...
    AppliedGuildBoostsRequiredForBoostedGuildTier[arg1] -
      useGuildPowerupsBoostCount.getGuildPowerupsBoostCount(id.id).available,
  );
};
