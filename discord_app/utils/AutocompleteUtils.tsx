...
import RegexUtilsDefault from "RegexUtils" /* 4744 */;
...
import useGuildOnboardingAvailable from "useGuildOnboardingAvailable" /* 7580 */;
import compareChannelsByScoreAndPositionDefault from "compareChannelsByScoreAndPosition" /* 7581 */;
...
function getMatchValue(toLocaleLowerCaseResult1, nextResult) {
    ...
    do {
      ...
      if (null != tmp9) {
        if (null == filter) {
          ...
          let tmp18Result = _slicedToArray(items3, 2);
                  ...
                  if (substr1 !== toLocaleLowerCaseResult) {
                    ...
                    if (substr2 !== toLocaleLowerCaseResult) {
                      ...
                      if (substr3 !== toLocaleLowerCaseResult) {
                        if (str5.substring(0, normalizeResult.length) !== normalizeResult) {
                          ...
                          if (substr4 !== normalizeResult) {
                            ...
                            if (substr5 !== normalizeResult) {
                              ...
                              sum = num;
                              if (tmp33) {
                                let obj = { type: AutocompleterResultTypes.USER, record: tmp9, score: null, comparator: null, sortable: null };
                                let num3;
                                if (boosters != null) {
                        ...
                        let obj3 = { type: AutocompleterResultTypes.USER, record: tmp9, score: null, comparator: null, sortable: null };
                        let num4;
                        if (boosters != null) {
          ...
          let obj5 = { type: AutocompleterResultTypes.USER, record: tmp9, score: null, comparator: null, sortable: null };
          let num5 = c46;
          let num6;
          if (boosters != null) {
...
function getPriorityForStickerMetadataType(type) {
  if (StickersTypes.StickerMetadataTypes.STICKER_NAME === type) {
    return 11;
  } else if (StickersTypes.StickerMetadataTypes.CORRELATED_EMOJI === type) {
    return 6;
  } else if (StickersTypes.StickerMetadataTypes.TAG === type) {
    return 1;
  } else {
    if (StickersTypes.StickerMetadataTypes.GUILD_NAME !== type) {
      if (StickersTypes.StickerMetadataTypes.PACK_NAME !== type) {
        return 1;
      }
    }
    return 8;
...
const InAppNavigationRecord = fn(5662);
...
const re48 = tmp7;
let closure_49 = [];
const MENTION_EVERYONE = fn(5735).default.MENTION_EVERYONE;
const MENTION_HERE = fn(5735).default.MENTION_HERE;
const MENTION_GAME = fn(5735).default.MENTION_GAME;
const MENTION_TIMESTAMP = fn(5735).default.MENTION_TIMESTAMP;
const LAUNCHABLE_APPLICATIONS = fn(5735).default.LAUNCHABLE_APPLICATIONS;
...
export default {
  ...
  queryChannels(guildId) {
    ...
    (function getSeparatedQueries(query) {
      ...
      const mapped = found.map((toLocaleLowerCase) => {
        ...
        const regExp = new RegExp("^" + closure_1_1(4744).escape(toLocaleLowerCaseResult), "i");
        obj.exactQuery = regExp;
        const obj2 = closure_1_1(4744);
        const regExp1 = new RegExp(closure_1_1(4744).escape(toLocaleLowerCaseResult), "i");
      ...
      if (query.includes(" ")) {
        ...
        let obj2 = closure_1(4744);
        const _HermesInternal = HermesInternal;
        let regExp = new RegExp("^" + closure_1(4744).escape(toLocaleLowerCaseResult).replace(" ", "( |-)"), "i");
        obj.exactQuery = regExp;
        const _RegExp2 = RegExp;
        const str = closure_1(4744).escape(toLocaleLowerCaseResult);
        const obj3 = closure_1(4744);
        let regExp1 = new RegExp(closure_1(4744).escape(toLocaleLowerCaseResult).replace(" ", "( |-)"), "i");
        obj.containQuery = regExp1;
        mapped.unshift(obj);
        const str6 = closure_1(4744).escape(toLocaleLowerCaseResult);
      }
      return mapped;
    })(guildId.query, flag2);
    ...
    const tmp10Result = _modDef12(GuildChannelStore.getChannels(guildId)[type]);
  },
  queryGuilds(fuzzy) {
    ({ query, limit } = fuzzy);
    if (limit === undefined) {
      ...
      if (filter(nextResult)) {
        ...
        if (allowSnowflake) {
          if (query === tmp5.id) {
            let tmp11 = c46;
            if (tmp11 > 0) {
              let obj2 = { type: AutocompleterResultTypes.GUILD, record: null, score: null, comparator: null, sortable: null };
              obj2.record = tmp5;
        ...
        if (filter(tmp6)) {
          ...
          for (const item10107 of recipients) {
            ...
            if (null != user) {
              ...
              let nickname = RelationshipStore.getNickname(item10107);
              if (null != username) {
          ...
          if (tmp17 > 0) {
            let obj3 = { type: AutocompleterResultTypes.GROUP_DM, record: null, score: null, comparator: null, sortable: null };
            obj3.record = tmp6;
  ...
  queryApplications(fuzzy) {
    ...
    while (iter !== undefined) {
      ...
      if (filter(application)) {
        ...
        if (tmp10 > 0) {
          let obj4 = { type: AutocompleterResultTypes.APPLICATION, record: null, score: null, comparator: null, sortable: null };
          obj4.record = tmp5;
          obj4.score = tmp11;
    ...
    for (const key10167 in obj4) {
      ...
      if (null == tmp21) {
        ...
        for (const item10171 of tmp21) {
          ...
          if (tmp9 > 0) {
            let obj5 = { type: AutocompleterResultTypes.IN_APP_NAVIGATION, record: hasOwnProperty.fromType(tmp20), score: null, comparator: null, sortable: null };
    ...
    while (iter2 !== undefined) {
      ...
      if (nextResult.type === constants4.DURABLE_PRIMARY) {
        if (filter(tmp7)) {
          ...
          if (tmp14 > 0) {
            let obj4 = { type: AutocompleterResultTypes.SKU, record: null, score: null, comparator: null, sortable: null };
            obj4.record = tmp7;
            obj4.score = tmp15;
    ...
    if (users.length < closure_42) {
      ...
      if (flag2) {
        ...
        if (null != guild) {
          ...
          const item = found.take(closure_42 - length).forEach((item) => {
            roles.push(item);
          });
  ...
  queryStickers(items, arg1) {
    ...
    items1 = [];
    closure_5 = items1;
    const FrecencyUserSettingsActionCreators = flag(2025).FrecencyUserSettingsActionCreators;
    const ifNecessary = FrecencyUserSettingsActionCreators.loadIfNecessary();
    function _loop() {
      if ("" === closure_6) {
        return 1;
        ...
        const stripDiacriticsResult = toLocaleLowerCaseResult(2010).stripDiacritics(toLocaleLowerCaseResult);
        const _RegExp = RegExp;
        let obj2 = toLocaleLowerCaseResult(2010);
        const _HermesInternal = HermesInternal;
        const regExp = new RegExp("^" + nextResult(4744).escape(stripDiacriticsResult), "i");
        const _RegExp2 = RegExp;
        const obj3 = nextResult(4744);
        const _HermesInternal2 = HermesInternal;
        const regExp1 = new RegExp("" + nextResult(4744).escape(stripDiacriticsResult), "i");
        ...
        let item = stickerMetadataArrays.forEach((arr) => {
          const item = arr.forEach((item, index) => {
            ...
            if (null != stickerById) {
              if (regExp1(stickerById, obj2.getStickerSendability(stickerById, dependencyMap, regExp))) {
                ...
                while (iter !== undefined) {
                  ...
                  if (toLocaleLowerCaseResult) {
                    if (value === closure_1_0) {
                      ...
                      let tmp18 = tmp7 !== flag(5487).StickerMetadataTypes.GUILD_NAME;
                      if (tmp18) {
                        tmp18 = tmp7 !== flag(5487).StickerMetadataTypes.PACK_NAME;
                      }
                      if (tmp18) {
                        tmp18 = tmp7 !== flag(5487).StickerMetadataTypes.STICKER_NAME;
                      }
                      if (!tmp18) {
  ...
  querySoundmoji(arg0, channel) {
    ...
    if (!isFetchingResult) {
      tmp3 = !SoundboardStore.hasFetchedAllSounds();
    }
    if (tmp3) {
