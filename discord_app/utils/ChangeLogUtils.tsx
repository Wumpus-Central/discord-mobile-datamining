...
import _modDef4947 from "../modules/markup/MarkupSubtextRule.tsx";

const require = arg1;
function defaultRules(uri) {
  let obj = {};
  const merged = Object.assign(require("native/ChangeLogUtils.tsx").baseRules);
  if (null != require("native/ChangeLogUtils.tsx").customRules.strong) {
    obj = {};
    const merged1 = Object.assign(tmp(7971).baseRules.strong);
    if (typeof tmp(7971).customRules.strong === "function") {
      const customRules = tmp(7971).customRules;
      let strong = customRules.strong(uri);
    } else {
      strong = tmp(7971).customRules.strong;
    }
    obj = { strong: null };
    ...
    if (typeof tmp(7971).customRules.image === "function") {
      const customRules2 = tmp(7971).customRules;
      image = customRules2.image(uri);
    } else {
      image = tmp(7971).customRules.image;
    }
    const merged5 = Object.assign(image);
    obj.image = obj2;
    const obj3 = {};
    const merged6 = Object.assign(link);
    if (typeof tmp(7971).customRules.link === "function") {
      const customRules3 = tmp(7971).customRules;
      link = customRules3.link(uri);
    } else {
      link = tmp(7971).customRules.link;
    }
    const merged7 = Object.assign(link);
    obj.link = obj3;
    const obj4 = {};
    const merged8 = Object.assign(list);
    if (typeof tmp(7971).customRules.list === "function") {
      const customRules4 = tmp(7971).customRules;
      list = customRules4.list(uri);
    } else {
      list = tmp(7971).customRules.list;
    }
    ...
    if (typeof tmp(7971).customRules.lheading === "function") {
      const customRules5 = tmp(7971).customRules;
      lheading = customRules5.lheading(uri);
    } else {
      lheading = tmp(7971).customRules.lheading;
    }
    const merged11 = Object.assign(lheading);
    obj.lheading = obj6;
    const obj7 = {};
    const merged12 = Object.assign(heading);
    if (typeof tmp(7971).customRules.heading === "function") {
      const customRules6 = tmp(7971).customRules;
      heading = customRules6.heading(uri);
    } else {
      heading = tmp(7971).customRules.heading;
    }
    const merged13 = Object.assign(heading);
    obj.heading = obj7;
    const obj8 = {};
    const merged14 = Object.assign(blockQuote);
    if (typeof tmp(7971).customRules.blockQuote === "function") {
      const customRules7 = tmp(7971).customRules;
      blockQuote = customRules7.blockQuote(uri);
    } else {
      blockQuote = tmp(7971).customRules.blockQuote;
    }
    const merged15 = Object.assign(blockQuote);
    obj.blockQuote = obj8;
    const obj9 = {};
    const merged16 = Object.assign(paragraph);
    if (typeof tmp(7971).customRules.paragraph === "function") {
      const customRules8 = tmp(7971).customRules;
      paragraph = customRules8.paragraph(uri);
    } else {
      paragraph = tmp(7971).customRules.paragraph;
    }
...
export default {
  ...
  getSpecialRules(uri) {
    ...
    if (typeof require("native/ChangeLogUtils.tsx").customRules.lheading === "function") {
      const customRules = tmp3(7971).customRules;
      lheading = customRules.lheading(uri);
    } else {
      lheading = tmp3(7971).customRules.lheading;
    }
    obj = { lheading: null };
