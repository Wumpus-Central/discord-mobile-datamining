// === Module 7986: SystemMessageUtils ===
    ...
    if (null == first) {
      ...
      if (constants.RECIPIENT_ADD === type) {
        if (null != null) {
          ...
          let tmp6Result = NicknameUtilsDefault;
        ...
        if (null != null) {
          ...
          if (null != author) {
            if (author.id !== null.id) {
              ...
              obj = { username: name, usernameOnClick, otherUsername: null, otherUsernameOnClick: null };
              tmp6Result = NicknameUtilsDefault;
              obj.otherUsername = tmp6Result.getName(null, channel_id, null);
              obj.otherUsernameOnClick = usernameOnClick;
          ...
          if (constants.GUILD_BOOST_TIER_2 !== type) {
            if (constants.GUILD_BOOST_TIER_3 !== type) {
              if (constants.GUILD_INVITE_REMINDER === type) {
                ...
                const obj8 = { username: name, threadName: useChannelName.computeChannelName(isForumPost, UserStore, RelationshipStore) };
                ...
                if (!(mentions instanceof MessageRecord)) {
                  ...
                  const obj9 = { username: name, guildId: isForumPost.guild_id, roleSubscriptionData: mentions.role_subscription_data };
                  astToStringResult2 = obj34.astToString(GuildRoleSubscriptionSystemMessageUtils.getRoleSubscriptionPurchaseSystemMessageContentMobile(obj9));
                }
                ...
                if (!(mentions instanceof MessageRecord)) {
                  ...
                  if (null != product_name) {
                    ...
                    const obj10 = { username: name, productName: mentions.purchase_notification.guild_product_purchase.product_name };
                    astToStringResult3 = obj31.astToString(GuildProductSystemMessageUtils.getGuildProductPurchaseSystemMessageContentMobile(obj10));
                  }
                }
                ...
                if (mentions instanceof MessageRecord) {
                  ...
                  return obj28.astToString(ApplicationSubscriptionSystemMessageUtils.getApplicationSubscriptionSystemMessageASTContent(obj11));
                }
                  ...
                  if (null != channel) {
                    ...
                    if (null != guild) {
                      if (AutomodNotificationEmbedTypeKeys.AutomodNotificationEmbedTypeKeys.ACTIVITY_ALERTS_ENABLED === value) {
                        let tmp34Result = MarkupParser;
                        const intl5 = util.intl;
                        const obj14 = { guildName: guild.name };
                        astToStringResult6 = tmp34Result.astToString(intl5.formatToParts(util.t.wt3ZUM, obj14));
                      } else if (AutomodNotificationEmbedTypeKeys.AutomodNotificationEmbedTypeKeys.INTERACTION_BLOCKED === value) {
                        tmp34Result = MarkupParser;
                        const intl4 = util.intl;
                        obj15 = { guildName: guild.name };
                        astToStringResult6 = tmp34Result.astToString(intl4.formatToParts(util.t.AkqI0g, obj15));
                      } else {
                        const intl3 = util.intl;
                        obj16 = { guildName: guild.name };
                        astToStringResult6 = MarkupParser.astToString(intl3.formatToParts(util.t["a+lJKl"], obj16));
                        const tmp34Result1 = MarkupParser;
                      }
                    }
                  }
