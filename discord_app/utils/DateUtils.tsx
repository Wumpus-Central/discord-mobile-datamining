...
import makeDateFormatterDefault from "makeDateFormatter" /* 4514 */;
...
function syncHourCycleToIntlConfig() {
  ...
  if (result) {
    ...
    const tmp4Result = SystemDateFormatter;
  }
  const values = Object.values(util.intl.formatConfig.time);
...
export const isWithinInterval = function isWithinInterval(timestamp, timestamp2, arg2) {
  return Math.abs(timestamp.valueOf() - timestamp2.valueOf()) < arg2;
};
export { dateFormat };
export const calendarFormat = function calendarFormat(timestamp, arg1) {
  let flag = arg1;
  if (arg1 === undefined) {
    flag = false;
  }
...
export const accessibilityLabelCalendarFormat = function accessibilityLabelCalendarFormat(timestamp) {
  ...
  let str5 = "LLL";
  if ("sameElse" !== str) {
    str5 = localeDataResult.calendar(str, _modDef4418(timestamp), _modDef4418(date));
    const tmp6 = _modDef4418(timestamp);
  }
