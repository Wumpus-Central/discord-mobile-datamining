...
export default {
  ...
  resetSettings() {
    dispatcher.dispatch({ type: "CREATE_INVITE_MODAL_RESET_SETTINGS" });
  },
  createInvite(arg0, arg1) {
    ...
    if (null != pendingSettings) {
      dispatcher.dispatch({ type: "CREATE_INVITE_MODAL_GENERATE_INVITE" });
      ...
      obj[7] = flags;
      obj[8] = roleIds;
      const invite1 = generateAcceptInviteOptions.createInvite(channelId, obj, arg0);
      invite1.then(() => {
      ...
      const tmp7Result = generateAcceptInviteOptions;
    }
  },
  close() {
    const onClose = store.onClose;
    dispatcher.dispatch({ type: "CREATE_INVITE_MODAL_CLOSE" });
    if (onClose != null) {
      onClose();
    }
  }
