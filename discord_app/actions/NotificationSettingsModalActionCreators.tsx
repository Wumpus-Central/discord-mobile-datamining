...
fn(4256).NotificationSettingsUpdateType;
...
export default {
  ...
  updateGuildAndChannelNotificationSettings(guildId, channel_overrides, OptedIn, location) {
    ...
    const item = keys1.forEach((channelId) => {
      ...
      const obj = { guildId, channelId, change: channel_overrides.channel_overrides[channelId], previous: value, label, location: _location };
  ...
  updateChannelOverrideSettingsBulk(guildId, channel_overrides, OptedOut, _location) {
    ...
    const item = keys1.forEach((channelId) => {
      const obj = { guildId, channelId, change: closure_1[channelId], previous: closure_4.get(channelId), label, location: _location };
  ...
  updateAppDMOverrideSettings(guildId, id, id2, settings, NotificationLabel2) {
    ...
    const intl = util.intl;
    AccessibilityAnnouncer.announce(intl.string(util.t.MlIsJ8));
    obj1 = { updateType: constants.AUTHORIZED_APP_DM, guildId, channelId: id, applicationId: id2, change: settings, previous: currentChannelSettings, label: NotificationLabel2 };
    const result1 = NotificationSettingsUtils.trackChannelNotificationSettingsUpdate(obj1);
  },
  setForumThreadsCreated(channel, arg1) {
    ...
    const obj = { flags: UserGuildSettingsStore.getChannelFlags(channel) & ~(arg1 ? tmp2.NEW_FORUM_THREADS_OFF : tmp2.NEW_FORUM_THREADS_ON) | NEW_FORUM_THREADS_OFF };
