...
const result = require("obj132").fileFinishedImporting("actions/UserActionCreators.tsx");
...
export const fetchCurrentUser = function fetchCurrentUser() {
  ...
  return value.then((result) => {
    const obj = { type: "CURRENT_USER_UPDATE", user: result.body, analyticsToken: null };
    let analytics_token;
    if (closure_0) {
      analytics_token = result.body.analytics_token;
    }
    obj[2] = analytics_token;
    obj.dispatch(obj);
    return new closure_1_4(result.body);
  });
};
export const acceptAgreements = function acceptAgreements() {
  ...
  const obj = { event: ImpressionNames.NetworkActionNames.USER_ACCEPT_AGREEMENTS };
  obj[1] = obj;
  obj[2] = { terms: flag, privacy: flag2 };
  obj[4] = sendRequest.rejectWithMigratedError();
  return obj.patch(obj).then((result) => true, () => false);
};
...
export const getUser = function getUser(arg0) {
  ...
  if (null != user) {
    ...
    resolved = value.then((result) => {
      const obj = { type: "USER_UPDATE", user: result.body };
      obj.dispatch(obj);
...
export const insertStaticUser = function insertStaticUser(id) {
  const obj = { type: "USER_UPDATE", user: id };
  obj.dispatch(obj);
