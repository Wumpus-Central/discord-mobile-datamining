...
export default function SoundboardSoundPreviewActionSheet(channel) {
  ...
  const soundboardSoundPreviewMenuEnabled = channel(soundGridLocation[11]).useSoundboardSoundPreviewMenuEnabled(
  ...
  stateFromStores1 = channel(soundGridLocation[14]).useStateFromStores(items3, () =>
    SoundboardStore.isFavoriteSound(sound.soundId),
  );
  const obj5 = channel(soundGridLocation[14]);
  const items4 = [stateFromStores1];
  const items5 = [sound];
  const stateFromStores2 = channel(soundGridLocation[14]).useStateFromStores(
    items4,
    () => SoundboardStore.isPlayingSound(sound.soundId),
    items5,
  );
  const obj6 = channel(soundGridLocation[14]);
  const items6 = [stateFromStores1];
  const items7 = [stateFromStores];
  let stateFromStores3 = channel(soundGridLocation[14]).useStateFromStores(
    items6,
    () => {
  ...
  obj16.name = str2;
  const items14 = [
    closure_12(sound(stringResult1[25]), obj16),
