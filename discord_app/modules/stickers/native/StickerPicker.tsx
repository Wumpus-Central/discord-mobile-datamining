...
const memoResult = importAllResult.memo(function StickerPicker(channel) {
  ...
  dependencyMap = tmp7;
  analyticsLocations = onPressSticker(5900)(onPressSticker(5920).STICKER_PICKER).analyticsLocations;
  const tmp9 = onPressSticker(5900);
  [tmp11, c4] = analyticsLocations(undefined.useState(null), 2);
  const tmp10 = analyticsLocations(undefined.useState(null), 2);
  ({ safeAreaStyle, safeAreaBottomKeyboardAware } = onPressSticker(10148)({ hasCategories: true }));
  const items1 = [, ];
  ...
  callback = importAllResult.useCallback((arg0) => {
    if ("" === arg0) {
      ...
      _undefined(channel(10180).searchAllStickers(arg0));
  ...
  const callback1 = importAllResult.useCallback((pack_id) => {
    let obj = channel(6127);
    const stickerSendability = obj.getStickerSendability(pack_id, closure_1_7.getCurrentUser(), channel);
    if (stickerSendability === channel(6127).StickerSendability.SENDABLE) {
      onPressSticker(pack_id);
    } else if (stickerSendability === tmp(6127).StickerSendability.SENDABLE_WITH_PREMIUM) {
      ...
      if (tmpResult.isStandardSticker(pack_id)) {
        ...
        if (null != stickerPack) {
          ...
          obj[0] = DM_CHANNEL2;
          obj[0] = obj;
          obj[1] = tmp(10181).AnalyticsPopoutType.STICKER_PACK_DETAIL;
          obj[2] = stickerPack;
          onPressSticker(10181)(obj);
          const tmp17 = onPressSticker(10181);
        }
      } else {
        tmpResult = tmp(4804);
        if (tmpResult.isGuildSticker(pack_id)) {
          let obj2 = onPressSticker(8585);
  ...
  if (0 !== stickerCategories.length) {
    ...
    obj2[2] = callback;
    obj2[3] = callback2;
    obj1[1] = tmp18(tmp2(6167).SearchField, obj2);
  ...
  if (stateFromStores) {
    if (0 === stickerCategories.length) {
      ...
      tmp18Result = tmp18(tmp8(10201), obj4);
      const tmp8Result = tmp8(10201);
    }
  } else {
    ...
    items5[2] = tmp18(tmp8(10206), obj6);
    obj[1] = items5;
    obj[1] = closure_15(tmp20, obj);
    return tmp18(channel(5900).AnalyticsLocationProvider, obj);
  }
});
