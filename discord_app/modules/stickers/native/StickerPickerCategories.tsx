...
export default function _default(categories) {
  ...
  const effect = first.useEffect(() => {
    const findIndexResult = categories.findIndex(
      (type) => type.type === categories(closure_1_2[12]).StickerCategoryTypes.PACK,
    );
  ...
  if (tmp22Result) {
    ...
    tmp22Result = closure_14(categories(5341).PressableOpacity, obj3);
  }
  items9[1] = tmp22Result;
  obj.children = items9;
  return closure_15(categoryIndex(10624), obj);
}
