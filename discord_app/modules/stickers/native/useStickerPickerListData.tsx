...
let closure_10 = fn(10646).PREMIUM_UPSELL_SECTION_DIVIDER_HEIGHT + fn(10646).PREMIUM_UPSELL_SECTION_DIVIDER_MARGIN;
const size = fn(2);
  ...
  return stickerCategories.useMemo(() => {
    ...
    const mapped1 = items2.map((item, index) => {
      ...
      if (item === flag) {
        ...
        const PremiumUpsellSectionDividerPosition = containerWidth(
          stickerFormats[4],
        ).PremiumUpsellSectionDividerPosition;
      }
    });
    num2 = 0;
    if (null != START) {
    ...
    return {
      sections: items,
      sectionHeights: items4,
      sectionSize,
      sectionFooterSize: 12,
      sectionFooterSizes: mapped2,
      sectionDividerPositions: mapped1,
      listHeaderDividerPosition: START,
      listHeaderSize: num2,
      sectionLabels: items1,
      sectionNitroLocked: items2,
      rowHeight,
      rowSize: rounded,
