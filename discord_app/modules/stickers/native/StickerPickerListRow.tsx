...
function StickerItemLockedOverlay() {
  ...
  const obj = {
    ...
    style: tmp.lockContainer,
    children: React6(LockIcon.LockIcon, { style: tmp.lock }),
  };
...
export default function StickerPickerListRow(stickers) {
  ...
  ({
    ...
    onLongPressStickerDetail: closure_3,
    focusedSticker: UserStore,
    setFocusedSticker: StickerAnimationSettings,
    channel: STICKER_SIZE,
    nativeRow,
  } = stickers);
  ...
  function rowTraits(id) {
    ...
    const obj2 = {
      isAnimated: StickersUtils.shouldAnimateSticker(closure_8, id.id === id),
      isOpaque: isSendableStickerResult,
      isDisabled: null == dependencyMap,
      isLocked: null,
    };
    let tmp10 = closure_9;
    if (closure_9) {
  ...
  closure_9 = stickers(9512).useMobileStickerPickerUpsellRestyleEnabled("native.StickerPickerListRow");
  let items = [];
  if (nativeRow) {
    let num4 = 0;
    if (0 < rowSize) {
      do {
        ...
        if (undefined !== tmp12) {
          ...
          obj4.stickerType = stickers(5571).StickerFormat.PNG;
    ...
    try {
      ...
      return closure_8(isSectionNitroLocked(10743), obj7);
    } catch (tmp26) {
      ...
      obj11.items = found.map((stickerId) => ({
      ...
      do {
        let tmp8 = (function _loop() {
          ...
          if (undefined === stickers[c12]) {
            ...
            let isSendableStickerResult = isSectionNitroLocked;
            const obj5 = stickers(5188);
            if (!isSectionNitroLocked) {
              isSendableStickerResult = null == closure_6;
            }
            if (!isSendableStickerResult) {
              isSendableStickerResult = tmp26(7665).isSendableSticker(tmp2, currentUser.getCurrentUser(), closure_6);
              const tmp26Result = tmp26(7665);
            }
            let tmp11 = closure_9;
            if (closure_9) {
            ...
            obj2.onPress = function onPress() {
              const result = closure_0(4795).triggerHapticFeedback(isSectionNitroLocked(4796).IMPACT_LIGHT);
              if (dependencyMap != null) {
                dependencyMap(closure_0);
              }
              const obj = closure_0(4795);
            };
            ...
            const obj3 = {
              sticker: tmp2,
              size,
              animated: stickers(5188).shouldAnimateSticker(closure_8, tmp2.id === id),
              opaque: isSendableStickerResult,
            };
            const items1 = [closure_8(isSectionNitroLocked(10518), obj3, c12)];
            let tmp17 = null;
            if (tmp11) {
            ...
            items.push(closure_9(stickers(5425).PressableOpacity, obj2, tmp2.id));
            const shouldAnimateStickerResult = stickers(5188).shouldAnimateSticker(closure_8, tmp2.id === id);
