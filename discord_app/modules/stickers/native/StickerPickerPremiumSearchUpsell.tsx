...
export default function StickerPickerPremiumSearchUpsell(guildId) {
  ...
  const premiumUpsellConfig = obj.usePremiumUpsellConfig(
    guildId(analyticsLocations[8]).getUpsellType(
      guildId(analyticsLocations[9]).EntitlementFeatureNames.STICKERS_EVERYWHERE,
    ),
    analyticsLocations,
  );
  ...
  obj3.icon = jsx(guildId(analyticsLocations[15]).NitroWheelIcon, {
    size: "sm",
