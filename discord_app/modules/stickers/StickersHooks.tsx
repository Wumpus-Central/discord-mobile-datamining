// === Module 10401: useStickerPackCategories ===
...
import closure_10 from "handleStickersStoreUpdate" /* 5451 */;
import closure_11 from "loadSavedGuildStickers" /* 5452 */;
...
export const useFetchStickerPack = function useFetchStickerPack(pack_id) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => obj.hasLoadedStickerPacks);
  ...
  return stickerNodeWidth.useMemo(() => {
    ...
    if (0 !== items2) {
      function addGridSection(sendable, SEARCH_RESULTS, arg2) {
        ...
        const findIndexResult = c6.findIndex((type) => type.type === SEARCH_RESULTS(5220).StickerCategoryTypes.FAVORITE);
...
export const useFavoriteStickers = function useFavoriteStickers() {
  ...
  const items = [closure_11];
  const items1 = [stickerIds];
  return stickerIds(586).useStateFromStoresArray(items, () => {
    const mapped = channel.map((arg0) => stickerById.getStickerById(arg0));
    return mapped.filter((body) => {
      let tmp = null != body;
      if (tmp) {
...
export const useLatestFrecentStickers = function useLatestFrecentStickers() {
  ...
  const items = [closure_11];
  const items1 = [keys];
  return keys(586).useStateFromStoresArray(items, () => {
...
export const useStickerForRenderableSticker = function useStickerForRenderableSticker(renderableSticker, arg1) {
  ...
  let isGuildStickerResult = _require(4852).isGuildSticker(renderableSticker);
  if (!isGuildStickerResult) {
