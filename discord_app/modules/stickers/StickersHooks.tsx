...
import StickersPersistedStore from "StickersPersistedStore.tsx";
...
function useStickerPackCategories(channel) {
  ...
  const stateFromStoresObject = require("initialize").useStateFromStoresObject(
    items2,
    () => ({
      packs: StickersStore.getPremiumPacks(),
      frequentlyUsedStickers: StickersPersistedStore.stickerFrecencyWithoutFetchingLatest.frequently,
    }),
    [],
  );
  ...
  memo = memo.useMemo(() => {
    ...
    if (null != guildId) {
      ...
      if (findIndexResult >= 1) {
        ...
        if (tmp15) {
          const obj5 = {
            type: StickersTypes.StickerCategoryTypes.EMPTY_GUILD_UPSELL,
            id: null,
            name: null,
            stickers: null,
          };
  ...
  return memo.useMemo(() => {
    ...
    const obj = {
      type: StickersTypes.StickerCategoryTypes.FAVORITE,
      id: StickersTypes.StickerCategoryTypes.FAVORITE,
      name: null,
      stickers: null,
    };
    ...
    const items = [obj];
    const obj2 = {
      type: StickersTypes.StickerCategoryTypes.RECENT,
      id: StickersTypes.StickerCategoryTypes.RECENT,
      name: null,
      stickers: null,
    };
    ...
    if (0 !== items2) {
      function addGridSection(sendable, SEARCH_RESULTS, arg2) {
        ...
        const findIndexResult = visibleRowIndex.findIndex(
          (type) => type.type === category(5575).StickerCategoryTypes.FAVORITE,
        );
        ...
        if (canCreateExpressions) {
          canCreateExpressions =
            sendable.length < collapsedStickersCategories(tmp2[16]).getTotalStickerCountForTier(guild.premiumTier);
        ...
        for (let num2 = 0; num2 < rounded; num2 = num2 + 1) {
          ...
          let mapped = substr.map((sticker, columnIndex) => {
            const obj = {
              type: StickersTypes.StickerGridItemTypes.STICKER,
              sticker,
              packId: null,
        ...
        findIndexResult1 = visibleRowIndex.findIndex(
          (type) => type.type === category(5575).StickerCategoryTypes.RECENT,
        );
      }
      if (null == items) {
...
export const useFavoriteStickers = function useFavoriteStickers() {
  const favoriteStickers = stickerIds(9821).useFrecencySettings().favoriteStickers;
...
export const useLatestFrecentStickers = function useLatestFrecentStickers() {
  const frecencySettings = keys(9821).useFrecencySettings();
  keys = closure_13;
  let stickers;
  if (frecencySettings != null) {
    const stickerFrecency = frecencySettings.stickerFrecency;
...
export const useStickerForRenderableSticker = function useStickerForRenderableSticker(renderableSticker, arg1) {
  ...
  if (!isGuildStickerResult) {
    isGuildStickerResult = tmp(5191).isStandardSticker(renderableSticker);
    const tmpResult = tmp(5191);
  }
  obj4 = {
    hasFetched: tmp6,
