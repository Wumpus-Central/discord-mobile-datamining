...
prototype["createFromServer"] = function createFromServer(display_name) {
  ...
  if (tmp14 != display_name.reward_config) {
    ...
    if (tmp14 != reward_config) {
      ...
      if (tmp14 != reward_config.discount) {
        let obj = { id: reward_config.discount.id, type: reward_config.discount.type, amount: reward_config.discount.amount, fiatEnabled: reward_config.discount.fiat_enabled, orbsEnabled: reward_config.discount.orbs_enabled };
        tmp18 = obj;
      }
      obj = { discount: tmp18 };
      tmp17 = obj;
  ...
  if (tmp14 != display_name.tenant_metadata) {
    ...
    if (tmp14 == tenant_metadata.collectibles) {
      ...
      if (collectibles.type !== StorefrontCollectiblesTypes.StorefrontPromotionCollectiblesType.COLLECT_AND_CLAIM) {
        if (collectibles.type === obj27(8880).StorefrontPromotionCollectiblesType.TARGETED_OFFER) {
          ...
          if (tmp14 == nagbar) {
            if (tmp14 == offer_notice) {
              const obj1 = { type: obj27(8880).StorefrontPromotionCollectiblesType.TARGETED_OFFER };
              let obj2 = obj1;
            }
          }
          obj2 = { type: obj27(8880).StorefrontPromotionCollectiblesType.TARGETED_OFFER, reward: null };
          let tmp36;
          if (tmp14 != nagbar) {
      ...
      const obj10 = { type: obj27(8880).StorefrontPromotionCollectiblesType.COLLECT_AND_CLAIM, subtype: obj27(8880).StorefrontPromotionCollectAndClaimSubtype.TAKEOVER, collectionId: collectibles.collection_id, shopHome: null, indexPage: null, shared: null };
      const obj11 = { title: collectibles.shop_home.title, description: collectibles.shop_home.description, rewardStates: null, style: null };
      ...
      const obj25 = { hiddenAssetPath: progress_indicator.assets.reward_preview.hidden_asset_path, revealedAssetPath: progress_indicator.assets.reward_preview.revealed_asset_path };
      obj24.rewardPreview = obj25;
      obj23.assets = obj24;
      obj27 = undefined;
      if (tmp14 != progress_indicator.style) {
        const obj26 = { contentTheme: progress_indicator.style.content_theme, backgroundColor: progress_indicator.style.background_color, progressColor: progress_indicator.style.progress_color };
        obj27 = obj26;
      }
  ...
  if (typeof prototype === "function") {
    const tmp49 = new prototype(tmp22, tmp12, tmp11, tmp10, tmp9, tmp8, tmp7, tmp6, tmp5, tmp4, tmp3, tmp2, tmp, _Date2, _Date, prototype, new.target, id);
    tmp49.id = id;
