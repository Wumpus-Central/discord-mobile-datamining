  ...
  const memo = pricesForPurchaseType.useMemo(() => {
    ...
    if (null != sku) {
      if (null != stateFromStores1) {
        ...
        if (tmp12 == null) {
          tmp12 = stateFromStores1[tmp4(undefined, 7564).StorefrontPurchaseType.SELF_PURCHASE];
        }
        let found;
        if (tmp12 != null) {
  ...
  return pricesForPurchaseType.useMemo(() => {
    if (null == sku) {
      ...
      if (sku.productLine === constants4.SOCIAL_LAYER_GAME_ITEM) {
        let price = SlayerStorefrontUtils.getPrice(sku, DEFAULT);
      } else {
        let premiumType;
        if (stateFromStores1 != null) {
          premiumType = stateFromStores1.premiumType;
...
export const isSlayerSkuAvailableOnThisPlatform = function isSlayerSkuAvailableOnThisPlatform(sku) {
  if (null != sku) {
    if (sku.productLine === constants4.SOCIAL_LAYER_GAME_ITEM) {
      ...
      if (obj.isIOS()) {
        let hasFlagResult = FlagUtils.hasFlag(num, constants3.AVAILABLE_ON_IOS);
