...
{ [fn(5439).AccountStandingState.ALL_GOOD]: { left: "0%" } };
let obj = { translateX: -0.5 * require("SafetyHubAccountStandingSubwayMarker").SUBWAY_MARKER_WIDTH };
let items = [obj];
obj[1] = items;
...
const createCacheKey = { position: "relative", justifyContent: "center", alignItems: "center", padding: ThemesDefault.space.PX_4, backgroundColor: ThemesDefault.colors.BACKGROUND_MOD_STRONG, borderRadius: ThemesDefault.radii.round };
...
const result = require("obj132").fileFinishedImporting("modules/safety_hub/native/SafetyHubAccountStanding.tsx");
...
export default function SafetyHubAccountStanding() {
  ...
  let items = [tmp4];
  memo = memo.useMemo(() => {
    { title: accountStanding(1236).t.uaKrRi, description: null, status: null, style: null, CustomIcon: null };
  ...
  const memo1 = memo.useMemo(() => {
    ...
    return entries.map((item, index) => {
      ...
      const items = [closure_3.subwayMarker, closure_1_11[parsed]];
      ...
      obj[1] = closure_1_9(first(closure_1_2[8]), obj, index);
      return closure_1_9(style, obj, index);
    });
  }, items1);
  ...
  obj1[2] = str;
  obj[1] = callback(accountStanding(1297).Avatar, obj1);
  const items4 = [callback(style, obj), ];
  ...
  obj4[3] = intl.format(title, {
    hook(children) {
      return closure_1_9(accountStanding(4734).Text, { style, variant: "heading-lg/bold", children }, arg1);
    }
  });
  const items5 = [callback(accountStanding(4734).Text, obj4), callback(accountStanding(4734).Text, { variant: "text-sm/medium", color: "text-muted", style: { textAlign: "center" }, children: description })];
  obj3[1] = items5;
  const items6 = [callback2(style, obj3), ];
  const obj6 = { style: items7, children: null };
  items7 = [tmp4.health, { height: first }];
  const items8 = [callback(style, { style: tmp4.line }), memo1];
  obj6[1] = items8;
  items6[1] = callback2(style, obj6);
  obj2[1] = items6;
  items4[1] = callback2(style, obj2);
  obj[1] = items4;
  return callback2(style, obj);
};