...
export default function AppealIngestionPolicySummary(classification) {
  ...
  let obj = parseMessageEmbedForProps /* parseMessageEmbedForProps */;
  let description;
  ...
  obj[2] = intl.string(getSystemLocale /* getSystemLocale */.t.xsdcxh);
  const items = [callback(Text /* Text */.Text, obj), ];
  const hexWithOpacityResult = hexToRgba /* hexToRgba */.hexWithOpacity(tmp.borderColor.color, 0.08);
  const items1 = [tmp.userContainer, { borderColor: hexWithOpacityResult }];
  items[1] = callback(View, { style: items1, children: callback(Text /* Text */.Text, { variant: "text-md/semibold", children: capitalizeTextResult }) });
  obj[1] = items;
