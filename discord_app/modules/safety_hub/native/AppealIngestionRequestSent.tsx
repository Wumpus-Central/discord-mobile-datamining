...
export default function AppealIngestionRequestSent() {
  ...
  const items = [callback(closure_4, obj1), callback(_require(11101).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(_require(1236).t.Qdx8AP) }), , , ];
  ...
  items[3] = callback(importDefault(11106), obj3);
  const obj4 = { text: null, url: null, onPress: null };
  const tmp4 = importDefault(11106);
  ...
  items[4] = callback(importDefault(11106), obj4);
  obj[1] = items;
  obj[0] = callback2(closure_3, obj);
  return callback(_require(11101).AppealIngestionModalScreen, obj);
};