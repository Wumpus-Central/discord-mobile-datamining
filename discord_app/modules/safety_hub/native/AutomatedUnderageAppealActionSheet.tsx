...
import { intl, intl as intl2, intl as intl3, intl as intl4, intl as intl5, intl as intl6 } from "../../../intl/index.native.tsx";
...
export default function AutomatedUnderageAppealActionSheet(onClose) {
  ...
  const items5 = [callback(onClose(4440).Text, obj4), ];
  ...
  items5[1] = callback(onClose(4440).Text, obj5);
  obj3[1] = items5;
  const items6 = [callback2(callback, obj3), , , ];
  const obj6 = {
    children: callback(onClose(5939).TableRowGroup, {
      hasIcons: true,
  ...
  obj9[2] = callback(onClose(5546).TableRow, obj10);
  obj8[1] = callback(onClose(5939).TableRowGroup, obj9);
  ...
  const items7 = [callback(onClose(4879).Button, obj12), ];
  ...
  items7[1] = callback(onClose(4440).Text, obj13);
  obj11[1] = items7;
  items6[3] = callback2(callback, obj11);
  obj1[1] = items6;
  obj[1] = callback2(callback, obj1);
  obj[2] = callback(onClose(5571).BottomSheetScrollView, obj);
  return callback(onClose(5569).BottomSheet, obj);
};