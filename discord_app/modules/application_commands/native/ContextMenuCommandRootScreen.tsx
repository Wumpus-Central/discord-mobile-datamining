...
export default function ContextMenuCommandRootScreen(navigation) {
  ...
  let items = [closure_7];
  const stateFromStores = navigation(onPressAppCommand[8]).useStateFromStores(items, () =>
  ...
  const memo = obj2.useMemo(() => {
    if (!loading) {
      if (0 !== commands.length) {
        ...
        const obj = {
          frecencyItems: mapped,
          appItems: found1.map((section) => ({ type: "app", section: section.section })),
        };
        return obj;
      }
    }
