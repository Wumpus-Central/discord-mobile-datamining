...
import ApplicationCommandActionCreators from "../ApplicationCommandActionCreators.tsx";
...
import ChatInputCommandOptionParser from "../../chat_input/native/ChatInputCommandOptionParser.tsx";
import ChatInputParser from "../../chat_input/native/ChatInputParser.tsx";
import ApplicationCommandOptionValueParser from "../../chat_input/native/ApplicationCommandOptionValueParser.tsx";
import DraftCommandUtils from "../DraftCommandUtils.tsx";
import useCommandContext from "../../app_launcher/hooks/useCommandContext.tsx";
import application_commands_ApplicationCommandValidationUtils from "ApplicationCommandValidationUtils.tsx";
  ...
  constructor(arg0) {
    ...
    obj.setAutoCompleteResult = function setAutoCompleteResult(id, addTimestampMentionResult, arg2, type) {
      ...
      if (insertOrJumpCommandOption.type !== constants2.SLASH) {
        ...
        ({
          command: obj2.command,
          section: obj2.section,
          location: obj2.location,
          visualSection: obj2.visualSection,
          query: obj2.query,
    ...
    obj.setPartialCommand = function setPartialCommand(commandId, commandName, MENTION) {
      ...
      if (commandId !== id) {
        ...
        const obj3 = {
          id: commandId,
          untranslatedName: commandName,
          displayName: commandName,
          preferredCommandType: constants.PARTIAL_COMMAND,
        };
    ...
    obj.setPastedCommand = function setPastedCommand(arg0, channel) {
      ...
      if (null != command) {
        ...
        if (null != application) {
          const obj5 = {
            type: ApplicationCommandTypes.ApplicationCommandSectionType.APPLICATION,
            id: null,
            icon: null,
            name: null,
            application: null,
        ...
        const initialValuesFromInteractionOptions =
          ApplicationCommandOptionUtils.getInitialValuesFromInteractionOptions(command, interactionOptions);
        ...
        const obj6 = {
          ...
          command,
          section: tmp8,
          location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.PASTE,
          commandText: null,
        };
    ...
    obj.updateStyles = function updateStyles(styles) {
      ...
      obj.chatInputNodes = chatInputNodes.map((style) => {
        if (null == style.style) {
          ...
          if (obj(11460).ChatInputNodeType.COMMAND_OPTION !== type) {
            if (tmp30(11460).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE !== type) {
              if (tmp30(11460).ChatInputNodeType.GAME_HIGHLIGHT === type) {
                const obj2 = {};
                const merged = Object.assign(style);
                const styles4 = closure_1_0.styles;
                obj2.style = styles4.gameMention();
                return obj2;
              } else if (tmp30(11460).ChatInputNodeType.GAME_MENTION_INPUT === type) {
                const obj3 = {};
                const merged1 = Object.assign(style);
                const styles3 = closure_1_0.styles;
                obj3.style = styles3.commandOption();
                return obj3;
              } else if (tmp30(11460).ChatInputNodeType.ROLE_HIGHLIGHT === type) {
                ...
                if (tmp30(11460).ChatInputNodeType.EMOJI_HIGHLIGHT !== type) {
                  if (tmp30(11460).ChatInputNodeType.USER_HIGHLIGHT !== type) {
                    if (tmp30(11460).ChatInputNodeType.CHANNEL_HIGHLIGHT !== type) {
                      if (tmp30(11460).ChatInputNodeType.SILENT_HIGHLIGHT !== type) {
                        return style;
                      }
                    }
                  }
    ...
    obj.addCommandOptionParserRules = function addCommandOptionParserRules() {
      ...
      obj = {
        ...
        matchFunction(c22, activeCommand) {
          return obj(11459).getMatchedOptions(c22, activeCommand);
        },
        style() {
      ...
      parser2.addRule({
        ...
        matchFunction(length2, activeCommand) {
          return obj(11459).getMatchedOptionsWithValue(length2, activeCommand);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
      ...
      parser7.addRule({
        ...
        matchFunction(arg0) {
          return obj(11459).getSilentHighlightNodes(arg0);
        },
        style() {
          const styles = obj.styles;
          return styles.autocomplete();
      ...
      parser9.addRule({
        ...
        matchFunction(arr) {
          return obj(11459).getGameMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
      ...
      parser11.addRule({
        ...
        matchFunction(arr) {
          return obj(11459).getTimestampMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
        ...
        if (!obj.props.commandsDisabled) {
          if (text.startsWith(closure_2_11)) {
            ...
            if (null == tmp.contextCommands) {
              ...
              const commandContext = useCommandContext.getCommandContext(obj3);
              ...
              if (preferredCommandType === constants.PARTIAL_COMMAND) {
                ...
                if (null != found) {
                  ...
                  let cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(
                    obj4,
                ...
                const draftCommand = DraftCommandUtils.resolveDraftCommand(
                  channel,
                  text,
                  DraftStore.getDraftCommand(channel.id, DraftType.ChannelMessage),
                );
                  ...
                  if (0 < parts.length) {
                    function _loop() {
                      ...
                      const found = matchingGroupCommands.filter((inputType) => {
                        let tmp =
                          inputType.inputType !== joined(dependencyMap[11]).ApplicationCommandInputType.PLACEHOLDER;
                        if (tmp) {
                      ...
                      if (found.length > 0) {
                        ...
                        const cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(
                          obj,
                          tmp3(1978).ApplicationCommandType.CHAT,
                          first.applicationId,
                        );
                        if (null == cachedApplicationSection) {
                          let obj5 = { v: null };
                ...
                const tmp3Result2 = DraftCommandUtils;
              }
              let tmp17 = null;
              if (flag) {
                tmp17 = null;
            ...
            if (null != optionValue) {
              ...
              let items1 = [
                optionValueParser.parse(
                  text.substring(tmp7.location + option.displayName.length + 1, tmp7.location + tmp7.length),
                  option,
                ),
              ];
              obj[tmp6] = items1;
        ...
        if (null == displayText) {
          if (null == value) {
            ...
            current2.setSelectedRange(
              value.location + found.displayName.length + 1,
              value.length - found.displayName.length - 1,
            );
          }
        } else {
          if (null != value) {
    ...
    obj.sendCommand = function sendCommand(text, channel, fn) {
      ...
      if (null != activeCommand) {
        ...
        if (null != query) {
          ...
          ({ commands, sections } = ApplicationCommandQueryApiAll.getCachedResults(
            obj6,
            Server.ApplicationCommandType.CHAT,
            query,
          ));
          ...
          const cachedResults = ApplicationCommandQueryApiAll.getCachedResults(
            obj6,
            Server.ApplicationCommandType.CHAT,
            query,
          );
...
prototype["addTimestampMention"] = function addTimestampMention(formatted, mention) {
  ...
  const result = obj.uniqueTimestampPillText(
    ...
    ChatInputCommandOptionParser.formatTimestampPillText(formatted),
    mention,
  );
...
prototype["mergePropsAndUpdate"] = function mergePropsAndUpdate(editId) {
  ...
  if (text === editId.text) {
    ...
    if (tmp84) {
      ...
      if (!focused2) {
        const keyboardType = obj10(4699).getKeyboardType();
        focused2 = keyboardType !== obj10(1610).KeyboardTypes.SYSTEM;
        const obj6 = obj10(4699);
      }
    ...
    if (canAutoInsertFirstOption) {
      ...
      if (!self.insertFirstOptionIfValid(editId.text, activeCommand, activeCommand.displayName, tmp118, tmp123)) {
        let result1 = self.insertFirstOptionIfValid(
          editId.text,
          activeCommand,
          ...
          if (tmp99) {
            ...
            if (tmp99) {
              ...
              if (null != self.activeOption) {
                ...
                if (hasValue) {
                  obj10[self.activeOption.name].lastValidationResult =
    ...
    if (tmp140) {
      const keyboardType1 = tmp113(4699).getKeyboardType();
      tmp140 = keyboardType1 !== tmp113(1610).KeyboardTypes.SYSTEM;
      const tmp113Result2 = tmp113(4699);
    }
    if (tmp140) {
        ...
        while (iter !== undefined) {
          ...
          if (!tmp60) {
            ...
            if (tmp63) {
              ...
              tmp63 =
                str2.substring(value4.location + tmp56.displayName.length + 1, value4.location + value4.length) ===
                tmp59.displayText;
            }
            tmp60 = tmp63;
          }
