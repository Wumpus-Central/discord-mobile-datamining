...
function getCommandOptionComponents(option) {
      ...
      if (null != iter.value) {
        ...
        if (tmp6(1894).ApplicationCommandOptionType.USER === type) {
          ...
          if (null != channel) {
            ...
            const items1 = [closure_20, tmp6(4789).computeChannelName(channel, UserStore, RelationshipStore)];
            obj.children = items1;
  ...
  if (null != option.option.options) {
    ...
    while (iter2 !== undefined) {
      let obj2 = { option: nextResult, channel, guild, messageId, parentOptionName: text, commandOptionSpec: fromEntriesResult[nextResult.name], styles, analyticsLocations };
...
function CommandContentContainer(channelId) {
  ...
  const items4 = [closure_23(styles, obj2, "commandName-" + data.name), closure_23(channelId(data[22]).Text, { style: tmp.commandOptionText, variant: "text-md/medium", color: "text-default", children: memo }, "commandOption-" + data.name)];
...
function CommandActionsContainer(channelId) {
  ...
  if (0 !== items3.length) {
    ...
    let obj3 = { hasIcons: false, children: items3.map((children, index) => closure_1_23(guild.Fragment, { children }, index)) };
...
fn(4636);
let createStyles = { container: { paddingVertical: 8, paddingHorizontal: 16, gap: 16 }, activityIndicator: { padding: 16 }, application: null, applicationIcon: null, commandName: null, commandOptionText: null, commandOptionMentionText: null, commandText: null };
...
export default function ExecutedCommandPopout(channelId) {
  ...
  obj = { value: messageId(7265)(messageId(7285).EXECUTED_COMMAND).analyticsLocations, children: null };
  ...
  obj.children = closure_23(channelId(7253).BottomSheet, obj);
  return closure_23(channelId(7265).AnalyticsLocationProvider, obj);
};