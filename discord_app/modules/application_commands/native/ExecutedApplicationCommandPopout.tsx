...
function getCommandOptionComponents(option) {
  ...
  if (option.option.type !== option.option(1978).ApplicationCommandOptionType.SUB_COMMAND) {
  ...
  items.push(
    closure_23(
      noop.Fragment,
      {
        children: closure_23(option.option(1177).LegacyText, { children: combined }, "optionKey-" + option.option.name),
      },
      text,
    ),
  );
...
function CommandContentContainer(channelId) {
  ...
  const stateFromStoresObject = channelId(data[26]).useStateFromStoresObject(
    items,
    () => {
  ...
  const obj3 = {
    ...
    applicationHook() {
      ...
      const items = [
        __initData3(tmp, obj2, "icon-" + avatarSource.id),
        __initData3(Text_Text.Text, {
          variant: "text-md/semibold",
          color: "mobile-text-heading-primary",
          children: avatarSource.username,
        }),
      ];
      obj.children = items;
  ...
  obj5.children = Children.map(
    intl.format(channelId(data[24]).t["sj/RT9"], {
      userHook() {
        let tmp2;
        if (closure_14) {
          tmp2 = processColorStringsArray;
  ...
  const items4 = [
    closure_23(styles, obj5, "commandName-" + data.name),
    closure_23(
...
function CommandActionsContainer(channelId) {
  ...
  const stateFromStoresObject = channelId(channel[26]).useStateFromStoresObject(
    items,
    () => {
      channel = ChannelStore.getChannel(channelId);
      const obj = { channel, guild: null };
...
let obj2 = {
  ...
  application: { flexDirection: "row", alignItems: "center", gap: nativeDefault.space.PX_4 },
...
export default function ExecutedCommandPopout(channelId) {
  ...
  if (null != interactionData1) {
    ...
    const items2 = [closure_23(CommandContentContainer, obj4)];
    ...
    items2[1] = closure_23(CommandActionsContainer, obj5);
