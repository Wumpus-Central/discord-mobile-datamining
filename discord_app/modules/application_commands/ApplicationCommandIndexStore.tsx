...
function updateIndex(target, arg1) {
  ...
  if (undefined !== tmp3) {
    ...
    if ("application" === target.type) {
      ...
      applicationCommandIndexStore.applicationIndicesVersion =
        applicationCommandIndexStore.applicationIndicesVersion + 1;
    }
  }
...
function handleFetchSuccess(arg0) {
  ...
  if (null == id) {
    ...
    const applicationCommands = id(7535).buildApplicationCommands(
...
function useQueryState(type, commandTypes, allowFetch) {
            ...
            if (tmp34) {
              if (null == guild_id.channel.guild_id) {
                ...
                const applicationCommandIndex =
                  ApplicationCommandIndexActionCreators.requestApplicationCommandIndex(obj);
              }
            }
  ...
  return stateFromStoresObject1.useMemo(() => {
    ...
    ({
      ...
      installOnDemand: obj.installOnDemand,
    } = allowFetch);
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns !== NONE(9330).BuiltInCommandFilter.DENY) {
    ...
    let builtInCommands = NONE(9332).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = NONE(9332);
  } else {
    builtInCommands = [];
  }
  const items = [];
