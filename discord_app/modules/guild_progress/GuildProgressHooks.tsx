...
export const useGuildMessaged = function useGuildMessaged(arg0) {
  ...
  return require("initialize").useStateFromStores(items3, () =>
    _modDef12.some(closure_0, (id) => {
      messages = messages.getMessages(id.id);
      const toArrayResult = messages.toArray();
      return closure_1(stateFromStoresArray[13]).some(toArrayResult, (author) => {
...
export const useCompletedStates = function useCompletedStates(guild) {
  ...
  const items2 = [GuildMemberCountStore];
  const items3 = [guild, stateFromStoresArray];
  obj.guildPopulated = require("initialize").useStateFromStores(
    items2,
    () => {
      id = undefined;
      if (guild != null) {
  ...
  obj.guildMessaged = require("initialize").useStateFromStores(items7, () =>
    _modDef12.some(closure_0, (id) => {
      messages = messages.getMessages(id.id);
      const toArrayResult = messages.toArray();
      return closure_1(stateFromStoresArray[13]).some(toArrayResult, (author) => {
  ...
  const items11 = [guild];
  obj.guildChannelCreated = tmp(tmp2[10]).useStateFromStores(
    items10,
    () => {
      id = undefined;
      if (guild != null) {
