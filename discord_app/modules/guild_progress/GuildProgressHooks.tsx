...
export const usePermissions = function usePermissions(channel, guild) {
  ...
  return require("../../../discord_common/js/packages/flux/index.tsx").useStateFromStoresObject(
    ...
    () => {
      const obj = {
        canInvite: channel(closure_1_2[11]).canViewInviteModal(closure_1_13, closure_1, channel),
        canManageGuild: null,
        canMessage: null,
...
export const useGuildPopulated = function useGuildPopulated(guild) {
  ...
  return _require(stateFromStoresArray[10]).useStateFromStores(
    items2,
    () => {
      let id;
      if (guild != null) {
...
export const useChannelsMessaged = function useChannelsMessaged(items3) {
  ...
  return require("../../../discord_common/js/packages/flux/index.tsx").useStateFromStores(items1, () =>
    callback(stateFromStoresArray[13]).some(closure_0, (id) => {
...
export const useGuildMessaged = function useGuildMessaged(arg0) {
  ...
  closure_1 = require("../../../discord_common/js/packages/flux/index.tsx").useStateFromStores(items2, () =>
    id.getId(),
  );
  const obj2 = initialize;
  const items3 = [closure_12];
  return require("../../../discord_common/js/packages/flux/index.tsx").useStateFromStores(items3, () =>
    callback(stateFromStoresArray[13]).some(closure_0, (id) => {
      const messages = closure_1_12.getMessages(id.id);
      const toArrayResult = messages.toArray();
      return closure_1_1(closure_1_2[13]).some(toArrayResult, (author) => {
...
export const useCompletedStates = function useCompletedStates(guild) {
  ...
  const items2 = [closure_9];
  const items3 = [guild, stateFromStoresArray];
  obj[0] = _require(stateFromStoresArray[10]).useStateFromStores(
    items2,
    () => {
      let id;
      if (guild != null) {
  ...
  obj[1] = _require(stateFromStoresArray[10]).useStateFromStores(items7, () =>
    callback(stateFromStoresArray[13]).some(closure_0, (id) => {
      const messages = closure_1_12.getMessages(id.id);
      const toArrayResult = messages.toArray();
      return closure_1_1(closure_1_2[13]).some(toArrayResult, (author) => {
  ...
  const items11 = [guild];
  obj[3] = tmp(tmp2[10]).useStateFromStores(
    items10,
    () => {
      let id;
      if (guild != null) {
