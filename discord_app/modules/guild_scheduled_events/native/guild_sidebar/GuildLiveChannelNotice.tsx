// === Module 15794: GuildLiveChannelNotice ===
...
import LocationIcon from "LocationIcon" /* 8982 */;
...
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5738).useStageParticipants(channel.id, channel(5732).StageChannelParticipantNamedIndex.SPEAKER);
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(15793).useActiveEventOrStageInstanceChannel(guild.id);
