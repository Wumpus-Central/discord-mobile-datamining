...
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5519).useStageParticipants(
    channel.id,
    channel(5513).StageChannelParticipantNamedIndex.SPEAKER,
  );
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16347).useActiveEventOrStageInstanceChannel(
    guild.id,
  );
