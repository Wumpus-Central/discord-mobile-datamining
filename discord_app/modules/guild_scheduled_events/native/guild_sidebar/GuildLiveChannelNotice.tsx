...
function UserSummaryRow(arg0) {
  ...
  if (0 !== users.length) {
    ...
    if (tmp8Result) {
      ...
      obj3.style = items2;
      let obj4 = { size: "custom", style: max(9569).makeSizeStyle(14) };
      const items3 = [closure_13(max(12008).HeadphonesIcon, obj4)];
      let obj6 = {
...
function JoinChannelButton(label) {
  ...
  obj.children = closure_13(channel(5274).Button, obj2);
...
function GuildVoiceEventNotice(channel) {
  ...
  obj4.LocationIcon = channel(5328).getChannelIconComponent(channel);
  obj4.LiveIcon = channel(9065).CalendarIcon;
...
function SeeDetailButton(guildEvent) {
  ...
  obj.children = closure_13(guildEvent(5274).Button, obj2);
  return closure_13(View, obj);
}
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5738).useStageParticipants(
    channel.id,
    channel(5732).StageChannelParticipantNamedIndex.SPEAKER,
  );
  ...
  const obj3 = channel(504);
  const stageHasStream = channel(5724).useStageHasStream(channel.id);
  const obj4 = channel(5724);
  const guildActiveEvent = channel(8932).useGuildActiveEvent(channel.guild_id);
  const obj6 = {
    heading: null,
    location: null,
    LocationIcon: null,
  ...
  if (null != guildActiveEvent) {
    channelIconComponent = tmp3(5328).getChannelIconComponent(channel);
    const tmp3Result = tmp3(5328);
  }
  obj6.LocationIcon = channelIconComponent;
  if (null != guildActiveEvent) {
    let StageIcon = tmp3(9065).CalendarIcon;
  } else {
    StageIcon = tmp3(5404).StageIcon;
  }
...
obj2.react = fn(9576).inlineChannelMentionReact;
...
obj.channel = { react: fn(9576).inlineChannelReact };
...
let obj3 = { react: fn(9576).inlineChannelReact };
size = fn(2);
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(15793).useActiveEventOrStageInstanceChannel(
    guild.id,
  );
  let obj = activeEventOrStageInstanceChannel(15793);
  const guildActiveEvent = activeEventOrStageInstanceChannel(8932).useGuildActiveEvent(guild.id);
  let obj2 = activeEventOrStageInstanceChannel(8932);
  ...
  if (null != tmp13) {
    ...
    obj7.children = tmp13;
    tmp20 = closure_13(tmp2(5914).Card, obj7);
  }
  return tmp20;
});
