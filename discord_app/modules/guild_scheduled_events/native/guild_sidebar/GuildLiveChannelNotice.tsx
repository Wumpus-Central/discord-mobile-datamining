// === Module 16590: GuildLiveChannelNotice ===
...
import MarkupRulesUtils from "MarkupRulesUtils" /* 8440 */;
...
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5734).useStageParticipants(channel.id, channel(5728).StageChannelParticipantNamedIndex.SPEAKER);
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16589).useActiveEventOrStageInstanceChannel(guild.id);
