...
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5517).useStageParticipants(
    channel.id,
    channel(5511).StageChannelParticipantNamedIndex.SPEAKER,
  );
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16317).useActiveEventOrStageInstanceChannel(
    guild.id,
  );
