...
function GuildLiveStageNotice(channel) {
  channel = channel.channel;
  const tmp2 = useChannelNameDefault(channel);
  const stageParticipants = channel(5650).useStageParticipants(
    channel.id,
    channel(5644).StageChannelParticipantNamedIndex.SPEAKER,
  );
...
export default noop.memo((guild) => {
  ...
  activeEventOrStageInstanceChannel = activeEventOrStageInstanceChannel(16537).useActiveEventOrStageInstanceChannel(
    guild.id,
  );
