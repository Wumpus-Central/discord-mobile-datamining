...
function GuildEventsListHeader(arg0) {
  ...
  if (tmp3Result) {
    ...
    tmp3Result = jsx(guild(9788).ActionSheetHeaderPressableText, { accessibilityLabel: null, label: null, onPress: null });
  }
...
export default function GuildEventsListActionSheet(guild) {
  ...
  obj1.lastAckedId = events(5636)(ref);
  obj.children = jsx(events(9876), { inActionSheet: true, events, onPressEvent: callback1, onCloseAction: callback, guild, lastAckedId: null });
  obj.children = <View style={tmp.container}>{null}</View>;
  return jsx(guild(7202).BottomSheet, { style: tmp.container, children: null });
};