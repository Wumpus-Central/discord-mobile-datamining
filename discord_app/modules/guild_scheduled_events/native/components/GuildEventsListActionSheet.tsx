...
function GuildEventsListHeader(arg0) {
  ...
  if (tmp3Result) {
    ...
    tmp3Result = jsx(guild(9750).ActionSheetHeaderPressableText, { accessibilityLabel: null, label: null, onPress: null });
  }
...
export default function GuildEventsListActionSheet(guild) {
  ...
  obj1.lastAckedId = events(5600)(ref);
  obj.children = jsx(events(9838), { inActionSheet: true, events, onPressEvent: callback1, onCloseAction: callback, guild, lastAckedId: null });
  obj.children = <View style={tmp.container}>{null}</View>;
  return jsx(guild(7164).BottomSheet, { style: tmp.container, children: null });
};