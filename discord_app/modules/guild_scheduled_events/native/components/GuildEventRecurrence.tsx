// === Module 9903: GuildEventRecurrence ===
...
export default function GuildEventRecurrence(recurrenceId) {
  ...
  let obj2 = guildEventId(9763)(recurrenceId, id);
  const tmp6 = guildEventId(9763);
  const items1 = [c6];
  ...
  closure_5 = recurrenceId(9765).useManageResourcePermissions(stateFromStores2).canManageGuildEvent(stateFromStores);
  const tmp2Result5 = recurrenceId(9765);
  const eventScheduleById = recurrenceId(9762).useEventScheduleById(guildEventId, recurrenceId);
  ...
  if (null == stateFromStores) {
    ...
    obj4.children = closure_8(tmp2(9875).GuildEventCardHeader, obj5);
    ...
    if (tmp21Result) {
      ...
      tmp21Result = closure_8(tmp2(4753).Text, obj7);
    }
    ...
    const obj9 = { source: tmp5(9904), size: tmp2(1177).Icon.Sizes.REFRESH_SMALL_16, style: tmp.secondarySmallIcon };
  ...
  const tmp2Result6 = recurrenceId(9762);
};