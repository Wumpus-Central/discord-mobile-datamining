// === Module 9692: GuildEventSchedule ===
...
export default function GuildEventSchedule(schedule) {
  ...
  let obj = onChange(4074)();
  const addResult = obj.add(schedule(9647).MAX_DAYS_AHEAD_AN_EVENT_CAN_START, "days");
  const items = [schedule.startDate];
  const memo = React.useMemo(() => onChange(closure_1_2[2])(schedule.startDate).add(15, "minutes"), items);
  const tmp2 = onChange(4074)();
  const addResult1 = onChange(4074)().add(schedule(9647).MAX_DAYS_AHEAD_AN_EVENT_CAN_END, "days");
  if (null != recurrenceId) {
    addResult.add(tmp3(9647).MAX_YEARS_AHEAD_RECURRING_EVENT, "years");
    addResult1.add(tmp3(9647).MAX_YEARS_AHEAD_RECURRING_EVENT, "years");
  }
  obj = {
  ...
  obj[4] = intl.string(schedule(1233).t.kKOIwJ);
  const intl2 = tmp3(1233).intl;
  obj[5] = intl2.string(schedule(1233).t["6dGmCD"]);
  const children = [callback(schedule(9693).GuildEventDatetime, obj), ];
  ...
  if (tmp9Result) {
    ...
    obj[4] = intl3.string(tmp3(1233).t.CTLgZJ);
    const intl4 = tmp3(1233).intl;
    obj[5] = intl4.string(tmp3(1233).t.j2RuXF);
    tmp9Result = callback(tmp3(9693).GuildEventDatetime, obj);
  }
