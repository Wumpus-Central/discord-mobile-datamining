  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        const baseScheduleForRecurrence = tmp2(9759).getBaseScheduleForRecurrence(
          nextRecurrenceIdInEvent,
          stateFromStores,
        );
        const tmp2Result3 = tmp2(9759);
        const scheduleForRecurrenceWithException = tmp2(9759).getScheduleForRecurrenceWithException(
          baseScheduleForRecurrence,
          tmp7,
        );
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
