...
function GuildPill(guild) {
  guild = guild.guild;
  const tmp = callback4();
  let obj = { guild, size: GuildIconSizes.GuildIconSizes.SMALL_32, animate: true };
...
let createCacheKey = { flex: 1, backgroundColor: ThemesDefault.colors.BACKGROUND_SURFACE_HIGH, marginVertical: 8, padding: 0 };
...
let result = require("obj132").fileFinishedImporting("modules/notifications/native/GuildHighlightsNotificationsActionSheet.tsx");
...
export default function SummaryFeedbackActionSheet(guildId) {
  ...
  if (null == reasons) {
    ...
    items5[1] = callback(tmp11(tmp12[11]).Text, obj4);
    ...
    items5[2] = callback(tmp11(tmp12[11]).Text, obj5);
    obj2[1] = items5;
    ...
    if (tmp27Result) {
      ...
      obj7[0] = tmp.feedback;
      obj7[1] = null;
      tmp27Result = callback(tmp21(tmp12[20]).View, obj7);
    }
    ...
    if (first.rating !== FeedbackRating.GOOD) {
      ...
      obj9[3] = function onValueChange(arg0) {
        feedbackSettings(ref[23]);
        const obj = { notify_highlights: arg0 ? closure_1_9.DISABLED : closure_1_9.ENABLED };
      ...
      obj8[3] = callback(tmp11(tmp12[22]).FormSwitchRow, obj9);
      tmp27Result = callback(tmp11(tmp12[21]).Card, obj8);
    }
    ...
    items6[3] = callback(tmp11(tmp12[11]).Text, obj11);
    obj10[0] = items6;
    obj1[2] = callback2(first1, obj10);
    obj[4] = callback(tmp11(tmp12[18]).BottomSheetScrollView, obj1);
    return callback(tmp11(tmp12[17]).BottomSheet, obj);
  } else {
    let obj13 = first;
    if (first == null) {
      obj13 = {};
    ...
    if (tmp17) {
      ...
      let tmp18Result = callback(tmp11(tmp12[11]).Text, obj14);
    } else {
      ...
      obj15[4] = callback1;
      tmp18Result = callback(tmp11(tmp12[15]).FeedbackForm, obj15);
    }
