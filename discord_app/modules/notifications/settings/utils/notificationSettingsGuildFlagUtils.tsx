// === Module 10471: updateGuildPreset ===
...
export const updateGuildPreset = function updateGuildPreset(guildId, arg1) {
  ...
  if (arg1 === require(4454) /* Presets */.Presets.ALL_MESSAGES) {
    ...
    const result = importDefault(5201).updateGuildNotificationSettings(guildId, obj, tmp2(5196).NotificationLabels.PresetAll);
    const obj4 = importDefault(5201);
  } else if (arg1 === tmp2(4454).Presets.MENTIONS) {
    obj = importDefault(5201);
    obj = { message_notifications: null, flags: null };
    obj[0] = UserNotificationSettings.ONLY_MENTIONS;
    tmp2Result = tmp2(10466);
    obj[1] = tmp2Result.withGuildUnreadFlags(guildFlags, constants.UNREADS_ONLY_MENTIONS);
    const result1 = obj.updateGuildNotificationSettings(guildId, obj, tmp2(5196).NotificationLabels.PresetMentions);
  } else if (arg1 === tmp2(4454).Presets.NOTHING) {
    const obj1 = { message_notifications: null, flags: null };
    obj1[0] = UserNotificationSettings.NO_MESSAGES;
    const obj7 = importDefault(5201);
    obj1[1] = tmp2(10466).withGuildUnreadFlags(guildFlags, constants.UNREADS_ONLY_MENTIONS);
    const result2 = obj7.updateGuildNotificationSettings(guildId, obj1, tmp2(5196).NotificationLabels.PresetNothing);
    const tmp2Result1 = tmp2(10466);
  }
};
export const useGuildPresetSettings = function useGuildPresetSettings(guildId) {
  ...
  obj[2] = _require(4454).presetFromSettings(stateFromStores, stateFromStores1);
  return obj;
};