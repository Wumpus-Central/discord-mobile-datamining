...
export const updateGuildPreset = function updateGuildPreset(guildId, arg1) {
  ...
  if (arg1 === Presets.Presets.ALL_MESSAGES) {
    ...
    let tmp2Result = tmp2(9713);
    obj[1] = tmp2Result.withGuildUnreadFlags(guildFlags, constants.UNREADS_ALL_MESSAGES);
    const result = NotificationSettingsModalActionCreators.updateGuildNotificationSettings(guildId, obj, tmp2(5210).NotificationLabels.PresetAll);
    const obj4 = NotificationSettingsModalActionCreators;
  } else if (arg1 === tmp2(4453).Presets.MENTIONS) {
    ...
    tmp2Result = tmp2(9713);
    obj[1] = tmp2Result.withGuildUnreadFlags(guildFlags, constants.UNREADS_ONLY_MENTIONS);
    const result1 = obj.updateGuildNotificationSettings(guildId, obj, tmp2(5210).NotificationLabels.PresetMentions);
  } else if (arg1 === tmp2(4453).Presets.NOTHING) {
    ...
    obj1[1] = tmp2(9713).withGuildUnreadFlags(guildFlags, constants.UNREADS_ONLY_MENTIONS);
    const result2 = obj7.updateGuildNotificationSettings(guildId, obj1, tmp2(5210).NotificationLabels.PresetNothing);
    const tmp2Result1 = tmp2(9713);
  }
};
