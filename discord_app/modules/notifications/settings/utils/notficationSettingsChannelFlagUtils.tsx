// === Module 10149: notficationSettingsChannelFlagUtils ===
...
export const useChannelPresetInheritance = function useChannelPresetInheritance(channel) {
  ...
  const stateFromStoresArray = obj.useStateFromStoresArray(items, () => notifications_NotificationUtils.filterOverrides(UserGuildSettingsStore.getChannelOverrides(channel.guild_id), { ignoreMute: true, ignoreUnreadSetting: false, ignoreNotificationSetting: false }));
  ...
  [tmp2, tmp3] = _slicedToArray(require("useStateFromStores").useStateFromStoresArray(items1, () => {
...
export const updateChannelPreset = function updateChannelPreset(guild_id, id, arg2) {
  ...
  if (arg2 === notificationSettingsPresetUtils.Presets.ALL_MESSAGES) {
    ...
    const result = obj4.updateChannelOverrideSettings(guild_id, id, obj, NotificationSettingsUtils.NotificationLabels.PresetAll);
    ...
    const result1 = obj.updateChannelOverrideSettings(guild_id, id, obj, NotificationSettingsUtils.NotificationLabels.PresetMentions);
  } else if (arg2 === notificationSettingsPresetUtils.Presets.NOTHING) {
    const obj7 = NotificationSettingsModalActionCreatorsDefault;
    const obj1 = { message_notifications: UserNotificationSettings.NO_MESSAGES, flags: notificationSettingsFlagUtils.withChannelUnreadFlags(channelIdFlags, constants.UNREADS_ONLY_MENTIONS) };
    const result2 = obj7.updateChannelOverrideSettings(guild_id, id, obj1, NotificationSettingsUtils.NotificationLabels.PresetNothing);
...
export const updateChannelToGuildDefault = function updateChannelToGuildDefault(guild_id, id) {
  const obj = { message_notifications: UserNotificationSettings.NULL, flags: notificationSettingsFlagUtils.resetChannelUnreadFlags(UserGuildSettingsStore.getChannelIdFlags(guild_id, id)) };
  const result = obj.updateChannelOverrideSettings(guild_id, id, obj, NotificationSettingsUtils.NotificationLabels.PresetDefault);
};
