            ...
            if (arg0 === 1) {
              ...
              if (!tmp8.current) {
                ...
                if (!limit.current) {
                  ...
                  const merged = Object.assign((function getRequestPaginationParams(after, status, guildId2, status2, flag) {
                    const tmp2 = status2 === guildId(4461).GuildJoinRequestApplicationStatuses.SUBMITTED;
                    if (guildId2 === guildId(4461).GuildJoinRequestSortOrders.TIMESTAMP_DESC) {
                      if (!flag) {
                        if (0 !== status.length) {
                          let obj = { before: tmp2 ? status[status.length - 1].joinRequestId : status[status.length - 1].actionedAt };
                          return obj;
                        }
                      }
                      obj = { before: null };
                      ...
                      if (!flag) {
                        if (0 !== status.length) {
                          obj = { after: tmp2 ? status[status.length - 1].joinRequestId : status[status.length - 1].actionedAt };
                          return obj;
                        }
                      }
                      const obj1 = { after };
