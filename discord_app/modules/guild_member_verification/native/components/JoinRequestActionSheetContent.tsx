...
function OpenInterviewButton(arg0) {
  ...
  const joinRequestButtonActions = interviewChannelId(11989).useJoinRequestButtonActions(joinRequest, interviewChannelId);
  ({ handleOpenInterview, submitting } = joinRequestButtonActions);
  const obj2 = interviewChannelId(11989);
...
let closure_12 = importAllResult.memo((user) => {
  ...
  const tmp3 = joinRequest(8835)();
  const tmp4 = joinRequest(8825)(ACTION_SHEET_MAX_WIDTH);
  ({ primaryColor, secondaryColor, theme } = joinRequest(8823)({ user, displayProfile }));
  let obj = user(8832);
  ...
  if (null != user) {
    ...
    obj[2] = tmp4;
    const items1 = [callback(tmp(8839), obj), ];
    ...
    obj2[3] = obj3;
    obj2[4] = tmp8;
    const items2 = [callback(tmp(8842), obj2), ];
    ...
    const items4 = [callback(user(12335).PrimaryInfo, obj7), ];
    const obj8 = { user: null };
    obj8[0] = user;
    items4[1] = callback(tmp(12511), obj8);
    obj6[1] = items4;
    obj4[4] = callback2(View, obj6);
    items2[1] = callback(tmp(9449), obj4);
    obj1[0] = items2;
...
let closure_14 = importAllResult.memo((joinRequest) => {
  ...
  if (applicationStatus === MAX_RESULTS_PER_PAGE.GuildJoinRequestApplicationStatuses.SUBMITTED) {
    if (null != joinRequest.interviewChannelId) {
      ...
      const items = [callback(tmp2(12279).HourglassIcon, obj1), ];
  ...
  if (MAX_RESULTS_PER_PAGE.GuildJoinRequestApplicationStatuses.REJECTED === applicationStatus) {
    ...
    const items2 = [callback(tmp2(5996).CircleXIcon, obj7), ];
  ...
  } else if (tmp2(4127).GuildJoinRequestApplicationStatuses.APPROVED === applicationStatus) {
    obj = { style: null, children: null };
    obj[0] = tmp.statusContainer;
...
let closure_15 = importAllResult.memo((joinRequest) => {
  ...
  obj[4] = submitting;
  const children = [callback(joinRequest(7751).IconButton, obj), , ];
  ...
  obj1[4] = submitting;
  children[1] = callback(joinRequest(7751).IconButton, obj1);
  ...
  if (tmp6Result) {
    ...
    obj3[4] = submitting;
    tmp6Result = tmp6(tmp2(7751).IconButton, obj3);
  }
...
let closure_16 = importAllResult.memo((arg0) => {
  ...
  } else if (tmp2(4127).VerificationFormFieldTypes.MULTIPLE_CHOICE === field_type) {
...
const memoResult = importAllResult.memo(function JoinRequestActionSheetContent(displayProfile) {
  ...
  if (joinRequest.applicationStatus === joinRequest(4127).GuildJoinRequestApplicationStatuses.SUBMITTED) {
  ...
  items1[4] = callback(memo(15688), { guildId: joinRequest.guildId, userId: joinRequest.userId, selectedJoinRequestId: joinRequest.joinRequestId });
  obj[1] = items1;
  return closure_9(View, obj);
});
