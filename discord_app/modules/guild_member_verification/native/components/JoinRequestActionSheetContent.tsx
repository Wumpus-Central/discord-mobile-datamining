...
function OpenInterviewButton(arg0) {
  ...
  const joinRequestButtonActions = interviewChannelId(11902).useJoinRequestButtonActions(joinRequest, interviewChannelId);
  ({ handleOpenInterview, submitting } = joinRequestButtonActions);
  const obj2 = interviewChannelId(11902);
...
let closure_12 = importAllResult.memo((user) => {
  ...
  const tmp3 = joinRequest(8757)();
  const tmp4 = joinRequest(8747)(ACTION_SHEET_MAX_WIDTH);
  ({ primaryColor, secondaryColor, theme } = joinRequest(8745)({ user, displayProfile }));
  let obj = user(8754);
  ...
  if (null != user) {
    ...
    obj[2] = tmp4;
    const items1 = [callback(tmp(8761), obj), ];
    ...
    obj2[3] = obj3;
    obj2[4] = tmp8;
    const items2 = [callback(tmp(8764), obj2), ];
    ...
    const items4 = [callback(user(12247).PrimaryInfo, obj7), ];
    const obj8 = { user: null };
    obj8[0] = user;
    items4[1] = callback(tmp(12420), obj8);
    obj6[1] = items4;
...
let closure_14 = importAllResult.memo((joinRequest) => {
  ...
  } else if (tmp2(4105).GuildJoinRequestApplicationStatuses.APPROVED === applicationStatus) {
    obj = { style: null, children: null };
    obj[0] = tmp.statusContainer;
...
let closure_15 = importAllResult.memo((joinRequest) => {
  ...
  return closure_9(joinRequest(5117).ButtonGroup, { direction: "horizontal", align: "flex-start", justify: "space-evenly", children });
});
let closure_16 = importAllResult.memo((arg0) => {
  ...
  } else if (tmp2(4105).VerificationFormFieldTypes.MULTIPLE_CHOICE === field_type) {
...
const memoResult = importAllResult.memo(function JoinRequestActionSheetContent(displayProfile) {
  ...
  if (joinRequest.applicationStatus === joinRequest(4105).GuildJoinRequestApplicationStatuses.SUBMITTED) {
  ...
  items1[4] = callback(memo(15570), { guildId: joinRequest.guildId, userId: joinRequest.userId, selectedJoinRequestId: joinRequest.joinRequestId });
  obj[1] = items1;
  return closure_9(View, obj);
});
