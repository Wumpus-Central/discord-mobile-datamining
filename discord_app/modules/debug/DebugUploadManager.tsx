            ...
            if (closure_2 instanceof callback(4273).UploadVoiceDebugLogsError) {
              closure_1 = (function uploadErrorToAVUnderlyingError(closure_2) {
                ...
                if (callback(tmp3[5]).UploadErrorCodes.GENERAL === code) {
                  return callback(tmp3[6]).AVUnderlyingError.UploadErrorGeneral;
                } else if (callback(tmp3[5]).UploadErrorCodes.NO_FILE === code) {
                  return callback(tmp3[6]).AVUnderlyingError.UploadErrorNoFile;
                } else if (callback(tmp3[5]).UploadErrorCodes.PROGRESS === code) {
                  return callback(tmp3[6]).AVUnderlyingError.UploadErrorProgress;
                } else if (callback(tmp3[5]).UploadErrorCodes.UPLOAD === code) {
                  return callback(tmp3[6]).AVUnderlyingError.UploadErrorUpload;
                } else if (callback(tmp3[5]).UploadErrorCodes.READ === code) {
                  return callback(tmp3[6]).AVUnderlyingError.UploadErrorRead;
                }
              })(closure_2);
            }
          ...
          if (0 === c6) {
            if (arg0 === 1) {
              ...
              if (null != getSystemLog) {
                ...
                promise.then((result) => callback(table[10])(result, true));
                c6 = 7;
                c7 = 1;
              }
...
let result = require("obj132").fileFinishedImporting("modules/debug/DebugUploadManager.tsx");
