...
export default function RuleExemptionRows(rule) {
  ...
  const stateFromStores = rule(exemptRoles[8]).useStateFromStores(
    items,
    () => {
  ...
  const stateFromStores1 = rule(exemptRoles[8]).useStateFromStores(
    items2,
    () => {
      const mapped = Array.from(exemptChannels).map((item) => channel.getChannel(item));
      const found = mapped.filter((item) => null != item);
      const mapped1 = found.map((item) =>
        rule(exemptRoles[9]).computeChannelName(item, closure_1_6, closure_1_5, true),
      );
      if (0 === mapped1.length) {
        const intl = util.intl;
        let stringResult = intl.string(util.t.PoWNfe);
  ...
  const obj3 = {
    title: intl.string(rule.triggerType === AutomodTriggerType.USER_PROFILE ? t.u5xPPW : t.eq3gjh),
    helperText: null,
    hasIcons: true,
    children: null,
  };
  ...
  obj4.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(18076, dependencyMap.paths), "AutomodExemptRoles", {
  ...
  if (rule.triggerType !== AutomodTriggerType.USER_PROFILE) {
    ...
    obj6.icon = closure_8(tmp2(tmp3[11]).TableRow.Icon, obj7);
    ...
    obj6.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(18078, dependencyMap.paths), "AutomodExemptChannels", {
        guildId: rule.guildId,
        exemptChannels,
    ...
    tmp7Result = closure_8(tmp2(tmp3[11]).TableRow, obj6);
  }
  items4[1] = tmp7Result;
  obj3.children = items4;
  return closure_9(rule(exemptRoles[10]).TableRowGroup, obj3);
}
