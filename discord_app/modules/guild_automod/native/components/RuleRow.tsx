...
export default function RuleRow(triggerType) {
  ...
  const tmp = closure_8();
  const token = triggerType(4458).useToken(nativeDefault.modules.mobile.TABLE_ROW_LABEL_TEXT_STYLE);
  const obj = triggerType(4458);
  const token1 = triggerType(4458).useToken(nativeDefault.modules.mobile.TABLE_ROW_LABEL_COLOR);
  const obj2 = triggerType(4458);
  const ruleInfo = triggerType(17962).getRuleInfo(triggerType, rule);
  if (null == ruleInfo) {
    return null;
  } else {
    ({ headerText, headerSubtext, icon, descriptionText } = ruleInfo);
    if (null != rule) {
      const ruleActionsInOrder = tmp2(17957).getRuleActionsInOrder(rule);
      ...
      const tmp2Result = tmp2(17957);
    } else {
      const availableActionTypes = tmp2(17956).getAvailableActionTypes(triggerType);
      mapped = availableActionTypes.map((actionType) => hasOwnProperty(ActionPill, { actionType, triggerType }, actionType));
      const tmp2Result2 = tmp2(17956);
    }
    let tmp7 = null;
    if (mapped.length > 0) {
    ...
    if (null == rule) {
      ...
      const items = [closure_5(tmp2(4753).Text, obj6), tmp7];
      obj5.children = items;
    ...
    if (null == rule) {
      ...
      closure_5(tmp2(5187).Button, obj7);
    } else {
      let tmp14;
      if (!rule.enabled) {
        ...
        tmp14 = closure_5(tmp2(5824).TableRow.TrailingText, obj8);
      }
      ...
      if ("" !== headerSubtext) {
        ...
        items1[2] = closure_5(tmp2(4753).Text, obj10);
        obj9.children = items1;
        tmp19 = closure_6(tmp2(4753).Text, obj9);
      }
      const obj11 = { label: tmp19, subLabel: tmp10, icon: null, trailing: null, arrow: null, onPress: null };
      const obj12 = {};
      const merged = Object.assign(icon);
      obj11.icon = closure_5(tmp2(5824).TableRow.Icon, obj12);
      obj11.trailing = tmp14;
      obj11.arrow = null != rule;
      obj11.onPress = onPress;
      return closure_5(tmp2(5824).TableRow, obj11);
    }
  }
  const obj3 = triggerType(17962);
};