...
export default noop.forwardRef((animate, ref) => {
  ...
  const items2 = [id];
  const stateFromStoresObject = obj2.useStateFromStoresObject(items1, () => ({ isMobileOnline: PresenceStore.isMobileOnline(id), isVROnline: PresenceStore.isVROnline(id), status: PresenceStore.getStatus(id), activities: PresenceStore.getActivities(id), customStatusActivity: PresenceStore.findActivity(id, (type) => type.type === constants.CUSTOM_STATUS) }), items2);
  ...
  obj.streaming = tmp9(8375)(activities);
  if (flag) {
    flag = !stateFromStores;
  }
  obj.animate = flag;
  obj.avatarDecoration = id(8374)(obj4.getProfilePreviewValue(obj));
  if (null != onPress) {
  ...
  const tmp10Result = id(8374)(obj4.getProfilePreviewValue(obj));
});