// === Module 9839: QuickSwitcherStore ===
...
import ReadStateUtils from "ReadStateUtils" /* 9850 */;
...
function generateResultFromId(found) {
  ...
  if (null == tmp2) {
    ...
    if (tmp3) {
      if (tmp2.type !== sortByMatchScore.AutocompleterResultTypes.USER) {
        return null;
      }
...
function generateInitialResults() {
  ...
  if (channelId(set[15]).AutocompleterResultTypes.USER_GLOBAL !== queryMode) {
    if (tmp4(tmp5[15]).AutocompleterResultTypes.USER !== queryMode) {
      ...
      } else if (tmp4(tmp5[15]).AutocompleterResultTypes.GAME_PROFILE === queryMode) {
        return [];
          ...
          const item = recentlyEditedDrafts.forEach((channelId) => {
            ...
            if (!closure_0(channelId)) {
              ...
              if (null != tmp3) {
                ...
                tmp8 = queryMode === channelId(set[15]).AutocompleterResultTypes.USER || queryMode === channelId(set[15]).AutocompleterResultTypes.USER_GLOBAL;
              }
              if (null != tmp5) {
        ...
        if (null != guildId) {
          ...
          const mapped = found1.map((item) => {
            ...
            if (null != tmp2) {
              ...
              tmp6 = queryMode === channelId(set[15]).AutocompleterResultTypes.USER || queryMode === channelId(set[15]).AutocompleterResultTypes.USER_GLOBAL;
            }
            return tmp3;
          });
          ...
          let item = values.forEach((item) => {
            for (const key10004 in arg0) {
              ...
              if (null != tmp14) {
                ...
                if (!tmp3) {
                  tmp3 = queryMode === sortByMatchScore.AutocompleterResultTypes.USER_GLOBAL;
                }
                if (tmp3) {
                  tmp5 = tmp14;
                  if (tmp14.type !== sortByMatchScore.AutocompleterResultTypes.USER) {
                    tmp5 = null;
                  }
                } else {
                  tmp5 = tmp14;
...
function handleUserSearchUpdate(arr, str) {
  ...
  if (!flag) {
    ...
    if (sortByMatchScore.AutocompleterResultTypes.USER_GLOBAL !== queryMode) {
      if (DOWN(9840).AutocompleterResultTypes.USER !== queryMode) {
        if (DOWN(9840).AutocompleterResultTypes.TEXT_CHANNEL === queryMode) {
        ...
        } else if (DOWN(9840).AutocompleterResultTypes.VOICE_CHANNEL === queryMode) {
        ...
        } else if (DOWN(9840).AutocompleterResultTypes.GUILD === queryMode) {
        ...
        } else if (DOWN(9840).AutocompleterResultTypes.APPLICATION === queryMode) {
        ...
        } else if (DOWN(9840).AutocompleterResultTypes.GAME_PROFILE === queryMode) {
        ...
        if (str !== c28) {
          ...
          if (tmp31) {
            selectedIndex = DOWN(9840).findNextSelectedResult(DOWN(9840).FindResultDirections.DOWN, selectedIndex, arr);
    ...
    let guild = null;
    if (queryMode !== DOWN(9840).AutocompleterResultTypes.USER_GLOBAL) {
    ...
    if (null == _null) {
      ...
      if (queryMode !== queryMode) {
        if (null == queryMode) {
          ...
          if (tmp2) {
            const items1 = [sortByMatchScore.AutocompleterResultTypes.USER];
