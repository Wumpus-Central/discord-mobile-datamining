// === Module 16017: WebAuthnScreen ===

// Module 16017 (WebAuthnScreen)
import nativeDefault from "native" /* 576 */;
import util from "util" /* 1115 */;
import native from "native" /* 1177 */;
import NativeSecurityKeyManagerModuleDefault from "NativeSecurityKeyManagerModule" /* 6927 */;
...
const buttonDefault = tmp15(16021);
require = fn;
...
export default function WebAuthnScreen(arg0) {
  ...
  const callback = obj.useCallback(() => {
    ...
    memo(challenge).then((data) => finish({ mfaType: "webauthn", data })).then(() => closure_1_4(true)).catch((error) => {
      if (error instanceof finish(1271).HTTPResponseError) {
        ...
        const result = finish(7280).captureWebAuthnException(error, {});
        dependencyMap(error.message);
        const tmpResult = finish(7280);
      }
  ...
  obj3.headerImage = challenge(finish(15038).KeyImage, {});
