...
const memoResult = importAllResult.memo((active) => {
  ...
  const token = onPress(3988).useToken(Themes.modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const obj2 = onPress(3988);
  const tmp4 = importDefault;
  const token1 = onPress(3988).useToken(Themes.modules.mobile.CHAT_INPUT_PILL_PADDING);
  const obj3 = onPress(3988);
  const token2 = onPress(3988).useToken(Themes.modules.mobile.CHAT_INPUT_PILL_BORDER_WIDTH);
  const obj4 = onPress(3988);
  const token3 = onPress(3988).useToken(Themes.modules.mobile.CHAT_INPUT_ICON_SIZE);
  const obj5 = onPress(3988);
  let diff = token;
  const token4 = onPress(3988).useToken(Themes.modules.mobile.CHAT_INPUT_BUTTON_MIN_TOUCH_TARGET_SIZE);
  if (!enabled) {
