...
import VoiceMessageButtonDefault from "VoiceMessageButton" /* 12348 */;
...
const forwardRefResult = noop.forwardRef((channel, arg1) => {
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = jsx(onSendMessage(12348), { disabled: stateFromStores1, channelId: channel.id });
    } else {
      obj1 = { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null };
      ({ button: obj11.style, buttonActive: obj11.activeStyle, iconActive: obj11.activeIconStyle } = tmp7);
      obj1.IconComponent = tmp(4551).SendMessageIcon;
      const intl = tmp(1114).intl;
      obj1.accessibilityLabel = intl.string(tmp(1114).t.TXNS7S);
      obj1.onPress = onSendMessage;
      obj1.disabled = !tmp12;
      tmp15Result = jsx(onSendMessage(12332), { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null });
      const tmp3Result = onSendMessage(12332);
    }
    ...
    obj3 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: jsx(tmp(4317).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey }) };
    return <FloatingSlot buttonWidth={token} buttonHeight={token1} buttonMargin={token2} sendVoiceMessageEnabled={canSendVoiceMessage}>{jsx(tmp(4317).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey })}</FloatingSlot>;
  }
