// === Module 12511: ChatInputSendButton ===
...
import VoiceMessageButtonDefault from "VoiceMessageButton" /* 12512 */;
import useChatInputFloatingWidthDefault from "useChatInputFloatingWidth" /* 12516 */;
...
let closure_8 = fn(12219).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsx = fn(21).jsx;
...
const forwardRefResult = noop.forwardRef((channel, arg1) => {
  ...
  const token = channel(4458).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_WIDTH);
  const obj = channel(4458);
  const token1 = channel(4458).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT);
  const obj2 = channel(4458);
  const token2 = channel(4458).useToken(onSendMessage(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  const obj3 = channel(4458);
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = jsx(onSendMessage(12512), { disabled: stateFromStores1, channelId: channel.id });
    } else {
      ...
      obj9.IconComponent = tmp(4701).SendMessageIcon;
      const intl = tmp(1115).intl;
      obj9.accessibilityLabel = intl.string(tmp(1115).t.TXNS7S);
      obj9.onPress = onSendMessage;
      obj9.disabled = !tmp12;
      tmp15Result = jsx(onSendMessage(12496), { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null });
      const tmp3Result = onSendMessage(12496);
    }
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: jsx(tmp(4467).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey }) };
    return <FloatingSlot buttonWidth={token} buttonHeight={token1} buttonMargin={token2} sendVoiceMessageEnabled={canSendVoiceMessage}>{jsx(tmp(4467).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey })}</FloatingSlot>;
  }
