...
import VoiceMessageButtonDefault from "VoiceMessageButton" /* 12497 */;
...
const forwardRefResult = noop.forwardRef((channel, arg1) => {
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = jsx(onSendMessage(12497), { disabled: stateFromStores1, channelId: channel.id });
    } else {
      ...
      obj9.disabled = !tmp12;
      tmp15Result = jsx(onSendMessage(12481), { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null });
      const tmp3Result = onSendMessage(12481);
    }
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: jsx(tmp(4432).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey }) };
    return <FloatingSlot buttonWidth={token} buttonHeight={token1} buttonMargin={token2} sendVoiceMessageEnabled={canSendVoiceMessage}>{jsx(tmp(4432).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey })}</FloatingSlot>;
  }
