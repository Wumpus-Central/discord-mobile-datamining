...
import VoiceMessageButtonDefault from "VoiceMessageButton" /* 12588 */;
...
const forwardRefResult = noop.forwardRef((channel, arg1) => {
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      obj9.disabled = !tmp12;
      tmp15Result = jsx(onSendMessage(12572), { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null });
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: jsx(tmp(4533).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey }) };
    return <FloatingSlot buttonWidth={token} buttonHeight={token1} buttonMargin={token2} sendVoiceMessageEnabled={canSendVoiceMessage}>{jsx(tmp(4533).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey })}</FloatingSlot>;
  }
