// === Module 12507: ChatInputSendButton ===
...
import VoiceMessageButtonDefault from "VoiceMessageButton" /* 12508 */;
import useChatInputFloatingWidthDefault from "useChatInputFloatingWidth" /* 12512 */;
...
let closure_8 = fn(12215).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsx = fn(21).jsx;
...
const forwardRefResult = noop.forwardRef((channel, arg1) => {
  ...
  if (stateFromStores) {
    ...
    if (canSendVoiceMessage) {
      ...
      let tmp15Result = jsx(onSendMessage(12508), { disabled: stateFromStores1, channelId: channel.id });
    } else {
      ...
      obj9.disabled = !tmp12;
      tmp15Result = jsx(onSendMessage(12492), { active: true, style: null, activeStyle: null, activeIconStyle: null, IconComponent: null, accessibilityLabel: null, onPress: null, disabled: null });
      const tmp3Result = onSendMessage(12492);
    }
    ...
    const obj12 = { buttonWidth: token, buttonHeight: token1, buttonMargin: token2, sendVoiceMessageEnabled: canSendVoiceMessage, children: jsx(tmp(4466).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey }) };
    return <FloatingSlot buttonWidth={token} buttonHeight={token1} buttonMargin={token2} sendVoiceMessageEnabled={canSendVoiceMessage}>{jsx(tmp(4466).TransitionGroup, { items: memo, renderItem: renderChatInputSendButton, getItemKey: getChatInputSendButtonItemKey })}</FloatingSlot>;
  }
