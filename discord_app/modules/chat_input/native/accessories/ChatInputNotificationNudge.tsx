// === Module 11577: ChatInputNotificationNudgeImpl ===
...
function ChatInputNotificationNudgeImpl(onDismiss) {
  ...
  const items = [surface];
  callback3(() => {
    importDefault(surface[13]);
    const obj = { action: closure_1_15.IMPRESSION, prompt_type: surface };
  ...
  const obj4 = {
    ...
    onPress() {
      ...
      importDefault(surface[13]).track(closure_1_10.CONTEXTUAL_REMINDER_ACTION, obj);
    },
...
let closure_19 = createCacheKey.createStyles((arg0) => {
  ...
  let num3 = 0;
  if (!arg0) {
    num3 = ThemesDefault.space.PX_12;
  }
...
let c20 = 604800000;
let result = require("obj132").fileFinishedImporting("modules/chat_input/native/accessories/ChatInputNotificationNudge.tsx");
...
export default function ChatInputNotificationNudge(channel) {
  ...
  const stateFromStores = obj.useStateFromStores(items, () => {
    ...
    if (channel.isThread()) {
      let isMutedResult = closure_1_7.isMuted(channel.id);
    } else {
      isMutedResult = closure_1_8.isChannelMuted(guildId, channel.id);
    }
  ...
  const tmpResult1 = channel(7366);
  [tmp13, tmp14] = callback(channel(7366).useSelectedTimeRecurringDismissibleContent(prop, obj, undefined, true), 2);
  importDefault = tmp14;
  const tmp12 = callback(channel(7366).useSelectedTimeRecurringDismissibleContent(prop, obj, undefined, true), 2);
  let prop1 = null;
  if (tmp5) {
    prop1 = null;
  ...
  obj = { cooldownDurationMs: c20 };
  const tmp11Result = callback(channel(7366).useSelectedTimeRecurringDismissibleContent(prop1, obj, undefined, true), 2);
  dependencyMap = tmp17;
  const items1 = [tmp14];
  const items2 = [tmp11Result[1]];
  const tmpResult2 = channel(7366);
  const tmp18 = callback2(() => _undefined(ContentDismissActionType.USER_DISMISS), items1);
  if (tmp13 === channel(1377).DismissibleContent.NOTIFICATION_NUDGE_CHAT_BOTTOM_BANNER) {
    ...
    obj1[0] = PermissionPromptType.CHANNEL_BANNER;
    ...
    if (tmp11Result[0] === tmp(1377).DismissibleContent.NOTIFICATION_NUDGE_POST_REACTION_BANNER) {
      ...
      obj2[0] = PermissionPromptType.POST_REACTION_BANNER;
