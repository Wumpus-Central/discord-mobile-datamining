...
import getChatInputPositionStyleDefault from "getChatInputPositionStyle" /* 12050 */;
import closure_3 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import { updateChatInputContainerHeight as closure_6 } from "updateChatInputContainerHeight" /* 9547 */;
...
export default function ChatInputGuard(type) {
  ...
  if ("simple-action" === type.type) {
    ...
    if (null != actionLabel) {
      if (null != actionOnPress) {
        ...
        if (actionIcon == null) {
          ...
          obj2[0] = tmp(709).colors.WHITE;
          actionIcon = tmp7(tmp9(10893).ArrowSmallRightIcon, obj2);
        }
        obj1[1] = actionIcon;
        obj1[3] = actionOnPress;
        let tmp7Result = tmp7(tmp9(8015).IconButton, obj1);
      }
      obj[6] = tmp7Result;
      const obj3 = { variant: "text-sm/semibold", children: null };
      obj3[1] = message;
      obj[7] = tmp7(tmp9(4474).Text, obj3);
      obj[8] = subtext;
    ...
    if (null != countdown) {
      ...
      obj4[0] = items;
      obj4[1] = countdown;
      tmp7Result = tmp7(tmp(10888), obj4);
    }
    tmp8 = closure_5;
  } else {
    ...
    if (null != subtext2) {
      ...
      if (typeof subtext2 === "string") {
        ...
        if (subtext2.length > 0) {
          ...
          obj[0] = tmp3.subtext;
          obj[3] = subtext2;
          tmp13Result = tmp13(tmp14(4474).Text, obj);
        }
      }
    }
    ...
    if (null != buttonSecondaryText) {
      ...
      if (null != buttonSecondaryOnPress) {
        ...
        items2[1] = tmp13(tmp14(4928).Button, obj9);
        obj8[0] = items2;
        tmp16Result = tmp16(tmp14(8998).TwinButtons, obj8);
      }
    }
    ...
    if (null != countdown2) {
      ...
      obj11[0] = items3;
      obj11[1] = countdown2;
      tmp13Result = tmp13(tmp(10888), obj11);
    }
...
export const ChatInputGuardContainer = function ChatInputGuardContainer(screenIndex) {
  ...
  obj[0] = items1;
  obj[1] = callback;
  const items2 = [callback(screenIndex(12051).ChatInputScrimGradient, {}), ];
  ...
  tmp10Result = null;
  if (tmp3) {
    tmp10Result = tmp10(tmp(5078), { absolute: true, wide: true, tall: true, mix: true });
  }
  ...
  if (tmp4Result.isIOS()) {
    ...
    if (null != channelId) {
      ...
      tmp10Result1 = tmp10(tmp(12057), obj3);
    }
  }
