...
import getChatInputPositionStyleDefault from "getChatInputPositionStyle" /* 11794 */;
import closure_3 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import { updateChatInputContainerHeight as closure_6 } from "updateChatInputContainerHeight" /* 9495 */;
...
export default function ChatInputGuard(type) {
  ...
  if ("simple-action" === type.type) {
    ...
    if (null != actionLabel) {
      if (null != actionOnPress) {
        ...
        if (actionIcon == null) {
          ...
          actionIcon = tmp7(tmp9(10634).ArrowSmallRightIcon, obj2);
        }
        obj1[1] = actionIcon;
        obj1[3] = actionOnPress;
        let tmp7Result = tmp7(tmp9(7974).IconButton, obj1);
      }
      obj[6] = tmp7Result;
      const obj3 = { variant: "text-sm/semibold", children: null };
      obj3[1] = message;
      obj[7] = tmp7(tmp9(4444).Text, obj3);
      obj[8] = subtext;
    ...
    if (null != countdown) {
      ...
      obj4[0] = items;
      obj4[1] = countdown;
      tmp7Result = tmp7(tmp(10629), obj4);
    }
    tmp8 = closure_5;
  } else {
    ...
    if (null != subtext2) {
      ...
      if (typeof subtext2 === "string") {
        ...
        if (subtext2.length > 0) {
          ...
          obj[0] = tmp3.subtext;
          obj[3] = subtext2;
          tmp13Result = tmp13(tmp14(4444).Text, obj);
        }
      }
    }
    ...
    if (null != buttonSecondaryText) {
      ...
      if (null != buttonSecondaryOnPress) {
        ...
        items2[1] = tmp13(tmp14(4896).Button, obj9);
        obj8[0] = items2;
        tmp16Result = tmp16(tmp14(8947).TwinButtons, obj8);
      }
    }
    ...
    if (null != countdown2) {
      ...
      obj11[0] = items3;
      obj11[1] = countdown2;
      tmp13Result = tmp13(tmp(10629), obj11);
    }
...
export const ChatInputGuardContainer = function ChatInputGuardContainer(screenIndex) {
  ...
  obj[0] = items1;
  obj[1] = callback;
  const items2 = [callback(screenIndex(11795).ChatInputScrimGradient, {}), ];
  ...
  tmp10Result = null;
  if (tmp3) {
    tmp10Result = tmp10(tmp(5038), { absolute: true, wide: true, tall: true, mix: true });
  }
  ...
  if (tmp4Result.isIOS()) {
    ...
    if (null != channelId) {
      ...
      tmp10Result1 = tmp10(tmp(11801), obj3);
    }
  }
