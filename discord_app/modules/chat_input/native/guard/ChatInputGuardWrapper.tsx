// discord_app/modules/chat_input/native/guard/ChatInputGuardWrapper.tsx
...
let closure_14 = fn(7288).PHONE_VERIFICATION_MODAL_KEY;
const jsx = fn(21).jsx;
const size = fn(2);
...
export default function ChatInputGuardWrapper(channel) {
  ...
  stateFromStores = channel(stateFromStores[12]).useStateFromStores(items, () =>
    MemberVerificationUtils.guildHasVerificationGate(GuildStore.getGuild(guildId)),
  );
  let obj = channel(stateFromStores[12]);
  const items1 = [GuildVerificationStore];
  const stateFromStores1 = channel(stateFromStores[12]).useStateFromStores(items1, () =>
    GuildVerificationStore.getCheck(guildId),
  );
  ...
  const stateFromStoresObject = channel(stateFromStores[12]).useStateFromStoresObject(
    items2,
    () => {
  ...
  if (tmp11) {
    ...
    const obj7 = {
      type: "simple-action",
      icon: jsx(tmp2(tmp3[34]).CircleInformationIcon, {}),
      message: null,
      subtext: null,
    };
          ...
          if (!showLinkedLobbyApplicationLoadingIndicator) {
            if (null == requiredLinkedLobbyApplication) {
              if (null != communicationDisabledGuildMember) {
                ...
                tmp21Result = jsx(tmp7(tmp3[33]), {
                  type: "simple-action",
                  icon: jsx(tmp2(tmp3[43]).ChatXIcon, {}),
                  message: null,
                });
                tmp23 = jsx;
                ...
                obj14.buttonPrimaryOnPress = function handleVerifyPhone() {
                  guildId(stateFromStores[22]).trackWithMetadata(constants2.TEXT_AREA_CTA_CLICKED, {
                    cta_type: constants.VERIFY_PHONE,
                  });
                ...
                obj15.buttonPrimaryOnPress = function handleVerifyEmail() {
                  guildId(stateFromStores[22]).trackWithMetadata(constants2.TEXT_AREA_CTA_CLICKED, {
                    cta_type: constants.VERIFY_EMAIL,
                  });
                  const obj = guildId(stateFromStores[22]);
                ...
                const obj16 = {
                  type: "simple-action",
                  icon: jsx(tmp2(tmp3[44]).CalendarIcon, {}),
                  message: null,
                  countdown: null,
                };
                  ...
                  if (null != verificationRole) {
                    ...
                    if (null === verificationRole.tags.guild_connections) {
                      ...
                      obj21.buttonPrimaryOnPress = function handleGetVerificationRole() {
                        ...
                        if (tmp2) {
                          const result =
                            GuildRoleConnectionsModalActionCreators.openGuildRoleConnectionsConnectAccountModal(
                              verificationRole,
                              guildId,
                            );
                        }
                    ...
                    tmp21Result = jsx(tmp7(tmp3[33]), obj22);
                    tmp23 = jsx;
                      ...
                      if (!verificationRole(channel.type)) {
                        if (canCreateThreads) {
                          ...
                          obj23.buttonPrimaryOnPress = function handleCreateThread() {
                            const result = navigateToThreadCreation.navigateToThreadCreation(
                              channel,
                              "chat input guard",
                            );
                          };
