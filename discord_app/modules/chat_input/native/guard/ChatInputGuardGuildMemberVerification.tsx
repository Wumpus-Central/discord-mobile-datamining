...
const memoResult = importAllResult.memo(function ChatInputGuardGuildMemberVerification(guildId) {
  ...
  if (guildId(4200).GuildJoinRequestApplicationStatuses.REJECTED === applicationStatus) {
    ...
    let fn = function _() {
      ...
      const result = guildId(dependencyMap[12]).openMemberVerificationRejectedAlert(obj);
    };
    ...
    fn = function _() {
      ...
      const intl = guildId(dependencyMap[10]).intl;
      obj[1] = intl.string(guildId(dependencyMap[10]).t["13tjTU"]);
      const result = guildId(dependencyMap[12]).openMemberVerificationCancelPendingAlert(obj);
    };
    ...
    fn = function _() {
      ...
      obj.trackWithMetadata(AnalyticEvents.TEXT_AREA_CTA_CLICKED, obj);
      const result = guildId(dependencyMap[15]).openMemberVerificationModal(guildId);
    };
  }
  ...
  if (null != tmp8) {
    ...
    obj[1] = tmp8;
    let tmp13Result = <Image style={null} source={null} />;
  } else {
    obj = { style: null, source: null, autoPlay: null };
    obj[0] = tmp.lottieAnimation;
    obj[1] = tmp2(11627);
    obj[2] = !stateFromStores;
    tmp13Result = jsx(playDefault, { style: null, source: null, autoPlay: null });
...
let result = require("obj132").fileFinishedImporting("modules/chat_input/native/guard/ChatInputGuardGuildMemberVerification.tsx");
