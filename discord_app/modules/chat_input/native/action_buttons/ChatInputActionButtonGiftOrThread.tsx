// === Module 11459: renderChatInputActionButtonGiftAndThread ===

// Module 11459 (renderChatInputActionButtonGiftAndThread)
import ThemesDefault from "Themes" /* 712 */;
import FadeTransitionItemDefault from "FadeTransitionItem" /* 11460 */;
...
function renderChatInputActionButtonGiftAndThread(arg0, styleButton, state, cleanup) {
  ...
  if (shouldShowThread) {
    ...
    obj[3] = onPress(11449).ThreadPlusIcon;
    obj[4] = function onPress(arg0) {
      return onPress(arg0, closure_1_5.THREAD);
    };
    obj[5] = styleButton;
    let tmpResult = tmp(tmp2(11453), obj);
    const tmp2Result = tmp2(11453);
  } else {
...
const memoResult = importAllResult.memo(function ChatInputActionButtonGiftOrThread(arg0) {
  ...
  let items = [arg0];
  const obj2 = _require(4100);
  obj = { style: callback(token, _require(4100).useToken(ThemesDefault.modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN)).container, children: null };
  ...
  obj[1] = jsx(_require(4107).TransitionGroup, { items: memo, renderItem: renderChatInputActionButtonGiftAndThread, getItemKey: getChatInputActionButtonGiftAndThreadKey });
