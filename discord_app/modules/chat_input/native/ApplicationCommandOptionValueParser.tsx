// === Module 11564: getUsers ===

// Module 11564 (getUsers)
import applyDefault from "apply" /* 12 */;
import isDiscordFrontendDevelopment from "isDiscordFrontendDevelopment" /* 1370 */;
import PermissionOverwriteType from "PermissionOverwriteType" /* 1955 */;
import rebuild from "rebuild" /* 7287 */;
import rebuildDefault from "rebuild" /* 7287 */;
import closure_3 from "noop" /* 19 */;
import closure_4 from "rebuild" /* 5329 */;
...
function getChannels(getGuildId, arr) {
  ...
  if (null == guildId) {
    ...
    return guildId(12)(items).map((id) => {
      const obj = { id: id.id, text: arr(4574).computeChannelName(id, closure_10, closure_9) };
      return obj;
    });
  } else {
    dependencyMap = textChannelNameDisambiguations.getTextChannelNameDisambiguations(guildId);
    const tmp9 = guildId(12);
    const tmp9Result = guildId(12)(_require(5266).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS);
    const combined = guildId(12)(_require(5266).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS).flatMap((arg0) => closure_1_6.getChannels(guildId)[arg0].map((channel) => channel.channel)).concat(closure_4.computeAllActiveJoinedThreads(guildId));
    ...
    return found.map((id) => {
      ...
      if (closure_1_5(id.type)) {
        ...
        if (name == null) {
          name = arr(4574).computeChannelName(id, closure_1_10, closure_1_9);
          const obj3 = arr(4574);
        }
        let channelName = name;
      } else {
        channelName = arr(4574).computeChannelName(id, closure_1_10, closure_1_9);
        const obj2 = arr(4574);
      }
      obj[1] = channelName;
      return obj;
