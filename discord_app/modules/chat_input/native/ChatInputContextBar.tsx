...
function ChatInputReplyBarNoAuthor(onCancelReplying) {
  ...
  const bound = Math.max(
    0,
    (token - useToken.useToken(nativeDefault.modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_SIZE)) / 2,
  );
...
function ChatInputEditBar(onCancelEditing) {
  ...
  const items = [tmp8];
  const obj5 = { source: _modDef7271, size: native.Icon.Sizes.CUSTOM, style: tmp.floatingCloseIcon };
...
function ChatInputScheduledMessageBar(scheduledTimestamp) {
  ...
  const items = [tmp8];
  const obj7 = { source: _modDef7271, size: native.Icon.Sizes.CUSTOM, style: tmp.floatingCloseIcon };
  items[1] = closure_1_12(component, {
    style: tmp.floatingRightActions,
...
let closure_17 = createStyles.createStyles(() => {
  obj = {
    ...
    floatingReplyTextWrapper: { flexShrink: 1, minWidth: 0 },
