...
import _modDef7011 from "../../../../_runtime/metro/07011__.js";
...
function ChatInputReplyBarNoAuthor(onCancelReplying) {
  ...
  const bound = Math.max(
    0,
    (token - obj1.useToken(nativeDefault.modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_SIZE)) / 2,
  );
  ...
  obj2.children = closure_1_12(native.Icon, {
    source: _modDef7011,
...
class ChatInputReplyBar {
  constructor(arg0) {
    ...
    bound = Math.max(
      0,
      (token - obj2.useToken(closure_1(colorString[9]).modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_SIZE)) / 2,
    );
...
function ChatInputEditBar(onCancelEditing) {
  ...
  obj1 = { source: _modDef7011, size: native.Icon.Sizes.CUSTOM, style: tmp.floatingCloseIcon };
  ...
  items[1] = closure_1_12(component, {
    ...
    children: closure_1_12(Pressables.PressableOpacity, obj),
  });
  obj3.children = items;
...
function ChatInputScheduledMessageBar(scheduledTimestamp) {
  ...
  const items = [tmp8];
  const obj3 = { source: _modDef7011, size: native.Icon.Sizes.CUSTOM, style: tmp.floatingCloseIcon };
  items[1] = closure_1_12(component, {
    style: tmp.floatingRightActions,
    children: closure_1_12(Pressables.PressableOpacity, obj2),
  });
  obj5.children = items;
...
let closure_17 = createStyles.createStyles(() => {
  ...
  floatingContextBar = {
    ...
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: nativeDefault.modules.mobile.CHAT_INPUT_CONTEXT_BAR_PADDING_HORIZONTAL,
    paddingVertical: nativeDefault.modules.mobile.CHAT_INPUT_CONTEXT_BAR_PADDING_VERTICAL,
    gap: nativeDefault.modules.mobile.CHAT_INPUT_CONTEXT_BAR_GAP,
  };
  floatingContextBar.contextBarRow = floatingContextBar;
  floatingContextBar.floatingReplyTextWrapper = { flexShrink: 1, minWidth: 0 };
  floatingContextBar = {
    borderBottomWidth: React4.hairlineWidth,
  ...
  const size = {
    width: nativeDefault.modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_SIZE,
    height: nativeDefault.modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_SIZE,
    tintColor: nativeDefault.colors.TEXT_MUTED,
    marginRight: nativeDefault.modules.mobile.CHAT_INPUT_REPLY_MENTION_ICON_MARGIN_RIGHT,
  };
  ...
  floatingContextBar.floatingRightActions = {
    flexGrow: 1,
    flexShrink: 0,
    flexDirection: "row",
    alignItems: "center",
  ...
  floatingContextBar.floatingDivider = {
    width: React4.hairlineWidth,
    alignSelf: "stretch",
...
let closure_24 = noop.forwardRef((children, arg1) => {
  ...
  backgroundColor = closure_16(obj.useGradientValue(backgroundColor(ref[21]).GradientPercentage.END)).contextBar
    .backgroundColor;
  const tmp = closure_17();
...
export default noop.memo(function ChatInputContextBar(channel) {
    ...
    () => ({
      ...
      onTapContextBarReply() {
        if (null != closure_1_2) {
          ...
          channel(pendingReply[27]).transitionTo(
            closure_2_11.CHANNEL(channel.getGuildId(), closure_1_2.channel.id, closure_1_2.message.id),
          );
      ...
      onToggleReplyMention() {
        if (null != closure_1_2) {
          const result = channel(pendingReply[24]).setPendingReplyShouldMention(
            closure_1_2.channel.id,
            !closure_1_2.shouldMention,
          );
  ...
  stateFromStores = channel(pendingReply[15]).useStateFromStores(items1, () =>
    DraftStore.getScheduledMessage(channel.id),
  );
  ...
  if (tmp20) {
    ...
    const obj7 = {
      ...
      onEditSchedule() {
        return ScheduledMessagesUtils.openScheduleMessageActionSheet(
          channel.id,
          ScheduledMessageTypes.ScheduledMessageEntryPoint.COMPOSER_BAR,
