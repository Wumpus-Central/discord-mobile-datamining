...
import createCacheKey from "createCacheKey";
import { registerAsset } from "../../../../_runtime/08251_registerAsset.js";
...
function ChatInputReplyBarNoAuthor(onCancelReplying) {
  ...
  if (obj.useMobileVisualRefreshConfig({ location: "ChatInputContextBar" }).chatInputFloating) {
    ...
    const items = [callback(tmp(4251).Text, obj), ];
    ...
    obj1[1] = callback(tmp(4797).PressableOpacity, obj2);
    ...
    items1[1] = callback(tmp(4251).Text, obj7);
    obj4[5] = items1;
    tmp7Result = tmp7(tmp(4797).PressableOpacity, obj4);
  }
  return tmp7Result;
}
class ChatInputReplyBar {
  constructor(arg0) {
    ...
    f84075 = tmpResult4.useDisplayNameStylesFont({ displayNameStyles: tmp10 });
...
function ChatInputEditBar(onCancelEditing) {
  ...
  tmp7Result = tmp7(tmp(4797).PressableOpacity, obj);
