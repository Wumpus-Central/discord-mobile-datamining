...
import registerAssetDefault from "../../../../_runtime/08049_registerAsset.js";
...
function buildGameMentionResult(id) {
  ...
  const obj = {
    location: 0,
    length: callback2(id.name).length,
    icon: null,
    iconSize: 16,
    iconCornerRadius: 4,
    iconSpacing: 4,
...
export const serializeComposerTimestampMentions = function serializeComposerTimestampMentions(
  content,
  mentionTimestamps,
) {
...
export const getTimestampHighlightNodes = function getTimestampHighlightNodes(mentionTimestamps, content) {
  return findAllTimestampPillMatches(mentionTimestamps, content).map((location) => ({
    location: location.location,
    length: location.pillText.length,
  }));
};
export const getTimestampMentionInputNodes = function getTimestampMentionInputNodes(arr) {
  const TimestampAutocompleteMobileExperiment =
    require("../../timestamp_autocomplete/TimestampAutocompleteMobileExperiment.tsx").TimestampAutocompleteMobileExperiment;
  const items = [];
...
export const getGameMentionInputNodes = function getGameMentionInputNodes(arr) {
  const IncludeGameMentionsInAutocomplete =
    require("../../user_settings/UserSettings.tsx").IncludeGameMentionsInAutocomplete;
  const items = [];
