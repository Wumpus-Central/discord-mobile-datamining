...
function findAllTimestampPillMatches(size, text) {
  ...
  if (0 === size.size) {
    ...
    function _loop3(pillText) {
      ...
      const tmp = findTokenLocations(closure_0, pillText, (arg0) => items1.every((location) => length + length.length <= location.location || tmp >= location.location + location.pillText.length));
    }
