...
function findGameMentionTokens(text, name, items) {
  ...
  const found = items.filter((item, index) => item !== arr2);
  ...
  if (-1 !== index) {
    do {
      closure_0 = text;
      let someResult = mapped.some((item, index) => {
...
const EmojiIntention = set2.EmojiIntention;
...
const here = "here";
const result = obj132.fileFinishedImporting("modules/chat_input/native/ChatInputCommandOptionParser.tsx");
...
export const getTextBeforeFirstOption = (text) => {
  ...
  let substr = text;
  if (null != match) {
    substr = text.slice(0, regex.lastIndex - match[0].length);
  }
  obj[1] = substr;
  return obj;
};
...
export const getUsernameHighlightNodes = function getUsernameHighlightNodes(channel, arg1) {
  ...
  if (null != match) {
    do {
      let str = match[1];
      _require = str.trim();
      if (null != users.find((item, index) => callback(item.text) === callback)) {
        obj = { location: null, length: null };
        obj[0] = match.index;
        obj[1] = match[0].length;
        let arr = items.push(obj);
      }
      match = regex3.exec(arg1);
    } while (null != match);
  }
  _require = (dependencyMap) => dependencyMap.split("#")[0];
...
export const serializeComposerGameMentions = function serializeComposerGameMentions(result, mentionGames) {
  ...
  function _loop2() {
    ...
    arr2 = closure_1_11(name);
    const found = arr2.filter((item, index) => item !== arr2);
    const mapped = found.map(closure_1_11);
    items = [];
    ...
    closure_1 = closure_1_10(arr2.id);
    closure_1 = items.reduceRight((acc, item, index) => {
      const sum = acc.slice(0, item) + closure_1;
      return sum + acc.slice(item + arr2.length);
    }, closure_1);
  }
  ...
  if (config.enabled) {
    ...
    if (IncludeGameMentionsInAutocomplete.getSetting()) {
      if (!tmp4) {
        ...
        return items.map((item, index) => ({ location: item, length: length.length }));
      }
    }
  }
  return [];
...
export const getRoleHighlightNodes = (channel, arg1) => {
  const items = [];
  if (closure_7.can(Permissions.MENTION_EVERYONE, channel)) {
  ...
  function _loop4() {
    ...
    if (index < 0) {
      ...
      if ("\"" === arr[closure_3 + 1]) {
        ...
        if (null != closure_2.find((item, index) => item.text === closure_0)) {
          obj = { location: null, length: null };
          obj[0] = tmp2;
        ...
        closure_1_18.lastIndex = closure_3;
        const match = closure_1_18.exec(arr);
        if (null != match) {
          items = match[1].trim();
          if (null != closure_2.find((item, index) => item.text === closure_1)) {
            obj = { location: null, length: null };
            obj[0] = match.index;
            obj[1] = match[0].length;
            arr = items.push(obj);
