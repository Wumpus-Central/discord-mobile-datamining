...
function buildGameMentionResult(id) {
  ...
  const obj = {
    location: 0,
    length: callback(id.name).length,
    icon: null,
    iconSize: 16,
    iconCornerRadius: 4,
    iconSpacing: 4,
...
const EmojiIntention = set3.EmojiIntention;
let tmp3 =
...
const here = "here";
const result = set2.fileFinishedImporting("modules/chat_input/native/ChatInputCommandOptionParser.tsx");
...
export const getRoleHighlightNodes = (getGuildId) => {
  const items = [];
  if (closure_7.can(Permissions.MENTION_EVERYONE, getGuildId)) {
