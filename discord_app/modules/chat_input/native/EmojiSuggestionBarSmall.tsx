  ...
  obj = {
    ...
    children: displayEmojis.map((emoji, index) => {
      ...
      obj = {
        emoji: emoji.emoji,
        disabled: locked,
        onPressEmoji: locked ? closure_3 : closure_2,
      ...
      return suggestionBarHeight(
        displayEmojis(closure_1_2[5]).EmojiEntranceAnimation,
        obj,
        displayEmojis(closure_1_2[5]).getEmojiEntranceKey(displayEmojis, index),
      );
    }),
  };
...
let closure_7 = createCacheKey.createStyles((arg0) => {
  ...
  obj = {
    position: "absolute",
    top: diff - ThemesDefault.space.PX_8,
    right: ThemesDefault.modules.mobile.CHAT_INPUT_CONTAINER_HORIZONTAL_PADDING,
    height: closure_5,
    flexDirection: "row",
    alignItems: "center",
...
const forwardRefResult = importAllResult.forwardRef((anchorTop, ref) => {
  ...
  const item = lockedEmojis.useMemo(() => {
    const obj = {
      displayEmojis: anchorTop(unlockedEmojis[5]).sortEmojisForDisplay(unlockedEmojis, lockedEmojis.slice(0, 2), 3),
      reducedMotion,
      handlePress,
