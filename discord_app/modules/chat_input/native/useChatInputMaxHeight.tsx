// === Module 12031: useChatInputMaxHeight ===
...
import useKeyboardStateSharedValue from "useKeyboardStateSharedValue" /* 12032 */;
...
let obj = { getKeyboardStateWorklet: fn(12032).getKeyboardStateWorklet, KeyboardTypes: fn(1609).KeyboardTypes, getWindowDimensionsWorklet: fn(12033).getWindowDimensionsWorklet, MAX_HEIGHT: 200, MIN_HEIGHT: CHAT_INPUT_PILL_CONTENT_SIZE };
...
function getChatInputHeightAnimationTimingWorklet(height, textFieldMinHeight) {
  if (typeof getChatInputMaxHeightWorklet === "function") {
    ...
    let tmp2Result = useWindowDimensionsSharedValue;
    const _Math = Math;
    const _Math2 = Math;
    const bound = Math.min(tmp, Math.min(c6, Math.max(2 * CHAT_INPUT_PILL_CONTENT_SIZE, tmp2Result.getWindowDimensionsWorklet({ ignoreKeyboard: true }).height - keyboardHeight - c6)));
    tmp2Result = timing;
...
obj = { getChatInputMaxHeightWorklet, withTiming: fn(4561).withTiming, timingFastDuration: fn(4564).timingFastDuration, Easing: fn(4296).Easing };
  ...
  const effect = noop.useEffect(() => {
    function maybeUpdateMaxHeight() {
      closure_1((arg0) => {
        ...
        const bound = Math.min(closure_2_6, Math.max(2 * closure_2_7, closure_0(1477).getWindowDimensions({ ignoreKeyboard: true }).height - systemKeyboardHeight - closure_2_6));
        if (arg0 !== bound) {
          tmp6 = bound;
          if (closure_1_0 != null) {
            closure_1_0();
