...
import _modDef4722 from "../../actions/ChannelRTCActionCreators.tsx";
...
function navigateToTabLocation(found) {
  if ("route" === found.kind) {
    ...
    if (tmp) {
      _modDef4722.updateChatOpen(channelId, true);
      obj = _modDef4722;
    }
    if (null != guildId) {
...
function openChannelTabActive(closure_0, guildId) {
  ...
  if (0 === store3.getTabs().length) {
    ...
    if (tmp4) {
      _modDef4722.updateChatOpen(closure_0, true);
      const obj4 = _modDef4722;
    }
    if (null != guildId) {
  ...
  if (!store3.isAtMaxTabs()) {
    ...
    if (obj.getCurrentlySelectedChannelId() !== closure_0) {
      ...
      if (tmp18) {
        tmp15(4722).updateChatOpen(closure_0, true);
        ...
        if (sum < activeTab.entries.length) {
          if ("route" === activeTab.entries[sum].kind) {
            ...
            if (null == store.getChannel(tmp34.channelId)) {
              ...
              if (tmp8) {
                tmp35(4722).updateChatOpen(channelId, true);
