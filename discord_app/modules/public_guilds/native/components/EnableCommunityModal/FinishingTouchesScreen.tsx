...
({
  ...
  MODERATOR_PERMISSIONS: unpackModuleId,
...
export default function FinishingTouchesScreen() {
  ...
  const first1 = tmp9[0];
  [tmp12, tmp13] = callback(
    obj.useState(!closure_11.some((VIEW_CHANNEL) => closure_1_2(closure_1_3[11]).canEveryone(VIEW_CHANNEL, guild))),
    2,
  );
  ...
  const callback1 = obj.useCallback((features) => {
    ...
    if (null != everyoneRole) {
      ...
      ({
        safetyAlertsChannelId: obj6[2],
        verificationLevel: obj6[3],
        explicitContentFilter: obj6[4],
  ...
  obj = {
    headerRef: ref,
    currentStep: tmp2(17247).EnableCommunityModalSteps.STEP_3,
    onSuccess: callback1,
