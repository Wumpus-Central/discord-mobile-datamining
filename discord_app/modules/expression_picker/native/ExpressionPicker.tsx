...
let obj = { flex: 1, overflow: "hidden", backgroundColor: ThemesDefault.colors.MOBILE_EXPRESSION_PICKER_BACKGROUND_DEFAULT, position: "relative", paddingHorizontal: PADDING_HORIZONTAL };
obj[0] = obj;
obj[1] = { flex: 1 };
const createCacheKey = { paddingTop: 2 * PADDING_HORIZONTAL, paddingHorizontal: 0 };
...
const memoResult = importAllResult.memo(function ExpressionPicker(expressionType) {
  ...
  let obj = {
    ...
    items: prop.map((item, index) => ({ id: item, label: item, page: null }))
  };
  ...
  const effect1 = memo.useEffect(() => {
    if (ref.current) {
      ...
      ref(expressionPickerViewType[13]).trackWithMetadata(AnalyticEvents.EXPRESSION_PICKER_TAB_CLICKED, obj);
      const obj4 = ref(expressionPickerViewType[13]);
    } else if (expressionPickerViewType === closure_1_5.EMOJI) {
      obj = { intention: null };
      obj[0] = EmojiIntention.CHAT;
  ...
  if (expressionPickerViewType === constants.EMOJI) {
    ...
    let tmp17Result = callback(tmp2(tmp3[18]), obj6);
    ...
    obj7[5] = onPressGIF;
    tmp17Result = callback(tmp2(tmp3[19]), obj7);
  } else {
    tmp17Result = null;
    if (expressionPickerViewType === constants.STICKER) {
      ...
      tmp17Result = callback(tmp2(tmp3[20]), obj8);
    }
  }
  obj5[1] = tmp17Result;
...
let result = require("obj132").fileFinishedImporting("modules/expression_picker/native/ExpressionPicker.tsx");
