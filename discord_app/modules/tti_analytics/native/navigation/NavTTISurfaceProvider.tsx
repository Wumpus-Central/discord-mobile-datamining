...
function VisualizedNavTTISurfaceView(arg0) {
  ...
  const items = [
    React6(NavigationTTIRegionHierarchy.NavigationTTIRegionHierarchyContext.Provider, {
      value: navigationTTIRegionHierarchy.contextValue,
      children,
    }),
    React6(NavigationTTIRegionDebugOverlay.NavigationTTIRegionDebugOverlay, {
      name,
      regionId: navigationTTIRegionHierarchy.regionId,
      tracking: "exclude",
      descendantTracking,
      includedDescendants: navigationTTIRegionHierarchy.includedDescendants,
      excludedDescendants: navigationTTIRegionHierarchy.excludedDescendants,
      hierarchyDepth: navigationTTIRegionHierarchy.depth,
      violation: navigationTTIRegionHierarchy.violation,
    }),
  ];
  obj4.children = items;
...
export const NavTTISurfaceProvider = function NavTTISurfaceProvider(navigationKey) {
  ...
  const merged = Object.assign(
    navigationKey,
    Object.assign({ name: 0, navigationKey: 0, definition: 0, descendantTracking: 0, visibilityMode: 0, isVisible: 0 }),
  );
  isVisible = undefined;
  ...
  value = syncExternalStore.useMemo(
    () => ({ definition, navigationKey, activeTraceId: syncExternalStore, visibilityMode, isVisible }),
    items2,
  );
  ...
  if (tmp2Result.useStateFromStores(items3, () => DevSettingsStore.get("navigation_tti_visualizer"))) {
    ...
    let children = closure_8(VisualizedNavTTISurfaceView, obj2);
    let tmp16 = closure_8;
  } else {
    const obj3 = {};
    const merged1 = Object.assign(merged);
    obj3.measurementProps = {};
    children = closure_8(NavTTISurfaceView, obj3);
    tmp16 = closure_8;
  }
