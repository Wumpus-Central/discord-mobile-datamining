...
function ChannelLongPressActionSheetConnected(channel) {
  ...
  if (channel.isGroupDM()) {
    ...
    if (tmp26) {
      ...
      obj8.onPress = function onPress() {
        ...
        obj.pushLazy(asyncRequireImpl(10936, dependencyMap.paths), obj);
      };
      buttons4.push(obj8);
    }
  }
  ...
  if (!isPrivateResult) {
    ...
    if (!closure_12(channel.type)) {
      ...
      tmp82 = closure_9(channel.type) || channel.isCategory() || channel.isGuildStageVoice() || channel.isForumLikeChannel();
      if (!tmp85) {
      ...
      if (!tmp89) {
        if (channel.isGuildVocal()) {
          ...
          if (isGuildStageVoiceResult) {
            ...
            obj33.onPress = function onPress() {
              ...
              obj.hideActionSheet();
              obj = { channel };
              ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(10970, dependencyMap.paths), obj);
        ...
        if (canManageChannel) {
          ...
          tmp105 = closure_9(channel.type) || channel.isForumLikeChannel() || channel.isGuildVoice() || channel.isCategory();
        }
        if (setting) {
