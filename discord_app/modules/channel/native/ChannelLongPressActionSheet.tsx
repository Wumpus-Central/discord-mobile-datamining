        ...
        if (0 === c2) {
          if (arg0 === 1) {
            ...
            if (isGuildStageVoiceResult) {
              c2 = 2;
              c3 = 1;
              const obj4 = { value: tmp21(dependencyMap[21], dependencyMap.paths), done: false };
              return obj4;
            } else {
              c2 = 1;
              c3 = 1;
              const obj5 = { value: tmp21(dependencyMap[23], dependencyMap.paths), done: false };
              return obj5;
            }
...
function ChannelLongPressActionSheetConnected(channel) {
  ...
  const stateFromStoresObject = channel(guildId[89]).useStateFromStoresObject(items1, () => ({
  ...
  const stateFromStores1 = tmp5(tmp2[89]).useStateFromStores(items3, () =>
    UserGuildSettingsStore.isChannelMuted(guildId, channel.id),
  );
  const tmp5Result14 = tmp5(tmp2[89]);
  const items4 = [ReadStateStore];
  const stateFromStores2 = tmp5(tmp2[89]).useStateFromStores(items4, () =>
  ...
  const stateFromStores4 = tmp5(tmp2[89]).useStateFromStores(items6, () =>
  ...
  const stateFromStores5 = tmp5(tmp2[89]).useStateFromStores(items7, () =>
  ...
  const stateFromStores8 = tmp5(tmp2[89]).useStateFromStores(items10, () =>
  ...
  const stateFromStores9 = tmp5(tmp2[89]).useStateFromStores(items11, () =>
  ...
  if (channel.isGroupDM()) {
    if (!isFavoritesGuildSelected) {
      ...
      obj12.onPress = function onPress() {
        ...
        if (channel.isManaged()) {
          ...
          formatToPlainStringResult1 = intl3.formatToPlainString(util.t.hVGjEW, obj2);
          const intl4 = util.intl;
          const obj3 = { name: channelName };
          formatResult = intl4.format(util.t.IK1Qvs, obj3);
        }
        ...
        obj5.onConfirm = function onConfirm() {
          onClose(guildId[28]).closePrivateChannel(
            user.id,
    ...
    if (tmp27) {
      ...
      obj14.onPress = function onPress() {
        ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11201, dependencyMap.paths), { channelId: channel.id });
      };
      buttons4.push(obj14);
    }
  }
  ...
  if (tmp60) {
    ...
    obj22.onPress = function onPress() {
      router_utils.transitionTo(
        __initData10.CHANNEL(ActiveThreadsStore, StaticChannelRoute.VIBEGRATIONS, StageInstanceStore),
      );
    };
    const items16 = [obj22];
  ...
  if (canCreateInstantInvite) {
    ...
    obj24.onPress = function onPress() {
      const result = instant_invite_InstantInviteUtils.showInstantInviteActionSheet(channel, {
        source: constants6.CONTEXT_MENU,
      });
    };
    buttons5.push(obj24);
  }
    ...
    if (channel.isCategory()) {
      if (optInEnabledForGuild) {
        ...
        if (isParentOptedIn) {
          ...
          obj25.onPress = function onPress() {
            OptInChannelsActionCreators.setOptInChannel(ActiveThreadsStore, channel.id, !guildId, {
      ...
      const obj26 = {
        ...
        onPress() {
          OptInChannelsActionCreators.setIsFavorite(ActiveThreadsStore, channel.id, !setting, {
            section: constants3.CHANNEL_ACTION_SHEET,
          });
          if (!setting) {
  ...
  if (!isPrivateResult) {
    ...
    if (tmp83) {
      ...
      obj36.onPress = function onPress() {
        ReadStateActionCreators.ackChannel(channel, {
          section: constants3.CHANNEL_ACTION_SHEET,
      ...
      if (!tmp100) {
        if (channel.isGuildVocal()) {
          ...
          if (isGuildStageVoiceResult) {
            ...
            obj42.onPress = function onPress() {
              ...
              ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11237, dependencyMap.paths), { channel });
        ...
        obj53.children = items12.map((buttons) => {
          ...
          obj.children = buttons.map((onPress, index) => {
            ...
            return closure_1_35(
              channel(7446).ActionSheetRow,
              {
                variant: str,
                label,
                icon: closure_1_35(channel(7446).ActionSheetRow.Icon, {
                  IconComponent,
                  style: iconStyle,
                  disableColor,
                }),
        ...
        onPress() {
          ...
          if (tmp) {
            const obj2 = {
              ...
              initialRouteName: constants.MUTE,
              source: "channel-long-press-sheet",
            };
...
({
  ...
  AnalyticsObjects: closure_25,
  AnalyticsSections: closure_26,
  ChannelSettingsSections: closure_27,
  ChannelTypes: closure_28,
  InstantInviteSources: closure_29,
  NULL_STRING_GUILD_ID: closure_30,
  Permissions: items,
  Routes: closure_32,
  ZERO_STRING_GUILD_ID: closure_33,
} = Constants);
let closure_34 = fn(11186).ChannelDetailsNavigatorScreens;
const jsx = fn(21).jsx;
const size = fn(2);
