...
let closure_6 = {
  [fn(10853).StorefrontPlatform.DESKTOP]: "PC",
  [fn(10853).StorefrontPlatform.XBOX]: "Xbox",
  [fn(10853).StorefrontPlatform.PLAYSTATION]: "PlayStation",
  [fn(10853).StorefrontPlatform.SWITCH]: "Switch",
  [fn(10853).StorefrontPlatform.APPLE_ARCADE]: "Apple Arcade",
  [fn(10853).StorefrontPlatform.NETFLIX]: "Netflix",
  [fn(10853).StorefrontPlatform.AMAZON_KIDS_PLUS]: "Amazon Kids+",
};
...
export const getFinePrintMessageForApplication = function getFinePrintMessageForApplication(
  ...
  if (shouldAppendDisclaimer.shouldAppendDisclaimer) {
    ...
    if (0 !== arr.length) {
      ...
      obj.platforms = listFormat.format(arr.map((item) => closure_1_6[item]));
      obj.count = arr.length;
      str2 = intl3.formatToPlainString(_modDef3430["5h8p5P"], obj);
    }
    const obj1 = { platforms_info: str2 };
    obj.platforms_info = intl2.format(_modDef3430.Q0dHYO, obj1);
    let formatResult = format(_modDef3430["3ah/a2"], obj);
  } else {
...
export const getMobileFinePrintMessageForApplication = function getMobileFinePrintMessageForApplication(
  ...
  if (shouldAppendDisclaimer.shouldAppendDisclaimer) {
    ...
    const intl2 = util.intl;
    items[1] = intl2.format(_modDef3430.Ufm9XX, obj);
    let items1 = items;
  } else {
...
export const getRedeemPurchaseDescriptionForApplication = function getRedeemPurchaseDescriptionForApplication(
  ...
  if (0 !== arr.length) {
    ...
    obj.platforms = listFormat.format(arr.map((item) => closure_1_6[item]));
    obj.count = arr.length;
    str = intl2.formatToPlainString(_modDef3430["5h8p5P"], obj);
  }
...
export const getGiftLinkAccountDescriptionForApplication = function getGiftLinkAccountDescriptionForApplication(
  ...
  if (0 !== arr.length) {
    ...
    obj.platforms = listFormat.format(arr.map((item) => closure_1_6[item]));
    obj.count = arr.length;
    str = intl2.formatToPlainString(tmp4(3430)["5h8p5P"], obj);
  }
  obj.platforms_info = str;
