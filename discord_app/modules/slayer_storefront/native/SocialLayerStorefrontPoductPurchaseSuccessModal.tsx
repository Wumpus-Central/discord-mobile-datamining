...
import { SOCIAL_LAYER_DAYS_TO_CLAIM_ITEM as closure_11 } from "../SocialLayerStorefrontConstants.tsx";
...
function PurchaseSuccessModalBase(sku) {
  ...
  callback1 = obj3.useCallback(() => {
    ...
    if (null != first) {
      if (0 === substr.length) {
        const result = stateFromStores(callback1[14]).triggerHapticFeedback(
          stateFromStores(callback1[14]).HapticFeedbackTypes.IMPACT_HEAVY,
        );
    ...
    if (substr.length >= closure_1_22.length / 2) {
      const result1 = stateFromStores(callback1[14]).triggerHapticFeedback(
        stateFromStores(callback1[14]).HapticFeedbackTypes.IMPACT_LIGHT,
      );
      const obj2 = stateFromStores(callback1[14]);
    } else {
      const result2 = stateFromStores(callback1[14]).triggerHapticFeedback(
        stateFromStores(callback1[14]).HapticFeedbackTypes.IMPACT_MEDIUM,
      );
  ...
  const items9 = [
    closure_15(stateFromStores(callback1[19]).Text, {
  ...
  items9[1] = closure_15(stateFromStores(callback1[19]).Text, {
  ...
  const obj13 = {
    onPress: onClose,
    backImage() {
      return closure_1_15(stateFromStores(callback1[25]).XSmallIcon, {
        size: "lg",
        style: closeButtonIcon.closeButtonIcon,
      });
    },
...
let closure_19 = {
  code: "function SocialLayerStorefrontPoductPurchaseSuccessModalTsx1(){const{interpolate,springInput}=this.__closure;return{opacity:interpolate(springInput.get(),[0,1],[0.1,1]),transform:[{scale:interpolate(springInput.get(),[0,1],[0,1])}]};}",
};
let closure_20 = {
  code: "function SocialLayerStorefrontPoductPurchaseSuccessModalTsx2(){const{interpolate,springInput}=this.__closure;return{opacity:interpolate(springInput.get(),[0,1],[0,1]),transform:[{scale:interpolate(springInput.get(),[0,1],[0.75,1])}]};}",
};
let closure_21 = {
  code: "function SocialLayerStorefrontPoductPurchaseSuccessModalTsx3(){const{interpolate,linearInput}=this.__closure;return{opacity:interpolate(linearInput.get(),[0,1],[0.5,0])};}",
};
...
export const SocialLayerStorefrontProductSelfPurchaseSuccessModal =
  function SocialLayerStorefrontProductSelfPurchaseSuccessModal(skuId) {
    ...
    analyticsLocations(stateFromStores[32])(() => {
      ...
      obj = {
        location_stack: memo,
        type: skuId(stateFromStores[33]).SOCIAL_LAYER_STOREFRONT_SELF_PURCHASE_SUCCESS_MODAL_KEY,
        sku_id: skuId,
        application_id: null,
      };
      let applicationId;
...
export const SocialLayerStorefrontProductGiftPurchaseSuccessModal =
  function SocialLayerStorefrontProductGiftPurchaseSuccessModal(analyticsLocations) {
    ...
    recipient(analyticsLocations[32])(() => {
      ...
      obj = {
        location_stack: closure_4,
        type: closure_1_0(analyticsLocations[33]).SOCIAL_LAYER_STOREFRONT_GIFT_PURCHASE_SUCCESS_MODAL_KEY,
        sku_id: closure_0,
        application_id: null,
      };
      let applicationId;
