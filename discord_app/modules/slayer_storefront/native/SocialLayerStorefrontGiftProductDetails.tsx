...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  let obj1 = getOrFetchApplication(10930);
  ...
  if (tmp6Result) {
    ...
    if (tmp8Result) {
      ...
      tmp8Result = closure_7(closure_4, obj3);
    }
  ...
  const items3 = [
    tmp6Result,
    closure_7(getOrFetchApplication(4632).Text, { variant: "text-md/semibold", children: sku.name }),
  ];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = closure_7(tmp2(4632).Text, obj7);
  }
  items1[2] = tmp8Result;
  obj.children = items1;
