...
const require = fn;
let result = require("obj132").fileFinishedImporting("modules/creator_monetization_eligibility/guild_settings/useCreatorMonetizationEligibilityItems.tsx");
...
export default function useCreatorMonetizationEligibilityItems(arg0) {
  ...
  const callback = isUserMFAEnabled.useCallback(sortedByIneligible(function*() {
    if (c2 === 2) {
      c2 = 3;
  ...
  const enableMFAHook = _require(actions[4]).useEnableMFAHook({ onEnableMFAClick: callback });
  ...
  return isUserMFAEnabled.useMemo(() => {
    if (null == callback) {
      ...
      ({ minimumOwnerAgeInYears, minimumSize, noRecentViolations } = callback);
      ...
      obj[4] = callback.noRecentViolations;
      let stringResult;
      if (!noRecentViolations) {
        const intl = callback(actions[5]).intl;
        stringResult = intl.string(callback(actions[5]).t["xU2fl+"]);
      }
      obj[5] = stringResult;
      let fn;
      if (!noRecentViolations) {
      ...
      if (tmp3) {
        ...
        const intl2 = callback(actions[5]).intl;
        obj[1] = intl2.string(callback(actions[5]).t["+F8haD"]);
        const intl3 = callback(actions[5]).intl;
        obj[2] = intl3.string(callback(actions[5]).t["5BwC/O"]);
        const intl4 = callback(actions[5]).intl;
        obj1 = { minimumOwnerAgeInYears: null };
        obj1[0] = minimumOwnerAgeInYears;
        obj[3] = intl4.formatToPlainString(callback(actions[5]).t.DW1Vae, obj1);
        obj[4] = callback.meetsOwnerAgeRequirement;
        items.push(obj);
      }
      if (tmp5) {
        const obj2 = { key: "member_count_requirement", checkedLabel: null, uncheckedLabel: null, description: null, checked: null };
        const intl5 = callback(actions[5]).intl;
        obj2[1] = intl5.string(callback(actions[5]).t.j7wXWo);
        const intl6 = callback(actions[5]).intl;
        obj2[2] = intl6.string(callback(actions[5]).t.W0suNz);
        const intl7 = callback(actions[5]).intl;
        const obj3 = { minimumSize: null };
        obj3[0] = minimumSize;
        obj2[3] = intl7.formatToPlainString(callback(actions[5]).t.up53zR, obj3);
        obj2[4] = callback.hasSufficientMembers;
        items.push(obj2);
      }
      if (tmp7) {
        const obj4 = { key: "server_age_requirement", checkedLabel: null, uncheckedLabel: null, description: null, checked: null };
        const intl8 = callback(actions[5]).intl;
        obj4[1] = intl8.string(callback(actions[5]).t.mjbvWw);
        const intl9 = callback(actions[5]).intl;
        obj4[2] = intl9.string(callback(actions[5]).t["9BV6L6"]);
        const intl10 = callback(actions[5]).intl;
        const obj5 = { minimumAge: null };
        obj5[0] = onEligibilityBecameStale(actions[8])(callback.minimumAgeInDays);
        obj4[3] = intl10.formatToPlainString(callback(actions[5]).t.Zwv84O, obj5);
        obj4[4] = callback.meetsServerAgeRequirement;
        items.push(obj4);
      }
      if (null != callback.weeklyCommunicators) {
        const obj6 = { key: "weekly_communicator_count_requirement", checkedLabel: null, uncheckedLabel: null, description: null, checked: null };
        const intl21 = callback(actions[5]).intl;
        obj6[1] = intl21.string(callback(actions[5]).t.Qw7qv4);
        const intl22 = callback(actions[5]).intl;
        obj6[2] = intl22.string(callback(actions[5]).t.b45kGG);
        const intl23 = callback(actions[5]).intl;
        obj6[3] = intl23.string(callback(actions[5]).t.NbtjEC);
        obj6[4] = callback.weeklyCommunicators;
        items.push(obj6);
      }
      if (null != callback.hasMemberRetention) {
        const obj7 = { key: "member_retention_requirement", checkedLabel: null, uncheckedLabel: null, description: null, checked: null };
        const intl24 = callback(actions[5]).intl;
        obj7[1] = intl24.string(callback(actions[5]).t.Qvq39M);
        const intl25 = callback(actions[5]).intl;
        obj7[2] = intl25.string(callback(actions[5]).t.azHboI);
        const intl26 = callback(actions[5]).intl;
        obj7[3] = intl26.string(callback(actions[5]).t.u4rCYO);
        obj7[4] = callback.hasMemberRetention;
        items.push(obj7);
      }
      const obj8 = { key: "nsfw_requirement", checkedLabel: null, uncheckedLabel: null, description: null, checked: null };
      const intl11 = callback(actions[5]).intl;
      obj8[1] = intl11.string(callback(actions[5]).t.bymfTb);
      const intl12 = callback(actions[5]).intl;
      obj8[2] = intl12.string(callback(actions[5]).t["718pRA"]);
      const intl13 = callback(actions[5]).intl;
      obj8[3] = intl13.string(callback(actions[5]).t["5ZqX+j"]);
      obj8[4] = callback.notNSFW;
      items.push(obj8);
      if (null != callback.hasEnabled2FA) {
        const hasEnabled2FA2 = callback.hasEnabled2FA;
        ...
        const hasEnabled2FA = callback.hasEnabled2FA;
        ...
        const intl14 = callback(actions[5]).intl;
        obj9[1] = intl14.string(callback(actions[5]).t.NqVyFk);
        const intl15 = callback(actions[5]).intl;
        obj9[2] = intl15.string(callback(actions[5]).t.VcDNIV);
        const intl16 = callback(actions[5]).intl;
        const obj10 = { enableMFAHook: null };
        obj10[0] = enableMFAHook;
        obj9[3] = intl16.format(callback(actions[5]).t["7NzkfV"], obj10);
        obj9[4] = callback.hasEnabled2FA;
        let stringResult1;
        if (tmp11) {
          const intl17 = callback(actions[5]).intl;
          stringResult1 = intl17.string(callback(actions[5]).t.BU4Diu);
        }
      ...
      if (true === sortedByIneligible) {
        const sorted = items.sort((checked, arg1) => {
          let num = -1;
          if (checked.checked) {
            num = 0;
          }
