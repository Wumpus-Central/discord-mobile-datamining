// === Module 18439: LocalPushNotificationActionCreators ===
...
import Constants2 from "Constants" /* 9394 */;
...
export const receiveLocalNotification = function receiveLocalNotification(getData) {
  if (null != getData.getData) {
    data(7803).trackAppOpened("notification");
    ...
    function dispatch() {
      ...
      if (constants.GUILD_VERIFICATION === type) {
        ...
        data(1980)(13266, dependencyMap.paths).then((result) => result.default(channelId.channelId));
        const promise2 = data(1980)(13266, dependencyMap.paths);
      } else if (constants.MESSAGE_SEND_FAILED === type) {
        data(1980)(4838, dependencyMap.paths).then((transitionToMessage) => {
          ({ channelId, messageId } = closure_1_0);
          return transitionToMessage.transitionToMessage(channelId, messageId, { jumpType: data(4757).JumpType.INSTANT });
        });
        const promise = data(1980)(4838, dependencyMap.paths);
