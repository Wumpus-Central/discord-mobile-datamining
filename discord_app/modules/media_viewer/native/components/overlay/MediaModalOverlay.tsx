...
import useVideoControls from "useVideoControls" /* 9000 */;
...
function OverlayMuteButton(arg0) {
  ...
  obj = { left: true, right: true, children: importDefault(isMuted ? 12052 : 12053) };
  obj[3] = ThemesDefault.unsafe_rawColors.WHITE;
  obj[4] = onToggleMute;
  ({ overlayButton: obj[5], overlayButtonIcon: obj[6] } = tmp);
  obj[2] = callback(renderDefault, obj);
...
function OverlayObscureToggleButton(arg0) {
  ...
  obj = { left: true, right: true, children: importDefault(spoilerActive ? 12054 : 12055) };
  obj[3] = ThemesDefault.unsafe_rawColors.WHITE;
...
function MediaModalOverlay(getVideoControls) {
  ...
  const callback = importAllResult.useCallback(() => index(dependencyMap[25]).toggleSpoiler(index), items);
  const effect = importAllResult.useEffect(() => {
    const result = index(dependencyMap[22]).setVideoStateControls(videoControls);
  }, items1);
  const obj = {};
...
let closure_13 = importAllResult.memo((arg0) => {
  ...
  if (null != slider) {
    ...
    items[2] = tmp16;
    obj[4] = items;
    let tmp21Result2 = callback2(SafeAreaPaddingView.SafeAreaPaddingView, obj);
  } else {
    tmp21Result2 = null;
  }
  tmp2Result = MediaViewerDimensionsProvider;
  ...
  if (!tmp7) {
    ...
    tmp21Result = callback2(_modDef12063, obj4);
  }
  ...
  if (null != slider) {
    ...
    if (tmp21Result) {
      ...
      obj6[1] = useVideoControls.toggleMuted;
      tmp21Result = callback2(OverlayMuteButton, obj6);
    }
    ...
    if (obscure) {
      ...
      tmp21Result1 = callback2(OverlayObscureToggleButton, obj7);
    }
    items4[1] = tmp21Result1;
    obj5[1] = items4;
    let tmp19Result = callback2(View, obj5);
  } else {
    tmp19Result = null;
  }
  ...
  if (disableMediaOverlayFooter) {
    ...
    tmp21Result2 = callback2(SafeAreaPaddingView.SafeAreaPaddingView, obj8);
    ...
    tmp21Result2 = callback2(clamp.MediaModalOverlayFooter, obj9);
  }
  const obj10 = { children: null };
  items3[2] = tmp21Result2;
  obj3[1] = items3;
  items1[1] = callback2(_modDef4115.View, obj3);
  obj10[0] = items1;
  return callback2(closure_9, obj10);
});
let result = require("obj132").fileFinishedImporting("modules/media_viewer/native/components/overlay/MediaModalOverlay.tsx");
