...
import MediaViewerOverlayButtonFavoriteGIFDefault from "MediaViewerOverlayButtonFavoriteGIF" /* 13097 */;
...
export const MediaModalOverlayHeader = function MediaModalOverlayHeader(arg0) {
  ...
  if (tmp10Result3) {
    ...
    if (tmp10Result) {
      const obj14 = { source: contextIcon, color: nativeDefault.unsafe_rawColors.PRIMARY_345, size: tmp2(1176).Icon.Sizes.CUSTOM, style: tmp.contextIcon };
    ...
    if (!disableMediaOverlayButton) {
      const obj19 = {
        ...
        children(ref) {
              ...
              if (obj2.isAndroid()) {
                let tmp2Result = closure_1_6(isModalOpen(13098).MoreVerticalIcon, { size: "md", color: "interactive-text-active" });
              } else {
                tmp2Result = closure_1_6(isModalOpen(8795).MoreHorizontalIcon, { size: "md", color: "interactive-text-active" });
              }
              obj.icon = tmp2Result;
              obj.ref = ref.ref;
