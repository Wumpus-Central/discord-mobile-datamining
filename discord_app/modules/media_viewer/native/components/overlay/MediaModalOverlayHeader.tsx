...
import _modDef12060 from "MediaViewerOverlayButtonFavoriteGIF.tsx";
...
const result = require("obj132").fileFinishedImporting("modules/media_viewer/native/components/overlay/MediaModalOverlayHeader.tsx");
...
export const MediaModalOverlayHeader = function MediaModalOverlayHeader(arg0) {
  ...
  const fn = function w() {
    let obj = isModalOpen(dependencyMap[10]);
    let num = 1;
    if (isModalOpen) {
      num = 0;
    }
  ...
  obj5[1] = callback(isModalOpen(6368).XSmallIcon, { size: "md", color: "interactive-text-active" });
  obj5[2] = onClose;
  const items1 = [callback(_modDef9075, obj5), ];
  ...
  if (tmp10Result) {
    ...
    if (tmp10Result) {
      ...
      obj8[0] = contextIcon;
      obj8[1] = ThemesDefault.unsafe_rawColors.PRIMARY_345;
      obj8[2] = tmp2(1297).Icon.Sizes.CUSTOM;
      obj8[3] = tmp.contextIcon;
      tmp10Result = callback(tmp2(1297).Icon, obj8);
    }
    ...
    obj10[5] = contextName;
    obj9[1] = callback(tmp2(4734).Text, obj10);
    items2[1] = callback(closure_4, obj9);
    obj7[1] = items2;
    obj6[1] = callback2(closure_4, obj7);
    tmp10Result = callback(closure_4, obj6);
  }
  items1[1] = tmp10Result;
  obj4[1] = items1;
  const items3 = [callback2(closure_4, obj4), ];
  ...
  if (!tmp7) {
    ...
    obj12[0] = source;
    const items4 = [callback(_modDef12060, obj12), ];
    ...
    if (!disableMediaOverlayButton) {
      ...
      obj13[1] = function children(ref) {
        ...
        if (obj2.isAndroid()) {
          let tmp2Result = callback(isModalOpen(12061).MoreVerticalIcon, { size: "md", color: "interactive-text-active" });
        } else {
          tmp2Result = callback(isModalOpen(9415).MoreHorizontalIcon, { size: "md", color: "interactive-text-active" });
        }
        obj[1] = tmp2Result;
        obj[2] = ref.ref;
      ...
      tmp10Result1 = callback(tmp2(9958).ContextMenu, obj13);
    }
    items4[1] = tmp10Result1;
    obj11[1] = items4;
    tmp12Result = callback2(closure_4, obj11);
  }
  items3[1] = tmp12Result;
  obj3[1] = items3;
  obj2[2] = callback2(isModalOpen(9074).MediaModalOverlayHeaderWrapper, obj3);
  obj[1] = callback(_modDef4115.View, obj2);
