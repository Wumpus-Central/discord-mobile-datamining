...
import PlayerState from "MediaModalWebView.tsx";
...
const memoResult = importAllResult.memo((style) => {
  ...
  callback = obj.useCallback((arg0) => {
    let trackMessageEmbedsActionCompleted = ((arg0) => {
      try {
        const _JSON = JSON;
    ...
    ({ type, value } = trackMessageEmbedsActionCompleted);
    switch (type) {
      case "t":
      break;
      case "to":
      ...
      break;
      case "Set":
        const result = trackMessageEmbedsActionCompleted({ platform: "file", action: "errored", error: "unknown" });
      break;
      case "format":
      break;
      ...
      break;
      case "String":
        trackMessageEmbedsActionCompleted = type.MediaViewerAnalytics.trackMessageEmbedsActionCompleted;
      break;
      case "Symbol":
      break;
      case "Text":
...
let result = require("obj132").fileFinishedImporting("modules/media_viewer/native/components/MediaModalWebVideoFile.tsx");
...
export const createWebFileVideoControls = function createWebFileVideoControls() {
  ...
  const obj = {
    ...
    onPlayerStateChange(arg0) {
      if (closure_1 != null) {
        tmp(arg0 === PlayerState.PlayerState.PAUSED || arg0 === PlayerState.PlayerState.ENDED);
      }
    },
