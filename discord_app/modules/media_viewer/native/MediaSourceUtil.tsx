// === Module 8500: isValidImageAttachment ===
...
import getInitialParserStateDefault from "getInitialParserState" /* 7564 */;
import frozen from "frozen" /* 7788 */;
import transformToRowGeneratedComponent from "transformToRowGeneratedComponent" /* 7951 */;
import MediaGalleryItemType from "MediaGalleryItemType" /* 7967 */;
import mapDefault from "map" /* 8501 */;
import MediaPlayerManager from "MediaPlayerManager" /* 8503 */;
        ...
        if (width.height > 0) {
          ...
          if (null != width.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              if (!hasFlagResult1) {
                hasFlagResult1 = tmp43(6126).isChannelSpoilerGated(store.getChannel(closure_0.channel_id));
                const tmp43Result2 = tmp43(6126);
              }
              ...
              const formatted = tmp(8058)(width).toLowerCase();
              const str3 = tmp(8058)(width);
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(8058)(width).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(8058)(width);
...
function toMediaSourceFromUnfurledMedia(id, guild_id, media, description, spoiler) {
  ...
  if (unfurledMediaItemType === MediaGalleryItemType.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    ...
    const tmp4 = unfurledMediaItemType === tmp(7967).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
