// === Module 12223: shouldShowInviter ===
...
import InviteTypes from "InviteTypes" /* 7448 */;
import getGuildBadgeSourceDefault from "getGuildBadgeSource" /* 7641 */;
import renderDefault from "render" /* 12164 */;
import InviteRolesListDefault from "InviteRolesList" /* 12225 */;
...
import { INVITE_ROUTING_HUB_GUILD_ID as closure_11 } from "HubEmailConnectionSteps" /* 12224 */;
...
function InviteDestinationIcon(invite) {
  ...
  if (obj2.isGroupDMInvite(invite)) {
    if (null != invite.inviter) {
      let tmp2Result = null;
      if (null != invite.inviter) {
        obj = { avatarStyle: null, user: null, guildId: "Array", size: "<string:285213222>" };
        obj[0] = tmp.avatar;
...
function InviteHeader(invite) {
  ...
  if (null != name) {
    ...
    obj2[3] = name;
    items[1] = callback2(tmp2(4444).Heading, obj2);
    obj[1] = items;
...
function InviteJoinContext(invite) {
  ...
  if (obj.isFriendInvite(invite)) {
    ...
    obj = { username: null };
    obj[0] = isGuildMember(4292).getFormattedName(invite.inviter);
    let formatResult = intl7.format(tmp3(1236).t.Quj7HX, obj);
    const obj11 = isGuildMember(4292);
  } else {
    let tmp3Result = tmp3(7448);
...
export default function InviteDetails(invite) {
  ...
  if (tmp8Result) {
    ...
    const items3 = [tmp10(tmp4(9702).GuildEventCardHeader, obj5), , ];
    const obj6 = { event: null };
    obj6[0] = guild_scheduled_event;
    items3[1] = tmp10(tmp4(9702).GuildEventCardMetaInfo, obj6);
    const obj7 = { event: null };
    obj7[0] = guild_scheduled_event;
    items3[2] = tmp10(tmp4(9702).GuildEventCardGuildInfo, obj7);
    obj4[1] = items3;
  ...
  if (null != userAvatarSource) {
    ...
    tmp10Result = tmp10(tmp4(4896).Button.Icon, obj8);
  }
  if (null != stateFromStores) {
  ...
  const items4 = [callback2(invite(4896).Button, { icon: tmp10Result, variant: "primary", size: "lg", text: tmp24, accessibilityLabel: stringResult, onPress: handleAcceptInvitePress, loading: tmp3 === constants2.ACCEPT, disabled: tmp3 === constants2.ACCEPT }), ];
  ...
  items4[1] = callback2(invite(4896).Button, {
    variant: "secondary",
    size: "lg",
    text: stringResult1,
  ...
  obj12[0] = items4;
  items2[6] = closure_13(invite(5344).ButtonGroup, obj12);
  obj11[0] = items2;
