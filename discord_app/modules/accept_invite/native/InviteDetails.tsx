...
function InviteDestinationIcon(invite) {
  ...
  if (obj2.isGroupDMInvite(invite)) {
    if (null != invite.inviter) {
      let tmp2Result = null;
      if (null != invite.inviter) {
        obj = { avatarStyle: null, user: null, guildId: "Array", size: "<string:778043393>" };
        obj[0] = tmp.avatar;
...
function InviteJoinContext(invite) {
  ...
  if (obj.isFriendInvite(invite)) {
    ...
    obj = { username: null };
    obj[0] = isGuildMember(4123).getFormattedName(invite.inviter);
    let formatResult = intl7.format(tmp3(1236).t.Quj7HX, obj);
    const obj11 = isGuildMember(4123);
  } else {
    let tmp3Result = tmp3(7116);
  ...
  if (null != memo) {
    ...
    if (!tmp3Result1.isFriendInvite(invite)) {
      ...
      obj4[1] = callback2(isGuildMember(5251), obj5);
      tmp22 = callback2(tmp21, obj4);
    }
    tmp3Result1 = tmp3(7116);
  }
  const items1 = [tmp22, ];
...
export default function InviteDetails(invite) {
  ...
  if (tmp8Result) {
    ...
    const items3 = [tmp10(tmp4(8880).GuildEventCardHeader, obj5), , ];
    const obj6 = { event: null };
    obj6[0] = guild_scheduled_event;
    items3[1] = tmp10(tmp4(8880).GuildEventCardMetaInfo, obj6);
    const obj7 = { event: null };
    obj7[0] = guild_scheduled_event;
    items3[2] = tmp10(tmp4(8880).GuildEventCardGuildInfo, obj7);
    obj4[1] = items3;
  ...
  obj12[0] = items4;
  items2[6] = closure_13(invite(5132).ButtonGroup, obj12);
  obj11[0] = items2;
