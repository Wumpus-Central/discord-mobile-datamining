...
import InviteRolesListDefault from "InviteRolesList" /* 13068 */;
...
function InviteJoinContext(invite) {
  ...
  if (obj.isFriendInvite(invite)) {
    ...
    let obj2 = { username: isGuildMember(4673).getFormattedName(invite.inviter) };
    let formatResult = intl7.format(tmp3(1115).t.Quj7HX, obj2);
    const obj11 = isGuildMember(4673);
  } else {
    if (tmp3Result.isGroupDMInvite(invite)) {
      if (null != invite.channel) {
        if (null != invite.inviter) {
          const intl6 = tmp3(1115).intl;
          let obj3 = { username: isGuildMember(4673).getFormattedName(invite.inviter) };
          let formatResult1 = intl6.format(tmp3(1115).t.Lu4h18, obj3);
          const obj9 = isGuildMember(4673);
        }
      ...
      if (tmp3Result3.isStreamInvite(invite)) {
        if (null != invite.target_user) {
          ...
          const obj4 = { username: isGuildMember(4673).getFormattedName(invite.target_user) };
          formatResult = intl4.formatToPlainString(tmp3(1115).t.x2L32Q, obj4);
          const obj7 = isGuildMember(4673);
        }
      }
      ...
      if (tmp7) {
        ...
        if (tmp10) {
          ...
          const obj6 = { username: isGuildMember(4673).getFormattedName(invite.inviter) };
          formatResult = intl2.format(tmp3(1115).t.spU2mI, obj6);
          const obj5 = isGuildMember(4673);
        }
      }
      tmp3Result3 = tmp3(8064);
    }
    tmp3Result = tmp3(8064);
  }
  ...
  if (null != memo) {
    ...
    if (!tmp3Result4.isFriendInvite(invite)) {
      ...
      obj10.children = closure_12(isGuildMember(5892), obj12);
      tmp22 = closure_12(View, obj10);
    }
    tmp3Result4 = tmp3(8064);
  }
...
let closure_11 = fn(13067).INVITE_ROUTING_HUB_GUILD_ID;
const jsxProd = fn(21);
...
obj2.featureIcon = { flexGrow: 0, marginRight: 8, opacity: fn(5746).DARK_1_LIGHT_04 };
...
let obj4 = { flexGrow: 0, marginRight: 8, opacity: fn(5746).DARK_1_LIGHT_04 };
...
export default function InviteDetails(invite) {
  ...
  if (tmp8Result) {
    ...
    const items3 = [closure_12(tmp4(9953).GuildEventCardHeader, obj10), , ];
    const obj11 = { event: guild_scheduled_event };
    items3[1] = closure_12(tmp4(9953).GuildEventCardMetaInfo, obj11);
    const obj12 = { event: guild_scheduled_event };
    items3[2] = closure_12(tmp4(9953).GuildEventCardGuildInfo, obj12);
  ...
  if (null != userAvatarSource) {
    ...
    tmp10Result = closure_12(tmp4(5273).Button.Icon, obj13);
  }
  if (null != stateFromStores) {
  ...
  const items4 = [closure_12(invite(5273).Button, { icon: tmp10Result, variant: "primary", size: "lg", text: tmp24, accessibilityLabel: stringResult, onPress: handleAcceptInvitePress, loading: tmp3 === constants3.ACCEPT, disabled: tmp3 === constants3.ACCEPT }), ];
  ...
  items4[1] = closure_12(invite(5273).Button, {
    variant: "secondary",
    size: "lg",
    text: stringResult1,
  ...
  items2[6] = closure_13(invite(5738).ButtonGroup, obj17);
