...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const stateFromStoresObject = obj1.useStateFromStoresObject(items1, () => ({
    isMuted: JoinedThreadsStore.isMuted(channel.id),
    hasJoined: JoinedThreadsStore.hasJoined(channel.id),
  }));
  ...
  const stateFromStores3 = obj9.useStateFromStores(
    items4,
    () => !_modDef12.isEmpty(VoiceStateStore.getVoiceStatesForChannel(channel.id)),
  );
  ...
  if (null != stateFromStores) {
    ...
    let tmp19 = closure_13(tmp13(5598), obj);
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj2.IconComponent = tmp(10276).ChatMarkUnreadIcon;
        ...
        if (null == tmp23) {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj9.IconComponent = tmp(10280).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            obj10.IconComponent = tmp(4517).LinkIcon;
            ...
            if (isMuted) {
              ...
              obj12.IconComponent = tmp(9808).BellIcon;
              ...
              obj12[1] = tmp(9091).BellSlashIcon;
              ...
              obj13.IconComponent = tmp(10993).ChannelNotificationIcon;
              ...
              obj14.header = tmp18(tmp(11030).ActionSheetIconHeader, obj15);
              const items8 = [
                items5.map((buttons) => {
                  ...
                  obj.children = buttons.map((onPress, index) => {
                    ...
                    obj.variant = str;
                    obj.icon = closure_1_13(channel(dependencyMap[45]).ActionSheetRow.Icon, {
                      IconComponent,
                      disableColor,
                    });
              ...
              if (setting) {
                ...
                const obj18 = { IconComponent: tmp(10661).IdIcon };
                obj17.icon = tmp18(tmp(7213).ActionSheetRow.Icon, obj18);
                ...
                obj16.children = tmp18(tmp(7213).ActionSheetRow, obj17);
                setting = tmp18(tmp(7213).ActionSheetRow.Group, obj16, "developer-actions");
              }
              items8[1] = setting;
              obj14.children = items8;
              return closure_14(tmp(7211).ActionSheet, obj14);
            }
          } else {
          ...
          const obj20 = { label: string3Result, IconComponent: tmp(7894).PhoneCallIcon, onPress: tmp23 };
  ...
  obj21.IconComponent = tmp(6984).EyeIcon;
  obj21.onPress = function onPress() {
    const obj = {
      section: constants3.THREAD_ACTION_SHEET,
      object: constants2.MARK_THREAD_AS_READ_BUTTON,
      objectType: constants.ACK_MANUAL,
    };
    obj.ack(channel.id, obj, true, true);
  };
  buttons10.push(obj21);
  tmp17 = onClose(11006)(channel, "ThreadLongPressActionSheet");
}
