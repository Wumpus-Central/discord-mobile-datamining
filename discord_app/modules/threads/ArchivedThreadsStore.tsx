...
function getSortValue(id, sortOrder) {
  if (sortOrder === set.ThreadSortOrder.LATEST_ACTIVITY) {
...
function resortListState(value) {
  ...
  if (null == id) {
    c4 = null;
    const found = applyDefault(store.getAllThreadsForParent(value.channelId)).filter((item, index) => item.isArchivedThread());
    const found1 = found.filter((item, index) => {
      if (0 !== size.size) {
        if (sortOrder(dependencyMap[8]).ThreadSearchTagSetting.MATCH_SOME === closure_2) {
          const appliedTags2 = item.appliedTags;
          let someResult;
          if (appliedTags2 != null) {
            someResult = appliedTags2.some((item, index) => set.has(item));
          }
          if (true !== someResult) {
            return false;
          }
        } else if (sortOrder(dependencyMap[8]).ThreadSearchTagSetting.MATCH_ALL === closure_2) {
          const values = size.values();
          const iter = values[Symbol.iterator]();
          while (iter !== undefined) {
            let appliedTags = item.appliedTags;
            let hasItem;
            if (appliedTags != null) {
      ...
      if (closure_3) {
        if (null != c4) {
          let tmp13 = null;
          if (null != item) {
            tmp13 = getSortValue(item, sortOrder);
          }
          let tmp16 = null != tmp13;
          if (tmp16) {
            tmp16 = DISCORD_EPOCHDefault.compare(tmp13, tmp11) >= 0;
          }
          return tmp16;
        }
      }
    ...
    const sorted = found1.sort((id, id2) => {
      if (sortOrder === sortOrder(dependencyMap[5]).ThreadSortOrder.LATEST_ACTIVITY) {
      ...
      return DISCORD_EPOCHDefault.compare(id, id2);
    });
    ...
    if (!(function removeThreadIdFromAllLists(id) {
      ...
      while (iter !== undefined) {
        ...
        if (threads.indexOf(id) >= 0) {
          ...
          tmp3.threads = threads1.filter((item, index) => item !== closure_0);
          flag = true;
        }
        continue;
      }
    ...
    if (obj.isForumPostPinned(channel.id)) {
      ...
      while (iter !== undefined) {
        ...
        if (tmp8) {
          ...
          tmp7.threads = threads1.filter((item, index) => item !== channel.id);
          flag = true;
        }
        continue;
      }
  ...
  LOAD_ARCHIVED_THREADS_SUCCESS: function handleLoadArchivedThreadsSuccess(tagFilter) {
    ...
    if (0 === iter.size) {
      ...
      if (null == value) {
        ...
        const found = threads.filter((item, index) => set.has(item.type));
        threads = value.threads;
        value.threads = threads.concat(found.map((item, index) => item.id));
...
let result = require("obj132").fileFinishedImporting("modules/threads/ArchivedThreadsStore.tsx");
