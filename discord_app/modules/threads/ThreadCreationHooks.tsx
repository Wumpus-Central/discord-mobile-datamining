...
import closure_6 from "maybeRebuildState" /* 6102 */;
...
function getDefaultThreadName(stateFromStores, parentMessageId) {
  ...
  if ("" !== str) {
    ...
    const tmp17Result = tmp17(6073);
    let str7 = tmp17(6073)(str4.split("\n")[0], true).replace(/^[ #-]+/, "");
    const items = [];
    ...
    return (function*(arg0, body) {
      ...
      if (1 === tmp9) {
        ...
        if (code === constants.TOO_MANY_THREADS) {
          ...
          obj2 = callback(4809).show(obj2);
          const obj18 = callback(4809);
        } else {
          throwTypeErrorResult = closure_6;
                  ...
                  if (set2.has(code5)) {
                    if (null == dependencyMap) {
                      ...
                      if (code6 !== constants.EXPLICIT_CONTENT) {
                        ...
                        obj6[4] = reason;
                        let result = lib(8097).handleUploadMessageAttachmentsErrors(obj6);
                        const obj27 = lib(8097);
                      }
                    }
                    let obj10 = lib(7431);
                    ...
                    if (tmp89) {
                      ...
                      throwTypeErrorResult = lib;
                      throwTypeErrorResult = closure_6;
                      throwTypeErrorResult = callback(8327)(lib.id, closure_6);
                    }
                  } else {
                    let obj8 = callback(4809);
              ...
              (function sendMessage(c7, closure_0, closure_1, closure_2, c72) {
                ...
                if (null != closure_1) {
                  if (closure_1.length > 0) {
                    const obj4 = callback(7135);
                    const id = c7.id;
                    obj = { location: null };
                    obj[0] = constants.THREAD_CREATION;
                    let sendStickersResult = obj4.sendStickers(id, closure_1, callback(7360).parse(c7, closure_0), obj);
                    const obj5 = callback(7360);
                  }
                  return sendStickersResult;
                }
                obj = callback(7135);
                obj = { location: constants.THREAD_CREATION };
                sendStickersResult = obj.sendMessage(c7.id, callback(7360).parse(c7, closure_0), undefined, obj);
