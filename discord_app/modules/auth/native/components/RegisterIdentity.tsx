...
function RegisterIdentityBase(inputMode) {
  ...
  const tmp4 = setInputMode(8255)();
  [tmp8, c8] = loginEmail(identityErrorMessage.useState(false), 2);
  closure_9 = identityErrorMessage.useContext(inputMode(14955).TrackRegistrationContext);
  ...
  setInputMode(14972)(inputMode(691).AuthStates.REGISTER_IDENTITY);
  const tmp10 = setInputMode(14972);
  let obj3 = inputMode(14956);
  setInputMode(14973)(obj3.getPreviousRegistrationTransitionStep(inputMode(691).AuthStates.REGISTER_IDENTITY));
  setInputMode(4681)(() => {
  ...
  const tmp12 = setInputMode(14973);
  items1[1] = callback3(inputMode(14974).RegisterPhoneOrEmailInput, { loginPhone, loginEmail, setLoginPhone: updateLoginPhone, setLoginEmail, inputMode, onSubmit: handleSubmit, inputError: identityError, autoFocus: true });
  ...
  obj1[1] = callback3(inputMode(4665).Button, obj2);
  ...
  if (null != memo) {
    ...
    if ("" !== memo) {
      ...
      obj3[0] = tmp2.errors;
      obj3[1] = memo;
      tmp15Result = tmp15(tmp3(8252), obj3);
    }
  }
  items1[3] = tmp15Result;
  obj[3] = items1;
  obj[2] = closure_15(c7, obj);
  tmp15Result = tmp15(setInputMode(8282), obj);
  let tmp15Result1 = tmp15Result;
  if (!tmp4) {
    let obj4 = { style: null, children: null };
    obj4[0] = tmp2.page;
    obj4[1] = tmp15Result;
    tmp15Result1 = tmp15(tmp3(5718), obj4);
  }
  return tmp15Result1;
}
...
export const RegisterIdentity = function RegisterIdentity() {
  let obj = hasItem(4671);
  let obj1 = hasItem(14976);
  const deviceCountry = obj1.getDeviceCountry();
  hasItem = null != deviceCountry;
  if (hasItem) {
    const EMAIL_FIRST_COUNTRIES = tmp(14977).EMAIL_FIRST_COUNTRIES;
  ...
  const segmentedControlState = hasItem(8850).useSegmentedControlState(obj);
  ...
  obj1 = { style: tmp3.segmentedControl, children: callback3(tmp(9035).SegmentedControl, { state: segmentedControlState, keyboardShouldPersistTaps: "handled" }) };
