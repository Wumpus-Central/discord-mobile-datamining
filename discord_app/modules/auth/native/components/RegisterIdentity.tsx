...
function RegisterIdentityBase(inputMode) {
          ...
          if (arg0 === 1) {
            ...
            if (inputMode === tmp3(tmp31[18]).PhoneOrEmailSelectorForceMode.EMAIL) {
              ...
              if (null != tmp26) {
                ...
                const result = tmp3(tmp31[16]).handleNextOrSubmitRegistration(tmp3(tmp31[12]).AuthStates.REGISTER_IDENTITY, dependencyMap, React7);
  ...
  const tmp2 = closure_16(45 * Math.min(2, inputMode(5281).useFontScale()));
  let obj = inputMode(5281);
  const tmp4 = setInputMode(6358)();
  dependencyMap = inputMode(1484).useNavigation();
  const tmp5 = closure_11((errors) => errors.errors);
  let message = tmp5;
  let obj2 = inputMode(1484);
  const identityRegistrationStep = inputMode(15553).useIdentityRegistrationStep(inputMode(1094).AuthStates.REGISTER_IDENTITY, inputMode);
  ...
  let obj3 = inputMode(15553);
  [tmp8, c8] = loginEmail(identityErrorMessage.useState(false), 2);
  closure_9 = identityErrorMessage.useContext(inputMode(15540).TrackRegistrationContext);
  ...
  setInputMode(15558)(inputMode(1094).AuthStates.REGISTER_IDENTITY);
  const tmp10 = setInputMode(15558);
  const tmp12 = setInputMode(15559);
  tmp12(inputMode(15542).getPreviousRegistrationTransitionStep(inputMode(1094).AuthStates.REGISTER_IDENTITY));
  setInputMode(5291)(() => {
  ...
  let obj4 = inputMode(15542);
  items1[1] = closure_14(inputMode(15560).RegisterPhoneOrEmailInput, { loginPhone, loginEmail, setLoginPhone: updateLoginPhone, setLoginEmail, inputMode, onSubmit: handleSubmit, inputError: identityError, autoFocus: true });
  ...
  obj7.children = closure_14(inputMode(5274).Button, obj8);
...
export const RegisterIdentity = function RegisterIdentity() {
  let obj = hasItem(5281);
  const tmp3 = closure_16(45 * Math.min(2, hasItem(5281).useFontScale()));
  const deviceCountry = hasItem(15562).getDeviceCountry();
  hasItem = null != deviceCountry;
  if (hasItem) {
    const EMAIL_FIRST_COUNTRIES = tmp(15563).EMAIL_FIRST_COUNTRIES;
  ...
  let obj2 = hasItem(15562);
  const tmpResult = hasItem(9072);
  ...
  obj4.controlComponent = closure_14(closure_6, { style: tmp3.segmentedControl, children: closure_14(hasItem(9073).SegmentedControl, { state: segmentedControlState, keyboardShouldPersistTaps: "handled" }) });
