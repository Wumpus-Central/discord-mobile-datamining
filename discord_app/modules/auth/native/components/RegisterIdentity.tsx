...
function RegisterIdentityBase(inputMode) {
  ...
  const tmp2 = closure_16(45 * Math.min(2, inputMode(5280).useFontScale()));
  let obj = inputMode(5280);
  const tmp4 = setInputMode(7275)();
  dependencyMap = inputMode(1484).useNavigation();
  const tmp5 = closure_11((errors) => errors.errors);
  let message = tmp5;
  let obj2 = inputMode(1484);
  const identityRegistrationStep = inputMode(16373).useIdentityRegistrationStep(inputMode(1094).AuthStates.REGISTER_IDENTITY, inputMode);
  ...
  let obj3 = inputMode(16373);
  [tmp8, c8] = loginEmail(identityErrorMessage.useState(false), 2);
  closure_9 = identityErrorMessage.useContext(inputMode(16360).TrackRegistrationContext);
  ...
  setInputMode(16378)(inputMode(1094).AuthStates.REGISTER_IDENTITY);
  const tmp10 = setInputMode(16378);
  const tmp12 = setInputMode(16379);
  tmp12(inputMode(16362).getPreviousRegistrationTransitionStep(inputMode(1094).AuthStates.REGISTER_IDENTITY));
  setInputMode(5290)(() => {
  ...
  let obj4 = inputMode(16362);
  items1[1] = closure_14(inputMode(16380).RegisterPhoneOrEmailInput, { loginPhone, loginEmail, setLoginPhone: updateLoginPhone, setLoginEmail, inputMode, onSubmit: handleSubmit, inputError: identityError, autoFocus: true });
  ...
  obj7.children = closure_14(inputMode(5273).Button, obj8);
...
export const RegisterIdentity = function RegisterIdentity() {
  let obj = hasItem(5280);
  const tmp3 = closure_16(45 * Math.min(2, hasItem(5280).useFontScale()));
  const deviceCountry = hasItem(16382).getDeviceCountry();
  hasItem = null != deviceCountry;
  if (hasItem) {
    const EMAIL_FIRST_COUNTRIES = tmp(16383).EMAIL_FIRST_COUNTRIES;
  ...
  let obj2 = hasItem(16382);
  const tmpResult = hasItem(9974);
  ...
  obj4.controlComponent = closure_14(closure_6, { style: tmp3.segmentedControl, children: closure_14(hasItem(9975).SegmentedControl, { state: segmentedControlState, keyboardShouldPersistTaps: "handled" }) });
