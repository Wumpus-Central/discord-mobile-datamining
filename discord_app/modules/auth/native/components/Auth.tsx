// === Module 14942: NavigatorWithCaptchaHook ===
...
function NavigatorWithCaptchaHook() {
  ...
  obj[0] = require(14994) /* registerAsset */;
  const children = [callback2(importDefault(9400), obj), ];
  if (tmp5) {
    ...
    obj6[10] = obj7;
    obj5[1] = tmp17(tmp(5636).Navigator, obj6);
    obj2[1] = tmp17(closure_6, obj5);
    obj[1] = tmp17(closure_6, obj2);
    let tmp17Result = tmp17(tmp(9398).WideAuthScrollContext.Provider, obj);
    tmpResult = tmp(501);
  } else {
    ...
    tmp17Result = tmp17(tmp(5636).Navigator, obj8);
  }
...
let closure_12 = Object.fromEntries(headerTitle.map((arg0) => {
  ...
  if (set.has(tmp)) {
    ...
    obj1[0] = require(5638) /* createStackNavigator */.CardStyleInterpolators.forFadeFromCenter;
    tmp8 = obj1;
  }
...
const memoResult = importAllResult.memo(function Auth() {
  const effect = importAllResult.useEffect(() => {
    callback2(14995).initialize();
    return () => callback(table[21]).terminate();
  }, []);
  const layoutEffect = importAllResult.useLayoutEffect(() => callback(6831).trackAppUIViewed(), []);
  importDefault(14999)();
