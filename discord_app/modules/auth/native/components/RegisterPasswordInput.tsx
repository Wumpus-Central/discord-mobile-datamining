...
let obj = {
  weak: { color: nativeDefault.colors.TEXT_FEEDBACK_CRITICAL },
  medium: null,
  strong: null,
...
obj7.layout = LinearTransition.easing(Easing.inOut(fn(4493).Easing.quad)).duration(300);
const easingResult = LinearTransition.easing(Easing.inOut(fn(4493).Easing.quad));
const size = fn(2);
...
export const RegisterPasswordInput = noop.forwardRef((arg0, ref) => {
  ...
  obj4.status = str;
  const children = [
    closure_11(onPasswordChange(isPasswordFocused[16]).TextInput, obj4),
  ...
  if (null != memo) {
    ...
    if (null == tmp14) {
      ...
      obj6.animated = true;
      obj6.children = memo;
      tmp24Result = closure_11(onPasswordChange(tmp4[11]).Text, obj6);
    }
  }
