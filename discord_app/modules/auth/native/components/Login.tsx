...
export default function Login(isMultiAccount) {
  ...
  const focusEffect = obj2.useFocusEffect(noop.useCallback(() => {
    _undefined2(false);
  }, []));
      ...
      try {
        ...
        if (0 === v3) {
          ...
          if (2 === tmp8) {
            ...
            authenticationErrorsFromV6OrEarlierAPIError = obj2.getAuthenticationErrorsFromV6OrEarlierAPIError(closure_128_2);
  ...
  if (require("MFAUtils").hasWebAuthn) {
    obj = { icon: closure_10(tmp4(tmp2[32]).KeyIcon, {}), size: "lg", variant: "tertiary", disabled: null, text: null, onPress: null };
    let tmp31 = tmp11;
    if (!tmp11) {
      tmp31 = first1;
    }
  ...
  if (tmp4Result.isMetaQuest()) {
    obj = { icon: closure_10(tmp4(tmp2[34]).MobilePhoneIcon, { color: "control-primary-text-default" }), size: "lg", variant: "primary", disabled: tmp11, text: null, onPress: null };
