...
import { FadeIn } from "module_4054";
import { FadeOut } from "module_4054";
import { LinearTransition } from "module_4054";
...
function UsernameStatusMessage(arg0) {
  ...
  if (type === require(13809) /* NameValidationState */.NameValidationState.ERROR) {
    ...
    const items = [callback3(tmp3(6733).CircleErrorIcon, obj), ];
    ...
    items[1] = callback3(tmp3(4189).Text, obj1);
    obj[3] = items;
    let tmp6 = callback4(tmp3(4602).Stack, obj);
  } else {
    if (isUsernameFocused) {
      ...
      if (type1 === tmp3(13809).NameValidationState.AVAILABLE) {
        const obj2 = {};
        ...
        obj2.children = callback3(tmp3(4189).Text, obj3);
        tmp6 = callback3(tmp3(4189).Text, obj2);
      }
    }
    tmp6 = null;
    if (isUsernameFocused) {
      ...
      tmp6 = callback3(tmp3(4189).Text, obj);
    }
  }
  return tmp6;
}
...
createCacheKey[0] = require("module_300");
const easingResult = LinearTransition.easing(Easing.inOut(require("module_4054").Easing.quad));
...
export const RegisterUsernameInput = function RegisterUsernameInput(setUsername) {
  ...
  const tmp3 = importDefault(14895);
  ...
  let str2;
  if (type === tmp14(13809).NameValidationState.ERROR) {
    str2 = "error";
  }
  const obj1 = { children: null };
  obj[16] = str2;
  obj[17] = submitBehavior;
  const items3 = [closure_9(setUsername(6730).TextInput, obj), closure_9(UsernameStatusMessage, { usernameStatus, isUsernameFocused: tmp5[0] })];
  obj1[0] = items3;
