// === Module 15494: UsernameStatusMessage ===

// Module 15494 (UsernameStatusMessage)
import ThemesDefault from "Themes" /* 712 */;
import NameValidationState from "NameValidationState" /* 14298 */;
import useFocusRefOnNavigationDefault from "useFocusRefOnNavigation" /* 15486 */;
...
function UsernameStatusMessage(arg0) {
  ...
  if (type === NameValidationState.NameValidationState.ERROR) {
    ...
    const items = [callback3(tmp3(7655).CircleErrorIcon, obj), ];
    ...
    items[1] = callback3(tmp3(4442).Text, obj1);
    obj[3] = items;
    let tmp6 = callback4(tmp3(4891).Stack, obj);
  } else {
    if (isUsernameFocused) {
      ...
      if (type1 === tmp3(14298).NameValidationState.AVAILABLE) {
        const obj2 = {};
        ...
        obj2.children = callback3(tmp3(4442).Text, obj3);
        tmp6 = callback3(tmp3(4442).Text, obj2);
      }
    }
    tmp6 = null;
    if (isUsernameFocused) {
      ...
      tmp6 = callback3(tmp3(4442).Text, obj);
    }
  }
  return tmp6;
}
...
export const RegisterUsernameInput = function RegisterUsernameInput(setUsername) {
  ...
  let str2;
  if (type === tmp14(14298).NameValidationState.ERROR) {
    str2 = "error";
  }
  obj1 = { children: null };
  obj[16] = str2;
  obj[17] = submitBehavior;
  const items3 = [closure_9(setUsername(7652).TextInput, obj), closure_9(UsernameStatusMessage, { usernameStatus, isUsernameFocused: tmp5[0] })];
  obj1[0] = items3;
