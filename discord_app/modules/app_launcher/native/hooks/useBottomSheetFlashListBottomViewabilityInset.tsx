// === Module 12431: useBottomSheetFlashListBottomViewabilityInset ===
...
export const useBottomSheetFlashListBottomViewabilityInset = function useBottomSheetFlashListBottomViewabilityInset() {
  const context = bottomVisibilityInsetRef.useContext(bottomSheetPosition(11561).AppLauncherContext);
  bottomSheetPosition = undefined;
  if (context != null) {
    bottomSheetPosition = context.bottomSheetPosition;
  }
  const flashListRef = obj.useRef(null);
  const diff = flashListRef(1478)().height - flashListRef(11699)().maximum;
  dependencyMap = diff;
  ...
  const tmp4Result = bottomSheetPosition(4559);
  fn2.__closure = { distanceBetweenExpandedScreenTopAndSheetTop: diff, runOnJS: bottomSheetPosition(4559).runOnJS, handleBottomViewabilityInsetDebounced: memo };
