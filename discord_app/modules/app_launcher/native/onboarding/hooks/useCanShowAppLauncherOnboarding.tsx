...
export default function useCanShowAppLauncherOnboarding(channelId) {
  ...
  const stateFromStoresObject = guild_id(504).useStateFromStoresObject(items5, () => ({
  ...
  const items6 = [tmp14];
  const stateFromStores4 = guild_id(504).useStateFromStores(items6, () =>
  ...
  result = guild_id(4380).useIsDismissibleContentDismissed_UNSAFE(
    tmp2(1943).DismissibleContent.APP_LAUNCHER_ONBOARDING_BOTS_BANNER,
  );
  const tmp2Result5 = guild_id(4380);
  let result1 = guild_id(4380).useIsDismissibleContentDismissed_UNSAFE(
  ...
  const result2 = guild_id(4380).useIsDismissibleContentDismissed_UNSAFE(
