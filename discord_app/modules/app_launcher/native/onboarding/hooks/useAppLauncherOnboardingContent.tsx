...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowAppsOrActivitiesBanner) {
    ...
    if (
      (function useHasUsedActivities(channel) {
        ...
        let result = channelId(4654).useIsDismissibleContentDismissed_UNSAFE(
          channelId(2029).DismissibleContent.APP_LAUNCHER_ONBOARDING_ACTIVITIES_BANNER,
        );
        const obj = channelId(4654);
        const result1 = channelId(4654).useIsDismissibleContentDismissed_UNSAFE(
          channelId(2029).DismissibleContent.APP_LAUNCHER_ONBOARDING_APPS_BANNER,
        );
  ...
  const tmp7 = _slicedToArray(
    channelId(6806).useSelectedDismissibleContent(items, constants.APP_LAUNCHER_ONBOARDING),
    2,
  );
