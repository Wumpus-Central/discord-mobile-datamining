...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowAppsOrActivitiesBanner) {
    ...
    if (
      (function useHasUsedActivities(channel) {
        ...
        const result1 = channelId(4394).useIsDismissibleContentDismissed_UNSAFE(
        ...
        const activityApplications = channelId(12068).useActivityApplications(obj);
        let flag = false;
        ...
        const tmpResult = channelId(12068);
  ...
  const tmp7 = _slicedToArray(
    channelId(7402).useSelectedDismissibleContent(items, constants.APP_LAUNCHER_ONBOARDING),
    2,
  );
