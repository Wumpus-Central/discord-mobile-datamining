...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowAppsOrActivitiesBanner) {
    ...
    if (
      (function useHasUsedActivities(channel) {
        ...
        let result = channelId(4577).useIsDismissibleContentDismissed_UNSAFE(
          channelId(2028).DismissibleContent.APP_LAUNCHER_ONBOARDING_ACTIVITIES_BANNER,
        );
        const obj = channelId(4577);
        const result1 = channelId(4577).useIsDismissibleContentDismissed_UNSAFE(
        ...
        const activityApplications = channelId(12295).useActivityApplications(obj4);
        let flag = false;
        ...
        const tmpResult = channelId(12295);
  ...
  const tmp7 = _slicedToArray(
    channelId(7633).useSelectedDismissibleContent(items, constants.APP_LAUNCHER_ONBOARDING),
    2,
  );
