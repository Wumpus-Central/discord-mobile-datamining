...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowAppsOrActivitiesBanner) {
    ...
    if (
      (function useHasUsedActivities(channel) {
        ...
        let result = channelId(4576).useIsDismissibleContentDismissed_UNSAFE(
          channelId(2027).DismissibleContent.APP_LAUNCHER_ONBOARDING_ACTIVITIES_BANNER,
        );
        const obj = channelId(4576);
        const result1 = channelId(4576).useIsDismissibleContentDismissed_UNSAFE(
          channelId(2027).DismissibleContent.APP_LAUNCHER_ONBOARDING_APPS_BANNER,
        );
        ...
        const activityApplications = channelId(12289).useActivityApplications(obj4);
        let flag = false;
        ...
        const tmpResult = channelId(12289);
  ...
  const tmp7 = _slicedToArray(
    channelId(7628).useSelectedDismissibleContent(items, constants.APP_LAUNCHER_ONBOARDING),
    2,
  );
