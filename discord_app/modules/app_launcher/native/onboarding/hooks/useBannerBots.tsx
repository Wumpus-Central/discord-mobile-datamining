...
export const useBannerBots = function useBannerBots(context) {
  ...
  let obj = (function useAppsMap(context) {
    const apps = first1(12097).useApplicationsInContext({
      context: context.context,
  ...
  let apps = first1(12097).useApplicationsInContext({
    context,
    onlyWithCommands: true,
    includeBuiltIn: false,
    includeEmbeddedApps: false,
  ...
  const obj2 = first1(12097);
  const tmp2 = first1;
  const obj3 = (function useCommandsMap(context) {
    const commands = first1(12097).useApplicationCommandsInContext({
      context: context.context,
      includeBuiltIn: false,
    }).commands;
  ...
  const tmp4 = (function useFrecencyCommandIds(context) {
    ...
    return context(9331).useTopCommands(obj);
  })({ context });
