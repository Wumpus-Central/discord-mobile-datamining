...
let result = require("obj132").fileFinishedImporting("modules/app_launcher/native/onboarding/banner/BannerBase.tsx");
...
export default function BannerBase(arg0) {
  ...
  const fn = function _() {
    ...
    let num = 0;
    if (sharedValue.get()) {
      const obj2 = _undefined(dependencyMap[8]);
      num = obj2.withDelay(150, _undefined(dependencyMap[12]).withSpring(1, closure_1_9));
      const obj3 = _undefined(dependencyMap[12]);
    }
    obj = { opacity: num, transform: null };
    let num4 = 30;
    if (obj.get()) {
      const obj5 = _undefined(dependencyMap[8]);
      num4 = obj5.withDelay(150, _undefined(dependencyMap[12]).withSpring(0, closure_1_9));
      const obj6 = _undefined(dependencyMap[12]);
    }
