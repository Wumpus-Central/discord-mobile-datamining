...
export default function CommandOptionView(option) {
  ...
  const ReduceMotion = option(4146).ReduceMotion;
  const tmp5 = stateFromStores ? ReduceMotion.Always : ReduceMotion.Never;
  const optionEnteringAnimation = option(11461).useOptionEnteringAnimation();
  ...
  if (set.has(option.type)) {
    ...
    const FadeOut = tmp2(4146).FadeOut;
    const FadeInUp = tmp2(4146).FadeInUp;
    obj = { transform: null };
    ...
    obj2[2] = tmp2(11461).ExitingAnimation;
    obj2[3] = tmp2(11461).LayoutAnimation;
    ...
    if (hasItem) {
      ...
      obj7[4] = items3;
      hasItem = tmp19(tmp18(4146).View, obj7);
    }
    items2[3] = hasItem;
    obj3[2] = items2;
