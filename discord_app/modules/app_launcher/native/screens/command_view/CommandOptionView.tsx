...
export default function CommandOptionView(option) {
  ...
  const ReduceMotion = option(4559).ReduceMotion;
  const tmp5 = stateFromStores ? ReduceMotion.Always : ReduceMotion.Never;
  const obj = option(504);
  const optionEnteringAnimation = option(12494).useOptionEnteringAnimation();
  ...
  if (set.has(option.type)) {
    ...
    const FadeOut = tmp2(4559).FadeOut;
    const FadeInUp = tmp2(4559).FadeInUp;
    ...
    const obj5 = {
      ...
      exiting: tmp2(12494).ExitingAnimation,
      layout: tmp2(12494).LayoutAnimation,
      onLayout(arg0) {
    ...
    if (hasItem1) {
      ...
      tmp17Result = closure_5(tmp2(4823).Text, obj7);
    }
    ...
    items2[2] = closure_5(tmp2(4823).Text, obj9);
    if (hasItem) {
      ...
      const obj11 = { style: tmp.optionErrorIcon, source: _modDef10756, size: tmp2(1177).IconSizes.REFRESH_SMALL_16 };
      ...
      items3[1] = closure_5(tmp2(4823).Text, obj12);
    ...
    obj3.children = closure_5(tmp2(12495).AwaitAnimationContext, obj4);
    return closure_5(tmp2(4559).LayoutAnimationConfig, obj3);
  } else {
    return null;
  }
  const tmp2Result = option(12494);
};