...
function AppLauncherCommandViewInner(command) {
          ...
          const item = options.forEach((required) => {
            ...
            if (true !== required.required) {
              ...
              if (someResult) {
                ...
                obj = { option: required, content: closure_32.current[required.name], guildId: guild_id, channelId: context.channel.id, allowEmptyValues: false, commandOrigin: ApplicationCommandTypes.CommandOrigin.APPLICATION_LAUNCHER };
              ...
              const obj2 = { option: required, content: closure_32.current[required.name], guildId: guild_id, channelId: context.channel.id, allowEmptyValues: false, commandOrigin: ApplicationCommandTypes.CommandOrigin.APPLICATION_LAUNCHER };
  ...
  callback3 = obj1.useCallback(() => {
    if (null != command) {
      ...
      if (null != firstInvalidOption) {
        ...
        obj = { command, optionValues: ApplicationCommandOptionValueParser.parseOptionValuesForSend(commandContext.channel, command, optionValues.current), context: commandContext, sectionName, commandOrigin: ApplicationCommandTypes.CommandOrigin.APP_LAUNCHER_APPLICATION_VIEW };
  ...
  const callback4 = obj1.useCallback(analyticsLocation(function*() {
    if (c2 === 2) {
      c2 = 3;
        ...
        if (0 === c1) {
          if (arg0 === 1) {
            ...
            if (null == command) {
              ...
              const obj1 = { applicationId: command.applicationId, channel: context.channel, commandIntegrationTypes: command.integration_types, appLauncherContext: null };
  ...
  onPressAttachmentOption = obj1.useCallback((option) => {
    ...
    if (fileTypeFiltering.mediaFilesAllowed) {
      ...
      obj.onPressItem = function onPressItem(item) {
        ...
        const result2 = tmpResult.addAttachmentForCommand(item.channelId, chatInputRef, result1, obj, Upload.UploadOrigin.IMAGE_PICKER);
      };
  ...
  const items22 = [tmp68(command(sectionName[13]).AppLauncherCommandViewHeader, { command, onPressBack: callback, scrollOffsetY: sharedValue1, section }), tmp71Result];
...
function NotFoundState(onPress) {
  ...
  obj = { style: tmp.emptyState, lightSource: _modDef12286, darkSource: _modDef12287, title: null, body: null, children: null };
...
function NoPermsState(command) {
  ...
  const obj3 = { style: tmp.emptyState, lightSource: _modDef12207, darkSource: _modDef12208, title: null, body: null, children: null };
  const intl = util.intl;
...
createStyles.optionsContainer = { paddingTop: fn(12247).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
...
let obj1 = { paddingTop: fn(12247).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
...
createStyles.emptyStateContainer = { paddingTop: fn(12247).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, flex: 1 };
createStyles.failureStateButtonWrapper = { marginTop: 24, alignSelf: "center" };
let obj4 = { paddingTop: fn(12247).EXPANDED_HEADER_TOTAL_CONSUMED_SPACE_IN_PARENT + nativeDefault.space.PX_4, flex: 1 };
