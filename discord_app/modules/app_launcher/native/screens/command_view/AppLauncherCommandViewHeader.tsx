// === Module 11332: DEFAULT_CONTENT_PADDING ===
...
let createCacheKey = { backgroundColor: ThemesDefault.colors.BACKGROUND_BASE_LOWEST };
...
const result = require("obj132").fileFinishedImporting("modules/app_launcher/native/screens/command_view/AppLauncherCommandViewHeader.tsx");
...
export const AppLauncherCommandViewHeader = function AppLauncherCommandViewHeader(onPress) {
  ...
  const memo = React.useMemo(() => {
    ...
    return scrollOffsetY(dependencyMap[7]).getAppLauncherIconSource(application);
  }, items);
  ...
  obj = { interpolate: scrollOffsetY(4115).interpolate, scrollOffsetY, TOTAL_SCROLL_RANGE: closure_8 };
  A.__closure = obj;
  A.__workletHash = 15596175827193;
  A.__initData = closure_10;
  const animatedStyle = obj.useAnimatedStyle(A);
  let obj2 = scrollOffsetY(4115);
  class E {
    constructor() {
  ...
  obj = { APP_ICON_SIZE: 72, APP_ICON_BORDER_WIDTH: 4, DEFAULT_CONTENT_PADDING, interpolate: scrollOffsetY(4115).interpolate, scrollOffsetY, TOTAL_SCROLL_RANGE: closure_8 };
  E.__closure = obj;
  E.__workletHash = 13563524587234;
  E.__initData = closure_11;
  const animatedStyle1 = obj2.useAnimatedStyle(E);
  let obj4 = scrollOffsetY(4115);
  class N {
    constructor() {
      obj = { opacity: null };
      obj2 = scrollOffsetY(closure_1_2[8]);
  ...
  N.__closure = { interpolate: scrollOffsetY(4115).interpolate, scrollOffsetY, TOTAL_SCROLL_RANGE: closure_8 };
  ...
  const tmp9 = section(8345);
  if (null != memo) {
    obj2 = { style: null, source: null };
    obj2[0] = tmp.appIcon;
    obj2[1] = memo;
    let tmp15 = callback(tmp8(5449), obj2);
    let tmp16 = callback;
  } else {
  ...
  const items4 = [tmp16(section(4115).View, { style: items3 }), tmp16(section(11309), { onPress: onPress.onPressBack }), , , ];
  ...
  obj5[5] = displayName;
  items4[2] = tmp16(scrollOffsetY(4734).Text, obj5);
  items4[3] = tmp16(scrollOffsetY(1297).Spacer, { size: 32 });
  const items6 = [tmp.appIconMask, animatedStyle1];
  items4[4] = tmp16(section(4115).View, { style: items6, children: tmp15 });
  obj4[1] = items4;
