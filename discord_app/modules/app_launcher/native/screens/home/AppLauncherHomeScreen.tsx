...
function ActivityRow(section) {
  ...
  return closure_19(section(5917).TableRow, {
    icon: tmp5,
  ...
  const memo4 = obj3.useMemo(() => {
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        const obj5 = {
          type: tmp8(11570).AppLauncherHomeListItemType.SECTION_HEADER,
          section: null,
          sectionName: "activities",
          numItems: null,
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(8720).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              if (tmp4) {
                ...
                collectionItemAssetUrl = found1(11606).getCollectionItemAssetUrl({ itemId: null, hash: null });
                const obj = { itemId: null, hash: null };
                const tmpResult = found1(11606);
              }
              const obj3 = {
                application: type.application,
                showsPromoted: found1(1385).hasFlag(type.flags, found1(11607).ApplicationCollectionItemFlags.PROMOTED),
      ...
      onViewAllSelected() {
        ...
        if (null != mapped) {
          const obj2 = {
            ...
            sectionItemType: FrecencySection.SectionItemType.APPS,
            commands: [],
            sectionDescriptors: mapped.map((item) => context(6941).getApplicationCommandSection(item)),
            title: null,
          };
          const intl = util.intl;
