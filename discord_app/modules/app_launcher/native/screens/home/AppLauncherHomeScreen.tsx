...
function ActivityRow(section) {
  ...
  return closure_19(section(5656).TableRow, {
    icon: tmp5,
  ...
  const memo4 = obj2.useMemo(() => {
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        obj = {
          type: tmp8(12181).AppLauncherHomeListItemType.SECTION_HEADER,
          section: null,
          sectionName: "activities",
          numItems: null,
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9508).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = {
                application: type.application,
                showsPromoted: found1(1384).hasFlag(type.flags, found1(12218).ApplicationCollectionItemFlags.PROMOTED),
              };
              return obj;
            }
          });
      ...
      onViewAllSelected() {
        ...
        if (null != mapped) {
          const obj = {
            ...
            sectionItemType: FrecencySection.SectionItemType.APPS,
            commands: [],
            sectionDescriptors: mapped.map((item) => context(7594).getApplicationCommandSection(item)),
            title: null,
          };
          const intl = util.intl;
