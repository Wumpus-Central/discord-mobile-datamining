// === Module 12083: AppLauncherHomeScreen ===
...
import ApplicationDirectoryActionCreators from "ApplicationDirectoryActionCreators" /* 12071 */;
import HeroMedia from "HeroMedia" /* 12084 */;
...
import ExpandableList from "ExpandableList" /* 12108 */;
import InThisServerSection from "InThisServerSection" /* 12110 */;
import ApplicationDirectoryCollectionType from "ApplicationDirectoryCollectionType" /* 12123 */;
import ApplicationCollectionFlags from "ApplicationCollectionFlags" /* 12126 */;
    ...
    memo = closure_5.useMemo(() => {
      ...
      if (null != sectionDescription) {
        ...
        if ("" !== sectionDescription) {
          obj = { content: sectionDescription, muted: false, layout: ChannelListLayoutTypes.ChannelListLayoutTypes.COMPACT, color: "text-muted" };
...
function ActivityItemTuple(arg0) {
  ...
  if (tmp4Result) {
    ...
    const obj2 = { context, sectionName, onPress, usesHandleActivityItemSelected, onActivityItemSelected, shelfItem: shelfItem2, entrypoint, imageWidth: null, imageHeight: null };
...
function ActivityItem(context) {
  ...
  const tmp5Result = _slicedToArray(tmp2Result.useStateFromStoresArray(items, () => {
  ...
  const obj6 = { style: tmp.activityDetailsContainer, children: closure_19(context(flag[16]).Text, { variant: "heading-sm/bold", color: "mobile-text-heading-primary", children: shelfItem.application.name }) };
...
function RecommendationItem(onPress) {
  ...
  const obj6 = { style: tmp.activityDetailsContainer, children: closure_1_19(Text_Text.Text, { variant: "heading-sm/bold", color: "mobile-text-heading-primary", children: application.name }) };
...
function getRecommendationItemsWithViewAll(found1, in_this_server, stringResult) {
  ...
  if (0 === found1.length) {
    ...
    dependencyMap = tmp12;
    items = [];
    let obj = { type: require("AppLauncherHomeTypes").AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER, section: stringResult, sectionName: in_this_server, numItems: length, numVisibleItems: bound };
    items.push(obj);
    ...
    if (bound < length) {
      obj = { type: require("AppLauncherHomeTypes").AppLauncherHomeListItemType.VIEW_ALL, applications: found1.map((application) => application.application), promotedApplicationIds: null, sectionName: null, sectionOverallPosition: null, title: null };
      const _Set = Set;
      const set = new Set();
...
createStyles.list = { paddingHorizontal: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
createStyles.searchBarContainer = { paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: DEFAULT_CONTENT_PADDING };
let obj1 = { paddingHorizontal: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
createStyles.divider = { borderColor: nativeDefault.colors.BORDER_STRONG, borderTopWidth: 1, marginTop: nativeDefault.space.PX_24 };
createStyles.appRowLabelWithPromotedContainer = { overflow: "hidden", flexDirection: "row", alignItems: "center", gap: nativeDefault.space.PX_4 };
...
createStyles.promotedLabel = { alignSelf: "center", justifyContent: "center", paddingVertical: 2, paddingHorizontal: nativeDefault.space.PX_4, backgroundColor: nativeDefault.colors.INTERACTIVE_BACKGROUND_ACTIVE, borderRadius: nativeDefault.radii.lg };
let obj3 = { alignSelf: "center", justifyContent: "center", paddingVertical: 2, paddingHorizontal: nativeDefault.space.PX_4, backgroundColor: nativeDefault.colors.INTERACTIVE_BACKGROUND_ACTIVE, borderRadius: nativeDefault.radii.lg };
createStyles.activityItemContainer = { borderRadius: nativeDefault.radii.md, overflow: "hidden", height: "100%", backgroundColor: nativeDefault.colors.BACKGROUND_APP_LAUNCHER_CARD_DEFAULT, borderWidth: 1, borderColor: nativeDefault.colors.BORDER_SUBTLE };
let obj4 = { borderRadius: nativeDefault.radii.md, overflow: "hidden", height: "100%", backgroundColor: nativeDefault.colors.BACKGROUND_APP_LAUNCHER_CARD_DEFAULT, borderWidth: 1, borderColor: nativeDefault.colors.BORDER_SUBTLE };
createStyles.activityImageContainer = { justifyContent: "center", alignItems: "center", height: 120, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWEST, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
let obj5 = { justifyContent: "center", alignItems: "center", height: 120, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWEST, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
export default function AppLauncherHomeScreen(route) {
  ...
  const callback1 = noop.useCallback((application, sectionName) => {
    const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application: null, navigation: null, context: null, sectionName: null, entrypoint: null };
  ...
  const callback2 = noop.useCallback((navigates) => {
    ...
    const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application: shelfData.application, navigation, context, sectionName, navigates: flag, entrypoint };
    const result = obj.handleApplicationSelected(obj);
  }, items3);
  ...
  memo1 = obj2.useMemo(() => length.map((applicationId, index) => ({ type: context(handleViewableItemsChanged[45]).AppLauncherHomeListItemType.APP, applicationId: applicationId.id, section: applicationId, isFirstRow: 0 === index, isLastRow: index === length.length - 1, sectionName: "recents" })), items5);
  ...
  const stateFromStores = tmpResult.useStateFromStores(items8, () => entrypoint.get("only_show_preview_app_collections"));
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        if (0 < arr.length - 1) {
          do {
            let sum = num + 1;
            let obj = { type: AppLauncherHomeTypes.AppLauncherHomeListItemType.SHELF_ITEM_TUPLE, shelfItem1: handleViewableItemsChanged[num], shelfItem2: handleViewableItemsChanged[sum], sectionName: "activities", shelfItem1SectionPosition: num, shelfItem2SectionPosition: sum, sectionOverallPosition: 0, isLastTuple: false };
            arr = items.push(obj);
            num = num + 2;
            tmp8 = require;
        ...
        obj = { type: tmp8(12088).AppLauncherHomeListItemType.SECTION_HEADER, section: null, sectionName: "activities", numItems: null, numVisibleItems: null };
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9417).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              obj = { application: type.application, showsPromoted: null, overrideImageUrl: null, sectionPosition: null };
          ...
          if (0 < found.length) {
            do {
              obj = { type: AppLauncherHomeTypes.AppLauncherHomeListItemType.RECOMMENDATION_TUPLE, sectionName: title.title, sectionOverallPosition, items: null, isLastTuple: null };
              sum = num3 + navigation;
          ...
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9417).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: type.application, showsPromoted: found1(1384).hasFlag(type.flags, found1(12125).ApplicationCollectionItemFlags.PROMOTED) };
              return obj;
            }
          });
  ...
  callback3 = obj2.useCallback((application) => {
    ...
    obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application, navigation, context, installOnDemand: !tmp4, sectionName: application.sectionName, entrypoint };
  ...
  const callback4 = obj2.useCallback((item) => {
    ...
    if (context(handleViewableItemsChanged[45]).AppLauncherHomeListItemType.PLACEHOLDER === type) {
    ...
    } else if (context(handleViewableItemsChanged[45]).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER === type) {
      const obj8 = { index, children: item.section };
      return memo(trackAppLauncherHomeItemImpression, obj8);
    } else if (context(handleViewableItemsChanged[45]).AppLauncherHomeListItemType.RECOMMENDATION_BANNER_CARD === type) {
      const obj9 = { application: item.item.application, isFirst: null, isLast: null, onPress: null, isLandscape: null, showsPromoted: null, overrideImageUrl: null };
      ...
      const obj11 = {
        ...
        onPress() {
            ...
            const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, navigation, context, sectionName: item.sectionName, sectionOverallPosition: item.sectionOverallPosition, applications: mapped, sectionItemType: FrecencySection.SectionItemType.APPS, commands: [], sectionDescriptors: mapped.map((item) => item(closure_1_3[51]).getApplicationCommandSection(item)), title: item.title, promotedApplicationIds: item.promotedApplicationIds };
  ...
  memo9 = obj2.useMemo(() => _modDef12.debounce((query) => {
  ...
  callback6 = obj2.useCallback((arg0) => {
    if (stateFromStores3) {
      ...
      const result = obj.UNSAFE_markDismissibleContentAsDismissed(dismissible_content.DismissibleContent.APP_LAUNCHER_GLOBAL_SEARCH_ONBOARDING, obj);
      obj = {};
  ...
  if (tmp51) {
    ...
    let obj7 = {
      ...
      onCommandSelected(command, section) {
          const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, context, command, section, sectionDescriptors, query: "", navigation, sectionName: AppLauncherTypes.AppLauncherSectionName.RECENT_COMMANDS, entrypoint };
          const result = obj.handleApplicationCommandSelected(obj);
        },
      onViewAllSelected(sectionItemType) {
          const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_APPLICATION_VIEW_FRECENCT, navigation, context, sectionName: null, applications: null, sectionItemType: null, commands: null, sectionDescriptors: null, title: null };
