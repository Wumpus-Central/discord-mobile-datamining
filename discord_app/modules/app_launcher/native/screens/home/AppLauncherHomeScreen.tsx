...
import getLastFetchTimeMs from "../../../../global_discovery_apps/stores/ApplicationDirectoryCollectionsStore.tsx";
import getUncachedChannelPermissions from "../../../../../stores/PermissionStore.tsx";
import initialize from "../../onboarding/stores/AppLauncherOnboardingPersistedStore.tsx";
...
class BaseAppRow {
  constructor(arg0) {
    ...
    memo = closure_5.useMemo(() => {
      let obj = application(dependencyMap[19]);
      let FAKE_BUILT_IN_APP = application;
      if (application == null) {
        FAKE_BUILT_IN_APP = application(dependencyMap[19]).FAKE_BUILT_IN_APP;
      }
      ...
      if (null != sectionDescription) {
        ...
        if ("" !== sectionDescription) {
          ...
          obj[2] = application(dependencyMap[21]).ChannelListLayoutTypes.COMPACT;
          result = application(dependencyMap[20]).renderMessagePreviewMarkup(obj);
          const tmpResult = application(dependencyMap[20]);
        }
      }
      return result;
    }, items);
...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = callback2(onPress(11229), obj);
  }
  const items = [section];
  ...
  const memo = React.useMemo(() => {
    ...
    if (null != description) {
      result = null;
      if ("" !== description) {
        section(dependencyMap[20]);
        const obj = { content: null, muted: false, layout: null, color: "text-muted" };
        obj[0] = description;
        obj[2] = section(dependencyMap[21]).ChannelListLayoutTypes.COMPACT;
  ...
  return callback2(section(6291).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
function ActivityItemTuple(arg0) {
  ...
  const size = React.useMemo(() => {
    if (null == containerWidth) {
      return { width: "r", height: "accessibilityRole" };
    } else {
      const tmp5 = tDefault(tmp / 2 - DEFAULT_CONTENT_PADDING - 6);
      const obj = { width: null, height: null };
      obj[0] = tmp5;
      obj[1] = tDefault(tmp5 / closure_1_22);
      return obj;
    }
  }, items);
  ...
  { style: tmp.activityItemTupleShelfItemContainer, children: callback2(ActivityItem, obj) };
  ...
  if (tmp4Result) {
    ...
    obj1[1] = callback2(ActivityItem, obj2);
    tmp4Result = callback2(closure_6, obj1);
  }
  items2[1] = tmp4Result;
  obj[1] = items2;
...
function ActivityItem(context) {
  ...
  obj5[1] = items3;
  const items4 = [callback2(closure_6, obj5), ];
  ...
  obj3[3] = items4;
  return callback2(context(flag[32]).PressableScale, obj3);
}
function RecommendationItemTuple(arg0) {
  ...
  let obj = { style: items1, children: null };
  items1[1] = { marginBottom: 12 };
  { style: tmp.activityItemTupleShelfItemContainer, children: callback2(RecommendationItem, obj) };
  ...
  if (tmp4Result) {
    ...
    obj1[1] = callback2(RecommendationItem, obj2);
    tmp4Result = callback2(closure_6, obj1);
  }
  items2[1] = tmp4Result;
  obj[1] = items2;
...
function RecommendationItem(onPress) {
  ...
  if (isEmbeddedAppResult) {
    ...
    obj6[0] = sectionName(tmp3[19]).getShelfBadgeTypeIfActive(application);
    isEmbeddedAppResult = callback2(tmp7Result, obj6);
    const tmp2Result = sectionName(tmp3[19]);
  }
  ...
  obj3[2] = items3;
  return callback2(sectionName(application[32]).PressableScale, obj3);
}
function Divider() {
...
function RecommendationSectionHeader(arg0) {
  ...
  style[1] = obj;
  return callback(Text.Text, { accessibilityRole: "header", variant: "heading-lg/bold", color: "mobile-text-heading-primary", style, children });
}
function getRecommendationItemsWithViewAll(arr) {
  const _require = arg1;
  let COLLAPSED_LIST_ITEM_MAX = arg3;
  if (arg3 === undefined) {
    COLLAPSED_LIST_ITEM_MAX = require("../../base_components/ExpandableList.tsx").COLLAPSED_LIST_ITEM_MAX;
  }
  closure_1 = tmp3;
  let bound;
  ...
  if (0 === arr.length) {
    ...
    obj[0] = require("AppLauncherHomeTypes.tsx").AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
    obj[1] = arg2;
    obj[2] = arg1;
    obj[3] = length;
    obj[4] = bound;
    ...
    const item = substr.forEach((item, index) => {
      const obj = { type: callback(table[44]).AppLauncherHomeListItemType.RECOMMENDATION_APP, application: item.application, showsPromoted: item.showsPromoted, isFirstRow: 0 === index, isLastRow: null, sectionName: null, sectionPosition: null, sectionOverallPosition: null };
      let tmp2 = index === bound - 1;
      if (tmp2) {
        tmp2 = !table;
      }
      obj[4] = tmp2;
    ...
    if (bound < length) {
      ...
      obj[0] = require("AppLauncherHomeTypes.tsx").AppLauncherHomeListItemType.VIEW_ALL;
      obj[1] = arr.map((item, index) => item.application);
      const _Set = Set;
      const set = new Set();
      obj[2] = arr.reduce((acc, item, index) => {
        if (item.showsPromoted) {
          acc.add(item.application.id);
        }
        return acc;
      }, set);
      obj[3] = arg1;
...
const createCacheKey = { backgroundColor: ThemesDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND, flex: 1 };
...
let result = require("obj132").fileFinishedImporting("modules/app_launcher/native/screens/home/AppLauncherHomeScreen.tsx");
...
export default function AppLauncherHomeScreen(route) {
  ...
  const tmp5 = entrypoint(8995)(hasViewedActivityItem);
  const tmp6 = entrypoint(8995)(hasViewedLearnMoreItem);
  let tmp7 = callback4();
  let React = tmp7;
  let tmp8 = entrypoint(8633)();
  obj1 = context(9565);
  ...
  const size = entrypoint(1494)();
  let obj2 = React;
  let items = [entrypoint];
  const effect = React.useEffect(() => {
    if (memo3 === context(9566).AppLauncherEntrypoint.VOICE) {
      const result = lib(8701).dismissNewActivityIndicator();
      const obj = lib(8701);
    }
  }, items);
  ...
  const callback = React.useCallback(() => {
    PREVIEW.current = context(9565).AppLauncherKeyboardCloseReason.COMMAND;
  ...
  const callback1 = React.useCallback((application) => {
    context(11224);
    const obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application: null, navigation: null, context: null, sectionName: null, entrypoint: null };
    let FAKE_BUILT_IN_APP = application.application;
    if (FAKE_BUILT_IN_APP == null) {
      FAKE_BUILT_IN_APP = context(8447).FAKE_BUILT_IN_APP;
    }
  ...
  const callback2 = React.useCallback((navigates) => {
    ...
    context(11224);
    const obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application: shelfData.application, navigation: closure_2, context, sectionName, navigates: flag, entrypoint: memo3 };
    const result = obj.handleApplicationSelected(obj);
  }, items3);
  closure_2 = undefined;
  dependencyMap = undefined;
  ...
  let obj3 = context(11295);
  const fetchDeveloperActivityShelfItems = obj3.useFetchDeveloperActivityShelfItems();
  let guild_id;
  if ("channel" === context.type) {
    guild_id = context.channel.guild_id;
  }
  const tmp16Result = entrypoint(11212)({ guildId: guild_id });
  closure_2 = tmp16Result;
  const tmp19 = entrypoint === context(9566).AppLauncherEntrypoint.VOICE;
  dependencyMap = tmp19;
  let tmp20 = entrypoint(11296)({ context, onlyActivityApps: tmp19 });
  ({ frecencyCommands, frecentApps } = tmp20);
  ({ sectionDescriptors, loading } = tmp20);
  const tmp21 = entrypoint === context(9566).AppLauncherEntrypoint.TEXT;
  React = tmp21;
  appsInThisServer = tmp4(11299)({ context }).appsInThisServer;
  const items4 = [appsInThisServer, tmp21];
  memo = obj2.useMemo(() => {
    const intl = context(1236).intl;
    const mapped = closure_5 ? appsInThisServer : c23.map((item, index) => ({ application: item.application }));
    return callback8(mapped, "in_this_server", intl.string(context(1236).t.oJyzCu), context(11287).IN_THIS_SERVER_ITEM_MAX);
  }, items4);
  const items5 = [frecentApps];
  memo1 = obj2.useMemo(() => frecentApps.map((item, index) => ({ type: context(closure_1_3[44]).AppLauncherHomeListItemType.APP, applicationId: item.id, section: item, isFirstRow: 0 === index, isLastRow: index === length.length - 1, sectionName: "recents" })), items5);
  const items6 = [tmp21, frecentApps];
  memo2 = obj2.useMemo(() => {
    let tmp = !closure_5;
    if (closure_5) {
      tmp = !frecentApps.some((item, index) => null != item.application);
    }
    return tmp;
  }, items6);
  ...
  const effect1 = obj2.useEffect(() => {
    ...
    if (!isActivitiesInTextEnabled) {
      tmp = entrypoint === context(9566).AppLauncherEntrypoint.VOICE;
    }
    if (tmp) {
      let obj = context(8701);
      let guild_id;
      if ("channel" === context.type) {
        guild_id = context.channel.guild_id;
      }
  ...
  let tmp4Result = tmp4(11294);
  const ApplicationCollectionActiveState = tmp(11254).ApplicationCollectionActiveState;
  if (stateFromStores) {
  ...
  memo3 = obj2.useMemo(() => {
    if (dependencyMap) {
      let result = context(8447).ensureRecommendationSectionsOnlyContainActivities(stateFromStores1);
      const obj = context(8447);
    } else {
  ...
  const effect2 = obj2.useEffect(() => {
    let obj = context(8447);
    if (obj.appLauncherShowsRecommendations(entrypoint)) {
      obj = { surface: null, activeState: null };
      obj[0] = memo9;
      obj[1] = PREVIEW;
      const collections = context(11247).fetchCollections(obj);
      const tmpResult = context(11247);
    }
  }, items11);
  ...
  memo4 = obj2.useMemo(() => {
    ...
    let arr = lib;
    if (0 !== lib.length) {
      if (entrypoint === context(9566).AppLauncherEntrypoint.VOICE) {
        const items = [];
        let tmp8 = context;
        let num = 0;
        if (0 < arr.length - 1) {
          do {
            ...
            obj[0] = context(11265).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE;
            obj[1] = lib[num];
            obj[2] = lib[sum];
            obj[4] = num;
            obj[5] = sum;
        ...
        if (arr.length % 2 === 1) {
          ...
          obj[0] = tmp8(11265).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE;
        ...
        obj[0] = tmp8(11265).AppLauncherHomeListItemType.SECTION_HEADER;
        const intl = tmp8(1236).intl;
        obj[1] = intl.string(tmp8(1236).t.aeuOoh);
  ...
  let memo5 = obj2.useMemo(() => {
    const result = closure_12.result;
    let sections;
    if (result != null) {
      sections = result.sections;
    }
    ...
    let someResult = values.some((item, index) => Object.keys(item.commands).length > 0);
    if (!someResult) {
      const _Object = Object;
      values = Object.values(sections1);
      someResult = values.some((item, index) => Object.keys(item.commands).length > 0);
    }
    return someResult;
  }, items13);
  const tmpResult1 = context(589);
  const items14 = [closure_12];
  let channel = null;
  const stateFromStores2 = context(589).useStateFromStores(items14, () => {
    ...
    if (tmp2) {
      ...
      if (!isDMResult) {
        isDMResult = closure_12.can(memo1.USE_APPLICATION_COMMANDS, context.channel);
      }
      tmp2 = isDMResult;
    }
    return tmp2;
  ...
  const memo6 = obj2.useMemo(() => {
    ...
    if (memo2) {
      ...
      const _Set = Set;
      let item = memo3.forEach((item, index) => {
        const prop = item.application_directory_collection_items;
        item = prop.forEach((item, index) => {
          if (item.type === items(closure_1_3[79]).ApplicationDirectoryCollectionItemType.APPLICATION) {
            set.add(item.application.id);
          }
        });
      });
      const item1 = lib.forEach((item, index) => {
        if (!set.has(item.application.id)) {
          items.push(item);
        }
      });
      return items;
  ...
  const memo7 = obj2.useMemo(() => {
    ...
    if (memo2) {
      ...
      let item = memo3.forEach((item, index) => {
        ({ type, application_directory_collection_items } = item);
        if (type === context(11301).ApplicationDirectoryCollectionType.BANNER_CARDS) {
          const _Math = Math;
          let obj = { type: null, section: null, sectionName: null, numItems: null, numVisibleItems: null };
          const bound = Math.min(length, context(11285).COLLAPSED_LIST_ITEM_MAX);
          obj[0] = context(11265).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ({ title: obj[1], title: obj[2] } = item);
          ...
          for (let num3 = 0; num3 < application_directory_collection_items.length; num3 = num3 + 2) {
            ...
            if (tmp20.type === context(11300).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let tmp26 = null != tmp20.id && null != tmp20.image_hash;
              let collectionItemAssetUrl;
              if (tmp26) {
                let tmp24Result = context(11302);
              ...
              tmp24Result = context(1403);
              obj[1] = tmp24Result.hasFlag(tmp20.flags, context(11303).ApplicationCollectionItemFlags.PROMOTED);
              obj[2] = collectionItemAssetUrl;
              let tmp28;
              if (null != tmp23) {
                if (tmp23.type === context(11300).ApplicationDirectoryCollectionItemType.APPLICATION) {
                  let tmp29 = null != tmp23.id && null != tmp23.image_hash;
                  let collectionItemAssetUrl1;
                  if (tmp29) {
                    let tmp24Result1 = context(11302);
                    obj1 = { itemId: null, hash: null };
                  ...
                  let tmp24Result2 = context(1403);
                  obj2[1] = tmp24Result2.hasFlag(tmp23.flags, context(11303).ApplicationCollectionItemFlags.PROMOTED);
              ...
              obj3[0] = context(11265).AppLauncherHomeListItemType.RECOMMENDATION_TUPLE;
              obj3[1] = item.title;
              obj3[2] = index;
              obj3[3] = obj;
              obj3[4] = tmp28;
        ...
        } else if (type === context(11301).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          const prop = item.application_directory_collection_items;
          const mapped = prop.map((item, index) => {
            if (item.type === found(table[79]).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: null, showsPromoted: null };
              obj[0] = item.application;
              obj[1] = found(table[83]).hasFlag(item.flags, found(table[84]).ApplicationCollectionItemFlags.PROMOTED);
              return obj;
            }
          });
          found = mapped.filter(context(1370).isNotNullish);
          if (tmpResult.hasFlag(item.flags, context(11304).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
            item = memo8.forEach((item, index) => {
              found.push({ application: item.application, showsPromoted: false });
            });
          }
          const push = found.push;
          items = [];
          HermesBuiltin.arraySpread(callback8(found, item.title, item.title, undefined, index), 0);
          HermesBuiltin.apply(items, found);
          tmpResult = context(1403);
        }
      });
    }
    return items;
  ...
  memo8 = obj2.useMemo(() => {
    ...
    if (obj.appLauncherShowsRecommendations(entrypoint)) {
      obj = { type: null };
      obj[0] = context(11265).AppLauncherHomeListItemType.LEARN_MORE;
      const items = [];
  ...
  callback3 = obj2.useCallback((application) => {
    ...
    if (!isEmbeddedAppResult) {
      tmp4 = null != sectionDescriptors.find((item, index) => item.id === application.id);
    }
    obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, application, navigation: closure_2, context: application, installOnDemand: !tmp4, sectionName: application.sectionName, entrypoint: memo3 };
    const result = context(11224).handleApplicationSelected(obj);
    const tmpResult = context(11224);
  }, items18);
  const tmpResult2 = context(589);
  clickOnHomeActivityOpensAppDetails = context(11266).useClickOnHomeActivityOpensAppDetails();
  ...
  obj2.useCallback((item) => {
    item = item.item;
    const index = item.index;
    const type = item.type;
    if (context(11265).AppLauncherHomeListItemType.PLACEHOLDER === type) {
      ...
      obj[2] = obj;
      return memo(entrypoint(11267), obj);
    } else if (context(11265).AppLauncherHomeListItemType.SECTION_HEADER === type) {
      ...
      obj2[3] = items;
      obj2[4] = item.section;
      return memo(context(4734).Text, obj2);
    } else if (context(11265).AppLauncherHomeListItemType.SHELF_ITEM === type) {
      const obj3 = { section: null, onPress: null, isFirstRow: null, isLastRow: null };
      obj3[0] = item.section;
      obj3[1] = function onPress(shelfData) {
        memo2({ shelfData, sectionName: item.sectionName });
      };
      ({ isFirstRow: obj10[2], isLastRow: obj10[3] } = item);
      return memo(pinnedSearchBarBottomBorder, obj3, item.section.application.id);
    } else if (context(11265).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE === type) {
      ...
      obj4[0] = item;
      obj4[1] = context(9566).AppLauncherSectionName.ACTIVITIES;
      obj4[2] = function onPress(shelfData) {
        ...
        if (!tmp2) {
          tmp2 = memo3 !== context(9566).AppLauncherEntrypoint.VOICE;
        }
        obj[2] = tmp2;
        memo2(obj);
      };
      ...
      if (!clickOnHomeActivityOpensAppDetails) {
        tmp31 = memo3 === context(9566).AppLauncherEntrypoint.VOICE;
      }
      obj4[3] = tmp31;
      obj4[4] = memo4;
    ...
    } else if (context(11265).AppLauncherHomeListItemType.RECOMMENDATION_TUPLE === type) {
    ...
    } else if (context(11265).AppLauncherHomeListItemType.APP === type) {
      const obj6 = { section: null, onPress: null, isFirstRow: null, isLastRow: null, style: null };
      obj6[0] = item.section;
      obj6[1] = function onPress() {
        closure_1_13(item.section, context(9566).AppLauncherSectionName.INSTALLED);
      };
    ...
    } else if (context(11265).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER === type) {
      const obj8 = { index: null, children: null };
      obj8[0] = index;
      obj8[1] = item.section;
      return memo(trackAppLauncherHomeItemImpression, obj8);
    } else if (context(11265).AppLauncherHomeListItemType.RECOMMENDATION_BANNER_CARD === type) {
      ...
      return memo(entrypoint(11268), obj9);
    } else if (context(11265).AppLauncherHomeListItemType.RECOMMENDATION_APP === type) {
      ...
      return memo(entrypoint(11270), obj10, item.application.id);
    } else if (context(11265).AppLauncherHomeListItemType.VIEW_ALL === type) {
      const obj11 = { title: null, onPress: null };
      obj11[0] = item.title;
      obj11[1] = function onPress() {
        const applications = item.applications;
        const mapped = applications.map((item, index) => item);
        context(11224);
        const obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, navigation: closure_1_2, context: item, sectionName: item.sectionName, sectionOverallPosition: item.sectionOverallPosition, applications: mapped, sectionItemType: context(11225).SectionItemType.APPS, commands: [], sectionDescriptors: mapped.map((item, index) => callback(table[50]).getApplicationCommandSection(item)), title: item.title, promotedApplicationIds: item.promotedApplicationIds };
        const result = obj.handleViewAllSelected(obj);
      };
      return memo(entrypoint(11271), obj11);
    } else if (context(11265).AppLauncherHomeListItemType.DIVIDER_ITEM === type) {
      return memo(callback6, {});
    } else if (context(11265).AppLauncherHomeListItemType.LEARN_MORE === type) {
      obj = { visible: null };
      obj[0] = frecentApps.valueOf();
      return memo(entrypoint(11272), obj);
    } else {
      return null;
    }
  }, items19);
  ...
  const tmpResult3 = context(11266);
  [tmp51, c23] = frecentApps(obj2.useState(false), 2);
  const items20 = [entrypoint];
  memo9 = obj2.useMemo(() => entrypoint(12).debounce((query) => {
    context(closure_1_3[53]);
    const obj = { query, source: closure_10 };
  ...
  callback5 = obj2.useCallback((arg0) => {
    ...
    ref.current = context(6548).runAfterInteractions(() => {
      const current = ref.current;
      if (current != null) {
  ...
  pinnedSearchBarBottomBorder = context(11274).usePinnedSearchBarBottomBorder({ key: str, triggerScrollHeight: 5 });
  ...
  const layoutEffect = obj2.useLayoutEffect(() => {
    if (null != entrypoint) {
      ...
      if (current != null) {
        current.setText(entrypoint);
      }
      const current2 = ref1.current;
      if (current2 != null) {
        current2.focus();
      }
      ref2.current(entrypoint);
    }
  }, items24);
  let sum = entrypoint(1629)().bottom + memo2;
  c31 = sum;
  const tmpResult4 = context(11274);
  const bottomSheetFlashListBottomViewabilityInset = context(11275).useBottomSheetFlashListBottomViewabilityInset();
  ({ flashListRef, bottomVisibilityInsetRef } = bottomSheetFlashListBottomViewabilityInset);
  obj = { type: null, name: null };
  tmp4Result = tmp4(9177);
  ...
  const tmpResult5 = context(11275);
  ...
  stateFromStores3 = context(589).useStateFromStores(items26, () => store.getTriggeredOnboardingContentMetadata().willShowGlobalSearchOnboarding);
  ...
  callback6 = obj2.useCallback((arg0) => {
    if (stateFromStores3) {
      ...
      obj[0] = tmp;
      const result = obj.UNSAFE_markDismissibleContentAsDismissed(context(1377).DismissibleContent.APP_LAUNCHER_GLOBAL_SEARCH_ONBOARDING, obj);
      obj = {};
      const merged = Object.assign(store.getTriggeredOnboardingContentMetadata());
      obj.willShowGlobalSearchOnboarding = false;
      const result1 = context(11276).setTriggeredOnboardingContentMetadata(obj);
      const obj3 = context(11276);
    }
  }, items27);
  const tmpResult6 = context(589);
  trackAppLauncherHomeItemImpression = context(11277).useTrackAppLauncherHomeItemImpression().trackAppLauncherHomeItemImpression;
  ...
  const tmpResult7 = context(11277);
  const appLauncherFlashListProps = context(11279).useAppLauncherFlashListProps({ onScrollHandler: callback8 });
  ...
  obj3[1] = memo(context(7337).SearchField, obj4);
  ...
  if (tmp51) {
    ...
    obj6[3] = callback;
    obj6[4] = entrypoint;
    let tmp77Result = tmp77(tmp(11281).SearchLocalAndGlobalContentView, obj6);
  } else {
    ...
    obj7[6] = function onCommandSelected(command, section) {
      context(11224);
      const obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, context, command, section, sectionDescriptors, query: "", navigation: closure_2, sectionName: context(9566).AppLauncherSectionName.RECENT_COMMANDS, entrypoint: memo3 };
      const result = obj.handleApplicationCommandSelected(obj);
    };
    obj7[7] = function onViewAllSelected(arg0) {
      context(11224);
      const obj = { location: context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_APPLICATION_VIEW_FRECENCT, navigation: closure_2, context, sectionName: null, applications: null, sectionItemType: null, commands: null, sectionDescriptors: null, title: null };
      let str = "recent_apps_view_more";
      if (arg0 === context(11225).SectionItemType.COMMANDS) {
        str = "recent_commands_view_more";
      }
      obj[3] = str;
      const mapped = memo1.map((item, index) => {
      ...
      obj[4] = mapped.filter(context(1370).isNotNullish);
      obj[5] = arg0;
      obj[6] = frecencyCommands;
      obj[7] = sectionDescriptors;
      if (memo3 === context(9566).AppLauncherEntrypoint.VOICE) {
        const intl3 = context(1236).intl;
        let stringResult = intl3.string(context(1236).t["2pFD8L"]);
      } else if (arg0 === context(11225).SectionItemType.COMMANDS) {
        const intl2 = context(1236).intl;
        stringResult = intl2.string(context(1236).t.V3Sq95);
      } else {
        const intl = context(1236).intl;
        stringResult = intl.string(context(1236).t.SCViVk);
      }
      obj[8] = stringResult;
      const result = obj.handleViewAllSelected(obj);
    };
    const items35 = [tmp77(tmp4(11225), obj7), , , ];
    let obj8 = { items: null, onAppSelected: null, onViewAllSelected: null };
    obj8[0] = memo;
    obj8[1] = callback3;
    obj8[2] = function onViewAllSelected() {
      const found = memo.find((item, index) => item.type === callback(11265).AppLauncherHomeListItemType.VIEW_ALL);
      let mapped;
      if (found != null) {
        const applications = found.applications;
        if (applications != null) {
          mapped = applications.map((item, index) => item);
        }
      }
      if (null != mapped) {
        context(11224);
        const obj = { location: null, navigation: null, context: null, sectionName: null, applications: null, sectionItemType: null, commands: null, sectionDescriptors: null, title: null };
        obj[0] = context(5248).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME;
        obj[1] = closure_2;
        obj[2] = context;
        obj[3] = context(9566).AppLauncherSectionName.APPS_IN_THIS_SERVER;
        obj[4] = mapped;
        obj[5] = context(11225).SectionItemType.APPS;
        obj[6] = [];
        obj[7] = mapped.map((item, index) => callback(5245).getApplicationCommandSection(item));
        const intl = context(1236).intl;
        obj[8] = intl.string(context(1236).t.oJyzCu);
        const result = obj.handleViewAllSelected(obj);
      }
    };
    items35[1] = tmp77(tmp4(11287), obj8);
    tmp77Result = null;
    if (tmp40) {
      tmp77Result = null;
      if (tmp64) {
        tmp77Result = tmp77(tmp4(11288), {});
      }
    }
    items35[2] = tmp77Result;
    let tmp77Result1 = null;
    if (memo2) {
      tmp77Result1 = tmp77(tmp4(11290), {});
    }
    ...
    if (entrypoint === tmp(9566).AppLauncherEntrypoint.VOICE) {
    ...
    tmp77Result = tmp77(tmp4(11279), obj9);
    const tmp4Result1 = tmp4(11279);
  }
  items33[2] = tmp77Result;
  let tmp77Result2 = entrypoint === tmp(9566).AppLauncherEntrypoint.TEXT;
  if (tmp77Result2) {
    let obj11 = { windowDimensions: null, visible: null, markAsDismissed: null };
    obj11[0] = size;
    obj11[1] = stateFromStores3;
    obj11[2] = callback6;
    tmp77Result2 = tmp77(tmp4(11293), obj11);
  }
