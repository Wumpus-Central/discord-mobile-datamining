...
import ActivityShelfBadgeDefault from "ActivityShelfBadge" /* 12215 */;
...
import ExpandableList from "ExpandableList" /* 12237 */;
...
import ApplicationCollectionFlags from "ApplicationCollectionFlags" /* 12255 */;
...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = closure_19(onPress(12185), obj);
  }
  const items = [section];
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        if (arr.length % 2 === 1) {
          ...
          obj.type = tmp8(12217).AppLauncherHomeListItemType.SHELF_ITEM_TUPLE;
        ...
        obj = { type: tmp8(12217).AppLauncherHomeListItemType.SECTION_HEADER, section: null, sectionName: "activities", numItems: null, numVisibleItems: null };
        ...
        if (type === ApplicationDirectoryCollectionType.ApplicationDirectoryCollectionType.BANNER_CARDS) {
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9547).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              obj.showsPromoted = tmpResult.hasFlag(type.flags, found1(12254).ApplicationCollectionItemFlags.PROMOTED);
          ...
          const mapped1 = prop.map((type) => {
            if (type.type === found1(9547).ApplicationDirectoryCollectionItemType.APPLICATION) {
              const obj = { application: type.application, showsPromoted: found1(1384).hasFlag(type.flags, found1(12254).ApplicationCollectionItemFlags.PROMOTED) };
              return obj;
            }
          });
