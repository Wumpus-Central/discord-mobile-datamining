...
function ActivityRow(section) {
  ...
  return closure_19(section(5824).TableRow, {
    icon: tmp5,
  ...
  const memo4 = obj3.useMemo(() => {
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        const obj5 = {
          type: tmp8(12345).AppLauncherHomeListItemType.SECTION_HEADER,
          section: null,
          sectionName: "activities",
          numItems: null,
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9537).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              const obj3 = {
                ...
                showsPromoted: found1(1385).hasFlag(type.flags, found1(12382).ApplicationCollectionItemFlags.PROMOTED),
      ...
      onViewAllSelected() {
        ...
        if (null != mapped) {
          const obj2 = {
            ...
            sectionItemType: FrecencySection.SectionItemType.APPS,
            commands: [],
            sectionDescriptors: mapped.map((item) => context(7766).getApplicationCommandSection(item)),
            title: null,
          };
          const intl = util.intl;
