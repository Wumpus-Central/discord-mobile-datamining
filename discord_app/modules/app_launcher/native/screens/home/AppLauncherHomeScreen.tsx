...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = callback2(onPress(11628), obj);
  }
  const items = [section];
  ...
  return callback2(section(5599).TableRow, {
    icon: tmp5,
...
function ActivityItemTuple(arg0) {
  ...
  const size = React.useMemo(() => {
    if (null == containerWidth) {
      return { width: "children", height: "c" };
    } else {
...
function ActivityItem(context) {
  ...
  const items3 = [
    tmp17,
    callback2(shelfItem(flag[33]), { labelType: shelfBadgeTypeIfActive }),
    callback2(context(flag[34]).SubmittingOverlay, { submitting: isLaunching, style: tmp.submittingOverlay }),
  ];
  obj5[1] = items3;
  ...
  items4[1] = callback2(closure_6, {
    style: tmp.activityDetailsContainer,
    children: callback2(context(flag[16]).Text, obj8),
  });
  obj3[3] = items4;
...
function RecommendationItemTuple(isLastTuple) {
  ...
  return closure_19(num, {
    ...
    children: items.map((application) => {
      ...
      obj = {
        context: closure_0,
        sectionName: closure_1,
        onPress: closure_2,
        item: application,
        entrypoint: closure_3,
...
function RecommendationItem(onPress) {
  ...
  items3[1] = callback2(closure_6, {
    style: tmp.activityDetailsContainer,
    children: callback2(sectionName(application[16]).Text, obj8),
  });
  obj3[2] = items3;
...
function Divider() {
  ...
  const style = [
    callback4().divider,
    { marginLeft: -DEFAULT_CONTENT_PADDING - rect.left, marginRight: -DEFAULT_CONTENT_PADDING - rect.right },
  ];
...
function getRecommendationItemsWithViewAll(arr) {
  ...
  if (0 === arr.length) {
    ...
    const item = substr.forEach((application) => {
      const obj = {
        type: callback(table[45]).AppLauncherHomeListItemType.RECOMMENDATION_APP,
...
export default function AppLauncherHomeScreen(route) {
  ...
  const tmp5 = entrypoint(8569)(hasViewedActivityItem);
  const tmp6 = entrypoint(8569)(hasViewedLearnMoreItem);
  callback = tmp6;
  let tmp7 = callback4();
  frecentApps = tmp7;
  let tmp8 = entrypoint(7775)();
  closure_6 = tmp8;
  obj1 = context(9049);
  ...
  const callback1 = frecentApps.useCallback((application) => {
    ...
    obj = {
      location: context(closure_3[29]).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME,
      application: null,
      navigation: null,
      context: null,
      sectionName: null,
  ...
  let tmp19 = entrypoint === context(8435).AppLauncherEntrypoint.VOICE;
  callback = tmp19;
  let tmp20 = entrypoint(11691)({ context, onlyActivityApps: tmp19 });
  ({ frecencyCommands, frecentApps } = tmp20);
  ({ sectionDescriptors, loading } = tmp20);
  let tmp21 = entrypoint === context(8435).AppLauncherEntrypoint.TEXT;
  closure_6 = tmp21;
  appsInThisServer = tmp4(11694)({ context }).appsInThisServer;
  ...
  memo = obj2.useMemo(() => {
    ...
    return callback8(
      mapped,
      "in_this_server",
      intl.string(context(closure_3[17]).t.oJyzCu),
      context(closure_3[68]).IN_THIS_SERVER_ITEM_MAX,
    );
  }, items4);
  const items5 = [frecentApps];
  memo1 = obj2.useMemo(
    () =>
      frecentApps.map((applicationId) => ({
        type: closure_1_0(closure_1_3[45]).AppLauncherHomeListItemType.APP,
        applicationId: applicationId.id,
        section: applicationId,
        isFirstRow: 0 === arg1,
        isLastRow: arg1 === length.length - 1,
        sectionName: "recents",
      })),
    items5,
  );
  ...
  const stateFromStores = tmpResult.useStateFromStores(items8, () =>
    stateFromStores1.get("only_show_preview_app_collections"),
  );
  tmp4Result = tmp4(11689);
  const ApplicationCollectionActiveState = tmp(11650).ApplicationCollectionActiveState;
  if (stateFromStores) {
  ...
  const items9 = [memo3];
  stateFromStores1 = context(589).useStateFromStores(items9, () =>
    memo3.getCollections({ surface: memo9, activeState: PREVIEW }),
  );
  ...
  const memo7 = obj2.useMemo(() => {
    ...
    if (memo8) {
      ...
      let item = memo3.forEach((title) => {
        ...
        if (type === context(11695).ApplicationDirectoryCollectionType.BANNER_CARDS) {
          const _Math = Math;
          let obj = { type: null, section: null, sectionName: null, numItems: null, numVisibleItems: null };
          const bound = Math.min(length, tmp(11680).COLLAPSED_LIST_ITEM_MAX);
          obj[0] = tmp(11660).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          const mapped = application_directory_collection_items.map((type) => {
            if (type.type === found1(8444).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let collectionItemAssetUrl;
              if (tmp4) {
                let tmpResult = tmp(11696);
              ...
              tmpResult = tmp(1399);
              obj[1] = tmpResult.hasFlag(type.flags, tmp(11697).ApplicationCollectionItemFlags.PROMOTED);
          ...
          if (0 < found.length) {
            do {
              ...
              obj[0] = context(11660).AppLauncherHomeListItemType.RECOMMENDATION_TUPLE;
              obj[1] = title.title;
              obj[2] = arg1;
        ...
        } else if (type === tmp(11695).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          ...
          const mapped1 = prop.map((type) => {
            if (type.type === found1(8444).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              obj[0] = type.application;
              obj[1] = tmp(1399).hasFlag(type.flags, tmp(11697).ApplicationCollectionItemFlags.PROMOTED);
              return obj;
            }
          });
          found1 = mapped1.filter(tmp(1471).isNotNullish);
          if (tmpResult.hasFlag(title.flags, tmp(11698).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  callback3 = obj2.useCallback((application) => {
    ...
    obj = {
      location: tmp(tmp2[29]).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME,
      application,
      navigation: num,
      context: application,
  ...
  clickOnHomeActivityOpensAppDetails = context(11661).useClickOnHomeActivityOpensAppDetails();
  const items19 = [
  ...
  callback4 = obj2.useCallback((item) => {
    ...
    if (context(closure_3[45]).AppLauncherHomeListItemType.PLACEHOLDER === type) {
      ...
      obj4[2] = function onPress(shelfData) {
        ...
        if (!tmp2) {
          tmp2 = closure_1_10 !== context(8435).AppLauncherEntrypoint.VOICE;
        }
        obj[2] = tmp2;
        closure_1_14(obj);
      };
      ...
      obj6[1] = function onPress() {
        closure_1_13(item.section, context(8435).AppLauncherSectionName.INSTALLED);
      };
      ...
      obj11[1] = function onPress() {
        ...
        let obj = context(11623);
        obj = {
          location: context(7268).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME,
          navigation: closure_1_2,
          context: item,
          sectionName: item.sectionName,
          sectionOverallPosition: item.sectionOverallPosition,
          applications: mapped,
          sectionItemType: context(11624).SectionItemType.APPS,
          commands: [],
          sectionDescriptors: mapped.map((application) =>
            callback(table[51]).getApplicationCommandSection(application),
          ),
          title: item.title,
  ...
  pinnedSearchBarBottomBorder = context(11669).usePinnedSearchBarBottomBorder({ key: str, triggerScrollHeight: 5 });
  ...
  let sum = entrypoint(1628)().bottom + closure_14;
  c31 = sum;
  const tmpResult4 = context(11669);
  const bottomSheetFlashListBottomViewabilityInset = context(11670).useBottomSheetFlashListBottomViewabilityInset();
  ({ flashListRef, bottomVisibilityInsetRef } = bottomSheetFlashListBottomViewabilityInset);
  obj = { type: null, name: null };
  const tmpResult5 = context(11670);
  ...
  const items25 = [tmp5];
  entrypoint(8892)(obj, obj, items25);
  const tmp4Result1 = entrypoint(8892);
  ...
  stateFromStores3 = context(589).useStateFromStores(
    items26,
    () => store.getTriggeredOnboardingContentMetadata().willShowGlobalSearchOnboarding,
  );
  ...
  callback6 = obj2.useCallback((arg0) => {
    if (stateFromStores3) {
      ...
      const result = obj.UNSAFE_markDismissibleContentAsDismissed(
        context(closure_3[61]).DismissibleContent.APP_LAUNCHER_GLOBAL_SEARCH_ONBOARDING,
        obj,
      );
      obj = {};
  ...
  trackAppLauncherHomeItemImpression =
    context(11672).useTrackAppLauncherHomeItemImpression().trackAppLauncherHomeItemImpression;
  ...
  const tmpResult7 = context(11672);
  const appLauncherFlashListProps = context(11674).useAppLauncherFlashListProps({ onScrollHandler: callback8 });
  ...
  if (entrypoint === context(8435).AppLauncherEntrypoint.VOICE) {
  ...
  obj3[1] = memo(context(6221).SearchField, obj4);
  ...
  if (tmp51) {
    ...
    obj6[3] = callback;
    obj6[4] = entrypoint;
    let tmp77Result = tmp77(tmp(11676).SearchLocalAndGlobalContentView, obj6);
  } else {
    let obj7 = {
      context: null,
    ...
    obj7[6] = function onCommandSelected(command, section) {
      ...
      obj = {
        ...
        query: "",
        navigation: num,
        sectionName: context(closure_3[42]).AppLauncherSectionName.RECENT_COMMANDS,
        entrypoint: stateFromStores1,
      };
    ...
    obj7[7] = function onViewAllSelected(arg0) {
      let obj = context(closure_3[23]);
      obj = {
        location: context(closure_3[29]).ApplicationCommandTriggerLocations.APP_LAUNCHER_APPLICATION_VIEW_FRECENCT,
        navigation: num,
        context,
        sectionName: null,
        applications: null,
    ...
    const items35 = [tmp77(tmp4(11624), obj7), , ,];
    let obj8 = { items: null, onAppSelected: null, onViewAllSelected: null };
    obj8[0] = memo;
    obj8[1] = callback3;
    obj8[2] = function onViewAllSelected() {
      const found = memo.find((type) => type.type === callback(11660).AppLauncherHomeListItemType.VIEW_ALL);
      let mapped;
      if (found != null) {
      ...
      if (null != mapped) {
        ...
        obj[6] = [];
        obj[7] = mapped.map((application) => callback(7266).getApplicationCommandSection(application));
        const intl = context(closure_3[17]).intl;
        obj[8] = intl.string(context(closure_3[17]).t.oJyzCu);
        const result = obj.handleViewAllSelected(obj);
      }
    };
    items35[1] = tmp77(tmp4(11682), obj8);
    tmp77Result = null;
    if (tmp40) {
      tmp77Result = null;
      if (tmp64) {
        tmp77Result = tmp77(tmp4(11683), {});
      }
    }
    items35[2] = tmp77Result;
    let tmp77Result1 = null;
    if (memo2) {
      tmp77Result1 = tmp77(tmp4(11685), {});
    }
    let obj9 = {
    ...
    if (entrypoint === tmp(8435).AppLauncherEntrypoint.VOICE) {
    ...
    tmp77Result = tmp77(tmp4(11674), obj9);
    const tmp4Result2 = tmp4(11674);
  }
  items33[2] = tmp77Result;
  let tmp77Result2 = entrypoint === tmp(8435).AppLauncherEntrypoint.TEXT;
  if (tmp77Result2) {
    let obj11 = { windowDimensions: null, visible: null, markAsDismissed: null };
    obj11[0] = size;
    obj11[1] = stateFromStores3;
    obj11[2] = callback6;
    tmp77Result2 = tmp77(tmp4(11688), obj11);
  }
