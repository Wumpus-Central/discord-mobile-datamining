...
function ActivityRow(section) {
  ...
  return closure_19(section(5775).TableRow, {
    icon: tmp5,
  ...
  const memo4 = obj3.useMemo(() => {
    ...
    if (0 !== handleViewableItemsChanged.length) {
      if (initialSearchQuery === AppLauncherTypes.AppLauncherEntrypoint.VOICE) {
        ...
        const obj5 = {
          type: tmp8(12330).AppLauncherHomeListItemType.SECTION_HEADER,
          section: null,
          sectionName: "activities",
          numItems: null,
          ...
          const mapped = application_directory_collection_items.map((type, sectionPosition) => {
            if (type.type === found1(9655).ApplicationDirectoryCollectionItemType.APPLICATION) {
              ...
              if (tmp4) {
                ...
                collectionItemAssetUrl = found1(12366).getCollectionItemAssetUrl({ itemId: null, hash: null });
                const obj = { itemId: null, hash: null };
                const tmpResult = found1(12366);
              }
              const obj3 = {
                application: type.application,
                showsPromoted: found1(1385).hasFlag(type.flags, found1(12367).ApplicationCollectionItemFlags.PROMOTED),
      ...
      onViewAllSelected() {
        ...
        if (null != mapped) {
          const obj2 = {
            ...
            sectionItemType: FrecencySection.SectionItemType.APPS,
            commands: [],
            sectionDescriptors: mapped.map((item) => context(7717).getApplicationCommandSection(item)),
            title: null,
          };
          const intl = util.intl;
