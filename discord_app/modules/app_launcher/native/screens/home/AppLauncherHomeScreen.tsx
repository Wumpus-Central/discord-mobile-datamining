...
function ActivityRow(section) {
  ...
  return callback2(section(5315).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
...
export default function AppLauncherHomeScreen(route) {
  ...
  const tmp5 = entrypoint(8654)(hasViewedActivityItem);
  const tmp6 = entrypoint(8654)(hasViewedLearnMoreItem);
  frecentApps = tmp6;
  let tmp7 = createCacheKey();
  React = tmp7;
  let tmp8 = entrypoint(8334)();
  ...
  dependencyMap = tmp19;
  let tmp20 = entrypoint(11418)({ context, onlyActivityApps: tmp19 });
  ...
  React = tmp21;
  appsInThisServer = tmp4(11421)({ context }).appsInThisServer;
  ...
  const memo7 = obj2.useMemo(() => {
    ...
    if (memo2) {
      ...
      let item = memo3.forEach((title) => {
        ...
        if (type === context(11423).ApplicationDirectoryCollectionType.BANNER_CARDS) {
          const _Math = Math;
          let obj = { type: null, section: null, sectionName: null, numItems: null, numVisibleItems: null };
          const bound = Math.min(length, tmp(11407).COLLAPSED_LIST_ITEM_MAX);
          obj[0] = tmp(11387).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          for (let num3 = 0; num3 < application_directory_collection_items.length; num3 = num3 + 2) {
            ...
            if (tmp20.type === context(11422).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let tmp26 = null != tmp20.id && null != tmp20.image_hash;
              let collectionItemAssetUrl;
              if (tmp26) {
                let tmp24Result = tmp24(11424);
              ...
              obj[1] = tmp24Result.hasFlag(tmp20.flags, tmp24(11425).ApplicationCollectionItemFlags.PROMOTED);
              obj[2] = collectionItemAssetUrl;
              let tmp28;
              if (null != tmp23) {
                if (tmp23.type === tmp24(11422).ApplicationDirectoryCollectionItemType.APPLICATION) {
                  let tmp29 = null != tmp23.id && null != tmp23.image_hash;
                  let collectionItemAssetUrl1;
                  if (tmp29) {
                    let tmp24Result1 = tmp24(11424);
                  ...
                  obj2[1] = tmp24Result2.hasFlag(tmp23.flags, tmp24(11425).ApplicationCollectionItemFlags.PROMOTED);
              ...
              let tmp31 = found;
              obj3[0] = tmp24(11387).AppLauncherHomeListItemType.RECOMMENDATION_TUPLE;
              obj3[1] = title.title;
              obj3[2] = arg1;
              obj3[3] = obj;
              obj3[4] = tmp28;
        ...
        } else if (type === tmp(11423).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          ...
          if (tmpResult.hasFlag(title.flags, tmp(11426).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  clickOnHomeActivityOpensAppDetails = context(11388).useClickOnHomeActivityOpensAppDetails();
  ...
  const tmpResult3 = context(11388);
  ...
  pinnedSearchBarBottomBorder = context(11396).usePinnedSearchBarBottomBorder({ key: str, triggerScrollHeight: 5 });
  ...
  c31 = sum;
  const tmpResult4 = context(11396);
  const bottomSheetFlashListBottomViewabilityInset = context(11397).useBottomSheetFlashListBottomViewabilityInset();
  ...
  const tmpResult5 = context(11397);
  ...
  trackAppLauncherHomeItemImpression = context(11399).useTrackAppLauncherHomeItemImpression().trackAppLauncherHomeItemImpression;
  ...
  const tmpResult7 = context(11399);
  const appLauncherFlashListProps = context(11401).useAppLauncherFlashListProps({ onScrollHandler: callback8 });
  ...
  obj3[1] = memo(context(5901).SearchField, obj4);
  ...
  if (tmp51) {
    ...
    obj6[3] = callback;
    obj6[4] = entrypoint;
    let tmp77Result = tmp77(tmp(11403).SearchLocalAndGlobalContentView, obj6);
  } else {
    ...
    obj8[2] = function onViewAllSelected() {
      const found = memo.find((type) => type.type === callback(11387).AppLauncherHomeListItemType.VIEW_ALL);
      let mapped;
      if (found != null) {
      ...
      if (null != mapped) {
        ...
        obj[6] = [];
        obj[7] = mapped.map((application) => callback(6889).getApplicationCommandSection(application));
        const intl = context(_undefined2[17]).intl;
        obj[8] = intl.string(context(_undefined2[17]).t.oJyzCu);
        const result = obj.handleViewAllSelected(obj);
      }
    };
    items35[1] = tmp77(tmp4(11409), obj8);
    tmp77Result = null;
    if (tmp40) {
      tmp77Result = null;
      if (tmp64) {
        tmp77Result = tmp77(tmp4(11410), {});
      }
    }
    items35[2] = tmp77Result;
    let tmp77Result1 = null;
    if (memo2) {
      tmp77Result1 = tmp77(tmp4(11412), {});
    }
    ...
    tmp77Result = tmp77(tmp4(11401), obj9);
    const tmp4Result1 = tmp4(11401);
  }
  ...
  if (tmp77Result2) {
    ...
    obj11[2] = callback6;
    tmp77Result2 = tmp77(tmp4(11415), obj11);
  }
