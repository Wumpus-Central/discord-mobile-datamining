...
function ActivityRow(section) {
  ...
  if (tmp5) {
    ...
    tmp5 = callback2(onPress(11446), obj);
  }
  const items = [section];
  ...
  return callback2(section(5374).TableRow, { icon: tmp5, label: section.application.name, subLabel: memo, subLabelLineClamp: 1, start: flag, end: flag2, arrow: true, onPress: callback });
}
...
export default function AppLauncherHomeScreen(route) {
  ...
  const tmp5 = entrypoint(8738)(hasViewedActivityItem);
  const tmp6 = entrypoint(8738)(hasViewedLearnMoreItem);
  frecentApps = tmp6;
  let tmp7 = createCacheKey();
  React = tmp7;
  let tmp8 = entrypoint(8529)();
  appsInThisServer = tmp8;
  let obj1 = context(9554);
  ...
  memo8 = undefined;
  frecencyCommands = undefined;
  let obj3 = context(11508);
  const fetchDeveloperActivityShelfItems = obj3.useFetchDeveloperActivityShelfItems();
  let guild_id;
  if ("channel" === context.type) {
    guild_id = context.channel.guild_id;
  }
  const tmp16Result = entrypoint(11429)({ guildId: guild_id });
  c2 = tmp16Result;
  const tmp19 = entrypoint === context(9555).AppLauncherEntrypoint.VOICE;
  dependencyMap = tmp19;
  let tmp20 = entrypoint(11509)({ context, onlyActivityApps: tmp19 });
  ({ frecencyCommands, frecentApps } = tmp20);
  ({ sectionDescriptors, loading } = tmp20);
  const tmp21 = entrypoint === context(9555).AppLauncherEntrypoint.TEXT;
  React = tmp21;
  appsInThisServer = tmp4(11512)({ context }).appsInThisServer;
  ...
  let tmp4Result = tmp4(11507);
  const ApplicationCollectionActiveState = tmp(11468).ApplicationCollectionActiveState;
  if (stateFromStores) {
  ...
  const memo7 = obj2.useMemo(() => {
    ...
    if (memo2) {
      ...
      let item = memo3.forEach((title) => {
        ...
        if (type === context(11514).ApplicationDirectoryCollectionType.BANNER_CARDS) {
          const _Math = Math;
          let obj = { type: null, section: null, sectionName: null, numItems: null, numVisibleItems: null };
          const bound = Math.min(length, tmp(11498).COLLAPSED_LIST_ITEM_MAX);
          obj[0] = tmp(11478).AppLauncherHomeListItemType.RECOMMENDATION_SECTION_HEADER;
          ...
          for (let num3 = 0; num3 < application_directory_collection_items.length; num3 = num3 + 2) {
            ...
            if (tmp20.type === context(11513).ApplicationDirectoryCollectionItemType.APPLICATION) {
              let tmp26 = null != tmp20.id && null != tmp20.image_hash;
              let collectionItemAssetUrl;
              if (tmp26) {
                let tmp24Result = tmp24(11515);
              ...
              obj[1] = tmp24Result.hasFlag(tmp20.flags, tmp24(11516).ApplicationCollectionItemFlags.PROMOTED);
              obj[2] = collectionItemAssetUrl;
              let tmp28;
              if (null != tmp23) {
                if (tmp23.type === tmp24(11513).ApplicationDirectoryCollectionItemType.APPLICATION) {
                  let tmp29 = null != tmp23.id && null != tmp23.image_hash;
                  let collectionItemAssetUrl1;
                  if (tmp29) {
                    let tmp24Result1 = tmp24(11515);
                  ...
                  obj2[1] = tmp24Result2.hasFlag(tmp23.flags, tmp24(11516).ApplicationCollectionItemFlags.PROMOTED);
              ...
              let tmp31 = found;
              obj3[0] = tmp24(11478).AppLauncherHomeListItemType.RECOMMENDATION_TUPLE;
              obj3[1] = title.title;
              obj3[2] = arg1;
              obj3[3] = obj;
              obj3[4] = tmp28;
        ...
        } else if (type === tmp(11514).ApplicationDirectoryCollectionType.EXPANDABLE_LIST) {
          ...
          if (tmpResult.hasFlag(title.flags, tmp(11517).ApplicationCollectionFlags.APPENDS_REMAINING_ACTIVITIES)) {
  ...
  clickOnHomeActivityOpensAppDetails = context(11479).useClickOnHomeActivityOpensAppDetails();
  ...
  const callback4 = obj2.useCallback((item) => {
    ...
    if (context(_undefined2[44]).AppLauncherHomeListItemType.PLACEHOLDER === type) {
      ...
      obj4[2] = function onPress(shelfData) {
        ...
        let tmp2 = outer1_21;
        if (!tmp2) {
          tmp2 = outer1_10 !== context(9555).AppLauncherEntrypoint.VOICE;
        }
        obj[2] = tmp2;
        outer1_14(obj);
      };
      ...
      obj6[1] = function onPress() {
        outer1_13(item.section, context(9555).AppLauncherSectionName.INSTALLED);
      };
      ...
      obj11[1] = function onPress() {
        ...
        let obj = context(11441);
        obj = { location: context(6985).ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME, navigation: outer1_2, context: item, sectionName: item.sectionName, sectionOverallPosition: item.sectionOverallPosition, applications: mapped, sectionItemType: context(11442).SectionItemType.APPS, commands: [], sectionDescriptors: mapped.map((application) => callback(table[50]).getApplicationCommandSection(application)), title: item.title, promotedApplicationIds: item.promotedApplicationIds };
  ...
  const tmpResult3 = context(11479);
  ...
  pinnedSearchBarBottomBorder = context(11487).usePinnedSearchBarBottomBorder({ key: str, triggerScrollHeight: 5 });
  ...
  c31 = sum;
  const tmpResult4 = context(11487);
  const bottomSheetFlashListBottomViewabilityInset = context(11488).useBottomSheetFlashListBottomViewabilityInset();
  ...
  const tmpResult5 = context(11488);
  ...
  trackAppLauncherHomeItemImpression = context(11490).useTrackAppLauncherHomeItemImpression().trackAppLauncherHomeItemImpression;
  ...
  const tmpResult7 = context(11490);
  const appLauncherFlashListProps = context(11492).useAppLauncherFlashListProps({ onScrollHandler: callback8 });
  ...
  if (entrypoint === context(9555).AppLauncherEntrypoint.VOICE) {
  ...
  obj3[1] = memo(context(5994).SearchField, obj4);
  ...
  if (tmp51) {
    ...
    obj6[3] = callback;
    obj6[4] = entrypoint;
    let tmp77Result = tmp77(tmp(11494).SearchLocalAndGlobalContentView, obj6);
  } else {
    ...
    const items35 = [tmp77(tmp4(11442), obj7), , , ];
    let obj8 = { items: null, onAppSelected: null, onViewAllSelected: null };
    obj8[0] = memo;
    obj8[1] = callback3;
    obj8[2] = function onViewAllSelected() {
      const found = memo.find((type) => type.type === callback(11478).AppLauncherHomeListItemType.VIEW_ALL);
      let mapped;
      if (found != null) {
      ...
      if (null != mapped) {
        ...
        obj[6] = [];
        obj[7] = mapped.map((application) => callback(6983).getApplicationCommandSection(application));
        const intl = context(_undefined2[17]).intl;
        obj[8] = intl.string(context(_undefined2[17]).t.oJyzCu);
        const result = obj.handleViewAllSelected(obj);
      }
    };
    items35[1] = tmp77(tmp4(11500), obj8);
    tmp77Result = null;
    if (tmp40) {
      tmp77Result = null;
      if (tmp64) {
        tmp77Result = tmp77(tmp4(11501), {});
      }
    }
    items35[2] = tmp77Result;
    let tmp77Result1 = null;
    if (memo2) {
      tmp77Result1 = tmp77(tmp4(11503), {});
    }
    ...
    if (entrypoint === tmp(9555).AppLauncherEntrypoint.VOICE) {
    ...
    tmp77Result = tmp77(tmp4(11492), obj9);
    const tmp4Result1 = tmp4(11492);
  }
  items33[2] = tmp77Result;
  let tmp77Result2 = entrypoint === tmp(9555).AppLauncherEntrypoint.TEXT;
  if (tmp77Result2) {
    let obj11 = { windowDimensions: null, visible: null, markAsDismissed: null };
    obj11[0] = size;
    obj11[1] = stateFromStores3;
    obj11[2] = callback6;
    tmp77Result2 = tmp77(tmp4(11506), obj11);
  }
