...
export default noop.memo(function DetailsHeader(viewContainerStyle) {
  ...
  const isPartnerApplicationResult = sharedValue(9409).isPartnerApplication(application);
  sharedValue = sharedValue(4493).useSharedValue(null);
  let obj3 = sharedValue(4493);
  let fn = function t() {
  ...
  const obj4 = sharedValue(4493);
  const sectionName = sharedValue(9409).getSectionName(application);
  const obj5 = sharedValue(9409);
  const str = sharedValue(9409).getSectionDescription(application);
  ...
  isScreenLandscape = sharedValue(5344).useIsScreenLandscape();
  const tmp24 = first(8547)(isScreenLandscape);
  closure_18 = tmp24;
  ...
  let tmp27 = first;
  const tmp2Result = sharedValue(5344);
  if (first) {
    tmp27 = !first1;
  }
  ...
  if (!hideName) {
    ...
    tmp30Result = tmp30(tmp2(4753).Heading, obj10);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj11.children = tmp30(tmp2(4753).Text, obj12);
  ...
  if (tmp28Result5) {
    ...
    if (tmp27) {
      ...
      const obj19 = {
        start: tmp2(1094).HorizontalGradient.START,
        end: tmp2(1094).HorizontalGradient.END,
        colors,
        style: tmp.maskFade,
      };
      items8[1] = tmp30(tmp23(5199), obj19);
      if (num2 == null) {
        num2 = 0;
      }
    ...
    obj22.children = tmp30(tmp2(4753).Text, obj23);
    const items10 = [tmp30(tmp23(5883), obj22), ,];
    ...
    if (tmp27) {
      ...
      obj24.children = tmp28(tmp2(4753).Text, obj25);
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      tmp30Result9 = tmp30(tmp2(4753).Heading, obj28);
    }
    ...
    if (tmp28Result5) {
      ...
      const items15 = [tmp30(tmp2(4753).Text, obj30)];
      const obj31 = {
        variant: "text-sm/medium",
        color: "text-brand",
        style: tmp.collapseDescriptionCTA,
      ...
      items15[1] = tmp30(tmp2(4753).Text, obj31);
