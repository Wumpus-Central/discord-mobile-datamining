...
function TryItOutButton(botUserId) {
  ...
  const callback = noop.useCallback(
    asyncGeneratorStep(async () => {
      tmp3(1241).track(constants.APP_DETAIL_PAGE_ENTRY_POINT_COMMAND_BUTTON_CLICKED, {
        ...
        button_action: botUserId(9529).EntryPointCommandButtonActions.OPEN_APP_DM,
      });
      const _setTimeout = setTimeout;
      closure_4.current = setTimeout(() => {
        dependencyMap(true);
      }, 250);
      await tmp3(4770).openPrivateChannel({ recipientIds: botUserId });
      if (1 === tmp7) {
        dependencyMap = 0;
  ...
  return closure_11(botUserId(5187).Button, obj);
}
...
export default function ActivityDetailContent(application) {
  ...
  const tmp = closure_15();
  const width = application(11485).useRequiredAppLauncherContext().width;
  let obj = application(11485);
  const getPrimaryAppCommand = application(9607).useGetPrimaryAppCommand(context, application.id);
  const obj2 = application(9607);
  ...
  let detailsContainerLandscape = entrypoint !== application(9529).AppLauncherEntrypoint.VOICE && isScreenLandscape;
  ...
  if (tmp2Result7.isRealApplication(application)) {
    ...
    if (isDeveloperOfThisApp) {
      ...
      obj5.style = obj4;
      const items1 = [closure_11(tmp2(5821).TableRowDivider, {}), ,];
      ...
      items1[1] = closure_11(tmp2(7447).TableSwitchRow, obj6);
      ...
      if (useActivityUrlOverride) {
        ...
        obj7.children = closure_11(tmp2(6850).TextInput, obj8);
    ...
    const activityAction = tmp2(12314).useActivityAction(obj9);
    const tmp2Result9 = tmp2(12314);
    const delayedSwapToActivityActionLeave = tmp2(12399).useDelayedSwapToActivityActionLeave(activityAction);
    const obj10 = { context, application, activityAction: delayedSwapToActivityActionLeave };
    const tmp2Result10 = tmp2(12399);
    ({ reason, disabled } = tmp5(12400)(obj10));
    ...
    if (null != getPrimaryAppCommand) {
      ...
      if (tmp39Result2) {
        ...
        tmp39Result2 = closure_11(tmp2(4753).Text, obj17);
      }
      ...
      if (isDeveloperOfThisApp) {
        isDeveloperOfThisApp = tmp2(9409).isEmbeddedApp(application);
        const tmp2Result11 = tmp2(9409);
      }
      if (isDeveloperOfThisApp) {
    ...
    let result = width;
    const tmp29 = tmp5(12400)(obj10);
    if (detailsContainerLandscape) {
      result = (65 * width) / 100;
    }
    obj25.width = result;
    obj25.contentWidth = tmp8;
    obj24.children = closure_11(tmp5(12341), obj25);
    ...
    const items7 = [closure_11(tmp5(9408), obj27), , , ,];
    ...
    const tmp5Result3 = tmp5(12341);
    let num5 = num;
    if (num == null) {
      num5 = 0;
    }
    obj29.tagName = tmp2(12403).getItemSubtitleForMaxPlayersShort(num5);
    const tmp2Result12 = tmp2(12403);
    if (num == null) {
      num = 0;
    }
    obj29.accessibilityLabel = tmp5(12403)(num);
