...
function CommandRow(arg0) {
  ...
  let obj = hasOptions(12081);
  const appLauncherIconSource = obj.getAppLauncherIconSource(application);
  let tmp4 = null != appLauncherIconSource;
  if (tmp4) {
    obj = { iconSource: appLauncherIconSource };
    tmp4 = closure_10(onPressSend(12086), obj);
  }
  let tmpResult = tmp(12135);
  obj = { command, context, beforeExecuteCommand, onExecuteCommand, sectionName: tmp(9441).AppLauncherSectionName.SEARCH };
  ...
  obj1.trailing = closure_10(onPressSend(12135), { hasOptions, sending: commandRowSend.sending, onPressSend });
  return closure_10(hasOptions(5619).TableRow, obj1);
}
...
function CommandsExpandableList(expandedOverride) {
  ...
  items = onExecuteCommand.useMemo(() => commandData.map((item, index) => {
    closure_0 = index;
...
let style = { PLACERHOLDER: 0, [0]: "PLACERHOLDER", LOCAL_APPLICATION: 1, [1]: "LOCAL_APPLICATION", GLOBAL_APPLICATION: 2, [2]: "GLOBAL_APPLICATION" };
...
let obj2 = { paddingHorizontal: DEFAULT_CONTENT_PADDING, backgroundColor: nativeDefault.colors.MOBILE_KEYBOARD_PANEL_BACKGROUND };
obj1.loadingTextPlaceholder = { backgroundColor: nativeDefault.colors.BACKGROUND_MOD_SUBTLE, height: 16, marginBottom: 4, borderRadius: nativeDefault.radii.lg, alignSelf: "flex-start" };
...
obj1.loadingTextPlaceholderSmall = { backgroundColor: nativeDefault.colors.BACKGROUND_MOD_SUBTLE, height: 16, borderRadius: nativeDefault.radii.lg, alignSelf: "flex-start" };
...
obj1.commandsCTA = { borderRadius: nativeDefault.radii.sm, paddingHorizontal: nativeDefault.space.PX_4, marginBottom: nativeDefault.space.PX_4, justifyContent: "center" };
let obj6 = { borderRadius: nativeDefault.radii.sm, paddingHorizontal: nativeDefault.space.PX_4, marginBottom: nativeDefault.space.PX_4, justifyContent: "center" };
...
export const SearchLocalAndGlobalContentView = noop.forwardRef((context, arg1) => {
  ...
  obj = { context, query, commandLimit: 20, applicationLimit: 10, searchesActivities: entrypoint === require("AppLauncherTypes").AppLauncherEntrypoint.VOICE || isActivitiesInTextEnabled, searchesCommands: entrypoint === require("AppLauncherTypes").AppLauncherEntrypoint.TEXT, searchesBots: entrypoint === require("AppLauncherTypes").AppLauncherEntrypoint.TEXT };
  ...
  const callback1 = obj.useCallback((applicationId, searchResultsPosition) => {
    ...
    const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME_SEARCH, context, command: applicationId, section: descriptor, sectionDescriptors: null, query, navigation, sectionName: AppLauncherTypes.AppLauncherSectionName.SEARCH, searchResultsPosition, entrypoint };
  ...
  const callback2 = obj.useCallback((command) => {
    ...
    const obj = { command, location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME_SEARCH, triggerSection: ApplicationCommandUtils.getCommandTriggerSection(getSection(c0, command.applicationId).descriptor), queryLength: query.length, sectionName: AppLauncherTypes.AppLauncherSectionName.SEARCH, query, searchResultsPosition: command.searchResultsPosition };
    obj.trackCommandSelected(obj);
  }, items2);
  callback3 = obj.useCallback((arg0) => {
    ({ section, installOnDemand, searchResultsPosition } = arg0);
    const obj = { location: ApplicationCommandTypes.ApplicationCommandTriggerLocations.APP_LAUNCHER_HOME_SEARCH, application: section, navigation, context, sectionName: AppLauncherTypes.AppLauncherSectionName.SEARCH, installOnDemand, query, searchResultsPosition, entrypoint };
  ...
  if (0 !== memo.length) {
    ...
    obj1 = { accessibilityRole: "header", variant: "text-md/medium", color: "text-default", style: tmp.sectionHeader, children: null };
    ...
    if (memo.length <= tmp10(tmp3[30]).COLLAPSED_LIST_ITEM_MAX) {
      ...
      const obj3 = { commandData: memo, context, beforeExecuteCommand: callback2, onPressCommand: callback1, onExecuteCommand: context.onSend, expanded: tmp17 };
      items12[1] = tmp31(CommandsExpandableList, obj3);
      obj2.children = items12;
      tmp33Result = tmp33(tmp34, obj2);
    } else {
      const obj4 = { style: tmp.commandsCTA, underlayColor: tmp.commandsCTAUnderlayColor.color, accessibilityLabel: null, onPress: null, children: null };
  ...
  if (0 !== memo1.length) {
    const obj7 = { accessibilityRole: "header", variant: "text-md/medium", color: "text-default", style: tmp.sectionHeader, children: null };
