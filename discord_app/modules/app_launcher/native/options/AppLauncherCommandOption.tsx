...
import { AppLauncherChoicesOption } from "choices/AppLauncherChoicesOption.tsx";
...
export default function AppLauncherCommandOption(option) {
    ...
    if (tmp2(1954).ApplicationCommandOptionType.INTEGER !== type) {
      if (tmp2(1954).ApplicationCommandOptionType.NUMBER !== type) {
        if (tmp2(1954).ApplicationCommandOptionType.ATTACHMENT === type) {
          ...
          obj[7] = hasError;
          obj[8] = onPress;
          tmp28Result = tmp5(AppLauncherChannelOption, obj, name.name);
          tmp13 = tmp5;
          const tmp7 = AppLauncherChannelOption;
        } else {
          return null;
        }
        ...
        if (null != onDismiss) {
          ...
          obj6[2] = tmp13(tmp2(6060).CircleXIcon, { size: "md" });
          items[1] = tmp13(tmp2(4909).PressableOpacity, obj6);
          obj4[1] = items;
  ...
  if (null != name.choices) {
    ...
    obj7[7] = hasError;
    name = name.name;
    onDismiss(AppLauncherChoicesOption, obj7, name);
    const tmp57 = AppLauncherChoicesOption;
