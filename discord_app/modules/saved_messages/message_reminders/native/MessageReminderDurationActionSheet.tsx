...
export default function MessageReminderDurationActionSheet(createReminder) {
  ...
  const tmp = closure_8();
  const items = [c5];
  const stateFromStores = createReminder(504).useStateFromStores(items, () =>
    SavedMessagesStore.getSavedMessage(paths, _slicedToArray),
  );
  ...
  let obj3 = createReminder(12045);
  const dueInString = obj3.useDueInString({ dueAt, now: tmp7, type: createReminder(12045).DueInStringTypes.SHORT });
  ...
  const memo = onBack.useMemo(() => {
    ...
    if (tmpResult) {
      ...
      obj2.accessibilityLabel = intl2.string(util.t["13/7kX"]);
      obj2.onPress = tmp4;
      obj2.children = timestampProducer(ArrowLargeLeftIcon.ArrowLargeLeftIcon, { size: "md" });
      tmpResult = timestampProducer(Pressables.PressableOpacity, obj2);
    }
  ...
  const memo1 = onBack.useMemo(() => {
    ...
    const mapped = prop.map((item) => {
      ...
      return dueInText(
        createReminder(paths[14]).TableRow,
        {
          label: getLabel(),
          onPress() {
  ...
  const items3 = [dueInText(createReminder(5990).TableRowGroup, { hasIcons: false, children: memo1 })];
  let tmp14Result = null != removeReminder;
  if (tmp14Result) {
    const obj6 = { icon: tmp14(tmp2(4777).CheckmarkLargeIcon, {}), label: null, onPress: null, start: true, end: true };
    ...
    tmp14Result = tmp14(tmp2(5908).TableRow, obj6, "remove-reminder");
  }
  items3[1] = tmp14Result;
  obj5.children = items3;
  return isOverdue(createReminder(7481).BottomSheet, obj5);
}
