...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9787, dependencyMap.paths), "DatePicker", obj);
    const tmp3 = asyncRequireImpl(9787, dependencyMap.paths);
  }
  const tmp = onPress();
  let obj = first;
  ...
  obj.headerLeft = onClose(5674).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5681).Header, obj), ];
  ...
  const items2 = [error(onClose(4601).Text, obj3), ];
  ...
  obj4.children = error(onClose(4601).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5173).PressableOpacity, obj4);
  ...
  const items4 = [error(onClose(4601).Text, obj5), , ];
  ...
  obj6.children = error(onClose(4601).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5173).PressableOpacity, obj6);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4601).Text, obj7);
  }
