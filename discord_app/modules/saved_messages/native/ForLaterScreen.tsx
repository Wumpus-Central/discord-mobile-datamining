// === Module 12856: keyExtractor ===

// Module 12856 (keyExtractor)
import ThemesDefault from "Themes" /* 712 */;
import _modDef4185 from "module_4185" /* 4185 */;
import getSavedMessagesForTypeDefault from "getSavedMessagesForType" /* 12857 */;
import closure_3 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_6 from "getTimeSafe" /* 11159 */;
...
function ForLaterPage(type) {
  ...
  const tmp5 = type === first(7537).SavedMessageSortTypes.REMINDER;
  let obj = first(7532);
  ...
  const analyticsLocations = tmp2Result(tmp2(5925).FOR_LATER_POPOUT).analyticsLocations;
  obj = { type: null, name: null, properties: null };
  tmp2Result = tmp2(8815);
  ...
  if (0 === arr.length) {
    ...
    obj2[0] = type;
    obj1[1] = callback2(tmp2(12865), obj2);
    let tmp24Result = callback2(tmp4(5905).AnalyticsLocationProvider, obj1);
  } else {
    ...
    obj4[1] = callback2(tmp4(8037).FlashList, obj5);
    ...
    if (tmp9) {
      ...
      obj6[0] = tmp5;
      obj6[1] = tmp8;
      tmp25Result = tmp25(tmp2(12869), obj6);
    }
    items2[1] = tmp25Result;
    obj3[1] = items2;
    tmp24Result = closure_8(tmp4(5905).AnalyticsLocationProvider, obj3);
...
const memoResult = importAllResult.memo((type) => {
  ...
  const items2 = [callback2(_modDef4185.View, { style: items1 }), callback2(ForLaterPage, { type: type.type, handleScroll: callback })];
  obj[1] = items2;
