...
import _modDef13701 from "../../../../discord_assets/assets/premium/wumpus-avatar.png.js";
...
function IntroDemo(isReminder) {
  ...
  const obj3 = { source: { uri: _modDef13701 }, style: tmp.avatar };
...
function SheetRow(hasArrow) {
  ...
  const items1 = [
    timestampProducer(icon, { size: "sm", color: nativeDefault.colors.INTERACTIVE_TEXT_DEFAULT }),
    timestampProducer(Text_Text.Text, {
      variant: "text-sm/medium",
      color: "text-default",
      style: tmp.sheetRowLabel,
      children: label,
    }),
  ];
  let tmp5Result = null;
...
export default function ForLaterIntro(type) {
  ...
  const items = [timestampProducer(React3, { source: importDefault(tmp4 ? 13699 : 13700), style: tmp.upsellImage }), ,];
  ...
  const intl3 = util.intl;
  const t3 = util.t;
  const obj3 = { source: importDefault(tmp4 ? 13699 : 13700), style: tmp.upsellImage };
