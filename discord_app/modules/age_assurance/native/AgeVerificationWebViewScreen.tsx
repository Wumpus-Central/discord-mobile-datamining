...
import { AGE_VERIFICATION_MODAL_KEY as closure_6 } from "set" /* 5427 */;
import AGE_VERIFICATION_INCODE_PATH from "AGE_VERIFICATION_INCODE_PATH" /* 5429 */;
...
const createCacheKey = { flex: 1, backgroundColor: ThemesDefault.colors.BACKGROUND_BASE_LOWEST };
...
let result = require("obj132").fileFinishedImporting("modules/age_assurance/native/AgeVerificationWebViewScreen.tsx");
...
export default function AgeVerificationWebViewScreen(webviewUrl) {
  ...
  const callback2 = React.useCallback(() => {
    ...
    const obj = webviewUrl(onClose[9]);
  }, items2);
    ...
    try {
      ...
      if (null != tmp3) {
        if ("capture_complete" === tmp4.kind) {
          ...
          incodeSessionBootstrap.then((result) => {
            const incode_parameters = result.incode_parameters;
            let session_token;
          ...
          const nextPromise1 = incodeSessionBootstrap.then((result) => {
            const incode_parameters = result.incode_parameters;
            let session_token;
            if (incode_parameters != null) {
              session_token = incode_parameters.session_token;
