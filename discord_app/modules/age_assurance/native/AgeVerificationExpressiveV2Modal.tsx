...
function MethodsScreen(onClose) {
  ...
  initiateAgeVerificationV2 = obj2.useInitiateAgeVerificationV2({
    onComplete: callback,
    entryPoint,
  ...
  if (isAgeVerificationCustomTabOpen) {
    ...
    if (shouldShowExpressiveModalSubtitleAlt) {
      fn = () => {
        ...
        const result = callback(onComplete[20]).trackAgeVerificationModalClicked(
          modalSessionId,
          callback(onComplete[20]).AgeVerificationModalVersion.EXPRESSIVE_V2,
          callback(onComplete[20]).AgeVerificationModalCta.TRUSTED_PROVIDERS,
        );
      };
    }
    ...
    obj2[3] = tmp2Result.getAgeVerificationGetStartedSubtitle(
      ...
      () => {
        ...
        const result = callback(onComplete[20]).trackAgeVerificationModalClicked(
          modalSessionId,
          callback(onComplete[20]).AgeVerificationModalVersion.EXPRESSIVE_V2,
          callback(onComplete[20]).AgeVerificationModalCta.LEARN_MORE,
        );
      },
      undefined,
      fn,
      ...
      if (tmp10) {
        ...
        if (tmp10) {
          ...
          obj7[1] = memo1.map((children) => {
            ...
            const items = [
              tmp5(callback(onComplete[28]).Text, {
            ...
            items[1] = tmp5Result;
            obj3[4] = closure_10(callback(onComplete[26]).Stack, {
              direction: "vertical",
              spacing: 4,
              children: items,
            });
...
export default function AgeVerificationExpressiveV2Modal(entryPoint) {
  ...
  const effect = React.useEffect(() => {
    const result = entryPoint(onComplete[20]).trackAgeVerificationModalViewed(
      memo,
      entryPoint(onComplete[20]).AgeVerificationModalVersion.EXPRESSIVE_V2,
      entryPoint,
    );
  }, items1);
