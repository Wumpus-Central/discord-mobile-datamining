...
export default function GetStartedScreen(onComplete) {
  ...
  obj4[1] = callback2(modalSessionId).map((children) => {
    ...
    obj[1] = callback(modalSessionId(4338).Text, { variant: "text-md/medium", color: "mobile-text-heading-primary", children: children.title });
    let tmpResult = null;
    if (null != description) {
      obj = { variant: "text-xs/medium", color: "text-subtle", children: null };
      obj[2] = description;
      tmpResult = tmp(modalSessionId(4338).Text, obj);
    }
    obj[2] = tmpResult;
    return callback(_undefined(7999), obj, arg1);
  });
  items1[1] = callback3(modalSessionId(5807).TableRowGroup, obj4);
  ...
  obj5[1] = callback3(modalSessionId(4338).Text, obj6);
  ...
  items3 = [tmp.getStartedFooterContainer, { paddingBottom: useSafeAreaInsets().bottom }];
  ...
  if (!isManualAgeVerificationHidden) {
    ...
    obj9[1] = tmp9(tmp4(4338).Text, obj10);
  ...
  obj14[4] = callback3(modalSessionId(8000).LinkExternalSmallIcon, obj15);
  obj14[5] = loading;
  obj13[1] = callback3(modalSessionId(4755).Button, obj14);
