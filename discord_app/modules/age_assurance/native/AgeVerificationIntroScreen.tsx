...
export default function GetStartedScreen(onComplete) {
  ...
  const initiateAgeVerification = modalSessionId(5039).useInitiateAgeVerification({
    onComplete: onComplete.onClose,
    entryPoint,
  });
  ({ initiateAgeVerification: c1, loading } = initiateAgeVerification);
  let obj = modalSessionId(5039);
  const isManualAgeVerificationHidden = modalSessionId(8929).useIsManualAgeVerificationHidden(
  ...
  obj10.children = closure_6(modalSessionId).map((children, index) => {
    ...
    const obj = {
      index: index + 1,
      tip: closure_1_9(modalSessionId(4823).Text, {
      ...
      try {
        ...
        if (0 === v1) {
          if (arg0 === 1) {
            ...
            const result = v3(8759).trackAgeVerificationModalClicked(
              modalSessionId,
              v3(8759).AgeVerificationModalVersion.PRIMARY,
              v3(8759).AgeVerificationModalCta.GET_STARTED,
            );
            v1 = 1;
            v3 = 1;
