...
export default function GetStartedScreen(onComplete) {
  ...
  const initiateAgeVerification = modalSessionId(5041).useInitiateAgeVerification({
    onComplete: onComplete.onClose,
    entryPoint,
  });
  ({ initiateAgeVerification: c1, loading } = initiateAgeVerification);
  let obj = modalSessionId(5041);
  const isManualAgeVerificationHidden = modalSessionId(8933).useIsManualAgeVerificationHidden(
  ...
  obj10.children = closure_6(modalSessionId).map((children, index) => {
    ...
    const obj = {
      index: index + 1,
      tip: closure_1_9(modalSessionId(4825).Text, {
      ...
      try {
        ...
        if (0 === v1) {
          if (arg0 === 1) {
            ...
            const result = v3(8763).trackAgeVerificationModalClicked(
              modalSessionId,
              v3(8763).AgeVerificationModalVersion.PRIMARY,
              v3(8763).AgeVerificationModalCta.GET_STARTED,
            );
            v1 = 1;
            v3 = 1;
