...
export default function AgeVerificationEmbeddedIntroScreen(arg0) {
  ...
  const obj5 = { variant: "text-md/medium", color: "text-strong", style: tmp.header, children: navigation(4992).getAgeVerificationGetStartedSubtitle(entryPoint, undefined, isSuspendedUser) };
  items2[1] = callback(navigation(4734).Text, obj5);
  obj3[3] = items2;
  items1[1] = callback2(navigation(4733).Stack, obj3);
  obj2[3] = items1;
  const items3 = [callback2(navigation(4733).Stack, obj2), callback(navigation(6699).AgeVerificationMethodsContainer, { ageVerificationMethods: useAgeVerificationMethodsDefault({ onClose, classificationId, onGoogleWalletSelect: callback }).ageVerificationMethods, modalSessionId })];
  obj1[3] = items3;
