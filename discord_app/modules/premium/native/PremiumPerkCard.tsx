...
export default function PremiumPerkCard(variant) {
  ...
  if (null != imageSrc) {
    if (null != imageOverlayText) {
      ...
      obj3[0] = items2;
      obj3[1] = imageSrc;
      tmp15 = callback(importDefault(5207), obj3);
    }
  } else {
    let tmp10 = null;
    ...
    if (tmp26) {
      ...
      tmp26 = callback(importDefault(12582), obj7);
    }
    ...
    if (tmp29Result) {
      ...
      obj10[1] = tmp29(tmp6(4666).Button, obj11);
...
export const usePerkCardHeight = function usePerkCardHeight(NARROW) {
  return require(4672) /* getFontScale */.useFontScale() > 1 ? frozen[NARROW].scaledFontHeight : frozen[NARROW].height;
};
export const usePremiumPerkCard = function usePremiumPerkCard() {
  let obj = subscriptionPlansLoaded(12564);
  ...
  const callback = React.useCallback(() => {
    let obj = subscriptionPlansLoaded(5915);
    obj = { screen: constants.COLLECTIBLES_SHOP, params: null };
    obj = { analyticsSource: callback(5601).PREMIUM_MARKETING_PERK_CARD };
    obj[1] = obj;
  ...
  obj16[1] = { uri: importDefault(12581) };
  ...
  const obj17 = { uri: importDefault(12581) };
