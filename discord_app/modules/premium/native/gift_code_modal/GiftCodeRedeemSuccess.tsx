...
export default function GiftCodeRedeemSuccess(giftCode) {
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result7.isGameItemSKU(stateFromStores)) {
        ...
        obj10.children = closure_8(user(8288), obj11);
        ...
        tmp24Result = closure_8(user(6593), obj12);
  ...
  if (type === giftCode(1974).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        tmp20Result = closure_8(user(8260), obj15);
      }
    ...
    if (tmp2Result9.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = closure_8(tmp2(4832).Text, obj21);
        let tmp36 = closure_8;
      }
      ...
      if (null != first) {
        ...
        return closure_9(tmp2(6544).SafeAreaPaddingView, obj8);
      }
    ...
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        const obj26 = {
          ...
          style: tmp.message,
          children: tmp2(5089).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan),
        };
  ...
  const withResult1 = withResult.with({ type: giftCode(1974).CollectiblesItemType.PROFILE_EFFECT }, (profileEffect) =>
  ...
  const withResult2 = withResult1.with({ type: giftCode(1974).CollectiblesItemType.PROFILE_FRAME }, (profileFrame) =>
  ...
  tmp20Result2 = withResult2
    .with({ type: giftCode(1974).CollectiblesItemType.NAMEPLATE }, (nameplate) => {
      const obj = {
