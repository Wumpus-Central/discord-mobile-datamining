...
import actions_GiftCodeActionCreatorsDefault from "actions/GiftCodeActionCreators" /* 11432 */;
import GiftCodeRedeemModal from "GiftCodeRedeemModal" /* 11438 */;
import GiftBoxAnimationDefault from "GiftBoxAnimation" /* 11448 */;
...
createStyles.confettiBackground = { justifyContent: "center", width: "100%", position: "absolute", top: 0, left: 0, opacity: 0.4, height: 275 };
...
createStyles.collectiblesAssetBundle = { margin: 20, alignSelf: "stretch", minHeight: 250, alignItems: "center", justifyContent: "center" };
  ...
  if (giftCode.isSubscription) {
    ...
    if (null == first) {
      if (null != getOrFetchApplication) {
        if (tmp2Result7.isGameItemSKU(stateFromStores1)) {
          const obj8 = { sku: stateFromStores1, application: getOrFetchApplication, sender: str, hasAccountLinked: hasAlreadyLinked, canStartAuthorization, mobileAccountLinkingDisabled: socialLayerStorefrontMobileAccountLinkingDisabled };
              ...
              if (canStartAuthorization) {
                if (!socialLayerStorefrontMobileAccountLinkingDisabled) {
                  ...
                  obj16.onPress = function onPress() {
                    let obj = { location_stack: analyticsLocations, sku_id: giftCode.skuId, application_id: giftCode.applicationId, is_gift: true };
    ...
    if (tmp31) {
      if (null != product) {
        ...
        if (tmp30Result3) {
          const obj22 = { deco: firstAvatarDecoration, pfx: firstProfileEffect, nameplate: firstNameplate, previewAssets: product.previewAssets, disableStaticBackground: true, size: "large", targetSize: tmp25 };
    ...
    const withResult = match.with(obj24, (avatarDecoration) => map1(native.Avatar, { source: user.getAvatarSource(null, true, native.AVATAR_SIZE_MAP[native.AvatarSizes.GIFT_START]), avatarDecoration, size: native.AvatarSizes.GIFT_START, animate: true }));
    const obj26 = { type: tmp2(tmp3[12]).CollectiblesItemType.PROFILE_FRAME };
    const withResult1 = match.with(obj24, (avatarDecoration) => map1(native.Avatar, { source: user.getAvatarSource(null, true, native.AVATAR_SIZE_MAP[native.AvatarSizes.GIFT_START]), avatarDecoration, size: native.AvatarSizes.GIFT_START, animate: true })).with(obj25, (profileEffect) => map1(ProfileEffectUserPreviewDefault, { user, profileEffect }));
    const obj27 = { type: tmp2(tmp3[12]).CollectiblesItemType.NAMEPLATE };
    const withResult2 = match.with(obj24, (avatarDecoration) => map1(native.Avatar, { source: user.getAvatarSource(null, true, native.AVATAR_SIZE_MAP[native.AvatarSizes.GIFT_START]), avatarDecoration, size: native.AvatarSizes.GIFT_START, animate: true })).with(obj25, (profileEffect) => map1(ProfileEffectUserPreviewDefault, { user, profileEffect })).with(obj26, (profileFrame) => map1(ProfileFrameUserPreviewDefault, { user, profileFrame }));
    obj23.children = match.with(obj24, (avatarDecoration) => map1(native.Avatar, { source: user.getAvatarSource(null, true, native.AVATAR_SIZE_MAP[native.AvatarSizes.GIFT_START]), avatarDecoration, size: native.AvatarSizes.GIFT_START, animate: true })).with(obj25, (profileEffect) => map1(ProfileEffectUserPreviewDefault, { user, profileEffect })).with(obj26, (profileFrame) => map1(ProfileFrameUserPreviewDefault, { user, profileFrame })).with(obj27, (nameplate) => {
