...
export default function GiftCodeRedeemStart(giftCode) {
  ...
  let items = [closure_8];
  const stateFromStores = giftCode(soundId[16]).useStateFromStores(items, () =>
    GiftCodeStore.getIsAccepting(giftCode.code),
  );
  let obj2 = giftCode(soundId[16]);
  const items1 = [stateFromStores1];
  let str = giftCode(soundId[16]).useStateFromStores(items1, () =>
  ...
  if (giftCode.isSubscription) {
              ...
              if (canStartAuthorization) {
                if (!socialLayerStorefrontMobileAccountLinkingDisabled) {
                  ...
                  obj21.onPress = function onPress() {
                    AnalyticsUtilsDefault.track(constants.SLAYER_STOREFRONT_ACCOUNT_LINK_CLICKED, {
                      location_stack: analyticsLocations,
      ...
      if (null != getOrFetchApplication) {
        if (null == giftCode.giftStyle) {
          const obj25 = {
            ...
            size: tmp2(tmp3[36]).GameIconSizes.LARGE,
            skuId: giftCode.skuId,
          };
    ...
    const withResult = match.with(obj29, (avatarDecoration) =>
      map1(native.Avatar, {
    ...
    const withResult1 = match
      ...
      .with(obj30, (profileEffect) => map1(ProfileEffectUserPreviewDefault, { user, profileEffect }));
    ...
    const withResult2 = match
      ...
      .with(obj30, (profileEffect) => map1(ProfileEffectUserPreviewDefault, { user, profileEffect }))
      .with(obj31, (profileFrame) => map1(ProfileFrameUserPreviewDefault, { user, profileFrame }));
    obj28.children = match
      .with(obj29, (avatarDecoration) =>
        map1(native.Avatar, {
          source: user.getAvatarSource(null, true, native.AVATAR_SIZE_MAP[native.AvatarSizes.GIFT_START]),
          avatarDecoration,
