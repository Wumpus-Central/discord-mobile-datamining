...
export default function PremiumGiftSuccessBody(giftCodeRecord) {
  ...
  if (null != giftCodeRecord.subscriptionPlanId) {
    ...
    subscriptionPlanId = tmp2(3931).getPlanIdForPremiumType(premiumType, planInterval);
    const tmp2Result = tmp2(3931);
  }
  let obj3 = giftCodeURL(3931);
  const tierDisplayNameByPlanId = obj3.getTierDisplayNameByPlanId(subscriptionPlanId);
  let obj4 = giftCodeURL(3931);
  ...
  obj5[3] = callback;
  items2[1] = callback(giftCodeRecord(8585).InputButton, obj5);
  obj3[1] = items2;
