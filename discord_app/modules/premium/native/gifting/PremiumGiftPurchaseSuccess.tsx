// === Module 10275: PremiumGiftSuccessBody ===
...
export default function PremiumGiftSuccessBody(giftCodeRecord) {
  ...
  if (null != giftCodeRecord.subscriptionPlanId) {
    ...
    subscriptionPlanId = tmp2(4043).getPlanIdForPremiumType(premiumType, planInterval);
    const tmp2Result = tmp2(4043);
  }
  let obj3 = giftCodeURL(4043);
  const tierDisplayNameByPlanId = obj3.getTierDisplayNameByPlanId(subscriptionPlanId);
  let obj4 = giftCodeURL(4043);
  ...
  items1[2] = callback(giftCodeRecord(4376).Text, obj1);
  ...
  const items2 = [callback(giftCodeRecord(4376).Text, obj4), callback(giftCodeRecord(8810).InputButton, { text: giftCodeURL, icon: giftCodeURL(4328), iconPosition: "end", onPress: callback })];
  obj3[1] = items2;
  ...
  items1[4] = callback(giftCodeRecord(4376).Text, obj6);
  obj2[0] = items1;
