// === Module 11348: PremiumGiftPurchaseSuccess ===
...
export default function PremiumGiftSuccessBody(giftCodeRecord) {
  ...
  const nativeGiftContext = giftCodeRecord(10965).useNativeGiftContext();
  ({ giftStyle, premiumType, planInterval } = nativeGiftContext);
  let obj = giftCodeRecord(10965);
  const giftCodeURL = giftCodeRecord(4995).getGiftCodeURL(giftCodeRecord.code);
  ...
  if (null != giftCodeRecord.subscriptionPlanId) {
    ...
    subscriptionPlanId = tmp2(4415).getPlanIdForPremiumType(premiumType, planInterval);
    const tmp2Result = tmp2(4415);
  }
  let obj2 = giftCodeRecord(4995);
  const tmp6 = giftCodeURL;
  const tierDisplayNameByPlanId = giftCodeURL(4415).getTierDisplayNameByPlanId(subscriptionPlanId);
  const obj4 = giftCodeURL(4415);
  ...
  obj3.children = closure_8(giftCodeURL(11098), { giftStyle });
  ...
  items1[1] = closure_8(giftCodeRecord(4753).Text, obj6);
  ...
  items1[2] = closure_8(giftCodeRecord(4753).Text, obj7);
  ...
  const items2 = [closure_8(giftCodeRecord(4753).Text, obj10), ];
  obj5 = giftCodeURL(4415);
  items2[1] = closure_8(giftCodeRecord(9189).InputButton, { text: giftCodeURL, icon: tmp6(4704), iconPosition: "end", onPress: callback });
  ...
  items1[4] = closure_8(giftCodeRecord(4753).Text, obj12);
