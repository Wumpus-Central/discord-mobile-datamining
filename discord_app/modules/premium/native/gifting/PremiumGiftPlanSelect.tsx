...
export default function PremiumGiftPlanSelect(shouldUseDMWishlistGiftingDesign) {
  ...
  const stateFromStoresObject = obj7.useStateFromStoresObject(items1, () => ({ nextTier: closure_9.getNextTier(navigation(onClose[20]).BadgeId.GIFTING), giftsToNextTier: closure_9.getRemainingToNextTier(navigation(onClose[20]).BadgeId.GIFTING) }));
  ...
  if (shouldUseDMWishlistGiftingDesign.isLoadingWishlist) {
    ...
    if (null == recipientUser) {
      ...
      const obj24 = { style: tmp13.description, variant: "heading-sm/medium", color: "text-overlay-light", children: null };
