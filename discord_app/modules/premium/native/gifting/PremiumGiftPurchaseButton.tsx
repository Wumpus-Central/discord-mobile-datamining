...
export default function PremiumGiftPurchaseButton(defaultSelection) {
  ...
  const nativeGiftContext = defaultSelection(10965).useNativeGiftContext();
  ...
  let obj2 = defaultSelection(10965);
  const canPurchaseIAP = defaultSelection(11322).useCanPurchaseIAP(productId);
  const obj3 = defaultSelection(11322);
  let items = [c7];
  const stateFromStores = defaultSelection(504).useStateFromStores(items, () => {
    const marketingComponentByType = _undefined2.getMarketingComponentByType(
      defaultSelection(_undefined[13]).MarketingComponentType.GIFT_CUSTOMIZATION_BANNER,
    );
    let prop = null;
  ...
  closure_10 = tmp11;
  const GiftingBadgeExperiment = tmp4(11006).GiftingBadgeExperiment;
  ...
  const stateFromStoresObject = defaultSelection(504).useStateFromStoresObject(items1, () => ({
    nextTier: setSelectedGiftingPromotionReward.getNextTier(defaultSelection(_undefined[16]).BadgeId.GIFTING),
    giftsToNextTier: setSelectedGiftingPromotionReward.getRemainingToNextTier(
      defaultSelection(_undefined[16]).BadgeId.GIFTING,
    ),
  }));
  ...
  const isGiftingBadgeComplexArtEnabled = defaultSelection(11010).useIsGiftingBadgeComplexArtEnabled(
    `PremiumGiftPurchaseButton${str}`,
  );
  ...
  const tmp4Result5 = defaultSelection(11010);
  const product = defaultSelection(11317).useFetchCollectiblesProduct(selectedGiftingPromotionReward).product;
  ...
  if (tmp11) {
    if (tmp8) {
      ...
      let tmp22Result = closure_9(tmp(11021), obj6);
    ...
    if (!tmp11) {
      ...
      tmp31 = closure_9(tmp4(4753).Text, obj7);
    ...
    if (!isPurchasing) {
      fn = () => {
          ...
          if (closure_8) {
            if (closure_8) {
              ...
              obj2.onSelect = function onSelect(arg0) {
                ...
                navigation.navigate(defaultSelection(10928).PremiumGiftScreens.CUSTOMIZATION);
              };
        ...
        _undefined(() => {
          navigation.navigate(defaultSelection(10928).PremiumGiftScreens.SUCCESS);
        });
      };
    }
    obj9.onPress = fn;
    items3[2] = closure_9(tmp4(5187).Button, obj9);
  ...
  if (tmp15) {
    if (tmp10) {
      if (null != selectedGiftingPromotionReward) {
        ...
        obj11.subtitle = name;
        const items5 = [closure_9(tmp4(11021).PremiumGiftPromotionCollectibleRewardDetails, obj11)];
        let tmp26Result = !tmp9;
        if (!tmp9) {
          tmp26Result = tmp26(tmp4(10514).PencilIcon, { size: "sm" });
        }
  ...
  if (enabled) {
    ...
    const tmpResult4 = tmp(11024);
    obj12.nextTierIcon = tmp4(11010).getGiftingBadgeTierIconUrl(nextTier, isGiftingBadgeComplexArtEnabled);
    obj12.analyticsLocation = tmp(7429).PREMIUM_GIFT_CUSTOMIZATION;
    tmp22Result = tmp22(tmpResult4, obj12);
    const tmp4Result8 = tmp4(11010);
  }
}
