...
import useShouldShowGiftingPromotionDecoDefault from "useShouldShowGiftingPromotionDeco" /* 10263 */;
import closure_3 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "initialize" /* 9005 */;
import { isAvatarDecorationRecord, isNameplateRecord } from "fromServer" /* 7219 */;
...
function RewardPreview(arg0) {
  ...
  if (isNameplateRecord(collectiblesItem)) {
    ...
    if (isAvatarDecorationRecord(collectiblesItem)) {
      ...
      obj6[1] = callback(tmp(10340), obj7);
...
export default function PremiumGiftPurchaseButton(defaultSelection) {
  ...
  let obj3 = defaultSelection(7630);
  ...
  closure_10 = tmp12;
  let tmpResult = tmp(9830);
  const config = tmpResult.useConfig({ location: "PremiumGiftPurchaseButton" });
  const GiftingBadgeExperiment = tmp4(9832).GiftingBadgeExperiment;
  ...
  const themeAndReducedMotionAwareAssetUrl = defaultSelection(10261).useThemeAndReducedMotionAwareAssetUrl(prop);
  const tmp4Result1 = defaultSelection(10261);
  let asset;
  if (stateFromStores1 != null) {
    asset = stateFromStores1.asset;
  }
  const themeAndReducedMotionAwareAssetUrl1 = defaultSelection(10261).useThemeAndReducedMotionAwareAssetUrl(asset);
  let first;
  if (product != null) {
  ...
  if (tmp11) {
    if (config.enabled) {
      if (selectedGiftingPromotionReward === tmp4(9828).GiftingPromotionRewardSKUIds.SUMMER_2026_GOGO_FAKE_SKU_ID) {
      ...
      if (!tmp12) {
        ...
        tmp41 = callback(tmp4(4440).Text, obj1);
      ...
      obj3[4] = fn;
      items5[2] = callback(tmp4(4879).Button, obj3);
      obj[1] = items5;
  ...
  if (tmp12) {
    if (tmp9) {
      ...
      obj6[3] = { x: 0.5, y: 0.5 };
      obj6[4] = colors;
      const items6 = [callback(tmp(4891), obj6), ];
      ...
      if (tmp33Result) {
        ...
        obj7[2] = obj8;
        tmp33Result = tmp33(tmp(5440), obj7);
      }
      ...
      if (tmp33Result) {
        let inAppFlowImageLargeTilted = assetVariant === tmp4(7570).GiftCustomizationBanner_AssetVariant.LARGE_TILTED;
        ...
        obj9[2] = obj10;
        tmp33Result = tmp33(tmp(5440), obj9);
        const tmpResult3 = tmp(5440);
      }
      ...
      items7[2] = callback(tmp4(4440).Text, obj11);
      obj4[1] = items7;
      tmp27Result = tmp24(tmp25, obj4);
      const tmpResult2 = tmp(4891);
    }
  }
  if (null != first) {
    if (tmp11) {
      if (null != selectedGiftingPromotionReward) {
        ...
        if (!tmp10) {
          ...
          obj15[2] = tmp29(tmp4(9889).PencilIcon, { size: "sm" });
          ...
          tmp29Result = tmp29(tmp4(7963).IconButton, obj15);
        }
  ...
  if (GiftingBadgeExperiment.useConfig({ location: "PremiumGiftPurchaseButton" }).enabled) {
    ...
    if (null != nextTier) {
      ...
      obj16[3] = tmp(5920).PREMIUM_GIFT_CUSTOMIZATION;
      tmp27Result = callback(tmp(10267), obj16);
