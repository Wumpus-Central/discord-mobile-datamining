...
export default function PremiumGiftPurchaseButton(defaultSelection) {
  ...
  let obj1 = defaultSelection(10769);
  ...
  let obj2 = defaultSelection(11119);
  ...
  const stateFromStoresObject = tmp4Result.useStateFromStoresObject(items1, () => ({
    nextTier: setSelectedGiftingPromotionReward.getNextTier(defaultSelection(_undefined[18]).BadgeId.GIFTING),
    giftsToNextTier: setSelectedGiftingPromotionReward.getRemainingToNextTier(
      defaultSelection(_undefined[18]).BadgeId.GIFTING,
    ),
  }));
  ...
  if (tmp10) {
    if (config.enabled) {
      ...
      if (!tmp11) {
        ...
        tmp33 = closure_9(tmp4(4601).Text, obj1);
      ...
      obj3.onPress = fn;
      items3[2] = closure_9(tmp4(5025).Button, obj3);
      obj.children = items3;
  ...
  if (tmp11) {
    if (tmp8) {
      ...
      tmp22Result = closure_9(tmp(10824), obj4);
      const tmpResult2 = tmp(10824);
    }
  }
  if (tmp15) {
    if (tmp10) {
      if (null != selectedGiftingPromotionReward) {
        ...
        obj6.subtitle = name;
        const items5 = [closure_9(tmp4(10824).PremiumGiftPromotionCollectibleRewardDetails, obj6)];
  ...
  if (GiftingBadgeExperiment.useConfig({ location: "PremiumGiftPurchaseButton" }).enabled) {
    ...
    if (null != nextTier) {
      const obj7 = {
        giftsToNextTier: stateFromStoresObject.giftsToNextTier,
        nextTierName: null,
        nextTierIcon: null,
      ...
      obj7.analyticsLocation = tmp(7234).PREMIUM_GIFT_CUSTOMIZATION;
      tmp22Result = closure_9(tmp(10826), obj7);
      const tmpResult3 = tmp(10826);
    }
  }
}
