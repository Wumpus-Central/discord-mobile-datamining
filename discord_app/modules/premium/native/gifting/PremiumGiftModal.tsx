...
let obj2 = {
  [PLAN_SELECT]: fn(10929).PaymentFlowStep.SKU_SELECT,
  [REWARD_SELECT]: fn(10929).PaymentFlowStep.REWARD_SKU_SELECT,
  [CUSTOMIZATION]: fn(10929).PaymentFlowStep.PLAN_SELECT,
  [SUCCESS]: fn(10929).PaymentFlowStep.CONFIRM,
  [GIFTING_BADGE]: fn(10929).PaymentFlowStep.CONFIRM,
};
...
export default function PremiumGiftModal(analyticsLocations) {
  ...
  if (initialRoute != null) {
    ...
    const fetchWishlistAndProfileInfoForUser = analyticsLocation(onDismiss[8]).useFetchWishlistAndProfileInfoForUser(
      recipientUserId,
    );
    ({
    ...
    obj12.render = function render(currentProgress) {
      return jsx(analyticsLocations(onDismiss[16]), {
        ...
        onSendGift() {
          ...
          const items = [analyticsLocations(7429).GIFTING_BADGE_POST_PURCHASE];
          obj2.analyticsLocations = items;
          analyticsLocation(10927).openGiftModal(obj2);
        },
      });
    };
    ...
    if (tmp11Result14.isPremiumGiftingSupported()) {
      ...
      obj13.children = jsx(analyticsLocation(onDismiss[25]).NativeGiftContextProvider, {
        basePurchaseAnalytics: tmp4,
        recipientUserId,
        onClose: callback,
        setCurrentAnalyticsStep: tmp22,
