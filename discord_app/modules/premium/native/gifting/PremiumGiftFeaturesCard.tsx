...
import PremiumFeaturesBackgroundDefault from "PremiumFeaturesBackground" /* 8958 */;
...
function PremiumGiftPlanSelectPromotionDetails(config) {
  ...
  obj = { style: closure_15(config.isLargeSize).promotionDetailsContainer, colors: [4294967102, 4294967053], children: null };
  const tmp = closure_15(config.isLargeSize);
  const obj1 = { imageUrl: themeAndReducedMotionAwareAssetUrl, title: null, subtitle: null, subtitleColor: "text-default", shouldAnimate: null };
...
let closure_15 = createStyles.createStyles(() => {
  ...
  logo.promotionDetailsContainer = { marginHorizontal: nativeDefault.space.PX_24, marginTop: nativeDefault.space.PX_20, marginBottom: nativeDefault.space.PX_32, padding: nativeDefault.space.PX_12, gap: nativeDefault.space.PX_12, borderRadius: nativeDefault.radii.sm };
  return logo;
});
...
export default noop.memo(function PremiumGiftFeaturesCard(variant) {
  ...
  const items3 = [React7(PremiumFeaturesWumpusDefault, { premiumType }), React7(PremiumFeaturesLogoDefault, { style: tmp6.logo, premiumType }), , , , , ];
