// === Module 10734: PostPurchaseFooter ===
...
import GiftingBadgeProgressBarDefault from "GiftingBadgeProgressBar" /* 10737 */;
import GiftingBadgeLevelUpProgressDefault from "GiftingBadgeLevelUpProgress" /* 10738 */;
import closure_3 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_5 from "initialize" /* 8528 */;
import getTierForProgress from "getTierForProgress" /* 10735 */;
...
function PostPurchaseFooter(onSendGift) {
  ...
  const callback1 = React.useCallback(() => {
    ...
    arr = arr.pop();
    const rootNavigationRef = onSendGift(4303).getRootNavigationRef();
  ...
  obj[2] = callback4(onSendGift(10736).GiftIcon, obj);
  const intl = onSendGift(1236).intl;
  obj[3] = intl.string(messagesProxyDefault.g86YiI);
  obj[4] = callback;
  const items1 = [callback4(onSendGift(4896).Button, obj), ];
  ...
  obj1[3] = callback1;
  items1[1] = callback4(onSendGift(4896).Button, obj1);
  obj[1] = items1;
...
function LevelUpScreen(arg0) {
  ...
  const effect = React.useEffect(() => {
    const result = callback(4416).triggerHapticFeedback(callback2(4417).IMPACT_HEAVY);
  }, []);
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp(10484), obj1);
  }
  obj[1] = tmp7Result;
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp9(4444).Text, obj6);
  }
  items2[1] = tmp7Result;
  obj4[1] = items2;
...
export default function GiftBadgePostPurchase(arg0) {
  ...
  const effect = React.useEffect(() => {
    let obj = callback(4268);
    obj = { dismissAction: constants.INDIRECT_ACTION };
    const result = obj.UNSAFE_markDismissibleContentAsDismissed(callback(1373).DismissibleContent.NEW_GIFTING_BADGES_COACHMARK, obj);
  }, []);
  let obj = initialize;
  const items = [closure_5];
  const stateFromStores = obj.useStateFromStores(items, () => {
    badgeById = badgeById.getBadgeById(callback(8525).BadgeId.GIFTING);
    let tiers;
    if (badgeById != null) {
