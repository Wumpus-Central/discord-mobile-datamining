...
function PostPurchaseFooter(onSendGift) {
  ...
  { style: callback6(useSafeAreaInsetsDefault().bottom).footer, children: null };
  ...
  const obj = { size: "sm", color: ThemesDefault.colors.CONTROL_PRIMARY_TEXT_DEFAULT };
...
function InProgressScreen(arg0) {
  ...
  const obj = { style: tmp.progressWrapper, children: callback4(GiftingBadgeProgressBarDefault, { progress, currentTier, nextTier, iconSize: 48, title: progressBarTitle }) };
...
function LevelUpScreen(arg0) {
  ...
  const obj = { style: tmp3.levelUpIconWrapper, children: null };
  let tmp7Result = null != newTier.simple_icon_url;
  if (tmp7Result) {
    obj1 = { icon: null, size: 140 };
    obj1[0] = newTier.simple_icon_url;
    tmp7Result = callback(GiftingBadgeIconDefault, obj1);
  }
  obj[1] = tmp7Result;
  const items = [callback(View, obj), ];
  const obj2 = { style: tmp3.levelUpBody, children: null };
  const items1 = [callback(GiftingBadgeLevelUpProgressDefault, { progress: simulatedProgress, currentTier, newTier, style: tmp3.levelUpProgress }), ];
  ...
  if (tmp7Result) {
    ...
    obj7[1] = str2;
    obj6[3] = intl2.format(messagesProxyDefault["6QVlxw"], obj7);
    tmp7Result = callback(Text.Text, obj6);
  }
  items2[1] = tmp7Result;
  obj4[1] = items2;
  ...
  obj[1] = items;
  const items3 = [callback2(View, obj), callback(PostPurchaseFooter, { onSendGift })];
  obj[1] = items3;
...
let closure_12 = createCacheKey.createStyles((arg0) => {
  let obj = { flex: 1, alignItems: "center", justifyContent: "center", padding: ThemesDefault.space.PX_16 };
  obj[1] = obj;
...
let result = require("obj132").fileFinishedImporting("modules/premium/native/gifting/GiftBadgePostPurchase.tsx");
  ...
  if (null == stateFromStores) {
    ...
    if (1 === tmp10) {
      ...
      if (null != name) {
        ...
        obj3[0] = name1;
        str = intl.formatToPlainString(messagesProxyDefault.bwyQt8, obj3);
      }
      obj2[2] = str;
      obj2[3] = sum;
      obj2[4] = tmp26;
