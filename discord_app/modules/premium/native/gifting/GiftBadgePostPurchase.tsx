// === Module 9983: PostPurchaseFooter ===
...
function PostPurchaseFooter(onSendGift) {
  ...
  obj[2] = callback4(onSendGift(8954).GiftIcon, obj);
  const intl = onSendGift(1236).intl;
  obj[3] = intl.string(importDefault(2351).g86YiI);
  obj[4] = callback;
  const items1 = [callback4(onSendGift(4666).Button, obj), ];
  const obj1 = { grow: true, variant: "secondary", text: null, onPress: null };
  const intl2 = onSendGift(1236).intl;
  obj1[2] = intl2.string(importDefault(2351)["sa/cfM"]);
  obj1[3] = callback1;
  items1[1] = callback4(onSendGift(4666).Button, obj1);
  obj[1] = items1;
...
function InProgressScreen(arg0) {
  ...
  obj = { style: tmp.progressWrapper, children: callback4(importDefault(9985), { progress, currentTier, nextTier, iconSize: 48, title: progressBarTitle }) };
  const items = [callback4(View, obj), ];
  const obj1 = { style: tmp.messageSection, children: null };
  const items1 = [callback4(require(4251) /* Text */.Text, { variant: "heading-xxl/bold", style: tmp.centerText, children: title }), callback4(require(4251) /* Text */.Text, { variant: "text-md/medium", color: "text-subtle", style: tmp.centerText, children: description })];
  obj1[1] = items1;
...
function LevelUpScreen(arg0) {
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp(9865), obj1);
  }
  obj[1] = tmp7Result;
  const items = [closure_10(View, obj), ];
  const obj2 = { style: tmp3.levelUpBody, children: null };
  const items1 = [closure_10(importDefault(9986), { progress: simulatedProgress, currentTier, newTier, style: tmp3.levelUpProgress }), ];
  ...
  obj5[2] = intl.format(importDefault(2351).k8MmO8, { tierName: str });
  const items2 = [closure_10(require(4251) /* Text */.Text, obj5), ];
...
export default function GiftBadgePostPurchase(arg0) {
  ...
  const stateFromStores = obj.useStateFromStores(items, () => {
    badgeById = badgeById.getBadgeById(callback(8441).BadgeId.GIFTING);
    let tiers;
    if (badgeById != null) {
  ...
  if (null == stateFromStores) {
    ...
    if (1 === tmp10) {
      ...
      obj1[0] = str2;
      obj[0] = intl2.formatToPlainString(importDefault(2351).KjdBPz, obj1);
      const intl3 = tmp2(1236).intl;
      obj[1] = intl3.string(importDefault(2351).oqDrEM);
      const intl4 = tmp2(1236).intl;
      obj[2] = intl4.string(importDefault(2351)["Ka5s+Q"]);
      obj[3] = sum;
      obj[4] = tmp26;
      obj[5] = tmp8;
      obj[6] = onSendGift;
      ...
      obj2[0] = intl5.string(importDefault(2351)["/rBQud"]);
      const intl6 = tmp2(1236).intl;
      obj2[1] = intl6.string(importDefault(2351).DDQMlx);
      let name;
      if (tmp24 != null) {
        name = tmp24.name;
      }
