...
function PostPurchaseFooter(onSendGift) {
  ...
  obj.icon = closure_10(onSendGift(11102).GiftIcon, obj);
  const intl = onSendGift(1114).intl;
  obj.text = intl.string(_modDef2492.g86YiI);
  obj.onPress = callback;
  const items1 = [closure_10(onSendGift(5025).Button, obj), ];
  ...
  items1[1] = closure_10(onSendGift(5025).Button, obj1);
  obj.children = items1;
...
function InProgressScreen(arg0) {
  ...
  obj = { style: tmp.progressWrapper, children: closure_1_10(GiftingBadgeProgressDefault, { progress, currentTier, nextTier, iconSize: 48, title: progressBarTitle }) };
...
let closure_12 = createStyles.createStyles((arg0) => {
  ...
  const obj1 = { gap: nativeDefault.space.PX_12, alignItems: "center", width: "100%", paddingHorizontal: nativeDefault.space.PX_16 };
...
export default function GiftBadgePostPurchase(arg0) {
  ...
  const effect = noop.useEffect(() => {
    ...
    const result = obj.UNSAFE_markDismissibleContentAsDismissed(dismissible_content.DismissibleContent.NEW_GIFTING_BADGES_COACHMARK, obj);
  }, []);
  let obj = initialize;
  ...
  if (null == stateFromStores) {
    ...
    if (key !== key1) {
      if (null != tmp26) {
        obj = { simulatedProgress: sum, currentTier: tmp24, newTier: tmp26, nextTier: tmp8, giftsToNextTier: tmp10, onSendGift };
