// === Module 13049: GuildBoostSlotCooldown ===
...
import registerAssetDefault from "registerAsset" /* 13050 */;
import getSubscriptionPlaceholderPatternSource from "getSubscriptionPlaceholderPatternSource" /* 13051 */;
import closure_4 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_7 from "handleThemeChange" /* 1302 */;
import closure_8 from "createGuildRecordFromRust" /* 1910 */;
import closure_9 from "handleGuildBoostsUpdate" /* 4336 */;
...
function GuildBoostSlotCooldown(cooldownEndsAt) {
  ...
  if (valueOfResult > Date.now()) {
    ...
    tmp5 = callback(cooldownEndsAt(4440).Text, obj);
  }
  return tmp5;
}
...
function GuildBoostSlot(guildBoostSlot) {
  ...
  if (null == guildBoostSlot.guild) {
    ...
    if (!guildBoostSlot.isOnCooldown()) {
      ...
      obj2[2] = tmp6(guildBoostSlot(4440).Text, obj3);
      tmp6Result = tmp6(guildBoostSlot(5015).PressableOpacity, obj2);
    }
    obj[2] = tmp6Result;
    obj[3] = tmp;
...
function UnusedGuildBoostSlots(unusedSlots) {
  ...
  if (found1.length > 0) {
    ...
    obj1[2] = callback(found(4440).Text, obj2);
    obj[1] = callback(found(5015).PressableOpacity, obj1);
...
function BoostedGuild(arg0) {
  ...
  items3[2] = closure_11(guildBoostSlots(13055), { guild: stateFromStores, theme: stateFromStores1 });
  obj1[1] = items3;
...
export default function GuildBoostSlotsInventory() {
  ...
  const effect = React.useEffect(() => {
    const subscriptions = callback(4789).fetchSubscriptions();
    const obj = callback(4789);
    const guildBoostSlots = valueResult(4339).fetchGuildBoostSlots();
  }, []);
  let obj = initialize;
  ...
  if (0 !== found.length) {
    ...
    if (null != stateFromStores) {
      ...
      if (found.length > 0) {
        ...
        const items3 = [callback(tmp3(4440).Text, obj2), found.map((guildId) => closure_1_11(closure_1_20, { guildId, guildBoostSlots: valueResult[guildId] }, guildId))];
        obj1[0] = items3;
