...
import { registerAsset } from "../../../../_runtime/12650_registerAsset.js";
...
function GuildBoostSlotCooldown(cooldownEndsAt) {
  ...
  ({ days, hours, minutes } = useCountdown(memo, 15000));
  const tmp3 = useCountdown(memo, 15000);
  let tmp5 = null;
...
function GuildBoostSlot(guildBoostSlot) {
  ...
  if (null == guildBoostSlot.guild) {
    ...
    if (!guildBoostSlot.isOnCooldown()) {
      ...
      tmp6Result = tmp6(guildBoostSlot(4797).PressableOpacity, obj2);
    }
    obj[2] = tmp6Result;
    obj[3] = tmp;
...
function UnusedGuildBoostSlots(unusedSlots) {
  ...
  if (found1.length > 0) {
    ...
    obj[1] = callback(found(4797).PressableOpacity, obj1);
...
function BoostedGuild(arg0) {
  ...
  items3[2] = closure_11(guildBoostSlots(12655), { guild: stateFromStores, theme: stateFromStores1 });
  obj1[1] = items3;
...
export default function GuildBoostSlotsInventory() {
  ...
  const effect = React.useEffect(() => {
    const subscriptions = callback(4574).fetchSubscriptions();
