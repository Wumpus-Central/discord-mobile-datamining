...
const setGPlayAnalytics = fn(9559).setGPlayAnalytics;
    ...
    if (1 === tmp10) {
      if (premiumSubscription === 1) {
        ...
        closure_130_20 = (function getOfferId(arg0, arg1, arg2, dependencyMap, offerIds) {
          ...
          if (null != offerIds) {
            if (null != arg2) {
              const tmp13 = premiumSubscription(7571).TrialIdToProductOfferId[arg2.trialId];
              let tmp14;
              if (tmp13 != null) {
                tmp14 = tmp13[arg0];
              }
              ...
              const tmp9 = premiumSubscription(7571).DiscountIdToProductOfferId[dependencyMap.discountId];
              let tmp10;
              if (tmp9 != null) {
                tmp10 = tmp9[arg0];
              }
          ...
          if (arg1) {
            BOGO_OFFER_ID = null;
            if (arg0 === premiumSubscription(7571).ProductIds.PREMIUM_TIER_2_MONTHLY) {
              BOGO_OFFER_ID = premiumSubscription(7571).BOGO_OFFER_ID;
            }
          }
      ...
      (function showPurchaseErrorModal(combined) {
        ...
        if (!(combined instanceof premiumSubscription(4727).BillingError)) {
          billingError = new premiumSubscription(4727).BillingError(combined);
        }
        if (tmpResult.isSpendingLimitError(billingError)) {
          const result = premiumSubscription(11047).showSpendingLimitReachedAlert();
          const tmpResult2 = premiumSubscription(11047);
        } else {
          const intl = premiumSubscription(1115).intl;
          let tmp8 = billingError.code !== premiumSubscription(4503).ErrorCodes.UNKNOWN;
          if (tmp8) {
          ...
          obj.body = message;
          closure_1_1(5193).show(obj);
          const obj2 = closure_1_1(5193);
        }
        tmpResult = premiumSubscription(11047);
      })(closure_130_23);
