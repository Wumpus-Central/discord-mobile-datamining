// === Module 17479: GooglePlayPriceChangeActionSheet ===
...
export default function GooglePlayPriceChangeActionSheet(markAsDismissed) {
  ...
  const tierDisplayNameByPlanId = markAsDismissed(4481).getTierDisplayNameByPlanId(str);
  const tmp2Result = markAsDismissed(4481);
  const tmp2Result5 = markAsDismissed(4481);
  const intervalStringAsNoun = markAsDismissed(4481).getIntervalStringAsNoun(tmp2Result5.getInterval(str).intervalType);
  const tmp2Result6 = markAsDismissed(4481);
  const tmp2Result7 = markAsDismissed(7565);
  const formatPriceResult = markAsDismissed(7565).formatPrice(stateFromStores.oldPrice, stateFromStores.oldCurrency);
  const tmp2Result8 = markAsDismissed(7565);
  ...
  const items2 = [closure_8(markAsDismissed(4823).Text, obj6), ];
  ...
  const formatPriceResult1 = markAsDismissed(7565).formatPrice(stateFromStores.newPrice, stateFromStores.newCurrency);
  ...
  items2[1] = closure_8(markAsDismissed(4823).Text, obj7);
  ...
  items3[1] = closure_8(markAsDismissed(5271).Button, obj9);
