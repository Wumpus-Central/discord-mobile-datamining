...
export default function GooglePlayPriceChangeActionSheet(markAsDismissed) {
  ...
  let tmp2Result = tmp2(3931);
  const tierDisplayNameByPlanId = tmp2Result.getTierDisplayNameByPlanId(str);
  tmp2Result = tmp2(3931);
  const intervalStringAsNoun = markAsDismissed(3931).getIntervalStringAsNoun(tmp2Result.getInterval(str).intervalType);
  const tmp2Result1 = markAsDismissed(3931);
  const tmp2Result2 = markAsDismissed(5764);
  const formatPriceResult = markAsDismissed(5764).formatPrice(stateFromStores.oldPrice, stateFromStores.oldCurrency);
  const tmp2Result3 = markAsDismissed(5764);
  obj = { children: null };
  ...
  const items2 = [callback(markAsDismissed(4281).Text, obj2), ];
  ...
  const formatPriceResult1 = markAsDismissed(5764).formatPrice(stateFromStores.newPrice, stateFromStores.newCurrency);
  ...
  items3[1] = callback(markAsDismissed(4695).Button, obj5);
  obj[1] = items3;
