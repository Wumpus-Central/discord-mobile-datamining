// === Module 16358: GooglePlayPriceChangeActionSheet ===
...
import closure_5 from "onInitializeSync" /* 16359 */;
...
export default function GooglePlayPriceChangeActionSheet(markAsDismissed) {
  ...
  tmp2Result = tmp2(4107);
  const intervalStringAsNoun = markAsDismissed(4107).getIntervalStringAsNoun(tmp2Result.getInterval(str).intervalType);
  const tmp2Result1 = markAsDismissed(4107);
  const tmp2Result2 = markAsDismissed(6028);
  const formatPriceResult = markAsDismissed(6028).formatPrice(stateFromStores.oldPrice, stateFromStores.oldCurrency);
  const tmp2Result3 = markAsDismissed(6028);
  obj = { children: null };
  ...
  const items2 = [callback(markAsDismissed(4440).Text, obj2), ];
  ...
  const formatPriceResult1 = markAsDismissed(6028).formatPrice(stateFromStores.newPrice, stateFromStores.newCurrency);
  ...
  items2[1] = callback(markAsDismissed(4440).Text, obj3);
  obj1[1] = items2;
  ...
  items3[1] = callback(markAsDismissed(4879).Button, obj5);
  obj[1] = items3;
