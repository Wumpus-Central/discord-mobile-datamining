  ...
  if (flag) {
          ...
          if (currencyCode.currencyCode !== stateFromStores.currencyCode) {
            ...
            if (diff > 0) {
              if (diff < currencyCode.price) {
                ...
                obj5.priceString = tmp4(7344).formatPrice(result, currencyCode.currencyCode, {
    ...
    if (null == checkoutContext) {
      ...
      const availablePlanForItems = checkoutContext.getAvailablePlanForItems(
        tmp4(7519).getSubscriptionItemsForProduct(plan.productId),
      );
      if (null == availablePlanForItems) {
        ...
        if (null != addOnPrice) {
          if (addOnPrice.majorUnits > 0) {
              ...
              if (null != tmp17) {
                if (0 !== tmp17.numPremiumGuild) {
                  const availablePlanForItems1 = checkoutContext.getAvailablePlanForItems(
                    tmp4(7519).getSubscriptionItemsForProduct(tmp3),
                  );
                  let addOnPrice1;
                  ...
                  if (null != addOnPrice1) {
                    ...
                    if (diff1 > 0) {
                      const obj8 = {
                        priceString: tmp4(7344).formatPrice(diff1, addOnPrice.currency, { convertToMajorUnits: false }),
                        failure: null,
                      };
            ...
            const obj9 = {
              priceString: tmp4(7344).formatPrice(addOnPrice.majorUnits, addOnPrice.currency, {
                convertToMajorUnits: false,
              }),
              failure: null,
            };
