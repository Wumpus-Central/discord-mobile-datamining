...
import ServerThemeApexShadowExperiment2 from "ServerThemeApexShadowExperiment.tsx";
...
const items = [
  { id: 0, label: "Control", config: { enabled: false, inExperiment: true, gatesApex: false, rollbackEnabled: false } },
  {
    id: 1,
    label: "Enable Server Theme",
    config: { enabled: true, inExperiment: true, gatesApex: false, rollbackEnabled: false },
  },
  {
    id: 2,
    label: "Rollback UI for Server Theme",
    config: { enabled: true, inExperiment: true, gatesApex: false, rollbackEnabled: true },
  },
];
...
export const getServerThemeEnabled = function getServerThemeEnabled(guildId, GuildPowerupsConstants) {
  let currentConfig = experiment.getCurrentConfig(
    { guildId, location: GuildPowerupsConstants },
...
export const useServerThemeEnabled = function useServerThemeEnabled(guildId, useGuildPowerupNewPerkMarketingVersion) {
  experiment = experiment.useExperiment(
    { guildId, location: useGuildPowerupNewPerkMarketingVersion },
    { autoTrackExposure: false },
  );
  const ServerThemeApexShadowExperiment = ServerThemeApexShadowExperiment2.ServerThemeApexShadowExperiment;
  let tmp3 = guildId;
  ...
  let defaultConfig = ServerThemeApexShadowExperiment.useConfig({
    guildId: tmp3,
    location: useGuildPowerupNewPerkMarketingVersion,
  });
...
export const getServerThemeRollbackEnabled = function getServerThemeRollbackEnabled(guildId, GuildPowerupsManager) {
  let currentConfig = experiment.getCurrentConfig(
    { guildId, location: GuildPowerupsManager },
    { autoTrackExposure: false },
  );
  if (null != guildId) {
    const ServerThemeApexShadowExperiment = ServerThemeApexShadowExperiment2.ServerThemeApexShadowExperiment;
...
export const useServerThemeRollbackEnabled = function useServerThemeRollbackEnabled(
  guildId,
