...
export default function useMaybeGetSortedBoosts(arg0, arg1) {
  ...
  const items1 = [arg0];
  stateFromStoresArray = _require(stateFromStoresArray[6]).useStateFromStoresArray(
    items,
    () => {
  ...
  stateFromStoresArray1 = _require(stateFromStoresArray[6]).useStateFromStoresArray(
    items3,
    () => {
      const set = new Set();
  ...
  const items9 = [arg0];
  stateFromStores1 = _require(stateFromStoresArray[6]).useStateFromStores(
    items8,
    () => null != stateFromStoresArray1.getLastFetchedAtForGuild(closure_0),
    items9,
  );
  const items10 = [arg0, stateFromStores, memo1, stateFromStores1];
  const effect1 = memo.useEffect(() => {
    if (!tmp) {
      const appliedGuildBoostsForGuild = callback(stateFromStoresArray[8]).fetchAppliedGuildBoostsForGuild(callback, {
        includeEnded: true,
      });
