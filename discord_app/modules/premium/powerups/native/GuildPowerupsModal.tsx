// === Module 11649: GuildPowerupsModal ===
...
const result = require("obj132").fileFinishedImporting("modules/premium/powerups/native/GuildPowerupsModal.tsx");
...
export default function GuildPowerupsModal(guildId) {
  ...
  { value: tmp10(...items1).analyticsLocations, children: null };
  ...
  const items5 = [
    ...
    buildGuildPowerupsSections.map((item, index) => {
      const type = item.type;
      if (closure_1_7.LEVEL === type) {
        let obj = { guildId: null, listings: null };
        obj[0] = guildId;
        obj[1] = item.listings;
        return closure_1_8(autoOpenPerkId(autoOpenRequestId[24]), obj, item.type);
      } else if (tmp.PERK === type) {
        obj = { guildId: null, listings: null };
        obj[0] = guildId;
        obj[1] = item.listings;
        return closure_1_8(autoOpenPerkId(autoOpenRequestId[25]), obj, item.type);
      } else {
        return null;
      }
    }),
  ...
  const tmp13 = autoOpenPerkId(autoOpenRequestId[16])(guildId);
