// === Module 15433: useGuildPowerupsCoachmark ===
...
let closure_14 = createCacheKey.createStyles((arg0) => {
  const obj = { height: 120, width: 260 - 2 * ThemesDefault.space.PX_16, borderRadius: ThemesDefault.radii.md };
  obj[0] = obj;
...
const result = require("obj132").fileFinishedImporting("modules/premium/powerups/native/hooks/useGuildPowerupsCoachmark.tsx");
  ...
  const memo = React.useMemo(() => {
    ...
    if (null == guildPowerupBannerImage) {
      ...
      if (callback(stateFromStores[14]).GuildPowerupNotificationPopoutType.LEVEL_REACHED === type) {
        ...
        return obj2;
      } else if (callback(stateFromStores[14]).GuildPowerupNotificationPopoutType.PERKS_PURCHASABLE === type) {
        ...
        if (0 === powerups.length) {
          ...
          obj6.renderImgComponent = function renderImgComponent() {
            ...
            obj = { imageUrl: str, isAnimated: 1 === powerups.length, style: items };
            items = [, ];
            ({ coachmarkImage: arr2[0], coachmarkCover: arr2[1] } = powerup);
            return jsx(guildPowerupBannerImage(stateFromStores[20]), { imageUrl: str, isAnimated: 1 === powerups.length, style: items });
          };
      ...
      } else if (callback(stateFromStores[14]).GuildPowerupNotificationPopoutType.NEW_PERK_AVAILABLE === type) {
        const powerups1 = tmp.powerups;
        const found = powerups1.find((item, index) => item.skuId === powerups(found1[23]).GUILD_POWERUP_TAG_SKU_ID);
        if (null != found) {
          guildPowerupBannerImage = callback(stateFromStores[22]).getGuildPowerupBannerImage(found, stateFromStores1, true);
          if (null != guildPowerupBannerImage) {
            const obj8 = {};
            const merged3 = Object.assign(obj);
            obj8.visible = true;
            obj8.renderImgComponent = function renderImgComponent() {
              return jsx(guildPowerupBannerImage(stateFromStores[20]), { imageUrl: guildPowerupBannerImage, isAnimated: !stateFromStores1, style: powerup.coachmarkImage });
            };
          ...
          const tmp50Result = callback(stateFromStores[22]);
        }
        found1 = powerups1.find((item, index) => item.skuId === powerups(found1[23]).GUILD_POWERUP_GUILD_THEME_SKU_ID);
        if (null != found1) {
          const obj9 = {};
          ...
          obj9.renderImgComponent = function renderImgComponent() {
            ...
            obj = { imageUrl: guildPowerupBannerImage, isAnimated: !stateFromStores1, style: items };
            items = [, ];
            ({ coachmarkImage: arr[0], coachmarkCover: arr[1] } = powerup);
            return jsx(guildPowerupBannerImage(stateFromStores[20]), { imageUrl: guildPowerupBannerImage, isAnimated: !stateFromStores1, style: items });
          };
          ...
          return obj9;
        } else if (null != powerups1.find((item, index) => item.skuId === powerups(found1[23]).VANITY_URL_POWERUP_SKU_ID)) {
          const obj10 = {};
          ...
          return obj10;
        } else if (null != powerups1.find((item, index) => set2.has(item.skuId))) {
          const obj11 = {};
          ...
          return obj11;
        } else if (null != powerups1.find((item, index) => set.has(item.skuId))) {
          const obj12 = {};
          ...
          let tmp52 = obj;
          if (null != powerups1.find((item, index) => item.skuId === powerups(found1[23]).GUILD_POWERUP_MAX_FILE_SIZE_250_MB_SKU_ID)) {
            const obj13 = {};
      ...
      } else if (callback(stateFromStores[14]).GuildPowerupNotificationPopoutType.BOOST_TO_UNLOCK === type) {
        ...
        obj14.renderImgComponent = function renderImgComponent() {
          if (powerup.type === closure_2_8.LEVEL) {
            ...
            let tmp9Result = jsx(guildPowerupBannerImage(stateFromStores[17]), { style: null, gemWidth: 30, gemHeight: 30 });
          } else {
            const tmp12 = guildPowerupBannerImage(stateFromStores[20]);
            let str = powerups(stateFromStores[22]).getGuildPowerupBannerImage(tmp, stateFromStores1, true);
            if (str == null) {
              str = "";
            }
            ...
            obj[2] = items;
            tmp9Result = <tmp12 imageUrl={null} isAnimated={null} style={null} />;
