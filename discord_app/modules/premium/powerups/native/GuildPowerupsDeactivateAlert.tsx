// === Module 12171: GuildPowerupsDeactivateAlert ===
...
import useGuildPowerupOnDeactivateDefault from "useGuildPowerupOnDeactivate" /* 12172 */;
import useDeactivateWarningTextDefault from "useDeactivateWarningText" /* 12173 */;
...
export default function GuildPowerupsDeactivateAlert(arg0) {
  ...
  let obj = _require(12174);
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, _require(12174).ModalType.DEACTIVATE);
  ...
  const tmp8 = View;
  obj[1] = callback(_require(8005).CircleErrorIcon, { color: ThemesDefault.colors.INTERACTIVE_ICON_DEFAULT, size: "custom", style: { width: 40, height: 40 } });
  ...
  if (tmp7Result) {
    ...
    obj4[3] = error;
    tmp7Result = tmp7(tmp5(4440).Text, obj4);
  }
  ...
  items[1] = callback(_require(4814).AlertActionButton, obj6, "deactivate");
  const obj7 = {
    onPress() {
  ...
  items[2] = callback(_require(4814).AlertActionButton, obj7, "cancel");
  obj5[0] = items;
  obj[3] = closure_5(_require(4814).AlertActions, obj5);
  ...
  return callback(_require(4814).AlertModal, obj);
};