...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  let obj = analyticsLocations(12802);
  ...
  let everyResult = stateFromStores.size === analyticsLocations(12802).MAX_REFERRALS_SENT;
  if (everyResult) {
    const items2 = [];
  ...
  const items4 = [callback(obj4(12810), obj), callback(ProgressIndicator, { referralSentUsers: obj.useReferralProgramBannerDetails().referralSentUsers }), , ];
  ...
  if (false !== stateFromStores1) {
    if (tmp14 !== tmp2(12802).MAX_REFERRALS_SENT) {
    ...
    if (!tmp22) {
      tmp22 = stateFromStores.size === tmp2(12802).MAX_REFERRALS_SENT;
    }
    ...
    obj8[4] = callback;
    obj7[1] = tmp12(tmp2(4777).Button, obj8);
