// === Module 13018: ReferredFriendAvatar ===
...
import ProgressWheelDefault from "ProgressWheel" /* 13027 */;
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  const tmp = callback2();
  let obj = analyticsLocations(13019);
  ...
  let everyResult = stateFromStores.size === analyticsLocations(13019).MAX_REFERRALS_SENT;
  if (everyResult) {
    const items2 = [];
  ...
  if (false !== stateFromStores1) {
    ...
    if (!tmp22) {
      tmp22 = stateFromStores.size === tmp2(13019).MAX_REFERRALS_SENT;
    }
    ...
    obj8[4] = callback;
    obj7[1] = tmp12(tmp2(4880).Button, obj8);
