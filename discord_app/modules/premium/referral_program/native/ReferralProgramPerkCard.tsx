...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  let obj = analyticsLocations(12783);
  ...
  let everyResult = stateFromStores.size === analyticsLocations(12783).MAX_REFERRALS_SENT;
  if (everyResult) {
    const items2 = [];
  ...
  const items4 = [callback(obj4(12791), obj), callback(ProgressIndicator, { referralSentUsers: obj.useReferralProgramBannerDetails().referralSentUsers }), , ];
  ...
  if (false !== stateFromStores1) {
    ...
    if (!tmp22) {
      tmp22 = stateFromStores.size === tmp2(12783).MAX_REFERRALS_SENT;
    }
