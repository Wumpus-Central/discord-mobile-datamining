...
function HasBadgeCoachmark(markAsDismissed) {
  ...
  return closure_8(markAsDismissed(5377).BottomSheet, obj);
}
...
export default function GiftingBadgesCoachmarkActionSheet(markAsDismissed) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => {
    ...
    const obj = { currentTier: getCurrentTier(callback(8709).BadgeId.GIFTING), giftCount: null };
    ({ getCurrentTier, getSingleRequirementProgress } = map);
    const singleRequirementProgress = getSingleRequirementProgress(callback(8709).BadgeId.GIFTING);
    let current;
