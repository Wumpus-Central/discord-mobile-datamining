...
function HasBadgeCoachmark(markAsDismissed) {
  ...
  const items3 = [closure_8(markAsDismissed(4298).Text, obj3), ];
  ...
  items3[1] = closure_8(markAsDismissed(4298).Text, obj4);
  obj2[1] = items3;
  ...
  obj6[2] = callback;
  obj5[1] = closure_8(markAsDismissed(4712).Button, obj6);
  items2[2] = closure_8(closure_5, obj5);
  obj[1] = items2;
  obj[2] = closure_9(closure_5, obj);
  return closure_8(markAsDismissed(5396).BottomSheet, obj);
}
function NewBadgeCoachmark(markAsDismissed) {
  ...
  const items3 = [callback(markAsDismissed(4298).Text, obj4), ];
  ...
  items3[1] = callback(markAsDismissed(4298).Text, obj5);
  obj3[1] = items3;
  ...
  obj7[2] = callback(markAsDismissed(9510).GiftIcon, obj8);
  obj7[3] = callback;
  obj6[1] = callback(markAsDismissed(4712).Button, obj7);
...
export default function GiftingBadgesCoachmarkActionSheet(markAsDismissed) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => {
    ...
    const obj = { currentTier: getCurrentTier(callback(8726).BadgeId.GIFTING), giftCount: null };
    ({ getCurrentTier, getSingleRequirementProgress } = map);
    const singleRequirementProgress = getSingleRequirementProgress(callback(8726).BadgeId.GIFTING);
    let current;
