...
function HasBadgeCoachmark(markAsDismissed) {
  ...
  if (tmp4Result) {
    ...
    tmp4Result = tmp4(importDefault(9720), obj1);
  }
  obj[1] = tmp4Result;
  ...
  obj3[3] = intl.format(importDefault(2321)["a+jfuy"], { tierName: str });
  const items3 = [closure_8(markAsDismissed(4189).Text, obj3), ];
  const obj4 = { style: tmp.text, variant: "text-sm/medium", color: "text-default", children: null };
  const intl2 = tmp5(1236).intl;
  if (giftCount == null) {
    giftCount = 0;
  }
  obj4[3] = intl2.formatToPlainString(importDefault(2321).QxRA6w, { giftCount });
  items3[1] = closure_8(markAsDismissed(4189).Text, obj4);
  obj2[1] = items3;
  ...
  obj6[2] = callback;
  obj5[1] = closure_8(markAsDismissed(4604).Button, obj6);
  items2[2] = closure_8(closure_5, obj5);
  obj[1] = items2;
  obj[2] = closure_9(closure_5, obj);
  return closure_8(markAsDismissed(5247).BottomSheet, obj);
}
function NewBadgeCoachmark(markAsDismissed) {
  ...
  obj4[3] = intl.string(importDefault(2321).Q2RQka);
  const items3 = [callback(markAsDismissed(4189).Text, obj4), ];
  const obj5 = { style: tmp.text, variant: "text-sm/medium", color: "text-muted", children: null };
  const intl2 = markAsDismissed(1236).intl;
  obj5[3] = intl2.string(importDefault(2321)["3EQnkg"]);
  items3[1] = callback(markAsDismissed(4189).Text, obj5);
  obj3[1] = items3;
  ...
  obj7[1] = intl3.string(importDefault(2321).DZnomS);
  const obj8 = { size: "sm", color: null };
  obj8[1] = importDefault(712).colors.CONTROL_PRIMARY_TEXT_DEFAULT;
  obj7[2] = callback(markAsDismissed(8777).GiftIcon, obj8);
  obj7[3] = callback;
  obj6[1] = callback(markAsDismissed(4604).Button, obj7);
...
export default function GiftingBadgesCoachmarkActionSheet(markAsDismissed) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => {
    ...
    const obj = { currentTier: getCurrentTier(callback(8110).BadgeId.GIFTING), giftCount: null };
    ({ getCurrentTier, getSingleRequirementProgress } = map);
    const singleRequirementProgress = getSingleRequirementProgress(callback(8110).BadgeId.GIFTING);
    let current;
