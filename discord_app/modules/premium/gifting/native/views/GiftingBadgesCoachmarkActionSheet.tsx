...
function HasBadgeCoachmark(markAsDismissed) {
  ...
  obj6[2] = callback;
  obj5[1] = closure_8(markAsDismissed(4695).Button, obj6);
  items2[2] = closure_8(closure_5, obj5);
  obj[1] = items2;
  obj[2] = closure_9(closure_5, obj);
  return closure_8(markAsDismissed(5338).BottomSheet, obj);
}
...
export default function GiftingBadgesCoachmarkActionSheet(markAsDismissed) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => {
    ...
    const obj = { currentTier: getCurrentTier(callback(8708).BadgeId.GIFTING), giftCount: null };
    ({ getCurrentTier, getSingleRequirementProgress } = map);
    const singleRequirementProgress = getSingleRequirementProgress(callback(8708).BadgeId.GIFTING);
    let current;
