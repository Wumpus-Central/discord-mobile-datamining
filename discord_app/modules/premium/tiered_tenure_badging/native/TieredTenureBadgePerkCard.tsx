// === Module 13100: TieredTenureBadgePerkCard ===
...
export const TieredTenureBadgePerkCard = function TieredTenureBadgePerkCard() {
  let obj = tieredTenureBadgeData(8966);
  tieredTenureBadgeData = obj.useTieredTenureBadgeData();
  obj1 = tieredTenureBadgeData(8966);
  const premiumSince = obj1.usePremiumSince();
  let obj2 = tieredTenureBadgeData(13101);
  ...
  let obj4 = tieredTenureBadgeData(8940);
  let id;
  ...
  if (null == tieredTenureBadgeData) {
    ...
    if (tieredTenureBadgeData.status === tmp(8966).TieredTenureBadgeStatus.UPCOMING) {
      const intl2 = tmp(1236).intl;
      stringResult = intl2.string(tmp(1236).t.O9TBwQ);
    } else if (tieredTenureBadgeData.status === tmp(8966).TieredTenureBadgeStatus.WITHHELD) {
    ...
    if (null != premiumSince) {
      ...
      if (tmp(8966).TieredTenureBadgeStatus.EARNED !== status) {
        if (tmp(8966).TieredTenureBadgeStatus.WITHHELD !== status) {
          formatResult = null;
          tmp14 = large;
          if (tmp(8966).TieredTenureBadgeStatus.UPCOMING === status) {
            formatResult = null;
            tmp14 = large;
    ...
    obj4[3] = tmp(8965).getTenureBadgeRequirementString(tieredTenureBadgeData.id, tieredTenureBadgeData.tenureReqNumMonths);
    ...
    const tmp34 = stateFromStores(13077);
    let upcomingBadge = tieredTenureBadgeData.status === tmp(8966).TieredTenureBadgeStatus.UPCOMING;
    if (!upcomingBadge) {
      upcomingBadge = tieredTenureBadgeData.status === tmp(8966).TieredTenureBadgeStatus.WITHHELD;
    }
    if (upcomingBadge) {
    ...
    obj10[0] = tmp14;
    obj9[2] = obj10;
    obj8[1] = callback(stateFromStores(5493), obj9);
