...
function PremiumFeatureUpsellPill(featureName) {
  ...
  if (!mobileEmojiPickerUpsellRestyleEnabledForFeature) {
    mobileEmojiPickerUpsellRestyleEnabledForFeature = tmp(8614).getMobileStickerPickerUpsellRestyleEnabledForFeature(
      featureName,
  ...
  if (featureName(7268).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === featureName) {
    ...
    const obj7 = {
      maxFileSize: tmp(4485).getMaxFileSizeForPremiumType(tmp7),
  ...
  const tmp11Result = tmp11(
    ...
    featureName(9410).getAnalyticsPage(featureName),
  );
  ...
  const tmpResult6 = featureName(9410);
}
