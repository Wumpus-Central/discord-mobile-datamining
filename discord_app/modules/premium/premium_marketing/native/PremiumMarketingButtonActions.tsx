// === Module 7967: getButtonActionHandler ===

// Module 7967 (getButtonActionHandler)
import create from "create" /* 7560 */;
import closure_3 from "createEmptyPromotionsByType" /* 7553 */;
...
export const getButtonActionHandler = function getButtonActionHandler(arg0) {
  ...
  if (create.ButtonAction.OPEN_SOCIAL_LAYER_STOREFRONT === buttonAction) {
  ...
  } else if (tmp(7560).ButtonAction.OPEN_TIER_1_PAYMENT_MODAL === buttonAction) {
    return () => {
    ...
    if (tmp(7560).ButtonAction.OPEN_TIER_2_PAYMENT_MODAL !== buttonAction) {
      if (tmp(7560).ButtonAction.OPEN_TIER_2_PAYMENT_MODAL_CUSTOM_CONFIRMATION_FOOTER !== buttonAction) {
        if (tmp(7560).ButtonAction.OPEN_PLAN_SELECTION_MODAL === buttonAction) {
          return () => {
          ...
          const OPEN_MARKETING_PAGE = tmp(7560).ButtonAction.OPEN_MARKETING_PAGE;
          return () => {
