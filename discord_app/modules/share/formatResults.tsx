...
let closure_12 = async function _getOrResolveChannelIdFromDestinationId(arg0) {
  if (c1 === 2) {
    c1 = 3;
...
function canShareToChannel(type, arg1) {
  if (type.type === sortByMatchScore.AutocompleterResultTypes.USER) {
    ...
    if (!tmp7) {
      ...
      if (tmp4) {
        tmp4 =
          PermissionStore.can(constants.VIEW_CHANNEL, type.record) &&
          PermissionStore.can(constants.SEND_MESSAGES, type.record);
        const tmp6 =
          PermissionStore.can(constants.VIEW_CHANNEL, type.record) &&
          PermissionStore.can(constants.SEND_MESSAGES, type.record);
      }
...
const isAllowedType = fn(11336).isAllowedType;
...
export default function formatResults(hasQuery) {
  ...
  if (hasQuery.hasQuery) {
    ...
    return mergeAndDedupeResultsWithHeaders(
      results.filter((type) => {
    ...
    if (null != pinnedDestinations) {
      if (pinnedDestinations.length > 0) {
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            let tmp6 = null;
            if (null != user) {
              const obj = { type: queryMode(10182).AutocompleterResultTypes.USER, record: user, score: 0 };
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = originDestination(10191)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((item) => originDestination(10191)(item));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => originDestination(10191)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
      ...
      if (null == targetDestination) {
        ...
        if (null != originDestination) {
          ...
          if (null != queryMode) {
            let substr = mergeAndDedupeResultsWithHeaders(
              found.filter((type) => type.type === queryMode),
              items1,
            );
          } else {
        ...
        if (null != user) {
          let obj = { type: queryMode(10182).AutocompleterResultTypes.USER, record: user, score: 0 };
          targetDestination = obj;
        }
      } else {
        const tmp9 = originDestination(10191)(targetDestination.id);
      }
    }
    mapped = [];
  }
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const headerResult = queryMode(10182).createHeaderResult(intl.string(queryMode(1115).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      HermesBuiltin.arraySpread(
        found1.filter((type) => type.type === queryMode),
        1,
      );
      let items4 = items3;
    } else {
      const intl2 = queryMode(1115).intl;
      items4 = [queryMode(10182).createHeaderResult(intl2.string(queryMode(1115).t["80lOZ1"])), ,];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === queryMode(10182).AutocompleterResultTypes.USER;
            if (!canResult) {
      ...
      const tmpResult = queryMode(10182);
    }
    return mergeAndDedupeResultsWithHeaders(items4, items1);
  }
  const obj = queryMode(10182);
};
