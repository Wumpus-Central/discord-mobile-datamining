  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
      if (pinnedDestinations.length > 0) {
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            let tmp6 = null;
            if (null != user) {
              const obj = { type: queryMode(10097).AutocompleterResultTypes.USER, record: user, score: 0 };
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = originDestination(10106)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((item) => originDestination(10106)(item));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => originDestination(10106)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
      ...
      if (null == targetDestination) {
        ...
        if (null != user) {
          let obj = { type: queryMode(10097).AutocompleterResultTypes.USER, record: user, score: 0 };
          targetDestination = obj;
        }
      } else {
        const tmp9 = originDestination(10106)(targetDestination.id);
      }
    }
    mapped = [];
  }
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const headerResult = queryMode(10097).createHeaderResult(intl.string(queryMode(1115).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [queryMode(10097).createHeaderResult(intl2.string(queryMode(1115).t["80lOZ1"])), ,];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === queryMode(10097).AutocompleterResultTypes.USER;
            if (!canResult) {
      ...
      const tmpResult = queryMode(10097);
    }
    return mergeAndDedupeResultsWithHeaders(items4, items1);
  }
  const obj = queryMode(10097);
};
