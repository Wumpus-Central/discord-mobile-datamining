...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [queryMode(9975).createHeaderResult(intl2.string(queryMode(1115).t["80lOZ1"])), ,];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === queryMode(9975).AutocompleterResultTypes.USER;
            if (!canResult) {
      ...
      const tmpResult = queryMode(9975);
    }
