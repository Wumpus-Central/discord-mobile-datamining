    ...
    if (null != pinnedDestinations) {
      ...
      if (null == targetDestination) {
        ...
        arraySpreadResult = HermesBuiltin.arraySpread(
          mapped2,
          HermesBuiltin.arraySpread(mapped1, arraySpreadResult + 1),
        );
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    const found = mapped.filter(queryMode(1369).isNotNullish);
    ...
    if (null != queryMode) {
      ...
      items4 = [queryMode(9965).createHeaderResult(intl2.string(queryMode(1114).t["80lOZ1"])), ,];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === queryMode(9965).AutocompleterResultTypes.USER;
            if (!canResult) {
      ...
      const tmpResult = queryMode(9965);
    }
