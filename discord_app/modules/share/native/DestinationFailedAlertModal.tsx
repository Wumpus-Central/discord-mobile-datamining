...
function FailedUserRow(user) {
  ...
  if (null != user) {
    ...
    obj = { user, guildId: "Boolean", status: true, isMobileOnline: "done", isVROnline: true, size: null, avatarDecoration: "primary", autoStatusCutout: null };
    let tmp13 = null;
    ...
    items2[1] = closure_10(tmp2(4570).Text, obj1);
    obj.children = items2;
...
function FailedChannelRow(channel) {
  ...
  obj = { style: tmp.row, children: null };
  obj = { "aria-label": "", guild: stateFromStores, channel, size: channel(11031).GuildIconWithChannelTypeSizes.SMALL_32 };
  const items1 = [closure_10(channel(11031).GuildIconWithChannelType, obj), ];
  const tmp3 = useChannelNameDefault(channel);
  items1[1] = closure_10(channel(4570).Text, { style: tmp.label, variant: "text-md/medium", lineClamp: 1, ellipsizeMode: "tail", children: useChannelNameDefault(channel) });
  obj.children = items1;
