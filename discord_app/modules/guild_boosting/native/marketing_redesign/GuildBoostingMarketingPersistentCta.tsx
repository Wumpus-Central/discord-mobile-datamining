// === Module 6760: GuildBoostingMarketingPersistentCta ===
...
export default function GuildBoostingMarketingPersistentCta(arg0) {
  ...
  obj = { useReducedMotion: stateFromStores, VISIBILITY_OFFSET: 120, withSpring: isVisible(4665).withSpring, isVisible, SPRING_CONFIG: closure_8 };
  fn.__closure = obj;
  ...
  const tmp4 = stateFromStores(4677);
  obj4[3] = isVisible(5632).GuildIconSizes.LARGE;
  const items3 = [callback(stateFromStores(5632), obj4), callback(isVisible(4251).Text, { style: tmp.guildName, variant: "text-md/bold", lineClamp: 1, children: guild.name })];
  obj3[1] = items3;
  const items4 = [callback2(View, obj3), ];
  const obj5 = { style: tmp.guildName, variant: "text-md/bold", lineClamp: 1, children: guild.name };
  const tmp5 = stateFromStores(5632);
  items4[1] = callback(View, { style: tmp.buttonContainer, children: callback(stateFromStores(6761), obj7) });
  obj2[1] = items4;
  obj1[6] = callback2(View, obj2);
  obj[1] = callback(tmp4, obj1);
  return callback(stateFromStores(4116).View, obj);
};
