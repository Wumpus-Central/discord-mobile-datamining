...
import ReleaseChannelUtils from "ReleaseChannelUtils" /* 5172 */;
import MetricEvents from "MetricEvents" /* 5176 */;
import AppCrashedReasons2 from "AppCrashedReasons" /* 14446 */;
...
function trackCrash(event, hint, arg2) {
  ...
  items[1] = "level:" + level;
  obj6.tags = items;
  tmp26(5171).increment(obj6, true);
  const tmp26Result = tmp26(5171);
}
    ...
    const isUserStaffForCrashReporting = CrashReportingManager.getIsUserStaffForCrashReporting((arg0) => {
      ...
      if (-1 === ReleaseChannel.indexOf("debug")) {
        if (-1 === ReleaseChannel.indexOf("developer")) {
          ...
          const obj3 = { tunnel: `/error-reporting-proxy/${str2}`, autoInitializeNativeSdk: false, beforeSend, dist: "6502", dsn: SentryStaffDsn, environment: ReleaseChannel, tracesSampleRate: 0, sampleRate: 1, ignoreErrors, release: "discord_android@348.3.0-2+348203", tracePropagationTargets: null, integrations: null, beforeBreadcrumb: null };
          ...
          _mod675.setTag("buildNumber", "6502");
