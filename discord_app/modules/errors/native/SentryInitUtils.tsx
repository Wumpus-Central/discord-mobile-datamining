...
import ReleaseChannelUtils from "ReleaseChannelUtils" /* 5170 */;
import MetricEvents from "MetricEvents" /* 5174 */;
import AppCrashedReasons2 from "AppCrashedReasons" /* 14437 */;
...
function trackCrash(event, hint, arg2) {
  ...
  items[1] = "level:" + level;
  obj6.tags = items;
  tmp26(5169).increment(obj6, true);
  const tmp26Result = tmp26(5169);
}
    ...
    const isUserStaffForCrashReporting = CrashReportingManager.getIsUserStaffForCrashReporting((arg0) => {
      ...
      if (-1 === ReleaseChannel.indexOf("debug")) {
        if (-1 === ReleaseChannel.indexOf("developer")) {
          ...
          const obj3 = { tunnel: `/error-reporting-proxy/${str2}`, autoInitializeNativeSdk: false, beforeSend, dist: "6493", dsn: SentryStaffDsn, environment: ReleaseChannel, tracesSampleRate: 0, sampleRate: 1, ignoreErrors, release: "discord_android@348.2.0-2+348202", tracePropagationTargets: null, integrations: null, beforeBreadcrumb: null };
          ...
          _mod675.setTag("buildNumber", "6493");
