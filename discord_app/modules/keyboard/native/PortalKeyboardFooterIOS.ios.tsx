...
createStyles.keyboardStickyFooter = rect;
...
export default function PortalKeyboardFooterIOS(animatedSheetIndex) {
  ...
  const obj2 = animatedSheetIndex(4441);
  class S {
    constructor() {
      tmp = closure_0;
      tmp2 = closure_2;
      ...
      if (c1) {
        ...
        if (animatedSheetIndex.get() >= 0) {
          ...
          tmp10 = height;
          obj1.translateY = height.get() + tmp5;
          items = [];
          items[0] = obj1;
          obj.transform = items;
        }
      ...
      items1[1] = 0;
      obj2.translateY = tmpResult.interpolate(value1, [-1, 0], items1, "clamp");
      items2 = [];
      items2[0] = obj2;
  ...
  obj = { interpolate: animatedSheetIndex(4310).interpolate, progress, bottom, followSystemKeyboard: flag, keyboardType: keyboardTypeSharedValue, KeyboardTypes: animatedSheetIndex(1609).KeyboardTypes, animatedSheetIndex, height, EXPRESSION_FOOTER_HEIGHT: animatedSheetIndex(1093).EXPRESSION_FOOTER_HEIGHT };
  S.__closure = obj;
  S.__workletHash = 9444646970651;
  S.__initData = keyboardTypeSharedValue;
  animatedStyle = animatedSheetIndex(4310).useAnimatedStyle(S);
  let items = [animatedStyle, tmp];
  const memo = bottom.useMemo(() => {
    const items = [keyboardStickyFooter.keyboardStickyFooter, animatedStyle];
    return items;
  }, items);
  obj = { style: memo, children: height(animatedSheetIndex(4446).PortalHost, { name: str }) };
  return height(flag(4310).View, obj);
};