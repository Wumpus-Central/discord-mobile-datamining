...
export default function PortalKeyboardFooterIOS(animatedSheetIndex) {
  ...
  const obj2 = animatedSheetIndex(4504);
  class S {
    constructor() {
      tmp = closure_0;
      tmp2 = closure_2;
  ...
  obj = { interpolate: animatedSheetIndex(4373).interpolate, progress, bottom, followSystemKeyboard: flag, keyboardType: keyboardTypeSharedValue, KeyboardTypes: animatedSheetIndex(1609).KeyboardTypes, animatedSheetIndex, height, EXPRESSION_FOOTER_HEIGHT: animatedSheetIndex(1093).EXPRESSION_FOOTER_HEIGHT };
  S.__closure = obj;
  S.__workletHash = 9444646970651;
  S.__initData = keyboardTypeSharedValue;
  animatedStyle = animatedSheetIndex(4373).useAnimatedStyle(S);
  let items = [animatedStyle, tmp];
  const memo = bottom.useMemo(() => {
    const items = [keyboardStickyFooter.keyboardStickyFooter, animatedStyle];
    return items;
  }, items);
  obj = { style: memo, children: height(animatedSheetIndex(4509).PortalHost, { name: str }) };
  return height(flag(4373).View, obj);
};