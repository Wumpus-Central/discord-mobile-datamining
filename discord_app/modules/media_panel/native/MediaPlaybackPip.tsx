    ...
    if (null != contentMessage) {
      if (null != activeMediaPlayerSource) {
        if (isVoiceMessage) {
          ...
          if (contentMessage.attachments.length > 0) {
            ...
            if (null != activeMediaPlayerSource.attachmentIndex) {
              str2 = tmp4(8102)(contentMessage.attachments[activeMediaPlayerSource.attachmentIndex]);
            }
          }
        }
        ...
        if (memo) {
          ...
          obj3[2] = tmp14;
          const items3 = [tmp13(tmp2(16144).Marquee, obj3), ];
...
function PiPControls(message) {
  ...
  const items2 = [callback2(message(8557).BackgroundBlurFill, { blurAmount: 0.05 }), , ];
  const items3 = [, ];
  ({ pipButton: arr4[0], backButton: arr4[1] } = tmp);
  items2[1] = callback2(closure_6, { disabled: !visible, style: items3, onPress: callback, accessible: true, accessibilityRole: "button", accessibilityLabel: stringResult, children: callback2(message(5227).ArrowLargeLeftIcon, { size: "sm" }) });
  const obj1 = { disabled: !visible, style: items3, onPress: callback, accessible: true, accessibilityRole: "button", accessibilityLabel: stringResult, children: callback2(message(5227).ArrowLargeLeftIcon, { size: "sm" }) };
  const items4 = [, ];
