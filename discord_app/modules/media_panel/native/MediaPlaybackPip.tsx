...
function MediaInfo(message) {
  ...
  const token = message(4528).useToken(first(576).colors.BACKGROUND_SURFACE_HIGH);
  let obj = message(4528);
  ...
  if (null != message) {
    if (null != contentMessage) {
      if (null != activeMediaPlayerSource) {
        if (isVoiceMessage) {
          ...
          if (contentMessage.attachments.length > 0) {
            ...
            if (null != activeMediaPlayerSource.attachmentIndex) {
              str2 = tmp4(7706)(contentMessage.attachments[activeMediaPlayerSource.attachmentIndex]);
            }
          }
        }
        const obj3 = {
        ...
        const tmp14 = closure_15(tmp2(4825).Text, obj3);
        const obj4 = {
        ...
        if (memo) {
          ...
          const items3 = [closure_15(tmp2(17017).Marquee, obj7)];
          const obj8 = {
            start: { x: 0, y: 0 },
            end: { x: 1, y: 0 },
          ...
          items3[1] = closure_15(tmp4(5286), obj8);
        ...
        if (tmp13Result) {
          ...
          tmp13Result = closure_15(tmp2(4825).Text, obj9);
        }
...
function PiPControls(message) {
  ...
  let obj = message(4563);
  fn.__closure = { withTiming: message(4830).withTiming, visible };
  ...
  obj3.style = items1;
  const items2 = [closure_15(message(8362).BackgroundBlurFill, { blurAmount: 0.05 }), ,];
  const obj4 = {
    disabled: !visible,
    style: null,
    onPress: callback,
    ...
    children: closure_15(message(5935).ArrowLargeLeftIcon, { size: "sm" }),
  };
  const items3 = [,];
  ...
  const obj5 = {
    ...
    children: closure_15(message(4781).XLargeIcon, { size: "sm" }),
  };
  const items4 = [,];
  ...
  return closure_16(visible(4563).View, obj3);
}
...
const SquarePIPReferenceDimensions = fn(16880).SquarePIPReferenceDimensions;
const jsxProd = fn(21);
...
export default function MediaPlaybackPip() {
  ...
  const mediaPlayerManagerStore = obj2.useMediaPlayerManagerStore(
    progress(closePip[29]).useShallow((isPlaying) => ({
      isPlaying: isPlaying.isPlaying,
      progress: isPlaying.progress,
      activeMediaPlayerSource: isPlaying.activeMediaPlayerSource,
      mediaSourceMessage: isPlaying.mediaSourceMessage,
      closePip: isPlaying.closePip,
    })),
  );
  ...
  const effect1 = obj.useEffect(() => {
    ...
    return () => {
      ...
      const obj = {
        ...
        sender_user_id: null,
        type: null,
        is_voice_message: null,
        total_duration_secs: null,
        pip_playback_start_time_secs: null,
        pip_playback_end_time_secs: null,
        pip_opened_timestamp: null,
        pip_closed_timestamp: null,
      };
      let id;
      if (message != null) {
  ...
  const obj7 = {
    style: tmp.progressBar,
    size: 48,
    width: 2,
    prefill: num,
    easing: first.out(first.linear),
    duration: null,
    fill: null,
    rotation: 0,
    lineCap: "round",
