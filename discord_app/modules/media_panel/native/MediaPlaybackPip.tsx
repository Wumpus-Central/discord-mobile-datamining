...
function MediaInfo(message) {
  ...
  const token = message(4458).useToken(first(576).colors.BACKGROUND_SURFACE_HIGH);
  ...
  if (null != message) {
    if (null != contentMessage) {
      if (null != activeMediaPlayerSource) {
        ...
        const tmp14 = closure_15(tmp2(4753).Text, obj3);
        const obj4 = {
        ...
        if (memo) {
          ...
          const items3 = [closure_15(tmp2(17701).Marquee, obj7)];
          const obj8 = {
            start: { x: 0, y: 0 },
            end: { x: 1, y: 0 },
          ...
          items3[1] = closure_15(tmp4(5199), obj8);
        ...
        if (tmp13Result) {
          ...
          tmp13Result = closure_15(tmp2(4753).Text, obj9);
        }
...
function PiPControls(message) {
  ...
  let obj = message(4493);
  fn.__closure = { withTiming: message(4758).withTiming, visible };
  ...
  obj3.style = items1;
  const items2 = [closure_15(message(9189).BackgroundBlurFill, { blurAmount: 0.05 }), ,];
  const obj4 = {
    disabled: !visible,
    style: null,
    onPress: callback,
    ...
    children: closure_15(message(5847).ArrowLargeLeftIcon, { size: "sm" }),
  };
  const items3 = [,];
  ...
  const obj5 = {
    ...
    children: closure_15(message(4709).XLargeIcon, { size: "sm" }),
  };
  const items4 = [,];
  ...
  return closure_16(visible(4493).View, obj3);
}
...
const SquarePIPReferenceDimensions = fn(17565).SquarePIPReferenceDimensions;
const jsxProd = fn(21);
