...
fn(4560);
let createStyles = { container: { padding: 16 }, horizontal: { flexDirection: "row", alignItems: "center" }, spaceBetween: { justifyContent: "space-between" }, spaceBelow: { marginBottom: 8 }, progressBackground: null, progressForeground: null };
createStyles = { borderRadius: nativeDefault.radii.round, height: 8, backgroundColor: nativeDefault.colors.BACKGROUND_MOD_NORMAL };
createStyles.progressBackground = createStyles;
createStyles.progressForeground = { backgroundColor: nativeDefault.colors.STATUS_POSITIVE_BACKGROUND, borderRadius: nativeDefault.radii.round, height: 8 };
...
export const NewMemberActionsProgress = function NewMemberActionsProgress(guildId) {
  ...
  stateFromStores1 = obj1.useStateFromStores(items2, () => GuildOnboardingMemberActionStore.getCompletedActions(guildId));
  ...
  if (!tmp2Result.hasFlag(num2, GuildMemberFlags.COMPLETED_HOME_ACTIONS)) {
    ...
    if (0 !== num) {
      ...
      if (memo + num !== 0) {
        ...
        const items7 = [closure_10(tmp2(tmp3[15]).Text, obj3), closure_10(tmp2(tmp3[15]).Text, { variant: "text-xs/medium", color: "text-default", children: "/" }), , ];
