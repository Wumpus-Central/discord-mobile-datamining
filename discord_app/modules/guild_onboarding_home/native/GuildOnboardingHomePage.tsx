// === Module 16103: GuildOnboardingHomePage ===

// Module 16103 (GuildOnboardingHomePage)
import closure_3 from "noop" /* 19 */;
import closure_4 from "getHash" /* 4361 */;
import closure_5 from "handleSettingsLoadSuccess" /* 4675 */;
import { NO_SETTINGS } from "handleSettingsLoadSuccess" /* 4675 */;
...
function GuildOnboardingHomePage(guildId) {
  ...
  const tmp5 = stateFromStores(6038)(guildId);
  let tmp6 = !tmp5;
  if (!tmp5) {
    let num;
  ...
  const effect = React.useEffect(() => {
    if (stateFromStores === closure_1_6) {
      const guildHomeSettings = guildId(11818).fetchGuildHomeSettings(guildId);
      const obj2 = guildId(11818);
    } else if (null != tmp) {
      const obj = {};
  ...
  const effect1 = React.useEffect(() => {
    ...
    if (tmp) {
      const result = stateFromStores(5425).escapeToDefaultChannel(guildId);
      const obj = stateFromStores(5425);
    }
  }, items2);
  if (tmp6) {
    return null;
    ...
    obj[0] = guildId;
    obj[1] = tmp5;
    const items3 = [callback(tmp4(16105), obj), ];
    if (tmp5) {
      obj1 = { children: null };
      let obj2 = { guildId: null };
      obj2[0] = guildId;
      const items4 = [tmp12(tmp4(16110), obj2), , ];
      let obj3 = { guildId: null };
      obj3[0] = guildId;
      items4[1] = tmp12(tmp4(16114), obj3);
      tmp4 = tmp4(16115);
      ...
      obj5[0] = guildId;
      tmp12Result = tmp12(tmp4(16118), obj5);
    }
    items3[1] = tmp12Result;
    obj[2] = items3;
    closure_11(tmp4(16104), obj);
    const tmp4Result = tmp4(16104);
  }
}
