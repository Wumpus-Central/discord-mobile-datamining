...
export default function NewMemberActionsCompleted(arg0) {
  ...
  obj = { withDelay: sharedValue(4146).withDelay, withTiming: sharedValue(4286).withTiming, barWidth: sharedValue };
  fn.__closure = obj;
  fn.__workletHash = 7643178959760;
  fn.__initData = closure_8;
  const animatedStyle = obj1.useAnimatedStyle(fn);
  const effect1 = React.useEffect(() => {
    const timerId = setTimeout(() => callback2(4490).popWithKey(callback(11570).NEW_MEMBER_ACTION_COMPLETE_MODAL_KEY), 2500);
  }, []);
  ...
  items1[1] = callback(View, { style: tmp.progressBackground, children: callback(importDefault(4146).View, { style: items2 }) });
  obj[1] = items1;
