...
fn(4560);
let createStyles = { relativeContainer: { position: "relative" }, welcomeContainer: null, welcomeContent: null, avatarBackground: null, avatarBorder: null, avatar: null, adminUsernameContainer: null, adminUsername: null, message: null, icon: null };
createStyles = { marginHorizontal: 12, marginVertical: 16, borderRadius: nativeDefault.radii.sm, padding: 2, display: "flex", flexDirection: "column" };
createStyles.welcomeContainer = createStyles;
createStyles.welcomeContent = { borderRadius: nativeDefault.radii.sm, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOW, paddingHorizontal: 12, paddingBottom: 12, paddingTop: 4 };
...
createStyles.adminUsernameContainer = { display: "flex", flexDirection: "row", alignItems: "center", marginBottom: 4, paddingLeft: 44 };
let obj1 = { borderRadius: nativeDefault.radii.sm, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOW, paddingHorizontal: 12, paddingBottom: 12, paddingTop: 4 };
...
export default function HomeWelcomeMessage(guildId) {
  ...
  const stateFromStores1 = obj1.useStateFromStores(items1, () => GuildOnboardingHomeSettingsStore.getWelcomeMessage(guildId));
    ...
    if (null != stateFromStores) {
      if (null != stateFromStores2) {
        ...
        if (null != stateFromStores2.avatarDecoration) {
          ...
          const obj4 = { containerStyle: tmp.welcomeContainer, primaryColor, secondaryColor, fallbackBackground: items5, children: null };
          ...
          const obj7 = { style: tmp.adminUsername, variant: "text-md/semibold", children: tmp7(tmp3[20]).getName(guildId, null, stateFromStores2) };
          const items7 = [closure_8(tmp2(tmp3[19]).Text, obj7), ];
          let tmp17Result = null;
          if (tmp16) {
            const obj8 = { size: tmp2(tmp3[17]).Icon.Sizes.REFRESH_SMALL_16, style: tmp.icon, source: tmp7(tmp3[21]), disableColor: true };
