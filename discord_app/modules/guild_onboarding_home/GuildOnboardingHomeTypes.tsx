...
const result = require("obj132").fileFinishedImporting("modules/guild_onboarding_home/GuildOnboardingHomeTypes.tsx");
...
export const settingsFromServer = function settingsFromServer(body) {
  if (null == body) {
    ...
    const obj = { authorIds: null, message: null };
    ({ author_ids: obj2[0], message: obj2[1] } = welcome_message);
    obj[0] = obj;
    const found = new_member_actions.filter((item, index) => callback(1370).isNotNullish(store.getChannel(item.channel_id)));
    obj[1] = found.map(newMemberActionFromServer);
    const found1 = resource_channels.filter((item, index) => callback(1370).isNotNullish(store.getChannel(item.channel_id)));
...
export const settingsToServer = function settingsToServer(arg0, enabled) {
  if (null == enabled) {
    ...
    const found = newMemberActions.filter((item, index) => callback(1370).isNotNullish(store.getChannel(item.channelId)));
    obj[2] = found.map((item, index) => {
      let obj = { channel_id: item.channelId, action_type: item.actionType, title: item.title, description: item.description, emoji: null, icon: null };
    ...
    const found1 = resourceChannels.filter((item, index) => callback(1370).isNotNullish(store.getChannel(item.channelId)));
    obj[3] = found1.map((item, index) => {
      let obj = { channel_id: item.channelId, title: item.title, description: item.description, emoji: null, icon: null };
      const emoji = item.emoji;
      let id;
      if (emoji != null) {
        id = emoji.id;
