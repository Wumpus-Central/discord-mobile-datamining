...
const memoResult = importAllResult.memo(function VoicePanelSystemUIManager() {
  ...
  const memo = windowDimensions.useMemo(() => {
    closure_0 = channelId(mode[10])(function updateState(arg0) {
      ...
      lib(mode[8]).batchUpdates(() => {
        ...
        let participant;
        if (null != c0) {
          participant = closure_2_5.getParticipant(c1, c0);
        }
        let type;
        if (participant != null) {
          type = participant.type;
        }
        const tmp11 = _undefined(mode[9]).isIOS() && type === closure_2_8.ACTIVITY;
        closure_1_8(tmp11);
        let tmp14 = !tmp4;
        if (!tmp4) {
          tmp14 = !tmp11;
  ...
  obj1[0] = items2;
  return callback(handleStateChange, obj1);
});
const result = require("obj132").fileFinishedImporting("modules/voice_panel/native/VoicePanelSystemUIManager.tsx");
