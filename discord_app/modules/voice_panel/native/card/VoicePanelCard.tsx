...
function SelfStreamCard(sharedCoords) {
  ...
  if (isFocused) {
    ...
    const obj3 = { style: tmp.selfStreamFocusedSubtitle, variant: "text-sm/medium", color: "text-overlay-light", children: null };
...
function SpeakingIndicator(id) {
  ...
  __closure = { mode, VoicePanelModes, focused, id, withSpring: id(speaking[37]).withSpring, computeCardBorderRadius: isSelf(speaking[41]), isSelf, defaultBorderRadius: token, SPEAKING_PHYSICS };
  ...
  __closure = { mode, VoicePanelModes, focused, id, withSpring: id(speaking[37]).withSpring, computeCardBorderRadius: isSelf(speaking[41]), isSelf, defaultBorderRadius: token, SPEAKING_PHYSICS, speaking, roundToNearestPixel: isSelf(speaking[42]), SPEAKING_BORDER_SIZE: 3, SPEAKING_INSET: 2 };
  ...
  obj1 = { mode, VoicePanelModes, focused, id, withSpring: id(speaking[37]).withSpring, computeCardBorderRadius: isSelf(speaking[41]), isSelf, defaultBorderRadius: token, SPEAKING_PHYSICS, speaking, SPEAKING_BORDER_SIZE: 3 };
  fn3.__closure = obj1;
  ...
  class A {
    constructor() {
      ...
      if (closure_31) {
        ...
        obj2 = { sharedVisible: null, sharedTransitionState: null, TransitionStates: null, runOnJS: null, cleanUp: null };
...
let c28 = 0.75;
fn(4636);
__closure = { positionWrapper: null, userRoundedCard: null, nonUserRoundedCard: null, blackBackground: null, selfStreamFocusedSubtitle: null, avatarImageMaskStyles: null, avatarPlaceholder: null, image: null, speakingIndicatorWrapper: null, speakingIndicatorUnderlay: null, speakingIndicatorGreenBar: null };
...
let closure_34 = noop.memo((id) => {
  ...
  const stateFromStoresObject = __closure.useStateFromStoresObject(items, () => ({ stream: ApplicationStreamingStore.getStreamForUser(userId, streamGuildId), activeStream: ApplicationStreamingStore.getActiveStreamForUser(userId, streamGuildId) }), items1);
  ...
  if (isSelf) {
    ...
    if (null == tmp11Result) {
      if (activeStream.state !== ApplicationStreamStates.FAILED) {
        if (activeStream.state === ApplicationStreamStates.ENDED) {
          ...
          const obj6 = { layout, id, streamId: null, userId: null, streamKey: null, isScrollVisible: null, videoSpinnerContext: null, sharedCoords: null, isCamera: false, paused: null };
    ...
    const obj8 = { avError: tmp11Result, stream: activeStream, removeSplashImage: !tmp8, type: tmp4(streamGuildId[30]).VideoEmptyTypes.STREAM_FAILED, style: stream.absoluteFill };
...
let closure_37 = noop.memo(function AnimatedUserCardInner(isRinging) {
  ...
  if (null != tmp6) {
    const obj2 = { colors: tmp6, start: { x: 0, y: 0 }, end: { x: 0, y: 1 }, style: StyleSheet.absoluteFill, layout, pointerEvents: "none" };
...
function layoutTransitionFunction(originX, SUBTLE_SPRING) {
  ...
  const size = { originX: obj.withSpring(originX.targetOriginX, SUBTLE_SPRING, str2), originY: null, width: null, height: null };
...
export default noop.memo(function VoicePanelCard(cleanUp) {
  ...
  const memo = __closure.useMemo(() => {
    __closure = { mass: closure_11.mass, damping: windowDimensions(id2[55])(closure_11.damping - 2, closure_11.damping + 2), stiffness: windowDimensions(id2[55])(closure_11.stiffness - 20, closure_11.stiffness + 20) };
  ...
  if (item.type === constants2.CTA) {
    ...
    __closure = { cleanUp: cleanUp.cleanUp, coords: cardLayoutCoordsSubscription, id, isRTCConnected: stateFromStores, isScrollVisible: derivedValue, layoutPhysics: physics, transitionState, sharedVisible: sharedValue, children: null };
    ...
    if (ParticipantTypes.USER === type2) {
        ...
        if (stateFromStores) {
          if (tmp4.canRenderVideo) {
            if (null == tmpResultResult) {
              ...
              const obj3 = { id: id3, userId: user.id, streamId: null, isScrollVisible: null, videoSpinnerContext: null, sharedCoords: null, isCamera: true, focusOnReady: null, layout: null };
              if (streamId == null) {
                streamId = null;
              }
      ...
      const obj5 = { isRinging: tmp8, avatarURI: user.getAvatarURL(guildId, 80, false), avatarDecoration: null, layout: null, layoutPhysics: null, userId: null, guildId: null };
      ...
      const obj6 = { userId: tmp4.user.id, id: null, streamGuildId: null, streamId: null, userNick: null, isSelf: null, sharedCoords: null, isScrollVisible: null, layout: null };
  ...
  const obj8 = { isRinging: tmp8, avatarURI: "r", avatarDecoration: "call", layout: layoutTransition, layoutPhysics: physics };
