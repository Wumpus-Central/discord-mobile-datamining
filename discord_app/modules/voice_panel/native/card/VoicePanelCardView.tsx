...
function CardContentFreezer(children) {
  ...
  const fn = function o() {
      ...
      if (closure_1_6.get() !== VoicePanelModes.PIP) {
        ...
        if (animatedStyle.get().mode !== VoicePanelControlsModes.HIDDEN) {
          ...
          if (null == ChannelRTCStore.get()) {
            ...
            const diff1 = diff - animatedStyle.get().height - bound;
            const diff2 = absoluteFill.get().height - paddingTop - bound;
            const diff3 = absoluteFill.get().height - height;
            const diff4 = diff3 - animatedStyle.get().height - bound;
            let result = num;
    ...
    if (!closure_1_8.get().gestureActive) {
      if (closure_1_6.get() !== VoicePanelModes.PIP) {
        let tmp27 = animatedStyle;
      }
      const obj4 = { translateY: obj5.withSpring(sum1, tmp27) };
      const items = [obj4];
      const obj6 = { scale: spring.withSpring(num3, animatedStyle) };
      items[1] = obj6;
  ...
  fn.__closure = {
    ...
    safeArea,
    EDGE_GUTTER,
    calculateVoicePanelHeaderSpecs: require("calculateVoicePanelHeaderSpecs"),
    edgeGutter: token,
    connected,
    contentDimensions,
    windowDimensions,
    ...
    withSpring: children(animatedStyle[18]).withSpring,
    wrapperOffset,
    LAYOUT_PHYSICS,
    SCALE_PHYSICS,
    freeze: tmp6,
...
const isUserParticipant = fn(4778).isUserParticipant;
const jsx = fn(21).jsx;
let SCALE_PHYSICS = {};
