...
import _modDef17721 from "../../../../../_runtime/metro/17721__.js";
import _modDef17722 from "../../../../../_runtime/metro/17722__.js";
...
function StreamIcon(voicePlatform) {
  ...
  const animatedStyle = controlsSpecs(4561).useAnimatedStyle(fn);
  let tmp2Result = _modDef17676;
  if (voicePlatform === constants2.XBOX) {
    tmp2Result = _modDef17721;
  } else if (voicePlatform === constants2.MOBILE) {
    tmp2Result = _modDef10411;
  } else if (voicePlatform === constants2.QUEST) {
    tmp2Result = _modDef17722;
  }
...
const VOICE_PANEL_CARD_INNER_PADDING = fn(12618).VOICE_PANEL_CARD_INNER_PADDING;
...
let closure_22 = createStyles.createStyles(() => {
  const obj = {
    ...
    initialIcons: { display: "flex", flexDirection: "row", marginStart: -4, marginEnd: -4 },
    icon: null,
...
let closure_27 = noop.memo((participant) => {
  ...
  guildId = noop.useContext(guildId(12614)).guildId;
  ...
  let obj = participant(4561);
  fn.__closure = {
    controlsHidden,
    FLOATING_BAR_HEIGHT: v28,
    VOICE_PANEL_CARD_INNER_PADDING,
    withSpring: participant(5272).withSpring,
    MODE_CHANGE_PHYSICS,
  };
  ...
  obj3.icon = guildId(17723);
  obj3.onPress = callback;
  obj3.style = animatedStyle;
  obj3.layout = layout;
  const tmp4 = guildId(17602);
  const result = participant(17672).isStableActivityParticipant(participant);
...
let closure_37 = noop.memo((controlsHidden) => {
  ...
  const stateFromStores = controlsHidden(sharedValue1[36]).useStateFromStores(
    items,
  ...
  obj10.children = closure_16(controlsHidden(sharedValue1[13]).Icon, {
    source: tmp7(sharedValue1[39]),
    size: controlsHidden(sharedValue1[13]).IconSizes.SMALL_20,
    style: tmp.ringingIcon,
  });
...
let closure_39 = noop.memo((arg0) => {
  ...
  noop = undefined;
  const showGameTag = gameRecord(17726).useConfig({ location: "VoicePanelCardFloatingControls" }).showGameTag;
  const first = gameRecord(10083)(userId, arg0.guildId, showGameTag)[0];
  let tmp5;
  let obj = gameRecord(17726);
  if (showGameTag) {
    let application_id;
    if (first != null) {
  ...
  gameRecord = gameRecord(9029)({ applicationId: tmp5 }).gameRecord;
  const tmp8 = gameRecord(5415)(gameRecord);
  dependencyMap = tmp8;
  ...
  if (showGameTag) {
    if (null != gameRecord) {
      if (!tmp8) {
        ...
        let tmp12 = closure_16(tmp(17727), obj2);
      }
      return tmp12;
    }
  }
  tmp12 = closure_16(tmp(10097), { userId, textColor });
  const tmp4 = gameRecord(9029);
});
const __initData8 = {
...
let closure_45 = noop.memo((controlsHidden) => {
  ...
  let isUserSecureFramesVerified = controlsHidden(controlsSpecs[52]).useIsUserSecureFramesVerified({
    userId: id,
    channelId,
  });
  ...
  fn3.__closure = {
    ...
    withSpring: controlsHidden(controlsSpecs[28]).withSpring,
    MODE_CHANGE_PHYSICS,
  };
  ...
  Z.__closure = {
    ...
    hasIcon: tmpResultResult.length >= 1,
    GAP: 4,
  };
  ...
  if (tmp20Result) {
    ...
    tmp20Result = closure_16(tmp(tmp2[26]), obj10);
  ...
  if (tmp20Result2) {
    ...
    tmp20Result2 = closure_16(closure_39, obj17);
  }
  ...
  if (isUserSecureFramesVerified) {
    ...
    isUserSecureFramesVerified = closure_16(tmp4(tmp2[56]).ShieldLockIcon, obj18);
  }
  items6[2] = isUserSecureFramesVerified;
  obj11.children = items6;
  items3[1] = closure_17(channelId(controlsSpecs[38]), obj11);
  obj9.children = items3;
  obj8.children = closure_17(closure_20, obj9);
...
export default noop.memo(function FloatingControls(arg0) {
  ...
  if (stateFromStores) {
    const obj7 = {
      ...
      targetName: label,
      isActivityParticipant: participant(tmp2[29]).isStableActivityParticipant(participant),
      layout,
    };
    stateFromStores = closure_16(closure_29, obj7);
    const tmp4Result4 = participant(tmp2[29]);
  }
