...
import trackActivityThermalStateNoticeShown from "trackActivityThermalStateNoticeShown" /* 17053 */;
import VoicePanelFloatingCTAUtils from "VoicePanelFloatingCTAUtils" /* 17054 */;
import useIsVoicePanelParticipantFocusable from "useIsVoicePanelParticipantFocusable" /* 17081 */;
import VoicePanelPIPStateContext from "VoicePanelPIPStateContext" /* 17092 */;
...
export default function VoicePanelController(channelId) {
  ...
  obj1 = { drawerHeight: size.height, drawerWidth: maxPanelWidth, drawerX: null, drawerY: null, pipX: -1, pipY: -1, animated: true, mode: null };
  ...
  first2 = tmp5(obj1.useState(() => {
    new VoicePanelCardLayoutManagerDefault(channelId);
    const obj = { windowWidth: type.width, windowHeight: type.height, safeAreaLeft: sharedValue.left, safeAreaRight: sharedValue.right, safeAreaTop: sharedValue.top, safeAreaBottom: sharedValue.bottom, controlBarSize: first ? closure_2_19 : collapsedCategories };
  ...
  if (stateFromStores) {
    ...
    let obj5 = { VoicePanelModes: tmp14, runOnJS: tmp(tmp2[22]).runOnJS, _queueHideControls: callback2, _clearHideControlsQueue: callback1 };
    fn3.__closure = obj5;
    ...
    const obj7 = { updateSharedValueIfChanged: tmp8(tmp2[39]), controlsSpecs: tmp35Result, pushToTalk: stateFromStores, CONTROLS_HEIGHT_PTT: sharedValue11, CONTROLS_HEIGHT: null };
    ...
    const stateFromStores1 = tmp(tmp2[26]).useStateFromStores(items15, () => ChannelRTCStore.getSelectedParticipantId(guildId));
    ...
    const obj8 = { timeout: -1, layoutKey: managerSubscription, connected: isConnected, windowState: first4, safeAreaState: first5, contentDimensions: { width: 0, height: 0 } };
    ...
    const obj9 = { windowWidth: null, windowHeight: null, safeAreaLeft: null, safeAreaRight: null, safeAreaTop: null, safeAreaBottom: null, controlBarSize: null };
    ...
    const layoutEffect8 = obj1.useLayoutEffect(() => {
      ...
      let obj = { isConnected: first1, cheapWorkletShallowEqual: channelId(contentDimensions[23]).cheapWorkletShallowEqual, contentDimensions, windowDimensions: executeLayoutManagerEffect, safeArea: guildId, runOnJS: channelId(contentDimensions[22]).runOnJS, executeLayoutManagerEffect };
      fn.__closure = obj;
    ...
    const obj11 = { cheapWorkletArrayShallowEqual: tmp(tmp2[23]).cheapWorkletArrayShallowEqual, VoicePanelControlsModes: tmp36, VoicePanelModes: tmp14, runOnJS: tmp(tmp2[22]).runOnJS, dismissKeyboard: tmp(tmp2[55]).dismissKeyboard };
    fn6.__closure = obj11;
    ...
    const obj13 = { cheapWorkletArrayShallowEqual: tmp(tmp2[23]).cheapWorkletArrayShallowEqual, TransitionStates: tmp(tmp2[51]).TransitionStates, VoicePanelModes: tmp14, runOnJS: tmp(tmp2[22]).runOnJS, setMode: tmp5Result[1] };
    fn8.__closure = obj13;
    ...
    const obj14 = { channelId, connected: isConnected, focusedId: stateFromStores1, layoutManager: first2, mode: first3 };
    const controllerPIPState = tmp(tmp2[60]).useControllerPIPState(obj14);
    const obj15 = { mode: sharedValue1, controlsSpecs: tmp35Result, safeArea: sharedValue3, windowDimensions: sharedValue2 };
      ...
      if (tmp3) {
        if (result) {
          if (!result1) {
            ...
            if (tmp15) {
              const obj = { key: "EMBEDDED_ACTIVITIES_VIDEO_DISABLED_FOR_THERMAL_STATE", icon: _modDef17052, content: null, disableAnimations: true, toastDurationMs: 3000 };
    ...
    const obj17 = { cheapWorkletArrayShallowEqual: tmp(tmp2[23]).cheapWorkletArrayShallowEqual, runOnJS: tmp(tmp2[22]).runOnJS, handleStateUpdates: callback8 };
    ...
    const obj19 = { cheapWorkletArrayShallowEqual: tmp(tmp2[23]).cheapWorkletArrayShallowEqual, runOnJS: tmp(tmp2[22]).runOnJS, handleAnimatedReaction: callback9 };
