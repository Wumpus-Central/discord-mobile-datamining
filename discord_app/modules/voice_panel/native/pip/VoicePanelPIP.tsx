...
function VoicePanelPIP() {
  ...
  obj = { pipState: pIPState1, getVoicePanelPIPBorderRadius: require("VoicePanelPIPUtils").getVoicePanelPIPBorderRadius };
  fn.__closure = obj;
  ...
  const memo1 = stateFromStores2.useMemo(() => {
    ...
    const obj1 = { getTouchesCentroid, initialGestureOffset, updateSharedValueIfChanged: updateSharedValueIfChangedDefault, State: LegacyBaseButton.State, MIN_GESTURE_START: 10, wrapperOffset, runOnJS: ReanimatedRexport.runOnJS, triggerIOSHaptic: utils_triggerIOSHapticDefault };
    const fn4 = function o(arg0) {
      ({ velocityX, velocityY, absoluteX, absoluteY } = arg0);
      closure_0(first[16]);
      const obj = { velocityX, velocityY, absoluteX, absoluteY, windowDimensions: windowDimensions.get(), safeArea: stateFromStores2.get() };
  ...
  const memo2 = stateFromStores2.useMemo(() => {
    ...
    const onStartResult = Gesture2.Tap().enabled(pipMode !== VoicePanelPIPModes.IN_APP).maxDistance(30).onStart(fn);
    const Gesture3 = LegacyBaseButton.Gesture;
    const numberOfTapsResult = Gesture2.Tap().enabled(pipMode !== VoicePanelPIPModes.IN_APP).maxDistance(30).onStart(fn).numberOfTaps(2);
    ...
    obj = { pipMode, VoicePanelPIPModes, runOnJS: ReanimatedRexport.runOnJS, setMode, VoicePanelModes, controlsSpecs: pIPState, VoicePanelControlsModes, showControls, hideControls };
    fn2.__closure = obj;
  ...
  if (null != pIPState.mode || hideControls) {
    tmp38Result1 = null;
    if (pushToTalk) {
      const obj12 = { pointerEvents: "box-none", style: memo6, layout: tmp5(tmp3[16]).layoutTransition, children: closure_16(tmp2(tmp3[32]), {}) };
...
let closure_35 = noop.memo((transitionState) => {
  ...
  obj = { transitionState, TransitionStates: transitionState(4284).TransitionStates, panelCardStillInPIP };
  I.__closure = obj;
  ...
  obj = { runOnJS: transitionState(4310).runOnJS, fireCleanup };
  S.__closure = obj;
  S.__workletHash = 5494714290933;
  S.__initData = __initData5;
  const animatedReaction = transitionState(4310).useAnimatedReaction(I, S);
  let tmp10 = null;
  if (tmp5) {
