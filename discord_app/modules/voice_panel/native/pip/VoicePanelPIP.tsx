...
import VoicePanelPIPScaleCache from "VoicePanelPIPScaleCache.tsx";
...
function VoicePanelPIP() {
  ...
  let fn = function z(baseX, velocityX, velocityY) {
    ...
    const scaledPIPContainerHeight = VoicePanelPIPUtils.getScaledPIPContainerHeight({
      height: pipState.height,
      containerHeight: pipState.containerHeight,
    ...
    const result1 = obj3.calculatePIPPositionFromVelocity({
      velocityX,
      velocityY,
      absoluteX: baseX.baseX + baseX.offsetX + result / 2,
      absoluteY: baseX.baseY + baseX.offsetY + scaledPIPContainerHeight / 2,
  ...
  fn.__closure = {
    pipState: pIPState1,
    getScaledPIPContainerHeight: setMode(stateFromStores2[16]).getScaledPIPContainerHeight,
    calculatePIPPositionFromVelocity: setMode(stateFromStores2[16]).calculatePIPPositionFromVelocity,
    windowDimensions,
    safeArea,
    updateSharedValueIfChanged: pIPState(stateFromStores2[17]),
    wrapperDimensions,
  };
  ...
  const items1 = [, , , , , , ,];
  ({
    containerHeight: arr2[0],
    height: arr2[1],
    scale: arr2[2],
    showSecondaryPIP: arr2[3],
    width: arr2[4],
  } = pIPState1);
  items1[5] = safeArea;
  ...
  class K {
    constructor() {
      ...
      obj1 = {
        height: closure_1_14.height,
        containerHeight: closure_1_14.containerHeight,
      ...
      size1 = {
        width: result,
        height: scaledPIPContainerHeight,
        opacity: closure_1_16.get(),
        transform: null,
        borderRadius: null,
      };
  ...
  K.__closure = {
    pipState: pIPState1,
    getScaledPIPContainerHeight: setMode(stateFromStores2[16]).getScaledPIPContainerHeight,
    mainTileInLayout: tmp12,
    gestureState: sharedValue,
    getClampedPIPPosition: setMode(stateFromStores2[16]).getClampedPIPPosition,
    wrapperDimensions,
    windowDimensions,
    safeArea,
    pipAvoidanceSpecs,
    DRAWER_SPRING_PHYSICS,
    PIP_LAYOUT_PHYSICS: setMode(stateFromStores2[16]).PIP_LAYOUT_PHYSICS,
    opacity: sharedValue1,
    withSpring: setMode(stateFromStores2[18]).withSpring,
    getVoicePanelPIPBorderRadius: setMode(stateFromStores2[16]).getVoicePanelPIPBorderRadius,
  };
  ...
  ({
    containerHeight: arr3[2],
    height: arr3[3],
    scale: arr3[4],
    showSecondaryPIP: arr3[5],
    width: arr3[6],
  } = pIPState1);
  items2[7] = safeArea;
  items2[8] = callback;
  ...
  const memo = obj.useMemo(() => {
    ...
    const fn = function c(allTouches) {
      ...
      if (gestureState.get().pressed) {
        ...
        obj5.spread = tmp2;
        pIPState(stateFromStores2[17])(gestureState, obj5);
      } else {
        const obj6 = {};
        ...
        obj6.spread = tmp2;
        const result = gestureState.set(obj6);
      }
    };
      ...
      if (value.active) {
        ...
        if (value.spread > 8) {
          ...
          if (8 < tmp) {
            ...
            ({
              width: obj10.width,
              containerHeight: obj10.containerHeight,
              showSecondaryPIP: obj10.showSecondaryPIP,
            } = styles);
        ...
        obj3.spread = tmp;
        const result2 = gestureState.set(obj3);
        ...
        const size = {
          pipX: wrapperDimensions.get().pipX,
          pipY: wrapperDimensions.get().pipY,
          width: styles.width * value4,
          height: null,
          windowDimensions: null,
          safeArea: null,
          bottomAvoidanceRegion: null,
        ...
        const tmp3Result = setMode(stateFromStores2[16]);
        ...
        obj4.scale = value4;
        size.height = setMode(stateFromStores2[16]).getScaledPIPContainerHeight(obj4);
        ...
        pIPState(stateFromStores2[17])(wrapperOffset, { gestureActive: true, x: 0, y: 0 });
        activate.activate();
        const tmp3Result3 = setMode(stateFromStores2[16]);
        setMode(stateFromStores2[15]).runOnJS(pIPState(stateFromStores2[20]))();
        const tmp3Result4 = setMode(stateFromStores2[15]);
      }
    };
    ...
    fn3.__closure = {
      ...
      MIN_PINCH_SPAN: 8,
      pipState,
      clampPIPScale: VoicePanelPIPUtils.clampPIPScale,
      windowDimensions,
      safeArea,
      pipAvoidanceSpecs,
      State: LegacyBaseButton.State,
      MIN_GESTURE_START: 10,
      getClampedPIPPosition: VoicePanelPIPUtils.getClampedPIPPosition,
      wrapperDimensions,
      getScaledPIPContainerHeight: VoicePanelPIPUtils.getScaledPIPContainerHeight,
      updateSharedValueIfChanged: updateSharedValueIfChangedDefault,
      wrapperOffset,
    ...
    fn4.__closure = {
      ...
      setVoicePanelPIPScaleCached: VoicePanelPIPScaleCache.setVoicePanelPIPScaleCached,
      pipState,
    };
  ...
  const memo1 = obj.useMemo(() => {
    ...
    const maxDistanceResult = Gesture2.Tap()
      .enabled(setMode !== VoicePanelPIPModes.IN_APP)
      .maxDistance(30);
    ...
    fn2.__closure = {
      ...
      setMode,
      VoicePanelModes,
      controlsSpecs: stateFromStores2,
      VoicePanelControlsModes,
      showControls,
      hideControls: callback2,
    };
  ...
  if (pushToTalk) {
    pushToTalk = pIPState.mode !== VoicePanelPIPModes.IN_PANEL || tmp35;
    const tmp36 = pIPState.mode !== VoicePanelPIPModes.IN_PANEL || tmp35;
  }
  ...
  let fn4 = function h(originX) {
    ...
    const size = {
      originX: spring.withSpring(originX.targetOriginX, VoicePanelPIPUtils.PIP_LAYOUT_PHYSICS),
      originY: null,
      width: null,
      height: null,
    };
  ...
  if (tmp12) {
    ...
    obj16.children = gestureState(tmp2(tmp3[30]), obj17);
    obj15.children = gestureState(tmp2(tmp3[29]), obj16);
    obj14.children = gestureState(tmp5(tmp3[19]).GestureDetector, obj15);
    tmp41Result = gestureState(tmp2Result9, obj14);
  ...
  if (pIPState.showSecondaryPIP) {
    ...
    obj21.children = gestureState(tmp2(tmp3[32]), {});
    obj20.children = gestureState(tmp2(tmp3[31]), obj21);
    obj19.children = gestureState(tmp5(tmp3[19]).GestureDetector, obj20);
    obj18.children = gestureState(tmp2Result12, obj19);
    tmp41Result3 = gestureState(tmp2Result11, obj18);
  ...
  if (tmp13) {
    ...
    if (pushToTalk) {
      ...
      obj22.children = gestureState(tmp2(tmp3[33]), {});
      tmp41Result4 = gestureState(tmp2(tmp3[29]), obj22);
...
getTouchesSpread.__initData = {
  code: "function getTouchesSpread_VoicePanelPIPTsx2(touches,centroid){if(touches.length<2)return 0;let total=0;for(const touch of touches){const dx=touch.absoluteX-centroid.x;const dy=touch.absoluteY-centroid.y;total+=Math.sqrt(dx*dx+dy*dy);}return total/touches.length;}",
};
let closure_20 = {
  pressed: false,
  active: false,
...
let closure_37 = noop.memo((transitionState) => {
  ...
  const effect = noop.useEffect(() => {
    ...
    tmp = transitionState === native.TransitionStates.YEETED && pIPPanelLayoutCommitted;
  }, items);
