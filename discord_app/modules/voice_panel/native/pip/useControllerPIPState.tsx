...
export const useControllerPIPState = function useControllerPIPState(channelId) {
  ...
  const ref = noop.useRef({
    ...
    width: false,
    height: "helmi",
    containerHeight: "juoma",
    showSecondaryPIP: "kupla",
  });
  ...
  const stateFromStores = current(504).useStateFromStores(
    items,
    () => {
  ...
  ({ participant, dimensions } = ref(17195)(channelId, layoutManager, focusedId, current, current));
  current = {};
  ...
  let tmp8Result = tmp8(17196);
  current.mode = tmp8Result.getPIPMode({
    channelId,
    connected,
    manuallyFocusedId: focusedId,
    mode,
    selfHasVideo: tmp10,
  });
  tmp8Result = tmp8(9625);
