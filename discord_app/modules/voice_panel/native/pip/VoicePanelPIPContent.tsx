...
function User(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9637).useSurfaceDirectRendererExperiment(id1, {
  ...
  const tmp3Result1 = participantId(9637);
  class S {
    constructor() {
      value = focused.get();
      id = undefined;
  ...
  obj = { runOnJS: tmp3(4310).runOnJS, updateIsActivityFocused, mode };
  fn.__closure = obj;
  fn.__workletHash = 5971237403457;
  fn.__initData = __initData4;
  const animatedReaction = participantId(4310).useAnimatedReaction(S, fn);
  const tmp3Result2 = participantId(4310);
  ...
  obj = { runOnJS: tmp3(4310).runOnJS, updateIsActivityFocused, focused };
  I.__closure = obj;
  I.__workletHash = 12552370107483;
  I.__initData = __initData6;
  const animatedReaction1 = participantId(4310).useAnimatedReaction(fn2, I);
  const tmp3Result3 = participantId(4310);
  const items4 = [c9];
  ...
  if (canRenderParticipantVideo) {
    ...
    if (null != tmp6ResultResult) {
      const obj4 = {
        ...
        layout: tmp3(17121).layoutTransition,
        pointerEvents: "none",
      };
    ...
    if (null != user) {
      const obj6 = {
        source: tmp3(8860).getCachedSourceFromURI(user.getAvatarURL(guildId, 80, false)),
        size: null,
      ...
      const tmp3Result6 = tmp3(8860);
    }
    obj5.children = tmp35Result;
    items10[1] = closure_22(tmp6(5603), obj5);
    ...
    if (tmp18) {
      ...
      if (consumedRequestToRespondToSeriousThermalState) {
        ...
        const obj9 = { style: tmp.thermalAlertIcon, source: tmp6(9657), color: tmp.thermalAlertIcon.color };
        obj8.children = closure_22(tmp3(1178).Icon, obj9);
        tmp35Result = closure_22(tmp6(5603), obj8);
        const tmp6Result2 = tmp6(5603);
      }
    }
    items10[3] = tmp35Result;
    obj1.children = items10;
    tmp42 = obj1;
    const tmp6Result1 = tmp6(5603);
  }
...
function Stream(participantId) {
  ...
  const context = obj1.useContext(mode(12302));
  ...
  if (stateFromStores === constants.ENDED) {
    ...
    let tmp18Result = closure_22(tmp2(9630).StreamEnded, obj);
  } else if (stateFromStores === tmp17.FAILED) {
    obj = { style: tmp.streamEmptyImage, resizeMode: "contain" };
    tmp18Result = closure_22(tmp2(9633).StreamFailed, obj);
  } else if (tmp15) {
