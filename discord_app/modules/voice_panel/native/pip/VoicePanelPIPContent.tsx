...
function User(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9850).useSurfaceDirectRendererExperiment(id1, {
  ...
  if (canRenderParticipantVideo) {
    ...
    if (null != user1) {
      const obj9 = {
        source: tmp3(9067).getCachedSourceFromURI(user1.getAvatarURL(guildId, 80, false)),
        size: null,
    ...
    if (tmp18) {
      ...
      if (consumedRequestToRespondToSeriousThermalState) {
        ...
        tmp35Result2 = closure_22(tmp6(5759), obj11);
...
function Stream(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9850).useSurfaceDirectRendererExperiment(
