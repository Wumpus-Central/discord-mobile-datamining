...
function ActivityInVoice(participantId) {
  ...
  const tmp7 = windowDimensions(
    first.useState(transitionState === participantId(items10[24]).TransitionStates.MOUNTED),
    2,
  );
  first = tmp7[0];
  closure_5 = tmp7[1];
  ...
  const stateFromStores = participantId(items10[28]).useStateFromStores(
    items2,
    () => {
  ...
  stateFromStores1 = participantId(items10[28]).useStateFromStores(
    items4,
  ...
  let obj2 = { runOnJS: participantId(items10[18]).runOnJS, handleTargetAspectRatioParams };
}
function User(participantId) {
  ...
  let tmp5 = transitionState === participantId(4533).TransitionStates.YEETED ? tmp.onTop : tmp.onBottom;
  dependencyMap = tmp5;
  ...
  dominantColorFromImage = participantId(9183).useDominantColorFromImage(avatarURL);
  let id;
  const tmp3Result8 = participantId(9183);
  if (user1 != null) {
    id = user1.id;
  }
  const tmp6ResultResult = useProfileTileGradientDefault({
    userId: id,
    guildId,
  ...
  const surfaceDirectRendererExperiment = participantId(9771).useSurfaceDirectRendererExperiment(id1, {
  ...
  const tmp3Result9 = participantId(9771);
  class I {
    constructor() {
      value = focused.get();
      id = undefined;
  ...
  const tmp3Result10 = participantId(4559);
  S.__closure = { runOnJS: participantId(4559).runOnJS, updateIsActivityFocused, mode };
  S.__workletHash = 5971237403457;
  S.__initData = __initData4;
  const animatedReaction = tmp3Result10.useAnimatedReaction(I, S);
  const obj2 = { runOnJS: participantId(4559).runOnJS, updateIsActivityFocused, mode };
  const fn = function v() {
    return mode.get();
  };
  fn.__closure = { mode };
  ...
  const tmp3Result11 = participantId(4559);
  fn2.__closure = { runOnJS: participantId(4559).runOnJS, updateIsActivityFocused, focused };
  fn2.__workletHash = 12552370107483;
  fn2.__initData = __initData6;
  const animatedReaction1 = tmp3Result11.useAnimatedReaction(fn, fn2);
  const obj3 = { runOnJS: participantId(4559).runOnJS, updateIsActivityFocused, focused };
  const items4 = [c9];
  const stateFromStoresObject = participantId(563).useStateFromStoresObject(items4, () => ({
  ...
  canRenderParticipantVideo = participantId(9789).useCanRenderParticipantVideo(stateFromStores);
  ...
  const tmp3Result13 = participantId(9789);
  if (canRenderParticipantVideo) {
    const items7 = [,];
    ...
    if (null != tmp6ResultResult) {
      const obj7 = {
        ...
        layout: participantId.layoutTransition,
        pointerEvents: "none",
      };
    ...
    if (null != user1) {
      const obj9 = {
        source: tmp3(9183).getCachedSourceFromURI(user1.getAvatarURL(guildId, 80, false)),
        size: null,
      ...
      tmp35Result = closure_22(tmp3(1177).Avatar, obj9);
      const tmp3Result14 = tmp3(9183);
    }
    obj8.children = tmp35Result;
    items10[1] = closure_22(tmp6(5892), obj8);
    ...
    if (tmp18) {
      ...
      if (consumedRequestToRespondToSeriousThermalState) {
        ...
        const obj12 = { style: tmp.thermalAlertIcon, source: tmp6(9792), color: tmp.thermalAlertIcon.color };
        obj11.children = closure_22(tmp3(1177).Icon, obj12);
        tmp35Result2 = closure_22(tmp6(5892), obj11);
        const tmp6Result6 = tmp6(5892);
      }
    }
    items10[3] = tmp35Result2;
    obj4.children = items10;
    tmp42 = obj4;
    const tmp6Result5 = tmp6(5892);
  }
  return closure_23(NativeViewDefault, tmp42);
}
...
function Stream(participantId) {
  ...
  let callback2;
  let tmp = closure_24();
  const mode = participantId(17636).usePIPState().mode;
  closure_129_0 = transitionState;
  closure_129_1 = transitionCleanUp;
  const items = [transitionState, transitionCleanUp];
  ...
  let tmp5 = transitionState === participantId(4533).TransitionStates.YEETED ? tmp.onTop : tmp.onBottom;
  const context = obj2.useContext(mode(12605));
  ({ channelId: c2, layoutManager } = context);
  let obj = participantId(17636);
  ...
  const surfaceDirectRendererExperiment = participantId(9771).useSurfaceDirectRendererExperiment(
    stateFromStoresObject.userId,
    { location: "VoicePanelPIPContent.Stream" },
  );
  const tmp2Result3 = participantId(9771);
  ...
  if (stateFromStores === constants.ENDED) {
    ...
    let tmp24 = closure_22(tmp2(9767).StreamEnded, obj3);
  } else {
    tmp24 = null;
    if (stateFromStores !== tmp17.FAILED) {
      if (tmp15) {
        ...
        participantId = closure_22(VideoStream, obj5);
        obj4.children = participantId;
        let tmp18Result = closure_22(AnimatedVideoWrapper, obj4);
      } else {
        const obj6 = { participantId, layoutTransition: participantId.layoutTransition };
        tmp18Result = closure_22(User, obj6);
      }
    }
  }
  return tmp24;
...
let closure_40 = noop.memo(function EmptyPIP(transitionState) {
  ...
  const tmp = closure_24();
  return closure_22(transitionCleanUp(5892), { style: closure_24().emptyPip });
});
size = fn(2);
...
export default noop.memo(function VoicePanelPIPContent(layoutTransition) {
  ...
  const stateFromStores = layoutTransition(id[28]).useStateFromStores(
    items,
    () => {
      let tmp = id;
      if (null != id) {
  ...
  obj3.children = closure_22(layoutTransition(id[24]).TransitionGroup, {
