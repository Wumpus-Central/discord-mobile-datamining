// === Module 17733: VoicePanelPIPContent ===
...
import useProfileTileGradientDefault from "useProfileTileGradient" /* 8599 */;
...
import VoicePanelCardLayoutManager from "VoicePanelCardLayoutManager" /* 12617 */;
import VideoActionCreators from "VideoActionCreators" /* 17571 */;
import VoicePanelStreamOutputSinkStack from "VoicePanelStreamOutputSinkStack" /* 17648 */;
...
function User(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9775).useSurfaceDirectRendererExperiment(id1, { location: "VoicePanelPIPContent" });
  ...
  if (canRenderParticipantVideo) {
    ...
    if (null != user1) {
      const obj9 = { source: tmp3(9187).getCachedSourceFromURI(user1.getAvatarURL(guildId, 80, false)), size: null, avatarDecoration: null };
...
function Stream(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9775).useSurfaceDirectRendererExperiment(stateFromStoresObject.userId, { location: "VoicePanelPIPContent.Stream" });
