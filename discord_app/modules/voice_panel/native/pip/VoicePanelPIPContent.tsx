...
import VideoRendererNativeComponentDefault from "VideoRendererNativeComponent" /* 9622 */;
...
function User(participantId) {
  ...
  const surfaceDirectRendererExperiment = participantId(9610).useSurfaceDirectRendererExperiment(id1, { location: "VoicePanelPIPContent" });
  ...
  if (canRenderParticipantVideo) {
    ...
    if (null != tmp6ResultResult) {
      const obj4 = { colors: tmp6ResultResult, start: { x: 0, y: 0 }, end: { x: 0, y: 1 }, style: layoutManager.absoluteFill, layout: tmp3(17090).layoutTransition, pointerEvents: "none" };
    ...
    if (null != user) {
      const obj6 = { source: tmp3(8831).getCachedSourceFromURI(user.getAvatarURL(guildId, 80, false)), size: null, avatarDecoration: null };
