...
function VideoDisabledSvgIcon(size) {
  ...
  if (MEDIUM === undefined) {
    MEDIUM = Button /* Button */.IconSizes.MEDIUM;
  }
  const merged = Object.assign(size, Object.create(null));
  const tmp4 = callback3();
  let obj = Button /* Button */;
  ...
  const items1 = [callback(inlineStyles /* inlineStyles */.Path, { d: "M8.48485 19H13C15.2091 19 17 17.2091 17 15L19.563 17.0504C20.5451 17.8361 22 17.1368 22 15.8791V8.12094C22 7.35968 21.467 6.80301 20.8285 6.65636L8.48485 19Z", fill: tintColor }), callback(inlineStyles /* inlineStyles */.Path, { d: "M14.9873 5.52783C14.4019 5.192 13.7233 5 13 5H6C3.79086 5 2 6.79086 2 9V15C2 15.9811 2.35325 16.8798 2.93949 17.5757L14.9873 5.52783Z", fill: tintColor }), ];
  ...
  items1[2] = callback(inlineStyles /* inlineStyles */.Path, obj);
  obj.children = items1;
...
const memoResult = importAllResult.memo((arg0) => {
  ...
  const tmp = callback3();
  if (isStableVoiceStateEqual /* isStableVoiceStateEqual */.MuteDeafenIconState.DEAFENED_SERVER === state) {
    const items = [style, ];
    let redTint = null;
    if (!alwaysWhite) {
      redTint = tmp.redTint;
...
const memoResult1 = importAllResult.memo((state) => {
  ...
  if (isStableVoiceStateEqual /* isStableVoiceStateEqual */.VideoIconState.VIDEO_DISABLED_LOCAL_AUTO === state) {
    let obj = {};
    const merged1 = Object.assign(merged);
