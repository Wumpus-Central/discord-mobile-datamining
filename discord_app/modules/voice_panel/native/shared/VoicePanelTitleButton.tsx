// === Module 17652: VoicePanelTitleButton ===
...
import QuestActivityButtonDefault from "QuestActivityButton" /* 17575 */;
import VoicePanelHeaderUserState from "VoicePanelHeaderUserState" /* 17650 */;
...
function ChannelButtonIcons() {
  ...
  const items = [React7(ShieldLockIcon.ShieldLockIcon, obj2), React7(BaseTextButton.BaseTextButton.Icon, { source: _modDef7473 })];
  obj.children = items;
...
function ChannelButton(channelId) {
  ...
  const isCallSecureFramesVerified = channelId(10030).useIsCallSecureFramesVerified({ channelId });
...
function ActivityButton(participant) {
  ...
  obj3.text = str;
  obj3.icon = _modDef5330;
  obj3.onPress = participant.onPress;
  const items1 = [closure_9(participant(9260).HeaderButton, obj3), closure_9(QuestActivityButtonDefault, { applicationId: participant.applicationId })];
  obj2.children = items1;
  return closure_10(participant(5269).Stack, obj2);
}
...
export default noop.memo(function VoicePanelTitleButton() {
  const context = noop.useContext(channelId(12605));
  const guildId = context.guildId;
  channelId = context.channelId;
  ({ channelType, focused } = context);
  const derivedStateFromSharedValue = guildId(8613).useDerivedStateFromSharedValue(focused, (id) => {
    id = undefined;
    if (id != null) {
      id = id.id;
    }
  ...
  const tmp3 = channelId(17649)(derivedStateFromSharedValue, channelId, guildId);
