...
export default require("noop").memo(function HeaderGlassBlur(shown) {
  ...
  let tmp6Result = tmp6(5702);
  const items3 = [closure_4(set, { style: tmp.stroke }), ];
  tmp6Result = tmp6(5702);
  const obj3 = { style: tmp.stroke };
  obj6 = shown(4130);
  const tmp2Result = tmp2(4130);
  items3[1] = closure_4(tmp6Result, { style: tmp2(4130).isThemeDark(tmp7) ? tmp.strokeAlt : tmp.strokeAltLight });
  obj2[1] = items3;
