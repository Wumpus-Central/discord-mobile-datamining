...
let closure_9 = require("module_4146").createAnimatedComponent(require("GameConsoleAccountButton").ScrollView);
let closure_10 = importAllResult.memo((arg0) => {
  ...
  if (!tmp2) {
    ...
    obj[2] = items1;
    items[3] = tmp7(tmp3(5649).TableRowGroup, obj);
    ...
    obj6[1] = items2;
    items[5] = tmp7(tmp3(5649).TableRowGroup, obj6);
    let tmp7Result = null;
    if (tmp2) {
      tmp7Result = null;
      if (setting) {
        tmp7Result = null;
        if (embeddedActivityLaunchability === tmp3(10892).EmbeddedActivityLaunchability.CAN_LAUNCH) {
