...
const memoResult = importAllResult.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  let derivedValue;
  let derivedValue1;
  let obj = gestureSpecs(8391);
  const gradientTop = obj.useGradientTop();
  const context = importAllResult.useContext(setControlsMode(11644));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8732)(windowDimensions);
  importAllResult = tmp4;
  rect = setControlsMode(8732)(safeArea);
  ...
  const controlsDrawerOpenWidth = gestureSpecs(11651).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  ...
  const obj3 = gestureSpecs(11651);
  const appLauncherChatInputRefDummy = gestureSpecs(9548).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(9548);
  const ref = importAllResult.useRef(gestureSpecs(9548).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = importAllResult.useRef(undefined);
  const sharedValue = gestureSpecs(4036).useSharedValue(0);
  const obj5 = gestureSpecs(4036);
  const sharedValue1 = gestureSpecs(4036).useSharedValue(0);
  const obj6 = gestureSpecs(4036);
  const fn = function y() {
  ...
  derivedValue = gestureSpecs(4036).useDerivedValue(fn);
  const obj7 = gestureSpecs(4036);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4036).useDerivedValue(E);
  const obj8 = gestureSpecs(4036);
  class L {
    constructor() {
      if (c5.get()) {
