// === Module 17764: VoicePanelControlsAppLauncher ===
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(8201).useGradientTop();
  const context = noop.useContext(setControlsMode(12605));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8613)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(8613)(safeArea);
  const obj = gestureSpecs(8201);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(12613).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(12613);
  const appLauncherChatInputRefDummy = gestureSpecs(11561).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(11561);
  const ref = noop.useRef(gestureSpecs(11561).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = noop.useRef(undefined);
  const sharedValue = gestureSpecs(4559).useSharedValue(0);
  const obj5 = gestureSpecs(4559);
  const sharedValue1 = gestureSpecs(4559).useSharedValue(0);
  const obj6 = gestureSpecs(4559);
  const fn = function y() {
  ...
  const derivedValue = gestureSpecs(4559).useDerivedValue(fn);
  const obj7 = gestureSpecs(4559);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4559).useDerivedValue(E);
  const obj8 = gestureSpecs(4559);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
  ...
  items3[1] = closure_7(setControlsMode(12614), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4559).View, obj11);
