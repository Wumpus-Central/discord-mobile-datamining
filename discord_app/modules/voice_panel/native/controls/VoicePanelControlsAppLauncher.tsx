...
const memoResult = importAllResult.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  let derivedValue;
  let derivedValue1;
  let obj = gestureSpecs(8462);
  const gradientTop = obj.useGradientTop();
  const context = importAllResult.useContext(setControlsMode(11720));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8795)(windowDimensions);
  importAllResult = tmp4;
  rect = setControlsMode(8795)(safeArea);
  ...
  const controlsDrawerOpenWidth = gestureSpecs(11727).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  ...
  const obj3 = gestureSpecs(11727);
  const appLauncherChatInputRefDummy = gestureSpecs(9611).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(9611);
  const ref = importAllResult.useRef(gestureSpecs(9611).AppLauncherKeyboardCloseReason.DISMISSED);
  ...
  const sharedValue1 = gestureSpecs(4083).useSharedValue(0);
  const obj6 = gestureSpecs(4083);
  const fn = function y() {
  ...
  derivedValue = gestureSpecs(4083).useDerivedValue(fn);
  const obj7 = gestureSpecs(4083);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4083).useDerivedValue(E);
  const obj8 = gestureSpecs(4083);
  class L {
    constructor() {
      if (c5.get()) {
  ...
  items3[1] = callback(setControlsMode(11728), obj1);
  obj[1] = items3;
