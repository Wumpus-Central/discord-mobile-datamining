...
const memoResult = importAllResult.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  let derivedValue;
  let derivedValue1;
  let obj = gestureSpecs(8397);
  const gradientTop = obj.useGradientTop();
  const context = importAllResult.useContext(setControlsMode(11648));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8738)(windowDimensions);
  importAllResult = tmp4;
  rect = setControlsMode(8738)(safeArea);
  ...
  const controlsDrawerOpenWidth = gestureSpecs(11655).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  ...
  const appLauncherChatInputRefDummy = gestureSpecs(9554).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(9554);
  const ref = importAllResult.useRef(gestureSpecs(9554).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = importAllResult.useRef(undefined);
  const sharedValue = gestureSpecs(4042).useSharedValue(0);
  const obj5 = gestureSpecs(4042);
  const sharedValue1 = gestureSpecs(4042).useSharedValue(0);
  const obj6 = gestureSpecs(4042);
  const fn = function y() {
  ...
  derivedValue = gestureSpecs(4042).useDerivedValue(fn);
  const obj7 = gestureSpecs(4042);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4042).useDerivedValue(E);
  const obj8 = gestureSpecs(4042);
  class L {
    constructor() {
      if (c5.get()) {
