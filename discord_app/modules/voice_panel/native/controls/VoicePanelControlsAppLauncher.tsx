// === Module 17787: VoicePanelControlsAppLauncher ===
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(8203).useGradientTop();
  const context = noop.useContext(setControlsMode(12614));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8617)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(8617)(safeArea);
  const obj = gestureSpecs(8203);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(12622).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(12622);
  const appLauncherChatInputRefDummy = gestureSpecs(11567).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(11567);
  const ref = noop.useRef(gestureSpecs(11567).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = noop.useRef(undefined);
  const sharedValue = gestureSpecs(4561).useSharedValue(0);
  const obj5 = gestureSpecs(4561);
  const sharedValue1 = gestureSpecs(4561).useSharedValue(0);
  const obj6 = gestureSpecs(4561);
  const fn = function y() {
  ...
  const derivedValue = gestureSpecs(4561).useDerivedValue(fn);
  const obj7 = gestureSpecs(4561);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4561).useDerivedValue(E);
  const obj8 = gestureSpecs(4561);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
  ...
  items3[1] = closure_7(setControlsMode(12623), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4561).View, obj11);
