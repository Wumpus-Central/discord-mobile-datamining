...
const memoResult = importAllResult.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  let derivedValue;
  let derivedValue1;
  let obj = gestureSpecs(8616);
  const gradientTop = obj.useGradientTop();
  const context = importAllResult.useContext(setControlsMode(11913));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(8947)(windowDimensions);
  importAllResult = tmp4;
  rect = setControlsMode(8947)(safeArea);
  ...
  const controlsDrawerOpenWidth = gestureSpecs(11920).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  ...
  const obj3 = gestureSpecs(11920);
  const appLauncherChatInputRefDummy = gestureSpecs(9766).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(9766);
  const ref = importAllResult.useRef(gestureSpecs(9766).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = importAllResult.useRef(undefined);
  const sharedValue = gestureSpecs(4184).useSharedValue(0);
  const obj5 = gestureSpecs(4184);
  const sharedValue1 = gestureSpecs(4184).useSharedValue(0);
  const obj6 = gestureSpecs(4184);
  const fn = function y() {
  ...
  derivedValue = gestureSpecs(4184).useDerivedValue(fn);
  const obj7 = gestureSpecs(4184);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4184).useDerivedValue(E);
  const obj8 = gestureSpecs(4184);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
