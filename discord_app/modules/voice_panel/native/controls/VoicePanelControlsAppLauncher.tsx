// === Module 17011: VoicePanelControlsAppLauncher ===
...
export default noop.memo(function VoicePanelControlsAppLauncher(gestureSpecs) {
  ...
  const gradientTop = gestureSpecs(7292).useGradientTop();
  const context = noop.useContext(setControlsMode(11740));
  setControlsMode = context.setControlsMode;
  ({ channelId: c2, safeArea, windowDimensions } = context);
  const tmp4 = setControlsMode(7707)(windowDimensions);
  noop = tmp4;
  const rect = setControlsMode(7707)(safeArea);
  const obj = gestureSpecs(7292);
  const items = [rect];
  ...
  const controlsDrawerOpenWidth = gestureSpecs(11748).getControlsDrawerOpenWidth(tmp4.width, rect.left, rect.right);
  const callback = noop.useCallback(() => {
    const ComponentDispatch = ComponentDispatchUtils.ComponentDispatch;
    ComponentDispatch.dispatch(ComponentActions.SELECT_ACTIVITY);
    setControlsMode({ mode: hasOwnProperty.HIDDEN });
  }, items1);
  const obj3 = gestureSpecs(11748);
  const appLauncherChatInputRefDummy = gestureSpecs(10772).useAppLauncherChatInputRefDummy({ noop: true });
  const obj4 = gestureSpecs(10772);
  const ref = noop.useRef(gestureSpecs(10772).AppLauncherKeyboardCloseReason.DISMISSED);
  const ref1 = noop.useRef(undefined);
  const sharedValue = gestureSpecs(4563).useSharedValue(0);
  const obj5 = gestureSpecs(4563);
  const sharedValue1 = gestureSpecs(4563).useSharedValue(0);
  const obj6 = gestureSpecs(4563);
  const fn = function y() {
  ...
  const derivedValue = gestureSpecs(4563).useDerivedValue(fn);
  const obj7 = gestureSpecs(4563);
  class E {
    constructor() {
  ...
  derivedValue1 = gestureSpecs(4563).useDerivedValue(E);
  const obj8 = gestureSpecs(4563);
  class L {
    constructor() {
      if (closure_5.get()) {
        tmp2 = closure_3;
  ...
  items3[1] = closure_7(setControlsMode(11749), obj13);
  obj11.children = items3;
  obj10.children = closure_8(setControlsMode(4563).View, obj11);
