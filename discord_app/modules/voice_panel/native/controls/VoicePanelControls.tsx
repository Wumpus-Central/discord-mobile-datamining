...
export default noop.memo(function VoicePanelControls(gestureState) {
  ...
  const layoutEffect1 = setControlsMode.useLayoutEffect(() => {
    ...
    if (chatOpen !== chatOpen) {
      if (chatOpen) {
        obj = { tab: "chat", source: gestureState(channelId[19]).VoicePanelTabAnalyticsSources.STORE, controlsProps: { debounce: true } };
        callback(obj);
      }
    }
  ...
  obj = { withSpring: gestureState(channelId[34]).withSpring, wrapperSpecs: sharedValue1, borderRadius: token, BORDER_RADIUS_PHYSICS, PANEL_CONTROLS_HEIGHT_PHYSICS: sharedValue1, MODE_CHANGE_PHYSICS: openTab, roundToNearestPixel: isScreenReaderEnabled(channelId[43]), UI_SHOW_HIDE_PHYSICS: sharedValue, gestureState, CALL_TILE_GUTTER, accessoryHeights: tmp14 };
  ie.__closure = obj;
  ...
  const memo1 = setControlsMode.useMemo(() => {
    ...
    const fn = function h(absoluteY) {
      ...
      if (diff1 > callback.get().drawerTransitionHeight) {
        ...
        obj.width = gestureState(channelId[21]).getControlsDrawerOpenWidth(first.get().width, sharedValue.get().left, sharedValue.get().right);
        const _Math = Math;
        ...
        obj2.width = gestureState(channelId[21]).getControlsDefaultWidth(first.get().width, sharedValue.get().left, sharedValue.get().right);
        obj2.height = height;
  ...
  obj = { runOnJS: gestureState(channelId[15]).runOnJS, triggerHapticFeedback: gestureState(channelId[22]).triggerHapticFeedback, HapticFeedbackTypes: gestureState(channelId[22]).HapticFeedbackTypes };
  fn2.__closure = obj;
  ...
  obj2 = { isScreenReaderEnabled, VoicePanelControlsModes, runOnJS: gestureState(channelId[15]).runOnJS, setIsDrawer: tmp38 };
  Ee.__closure = obj2;
  ...
  const obj5 = { nativeID: id, style: tmp6.accessibilityWrapper, accessibilityViewIsModal: tmp37[0], onAccessibilityEscape: callback1, pointerEvents: "box-none", children: null };
  ...
  const items8 = [closure_18(gestureState(channelId[48]).VoicePanelVisualEffectView, obj9), closure_18(closure_39, { openTab, wrapperSpecs: sharedValue1, sharedTab: sharedValue })];
  obj8.children = items8;
  const items9 = [closure_20(gestureState(channelId[29]).ThemeContextProvider, obj8), closure_18(tmp4(channelId[49]), { wrapperSpecs: sharedValue1, tab, sharedTab: sharedValue, gestureSpecs: sharedValue2, openTab }), ];
