// === Module 17727: useControlsButtons ===

// Module 17727 (useControlsButtons)
import VoicePanelControlsUtils from "VoicePanelControlsUtils" /* 12613 */;
import VoicePanelMicButton from "VoicePanelMicButton" /* 17728 */;
import VoicePanelConnectButtonDefault from "VoicePanelConnectButton" /* 17731 */;
import VoicePanelChatButtonDefault from "VoicePanelChatButton" /* 17737 */;
import VoicePanelDisconnectCancelButtonDefault from "VoicePanelDisconnectCancelButton" /* 17739 */;
import VoicePanelVideoButtonDefault from "VoicePanelVideoButton" /* 17741 */;
import VoicePanelSoundboardButtonDefault from "VoicePanelSoundboardButton" /* 17743 */;
import VoicePanelScreenshareButtonDefault from "VoicePanelScreenshareButton" /* 17745 */;
import VoicePanelDrawerToggleButtonDefault from "VoicePanelDrawerToggleButton" /* 17750 */;
...
export default function useControlsButtons() {
  const context = treatment.useContext(safeArea(12605));
  const windowDimensions = context.windowDimensions;
  safeArea = context.safeArea;
  const tmp2 = safeArea(17581)(context.channelId);
  dependencyMap = tmp2;
  treatment = safeArea(17646).useConfig({ location: "VoicePanelControlButtons" }).treatment;
  let obj = safeArea(17646);
  ...
  let obj3 = windowDimensions(4559);
  fn.__closure = { getControlsDefaultWidth: windowDimensions(12613).getControlsDefaultWidth, windowDimensions, safeArea };
  fn.__workletHash = 16456936876254;
  fn.__initData = __initData;
  const derivedValue = obj3.useDerivedValue(fn);
  const tmp5 = safeArea(8613)(derivedValue);
  closure_5 = tmp5;
  ...
  return treatment.useMemo(() => {
    ...
    const mapped = (function getButtons(dependencyMap, stateFromStores, treatment) {
      ...
      if (dependencyMap) {
        ...
        if (treatment === _true(17646).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_CHAT) {
        ...
        if (treatment === _true(17646).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_SOUNDBOARD) {
