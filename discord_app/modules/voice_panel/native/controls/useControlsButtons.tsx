// === Module 17750: useControlsButtons ===

// Module 17750 (useControlsButtons)
import VoicePanelControlsUtils from "VoicePanelControlsUtils" /* 12622 */;
import VoicePanelMicButton from "VoicePanelMicButton" /* 17751 */;
import VoicePanelConnectButtonDefault from "VoicePanelConnectButton" /* 17754 */;
import VoicePanelChatButtonDefault from "VoicePanelChatButton" /* 17760 */;
import VoicePanelDisconnectCancelButtonDefault from "VoicePanelDisconnectCancelButton" /* 17762 */;
import VoicePanelVideoButtonDefault from "VoicePanelVideoButton" /* 17764 */;
import VoicePanelSoundboardButtonDefault from "VoicePanelSoundboardButton" /* 17766 */;
import VoicePanelScreenshareButtonDefault from "VoicePanelScreenshareButton" /* 17768 */;
import VoicePanelDrawerToggleButtonDefault from "VoicePanelDrawerToggleButton" /* 17773 */;
...
export default function useControlsButtons() {
  const context = treatment.useContext(safeArea(12614));
  const windowDimensions = context.windowDimensions;
  safeArea = context.safeArea;
  const tmp2 = safeArea(17604)(context.channelId);
  dependencyMap = tmp2;
  treatment = safeArea(17669).useConfig({ location: "VoicePanelControlButtons" }).treatment;
  let obj = safeArea(17669);
  ...
  let obj3 = windowDimensions(4561);
  fn.__closure = { getControlsDefaultWidth: windowDimensions(12622).getControlsDefaultWidth, windowDimensions, safeArea };
  fn.__workletHash = 16456936876254;
  fn.__initData = __initData;
  const derivedValue = obj3.useDerivedValue(fn);
  const tmp5 = safeArea(8617)(derivedValue);
  closure_5 = tmp5;
  ...
  return treatment.useMemo(() => {
    ...
    const mapped = (function getButtons(dependencyMap, stateFromStores, treatment) {
      ...
      if (dependencyMap) {
        ...
        if (treatment === _true(17669).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_CHAT) {
        ...
        if (treatment === _true(17669).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_SOUNDBOARD) {
