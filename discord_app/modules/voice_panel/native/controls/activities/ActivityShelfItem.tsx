...
import getItemSubtitleForMaxPlayers from "getItemSubtitleForMaxPlayers" /* 12239 */;
...
function ActivityActionOverlay(arg0) {
  ...
  obj = { children: null };
  items[1] = timestampProducer(ActivityShelfItemSummaryDefault, { channelId: id, applicationId: id, applicationName: name, submitting: useLaunchingActivityButtonStateDefault({ applicationId, context, launchingComponentId }).submitting });
  obj.children = items;
...
function ParticipantsText(arg0) {
  ...
  obj.style = items;
  obj = { source: _modDef12871, size: native.Icon.Sizes.EXTRA_SMALL, color: "white" };
...
export default function ActivityShelfItem(arg0) {
  ...
  const items2 = [timestampProducer(tmp3Result, obj3), timestampProducer(ActivityActionOverlay, { action: activityAction, applicationId: activityItem.application.id, context, activityItem, launchingComponentId: id })];
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj7 = { size: native.Icon.Sizes.REFRESH_SMALL_16, source: _modDef17233, color: tmp.developerIconColor.color };
