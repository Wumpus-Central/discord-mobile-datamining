...
createStyles = { justifyContent: "center", alignItems: "center", borderRadius: nativeDefault.modules.button.BORDER_RADIUS_LG };
  ...
  const memo1 = pressed.useMemo(() => {
    ...
    if (!obj.isAndroid()) {
      const fn = function t(windowWidth) {
        ...
        obj = { originX: onPressIn(style[9]).withSpring(windowWidth.targetOriginX, closure_4), opacity: null, transform: null };
  ...
  const memo2 = pressed.useMemo(() => {
    ...
    if (!obj.isAndroid()) {
      const fn = function t(currentOriginX) {
        ...
        obj.transform = items;
        obj.initialValues = obj;
        obj = { originX: onPressIn(style[9]).withSpring(currentOriginX.currentOriginX + __initData.get(), closure_4), opacity: null, transform: null };
        const obj4 = onPressIn(style[9]);
