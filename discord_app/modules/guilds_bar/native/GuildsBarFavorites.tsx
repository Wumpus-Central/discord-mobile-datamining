...
const memoResult = importAllResult.memo(function GuildsBarFavorites() {
  ...
  let obj = shouldShowPopover(15734);
  obj1 = shouldShowPopover(9927);
  ...
  ({ badge, unread } = markPopoverAsDismissed(15751)(stateFromStores));
  const tmp8 = markPopoverAsDismissed(15751)(stateFromStores);
  ({ badge: badge2, cutouts } = markPopoverAsDismissed(15736)({ mentionCount: badge }));
  const ref = importAllResult.useRef(null);
  const tmp9 = markPopoverAsDismissed(15736)({ mentionCount: badge });
  const tmp11 = callback();
  const favoritesIntroPopover = shouldShowPopover(9936).useFavoritesIntroPopover();
  ...
  let tmp19 = !tmp3;
  const obj4 = shouldShowPopover(9936);
  ...
  obj[10] = badge2;
  obj[11] = closure_8(shouldShowPopover(15752).HomeDrawerFavoritesRowExpandedChildren, {});
  const colors = tmp(712).colors;
  obj = { color: isFavoritesGuildSelected ? colors.WHITE : colors.MOBILE_GUILDBAR_ICON_DEFAULT };
  obj[12] = closure_8(shouldShowPopover(8959).StarIcon, obj);
  const children = [closure_8(markPopoverAsDismissed(15734), obj), , ];
  ...
  if (shouldShowPopover) {
    ...
    shouldShowPopover = tmp17(tmp(15753), obj2);
  }
