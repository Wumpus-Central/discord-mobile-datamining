...
function TransitionWrapper(children) {
  ...
  fn.__closure = {
    withSpring: flag(state[14]).withSpring,
    visible: sharedValue,
    FOLDER_SPRING_PHYSICS,
    state,
    TransitionStates: flag(state[19]).TransitionStates,
    runOnJS: flag(state[18]).runOnJS,
    cleanUp,
    fromTop: flag,
    guildItemSize: token,
  };
...
let closure_18 = createStyles.createStyles(() => {
  ...
  obj.guildPreviewIcon = {
    position: "absolute",
    margin: nativeDefault.modules.mobile.GUILD_FOLDER_PREVIEW_ICON_MARGIN,
  };
...
const memoResult = noop.memo(function FolderBGInner(color) {
  ...
  const fn = function s(height) {
    ...
    const obj2 = {
      height: color(dependencyMap[14]).withSpring(height.targetHeight, TRANSITION_PHYSICS, "animate-always"),
    };
    obj.animations = obj2;
...
export default noop.memo(function GuildsBarGuildFolder(id) {
  ...
  const tmp3 = expanded;
  let obj2 = id(name[22]);
  const guildsBarAnimatedWrapperStyles = id(name[22]).useGuildsBarAnimatedWrapperStyles({
  ...
  const stateFromStoresObject1 = id(name[12]).useStateFromStoresObject(
    items1,
    () => {
      let num = 0;
  ...
  obj10.label = obj5.useStateFromStores(
    items3,
    () => {
      if (null != name) {
  ...
  if (expanded) {
    ...
    if (!flag) {
      ...
      tmp19Result = closure_16(closure_21, obj11);
    }
  }
  ...
  if (!flag2) {
    ...
    tmp19Result2 = closure_16(tmp3(tmp2[30]), obj12);
  }
  obj10.expandedChildren = tmp19Result2;
  obj10.children = closure_16(tmp(name[19]).TransitionGroup, {
