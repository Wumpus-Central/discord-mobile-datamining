// === Module 15252: GuildFolderSettingsScene ===
...
function GuildFolderSettingsScene(arg0) {
  ...
  obj[1] = callback(require(7765) /* Form */.FormInput, obj);
  const items = [callback(require(7765) /* Form */.FormSection, obj), ];
  const obj1 = { title: null, children: null };
  const intl3 = require(1236) /* getSystemLocale */.intl;
  obj1[0] = intl3.string(require(1236) /* getSystemLocale */.t.xpurRF);
  obj1[1] = callback(importDefault(15254), { color, onChange: onColorChange });
  items[1] = callback(require(7765) /* Form */.FormSection, obj1);
  obj[0] = items;
...
export default function GuildFolderSettingsModal(folderId) {
  ...
  callback = obj2.useCallback(() => {
    ...
    folderId(8175).saveGuildFolders(guildFolders.map((folderId) => {
      let tmp = folderId;
    ...
    let obj = folderId(8175);
    const result = folderId(15251).hideGuildsBarFolderModal();
  }, items2);
  ...
  const memo = obj2.useMemo(() => {
    ...
    obj[2] = folderId(5209).getHeaderCloseButton(callback1);
    if (c5) {
      const intl2 = tmp3(1236).intl;
      let fn = tmp3(5209).getHeaderTextButton(intl2.string(tmp3(1236).t["R3BPH+"]), callback);
      const tmp3Result = tmp3(5209);
    } else {
      fn = () => null;
    }
    obj[3] = fn;
  ...
  return callback1(folderId(5636).Navigator, obj);
};