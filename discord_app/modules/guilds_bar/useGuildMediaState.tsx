...
export default function useGuildMediaState(guild_id) {
  ...
  const stateFromStores = _require(isDontBadgeMutedVcsEnabled[14]).useStateFromStores(items, () =>
    closure_1_13.isMuted(closure_0),
  );
  let obj = _require(isDontBadgeMutedVcsEnabled[14]);
  isDontBadgeMutedVcsEnabled = _require(isDontBadgeMutedVcsEnabled[15]).useIsDontBadgeMutedVcsEnabled(
    "useGuildMediaState",
  );
  ...
  const stateFromStoresArray = _require(isDontBadgeMutedVcsEnabled[14]).useStateFromStoresArray(items1, () => {
    ...
    return embeddedActivitiesForGuild.filter((location) => {
      basicChannel = basicChannel.getBasicChannel(
        callback(table[17]).getEmbeddedActivityLocationChannelId(location.location),
      );
      let type;
  ...
  const embeddedActivityLocationChannelId = _require(
    isDontBadgeMutedVcsEnabled[17],
  ).getEmbeddedActivityLocationChannelId(_location);
  ...
  return _require(isDontBadgeMutedVcsEnabled[14]).useStateFromStoresObject(
    items4,
    () => {
      const voiceChannelId = closure_1_12.getVoiceChannelId();
      ...
      const keys = stateFromStores(isDontBadgeMutedVcsEnabled[20]).keys(
        stateFromStoresArray.getStageInstancesByGuild(tmp4),
      );
      let tmp9 = tmp5;
