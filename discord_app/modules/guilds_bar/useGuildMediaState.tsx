...
export default function useGuildMediaState(guild_id) {
  ...
  const isActivitiesInTextEnabled = require("ActivitiesInTextUtils").useIsActivitiesInTextEnabled(
  ...
  const stateFromStoresObject = require("initialize").useStateFromStoresObject(
    ...
    () => {
      ...
      const result = guild_id(isDontBadgeMutedVcsEnabled[18]).filterBlockedUsersFromVoiceStates(
        authStore.getVoiceStates(tmp3),
      );
