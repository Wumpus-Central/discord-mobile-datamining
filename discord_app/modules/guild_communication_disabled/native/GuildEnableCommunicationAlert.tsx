// === Module 11062: GuildEnableCommunicationAlert ===
...
export default function GuildEnableCommunicationAlert(guildId) {
  ...
  const tmp2 = callback();
  dependencyMap = tmp2;
  closure_3 = _handleConfirmRemoveTimeout(userId(8240)(userId, guildId), 1)[0];
  userId(4766)(() => {
  ...
  const tmp12 = View;
  const tmp3 = userId;
  const tmp7 = userId(4768);
  let str = userId(4801).getName(guildId, null, user);
  if (str == null) {
    str = "";
  }
  ...
  const items = [closure_10(guildId(4739).Text, obj), ];
  ...
  items[1] = closure_10(guildId(4739).Text, obj2);
  obj[1] = items;
