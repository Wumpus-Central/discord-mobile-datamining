// === Module 16992: useMediaKeyboardConfig ===

// Module 16992 (useMediaKeyboardConfig)
import Server from "Server" /* 1978 */;
import useUploadDisabledDefault from "useUploadDisabled" /* 12493 */;
...
export default function useMediaKeyboardConfig(arg0) {
  ...
  const tmp4 = context(8006).useCanPostPollsInChannel(channel) && context.target !== MediaKeyboardTarget.COMMAND;
  importDefault = tmp4;
  const tmp5 = useUploadDisabledDefault(channel);
  dependencyMap = tmp5;
  let obj = context(8006);
  ...
  const tmp8 = context(9606).useIsAppLauncherEnabled(channel.id) && context.target !== MediaKeyboardTarget.COMMAND;
  MediaKeyboardTarget = tmp8;
  const tmp2Result3 = context(9606);
  mediaKeyboardDraftType = context(10901).getMediaKeyboardDraftType(context.target);
