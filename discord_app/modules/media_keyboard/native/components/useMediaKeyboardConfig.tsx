// === Module 16268: useMediaKeyboardConfig ===

// Module 16268 (useMediaKeyboardConfig)
import Server from "Server" /* 1978 */;
import useUploadDisabledDefault from "useUploadDisabled" /* 11704 */;
...
export default function useMediaKeyboardConfig(arg0) {
  ...
  const tmp4 = context(7175).useCanPostPollsInChannel(channel) && context.target !== MediaKeyboardTarget.COMMAND;
  importDefault = tmp4;
  const tmp5 = useUploadDisabledDefault(channel);
  dependencyMap = tmp5;
  let obj = context(7175);
  let canStartThread = context(6682).useCanStartThread(channel);
  if (canStartThread) {
  ...
  const tmp2Result = context(6682);
  const tmp8 = context(8781).useIsAppLauncherEnabled(channel.id) && context.target !== MediaKeyboardTarget.COMMAND;
  MediaKeyboardTarget = tmp8;
  const tmp2Result3 = context(8781);
  mediaKeyboardDraftType = context(10087).getMediaKeyboardDraftType(context.target);
