// === Module 16189: useMediaKeyboardConfig ===

// Module 16189 (useMediaKeyboardConfig)
import useUploadDisabledDefault from "useUploadDisabled" /* 11877 */;
...
export default function useMediaKeyboardConfig(arg0) {
  ...
  const tmp4 = context(7430).useCanPostPollsInChannel(channel) && context.target !== MediaKeyboardTarget.COMMAND;
  importDefault = tmp4;
  const tmp5 = useUploadDisabledDefault(channel);
  dependencyMap = tmp5;
  let tmp2Result = tmp2(6062);
  ...
  let obj = context(7430);
  mediaKeyboardDraftType = context(10226).getMediaKeyboardDraftType(context.target);
