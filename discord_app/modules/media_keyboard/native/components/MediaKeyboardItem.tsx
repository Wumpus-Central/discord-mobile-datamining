...
function NewCaption(arg0) {
  ...
  obj.style = items;
  obj = { source: _modDef10486, style: tmp.icon };
...
function MediaKeyboardImage(draftType) {
  ...
  if (constants.PHOTO !== type) {
    ...
    if (tmp9.VIDEO !== type) {
      ...
      obj1 = { source: numItemsPerRow(tmp3[19]), disableColor: false, color: tmp.checkIcon.color, style: tmp.checkIcon };
      obj.children = closure_9(tmp2(tmp3[9]).Icon, obj1);
      const items3 = [index, numItemsPerRow, totalNumItems];
      const obj2 = { accessibilityRole: "button", accessibilityLabel: stringResult, accessibilityState: null, onPress: null, onLongPress: null, disabled: null, style: null, children: null };
...
let obj3 = { backgroundColor: nativeDefault.colors.CONTROL_SECONDARY_BACKGROUND_DEFAULT, pressedBackgroundColor: nativeDefault.colors.CONTROL_SECONDARY_BACKGROUND_ACTIVE };
size = fn(2);
  ...
  return disableWhenReachedLimit(closure_4, {
    ...
    children: items.map((type, index) => {
      if (null == type) {
        ...
        if (hasItem) {
          obj = { size, onPress: null, disabled: null, accessibilityLabel: null, index: null, totalNumItems: null, numItemsPerRow: null, children: null };
