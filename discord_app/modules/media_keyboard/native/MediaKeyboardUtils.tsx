// === Module 10347: handleLimitedPickerDialog ===
...
import NativePermissionsRequestOptionsDefault from "NativePermissionsRequestOptions" /* 5035 */;
import _modDef8095 from "module_8095" /* 8095 */;
import navigateToThreadCreation from "navigateToThreadCreation" /* 10780 */;
        ...
        try {
          ...
          if (0 === v0) {
            ...
            if (arg0 === 1) {
              ...
              obj4[0] = lib(10781).handleDocumentSelection(obj3);
              return obj4;
            }
          } else {
            if (2 === tmp5) {
...
function handleSelectKeyboardItem(channelId, item, isIncluded, arg3) {
  ...
  if (isIncluded) {
    ...
    if (null != findUploadResult) {
      _modDef8095.remove(channelId, findUploadResult.id, tmp13.ChannelMessage);
      const obj5 = _modDef8095;
    }
    tmp13 = DraftType;
  } else {
    let obj = _modDef8095;
...
function showSimpleMediaKeyboard(channel) {
  ...
  obj = {
        ...
        resolved.then((arg0) => {
          if (arg0) {
            ...
            if (_undefined3 <= 1) {
                    ...
                    if (0 !== lib.length) {
                      ...
                      for (const item10018 of assets) {
                        ...
                        if (null != item10018.uri) {
                          ...
                          obj[8] = lib(5025).UploadPlatform.REACT_NATIVE;
              ...
              if (obj3.isIOS()) {
                let length = arr2.filter((origin) => origin.origin !== found(5025).UploadOrigin.IMAGE_PICKER).length;
              } else {
                length = arr2.length;
              }
    ...
    onClose: _require(10348).hideMediaKeyboardActionSheet,
    onBack: _require(10348).hideMediaKeyboardActionSheet
  };
...
export const addImagesFromPicker = function addImagesFromPicker(id, items, IMAGE_PICKER) {
  ...
  if (IMAGE_PICKER !== _require(5025).UploadOrigin.FILE_ATTACHMENT) {
    ...
    obj[1] = id;
    obj[2] = DraftType.ChannelMessage;
    _modDef8095.addFiles(obj);
    const obj3 = _modDef8095;
  } else {
    obj = _modDef8095;
    obj = { files: null, channelId: null, draftType: null };
    obj[0] = items;
...
export const addAttachmentForCommand = function addAttachmentForCommand(channelId, onSelectFiles, result1, closure_1_1, IMAGE_PICKER) {
  ...
  _modDef8095.remove(channelId, closure_1_1.option.name, InteractionModal);
  const obj4 = _modDef8095;
...
export const handleSelectGift = function handleSelectGift(analyticsLocations, chatInput, current2) {
  ...
  obj1[4] = fn;
  _require(10373).openGiftModal(obj1);
};
