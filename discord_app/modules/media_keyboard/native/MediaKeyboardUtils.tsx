        ...
        try {
          ...
          if (0 === v0) {
            ...
            if (arg0 === 1) {
              ...
              obj4[0] = lib(10225).handleDocumentSelection(obj3);
              return obj4;
            }
          } else {
            if (2 === tmp5) {
...
function handleSelectKeyboardItem(channelId, item, isIncluded, arg3) {
  ...
  if (isIncluded) {
    ...
    if (null != findUploadResult) {
      dispatcher.remove(channelId, findUploadResult.id, tmp13.ChannelMessage);
...
function showSimpleMediaKeyboard(channel) {
  ...
  let obj = _hideMediaKeyboardActionSheet;
  obj = {
    channel,
        ...
        resolved.then((arg0) => {
          if (arg0) {
            ...
            if (_undefined3 <= 1) {
                    ...
                    if (0 !== lib.length) {
                      ...
                      for (const item10018 of assets) {
                        ...
                        if (null != item10018.uri) {
                          ...
                          obj[8] = lib(4914).UploadPlatform.REACT_NATIVE;
              ...
              if (obj3.isIOS()) {
                let length = arr2.filter((origin) => origin.origin !== found(4914).UploadOrigin.IMAGE_PICKER).length;
              } else {
                length = arr2.length;
              }
    ...
    onClose: _hideMediaKeyboardActionSheet.hideMediaKeyboardActionSheet,
    onBack: _hideMediaKeyboardActionSheet.hideMediaKeyboardActionSheet
  };
...
export const addImagesFromPicker = function addImagesFromPicker(id, items, IMAGE_PICKER) {
  ...
  if (IMAGE_PICKER !== _cancel.UploadOrigin.FILE_ATTACHMENT) {
...
export const addAttachmentForCommand = function addAttachmentForCommand(channelId, onSelectFiles, result1, outer1_1, IMAGE_PICKER) {
  ...
  dispatcher.remove(channelId, outer1_1.option.name, InteractionModal);
...
export const handleSelectGift = function handleSelectGift(analyticsLocations, chatInput, current2) {
  ...
  const ComponentDispatch = _ComponentDispatcher.ComponentDispatch;
  ...
  obj[2] = guild_id1;
  expandEventProperties.track(constants2.CHAT_INPUT_COMPONENT_VIEWED, obj);
  ...
  obj1[4] = fn;
  _openGiftModal.openGiftModal(obj1);
};
...
export const cropResultToUploadItem = function cropResultToUploadItem(path) {
  ...
  const arr = parts.pop();
  const str = path.path;
  obj[0] = apply.uniqueId(path.path);
  obj[1] = combined;
  obj[2] = combined;
