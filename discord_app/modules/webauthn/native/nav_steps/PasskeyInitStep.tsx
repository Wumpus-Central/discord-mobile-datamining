...
function CredentialList(navigation) {
  ...
  if (0 === credentials.length) {
    ...
    const items1 = [closure_8(tmp(5911).PasskeysSpotIllustration, { scale: 0.6 }), ];
    ...
    items1[1] = closure_8(tmp(4753).Text, obj3);
    ...
    obj4.children = credentials.map((label) => {
      ...
      obj3.onPress = function onPress() {
        return ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14952, dependencyMap.paths), "WEBAUTHN_DELETE_SHEET_KEY", { credential, deleting, setDeleting });
      };
    ...
    return closure_8(tmp(5906).TableRowGroup, obj4);
  }
