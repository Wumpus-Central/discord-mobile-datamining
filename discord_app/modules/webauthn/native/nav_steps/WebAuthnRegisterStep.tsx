...
let obj5 = {
  [PASSKEY_CREDENTIAL_MANAGER]: NativeCeremoniesDefault.registerPasskey,
  [PASSKEY_DEVICE]: NativeCeremoniesDefault.registerAndroidDevicePasskey,
  [OTHER_AND_ANDROID_NONDISCOVERABLE]: NativeCeremoniesDefault.registerSecurityKey,
};
...
export default function WebAuthnRegisterStep() {
  ...
  const announceError = navigation(15037).useAnnounceError(tmp10);
  ...
  const items3 = [closure_7(navigation(15038).KeyImage, {}), ,];
  ...
  const obj8 = {
    children: closure_7(navigation(5271).Button, {
      text: string2Result,
      disabled: tmp7,
      loading: tmp7,
      onPress() {
  ...
  items4[2] = closure_7(navigation(5736).ButtonGroup, obj8);
  rect.children = items4;
  return closure_8(navigation(7454).SafeAreaPaddingView, rect);
}
