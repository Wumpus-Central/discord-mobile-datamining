...
import GatewayEncodingDefault from "GatewayEncoding" /* 13159 */;
import GatewaySocketOpCodes2 from "GatewaySocketOpCodes" /* 13162 */;
import AltGatewayTrackerDefault from "AltGatewayTracker" /* 13163 */;
import GatewaySocketDispatcherDefault from "GatewaySocketDispatcher" /* 13166 */;
import GatewaySocketAnalytics from "GatewaySocketAnalytics" /* 13171 */;
import ConnectionStateDefault from "ConnectionState" /* 13173 */;
import GatewayCompressionHandler from "GatewayCompressionHandler" /* 13175 */;
    ...
    if (obj.getIsPaused()) {
      ...
      closure_129_2 = (byteLength, compressed_byte_size) => {
        ...
        if (op !== GatewaySocketOpCodes2.Opcode.DISPATCH) {
          const _HermesInternal = HermesInternal;
          AppStartPerformanceDefault.mark("\u{1F310}", "GatewaySocket.onMessage " + op + " " + GatewaySocketOpCodes2.Opcode[op]);
        }
        if (DeveloperOptionsStore.isLoggingGatewayEvents) {
          const items = [op];
          if (op === GatewaySocketOpCodes2.Opcode.DISPATCH) {
      ...
      if (null != _ws) {
        ...
        if (_ws.state.gateway !== str1) {
          ...
          logger.log("[FAST CONNECT] refusing to adopt socket: identified user " + userId + " does not match switch target " + global);
          ws.close(1000);
          messages2 = null;
          flag3 = false;
          flag4 = false;
...
prototype["_handleClose"] = function _handleClose(wasClean, c13, reason) {
  ...
  if (code === code) {
    ...
    if (self.nextReconnectIsImmediate) {
      ...
      const _HermesInternal = HermesInternal;
      closure_9.info("[WS CLOSED] (" + str1 + ", " + code + ", " + reason + ") retrying in " + result2.toFixed(2) + " seconds.");
