...
export const fetchGameServerRegions = function fetchGameServerRegions(arg0) {
  ...
  const value = HTTP.get({
    url: closure_8.GAME_SERVER_REGIONS(arg0),
  ...
  return value.then((body) => {
    ...
    body = body.body;
    const mapped = body.map(callback(12487));
...
export const fetchMyGameServerRegions = function fetchMyGameServerRegions() {
  const HTTP = sendRequest.HTTP;
  const value = HTTP.get({
    url: closure_8.GAME_SERVER_MY_REGIONS,
    rejectWithError: true,
    oldFormErrors: true,
    retries: 3,
  });
  return value.then((body) => {
    ...
    const mapped = regions.map(callback(12487));
    obj[1] = mapped.sort((name, name2) => {
      name = name.name;
      return name.localeCompare(name2.name);
    });
...
export const optimisticallyMarkGameServerResizing = function optimisticallyMarkGameServerResizing(arg0) {
  ...
  if (null != found) {
    let obj = dispatcherDefault;
    obj = { type: "GAME_SERVER_UPDATE", guildId: "Array", gameServer: "LOCAL_PAYMENT_NOT_ENABLED" };
    obj = {};
    const merged = Object.assign(found);
    obj.status =
      require("../../../discord_common/js/shared/shared-constants/GameServerStatus.tsx").GameServerStatus.STARTING;
    obj[2] = obj;
    obj.dispatch(obj);
  }
};
export const updateMyGameServerName = function updateMyGameServerName(arg0, name) {
  ...
  if (null == found) {
    ...
    resolved = HTTP.patch(obj1)
      .then(() => {
        const HTTP = callback(1272).HTTP;
        const value = HTTP.get({
          url: constants.GAME_SERVERS_ME,
          rejectWithError: true,
          oldFormErrors: true,
          retries: 3,
        });
...
export const wakeMyGameServer = function wakeMyGameServer(arg0) {
  ...
  return HTTP.post({ url: closure_8.GAME_SERVER_ME_WAKE(arg0), rejectWithError: true })
    .then((body) => {
      body = body.body;
      let obj = callback2(573);
      let tmp3 = body;
      if (body.status === callback(12485).GameServerStatus.SLEEPING) {
        obj = {};
        const merged = Object.assign(body);
        obj.status = callback(12485).GameServerStatus.STARTING;
        tmp3 = obj;
      }
      obj = { type: "GAME_SERVER_UPDATE", guildId: "Array", gameServer: "LOCAL_PAYMENT_NOT_ENABLED" };
      obj[2] = tmp3;
...
export const wakeGameServer = function wakeGameServer(arg0, arg1) {
  ...
  return HTTP.post({ url: closure_8.GAME_SERVER_WAKE(arg0, arg1), rejectWithError: true }).then((body) => {
    ...
    obj = {
      ...
      guildId: closure_0,
      instance: closure_1_1(closure_1_2[10])(body.body),
    };
    obj.dispatch(obj);
  });
};
