...
createStyles.previewContainer = { height: 140, display: "flex", alignItems: "center", justifyContent: "center", paddingHorizontal: nativeDefault.space.PX_8 };
let obj1 = { height: 140, display: "flex", alignItems: "center", justifyContent: "center", paddingHorizontal: nativeDefault.space.PX_8 };
...
export default function CustomTypingIndicatorEditScreen() {
  ...
  const callback1 = first1.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(15439, dependencyMap.paths), "CustomTypingIndicatorTypingSuggestionPickerSheet", obj);
  }, items4);
  const callback2 = first1.useCallback(() => {
    const obj = { emojis: memo, initialAnimation: first3, onChange: onChange2 };
    obj.openLazy(asyncRequireImpl(15440, dependencyMap.paths), "CustomTypingIndicatorAnimationPickerSheet", obj);
  }, items5);
  first1.useRef(null);
  ...
  let callback5 = first1.useCallback(analyticsLocations(function*() {
    if (c4 === 2) {
      ...
      try {
        ...
        const obj6 = { emoji_names: closure_130_11.map((name) => name.name), animation_name: nativeStackNavigation(tmp2[9]).TypingIndicatorAnimation[closure_130_9], typing_suggestion: nativeStackNavigation(tmp2[9]).TypingSuggestion[closure_130_7], custom_emoji_count: closure_130_11.filter((id) => null != id.id).length };
