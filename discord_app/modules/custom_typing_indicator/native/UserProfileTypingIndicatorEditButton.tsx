...
export default function UserProfileTypingIndicatorEditButton(isTryItOut) {
  ...
  const stringResult = intl.string(
    isTryItOut(11975).getCustomTypingIndicatorSuggestionMessage(currentCustomTypingIndicatorConfig.typingSuggestion),
  );
  ...
  obj.labelTrailing = jsx(isTryItOut(14621).UserProfileEditFormLabelBadges, {
    showPremiumIcon: true,
    showNewBadge: tmp3[0] === isTryItOut(1943).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_NEW_BADGE_PROFILE_PAGE,
  });
  obj.leading = jsx(nativeStackNavigation(11985), { config: currentCustomTypingIndicatorConfig, size: 24 });
  obj.buttonText = stringResult;
  obj.accessibilityValue = { text: stringResult };
  obj.onPress = tmp5;
  return jsx(isTryItOut(14621).UserProfileEditFormButton, {
    showPremiumIcon: true,
    showNewBadge: tmp3[0] === isTryItOut(1943).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_NEW_BADGE_PROFILE_PAGE,
  });
}
