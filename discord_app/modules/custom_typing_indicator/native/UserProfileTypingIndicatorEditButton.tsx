// === Module 14921: UserProfileTypingIndicatorEditButton ===
...
export default function UserProfileTypingIndicatorEditButton(isTryItOut) {
  ...
  const currentCustomTypingIndicatorConfig = isTryItOut(12228).useCurrentCustomTypingIndicatorConfig(isTryItOut);
  const obj2 = isTryItOut(12228);
  const items = [isTryItOut(2028).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_NEW_BADGE_PROFILE_PAGE];
  const tmp3 = _slicedToArray(isTryItOut(7633).useSelectedDismissibleContent(items, undefined, true), 2);
  dependencyMap = tmp4;
  const items1 = [nativeStackNavigation, isTryItOut, tmp3[1]];
  const obj3 = isTryItOut(7633);
  ...
  const stringResult = intl.string(isTryItOut(12228).getCustomTypingIndicatorSuggestionMessage(currentCustomTypingIndicatorConfig.typingSuggestion));
  ...
  const obj4 = isTryItOut(12228);
  obj5.labelTrailing = jsx(isTryItOut(14900).UserProfileEditFormLabelBadges, { showPremiumIcon: true, showNewBadge: tmp3[0] === isTryItOut(2028).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_NEW_BADGE_PROFILE_PAGE });
  obj5.leading = jsx(nativeStackNavigation(12238), { config: currentCustomTypingIndicatorConfig, size: 24 });
  obj5.buttonText = stringResult;
  obj5.accessibilityValue = { text: stringResult };
  obj5.onPress = tmp5;
  return jsx(isTryItOut(14900).UserProfileEditFormButton, { label: null, labelTrailing: null, leading: null, buttonText: null, accessibilityValue: null, onPress: null });
};