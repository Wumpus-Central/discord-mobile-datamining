// === Module 14891: MotionOptionButton ===
...
export default function CustomTypingIndicatorAnimationPickerSheet(onChange) {
  ...
  let obj = { value: onChange(1939).TypingIndicatorAnimation.UNSPECIFIED, label: null };
  const intl = onChange(1236).intl;
  obj[1] = intl.string(onChange(1236).t.PoWNfe);
  const items = [obj, , , ];
  obj = { value: onChange(1939).TypingIndicatorAnimation.PULSE, label: null };
  const intl2 = onChange(1236).intl;
  obj[1] = intl2.string(first(3472)["gyL/ce"]);
  items[1] = obj;
  obj = { value: onChange(1939).TypingIndicatorAnimation.RING, label: null };
  const intl3 = onChange(1236).intl;
  obj[1] = intl3.string(first(3472).EgekTm);
  items[2] = obj;
  obj1 = { value: onChange(1939).TypingIndicatorAnimation.WAVE, label: null };
  const intl4 = onChange(1236).intl;
  obj1[1] = intl4.string(first(3472)["8t5EiI"]);
  items[3] = obj1;
  const obj2 = { contentStyles: tmp.content, dismissAccessibilityLabel: null, children: null };
  const intl5 = onChange(1236).intl;
  obj2[1] = intl5.string(first(3472)["q+qHax"]);
  const obj3 = { style: tmp.previewRow, children: null };
  const obj4 = { config: null, size: 54 };
  const obj5 = { emojis, animation: first, typingSuggestion: onChange(1939).TypingSuggestion.UNSPECIFIED };
  obj4[0] = obj5;
  obj3[1] = callback2(first(11491), obj4);
  ...
  items1[1] = callback2(onChange(4891).Stack, obj6);
  obj2[2] = items1;
  return callback3(onChange(5954).ActionSheet, obj2);
};