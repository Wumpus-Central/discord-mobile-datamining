...
prototype["createWithPayload"] = function createWithPayload(searchTabs) {
    ...
    if (tmp2.GUILD_CHANNEL !== type) {
      if (tmp2.THREAD !== type) {
        if (tmp2.CHANNEL === type) {
          const searchTabFetcherImpl = new fetch /* fetch */.SearchTabFetcherImpl(searchContext.channelId, searchContext.type, searchQuery, requestPayload);
          return searchTabFetcherImpl;
        } else if (tmp2.DMS === type) {
          const searchTabFetcherImpl1 = new fetch /* fetch */.SearchTabFetcherImpl(searchContext.type, searchContext.type, searchQuery, requestPayload);
  ...
  const searchTabFetcherImpl2 = new fetch /* fetch */.SearchTabFetcherImpl(searchContext.guildId, searchContext.type, searchQuery, requestPayload);
