...
import SearchTokensUtils from "SearchTokensUtils" /* 12687 */;
...
function generateDateAutocompletions() {
  ...
  const weekdaysResult = _modDef4416.weekdays();
  const set1 = new Set(_modDef4416.weekdays().map((item) => item.toLowerCase()));
  const arraySpreadResult = HermesBuiltin.arraySpread(Array.from(new Set(_modDef4416.weekdays().map((item) => item.toLowerCase()))), tmp2);
