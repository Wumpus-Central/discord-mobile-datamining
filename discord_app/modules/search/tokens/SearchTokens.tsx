...
function dateValidator(getFullMatch, after) {
  ...
  if (null != tmp2) {
    ...
    if (set.has(formatted)) {
      const localResult = _modDef4198(formatted, "MMMM").local();
      const items = [localResult];
      const obj17 = _modDef4198(formatted, "MMMM");
      ...
      const _Set = Set;
      let tmp32Result = _modDef4198;
      const set1 = new Set(tmp32Result.weekdays().map((item) => item.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = _modDef4198(formatted, "dddd").local();
        const items1 = [localResult1];
        const obj14 = _modDef4198(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = _modDef4198(formatted, "YYYY").local();
          const items2 = [localResult2];
          const obj11 = _modDef4198(formatted, "YYYY");
          ...
          const localResult3 = _modDef4198(formatted, value2).local();
          const items3 = [localResult3];
          const obj6 = _modDef4198(formatted, value2);
...
function generateDateAutocompletions() {
  ...
  let arraySpreadResult = HermesBuiltin.arraySpread(
    Array.from(new Set(_modDef4198.weekdays().map((item) => item.toLowerCase()))),
    tmp2,
  );
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith('"')) {
    ...
    if (obj2.isGuildLikeSearchContext(searchContext)) {
      ...
      let tmpResult = tmp(5493);
      obj.boosters = tmpResult.getBoosterMap(tmp(9926).AutocompleterResultTypes.TEXT_CHANNEL);
      ...
      tmpResult = tmp(5493);
      obj.boosters = tmpResult.getBoosterMap(tmp(9926).AutocompleterResultTypes.VOICE_CHANNEL);
      ...
      if (searchContext.type === constants.DMS) {
        if (!StreamerModeStore.hidePersonalInformation) {
          ...
          obj1.boosters = tmp(5493).getBoosterMap(tmp(9926).AutocompleterResultTypes.GROUP_DM);
          const tmpResult1 = tmp(5493);
          ...
          obj2.boosters = tmp(5493).getBoosterMap(tmp(9926).AutocompleterResultTypes.USER);
          const tmpResult2 = tmp(5493);
