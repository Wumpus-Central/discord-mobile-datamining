...
function getShortcuts() {
  ...
  obj[intl.string(util.t.HYiVEQ)] = () => {
    ...
    const addResult = _modDef4416().startOf("day").add(0, "day");
  ...
  obj[intl2.string(util.t.cu86KC)] = () => {
    const obj = _modDef4416();
    const addResult = _modDef4416().startOf("day").add(-1, "day");
    const items = [addResult];
    const startOfResult = _modDef4416().startOf("day");
    items[1] = addResult.clone().add(1, "day");
    return items;
  ...
  obj[intl3.string(util.t["FvBj/6"])] = () => {
    const obj = _modDef4416();
    const addResult = _modDef4416().startOf("week").add(0, "week");
    const items = [addResult];
    const startOfResult = _modDef4416().startOf("week");
  ...
  obj[intl4.string(util.t["20uWCw"])] = () => {
    const obj = _modDef4416();
    const addResult = _modDef4416().startOf("month").add(0, "month");
    const items = [addResult];
    const startOfResult = _modDef4416().startOf("month");
  ...
  obj[intl5.string(util.t["dXC/hn"])] = () => {
    const obj = _modDef4416();
    const addResult = _modDef4416().startOf("year").add(0, "year");
    const items = [addResult];
    const startOfResult = _modDef4416().startOf("year");
...
function dateValidator(getFullMatch, after) {
  ...
  if (null != tmp2) {
    ...
    const _Set3 = Set;
    const set = new Set(_modDef4416.months().map((item) => item.toLowerCase()));
    if (set.has(formatted)) {
      const localResult = _modDef4416(formatted, "MMMM").local();
      const items = [localResult];
      const obj17 = _modDef4416(formatted, "MMMM");
      ...
      const _Set = Set;
      const tmp32Result = _modDef4416;
      const set1 = new Set(_modDef4416.weekdays().map((item) => item.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = _modDef4416(formatted, "dddd").local();
        const items1 = [localResult1];
        const obj14 = _modDef4416(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = _modDef4416(formatted, "YYYY").local();
          const items2 = [localResult2];
          const obj11 = _modDef4416(formatted, "YYYY");
          items2[1] = localResult2.clone().add(1, "year");
          const cloneResult2 = localResult2.clone();
          [obj9, obj10] = items2;
          const tmp17 = _slicedToArray(items2, 2);
        } else {
          const localResult3 = _modDef4416(formatted, value2).local();
          const items3 = [localResult3];
          const obj6 = _modDef4416(formatted, value2);
      ...
      const weekdaysResult = _modDef4416.weekdays();
    }
    const monthsResult = _modDef4416.months();
  }
...
function isValidChannelAutocomplete(token, items) {
  ...
  if (regex2.test(str)) {
    ...
    if (obj2.isGuildLikeSearchContext(items)) {
      ...
      const combined = GuildChannelStore.getChannels(guildId)[closure_5].concat(
        GuildChannelStore.getChannels(guildId)[closure_6],
      );
...
function generateDateAutocompletions() {
  const monthsResult = _modDef4416.months();
  const items = [...Array.from(new Set(_modDef4416.months().map((item) => item.toLowerCase())))];
  const set = new Set(_modDef4416.months().map((item) => item.toLowerCase()));
  const weekdaysResult = _modDef4416.weekdays();
  const set1 = new Set(_modDef4416.weekdays().map((item) => item.toLowerCase()));
  const arraySpreadResult = HermesBuiltin.arraySpread(
    Array.from(new Set(_modDef4416.weekdays().map((item) => item.toLowerCase()))),
    tmp2,
  );
  ...
  HermesBuiltin.arraySpread(
    Object.keys(getShortcuts()),
    HermesBuiltin.arraySpread(
      Array.from(new Set(_modDef12.range(2015, fullYear + 1).map((item) => item.toString()))),
      arraySpreadResult,
    ),
  );
  return items;
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith('"')) {
    ...
    if (obj3.isGuildLikeSearchContext(searchContext)) {
      ...
      obj.boosters = tmp(5747).getBoosterMap(tmp(10182).AutocompleterResultTypes.TEXT_CHANNEL);
      ...
      obj2.boosters = tmp(5747).getBoosterMap(tmp(10182).AutocompleterResultTypes.VOICE_CHANNEL);
      ...
      if (searchContext.type === constants.DMS) {
        if (!StreamerModeStore.hidePersonalInformation) {
          ...
          obj5.boosters = tmp(5747).getBoosterMap(tmp(10182).AutocompleterResultTypes.GROUP_DM);
          const tmpResult5 = tmp(5747);
          ...
          obj6.boosters = tmp(5747).getBoosterMap(tmp(10182).AutocompleterResultTypes.USER);
          const tmpResult6 = tmp(5747);
          const queryDMChannelsResult = obj8.queryDMChannels(obj6);
          const sorted = _modDef12(queryGroupDMsResult.concat(queryDMChannelsResult)).sort(tmp(10182).sortByMatchScore);
...
function makeSearchTokenConfigs(arg0) {
  ...
  obj6.getAutocompletions = function getAutocompletions(query) {
    ...
    const found = _modDef12(items1).filter((toLocaleLowerCase) =>
      items(items1[17])(closure_0, toLocaleLowerCase.toLocaleLowerCase()),
    );
  ...
  const obj7 = {
    regex: require("SearchTokensUtils").makeRegexForOptionsWithNegation(items1),
    follows: null,
    validator: isValidHasAutocomplete,
    componentType: obj.ANSWER,
    queryKey: "has",
  };
  ...
  const obj10 = {
    regex: require("SearchTokensUtils").GENERIC_REGEX,
    follows: null,
    mutable: true,
    componentType: obj.ANSWER,
  ...
  obj15.getAutocompletions = function getAutocompletions(query) {
    ...
    const found = items(items1[11])(closure_30()).filter((toLocaleLowerCase) =>
      items(items1[17])(closure_0, toLocaleLowerCase.toLocaleLowerCase()),
    );
  ...
  const regExp7 = new RegExp(
    "" + "(" + intl34.string(require("util").t.tIxkOo) + "|" + intl35.string(require("util").t.h2NzSd) + ")" + ":",
    "i",
  );
  obj16.regex = regExp7;
  ...
  obj25.getAutocompletions = function getAutocompletions(query) {
    query = query.query;
    closure_0 = query.toLocaleLowerCase();
    const found = _modDef12(items).filter((toLocaleLowerCase) =>
      items(items1[17])(closure_0, toLocaleLowerCase.toLocaleLowerCase()),
    );
