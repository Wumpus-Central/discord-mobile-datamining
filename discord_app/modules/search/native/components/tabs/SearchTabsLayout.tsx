...
export default function ConnectedSearchTabsLayout(width) {
  ...
  const autoSearchGuildChannelTab = searchContext(16968).useAutoSearchGuildChannelTab(
    searchContext,
    !memo.has(constants.GUILD_CHANNELS),
  );
  const obj3 = searchContext(16968);
  const autoSearchMembersTab = searchContext(16969).useAutoSearchMembersTab(
    searchContext,
  ...
  const autoTrackSearchTabCountsViewedAnalytics = searchContext(16971).useAutoTrackSearchTabCountsViewedAnalytics({
    searchContext,
    visibleTabCounts,
    visibleTabs,
  });
