...
import IntelligenceSearchEmptyScreenDefault from "../../../../intelligence_layer/search/native/components/IntelligenceSearchEmptyScreen.tsx";
...
let closure_21 = noop.memo((searchContext) => {
  ...
  const searchSegmentedControlState = searchContext(visibleTabCounts[20]).useSearchSegmentedControlState({
    items: memo,
    visibleTabs,
...
export default function ConnectedSearchTabsLayout(width) {
  ...
  const autoSearchGuildChannelTab = searchContext(17276).useAutoSearchGuildChannelTab(
    searchContext,
    !memo.has(constants.GUILD_CHANNELS),
  );
  const obj3 = searchContext(17276);
  const autoSearchMembersTab = searchContext(17277).useAutoSearchMembersTab(
    searchContext,
  ...
  const autoTrackSearchTabCountsViewedAnalytics = searchContext(17279).useAutoTrackSearchTabCountsViewedAnalytics({
    searchContext,
    visibleTabCounts,
    visibleTabs,
  });
