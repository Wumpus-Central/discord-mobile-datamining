...
function SearchableMembersScreen(searchContext) {
  ...
  const tmp4Result5 = searchContext(16797);
...
export default noop.memo(function MembersScreen(searchContext) {
  ...
  if (constants3.CHANNEL === type) {
    ...
    if (stateFromStores) {
      ...
      tmp21Result = jsx(tmp2(16800), { location: "GroupDMDetailsMembers", memberCount: stateFromStores1, recipientLimit: tmp11, wrapperStyle: tmp.promoBanner });
    }
    obj1.listHeaderContent = tmp21Result;
    obj.children = jsx(tmp2(12315), { channelId: searchContext.channelId, disableStickySections: true, listStyleOverride: tmp.userList, onUserPress: tmp7(1874).dismissGlobalKeyboard, listHeaderContent: null });
    obj.children = <View style={tmp.container}>{null}</View>;
    return jsx(tmp7(7265).AnalyticsLocationProvider, { style: tmp.container, children: null });
