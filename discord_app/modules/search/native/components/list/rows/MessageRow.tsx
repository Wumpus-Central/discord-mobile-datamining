...
function GuildChannelMessageRowHeader(channel) {
  ...
  const items1 = [callback(channel(1297).Icon, obj1), callback(channel(4251).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: computeChannelName(channel) }), , , ];
  if (muted) {
  ...
  if (isSystemDMResult) {
    ...
    obj4[0] = tmp6(9025).Types.SYSTEM_DM;
    isSystemDMResult = tmp10(tmp6(9025), obj4);
    const tmp6Result = tmp6(9025);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  if (isSystemDMResult) {
    ...
    obj2[0] = channel(9025).Types.SYSTEM_DM;
    isSystemDMResult = tmp9(channel(9025), obj2);
    const tmp13 = channel(9025);
  }
  ...
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    ...
    tmp9Result = tmp9(tmp4(12442).BellZIcon, obj4);
  }
  items3[2] = tmp9Result;
  tmp9Result = null;
  if (message.isPoll()) {
    ...
    tmp9Result = tmp9(channel(15609), obj5);
  }
  items3[3] = tmp9Result;
  obj[1] = items3;
  return closure_12(closure_4, obj);
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const searchMessageTimestamp = tmp2(15608).useSearchMessageTimestamp(message, channel);
    ...
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      ...
      tmp21Result = tmp21(tmp2(12442).BellZIcon, obj5);
    }
