...
function GuildChannelMessageRowHeader(channel) {
  ...
  const items1 = [callback(channel(1297).Icon, obj1), callback(channel(4281).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: importDefault(4475)(channel) }), , , ];
  if (muted) {
  ...
  if (isSystemDMResult) {
    ...
    obj4[0] = tmp6(9053).Types.SYSTEM_DM;
    isSystemDMResult = tmp10(tmp6(9053), obj4);
    const tmp6Result = tmp6(9053);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const items2 = [callback(message(4281).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
  ...
  if (isSystemDMResult) {
    ...
    obj2[0] = channel(9053).Types.SYSTEM_DM;
    isSystemDMResult = tmp9(channel(9053), obj2);
    const tmp13 = channel(9053);
  }
  items2[2] = isSystemDMResult;
  obj[1] = items2;
  const items3 = [closure_12(closure_4, obj), callback(message(4281).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  ({ colorString, colorStrings } = importDefault(4513)(message));
  if ("username" === stateFromStores) {
    if (null != colorString) {
      obj = { color: null };
      obj[0] = colorString;
    }
    let tmp2Result = tmp2(7880);
    ...
    obj3[4] = tmp22;
    obj3[5] = tmp7;
    items1[1] = callback(tmp2(4281).Text, obj3);
    obj1[1] = items1;
    ...
    obj4[5] = timestamp;
    items2[1] = callback(tmp2(4281).Text, obj4);
