...
import BellZIcon from "BellZIcon" /* 13314 */;
import useSearchMessageTimestamp from "useSearchMessageTimestamp" /* 16710 */;
import PollBadgeDefault from "PollBadge" /* 16711 */;
...
function GuildChannelMessageRowHeader(channel) {
  ...
  const items1 = [closure_11(channel(1178).Icon, obj1), closure_11(channel(4570).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: useChannelNameDefault(channel) }), , , ];
  if (muted) {
    const obj2 = { source: _modDef10177, size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = closure_11(tmp2(1178).Icon, obj2);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj3 = { source: _modDef10423, size: tmp2(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
  ...
  if (isSystemDMResult) {
    ...
    isSystemDMResult = closure_11(BotTagDefault, obj4);
...
function PrivateChannelMessageRowLabel(message) {
  ...
  if (muted) {
    const obj1 = { source: channel(10177), size: tmp4(1178).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = closure_11(tmp4(1178).Icon, obj1);
  }
  items2[1] = muted;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj2 = { type: channel(9470).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = closure_11(channel(9470), obj2);
    const tmp13 = channel(9470);
  }
  items2[2] = isSystemDMResult;
  obj.children = items2;
  const items3 = [closure_12(closure_4, obj), closure_11(message(4570).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj4 = { size: "xs", style: tmp.suppressNotificationsIcon };
    tmp9Result = closure_11(tmp4(13314).BellZIcon, obj4);
  }
  items3[2] = tmp9Result;
  tmp9Result = null;
  if (message.isPoll()) {
    const obj5 = { style: tmp.pollBadge };
    tmp9Result = closure_11(channel(16711), obj5);
  }
  items3[3] = tmp9Result;
  obj.children = items3;
  return closure_12(closure_4, obj);
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    if (tmp18) {
      ...
      tmp18 = closure_1_11(native.RoleDot, obj2);
    }
    const items1 = [tmp18, ];
    const obj3 = { variant: "text-sm/semibold", color: "interactive-text-active", lineClamp: 1, style: {}, gradientColors: null, children: null };
    let tmp22;
    ...
    const obj4 = { variant: "text-xs/medium", color: "text-default", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp };
    items2[1] = closure_1_11(Text_Text.Text, obj4);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj5 = { size: "xs", style: tmp.suppressNotificationsIcon };
      tmp21Result = closure_1_11(BellZIcon.BellZIcon, obj5);
    }
    items2[2] = tmp21Result;
    tmp21Result = null;
    if (message.isPoll()) {
      const obj6 = { style: tmp.pollBadge };
      tmp21Result = closure_1_11(PollBadgeDefault, obj6);
    }
...
export default noop.memo(function MessageRow(message) {
  ...
  stateFromStores2 = tmpResult.useStateFromStores(items2, () => UserGuildSettingsStore.isChannelMuted(guild_id, message.channel_id));
  const items3 = [, , ];
  ...
  if (null == stateFromStores) {
    ...
    if (isChannelSpoilerGated) {
      ...
      let tmp13Result = closure_11(tmp(stringResult[15]).Text, obj2);
    } else {
      const obj3 = { message, channel: stateFromStores, muted: false, layout: tmp(stringResult[30]).ChannelListLayoutTypes.COZY, color: "interactive-text-default", lineClamp, messageSizeCacheRef };
      tmp13Result = closure_11(tmp(stringResult[29]).NativeMessageChannelRowPreview, obj3);
    }
    obj.subLabel = tmp13Result;
    obj.onPress = callback;
    body = body.body;
    obj.bodyStyle = body;
    tmp13Result = closure_11(tmp(stringResult[27]).SearchListRow, obj);
  }
