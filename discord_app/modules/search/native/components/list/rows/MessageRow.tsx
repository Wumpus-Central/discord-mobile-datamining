// === Module 16184: GuildChannelMessageRowHeader ===
...
import useNullableMessageAuthorDefault from "useNullableMessageAuthor" /* 4712 */;
import PollBadgeDefault from "PollBadge" /* 16186 */;
...
function GuildChannelMessageRowHeader(channel) {
  ...
  const items1 = [callback(channel(1297).Icon, obj1), callback(channel(4442).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: computeChannelNameDefault(channel) }), , , ];
  if (muted) {
  ...
  if (isSystemDMResult) {
    ...
    obj4[0] = tmp6(8873).Types.SYSTEM_DM;
    isSystemDMResult = tmp10(tmp6(8873), obj4);
    const tmp6Result = tmp6(8873);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const items2 = [callback(message(4442).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
    obj1 = { source: null, size: null, style: null };
    obj1[0] = channel(10166);
  ...
  if (isSystemDMResult) {
    ...
    obj2[0] = channel(8873).Types.SYSTEM_DM;
    isSystemDMResult = tmp9(channel(8873), obj2);
    const tmp13 = channel(8873);
  }
  items2[2] = isSystemDMResult;
  obj[1] = items2;
  const items3 = [closure_12(closure_4, obj), callback(message(4442).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj4 = { size: "xs", style: null };
    obj4[1] = tmp.suppressNotificationsIcon;
    tmp9Result = tmp9(tmp4(12881).BellZIcon, obj4);
  }
  items3[2] = tmp9Result;
  tmp9Result = null;
  if (message.isPoll()) {
    ...
    tmp9Result = tmp9(channel(16186), obj5);
  }
  items3[3] = tmp9Result;
  obj[1] = items3;
  return closure_12(closure_4, obj);
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const searchMessageTimestamp = tmp2(16185).useSearchMessageTimestamp(message, channel);
    ...
    obj3[4] = tmp22;
    obj3[5] = tmp7;
    items1[1] = callback(tmp2(4442).Text, obj3);
    obj1[1] = items1;
    ...
    obj4[5] = timestamp;
    items2[1] = callback(tmp2(4442).Text, obj4);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj5 = { size: "xs", style: null };
      obj5[1] = tmp.suppressNotificationsIcon;
      tmp21Result = tmp21(tmp2(12881).BellZIcon, obj5);
    }
