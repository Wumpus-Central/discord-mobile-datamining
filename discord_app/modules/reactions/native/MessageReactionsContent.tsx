...
import SwipeableFastListDefault from "SwipeableFastList" /* 11335 */;
...
function EmojiOptionsButton(channelId) {
  ...
  obj = {
    onPress: noop.useCallback(() => {
      ...
      obj.openLazy(asyncRequireImpl(11333, dependencyMap.paths), "ReactionEmojiOptionsActionSheet", obj, "replaceTopSheet");
    }, items2),
  ...
  const items3 = [closure_17(channelId(4601).Text, obj), ];
  const tmp = closure_20();
...
function ReactionTabs(setReactionSelectedIndex) {
  ...
  obj = {
    ...
    initialNumTabsToRender: reactionSelectedIndex(4255).MAX_REACTIONS
  };
  ({ tab: obj3.tabStyle, tabActive: obj3.tabStyleActive, tabSelected: obj3.tabStyleSelected } = tmp);
  const items = [closure_17(setReactionSelectedIndex(11334), obj), closure_17(setReactionSelectedIndex(8669), { outer: true }), ];
  ...
  obj2.children = reactionSelectedIndex(4255).getReactionEmojiName(reactions[reactionSelectedIndex].emoji);
  const items1 = [closure_17(reactionSelectedIndex(4601).Text, obj2), ];
  const obj3 = {};
...
export const MessageReactionsContent = function MessageReactionsContent(channelId) {
  ...
  obj = { scrollable: true, backdropOpacity: tmp10(7835).BACKDROP_OPACITY, backdropChildren: null, header: null, children: null };
  ...
  return closure_17(tmp10(7202).BottomSheet, obj);
};