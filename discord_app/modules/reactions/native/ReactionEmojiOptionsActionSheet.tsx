...
export default function ReactionEmojiOptionsActionSheet(channelId) {
  ...
  const stateFromStores1 = channelId(emoji[10]).useStateFromStores(
    items1,
    () => {
  ...
  const items9 = [
    closure_9(messageId(emoji[24]), {
      src: emojiURL,
      name: str,
      textEmojiStyle: tmp.emojiText,
      fastImageStyle: tmp.emoji,
    }),
    closure_9(channelId(emoji[16]).Text, {
  ...
  items10[1] = closure_9(channelId(emoji[16]).Text, {
  ...
  if (!tmp24) {
    ...
    if (tmp21Result) {
      ...
      tmp21Result = closure_9(tmp3(tmp4[26]).TableRow, obj12);
    }
    ...
    if (canRemoveReactions) {
      ...
      obj14.label = closure_9(tmp3(tmp4[16]).Text, obj15);
    ...
    items11[1] = closure_10(tmp3(tmp4[25]).TableRowGroup, obj17);
    obj16.children = items11;
    return closure_10(tmp3(tmp4[23]).ActionSheet, obj16);
  } else {
