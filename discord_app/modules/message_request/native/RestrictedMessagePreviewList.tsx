...
import RestrictedBlockedMessageGroupDefault from "RestrictedBlockedMessageGroup.tsx";
...
function HiddenMedia(length) {
  ...
  const items1 = [
    closure_6(require("CircleInformationIcon").CircleInformationIcon, { size: "sm", color: "text-muted" }),
  ];
...
let obj2 = {
  ...
  hiddenMedia: { marginLeft: fn(16718).RESTRICTED_CONTENT_INSET },
...
let size = {
  ...
  top: 0,
  left: 0,
  width: fn(16718).RESTRICTED_CONTENT_INSET,
  height: fn(16718).RESTRICTED_AVATAR_SIZE,
};
...
export default function RestrictedMessagePreviewList(channelId) {
  ...
  const stateFromStoresArray = channelId(analyticsLocations[13]).useStateFromStoresArray(
    items,
    () => {
      ...
      if (null == arr5[index - 1]) {
        ...
        if (tmp6Result) {
          ...
          const obj4 = {
            ...
            color: "text-muted",
            children: DateUtils.dateFormat(message.timestamp, "LL"),
          };
