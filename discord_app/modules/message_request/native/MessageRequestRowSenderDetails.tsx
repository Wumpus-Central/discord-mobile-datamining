// === Module 17426: MessageRequestRowSenderDetails ===

// Module 17426 (MessageRequestRowSenderDetails)
import nativeDefault from "native" /* 576 */;
import utils_AvatarUtilsDefault from "utils/AvatarUtils" /* 1400 */;
import UserUtilsDefault from "UserUtils" /* 4671 */;
import MessageRequestPreviewDefault from "MessageRequestPreview" /* 17428 */;
import MessageRequestMutualServersDefault from "MessageRequestMutualServers" /* 17429 */;
...
export default function MessageRequestRowSenderDetails(isRestricted) {
  ...
  const messageRequestRelativeTimestampText = otherUser(17427).useMessageRequestRelativeTimestampText(channel);
  ...
  const items2 = [closure_5(otherUser(4823).Text, obj9), ];
  let tmp9Result = null != stateFromStores;
  if (tmp9Result) {
    const obj10 = { variant: "text-md/medium", color: "text-muted", children: null };
    const items3 = [" ", otherUser(4671).getUserTag(otherUser)];
    obj10.children = items3;
    tmp9Result = closure_6(otherUser(4823).Text, obj10);
    const tmp2Result = otherUser(4671);
  }
  items2[1] = tmp9Result;
  obj8.children = items2;
  const items4 = [closure_6(otherUser(4823).Text, obj8), closure_5(otherUser(4823).Text, { style: tmp.timestampSeparator, variant: "text-xs/medium", color: "text-muted", children: "\u00B7" }), closure_5(otherUser(4823).Text, { variant: "text-xs/semibold", color: "text-muted", children: messageRequestRelativeTimestampText })];
