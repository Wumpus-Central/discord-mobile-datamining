// === Module 9220: authorizeConnection ===
...
export default function authorizeConnection(overrideUrl) {
  ...
  if (platformType === PlatformTypes.XBOX) {
    overrideUrl(4412).hideActionSheet();
    const obj15 = overrideUrl(4412);
    const tmp23 = overrideUrl;
    const items = [_location];
    overrideUrl(9221).showModal(items);
    if (null != onClose) {
    ...
    if (platformType !== tmp.PLAYSTATION) {
      if (platformType !== tmp.PLAYSTATION_STAGING) {
        if (platformType === tmp.CRUNCHYROLL) {
          ...
          const obj12 = overrideUrl(9260);
        } else if (platformType === tmp.DOMAIN) {
          overrideUrl(4412).hideActionSheet();
          const obj8 = overrideUrl(4412);
          const tmp10 = overrideUrl;
          let obj = { locationStack: null };
          const items2 = [_location];
          obj[0] = items2;
          overrideUrl(4676).pushLazy(handleModalClose5(2009)(9271, dependencyMap.paths), obj);
          if (null != onClose) {
          ...
          const obj9 = overrideUrl(4676);
        } else {
          const value = overrideUrl(5179).get(platformType);
          let isFederated;
          if (value != null) {
            isFederated = value.isFederated;
          }
          if (true === isFederated) {
            let tmp28Result = tmp28(4412);
            tmp28Result.hideActionSheet();
            tmp28Result = tmp28(4676);
            obj = { platformType: null, location: null, successRedirect: null };
            obj[0] = platformType;
            obj[1] = _location;
            obj[2] = successRedirect;
            tmp28Result.pushLazy(handleModalClose5(2009)(9273, tmp29.paths), obj);
            if (null != onClose) {
            ...
            if (null != overrideUrl) {
              if (platformType === closure_3) {
                let obj2 = handleModalClose5(8572);
            ...
            const tmp28Result2 = tmp28(5302);
            tmp28(5302).authorize(platformType, obj2).then((body) => {
            ...
            const authorizeResult = tmp28(5302).authorize(platformType, obj2);
          }
          const obj18 = overrideUrl(5179);
          tmp29 = dependencyMap;
        }
      }
    }
    overrideUrl(4412).hideActionSheet();
    const obj13 = overrideUrl(4412);
    const tmp19 = overrideUrl;
    const items3 = [_location];
    overrideUrl(9249).showModal(items3, platformType);
    if (null != onClose) {
