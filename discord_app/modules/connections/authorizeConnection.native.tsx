// === Module 10565: authorizeConnection ===
...
export default function authorizeConnection(overrideUrl) {
  ...
  if (platformType === PlatformTypes.XBOX) {
    overrideUrl(4347).hideActionSheet();
    const obj15 = overrideUrl(4347);
    const tmp23 = overrideUrl;
    const items = [_location];
    overrideUrl(10566).showModal(items);
    if (null != onClose) {
    ...
    if (platformType !== tmp.PLAYSTATION) {
      if (platformType !== tmp.PLAYSTATION_STAGING) {
        if (platformType === tmp.CRUNCHYROLL) {
          ...
          const obj12 = overrideUrl(10642);
        } else if (platformType === tmp.DOMAIN) {
          overrideUrl(4347).hideActionSheet();
          const obj8 = overrideUrl(4347);
          const tmp10 = overrideUrl;
          let obj = { locationStack: null };
          const items2 = [_location];
          obj[0] = items2;
          overrideUrl(4611).pushLazy(handleModalClose5(2009)(10653, dependencyMap.paths), obj);
          if (null != onClose) {
          ...
          const obj9 = overrideUrl(4611);
        } else {
          const value = overrideUrl(5108).get(platformType);
          let isFederated;
          if (value != null) {
            isFederated = value.isFederated;
          }
          if (true === isFederated) {
            let tmp28Result = tmp28(4347);
            tmp28Result.hideActionSheet();
            tmp28Result = tmp28(4611);
            obj = { platformType: null, location: null, successRedirect: null };
            obj[0] = platformType;
            obj[1] = _location;
            obj[2] = successRedirect;
            tmp28Result.pushLazy(handleModalClose5(2009)(10655, tmp29.paths), obj);
            if (null != onClose) {
            ...
            if (null != overrideUrl) {
              if (platformType === closure_3) {
                let obj2 = handleModalClose5(9079);
            ...
            const tmp28Result2 = tmp28(5231);
            tmp28(5231).authorize(platformType, obj2).then((body) => {
            ...
            const authorizeResult = tmp28(5231).authorize(platformType, obj2);
          }
          const obj18 = overrideUrl(5108);
          tmp29 = dependencyMap;
        }
      }
    }
    overrideUrl(4347).hideActionSheet();
    const obj13 = overrideUrl(4347);
    const tmp19 = overrideUrl;
    const items3 = [_location];
    overrideUrl(10631).showModal(items3, platformType);
    if (null != onClose) {
