...
export default function authorizeConnection(overrideUrl) {
  ...
  if (platformType === PlatformTypes.XBOX) {
    ...
    overrideUrl(10492).showModal(items);
    if (null != onClose) {
    ...
    if (platformType !== tmp.PLAYSTATION) {
      if (platformType !== tmp.PLAYSTATION_STAGING) {
        if (platformType === tmp.CRUNCHYROLL) {
          ...
          overrideUrl(10568).showModal(items1);
          if (null != onClose) {
          ...
          obj[0] = items2;
          overrideUrl(4509).pushLazy(handleModalClose5(1988)(10579, dependencyMap.paths), obj);
          if (null != onClose) {
          ...
          if (true === isFederated) {
            ...
            tmp28Result.pushLazy(handleModalClose5(1988)(10581, tmp29.paths), obj);
            if (null != onClose) {
    ...
    overrideUrl(10557).showModal(items3, platformType);
    if (null != onClose) {
