// === Module 13046: ContactSyncSuggestions ===
...
let obj2 = { container: { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5987).NAV_BAR_HEIGHT + 32, justifyContent: "center" }, header: null, title: null, subtitle: null, list: null, divider: null, linearGradient: null, redesignButton: null, sectionHeader: null };
let obj3 = { flex: 1, backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWER, paddingTop: fn(5987).NAV_BAR_HEIGHT + 32, justifyContent: "center" };
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  const token = friendSuggestions(4526).useToken(onSubmit(576).colors.BACKGROUND_BASE_LOW);
  let obj = friendSuggestions(4526);
  let items = [friendSuggestions(4678).hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4678);
  items[1] = friendSuggestions(4678).hexOpacityToRgba(token, 100);
  const tmp4 = first(noop.useState(reduced), 2);
  first = tmp4[0];
  noop = tmp4[1];
  let obj3 = friendSuggestions(4678);
  const fontScale = friendSuggestions(5280).useFontScale();
  ...
  let obj4 = friendSuggestions(5280);
  ...
  const items5 = [closure_8(friendSuggestions(9077).FlashList, obj7), , ];
  ...
  obj9.colors = items;
  items5[1] = closure_8(onSubmit(5285), obj9);
  ...
  obj10.children = closure_8(friendSuggestions(5273).Button, obj11);
