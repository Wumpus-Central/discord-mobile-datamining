// === Module 11905: SuggestedFriendsSectionHeader ===
...
function SuggestedFriendsSectionHeader(children) {
  return callback(View, { style: createCacheKey().sectionHeader, children: callback(require(4251) /* Text */.Text, { color: "text-muted", variant: "text-sm/semibold", children: children.label }) });
}
function SuggestionRow(suggestion) {
  ...
  obj[3] = selected;
  obj[4] = tmp6;
  const items = [callback(suggestion(4251).Text, { variant: "text-xs/medium", color: "text-muted", children: userTag }), ];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4251).Text, obj1);
  }
  items[1] = tmp8Result;
  ...
  return callback(suggestion(9272).TableCheckboxRow, obj);
}
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  obj[1] = memo;
  obj[2] = callback1;
  const items5 = [callback(friendSuggestions(7654).FlashList, obj), , ];
  ...
  obj2[4] = items;
  items5[1] = callback(onSubmit(4677), obj2);
  ...
  obj4[4] = !someResult;
  obj3[1] = callback(friendSuggestions(4666).Button, obj4);
