...
function SuggestionRow(suggestion) {
  ...
  obj[3] = selected;
  obj[4] = tmp6;
  const items = [callback(suggestion(4441).Text, { variant: "text-xs/medium", color: "text-muted", children: userTag }), ];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4441).Text, obj1);
  }
  items[1] = tmp8Result;
  ...
  return callback(suggestion(7616).TableCheckboxRow, obj);
}
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  obj[1] = memo;
  obj[2] = callback1;
  const items5 = [callback(friendSuggestions(8037).FlashList, obj), , ];
  ...
  obj2[4] = items;
  items5[1] = callback(onSubmit(4892), obj2);
  ...
  obj4[4] = !someResult;
  obj3[1] = callback(friendSuggestions(4880).Button, obj4);
