...
function SuggestionRow(suggestion) {
  ...
  obj[3] = selected;
  obj[4] = tmp6;
  const items = [callback(suggestion(4338).Text, { variant: "text-xs/medium", color: "text-muted", children: userTag }), ];
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp3(4338).Text, obj1);
  }
  items[1] = tmp8Result;
...
export default function ContactSyncSuggestions(friendSuggestions) {
  ...
  let obj1 = friendSuggestions(4191);
  let items = [obj1.hexOpacityToRgba(token, 0), ];
  let obj2 = friendSuggestions(4191);
