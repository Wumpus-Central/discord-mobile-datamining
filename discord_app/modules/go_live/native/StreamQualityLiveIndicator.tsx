...
const createCacheKey = { borderBottomLeftRadius: ThemesDefault.radii.none, borderTopLeftRadius: ThemesDefault.radii.none, height: 18, textAlign: "center" };
...
const result = require("obj132").fileFinishedImporting("modules/go_live/native/StreamQualityLiveIndicator.tsx");
...
export default function StreamQualityLiveIndicator(arg0) {
  ...
  let obj = { withTiming: _require(reveal[12]).withTiming, reveal, STANDARD_EASING: _require(reveal[13]).STANDARD_EASING };
  fn.__closure = obj;
  ...
  try {
    ...
    try {
      ...
      const effect = obj.useEffect(() => {
        ...
        if (tmp2) {
          _undefined2(reveal[16]);
          const obj = { type: null, has_premium_stream_fps: null, has_premium_stream_resolution: null, location_stack: null };
      ...
      if (null != maxQuality) {
        ...
        if (tmp21Result) {
          ...
          tmp21Result = callback(tmp5(tmp4[21]), obj3);
        ...
        items4[1] = callback(tmp3(tmp4[13]).LegacyText, obj4);
        obj2[1] = items4;
        const items6 = [callback2(tmp23, obj2), ];
        const obj5 = { style: null };
        obj5[0] = tmp2.liveTag;
        items6[1] = callback(tmp3(tmp4[13]).LiveTag, obj5);
        obj1[3] = items6;
        obj[2] = callback2(tmp3(tmp4[20]).PressableOpacity, obj1);
        tmp21Result = callback(tmp5(tmp4[11]).View, obj);
