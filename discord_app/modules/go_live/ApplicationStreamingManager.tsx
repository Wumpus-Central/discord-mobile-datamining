...
const prototype = function BaseApplicationStreamingManager() {
                ...
                if (null != streamForUser) {
                  ...
                  if (encodeStreamKeyResult !== c17) {
                    ...
                    if (!isStreamMarkedFullResult) {
                      ...
                      applyArgumentsResult(4864).watchStream(streamForUser, { noFocus: true });
                      const tmp2Result = applyArgumentsResult(4864);
                    }
                  }
                    ...
                    if (null != streamForUser) {
                      ...
                      if (tmp16) {
                        ...
                        if (!isStreamMarkedFullResult) {
                          ...
                          applyArgumentsResult(4864).watchStream(streamForUser, { noFocus: true });
                          flag2 = true;
                          const tmp12Result = applyArgumentsResult(4864);
                        }
                        tmp16 = flag2;
                      }
                      const obj2 = applyArgumentsResult(4774);
                ...
                if (activeStreamForUser.state === constants.ENDED) {
                  ...
                  if (null != streamForUser1) {
                    if (!authStore.isStreamMarkedFull(tmp34Result.encodeStreamKey(streamForUser1))) {
                      applyArgumentsResult(4864).watchStream(streamForUser1);
                      const tmp34Result2 = applyArgumentsResult(4864);
                    }
                    tmp34Result = applyArgumentsResult(4774);
                  }
                }
              }
            }
  ...
  applyArgumentsResult.handleCallUpdate = function handleCallUpdate(region) {
    ...
    if (channelId === region.channelId) {
      ...
      if (tmp7) {
        applyArgumentsResult(4864).changeStreamRegion(encodeStreamKeyResult, region);
        const tmp3Result = applyArgumentsResult(4864);
      }
      const obj = applyArgumentsResult(4774);
    }
  };
