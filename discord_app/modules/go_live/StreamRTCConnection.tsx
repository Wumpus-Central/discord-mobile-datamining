...
import getEventHistoryStringDefault from "getEventHistoryString" /* 4521 */;
import traceEventDefault from "traceEvent" /* 4541 */;
import startDefault from "start" /* 4542 */;
import removeExecutablePathPrefix from "removeExecutablePathPrefix" /* 4618 */;
import getSoundshareAnalyticsContextDefault from "getSoundshareAnalyticsContext" /* 4624 */;
import getStreamSourceMetadataDefault from "getStreamSourceMetadata" /* 4625 */;
import closure_3 from "_migrateDefaultStorage" /* 4502 */;
...
import closure_8 from "getHookError" /* 4538 */;
import closure_9 from "_detectH265HardwareDecode" /* 4496 */;
import closure_10 from "handleConnectionInfoChange" /* 4539 */;
import closure_11 from "createRTCConnection" /* 4519 */;
import closure_12 from "initialize" /* 4540 */;
...
prototype["initializeEvents"] = function initializeEvents() {
  ...
  this.on(_require(4543).RTCConnectionEvent.State, (state) => {
  ...
  this.on(_require(4543).RTCConnectionEvent.Video, (arg0, arg1, arg2, arg3, arg4) => {
  ...
  this.on(_require(4543).RTCConnectionEvent.VideoSourceQualityChanged, (guildId, channelId, senderUserId, maxResolution, maxFrameRate, context) => {
    let obj = self(709);
    obj = { type: "MEDIA_ENGINE_VIDEO_SOURCE_QUALITY_CHANGED", guildId, channelId, senderUserId, maxResolution, maxFrameRate, context };
    obj.dispatch(obj);
  });
  this.on(_require(4543).RTCConnectionEvent.SecureFramesUpdate, () => {
    self(709).dispatch({ type: "RTC_CONNECTION_SECURE_FRAMES_UPDATE" });
  });
  this.on(_require(4543).RTCConnectionEvent.RosterMapUpdate, (userIds) => {
    let obj = self(709);
...
prototype["trackVideoEndStats"] = function trackVideoEndStats(arg0) {
  ...
  if (null != _videoQuality) {
    ...
    obj[0] = obj2(4626).default.getApplicationNames();
    if (self.isOwner) {
      obj = { clips_enabled: null, clips_buffer_length: null };
      obj[0] = tmp5(4506).isClipsEnabled();
    ...
    const item = outboundStats.forEach((num_frames) => {
      ...
      if (num > 0) {
        ...
        obj.app_hardware_acceleration_enabled = callback(4439).getAppHardwareAccelerationEnabled();
        ...
        const obj4 = callback(4439);
        const obj5 = closure_1_9;
        obj.discord_is_elevated = callback(4439).getDiscordIsElevated();
        obj2.track(closure_1_13.VIDEO_STREAM_ENDED, obj);
        const tmp4Result = callback(4439);
      }
    });
    ...
    const item1 = _videoQuality.forEach((arg0) => {
      ...
      if (num > 0) {
        ...
        const obj3 = callback(4439);
        tmp4 = callback;
      }
    });
    const _default = obj2(4626).default;
    tmp5 = obj2;
  }
};
