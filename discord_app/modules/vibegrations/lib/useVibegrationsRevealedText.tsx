// discord_app/modules/vibegrations/lib/useVibegrationsRevealedText.tsx
import VibegrationsStreamReveal from "VibegrationsStreamReveal.tsx";
import vibegrationsPageVisibility from "vibegrationsPageVisibility.native.tsx";
...
export const useVibegrationsRevealedText = function useVibegrationsRevealedText(source, streaming) {
  ...
  if (tmp2.target === source) {
    ...
    if (streaming) {
      ({ target, length } = tmp2);
      let length2 = target(17032).reconcileRevealedLength(target, source, length);
      const targetResult = target(17032);
    } else {
