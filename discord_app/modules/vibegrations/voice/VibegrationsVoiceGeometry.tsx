// discord_app/modules/vibegrations/voice/VibegrationsVoiceGeometry.tsx
...
export const toListenerRelativePosition = function toListenerRelativePosition(forward, position) {
  ...
  if (Math.sqrt(diff6 * diff6 + diff7 * diff7 + diff8 * diff8) < 0.001) {
    ...
    point7 = {
      x: diff6 * point3.x + diff7 * point3.y + diff8 * point3.z,
      y: diff6 * point4.x + diff7 * point4.y + diff8 * point4.z,
      z: -diff6 * point.x + diff7 * point.y + diff8 * point.z,
    };
  }
  return point7;
};
