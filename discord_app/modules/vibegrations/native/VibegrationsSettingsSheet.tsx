...
function SettingsTabStrip(tabs) {
  ...
  if (tabs.length > 3) {
    ...
    let tmp8Result = closure_10(tmp5(12093).Tabs, obj4);
  } else {
    const obj5 = { state: segmentedControlState };
    tmp8Result = closure_10(tmp5(9073).SegmentedControl, obj5);
  }
...
let obj = {
  project: _modDef3714.wo1EUp,
  app: _modDef3714["8drwHu"],
  secrets: _modDef3714.iD7xfZ,
  model: _modDef3714.aMTBSX,
};
...
export default function VibegrationsSettingsSheet(projectId) {
  ...
  const stateFromStores2 = projectId(504).useStateFromStores(
    items4,
    () => "open" === VibegrationsConnectionStore.getConnState(projectId),
    items5,
  );
  if (tmp8) {
  ...
  obj5.header = found(projectId(6565).BottomSheetTitleHeader, {
  ...
  const tmp24 = loaded;
  items9[6] = found(projectId(5274).Button, {
    text: intl4.string(Tuz9vw),
    variant: "primary",
