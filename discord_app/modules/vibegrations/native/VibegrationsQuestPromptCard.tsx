...
export default function VibegrationsQuestPromptCard(open) {
  ...
  const callback1 = noop.useCallback(asyncGeneratorStep(async () => {
    if (c3 === 2) {
      c3 = 3;
  ...
  if (tmp17) {
    if (null != tmp29) {
      ...
      const url = tmp2Result13.getQuestAsset(tmp29, tmp2(11546).QuestAssetType.GAME_TILE, tmp2(4464).isThemeDark(stateFromStores1) ? ThemeTypes.DARK : ThemeTypes.LIGHT).url;
