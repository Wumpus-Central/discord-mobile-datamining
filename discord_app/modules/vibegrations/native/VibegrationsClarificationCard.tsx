// === Module 15926: VibegrationsClarificationCard ===
...
export default function VibegrationsClarificationCard(clarification) {
  ...
  if (clarification.questions.length > 1) {
    ...
    let intl = clarification(1236).intl;
    obj1 = { index: null, total: null };
    obj1[0] = bound + 1;
    obj1[1] = length;
    obj[2] = intl.formatToPlainString(onSubmit(3404)["7bypa+"], obj1);
    tmp14 = callback(clarification(4739).Text, obj);
  }
  const items4 = [tmp14, callback(clarification(4739).Text, { variant: "text-md/semibold", color: "text-default", children: clarification.questions[bound].question }), , ];
  ...
  if (bound > 0) {
    ...
    if (!tmp6) {
      ...
      obj4[2] = intl2.string(onSubmit(3404).yKdgqw);
      obj4[3] = callback1;
      tmp19Result = tmp19(tmp20(4750).Button, obj4);
    }
  }
  ...
  obj6[2] = intl3.string(onSubmit(3404).qifsdL);
  obj6[3] = str;
  ...
  obj6[5] = callback2;
  obj5[1] = callback(clarification(13592).GhostInput, obj6);
