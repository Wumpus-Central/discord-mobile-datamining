...
export default function VibegrationsNativeComposer(projectId) {
  ...
  let tmp2 = onInterrupt(
    str.useState(() => VibegrationsComposerDraftStore.getDraft(closure_0)),
    2,
  );
  str = tmp2[0];
  closure_6 = tmp3;
  ...
  const tmp5 = onInterrupt(str.useState(null), 2);
        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              const obj5 = {
                mediaType: "any",
                selectionLimit: tmp2(7359).VIBEGRATIONS_MAX_ATTACHMENTS_PER_MESSAGE,
                skipProcessing: true,
              };
              dependencyMap = 1;
              c3 = 1;
  ...
  callback10 = obj.useCallback((key) => {
    if ("stop" === key.key) {
      const obj2 = {
        style: closure_13.trailingButton,
  ...
  if (arr3.length > 0) {
    const obj9 = {
      ...
      children: arr3.map((children) => {
        ...
        const items1 = [
          closure_13(projectId(running[26]).Text, {
        ...
        if (null != children.errorText) {
          ...
          tmp10Result = closure_13(tmp11(running[26]).Text, obj6);
        }
