// discord_app/modules/vibegrations/native/VibegrationsTemplateWizardSheet.tsx
...
import VibegrationsCreateErrors from "../lib/VibegrationsCreateErrors.tsx";
import VibegrationsTemplateWizard from "../lib/VibegrationsTemplateWizard.tsx";
...
export default function VibegrationsTemplateWizardSheet(template) {
  ...
  const stateFromStores = template(504).useStateFromStores(items, () =>
    template(17013).vibegrationsTemplateWizardGuilds(first.getGuildsArray(), "VibegrationsTemplateWizardSheet"),
  );
  ...
  const obj7 = {
    ...
    onDismiss: callback,
    header: tmp31(template(7480).BottomSheetTitleHeader, { title: name }),
    children: null,
  };
  ...
  if ("about" === tmp23) {
    ...
    if (null != result) {
      ...
      items7[1] = points.map((children, index) => {
        ...
        const items = [
          map1(obj9[children.icon], {
            size: "sm",
            color: nativeDefault.colors.ICON_DEFAULT,
            style: closure_4.pointIcon,
          }),
        ];
        const obj3 = { style: closure_4.point, children: null };
        const items1 = [
          map1(Text_Text.Text, { variant: "text-md/normal", color: "text-default", children: children.title }),
        ];
        let tmp3Result = null;
  ...
  const tmp2Result8 = template(17013);
}
