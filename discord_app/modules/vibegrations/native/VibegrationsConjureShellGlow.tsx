// discord_app/modules/vibegrations/native/VibegrationsConjureShellGlow.tsx
...
pulseAt.__initData = {
  code: "function pulseAt_VibegrationsConjureShellGlowTsx1(progress){const{PULSE_KEYFRAMES}=this.__closure;for(let i=1;i<PULSE_KEYFRAMES.length;i++){const[t1,v1]=PULSE_KEYFRAMES[i];if(progress<=t1){const[t0,v0]=PULSE_KEYFRAMES[i-1];const span=t1-t0;const local=span>0?(progress-t0)/span:1;return v0+(v1-v0)*local;}}return 1;}",
};
...
let closure_22 = createStyles.createStyles({
  ...
  sweep: { position: "absolute", top: 0, bottom: 0, left: 0, width: "200%" },
  fill: { flex: 1 },
  ditherClip: { position: "absolute", left: 0, right: 0, bottom: 0, overflow: "hidden" },
  ditherField: { position: "absolute", left: 0, right: 0, bottom: 0, height: 500 },
});
const __initData = {
  code: "function VibegrationsConjureShellGlowTsx2(){const{lift,pulseAt,pulse,BAND_HEIGHT}=this.__closure;const scale=lift.get()*pulseAt(pulse.get());return{transform:[{translateY:BAND_HEIGHT*(1-scale)/2},{scaleY:scale}]};}",
};
const __initData2 = {
  code: "function VibegrationsConjureShellGlowTsx3(){const{chromaMix,layerAlpha,driftBase,width}=this.__closure;return{opacity:(1-chromaMix.get())*layerAlpha,transform:[{translateX:-driftBase.get()*width}]};}",
};
const __initData3 = {
  code: "function VibegrationsConjureShellGlowTsx4(){const{chromaMix,layerAlpha,driftChroma,width}=this.__closure;return{opacity:chromaMix.get()*layerAlpha,transform:[{translateX:-driftChroma.get()*width}]};}",
};
const __initData4 = {
  code: "function VibegrationsConjureShellGlowTsx5(){const{ANDROID,BAND_HEIGHT,lift,pulseAt,pulse,ditherHeight}=this.__closure;return{height:ANDROID?BAND_HEIGHT*lift.get()*pulseAt(pulse.get()):ditherHeight.get()};}",
};
let size = fn(2);
...
export default function VibegrationsConjureShellGlow(thinking) {
  ...
  const memo1 = num2.useMemo(() => {
    ...
    if (c3) {
      ...
      items = [
        ...
        unsafe_rawColors.BG_GRADIENT_MIDNIGHT_BLURPLE_2,
        BG_GRADIENT_MIDNIGHT_BLURPLE_1,
      ];
      obj.base = items;
      const items1 = [
        ...
        unsafe_rawColors.BG_GRADIENT_CHROMA_GLOW_3,
        BG_GRADIENT_CHROMA_GLOW_2,
        BG_GRADIENT_CHROMA_GLOW_1,
      ];
      obj.chroma = items1;
  ...
  const effect = obj4.useEffect(() => {
    ...
    const result1 = sharedValue1.set(
      timing.withTiming(num2, { duration: 700, easing: bezierResult }, "animate-always"),
    );
    ...
    const result2 = sharedValue2.set(
      timing.withTiming(num3, { duration: 400, easing: bezierResult }, "animate-always"),
    );
  ...
  Z.__closure = {
    ANDROID: sharedValue5,
    BAND_HEIGHT: v500,
    lift: sharedValue,
    pulseAt,
    pulse: sharedValue3,
    ditherHeight: sharedValue2,
  };
  ...
  const obj9 = {
    style: null,
    children: sharedValue2(stateFromStores(stateFromStores1[14]), {
      style: tmp.fill,
      colors: memo1.base,
      start: start2,
      end: end2,
