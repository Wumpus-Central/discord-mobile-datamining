// discord_app/modules/vibegrations/native/VibegrationsConjureShimmer.tsx
...
const __initData = {
  code: "function VibegrationsConjureShimmerTsx1(){const{bandWidth,progress,width}=this.__closure;return{transform:[{translateX:-bandWidth+progress.get()*(bandWidth+width)}]};}",
};
const size = fn(2);
...
export default function VibegrationsConjureShimmer(epoch) {
  ...
  let obj = tint(504);
  sharedValue = tint(4559).useSharedValue(0);
  if (live) {
  ...
  const animatedStyle = tint(4559).useAnimatedStyle(W);
  let obj4 = {
    style: tmp.root,
  ...
  if (live) {
    ...
    obj8.children = closure_8(width(5283), obj10);
    obj6.children = closure_8(width(4559).View, obj8);
    obj5.children = closure_8(width(5967), obj6);
    tmp14 = closure_8(closure_6, obj5);
    const tmp18 = width(5967);
  }
  items3[1] = tmp14;
