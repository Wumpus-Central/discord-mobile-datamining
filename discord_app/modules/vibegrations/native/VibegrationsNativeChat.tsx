...
import VibegrationsNativeMarkdown from "VibegrationsNativeMarkdown.tsx";
...
import VibegrationsSubagentMark from "VibegrationsSubagentMark.tsx";
...
import VibegrationsChatGrouping from "../lib/VibegrationsChatGrouping.tsx";
import vibegrations_VibegrationsRepliedMessage from "../lib/VibegrationsRepliedMessage.tsx";
...
function PlanDesign(arg0) {
  ...
  if (vibegrationsPlanDesign.gone) {
    ...
    if (null == src) {
      ...
      obj3.children = closure_1_17(hasOwnProperty, obj4);
      let tmp9Result = closure_1_17(React6, obj3);
    } else {
      const obj5 = {
        source: null,
        style: null,
      ...
      tmp9Result = closure_1_17(timestampProducer, obj5);
    }
    const obj7 = { direction: "vertical", spacing: 4, children: null };
    items[1] = tmp9Result;
    obj7.children = items;
    return collapsedCategories(Stack_Stack.Stack, obj7);
  }
}
function ProposalCard(proposal) {
  ...
  if (null != proposal.design_image) {
    ...
    tmp3Result = closure_1_17(PlanDesign, obj3);
  }
  items[2] = tmp3Result;
  let tmp5Result = null;
  if (proposal.changes.length > 0) {
    ...
    obj5.children = intl3.string(_modDef3714.KLyB8Y);
    ...
    tmp5Result = collapsedCategories(Stack_Stack.Stack, obj4);
  }
  items[3] = tmp5Result;
  ...
  if (bot_permissions.length > 0) {
    ...
    obj7.children = intl4.string(_modDef3714.ieqTtP);
    ...
    tmp5Result3 = collapsedCategories(Stack_Stack.Stack, obj6);
  }
  items[4] = tmp5Result3;
  ...
  if (privileged_intents.length > 0) {
    ...
    obj10.children = intl5.string(_modDef3714.Cn9qix);
    ...
    tmp5Result4 = collapsedCategories(Stack_Stack.Stack, obj9);
  }
  items[5] = tmp5Result4;
  ...
  if (actionable) {
    ...
    obj12.text = intl6.string(_modDef3714.GYoWRk);
  ...
  items = [
    ...
    ideas.map((title) => {
      ...
      if ("" !== title.value) {
        ...
        tmpResult = closure_1_17(Text_Text.Text, obj4);
      }
      items[1] = tmpResult;
...
function AttachmentPills(projectId) {
  ...
  dependencyMap = noop.useCallback((arg0) => {
    ...
    map1(projectId, arg0)
      .then((result) => closure_1_1(dependencyMap[21]).openURL(result))
      .catch(() => {});
  }, items);
...
function TimelineRow(live) {
  ...
  if (null != node.durationMs) {
    const obj3 = {
      ...
      color: "text-subtle",
      children: VibegrationsDuration.describeDuration(node.durationMs),
    };
    tmp4Result = closure_1_17(Text_Text.Text, obj3);
    const tmp7Result = VibegrationsDuration;
  }
  ...
  if (node.detail.length > 0) {
    ...
    obj4.children = detail.map((children, index) =>
      closure_1_17(Text_Text.Text, { variant: "text-sm/normal", color: "text-muted", children }, index),
    );
    tmp4Result2 = closure_1_17(React6, obj4);
  }
  children[1] = tmp4Result2;
  return collapsedCategories(React6, { children });
}
function TurnStatusLine(epoch) {
  ...
  if (null != tmp8) {
    groupLabel = turnActive(16322).describeTurnDuration(tmp8);
    const tmp5Result = turnActive(16322);
  } else if (null != currentStepResult) {
    groupLabel = turnActive(16321).describeNode(currentStepResult);
    const tmp5Result2 = turnActive(16321);
  ...
  if (tmp3) {
    ...
    if (someResult) {
      ...
      tmp17Result = closure_17(closure_8, obj3);
    }
  }
...
function LaneStatusLine(arg0) {
  ...
  if ("running" === lane.task.status) {
    ...
    const obj5 = {
      line: mark(16323).describeTaskOutcome(lane.task),
      live: turnActive,
      settled: null,
      failed: null,
      glyph: null,
    ...
    if (tmp3) {
      ...
      if (tmp9) {
        ...
        items1[1] = steps.map((node) =>
          closure_2_17(TimelineRow, { node, live: node === c3, crestColor: mark.tint, epoch }, node.id),
        );
        obj6.children = items1;
        tmp21Result = closure_18(closure_8, obj6);
      }
    }
...
({
  ...
  interruptTurn: closure_14,
  sendUserMessage: closure_15,
...
obj.row = {
  position: "relative",
  paddingLeft: fn(16307).MESSAGE_CONTENT_INSET,
  paddingRight: fn(16307).MESSAGE_EDGE_INSET,
  paddingVertical: 2,
  gap: PX_8,
};
...
let closure_35 = noop.memo((onToggleChecklist) => {
  ...
  const memo5 = onJumpToReplied.useMemo(
    () => VibegrationsDesignFeedback.parseVibegrationsDesignRemark(message.content),
    items7,
  );
  let body;
  if (memo5 != null) {
  ...
  if ("user" === message.role) {
    ...
    if (tmp12) {
      let combined;
      if (!groupStart) {
        const intl3 = message(tmp90[13]).intl;
        const _HermesInternal = HermesInternal;
        combined = "" + intl3.string(message(tmp90[13]).t.KD6OJJ) + ": " + trimmed;
      }
      ...
      let tmp76Result = closure_18(message(onToggleChecklist[16]).Text, obj7);
    } else {
      tmp76Result = null;
    }
    ...
    tmp76Result2 = closure_18(trimmed, obj2);
    ...
    if (memo1.length > 0) {
      ...
      const items14 = [
        ...
        memo1.map((children) =>
          closure_1_17(
            message(onToggleChecklist[16]).Text,
            { variant: "text-md/normal", color: "text-default", children: children.content },
            children.key,
          ),
        ),
      ];
      obj20.children = items14;
      tmp27Result3 = closure_18(tmp36, obj20);
    }
    ...
    if (tmp26) {
      ...
      if (tmp23) {
        ...
        tmp35Result = closure_17(checklistSuperseded(tmp38[18]), obj23);
      }
      ...
      if (null != items18) {
        ...
        obj27.live = message(tmp38[36]).checklistLive(message);
        obj27.superseded = checklistSuperseded;
        obj27.expanded = checklistExpanded;
        obj27.onToggleExpanded = callback;
        obj26.children = closure_17(tmp55, obj27);
        let tmp35Result13 = closure_17(tmp36, obj26);
        const tmp37Result = message(tmp38[36]);
      } else {
        tmp35Result13 = null;
      }
      ...
      if (null != found) {
        ...
        if ("message" in found) {
          ...
          tmp35Result20 = closure_17(message(tmp38[16]).Text, obj35);
        }
      }
      items17[12] = tmp35Result20;
      obj22.children = items17;
      tmp27Result4 = closure_18(tmp36, obj22);
    }
...
export default function VibegrationsNativeChat(projectId) {
  ...
  let tmp = ref3();
  items = [state];
  const stateFromStores = projectId(stateFromStores1[40]).useStateFromStores(
    items,
  ...
  stateFromStores1 = projectId(stateFromStores1[40]).useStateFromStores(
    items2,
    () => VibegrationsChatStore.getMessages(projectId),
    items3,
  );
  const obj3 = projectId(stateFromStores1[40]);
  const tmp7 = onApprove;
  const items4 = [onApprove];
  const items5 = [projectId];
  const stateFromStores2 = projectId(stateFromStores1[40]).useStateFromStores(
    items4,
  ...
  const stateFromStores3 = projectId(stateFromStores1[40]).useStateFromStores(
    items6,
  ...
  const stateFromStores4 = projectId(stateFromStores1[40]).useStateFromStores(
    items8,
  ...
  const stateFromStores5 = projectId(stateFromStores1[40]).useStateFromStores(
    items10,
  ...
  const stateFromStores6 = projectId(stateFromStores1[40]).useStateFromStores(
    items13,
  ...
  const stateFromStores7 = projectId(stateFromStores1[40]).useStateFromStores(
    items15,
  ...
  render_id = null;
  const stateFromStores8 = projectId(stateFromStores1[40]).useStateFromStores(
    items17,
  ...
  state = obj2.useMemo(
    () =>
      VibegrationsChatGrouping.groupChatRows(
        stateFromStores1.map((key) => {
  ...
  const tmp12Result = _slicedToArray(
    render_id.useState(() => new Map()),
    2,
  );
  ...
  const memo3 = obj2.useMemo(
    () => ({ itemVisiblePercentThreshold: projectId(stateFromStores1[43]).MIN_VISIBLE_PERCENT }),
    [],
  );
  ...
  const items40 = [
    c17(stateFromStores(stateFromStores1[48]), {
      thinking: stateFromStores2,
      bleedBottom: stateFromStores(stateFromStores1[41])().bottom,
    }),
    ,
  ];
  ...
  const items43 = [c17(TranscriptFade, obj10), , ,];
  ...
  if (null != tmp56) {
    const obj19 = {
      line: tmp56,
      onJumpToActivity: callback6,
      bottom: tmp5(tmp3[7]).space.PX_12 + tmp35,
      todos: memo4,
  ...
  const items44 = [
    c17(stateFromStores(stateFromStores1[53]), {
      projectId,
      thinking: stateFromStores2,
      compacting: stateFromStores3,
      activity: stateFromStores4,
      projectUsage: stateFromStores5,
      connLabel: connectionLabelResult,
      controlling: vibegrationsControlActive,
      connFailed: "failed" === stateFromStores6,
      thinkingOpen: tmp20,
  ...
  const obj22 = {
    projectId,
    canSend: tmp27,
    running: stateFromStores2,
    stopped: stateFromStores7,
    onSend: callback8,
    onInterrupt: null,
    tipOpen: null,
    onDismissTip: null,
