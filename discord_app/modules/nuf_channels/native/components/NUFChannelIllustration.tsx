...
export default function NUFChannelIllustration() {
  ...
  const effect1 = sharedValue1.useEffect(() => {
    if (first.length >= 2) {
      const result = sharedValue1.set(sharedValue1(4194).withTiming(1, { duration: 250 }));
      const obj = sharedValue1(4194);
    }
  }, items);
  let items1 = [sharedValue, first];
  const effect2 = sharedValue1.useEffect(() => {
    const result = sharedValue.set(0);
    const result1 = sharedValue.set(tmp(4194).withTiming(1, { duration: 200 }));
  }, items1);
  let obj2 = require(4054);
  const fn = function b() {
    let obj = { transform: null };
    obj = { translateY: null };
    obj[0] = tmp(4054).interpolate(sharedValue.get(), [0, 1], [50, 0]);
    const items = [obj];
    obj[0] = items;
    return obj;
  };
  obj = { interpolate: require(4054).interpolate, messageListAnimation: sharedValue };
  fn.__closure = obj;
  ...
  obj2 = { source: first(12850), style: items4 };
  items4 = [, ];
  ({ starMedium: arr6[0], starPink: arr6[1] } = tmp);
  items3[1] = callback(closure_6, obj2);
  const items5 = [, ];
  ({ starMedium: arr7[0], starGreen: arr7[1] } = tmp);
  items3[2] = callback(closure_6, { source: first(12851), style: items5 });
  const obj3 = { source: first(12851), style: items5 };
  const items6 = [, ];
  ({ starSmall: arr8[0], starPurple: arr8[1] } = tmp);
  items3[3] = callback(closure_6, { source: first(12852), style: items6 });
  ...
  const items7 = [callback(require(4699) /* TextIcon */.TextIcon, { size: "sm" }), ];
  ...
  obj8[2] = items8;
  items7[1] = callback2(require(4189) /* Text */.Text, obj8);
  obj7[1] = items7;
  ...
  obj10[1] = first.map((children) => {
    ...
    const items1 = [outer1_7(tmp(4189).Text, { variant: "text-md/semibold", allowFontScaling: false, children: children.name }), outer1_7(tmp(4189).Text, { variant: "text-md/medium", allowFontScaling: false, children: children.message })];
    obj[1] = items1;
    items[1] = outer1_8(outer1_5, obj);
    obj[1] = items;
    return outer1_8(outer1_5, obj, children.message);
  });
  obj9[1] = callback(first(4054).View, obj10);
  items9[1] = callback(closure_5, obj9);
  obj6[3] = items9;
  items3[5] = callback2(require(5226) /* PressableCard */.Card, obj6);
  obj[1] = items3;
