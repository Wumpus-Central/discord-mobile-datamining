...
function RoleCreateScene() {
  ...
  const callback1 = callback.useCallback(callback(function*() {
    ...
    yield outer1_1(5215).createRole(outer1_1.id, c2, num5);
    if (1 === tmp7) {
      c2 = 0;
      callback(false);
      ...
      const result1 = outer1_12.addConditionalChangeListener(() => {
        ...
        if (null != selectedRoleId) {
          if (null != role) {
            if (null != guild) {
              tmp3(3984).roleCreatedToast();
              const obj5 = tmp3(3984);
              const tmp13 = tmp3;
              tmp3(16587).setRoleJustCreated(true);
...
export default function GuildSettingsRoleCreateModal() {
  ...
  return callback3(stateFromStores(5704).Navigator, obj);
};