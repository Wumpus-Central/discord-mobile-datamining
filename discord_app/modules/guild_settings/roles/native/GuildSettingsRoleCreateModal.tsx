...
function RoleCreateScene() {
          ...
          if (null != selectedRoleId) {
            if (null != role) {
              if (null != guild) {
                ...
                const merged = Object.assign(closure_0(4819).collectGuildAnalyticsMetadata(guild.id));
  ...
  const callback2 = onSelect.useCallback(() => {
    React6.dismiss();
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16401, dependencyMap.paths), "RoleColorPicker", {
      color,
      onSelect,
    });
  }, items3);
...
let obj = {
  container: { marginTop: fn(5767).NAV_BAR_HEIGHT, flexGrow: 1, paddingBottom: nativeDefault.space.PX_16 },
