...
function RoleCreateScene() {
  ...
  const callback1 = onSelect.useCallback(
    asyncGeneratorStep(async () => {
      ...
      await tmp3(5825).createRole(stateFromStores.id, str, num5);
      if (1 === tmp7) {
        c2 = 0;
        closure_129_5(false);
        const result = tmp3(4522).roleCreateFailedToast();
        c4 = 3;
        tmp3(4522);
      } else if (arg0 === 1) {
        c4 = 3;
        ...
        const result1 = GuildSettingsStore.addConditionalChangeListener(() => {
          ...
          if (null != selectedRoleId) {
            if (null != role) {
              if (null != guild) {
                closure_0(4522).roleCreatedToast();
                const obj5 = closure_0(4522);
                closure_0(18144).setRoleJustCreated(true);
                ...
                const merged = Object.assign(closure_0(5009).collectGuildAnalyticsMetadata(guild.id));
  ...
  const callback2 = onSelect.useCallback(() => {
    React6.dismiss();
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16717, dependencyMap.paths), "RoleColorPicker", {
      color,
      onSelect,
    });
  }, items3);
...
const GuildSettingsRoleConstants = fn(18147);
({
...
let obj = {
  container: { marginTop: fn(5987).NAV_BAR_HEIGHT, flexGrow: 1, paddingBottom: nativeDefault.space.PX_16 },
...
let obj3 = { marginTop: fn(5987).NAV_BAR_HEIGHT, flexGrow: 1, paddingBottom: nativeDefault.space.PX_16 };
...
export default function GuildSettingsRoleCreateModal() {
  ...
  return closure_21(stateFromStores(7333).Navigator, { screens, initialRouteName: obj7.STEP_DISPLAY });
}
