...
function RoleCreateScene() {
    ...
    let obj = {
      headerLeft: NavigatorHeader.getHeaderCloseButton(() => {
        ...
        const obj2 = {
          ...
          from_step: constants4[constants3.STEP_DISPLAY],
  ...
  const callback1 = onSelect.useCallback(
    asyncGeneratorStep(async () => {
      ...
      await tmp3(5823).createRole(stateFromStores.id, str, num5);
      if (1 === tmp7) {
        c2 = 0;
        closure_129_5(false);
        const result = tmp3(4520).roleCreateFailedToast();
        c4 = 3;
        tmp3(4520);
      } else if (arg0 === 1) {
        c4 = 3;
        ...
        const result1 = GuildSettingsStore.addConditionalChangeListener(() => {
          ...
          if (null != selectedRoleId) {
            if (null != role) {
              if (null != guild) {
                closure_0(4520).roleCreatedToast();
                const obj5 = closure_0(4520);
                closure_0(18121).setRoleJustCreated(true);
                ...
                const tmp18Result = closure_1(1241);
                const merged = Object.assign(closure_0(5007).collectGuildAnalyticsMetadata(guild.id));
  ...
  const callback2 = onSelect.useCallback(() => {
    React6.dismiss();
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(16693, dependencyMap.paths), "RoleColorPicker", {
      color,
      onSelect,
    });
  }, items3);
...
function RolePermissionTemplate() {
  ...
  onSelect = noop.useCallback((arg0) => {
    ...
    const obj3 = {
      flow_type: constants2.GUILD_ROLE_CREATION_MODAL,
      from_step: constants4[obj7.STEP_PERMISSIONS],
      to_step: constants4[obj7.STEP_MEMBERS],
      skip: false,
    };
  ...
  const items5 = [
    closure_21(stateFromStores(onSelect[38]), {
      onSelect,
      location: constants2.GUILD_ROLE_CREATION_MODAL,
      guildId: guild.id,
    }),
  ];
...
function RoleMembers() {
  ...
  const items = [GuildSettingsStore];
  const stateFromStoresObject = navigation(pendingAdditions[19]).useStateFromStoresObject(items, () =>
    props.getProps(),
  );
  const guild = stateFromStoresObject.guild;
  closure_129_0 = guild;
  ...
  stateFromStores(pendingAdditions[20])(
    null != stateFromStores,
    "useGuildSettingsStoreGuildWithRole: Role cannot be null",
  );
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    let obj = {
      headerLeft: NavigatorHeader.getHeaderCloseButton(() => {
        ...
        const obj2 = {
          flow_type: constants2.GUILD_ROLE_CREATION_MODAL,
          from_step: constants4[constants3.STEP_MEMBERS],
          to_step: constants4.FLOW_DISMISSED,
          skip: false,
        };
        const merged = Object.assign(navigation(first[15]).collectGuildAnalyticsMetadata(id.id));
  ...
  const callback1 = noop.useCallback(() => {
    ...
    const obj3 = {
      flow_type: constants2.GUILD_ROLE_CREATION_MODAL,
      from_step: constants4[obj7.STEP_MEMBERS],
      to_step: constants4.FLOW_COMPLETED,
      skip: false,
    };
    const merged = Object.assign(AppAnalyticsUtils.collectGuildAnalyticsMetadata(guild.id));
    obj2.track(constants.USER_FLOW_TRANSITION, obj3);
  ...
  const items6 = [
    closure_21(navigation(pendingAdditions[41]).AddMembersBody, {
...
const GuildSettingsRoleConstants = fn(18124);
({
  PermissionTemplates: map1,
  DEFAULT_TEMPLATE_TYPE: closure_14,
  MAX_BULK_ROLE_MEMBERS_ADD: closure_15,
...
let obj = {
  container: { marginTop: fn(5985).NAV_BAR_HEIGHT, flexGrow: 1, paddingBottom: nativeDefault.space.PX_16 },
...
let obj3 = { marginTop: fn(5985).NAV_BAR_HEIGHT, flexGrow: 1, paddingBottom: nativeDefault.space.PX_16 };
...
let closure_26 = {
  [obj7.STEP_DISPLAY]: "Role Display",
  [obj7.STEP_PERMISSIONS]: "Role Permissions",
  [obj7.STEP_MEMBERS]: "Role Members",
...
export default function GuildSettingsRoleCreateModal() {
  ...
  useMountEffectDefault(() => {
    ...
    const obj2 = {
      flow_type: constants2.GUILD_ROLE_CREATION_MODAL,
      from_step: constants.FLOW_INITIALIZED,
      to_step: constants[obj7.STEP_DISPLAY],
      skip: false,
    };
    const merged = Object.assign(AppAnalyticsUtils.collectGuildAnalyticsMetadata(stateFromStores.id));
    obj.track(constants.USER_FLOW_TRANSITION, obj2);
  });
  return closure_21(stateFromStores(7331).Navigator, { screens, initialRouteName: obj7.STEP_DISPLAY });
}
