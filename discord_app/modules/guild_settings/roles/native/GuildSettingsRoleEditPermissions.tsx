...
export default function GuildSettingsRoleEditPermission(guild) {
  ...
  if (tmp18Result) {
    ...
    tmp18Result = tmp18(guild(4298).Text, obj);
  }
  tmp18Result = tmp18(tmp19, { children: tmp18Result });
  canResult = importAll(3934).can(obj);
  const guildPermissionSpec = role(15819).generateGuildPermissionSpec(guild);
  ...
  const children = [closure_14(role(16631), { role }), , , ];
  const tmp14Result = role(15819);
  const tmp24 = mapped1.length > 0;
  const tmp25 = closure_16;
  const tmp26 = closure_15;
  children[1] = closure_14(c6, { children: closure_14(guild(5932).SearchField, obj3) });
  ...
  if (tmp24) {
    ...
    obj5[0] = tmp27(8954).NoResultsAlt;
