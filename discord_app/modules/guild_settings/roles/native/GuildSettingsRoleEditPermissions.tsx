...
export default function GuildSettingsRoleEditPermission(guild) {
  ...
  const tmp6Result = importAll(3917);
  ...
  const guildPermissionSpec = role(15795).generateGuildPermissionSpec(guild);
  ...
  const children = [closure_14(role(16611), { role }), , , ];
  const tmp14Result = role(15795);
  const tmp24 = mapped1.length > 0;
  const tmp25 = closure_16;
  const tmp26 = closure_15;
  children[1] = closure_14(c6, { children: closure_14(guild(5913).SearchField, obj3) });
  ...
  if (tmp24) {
    ...
    obj5[0] = tmp27(8937).NoResultsAlt;
