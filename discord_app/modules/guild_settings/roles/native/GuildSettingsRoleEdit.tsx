    ...
    applyArgumentsResult.handleSaveRole = function handleSaveRole() {
      return new Promise((arg0) => {
        ...
        if (effectiveSection === constants2.PERMISSIONS) {
          ...
          const obj1 = { name, color: primary_color1, colors: tmp7, hoist, mentionable, icon, unicodeEmoji };
        }
        ...
        function success() {
          let obj = applyArgumentsResult(17738);
            ...
            if (0 === v1) {
              if (arg0 === 1) {
                ...
                if (null === guild_connections) {
                  v1 = 1;
                  c2 = 1;
                  let obj1 = { value: tmp4(11702).putRoleConnectionsConfigurations(id.id, role.id, []), done: false };
                  return obj1;
                }
              }
            } else if (arg0 === 1) {
...
prototype["renderSubScreenButtons"] = function renderSubScreenButtons() {
  ...
  const items = [closure_22(self(5686).TableRow, obj), , ];
  obj = { label: null, onPress: null, arrow: true };
  ...
  items[1] = closure_22(self(5686).TableRow, obj);
  ...
  items[2] = closure_22(self(5686).TableRow, obj1);
  obj.children = items;
...
export default function ConnectedGuildSettingsRoleEdit(guildId) {
  ...
  const effect1 = noop.useEffect(() => {
    ...
    if (null != id) {
      const roleConnectionsConfiguration = ConnectionsRoleActionCreators.fetchRoleConnectionsConfiguration(guildId, role.id);
    }
  }, items2);
  let tmp10 = null;
  if (null != guild) {
