...
export default function ConnectedGuildSettingsModalRoles(guildId) {
  ...
  const stateFromStoresObject = obj3.useStateFromStoresObject(items1, () => {
    ...
    if (null != guild) {
      highestRole = str(3918).getHighestRole(guild, id);
      const obj2 = str(3918);
    }
    obj[7] = highestRole;
    return obj;
  ...
  first = undefined;
  let obj4 = sortedGuildRoles(16567);
  ...
  const effect = React.useEffect(() => {
    ...
    if (!tmp) {
      first(16472).terminate();
      const obj = first(16472);
      first(8943).close();
      const obj2 = first(8943);
    }
  }, items4);
  ...
  callback6 = obj.useCallback(() => {
    ...
    if (tmp) {
      sortedGuildRoles(16577).batchRoleUpdate(ref.id, updates);
      const obj = sortedGuildRoles(16577);
    }
    callback5();
  }, items14);
  ...
  callback7 = obj.useCallback((to) => {
    ...
    first(16578).updateRoleOrder(to.from, to);
  }, items15);
  ...
  const callback8 = obj.useCallback(() => {
    ...
    const items1 = [hasRoles(sortedGuildRoles(5649).TableRowGroupTitle, obj), ];
    let tmpResult = null;
    if (!first) {
      tmpResult = null;
      if (!c16) {
        ...
        const items2 = [tmp6(tmp7(11451).ArrowsUpDownIcon, obj2), ];
        ...
        obj1[4] = items2;
        tmpResult = tmp(tmp7(4812).PressableOpacity, obj1);
      }
    }
    items1[1] = tmpResult;
    obj[1] = items1;
  ...
  const callback9 = obj.useCallback(() => {
    ...
    if (dependencyMap) {
      if (isThemeDarkResult) {
        ...
        tmp3Result = tmp3(16580);
      }
    } else {
      if (isThemeDarkResult) {
        tmp3Result = tmp3(16581);
      } else {
        tmp3Result = tmp3(16582);
      }
      if (hasRoles) {
        ...
        obj6[0] = items2;
        obj6[1] = tmp3Result;
        obj5[1] = hasRoles(first(5236), obj6);
  ...
  const callback10 = obj.useCallback(() => {
    if (null != setRoleJustCreated) {
      if (null != callback) {
        ...
        obj[3] = tmp.id;
        obj[1] = hasRoles(first(16583), obj);
  ...
  const callback11 = obj.useCallback((role) => {
    ...
    if (null == setRoleJustCreated) {
      ...
      obj[11] = fn2;
      return tmp22(first(16583), obj, role.id);
    }
  }, items19);
  ...
  const effect3 = obj.useEffect(() => {
    if (first) {
      if (null != setRoleJustCreated) {
        first(16578).startReordering(tmp2.id);
        const obj2 = first(16578);
      }
      if (obj3.isIOS()) {
  ...
  const effect4 = obj.useEffect(() => {
    if (null != setRoleJustCreated) {
      if (memberCount <= sortedGuildRoles(16575).MAX_PREFETCH_MEMBER_COUNT) {
        const obj = first(5200);
        const members = obj.requestMembers(tmp.id, "", 0, false);
      }
      const memberCounts = str(5300).fetchMemberCounts(tmp.id);
      const obj2 = str(5300);
    }
  }, items22);
  const items23 = [first];
  ...
  if (!tmp25) {
    ...
    obj[1] = callback1;
    obj[1] = hasRoles(tmp3(5901).SearchField, obj);
  ...
  if (first) {
    ...
    if (!hasRoles) {
      ...
      tmp47Result = tmp47(tmp3(7777).FormRow, obj1);
    }
    obj4 = { children: null };
  ...
  obj6[8] = tmp56;
  obj6[9] = callback11;
  obj5[1] = hasRoles(first(15371), obj6);
  items24[2] = hasRoles(callback, obj5);
  items24[3] = hasRoles(sortedGuildRoles(5596).NavScrim, {});
  obj8[0] = items24;
