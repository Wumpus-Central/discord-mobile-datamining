...
export default function ConnectedGuildSettingsModalRoles(guildId) {
  ...
  const callback8 = obj.useCallback(() => {
    ...
    let tmp6Result = null;
    if (first) {
      const obj4 = { style: tmp3.rolesBody, variant: "text-sm/medium", color: "interactive-text-default", children: null };
  ...
  const callback10 = obj.useCallback(() => {
    if (null != guild) {
      if (null != guildEveryoneRole) {
        ...
        obj = { role: tmp2, locked: null, onPress: null, guildId: null, sorting: false, numMembers: 0, isEveryoneRole: true, isLastRole: true, isFirstRole: true };
