...
function GraceOption(option) {
  ...
  return callback(iter(7659).TableRadioRow, {
    value: iter.value,
    label: iter.label,
...
function ForceSyncIcon(isSyncing) {
  if (isSyncing.isSyncing) {
    ...
    obj[2] = tmp;
    obj = { source: null };
    obj[0] = importDefault(16454);
    obj[3] = tmp2(require(5224) /* TableRowInner */.TableRow.Icon, obj);
    tmp2Result = tmp2(require(4721) /* PressableBase */.PressableOpacity, obj);
  }
  return tmp2Result;
}
...
GuildSettingsModalIntegrationSettings.prototype["render"] = function render() {
  ...
  if (null != tmp) {
    ...
    obj[0] = iter.value;
    obj[3] = tmp.name;
    obj[1] = callback(integration(4189).Text, obj);
    const tmp2 = callback(integration(5224).TableRow, obj);
  }
  ...
  if (null == value) {
    ...
    const values = Object.values(tmp12(16453).IntegrationExpireGracePeriodTypes);
    ...
    obj4[1] = combined;
    obj4[2] = null;
    obj3[1] = callback(tmp15(5224).TableRow, obj4);
    const items1 = [callback(tmp15(5558).TableRowGroup, obj3), , , , ];
    ...
    let obj14 = tmp16(3775)(integration.synced_at);
    ...
    const items2 = [callback(tmp15(5224).TableRow, obj6), tmp2];
    obj5[2] = items2;
    items1[1] = closure_8(tmp15(5558).TableRowGroup, obj5);
    ...
    obj12[0] = tmp15(16455).IntegrationExpireBehaviorTypes.REMOVE_ROLE;
    const intl6 = tmp15(1236).intl;
    obj12[1] = intl6.string(tmp15(1236).t["6kpw4i"]);
    const items3 = [callback(tmp15(7659).TableRadioRow, obj12), ];
    const obj13 = { value: null, label: null };
    obj13[0] = tmp15(16455).IntegrationExpireBehaviorTypes.KICK;
    const intl7 = tmp15(1236).intl;
    obj13[1] = intl7.string(tmp15(1236).t.fQUQIJ);
    items3[1] = callback(tmp15(7659).TableRadioRow, obj13);
    obj11[4] = items3;
    items1[2] = closure_8(tmp15(7660).TableRadioGroup, obj11);
    ...
    items1[3] = callback(tmp15(5558).TableRowGroup, obj14);
    items1[4] = tmp13;
    obj2[2] = items1;
    obj1[2] = closure_8(tmp15(4602).Stack, obj2);
    const items4 = [callback(tmp15(7639).Form, obj1), callback(tmp15(5505).NavScrim, {})];
    obj9[0] = items4;
    return closure_8(closure_9, obj9);
  } else {
    let tmp15Result = tmp15(1416);
    tmp15Result = tmp15(4039);
    let icon = value.icon;
