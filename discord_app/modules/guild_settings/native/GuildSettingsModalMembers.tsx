...
let obj = { paddingHorizontal: ThemesDefault.space.PX_12, flex: 1 };
obj[0] = obj;
...
let closure_20 = importAllResult.memo((guild) => {
  ...
  if (null == guild) {
    ...
    const mapped = found.map((item, index) => closure_1_15(guildMember(arr[17]), { role: item, guildId: guild.id }, item.id));
    let formatToPlainStringResult;
    if (found.length > 0) {
      user = user.getUser(guildMember.userId);
      obj1 = guildMember(4796);
      ...
      const mapped1 = found.map((item, index) => item.name);
    ...
    if (mapped.length > 0) {
      ...
      obj1[0] = tmp.roleList;
      obj1[2] = mapped;
      tmp12Result = callback(View, obj1);
    }
    obj[3] = tmp12Result;
    obj[4] = tmp5;
    ...
    obj[7] = start;
    obj[8] = end;
    return callback(guildMember(9965), obj);
  }
  tmp = callback();
});
const memoResult = importAllResult.memo(function GuildSettingsModalMembers(guildId) {
  ...
  first2 = tmp15[0];
  closure_14 = tmp15[1];
  const first3 = guildLoaded(sortedGuildRoles.useState(() => new navigation(guild[13])((arg0) => {
    callback(arg0);
    callback2(false);
  }, first4, 100)), 2)[0];
  ...
  const memo = sortedGuildRoles.useMemo(() => {
    function guildRoleIsFiltered(roles) {
      ...
      if (tmp2) {
        tmp2 = !guildId(guild[25]).isEveryoneRoleId(closure_0, closure_6);
        const obj = guildId(guild[25]);
      }
      if (tmp2) {
        roles = roles.roles;
        tmp2 = -1 === roles.indexOf(closure_6);
      }
      return tmp2;
    }
    items = [];
  ...
  const effect2 = sortedGuildRoles.useEffect(() => {
    if (guildLoaded) {
      let obj = { guild: null, strict: true };
      obj[0] = guildId;
      obj[0] = obj;
    ...
    return () => {
      if (closure_3) {
        ...
        const role = obj.selectRole(navigation(guild[32]).castGuildIdAsEveryoneGuildRoleId(closure_0));
...
const result = require("obj132").fileFinishedImporting("modules/guild_settings/native/GuildSettingsModalMembers.tsx");
