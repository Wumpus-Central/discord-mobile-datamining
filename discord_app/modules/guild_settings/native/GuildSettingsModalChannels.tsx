...
import _modDef16182 from "../../../../_runtime/metro/16182__.js";
...
function ChannelItem(isFavoritesGuild) {
  ...
  let tmpResult = channel(5042);
  const channelIcon = tmpResult.getChannelIcon(channel);
  tmpResult = channel(5042);
  const channelIconComponent = tmpResult.getChannelIconComponent(channel);
  const tmpResult1 = channel(4425);
  const isThemeDarkResult = channel(4425).isThemeDark(obj1.useThemeContext().theme);
  ...
  if (null != channelIconComponent) {
    ...
    tmp7Result = closure_16(channel(8626).FormRow.Icon, obj3);
  }
  const obj4 = {
    leading: tmp7Result,
  ...
  obj4.label = channel(4727).computeChannelName(channel, UserStore, RelationshipStore);
  let fn;
  if (!sortingEnabled) {
  ...
  if (stateFromStores) {
    tmp7Result = null;
    if (sortingEnabled) {
      const obj5 = { source: tmp13(16179), style: actionIconStyle };
      tmp7Result = closure_16(channel(8626).FormRow.Icon, obj5);
    }
  }
  ...
  obj1.children = closure_16(channel(8626).FormRow, obj4);
...
function CreateButton(guild) {
  ...
  if (PermissionStore.can(constants2.MANAGE_CHANNELS, guild)) {
    ...
    obj.icon = closure_16(tmp(8899).PlusSmallIcon, { color: "white" });
    obj.children = closure_16(tmp(4989).Button, obj);
...
const merged1 = Object.assign(
  TextStyles(Fonts.DISPLAY_EXTRABOLD, nativeDefault.colors.TEXT_SUBTLE, 12, { uppercase: true }),
);
...
createStyles.floatingActionButtonContainer = {
  position: "absolute",
  bottom: 16,
  right: 0,
  left: 0,
...
Category.prototype["render"] = function render() {
  ...
  if (null != onPress) {
    ...
    if (null != sortHandlers) {
      obj2 = { source: tmp11(16179), style: actionIconStyle };
  ...
  constructor() {
    ...
    applyArgumentsResult.renderItem = function renderItem(item) {
      ...
      if (null != channel) {
        const obj = {
          channel,
          isFavoritesGuild: FavoritesUtils.isFavoritesGuildId(applyArgumentsResult.props.guild.id),
          sortingEnabled: null != item.sortingType,
          onPress: applyArgumentsResult.handleChannelPress,
          style: tmp2.row,
        };
    ...
    applyArgumentsResult.handleSortStart = function handleSortStart() {
      ...
      obj.icon = _modDef11835;
      obj.onPress = function onPress() {
        closure_1_1(16177).startReordering(
    ...
    applyArgumentsResult.handleDrop = function handleDrop(arg0) {
      ...
      const dropData = ChannelSortingUtils.getDropData(
        localChannel,
        arg0.from,
        localChannel1,
        arg0.to,
            ...
            if (null != localChannel1) {
              ...
              if (tmp14) {
                ...
                if (isFavoritesGuildIdResult) {
                  ...
                  canResult = PermissionStore.can(
                    constants2.MANAGE_CHANNELS,
                    ChannelStore.getChannel(dropData.parentId),
                  );
                }
              }
              if (tmp14) {
                          ...
                          if (PermissionStore.can(constants2.MANAGE_ROLES, localChannel)) {
                            if (PermissionStore.can(constants2.MANAGE_ROLES, channel)) {
                              const appChannelBotUserId =
                                applyArgumentsResult(11641).getAppChannelBotUserId(localChannel);
                              ...
                              let areChannelsLockedResult1 = PermissionUtilsAll.areChannelsLocked(
                                localChannel,
                                ChannelStore.getChannel(localChannel.parent_id),
                                appChannelBotUserId,
                              );
        ...
        if (null != stateFromStores) {
          ...
          if (null != stateFromStores2) {
            obj = {
              ...
              channels: stateFromStores1,
              user: stateFromStores2,
              channelList: stateFromStores3,
              order: stateFromStores4,
              sortingType: stateFromStores5,
