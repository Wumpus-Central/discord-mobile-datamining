...
import closure_9 from "sortCategoryList" /* 15696 */;
import closure_10 from "handleFormInit" /* 9690 */;
...
function SettingsSection(guild) {
  ...
  obj = { IconComponent: guild(4403).CircleInformationIcon };
  obj[2] = callback(guild(5567).TableRow.Icon, obj);
  obj[3] = function onPress() {
    return callback(closure_1_12.OVERVIEW);
  };
  const items = [callback(guild(5567).TableRow, obj, "overview")];
  ...
  if (canManageGuild) {
    ...
    obj1[0] = tmp2(17067).ModerationIcon;
    obj[2] = tmp(tmp2(5567).TableRow.Icon, obj1);
    obj[3] = function onPress() {
      return callback(closure_1_12.MODERATION);
    };
    items.push(tmp(tmp2(5567).TableRow, obj, "moderation"));
  }
  if (canViewAuditLog) {
    ...
    obj3[0] = tmp2(7626).ClipboardListIcon;
    obj2[2] = tmp(tmp2(5567).TableRow.Icon, obj3);
    obj2[3] = function onPress() {
      return callback(closure_1_12.AUDIT_LOG);
    };
    items.push(tmp(tmp2(5567).TableRow, obj2, "auditlogs"));
  }
  ...
  if (canManageChannels) {
    ...
    obj5[0] = tmp2(17069).ChannelListIcon;
    obj4[2] = tmp(tmp2(5567).TableRow.Icon, obj5);
    obj4[3] = function onPress() {
      guild = closure_1_9.initGuild(guild.id);
      callback(closure_1_12.CHANNELS);
    };
    items.push(tmp(tmp2(5567).TableRow, obj4, "channels"));
  }
  ...
  if (tmp11) {
    ...
    obj7[0] = tmp2(14527).PuzzlePieceIcon;
    obj6[2] = tmp(tmp2(5567).TableRow.Icon, obj7);
    obj6[3] = function onPress() {
      return callback(closure_1_12.INTEGRATIONS);
    };
    items.push(tmp(tmp2(5567).TableRow, obj6, "integrations"));
  }
  if (canManageGuildExpressions) {
    ...
    obj9[0] = tmp2(8196).ReactionIcon;
    obj8[2] = tmp(tmp2(5567).TableRow.Icon, obj9);
    obj8[3] = function onPress() {
      return callback(closure_1_12.EMOJI);
    };
    items.push(tmp(tmp2(5567).TableRow, obj8, "emoji"));
    ...
    obj11[0] = tmp2(10158).StickerIcon;
    obj10[2] = tmp(tmp2(5567).TableRow.Icon, obj11);
    obj10[3] = function onPress() {
      return callback(closure_1_12.STICKERS);
    };
    items.push(tmp(tmp2(5567).TableRow, obj10, "stickers"));
  }
  if (canConfigureOfficialMessages) {
    ...
    obj13[0] = tmp2(11280).StampIcon;
    obj12[2] = tmp(tmp2(5567).TableRow.Icon, obj13);
    obj12[3] = function onPress() {
      return callback(closure_1_12.OFFICIAL_MESSAGES);
    };
    items.push(tmp(tmp2(5567).TableRow, obj12, "official-messages"));
  }
  let tmp2Result = tmp2(9692);
  if (tmp2Result.canUseMobileServerTagSettings(guild.id)) {
    ...
    obj15[0] = tmp2(10381).TagIcon;
    obj14[2] = tmp(tmp2(5567).TableRow.Icon, obj15);
    obj14[3] = function onPress() {
      return callback(closure_1_12.TAG);
    };
    items.push(tmp(tmp2(5567).TableRow, obj14, "server-tag"));
  }
  if (canManageGuild) {
    ...
    obj17[0] = tmp2(9674).ShieldUserIcon;
    obj16[2] = tmp(tmp2(5567).TableRow.Icon, obj17);
    obj16[3] = function onPress() {
      return callback(closure_1_12.SECURITY);
    };
    items.push(tmp(tmp2(5567).TableRow, obj16, "security"));
  }
  if (isGuildAdmin) {
    tmp2Result = tmp2(17071);
  ...
  if (isGuildAdmin) {
    ...
    obj19[0] = tmp2(4391).LinkIcon;
    obj18[2] = tmp(tmp2(5567).TableRow.Icon, obj19);
    obj18[3] = function onPress() {
      return callback(closure_1_12.VANITY_URL);
    };
    items.push(tmp(tmp2(5567).TableRow, obj18, "vanity"));
  }
  let tmpResult = null;
  if (0 !== items.length) {
    ...
    obj20[2] = items;
    tmpResult = tmp(tmp2(5960).TableRowGroup, obj20);
  }
  return tmpResult;
}
function CommunitySection(pushScreen) {
  ...
  const TableRow = pushScreen(5567).TableRow;
  obj = { label: null, arrow: true, icon: null, onPress: null };
  ...
  if (hasItem) {
    ...
    obj[0] = tmp3(15157).TreehouseIcon;
    obj[2] = tmp2(tmp3(5567).TableRow.Icon, obj);
    obj[3] = function onPress() {
    ...
    obj1 = { IconComponent: null };
    obj1[0] = tmp3(15157).TreehouseIcon;
    obj[2] = tmp2(tmp3(5567).TableRow.Icon, obj1);
    obj[3] = function onPress() {
  ...
  obj[2] = tmp2Result;
  return closure_15(pushScreen(5960).TableRowGroup, obj);
}
function UserManagementSection(pushScreen) {
  ...
  obj = { IconComponent: pushScreen(5006).GroupIcon };
  obj[2] = callback(pushScreen(5567).TableRow.Icon, obj);
  obj[3] = function onPress() {
    return pushScreen(closure_1_12.MEMBERS);
  };
  const items = [callback(pushScreen(5567).TableRow, obj, "members")];
  if (canManageRoles) {
    obj = { label: null, arrow: true, icon: null, onPress: null };
    const intl2 = tmp2(1236).intl;
    obj[0] = intl2.string(tmp2(1236).t["LPJmL/"]);
    obj1 = { IconComponent: null };
    obj1[0] = tmp2(9674).ShieldUserIcon;
    obj[2] = tmp(tmp2(5567).TableRow.Icon, obj1);
    obj[3] = function onPress() {
      return pushScreen(closure_1_12.ROLES);
    };
    items.push(tmp(tmp2(5567).TableRow, obj, "roles"));
  }
  if (canManageGuild) {
    const obj2 = { label: null, arrow: true, icon: null, onPress: null };
    const intl3 = tmp2(1236).intl;
    ...
    const obj3 = { IconComponent: null };
    obj3[0] = tmp2(4391).LinkIcon;
    obj2[2] = tmp(tmp2(5567).TableRow.Icon, obj3);
    obj2[3] = function onPress() {
      return pushScreen(closure_1_12.INSTANT_INVITES);
    };
    items.push(tmp(tmp2(5567).TableRow, obj2, "invites"));
  }
  if (canManageBans) {
    const obj4 = { label: null, arrow: true, icon: null, onPress: null };
    const intl4 = tmp2(1236).intl;
    ...
    const obj5 = { IconComponent: null };
    obj5[0] = tmp2(9334).HammerIcon;
    obj4[2] = tmp(tmp2(5567).TableRow.Icon, obj5);
    obj4[3] = function onPress() {
      return pushScreen(closure_1_12.BANS);
    };
    items.push(tmp(tmp2(5567).TableRow, obj4, "bans"));
  }
  let tmpResult = null;
  if (0 !== items.length) {
    ...
    obj6[2] = items;
    tmpResult = tmp(tmp2(5960).TableRowGroup, obj6);
  }
  return tmpResult;
}
function GuildSettingsModalLandingInner(guild) {
  ...
  let obj5 = guild(17072);
  const tmp11 = obj5.useChannelsAllowedToUnlink(guild.id).length > 0;
  const canManageGuildRoleSubscriptions = guild(6075).useCanManageGuildRoleSubscriptions(guild);
  let result = canManageGuild;
  if (canManageGuild) {
    result = tmp(6082).isGuildOfficialMessagesEnabled(guild, "GuildSettingsModalLanding");
  ...
  const items5 = [callback(updateErrors(16395), obj1), callback(SettingsSection, { guild, categories: stateFromStores, isGuildAdmin, canManageGuild, canManageGuildExpressions, canManageChannels, canUnlinkChannelLobbies: tmp11, canViewAuditLog, canManageWebhooks, canConfigureOfficialMessages: result, pushScreen: callback }), , , ];
  ...
  if (canManageGuildRoleSubscriptions) {
    ...
    obj4[0] = guild;
    obj4[1] = callback;
    tmp17Result = tmp17(tmp3(17073), obj4);
  }
  obj5 = { children: null };
  ...
  obj[2] = items5;
  obj[2] = closure_16(guild(4894).Stack, obj);
  const items6 = [callback(guild(8331).Form, obj), callback(guild(5907).NavScrim, {})];
  obj5[0] = items6;
