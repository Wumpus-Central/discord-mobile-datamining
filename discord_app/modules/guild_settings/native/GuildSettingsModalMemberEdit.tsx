// === Module 11323: GuildSettingsModalMemberEditScene ===
...
import closure_15 from "handleStopEditingRoles" /* 11324 */;
...
class GuildSettingsModalMemberEdit extends PureComponent2 {
  constructor() {
    ...
    applyArgumentsResult.handleToggleRole = function handleToggleRole(roleId, state) {
      callback(11325).toggleRole(roleId, state);
    };
    ...
    applyArgumentsResult.handleCancelEditMemberRoles = function handleCancelEditMemberRoles() {
      callback(11325).stopEditingRoles();
    };
  ...
  if (null == member) {
    ...
    if (null == member) {
      ...
      if (!bot) {
        ...
        obj1[1] = tmp18(tmp19(5551).TableRow, obj2);
        tmp18Result = tmp18(tmp19(5944).TableRowGroup, obj1);
      }
      ...
      obj6[1] = tmp7;
      obj5[1] = tmp18(tmp19(5551).TableRow, obj6);
      const items2 = [tmp18(tmp19(5944).TableRowGroup, obj5), , , , , , ];
      if (!canChangeNick) {
      ...
      if (canChangeNick) {
        ...
        obj8[6] = tmp3;
        canChangeNick = tmp18(tmp19(7638).TextInput, obj8);
      }
      ...
      if (canBan) {
        ...
        obj15[0] = tmp21(4289).getName(user);
        obj14[1] = intl6.formatToPlainString(tmp19(1236).t.TuAZuW, obj15);
        obj14[2] = self.handleBan;
        obj13[1] = tmp18(tmp19(5551).TableRow, obj14);
        canBan = tmp18(tmp19(5944).TableRowGroup, obj13);
        const tmp21Result1 = tmp21(4289);
      }
      items2[5] = canBan;
      items2[6] = tmp18Result;
      obj4[2] = items2;
      obj3[2] = tmp16(tmp19(4878).Stack, obj4);
      return tmp18(tmp19(8295).Form, obj3);
    } else {
      const result = tmp19(4078).isMemberCommunicationDisabled(member);
      const TableRowGroup = tmp19(5944).TableRowGroup;
      const obj16 = { hasIcons: false, children: null };
      const TableRow = tmp19(5551).TableRow;
      ...
      if (result) {
        ...
        obj18[0] = tmp21(4289).getName(user);
        ...
        const tmp21Result2 = tmp21(4289);
      } else {
        const obj19 = { user: null };
        obj19[0] = tmp21(4289).getName(user);
...
export default function MemberModalEdit(onClose) {
  ...
  const tmp = onRemove(5893)(guildId);
  closure_2 = tmp;
  ...
  items1 = [obj];
  return callback3(onClose(5960).Navigator, obj);
};
