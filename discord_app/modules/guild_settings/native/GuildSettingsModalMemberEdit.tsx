...
class GuildSettingsModalMemberEdit extends PureComponent2 {
  constructor() {
    ...
    applyArgumentsResult.handleToggleRole = function handleToggleRole(roleId, state) {
      callback(11171).toggleRole(roleId, state);
    };
    ...
    applyArgumentsResult.handleCancelEditMemberRoles = function handleCancelEditMemberRoles() {
      callback(11171).stopEditingRoles();
    };
...
prototype["updateNavigator"] = function updateNavigator(submitting) {
  ...
  if (!tmp) {
    if (submitting) {
      ...
      let setOptionsResult = self(5269);
  ...
  if (null == member) {
    ...
    if (null == member) {
      ...
      if (!bot) {
        ...
        obj1[1] = tmp18(tmp19(5373).TableRow, obj2);
        tmp18Result = tmp18(tmp19(5707).TableRowGroup, obj1);
      }
      ...
      obj6[1] = tmp7;
      obj5[1] = tmp18(tmp19(5373).TableRow, obj6);
      const items2 = [tmp18(tmp19(5707).TableRowGroup, obj5), , , , , , ];
      if (!canChangeNick) {
      ...
      if (canChangeNick) {
        ...
        obj8[6] = tmp3;
        canChangeNick = tmp18(tmp19(7730).TextInput, obj8);
      }
      ...
      if (canBan) {
        ...
        obj15[0] = tmp21(4140).getName(user);
        obj14[1] = intl6.formatToPlainString(tmp19(1236).t.TuAZuW, obj15);
        obj14[2] = self.handleBan;
        obj13[1] = tmp18(tmp19(5373).TableRow, obj14);
        canBan = tmp18(tmp19(5707).TableRowGroup, obj13);
        const tmp21Result1 = tmp21(4140);
      }
      items2[5] = canBan;
      items2[6] = tmp18Result;
      obj4[2] = items2;
      obj3[2] = tmp16(tmp19(4710).Stack, obj4);
      return tmp18(tmp19(7907).Form, obj3);
    } else {
      const result = tmp19(3918).isMemberCommunicationDisabled(member);
      const TableRowGroup = tmp19(5707).TableRowGroup;
      const obj16 = { hasIcons: false, children: null };
      const TableRow = tmp19(5373).TableRow;
      ...
      if (result) {
        ...
        obj18[0] = tmp21(4140).getName(user);
        ...
        const tmp21Result2 = tmp21(4140);
      } else {
        const obj19 = { user: null };
        obj19[0] = tmp21(4140).getName(user);
...
export default function MemberModalEdit(onClose) {
  ...
  const tmp = onRemove(5656)(guildId);
  c2 = tmp;
  ...
  items1 = [obj];
  return callback3(onClose(5723).Navigator, obj);
};
