...
class GuildSettingsModalMemberEdit extends PureComponent2 {
  constructor() {
    ...
    applyArgumentsResult.handleToggleRole = function handleToggleRole(roleId, state) {
      callback(11043).toggleRole(roleId, state);
    };
    ...
    applyArgumentsResult.handleCancelEditMemberRoles = function handleCancelEditMemberRoles() {
      callback(11043).stopEditingRoles();
    };
...
prototype["render"] = function render() {
  ...
  if (null == member) {
    ...
    obj[1] = importDefault(4032).getUserTag(user);
    const items = [callback3(require(4189) /* Text */.Text, obj), ];
    let tmp18Result = null;
    if (user.bot) {
      obj = { verified: null };
      let tmp21Result = tmp21(8400);
    ...
    if (null == member) {
      ...
      if (!bot) {
        ...
        obj1[1] = tmp18(tmp19(5224).TableRow, obj2);
        tmp18Result = tmp18(tmp19(5558).TableRowGroup, obj1);
      }
      ...
      obj6[1] = tmp7;
      obj5[1] = tmp18(tmp19(5224).TableRow, obj6);
      const items2 = [tmp18(tmp19(5558).TableRowGroup, obj5), , , , , , ];
      if (!canChangeNick) {
      ...
      if (canChangeNick) {
        ...
        obj8[6] = tmp3;
        canChangeNick = tmp18(tmp19(6730).TextInput, obj8);
      }
      ...
      if (canBan) {
        ...
        obj15[0] = tmp21(4032).getName(user);
        obj14[1] = intl6.formatToPlainString(tmp19(1236).t.TuAZuW, obj15);
        obj14[2] = self.handleBan;
        obj13[1] = tmp18(tmp19(5224).TableRow, obj14);
        canBan = tmp18(tmp19(5558).TableRowGroup, obj13);
        const tmp21Result1 = tmp21(4032);
      }
      items2[5] = canBan;
      items2[6] = tmp18Result;
      obj4[2] = items2;
      obj3[2] = tmp16(tmp19(4602).Stack, obj4);
      return tmp18(tmp19(7639).Form, obj3);
    } else {
      const result = tmp19(3810).isMemberCommunicationDisabled(member);
      const TableRowGroup = tmp19(5558).TableRowGroup;
      const obj16 = { hasIcons: false, children: null };
      const TableRow = tmp19(5224).TableRow;
      ...
      if (result) {
        ...
        obj18[0] = tmp21(4032).getName(user);
        ...
        const tmp21Result2 = tmp21(4032);
      } else {
        const obj19 = { user: null };
        obj19[0] = tmp21(4032).getName(user);
        ...
        const tmp21Result3 = tmp21(4032);
      }
      const tmp19Result = tmp19(3810);
    }
    const obj28 = importDefault(4032);
    tmp16 = closure_20;
  }
};
...
export default function MemberModalEdit(onClose) {
  ...
  const tmp = onRemove(5507)(guildId);
  c2 = tmp;
  ...
  items1 = [obj];
  return callback3(onClose(5574).Navigator, obj);
};
