...
function GuildSettingsOwnerConfiguredContentLevel(guild) {
  ...
  if (!userIsTeen) {
    ...
    obj1[2] = tmp8;
    obj1[3] = tmp7;
    obj[3] = callback(tmp3(5808).TableSwitchRow, obj1);
    tmp9 = callback(tmp3(5807).TableRowGroup, obj, "filter-section");
    const obj4 = combined;
  }
  return tmp9;
}
...
prototype["renderVerificationLevelSection"] = function renderVerificationLevelSection() {
  ...
  const verificationLevelOptions = self(14158).generateVerificationLevelOptions(features.has(constants.COMMUNITY));
  ...
  return callback(self(8030).TableRadioGroup, obj, "level-section");
};
prototype["renderExplicitContentFilter"] = function renderExplicitContentFilter() {
  ...
  const obj3 = combined;
  const features = guild.features;
  const contentFilterOptions = self(14158).generateContentFilterOptions(features.has(constants.COMMUNITY));
  ...
  return callback(self(8030).TableRadioGroup, obj, "filter-section");
};
