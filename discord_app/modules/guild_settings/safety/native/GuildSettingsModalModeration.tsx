...
function GuildSettingsOwnerConfiguredContentLevel(guild) {
  ...
  let obj = require(7713) /* useUserIsTeen */;
  ...
  if (!userIsTeen) {
    ...
    obj1[2] = tmp8;
    obj1[3] = tmp7;
    obj[3] = callback(tmp3(5559).TableSwitchRow, obj1);
    tmp9 = callback(tmp3(5558).TableRowGroup, obj, "filter-section");
...
prototype["renderVerificationLevelSection"] = function renderVerificationLevelSection() {
  ...
  const verificationLevelOptions = self(13887).generateVerificationLevelOptions(features.has(constants.COMMUNITY));
...
prototype["renderExplicitContentFilter"] = function renderExplicitContentFilter() {
  ...
  const features = guild.features;
  const contentFilterOptions = self(13887).generateContentFilterOptions(features.has(constants.COMMUNITY));
...
prototype["render"] = function render() {
  ...
  obj[2] = items1;
  obj[1] = closure_11(require(4602) /* Stack */.Stack, obj);
  const items2 = [closure_10(require(7639) /* Form */.Form, obj), closure_10(require(5505) /* NavScrim */.NavScrim, {})];
  obj1[0] = items2;
  return closure_11(closure_12, obj1);
};
prototype["componentWillUnmount"] = function componentWillUnmount() {
  if (this.props.hasChanges) {
    importDefault(8308).cancelChanges(tmp.props.guild.id);
    const obj = importDefault(8308);
  }
};
