// === Module 11334: TransferOwnership ===
...
export default function TransferOwnership(guild) {
          ...
          if (0 === v0) {
            if (arg0 === 1) {
              ...
              v0(arr[14]).transferOwnership(closure_1_0.id, v0.id, MFA).then(() => {
                ...
                v1(9649).close();
                const obj2 = v1(9649);
                const result = tmp2(4161).showTransferOwnershipSuccess();
              }, closure_1_12);
              arr = 3;
  ...
  obj3 = { source: toUser(11335), style: tmp5.arrow };
  ...
  items1[2] = callback2(guild(4441).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1236).intl;
      const obj9 = { GuildHook: null, user: null };
      obj9[0] = GuildWithSmallIcon;
      let tmp3Result = tmp3(4289);
    ...
    obj11[2] = tmp11(tmp(7616).TableCheckboxRow, obj12);
    items3[1] = tmp11(tmp(5944).TableRowGroup, obj11);
    ...
    obj14[2] = !tmp8;
    items3[2] = tmp11(tmp(4880).Button, obj14);
    obj[1] = items3;
  ...
  obj15[1] = toUser(4289).getUserTag(toUser);
  obj15[2] = function NicknameAKA() {
    ...
    const items = [closure_1_15(guild(4441).Text, obj), , ];
    let tmp6 = null;
    if (c7) {
    ...
    obj = { children: null };
    items[2] = closure_1_13(guild(4441).Text, { variant: "text-md/medium", children: str });
    obj[0] = items;
