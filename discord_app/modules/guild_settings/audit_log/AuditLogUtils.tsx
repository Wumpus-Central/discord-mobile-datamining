...
import GuildFeedItemTypes from "GuildFeedItemTypes" /* 18083 */;
...
const AuditLogChange = fn(18081).AuditLogChange;
...
const GuildOnboardingMode = fn(7434).GuildOnboardingMode;
...
let items = [fn(4858).TimeUnits.DAYS, fn(4858).TimeUnits.HOURS, fn(4858).TimeUnits.MINUTES, fn(4858).TimeUnits.SECONDS];
    ...
    if (AuditLogTargetTypes.CHANNEL !== targetType) {
      if (AuditLogTargetTypes.CHANNEL_OVERWRITE !== targetType) {
        if (AuditLogTargetTypes.USER === targetType) {
          ...
          const obj23 = {};
          obj23[__3TkD(8742).GuildInviteFlags.IS_GUEST_INVITE] = __3TkD(1115).t.XYZMbL;
          __3TkD = obj23;
            ...
            const item = changes.forEach((newValue) => {
              if (newValue.newValue) {
                if (newValue.newValue.permission) {
                  __3TkD[key] = () => closure_1_0(1115).t["JH+89C"];
                } else {
                  __3TkD[key] = () => closure_1_0(1115).t.HUrFDu;
                }
              } else {
    ...
    if (constants2.GUILD !== targetType) {
      ...
      if (constants2.GUILD_HOME !== targetType) {
        ...
        if (constants2.GUILD_PROFILE !== targetType) {
                      ...
                      if (constants2.GUILD_SCHEDULED_EVENT !== targetType) {
                        if (constants2.GUILD_SCHEDULED_EVENT_EXCEPTION !== targetType) {
                          if (constants2.THREAD === targetType) {
                            ...
                            let tmp10 = null;
                            if (tmp9) {
                              tmp10 = ((channel) => result3(4982).computeChannelName(channel, user, closure_1_12, true))(tmp7);
                            }
                            let tmp11 = tmp10;
                            if (null == tmp10) {
                              tmp11 = tmp10;
          ...
          let tmp208 = null;
          if (tmp207) {
            tmp208 = ((channel) => result3(4982).computeChannelName(channel, user, closure_1_12, true))(tmp205);
          }
          let tmp209 = tmp208;
          if (null == tmp208) {
            tmp209 = tmp208;
    ...
    if (null != tmp3) {
      ...
      if (null != result1.options) {
        ...
        if (null != result1.options.event_exception_id) {
          ...
          let obj5 = closure_0(4507);
          const tmp244 = items(4416);
          let str5;
          if (found20 != null) {
                                        ...
                                        if (AuditLogChangeKeys.AUTO_MODERATION_REMOVE_ALLOW_LIST !== key19) {
                                          if (AuditLogChangeKeys.AUTO_MODERATION_EXEMPT_CHANNELS === key19) {
                                            ...
                                            if (null != newValue.newValue) {
                                              ...
                                              newValue = obj.dateFormat(_modDef4416(date), "LLLL");
                                            }
                                            if (null != newValue.oldValue) {
                                              ...
                                              oldValue = obj2.dateFormat(_modDef4416(date1), "LLLL");
                                            }
                                            if (!oldValue) {
                                              oldValue = newValue.oldValue;
                                            }
