...
prototype["renderChangeSummary"] = function renderChangeSummary() {
  ...
  if (this.props.expanded) {
    renderChangeDetailsResult = self.renderChangeDetails(importAll(16616).getChangeStrings(tmp));
    const obj = importAll(16616);
  }
...
prototype["render"] = function render() {
  ...
  const items2 = [callback(user(16625), { action: log.action }), , , ];
  ...
  if (log.action !== constants.AUTO_MODERATION_BLOCK_MESSAGE) {
    if (log.action !== tmp21.AUTO_MODERATION_FLAG_TO_CHANNEL) {
      if (log.action !== tmp21.AUTO_MODERATION_USER_COMMUNICATION_DISABLED) {
        ...
        items2[1] = tmp17(tmp10(4846).PressableOpacity, obj4);
        ...
        obj6[5] = items3;
        items2[2] = tmp15(tmp10(4846).PressableOpacity, obj6);
        ...
        obj1[5] = items5;
        return tmp15(tmp10(5376).Card, obj1);
      }
    }
  }
