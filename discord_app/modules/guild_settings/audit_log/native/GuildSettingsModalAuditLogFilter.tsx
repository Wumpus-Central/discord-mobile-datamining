...
import { AuditLogFilterTypes } from "../../../../Constants.tsx";
...
const createCacheKey = { paddingHorizontal: ThemesDefault.space.PX_12, paddingVertical: ThemesDefault.space.PX_16 };
createCacheKey[0] = createCacheKey;
createCacheKey[1] = { height: 30, width: 30, alignItems: "center" };
let closure_12 = createCacheKey.createStyles(createCacheKey);
const result = require("obj132").fileFinishedImporting("modules/guild_settings/audit_log/native/GuildSettingsModalAuditLogFilter.tsx");
...
export default function GuildSettingsModalAuditLogFilter(data) {
  ...
  const memo = navigation.useMemo(() => ({
    data: data.filter((item, index) => {
      ...
      return filterType(guildId[14])(formatted, item.label.toLowerCase());
    }),
    keyExtractor(value) {
      if (null != value.value) {
  ...
  callback = navigation.useCallback((arg0, navigation) => {
    if (arg0) {
      let tmp = navigation;
  ...
  const callback1 = navigation.useCallback((item) => {
    ...
    if (selected === AuditLogFilterTypes.USER) {
      if (null !== value) {
        ...
        obj[4] = function onPress() {
          return callback(!selected, closure_0);
        };
        obj = { selected: null };
        obj[0] = selected;
    ...
    if (tmp === AuditLogFilterTypes.USER) {
      obj1 = { style: null, children: null };
      obj1[0] = obj.allUsersIconContainer;
      const obj2 = { size: null, source: null };
      obj2[0] = tmp5(guildId[19]).Icon.Sizes.MEDIUM;
      obj2[1] = filterType(guildId[20]);
      obj1[1] = closure_1_9(tmp5(guildId[19]).Icon, obj2);
  ...
  obj[1] = callback(data(guildId[22]).SearchField, { size: "md", placeholder: stringResult, onChange: tmp6[1] });
  const items4 = [callback(first, obj), , ];
  if (0 === data.length) {
    ...
    let tmp14Result = callback(tmp4(tmp3[19]).EmptyState, obj);
  } else {
    ...
    obj1[3] = obj2;
    tmp14Result = callback(tmp4(tmp3[24]).FlashList, obj1);
  }
  let obj3 = { children: null };
  items4[1] = tmp14Result;
  items4[2] = callback(data(guildId[25]).NavScrim, {});
  obj3[0] = items4;
  return callback2(closure_10, obj3);
};
export const createAuditLogFilterUserData = function createAuditLogFilterUserData(closure_5) {
  ...
  obj[2] = null == closure_5;
  items.push(obj);
  set = new Set();
  const logs = closure_7.logs;
  const item = logs.forEach((item, index) => {
  ...
  const item1 = userIds.forEach((item, index) => {
    if (null != item) {
...
export const createAuditLogFilterActionData = function createAuditLogFilterActionData(closure_4) {
  ...
  const mapped = require("../AuditLogUtils.tsx").ACTION_FILTER_ITEMS().map((item, index) => ({ label: item.label, value: item.value, selected: closure_0 === item.value, index }));
