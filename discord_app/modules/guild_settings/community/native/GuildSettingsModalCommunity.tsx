...
require = fn;
let closure_6 = fn(4394).GUILD_SELECTABLE_CHANNELS_KEY;
const calculateLocaleOptions = fn(16828).calculateLocaleOptions;
...
export default function GuildSettingsModalCommunity(guildId) {
  ...
  const callback2 = obj6.useCallback(() => {
    ...
    obj2.onItemSelect = function onItemSelect(rulesChannelId) {
      onClose(9861).updateGuild({ rulesChannelId });
      const obj = onClose(9861);
      const obj2 = { rulesChannelId };
      onClose(4724).hideActionSheet();
    };
    let id;
  ...
  const callback3 = obj6.useCallback(() => {
    ...
    obj2.onItemSelect = function onItemSelect(publicUpdatesChannelId) {
      onClose(9861).updateGuild({ publicUpdatesChannelId });
      const obj = onClose(9861);
  ...
  const callback4 = obj6.useCallback(() => {
    ...
    obj2.onItemSelect = function onItemSelect(safetyAlertsChannelId) {
      onClose(9861).updateGuild({ safetyAlertsChannelId });
      const obj = onClose(9861);
    ...
    obj2.selectedItem = id;
    obj.openLazy(asyncRequireImpl(9546, dependencyMap.paths), "SelectSafetyAlertsChannel", obj2);
  }, items9);
  if (guild != null) {
    preferredLocale = guild.preferredLocale;
  }
