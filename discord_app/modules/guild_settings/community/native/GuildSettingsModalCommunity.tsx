...
export default function GuildSettingsModalCommunity(guildId) {
  ...
  callback1 = obj5.useCallback(() => {
    ...
    if (null != channels) {
      ...
      const mapped = found.map((channel) => {
        ...
        const obj = {
          value: channel.id,
          label: closure_1_0(navigation[19]).computeChannelName(channel, closure_1_9, callback1, true),
        };
        return obj;
      });
    }
  ...
  const callback2 = obj5.useCallback(() => {
    ...
    obj.selectedItem = id;
    obj.openLazy(asyncRequireImpl(9458, dependencyMap.paths), "SelectRulesChannel", obj);
  }, items7);
  ...
  const callback3 = obj5.useCallback(() => {
    ...
    obj.selectedItem = id;
    obj.openLazy(asyncRequireImpl(9458, dependencyMap.paths), "SelectUpdatesChannel", obj);
  }, items8);
  if (guild != null) {
    preferredLocale = guild.preferredLocale;
  }
