...
export default function GuildSettingsAnalyticsCard(metricKey) {
  ...
  const items1 = [
    closure_6(metricKey(4823).Text, { variant: "text-md/medium", color: "text-subtle", children: title }),
  ];
  let tmp7Result = null;
  if (null != description) {
    const obj3 = {
    ...
    obj3.children = closure_6(tmp4(4781).CircleInformationIcon, obj4);
  ...
  if (null != subtext) {
    ...
    if (isTrendingUp) {
      ...
      tmp7Result3 = closure_6(tmp4(11758).ArrowLargeUpIcon, obj7);
    }
    ...
    if (isTrendingDown) {
      ...
      tmp7Result4 = closure_6(tmp4(18217).ArrowLargeDownIcon, obj8);
    }
    items3[1] = tmp7Result4;
    const obj9 = { variant: "text-xs/normal", color: "text-subtle", children: subtext };
    items3[2] = closure_6(tmp4(4823).Text, obj9);
