...
export default noop.memo(function GuildSettingsModalStickerInfoActionSheet(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj.header = closure_9(tmp(tmp2[12]).BottomSheetTitleHeader, obj);
    ...
    obj1.style = obj2;
    const obj3 = { icon: closure_9(tmp(tmp2[16]).PencilIcon, {}), label: null, onPress: null };
    let intl = tmp(tmp2[10]).intl;
    obj3.label = intl.string(tmp(tmp2[10]).t.tdhW5b);
    obj3.onPress = function onPress() {
      showGuildSettingsStickerCreateModalDefault({ guildId, stickerId });
    };
    const items2 = [closure_9(tmp(tmp2[15]).TableRow, obj3), ];
    const obj4 = { icon: closure_9(tmp(tmp2[18]).TrashIcon, { color: "text-feedback-critical" }), trailing: null, label: null, variant: "danger", disabled: null, onPress: null };
    tmp8Result = null;
    if (tmp5) {
    ...
    obj4.label = intl2.string(tmp(tmp2[10]).t["+ZhGOk"]);
    obj4.disabled = tmp5;
    ...
    items2[1] = closure_9(tmp(tmp2[15]).TableRow, obj4);
    obj5.children = items2;
    obj1.children = closure_10(tmp(tmp2[14]).TableRowGroup, obj5);
