// discord_app/modules/guild_tag/native/badges/GuildBadgeFire.tsx
import inlineStyles from "../../../../../_runtime/08732_inlineStyles.js";
import GuildBadgeUtils from "GuildBadgeUtils.tsx";
import noop from "../../../../../_runtime/metro/00019__.js";

require = fn;
const jsxProd = fn(21);
({ jsx: c2, jsxs: c3 } = jsxProd);
const primaryBaseColors = ["#ba3500", "#fd6214", "#f0f0f0"];
const primaryTintLuminances = [0.1, 0.3, 1];
let items = [
  { base: 3, tint: 1 },
  { base: 3, tint: 1 },
  { base: 10, tint: 1 },
];
const size = fn(2);
const result = size.fileFinishedImporting("modules/guild_tag/native/badges/GuildBadgeFire.tsx");

export const GuildBadgeFire = function GuildBadgeFire(primaryTintColor) {
  let num = primaryTintColor.width;
  if (num === undefined) {
    num = 24;
  }
  let num2 = primaryTintColor.height;
  if (num2 === undefined) {
    num2 = 24;
  }
  const merged = Object.assign(primaryTintColor, Object.assign({ width: 0, height: 0, primaryTintColor: 0 }));
  const primaryColorsTransformed = GuildBadgeUtils.getTransformedBadgeColors({
    primaryBaseColors,
    primaryTintColor: primaryTintColor.primaryTintColor,
    primaryTintLuminances,
    primaryLuminanceWeights: items,
  }).primaryColorsTransformed;
  const obj3 = {};
  const merged1 = Object.assign(merged);
  obj3.width = num;
  obj3.height = num2;
  obj3.viewBox = "0 0 16 16";
  obj3.fill = "none";
  items = [
    React2(inlineStyles.Path, {
      d: "M13 6V4h-1V1h-1v1h-1v1H9v1H8v1H3v1H2v2H1v4h1v1h1v1h1v1h8v-1h1v-1h1V6h-1Z",
      fill: primaryColorsTransformed[1],
    }),
    React2(inlineStyles.Path, { d: "M0 8v4h1V8H0ZM15 9v3h1V9h-1ZM14 6v3h1V6h-1ZM13 4v2h1V4h-1Z", fill: "#000" }),
    React2(inlineStyles.Path, {
      d: "M14 9v3h1V9h-1ZM13 6v3h1V6h-1ZM12 4v2h1V4h-1Z",
      fill: primaryColorsTransformed[0],
    }),
    React2(inlineStyles.Path, {
      d: "M1 6v2h1V6H1ZM7 3H6v1h1V3ZM9 3H8v1h1V3ZM8 4H7v1h1V4ZM2 5v1h1V5H2ZM1 12v1h1v-1H1ZM2 13v1h1v-1H2ZM3 14v1h1v-1H3ZM14 12v1h1v-1h-1ZM13 13v1h1v-1h-1ZM12 14v1h1v-1h-1ZM4 15v1h8v-1H4ZM3 4v1h1V4H3ZM4 3v1h1V3H4ZM5 2v1h1V2H5ZM9 2v1h1V2H9ZM10 1v1h1V1h-1ZM11 0v1h1V0h-1Z",
      fill: "#000",
    }),
    React2(inlineStyles.Path, {
      d: "M11 1v1h1V1h-1ZM10 2v1h1V2h-1ZM9 3v1h1V3H9ZM8 4v1h1V4H8ZM5 3v1h1V3H5ZM4 4v1h1V4H4ZM6 4v1h1V4H6Z",
      fill: primaryColorsTransformed[2],
    }),
    React2(inlineStyles.Path, { d: "M5 4v1h1V4H5Z", fill: primaryColorsTransformed[1] }),
    React2(inlineStyles.Path, {
      opacity: 0.5,
      d: "M11 4v1h1V4h-1ZM12 9V8h-1V6h-1v1H9v1H6V7H5v2H3v2h1v2h1v1h6v-1h1v-2h1V9h-1Z",
      fill: "#fff",
    }),
    React2(inlineStyles.Path, { d: "M3 5v1h1V5H3ZM2 6v2h1V6H2ZM4 8v1h1V8H4Z", fill: primaryColorsTransformed[2] }),
    React2(inlineStyles.Path, {
      d: "M2 12v1h1v-1H2ZM3 13v1h1v-1H3ZM12 13v1h1v-1h-1ZM13 12v1h1v-1h-1ZM4 14v1h8v-1H4Z",
      fill: primaryColorsTransformed[0],
    }),
    React2(inlineStyles.Path, {
      d: "M1 8v2h1V8H1ZM11 10h-1V9H6v1H5v2h1v1h4v-1h1v-2Z",
      fill: primaryColorsTransformed[2],
    }),
    React2(inlineStyles.Path, { d: "M12 1v3h1V1h-1Z", fill: "#000" }),
  ];
  obj3.children = items;
  return React3(inlineStyles.Svg, obj3);
};
