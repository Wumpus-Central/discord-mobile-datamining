...
function useIsTopicalNavEnabled(guild_id, channel_header) {
  ...
  const tmp5 =
    require("initialize").useStateFromStores(
      ...
      items1,
    ) && apexExperiment.useConfig({ location: channel_header }).enabled;
  const items2 = [GuildStore];
  const items3 = [guild_id, CONVERSATIONS_EXTRACTION_PROCESSING2];
  let enabled = require("initialize").useStateFromStores(
    items2,
    () => {
      let tmp2 = null != closure_0;
      if (tmp2) {
...
export const useIsConversationTopicHeaderEnabled = function useIsConversationTopicHeaderEnabled(
  guild_id,
