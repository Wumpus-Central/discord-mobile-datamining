...
export default function GuildProfileGamesActionSheet(profile) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(9254, dependencyMap.paths), "GuildProfileActionSheet:" + id, obj);
  }, items);
  ...
  const tmp = closure_7();
  obj1.children = jsx(id(5737).TableRowGroup, { title: str, hasIcons: true, children: allGuildProfileGames.map((game) => <GuildProfileGameRow key={game.id} game={game} activityLevel={gameActivity[game.id]} />) });
  obj.children = <View style={closure_7().container}>{null}</View>;
  obj.children = jsx(id(6677).BottomSheetScrollView, { children: null });
  return jsx(id(7202).BottomSheet, { children: null });
};