    ...
    if (null != invite) {
      if (invite.state !== validInviteKey.BANNED) {
        if (invite.state !== tmp8.EXPIRED) {
          ...
          obj[1] = id(1384).hasFlag(num, id(6712).GuildInviteFlags.IS_APPLICATION_BYPASS);
          obj[2] = invite.roles;
          const obj2 = id(1384);
          const tmp6 = id;
        }
  ...
  if (null != invite) {
    ...
    if (invite.state !== constants.BANNED) {
      ...
      if (invite.state !== tmp12.EXPIRED) {
        ...
        flag = obj.hasFlag(num, set(6712).GuildInviteFlags.IS_APPLICATION_BYPASS);
        tmp11 = inviteKeyForGuildId;
        const tmp13 = set;
      }
    }
  }
  const pendingFolderGuildIds = set(10998).getPendingFolderGuildIds();
  if (tmp7) {
    let roles;
    if (invite != null) {
      roles = invite.roles;
