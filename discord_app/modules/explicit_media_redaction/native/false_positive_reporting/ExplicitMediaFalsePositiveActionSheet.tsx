...
function ExplicitMediaFalsePositivePreview(url) {
  ...
  if (obj.isVideo(url)) {
    ...
    const obj1 = { uri: url };
    obj.source = obj1;
    let tmp3Result = React5(_modDef8426, obj);
  } else {
...
fn(4636);
let createStyles = { content: null, contentContainer: null, heading: null, mediaContainer: null, elevationShadow: null, image: null, media: null, footer: null };
...
createStyles.mediaContainer = { width: "100%", padding: nativeDefault.space.PX_16, backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH, borderRadius: nativeDefault.radii.xs, marginTop: nativeDefault.space.PX_8, aspectRatio: "4 / 3" };
...
let obj2 = { width: "100%", padding: nativeDefault.space.PX_16, backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH, borderRadius: nativeDefault.radii.xs, marginTop: nativeDefault.space.PX_8, aspectRatio: "4 / 3" };
createStyles.media = { flex: 1, borderRadius: nativeDefault.radii.xs };
let obj3 = { flex: 1, borderRadius: nativeDefault.radii.xs };
createStyles.footer = { backgroundColor: nativeDefault.colors.MOBILE_ACTIONSHEET_GRADIENT_BACKGROUND_DEFAULT, paddingVertical: nativeDefault.space.PX_12, paddingHorizontal: nativeDefault.space.PX_16, gap: nativeDefault.space.PX_8 };
...
export const handleSuccess = function handleSuccess(arg0) {
  ...
  obj = { key: "explicit_media_report_false_positive_success", icon: _modDef9531, IconComponent: ShieldIcon.ShieldIcon, iconColor: "text-brand", content: null };
...
export const ExplicitMediaFalsePositiveActionSheet = function ExplicitMediaFalsePositiveActionSheet(channelId) {
  ...
  const callback = analyticsContext.useCallback(() => {
    const obj = { action: ExplicitMediaRedactionUtils.TrackMediaRedactionActionType.EXPLICIT_MEDIA_FALSE_POSITIVE_CLICK_CANCEL, channelId, messageId, context: analyticsContext };
  ...
  const callback1 = analyticsContext.useCallback(() => {
    ...
    const obj = { action: ExplicitMediaRedactionUtils.TrackMediaRedactionActionType.EXPLICIT_MEDIA_FALSE_POSITIVE_CLICK_CONFIRM, channelId, messageId, context: analyticsContext };
    const result = obj.trackMediaRedactionAction(obj);
  }, items1);
  const effect = analyticsContext.useEffect(() => {
    const obj = { action: ExplicitMediaRedactionUtils.TrackMediaRedactionActionType.EXPLICIT_MEDIA_FALSE_POSITIVE_VIEWED, channelId, messageId, context: analyticsContext };
    const result = obj.trackMediaRedactionAction(obj);
  }, items2);
