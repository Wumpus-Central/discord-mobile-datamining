...
import uniqWithDefault from "../../../_runtime/07808_uniqWith.js";
...
function handleUnscannedMessages(found2, isMessageUpdate) {
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == closure_14[combined]) {
      let obj2 = { name: channel_id(7803).MetricEvents.EXPLICIT_MEDIA_SCAN_CLIENT_TIMEOUT_CREATE };
      ...
      obj3.timeout = setTimeout(() => {
        if (maybeCancelTimeout(channel_id, found1(7796).TimeoutCancelSource.TIMEOUT)) {
          message = message.getMessage(channel_id.channel_id, channel_id.id);
          if (null != message) {
            const unscannedMediaIds = found1(7488).getUnscannedMediaIds(message);
            ({ attachmentIds, embedIds } = unscannedMediaIds);
            const tmp2Result = found1(7488);
            ...
            const result = found1(7796).trackScanningTimedOut(obj);
            const tmp2Result2 = found1(7796);
          }
  ...
  const item = arr4.forEach((channel_id) => {
    ...
    if (attachments1 != null) {
      num3 = attachments1.filter((media) => {
        ...
        return obj.isMediaFlaggedForHarmType(closure_1_0(7491).ContentHarmType.EXPLICIT, {
          type: closure_1_0(7493).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
    }
    ...
    if (embeds1 != null) {
      num4 = embeds1.filter((media) => {
        obj = closure_1_0(7488);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7491).ContentHarmType.EXPLICIT, {
          type: closure_1_0(7493).ObscuredMediaTypes.Embed,
          media,
        });
      }).length;
    }
    ...
    if (attachments2 != null) {
      num5 = attachments2.filter((media) => {
        obj = closure_1_0(7488);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7491).ContentHarmType.GORE, {
          type: closure_1_0(7493).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
    ...
    if (attachments3 != null) {
      num7 = attachments3.filter((media) => {
        obj = closure_1_0(7488);
        return obj.isMediaFlaggedForHarmType(closure_1_0(7491).ContentHarmType.SELF_HARM, {
          type: closure_1_0(7493).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
