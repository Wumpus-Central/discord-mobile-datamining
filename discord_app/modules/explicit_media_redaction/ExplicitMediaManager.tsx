      ...
      if (null != dependencyMap[combined]) {
        ...
        if (UPDATE === redactionSettingToRenderedString.TimeoutCancelSource.UPDATE) {
          ...
          const found = attachments.filter((arg0) => {
            let obj = callback(5830);
            obj = { type: null, media: null };
            obj[0] = callback(5835).ObscuredMediaTypes.Attachment;
            obj[1] = arg0;
            return obj.isMediaFlaggedForHarmType(callback(5833).ContentHarmType.EXPLICIT, obj);
          });
          const found1 = embeds.filter((arg0) => {
            let obj = callback(5830);
            obj = { type: null, media: null };
            obj[0] = callback(5835).ObscuredMediaTypes.Embed;
            obj[1] = arg0;
            return obj.isMediaFlaggedForHarmType(callback(5833).ContentHarmType.EXPLICIT, obj);
          });
          let tmp5Result = tmp5(6980);
...
function handleUnscannedMessages(found2, isMessageUpdate) {
  ...
  if (isMessageUpdate) {
    let found = filter((message) => {
      let result = found1(5830).isEligibleForScanning(message);
      if (result) {
        result = found1(5830).hasUnscannedMedia(message);
        const tmpResult = found1(5830);
      }
      return result;
    });
    let found1 = found.filter(withoutScheduledTimeout);
  } else {
    found2 = filter((components) => found1(5830).isEligibleForScanning(components));
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == table[combined]) {
      let obj = tmp(6982);
      obj = { name: null };
      obj[0] = callback(6987).MetricEvents.EXPLICIT_MEDIA_SCAN_CLIENT_TIMEOUT_CREATE;
      obj.increment(obj);
...
function processMessagesFromAction(firstMessages, isMessageUpdate) {
  ...
  const arr4 = obj(6992)(tmp2, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
  ...
  const item = arr4.forEach((channel_id) => {
    ...
    if (attachments2 != null) {
      num5 = attachments2.filter((arg0) => {
        ...
        obj[0] = callback(5835).ObscuredMediaTypes.Attachment;
        obj[1] = arg0;
        return obj.isMediaFlaggedForHarmType(callback(5833).ContentHarmType.GORE, obj);
      }).length;
    }
    if (num5 == null) {
      num5 = 0;
    ...
    if (attachments3 != null) {
      num7 = attachments3.filter((arg0) => {
        ...
        obj[0] = callback(5835).ObscuredMediaTypes.Attachment;
        obj[1] = arg0;
        return obj.isMediaFlaggedForHarmType(callback(5833).ContentHarmType.SELF_HARM, obj);
      }).length;
    }
    if (num7 == null) {
      num7 = 0;
    ...
    if (null != message.id) {
      if (obj6.getChannelIdAndAuthorIdFromMessage(message).authorId !== store.getId()) {
        ...
        if (!tmp20Result1.hasUnscannedMedia(message)) {
          ...
          if (!hasUnscannedMediaResult) {
            maybeCancelTimeout(message, tmp20(6980).TimeoutCancelSource.UPDATE);
          }
        }
...
function maybeScanMessagesForChannelId(channelId) {
  ...
  if (tmp) {
    const found = messages.filter((message) => {
      ...
      let result = obj.hasAttachmentsEmbedsComponentsOrSnapshots(message);
      if (result) {
        result = 0 !== obj(5830).getEnabledHarmTypesForMessage(message);
        const tmpResult = obj(5830);
      }
      return result;
    });
    ...
    const arr5 = obj(6992)(tmp3, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
