...
import uniqWithDefault from "../../../_runtime/07026_uniqWith.js";
...
function handleUnscannedMessages(found2, isMessageUpdate) {
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == closure_14[combined]) {
      let obj2 = { name: channel_id(5184).MetricEvents.EXPLICIT_MEDIA_SCAN_CLIENT_TIMEOUT_CREATE };
      ...
      obj3.timeout = setTimeout(() => {
        if (maybeCancelTimeout(channel_id, found1(7020).TimeoutCancelSource.TIMEOUT)) {
          message = message.getMessage(channel_id.channel_id, channel_id.id);
          if (null != message) {
            const unscannedMediaIds = found1(6710).getUnscannedMediaIds(message);
            ({ attachmentIds, embedIds } = unscannedMediaIds);
            const tmp2Result = found1(6710);
            ...
            const result = found1(7020).trackScanningTimedOut(obj);
            const tmp2Result2 = found1(7020);
          }
  ...
  const item = arr4.forEach((channel_id) => {
    ...
    if (attachments1 != null) {
      num3 = attachments1.filter((media) => {
        ...
        return obj.isMediaFlaggedForHarmType(closure_1_0(6713).ContentHarmType.EXPLICIT, {
          type: closure_1_0(6715).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
    }
    ...
    if (embeds1 != null) {
      num4 = embeds1.filter((media) => {
        obj = closure_1_0(6710);
        return obj.isMediaFlaggedForHarmType(closure_1_0(6713).ContentHarmType.EXPLICIT, {
          type: closure_1_0(6715).ObscuredMediaTypes.Embed,
          media,
        });
      }).length;
    }
    ...
    if (attachments2 != null) {
      num5 = attachments2.filter((media) => {
        obj = closure_1_0(6710);
        return obj.isMediaFlaggedForHarmType(closure_1_0(6713).ContentHarmType.GORE, {
          type: closure_1_0(6715).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
    ...
    if (attachments3 != null) {
      num7 = attachments3.filter((media) => {
        obj = closure_1_0(6710);
        return obj.isMediaFlaggedForHarmType(closure_1_0(6713).ContentHarmType.SELF_HARM, {
          type: closure_1_0(6715).ObscuredMediaTypes.Attachment,
          media,
        });
      }).length;
