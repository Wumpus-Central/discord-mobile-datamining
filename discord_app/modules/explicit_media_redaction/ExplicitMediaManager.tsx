...
import closure_6 from "handlePermissionsChange" /* 6127 */;
...
import closure_11 from "handleSearchMessagesSuccess" /* 7354 */;
      ...
      if (null != dependencyMap[combined]) {
        ...
        if (UPDATE === redactionSettingToRenderedString.TimeoutCancelSource.UPDATE) {
          ...
          const found = attachments.filter((media) => {
            let obj = callback(6138);
            obj = { type: callback(6143).ObscuredMediaTypes.Attachment, media };
            return obj.isMediaFlaggedForHarmType(callback(6141).ContentHarmType.EXPLICIT, obj);
          });
          const found1 = embeds.filter((media) => {
            let obj = callback(6138);
            obj = { type: callback(6143).ObscuredMediaTypes.Embed, media };
            return obj.isMediaFlaggedForHarmType(callback(6141).ContentHarmType.EXPLICIT, obj);
          });
...
function handleUnscannedMessages(found2, isMessageUpdate) {
  ...
  if (isMessageUpdate) {
    let found = filter((message) => {
      let result = found1(6138).isEligibleForScanning(message);
      if (result) {
        result = found1(6138).hasUnscannedMedia(message);
        const tmpResult = found1(6138);
      }
      return result;
    });
    let found1 = found.filter(withoutScheduledTimeout);
  } else {
    found2 = filter((components) => found1(6138).isEligibleForScanning(components));
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == table[combined]) {
      let obj = callback2(7347);
      obj = { name: null };
      obj[0] = callback(7352).MetricEvents.EXPLICIT_MEDIA_SCAN_CLIENT_TIMEOUT_CREATE;
      obj.increment(obj);
...
function processMessagesFromAction(firstMessages, isMessageUpdate) {
  ...
  const arr4 = obj(7357)(tmp2, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
  ...
  const item = arr4.forEach((channel_id) => {
    ...
    if (attachments2 != null) {
      num5 = attachments2.filter((media) => {
        obj = callback(6138);
        obj = { type: callback(6143).ObscuredMediaTypes.Attachment, media };
        return obj.isMediaFlaggedForHarmType(callback(6141).ContentHarmType.GORE, obj);
      }).length;
    }
    if (num5 == null) {
      num5 = 0;
    ...
    if (attachments3 != null) {
      num7 = attachments3.filter((media) => {
        obj = callback(6138);
        obj = { type: callback(6143).ObscuredMediaTypes.Attachment, media };
        return obj.isMediaFlaggedForHarmType(callback(6141).ContentHarmType.SELF_HARM, obj);
      }).length;
    }
    if (num7 == null) {
      num7 = 0;
    ...
    if (null != message.id) {
      if (obj6.getChannelIdAndAuthorIdFromMessage(message).authorId !== store.getId()) {
        ...
        if (!tmp20Result1.hasUnscannedMedia(message)) {
          ...
          if (!hasUnscannedMediaResult) {
            maybeCancelTimeout(message, tmp20(7345).TimeoutCancelSource.UPDATE);
          }
        }
...
function maybeScanMessagesForChannelId(channelId) {
  ...
  if (tmp) {
    const found = messages.filter((message) => {
      ...
      let result = obj.hasAttachmentsEmbedsComponentsOrSnapshots(message);
      if (result) {
        result = 0 !== obj(6138).getEnabledHarmTypesForMessage(message);
        const tmpResult = obj(6138);
      }
      return result;
    });
    ...
    const arr5 = obj(7357)(tmp3, (id, id2) => id.id === id2.id && id.channel_id === id2.channel_id);
