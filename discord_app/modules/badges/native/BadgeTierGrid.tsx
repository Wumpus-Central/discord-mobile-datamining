// discord_app/modules/badges/native/BadgeTierGrid.tsx
...
export default function BadgeTierGrid(badge) {
  ...
  obj4.children = tiers.map((owned) => {
    ...
    const tierRowSubtitle = BadgeUtils.getTierRowSubtitle({
      tier: owned,
      isUnlocked: owned,
      isViewingOtherUser,
      isViewerOnUpgradeableNitro,
    });
    ...
    if (tmp13Result) {
      ...
      tmp13Result = React4(Text_Text.Text, obj5);
    }
    ...
    if (tmp5Result) {
      ...
      if (!owned) {
        ...
        tmp15 = React4(LockIcon.LockIcon, obj7);
      }
      const items3 = [tmp15];
      let str2 = "text-muted";
      if (owned) {
      ...
      tmp5Result = hasOwnProperty(View, obj6);
    }
    items2[2] = tmp5Result;
