  ...
  if (badges.length > 0) {
    ...
    obj2.children = map((accessibilityLabel) => {
      const obj = {
        ...
        children: closure_2_10(BadgeCatalogIconDefault, { badge: accessibilityLabel, size: 44 }),
      };
...
let closure_9 = fn(11543).UserProfileEditAutoFocusElement;
const jsxProd = fn(21);
...
export default function BadgeDirectoryView(targetUserId) {
  ...
  let items = [
    ...
    {
      paddingBottom:
        targetUserId(stateFromStoresArray[12])().bottom + targetUserId(stateFromStoresArray[8]).space.PX_16,
    },
  ];
  let obj = {
    paddingBottom: targetUserId(stateFromStoresArray[12])().bottom + targetUserId(stateFromStoresArray[8]).space.PX_16,
  };
  ...
  const items4 = [tmp9];
  stateFromStoresArray = targetUserId(stringResult1[13]).useStateFromStoresArray(
    items3,
    () => {
      if (null != targetUserId) {
  ...
  const items6 = [tmp9];
  const stateFromStores2 = targetUserId(stringResult1[13]).useStateFromStores(
    items5,
    () => {
  ...
  const items8 = [tmp9];
  const items9 = [tmp9];
  const stateFromStores3 = targetUserId(stringResult1[13]).useStateFromStores(
    items7,
  ...
  if (null != targetUserId && targetUserId !== stateFromStores) {
    ...
    if (!tmp8) {
      ...
      tmp28Result = closure_10(BadgeSection, obj14);
    }
    ...
    if (tmp8) {
      ...
      items = closure_10(tmp5(stringResult1[19]).Button, obj16);
      obj15.children = items;
      let tmp28Result2 = closure_10(closure_5, obj15);
    } else {
      ...
      if (tmp28Result2) {
        ...
        obj17.children = closure_10(tmp5(stringResult1[19]).Button, obj18);
        tmp28Result2 = closure_10(closure_5, obj17);
      }
    }
  ...
  const tmp5Result4 = targetUserId(stringResult1[13]);
}
