...
function useNativeIAPPayments() {
  ...
  const tmp2 = nativePaymentsConnected(
    genericProductsLoaded(504).useStateFromStoresArray(items, () => {
  ...
  const canMakePayments = genericProductsLoaded(504).useStateFromStores(items1, () =>
    GenericIAPStore.canMakePurchase(),
  );
  ...
  storeFront(11016)(() => {
    ...
    if (!tmp) {
      ...
      let mapped;
      if (closure_2 != null) {
        mapped = closure_2.map((identifier) => identifier.identifier);
      }
      if (mapped == null) {
        mapped = [];
      }
...
function useCreateSubscription(planId) {
  ...
  const items1 = [planId];
  const stateFromStores = planId(applicationId[9]).useStateFromStores(
    items,
    () => SubscriptionPlanStore.get(planId),
    items1,
  );
  ...
  callback = noop.useCallback(() => {
    ...
    const result = obj.trackPaymentFlowStartedAnalyticsAndCTP({
      load_id,
      payment_type: closure_2_10[constants.SUBSCRIPTION],
      subscription_plan_gateway_plan_id: ProductIds.ProductIds.GENERIC_SUBSCRIPTION,
      subscription_type: constants2.GUILD,
      is_gift: false,
  ...
  obj2.createSubscription = noop.useCallback(
...
function useCancelSubscription(arg0, arg1) {
  ...
  const stateFromStores = require("initialize").useStateFromStores(
    items,
    () => SubscriptionStore.getSubscriptionById(closure_0),
    items1,
  );
  ...
  obj2.cancelSubscription = memo.useCallback(
    storeFront(function* () {
      ...
      yield tmp2(7661).cancelGenericSubscription(closure_6.requestIdentifier, closure_6.subscriptionId, closure_1);
      closure_128_0 = value;
...
function useResubscribeSubscription(arg0) {
  ...
  obj2.resubscribeSubscription = noop.useCallback(
    stateFromStores(function* () {
      ...
      yield tmp2(7661).resubscribeGenericSubscription(closure_5, stateFromStores.isACOM);
      closure_128_0 = value;
      yield tmp2(5079).fetchSubscriptions();
      return closure_128_0;
    }),
