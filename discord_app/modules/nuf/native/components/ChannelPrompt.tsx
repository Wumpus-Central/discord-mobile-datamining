...
let obj2 = {
  flex: { flex: 1 },
  safePadding: { marginTop: fn(5767).NAV_BAR_HEIGHT, flex: 1 },
...
export default function ChannelPrompt(guildId) {
  ...
  const callback = value.useCallback(
    onCancel(function* () {
      ...
      yield tmp3(tmp33[13]).createTextChannel(
        guildId,
        topic,
        parent_id,
        intl.formatToPlainString(closure_0(tmp33[12]).t.V4lepJ, { topic }),
      );
      if (1 === tmp7) {
        c3 = 0;
        closure_128_0 = tmp33;
  ...
  if (null != stateFromStores) {
    ...
    if (hasFieldErrorsResult) {
      ...
      tmp14Result = closure_10(tmp17(tmp3[21]), obj15);
      const tmp17Result4 = tmp17(tmp3[21]);
    }
    items3[6] = tmp14Result;
    obj5.children = items3;
    obj4.children = closure_11(tmp16, obj5);
    tmp14Result2 = closure_10(tmp2(tmp3[15]).SafeAreaPaddingView, obj4);
