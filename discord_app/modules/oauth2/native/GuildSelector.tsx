...
export default function GuildSelector(disabled) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(9619, dependencyMap.paths), GuildSelector, obj2);
  ...
  const items1 = [closure_7(selectedGuildId(4823).Text, obj2), , , ];
  let tmp6Result = null;
  if (null != error) {
    tmp6Result = null;
    if ("" !== error) {
  ...
  items1[2] = closure_7(selectedGuildId(8947).FormRow, { label: name, disabled: disabled.disabled, trailing: closure_7(selectedGuildId(8947).FormRow.Arrow, {}), DEPRECATED_style: tmp.select, onPress: callback });
