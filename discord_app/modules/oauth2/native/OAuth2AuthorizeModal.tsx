...
let result = require("obj132").fileFinishedImporting("modules/oauth2/native/OAuth2AuthorizeModal.tsx");
...
export default function OAuth2AuthorizeModal(arg0) {
  ...
  callback = React.useCallback((nativeEvent) => {
    ...
    const result = sharedValue.set(lib(dependencyMap[8]).withSpring(num, closure_1_7));
  ...
  if (null != tmp4.backStep) {
    ...
    let tmp11Result = callback(tmp5(10767).ArrowSmallLeftIcon, obj3);
  } else {
    const obj4 = { color: null };
    obj4[0] = tmp2(712).colors.INTERACTIVE_TEXT_DEFAULT;
    tmp11Result = callback(tmp5(6368).XSmallIcon, obj4);
  }
  obj2[3] = tmp11Result;
  const items2 = [callback(require("../../../design/void/Pressables/native/Pressables.tsx").PressableOpacity, obj2), , ];
  ...
  obj5[1] = callback(require("../../../design/components/Text/native/Text.tsx").Text, obj6);
  items2[1] = callback(View, obj5);
  const items3 = [tmp.titleContainerBorder, animatedStyle];
  items2[2] = callback(sharedValue(4115).View, { style: items3 });
  obj[1] = items2;
  ...
  obj7[2] = callback(sharedValue(10769), obj8);
  items4[1] = callback(require("../../../components_native/common/SafeAreaView.tsx").SafeAreaPaddingView, obj7);
  obj[1] = items4;
