...
export const useGameFriendsForUser = function useGameFriendsForUser(id) {
  ...
  return callback(
    _require(FRIEND[3]).useStateFromStores(
      items,
      () => {
        const items = [
          closure_1_3.getGameRelationshipsForUserByType(closure_0, PENDING_INCOMING),
          closure_1_3.getGameRelationshipsVersion(),
        ];
        return items;
      },
      items1,
      _require(FRIEND[4]).isVersionEqual,
    ),
    1,
  )[0];
};
export const useIncomingGameRelationshipsForUser = function useIncomingGameRelationshipsForUser(id) {
  ...
  return callback(
    _require(PENDING_INCOMING[3]).useStateFromStores(
      items,
      () => {
        const items = [
          closure_1_3.getGameRelationshipsForUserByType(closure_0, PENDING_INCOMING),
      ...
      items1,
      _require(PENDING_INCOMING[4]).isVersionEqual,
    ),
    1,
  )[0];
};
