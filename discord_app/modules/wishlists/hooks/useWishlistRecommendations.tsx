// === Module 10236: useWishlistRecommendationsWithWishlists ===
...
import closure_7 from "handleUserSettingsStoreUpdate" /* 10237 */;
import importDefaultResult from "fromServer" /* 5955 */;
import { WishlistRecommendationReason as closure_8 } from "fromServer" /* 5955 */;
...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  if (source === undefined) {
    source = userId(9503).WishlistFetchSource.USER_PROFILE;
  }
  const items = [userId];
  ...
  obj = {
    ...
    applicationIds: userId(10238).useWishlistApplicationIds(userId),
    numItems,
    source
  };
  ...
  const wishlistSkuFilter = userId(10239).useWishlistSkuFilter({ wishlistAndRecommendations, skusToUserAndReason, userId, numItems });
...
export const useRecommendationsForSingleUser = function useRecommendationsForSingleUser(source) {
  ...
  if (USER_PROFILE === undefined) {
    USER_PROFILE = userId(9503).WishlistFetchSource.USER_PROFILE;
  }
  let recommendations;
  ...
  obj[1] = userId(10238).useWishlistApplicationIds(userId);
  obj[2] = numItems;
  obj[3] = USER_PROFILE;
  const tmp4 = useWishlistRecommendationsWithWishlists(obj);
