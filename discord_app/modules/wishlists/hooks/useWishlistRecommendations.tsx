// === Module 10385: useWishlistRecommendationsWithWishlists ===
...
import closure_7 from "handleUserSettingsStoreUpdate" /* 10386 */;
import importDefaultResult from "fromServer" /* 7239 */;
import { WishlistRecommendationReason as closure_8 } from "fromServer" /* 7239 */;
...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  if (source === undefined) {
    source = userId(9222).WishlistFetchSource.USER_PROFILE;
  }
  const items = [userId];
  ...
  obj = {
    ...
    applicationIds: userId(10387).useWishlistApplicationIds(userId),
    numItems,
    source
  };
  const obj3 = userId(10387);
  ...
  const wishlistSkuFilter = userId(10388).useWishlistSkuFilter({ wishlistAndRecommendations, skusToUserAndReason, userId, numItems });
...
export const useRecommendationsForSingleUser = function useRecommendationsForSingleUser(source) {
  ...
  if (USER_PROFILE === undefined) {
    USER_PROFILE = userId(9222).WishlistFetchSource.USER_PROFILE;
  }
  let recommendations;
  ...
  obj[1] = userId(10387).useWishlistApplicationIds(userId);
  obj[2] = numItems;
  obj[3] = USER_PROFILE;
  const tmp4 = useWishlistRecommendationsWithWishlists(obj);
