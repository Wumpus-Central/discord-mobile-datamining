...
function useWishlistRecommendationsWithWishlists(userIdsAndWishlistIds) {
  ...
  let stateFromStores = userIdsAndWishlistIds(memo3[7]).useStateFromStores(items1, () =>
    WishlistRecommendationsStore.getRecommendations(userIdsAndWishlistIds, memo3),
  );
...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  const wishlistSkuFilter = userId(11139).useWishlistSkuFilter({
    wishlistAndRecommendations,
    skusToUserAndReason,
    userId,
    numItems,
...
export const useRecommendationsForApplicationIds = function useRecommendationsForApplicationIds(userIds) {
  ...
  const stateFromStoresArray = userIds(wishlistAndRecommendations[7]).useStateFromStoresArray(items2, () =>
    userIds.map((item) => {
  ...
  obj.userIdsAndWishlistIds = noop.useMemo(
    () => userIds.map((userId, index) => ({ userId, wishlistId: numItems[index] })),
    items3,
  );
