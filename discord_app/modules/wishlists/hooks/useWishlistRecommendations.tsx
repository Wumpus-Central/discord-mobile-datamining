...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  if (source === undefined) {
    source = userId(8683).WishlistFetchSource.USER_PROFILE;
  }
  const items = [userId];
  ...
  obj[1] = userId(9765).useWishlistApplicationIds(userId);
  obj[2] = numItems;
  obj[3] = source;
  const obj3 = userId(9765);
  ...
  const wishlistSkuFilter = userId(9766).useWishlistSkuFilter({ wishlistAndRecommendations, skusToUserAndReason, userId, numItems });
...
export const useRecommendationsForSingleUser = function useRecommendationsForSingleUser(source) {
  ...
  if (USER_PROFILE === undefined) {
    USER_PROFILE = userId(8683).WishlistFetchSource.USER_PROFILE;
  }
  let recommendations;
