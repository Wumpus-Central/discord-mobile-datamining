...
function useWishlistRecommendationsWithWishlists(userIdsAndWishlistIds) {
  ...
  const stateFromStores = applicationIds(memo3[7]).useStateFromStores(items1, () =>
    recommendations.getRecommendations(applicationIds, memo3),
  );
  const effect = memo1.useEffect(() => {
    if (0 !== applicationIds.length) {
      if (0 !== memo3.length) {
        ...
        const wishlistRecommendations = stateFromStores1(memo3[10]).fetchWishlistRecommendations(
          tmp8,
          tmp,
          stateFromStores1,
        );
  ...
  const getOrFetchStorefrontPricesForSkuIds = applicationIds(memo3[12]).useGetOrFetchStorefrontPricesForSkuIds({
    skuIds: memo7,
  });
  return {
    recommendations,
...
export const useWishlistRecommendationsForSingleUser = function useWishlistRecommendationsForSingleUser(arg0) {
  ...
  if (source === undefined) {
    source = userId(10678).WishlistFetchSource.USER_PROFILE;
  }
  const items = [userId];
  ...
  obj = {
    ...
    applicationIds: userId(10790).useWishlistApplicationIds(userId),
    numItems,
    source,
  };
  const obj3 = userId(10790);
  ...
  const wishlistSkuFilter = userId(10791).useWishlistSkuFilter({
    wishlistAndRecommendations,
    skusToUserAndReason,
    userId,
    numItems,
...
export const useRecommendationsForApplicationIds = function useRecommendationsForApplicationIds(userIds) {
  ...
  stateFromStoresArray = memo(wishlistAndRecommendations[7]).useStateFromStoresArray(items2, () =>
    memo.map((id) => {
  ...
  obj = {
    recommendations: React.useMemo(() => wishlistAndRecommendations.slice(0, stateFromStoresArray), items4),
    skusToUserAndReason,
    status,
  };
...
export const useRecommendationsForSingleUser = function useRecommendationsForSingleUser(source) {
  ...
  if (USER_PROFILE === undefined) {
    USER_PROFILE = userId(10678).WishlistFetchSource.USER_PROFILE;
  }
  let recommendations;
  ...
  obj[1] = userId(10790).useWishlistApplicationIds(userId);
  obj[2] = numItems;
  obj[3] = USER_PROFILE;
  const tmp4 = useWishlistRecommendationsWithWishlists(obj);
