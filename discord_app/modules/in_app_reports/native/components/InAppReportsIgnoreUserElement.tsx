...
export default function IgnoreUserElement(user) {
  ...
  const items1 = [user];
  const stateFromStoresObject = user(reportId[5]).useStateFromStoresObject(
    items,
  ...
  const stateFromStores = user(reportId[5]).useStateFromStores(
    items2,
  ...
  if (null != user) {
    ...
    tmp8Result = jsx(channelId(tmp2[9]), {
      title: null,
      disabledTitle: null,
      description: null,
      disabled: null,
