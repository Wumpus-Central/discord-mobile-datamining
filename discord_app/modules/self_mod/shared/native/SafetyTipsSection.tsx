...
export default function SafetyTipsContainer(children) {
  ...
  const tmp = closure_6();
  const items = [
    closure_4(View, { style: tmp.image, children: closure_4(safetyTips(5995).SafetyBookletSpotIllustration, {}) }),
    ,
  ];
  if (showHeader) {
  ...
  const items1 = [
    showHeader,
    closure_4(safetyTips(4823).Text, {
      style: tmp.text,
  ...
  items[2] = closure_4(View, {
    ...
    children: safetyTips.map((tip, index) =>
      React4(SafetyTipsRowDefault, { index: index + 1, tip, end: index === safetyTips.length - 1 }, index),
    ),
  });
  obj3.children = items;
