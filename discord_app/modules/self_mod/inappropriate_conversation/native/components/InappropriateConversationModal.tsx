...
function CrisisTextLineScreen(trackAnalyticsEvent) {
  ...
  const items1 = [callback2(trackAnalyticsEvent(4299).Text, obj), ];
  const obj1 = { variant: "text-md/medium", style: tmp.takeoverDescription, children: null };
  const intl2 = trackAnalyticsEvent(1236).intl;
  obj1[2] = intl2.string(trackAnalyticsEvent(1236).t.uicS5l);
  items1[1] = callback2(trackAnalyticsEvent(4299).Text, obj1);
  obj[1] = items1;
  ...
  const items2 = [callback2(trackAnalyticsEvent(4714).Button, obj3), ];
  ...
  items2[1] = callback2(trackAnalyticsEvent(4714).Button, obj4);
  obj2[1] = items2;
