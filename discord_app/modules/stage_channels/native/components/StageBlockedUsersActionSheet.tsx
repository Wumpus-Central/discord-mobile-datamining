...
export default function StageBlockedUsersActionSheet(channel) {
  ...
  obj[1] = function onPress() {
    onAccept(channel);
    onAccept(4270).hideActionSheet(outer1_8);
  };
  const items = [callback(channel(4712).Button, obj), ];
  obj1 = { variant: "secondary", text: null, onPress: null };
  const intl2 = channel(1236).intl;
  obj1[1] = intl2.string(channel(1236).t.CZGqeT);
  obj1[2] = function handleDismiss() {
    onAccept(4270).hideActionSheet(closure_8);
  };
  items[1] = callback(channel(4712).Button, obj1);
  obj[3] = items;
  items1 = [];
  ...
  obj2[2] = callback2(channel(5327).SafeAreaPaddingView, obj);
  ...
  const tmp4 = callback2(channel(5327).SafeAreaPaddingView, obj);
  ...
  const items3 = [callback(onAccept(7948), obj3), callback(View, { style: { height: tmp2[0] } })];
  obj2[3] = items3;
  return callback2(channel(5396).BottomSheet, obj2);
};