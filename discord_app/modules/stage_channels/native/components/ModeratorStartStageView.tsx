...
import StageViewWithPromptsDefault from "StageViewWithPrompts" /* 9680 */;
  ...
  if (null != stateFromStores) {
    ...
    if (canManageGuildEventResult) {
      ...
      if (null != first) {
        ...
        tmp10Result = closure_5(tmp(9702).StartEventPrompt, obj);
      }
    }
    ...
    if (useCurrentUserStageRolesDefault(channel.id, true).moderator) {
      obj1 = { channel, isLive };
      tmp10Result = closure_5(tmp(9903).StartStagePrompt, obj1);
    }
    items2[1] = tmp10Result;
    let tmp10Result1 = null;
    if (tmp7) {
      obj2 = { channel, isLive, guild: stateFromStores };
      tmp10Result1 = closure_5(tmp(9702).ScheduleEventPrompt, obj2);
    }
    obj3 = { children: null };
