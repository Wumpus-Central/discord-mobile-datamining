// === Module 8931: ModeratorStartStageView ===
...
export default function ModeratorStartStageView(channel) {
  ...
  const first = _slicedToArray(guild_id(8932).useGuildChannelScheduledEvents(channel.id), 1)[0];
  const obj2 = guild_id(8932);
  const obj3 = guild_id(8941);
  const canManageGuildEventResult = guild_id(8941).useManageResourcePermissions(channel).canManageGuildEvent(first);
  const tmp7 = useCanCreateAnEventDefault(guild_id);
  const isLive = guild_id(8944).useStageChannelStartEvent(channel.id).isLive;
  const obj4 = guild_id(8944);
  const nextRecurrenceIdInEvent = guild_id(8935).getNextRecurrenceIdInEvent(first);
  ...
  if (null != stateFromStores) {
    ...
    if (canManageGuildEventResult) {
      ...
      if (null != first) {
        ...
        tmp10Result = closure_5(tmp(8964).StartEventPrompt, obj7);
      }
    }
    ...
    if (useCurrentUserStageRolesDefault(channel.id, true).moderator) {
      ...
      tmp10Result4 = closure_5(tmp(9342).StartStagePrompt, obj8);
    }
    items2[1] = tmp10Result4;
    let tmp10Result5 = null;
    if (tmp7) {
      const obj9 = { channel, isLive, guild: stateFromStores };
      tmp10Result5 = closure_5(tmp(8964).ScheduleEventPrompt, obj9);
    }
    const obj10 = { children: null };
    items2[2] = tmp10Result5;
    const obj11 = { onContinue: channel.onSkip };
    items2[3] = closure_5(tmp(9342).ContinueToStagePrompt, obj11);
