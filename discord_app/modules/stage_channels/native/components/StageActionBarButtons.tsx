...
export const MusicMuteButton = function MusicMuteButton(arg0) {
  ...
  if (!obj2.useShowStageMusicMuteButton(channel.id)) {
    ...
    obj = {
      ...
      source: importDefault(stateFromStores ? 9939 : 9940),
      IconComponent: null,
      imageStyle: null,
      onPress: null,
      isSmallSize: null,
...
export const RequestToSpeakListButton = function RequestToSpeakListButton(channel) {
  ...
  function handleOpenAudienceList() {
    ...
    obj.openLazy(asyncRequireImpl(9947, dependencyMap.paths), closure_10, obj);
  }
  ...
  const stageParticipantsCount = obj.useStageParticipantsCount(
    channel.id,
    channel(5439).StageChannelParticipantNamedIndex.REQUESTED_TO_SPEAK_ONLY,
  );
  if (stageParticipantsCount > 0) {
    ...
    obj.imageStyle = obj1;
    obj.IconComponent = tmp3(9960).HandRequestSpeakListIcon;
...
export const ChatButton = function ChatButton(channel) {
  ...
  if (mentionCount <= 0) {
    if (unreadCount <= 0) {
      ...
      obj.IconComponent = tmp(5085).ChatIcon;
      obj.source = isVoiceChannelLocked(9970);
      obj.onPress = onPress;
      obj.appearsDisabled = isVoiceChannelLocked;
      obj.isSmallSize = isSmallSize;
      let tmp7Result = closure_12(openChat(9529).ActionButton, obj);
    }
    return tmp7Result;
  }
  if (mentionCount > 0) {
  ...
  obj1.IconComponent = channel(5085).ChatIcon;
  obj1.source = isVoiceChannelLocked(9970);
  obj1.onPress = onPress;
  obj1.appearsDisabled = isVoiceChannelLocked;
  obj1.isSmallSize = isSmallSize;
  tmp7Result = closure_12(openChat(9529).NotifiedActionButton, obj1);
};
