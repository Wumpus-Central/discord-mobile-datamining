...
class AgeVerificationSpeakerActionSheet {
  constructor(arg0) {
    ...
    obj = {
      ...
      contentStyles: { paddingBottom: closure_1(closure_3[35])().bottom },
      header: null,
      children: null,
    };
    obj = {
      style: tmp.header,
      children: jsx(onClose(closure_3[37]).TrafficConeSpotIllustration, { width: 120, height: 120 }),
    };
...
createStyles.iconContainerStyle = {
...
export const MusicMuteButton = function MusicMuteButton(arg0) {
  ...
  if (!obj2.useShowStageMusicMuteButton(channel.id)) {
    ...
    obj = {
      ...
      source: importDefault(stateFromStores ? 9912 : 9913),
      IconComponent: null,
      imageStyle: null,
      onPress: null,
      isSmallSize: null,
...
export const DisconnectStageButton = function DisconnectStageButton(channel) {
  ...
  obj.source = _modDef8620;
  obj.IconComponent = channel(9078).DoorExitIcon;
  obj.onPress = function onPress() {
...
export const RequestToSpeakListButton = function RequestToSpeakListButton(channel) {
  ...
  function handleOpenAudienceList() {
    ...
    obj.openLazy(asyncRequireImpl(9920, dependencyMap.paths), closure_10, obj);
  }
  ...
  const stageParticipantsCount = obj.useStageParticipantsCount(
    channel.id,
    channel(5425).StageChannelParticipantNamedIndex.REQUESTED_TO_SPEAK_ONLY,
  );
  if (stageParticipantsCount > 0) {
    ...
    obj.imageStyle = obj1;
    obj.IconComponent = tmp3(9933).HandRequestSpeakListIcon;
    ...
    obj2.source = tmp(9932);
    const obj3 = { tintColor: tmp(576).unsafe_rawColors.WHITE };
    obj2.imageStyle = obj3;
    obj2.IconComponent = tmp3(9933).HandRequestSpeakListIcon;
...
export const RequestToSpeakButton = function RequestToSpeakButton(channel) {
  ...
  shouldAgeVerifyToSpeakForCurrentUser =
  ...
  shouldShowAgeVerificationPopover = require("useStageSpeakingForCurrentUser").useShouldShowAgeVerificationPopover(
    channel.id,
  );
...
export const ChatButton = function ChatButton(channel) {
  ...
  if (mentionCount <= 0) {
    if (unreadCount <= 0) {
      ...
      obj.source = isVoiceChannelLocked(9943);
      obj.onPress = onPress;
      obj.appearsDisabled = isVoiceChannelLocked;
      obj.isSmallSize = isSmallSize;
      let tmp7Result = closure_12(openChat(9502).ActionButton, obj);
    }
    return tmp7Result;
  }
  if (mentionCount > 0) {
  ...
  obj1.source = isVoiceChannelLocked(9943);
  obj1.onPress = onPress;
  obj1.appearsDisabled = isVoiceChannelLocked;
  obj1.isSmallSize = isSmallSize;
  tmp7Result = closure_12(openChat(9502).NotifiedActionButton, obj1);
};
...
export const JoinStagePrompt = function JoinStagePrompt(channel) {
  ...
  return closure_12(channel(8598).FormCTA, obj);
};
export const ContinueToStagePrompt = function ContinueToStagePrompt(onContinue) {
  ...
  const obj2 = {
    children: closure_1_12(native.Icon, {
