...
let obj2 = {
  ...
  icon: { color: nativeDefault.colors.INTERACTIVE_TEXT_DEFAULT },
  warning: null,
};
...
export default function StageSettingsActionSheet(channelId) {
  ...
  const stateFromStores1 = channelId(stateFromStores[12]).useStateFromStores(items1, () =>
  ...
  const isStageSpeakingDisabledForCurrentUser = channelId(
    stateFromStores[14],
  ).useIsStageSpeakingDisabledForCurrentUser();
  let obj3 = channelId(stateFromStores[14]);
  const items2 = [ChannelRTCStore];
  const items3 = [channelId];
  const stateFromStores2 = channelId(stateFromStores[12]).useStateFromStores(
    items2,
  ...
  const stateFromStores3 = channelId(stateFromStores[12]).useStateFromStores(items4, () =>
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores1) {
      ...
      if (null == stateFromStores2) {
        ...
        if (!isStageSpeakingDisabledForCurrentUser) {
          ...
          obj7.leading = closure_12(tmp2(tmp3[18]).Icon, obj8);
          ...
          closure_12(tmp2(tmp3[16]).FormRow, obj7);
        }
      }
    }
    const items6 = [tmp14, , , ,];
    ...
    if (tmp11Result3) {
      ...
      obj13.leading = closure_12(tmp2(tmp3[18]).Icon, obj14);
      ...
      tmp11Result3 = closure_12(tmp2(tmp3[16]).FormRow, obj13);
    }
    ...
    tmp11Result4 = closure_12(tmp2(tmp3[15]).ActionSheet, obj15);
  }
  return tmp11Result4;
}
