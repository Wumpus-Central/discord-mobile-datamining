...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 =
    useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4976).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  const stageParticipantsCount = channelId(5736).useStageParticipantsCount(
    channelId,
    channelId(5730).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK,
  );
