...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 =
    useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4904).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  const stageParticipantsCount = channelId(5650).useStageParticipantsCount(
    channelId,
    channelId(5644).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK,
  );
