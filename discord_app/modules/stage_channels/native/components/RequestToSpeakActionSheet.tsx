...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 =
    useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4974).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  const stageParticipantsCount = channelId(5734).useStageParticipantsCount(
    channelId,
    channelId(5728).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK,
  );
