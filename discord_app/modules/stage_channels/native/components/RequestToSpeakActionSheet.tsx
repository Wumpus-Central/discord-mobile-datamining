...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 =
    useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4903).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  const stageParticipantsCount = channelId(5643).useStageParticipantsCount(
    channelId,
    channelId(5637).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK,
  );
