...
export default function SpeakerPulse(style) {
  ...
  const effect = React.useEffect(() => {
    if (stateFromStores) {
      ...
      const withDelayResult = obj4.withDelay(
        100,
        stateFromStores(sharedValue1[8]).withTiming(closure_1_9, { duration: 250 }),
      );
      ...
      const withRepeatResult = obj2.withRepeat(
        obj3.withSequence(
          withDelayResult,
          obj6.withDelay(250, stateFromStores(sharedValue1[8]).withTiming(0, { duration: 500 })),
        ),
        -1,
        false,
      );
      ...
      const withDelayResult1 = obj10.withDelay(
        350,
        stateFromStores(sharedValue1[8]).withTiming(closure_1_9, { duration: 250 }),
      );
      ...
      const result3 = sharedValue1.set(
        obj8.withRepeat(
          obj9.withSequence(withDelayResult1, stateFromStores(sharedValue1[8]).withTiming(0, { duration: 500 })),
          -1,
          false,
        ),
      );
