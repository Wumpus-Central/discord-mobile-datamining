    ...
    const tmp = asyncGeneratorStep(async () => {
      ...
      const mapped = found.map((row) => {
        ...
        if (row.rowType === constants.ROLE) {
          ...
          let moderatorOverwrite = channel(5727).createModeratorOverwrite(
            row.id,
          ...
          moderatorOverwrite = channel(5727).createModeratorOverwrite(
            row.id,
  ...
  if (null == stateFromStores) {
    ...
    if (channel.canSkip) {
      ...
      const obj6 = {
        ...
        permission: tmp4(2053).MODERATE_STAGE_CHANNEL_PERMISSIONS,
        pendingAdditions,
        setPendingAdditions: tmp2[1],
      };
      obj5.children = jsx(tmp7(9045), {
        inActionSheet: true,
        channel,
        guild: stateFromStores,
        permission: tmp4(2053).MODERATE_STAGE_CHANNEL_PERMISSIONS,
