          ...
          if (0 === c1) {
            if (arg0 === 1) {
              ...
              if (obj11.isEmptyOverwrite(id)) {
                c1 = 2;
                c2 = 1;
                let obj1 = { value: v1(4649).clearPermissionOverwrite(tmp2.id, id.id), done: false };
                return obj1;
              } else {
                const items = [id];
                c1 = 1;
                c2 = 1;
                let obj2 = { value: tmp2(9869).savePermissionUpdates(tmp2.id, items), done: false };
                return obj2;
              }
              obj11 = tmp2(5496);
            }
  ...
  if (null != guild) {
    if (null != sortedGuildRoles) {
      ...
      if (isGuildStageVoiceResult) {
        ...
        let obj2 = { icon: closure_10(tmp(tmp2[28]).CirclePlusIcon, {}), label: null, onPress: null, disabled: null, arrow: true };
        ...
        obj2.onPress = function onPress() {
          if (null != channel) {
            ...
            obj2.openLazy(asyncRequireImpl(16937, dependencyMap.paths), "channel-add-moderators-" + channel.id, obj);
        ...
        obj3.children = existingRolesRowWithPermissionDisabled.map((item) => closure_2_10(ChannelOverwritesItemDefault, { guildId: channel.guild_id, item, channelId: channel.id, showType: true, showRemove: canUpdateStageChannelModerators, onRemove: handleRemovePermission }, item.id));
        ...
        obj4.children = existingMembersRows.map((item) => closure_2_10(ChannelOverwritesItemDefault, { guildId: channel.guild_id, item, channelId: channel.id, showType: true, showRemove: canUpdateStageChannelModerators, onRemove: handleRemovePermission }, item.id));
