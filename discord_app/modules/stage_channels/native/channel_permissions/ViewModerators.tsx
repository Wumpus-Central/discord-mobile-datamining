          ...
          if (0 === c1) {
            if (arg0 === 1) {
              ...
              if (obj11.isEmptyOverwrite(id)) {
                c1 = 2;
                c2 = 1;
                const obj8 = { value: v1(4651).clearPermissionOverwrite(tmp2.id, id.id), done: false };
                return obj8;
              } else {
                const items = [id];
                c1 = 1;
      ...
      if (isGuildStageVoiceResult) {
        ...
        obj9.onPress = function onPress() {
          if (null != channel) {
            ...
            obj2.openLazy(asyncRequireImpl(17034, dependencyMap.paths), "channel-add-moderators-" + channel.id, obj3);
...
export const openAddModeratorsActionSheet = function openAddModeratorsActionSheet(channel) {
  ...
  AnalyticsUtilsDefault.track(AnalyticEvents.OPEN_POPOUT, { type: "Grant Channel Access" });
  const obj2 = ActionSheetActionCreatorsDefault;
  obj2.openLazy(asyncRequireImpl(17034, dependencyMap.paths), "channel-add-moderators-" + channel.id, {
    channel,
    canSkip: flag,
  });
};
