// === Module 8077: StageChannelAgeVerificationNoticeContent ===
...
function StageChannelAgeVerificationNoticeContent(onConfirmPress) {
  ...
  if (isVerifiedTeen) {
    ...
    obj[0] = function hook(children) {
      return closure_1_5(onConfirmPress(dependencyMap[8]).Text, {
    ...
    obj[0] = function hook(children) {
      return closure_1_5(onConfirmPress(dependencyMap[8]).Text, {
        ...
        onPress() {
          ...
          const obj = { entryPoint: onConfirmPress(closure_1_2[12]).AgeVerificationModalEntryPoint.START_STAGE_PROMPT };
...
function StageChannelAgeVerificationNoticeWrapper(onConfirmPress) {
  ...
  if (onConfirmPress.noBackground) {
    ...
    obj[2] = tmp.icon;
    const items = [callback(WarningIcon, obj), ];
    Text = Text2.Text;
    ...
    obj1[3] = callback(StageChannelAgeVerificationNoticeContent, obj2);
    tmp8Result = callback(Text, obj1);
    items[1] = tmp8Result;
    obj[1] = items;
    callback2(View, obj);
  } else {
    ...
    obj3[1] = callback(StageChannelAgeVerificationNoticeContent, obj4);
    return callback(Button.HelpMessage, obj3);
  }
}
noopAll;
...
let result = require("obj132").fileFinishedImporting("modules/stage_channels/native/StageChannelAgeVerificationNotice.tsx");
  ...
  if (obj.useShouldAgeVerifyToSpeakForCurrentUser(channelId)) {
    ...
    if (Array.isArray(divider)) {
      ...
      if (hasItem1) {
        ...
        obj2[0] = tmp.divider;
        hasItem1 = callback(View, obj2);
      }
      const obj3 = { children: null };
      items[2] = hasItem1;
      obj3[0] = items;
      return callback2(closure_7, obj3);
