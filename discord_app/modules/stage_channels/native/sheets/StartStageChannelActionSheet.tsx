...
export default function StartStageChannelEventActionSheet(channel) {
  ...
  const shouldAgeVerifyToSpeakForCurrentUser = channel(5140).useShouldAgeVerifyToSpeakForCurrentUser(channel.id);
  ...
  obj[4] = stringResult;
  items1[1] = callback(channel(4299).Text, obj);
  ...
  items1[2] = callback(channel(4299).Text, obj1);
  obj[1] = items1;
  let stringResult2;
  const tmp11 = stateFromStores;
  const tmp2Result = channel(5140);
  if (null == stateFromStores) {
  ...
  if (null != obj2) {
    ...
    tmp15Result = tmp15(tmp2(4299).Text, obj5);
  }
  ...
  obj6[1] = callback(channel(4714).Button, { text: stringResult3, onPress: handleSave, disabled: "" === first, loading: tmp8, accessibilityHint: stringResult2 });
  ...
  if (tmp15Result) {
    ...
    tmp15Result = tmp15(tmp2(4299).Text, obj8);
  }
  const obj4 = {
    onConfirmPress() {
      return stateFromStores(_undefined[17]).hideActionSheet(closure_10);
  ...
  obj2[2] = items2;
  return callback(channel(5397).BottomSheet, { keyboardShouldPersistTaps: "always", children: closure_14(channel(5328).SafeAreaPaddingView, obj2) });
};