...
export default function StartStageChannelEventActionSheet(channel) {
  ...
  const shouldAgeVerifyToSpeakForCurrentUser = channel(5092).useShouldAgeVerifyToSpeakForCurrentUser(channel.id);
  ...
  const items1 = [callback(stateFromStores(7682), {}), , ];
  ...
  obj3[9] = handleSave;
  items2[1] = callback(channel(7685).TextInput, obj3);
  items2[2] = callback(tmp11(7691), {
    onConfirmPress() {
  ...
  obj6[1] = callback(channel(4665).Button, { text: stringResult3, onPress: handleSave, disabled: "" === first, loading: tmp8, accessibilityHint: stringResult2 });
  ...
  obj2[2] = items2;
  return callback(channel(5323).BottomSheet, { keyboardShouldPersistTaps: "always", children: closure_14(channel(5257).SafeAreaPaddingView, obj2) });
};