...
export default function AddMembersBody(pendingAdditions) {
  ...
  if (inActionSheet) {
    let BottomSheetScrollView = guild(6957).BottomSheetScrollView;
  } else {
    BottomSheetScrollView = c8;
  }
  if (inActionSheet) {
    let BottomSheetSectionList = guild(6957).BottomSheetSectionList;
  } else {
  ...
  if ("" !== str) {
    if (0 === items2.length) {
      if (0 === membersRows.length) {
        ...
        const obj16 = { Illustration: guild(9932).NoResultsAlt, style: null, bodyStyle: null, body: null };
  ...
  obj19.contentContainerStyle = {
    ...
    paddingBottom: pendingAdditions(576).space.PX_16 + pendingAdditions(7314)(obj).insets.bottom,
  };
