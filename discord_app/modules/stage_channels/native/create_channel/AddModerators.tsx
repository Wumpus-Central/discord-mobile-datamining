  ...
  onPress = noop.useCallback(() => {
    ...
    const mapped = found.map((row) => {
      ...
      if (row.rowType === constants.ROLE) {
        let moderatorOverwrite = guildId(5585).createModeratorOverwrite(
          row.id,
          guildId(1895).PermissionOverwriteType.ROLE,
        );
        const obj2 = guildId(5585);
      } else {
        moderatorOverwrite = guildId(5585).createModeratorOverwrite(
          row.id,
          guildId(1895).PermissionOverwriteType.MEMBER,
        );
