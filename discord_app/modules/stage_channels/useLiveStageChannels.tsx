  ...
  return require("initialize").useStateFromStoresArray(
    ...
    () =>
      stateFromStoresArray.filter((item) =>
        closure_1_4.can(stateFromStores(closure_1_2[3]).JOIN_VOCAL_CHANNEL_PERMISSIONS, item),
      ),
    items5,
  );
}
...
export const useAllLiveStageChannels = function useAllLiveStageChannels() {
  ...
  const stateFromStoresArray = stateFromStores(504).useStateFromStoresArray(
    items1,
    () => {
      const mapped = stateFromStores.map((item) => channel.getChannel(item));
      return mapped.filter(GlobalUtils.isNotNullish);
