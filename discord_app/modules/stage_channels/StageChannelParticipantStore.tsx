// === Module 5362: getActiveStageChannelIds ===

// Module 5362 (getActiveStageChannelIds)
import applyDefault from "apply" /* 12 */;
import initializeDefault from "initialize" /* 589 */;
import dispatcherDefault from "dispatcher" /* 709 */;
import sortKeyDefault from "sortKey" /* 5368 */;
...
import closure_13 from "buildStageChannelUserRoles" /* 5365 */;
import closure_14 from "handleStageInstanceCreateOrUpdate" /* 1392 */;
...
function updateParticipant(arg0) {
  ...
  f78048 = (updateParticipant) => updateParticipant.updateParticipant(f78048);
  if (mapped === undefined) {
...
function handleRelationshipUpdate(relationship) {
  let f78048 = relationship.relationship.id;
  let values = secondaryIndexMap.values(undefined, true);
  const mapped = values.map((id) => id.id);
  f78048 = (updateParticipant) => updateParticipant.updateParticipant(f78048);
  let mapped1 = mapped;
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f78048(4544).decodeStreamKey(streamKey.streamKey);
...
const stageChannelParticipantStore = new StageChannelParticipantStore(dispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const f78052 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
