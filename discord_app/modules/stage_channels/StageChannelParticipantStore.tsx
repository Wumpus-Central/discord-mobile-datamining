// === Module 5723: StageChannelParticipantStore ===
...
import StageChannelParticipantsDefault from "StageChannelParticipants" /* 5730 */;
...
function updateParticipant(arg0) {
  ...
  f81706 = (dependencyMap) => dependencyMap.updateParticipant(f81706);
  if (mapped === undefined) {
...
function handleUserUpdate(user) {
  ...
  const f81706 = (dependencyMap) => dependencyMap.updateParticipant(f81706);
  let mapped1 = mapped;
  if (mapped === undefined) {
    const values2 = secondaryIndexMap.values(undefined, true);
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f81706(4881).decodeStreamKey(streamKey.streamKey);
...
const secondaryIndexMap = new fn(4459).SecondaryIndexMap((getGuildId) => {
...
const stageChannelParticipantStore = new StageChannelParticipantStore(DispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const f81710 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
