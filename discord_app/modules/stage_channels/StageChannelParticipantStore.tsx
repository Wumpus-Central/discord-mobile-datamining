...
function updateParticipant(arg0) {
  ...
  f81327 = (dependencyMap) => dependencyMap.updateParticipant(f81327);
  if (mapped === undefined) {
...
function handleUserUpdate(user) {
  ...
  const f81327 = (dependencyMap) => dependencyMap.updateParticipant(f81327);
  let mapped1 = mapped;
  if (mapped === undefined) {
    const values2 = secondaryIndexMap.values(undefined, true);
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f81327(4808).decodeStreamKey(streamKey.streamKey);
...
const stageChannelParticipantStore = new StageChannelParticipantStore(DispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const f81331 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
