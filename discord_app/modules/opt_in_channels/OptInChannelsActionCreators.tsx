  ...
  if (null != id) {
    if (ImpersonateStore.isFullServerPreview(id)) {
      ...
      if (flag) {
        ...
        const result2 = channel_overrides(7315).updateGuildAndChannelNotificationSettings(
          id,
          obj4,
        ...
        const result3 = channel_overrides(7315).updateChannelOverrideSettingsBulk(
          id,
          channel_overrides,
          require("NotificationSettingsUtils").NotificationLabels.OptedIn,
        );
...
export const bulkOptOutChannels = function bulkOptOutChannels(id, arr, location) {
  ...
  if (null != id) {
    if (ImpersonateStore.isFullServerPreview(id)) {
      ...
      const result1 = obj(7315).updateChannelOverrideSettingsBulk(
        id,
        obj,
...
export const clearGuildNotice = function clearGuildNotice(guildId) {
  ...
  const result2 = require("UserSettingsProtoActionCreators").updateUserGuildSettings(
    guildId,
    (guildOnboardingProgress) => {
      guildOnboardingProgress.guildOnboardingProgress = guildId(1385).setFlag(
        guildOnboardingProgress.guildOnboardingProgress,
        guildId(1186).GuildOnboardingProgress.GUILD_NOTICE_SHOWN,
        false,
      );
      obj = guildId(1385);
      guildOnboardingProgress.guildOnboardingProgress = guildId(1385).setFlag(
        guildOnboardingProgress.guildOnboardingProgress,
        guildId(1186).GuildOnboardingProgress.GUILD_NOTICE_CLEARED,
        false,
      );
      return true;
    },
