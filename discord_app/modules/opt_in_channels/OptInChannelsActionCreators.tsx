  ...
  if (c5 === 2) {
    ...
    try {
      ...
      if (0 === c4) {
        ...
        const obj4 = { type: "USER_GUILD_SETTINGS_REMOVE_PENDING_CHANNEL_UPDATES", guildId: closure_130_0, updates: closure_130_1 };
        obj.dispatch(obj4);
      }
      c5 = 3;
...
export const setOptInChannel = function setOptInChannel(guild_id, id, optInEnabled, location) {
  if (null != guild_id) {
    if (ImpersonateStore.isFullServerPreview(guild_id)) {
      ...
      obj4 = { guildId: guild_id, channelId: id, change: obj, previous: currentChannelSettings, label: null, location: null };
      ...
      const result3 = UserSettingsProtoActionCreators.updateUserGuildSettings(guild_id, (guildOnboardingProgress) => {
        ...
        if (!hasFlagResult) {
          ...
          guildOnboardingProgress.guildOnboardingProgress = tmpResult.addFlag(guildOnboardingProgress.guildOnboardingProgress, require("preloaded_user_settings").GuildOnboardingProgress.GUILD_NOTICE_CLEARED);
...
export const bulkOptInChannels = function bulkOptInChannels(id, arr, arg2) {
  ...
  if (null != id) {
    if (ImpersonateStore.isFullServerPreview(id)) {
      ...
      if (flag) {
        const result1 = tmp22(5566).updateImpersonatedData(id, { optInEnabled: true });
        const tmp22Result = tmp22(5566);
      }
      ...
      if (flag) {
        ...
        const result2 = channel_overrides(7133).updateGuildAndChannelNotificationSettings(id, channel_overrides, require("NotificationSettingsUtils").NotificationLabels.OptedIn);
        ...
        let obj1 = channel_overrides(7133);
        const result3 = obj1.updateChannelOverrideSettingsBulk(id, channel_overrides, require("NotificationSettingsUtils").NotificationLabels.OptedIn);
        obj1 = {};
