...
obj.header = obj;
const createStyles = { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingTop: nativeDefault.space.PX_24, paddingBottom: nativeDefault.space.PX_8, borderRadius: nativeDefault.radii.md };
...
let obj3 = { position: "relative", padding: 0, marginTop: nativeDefault.space.PX_16, paddingTop: nativeDefault.space.PX_16, marginBottom: 0, borderRadius: nativeDefault.radii.md, alignItems: "center" };
...
let closure_17 = noop.memo((channel) => {
  ...
  obj = { style: tmp.categoryTitle, title: onChannelClick(4789)(channel), lineClamp: 1 };
  const items1 = [closure_12(channel(5768).TableRowGroupTitle, obj), ];
  let tmp10Result = null;
  if ("null" !== channel.id) {
    obj1 = { style: tmp.selectAllContainer, children: null };
    const obj2 = { style: tmp.selectAllCheckbox, children: null };
    const obj3 = { checked: stateFromStores };
    obj2.children = closure_12(tmp4(5698).FormCheckbox, obj3);
    ...
    items2[1] = closure_12(tmp4(4632).Text, obj4);
...
let closure_18 = noop.memo((channel) => {
  ...
  const items = [UserGuildSettingsStore];
  let stateFromStores = obj1.useStateFromStores(items, () => UserGuildSettingsStore.isChannelOptedIn(channel.guild_id, channel.id));
  ...
  if (null == guild) {
    ...
    obj = { start: isFirst, end: isLast, disabled: stateFromStores2, icon: null, label: null, subLabel: null, subLabelLineClamp: 1, onPress: null, checked: null };
...
export default function ChannelBrowser(guildId) {
  ...
  obj10.data = memo;
  obj10.contentContainerStyle = { paddingBottom: require("useSafeAreaInsetsKeyboardAware")({ includeKeyboardHeight: true }).insets.bottom + require("native").space.PX_16, paddingHorizontal: require("native").space.PX_16 };
