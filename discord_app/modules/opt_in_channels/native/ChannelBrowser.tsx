...
let closure_17 = importAllResult.memo((channel) => {
  ...
  const stateFromStores = obj.useStateFromStores(items, () =>
    closure_1_10.isChannelOptedIn(channel.guild_id, channel.id),
  );
  obj1 = channel(4205);
...
export default function ChannelBrowser(guildId) {
  ...
  obj10[4] = {
    paddingBottom:
      importDefault(onChannelClick[13])({ includeKeyboardHeight: true }).insets.bottom +
      importDefault(onChannelClick[12]).space.PX_16,
    paddingHorizontal: importDefault(onChannelClick[12]).space.PX_16,
  };
