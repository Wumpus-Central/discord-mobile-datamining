...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const baseChannelUnreadBadgeState = channel(16746).useBaseChannelUnreadBadgeState(
    channel,
  ...
  const stageParticipantsCount = channel(5734).useStageParticipantsCount(
    channel.id,
    channel(5728).StageChannelParticipantNamedIndex.AUDIENCE,
  );
  ...
  const channelAccessibilityProps = channel(17204).getChannelAccessibilityProps({
    channel,
    unread,
    mentionCount,
    voiceStates,
