...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const baseChannelUnreadBadgeState = channel(16694).useBaseChannelUnreadBadgeState(
    channel,
  ...
  const stageParticipantsCount = channel(5650).useStageParticipantsCount(
    channel.id,
    channel(5644).StageChannelParticipantNamedIndex.AUDIENCE,
  );
  ...
  const channelAccessibilityProps = channel(17144).getChannelAccessibilityProps({
    channel,
    unread,
    mentionCount,
    voiceStates,
