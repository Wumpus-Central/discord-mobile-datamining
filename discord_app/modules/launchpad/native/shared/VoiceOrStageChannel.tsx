...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const baseChannelUnreadBadgeState = channel(15954).useBaseChannelUnreadBadgeState(
    channel,
  ...
  const stageParticipantsCount = channel(5738).useStageParticipantsCount(
    channel.id,
    channel(5732).StageChannelParticipantNamedIndex.AUDIENCE,
  );
  ...
  obj7.onLongPress = noop.useCallback(
  ...
  const channelAccessibilityProps = channel(16445).getChannelAccessibilityProps({
    channel,
    unread,
    mentionCount,
    voiceStates,
  ...
  return closure_14(closure_20, {
    ...
    speakerVoiceStates: channel(504).useStateFromStoresArray(items1, () => {
      ...
      const found = mutableParticipants.filter(
        (type) => type.type === channel(closure_1_2[31]).StageChannelParticipantTypes.VOICE,
      );
