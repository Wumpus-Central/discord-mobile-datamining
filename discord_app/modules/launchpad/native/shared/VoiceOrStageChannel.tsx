...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const baseChannelUnreadBadgeState = channel(16770).useBaseChannelUnreadBadgeState(
    channel,
  ...
  const stageParticipantsCount = channel(5736).useStageParticipantsCount(
    channel.id,
    channel(5730).StageChannelParticipantNamedIndex.AUDIENCE,
  );
  ...
  const channelAccessibilityProps = channel(17230).getChannelAccessibilityProps({
    channel,
    unread,
    mentionCount,
    voiceStates,
