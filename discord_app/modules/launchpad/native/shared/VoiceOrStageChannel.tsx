        ...
        if (0 === c2) {
          if (arg0 === 1) {
            ...
            if (isGuildStageVoiceResult) {
              c2 = 2;
              c3 = 1;
              const obj4 = { value: tmp21(dependencyMap[13], dependencyMap.paths), done: false };
              return obj4;
            } else {
              c2 = 1;
              c3 = 1;
              const obj5 = { value: tmp21(dependencyMap[15], dependencyMap.paths), done: false };
              return obj5;
            }
...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const baseChannelUnreadBadgeState = channel(16474).useBaseChannelUnreadBadgeState(
    channel,
  ...
  const stageParticipantsCount = channel(5517).useStageParticipantsCount(
    channel.id,
    channel(5511).StageChannelParticipantNamedIndex.AUDIENCE,
  );
          ...
          if (0 === c2) {
            if (arg0 === 1) {
              ...
              if (obj7.getChannelRoleSubscriptionStatus(guildId.id).needSubscriptionToAccess) {
                ...
                const transitionToResult = tmp20(tmp21[19]).transitionTo(
  ...
  const channelAccessibilityProps = channel(16866).getChannelAccessibilityProps({
    channel,
    unread,
    mentionCount,
    voiceStates,
