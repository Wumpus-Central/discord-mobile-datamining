// === Module 16783: VoiceOrStageChannel ===
...
import useStageChannelSpeakerVoiceStates from "useStageChannelSpeakerVoiceStates" /* 15844 */;
...
let closure_20 = noop.memo(function UnmemoedVoiceOrStageChannelBase(channel) {
  ...
  const tmp4 = subtitle(7293)();
  const tmp6 = closure_18(tmp4, channel(4682).isThemeLight(tmp3));
  const tmp7 = subtitle(16446)();
  let obj = channel(4682);
  const isConnectedToVoiceChannel = channel(8825).useIsConnectedToVoiceChannel(channel);
  let obj2 = channel(8825);
  const baseChannelUnreadBadgeState = channel(15954).useBaseChannelUnreadBadgeState(channel, !isConnectedToVoiceChannel);
  ({ unread, mentionCount } = baseChannelUnreadBadgeState);
  let obj3 = channel(15954);
  ...
  const stageParticipantsCount = channel(5738).useStageParticipantsCount(channel.id, channel(5732).StageChannelParticipantNamedIndex.AUDIENCE);
  ...
  const arr5 = subtitle(11527)(channel);
  const obj5 = channel(5738);
  const channelAccessibilityProps = channel(16445).getChannelAccessibilityProps({ channel, unread, mentionCount, voiceStates, embeddedActivitiesCount: arr5.length });
  const obj8 = channel(16445);
  ...
  ({ isSubscriptionGated, needSubscriptionToAccess } = subtitle(5307)(channel.id));
  ...
  const tmp20 = subtitle(5307)(channel.id);
  ...
  if (!channel.isGuildStageVoice()) {
    ...
    if (voiceStates.length > 0) {
      ...
      obj17.children = tmp31(tmp(16784), obj18);
      tmp31Result = tmp31(View, obj17);
    }
    items8[1] = tmp31Result;
    obj11.children = items8;
    return tmpResult(closure_15(tmp5(5428).PressableHighlight, obj11));
  } else {
