...
export default noop.memo(function DMChannel(navigationReplace) {
  ...
  if (tmp11Result) {
    const obj6 = { channel, message: tmp7, color: str, muted, layout: channel(7995).ChannelListLayoutTypes.COMPACT };
    tmp11Result = jsx(channel(10247).ChannelRowPreview, {
      channel,
      message: tmp7,
      color: str,
      muted,
      layout: channel(7995).ChannelListLayoutTypes.COMPACT,
    });
  }
  ...
  return tmp2Result3(
    jsx(channel(5208).PressableHighlight, { style: null, underlayColor: tmp.pressableUnderlayColor.backgroundColor }),
  );
});
