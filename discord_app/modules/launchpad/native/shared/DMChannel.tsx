...
export default noop.memo(function DMChannel(navigationReplace) {
  ...
  obj3.onLongPress = noop.useCallback(
  ...
  if (tmp11Result) {
    const obj6 = { channel, message: tmp7, color: str, muted, layout: channel(8208).ChannelListLayoutTypes.COMPACT };
    tmp11Result = jsx(channel(10450).ChannelRowPreview, {
      channel,
      message: tmp7,
      color: str,
      muted,
      layout: channel(8208).ChannelListLayoutTypes.COMPACT,
    });
  }
  ...
  return tmp2Result3(
    jsx(channel(5425).PressableHighlight, { style: null, underlayColor: tmp.pressableUnderlayColor.backgroundColor }),
  );
});
