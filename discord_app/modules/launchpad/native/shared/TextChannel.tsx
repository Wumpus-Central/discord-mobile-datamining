  ...
  if (null != tmp8ResultResult) {
    if (!isChannelSpoilerGated) {
      const obj5 = {
        ...
        color: "text-muted",
        muted: flag,
        layout: tmp2(7299).ChannelListLayoutTypes.COMPACT,
      };
    ...
    return tmp8Result4(closure_11(tmp2(5428).PressableHighlight, obj6));
  }
  const tmp18 = arr4(5307)(channel.id);
  result = channel(16780).renderChannelSubtitle({
    subtitle: subtitle.subtitle,
    muted: flag,
    channelId: channel.id,
    guildId: channel.guild_id,
  });
  const tmp2Result14 = channel(16780);
});
