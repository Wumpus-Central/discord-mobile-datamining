...
export default function LaunchPadContainer(children) {
  ...
  let obj1 = launchPadSharedState(4341);
  const fn3 = function v() {
    let obj = { borderRadius: ReanimatedRexport.interpolate(launchPadSharedState.get(), [0, 1], [0, 16]), transform: null };
    obj = { scale: null };
    const obj4 = spring;
    value = launchPadSharedState.get();
    const diff = closure_2.get().width - 48;
    const items = [1, diff / closure_2.get().width];
    obj.scale = obj4.withSpring(ReanimatedRexport.interpolate(value, [0, 1], items), LAUNCH_PAD_SPRING_CONFIG, "animate-always");
    const items1 = [obj, ];
    obj = { translateY: null };
    const obj7 = spring;
    obj.translateY = obj7.withSpring(ReanimatedRexport.interpolate(launchPadSharedState.get(), [0, 1], [0, -4]), LAUNCH_PAD_SPRING_CONFIG, "animate-always");
    items1[1] = obj;
    obj.transform = items1;
    return obj;
  };
  obj = { interpolate: launchPadSharedState(4341).interpolate, launchPadSharedState, withSpring: launchPadSharedState(5024).withSpring, windowDimensions: tmp7, LAUNCH_PAD_SPRING_CONFIG };
  fn3.__closure = obj;
  ...
  obj3.style = items;
  let items1 = [closure_6(updaters(4341).View, obj3), ];
  if (tmp4 !== constants.DISABLED) {
    ...
    const tmp10Result = closure_6(updaters(17063), obj4);
  }
  items1[1] = tmp10Result;
  obj2.children = items1;
  obj1.children = closure_7(closure_3, obj2);
  obj.children = closure_6(launchPadSharedState(6705).GestureDetector, obj1);
  return closure_6(updaters(16090).Provider, obj);
};