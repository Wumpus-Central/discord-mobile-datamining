...
import _modDef15267 from "../../../../../_runtime/metro/15267__.js";
...
function WhatYouLose(subscription) {
  ...
  let obj4 = _modDef4166(subscription.subscription.currentPeriodEnd);
  const intl = util.intl;
  const formatResult = obj4.format(c12);
  const intl2 = util.intl;
  obj = {
    numEmojis: _slicedToArray(obj.useTierEmojiIds(listingId, guild.id), 1)[0].size,
    numChannels: _slicedToArray(obj1.useChannelBenefits(listingId), 1)[0].length,
    numIntangibles: _slicedToArray(obj2.useIntangibleBenefits(listingId), 1)[0].length,
  };
...
function CancelSubscriptionButtonFooter(guild) {
  ...
  const analyticsLocations = subscription(7176)(
    subscription(7196).GUILD_ROLE_SUBSCRIPTION_CANCELLATION_MODAL,
  ).analyticsLocations;
          ...
          if (0 === c4) {
            if (arg0 === 1) {
              ...
              obj1.body =
                ...
                tmp3(4166)(subscription.currentPeriodEnd).format(closure_1_12) +
                ".";
  ...
  obj.children = closure_10(guild(4989).Button, obj);
...
let obj1 = {
  ...
  cactus: { width: 99, position: "absolute", right: 16, bottom: 12 },
};
