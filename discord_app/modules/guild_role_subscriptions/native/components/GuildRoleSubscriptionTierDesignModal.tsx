...
function MemberPreviews(role) {
  ...
  const token1 = require(3989) /* map */.useToken(importDefault(712).colors.BACKGROUND_BASE_LOW, importDefault(712).themes.LIGHT);
  const obj2 = require(3989) /* map */;
  const token2 = require(3989) /* map */.useToken(importDefault(712).colors.TEXT_DEFAULT, importDefault(712).themes.DARK);
  const obj3 = require(3989) /* map */;
  obj = { style: tmp.memberPreviews, children: null };
  const token3 = require(3989) /* map */.useToken(importDefault(712).colors.TEXT_DEFAULT, importDefault(712).themes.LIGHT);
  const items = [, , ];
  ({ member: arr[0], memberLight: arr[1] } = tmp);
  items[2] = { backgroundColor: token1 };
  const items1 = [callback2(require(14375) /* GuildRoleSubscriptionMemberPreview */.GuildRoleSubscriptionMemberPreview, { style: items, textStyle: { color: token3 }, role }), ];
  const items2 = [, , ];
  ({ member: arr3[0], memberDark: arr3[1] } = tmp);
  items2[2] = { backgroundColor: token };
  items1[1] = callback2(require(14375) /* GuildRoleSubscriptionMemberPreview */.GuildRoleSubscriptionMemberPreview, { style: items2, textStyle: { color: token2 }, role });
  obj[1] = items1;
...
function Content() {
  ...
  let tmp7;
  let tmp8;
  const tmp3 = role(13153)();
  let obj = _require(16702);
  ...
  const roleSubscriptionSettingsDisabled = _require(16685).useRoleSubscriptionSettingsDisabled();
  obj = { children: null };
  ...
  obj2[3] = _require(16691).PreviewShape.SQUIRCLE;
  ...
  const tmp4Result = _require(16685);
  const intl3 = tmp4(1236).intl;
  obj3[1] = intl3.string(_require(1236).t["W7hH+z"]);
  items1[3] = callback2(role(8982), obj3);
  items1[4] = callback2(role(16729), { color: tmp7, onChange: tmp8, disabled: roleSubscriptionSettingsDisabled });
  obj[0] = items1;
