...
export default function FormEmojiPicker(emoji) {
  ...
  if (emojiId == null) {
    ...
    emojiId = tmp2(4476).convertSurrogateToName(emojiName, false);
    const tmp2Result = tmp2(4476);
  }
  const tmp4 = onChange(14255)();
  const emojiByIdOrName = guildId(15576).useEmojiByIdOrName(guildId, emojiId);
  if (null != emojiId) {
    let obj = { guildId, id: emojiId };
    let tmp9 = closure_4(tmp2(15575), obj);
    let tmp10 = closure_4;
  } else {
    const obj3 = { resizeMode: "contain", source: tmp2(18298) };
    tmp9 = closure_4(tmp2(5890), obj3);
    tmp10 = closure_4;
    const tmp2Result3 = tmp2(5890);
  }
  const obj4 = {
    style: null,
  ...
  obj4.style = items;
  const items1 = [tmp9, , ];
  let obj2 = guildId(15576);
  const items2 = [tmp.content, ];
  const obj5 = { style: items2, children: null };
  items2[1] = null != emojiByIdOrName ? tmp.text : tmp.placeholder;
  if (null != emojiByIdOrName) {
    let allEmojiNamesString = tmp5(4480).getAllEmojiNamesString(emojiByIdOrName);
    const tmp5Result = tmp5(4480);
  } else {
  ...
  const tmp2Result4 = onChange(10090);
  items1[2] = tmp10(guildId(1177).Icon, { size: guildId(1177).Icon.Sizes.MEDIUM, source: onChange(10283) });
