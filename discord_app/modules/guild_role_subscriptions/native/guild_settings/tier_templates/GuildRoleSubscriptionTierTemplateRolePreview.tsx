...
export const GuildRoleSubscriptionRolePreview = function GuildRoleSubscriptionRolePreview(content) {
  ...
  obj3[3] = name;
  const items3 = [
    callback(Text.Text, obj3),
    callback(Button.Spacer, { size: 4 }),
    callback(RoleIconDefault, { name: roleName, source: { uri: roleImage }, size: 16 }),
    callback(Button.Spacer, { size: 8 }),
    callback(Text.Text, { variant: "text-xs/medium", color: "text-muted", children: "4:20 PM" }),
  ];
  obj2[1] = items3;
  const items4 = [
    callback2(View, obj2),
    callback(Text.Text, { variant: "text-md/normal", color: "text-default", style: textStyle, children: content }),
  ];
  obj1[1] = items4;
