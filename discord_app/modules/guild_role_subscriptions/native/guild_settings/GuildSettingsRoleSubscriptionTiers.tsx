...
import GuildRoleSettingsActionCreatorsAll from "GuildRoleSettingsActionCreators" /* 17774 */;
...
function Row(disabled) {
  ...
  style[1] = disabled;
  return closure_1_17(TouchableHitBoxDefault, { style, accessibilityRole: "button", onPress, onLongPress, disabled, children });
}
  ...
  if (undefined === first) {
    obj = {
      ...
      onLongPress() {
          ...
          obj.openLazy(asyncRequireImpl(17772, dependencyMap.paths), "TierArchiveOrDelete", obj);
        },
      children: null
    };
    ...
    obj3 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: _slicedToArray(obj1.useName(editStateId), 1)[0] };
    const items2 = [closure_17(tmp2(4570).Text, obj3), ];
    ...
    items3[3] = closure_17(tmp2(4570).Text, obj6);
    ...
    const obj8 = { price: tmp2(7248).formatPrice(first, constants.USD), interval: null };
    const tmp2Result1 = tmp2(7248);
    const obj9 = { interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 };
    obj8.interval = tmp2(15263).formatPlanInterval(obj9);
    formatToPlainStringResult = intl2.formatToPlainString(tmp2(1114).t.CgmBaG, obj8);
    const tmp2Result2 = tmp2(15263);
  }
}
function GuildSettingsRoleSubscriptionsTiersInner(guildId) {
  ...
  if (groupListingsFetchContext) {
      ...
      mapped = memo.map((editStateId) => {
        ...
        return closure_1_17(EditListingButton, {
          ...
          onPress() {
            ...
            let obj = guildEligibleForTierTemplates(17774);
            let id;
            if (first != null) {
              id = first.id;
            }
    ...
    const obj7 = { style: tmp.createTierLabel, variant: "text-md/medium", color: "mobile-text-heading-primary", children: null };
...
fn(4574);
let createStyles = { container: { height: "100%" }, tierManagementDescription: { marginBottom: 16, paddingHorizontal: 16 }, tierManagementButton: null, tierColumn: null, tierIcon: null, tierPrice: null, draftBadge: null, draftBadgeLabel: null, archiveBadge: null, archiveBadgeLabel: null, unsavedBadge: null, unsavedBadgeLabel: null, detailsRow: null, createTierLabel: null, spinner: null, disabled: null };
createStyles = { backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH, borderRadius: nativeDefault.radii.sm, flexDirection: "row", alignItems: "center", alignSelf: "stretch", justifyContent: "flex-start", height: 72, padding: 16, marginHorizontal: 16, marginBottom: 8 };
...
createStyles.draftBadge = { backgroundColor: nativeDefault.unsafe_rawColors.YELLOW_300, borderRadius: nativeDefault.radii.sm, paddingHorizontal: 4 };
let obj1 = { backgroundColor: nativeDefault.unsafe_rawColors.YELLOW_300, borderRadius: nativeDefault.radii.sm, paddingHorizontal: 4 };
...
createStyles.archiveBadge = { backgroundColor: nativeDefault.unsafe_rawColors.PRIMARY_500, borderRadius: nativeDefault.radii.sm, paddingHorizontal: 4 };
createStyles.archiveBadgeLabel = { textTransform: "uppercase" };
let obj3 = { backgroundColor: nativeDefault.unsafe_rawColors.PRIMARY_500, borderRadius: nativeDefault.radii.sm, paddingHorizontal: 4 };
createStyles.unsavedBadge = { backgroundColor: nativeDefault.colors.BACKGROUND_MOD_MUTED, borderRadius: nativeDefault.radii.sm, paddingHorizontal: 4 };
