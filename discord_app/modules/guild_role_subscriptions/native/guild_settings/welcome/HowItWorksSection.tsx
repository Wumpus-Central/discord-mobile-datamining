...
function HowItWorksCard(iconSource) {
  ...
  const tmp = callback3();
  { style: tmp.card, accessible: true, accessibilityLabel: "" + cardNumber + " - " + description, children: null };
  let obj = { style: tmp.cardNumber, variant: "text-xs/bold", color: "text-overlay-light", children: cardNumber };
...
const createCacheKey = { flex: 1, marginVertical: 6, backgroundColor: ThemesDefault.colors.BACKGROUND_SURFACE_HIGH, alignItems: "center", borderRadius: ThemesDefault.radii.sm, overflow: "hidden" };
...
const result = require("obj132").fileFinishedImporting("modules/guild_role_subscriptions/native/guild_settings/welcome/HowItWorksSection.tsx");

export default function HowItWorksSection() {
  const tmp = callback3();
  const obj = { cardNumber: 1, description: null, iconSource: null };
