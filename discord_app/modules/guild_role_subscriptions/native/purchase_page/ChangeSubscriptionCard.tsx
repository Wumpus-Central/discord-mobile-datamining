// === Module 16961: ChangeSubscriptionCard ===
...
export default function ChangeSubscriptionCard(activeSubscription) {
  ...
  const obj3 = _modDef4414(activeSubscription.currentPeriodEnd);
  const obj4 = { style: null, children: null };
  const items = [tmp.container, ];
  const formatResult = _modDef4414(activeSubscription.currentPeriodEnd).format("MMMM Do");
  ...
  const items1 = [closure_7(activeSubscription(4823).Text, obj6), closure_7(activeSubscription(1177).Spacer, { size: 16 }), , ];
  ...
  items1[2] = closure_8(activeSubscription(4823).Text, obj7);
  let tmp7Result = null;
  if (activeSubscription.status !== SubscriptionStatusTypes.CANCELED) {
    ...
    obj11.onPress = function onPress() {
      ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(16962, dependencyMap.paths), { subscriptionId: activeSubscription.id });
      const obj2 = { subscriptionId: activeSubscription.id };
      ActionSheetActionCreatorsDefault.hideActionSheet();
    };
    items3[1] = closure_7(tmp6(16954).ArrowButton, obj11);
  ...
  return closure_7(activeSubscription(7481).BottomSheet, { startExpanded: true, children: closure_8(View, obj4) });
};