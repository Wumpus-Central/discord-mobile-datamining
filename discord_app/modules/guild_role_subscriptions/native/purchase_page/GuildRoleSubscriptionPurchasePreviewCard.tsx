// === Module 16172: GuildRoleSubscriptionPurchasePreviewCard ===
...
function ChannelBenefitShowCase(description) {
  ...
  if (null != stateFromStores) {
    ...
    const obj3 = { size: tmp(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp(5328).getChannelIcon(stateFromStores) };
...
export default function GuildRoleSubscriptionPurchasePreviewCard(listingId) {
  ...
  const formattedSubscriptionPlan = listingId(16168).useFormattedSubscriptionPlan(listingId);
  const obj6 = listingId(16168);
  const tmp11 = guildId;
  if (str == null) {
    str = "";
  }
  const items = [closure_8(guildId(5894), { source: { uri: str }, style: tmp.image }), closure_8(listingId(1177).Spacer, { size: 16 }), ];
  ...
  obj8.children = items;
  const items2 = [closure_9(closure_6, obj8), closure_8(listingId(1177).Spacer, { size: 16 }), closure_8(tmp11(16174), { listingId }), ];
  ...
  if (tmp8Result6) {
    ...
    obj11.children = closure_9(tmp4(9796).GappedList, obj12);
    items3[1] = closure_8(closure_6, obj11);
    const obj45 = {
      onPress() {
          const obj = ActionSheetActionCreatorsDefault;
          obj.openLazy(asyncRequireImpl(16173, dependencyMap.paths), "PurchaseCard:" + listingId, { listingId, guildId });
        }
    };
