// === Module 16958: GuildRoleSubscriptionPurchasePreviewCard ===
...
function ChannelBenefitShowCase(description) {
  ...
  if (null != stateFromStores) {
    ...
    const obj3 = { size: tmp(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp(5325).getChannelIcon(stateFromStores) };
    ...
    items2[2] = closure_8(tmp(4823).Text, obj4);
...
export default function GuildRoleSubscriptionPurchasePreviewCard(listingId) {
  ...
  const formattedSubscriptionPlan = listingId(16954).useFormattedSubscriptionPlan(listingId);
  const obj6 = listingId(16954);
  const tmp11 = guildId;
  if (str == null) {
    str = "";
  }
  const items = [closure_8(guildId(5890), { source: { uri: str }, style: tmp.image }), closure_8(listingId(1177).Spacer, { size: 16 }), ];
  const obj10 = { children: null };
  const items1 = [closure_8(listingId(4823).Text, { variant: "heading-md/semibold", color: "mobile-text-heading-primary", children: _slicedToArray(obj2.useName(listingId), 1)[0] }), closure_8(listingId(1177).Spacer, { size: 4 }), closure_8(listingId(4823).Text, { variant: "heading-md/medium", color: "text-default", children: formattedSubscriptionPlan })];
  obj10.children = items1;
  items[2] = closure_9(closure_6, obj10);
  obj8.children = items;
  const items2 = [closure_9(closure_6, obj8), closure_8(listingId(1177).Spacer, { size: 16 }), closure_8(tmp11(16960), { listingId }), ];
  ...
  if (tmp8Result6) {
    ...
    if (size > 0) {
      ...
      obj14.count = size;
      const items4 = [closure_8(ContentHeader, obj14), closure_8(tmp4(1177).Spacer, { size: 8 }), , ];
      ...
      items4[3] = closure_8(tmp4(1177).Spacer, { size: 4 });
    ...
    if (null != first3) {
      ...
      obj17.count = length;
      const items7 = [closure_8(ContentHeader, obj17), closure_8(tmp4(1177).Spacer, { size: 12 }), , ];
      ...
      items7[3] = closure_8(tmp4(1177).Spacer, { size: 6 });
    ...
    if (null != first4) {
      ...
      obj22.count = length2;
      const items8 = [closure_8(ContentHeader, obj22), closure_8(tmp4(1177).Spacer, { size: 12 }), , ];
      ({ name: obj23.title, description: obj23.description } = first4);
      items8[2] = closure_8(BenefitShowCase, { title: null, description: null });
      items8[3] = closure_8(tmp4(1177).Spacer, { size: 6 });
    ...
    obj11.children = closure_9(tmp4(10687).GappedList, obj12);
    items3[1] = closure_8(closure_6, obj11);
    const obj45 = {
      onPress() {
          const obj = ActionSheetActionCreatorsDefault;
          obj.openLazy(asyncRequireImpl(16959, dependencyMap.paths), "PurchaseCard:" + listingId, { listingId, guildId });
        }
    };
