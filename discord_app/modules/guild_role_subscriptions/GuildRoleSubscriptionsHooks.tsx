...
export const useFetchListingsForSubscriptions = (arg0) => {
  ...
  const stateFromStoresArray = require("initialize").useStateFromStoresArray(
    items1,
    () => memo.filter((item) => !didFetchListingForSubscriptionPlanId.getDidFetchListingForSubscriptionPlanId(item)),
    items2,
  );
  ...
  const effect = noop.useEffect(() => {
    ...
    if (tmp) {
      ...
      const allPromises = Promise.all(
        stateFromStoresArray.map((item) => closure_1_2(memo[6]).fetchSubscriptionListingForPlan(item)),
      );
      Promise.all(stateFromStoresArray.map((item) => closure_1_2(memo[6]).fetchSubscriptionListingForPlan(item)))
        .catch(() => {})
        .then(() => {
          closure_1_2(false);
        });
