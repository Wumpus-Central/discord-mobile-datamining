...
function handleRelationshipAdd(relationship) {
  ...
  if (!tmp) {
    ...
    const result = importAll(16573).showPendingNotification(relationship.user);
    const obj2 = importAll(16573);
  }
}
function handleFriendRequestAccepted(user) {
  ...
  const result = importAll(16573).showAcceptedNotification(user);
}
