...
function handleRelationshipAdd(relationship) {
  ...
  if (!tmp) {
    ...
    const result = importAll(16330).showPendingNotification(relationship.user);
    const obj2 = importAll(16330);
  }
}
function handleFriendRequestAccepted(user) {
  ...
  const result = importAll(16330).showAcceptedNotification(user);
}
