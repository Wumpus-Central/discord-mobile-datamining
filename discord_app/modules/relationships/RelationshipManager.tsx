...
function handleRelationshipAdd(relationship) {
  ...
  if (!tmp) {
    ...
    const result = importAll(16631).showPendingNotification(relationship.user);
    const obj2 = importAll(16631);
  }
}
function handleFriendRequestAccepted(user) {
  ...
  const result = importAll(16631).showAcceptedNotification(user);
}
