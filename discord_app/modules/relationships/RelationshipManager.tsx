...
function handleRelationshipAdd(relationship) {
  ...
  if (!tmp) {
    ...
    const result = importAll(16646).showPendingNotification(relationship.user);
    const obj2 = importAll(16646);
  }
}
function handleFriendRequestAccepted(user) {
  ...
  const result = importAll(16646).showAcceptedNotification(user);
}
