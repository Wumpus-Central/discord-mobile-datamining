// === Module 13073: HubEmailConnectionWaitlist ===
...
export default function HubEmailConnectionWaitlist(school) {
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft() {
        ...
        obj.onPress = onPress;
        return closure_2_6(onClose(7705).HeaderActionButton, obj);
      }
    });
  }, items);
  const obj2 = { style: tmp.container, children: null };
  let obj = onClose(1484);
  const items1 = [closure_6(closure_5, { source: navigation(13074), style: tmp.header }), , , ];
  ...
  items1[2] = closure_6(onClose(4823).Text, obj5);
  ...
  obj6.children = closure_6(onClose(5271).Button, obj7);
