...
export const useCreateGameInvitePost = function useCreateGameInvitePost(appliedTagIds) {
  ...
  const application = description(stateFromStores[6]).useGameInvitesChannelOfficialApplication(
    parentChannel.id,
  ).application;
  ...
  stateFromStores = description(stateFromStores[8]).useStateFromStores(
    items,
    () => {
