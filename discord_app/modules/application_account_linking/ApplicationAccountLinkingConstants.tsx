// discord_app/modules/application_account_linking/ApplicationAccountLinkingConstants.tsx
const date = new Date("2026-07-10Z-07:00");
const date1 = new Date("2026-09-22Z-07:00");
const result = require("set").fileFinishedImporting("modules/application_account_linking/ApplicationAccountLinkingConstants.tsx");

export const AuthorizeFlow = { RPC: "rpc", WEB: "web" };
export const RIOT_CONNECTION_DEPRECATION_DATE = date;
export const BATTLENET_CONNECTION_DEPRECATION_DATE = date1;