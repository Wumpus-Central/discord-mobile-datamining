// === Module 16385: ConnectGuardianShareScreen ===
...
function ConnectGuardianShareScreen() {
  ...
  const syncMessages = obj.useSyncMessages(getLinkCode(2255).messagesLoader);
  const callback = React.useCallback(() => {
    const intl = getLinkCode(1236).intl;
    getLinkCode(3955).presentFailedToast(intl.string(getLinkCode(1236).t.R0RpRX));
    let arr = callback(4461);
    arr = arr.pop();
  }, []);
  let obj1 = getLinkCode(11237);
  ...
  importDefault(13997)(importDefault(4461).pop);
  ...
  obj1[4] = intl.string(importDefault(2255).ITlV6p);
  const items2 = [callback(getLinkCode(4251).Text, obj1), ];
  obj2 = { style: tmp.body, variant: "text-sm/medium", color: "text-muted", children: null };
  const intl2 = getLinkCode(1236).intl;
  obj2[3] = intl2.format(importDefault(2255).F4GT2S, { link: "https://support.discord.com/hc/articles/14155060633623" });
  items2[1] = callback(getLinkCode(4251).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4664).Stack, obj), ];
  ...
  obj4[3] = intl3.string(importDefault(2255).pojgfk);
  const items4 = [callback(getLinkCode(4251).Text, obj4), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(13998).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4664).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4664).Stack, obj);
    obj6[0] = tmp11(getLinkCode(7746).ModalContent, obj7);
    return tmp11(getLinkCode(7745).ModalScreen, obj6);
  }
  const tmp9 = importDefault(13997);
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5703).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj[1] = callback(5209).getHeaderBackButton(callback2(4461).pop);
  ...
  return callback(require(11249) /* Modal */.Modal, obj);
};