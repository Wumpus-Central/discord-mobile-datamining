// === Module 16978: ConnectGuardianShareScreen ===
...
import useOnNewPendingRequestDefault from "useOnNewPendingRequest" /* 14419 */;
...
function ConnectGuardianShareScreen() {
  ...
  const syncMessages = obj.useSyncMessages(getLinkCode(2370).messagesLoader);
  const callback = React.useCallback(() => {
    const intl = getLinkCode(1236).intl;
    getLinkCode(4162).presentFailedToast(intl.string(getLinkCode(1236).t.R0RpRX));
    let arr = callback(4689);
    arr = arr.pop();
  }, []);
  obj1 = getLinkCode(11427);
  ...
  useOnNewPendingRequestDefault(_modDef4689.pop);
  ...
  const items2 = [callback(getLinkCode(4442).Text, obj1), ];
  ...
  items2[1] = callback(getLinkCode(4442).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4891).Stack, obj), ];
  ...
  const items4 = [callback(getLinkCode(4442).Text, obj4), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(14420).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4891).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4891).Stack, obj);
    obj6[0] = tmp11(getLinkCode(8639).ModalContent, obj7);
    return tmp11(getLinkCode(8638).ModalScreen, obj6);
  }
  const tmp9 = useOnNewPendingRequestDefault;
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(6013).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj = {
      headerShown: true,
      headerLeft: callback(5460).getHeaderBackButton(callback2(4689).pop),
      headerTitle() {
        return null;
      },
      render() {
