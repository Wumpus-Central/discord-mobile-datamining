...
function ConnectGuardianShareScreen() {
  ...
  const callback = React.useCallback(() => {
    ...
    getLinkCode(3984).presentFailedToast(intl.string(getLinkCode(1236).t.R0RpRX));
    let arr = callback(4490);
    arr = arr.pop();
  }, []);
  let obj1 = getLinkCode(11242);
  ...
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(14042).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4693).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4693).Stack, obj);
    obj6[0] = tmp11(getLinkCode(7731).ModalContent, obj7);
    return tmp11(getLinkCode(7730).ModalScreen, obj6);
  }
  const tmp9 = useOnNewPendingRequest;
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5744).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj[1] = callback(5253).getHeaderBackButton(callback2(4490).pop);
