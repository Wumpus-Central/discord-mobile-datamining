...
function ConnectGuardianShareScreen() {
  ...
  const items2 = [callback(getLinkCode(4340).Text, obj1), ];
  ...
  items2[1] = callback(getLinkCode(4340).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4752).Stack, obj), ];
  ...
  const items4 = [callback(getLinkCode(4340).Text, obj4), ];
  if (null != stateFromStores) {
    ...
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4752).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4752).Stack, obj);
    obj6[0] = tmp11(getLinkCode(7849).ModalContent, obj7);
    return tmp11(getLinkCode(7848).ModalScreen, obj6);
  }
  const tmp9 = useOnNewPendingRequest;
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5862).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj[1] = callback(5309).getHeaderBackButton(callback2(4549).pop);
