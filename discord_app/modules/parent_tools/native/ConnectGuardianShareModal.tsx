// === Module 16809: ConnectGuardianShareScreen ===
...
function ConnectGuardianShareScreen() {
  ...
  const syncMessages = obj.useSyncMessages(getLinkCode(2337).messagesLoader);
  const callback = React.useCallback(() => {
    const intl = getLinkCode(1236).intl;
    getLinkCode(4097).presentFailedToast(intl.string(getLinkCode(1236).t.R0RpRX));
    let arr = callback(4611);
    arr = arr.pop();
  }, []);
  obj1 = getLinkCode(11485);
  ...
  useOnNewPendingRequestDefault(_modDef4611.pop);
  ...
  const items2 = [callback(getLinkCode(4376).Text, obj1), ];
  ...
  items2[1] = callback(getLinkCode(4376).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4813).Stack, obj), ];
  ...
  const items4 = [callback(getLinkCode(4376).Text, obj4), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(14336).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4813).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4813).Stack, obj);
    obj6[0] = tmp11(getLinkCode(7953).ModalContent, obj7);
    return tmp11(getLinkCode(7952).ModalScreen, obj6);
  }
  const tmp9 = useOnNewPendingRequestDefault;
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5929).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj = {
      headerShown: true,
      headerLeft: callback(5376).getHeaderBackButton(callback2(4611).pop),
      headerTitle() {
        return null;
      },
      render() {
