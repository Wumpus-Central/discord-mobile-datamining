...
function maybePresentModal(daysRemaining) {
  ...
  if (tmp5) {
    ...
    obj[0] = daysRemaining;
    ACTION_SHEET_HEIGHT_HALF.openLazy(asyncRequireImpl(16541, dependencyMap.paths), "ParentalConsentWarningModal", obj);
...
let prototype = function ParentalConsentWarningManager() {
  ...
  applyArgumentsResult.actions = {
    ...
    POST_CONNECTION_OPEN() {
      ...
      callback(16543).maybeFetchWarning();
        ...
        if (tmp) {
          if (someResult) {
            ...
            if (warning != null) {
              ...
              if (surfaces != null) {
                hasItem = surfaces.includes(callback(14113).ParentalConsentWarningSurface.BANNER);
              }
            }
            if (true === hasItem) {
              callback(16543).forceFetchWarning();
              const obj2 = callback(16543);
            }
          } else {
            callback(16543).forceFetchWarning();
            const obj = callback(16543);
    ...
    LOGOUT() {
      let c3;
      callback(16543).resetFetchState();
    }
  };
