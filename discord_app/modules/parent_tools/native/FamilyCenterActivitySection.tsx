// === Module 15236: FamilyCenterActivitySection ===
...
import useFamilyCenterActivities from "useFamilyCenterActivities" /* 15231 */;
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  const actionsForDisplayType = displayType(15231).useActionsForDisplayType(displayType);
  const obj = displayType(15231);
  const actionTotalsForDisplayType = displayType(15231).useActionTotalsForDisplayType(displayType);
  const obj2 = displayType(15231);
  const familyCenterActions = displayType(12252).useFamilyCenterActions({});
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const formatToPlainStringResult = intl.formatToPlainString(loadMore(2484)["7dMmJY"], obj4);
    const obj5 = { style: loadMoreButton.container, children: null };
    const obj6 = { displayType };
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj6), substr.map((action) => closure_1_9(loadMore(15237), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj10.children = closure_9(tmp(4823).Text, obj11);
        tmp17Result = closure_9(tmp(5425).PressableOpacity, obj10);
      }
      obj7.children = tmp17Result;
      closure_9(closure_6, obj7);
    }
  }
  const obj3 = displayType(12252);
};