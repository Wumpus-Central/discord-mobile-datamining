...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    obj1 = { displayType };
    const items1 = [closure_9(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => closure_1_9(loadMore(14955), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5.children = closure_9(tmp(4602).Text, obj6);
        tmp17Result = closure_9(tmp(5174).PressableOpacity, obj5);
      }
