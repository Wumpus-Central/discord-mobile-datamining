// === Module 11483: FamilyCenterModalRequestConfirm ===
...
class FamilyCenterModalRequestConfirm {
  constructor(arg0) {
    ...
    obj23[3] = require("module_4611").pop;
...
function FamilyCenterConfirmAgeScreen(teenIdentity) {
  ...
  const tmp7 = stateFromStores(9000)(stateFromStores);
  dependencyMap = tmp7;
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp2) {
      tmp2 = stateFromStores !== navigation(1955).AgeVerificationStatusUkAndAusOnly.UNVERIFIED;
    }
    if (tmp2) {
      const replaced = navigation.replace(navigation(11484).FamilyCenterModalRequestSections.VERIFYING);
    }
  }, items1);
  const callback = React.useCallback(() => {
    let obj = stateFromStores(7942);
    obj = { entryPoint: navigation(7944).AgeVerificationModalEntryPoint.FAMILY_CENTER_CONNECTION };
    const result = obj.showAgeVerificationGetStartedModal(obj);
  }, []);
  obj = { style: tmp.content, children: null };
  obj = { style: tmp.art, children: callback(navigation(6136).FamilyShieldSpotIllustration, {}) };
  ...
  obj1[3] = intl.formatToPlainString(stateFromStores(2337).pQQMJ7, { username: str });
  items2[1] = callback(navigation(4376).Text, obj1);
  const obj3 = { variant: "text-sm/medium", color: "text-muted", style: tmp.description, children: null };
  const intl2 = tmp2(1236).intl;
  obj3[3] = intl2.format(stateFromStores(2337)["0o3yg8"], { link: "https://support.discord.com/hc/articles/14155060633623" });
  items2[2] = callback(navigation(4376).Text, obj3);
  obj[1] = items2;
  ...
  obj6[1] = intl3.string(stateFromStores(2337)["3oUE4o"]);
  obj6[2] = callback;
  const items4 = [callback(navigation(4815).Button, obj6), ];
  ...
  items4[1] = callback(navigation(4815).Button, obj7);
  obj5[0] = items4;
  obj4[0] = closure_13(navigation(5257).ButtonGroup, obj5);
  items3[1] = callback(navigation(11495).ModalFooter, obj4);
  obj2[0] = items3;
  return closure_13(navigation(7952).ModalScreen, obj2);
}
...
function FamilyCenterPrereqMustBeAdultScreen() {
  ...
  const tmp3 = stateFromStores(9000)(stateFromStores);
  dependencyMap = tmp3;
  ...
  const intl = navigation(1236).intl;
  obj[0] = intl.string(stateFromStores(2337).BQFHXW);
  const intl2 = navigation(1236).intl;
  obj[1] = intl2.format(stateFromStores(2337).WDjaKn, { link: { onClick: callback } });
  obj = { text: null, onPress: null };
  const intl3 = navigation(1236).intl;
  obj[0] = intl3.string(navigation(1236).t["NX+WJN"]);
  obj[1] = stateFromStores(4611).pop;
  obj[2] = obj;
  return callback(FamilyCenterPrereqScreen, obj);
}
...
createCacheKey[1] = HeaderBackImage.getHeaderCloseButton(_modDef4611.pop);
...
export default function FamilyCenterRequestModal(userId) {
  ...
  const FamilyCenterConnectionPrereqExperiment = userId(11496).FamilyCenterConnectionPrereqExperiment;
  ...
  const FamilyCenterModalRequestSections = userId(11484).FamilyCenterModalRequestSections;
  ...
  return callback(userId(11497).Modal, obj);
};
