...
function FamilyCenterTeenQRCodeButtonInner() {
  ...
  const callback2 = obj.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14957, dependencyMap.paths), React5, obj);
    }
  }, items4);
  ...
  obj.icon = closure_12(getLinkCode(stateFromStores[23]).ShareIcon, { size: "sm", color: "control-primary-text-default" });
  ...
  obj1.icon = closure_12(getLinkCode(stateFromStores[25]).QrCodeIcon, { size: "sm", color: "control-secondary-text-default" });
...
export const FamilyCenterParentQRCodeButton = function FamilyCenterParentQRCodeButton() {
      ...
      if (null != pathname) {
        ...
        if (tmpResult.isDiscordHostname(hostname)) {
          if (null !== pathname.match(closure_1_8)) {
            const result = handleQrCodeScanSucess(paths[28]).handleFamilyCenterQRCodeScan(pathname, "FamilyCenterQRCodeScan");
  ...
  let obj = handleQrCodeScanSucess(8715);
  if (obj.useHasMaxConnections()) {
    return null;
  } else {
