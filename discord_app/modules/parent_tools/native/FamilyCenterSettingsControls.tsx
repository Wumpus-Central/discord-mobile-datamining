...
function SpendingLimitRow(teenId) {
  ...
  if ("off" === kind) {
    ...
    obj[2] = intl2.string(importDefault(2255).YEnpaj);
    obj[0] = callback(tmp2(4251).Text, obj);
    let tmp8 = obj;
    const stringResult = intl2.string(importDefault(2255).YEnpaj);
  } else if ("on" === kind) {
    ...
    obj1[0] = callback(tmp2(4251).Text, obj2);
    tmp8 = obj1;
    ...
    obj3[0] = callback(tmp2(4251).Text, obj4);
    ...
    obj6[0] = callback(tmp2(4251).Text, obj7);
    const intl = tmp2(1236).intl;
    const obj8 = { variant: "text-sm/normal", style: null, children: null };
    obj8[1] = tmp.subLabelCritical;
    obj8[2] = intl.string(importDefault(2255).Q2msVQ);
    obj6[1] = callback(tmp2(4251).Text, obj8);
    tmp8 = obj6;
    const stringResult1 = intl.string(importDefault(2255).Q2msVQ);
    ...
    obj10[2] = intl4.string(importDefault(2255).kGFuGn);
    obj9[0] = callback(tmp2(4251).Text, obj10);
    const intl5 = tmp2(1236).intl;
    const stringResult2 = intl4.string(importDefault(2255).kGFuGn);
    const obj11 = { variant: "text-sm/normal", style: null, children: null };
    obj11[1] = tmp.subLabelCritical;
    obj11[2] = intl5.string(importDefault(2255).FUu2b0);
    obj9[1] = callback(tmp2(4251).Text, obj11);
    tmp8 = obj9;
    const stringResult3 = intl5.string(importDefault(2255).FUu2b0);
  }
  ({ trailing, subLabel } = tmp8);
  const obj12 = { label: null, trailing: null, subLabel: null, onPress: null, arrow: null, disabled: null };
  const intl3 = tmp2(1236).intl;
  obj12[0] = intl3.string(importDefault(2255).gMeekL);
  obj12[1] = trailing;
  obj12[2] = subLabel;
  let fn;
  if (null != teenId) {
  ...
  obj12[4] = null != teenId;
  obj12[5] = null == teenId;
  return callback(teenId(5286).TableRow, obj12);
}
...
function FamilyCenterSettingsParentalControls() {
  ...
  const ParentalControlledSpendingLimit = tmp2(13942).ParentalControlledSpendingLimit;
  let id;
  if (selectedTeenUser != null) {
  ...
  ({ subLabel, trailing } = shouldLoadSettingsForSelectedTeenUser(14027)(rules));
  obj = { style: tmp.parentalControlsContainer, children: null };
  obj = { variant: "text-sm/semibold", children: null };
  const intl = tmp2(1236).intl;
  obj[1] = intl.string(shouldLoadSettingsForSelectedTeenUser(2255).ahKIJO);
  const items1 = [callback(selectedTeenUser(4251).Text, obj), , ];
  obj1 = { variant: "text-sm/medium", color: "text-muted", children: null };
  const intl2 = tmp2(1236).intl;
  obj1[2] = intl2.string(shouldLoadSettingsForSelectedTeenUser(2255).Sv236e);
  items1[1] = callback(selectedTeenUser(4251).Text, obj1);
  ...
  const items2 = [callback(selectedTeenUser(5286).TableRow, obj3), , , ];
  ...
  items2[1] = callback(selectedTeenUser(5286).TableRow, obj4);
  ...
  if (tmp15Result) {
    ...
    const intl5 = tmp2(1236).intl;
    obj6[0] = intl5.string(shouldLoadSettingsForSelectedTeenUser(2255)["1Op+NP"]);
    obj6[1] = subLabel;
    obj6[2] = trailing;
    ...
    tmp15Result = tmp15(tmp2(5286).TableRow, obj6);
  }
  items2[3] = tmp15Result;
  obj2[1] = closure_8(selectedTeenUser(5620).TableRowGroup, { hasIcons: false, children: items2 });
  items1[2] = callback(View, obj2);
  obj[1] = items1;
  return closure_8(selectedTeenUser(4664).Stack, obj);
}
