...
function SpendingLimitRow(teenId) {
  ...
  obj12[4] = null != teenId;
  obj12[5] = null == teenId;
  return callback(teenId(5300).TableRow, obj12);
}
...
function FamilyCenterSettingsParentalControls() {
  ...
  const ParentalControlledSpendingLimit = tmp2(13949).ParentalControlledSpendingLimit;
  let id;
  if (selectedTeenUser != null) {
  ...
  ({ subLabel, trailing } = shouldLoadSettingsForSelectedTeenUser(14034)(rules));
  ...
  const items2 = [callback(selectedTeenUser(5300).TableRow, obj3), , , ];
  ...
  items2[1] = callback(selectedTeenUser(5300).TableRow, obj4);
  ...
  if (tmp15Result) {
    ...
    tmp15Result = tmp15(tmp2(5300).TableRow, obj6);
  }
  items2[3] = tmp15Result;
  obj2[1] = closure_8(selectedTeenUser(5634).TableRowGroup, { hasIcons: false, children: items2 });
  items1[2] = callback(View, obj2);
  obj[1] = items1;
  return closure_8(selectedTeenUser(4663).Stack, obj);
}
