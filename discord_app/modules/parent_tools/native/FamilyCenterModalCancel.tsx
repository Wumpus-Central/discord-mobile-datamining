...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  const callback = noop.useCallback(() => {
    let arr = cancelLinkRequest(4777);
    arr = arr.pop();
  }, []);
  ...
  cancelLinkRequest(38)(cancelLinkRequest(8679)(), "FamilyCenterCancelModal should only be rendered for parents.");
  obj = { children: null };
  obj = { children: null };
  const obj1 = { style: tmp.header, children: null };
  const obj2 = { otherUser, iconSrc: null };
  const tmp2 = cancelLinkRequest(8679)();
  obj2.iconSrc = cancelLinkRequest(7006);
  const items1 = [closure_5(cancelLinkRequest(14951), obj2), , ];
  ...
  const items2 = [closure_5(otherUser(8450).ModalContent, obj), ];
  ...
  const items3 = [closure_5(otherUser(4989).Button, obj6), ];
  ...
  obj7.onPress = cancelLinkRequest(4777).pop;
  items3[1] = closure_5(otherUser(4989).Button, obj7);
  obj5.children = items3;
  obj4.children = closure_6(otherUser(5447).ButtonGroup, obj5);
  items2[1] = closure_5(otherUser(11957).ModalFooter, obj4);
  obj.children = items2;
