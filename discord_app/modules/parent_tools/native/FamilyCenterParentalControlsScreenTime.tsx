...
function ScheduleRuleRow(rule) {
  ...
  obj = { label: scheduleRuleDateRange, subLabel: rule(10115).formatDays(rule.days), trailing: null, arrow: null, onPress: null };
  ...
  obj.trailing = closure_5(rule(4570).Text, { variant: "text-sm/medium", color: "text-subtle", children: stringResult });
  obj.arrow = !readOnly;
  let fn;
  if (!readOnly) {
    fn = () => {
  ...
  obj.onPress = fn;
  return closure_5(rule(5619).TableRow, obj);
}
