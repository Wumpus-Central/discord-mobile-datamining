// === Module 14351: FamilyCenterModalDeclineScreen ===
...
function FamilyCenterModalDeclineScreen(otherUser) {
  ...
  const callback = React.useCallback(() => {
    let arr = declineLinkRequest(5265);
    arr = arr.pop();
  }, []);
  ...
  declineLinkRequest(38)(!declineLinkRequest(7660)(), "FamilyCenterDeclineLinkModal should only be rendered for teens.");
  obj = { children: null };
  obj = { children: null };
  obj1 = { style: tmp.header, children: null };
  const obj2 = { otherUser, iconSrc: null };
  const tmp2 = declineLinkRequest(7660)();
  obj2[1] = declineLinkRequest(8182);
  const items1 = [callback(declineLinkRequest(14347), obj2), , ];
  const obj3 = { style: tmp.headerText, variant: "text-lg/bold", children: null };
  let intl = otherUser(1236).intl;
  obj3[2] = intl.string(declineLinkRequest(2336).teIRCR);
  items1[1] = callback(otherUser(4739).Text, obj3);
  items1[2] = callback(declineLinkRequest(14317), { user: otherUser });
  obj1[1] = items1;
  ...
  obj5[3] = intl2.string(declineLinkRequest(2336).cXgKMD);
  const items3 = [callback(otherUser(4739).Text, obj5), ];
  const obj6 = { variant: "text-sm/normal", color: "text-default", children: null };
  const intl3 = otherUser(1236).intl;
  obj6[2] = intl3.string(declineLinkRequest(2336).LcM8BS);
  items3[1] = callback(otherUser(4739).Text, obj6);
  obj4[1] = items3;
  items2[1] = callback2(View, obj4);
  obj[0] = items2;
  const items4 = [callback2(otherUser(5450).ModalContent, obj), ];
  ...
  obj9[3] = intl4.string(declineLinkRequest(2336).dKxFcn);
  obj9[4] = callback2;
  const items5 = [callback(otherUser(4750).Button, obj9), ];
  ...
  obj10[2] = declineLinkRequest(5265).pop;
  items5[1] = callback(otherUser(4750).Button, obj10);
  obj8[0] = items5;
  obj7[0] = callback2(otherUser(6723).ButtonGroup, obj8);
  items4[1] = callback(otherUser(11153).ModalFooter, obj7);
  obj[0] = items4;
