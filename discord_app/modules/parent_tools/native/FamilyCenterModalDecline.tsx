// === Module 14283: FamilyCenterModalDeclineScreen ===
...
function FamilyCenterModalDeclineScreen(otherUser) {
  ...
  const tmp2 = declineLinkRequest(7622)();
  obj2[1] = declineLinkRequest(8143);
  const items1 = [callback(declineLinkRequest(14279), obj2), , ];
  const obj3 = { style: tmp.headerText, variant: "text-lg/bold", children: null };
  let intl = otherUser(1236).intl;
  obj3[2] = intl.string(declineLinkRequest(2335).teIRCR);
  items1[1] = callback(otherUser(4734).Text, obj3);
  items1[2] = callback(declineLinkRequest(14249), { user: otherUser });
  obj1[1] = items1;
  ...
  obj5[3] = intl2.string(declineLinkRequest(2335).cXgKMD);
  const items3 = [callback(otherUser(4734).Text, obj5), ];
  const obj6 = { variant: "text-sm/normal", color: "text-default", children: null };
  const intl3 = otherUser(1236).intl;
  obj6[2] = intl3.string(declineLinkRequest(2335).LcM8BS);
  items3[1] = callback(otherUser(4734).Text, obj6);
  obj4[1] = items3;
  items2[1] = callback2(View, obj4);
  obj[0] = items2;
  const items4 = [callback2(otherUser(5445).ModalContent, obj), ];
  ...
  obj9[3] = intl4.string(declineLinkRequest(2335).dKxFcn);
  obj9[4] = callback2;
  const items5 = [callback(otherUser(4745).Button, obj9), ];
  ...
  obj10[2] = declineLinkRequest(5260).pop;
  items5[1] = callback(otherUser(4745).Button, obj10);
  obj8[0] = items5;
  obj7[0] = callback2(otherUser(6687).ButtonGroup, obj8);
  items4[1] = callback(otherUser(11114).ModalFooter, obj7);
  obj[0] = items4;
