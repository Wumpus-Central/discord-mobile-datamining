...
function ChangeSpendingLimitScreen(teenId) {
  ...
  if (null != monthlySpend) {
    ...
    if (monthlySpend > 0) {
      formatPriceResult = tmp2(5764).formatPrice(monthlySpend, tmp5);
      const tmp2Result = tmp2(5764);
    }
  }
  ...
  obj3[4] = intl4.string(_handleSave(2285).DjSv82);
  let str = "number-pad";
  if (exponent > 0) {
    str = "decimal-pad";
  }
  ...
  if (null == formatPriceResult) {
    ...
    obj10[0] = items4;
    obj9[0] = tmp8(tmp2(5117).ButtonGroup, obj10);
    items3[1] = tmp9(tmp2(11246).ModalFooter, obj9);
    obj8[0] = items3;
    return tmp8(tmp2(7757).ModalScreen, obj8);
    ...
    const items5 = [tmp9(tmp2(7772).WarningIcon, obj13), ];
    ...
    obj15[1] = renewalDate;
    formatToPlainStringResult = intl6.formatToPlainString(tmp10(2285).Tk6x4X, obj15);
    ...
    obj16[2] = intl5.formatToPlainString(tmp10(2285).pfAlRY, obj17);
...
export default function ChangeSpendingLimitModal(teenId) {
  ...
  const memo = React.useMemo(() => {
    ...
    obj[2] = function headerTitle() {
      ...
      obj[1] = intl.string(callback2(2285).xMRO6A);
  ...
  return callback(teenId(11248).Modal, obj);
};