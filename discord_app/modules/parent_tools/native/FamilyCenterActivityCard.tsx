...
import _modDef14402 from "../../../../_runtime/metro/14402__.js";
...
function FamilyCenterActivityCardPrefaceText() {
  ...
  const ageSpecificText = obj4.useAgeSpecificText(
    intl.formatToPlainString(_modDef2486.tazvHQ, { activeLinks: activeLinkUserIds.length }),
    intl2.string(_modDef2486.KrLnkE),
  );
  const obj6 = { style: tmp.container, children: null };
  let tmp12 = null;
  if (!tmp4) {
    const obj7 = { color: tmp.icon.color, source: _modDef14402, style: tmp.icon };
  ...
  const formatToPlainStringResult = intl.formatToPlainString(_modDef2486.tazvHQ, {
    ...
    closure_3 = closure_3.useMemo(
      () =>
        activeLinkUsers.map((id) => {
          ...
          const name = selectedTeenUser(4675).getName(id);
          const obj2 = selectedTeenUser(4675);
          obj.label = "" + name + " (" + selectedTeenUser(4675).getUserTag(id) + ")";
          obj.value = id.id;
          return obj;
        }),
      items,
    ...
    if (undefined !== selectedTeenUser) {
      ...
      obj10.onPress = function onPress() {
        if (undefined !== selectedTeenUser) {
          ...
          obj.openLazy(asyncRequireImpl(8721, dependencyMap.paths), FamilyCenterTeenAccountSelect, obj2);
          const tmp5 = asyncRequireImpl(8721, dependencyMap.paths);
        }
      };
      tmp12 = closure_15;
...
export default function FamilyCenterActivityCard() {
  ...
  if (undefined === selectedTeenUser) {
    ...
    const sortedActivityTypeConfigs = tmp2(7007).getSortedActivityTypeConfigs();
