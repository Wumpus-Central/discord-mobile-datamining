...
import _modDef15175 from "../../../../_runtime/metro/15175__.js";
...
import FamilyCenterActivitySectionDefault from "FamilyCenterActivitySection.tsx";
import FamilyCenterSettingsControlsDefault from "FamilyCenterSettingsControls.tsx";
...
function FamilyCenterActivityCardPrefaceText() {
  ...
  const ageSpecificText = obj4.useAgeSpecificText(
    intl.formatToPlainString(_modDef2482.tazvHQ, { activeLinks: activeLinkUserIds.length }),
    intl2.string(_modDef2482.KrLnkE),
  );
  const obj6 = { style: tmp.container, children: null };
  let tmp12 = null;
  if (!tmp4) {
    const obj7 = { color: tmp.icon.color, source: _modDef15175, style: tmp.icon };
  ...
  const formatToPlainStringResult = intl.formatToPlainString(_modDef2482.tazvHQ, {
  ...
  constructor() {
    ...
    obj1 = {
      ...
      onError() {
        ...
        return activeLinkUsers(selectTeenUser[25]).presentFailedToast(
          intl.string(selectedTeenUser(selectTeenUser[12]).Wu8BK2),
        );
      },
    };
    ...
    closure_3 = closure_3.useMemo(
      () =>
        activeLinkUsers.map((id) => {
          const obj = { label: null, value: null };
          const name = selectedTeenUser(4601).getName(id);
          const obj2 = selectedTeenUser(4601);
          obj.label = "" + name + " (" + selectedTeenUser(4601).getUserTag(id) + ")";
          obj.value = id.id;
          return obj;
        }),
      items,
    ...
    if (undefined !== selectedTeenUser) {
      ...
      obj10.onPress = function onPress() {
        if (undefined !== selectedTeenUser) {
          ...
          obj.openLazy(asyncRequireImpl(9546, dependencyMap.paths), FamilyCenterTeenAccountSelect, obj2);
          const tmp5 = asyncRequireImpl(9546, dependencyMap.paths);
        }
      };
      tmp12 = closure_15;
...
let size = {
  ...
  width: nativeDefault.space.PX_16,
...
export default function FamilyCenterActivityCard() {
  ...
  if (undefined === selectedTeenUser) {
    ...
    const sortedActivityTypeConfigs = tmp2(7837).getSortedActivityTypeConfigs();
    ...
    obj6.children = found.map((item, index) => {
      ...
      return React6(
        View,
        { style: other, children: React6(FamilyCenterActivityTotalDefault, { displayType: tmp }) },
        "total-" + tmp,
      );
    });
    ...
    if (tmp6) {
      ...
      tmp11Result = closure_8(View, obj7);
    }
