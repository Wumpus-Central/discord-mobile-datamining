// === Module 7799: buildPlatformedThemedEmojiColorPalette ===

// Module 7799 (buildPlatformedThemedEmojiColorPalette)
import set from "set" /* 2 */;
import hexToRgba from "hexToRgba" /* 4293 */;
import hslToRgbDefault from "hslToRgb" /* 4294 */;
import getURLDefault from "getURL" /* 7460 */;
  ...
  if (null != colors) {
    ...
    if (colors.length >= 1) {
      ...
      if (null != parseStringResult) {
        ...
        const tmp2Result1 = tmp2(4293);
        const tmp8 = c3;
        obj = { foreground: null, background: null, ratio: 5, saturationFactor: null };
        tmp5Result = tmp5(689);
        const tmp2Result2 = tmp2(4293);
        const colorLightnessAdjusted1 = tmp2(4293).getColorLightnessAdjusted(parseStringResult, 0.6, false);
        ...
        const accessibleForegroundColor2 = tmp2(4293).getAccessibleForegroundColor(obj1);
        const tmp2Result4 = tmp2(4293);
        ...
        const accessibleForegroundColor3 = tmp2(4293).getAccessibleForegroundColor(obj2);
        let hexResult;
