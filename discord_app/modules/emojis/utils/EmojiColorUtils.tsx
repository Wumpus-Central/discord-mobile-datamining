// === Module 7813: buildPlatformedThemedEmojiColorPalette ===

// Module 7813 (buildPlatformedThemedEmojiColorPalette)
import set from "set" /* 2 */;
import hexToRgba from "hexToRgba" /* 4294 */;
import hslToRgbDefault from "hslToRgb" /* 4295 */;
import getURLDefault from "getURL" /* 7474 */;
  ...
  if (null != colors) {
    ...
    if (colors.length >= 1) {
      ...
      if (null != parseStringResult) {
        ...
        const tmp2Result1 = tmp2(4294);
        const tmp8 = c3;
        obj = { foreground: null, background: null, ratio: 5, saturationFactor: null };
        tmp5Result = tmp5(689);
        const tmp2Result2 = tmp2(4294);
        const colorLightnessAdjusted1 = tmp2(4294).getColorLightnessAdjusted(parseStringResult, 0.6, false);
        ...
        const accessibleForegroundColor2 = tmp2(4294).getAccessibleForegroundColor(obj1);
        const tmp2Result4 = tmp2(4294);
        ...
        const accessibleForegroundColor3 = tmp2(4294).getAccessibleForegroundColor(obj2);
        let hexResult;
