...
import fromServer from "RawGuildEmojiStore.tsx";
import initialize from "top_emojis/TopEmojiStore.tsx";
import set from "EmojiConstants.tsx";
import { NULL_STRING_GUILD_ID } from "../../Constants.tsx";
import { EmojiCategories } from "../emoji_picker/EmojiPickerConstants.tsx";
import { UserSettingsTypes } from "../user_settings/UserSettingsConstants.tsx";
import importDefaultResult from "UnicodeEmojis.tsx";
import importDefaultResult2 from "../../lib/Frecency.tsx";
import importDefaultResult1 from "../../utils/SnowflakeUtils.tsx";
...
prototype["isUsable"] = function isUsable(emoji) {
  ...
  if (0 === emoji.roles.length) {
    ...
    if (tmp6) {
      ...
      let someResult = roles.some((item, index) => {
...
prototype["emojiIds"] = function emojiIds() {
  return importDefaultResult1.keys(this._emojiMap);
};
...
Object.defineProperty(prototype, "emoticons", {
  get: function emoticons() {
    ...
    if (null == this._emoticons) {
      ...
      self._emoticons = usableEmojis.filter((item, index) => !item.require_colons);
    }
...
let closure_25 = importDefaultResult1.fromTimestamp(timestamp - 60 * obj132Default.Millis.DAY);
let closure_26 = [];
let closure_27 = [];
const set1 = new Set();
  ...
  this.isFavoriteEmojiWithoutFetchingLatest = function isFavoriteEmojiWithoutFetchingLatest(id) {
    if (null == id) {
      ...
      if (null != id.id) {
        ...
        let result = importDefault(dependencyMap[18]).convertSurrogateToBase(id.surrogates);
        if (result == null) {
          result = id;
        }
  ...
  constructor(arg0) {
    ...
    item = arr2.forEach((item, index) => {
      ...
      if (item.uniqueName !== name) {
        const result = obj.set(item.uniqueName, 1);
      } else {
        const result1 = obj.set(name, num + 1);
      }
      const emojisByName = obj.emojisByName;
      const result2 = emojisByName.set(name, item);
      const disambiguatedEmoji = obj.disambiguatedEmoji;
      disambiguatedEmoji.push(item);
    });
    ...
    item1 = flattenedGuildIds.forEach((item, index) => {
      let tmp = item;
      if (item !== obj.guildId) {
        if (null == tmp) {
          tmp = NULL_STRING_GUILD_ID;
...
prototype2["getFrequentlyUsedEmojisWithoutFetchingLatest"] = function getFrequentlyUsedEmojisWithoutFetchingLatest() {
  ...
  if (null != this.frequentlyUsed) {
    ...
    const mapped = frequently.map((item, index) => {
      if (null != item.id) {
        let byId = self.getById(item.id);
      } else {
        byId = importDefault(dependencyMap[18]).getByName(item.name);
        obj = importDefault(dependencyMap[18]);
      }
      return byId;
    });
...
prototype2["rebuildFavoriteEmojisWithoutFetchingLatest"] = function rebuildFavoriteEmojisWithoutFetchingLatest() {
  ...
  const mapped = emojis.map((item, index) => {
    let byId = self.getById(item);
    if (byId == null) {
      byId = importDefault(dependencyMap[18]).getByName(item);
      obj = importDefault(dependencyMap[18]);
    }
    return byId;
  });
...
prototype2["getTopEmojiWithoutFetchingLatest"] = function getTopEmojiWithoutFetchingLatest(closure_0) {
  const self = this;
  if (null == this.topEmojis) {
    const value = map.get(closure_0);
    topEmojiIdsByGuildId = topEmojiIdsByGuildId.getTopEmojiIdsByGuildId(closure_0);
    if (null == value) {
    ...
    const mapped = emojiIds.map((item, index) => {
      let byId = self.getById(item);
      if (byId == null) {
        obj = items(self[18]);
        byId = obj.getByName(items(self[18]).convertSurrogateToName(item, false));
    ...
    const newlyAddedEmojiForGuild = self.getNewlyAddedEmojiForGuild(closure_0);
    closure_0 = newlyAddedEmojiForGuild.map((item, index) => item.id);
    self.topEmojis = items.filter((item, index) => !closure_0.includes(item.id));
  }
  return self.topEmojis;
};
prototype2["getNewlyAddedEmojiForGuild"] = function getNewlyAddedEmojiForGuild(closure_0) {
  if (null == this.newlyAddedEmoji) {
    return closure_26;
  } else {
    const newlyAddedEmoji = tmp.newlyAddedEmoji;
    let value = newlyAddedEmoji.get(closure_0);
    if (null == value) {
      value = closure_26;
    }
    return value;
...
prototype2["nameMatchesChain"] = function nameMatchesChain(matchComparator) {
  ...
  return applyDefault(this.getDisambiguatedEmoji()).filter((item, index) => {
    ({ names, name } = item);
    let someResult = null != names;
    if (someResult) {
      someResult = applyDefault.some(names, matchComparator);
    }
    let tmp5 = null != name;
    if (tmp5) {
    ...
    if (someResult1) {
      ...
      someResult1 = obj2.some(_modDef6734.getTermsForEmoji(name), matchComparator);
    }
    if (!someResult) {
      someResult = tmp5;
    }
...
let merged = Object.assign({
  ...
  lookupKey(emojiId) {
    let byName = require("UnicodeEmojis.tsx").getByName(emojiId);
    if (byName == null) {
          ...
          if (null != str) {
            ...
            if (null != id) {
              ...
              if (closure_0 === closure_1_18.REACTION) {
                let score = importDefaultResult3.getScore(id);
              } else {
                score = importDefaultResult2.getScore(id);
              }
...
prototype3["searchWithoutFetchingLatest"] = function searchWithoutFetchingLatest(channel) {
  ...
  const reduced = value.nameMatchesChain(matchComparator).reduce((acc, item, index) => {
    intention(flag[38]);
    obj = { emoji: item, channel, intention, forceIncludeExternalGuilds: flag };
    ...
    if (!tmp5) {
      let tmp6 = closure_3;
      if (closure_3) {
        tmp6 = item.type !== channel(flag[39]).EmojiTypes.UNICODE;
      }
      tmp5 = tmp6;
    }
    if (!tmp5) {
      const unlocked = acc.unlocked;
      unlocked.push(item);
    }
  }, { unlocked: [], locked: [] });
...
prototype3["getTopEmoji"] = function getTopEmoji(closure_0) {
  if (null == closure_0) {
    let topEmojiWithoutFetchingLatest = closure_26;
  } else {
    loadSavedEmojis();
    const value = EmojiDisambiguations.get(closure_0);
    topEmojiWithoutFetchingLatest = value.getTopEmojiWithoutFetchingLatest(closure_0);
  }
  return topEmojiWithoutFetchingLatest;
};
prototype3["getNewlyAddedEmoji"] = function getNewlyAddedEmoji(closure_0) {
  if (null == closure_0) {
    let newlyAddedEmojiForGuild = closure_26;
  } else {
    loadSavedEmojis();
    const value = EmojiDisambiguations.get(closure_0);
    newlyAddedEmojiForGuild = value.getNewlyAddedEmojiForGuild(closure_0);
  }
...
prototype3["hasUsableEmojiInAnyGuild"] = function hasUsableEmojiInAnyGuild() {
  loadSavedEmojis();
  const keys = importDefaultResult1.keys(closure_32);
  return keys.some((item, index) => table[item].usableEmojis.length > 0);
};
...
const emojiStore = new EmojiStore(dispatcherDefault, {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(unavailableGuilds) {
    ...
    if (0 === unavailableGuilds.unavailableGuilds.length) {
      ...
      if (guilds.every((item, index) => "full_sync" === item.emojis.op)) {
        Unloaded = obj.Loaded;
      }
      rebuildEmojis();
    }
  ...
  TOP_EMOJIS_FETCH_SUCCESS: function handleTopEmojisLoaded(topEmojisMetadata) {
    ...
    obj = { emojiIds: topEmojisMetadata.map((item, index) => item.emojiId), topEmojisTTL: null };
...
let result = require("obj132").fileFinishedImporting("modules/emojis/EmojiStore.tsx");
