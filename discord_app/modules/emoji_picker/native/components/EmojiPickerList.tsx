...
require = fn;
const IMAGE_SIZE = fn(10554).IMAGE_SIZE;
...
export default noop.memo(function EmojiPickerList(analyticsObject) {
  ...
  const trackOnEmojiPickerOpenedForReactions = emojiPickerListRef(guildId[13]).useTrackOnEmojiPickerOpenedForReactions({
  ...
  useTier0UpsellContent = tmp9Result.usePremiumUpsellConfig(
    emojiPickerListRef(guildId[16]).getUpsellType(tmp9(tmp2[17]).EntitlementFeatureNames.EMOJIS_EVERYWHERE),
  ...
  if (0 === tmp12Result.length) {
    ...
    if (!hasSearchUpsell) {
      let obj7 = {
        bottomSheetIndex,
        featureName: tmp9(tmp2[17]).EntitlementFeatureNames.EMOJIS_EVERYWHERE,
