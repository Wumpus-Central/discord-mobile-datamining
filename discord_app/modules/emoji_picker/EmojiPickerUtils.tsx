...
export const initializeSearch = function initializeSearch(location) {
  ...
  const EmojiPickerStore = require(9511) /* withEqualityFn */.EmojiPickerStore;
  ...
  importDefault(4450).trackWithMetadata(constants4.SEARCH_OPENED, { search_type: EMOJI, load_id: replaced, location: location.location });
};
export const useEmojiCategories = function useEmojiCategories(CHAT, channel, guildId) {
  ...
  const result = _require(9512).maybeFetchTopEmojisByGuild(tmp);
  ...
  ({ topEmojis, newlyAddedEmojis } = importDefault(9514)(tmp, CHAT));
  allEmojis = importDefault(9515)({ topEmojis, newlyAddedEmojis }).allEmojis;
  let tmp12 = importDefault(9514)(tmp, CHAT);
  const items3 = [c4];
  const items4 = [tmp];
  ...
  const isPremiumResult = _require(1897).isPremium(stateFromStores2);
  c11 = isPremiumResult;
  const obj8 = _require(1897);
  soundmojiEmojiPickerSectionExperiment = _require(9516).useSoundmojiEmojiPickerSectionExperiment({ location: "useEmojiCategories" });
...
export const getUnicodeEmojiCategories = function getUnicodeEmojiCategories() {
  const categories = importDefault(3896).getCategories();
