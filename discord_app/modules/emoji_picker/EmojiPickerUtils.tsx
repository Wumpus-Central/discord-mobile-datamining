...
export const initializeSearch = function initializeSearch(location) {
  ...
  const EmojiPickerStore = require(9352) /* withEqualityFn */.EmojiPickerStore;
  ...
  importDefault(4388).trackWithMetadata(constants4.SEARCH_OPENED, { search_type: EMOJI, load_id: replaced, location: location.location });
};
export const useEmojiCategories = function useEmojiCategories(CHAT, channel, guildId) {
  ...
  const result = _require(9353).maybeFetchTopEmojisByGuild(tmp);
  ...
  ({ topEmojis, newlyAddedEmojis } = importDefault(9355)(tmp, CHAT));
  allEmojis = importDefault(9356)({ topEmojis, newlyAddedEmojis }).allEmojis;
  let tmp12 = importDefault(9355)(tmp, CHAT);
  const items3 = [c4];
  const items4 = [tmp];
  ...
  soundmojiEmojiPickerSectionExperiment = _require(9357).useSoundmojiEmojiPickerSectionExperiment({ location: "useEmojiCategories" });
...
export const getUnicodeEmojiCategories = function getUnicodeEmojiCategories() {
  const categories = importDefault(3834).getCategories();
