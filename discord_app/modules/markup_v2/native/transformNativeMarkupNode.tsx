  ...
  switch (type.type) {
    ...
    case "quote":
      if (true === channelId.formatInline) {
        ...
        if (arr2.length <= 0) {
          const obj20 = { type: tmp55(5292).AST_KEY.TEXT, content: " " };
          const items1 = [obj20];
          arr2 = items1;
        }
        obj18.content = arr2;
    ...
    case "link":
      if ("mention" === type.value.type) {
        let result1 = require("transformNativeMarkupMentionLink").transformNativeMentionLink(
          type.value.value,
          channelId,
        );
        const obj3 = require("transformNativeMarkupMentionLink");
      } else {
        result1 = require("transformNativeMarkupLink").transformNativeLink(
          type.value,
          channelId,
          transformNativeInline,
        );
        if (result1 == null) {
          const obj24 = { type: tmp(5292).AST_KEY.TEXT, content: "" };
          result1 = obj24;
        }
...
const regExp = new RegExp(fn(4822).ANSI_CONTROL_SEQUENCE_RE, "g");
const size = fn(2);
