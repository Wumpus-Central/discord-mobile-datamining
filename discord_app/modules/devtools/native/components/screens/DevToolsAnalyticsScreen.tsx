...
function LoggedEvent(arg0) {
  ...
  let obj = {
    arrow: !tmp2,
    icon: closure_8(fingerprint(10473).AnalyticsIcon, {}),
    label: event,
    subLabel: null,
    onPress: null,
    start: null,
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [
    closure_8(trimmed(7273).TableSwitchRow, {
      icon: closure_8(trimmed(12244).ArrowsUpDownIcon, {}),
  ...
  items2[1] = closure_8(trimmed(5656).TableRow, {
    arrow: true,
    variant: "danger",
    icon: closure_8(trimmed(4564).TrashIcon, { color: "text-feedback-critical" }),
    label: "Clear Analytics Log",
    onPress: trimmed(15639).clearAnalyticsLog,
  });
  obj.children = items2;
  const items3 = [closure_9(trimmed(5738).TableRowGroup, obj), ,];
  const obj2 = {
    arrow: true,
    variant: "danger",
    icon: closure_8(trimmed(4564).TrashIcon, { color: "text-feedback-critical" }),
  ...
  items3[1] = closure_8(View, {
    ...
    children: closure_8(trimmed(7123).SearchField, { placeholder: "Search by event name", onChange: tmp8 }),
  });
  if (0 === loggedEvents.length) {
    let tmp11Result = closure_8(tmp2(5656).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = closure_8(tmp2(5656).TableRow, obj4);
    }
  }
  const obj5 = {
  ...
  items3[2] = closure_8(trimmed(5738).TableRowGroup, {
    title: "Analytics Events",
    hasIcons: false,
    children: tmp11Result,
  });
  obj6.children = items3;
  obj5.ListHeaderComponent = closure_9(trimmed(5024).Stack, obj6);
  ...
  obj.children = closure_8(trimmed(8964).FlashList, obj5);
