...
function LoggedEvent(arg0) {
  ...
  let obj = {
    arrow: !tmp2,
    icon: closure_8(fingerprint(10725).AnalyticsIcon, {}),
    label: event,
    subLabel: null,
    onPress: null,
    start: null,
  ...
  if (!tmp2) {
    ...
    const obj7 = {
      ...
      children: map(4414)(timestamp).calendar(),
    };
    obj6.children = closure_8(CopyIcon(4823).Text, obj7);
    let items1 = [closure_8(CommonProperty, obj6), ,];
    let tmp6Result = null != user;
    if (tmp6Result) {
      const obj8 = { name: "User ", children: null };
      const obj9 = { user };
      obj8.children = closure_8(tmp10(9980), obj9);
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [
    closure_8(trimmed(7531).TableSwitchRow, {
      icon: closure_8(trimmed(12484).ArrowsUpDownIcon, {}),
  ...
  items2[1] = closure_8(trimmed(5908).TableRow, {
    arrow: true,
    variant: "danger",
    icon: closure_8(trimmed(4784).TrashIcon, { color: "text-feedback-critical" }),
    label: "Clear Analytics Log",
    onPress: trimmed(15920).clearAnalyticsLog,
  });
  ...
  items3[1] = closure_8(View, {
    ...
    children: closure_8(trimmed(7381).SearchField, { placeholder: "Search by event name", onChange: tmp8 }),
  });
  if (0 === loggedEvents.length) {
    let tmp11Result = closure_8(tmp2(5908).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = closure_8(tmp2(5908).TableRow, obj7);
    }
  }
  const obj8 = {
  ...
  items3[2] = closure_8(trimmed(5990).TableRowGroup, {
    title: "Analytics Events",
    hasIcons: false,
    children: tmp11Result,
  });
  obj9.children = items3;
  obj8.ListHeaderComponent = closure_9(trimmed(5269).Stack, obj9);
