...
function LoggedEvent(arg0) {
  ...
  if (!tmp2) {
    ...
    const obj7 = {
      ...
      children: map(4421)(timestamp).calendar(),
    };
    obj6.children = closure_8(CopyIcon(4832).Text, obj7);
    let items1 = [closure_8(CommonProperty, obj6), ,];
    let tmp6Result = null != user;
    if (tmp6Result) {
      const obj8 = { name: "User ", children: null };
      const obj9 = { user };
      obj8.children = closure_8(tmp10(9094), obj9);
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [
    closure_8(trimmed(6621).TableSwitchRow, {
      icon: closure_8(trimmed(11633).ArrowsUpDownIcon, {}),
  ...
  items2[1] = closure_8(trimmed(5917).TableRow, {
    arrow: true,
    variant: "danger",
  ...
  items3[1] = closure_8(View, {
    ...
    children: closure_8(trimmed(6471).SearchField, { placeholder: "Search by event name", onChange: tmp8 }),
  });
  if (0 === loggedEvents.length) {
    ...
    if (0 === found.length) {
      ...
      tmp11Result = closure_8(tmp2(5917).TableRow, obj7);
    }
  }
  const obj8 = {
  ...
  items3[2] = closure_8(trimmed(5999).TableRowGroup, {
    title: "Analytics Events",
    hasIcons: false,
    children: tmp11Result,
  });
  obj9.children = items3;
  obj8.ListHeaderComponent = closure_9(trimmed(5279).Stack, obj9);
