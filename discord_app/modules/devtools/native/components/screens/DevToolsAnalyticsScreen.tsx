...
function LoggedEvent(arg0) {
  ...
  obj[1] = callback2(fingerprint(9840).AnalyticsIcon, {});
  obj[2] = event;
  ...
  obj[5] = start;
  obj[6] = end;
  let items = [callback2(fingerprint(5300).TableRow, obj), ];
  if (!tmp2) {
    ...
    if (null != fingerprint) {
      ...
      items3[1] = tmp6Result;
      obj6[2] = items3;
      let tmp6Result1 = tmp4(CopyIcon(4797).PressableOpacity, obj6);
    } else {
...
export default function DevToolsAnalyticsScreen() {
  ...
  obj1[0] = callback2(trimmed(11423).ArrowsUpDownIcon, {});
  obj1[2] = first;
  obj1[3] = tmp5[1];
  const items2 = [callback2(trimmed(5635).TableSwitchRow, obj1), ];
  ...
  obj2[4] = trimmed(13725).clearAnalyticsLog;
  items2[1] = callback2(trimmed(5300).TableRow, obj2);
  obj[2] = items2;
  const items3 = [callback3(trimmed(5634).TableRowGroup, obj), , ];
  const str2 = str.toLowerCase();
  const tmp12 = View;
  const tmp13 = callback3;
  items3[1] = callback2(View, { style: tmp.searchFieldContainer, children: callback2(trimmed(5886).SearchField, { placeholder: "Search by event name", onChange: tmp8 }) });
  if (0 === loggedEvents.length) {
    let tmp11Result = tmp11(tmp2(5300).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5300).TableRow, obj4);
    }
  }
  ...
  items3[2] = callback2(trimmed(5634).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj6[1] = items3;
  obj5[0] = tmp13(trimmed(4663).Stack, obj6);
  ...
  obj[1] = callback2(trimmed(7638).FlashList, obj5);
