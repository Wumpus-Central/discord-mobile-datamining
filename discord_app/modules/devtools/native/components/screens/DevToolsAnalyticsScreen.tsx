...
function LoggedEvent(arg0) {
  ...
  let obj = {
    arrow: !tmp2,
    icon: closure_8(fingerprint(10532).AnalyticsIcon, {}),
    label: event,
    subLabel: null,
    onPress: null,
    start: null,
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [
    closure_8(trimmed(7314).TableSwitchRow, {
      icon: closure_8(trimmed(12302).ArrowsUpDownIcon, {}),
  ...
  items3[1] = closure_8(View, {
    ...
    children: closure_8(trimmed(7164).SearchField, { placeholder: "Search by event name", onChange: tmp8 }),
  });
  if (0 === loggedEvents.length) {
    let tmp11Result = closure_8(tmp2(5693).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = closure_8(tmp2(5693).TableRow, obj7);
    }
  }
  const obj8 = {
  ...
  items3[2] = closure_8(trimmed(5775).TableRowGroup, {
    title: "Analytics Events",
    hasIcons: false,
    children: tmp11Result,
  });
  obj9.children = items3;
  obj8.ListHeaderComponent = closure_9(trimmed(5060).Stack, obj9);
