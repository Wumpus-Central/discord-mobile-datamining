...
function CommonProperty(arg0) {
  ...
  const items = [callback2(require(4189) /* Text */.Text, { variant: "text-sm/semibold", color: "text-default", children: name }), children];
  obj[1] = items;
...
function LoggedEvent(arg0) {
  ...
  obj[1] = callback2(fingerprint(9628).AnalyticsIcon, {});
  obj[2] = event;
  ...
  obj[5] = start;
  obj[6] = end;
  let items = [callback2(fingerprint(5224).TableRow, obj), ];
  if (!tmp2) {
    ...
    let obj4 = map(3775)(timestamp);
    obj3[2] = obj4.calendar();
    obj2[1] = tmp6(CopyIcon(4189).Text, obj3);
    ...
    if (null != fingerprint) {
      ...
      obj7[2] = items2;
      obj7[3] = fingerprint;
      const items3 = [tmp6(CopyIcon(4189).Text, obj7), ];
      CopyIcon = CopyIcon(4141).CopyIcon;
      tmp6Result = tmp6(CopyIcon, { size: "sm" });
      items3[1] = tmp6Result;
      obj6[2] = items3;
      let tmp6Result1 = tmp4(CopyIcon(4721).PressableOpacity, obj6);
    } else {
      const obj8 = { variant: "text-sm/medium", color: "text-muted", style: null, children: "null" };
      obj8[2] = map.monospace;
      tmp6Result1 = tmp6(CopyIcon(4189).Text, obj8);
    }
...
export default function DevToolsAnalyticsScreen() {
  ...
  obj1[0] = callback2(trimmed(11342).ArrowsUpDownIcon, {});
  obj1[2] = first;
  obj1[3] = tmp5[1];
  const items2 = [callback2(trimmed(5559).TableSwitchRow, obj1), ];
  const obj2 = { arrow: true, variant: "danger", icon: null, label: "Clear Analytics Log", onPress: null };
  obj2[2] = callback2(trimmed(4152).TrashIcon, { color: "text-feedback-critical" });
  obj2[4] = trimmed(13657).clearAnalyticsLog;
  items2[1] = callback2(trimmed(5224).TableRow, obj2);
  obj[2] = items2;
  const items3 = [callback3(trimmed(5558).TableRowGroup, obj), , ];
  const str2 = str.toLowerCase();
  const tmp12 = View;
  const tmp13 = callback3;
  items3[1] = callback2(View, { style: tmp.searchFieldContainer, children: callback2(trimmed(9248).SearchField, { placeholder: "Search by event name", onChange: tmp8 }) });
  if (0 === loggedEvents.length) {
    let tmp11Result = tmp11(tmp2(5224).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5224).TableRow, obj4);
    }
  }
  ...
  items3[2] = callback2(trimmed(5558).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj6[1] = items3;
  obj5[0] = tmp13(trimmed(4602).Stack, obj6);
  ...
  obj[1] = callback2(trimmed(6679).FlashList, obj5);
