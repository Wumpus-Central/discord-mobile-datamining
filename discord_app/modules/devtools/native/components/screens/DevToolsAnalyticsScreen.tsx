...
function LoggedEvent(arg0) {
  ...
  obj[5] = start;
  obj[6] = end;
  let items = [callback2(fingerprint(5374).TableRow, obj), ];
  if (!tmp2) {
    ...
    if (null != fingerprint) {
      ...
      obj7[2] = items2;
      obj7[3] = fingerprint;
      const items3 = [tmp6(CopyIcon(4299).Text, obj7), ];
      CopyIcon = CopyIcon(4251).CopyIcon;
      tmp6Result = tmp6(CopyIcon, { size: "sm" });
      items3[1] = tmp6Result;
      obj6[2] = items3;
      let tmp6Result1 = tmp4(CopyIcon(4846).PressableOpacity, obj6);
    } else {
      const obj8 = { variant: "text-sm/medium", color: "text-muted", style: null, children: "null" };
      obj8[2] = map.monospace;
      tmp6Result1 = tmp6(CopyIcon(4299).Text, obj8);
    }
...
export default function DevToolsAnalyticsScreen() {
  ...
  if (0 === loggedEvents.length) {
    ...
    if (0 === found.length) {
      ...
      tmp11Result = tmp11(tmp2(5374).TableRow, obj4);
    }
  }
  ...
  items3[2] = callback2(trimmed(5767).TableRowGroup, { title: "Analytics Events", hasIcons: false, children: tmp11Result });
  obj6[1] = items3;
  obj5[0] = tmp13(trimmed(4712).Stack, obj6);
