...
function LoggedEvent(arg0) {
  ...
  let obj = {
    arrow: !tmp2,
    icon: closure_8(fingerprint(10642).AnalyticsIcon, {}),
    label: event,
    subLabel: null,
    onPress: null,
    start: null,
  ...
  if (!tmp2) {
    ...
    if (tmp6Result) {
      ...
      const obj9 = { user };
      obj8.children = closure_8(tmp10(9900), obj9);
    ...
    if (null != fingerprint) {
      ...
      let tmp6Result3 = closure_9(CopyIcon(5334).PressableOpacity, obj10);
    } else {
...
export default function DevToolsAnalyticsScreen() {
  ...
  const items2 = [
    closure_8(trimmed(7440).TableSwitchRow, {
      icon: closure_8(trimmed(12402).ArrowsUpDownIcon, {}),
  ...
  items3[1] = closure_8(View, {
    ...
    children: closure_8(trimmed(7290).SearchField, { placeholder: "Search by event name", onChange: tmp8 }),
  });
  if (0 === loggedEvents.length) {
    let tmp11Result = closure_8(tmp2(5817).TableRow, { label: "No events logged." });
  } else {
    tmp11Result = null;
    if (0 === found.length) {
      ...
      tmp11Result = closure_8(tmp2(5817).TableRow, obj7);
    }
  }
  const obj8 = {
  ...
  items3[2] = closure_8(trimmed(5899).TableRowGroup, {
