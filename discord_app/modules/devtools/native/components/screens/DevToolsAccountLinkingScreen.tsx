...
export default function DevToolsAccountLinkingScreen() {
  ...
  let found = stateFromStores1(5601)(gameApplicationIds).filter((arg0) => null != arg0);
  let tmp8Result = tmp8(5601);
  ...
  const tmp11 = stateFromStores1(5598)(getOrFetchApplication, { debug: true });
  ({ startAuthorization: Button, hasAlreadyLinked, debug, connectionApp } = tmp11);
  id = undefined;
  if (connectionApp != null) {
    id = connectionApp.id;
  }
  const arr4 = stateFromStores1(5601)(gameApplicationIds);
  ...
  const callback = obj.useCallback(() => {
    if (null != stateFromStores1) {
      stateFromStores1(5603).delete(tmp.id);
      const obj = stateFromStores1(5603);
    }
  }, items4);
  ...
  if (null != stateFromStores) {
    if (found.length > 0) {
      let mapped1 = found.map((name) => {
        ...
        obj[2] = tmpResult;
        return outer1_12(id(5300).TableRow, obj, name.id);
      });
    } else {
      mapped1 = tmp18(tmp8(5300).TableRow, { label: "No official games" });
    }
  } else {
    obj1[2] = tmp18(tmp8(5300).TableRow, { label: "No guild selected" });
    const items6 = [tmp18(tmp8(5634).TableRowGroup, obj1), , , ];
    ...
    obj4[1] = first;
    obj4[2] = tmp7;
    obj2[1] = tmp18(tmp8(7685).TextInput, obj4);
    ...
    obj6[0] = "Name: " + str;
    items7[1] = tmp18(tmp8(5300).TableRow, obj6);
    const obj7 = { label: null };
    const _HermesInternal2 = HermesInternal;
    obj7[0] = "Linked Games: " + str2;
    items7[2] = tmp18(tmp8(5300).TableRow, obj7);
    obj5[2] = items7;
    items6[1] = tmp16(tmp8(5634).TableRowGroup, obj5);
    ...
    const items8 = [tmp18(tmp8(5300).TableRow, obj9), , ];
    let str8 = "text-muted";
    if (hasAlreadyLinked) {
    ...
    items8[1] = tmp18(tmp8(5300).TableRow, obj12);
    ...
    const items9 = [tmp18(tmp8(4665).Button, obj14), ];
    const obj15 = { disabled: null, onPress: null, variant: "critical-primary", text: "Deauthorize" };
    obj15[0] = !tmp15;
    obj15[1] = callback;
    items9[1] = tmp18(tmp8(4665).Button, obj15);
    obj13[1] = items9;
    items8[2] = tmp16(closure_7, obj13);
    obj11[2] = items8;
    items6[2] = tmp16(tmp8(5634).TableRowGroup, obj11);
    let prop;
    if (connectionApp != null) {
    ...
    if (tmp16Result) {
      ...
      items10[1] = tmp18(tmp8(5300).TableRow, obj21);
      obj20[2] = items10;
      tmp16Result = tmp16(tmp8(5634).TableRowGroup, obj20);
    }
