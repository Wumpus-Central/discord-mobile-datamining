// === Module 16103: DevToolsAccountLinkingScreen ===

// Module 16103 (DevToolsAccountLinkingScreen)
import nativeDefault from "native" /* 576 */;
import useSafeAreaInsetsDefault from "useSafeAreaInsets" /* 1612 */;
import useStartAuthorizeDefault from "useStartAuthorize" /* 7496 */;
...
export default function DevToolsAccountLinkingScreen() {
  ...
  getOrFetchApplication = value(7499).useGetOrFetchApplication(value);
  const tmp8Result = value(7499);
  ...
  if (null != stateFromStores) {
    if (found.length > 0) {
      let mapped1 = found.map((name) => {
        ...
        let tmpResult;
        if (name === name.id) {
          tmpResult = closure_1_12(first(dependencyMap[17]).Text, { variant: "text-sm/semibold", children: "Selected" });
        }
        obj.trailing = tmpResult;
        return closure_1_12(first(dependencyMap[16]).TableRow, obj, name.id);
      });
    } else {
      mapped1 = closure_12(tmp8(5908).TableRow, { label: "No official games" });
    }
  } else {
    obj6.children = closure_12(tmp8(5908).TableRow, { label: "No guild selected" });
    const items6 = [closure_12(tmp8(5990).TableRowGroup, obj6), , , ];
    const obj7 = { style: null, children: null };
    const obj8 = { padding: tmp2(576).space.PX_12 };
    obj7.style = obj8;
    const obj9 = { label: "Application ID", value, onChange: tmp7 };
    obj7.children = closure_12(tmp8(6934).TextInput, obj9);
    ...
    items7[1] = closure_12(tmp8(5908).TableRow, obj11);
    const obj12 = { label: null };
    const _HermesInternal2 = HermesInternal;
    obj12.label = "Linked Games: " + str2;
    items7[2] = closure_12(tmp8(5908).TableRow, obj12);
    obj10.children = items7;
    items6[1] = closure_13(tmp8(5990).TableRowGroup, obj10);
    ...
    obj13.children = str7;
    obj14.trailing = closure_12(tmp8(4823).Text, obj13);
    const items8 = [closure_12(tmp8(5908).TableRow, obj14), , ];
    let str8 = "text-muted";
    if (hasAlreadyLinked) {
    ...
    obj15.children = str9;
    obj17.trailing = closure_12(tmp8(4823).Text, obj15);
    items8[1] = closure_12(tmp8(5908).TableRow, obj17);
    ...
    if (tmp16Result) {
      ...
      items10[1] = closure_12(tmp8(5908).TableRow, obj26);
      obj25.children = items10;
      tmp16Result = closure_13(tmp8(5990).TableRowGroup, obj25);
    }
