// === Module 15196: DevToolsQuickActionsScreen ===
...
import OverridePremiumTypeActions from "OverridePremiumTypeActions" /* 15214 */;
...
function showVibingWumpus() {
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(10933, dependencyMap.paths), {
    onClose() {
...
export default function DevToolsQuickActionsScreen() {
  ...
  if (tmp9) {
    ...
    const obj12 = { label: str, disabled: usingSystemTheme, icon: closure_21(locale(14788).ThemeLightIcon, {}), value: locale(4682).isThemeLight(stateFromStoresObject.theme), onValueChange: handleThemeChange };
    const items6 = [closure_21(locale(6616).TableSwitchRow, obj12), ];
    const obj13 = { label: "Reduced Motion", icon: closure_21(locale(14848).AccessibilityIcon, {}), value: stateFromStores, onValueChange: handleReducedMotionChange };
    ...
    const obj15 = {
      ...
      children: closure_17.map((item) => {
          ({ label, value } = item);
          locale = value;
          return closure_1_21(locale(6616).TableSwitchRow, {
            onValueChange(arg0) {
    ...
    const obj18 = { label: str2, subLabel: "Toggle to a non-english locale for change log testing, etc.", onPress: tmp12, icon: closure_21(locale(14944).LanguageIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
    const items7 = [closure_21(locale(5912).TableRow, obj18), , , , , , , , , , ];
    const obj19 = { label: "Reset Double Tap Emoji State", subLabel: "Clears double tap emoji and resets dismissible content.", onPress: handleResetDoubleTapState, icon: closure_21(locale(6372).KeyIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
    ...
    const obj21 = { label: "Launch MFA Challenge Modal", subLabel: "Dismisses dev tools when launching.", onPress: launchMFA, icon: closure_21(locale(6372).KeyIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
    items7[3] = closure_21(locale(5912).TableRow, obj21);
    const obj22 = { label: "Show TOTP Setup Success", subLabel: "Dismisses dev tools when launching.", onPress: launchTotpSetupSuccess, icon: closure_21(locale(6372).KeyIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
    items7[4] = closure_21(locale(5912).TableRow, obj22);
    const obj23 = { label: "Launch Vibing Wumpus", subLabel: "Vibe with the one and only", onPress: showVibingWumpus, icon: closure_21(locale(6372).KeyIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
    ...
    const obj26 = { label: "Test captcha", onPress: locale(15256).showCaptchaTestModal, icon: closure_21(locale(6372).KeyIcon, {}), trailing: closure_21(locale(5919).TableRowArrow, {}) };
