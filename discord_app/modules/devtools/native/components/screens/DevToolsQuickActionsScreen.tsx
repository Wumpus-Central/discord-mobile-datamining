...
export default function DevToolsQuickActionsScreen() {
  ...
  if (tmp9) {
    ...
    const obj12 = { label: str, disabled: usingSystemTheme, icon: closure_21(locale(15384).ThemeLightIcon, {}), value: locale(4491).isThemeLight(stateFromStoresObject.theme), onValueChange: handleThemeChange };
    const items6 = [closure_21(locale(7314).TableSwitchRow, obj12), ];
    const obj13 = { label: "Reduced Motion", icon: closure_21(locale(15444).AccessibilityIcon, {}), value: stateFromStores, onValueChange: handleReducedMotionChange };
    ...
    const obj18 = { label: str2, subLabel: "Toggle to a non-english locale for change log testing, etc.", onPress: tmp12, icon: closure_21(locale(15538).LanguageIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
    const items7 = [closure_21(locale(5693).TableRow, obj18), , , , , , , , , ];
    const obj19 = { label: "Reset Double Tap Emoji State", subLabel: "Clears double tap emoji and resets dismissible content.", onPress: handleResetDoubleTapState, icon: closure_21(locale(7070).KeyIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
    ...
    const obj21 = { label: "Launch MFA Challenge Modal", subLabel: "Dismisses dev tools when launching.", onPress: launchMFA, icon: closure_21(locale(7070).KeyIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
    items7[3] = closure_21(locale(5693).TableRow, obj21);
    const obj22 = { label: "Show TOTP Setup Success", subLabel: "Dismisses dev tools when launching.", onPress: launchTotpSetupSuccess, icon: closure_21(locale(7070).KeyIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
    items7[4] = closure_21(locale(5693).TableRow, obj22);
    const obj23 = { label: "Launch Vibing Wumpus", subLabel: "Vibe with the one and only", onPress: showVibingWumpus, icon: closure_21(locale(7070).KeyIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
    ...
    const obj25 = { label: "Test captcha", onPress: locale(15819).showCaptchaTestModal, icon: closure_21(locale(7070).KeyIcon, {}), trailing: closure_21(locale(5700).TableRowArrow, {}) };
