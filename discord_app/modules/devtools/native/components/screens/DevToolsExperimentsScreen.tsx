...
function UserExperimentDebugView(id) {
  ...
  const mapped = obj3.sortBy(ExperimentStore.getRecentExposures(constants2.USER, id.id), (arg0) => {
    [, tmp] = arg0;
    return -tmp;
  }).map((item) => {
    [tmp, tmp2] = item;
...
function GuildExperimentDebugView(arg0) {
  ...
  let obj = _modDef12;
  const tmp = closure_13();
  let mapped = obj.sortBy(ExperimentStore.getRecentExposures(constants2.GUILD, id), (arg0) => {
    [, tmp] = arg0;
    return -tmp;
  }).map((item) => {
    [tmp, tmp2] = item;
  ...
  obj1.subLabel = str;
  const items1 = [closure_11(obj(5768).TableRowGroup, { title: "Overview", hasIcons: false, children: closure_11(obj(5686).TableRow, obj1) }), closure_11(obj(1178).Spacer, { size: 16 }), , , , , , , ];
  obj3 = { title: "Guild Assignments", hasIcons: false, children: null };
  const obj2 = { title: "Overview", hasIcons: false, children: closure_11(obj(5686).TableRow, obj1) };
  obj3.children = closure_11(obj(5686).TableRow, { label: items.join("\n") });
  items1[2] = closure_11(obj(5768).TableRowGroup, obj3);
  ...
  items1[4] = closure_11(obj(5768).TableRowGroup, { title: "Server Descriptor", hasIcons: false, children: closure_11(obj(5686).TableRow, { label: str3 }) });
  items1[5] = closure_11(obj(1178).Spacer, { size: 16 });
  if (null != override) {
    const _JSON2 = JSON;
    str2 = JSON.stringify(override, undefined, 2);
  }
  const obj5 = { title: "Server Descriptor", hasIcons: false, children: closure_11(obj(5686).TableRow, { label: str3 }) };
  items1[6] = closure_11(obj(5768).TableRowGroup, { title: "Override Descriptor", hasIcons: false, children: closure_11(obj(5686).TableRow, { label: str2 }) });
  items1[7] = closure_11(obj(1178).Spacer, { size: 16 });
  if (0 === mapped.length) {
    mapped = closure_11(tmp20(5686).TableRow, { label: "none" });
  } else {
  ...
  items1[8] = closure_11(obj(5768).TableRowGroup, { title: "Recent Exposures", hasIcons: false, children: mapped });
  obj.children = items1;
  ...
  constructor(arg0) {
    ...
    callback = closure_4.useCallback(() => {
      ClipboardUtils.copy(uRLForExperiment, () => {
        uRLForExperiment(4335);
        const obj = { key: "experiment-link-copied", content: "Copied experiment link", IconComponent: onCopyLink(4596).CircleCheckIcon, iconColor: "status-positive" };
        obj.open(obj);
        if (closure_1_0 != null) {
          closure_1_0();
        }
    ...
    obj4 = { title: "Share", hasIcons: false, children: jsx(onCopyLink(closure_2[23]).TableRow, { label: "Copy Link", subLabel: uRLForExperiment, onPress: callback }) };
...
export default noop.memo(() => {
  ...
  if (obj6.isEmpty(memo)) {
    obj = { Illustration: tmp5(tmp6[17]).NoResults, title: "No Experiments", body: "No experiments are currently running." };
    ...
    obj3 = { style: tmp.listContainer, sections: memo2, estimatedListSize: "windowSize", itemSize: tmp12, insetEnd: overridesInfo(experiments2[12])({ includeKeyboardHeight: true }).insets.bottom + tmp11(tmp6[9]).space.PX_16, renderItem: callback };
