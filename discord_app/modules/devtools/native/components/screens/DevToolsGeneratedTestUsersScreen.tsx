...
import closure_8 from "handleAddUser" /* 15416 */;
...
function PoolIdInput(onSubmit) {
  ...
  const items1 = [callback3(onSubmit(7714).TextInput, { size: "md", placeholder: "Enter Pool ID", onChange: tmp2[1], autoCapitalize: "none", autoCorrect: false, autoComplete: "off", isClearable: true }), callback3(onSubmit(7714).TextInput, { size: "md", secureTextEntry: true, placeholder: "Enter Password", onChange: tmp4[1], autoCapitalize: "none", autoCorrect: false, autoComplete: "off", isClearable: true }), ];
...
function PoolUsers(pool) {
  ...
  return callback3(pool(5607).TableRow, { icon: callback3(items[Number(undefined, id) % items.length], { size: "md", color: length[Number(undefined, id) % length.length] }), label: pool.summary, subLabel: "" + usersForPool.length + " users", arrow: true, onPress: callback, start, end });
}
...
export default function DevToolsGeneratedTestUsersScreen() {
  ...
  if (tmp5Result) {
    tmp5Result = tmp5(_require(5607).TableRow, { label: "No pools available." });
  }
  ...
  obj3[2] = items3;
  items2[1] = closure_11(_require(6000).TableRowGroup, obj3);
  obj2[1] = items2;
  obj[1] = closure_11(_require(4926).Stack, obj2);
