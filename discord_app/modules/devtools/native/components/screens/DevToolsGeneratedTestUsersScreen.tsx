...
function PoolIdInput(onSubmit) {
  ...
  const items1 = [callback3(onSubmit(7685).TextInput, { size: "md", placeholder: "Enter Pool ID", onChange: tmp2[1], autoCapitalize: "none", autoCorrect: false, autoComplete: "off", isClearable: true }), callback3(onSubmit(7685).TextInput, { size: "md", secureTextEntry: true, placeholder: "Enter Password", onChange: tmp4[1], autoCapitalize: "none", autoCorrect: false, autoComplete: "off", isClearable: true }), ];
  ...
  items1[2] = callback3(onSubmit(4665).Button, { size: "md", variant: "primary", text: "Get Pool", disabled: tmp11, loading: tmp7, onPress: callback });
  obj[2] = items1;
  return closure_11(onSubmit(4663).Stack, obj);
}
function UserActionSheet(pool) {
  ...
  fetchFingerprint = React.useCallback((arg0) => {
    ...
    pool(13759).loginAsGeneratedUser(pool.id, arg0);
  }, items);
  let obj = { header: null, children: null };
  obj = { title: pool.summary, subtitle: null };
  obj[1] = "" + usersForPool.length + " users";
  obj[0] = callback3(pool(5322).BottomSheetTitleHeader, obj);
  ...
  obj2[2] = usersForPool.map((id) => {
    ...
    obj[0] = outer1_10(pool(11108).UserIcon, { size: "md" });
    ...
    return outer1_10(pool(5300).TableRow, obj, id.id);
  });
  obj[1] = callback3(pool(5634).TableRowGroup, obj2);
  obj[1] = callback3(closure_6, obj);
  return callback3(pool(5323).BottomSheet, obj);
}
function PoolUsers(pool) {
  ...
  obj[5] = start;
  obj[6] = end;
  return callback3(pool(5300).TableRow, obj);
}
...
export default function DevToolsGeneratedTestUsersScreen() {
  ...
  obj1[0] = Themes.space.PX_16 + useSafeAreaInsetsKeyboardAware({ includeKeyboardHeight: true }).insets.bottom;
  items1[1] = obj1;
  obj[0] = items1;
  const items2 = [callback3(PoolIdInput, { onSubmit: callback }), ];
  let tmp5Result = 0 === stateFromStoresArray.length;
  if (tmp5Result) {
    tmp5Result = tmp5(_TableRowInner.TableRow, { label: "No pools available." });
  }
  ...
  obj3[2] = items3;
  items2[1] = closure_11(_TableRowGroupTitle.TableRowGroup, obj3);
  obj2[1] = items2;
  obj[1] = closure_11(_Stack.Stack, obj2);
