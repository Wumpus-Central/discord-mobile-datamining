...
export default function FileUploadActionComponent(maxValues) {
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    if (mediaFilesAllowed) {
      ...
      obj = {
        ...
        onPressItem(item) {
          ...
          if (item.isIncluded) {
            const found = currentUploads.find((item) =>
              InteractionModal(5186).doesImageMatchUpload(item.node.image, item),
            );
            if (null != found) {
              callback1(found.id);
            }
        ...
        onManageLimited() {
          InteractionModal(10705);
          obj = { onDismissKeyboard: InteractionModal(10706).hideMediaKeyboardActionSheet, onRestoreKeyboard };
  ...
  if (uploadIds.length >= maxValues) {
    ...
    let obj1 = {
      ...
      maxValues,
      types: fileTypeFiltering.typesFormattedString,
