...
let obj2 = {
  ...
  emojiWrapper: { flexShrink: 0, borderRadius: nativeDefault.radii.xs, overflow: "hidden" },
  textEmoji: { fontSize: 16, color: "#000000" },
  fastImageEmoji: { width: 24, height: 24 },
};
...
export default function StringSelectComponentActionSheet(selectionActionComponent) {
  ...
  let tmp3 = first(
    noop.useState(
      new Set(selectionActionComponent(8267).getInitialStringSelectOptions(selectionActionComponent, containerId)),
    ),
    2,
  );
  first = tmp3[0];
  ...
  let set = new Set(
    selectionActionComponent(8267).getInitialStringSelectOptions(selectionActionComponent, containerId),
  );
  const tmp9 = memo;
