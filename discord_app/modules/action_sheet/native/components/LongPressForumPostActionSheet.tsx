// === Module 9671: LongPressForumPostActionSheet ===
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 9672 */;
...
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 10426 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(9698).useCanMarkChannelUnread(thread);
  const obj5 = thread(9698);
  ...
  const firstMessage = thread(6717).useFirstForumPostMessage(thread).firstMessage;
  const obj7 = thread(6717);
  const isThreadModerator = thread(6682).useIsThreadModerator(parentChannel);
  const obj8 = thread(6682);
  const canManageThread = thread(6682).useCanManageThread(thread);
  const obj9 = thread(6682);
  const canUnarchiveThread = thread(6682).useCanUnarchiveThread(thread);
  const obj10 = thread(6682);
  const existingPin = thread(7305).useExistingPin(thread);
  const obj11 = thread(7305);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5891).GuildIconSizes.LARGE };
    let tmp21 = jsx(GuildIconDefault, { guild: stateFromStores, size: tmp(5891).GuildIconSizes.LARGE });
    let tmp20 = jsx;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(9696).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(9056).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(9700).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5402).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(9702).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(6793).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(10807).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(10805, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(10405).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10081).IdIcon;
            ...
            obj32.header = tmp20(tmp(10453).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(6615).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(6615).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(6613).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9056).BellIcon;
          ...
          obj36.IconComponent = tmp(9602).BellSlashIcon;
          obj36.onPress = function onPress() {
        ...
        obj37.IconComponent = tmp(10413).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(6384).EyeIcon;
