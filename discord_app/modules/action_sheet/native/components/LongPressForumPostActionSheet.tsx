// === Module 10485: LongPressForumPostActionSheet ===
...
import buildFavoritesSectionButtonsDefault from "buildFavoritesSectionButtons" /* 10486 */;
import markChannelUnreadDefault from "markChannelUnread" /* 10510 */;
import ForumComposerModalActionCreators from "ForumComposerModalActionCreators" /* 10515 */;
import useFavoritesGuildChannelActionsDefault from "useFavoritesGuildChannelActions" /* 11246 */;
...
export default function ForumPostLongPressActionSheet(thread) {
  ...
  const canMarkChannelUnread = thread(10510).useCanMarkChannelUnread(thread);
  const obj5 = thread(10510);
  ...
  const existingPin = thread(8132).useExistingPin(thread);
  const obj11 = thread(8132);
  ...
  if (null != stateFromStores) {
    const obj14 = { guild: stateFromStores, size: tmp(5803).GuildIconSizes.LARGE };
    let tmp21 = jsx(GuildIconDefault, { guild: stateFromStores, size: tmp(5803).GuildIconSizes.LARGE });
    let tmp20 = jsx;
  ...
  const MarkChannelUnreadExperiment = tmp(10507).MarkChannelUnreadExperiment;
  if (MarkChannelUnreadExperiment.getConfig({ location: "forum_post_action_sheet" }).enabled) {
    if (!stateFromStores3) {
      if (canMarkChannelUnread) {
        ...
        obj18.IconComponent = tmp(10508).ChatMarkUnreadIcon;
      ...
      if (!stateFromStores4) {
        ...
        if (stateFromStores1) {
          ...
          obj20.IconComponent = tmp(4697).UserMinusIcon;
          ...
          obj20.IconComponent = tmp(9880).BellIcon;
          obj20.onPress = function onPress() {
      ...
      if (archived) {
        if (canUnarchiveThread) {
          ...
          obj21.IconComponent = tmp(4719).ClockIcon;
        ...
        obj22.IconComponent = tmp(4709).XLargeIcon;
      ...
      if (canManageThread) {
        ...
        if (locked) {
          ...
          obj23.IconComponent = tmp(10512).LockUnlockedIcon;
          ...
          obj23.IconComponent = tmp(5315).LockIcon;
          obj23.onPress = function onPress() {
      ...
      if (tmp38) {
        ...
        obj24.IconComponent = tmp(10514).PencilIcon;
      ...
      if (canManageThread) {
        ...
        obj25.IconComponent = tmp(7625).SettingsIcon;
        ...
        if (parentChannel.availableTags.length > 0) {
          ...
          obj26.IconComponent = tmp(11520).TagsIcon;
          obj26.onPress = function onPress() {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11518, dependencyMap.paths), "ForumPostTagsActionSheet", { thread, parentChannel, canManageThread });
          };
          buttons7.push(obj26);
        }
      }
      ...
      obj27.IconComponent = tmp(4699).LinkIcon;
      ...
      if (stateFromStores4) {
        ...
        if (isThreadModerator) {
          ...
          if (hasFlagResult) {
            ...
            obj29.IconComponent = tmp(11225).PinIcon;
        ...
        if (!isThreadModerator) {
          if (!tmp24) {
            ...
            if (setting) {
              ...
              obj31.IconComponent = tmp(10895).IdIcon;
            ...
            obj32.header = tmp20(tmp(11273).ActionSheetIconHeader, obj33);
            obj32.children = items7.map((buttons) => {
              ...
              return jsx(ActionSheetRow.ActionSheetRow.Group, {
                ...
                children: buttons.map((item, index) => {
                  ...
                  return closure_1_14(thread(7446).ActionSheetRow, {
                    variant: str,
                    icon: closure_1_14(thread(7446).ActionSheetRow.Icon, { IconComponent, disableColor }),
                    label,
                    trailing,
                    onPress() {
                      closure_1_0();
            ...
            return tmp20(tmp(7444).ActionSheet, obj32);
          }
        }
        if (tmp24) {
        ...
        const obj34 = {
          label: string5Result,
          IconComponent: tmp(4714).TrashIcon,
          onPress() {
                  const intl = util.intl;
        ...
        if (stateFromStores2) {
          ...
          obj36.IconComponent = tmp(9880).BellIcon;
          ...
          obj36.IconComponent = tmp(10416).BellSlashIcon;
          obj36.onPress = function onPress() {
            ...
            obj.openLazy(asyncRequireImpl(10403, dependencyMap.paths), combined, obj2);
          };
          push3(obj36);
        }
        ...
        obj37.IconComponent = tmp(11233).ChannelNotificationIcon;
  ...
  obj38.IconComponent = tmp(7215).EyeIcon;
