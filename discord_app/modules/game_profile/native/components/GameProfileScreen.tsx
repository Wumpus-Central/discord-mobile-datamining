...
export default function GameProfileScreen(gameId) {
  ...
  const effect3 = obj1.useEffect(() => () => {
    ...
    ({ guildId, isVerified } = guildIdAndVerifiedFromInvite);
    obj = { viewId, gameId, gameName: null, playedFriendIds: null, playedFriendsData: null, similarGames: null, guildId: null, isVerified: null };
    let str = ref.current;
    if (str == null) {
      str = "";
    }
  ...
  obj2 = { ref: bottomSheetRef, startExpanded: true, scrollable: true, handleDisabled: true, onExpand: callback, children: null };
  if (!isLoading) {
    if (null != data) {
      let tmp44 = name;
