...
import buildSteamStoreUrl from "buildSteamStoreUrl" /* 9105 */;
import SteamReleaseStatus from "SteamReleaseStatus" /* 9106 */;
import calculateSteamReviewScoreDescription2 from "calculateSteamReviewScoreDescription" /* 9136 */;
import getSteamReviewScoreDescriptionColor from "getSteamReviewScoreDescriptionColor" /* 9137 */;
...
function SteamReviewRow(url) {
  ...
  obj = {
    onPress: React.useCallback(() => {
      trackAction(url(9101).GameProfileTrackActionActions.SteamReviews);
      dependencyMap(url);
    }, items),
  ...
  if (tmp12Result) {
    ...
    tmp12Result = callback(tmp6(4734).Text, obj4);
    const str = intl2.format(tmp6(1236).t.sgIoin, obj5);
  }
  items4[1] = tmp12Result;
  obj2[1] = items4;
  items3[1] = callback(closure_4, obj2);
  obj[4] = items3;
...
function OpenCriticReview(url) {
  ...
  const callback = React.useCallback(() => {
    trackAction(url(9101).GameProfileTrackActionActions.OpenCriticReviews);
    dependencyMap(url);
  }, items);
  if (null != tier) {
    obj1 = url(9138);
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        items4[1] = tmp12(closure_4, obj8);
        obj5[3] = items4;
        tmp9Result = callback(closure_4, obj5);
      }
    }
  }
  items2[1] = tmp9Result;
...
let result = require("obj132").fileFinishedImporting("modules/game_profile/native/components/GameProfileReviews.tsx");
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    let tmp10 = tmp6;
    if (tmp6) {
      tmp10 = tmp9 !== GameTheme.SteamReviewScoreDescription.NO_USER_REVIEWS;
    }
    tmp2Result = getSteamReviewScoreDescriptionColor;
    ...
    obj[3] = intl.string(getSystemLocale.t.GaAQXP);
    ...
    if (tmp10) {
      ...
      if (null != steamWebsiteUrl) {
        ...
        obj2[3] = intl2.string(getSystemLocale.t.MQGNsN);
        ...
        tmp22Result = callback(SteamReviewRow, obj2);
      }
    }
    ...
    if (tmp6) {
      ...
      if (null != steamWebsiteUrl) {
        ...
        obj3[4] = rating;
        obj3[5] = ratingCount;
        tmp22Result = callback(SteamReviewRow, obj3);
      }
    }
    ...
    if (null != opencritic && null != opencriticUrl) {
      ...
      if (null != opencriticUrl) {
        ...
        obj4[2] = trackAction;
        tmp22Result1 = callback(OpenCriticReview, obj4);
      }
    }
    ...
    obj[1] = items;
    tmp20Result = callback(closure_4, obj);
