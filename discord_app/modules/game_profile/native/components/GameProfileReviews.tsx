...
function SteamReviewRow(url) {
  ...
  const obj2 = url(9078);
  const items2 = [
    closure_7(url(9041).SteamNeutralIcon, { size: "sm", color: trackAction(576).colors.ICON_STRONG }),
    closure_7(url(4823).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title }),
  ];
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = {
        ...
        color: tmp11(9078).getSteamReviewScoreDescriptionColor(tmp11(2019).SteamReviewScoreDescription.NO_USER_REVIEWS),
        children: null,
      };
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 =
      game.steamReleaseStatus !== SteamReleaseStatus.SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
