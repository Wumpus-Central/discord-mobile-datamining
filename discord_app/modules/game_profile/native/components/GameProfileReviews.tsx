// === Module 8788: SteamReviewRow ===

// Module 8788 (SteamReviewRow)
import ThemesDefault from "Themes" /* 712 */;
import buildSteamStoreUrl from "buildSteamStoreUrl" /* 8757 */;
...
function SteamReviewRow(url) {
  ...
  const tmp = callback2();
  const tmp4Result = trackAction(8750)(trackAction(4159).openURL);
  ...
  obj = {
    onPress: React.useCallback(() => {
      trackAction(url(8753).GameProfileTrackActionActions.SteamReviews);
      dependencyMap(url);
    }, items),
  ...
  const items2 = [callback(url(8761).SteamNeutralIcon, obj1), callback(url(4442).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  obj[1] = items2;
  ...
  const tmp4 = trackAction(8750);
  obj3[2] = url(8790).getSteamReviewScoreDescriptionIntl(result);
  const items4 = [callback(url(4442).Text, obj3), ];
  let tmp12Result = null != ratingCount && result !== tmp6(4484).SteamReviewScoreDescription.NO_USER_REVIEWS;
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp6(4442).Text, obj4);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "container", tier: "to" };
  }
  ...
  const callback = React.useCallback(() => {
    trackAction(url(8753).GameProfileTrackActionActions.OpenCriticReviews);
    dependencyMap(url);
  }, items);
  if (null != tier) {
    obj1 = url(8791);
  ...
  const items1 = [callback(url(4442).Text, obj), ];
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(8797), obj7), ];
        ...
        obj8[1] = tmp12(tmp11(4442).Text, obj9);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      ...
      tmp11Result = tmp11(8790);
      obj10[1] = tmp11Result.getSteamReviewScoreDescriptionColor(tmp11(4484).SteamReviewScoreDescription.NO_USER_REVIEWS);
      const intl4 = tmp11(1236).intl;
      obj10[2] = intl4.string(tmp11(1236).t["0xYzpO"]);
      tmp12Result = tmp12(tmp11(4442).Text, obj10);
    }
  } else {
    tmp12Result = null;
  }
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8758).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    let tmp2Result = tmp2(8789);
    ...
    let tmp10 = tmp6;
    if (tmp6) {
      tmp10 = tmp9 !== tmp2(4484).SteamReviewScoreDescription.NO_USER_REVIEWS;
    }
    tmp2Result = tmp2(8790);
    ...
    const items = [callback(tmp2(4442).Text, obj), ];
