...
function SteamReviewRow(url) {
  ...
  const obj2 = url(8184);
  const items2 = [closure_7(url(8147).SteamNeutralIcon, { size: "sm", color: trackAction(576).colors.ICON_STRONG }), closure_7(url(4832).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = closure_7(tmp2(4832).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "channel", tier: "hd" };
  }
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        obj12.children = closure_7(tmp11(4832).Text, obj13);
