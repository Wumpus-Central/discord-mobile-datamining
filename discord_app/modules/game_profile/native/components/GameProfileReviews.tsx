// === Module 9000: GameProfileReviews ===
...
import useSteamWebsiteUrl from "useSteamWebsiteUrl" /* 8961 */;
import SteamReleaseStatus from "SteamReleaseStatus" /* 8962 */;
import calculateSteamReviewScoreDescription2 from "calculateSteamReviewScoreDescription" /* 9001 */;
import GameProfileReviewUtils from "GameProfileReviewUtils" /* 9002 */;
...
function SteamReviewRow(url) {
  ...
  const obj2 = url(9002);
  const items2 = [closure_7(url(8965).SteamNeutralIcon, { size: "sm", color: trackAction(576).colors.ICON_STRONG }), closure_7(url(4752).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = closure_7(tmp2(4752).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        obj12.children = closure_7(tmp11(4752).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = { variant: "text-xs/medium", color: tmp11(9002).getSteamReviewScoreDescriptionColor(tmp11(2018).SteamReviewScoreDescription.NO_USER_REVIEWS), children: null };
