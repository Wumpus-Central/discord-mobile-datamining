...
function SteamReviewRow(url) {
  ...
  const tmp4Result = trackAction(9344)(trackAction(4058).openURL);
  ...
  const items2 = [callback(url(9355).SteamNeutralIcon, obj1), callback(url(4338).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  obj[1] = items2;
  ...
  const tmp4 = trackAction(9344);
  obj3[2] = url(9383).getSteamReviewScoreDescriptionIntl(result);
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9390), obj7), ];
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(9352).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    let tmp2Result = tmp2(9382);
