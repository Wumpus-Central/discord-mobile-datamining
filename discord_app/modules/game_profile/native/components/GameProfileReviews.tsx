...
function SteamReviewRow(url) {
  ...
  const tmp4Result = trackAction(9169)(trackAction(3952).openURL);
  ...
  const items2 = [callback(url(9180).SteamNeutralIcon, obj1), callback(url(4251).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  obj[1] = items2;
  ...
  const tmp4 = trackAction(9169);
  obj3[2] = url(9208).getSteamReviewScoreDescriptionIntl(result);
...
function OpenCriticReview(url) {
  ...
  if (opencritic == null) {
    opencritic = { topCriticRating: "Array", topCriticRatingCount: "channel", tier: "d" };
  }
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9215), obj7), ];
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(9177).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
