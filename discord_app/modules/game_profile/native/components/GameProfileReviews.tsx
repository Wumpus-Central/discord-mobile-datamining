...
function SteamReviewRow(url) {
  ...
  const obj2 = url(8176);
  const items2 = [
    closure_7(url(8139).SteamNeutralIcon, { size: "sm", color: trackAction(576).colors.ICON_STRONG }),
    closure_7(url(4825).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title }),
  ];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = closure_7(tmp2(4825).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  const items1 = [closure_7(url(4825).Text, obj3)];
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        obj12.children = closure_7(tmp11(4825).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      const obj14 = {
        ...
        color: tmp11(8176).getSteamReviewScoreDescriptionColor(tmp11(2019).SteamReviewScoreDescription.NO_USER_REVIEWS),
        children: null,
      };
