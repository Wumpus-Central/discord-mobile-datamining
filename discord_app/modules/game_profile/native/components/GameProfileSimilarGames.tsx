...
export default function GameProfileSimilarGames(trackAction) {
  ...
  let importDefault;
  const tmp = createCacheKey();
  const GameProfileSimilarGamesMobileExperiment = trackAction(9197).GameProfileSimilarGamesMobileExperiment;
  ...
  if (tmp6.isFetching) {
    ...
    obj1[5] = ListPadding;
    obj1[6] = ListPadding;
    items[1] = callback(tmp2(7638).FlashList, obj1);
    obj[1] = items;
    ...
    if (0 !== similarGames.length) {
      ...
      obj5[6] = ListPadding;
      obj5[8] = result + PX_12;
      items1[1] = callback(tmp2(7638).FlashList, obj5);
      obj2[1] = items1;
