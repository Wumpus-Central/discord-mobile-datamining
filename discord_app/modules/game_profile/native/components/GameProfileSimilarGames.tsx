// === Module 9237: GameProfileSimilarGames ===
...
import useSimilarGamesDefault from "useSimilarGames" /* 9238 */;
...
let closure_8 = fn(9065).MOBILE_GAME_PROFILE_MAX_WIDTH;
...
let closure_18 = noop.memo((game) => {
  ...
  if (null != coverURL) {
    ...
    if (tmp12Result) {
      ...
      obj5.children = closure_10(trackAction(tmp6[12]), obj6);
      tmp12Result = closure_10(tmp8(tmp6[12]).GameProfileSkeletonContainer, obj5);
    }
    ...
    obj9.children = closure_10(tmp8(tmp6[13]).Text, obj10);
...
let closure_20 = noop.memo((cardWidth) => {
  ...
  return closure_10(cardWidth(9092).GameProfileSectionSkeleton, obj);
});
let size = fn(2);
...
export default function GameProfileSimilarGames(arg0) {
  ...
  if (set.has(gameId)) {
    ...
    const obj5 = {
      horizontal: true,
      renderScrollComponent: tmp2(9078),
      data: similarGames,
      renderItem(game) {
    ...
    obj3.children = closure_10(_mod9077.FlashList, obj5);
    tmp7 = closure_10(tmp2(9092), obj3);
    const tmp2Result = tmp2(9092);
  }
