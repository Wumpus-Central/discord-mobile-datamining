...
let items = [
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.EPICGAMES,
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.STEAM,
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.ROBLOX,
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.BATTLENET,
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.RIOT,
  fn(8812).ThirdPartyGameApplicationWebsiteCategory.MINECRAFT,
];
const size = fn(2);
      ...
      if (null != id1) {
        ...
        if (!someResult) {
          someResult = found.some(
            (category) => category.category === steamWebsiteUrl(8812).ThirdPartyGameApplicationWebsiteCategory.STEAM,
          );
        }
        if (!someResult) {
          let obj = {
        ...
        const sorted = found.sort((category, category2) => {
          let num = -1;
          if (category.category !== steamWebsiteUrl(8812).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
            let num2 = 0;
            if (category2.category === steamWebsiteUrl(8812).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
              num2 = 1;
            }
            num = num2;
          }
