...
const memoResult = importAllResult.memo((guildId) => {
  ...
  if (renderIcon) {
    ...
    const items1 = [callback(VoiceNormalIcon, obj), ];
    ...
    obj1[6] = function renderItem(user) {
      const obj = { user, guildId, size: guildId(dependencyMap[3]).AvatarSizes.XSMALL_20, cutout: null };
      let tmp2;
      if (!arg1) {
        tmp2 = closure_1_6;
      }
      obj[3] = tmp2;
      return closure_1_4(guildId(dependencyMap[3]).CutoutableAvatarImage, obj);
    };
    items1[1] = callback(tmp4(1297).SummarizedIconRow, obj1);
    obj[1] = items1;
    return tmp5(tmp6, obj);
  }
  VoiceNormalIcon = tmp4(6896).VoiceNormalIcon;
});
const result = require("obj132").fileFinishedImporting("modules/guild_sidebar/native/VoiceUserSummary.tsx");
