...
import closure_5 from "../channel/ChannelStatusStore.tsx";
import closure_6 from "../channel/GatedChannelStore.tsx";
import closure_7 from "../guild_scheduled_events/GuildScheduledEventStore.tsx";
import closure_8 from "../impersonate/ImpersonateStore.tsx";
import closure_9 from "../opt_in_channels/FavoritesSuggestionStore.tsx";
import closure_10 from "../opt_in_channels/RecentlyActiveCollapseStore.tsx";
import closure_11 from "../recent_channels/NewChannelsStore.tsx";
import closure_12 from "../threads/ActiveJoinedThreadsStore.tsx";
import closure_13 from "../threads/JoinedThreadsStore.tsx";
import createChannelRecord from "../../records/ChannelRecord.tsx";
import closure_18 from "../../stores/CategoryCollapseStore.tsx";
import closure_19 from "../../stores/ChannelStore.tsx";
import closure_20 from "../../stores/CollapsedVoiceChannelStore.tsx";
import closure_21 from "../../stores/GuildStore.tsx";
import closure_22 from "../../stores/PermissionStore.tsx";
import closure_23 from "../../stores/ReadStateStore.tsx";
import closure_24 from "../../stores/SelectedChannelStore.tsx";
import closure_25 from "../../stores/UserGuildSettingsStore.tsx";
import closure_26 from "../../stores/VoiceStateStore.tsx";
import closure_27 from "../../stores/views/SortedVoiceStateStore.tsx";
import closure_28 from "ChannelListVoiceCategoryStore.tsx";
...
function computeThreadIds(record, activeJoinedUnreadThreadsForParent, selectedChannel, closure_1_3, closure_1) {
  ...
  if (set.has(record.type)) {
    ...
    const mapped = applyDefault
      .sortBy(Object.values(activeJoinedUnreadThreadsForParent), (joinTimestamp) => -joinTimestamp.joinTimestamp)
    ...
    const sortByResult = applyDefault.sortBy(
      Object.values(activeJoinedUnreadThreadsForParent),
...
class ChannelListImpl {
  constructor(arg0, arg1, arg2) {
    ...
    for (const key10112 in obj2) {
      ...
      obj.categories[key10112] = new ChannelListCategoryWithParent(
        obj,
        obj1[key10112],
        obj2[key10112],
        initializationData,
    ...
    if (obj6.isRecentlyActiveChannelsEnabled()) {
      ...
      tmp27 = new.target;
      tmp28 = obj;
      tmp29 = new ChannelListRecentsCategory(
        obj,
        tmp22,
        tmp35,
        initializationData,
    ...
    closure_1 = undefined;
    tmp38 = new ChannelListVoiceChannelsCategory(
      obj,
      tmp22,
      tmp35,
      initializationData,
    ...
    tmp47 = require("../../../_runtime/metro/00038__.js")(
      !("null" in obj.categories),
...
Object.defineProperty(prototype, "initializationData", {
  get: function initializationData(arg0) {
    return {
      selectedChannel: store5.getChannel(store7.getChannelId()),
      selectedVoiceChannelId: store7.getVoiceChannelId(),
      activeJoinedRelevantThreads: store4.getActiveJoinedRelevantThreadsForGuild(this.id),
      activeJoinedUnreadThreads: store4.getActiveJoinedUnreadThreadsForGuild(this.id),
    };
  },
  set: undefined,
});
...
prototype["getRows"] = function getRows() {
  ...
  if (null == this.rows) {
    const _Object = Object;
    self.sortedNamedCategories = applyDefault.sortBy(
      Object.values(self.categories),
      ...
      if (null != arg0) {
        ...
        if (null == tmp7) {
          ...
          if (tmp7.type === constants.GUILD_CATEGORY) {
            ...
            obj2[0] =
              applyDefault.findIndex(self.getSortedNamedCategories(), (id) => {
                id = undefined;
                if (channel2 != null) {
                  id = channel2.id;
                }
...
prototype6["getShownChannelIds"] = function getShownChannelIds() {
  ...
  if (null == this.shownChannelIds) {
    ...
    const found = values.filter(
      (renderLevel) =>
        renderLevel.renderLevel === closure_35.Show || renderLevel.renderLevel === tmp.WouldShowIfUncollapsed,
    );
    const obj = applyDefault(self.channels);
    ...
    HermesBuiltin.arraySpread(
      valueResult,
      HermesBuiltin.arraySpread(found.filter((renderLevel) => renderLevel.renderLevel === closure_35.Show).value(), 0),
    );
    set = new Set(items);
    const items1 = [];
    ...
    self.shownChannelIds = applyDefault(items1)
      .sortBy((record) => record.record.position)
      .map((id) => id.id)
      .value();
    const iter3 = applyDefault(items1)
      .sortBy((record) => record.record.position)
      .map((id) => id.id);
  }
  return self.shownChannelIds;
...
prototype7["getHiddenChannelIds"] = function getHiddenChannelIds() {
  ...
  if (this.guild.optInEnabled) {
    if (null == self.hiddenChannelIds) {
      ...
      const valueResult = applyDefault(self.channels)
        .filter((renderLevel) => renderLevel.renderLevel === obj.WouldShowIfUncollapsed)
        .value();
      if (
      ...
      const iter = applyDefault(self.channels).filter(
        (renderLevel) => renderLevel.renderLevel === obj.WouldShowIfUncollapsed,
      );
    }
    return self.hiddenChannelIds;
  } else {
