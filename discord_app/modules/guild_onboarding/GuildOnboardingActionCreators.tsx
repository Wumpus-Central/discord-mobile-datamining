// === Module 5419: _updateOnboardingResponses ===
...
let obj = {
  ...
  completeOnboarding(guildId, prompts) {
    ...
    const selectedChannelIds = items1(5421).getSelectedChannelIds(selectedOptions);
    if (store2.getEnabled(guildId)) {
      let defaultChannelIds = obj.getDefaultChannelIds(guildId);
    } else {
      defaultChannelIds = [];
    }
    let tmp2Result = tmp2(5421);
    const obj3 = items1(5421);
    ...
    const providerConnectionState = items1(5421).getProviderConnectionState(connections);
    const tmp2Result1 = items1(5421);
    const applicationConnectionState = items1(5421).getApplicationConnectionState(connections);
    const tmp12 = importDefault;
    const tmp2Result2 = items1(5421);
    obj = {};
    const obj8 = expandEventPropertiesDefault;
    const merged = Object.assign(items1(4652).collectGuildAnalyticsMetadata(guildId));
    ...
    const tmp2Result3 = items1(4652);
    const tmp2Result4 = items1(5425);
    ...
    if (closure_5.isFullServerPreview(guildId)) {
      const result = tmp2(5420).updateImpersonatedChannels(guildId, items, []);
      const tmp2Result5 = tmp2(5420);
      const result1 = tmp2(5420).updateImpersonatedData(guildId, { optInEnabled: true });
      const tmp2Result6 = tmp2(5420);
      const _Array = Array;
      const result2 = tmp2(5420).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
