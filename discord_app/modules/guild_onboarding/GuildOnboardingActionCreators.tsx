// === Module 5424: _updateOnboardingResponses ===
...
let obj = {
  ...
  completeOnboarding(guildId, prompts) {
    ...
    const selectedChannelIds = items1(5426).getSelectedChannelIds(selectedOptions);
    if (store2.getEnabled(guildId)) {
      let defaultChannelIds = obj.getDefaultChannelIds(guildId);
    } else {
      defaultChannelIds = [];
    }
    let tmp2Result = tmp2(5426);
    const obj3 = items1(5426);
    ...
    const providerConnectionState = items1(5426).getProviderConnectionState(connections);
    const tmp2Result1 = items1(5426);
    const applicationConnectionState = items1(5426).getApplicationConnectionState(connections);
    const tmp12 = importDefault;
    const tmp2Result2 = items1(5426);
    obj = {};
    const obj8 = expandEventPropertiesDefault;
    const merged = Object.assign(items1(4653).collectGuildAnalyticsMetadata(guildId));
    ...
    const tmp2Result3 = items1(4653);
    const tmp2Result4 = items1(5430);
    ...
    if (closure_5.isFullServerPreview(guildId)) {
      const result = tmp2(5425).updateImpersonatedChannels(guildId, items, []);
      const tmp2Result5 = tmp2(5425);
      const result1 = tmp2(5425).updateImpersonatedData(guildId, { optInEnabled: true });
      const tmp2Result6 = tmp2(5425);
      const _Array = Array;
      const result2 = tmp2(5425).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
