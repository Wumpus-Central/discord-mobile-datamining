// === Module 5186: _updateOnboardingResponses ===
...
obj[2] = function updateRolesLocal(guildId, items2, differenceResult1) {
  ...
  if (initialize.isViewingRoles(guildId)) {
    const obj5 = require(5187) /* updateImpersonating */;
...
obj[3] = function completeOnboarding(guildId, prompts) {
  ...
  let obj1 = items1(5188);
  const selectedRoleIds = obj1.getSelectedRoleIds(selectedOptions);
  const selectedChannelIds = items1(5188).getSelectedChannelIds(selectedOptions);
  ...
  const providerConnectionState = items1(5188).getProviderConnectionState(connections);
  const tmp2Result1 = items1(5188);
  const applicationConnectionState = items1(5188).getApplicationConnectionState(connections);
  const tmp12 = importDefault;
  const tmp2Result2 = items1(5188);
  obj = {};
  const obj8 = importDefault(698);
  const merged = Object.assign(items1(4450).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result3 = items1(4450);
  const tmp2Result4 = items1(5192);
  ...
  if (initialize.isFullServerPreview(guildId)) {
    const result = tmp2(5187).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result5 = tmp2(5187);
    const result1 = tmp2(5187).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result6 = tmp2(5187);
    const _Array = Array;
    const result2 = tmp2(5187).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
