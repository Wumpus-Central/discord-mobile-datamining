...
let closure_12 = fn(7344).GuildOnboardingModalStates;
...
export default function GuildOnboardingConnectionPrompt(guildId) {
  ...
  const stateFromStores = guildId(isLastStep[16]).useStateFromStores(items, () =>
    GuildOnboardingPromptsStore.getOnboardingConnections(guildId),
  );
  let obj2 = guildId(isLastStep[16]);
  const items1 = [stateFromStores4];
  const stateFromStoresArray = guildId(isLastStep[16]).useStateFromStoresArray(items1, () =>
  ...
  const stateFromStores2 = guildId(isLastStep[16]).useStateFromStores(items3, () =>
  ...
  stateFromStores4 = guildId(isLastStep[16]).useStateFromStores(
    items5,
    () => {
    ...
    if (!stateFromStores3) {
      if (0 !== stateFromStores.length) {
        if (!ref.current) {
          ...
          obj6.required = false;
          ({ connected: obj4.provider_connections_connected, notConnected: obj4.provider_connections_not_connected } =
            providerConnectionState);
          ({
            connected: obj4.application_connections_connected,
            notConnected: obj4.application_connections_not_connected,
  ...
  const callback = navigation.useCallback(() => {
    ...
    ({ connected: obj4.provider_connections_connected, notConnected: obj4.provider_connections_not_connected } =
      providerConnectionState);
    ({ connected: obj4.application_connections_connected, notConnected: obj4.application_connections_not_connected } =
      applicationConnectionState);
