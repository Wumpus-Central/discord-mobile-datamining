...
function BackgroundImageGradient(uri) {
  ...
  obj = { style: tmp.backgroundColorGradient, start: ConstantsIOS.VerticalGradient.START, end: ConstantsIOS.VerticalGradient.END, colors: null };
...
fn(4560);
let createStyles = { flex: { flex: 1 }, container: null, subtitle: null, onboardingTitle: null, onboardingPolicy: null, onboardingPolicyText: null, landingOverlay: null, artWrapper: null, landingBody: null, backgroundImage: null, backgroundColorGradient: null, darkColorGradient: null };
...
createStyles.onboardingPolicy = { position: "absolute", paddingHorizontal: nativeDefault.space.PX_16, display: "flex", justifyContent: "center", width: "100%", marginBottom: nativeDefault.space.PX_16 };
createStyles.onboardingPolicyText = { textAlign: "center" };
createStyles.landingOverlay = { position: "absolute", width: "100%", height: "100%", display: "flex", justifyContent: "center" };
createStyles.artWrapper = { height: 350, position: "relative", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center" };
let obj2 = { position: "absolute", paddingHorizontal: nativeDefault.space.PX_16, display: "flex", justifyContent: "center", width: "100%", marginBottom: nativeDefault.space.PX_16 };
...
export default function GuildOnboardingPrompt(guildId) {
  ...
  const tmp8 = obj2.useStateFromStores(items2, () => GuildOnboardingPromptsStore.getOnboardingConnections(guildId)).length > 0;
  closure_7 = tmp8;
  if (isFirstOpen) {
  ...
  if (prompts.length > 0) {
    ...
    if (null != tmp11) {
      ...
      if (stateFromStores2.MULTIPLE_CHOICE === type) {
        const obj7 = { guildId, currentPrompt: tmp11, lastPrompt: tmp18, currentPromptIndex: currentPromptIdx, numberOfPrompts: prompts.length, selectOption, handleOnPress };
        tmp34Result = tmp34(tmp2(tmp3[36]).MultipleChoicePrompt, obj7);
      } else if (tmp36.DROPDOWN === type) {
        const obj8 = { guildId, currentPrompt: tmp11, lastPrompt: tmp18, currentPromptIndex: currentPromptIdx, numberOfPrompts: prompts.length, selectOption, handleOnPress };
  ...
  const obj14 = { style: null, children: guildSplashURL(currentPromptIdx(tmp3[38]), { source: guildId.landingAnimation, autoPlay: !stateFromStores1, style: { width: "100%" } }) };
  ...
  const obj18 = { style: tmp.onboardingTitle, accessibilityRole: "header", variant: "heading-xl/semibold", color: "text-overlay-light", children: null };
  ...
  obj19.style = items25;
  const obj20 = { style: tmp.onboardingPolicyText, variant: "heading-sm/normal", color: "text-default", children: null };
