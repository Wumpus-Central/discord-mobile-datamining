// === Module 6542: GuildOnboardingPromptOptionButton ===
...
let Easing = fn(4563).Easing;
obj.easing = Easing.out(fn(4563).Easing.ease);
...
export default function PromptOptionButton(option) {
  ...
  let obj3 = option(4563);
  R.__closure = { selected, withTiming: option(4830).withTiming, Easing: option(4563).Easing, useReducedMotion: stateFromStores1, withSequence: option(4563).withSequence, withSpring: option(5273).withSpring };
  R.__workletHash = 8281627194581;
  R.__initData = __initData;
  const animatedStyle = obj3.useAnimatedStyle(R);
  const tmp8 = selected(6543)(option.guildId);
  closure_5 = tmp8;
  num = 0;
  if (null != tmp8) {
    num = 0;
  ...
  let obj4 = { selected, withTiming: option(4830).withTiming, Easing: option(4563).Easing, useReducedMotion: stateFromStores1, withSequence: option(4563).withSequence, withSpring: option(5273).withSpring };
  ...
  let tmp2Result = option(4563);
  V.__closure = { showMemberCount, withDelay: option(4563).withDelay, withTiming: option(4830).withTiming, Easing: option(4563).Easing, useReducedMotion: stateFromStores1 };
  V.__workletHash = 9518487706997;
  V.__initData = token;
  const animatedStyle1 = tmp2Result.useAnimatedStyle(V);
  const obj6 = { showMemberCount, withDelay: option(4563).withDelay, withTiming: option(4830).withTiming, Easing: option(4563).Easing, useReducedMotion: stateFromStores1 };
  sharedValue = option(4563).useSharedValue(1);
  isUnseen = canBeNew;
  if (canBeNew) {
  ...
  const tmp2Result11 = option(4563);
  token = option(4528).useToken(tmp7(576).colors.BORDER_SUBTLE);
  const tmp2Result12 = option(4528);
  token1 = option(4528).useToken(tmp7(576).colors.BACKGROUND_BRAND);
  const tmp2Result13 = option(4528);
  token2 = option(4528).useToken(tmp7(576).colors.BORDER_STRONG);
  const tmp2Result14 = option(4528);
  token3 = option(4528).useToken(tmp7(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp2Result15 = option(4528);
  const fn = function q() {
    obj = timing;
    num = 0;
    if (selected) {
  ...
  const tmp2Result16 = option(4563);
  ...
  const tmp2Result17 = option(4563);
  ...
  const tmp2Result18 = option(4563);
  W.__closure = { interpolateColor: option(4563).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  W.__workletHash = 1340353593596;
  W.__initData = token3;
  const animatedStyle2 = tmp2Result18.useAnimatedStyle(W);
  const obj9 = { interpolateColor: option(4563).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  class K {
    constructor() {
      if (closure_4) {
        obj = {};
  ...
  const animatedStyle3 = option(4563).useAnimatedStyle(K);
  const tmp2Result19 = option(4563);
  const checkboxA11yNative = option(4545).useCheckboxA11yNative({ checked: selected });
  ...
  if (tmp38Result) {
    ...
    obj13.name = str;
    obj12.children = closure_9(tmp7(6546), obj13);
    tmp38Result = tmp38(closure_5, obj12);
    const tmp7Result = tmp7(6546);
  }
  ...
  const items10 = [ref(option(5428).PressableOpacity, obj11), , , ];
  ...
  if (num > 0) {
    ...
    tmp44Result2 = tmp44(tmp7(4563).View, obj18);
  }
  ...
  const tmp2Result20 = option(4545);
  obj21.children = closure_9(option(6549).CheckmarkSmallIcon, { size: "xs", color: selected(576).colors.WHITE });
  items10[2] = closure_9(selected(4563).View, obj21);
  if (canBeNew) {
    canBeNew = !selected;
  }
  if (canBeNew) {
  ...
  return ref(selected(4563).View, obj10);
};