// === Module 5549: PromptOptionButton ===
...
obj[1] = Easing.out(require("module_4187").Easing.ease);
...
export default function PromptOptionButton(option) {
  ...
  obj = { selected, withTiming: option(4449).withTiming, Easing: option(4187).Easing, useReducedMotion: stateFromStores1, withSequence: option(4187).withSequence, withSpring: option(4895).withSpring };
  R.__closure = obj;
  R.__workletHash = 8281627194581;
  R.__initData = closure_13;
  const animatedStyle = obj2.useAnimatedStyle(R);
  const tmp8 = selected(5550)(option.guildId);
  closure_5 = tmp8;
  num = 0;
  if (null != tmp8) {
    num = 0;
  ...
  obj = { showMemberCount: first, withDelay: tmp2(4187).withDelay, withTiming: tmp2(4449).withTiming, Easing: tmp2(4187).Easing, useReducedMotion: stateFromStores1 };
  V.__closure = obj;
  ...
  token = option(4167).useToken(tmp7(712).colors.BORDER_SUBTLE);
  let tmp2Result1 = option(4167);
  token1 = option(4167).useToken(tmp7(712).colors.BACKGROUND_BRAND);
  const tmp2Result2 = option(4167);
  token2 = option(4167).useToken(tmp7(712).colors.BORDER_STRONG);
  const tmp2Result3 = option(4167);
  token3 = option(4167).useToken(tmp7(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp2Result4 = option(4167);
  const fn = function q() {
    num = 0;
    if (selected) {
      num = 1;
  ...
  obj1 = { withTiming: tmp2(4449).withTiming, selected, SELECTION_TIMING: sharedValue };
  fn.__closure = obj1;
  fn.__workletHash = 11553377214675;
  fn.__initData = token1;
  derivedValue = option(4187).useDerivedValue(fn);
  const tmp2Result5 = option(4187);
  ...
  obj2 = { withTiming: tmp2(4449).withTiming, isNew: isUnseen, SELECTION_TIMING: sharedValue };
  fn2.__closure = obj2;
  fn2.__workletHash = 9359578148244;
  fn2.__initData = token2;
  derivedValue1 = option(4187).useDerivedValue(fn2);
  const tmp2Result6 = option(4187);
  class W {
    constructor() {
      obj = option(onSelect[7]);
      items = [, ];
  ...
  const tmp2Result7 = option(4187);
  W.__closure = { interpolateColor: option(4187).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  W.__workletHash = 1340353593596;
  W.__initData = token3;
  const animatedStyle2 = tmp2Result7.useAnimatedStyle(W);
  let obj3 = { interpolateColor: option(4187).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  class K {
    constructor() {
      if (closure_4) {
        obj = {};
  ...
  const animatedStyle3 = option(4187).useAnimatedStyle(K);
  const tmp2Result8 = option(4187);
  const checkboxA11yNative = option(4175).useCheckboxA11yNative({ checked: selected });
  ...
  const items9 = [callback2(option(4444).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: option.title }), ];
  ...
  if (tmp44Result) {
    ...
    tmp44Result = tmp44(tmp2(4444).Text, obj11);
  }
  items9[1] = tmp44Result;
  obj9[1] = items9;
  items8[1] = closure_10(closure_5, obj9);
  obj5[8] = items8;
  const items10 = [closure_10(option(5036).PressableOpacity, obj5), , , ];
  tmp44Result = null;
  if (num > 0) {
    ...
    obj12[4] = tmp44(tmp2(4444).Text, obj13);
    tmp44Result = tmp44(tmp7(4187).View, obj12);
  }
  ...
  const tmp2Result9 = option(4175);
  const tmp43 = closure_5;
  obj15[1] = callback2(option(5556).CheckmarkSmallIcon, { size: "xs", color: selected(712).colors.WHITE });
  items10[2] = callback2(selected(4187).View, obj15);
  if (canBeNew) {
    canBeNew = !selected;
  }
  if (canBeNew) {
  ...
  items10[3] = canBeNew;
  obj4[1] = items10;
  return closure_10(selected(4187).View, obj4);
};