...
let Easing = fn(4566).Easing;
obj.easing = Easing.out(fn(4566).Easing.ease);
...
export default function PromptOptionButton(option) {
  ...
  let tmp2Result = option(4566);
  V.__closure = { showMemberCount, withDelay: option(4566).withDelay, withTiming: option(4837).withTiming, Easing: option(4566).Easing, useReducedMotion: stateFromStores1 };
  V.__workletHash = 9518487706997;
  V.__initData = token;
  const animatedStyle1 = tmp2Result.useAnimatedStyle(V);
  const obj6 = { showMemberCount, withDelay: option(4566).withDelay, withTiming: option(4837).withTiming, Easing: option(4566).Easing, useReducedMotion: stateFromStores1 };
  sharedValue = option(4566).useSharedValue(1);
  isUnseen = canBeNew;
  if (canBeNew) {
  ...
  const tmp2Result11 = option(4566);
  token = option(4531).useToken(tmp7(576).colors.BORDER_SUBTLE);
  const tmp2Result12 = option(4531);
  token1 = option(4531).useToken(tmp7(576).colors.BACKGROUND_BRAND);
  const tmp2Result13 = option(4531);
  token2 = option(4531).useToken(tmp7(576).colors.BORDER_STRONG);
  const tmp2Result14 = option(4531);
  token3 = option(4531).useToken(tmp7(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp2Result15 = option(4531);
  const fn = function q() {
    obj = timing;
    num = 0;
    if (selected) {
  ...
  const tmp2Result16 = option(4566);
  fn.__closure = { withTiming: option(4837).withTiming, selected, SELECTION_TIMING: sharedValue };
  fn.__workletHash = 11553377214675;
  fn.__initData = token1;
  derivedValue = tmp2Result16.useDerivedValue(fn);
  const obj7 = { withTiming: option(4837).withTiming, selected, SELECTION_TIMING: sharedValue };
  ...
  const tmp2Result17 = option(4566);
  fn2.__closure = { withTiming: option(4837).withTiming, isNew: isUnseen, SELECTION_TIMING: sharedValue };
  fn2.__workletHash = 9359578148244;
  fn2.__initData = token2;
  derivedValue1 = tmp2Result17.useDerivedValue(fn2);
  const obj8 = { withTiming: option(4837).withTiming, isNew: isUnseen, SELECTION_TIMING: sharedValue };
  class W {
    constructor() {
      obj = closure_0(closure_2[7]);
      items = [, ];
  ...
  const tmp2Result18 = option(4566);
  W.__closure = { interpolateColor: option(4566).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  W.__workletHash = 1340353593596;
  W.__initData = token3;
  const animatedStyle2 = tmp2Result18.useAnimatedStyle(W);
  const obj9 = { interpolateColor: option(4566).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  class K {
    constructor() {
      if (closure_4) {
        obj = {};
  ...
  const animatedStyle3 = option(4566).useAnimatedStyle(K);
  const tmp2Result19 = option(4566);
  const checkboxA11yNative = option(4548).useCheckboxA11yNative({ checked: selected });
  ...
  const items9 = [closure_9(option(4832).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: option.title }), ];
  ...
  if (tmp44Result) {
    ...
    tmp44Result = tmp44(tmp2(4832).Text, obj17);
  }
  ...
  if (num > 0) {
    ...
    obj18.children = tmp44(tmp2(4832).Text, obj19);
    tmp44Result2 = tmp44(tmp7(4566).View, obj18);
  }
  ...
  return ref(selected(4566).View, obj10);
};