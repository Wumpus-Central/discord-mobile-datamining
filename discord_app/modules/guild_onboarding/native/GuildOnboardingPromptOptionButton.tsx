...
container.easing = Easing.out(fn(4310).Easing.ease);
...
export default function PromptOptionButton(option) {
  ...
  __closure = { selected, withTiming: option(4575).withTiming, Easing: option(4310).Easing, useReducedMotion: stateFromStores1, withSequence: option(4310).withSequence, withSpring: option(4988).withSpring };
  ...
  __closure = { showMemberCount, withDelay: tmp2(4310).withDelay, withTiming: tmp2(4575).withTiming, Easing: tmp2(4310).Easing, useReducedMotion: stateFromStores1 };
  ...
  obj1 = { withTiming: tmp2(4575).withTiming, selected, SELECTION_TIMING: sharedValue };
  fn.__closure = obj1;
  fn.__workletHash = 11553377214675;
  fn.__initData = token1;
  derivedValue = option(4310).useDerivedValue(fn);
  const tmp2Result5 = option(4310);
  ...
  obj2 = { withTiming: tmp2(4575).withTiming, isNew: isUnseen, SELECTION_TIMING: sharedValue };
  fn2.__closure = obj2;
  fn2.__workletHash = 9359578148244;
  fn2.__initData = token2;
  derivedValue1 = option(4310).useDerivedValue(fn2);
  const tmp2Result6 = option(4310);
  class W {
    constructor() {
      obj = closure_0(closure_2[7]);
      items = [, ];
  ...
  let obj3 = { interpolateColor: option(4310).interpolateColor, newProgress: derivedValue1, unselectedBorderColor: token, newBorderColor: token1, selectedProgress: derivedValue, selectedBorderColor: token2, selectedBackgroundColor: token3 };
  class K {
    constructor() {
      if (closure_4) {
        obj = {};
  ...
  if (tmp44Result) {
    ...
    tmp44Result = tmp44(tmp2(4570).Text, obj11);
  }
  items9[1] = tmp44Result;
  obj9.children = items9;
  items8[1] = ref(closure_5, obj9);
  obj5.children = items8;
  const items10 = [ref(option(5137).PressableOpacity, obj5), , , ];
  tmp44Result = null;
  if (num > 0) {
    ...
    obj12.children = tmp44(tmp2(4570).Text, obj13);
    tmp44Result = tmp44(tmp7(4310).View, obj12);
  }
  ...
  const tmp2Result9 = option(4292);
  obj15.children = closure_9(option(7147).CheckmarkSmallIcon, { size: "xs", color: selected(576).colors.WHITE });
  items10[2] = closure_9(selected(4310).View, obj15);
  if (canBeNew) {
    canBeNew = !selected;
  }
  if (canBeNew) {
  ...
  return ref(selected(4310).View, obj4);
};