// === Module 6964: PromptOptionButton ===
...
export default function PromptOptionButton(option) {
  ...
  obj = { selected, withTiming: option(4671).withTiming, Easing: option(4119).Easing, useReducedMotion: stateFromStores1, withSequence: option(4119).withSequence, withSpring: option(4749).withSpring };
  R.__closure = obj;
  R.__workletHash = 8281627194581;
  R.__initData = isUnseen;
  const animatedStyle = obj2.useAnimatedStyle(R);
  const tmp8 = selected(6965)(option.guildId);
  closure_5 = tmp8;
  num = 0;
  if (null != tmp8) {
    num = 0;
  ...
  obj = { showMemberCount: first, withDelay: tmp2(4119).withDelay, withTiming: tmp2(4671).withTiming, Easing: tmp2(4119).Easing, useReducedMotion: stateFromStores1 };
  G.__closure = obj;
  ...
  token = option(4100).useToken(tmp7(712).colors.BACKGROUND_BRAND);
  let tmp2Result1 = option(4100);
  token1 = option(4100).useToken(tmp7(712).colors.BORDER_SUBTLE);
  const tmp2Result2 = option(4100);
  token2 = option(4100).useToken(tmp7(712).colors.BORDER_STRONG);
  const tmp2Result3 = option(4100);
  token3 = option(4100).useToken(tmp7(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp2Result4 = option(4100);
  class H {
    constructor() {
      if (closure_4) {
        obj = { duration: 0 };
  ...
  obj1 = { useReducedMotion: stateFromStores1, Easing: tmp2(4119).Easing, withTiming: tmp2(4671).withTiming, selected, selectedBorderColor: token2, isNew: isUnseen, unselectedBrandBorderColor: token, unselectedBorderColor: token1, selectedBackgroundColor: token3 };
  H.__closure = obj1;
  H.__workletHash = 17180171486485;
  H.__initData = token;
  const animatedStyle2 = option(4119).useAnimatedStyle(H);
  const tmp2Result5 = option(4119);
  const fn = function q() {
    if (stateFromStores1) {
      let obj = {};
    } else {
  ...
  const animatedStyle3 = option(4119).useAnimatedStyle(fn);
  const tmp2Result6 = option(4119);
  const checkboxA11yNative = option(4108).useCheckboxA11yNative({ checked: selected });
  ...
  const items9 = [callback2(option(4739).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: option.title }), ];
  ...
  if (tmp42Result) {
    ...
    tmp42Result = tmp42(tmp2(4739).Text, obj9);
  }
  items9[1] = tmp42Result;
  obj7[1] = items9;
  items8[1] = closure_10(closure_5, obj7);
  obj3[8] = items8;
  const items10 = [closure_10(option(5438).PressableOpacity, obj3), , , ];
  tmp42Result = null;
  if (num > 0) {
    ...
    obj10[4] = tmp42(tmp2(4739).Text, obj11);
    tmp42Result = tmp42(tmp7(4119).View, obj10);
  }
  ...
  const tmp2Result7 = option(4108);
  const tmp41 = closure_5;
  obj13[1] = callback2(option(6971).CheckmarkSmallIcon, { size: "xs", color: selected(712).colors.WHITE });
  items10[2] = callback2(selected(4119).View, obj13);
  if (canBeNew) {
    canBeNew = !selected;
  }
  if (canBeNew) {
  ...
  items10[3] = canBeNew;
  obj2[1] = items10;
  return closure_10(selected(4119).View, obj2);
};