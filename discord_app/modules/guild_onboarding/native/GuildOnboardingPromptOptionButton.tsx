...
export default function PromptOptionButton(option) {
  ...
  const tmp8 = selected(5357)(option.guildId);
  c5 = tmp8;
  num = 0;
  if (null != tmp8) {
    num = 0;
  ...
  const items9 = [callback2(option(4299).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: option.title }), ];
  ...
  if (tmp42Result) {
    ...
    tmp42Result = tmp42(tmp2(4299).Text, obj9);
  }
  items9[1] = tmp42Result;
  obj7[1] = items9;
  items8[1] = closure_10(c5, obj7);
  obj3[8] = items8;
  const items10 = [closure_10(option(4846).PressableOpacity, obj3), , , ];
  tmp42Result = null;
  if (num > 0) {
