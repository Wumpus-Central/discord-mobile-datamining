...
export default function PromptOptionButton(option) {
  ...
  obj = { selected, withTiming: option(4303).withTiming, Easing: option(4162).Easing, useReducedMotion: stateFromStores1, withSequence: option(4162).withSequence, withSpring: option(4711).withSpring };
  R.__closure = obj;
  R.__workletHash = 8281627194581;
  R.__initData = isUnseen;
  const animatedStyle = obj2.useAnimatedStyle(R);
  const tmp8 = selected(5356)(option.guildId);
  c5 = tmp8;
  num = 0;
  if (null != tmp8) {
    num = 0;
  ...
  obj = { showMemberCount: first, withDelay: tmp2(4162).withDelay, withTiming: tmp2(4303).withTiming, Easing: tmp2(4162).Easing, useReducedMotion: stateFromStores1 };
  G.__closure = obj;
  ...
  obj1 = { useReducedMotion: stateFromStores1, Easing: tmp2(4162).Easing, withTiming: tmp2(4303).withTiming, selected, selectedBorderColor: token2, isNew: isUnseen, unselectedBrandBorderColor: token, unselectedBorderColor: token1, selectedBackgroundColor: token3 };
  H.__closure = obj1;
  H.__workletHash = 17180171486485;
  H.__initData = token;
  const animatedStyle2 = option(4162).useAnimatedStyle(H);
  const tmp2Result5 = option(4162);
  const fn = function q() {
    if (stateFromStores1) {
      let obj = {};
    } else {
  ...
  const animatedStyle3 = option(4162).useAnimatedStyle(fn);
  const tmp2Result6 = option(4162);
  ...
  const items9 = [callback2(option(4298).Text, { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: option.title }), ];
  ...
  if (tmp42Result) {
    ...
    tmp42Result = tmp42(tmp2(4298).Text, obj9);
  }
  items9[1] = tmp42Result;
  obj7[1] = items9;
  items8[1] = closure_10(c5, obj7);
  obj3[8] = items8;
  const items10 = [closure_10(option(4844).PressableOpacity, obj3), , , ];
  tmp42Result = null;
  if (num > 0) {
    ...
    obj10[4] = tmp42(tmp2(4298).Text, obj11);
    tmp42Result = tmp42(tmp7(4162).View, obj10);
  }
  ...
  obj13[1] = callback2(option(5362).CheckmarkSmallIcon, obj14);
  items10[2] = callback2(selected(4162).View, obj13);
  if (canBeNew) {
    canBeNew = !selected;
  }
  if (canBeNew) {
  ...
  items10[3] = canBeNew;
  obj2[1] = items10;
  return closure_10(selected(4162).View, obj2);
};