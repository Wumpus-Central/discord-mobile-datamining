...
let obj2 = {
  ...
  containerWithoutSplash: { backgroundColor: nativeDefault.colors.BACKGROUND_BASE_LOWEST },
...
export default function GuildOnboardingCompleted(guildId) {
  ...
  closure_7 = guildId(onClose[11]).useStateFromStoresArray(
    items2,
  ...
  const stateFromStoresArray = guildId(onClose[11]).useStateFromStoresArray(
    items5,
  ...
  const fn = function k() {
    ...
    obj16.rotate = tmpResult.withSequence(
      withTimingResult2,
      withDelayResult2,
      timing.withTiming("-5deg", { duration }),
    );
  ...
  fn.__closure = {
    withSequence: guildId(onClose[19]).withSequence,
    withTiming: guildId(onClose[20]).withTiming,
    withDelay: guildId(onClose[19]).withDelay,
    ANIMATION_DURATION: stateFromStores2,
    Easing: guildId(onClose[19]).Easing,
    ...
    if (null != stateFromStores1) {
      ...
      const items14 = [
        substr.map((role) => {
          const obj = {
            style: role.role,
            children: closure_2_10(UserProfileRolesCard.RoleItem, {
              role,
