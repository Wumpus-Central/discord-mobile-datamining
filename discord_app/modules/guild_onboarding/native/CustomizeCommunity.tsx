...
function PromptTitle(item) {
  ...
  if (item.required) {
    ...
    tmp3Result = tmp3(Text_Text.Text, obj);
  }
  items[1] = tmp3Result;
...
function PromptHelpText(arg0) {
  ...
  if ("" !== helpText) {
    ...
    obj.children = items3;
    let tmp9 = closure_17(tmp2(tmp3[22]).Text, obj);
  } else {
    tmp9 = null;
  }
  return tmp9;
...
export default function CustomizeCommunity(guildId) {
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp24Result) {
      ...
      const items5 = [closure_16(tmp4(4556).Text, obj5), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
