...
function EmptyCustomizeCommunity(setTab) {
  ...
  obj = { style: tmp.emptyContainerImage, source: importDefault(obj.isThemeDark(useTheme()) ? 11082 : 11083) };
  ...
  items1[2] = closure_16(setTab(4340).Text, obj2);
  obj[1] = items1;
...
function DropdownOption(option) {
  ...
  if (tmp10Result) {
    ...
    obj1[3] = str;
    obj[1] = callback(Emoji, obj1);
    tmp10Result = tmp10(tmp9, obj);
    tmp11 = importDefault;
    const tmp12 = Emoji;
  }
  const items1 = [tmp10Result, callback(option(4340).Text, { variant: "text-md/semibold", children: option.title })];
  obj[1] = items1;
  return closure_17(closure_4, obj);
}
...
export default function CustomizeCommunity(guildId) {
  ...
  const tmp7 = stateFromStores(11084)(guildId);
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp24Result) {
      ...
      const items5 = [callback(tmp4(4340).Text, obj5), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
