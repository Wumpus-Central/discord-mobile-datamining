...
function EmptyCustomizeCommunity(setTab) {
  ...
  obj = { style: tmp.emptyContainerImage, source: importDefault(obj.isThemeDark(useTheme()) ? 10942 : 10943) };
  ...
  items1[2] = closure_16(setTab(4281).Text, obj2);
  obj[1] = items1;
...
export default function CustomizeCommunity(guildId) {
  ...
  const tmp7 = stateFromStores(10944)(guildId);
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp24Result) {
      ...
      const items5 = [callback(tmp4(4281).Text, obj5), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
