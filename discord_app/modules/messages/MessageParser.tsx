...
function rebuild(arr) {
  ...
  const item = arr.forEach((content) => {
    (function handleEmoji(closure_0, type, f81808) {
      if (null != f81808) {
        if ("customEmoticon" === type.type) {
          f81808(type.emoji, false);
        }
        if ("emoticon" === type.type) {
          const result = callback(4134).translateSurrogatesToInlineEmoji(type.content);
          ...
          if (null !== match) {
            while (true) {
              ...
              let obj2 = callback(4134);
              byId = obj2.getByName(match[2]);
            }
          }
          const obj = callback(4134);
        }
      }
    })(callback, content, f81808);
    if (typeof content.content === "string") {
      ...
      if ("emoji" === type) {
        ...
        if (true === tmp.isNotification) {
          _var = _var + callback(f81808[27]).isolate(content.content);
          let obj2 = callback(f81808[27]);
        } else {
          _var = _var + content.content;
        }
      }
...
function createParserState(getGuildId, arr) {
  ...
  const mapped3 = tmp9Result(textChannelNameDisambiguations.getTextChannelNameDisambiguations(guildId)).map((id) => ({
    id: id.id,
    text: id.name,
  }));
  if (null != guildId) {
    tmp9Result = tmp9(12);
    const found1 = tmp9Result(guildId(5393).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS).filter((arg0) => arg0 !== closure_7);
    const tmp9Result1Result = tmp9Result(guildId(5393).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS);
    const flatMapResult = found1.flatMap((arg0) =>
      closure_1_6.getChannels(guildId)[arg0].map((channel) => {
    ...
    let items = found1
      ...
      .filter(guildId(1470).isNotNullish)
      .value();
    const iter = found1
      .flatMap((arg0) =>
        closure_1_6.getChannels(guildId)[arg0].map((channel) => {
  ...
  obj = {
    inline: true,
    mentionableRoles: mapped2,
    guild,
    users: tmp11Result,
    games: null,
    channels: null,
    emojiContext: null,
    customEmoticonsRegex: null,
    customEmoji: null,
    textExclusions: null,
...
function unparseWithMeta(content, id, isNotification) {
  ...
  if (isNotification) {
    ...
    translateSurrogatesToInlineEmoji = translateSurrogatesToInlineEmoji(4134).translateSurrogatesToInlineEmoji;
  }
  let obj = { inline: true, guild, channelId: id, isNotification };
  const obj4 = translateSurrogatesToInlineEmoji(4192);
  dependencyMap = undefined;
  c3 = "";
  const items = [];
  const item = translateSurrogatesToInlineEmoji(4192)
    .parserFor(omitResult)(content, obj)
    .forEach((content) => {
      (function handleEmoji(closure_0, type, f81808) {
        if (null != f81808) {
          if ("customEmoticon" === type.type) {
...
obj[9] = {
  match(str, games, str2) {
    ...
    if (null == str2[str2.length - 1]) {
      ...
      const AUTOCOMPLETE_BOUNDARY_CHARACTERS_SET = str7(mention[17]).AUTOCOMPLETE_BOUNDARY_CHARACTERS_SET;
    }
    games = games.games;
      ...
      if (null != firstResult1) {
        ...
        if (null != firstResult2) {
          ...
          if (null == firstResult3) {
            ...
            if ("" === str2) {
              const SILENT_RE = str7(mention[18]).SILENT_RE;
...
obj[11] = {
  match(arg0, arg1, arg2) {
    const ConvertEmoticons = str7(4166).ConvertEmoticons;
  ...
  parse(arg0) {
    const obj = {
      type: "emoticon",
      content: parseRawEmojiObjectDefault.convertShortcutToName(arg0[1]),
      isShortcut: true,
    };
    return obj;
  },
...
obj4.match = function match(arg0, textExclusions) {
  if (typeof textExclusions.textExclusions === "string") {
    if ("" !== textExclusions.textExclusions) {
      const result = str7(4958).textMarkupPatternWithExclusions(textExclusions.textExclusions);
  ...
  if (null == user) {
    ...
    if (isNotification) {
      ...
      if (null != id1) {
        ...
        let str5 = "";
        if (
          tmp13Result1.some(store3.getUnsafeMutableRoles(guild.id), (name) =>
            closure_0.startsWith(name.name.toLowerCase()),
          )
        ) {
          const _HermesInternal = HermesInternal;
...
obj12[1] = function parse() {
  ...
  const intl = str7(1233).intl;
  obj[0] = "<" + intl.string(str7(1233).t["F+x38C"]).toLowerCase() + ">";
  return obj;
};
obj5[8] = obj12;
...
const obj16 = {
  parse(getGuildId, content, arg2, arr) {
    ...
    importDefault = obj;
    closure_0 = tmp;
    importDefault = importDefault(f81808[20]).translateInlineEmojiToSurrogates;
    f81808 = (emoji) => {
      let obj = lib(f81808[30]);
