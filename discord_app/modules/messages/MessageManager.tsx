...
import isRefreshableAttachmentUrlAll from "isRefreshableAttachmentUrl" /* 9994 */;
import getMessageLimit from "getMessageLimit" /* 9995 */;
...
import closure_8 from "handlePermissionsChange" /* 6095 */;
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(isRefreshableAttachmentUrlAll.messageHasExpiredAttachmentUrl)) {
            ...
            let tmp9Result = tmp9(5184);
            tmp9Result.clear(channelId);
            tmp9Result = tmp9(5184);
          ...
          if (tmp15) {
            ...
            tmp9(5184).commit(mutation);
            obj7 = mutation;
            const tmp9Result1 = tmp9(5184);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5184).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result2 = tmp9(5184);
          }
          if (isPreload) {
            if (!closure_6.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8613)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5184).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7169).jumpToMessage(obj7);
                const tmp9Result7 = tmp9(7169);
              }
              const tmp9Result3 = tmp9(5184);
            }
          }
