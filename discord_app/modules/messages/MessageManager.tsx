    ...
    if (!isStaticChannelRoute(channelId)) {
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(require(9759) /* shouldRefreshAttachmentUrl */.messageHasExpiredAttachmentUrl)) {
            tmp3.log("Found expired attachment link, clearing messages");
            let tmp9Result = tmp9(4926);
            tmp9Result.clear(channelId);
            tmp9Result = tmp9(4926);
            orCreate1 = tmp9Result.getOrCreate(channelId);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            let obj = { jumpTargetId: null, jumped: false, jumpType: null };
            obj[2] = tmp11(4187).JumpType.ANIMATED;
            const mutation = orCreate1.mutate(obj);
            tmp9(4926).commit(mutation);
            obj7 = mutation;
            const tmp9Result1 = tmp9(4926);
          }
          let obj10 = obj7;
          if (tmp18) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(4926).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result2 = tmp9(4926);
          }
          if (isPreload) {
            if (!_handleConnectionOpen.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8515)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(4926).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                if (isThreadResult) {
                  ...
                  if (flag2) {
                    ...
                    obj1[6] = fetchKey;
                    return tmp9(6814).fetchMessages(obj1);
                  }
                }
                let isThreadResult1;
                if (channel != null) {
                ...
                if (isThreadResult1) {
                  if (generateOldThreadCutoff.hasTrackedUnread(channel.id)) {
                    if (!obj10.ready) {
                      ...
                      obj3[6] = fetchKey;
                      return tmp9(6814).fetchMessages(obj3);
                    }
                  }
                ...
                obj5[0] = channelId;
                const tmp9Result6 = tmp9(6814);
                obj5[1] = tmp11(10057).getMessageLimit("MessageManager.initialFetch");
                obj5[2] = isPreload;
                obj5[3] = skipLocalFetch;
                const obj6 = { jumpType: null };
                obj6[0] = tmp11(4187).JumpType.ANIMATED;
                obj5[4] = obj6;
                ...
                tmp9(6814).jumpToMessage(obj7);
                const tmp9Result7 = tmp9(6814);
              }
              const tmp9Result3 = tmp9(4926);
            }
          }
...
function handleConnectionOpen() {
  ...
  if (null != channelId) {
    ...
    if (null != channel1) {
      ...
      const obj7 = require(4077) /* matchPath */;
...
function handleChannelSectionStoreChange() {
  ...
  if (null != guildId) {
    if (null != channelId) {
      ...
      if (type !== require(5780) /* SidebarType */.SidebarType.VIEW_CHANNEL) {
  ...
  if (!isPreview) {
    ...
    if (Date.now() - num >= closure_20) {
      ...
      if (isStale) {
        ...
        obj[0] = channelId;
        obj[1] = require(10057) /* getMessageLimit */.getMessageLimit("MessageManager.staleFetch");
        obj[2] = jump.jump;
        const messages = obj.fetchMessages(obj);
        const obj3 = require(10057) /* getMessageLimit */;
      }
    }
    tmp = table;
  }
...
function handleAppWillBecomeActive() {
  ...
  if (null == channelId) {
    ...
    const newLocalMessages = importDefault(6814).fetchNewLocalMessages(channelId, closure_13);
  }
}
