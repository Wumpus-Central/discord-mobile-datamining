// === Module 9492: getVisibleMessages ===
  ...
  if ("attachment" === mediaType) {
    ...
    if (null != tmp13) {
      ...
      if (!obj9.isVideoFile(tmp13.filename)) {
        let tmp18Result = tmp18(4443);
        let str4 = "audio";
        if (!tmp18Result.isAudioFile(tmp13.filename)) {
          tmp18Result = tmp18(4443);
          let str5 = "file";
      ...
      obj[4] = content_type;
      tmp17 = obj;
      obj9 = require(4443) /* urlMatchesFileExtension */;
    }
    return tmp17;
    ...
    if (null == tmpResult) {
      ...
      const obj12 = require(4477) /* flattenComponents */;
      const value = require(4477) /* flattenComponents */.flattenComponents(obj.components).get(tmpResult);
      if (null == value) {
        return null;
      ...
      const flattenComponentsResult = require(4477) /* flattenComponents */.flattenComponents(obj.components);
    }
  } else {
    return null;
  }
...
export const handleAddOrRemoveReaction = function handleAddOrRemoveReaction(messageId, channel, reaction, isBurst, MESSAGE) {
  ...
  if (MESSAGE === undefined) {
    MESSAGE = require(7131) /* checkReactionResponse */.ReactionLocations.MESSAGE;
  }
  ...
  if (result) {
    let obj = require(4742) /* shouldShowMembershipVerificationGate */;
    ...
    if (flag) {
      if (true === !reaction.me_burst) {
        ...
        if (!tmp12Result.isPremium(currentUser)) {
          tmp12Result = tmp12(9495);
          return tmp12Result.handleOutOfSuperReactions();
        }
      }
    }
    const ReactionTypes = tmp12(7130).ReactionTypes;
    if (tmp12Result1.isMeReaction(reaction.me, reaction.me_burst, tmp23)) {
      const result2 = tmp12(4224).triggerHapticFeedback(importDefault(4225).IMPACT_LIGHT);
      const tmp12Result2 = tmp12(4224);
      ...
      obj1[0] = flag;
      obj[4] = obj1;
      tmp12(7131).removeReaction(obj);
    } else {
      if (!result) {
        if (channel.isPrivate()) {
          if (!result1) {
            const tmp12Result4 = tmp12(7131);
            const obj2 = { burst: null };
            obj2[0] = flag;
            tmp12Result4.addReaction(channel.id, messageId, reaction.emoji, MESSAGE, obj2);
            if (!flag) {
              const result3 = tmp12(4224).triggerHapticFeedback(importDefault(4225).IMPACT_LIGHT);
              const tmp12Result5 = tmp12(4224);
            }
          }
        }
      }
      if (result) {
        ...
        if (null != guildId1) {
          return tmp12(9323).openMemberVerificationModal(guildId1);
        }
      }
    }
    tmp12Result1 = tmp12(3894);
    tmp23 = flag ? ReactionTypes.BURST : ReactionTypes.NORMAL;
  } else {
    const tmp12Result7 = tmp12(9495);
...
export const handleToggleFollowForumPost = function handleToggleFollowForumPost(channel, outer1_19) {
  const result = require(4224) /* HapticFeedbackTypes */.triggerHapticFeedback(importDefault(4225).IMPACT_LIGHT);
  const obj2 = importDefault(7132);
  if (outer1_19) {
...
export const handleCopyLinkForumPost = function handleCopyLinkForumPost(guildId, id, location) {
  ...
  const result = require(7134) /* trackForumChannelSeenBatch */.trackForumPostLinkCopied(obj);
  if (flag) {
    let tmp4Result = tmp4(4450);
    obj = { media_post_id: null };
    obj[0] = id;
    tmp4Result.trackWithMetadata(constants3.MEDIA_POST_SHARE_PROMPT_CLICKED, obj);
  }
  tmp4Result = tmp4(4224);
  const result1 = tmp4Result.triggerHapticFeedback(importDefault(4225).IMPACT_LIGHT);
  if (null == channel) {
    const tmp4Result1 = tmp4(5609);
    let result2;
    if (true === flag) {
      result2 = tmp9(11).castChannelIdAsMessageId(id);
      const tmp9Result = tmp9(11);
    }
    tmp4Result1.copy(tmp4(4438).getChannelPermalink(guildId, id, result2));
    const tmp4Result2 = tmp4(4438);
  } else {
    const tmp4Result3 = tmp4(5609);
    tmp4Result3.copy(tmp4(4438).getChannelLinkToCopy(channel, channel1));
    const tmp4Result4 = tmp4(4438);
  }
  const obj2 = require(7134) /* trackForumChannelSeenBatch */;
  tmp9 = importDefault;
...
export const findMessageIndex = function findMessageIndex(previousRows, ChatTTITracker) {
  if (null != ChatTTITracker) {
    return require(9622) /* computeScrollData */.findMessageRowIndex(previousRows, ChatTTITracker);
  }
};
...
export const scrollToBottom = function scrollToBottom(current) {
  ...
  importDefault(9623).scrollToBottom(current.current, flag);
};
export const scrollToTop = function scrollToTop(current) {
  ...
  importDefault(9623).scrollToTop(current.current, flag);
};
export const scrollToRelativeOffset = function scrollToRelativeOffset(current) {
  ...
  const result = importDefault(9623).scrollToRelativeOffset(current.current, arg2, flag);
};
export const scrollToTopMessage = function scrollToTopMessage(current, getPreviousRows) {
  ...
  if (previousRows.length > 0) {
    importDefault(9623).scrollTo(current.current, previousRows.length - 1);
    const obj = importDefault(9623);
  }
};
...
export const clearRows = function clearRows(current, clear) {
  ...
  importDefault(9623).clearRows(current.current);
};
export const handleFirstLayout = function handleFirstLayout(arg0, firstVisibleMessageRowIndex, lastVisibleMessageRowIndex, firstVisibleMessagePercentVisible, lastVisibleMessagePercentVisible) {
  arg0({ firstVisibleMessageRowIndex, lastVisibleMessageRowIndex, firstVisibleMessagePercentVisible, lastVisibleMessagePercentVisible, source: require(5102) /* QuestsVisibleMessagesChangedSource */.QuestsVisibleMessagesChangedSource.FIRST_LAYOUT });
};
export const handleMessageVisibilityChanged = function handleMessageVisibilityChanged(arg0, firstVisibleMessageRowIndex, lastVisibleMessageRowIndex, firstVisibleMessagePercentVisible, lastVisibleMessagePercentVisible) {
  arg0({ firstVisibleMessageRowIndex, lastVisibleMessageRowIndex, firstVisibleMessagePercentVisible, lastVisibleMessagePercentVisible, source: require(5102) /* QuestsVisibleMessagesChangedSource */.QuestsVisibleMessagesChangedSource.VISIBILITY_CHANGED });
};
  ...
  if (null == jumpReturnTargetId.jumpReturnTargetId) {
    if (!jumpReturnTargetId.loadingMore) {
      if (jumpReturnTargetId.hasMoreAfter) {
        ...
        obj[1] = closure_21;
        obj[2] = { present: true };
        const messages = importDefault(6814).fetchMessages(obj);
        const obj3 = importDefault(6814);
      } else {
        arg2();
      }
    }
  } else {
    obj = importDefault(6814);
...
export const maybeRescrollToMessageId = function maybeRescrollToMessageId(arg0, jumpType) {
  ...
  if (undefined === INSTANT) {
    INSTANT = _require(4187).JumpType.INSTANT;
  }
  if (null != arg0) {
    const _setTimeout = setTimeout;
