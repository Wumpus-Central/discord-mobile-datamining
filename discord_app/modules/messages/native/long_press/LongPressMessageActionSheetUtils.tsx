...
import openCreateForumPostModal from "openCreateForumPostModal" /* 10265 */;
import createPendingReply from "createPendingReply" /* 11186 */;
import closure_4 from "getState" /* 7372 */;
import { isMessageComponentsV2 } from "hasFlag" /* 4100 */;
import closure_6 from "initialize" /* 7373 */;
import closure_7 from "initialize" /* 7529 */;
import closure_8 from "mergeGuildAvatar" /* 1923 */;
import closure_9 from "getOptions" /* 11185 */;
import ME from "ME" /* 676 */;
import { isStaticChannelRoute } from "set" /* 1398 */;
import { EXPLICIT_MEDIA_FALSE_POSITIVE_ACTION_SHEET_KEY as closure_20 } from "USER_SETTING_ACTION_SHEET_KEY" /* 7293 */;
  ...
  if (isForumPost.isForumPost()) {
    ...
    if (isForumPost.id === obj.castMessageIdAsChannelId(id.id)) {
      if (null != isForumPost.parent_id) {
        ...
        obj[3] = id.id;
        const items = [tmp(5938).FORUM_CHANNEL, tmp(5938).GUILD_CHANNEL];
        obj[5] = items;
...
export const handleCopyMessageLink = function handleCopyMessageLink(channel, message_id) {
  ...
  if (null != channelPermalink) {
    let tmp3Result = tmp3(5946);
    tmp3Result.copy(channelPermalink);
    tmp3Result = tmp3(4162);
  ...
  if (!disabled) {
    ...
    if (intl2.string(result(1236).t["+78Pfm"]) !== label) {
      ...
      if (intl26.string(result(1236).t.n5EBAJ) !== label) {
        ...
        result = id2;
        if (intl27.string(id2(2470)["1D+vqy"]) === label) {
          if (resultResult.canReportMessageToMods(message)) {
            result = result(8697).showReportToModMessageModal(message);
            const resultResult1 = result(8697);
          }
          resultResult = result(6088);
        } else {
          ...
          if (intl28.string(result(1236).t.k5WiPf) === label) {
            ...
            if (intl29.string(result(1236).t["+TSRGD"]) === label) {
              result = result(4457).transitionToMessage(channel.id, message.id);
              const resultResult3 = result(4457);
            } else {
              const intl30 = result(1236).intl;
              if (intl30.string(result(1236).t.zBoHlf) === label) {
                result = result(5946).copy(id2);
                const resultResult4 = result(5946);
                result = result(4162).presentMessageIdCopied();
                const resultResult5 = result(4162);
              } else {
                  ...
                  if (null != result) {
                    if (chatInputRef != null) {
                      ...
                      if (current7 != null) {
                        ...
                        result = current7.insertText("@" + result(4290).getUserTag(result, { decoration: "never" }), null, true);
                        const resultResult6 = result(4290);
                      }
                    }
                    if (chatInputRef != null) {
                  ...
                  if (intl32.string(result(1236).t.cduTBL) === label) {
                    ...
                    if (intl33.string(result(1236).t.fsBWmS) === label) {
                      ...
                      if (intl34.string(result(1236).t.MFGE51) === label) {
                        ...
                        obj1 = { channelId: null };
                        obj1[0] = id;
                        obj[1] = jsx(result(11187), { channelId: null });
                        ...
                        result = result(4822).show(obj);
                        const resultResult7 = result(4822);
                      } else {
                        ...
                        if (intl35.string(result(1236).t.CvQ18w) === label) {
                          ...
                          obj3[0] = message;
                          obj2[2] = jsx(result(11190), { message: null });
                          ...
                          result = result(4822).show(obj2);
                          const resultResult8 = result(4822);
                        } else {
                          ...
                          if (intl36.string(result(1236).t["Bse+F/"]) === label) {
                            ...
                            obj5[0] = message;
                            obj4[2] = jsx(result(11190), { message: null });
                            ...
                            result = result(4822).show(obj4);
                            const resultResult9 = result(4822);
                          } else {
                            const intl37 = result(1236).intl;
                            if (intl37.string(result(1236).t["lE/PG3"]) === label) {
                              const result1 = result(7148).patchMessageGuildOfficial(id, id2, true);
                              const resultResult10 = result(7148);
                            } else {
                              const intl38 = result(1236).intl;
                              if (intl38.string(result(1236).t["2km5Gf"]) === label) {
                                const result2 = result(7148).patchMessageGuildOfficial(id, id2, false);
                                const resultResult11 = result(7148);
                              } else {
                                const intl39 = result(1236).intl;
                                if (intl39.string(result(1236).t.xwMqD7) === label) {
                                  if (message.state === constants6.SENDING) {
                                    result(7525).cancelRequest(id2);
                                    const resultResult12 = result(7525);
                                    result(7148).deleteMessage(id, id2, true);
                                    const resultResult13 = result(7148);
                                  } else if (message.state === tmp86.SEND_FAILED) {
                                    result(7148).deleteMessage(id, id2, true);
                                    const resultResult14 = result(7148);
                                  } else {
                                    id2 = message;
                                    result = null;
                                    if (chatInputRef != null) {
                                    ...
                                    obj7[0] = message;
                                    obj6[2] = jsx(result(11190), { message: null });
                                    ...
                                    result(4822).show(obj6);
                                    const resultResult15 = result(4822);
                                  }
                                  const obj8 = { channel_id: null, guild_id: null, action_sheet_option: "delete", message_state: null };
                                  obj8[0] = id;
                                  obj8[1] = guild_id;
                                  obj8[3] = message.state;
                                  result(4666).trackWithMetadata(constants.MESSAGE_ACTION_SHEET_OPTION_PRESSED, obj8);
                                  const resultResult16 = result(4666);
                                } else {
                                  ...
                                  if (intl40.string(result(1236).t["5911Lb"]) === label) {
                                    ...
                                    result(11193)(channel, message, items, options.getOptions(message.id));
                                    const resultResult17 = result(11193);
                                    const obj9 = { channel_id: null, guild_id: null, action_sheet_option: "retry", message_state: null };
                                    obj9[0] = id;
                                    obj9[1] = guild_id;
                                    obj9[3] = message.state;
                                    result(4666).trackWithMetadata(constants.MESSAGE_ACTION_SHEET_OPTION_PRESSED, obj9);
                                    const resultResult18 = result(4666);
                                  } else {
                                    ...
                                    if (intl41.string(result(1236).t.JrGD7E) === label) {
                                      ...
                                      if (isMessageComponentsV2(contentMessage)) {
                                        const allTextDisplayContent = result(4705).getAllTextDisplayContent(contentMessage.components);
                                        if (null != allTextDisplayContent) {
                                          result(5946).copy(allTextDisplayContent);
                                          const resultResult20 = result(5946);
                                        }
                                        const resultResult19 = result(4705);
                                      } else {
                                        result(5946).copy(contentMessage.content);
                                        const resultResult21 = result(5946);
                                      }
                                      result(4162).presentMessageCopied();
                                      const resultResult22 = result(4162);
                                    } else {
                                      const intl42 = result(1236).intl;
                                      if (intl42.string(result(1236).t.lfIHs4) === label) {
                                        const result3 = result(10830).handleAddNewReactions(channel, id2);
                                        const resultResult23 = result(10830);
                                      } else {
                                        const intl43 = result(1236).intl;
                                        if (intl43.string(result(1236).t.gHp0C4) === label) {
                                          if ("Preview" === actionSheetSource) {
                                            const result4 = result(10830).handleViewPreviewReactions(id2, id);
                                            const resultResult24 = result(10830);
                                          } else {
                                            ...
                                            obj10[2] = obj11;
                                            result(10830).handleViewReactions(obj10);
                                            const resultResult25 = result(10830);
                                          }
                                        } else {
                                          const intl44 = result(1236).intl;
                                          if (intl44.string(result(1236).t.ZbtGBm) === label) {
                                            const result5 = result(10830).handleRemoveAllReactions(id, id2);
                                            const resultResult26 = result(10830);
                                          } else {
                                            ...
                                            if (intl45.string(result(1236).t["g33r/P"]) === label) {
                                              ...
                                              result(4459).openPrivateChannel(obj12);
                                              const resultResult27 = result(4459);
                                            } else {
                                              ...
                                              if (intl46.string(result(1236).t.Xrt5Po) === label) {
                                                ...
                                                const channelPermalink = result(4632).getChannelPermalink(channel.guild_id, channel.id, id2);
                                                if (null != channelPermalink) {
                                                  result = result(5946).copy(channelPermalink);
                                                  const resultResult30 = result(5946);
                                                  result = result(4162).presentLinkCopied();
                                                  const resultResult31 = result(4162);
                                                }
                                                const resultResult29 = result(4632);
                                              } else {
                                                ...
                                                if (intl47.string(result(1236).t.RpE9k7) === label) {
                                                  ...
                                                  if (intl48.string(result(1236).t["S/xNKV"]) === label) {
                                                    ...
                                                    if (null != mediaUrl) {
                                                      result = closure_15;
                                                      selectedMedia = result(4637).urlMatchesFileExtension(selectedMedia.mediaUrl, closure_15);
                                                      const resultResult32 = result(4637);
                                                      result = result(1487).toURLSafe(selectedMedia.mediaUrl);
                                                      if (null != result) {
                                                        if (obj40.isRefreshableAttachmentUrl(result)) {
                                                          let result6 = tmp53(9971).maybeRefreshAttachmentUrl(selectedMedia.mediaUrl);
                                                          const tmp53Result = tmp53(9971);
                                                        }
                                                        obj40 = selectedMedia(9971);
                                                        tmp53 = selectedMedia;
                                                    ...
                                                    if (intl49.string(result(1236).t.JVuuz3) === label) {
                                                      ...
                                                      if (null != mediaUrl1) {
                                                        result = constants5;
                                                        const result7 = result(8514).downloadMediaAssetWithContentType(selectedMedia.mediaUrl, constants5.VIDEO, selectedMedia.contentType);
                                                        result = result7.then(() => {
                                                          id(4162).presentVideoSaved();
                                                        }, () => {
                                                          let obj = id2(4822);
                                                          obj = { title: null, body: null, isDismissable: true };
                                                        ...
                                                        const resultResult34 = result(8514);
                                                      }
                                                    } else {
                                                      ...
                                                      if (intl50.string(result(1236).t.vbAEaA) === label) {
                                                        ...
                                                        if (null != mediaUrl2) {
                                                          ...
                                                          obj14[0] = mediaUrl2;
                                                          result = result(8587).handleClick(obj14);
                                                          const resultResult35 = result(8587);
                                                        }
                                                      } else {
                                                          ...
                                                          if (intl52.string(result(1236).t["8xHmxo"]) !== label) {
                                                            ...
                                                            if (intl53.string(result(1236).t["5IEsGx"]) === label) {
                                                              ...
                                                              if ("Preview" === actionSheetSource) {
                                                                ...
                                                                const resultResult36 = result(4457);
                                                              }
                                                            } else {
                                                              ...
                                                              if (intl54.string(result(1236).t.I3ltXO) === label) {
                                                                ...
                                                                obj16[0] = message;
                                                                result(11197).openForwardModal(obj16);
                                                                const resultResult37 = result(11197);
                                                              } else {
                                                                ...
                                                                if (intl55.string(result(1236).t.rBIGBL) === label) {
                                                                  ...
                                                                  const result8 = result(7456).openThreadCreationForMobile(channel, id, "Message");
                                                                  let result9 = null == message;
                                                                  if (!result9) {
                                                                    const resultResult39 = result(4300);
                                                                  ...
                                                                  const resultResult38 = result(7456);
                                                                } else {
                                                                  ...
                                                                  if (intl56.string(result(1236).t["39d0Wj"]) === label) {
                                                                    ...
                                                                    if (intl57.string(result(1236).t.PHjkRE) === label) {
                                                                      result(4413).hideActionSheet();
                                                                      const resultResult45 = result(4413);
                                                                      const obj17 = { channel: null, commandType: null, commandTargetId: null };
                                                                      obj17[0] = channel;
                                                                      obj17[1] = result(1956).ApplicationCommandType.MESSAGE;
                                                                      obj17[2] = message.id;
                                                                      const result10 = result(4300).navigateToContextMenuCommands(obj17);
                                                                      const resultResult46 = result(4300);
                                                                    } else {
                                                                      ...
                                                                      if (intl58.string(result(1236).t.tpxJto) === label) {
                                                                        ...
                                                                        obj18[0] = id;
                                                                        obj18[1] = id2;
                                                                        obj18[3] = result(11226).SavedMessageSources.LONG_PRESS_ACTION_SHEET;
                                                                        const result11 = result(11221).addOrUpdateSavedMessage(obj18);
                                                                        const resultResult47 = result(11221);
                                                                      } else {
                                                                        ...
                                                                        if (intl59.string(result(1236).t.SvXS1Z) === label) {
                                                                          ...
                                                                          obj19[0] = id;
                                                                          obj19[1] = id2;
                                                                          result(11221).removeSavedMessage(obj19);
                                                                          const resultResult48 = result(11221);
                                                                        } else {
                                                                          ...
                                                                          if (intl60.string(result(1236).t.mJ3P0N) === label) {
                                                                            ...
                                                                            obj20[3] = onBack;
                                                                            result(4413).openLazy(result(2010)(11227, tmp2.paths), "MessageReminderDurationActionSheet", obj20);
                                                                            const resultResult49 = result(4413);
                                                                          } else {
                                                                            ...
                                                                            if (intl61.string(result(1236).t.vrbqs1) === label) {
                                                                              ...
                                                                              obj21[4] = onBack;
                                                                              result(4413).openLazy(result(2010)(11227, tmp2.paths), "MessageReminderDurationActionSheet", obj21);
                                                                              const resultResult50 = result(4413);
                                                                            } else {
                                                                              ...
                                                                              if (intl62.string(result(1236).t.ZH7P2h) === label) {
                                                                                if (null != selectedMedia) {
                                                                                  ...
                                                                                  if (!result12) {
                                                                                    result12 = result(6103).messageHasObscurableMedia(message);
                                                                                    const resultResult51 = result(6103);
                                                                                  }
                                                                                  if (result12) {
                                                                                    ...
                                                                                    obj22[2] = id2;
                                                                                    obj22[3] = id1;
                                                                                    result(4413).openLazy(result(2010)(11194, tmp2.paths), closure_20, obj22);
                                                                                    const resultResult52 = result(4413);
                                                                                  }
                                                                                }
                                                                              } else {
                                                                                ...
                                                                                if (intl63.string(result(1236).t.grdwwt) === label) {
                                                                                  ...
                                                                                  result(11231).endPollEarly({ channelId: null, messageId: null });
                                                                                  const obj23 = { channelId: null, messageId: null };
                                                                                  const resultResult53 = result(11231);
                                                                                } else {
                                                                                  ...
                                                                                  if (intl64.string(result(1236).t.Rjezbz) === label) {
                                                                                    ...
                                                                                    obj24[1] = guild_id;
                                                                                    obj24[2] = onBack;
                                                                                    result(4413).openLazy(result(2010)(11238, tmp2.paths), "AppInteractionInfoActionSheet", obj24);
                                                                                    const resultResult54 = result(4413);
                                                                                  } else {
                                                                                      ...
                                                                                      if (intl66.string(result(1236).t.wUIMqa) !== label) {
                                                                                        ...
                                                                                        if (intl67.string(result(1236).t.kFwAsa) === label) {
                                                                                          ...
                                                                                          if ("attachment" === sourceType) {
                                                                                            ...
                                                                                            result(4822).show(obj25);
                                                                                            const resultResult55 = result(4822);
                                                                                          }
                                                                                        }
                                                                                      }
                                                                                    }
                                                                                    ...
                                                                                    result(4822).show(obj26);
                                                                                    const resultResult56 = result(4822);
                                                                                  }
                                                                                }
                                                                              }
                                                                            }
                                                        ...
                                                        if (flag) {
                                                          result(5946).copy(mediaUrl3);
                                                          const resultResult57 = result(5946);
                                                          result(4162).presentLinkCopied();
                                                          flag = true;
                                                          const resultResult58 = result(4162);
                                                        }
                                                        if (flag) {
                                                          let hostname;
                                                          if (null != mediaUrl3) {
                                                            hostname = result(8590).getHostname(mediaUrl3);
                                                            const resultResult60 = result(8590);
                                                          }
    ...
    if (resultResult61.canReportMessage(message)) {
      result = result(8697).showReportModalForMessage(message, "mobile_message_action_sheet");
      const resultResult62 = result(8697);
    }
    resultResult61 = result(6100);
  }
};