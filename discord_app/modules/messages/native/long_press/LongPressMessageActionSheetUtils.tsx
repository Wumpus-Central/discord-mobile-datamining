...
export const longPressMessageOptionHandler = function longPressMessageOptionHandler(analyticsLocations) {
  ...
  if (!disabled) {
      ...
      if (intl26.string(channel(1115).t.n5EBAJ) !== label) {
        ...
        if (intl27.string(id(2612)["1D+vqy"]) === label) {
          if (tmpResult.canReportMessageToMods(message)) {
            let result = channel(8867).showReportToModMessageModal(message);
            const tmpResult34 = channel(8867);
          }
          tmpResult = channel(7472);
        } else {
          ...
          if (intl28.string(channel(1115).t.k5WiPf) === label) {
            ...
            if (intl29.string(channel(1115).t["+TSRGD"]) === label) {
              channel(4733).transitionToMessage(channel.id, message.id);
              const tmpResult36 = channel(4733);
            } else {
              const intl30 = channel(1115).intl;
              if (intl30.string(channel(1115).t.zBoHlf) === label) {
                channel(7385).copy(id2);
                const tmpResult37 = channel(7385);
                const result1 = channel(4419).presentMessageIdCopied();
                const tmpResult38 = channel(4419);
              } else {
                    ...
                    if (intl33.string(channel(1115).t.fsBWmS) === label) {
                      ...
                      if (intl34.string(channel(1115).t.Y8ujqr) === label) {
                        ...
                        if ("attachment" === sourceType) {
                          ...
                          tmp132(4689).openLazy(channel(1897)(11919, dependencyMap.paths), "EditAttachmentActionSheet", obj3);
                                    ...
                                    if (intl41.string(channel(1115).t["5911Lb"]) === label) {
                                      ...
                                      if (intl42.string(channel(1115).t.JrGD7E) === label) {
                                        ...
                                        if (isMessageComponentsV2(contentMessage)) {
                                          const allTextDisplayContent = channel(4947).getAllTextDisplayContent(contentMessage.components);
                                          if (null != allTextDisplayContent) {
                                            channel(7385).copy(allTextDisplayContent);
                                            const tmpResult40 = channel(7385);
                                          }
                                          const tmpResult39 = channel(4947);
                                        } else {
                                          channel(7385).copy(contentMessage.content);
                                          const tmpResult41 = channel(7385);
                                        }
                                        channel(4419).presentMessageCopied();
                                        const tmpResult42 = channel(4419);
                                      } else {
                                        const intl43 = channel(1115).intl;
                                        if (intl43.string(channel(1115).t.lfIHs4) === label) {
                                          const result4 = channel(11506).handleAddNewReactions(channel, id2);
                                          const tmpResult43 = channel(11506);
                                        } else {
                                          const intl44 = channel(1115).intl;
                                          if (intl44.string(channel(1115).t.gHp0C4) === label) {
                                            if ("Preview" === actionSheetSource) {
                                              const result5 = channel(11506).handleViewPreviewReactions(id2, id);
                                              const tmpResult44 = channel(11506);
                                            } else {
                                              ...
                                              channel(11506).handleViewReactions(obj18);
                                              const tmpResult45 = channel(11506);
                                            }
                                          } else {
                                            const intl45 = channel(1115).intl;
                                            if (intl45.string(channel(1115).t.ZbtGBm) === label) {
                                              const result6 = channel(11506).handleRemoveAllReactions(id, id2);
                                              const tmpResult46 = channel(11506);
                                            } else {
                                              ...
                                              if (intl46.string(channel(1115).t["g33r/P"]) === label) {
                                                ...
                                                if (intl47.string(channel(1115).t.Xrt5Po) === label) {
                                                  ...
                                                  const channelPermalink = channel(4867).getChannelPermalink(channel.guild_id, channel.id, id2);
                                                  if (null != channelPermalink) {
                                                    channel(7385).copy(channelPermalink);
                                                    const tmpResult48 = channel(7385);
                                                    channel(4419).presentLinkCopied();
                                                    const tmpResult49 = channel(4419);
                                                  }
                                                  const tmpResult47 = channel(4867);
                                                } else {
                                                  ...
                                                  if (intl48.string(channel(1115).t.RpE9k7) === label) {
                                                    ...
                                                    if (intl49.string(channel(1115).t["S/xNKV"]) === label) {
                                                      ...
                                                      if (null != mediaUrl) {
                                                        closure_131_2 = channel(4872).urlMatchesFileExtension(selectedMedia.mediaUrl, closure_15);
                                                        const tmpResult50 = channel(4872);
                                                      ...
                                                      if (intl50.string(channel(1115).t.JVuuz3) === label) {
                                                        ...
                                                        if (null != mediaUrl1) {
                                                          const result8 = channel(8497).downloadMediaAssetWithContentType(selectedMedia.mediaUrl, constants6.VIDEO, selectedMedia.contentType);
                                                          result8.then(() => {
                                                          ...
                                                          const tmpResult51 = channel(8497);
                                                        }
                                                      } else {
                                                        ...
                                                        if (intl51.string(channel(1115).t.vbAEaA) === label) {
                                                          ...
                                                          if (null != mediaUrl2) {
                                                            ...
                                                            channel(8602).handleClick(obj22);
                                                            const tmpResult52 = channel(8602);
                                                          }
                                                        } else {
                                                              ...
                                                              if (intl54.string(channel(1115).t["5IEsGx"]) === label) {
                                                                ...
                                                                if (intl55.string(channel(1115).t.I3ltXO) === label) {
                                                                  ...
                                                                  if (intl56.string(channel(1115).t.rBIGBL) === label) {
                                                                    ...
                                                                    if (!result10) {
                                                                      const tmpResult55 = channel(4579);
                                                                    ...
                                                                    if (intl57.string(channel(1115).t["39d0Wj"]) === label) {
                                                                      ...
                                                                      if (intl58.string(channel(1115).t.PHjkRE) === label) {
                                                                        ...
                                                                        const obj25 = { channel, commandType: channel(1895).ApplicationCommandType.MESSAGE, commandTargetId: message.id };
                                                                        const result11 = channel(4579).navigateToContextMenuCommands(obj25);
                                                                        const tmpResult58 = channel(4579);
                                                                      } else {
                                                                        const intl59 = channel(1115).intl;
                                                                        if (intl59.string(channel(1115).t.tpxJto) === label) {
                                                                          const obj26 = { channelId: id, messageId: id2, displayToast: true, source: channel(11951).SavedMessageSources.LONG_PRESS_ACTION_SHEET };
                                                                          const result12 = channel(11946).addOrUpdateSavedMessage(obj26);
                                                                          const tmpResult59 = channel(11946);
                                                                        } else {
                                                                          ...
                                                                          if (intl60.string(channel(1115).t.SvXS1Z) === label) {
                                                                            ...
                                                                            channel(11946).removeSavedMessage(obj27);
                                                                            const tmpResult60 = channel(11946);
                                                                          } else {
                                                                            ...
                                                                            if (intl61.string(channel(1115).t.mJ3P0N) === label) {
                                                                              ...
                                                                              obj28.onBack = onBack;
                                                                              tmp132(4689).openLazy(channel(1897)(11952, dependencyMap.paths), "MessageReminderDurationActionSheet", obj28);
                                                                              ...
                                                                              if (intl62.string(channel(1115).t.vrbqs1) === label) {
                                                                                ...
                                                                                obj29.onBack = onBack;
                                                                                tmp132(4689).openLazy(channel(1897)(11952, dependencyMap.paths), "MessageReminderDurationActionSheet", obj29);
                                                                                ...
                                                                                if (intl63.string(channel(1115).t.ZH7P2h) === label) {
                                                                                  if (null != selectedMedia) {
                                                                                    ...
                                                                                    if (!result13) {
                                                                                      result13 = channel(7488).messageHasObscurableMedia(message);
                                                                                      const tmpResult61 = channel(7488);
                                                                                    }
                                                                                    if (result13) {
                                                                                      ...
                                                                                      obj30.embedId = id3;
                                                                                      tmp132(4689).openLazy(channel(1897)(11917, dependencyMap.paths), closure_20, obj30);
                                                                                  ...
                                                                                  if (intl64.string(channel(1115).t.grdwwt) === label) {
                                                                                    ...
                                                                                    if (intl65.string(channel(1115).t.Rjezbz) === label) {
                                                                                      ...
                                                                                      tmp132(4689).openLazy(channel(1897)(11969, dependencyMap.paths), "AppInteractionInfoActionSheet", obj32);
                                                          ...
                                                          if (flag) {
                                                            channel(7385).copy(mediaUrl3);
                                                            const tmpResult62 = channel(7385);
                                                            channel(4419).presentLinkCopied();
                                                            flag = true;
                                                            const tmpResult63 = channel(4419);
                                                          }
                                                          if (flag) {
                                                            let hostname;
                                                            if (null != mediaUrl3) {
                                                              hostname = channel(8605).getHostname(mediaUrl3);
                                                              const tmpResult64 = channel(8605);
                                                            }
    ...
    if (tmpResult65.canReportMessage(message)) {
      const result14 = channel(8867).showReportModalForMessage(message, "mobile_message_action_sheet");
      const tmpResult66 = channel(8867);
    }
    tmpResult65 = channel(7485);
  }
};