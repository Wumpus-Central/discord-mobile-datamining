// === Module 10539: OptInChannelBanner ===

// Module 10539 (OptInChannelBanner)
import ThemesDefault from "Themes" /* 712 */;
import ChatOverlaysDefault from "ChatOverlays" /* 10542 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  obj[4] = stringResult;
  const items1 = [closure_12(channel(4440).Text, obj), ];
  if (canUnarchiveThread) {
    ...
    obj1[1] = tmp8(tmp3(4879).Button, obj2);
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj[3] = callback(XSmallBoldIcon(4440).Text, obj);
      const items1 = [callback(XSmallBoldIcon(5015).PressableOpacity, obj), ];
      handleScrollToNewMessages = XSmallBoldIcon(5015).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8300).XSmallBoldIcon;
