...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  obj[4] = stringResult;
  const items1 = [closure_12(channel(4298).Text, obj), ];
  if (canUnarchiveThread) {
    ...
    obj1[1] = tmp8(tmp3(4712).Button, obj2);
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj[3] = callback(XSmallBoldIcon(4298).Text, obj);
      const items1 = [callback(XSmallBoldIcon(4844).PressableOpacity, obj), ];
      handleScrollToNewMessages = XSmallBoldIcon(4844).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8020).XSmallBoldIcon;
