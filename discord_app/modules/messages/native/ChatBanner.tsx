...
import useShowChannelOptInNoticeDefault from "useShowChannelOptInNotice" /* 11689 */;
import useAllowedChatOverlaysDefault from "useAllowedChatOverlays" /* 11690 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  const items1 = [closure_12(channel(4753).Text, obj3), ];
  if (canUnarchiveThread) {
    ...
    obj4.children = closure_12(tmp3(5187).Button, obj5);
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj2.children = closure_12(XSmallBoldIcon(4753).Text, obj3);
      ...
      XSmallBoldIcon = XSmallBoldIcon(8235).XSmallBoldIcon;
