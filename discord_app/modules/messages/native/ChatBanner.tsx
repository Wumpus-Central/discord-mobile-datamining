...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  if (canUnarchiveThread) {
    ...
    obj1[1] = tmp8(tmp3(4665).Button, obj2);
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      const items1 = [callback(XSmallBoldIcon(4797).PressableOpacity, obj), ];
      handleScrollToNewMessages = XSmallBoldIcon(4797).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(7862).XSmallBoldIcon;
