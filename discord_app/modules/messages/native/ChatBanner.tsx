...
import useShowChannelOptInNoticeDefault from "useShowChannelOptInNotice" /* 11765 */;
import useAllowedChatOverlaysDefault from "useAllowedChatOverlays" /* 11766 */;
...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  let canUnarchiveThread = channel(7597).useCanUnarchiveThread(channel);
  ...
  const items1 = [closure_12(channel(4823).Text, obj3), ];
  if (canUnarchiveThread) {
    ...
    obj4.children = closure_12(tmp3(5271).Button, obj5);
...
function LockedThreadChatBanner(channel) {
  ...
  let isThreadModerator = channel(7597).useIsThreadModerator(channel);
  let obj2 = { style: tmp.threadBannerContainer, children: null };
  let obj3 = { lineClamp: 4, style: tmp.threadBannerTitle, variant: "text-sm/medium", color: "text-default", children: null };
  let obj = channel(7597);
  const intl = channel(1115).intl;
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj2.children = closure_12(XSmallBoldIcon(4823).Text, obj3);
      const items1 = [closure_12(XSmallBoldIcon(5425).PressableOpacity, obj2), ];
      handleScrollToNewMessages = XSmallBoldIcon(5425).PressableOpacity;
      ...
      XSmallBoldIcon = XSmallBoldIcon(8317).XSmallBoldIcon;
