...
import WelcomeCTAUtils from "WelcomeCTAUtils" /* 8345 */;
...
import useGuildProfileCTA from "useGuildProfileCTA" /* 10111 */;
...
import ExperimentEmbedPlatformUtils from "ExperimentEmbedPlatformUtils" /* 12141 */;
import PremiumGiftingIntentUtils from "PremiumGiftingIntentUtils" /* 12150 */;
import system_message_GuildRoleSubscriptionSystemMessageUtils from "system_message/GuildRoleSubscriptionSystemMessageUtils" /* 12163 */;
import showModerateUserActionSheetDefault from "showModerateUserActionSheet" /* 12168 */;
...
import VoiceMessageAnalytics from "VoiceMessageAnalytics" /* 12209 */;
import MediaAnalytics from "MediaAnalytics" /* 12210 */;
...
import getInlineForwardOptions from "getInlineForwardOptions" /* 12265 */;
...
class MessagesHandlers {
  constructor(arg0) {
      ...
      if (null != messageData) {
        ...
        if (null != messageId) {
          if (tmpResult.isPublicSystemMessage(message)) {
            ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11980, dependencyMap.paths), "PublicGuildAnnouncementProfile");
          } else {
            let user1 = user;
            if (null == user) {
            ...
            if (isCrosspostDefault(message)) {
              ...
              if (null != guild_id) {
                ...
                tmp10Result.openLazy(asyncRequireImpl(10093, dependencyMap.paths), "GuildProfileActionSheet:" + guild_id, obj2);
                const tmp20 = asyncRequireImpl(10093, dependencyMap.paths);
              }
            }
            tmp7 = user1;
                    ...
                    if (isCrosspostDefault(messageByReference.message)) {
                      ...
                      if (null != messageByReference.message.messageReference) {
                        ...
                        if (null != messageByReference.message.messageReference.guild_id) {
                          ...
                          tmp10Result2.openLazy(asyncRequireImpl(10093, dependencyMap.paths), "GuildProfileActionSheet:" + guild_id2, obj3);
                          const tmp17 = asyncRequireImpl(10093, dependencyMap.paths);
                        }
                      }
                    }
                  }
              ...
              if (null != applicationActivity) {
                ...
                if (null != session_id) {
                  ...
                  if (null != application1) {
                    obj = { presenceActivity: applicationActivity, currentUserPresenceActivity: getCurrentUserPresenceActivityDefault(LocalActivityStore, closure_1_32, id), currentUserId: id.getId(), message, application: application1, isEmbeddedApplication: hasFlagResult, isFrameApplication: false, isGameLaunchable: flag };
    ...
    obj._questsEmbedOnPress = function _questsEmbedOnPress(code) {
      ...
      if (!obj.isMetaQuest()) {
        const result = tmp(dependencyMap[105]).findQuestOrReplacement(code, QuestStore.quests, QuestStore.excludedQuests);
        if (null != result) {
          const obj2 = { scrollToQuestId: result.id, fromContent: tmp(dependencyMap[107]).QuestContent.QUEST_EMBED_MOBILE };
        ...
        if (null != result) {
          if (null == QuestStore.questEnrollmentBlockedUntil) {
            if (!QuestStore.isQuestAccessSuspended) {
              ...
              if (!tmp9) {
                const obj2 = { questContent: tmp(dependencyMap[107]).QuestContent.QUEST_EMBED_MOBILE, questContentCTA: tmp(dependencyMap[109]).QuestContentCTA.ACCEPT_QUEST, sourceQuestContent: tmp(dependencyMap[107]).QuestContent.QUEST_EMBED_MOBILE };
                tmp(dependencyMap[108]).enrollInQuest(result.id, obj2);
                const tmpResult7 = tmp(dependencyMap[108]);
              }
              const obj3 = { scrollToQuestId: result.id, fromContent: tmp(dependencyMap[107]).QuestContent.QUEST_EMBED_MOBILE };
              tmp(dependencyMap[106]).openQuestHome(obj3);
              const tmpResult8 = tmp(dependencyMap[106]);
            }
          }
          const obj4 = { scrollToQuestId: result.id, fromContent: tmp(dependencyMap[107]).QuestContent.QUEST_EMBED_MOBILE };
            ...
            if (null != invite) {
              ...
              if (tmpResult24.isGuildScheduledEventInviteEmbed(invite)) {
                ...
                if (null != id4) {
                  ...
                  obj20.openLazy(asyncRequireImpl(10093, dependencyMap.paths), combined, obj14);
                  str8 = "show profile";
                  const tmp42 = asyncRequireImpl(10093, dependencyMap.paths);
    ...
    obj.handleGiftIntentCardViewed = function handleGiftIntentCardViewed(nativeEvent) {
      ...
      if (null != parseGiftIntentTypeResult) {
        const obj3 = { name: tmp(dependencyMap[141]).ImpressionNames.GIFT_INTENT_CARD, type: tmp(dependencyMap[141]).ImpressionTypes.VIEW, properties: null };
    ...
    obj.handleTapRoleIcon = function handleTapRoleIcon(nativeEvent) {
      ...
      if (roleName.startsWith(name(dependencyMap[142]).LEADERBOARD_WINNER_ROLE_NAME_PREFIX)) {
        const decodeWinnerDataResult = tmp(dependencyMap[142]).decodeWinnerData(roleName.slice(tmp(dependencyMap[142]).LEADERBOARD_WINNER_ROLE_NAME_PREFIX.length));
      ...
      if (null != messageData) {
        ...
        if (null != value) {
          ...
          if (Server.ComponentType.STRING_SELECT === type) {
            ...
            obj9.openLazy(asyncRequireImpl(12156, dependencyMap.paths), combined, obj3);
            const tmp19 = asyncRequireImpl(12156, dependencyMap.paths);
          } else {
              ...
              if (Server.ComponentType.ROLE_SELECT !== type) {
                if (Server.ComponentType.MENTIONABLE_SELECT !== type) {
                  if (Server.ComponentType.CHANNEL_SELECT === type) {
                    ...
                    obj12.openLazy(asyncRequireImpl(12162, dependencyMap.paths), combined1, obj4);
                    const tmp28 = asyncRequireImpl(12162, dependencyMap.paths);
                  }
                }
              }
            }
            ...
            obj7.openLazy(asyncRequireImpl(12158, dependencyMap.paths), combined2, obj5);
            const tmp11 = asyncRequireImpl(12158, dependencyMap.paths);
          }
            ...
            if (null != channel) {
              if (tmpResult5.isAutomodMessageRecord(message)) {
                if (tmpResult6.isAutomodNotification(message)) {
                  ...
                  ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12196, dependencyMap.paths), "GuildRaidResolveActionSheet", obj2);
                }
    ...
    obj.handleTapCtaButton = function handleTapCtaButton(nativeEvent) {
      ...
      if (obj(dependencyMap[184]).CtaButtonType.MARK_AS_FALSE_POSITIVE === callback) {
        ...
        const result2 = tmp(dependencyMap[179]).trackAgeVerificationDmClicked(tmp(dependencyMap[179]).AgeVerificationDmCta.RETRY, channelId);
    ...
    obj.handleTapClanTagChiplet = function handleTapClanTagChiplet(nativeEvent) {
      ...
      if (tmp4) {
        ...
        obj2.openLazy(tmp(dependencyMap[38])(dependencyMap[68], dependencyMap.paths), "GuildProfileActionSheet:" + nativeSyntheticEventData.guildId, obj3);
