// === Module 11797: handleMessagesTapImage ===
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (null != portal) {
    embedId(8539).markPortalAlive(portal);
    const obj = embedId(8539);
  }
  if (true === allowWithinModal) {
    ...
    if (type1 !== embedId(1097).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((item) => !embedId(8536).isThumbnailAttachment(item));
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = embedId(8536);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          const tmp10Result7 = embedId(8536);
          ...
          if (importDefault < result.length) {
            embedId(8536).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = embedId;
            const tmp10Result8 = embedId(8536);
          }
        }
        if (-1 !== importDefault) {
          ...
          tmp25(8530).openMediaModal(obj3);
          const tmp25Result = tmp25(8530);
        }
      }
      if ("component" === type) {
        if (null == componentId) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = embedId(8536);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, embedId(4987).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  embedId(8536).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = embedId;
                  const tmp10Result11 = embedId(8536);
                }
                const tmp10Result10 = embedId(4987);
              }
            }
          }
        ...
        const result2 = embedId(8536).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        let num2 = 0;
        found2 = result2;
        tmp25 = embedId;
        if (0 < result2.length) {
          while (true) {
            let obj4 = embedId(8536);
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            embedId(8536).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = embedId;
            const tmp20Result = embedId(8536);
          } else {
            importDefault = num2;
            embedId(8536).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = embedId;
            const tmp20Result2 = embedId(8536);
          }
        }
        const tmp10Result12 = embedId(8536);
      }
    } else {
