// === Module 11079: handleMessagesTapImage ===
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (true === allowWithinModal) {
    ...
    if (type !== embedId(694).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((flags) => !embedId(8514).isThumbnailAttachment(flags));
      if ("attachment" === type) {
        if (index < found.length) {
          let tmp10Result = tmp10(8514);
          if (null == tmp10Result.extractMediaFromAttachment(found[index], tmp7, index, messageChannel.guild_id)) {
            if (null != tmp15.url) {
              if ("" !== tmp15.url) {
                num2(4159).openURL(tmp15.url);
              }
            }
          }
        }
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (num2 < result.length) {
            tmp10(8514).setMediaSourcePortal(result[num2], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result1 = tmp10(8514);
          }
        }
        if (-1 !== num2) {
          let tmp25Result = tmp25(8512);
          ...
          let channelName;
          if (showContextName) {
            tmp25Result = tmp25(4640);
            channelName = tmp25Result.computeChannelName(messageChannel, closure_5, closure_4, false);
          }
          obj[6] = channelName;
          let channelIcon;
          if (showContextName) {
            channelIcon = tmp25(4946).getChannelIcon(messageChannel);
            const tmp25Result1 = tmp25(4946);
          }
          obj[7] = channelIcon;
      ...
      if ("component" === type) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result2 = tmp10(8514);
                const result1 = tmp10Result2.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, tmp10(4711).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: num2 } = result1);
                  tmp10(8514).setMediaSourcePortal(sources[num2], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result4 = tmp10(8514);
                }
                const tmp10Result3 = tmp10(4711);
              }
            }
          }
        ...
        const result2 = tmp10(8514).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        num2 = 0;
        found2 = result2;
        tmp25 = tmp10;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            num2 = 0;
            let tmp20Result = tmp20(8514);
            tmp20Result.setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
          } else {
            tmp20Result = tmp20(8514);
