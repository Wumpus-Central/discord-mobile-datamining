...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (true === allowWithinModal) {
    ...
    if (type !== embedId(694).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((flags) => !embedId(8769).isThumbnailAttachment(flags));
      if ("attachment" === type) {
        if (index < found.length) {
          let tmp10Result = tmp10(8769);
          if (null == tmp10Result.extractMediaFromAttachment(found[index], tmp7, index, messageChannel.guild_id)) {
            if (null != tmp15.url) {
              if ("" !== tmp15.url) {
                num2(3981).openURL(tmp15.url);
              }
            }
          }
        }
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (num2 < result.length) {
            tmp10(8769).setMediaSourcePortal(result[num2], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result1 = tmp10(8769);
          }
        }
        if (-1 !== num2) {
          let tmp25Result = tmp25(8767);
      ...
      if ("component" === type) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                ...
                if (null != result1) {
                  ...
                  tmp10(8769).setMediaSourcePortal(sources[num2], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result4 = tmp10(8769);
                }
        ...
        const result2 = tmp10(8769).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        num2 = 0;
        found2 = result2;
        tmp25 = tmp10;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            num2 = 0;
            let tmp20Result = tmp20(8769);
            tmp20Result.setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
          } else {
            tmp20Result = tmp20(8769);
