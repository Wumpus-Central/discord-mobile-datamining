    ...
    if (type1 !== embedId(1097).MessageReferenceTypes.FORWARD) {
      ...
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = embedId(8611);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          const tmp10Result7 = embedId(8611);
          ...
          if (importDefault < result.length) {
            embedId(8611).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = embedId;
            const tmp10Result8 = embedId(8611);
          }
        }
        if (-1 !== importDefault) {
      ...
      if ("component" === type) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = embedId(8611);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, embedId(5058).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  embedId(8611).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = embedId;
                  const tmp10Result11 = embedId(8611);
                }
                const tmp10Result10 = embedId(5058);
              }
            }
          }
        ...
        const result2 = embedId(8611).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        let num2 = 0;
        found2 = result2;
        tmp25 = embedId;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            embedId(8611).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = embedId;
            const tmp20Result = embedId(8611);
          } else {
            importDefault = num2;
            embedId(8611).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = embedId;
            const tmp20Result2 = embedId(8611);
          }
        }
        const tmp10Result12 = embedId(8611);
      }
    } else {
