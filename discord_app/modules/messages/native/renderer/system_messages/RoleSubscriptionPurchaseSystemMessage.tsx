// === Module 7927: createRoleSubscriptionPurchaseSystemMessage ===
...
export const createRoleSubscriptionPurchaseSystemMessage = function createRoleSubscriptionPurchaseSystemMessage(message) {
  ...
  if (null == roleSubscriptionData) {
    ...
    let obj1 = require(7867) /* getMessageAuthorWithProcessedColor */;
    ...
    if (null != guildId) {
      if (null != channel) {
        ...
        let tmp5Result = tmp5(7902);
        if (tmp5Result.computeIsStickerReplyEnabled(guildId, channel, message, tmp9)) {
          tmp5Result = tmp5(7903);
          const tmp5Result1 = tmp5(7897);
          const transformStickerResult = tmp5Result.transformSticker(tmp5(7897).pickRoleSubscriptionPurchaseSticker(message.id));
        }
      }
    }
    ...
    if (null != guildMemberAvatar) {
      ...
      obj3[2] = tmp;
      obj1[2] = importDefault(7869)(obj3);
      obj1[3] = obj;
      obj1[4] = roleSubscriptionData;
      obj2[0] = tmp5(7897).getRoleSubscriptionPurchaseSystemMessageContentMobile(obj1);
      ...
      const tmp22 = guildId;
      const tmp5Result4 = tmp5(7897);
      obj2[5] = tmp5(7897).getRoleSubscriptionPurchaseStickerCTA(message.id, false);
