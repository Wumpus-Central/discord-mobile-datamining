...
const result = obj132.fileFinishedImporting("modules/messages/native/renderer/ChatManager.tsx");
class ChatManager {
  constructor() {
    ...
    if (Array.isArray(self._messages)) {
      ...
      if (null != tmp) {
        if (!forceRender.forceRender) {
          ...
          if (!hasItem) {
            INSERT = isUndefinedOrNullDefault(tmp, message) ? constants.NOOP : constants.UPDATE;
          }
        }
...
prototype["createChangeset"] = function createChangeset() {
  ...
  if (null == this._messages) {
    ...
    rows = items.filter((item, index) => item.changeType !== constants.NOOP);
  }
