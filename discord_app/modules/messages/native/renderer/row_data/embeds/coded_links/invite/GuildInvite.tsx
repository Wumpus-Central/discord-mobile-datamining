// === Module 12876: createResolvingGuildInvite ===
...
import CodedLinkExtendedType2 from "CodedLinkExtendedType" /* 11151 */;
...
import getHeaderTextForInvite from "getHeaderTextForInvite" /* 12877 */;
import GuildNSFWContentLevel from "GuildNSFWContentLevel" /* 1434 */;
import closure_6 from "reset" /* 4466 */;
import closure_7 from "ensureGuildLoaded" /* 1391 */;
import closure_8 from "trackCommunicationDisabled" /* 1992 */;
import closure_9 from "createGuildRecordFromRust" /* 1910 */;
import closure_10 from "updateInvite" /* 4428 */;
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, closure_2) {
  ...
  let tmpResult = tmp(11153);
  const items = [closure_8];
  const tmp3 = getEmbedThemeColorsDefault(closure_2);
  const channel = tmp(11152)(invite).channel;
  ...
  if (tmp7) {
    if (flag2) {
      if (null != target_user) {
        if (tmp12) {
          ...
          obj2[0] = tmp(4288).getFormattedName(target_user);
          const tmpResult3 = tmp(4288);
    ...
    if (null != channel) {
      if (channel.type === constants2.GUILD_STAGE_VOICE) {
        const assetSource1 = Image.resolveAssetSource(tmp10(4932).getChannelIcon(channel));
        let uri1;
    ...
    if (null == approximate_member_count) {
      ...
      tmp25 = name;
      if (null != channel) {
        const assetSource2 = Image.resolveAssetSource(tmp10(4932).getChannelIcon(channel));
        let uri2;
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(9480).getGuildBadgeImageSource(guild, closure_2);
      const tmp10Result3 = tmp10(9480);
    }
    const obj6 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    let channelName;
    if (null != channel) {
      channelName = tmp10(4638).computeChannelName(channel, closure_12, closure_11);
      const tmp10Result4 = tmp10(4638);
    }
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(8273).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result5 = tmp10(8273);
    }
