// === Module 11313: ExperimentOverrideActionSheet ===
...
import { ExperimentEmbedType } from "ExperimentBuckets" /* 4360 */;
...
export const createExperimentEmbed = function createExperimentEmbed(url, closure_2) {
  ...
  let obj = experimentTreatmentFromEmbedURL(7580);
  const experimentFromEmbedURL = obj.getExperimentFromEmbedURL(url);
  obj1 = experimentTreatmentFromEmbedURL(7580);
  ...
  const legacyExperiments = experimentTreatmentFromEmbedURL(11018).getLegacyExperiments();
  ({ experiments, overridesInfo } = legacyExperiments);
  const obj3 = experimentTreatmentFromEmbedURL(11018);
  const apexExperiments = experimentTreatmentFromEmbedURL(11019).getApexExperiments();
  let tmp10 = null;
  ...
  if (null != experimentFromEmbedURL) {
    if (null != tmp10) {
      ...
      id = id.getId();
      tmp4Result = tmp4(11316);
      const experimentServerAssignment = tmp4Result.getExperimentServerAssignment(tmp10, id);
      const experimentServerAssignmentLabel = tmp4(7580).getExperimentServerAssignmentLabel(tmp10, experimentServerAssignment);
      if (EXPERIMENT === tmp13.EXPERIMENT_TREATMENT) {
        ...
        obj.thumbnailUrl = tmp4(7802).getAssetUriForEmbed(tmp(11317));
  ...
  const obj4 = experimentTreatmentFromEmbedURL(11019);
