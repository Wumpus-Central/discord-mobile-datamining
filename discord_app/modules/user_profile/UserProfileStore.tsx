...
function createUserWidgetFromServer(data) {
      ...
      if (WidgetType.WidgetType.PLAYED_GAMES !== type) {
        if (WidgetType.WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (WidgetType.WidgetType.APPLICATION === type) {
            ...
            obj3.header = str;
            obj3.sections = UserProfilePersonalWidget.parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new UserProfilePersonalWidget.UserProfilePersonalWidget(obj3);
            ...
            obj.clips = mapped.filter(GlobalUtils.isNotNullish);
  ...
  const baseGameWidget = new UserProfileGameWidgetTypes.BaseGameWidget({
    id: data.id,
    type,
    games: _modDef12.uniqBy(mapped1, "gameId"),
  });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new UserProfileGameWidgetTypes.BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
      ...
      if (null != expiresAt.expiresAt) {
        ...
        if (diff <= 0) {
          ...
          if (null == dependencyMap[closure_0][tmp5][expiresAt.skuId]) {
            ...
            dependencyMap[closure_0][tmp6][expiresAt.skuId] = timeout;
          }
          if (tmp2 == null) {
            tmp2 = closure_10;
          }
...
function handleProfileFetch(arg0) {
  ...
  if (null != userProfile.mutual_guilds) {
    ...
    const result = map5.set(
      userProfile.user.id,
      found.map((item) => ({ guild: set[item].guild, nick: set[item].nick })),
    );
  }
    ...
    let mapped1 = badges.map((id) => {
      ...
      if ("premium" === id.id) {
        if (null != date) {
          ...
          const obj2 = { date };
          let formatToPlainStringResult = intl2.formatToPlainString(util.t["8zbGNR"], obj2);
          if (null != tieredTenureBadgeData) {
            const intl3 = util.intl;
            const obj3 = { date };
            formatToPlainStringResult = intl3.formatToPlainString(util.t.Hu4jfi, obj3);
          }
          const obj4 = {};
          const merged = Object.assign(id);
      ...
      if (id.startsWith("guild_booster_lvl")) {
        ...
        if (null != date1) {
          ...
          obj5.description = intl.formatToPlainString(util.t.IWkAq7, obj6);
          tmp7 = obj5;
        }
      }
      return tmp7;
  ...
  if (null != application) {
    ({
      id: obj3.id,
      primary_sku_id: obj3.primarySkuId,
      custom_install_url: obj3.customInstallUrl,
      ...
      name: obj3.name,
      terms_of_service_url: obj3.termsOfServiceUrl,
      privacy_policy_url: obj3.privacyPolicyUrl,
    } = application);
    tmp42 = {
      id: null,
      primarySkuId: null,
...
function handleProfileUpdateSuccess(guild_id) {
  ...
  if (null != guild_id.guild_id) {
    ...
    if (null != value4) {
      ...
      const result1 = map1.set(userId2, obj3);
...
function handleWidgetsUpdateSuccess(arg0) {
  ...
  if (null == value) {
    ...
    const result = map1.set(userId, obj2);
  }
}
