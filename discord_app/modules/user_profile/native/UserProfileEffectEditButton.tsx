...
const createCacheKey = { height: COLLECTIBLES_PREVIEW_SIZE, width: COLLECTIBLES_PREVIEW_SIZE, borderRadius: ThemesDefault.radii.xs, backgroundColor: ThemesDefault.colors.BACKGROUND_MOD_MUTED, overflow: "hidden" };
...
const result = require("obj132").fileFinishedImporting("modules/user_profile/native/UserProfileEffectEditButton.tsx");
...
export default function UserProfileEffectEditButton(isTryItOut) {
  ...
  if (isFetching) {
    ...
    obj1[0] = intl4.string(user(tmp3[13]).t.wR5wOo);
    const intl5 = user(tmp3[13]).intl;
    obj1[1] = intl5.string(user(tmp3[13]).t.MKDeyL);
    obj1[2] = NOOP;
    obj1[3] = callback(user(tmp3[15]).ActivityIndicator, { animating: true, size: "large" });
    let obj2 = obj1;
  } else {
    obj2 = { label: null, buttonText: null, accessibilityValue: null, onPress: null, leading: null };
    const intl3 = user(tmp3[13]).intl;
    obj2[0] = intl3.string(user(tmp3[13]).t.wR5wOo);
    ...
    if (null != profilePreviewValue) {
      ...
      obj8[1] = tmp.noneIcon;
      tmp13Result = callback(user(tmp3[19]).Icon, obj8);
    }
    obj2[4] = tmp13Result;
  }
