// === Module 8096: EditCollectiblesPreviewDescription ===
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  let obj2 = require(6910) /* getItemRecordsFromPurchases */;
  ...
  if (null != purchase) {
    if (!result1) {
      ...
      if (null != expiresAt) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(3925).diffAsUnits(date, purchase.expiresAt);
        const tmpResult1 = tmp(3925);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = callback(tmp(4251).Text, obj);
      ...
      items1[1] = callback(tmp(4251).Text, obj1);
      let expiresAt1;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4251).Text, obj3);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      obj6[2] = stringResult;
      let tmp25Result = callback(tmp(4251).Text, obj6);
    } else {
      const tmp4Result = importDefault(3901);
      const Text = tmp(4251).Text;
...
export default function EditCollectiblesPreviewDetails(previewSkuId) {
  ...
  const tmp = callback2();
  ({ product, purchase } = importDefault(8076)(previewSkuId));
  let tmp5Result = null;
  if (null != previewSkuId) {
    if (null != product) {
      ...
      obj[2] = name;
      const items = [closure_5(require(4251) /* Text */.Text, obj), ];
