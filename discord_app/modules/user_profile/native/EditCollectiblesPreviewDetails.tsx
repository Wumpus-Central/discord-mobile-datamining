...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  let obj2 = require(5815) /* getItemRecordsFromPurchases */;
  ...
  if (null != purchase) {
    if (!result1) {
      ...
      if (null != expiresAt) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(3863).diffAsUnits(date, purchase.expiresAt);
        const tmpResult1 = tmp(3863);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = callback(tmp(4189).Text, obj);
      ...
      items1[1] = callback(tmp(4189).Text, obj1);
      let expiresAt1;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4189).Text, obj3);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      obj6[2] = stringResult;
      let tmp25Result = callback(tmp(4189).Text, obj6);
    } else {
      const tmp4Result = importDefault(3839);
      const Text = tmp(4189).Text;
...
export default function EditCollectiblesPreviewDetails(previewSkuId) {
  ...
  const tmp = callback2();
  ({ product, purchase } = importDefault(7959)(previewSkuId));
  let tmp5Result = null;
  if (null != previewSkuId) {
    if (null != product) {
      ...
      obj[2] = name;
      const items = [closure_5(require(4189) /* Text */.Text, obj), ];
