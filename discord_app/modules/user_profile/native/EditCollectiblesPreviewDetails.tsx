// === Module 8452: EditCollectiblesPreviewDescription ===
...
import useCollectiblesDataDefault from "useCollectiblesData" /* 8432 */;
...
function EditCollectiblesPreviewDescription(arg0) {
  ...
  if (null != purchase) {
    if (!result1) {
      ...
      if (null != expiresAt) {
        const _Date = Date;
        const date = new Date();
        diffAsUnitsResult = tmp(4067).diffAsUnits(date, purchase.expiresAt);
        const tmpResult1 = tmp(4067);
      }
      ...
      if (tmp17) {
        ...
        tmp17 = callback(tmp(4376).Text, obj);
      ...
      items1[1] = callback(tmp(4376).Text, obj1);
      let expiresAt1;
      if (purchase != null) {
      ...
      if (tmp19Result) {
        ...
        tmp19Result = tmp19(tmp(4376).Text, obj3);
      }
  ...
  if (result) {
    if (canUseCollectiblesResult) {
      ...
      obj6[2] = stringResult;
      let tmp25Result = callback(tmp(4376).Text, obj6);
    } else {
      const tmp4Result = getPremiumPlanItemDefault;
      const Text = tmp(4376).Text;
    ...
    return callback(tmp(4376).Text, obj8);
  }
}
noopAll;
