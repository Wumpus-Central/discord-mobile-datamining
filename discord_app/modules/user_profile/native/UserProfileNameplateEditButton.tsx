// === Module 14916: UserProfileNameplateEditButton ===
...
export default function UserProfileNameplateEditButton(user) {
  ...
  const profilePreviewValue = user(8439).getProfilePreviewValue(obj3);
  const obj2 = user(8439);
  let skuId;
  if (profilePreviewValue != null) {
    skuId = profilePreviewValue.skuId;
  }
  const fetchNameplate = user(14917).useFetchNameplate(skuId);
  ...
  if (isFetching) {
    ...
    return jsx(tmp3(14900).UserProfileEditFormButton, { label: null, buttonText: null, onPress: null, leading: null, loading: true, disabled: true, hideArrow: true });
  } else {
    let name;
    ...
    if (null != nameplateData) {
      if (null != nameplateRecord) {
        if (null != nameplateProduct) {
          ...
          obj8.children = jsx(guildId(9104), { nameplate: nameplateData, fullOpacity: true, isSquarePreview: true });
          let tmp15Result = <closure_5 style={tmp.previewContainer}>{null}</closure_5>;
        }
        obj6.leading = tmp15Result;
        return jsx(tmp3(14900).UserProfileEditFormButton, obj6);
      }
    }
    const obj10 = { source: guildId(13484), style: tmp.noneIcon };
    tmp15Result = jsx(tmp3(1177).Icon, { source: guildId(13484), style: tmp.noneIcon });
  }
  const tmp3Result = user(14917);
};