...
function Section(introText) {
  ...
  obj = { style: tmp.sectionHeading, variant: "text-sm/medium", color: "text-strong", accessibilityRole: "header", lineClamp: 1, children: heading };
...
fn(4606);
let createStyles = { section: null, sectionHeading: null, introText: null, learnMore: null, loading: null, loadingRow: null, loadingThumbnail: null, loadingLine: null };
...
createStyles.loadingRow = { flexDirection: "row", alignItems: "center", gap: nativeDefault.space.PX_12, padding: nativeDefault.space.PX_12 };
...
export default function UserProfileActivityTab(user) {
  ...
  ({ recent, isCurrentUser, hasCurrentActivity, hasRecentActivity } = cardStyle(13159)(obj));
  ...
  if (hasCurrentActivity) {
    ...
    obj.children = closure_5(cardStyle(13091), obj1);
  ...
  if (hasRecentActivity) {
    ...
    obj2.children = recent.map((entry) => hasOwnProperty(UserProfileRecentActivityCardDefault, { user, entry, style: cardStyle }, entry.id));
    hasRecentActivity = closure_5(Section, obj2);
  }
  items[1] = hasRecentActivity;
  tmp10Result = closure_6(closure_7, { children: items });
  const tmp3 = cardStyle(13159)(obj);
};