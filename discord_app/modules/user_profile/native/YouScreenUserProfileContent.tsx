...
function EditSection(navigateToProfileCustomization) {
  ...
  const tmp9 = useIsContentShown(
    navigateToProfileCustomization(first[23]).DismissibleContent.USER_PROFILE_PREMIUM_AND_SHOP_ENTRY_POINTS,
  );
  ...
  obj3 = {
    targetRef: ref,
    visible: first === tmp4(tmp2[23]).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK,
    markAsDismissed: tmp16,
  };
  items3[1] = closure_14(trackUserProfileAction(first[30]), obj3);
  obj4 = {
    targetRef: ref,
    visible: first === tmp4(tmp2[23]).DismissibleContent.BADGE_CUSTOMIZATION_COACHMARK,
    markAsDismissed: tmp16,
  ...
  items3[3] = closure_14(trackUserProfileAction(first[32]), {
    targetRef: ref,
    visible:
      first ===
      navigateToProfileCustomization(first[23]).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_PROFILE_COACHMARK,
...
export default function YouScreenUserProfileContent(user) {
  ...
  const profileSectionTabs = user(navigateToPremium[44]).useProfileSectionTabs({
  ...
  obj = {
    ...
    itemSpacing: tmp(tmp2[28]).space.PX_24,
    items: memo,
  ...
  const pagesHeightStyle = user(navigateToPremium[45]).usePagesHeightStyle(
  ...
  const focusEffect = user(navigateToPremium[33]).useFocusEffect(
    obj5.useCallback(() => {
  ...
  obj5.displayNameAccessibilityHint =
    "" +
    intl.formatToPlainString(user(navigateToPremium[29]).t["er+FRD"], obj6) +
    ", " +
    intl2.string(user(navigateToPremium[29]).t.C6COaT);
  ...
  obj12 = {
    ...
    children: tmp40(tmp4(tmp2[65]).SegmentedControlPages, { state: segmentedControlState }),
  };
