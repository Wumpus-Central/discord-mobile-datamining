// === Module 13496: EditWishlistActionSheet ===
...
import WishlistVisibility2 from "WishlistVisibility" /* 13494 */;
...
const getWishlistProductLines = fn(9138).getWishlistProductLines;
let closure_12 = fn(8531).TrackUserProfileWishlistActions;
...
export default function EditWishlistActionSheet(wishlistId) {
  ...
  analyticsLocations = analyticsContext(7495)(analyticsLocations1, tmp4(7515).USER_PROFILE_EDIT_WISHLIST_ACTION_SHEET).analyticsLocations;
  let obj2 = { maxWidth: ACTION_SHEET_MAX_WIDTH };
  const tmp5 = analyticsContext(7495);
  ({ cardWidth: c5, rowWidth } = analyticsContext(13493)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  if (null != rowWidth) {
    let obj3 = { width: rowWidth };
    let tmp7 = obj3;
  }
  let tmp6 = analyticsContext(13493)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  const items1 = [value];
  ...
  obj9.children = closure_14(wishlistId(7533).TableSwitchRow, obj10);
  obj8.children = closure_14(wishlistId(5992).TableRowGroup, obj9);
  const items8 = [closure_14(stateFromStores, obj8), ];
  if (stateFromStores1) {
    if (null == stateFromStores) {
      let obj11 = { style: tmp.loadingContainer, children: closure_14(c5, {}) };
      let tmp16Result = closure_14(tmp19, obj11);
    }
    items8[1] = tmp16Result;
    obj6.children = items8;
    obj5.children = closure_15(tmp19, obj6);
    obj4.children = closure_14(tmp2(6957).BottomSheetScrollView, obj5);
  ...
  if (0 !== memo.length) {
    ...
    obj12.children = memo.map((sku) => {
      ...
      if (null == sku.sku) {
        ...
        obj5 = closure_1_14(wishlistId(str[31]).IconButton, obj5);
        obj4.children = obj5;
        obj4 = closure_1_14(stateFromStores, obj4);
        items[1] = obj4;
        obj.children = items;
        skuId = skuId.skuId;
  ...
  tmp4Result = analyticsContext(11504);
};