...
const getWishlistProductLines = fn(9063).getWishlistProductLines;
let closure_12 = fn(8456).TrackUserProfileWishlistActions;
...
exitingAnimation.__closure = { withTiming: fn(4758).withTiming };
...
export default function EditWishlistActionSheet(wishlistId) {
  ...
  analyticsLocations = analyticsContext(7409)(
    analyticsLocations1,
    tmp4(7429).USER_PROFILE_EDIT_WISHLIST_ACTION_SHEET,
  ).analyticsLocations;
  let obj2 = { maxWidth: ACTION_SHEET_MAX_WIDTH };
  const tmp5 = analyticsContext(7409);
  ({ cardWidth: c5, rowWidth } = analyticsContext(13402)({ maxWidth: ACTION_SHEET_MAX_WIDTH }));
  if (null != rowWidth) {
    let obj3 = { width: rowWidth };
    let tmp7 = obj3;
  }
  let tmp6 = analyticsContext(13402)({ maxWidth: ACTION_SHEET_MAX_WIDTH });
  const items1 = [value];
  ...
  const callback = obj7.useCallback(() => {
    ...
    closure_9(!first);
    const result = WishlistActionCreatorsDefault.updateWishlistVisibility(
      wishlistId,
  ...
  obj9.children = closure_14(wishlistId(7447).TableSwitchRow, obj10);
  obj8.children = closure_14(wishlistId(5906).TableRowGroup, obj9);
  ...
  if (stateFromStores1) {
    ...
    obj4.children = closure_14(tmp2(6871).BottomSheetScrollView, obj5);
  ...
  tmp4Result = analyticsContext(11422);
}
