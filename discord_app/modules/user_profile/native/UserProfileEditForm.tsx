...
export default function UserProfileEditForm(currentUser) {
  ...
  const obj5 = { user: currentUser.currentUser, displayProfile: tmp16Result, pendingThemeColors: null, isPreview: null };
  ...
  const obj12 = { style: null, children: closure_13(pendingBadgeDisplayOrder(pendingBadgeHiddenBadges[47]), { user: currentUser.currentUser, disabled: isSubmitting, disableStatus: null != isTryItOut, statusStyle: obj6, isTryItOut, autoStartEditFlow: autoFocusElement === constants.AVATAR }) };
  const items9 = [, , , ];
  ...
  const obj13 = { user: currentUser.currentUser, disabled: isSubmitting, disableStatus: null != isTryItOut, statusStyle: obj6, isTryItOut, autoStartEditFlow: autoFocusElement === constants.AVATAR };
  ...
  const obj16 = { user: currentUser.currentUser, displayName: pendingGlobalName, badges: memo, catalogBadges: memo1, pronouns: null, badgeContainerBackground: null, displayNameAccessibilityRole: "header", pendingDisplayNameStyles: null };
  ...
  if (null != stringResult) {
    ...
    if ("" !== stringResult) {
      ...
      obj18.children = closure_13(tmp5(tmp2[44]).Text, obj19);
      tmp41Result = closure_13(tmp43, obj18);
    }
  }
  ...
  obj20.label = intl2.string(currentUser.currentUser(pendingBadgeHiddenBadges[21]).t["9AjdkD"]);
  ...
  obj20.placeholder = currentUser.currentUser.toString();
  ...
  if (tmp41Result) {
    ...
    tmp41Result = closure_13(tmp(tmp2[51]), obj21);
  }
  ...
  obj22.label = intl3.string(currentUser.currentUser(pendingBadgeHiddenBadges[21]).t["+T3RI/"]);
  ...
  if (!isTryItOut) {
    const obj23 = { badges: memo, catalogBadges: memo1, ownsAnyBadge: someResult, autoOpen: autoFocusElement === constants.BADGES };
    tmp41Result1 = closure_13(tmp(tmp2[52]), obj23);
  }
  ...
  obj24.label = intl4.string(currentUser.currentUser(pendingBadgeHiddenBadges[21]).t.ZzAR2Y);
  ...
  obj24.autoFocus = autoFocusElement === constants.BIO;
  ...
  const obj25 = { user: currentUser.currentUser, onProfileThemeColorsChanged: null, pendingAvatarSrc: null, pendingThemeColors: null, isTryItOut: null };
  ...
  const obj26 = { user: currentUser.currentUser, pendingAvatarDecoration: null, isTryItOut: null };
  ...
  const obj27 = { user: currentUser.currentUser, pendingProfileEffect: null, displayProfile: null, isTryItOut: null };
  ...
  if (tmp41Result2) {
    ...
    tmp41Result2 = closure_13(tmp(tmp2[58]), obj28);
  }
  ...
  if (tmp41Result4) {
    ...
    tmp41Result4 = closure_13(tmp5(tmp2[61]).UserProfilePremiumUpsellCard, obj33);
  }
  items15[1] = tmp41Result4;
  obj8.children = items15;
  obj7.children = closure_14(stateFromStores, obj8);
  return closure_13(currentUser.currentUser(pendingBadgeHiddenBadges[45]).ThemeContextProvider, obj7);
};