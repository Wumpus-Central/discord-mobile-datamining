...
function ColorSwatch(color) {
  ...
  obj1 = { accessibilityRole: "button", accessibilityLabel, accessibilityHint: null, style: null, onPress: null, children: null };
  const intl = util.intl;
  obj1.accessibilityHint = intl.string(util.t.Qp04hK);
  const items1 = [tmp.colorSwatch, { backgroundColor: int2hexResult }, style];
  obj1.style = items1;
  obj1.onPress = onPress;
  obj1.children = suggestedColors(PencilIcon.PencilIcon, { size: "xs", color: higherContrastColor, style: tmp.dropperIcon });
  const items2 = [suggestedColors(Pressables.PressableOpacity, obj1, color), suggestedColors(Text_Text.Text, { variant: "text-sm/normal", color: "text-default", accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants", children: label })];
  obj.children = items2;
...
fn(4560);
let createStyles = { container: { gap: 6 }, sectionHeader: { display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, themeColorContainer: { flexDirection: "row", gap: 12, justifyContent: "center" }, colorSwatchContainer: { position: "relative", flex: 1, flexDirection: "column", alignItems: "center", gap: 4 }, colorSwatch: null, dropperIcon: null, overflowMenu: null };
...
export default function UserProfileEditTheme(pendingThemeColors) {
  ...
  suggestedColors = obj.useAvatarColors(pendingAvatarSrc, f97636(tmp3[4]).unsafe_rawColors.PRIMARY_530, false);
  if (null != primaryColor) {
    if (null != secondaryColor) {
        ...
        obj2.onPress = function onPress() {
          const obj = {
            onResetTheme() {
              ...
              secondaryColor(tmp4);
              tmp = f97636(primaryColor[14]);
            }
          };
          obj.openLazy(asyncRequireImpl(14627, dependencyMap.paths), "Profile Theme", obj);
        };
      ...
      obj5.onPress = () => {
        ...
        const result = obj.UNSAFE_markDismissibleContentAsDismissed(secondaryColor(primaryColor[16]).DismissibleContent.PROFILE_THEMES_SETTINGS_VIEWED_V2);
        obj = { color: secondaryColor, onSelect: f97636, suggestedColors };
