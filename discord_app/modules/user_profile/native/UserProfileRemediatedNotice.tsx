// === Module 13435: UserProfileRemediatedNotice ===
...
export default function RemediatedUserNotice(user) {
  ...
  if (isBlocked) {
    ...
    if (isBlocked) {
      ...
      isBlocked = closure_6(tmp4(4753).Text, obj3);
    }
    ...
    if (isIgnored) {
      ...
      const items3 = [closure_6(tmp4(4753).Text, obj5), closure_6(tmp4(4753).Text, { variant: "text-sm/semibold", color: "text-default", accessibilityElementsHidden: true, importantForAccessibility: "no", children: "\u2022" }), ];
    ...
    let tmp8Result = closure_8(View, obj2);
  } else {
    tmp8Result = null;
    if (isIgnored) {
      tmp8Result = null;
