...
export default function UserProfileMutualGuildsActionSheet(user) {
  ...
  const mutualGuilds = onPressMutualGuild(12040)(user).mutualGuilds;
  let length;
  const tmp4 = onPressMutualGuild(9531);
  if (mutualGuilds != null) {
    length = mutualGuilds.length;
  }
  let obj = { scrollable: true, title: onPressMutualGuild(12041)(length), children: null };
  ...
  if (null == mutualGuilds) {
    ...
    obj1[1] = tmp3(user(12042).NoMutualServers, {});
    ...
    tmp3Result = tmp3(user(9531).UserProfileStackedActionSheetList, obj2);
  }
  obj[1] = tmp3Result;
