...
export default function UserProfileMutualGuildsActionSheet(user) {
  ...
  if (null == mutualGuilds) {
    ...
    const obj4 = { style: tmp.emptyState, children: jsx(user(12943).NoMutualServers, {}) };
    tmp3Result = <closure_3 style={tmp.emptyState}>{jsx(user(12943).NoMutualServers, {})}</closure_3>;
  } else {
    const obj5 = {
      data: mutualGuilds,
      keyExtractor(guild) {
    ...
    tmp3Result = jsx(user(11498).UserProfileStackedActionSheetList, {
      data: mutualGuilds,
      keyExtractor(guild) {
        return guild.guild.id;
      },
  ...
  return (
    <tmp4 scrollable title={onPressMutualGuild(12942)(length)}>
      {null}
    </tmp4>
  );
}
