...
export default function UserProfileMutualGuildsActionSheet(user) {
  ...
  if (null == mutualGuilds) {
    ...
    tmp3Result = <closure_3 style={tmp.emptyState}>{jsx(user(12101).NoMutualServers, {})}</closure_3>;
  } else {
    const obj5 = {
      data: mutualGuilds,
      keyExtractor(guild) {
    ...
    tmp3Result = jsx(user(10613).UserProfileStackedActionSheetList, {
      data: mutualGuilds,
      keyExtractor(guild) {
        return guild.guild.id;
      },
  ...
  return (
    <tmp4 scrollable title={onPressMutualGuild(12100)(length)}>
      {null}
    </tmp4>
  );
}
