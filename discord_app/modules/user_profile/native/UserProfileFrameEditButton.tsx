...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(14718, dependencyMap.paths), "Profile Frame", obj);
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj1.onPress = NOOP;
    obj1.leading = jsx(user(5658).ActivityIndicator, { animating: true, size: "large" });
    let obj2 = obj1;
  } else {
    ...
    obj2.labelTrailing = jsx(user(14705).UserProfileEditFormLabelBadges, { showNewBadge: tmp4[0] === user(1943).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE });
    ...
    if (null != first1) {
      ...
      obj5.children = jsx(guildId(8949), { profileFrame: first1, previewWidth: COLLECTIBLES_PREVIEW_SIZE - 2 * guildId(576).space.PX_8, previewHeight: COLLECTIBLES_PREVIEW_SIZE - 2 * PX_4, profileBackgroundColor: guildId(576).colors.BACKGROUND_SURFACE_HIGH });
      let tmp18Result = <View style={tmp.previewContainer}>{null}</View>;
      const tmp23 = guildId(8949);
    } else {
      const obj7 = { source: guildId(13302), style: tmp.noneIcon };
      tmp18Result = jsx(user(1178).Icon, { source: guildId(13302), style: tmp.noneIcon });
    }
    obj2.leading = tmp18Result;
  }
  return jsx(user(14705).UserProfileEditFormButton, obj2);
};