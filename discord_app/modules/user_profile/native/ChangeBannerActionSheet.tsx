...
function ChangeBannerColorRow(user) {
  ...
  const tmp7 = pendingAccentColor(8230)(user.id);
  if (pendingAvatarSrc == null) {
    pendingAvatarSrc = user.getAvatarURL(undefined, 80);
  }
  let tmp2Result = tmp2(8227);
  tmp2Result = tmp2(688);
  ...
  dependencyMap = React.useCallback((arg0) => {
    ...
    _undefined(8188).setPendingChanges({ accentColor: tmp });
  }, items1);
  ...
  const items2 = [callback(pendingAccentColor(13821), { style: tmp.bannerColor, color: pendingAccentColor }), , ];
  ...
  obj5[2] = pendingAccentColor(13826);
  items2[2] = callback(_Button.Icon, obj5);
  obj2[1] = items2;
  obj[1] = callback2(View, obj2);
  obj[2] = function handleChangeColor() {
    pendingAccentColor(13819)({ color: pendingAccentColor, onSelect: dependencyMap });
  };
...
export default function ChangeBannerActionSheet(isTryItOut) {
        ...
        try {
          ...
          if (0 === originalMd5) {
            if (arg0 === 1) {
              ...
              obj1[0] = outer1_1(4841).openImagePicker(outer1_8);
              return obj1;
            }
          } else if (arg0 === 1) {
            c3 = 3;
  ...
  let tmp = createCacheKey();
  if (!flag) {
    flag = tmp2(3930).canUsePremiumProfileCustomization(user);
    const tmp2Result = tmp2(3930);
  }
  let obj = { value: _handleBannerUploadSelect(5649)(isTryItOut.analyticsLocations).analyticsLocations, children: null };
