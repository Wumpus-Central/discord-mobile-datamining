...
function ChangeBannerColorRow(user) {
  ...
  const tmp7 = pendingAccentColor(8089)(user.id);
  if (pendingAvatarSrc == null) {
    pendingAvatarSrc = user.getAvatarURL(undefined, 80);
  }
  let tmp2Result = tmp2(8086);
  tmp2Result = tmp2(688);
  ...
  dependencyMap = React.useCallback((arg0) => {
    ...
    _undefined(8047).setPendingChanges({ accentColor: tmp });
  }, items1);
  ...
  const items2 = [callback(pendingAccentColor(13784), { style: tmp.bannerColor, color: pendingAccentColor }), , ];
  ...
  obj5[2] = pendingAccentColor(13789);
  items2[2] = callback(_Button.Icon, obj5);
  obj2[1] = items2;
  obj[1] = callback2(View, obj2);
  obj[2] = function handleChangeColor() {
    pendingAccentColor(13782)({ color: pendingAccentColor, onSelect: dependencyMap });
  };
...
export default function ChangeBannerActionSheet(isTryItOut) {
        ...
        try {
          ...
          if (0 === originalMd5) {
            if (arg0 === 1) {
              ...
              obj1[0] = outer1_1(4811).openImagePicker(outer1_8);
              return obj1;
            }
          } else if (arg0 === 1) {
            c3 = 3;
  ...
  let obj = { value: _handleBannerUploadSelect(5595)(isTryItOut.analyticsLocations).analyticsLocations, children: null };
  ...
  let tmp4Result = flag;
  if (flag) {
    tmp4Result = tmp4(tmp5(7597).NitroWheelIcon, {});
  }
  obj[1] = tmp4Result;
  ...
  if (!flag) {
    ...
    tmp4Result1 = tmp4(tmp5(7597).NitroWheelIcon, obj3);
  }
