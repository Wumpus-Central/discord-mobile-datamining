...
import openEditNoteModalDefault from "../utils/native/openEditNoteModal.tsx";
...
export default function UserProfileNote(userId) {
  ...
  obj2.label = jsx(userId(trackUserProfileAction[5]).Text, {
  ...
  if (!tmp3) {
    ...
    tmp4Result = jsx(tmp(tmp2[4]).TableRow.Icon, { IconComponent: tmp(tmp2[9]).PaperPlusIcon });
  }
  obj2.trailing = tmp4Result;
  obj2.arrow = tmp3;
  return jsx(userId(trackUserProfileAction[4]).TableRow, {
    label: null,
    subLabel: null,
    accessibilityHint: null,
    onPress: null,
