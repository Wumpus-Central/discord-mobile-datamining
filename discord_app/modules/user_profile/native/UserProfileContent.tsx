...
function EditSection(guildId) {
  ...
  obj = {
    style: trackUserProfileAction(8304)().primaryButtons,
...
function UserProfileActivityTabContainer(arg0) {
  ...
  const obj = {
    style: null,
    children: closure_1_19(UserProfileActivityTabDefault, { user, currentUser, guildId, channelId, cardStyle: items }),
  };
  const items1 = [,];
...
export default noop.memo(function UserProfileContent(user) {
  ...
  const tmp35 =
    user(displayProfile[61]).useIsRecentActivityMobileEnabled("UserProfileContent") && null != stateFromStores;
  closure_24 = tmp35;
  ...
  const wishlistViewerCoachmark = user(displayProfile[64]).useWishlistViewerCoachmark({
    isCurrentUser: tmp9,
  ...
  const profileSectionTabs = user(displayProfile[62]).useProfileSectionTabs({
  ...
  segmentedControlState = tmp6Result16.useSegmentedControlState({
    pageWidth: tmp38,
  ...
  const pagesHeightStyle = user(displayProfile[63]).usePagesHeightStyle(
