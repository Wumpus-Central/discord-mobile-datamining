...
const result = require("set").fileFinishedImporting(
  "modules/user_profile/native/PersonalWidgetExpandCollapseContext.tsx",
);
...
export const PersonalWidgetExpandCollapseProvider = function PersonalWidgetExpandCollapseProvider(children) {
  ...
  redux = tmp3[1];
  first2 = first(
    importAllResult.useState(() => new Set()),
    1,
  )[0];
  const items = [first2];
  ...
  return first1(redux.Provider, {
    value: importAllResult.useMemo(
      () => ({ isExpanded: first, setIsExpanded: closure_1, isAnyFieldClipped: first1, setAnyFieldClipped: callback }),
      items1,
    ),
