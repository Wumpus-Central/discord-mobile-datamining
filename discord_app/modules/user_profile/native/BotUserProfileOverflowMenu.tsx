...
export default function BotUserProfileOverflowMenu(application) {
  ...
  const stateFromStoresObject = application(channel[8]).useStateFromStoresObject(items, () => ({
  ...
  const stateFromStores = application(channel[8]).useStateFromStores(items1, () =>
    context.getChannel(analyticsLocations.getChannelId()),
  );
  ...
  if (isIgnored) {
    ...
    if (!tmp14) {
      ...
      obj7.action = function action() {
        ...
        const tmp4 = asyncRequireImpl(11726, dependencyMap.paths);
      };
      items3.push(obj7);
      if (result) {
        ...
        obj8.action = function action() {
          ...
          obj2.openLazy(
            asyncRequireImpl(13371, dependencyMap.paths),
            BotReportChooser.BOT_REPORT_CHOOSER_KEY,
            obj3,
            "replaceAll",
          );
          const tmp5 = asyncRequireImpl(13371, dependencyMap.paths);
        };
        items3.push(obj8);
      } else {
