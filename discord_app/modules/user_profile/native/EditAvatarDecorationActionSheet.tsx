// === Module 8058: EditAvatarDecorationInner ===
...
function AvatarDecorationSectionPreview(previewSkuId) {
  ...
  let _require;
  let purchase;
  const tmp4 = purchase(8076)(previewSkuId.previewSkuId);
  ...
  const items1 = [callback2(purchase(8098), obj), callback2(purchase(8112), { user, guildId, pendingAvatarSrc, pendingAvatarDecoration: memo })];
  obj[4] = items1;
...
export default function EditAvatarDecorationActionSheet(arg0) {
  ...
  let first;
  const tmp = createCacheKey();
  const tmp4 = first(8059)({ analyticsLocations, isTryItOut, guildId });
  setPendingAvatarDecoration = tmp4.setPendingAvatarDecoration;
  let obj = setPendingAvatarDecoration(8073);
  ...
  first = tmp7[0];
  let obj3 = setPendingAvatarDecoration(7313);
  if (analyticsLocations == null) {
    analyticsLocations = [];
  }
  const analyticsLocations2 = first(5581)(analyticsLocations, tmp2(5601).EDIT_AVATAR_DECORATION_SHEET).analyticsLocations;
  const items = [first, setPendingAvatarDecoration];
  callback = obj2.useCallback(() => {
    let obj = first(698);
    obj = { type: first(5601).EDIT_AVATAR_DECORATION_SHEET, is_fullscreen: true };
  ...
  obj5[6] = first(5601).EDIT_AVATAR_DECORATION_SHEET;
  items2[1] = callback2(first(8075), obj5);
  obj1[4] = items2;
  obj[1] = tmp13(setPendingAvatarDecoration(5309).BottomSheet, obj1);
  obj6[0] = callback2(setPendingAvatarDecoration(5581).AnalyticsLocationProvider, obj);
