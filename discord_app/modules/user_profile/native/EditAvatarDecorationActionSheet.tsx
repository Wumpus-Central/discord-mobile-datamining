...
function AvatarDecorationSectionPreview(previewSkuId) {
  ...
  let _require;
  let purchase;
  const tmp4 = purchase(8201)(previewSkuId.previewSkuId);
  ...
  const items1 = [callback2(purchase(8223), obj), callback2(purchase(8237), { user, guildId, pendingAvatarSrc, pendingAvatarDecoration: memo })];
  obj[4] = items1;
...
export default function EditAvatarDecorationActionSheet(arg0) {
  ...
  let first;
  const tmp = createCacheKey();
  const tmp4 = first(8184)({ analyticsLocations, isTryItOut, guildId });
  setPendingAvatarDecoration = tmp4.setPendingAvatarDecoration;
  let obj = setPendingAvatarDecoration(8198);
  ...
  const analyticsLocations2 = first(5649)(analyticsLocations, tmp2(5669).EDIT_AVATAR_DECORATION_SHEET).analyticsLocations;
  const items = [first, setPendingAvatarDecoration];
  callback = obj2.useCallback(() => {
    let obj = first(698);
    obj = { type: first(5669).EDIT_AVATAR_DECORATION_SHEET, is_fullscreen: true };
  ...
  obj5[6] = first(5669).EDIT_AVATAR_DECORATION_SHEET;
  items2[1] = callback2(first(8200), obj5);
  obj1[4] = items2;
  obj[1] = tmp13(setPendingAvatarDecoration(5377).BottomSheet, obj1);
  obj6[0] = callback2(setPendingAvatarDecoration(5649).AnalyticsLocationProvider, obj);
  return callback2(setPendingAvatarDecoration(3996).ThemeContextProvider, obj6);
};