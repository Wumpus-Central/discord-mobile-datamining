...
import closure_13 from "sortActivity" /* 4569 */;
import closure_14 from "filterPlayingActivities" /* 5231 */;
import closure_15 from "updateVoiceState" /* 4497 */;
import closure_16 from "createUserWidgetFromServer" /* 7375 */;
...
function ActivityCard(user) {
  ...
  const tmp3 = activity(9425)(activity);
  const analyticsLocations = activity(5961)(activity(5981).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(5961);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(9431)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  dependencyMap = tmp5Result;
  const application_id = activity.application_id;
  let tmp2Result = tmp2(8816);
  obj = { location: "User Profile Activity Card", applicationId: application_id, source: user(8827).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id };
  ...
  if (activity.type !== constants.CUSTOM_STATUS) {
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      const intl = ConnectPlatformButton(1233).intl;
      obj3 = { gameName: null };
      obj3[0] = activity.name;
      obj2[3] = intl.formatToPlainString(ConnectPlatformButton(1233).t["9sZWVp"], obj3);
      ...
      if (tmp34Result) {
        ...
        obj5[2] = ConnectPlatformButton(1296).IconSizes.SMALL_14;
        tmp34Result = tmp34(ConnectPlatformButton(1296).Icon, obj5);
      }
      obj4[3] = tmp34Result;
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp34Result1 = tmp34(tmp2(9436), obj8);
        }
      }
      items4[2] = tmp34Result1;
      if (tmp2(9400)(activity)) {
        ConnectPlatformButton = ConnectPlatformButton(9445).ConnectPlatformButton;
        obj = { type: null, onAction: null };
        obj[0] = constants2.XBOX;
        obj[1] = tmp5Result;
        let tmp34Result2 = tmp34(ConnectPlatformButton, obj);
      } else if (tmp2(9401)(activity)) {
        const obj9 = { type: null, onAction: null };
        obj9[0] = constants2.PLAYSTATION;
        obj9[1] = tmp5Result;
        tmp34Result2 = tmp34(ConnectPlatformButton(9445).ConnectPlatformButton, obj9);
      } else {
        tmp34Result2 = null;
        if (user.id !== currentUser.id) {
          if (tmp2(9269)(activity)) {
            ...
            obj10[0] = activity;
            obj10[1] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(9445).PlayOnSpotifyButton, obj10);
          } else if (tmp2(7494)(activity)) {
            ...
            obj11[4] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(9445).JoinActivityButton, obj11);
          } else {
            if (tmp2(9262)(activity)) {
              const result1 = ConnectPlatformButton(9459);
              ...
              if (set.has(currentActivityGamePlatform)) {
                if (null != activity.party) {
                  ...
                  if (null != deepLinkUri) {
                    ...
                    obj12[4] = tmp5Result;
                    tmp34Result2 = tmp34(ConnectPlatformButton(9445).JoinGameActivityButton, obj12);
                  }
                }
              }
            }
            if (tmp2(8116)(activity)) {
              const obj13 = { activity: null, onAction: null };
              obj13[0] = activity;
              obj13[1] = tmp5Result;
              let tmp34Result3 = tmp34(ConnectPlatformButton(9445).WatchActivityButton, obj13);
            } else {
              tmp34Result3 = null;
              if (null != activity.buttons) {
                ...
                if (activity.buttons.length > 0) {
                  ...
                  obj14[1] = buttons.map((arg0, index) => closure_1_21(user(9445).CustomActivityButton, { index, user, activity, onAction: dependencyMap }, index));
      ...
      obj2[4] = obj4;
      obj2 = tmp34(ConnectPlatformButton(5076).PressableOpacity, obj2);
      obj1[1] = obj2;
      closure_21(ConnectPlatformButton(5961).AnalyticsLocationProvider, obj1);
...
function StreamActivityCard(user) {
  ...
  obj4 = { mode: "a", stream: "<string:2466251611>", disabled: "<string:1207960145>", onPress: "dechov\u00FD" };
  obj4[1] = stream;
...
function VoiceCallActivityCard(arg0) {
  ...
  const tmp = callback2();
  const tmp5 = stateFromStores(4674)(channel);
  let obj = channel(586);
  ...
  const tmp4 = stateFromStores(9437)(channel);
  const tmp9 = stateFromStores(5961);
  ({ newestAnalyticsLocation: c2, analyticsLocations } = stateFromStores(5961)(stateFromStores(5981).USER_PROFILE_VOICE_ACTIVITY_CARD));
  obj = { display: "voice", activity: { type: "VOICE" }, voiceChannelId: channel.id, user, analyticsLocations };
  const tmp11 = stateFromStores(9431)(obj);
  closure_3 = tmp11;
  obj = { userId: user.id, onAction: tmp11 };
  stateFromStores(9433)(obj);
  obj1 = { style: items2, title: null, titleStyle: null, children: null };
  items2 = [tmp.card, style];
  const tmp9Result = stateFromStores(5961)(stateFromStores(5981).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    obj3[1] = id;
    const items3 = [closure_21(tmp2(12694), obj3), ];
    ...
    if (stateFromStores1) {
      ...
      obj6[0] = channel;
      obj5[1] = tmp2(9438)(obj6);
      const intl3 = tmp6(1233).intl;
      obj5[2] = intl3.string(tmp6(1233).t["9C444m"]);
      ...
      obj9[0] = channel;
      obj8[1] = tmp18(tmp2(9272), obj9);
      const items4 = [tmp18(tmp17, obj8), tmp5];
      obj7[2] = items4;
      obj5[4] = tmp13(tmp6(4474).Text, obj7);
      let tmp13Result = tmp18(tmp6(5076).PressableOpacity, obj5);
    } else {
    ...
    if (null != stateFromStores) {
      ...
      obj13[1] = intl4.string(tmp6(1233).t.KLOhbO);
      const intl5 = tmp6(1233).intl;
      const obj14 = { guildName: null };
      obj14[0] = stateFromStores.name;
      obj13[2] = intl5.formatToPlainString(tmp6(1233).t["hq/Qze"], obj14);
      ...
      obj15[1] = intl6.format(tmp6(1233).t["hq/Qze"], obj16);
      obj13[4] = tmp18(tmp6(4474).Text, obj15);
      tmp18Result = tmp18(tmp6(5076).PressableOpacity, obj13);
    }
    ...
    obj17[1] = isInChannel;
    obj17[2] = tmp11;
    items7[1] = closure_21(tmp6(9445).VoiceChannelButtons, obj17);
    obj1[3] = items7;
    return tmp13(tmp14, obj1);
  }
  const intl2 = tmp6(1233).intl;
  stringResult = intl2.string(tmp6(1233).t["9FaEzi"]);
}
