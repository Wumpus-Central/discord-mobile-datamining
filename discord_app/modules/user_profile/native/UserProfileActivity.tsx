...
function ActivityCardBody(user) {
  ...
  let IM4J4e = user;
  const imageForActivity = user(onAction[20]).useImageForActivity(
    activity,
    user.application,
    "user_profile_activity_native",
  );
  ...
  if (user.bot) {
    ...
    if (activity(tmp2[22])(activity)) {
      ...
      obj4.children = closure_21(IM4J4e(tmp2[24]).SpotifyTrack, obj5);
      let tmp24Result = closure_21(ActivityCardText, obj4);
      ...
      obj6.children = closure_21(ActivityCardText, obj7);
      tmp24Result = closure_21(MaybeLink, obj6);
    ...
    items[1] = closure_22(closure_7, obj3);
    obj2.children = items;
    ...
    if (null == largeImage) {
      if (null != largeImage) {
        ...
        if (tmp13Result) {
          ...
          obj22.children = closure_21(tmp12Result2, obj23);
          obj21.children = closure_21(MaybeLink, obj22);
          tmp13Result = closure_21(closure_7, obj21);
          const IM4J4eResult1 = IM4J4e(tmp2[26]);
        }
        items4[1] = tmp13Result;
        obj18.children = items4;
        let tmp51Result = closure_22(closure_7, obj18);
        ...
        tmp51Result = closure_21(closure_7, obj24);
...
function ActivityCard(user) {
  ...
  const tmp3 = activity(12553)(activity);
  const analyticsLocations = activity(6578)(activity(6598).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(6578);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(12559)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  dependencyMap = tmp5Result;
  const application_id = activity.application_id;
  const tmp5 = activity(12559);
  const tmp2Result = activity(8120);
  let PlayOnSpotifyButton = user;
  const tmp2ResultResult = tmp2Result({
    location: "User Profile Activity Card",
    applicationId: application_id,
    source: user(8131).GameProfileSources.UserProfile,
  ...
  activity(12560)({ userId: user.id, onAction: tmp5Result });
  const obj2 = {
    location: "User Profile Activity Card",
    applicationId: application_id,
    source: user(8131).GameProfileSources.UserProfile,
  ...
  if (activity.type !== constants.CUSTOM_STATUS) {
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      if (tmp33Result) {
        ...
        tmp33Result = closure_21(PlayOnSpotifyButton(1177).Icon, obj10);
      ...
      if (activity(12546)(activity)) {
        ({ start, end } = activity.timestamps);
        const obj12 = { start, end };
        tmp33Result5 = closure_21(activity(12561), obj12);
      }
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp33Result6 = closure_21(activity(12563), obj13);
        }
      }
      items4[2] = tmp33Result6;
      let tmp33Result7 = null;
      if (user.id !== currentUser.id) {
        if (activity(10339)(activity)) {
          PlayOnSpotifyButton = PlayOnSpotifyButton(12571).PlayOnSpotifyButton;
          obj = { activity, onAction: tmp5Result };
          tmp33Result7 = closure_21(PlayOnSpotifyButton, obj);
        } else if (activity(7153)(activity)) {
          const obj14 = { user, currentUser, activity, application: stateFromStores1, onAction: tmp5Result };
          tmp33Result7 = closure_21(PlayOnSpotifyButton(12571).JoinActivityButton, obj14);
        } else {
          if (activity(10334)(activity)) {
            let supported_platforms = activity.supported_platforms;
            const currentActivityGamePlatform = PlayOnSpotifyButton(12574).getCurrentActivityGamePlatform();
                ...
                if (null != deepLinkUri) {
                  if (null != activity.session_id) {
                    if (null != stateFromStores1) {
                      ...
                      tmp33Result7 = closure_21(PlayOnSpotifyButton(12571).JoinGameActivityButton, obj15);
                    }
                  }
                }
              }
            }
            const PlayOnSpotifyButtonResult1 = PlayOnSpotifyButton(12574);
          }
          if (activity(7697)(activity)) {
            const obj16 = { activity, onAction: tmp5Result };
            tmp33Result7 = closure_21(PlayOnSpotifyButton(12571).WatchActivityButton, obj16);
          } else {
            if (null != activity.buttons) {
              if (activity.buttons.length > 0) {
                const obj17 = { style: tmp.customButtons, children: null };
                const buttons = activity.buttons;
                obj17.children = buttons.map((item, index) =>
                  __initData(
                ...
                tmp33Result7 = closure_21(closure_7, obj17);
              }
            }
            tmp33Result7 = null;
            if (!activity(4870)(activity)) {
              if (!activity(12541)(activity)) {
                tmp33Result7 = null;
                if (activity(12542)(activity)) {
                  const obj18 = { type: constants3.PLAYSTATION, onAction: tmp5Result };
                  tmp33Result7 = closure_21(PlayOnSpotifyButton(12571).ConnectPlatformButton, obj18);
                }
              }
            }
            const obj19 = { type: constants3.XBOX, onAction: tmp5Result };
            tmp33Result7 = closure_21(PlayOnSpotifyButton(12571).ConnectPlatformButton, obj19);
          }
        }
      }
      items4[3] = tmp33Result7;
      obj9.children = items4;
      obj9 = closure_22(activity(6623), obj9);
      obj7.children = obj9;
      obj7 = closure_21(PlayOnSpotifyButton(5428).PressableOpacity, obj7);
      obj6.children = obj7;
      closure_21(PlayOnSpotifyButton(6578).AnalyticsLocationProvider, obj6);
      const tmp2Result2 = activity(6623);
    }
  }
  return tmp15;
}
function StreamActivityCard(user) {
  ...
  const stateFromStores3 = user(activity[40]).useStateFromStores(items3, () =>
    PresenceStore.findActivity(user.id, (arg0) => stream(activity[30])(arg0) && !stream(activity[53])(arg0)),
  );
  const obj4 = user(activity[40]);
  const items4 = [ApplicationStreamingStore];
  const stateFromStores4 = user(activity[40]).useStateFromStores(items4, () =>
  ...
  ({ effectiveVolume, handleVolumeChange } = stream(activity[54])(
    ownerId,
    user(activity[55]).MediaEngineContextTypes.STREAM,
  ));
  ...
  const tmp9Result1Result = stream(activity[36])({
    display: "live",
    voiceChannelId: id,
    user,
    stream,
  ...
  let tmp19Result =
    ...
    !tmp9Result6.useConfig({ location: "UserProfileVoiceSettings" }).nonContextualStreamOutputPresent;
  if (tmp19Result) {
    ...
    tmp19Result = closure_21(tmp9(tmp3[62]), obj12, "set-stream-volume");
  ...
  if (tmp19Result4) {
    ...
    tmp19Result4 = closure_21(ActivityCardBody, obj13);
  }
  ...
  if (tmp19Result5) {
    ...
    tmp19Result5 = closure_21(tmp9(tmp3[46]), obj14);
  }
  ...
  if (tmp19Result6) {
    ...
    tmp19Result6 = closure_21(tmp2(tmp3[47]).VoiceChannelButtons, obj15);
  }
...
function VoiceCallActivityCard(arg0) {
  ...
  const tmp4 = stateFromStores(12564)(channel);
  ...
  const tmp9 = stateFromStores(6578);
  ({ newestAnalyticsLocation: c2, analyticsLocations } = stateFromStores(6578)(
    stateFromStores(6598).USER_PROFILE_VOICE_ACTIVITY_CARD,
  ));
  const tmp11 = stateFromStores(12559)({
    display: "voice",
    activity: { type: "VOICE" },
  ...
  closure_3 = tmp11;
  stateFromStores(12560)({ userId: user.id, onAction: tmp11 });
  ...
  const tmp9Result = stateFromStores(6578)(stateFromStores(6598).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    obj7.guildId = id;
    const items3 = [closure_21(tmp2(12578), obj7)];
    ...
    if (stateFromStores1) {
      ...
      const obj10 = { channel };
      obj9.accessibilityLabel = tmp2(9049)(obj10);
      ...
      obj12.children = closure_21(tmp2(10341), obj13);
      const items4 = [closure_21(closure_7, obj12), tmp5];
      obj11.children = items4;
      obj9.children = closure_22(channel(4825).Text, obj11);
      let tmp13Result = closure_21(channel(5428).PressableOpacity, obj9);
    } else {
      ...
      obj15.children = closure_21(tmp2(10341), obj16);
      const items5 = [closure_21(closure_7, obj15), tmp5];
      obj14.children = items5;
      tmp13Result = closure_22(channel(4825).Text, obj14);
    }
    ...
    if (null != stateFromStores) {
      ...
      obj17.accessibilityHint = intl4.string(channel(1115).t.KLOhbO);
      const intl5 = channel(1115).intl;
      const obj18 = { guildName: stateFromStores.name };
      obj17.accessibilityLabel = intl5.formatToPlainString(channel(1115).t["hq/Qze"], obj18);
      ...
      obj17.children = closure_21(channel(4825).Text, obj19);
      tmp18Result2 = closure_21(channel(5428).PressableOpacity, obj17);
    }
    ...
    items7[1] = closure_21(channel(12571).VoiceChannelButtons, obj21);
    obj5.children = items7;
    return closure_22(tmp14, obj5);
  }
  const intl2 = channel(1115).intl;
  stringResult = intl2.string(channel(1115).t["9FaEzi"]);
  tmp14 = stateFromStores(6623);
}
...
let obj2 = {
  ...
  cardTitleIcon: { tintColor: nativeDefault.colors.TEXT_SUBTLE },
  body: { flexDirection: "row", alignItems: "center", gap: 16 },
  content: { flex: 1 },
  imageContainer: { position: "relative" },
  imageAspectRatio: { width: 60, maxHeight: 60, aspectRatio: "1 / 1" },
  crunchyrollImageAspectRatio: { width: 60, maxHeight: 100, aspectRatio: "2 / 3" },
  largeImage: null,
...
export default function UserProfileActivity(guildId) {
  ...
  if (stateFromStores1) {
    ...
    if (!tmp12) {
      ...
      return closure_22(closure_23, obj8);
    } else {
      ...
      if (stateFromStores) {
        ...
        let tmp10Result = closure_22(tmp2Result3, obj9);
