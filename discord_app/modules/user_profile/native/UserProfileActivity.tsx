...
function ActivityCard(user) {
  ...
  const tmp3 = activity(12207)(activity);
  const analyticsLocations = activity(5649)(activity(5669).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(5649);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(12213)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  dependencyMap = tmp5Result;
  const application_id = activity.application_id;
  let tmp2Result = tmp2(9190);
  obj = { location: "User Profile Activity Card", applicationId: application_id, source: user(9201).GameProfileSources.UserProfile, trackEntryPointImpression: true, sourceUserId: user.id };
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp34Result1 = tmp34(tmp2(12217), obj8);
        }
      }
      items4[2] = tmp34Result1;
      if (tmp2(12193)(activity)) {
        ConnectPlatformButton = ConnectPlatformButton(12225).ConnectPlatformButton;
        obj = { type: null, onAction: null };
        obj[0] = constants2.XBOX;
        obj[1] = tmp5Result;
        let tmp34Result2 = tmp34(ConnectPlatformButton, obj);
      } else if (tmp2(12194)(activity)) {
        const obj9 = { type: null, onAction: null };
        obj9[0] = constants2.PLAYSTATION;
        obj9[1] = tmp5Result;
        tmp34Result2 = tmp34(ConnectPlatformButton(12225).ConnectPlatformButton, obj9);
      } else {
        tmp34Result2 = null;
        if (user.id !== currentUser.id) {
          if (tmp2(9573)(activity)) {
            ...
            obj10[0] = activity;
            obj10[1] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(12225).PlayOnSpotifyButton, obj10);
          } else if (tmp2(7120)(activity)) {
            ...
            obj11[4] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(12225).JoinActivityButton, obj11);
          } else {
            if (tmp2(9566)(activity)) {
              const result1 = ConnectPlatformButton(12228);
              ...
              if (set.has(currentActivityGamePlatform)) {
                if (null != activity.party) {
                  ...
                  if (null != deepLinkUri) {
                    ...
                    obj12[4] = tmp5Result;
                    tmp34Result2 = tmp34(ConnectPlatformButton(12225).JoinGameActivityButton, obj12);
                  }
                }
              }
            }
            if (tmp2(8225)(activity)) {
              const obj13 = { activity: null, onAction: null };
              obj13[0] = activity;
              obj13[1] = tmp5Result;
              let tmp34Result3 = tmp34(ConnectPlatformButton(12225).WatchActivityButton, obj13);
            } else {
              tmp34Result3 = null;
      ...
      obj2[4] = obj4;
      obj2 = tmp34(ConnectPlatformButton(4827).PressableOpacity, obj2);
      obj1[1] = obj2;
      closure_21(ConnectPlatformButton(5649).AnalyticsLocationProvider, obj1);
...
function VoiceCallActivityCard(arg0) {
  ...
  const tmp4 = stateFromStores(12218)(channel);
  const tmp9 = stateFromStores(5649);
  ({ newestAnalyticsLocation: c2, analyticsLocations } = stateFromStores(5649)(stateFromStores(5669).USER_PROFILE_VOICE_ACTIVITY_CARD));
  obj = { display: "voice", activity: { type: "VOICE" }, voiceChannelId: channel.id, user, analyticsLocations };
  const tmp11 = stateFromStores(12213)(obj);
  c3 = tmp11;
  obj = { userId: user.id, onAction: tmp11 };
  stateFromStores(12214)(obj);
  obj1 = { style: items2, title: null, titleStyle: null, children: null };
  items2 = [tmp.card, style];
  const tmp9Result = stateFromStores(5649)(stateFromStores(5669).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    obj3[1] = id;
    const items3 = [closure_21(tmp2(12232), obj3), ];
    ...
    if (stateFromStores1) {
      ...
      let tmp13Result = tmp18(tmp6(4827).PressableOpacity, obj5);
    } else {
    ...
    if (null != stateFromStores) {
      ...
      tmp18Result = tmp18(tmp6(4827).PressableOpacity, obj13);
    }
    ...
    obj17[1] = isInChannel;
    obj17[2] = tmp11;
    items7[1] = closure_21(tmp6(12225).VoiceChannelButtons, obj17);
    obj1[3] = items7;
