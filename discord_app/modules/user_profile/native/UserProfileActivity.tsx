...
import closure_14 from "filterPlayingActivities" /* 5223 */;
import closure_15 from "updateVoiceState" /* 4497 */;
import closure_16 from "createUserWidgetFromServer" /* 7366 */;
...
function ActivityCard(user) {
  ...
  const tmp3 = activity(9412)(activity);
  const analyticsLocations = activity(5953)(activity(5973).USER_PROFILE_LIVE_ACTIVITY_CARD).analyticsLocations;
  let id;
  const tmp4 = activity(5953);
  if (voiceChannel != null) {
    id = voiceChannel.id;
  }
  const tmp5Result = activity(9418)({ display: "live", voiceChannelId: id, user, activity, analyticsLocations });
  ...
  if (activity.type !== constants.CUSTOM_STATUS) {
    ...
    if (activity.type !== tmp14.HANG_STATUS) {
      ...
      if (tmp2(9399)(activity)) {
        ...
        obj7[0] = start;
        obj7[1] = end;
        tmp34Result = tmp34(tmp2(9421), obj7);
      }
      ...
      if (null != voiceChannel) {
        ...
        if (null != stateFromStores) {
          ...
          tmp34Result1 = tmp34(tmp2(9423), obj8);
        }
      }
      items4[2] = tmp34Result1;
      if (tmp2(9387)(activity)) {
        ConnectPlatformButton = ConnectPlatformButton(9432).ConnectPlatformButton;
        obj = { type: null, onAction: null };
        obj[0] = constants2.XBOX;
        obj[1] = tmp5Result;
        let tmp34Result2 = tmp34(ConnectPlatformButton, obj);
      } else if (tmp2(9388)(activity)) {
        const obj9 = { type: null, onAction: null };
        obj9[0] = constants2.PLAYSTATION;
        obj9[1] = tmp5Result;
        tmp34Result2 = tmp34(ConnectPlatformButton(9432).ConnectPlatformButton, obj9);
      } else {
        tmp34Result2 = null;
        if (user.id !== currentUser.id) {
          if (tmp2(9256)(activity)) {
            ...
            obj10[0] = activity;
            obj10[1] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(9432).PlayOnSpotifyButton, obj10);
          } else if (tmp2(7484)(activity)) {
            ...
            obj11[4] = tmp5Result;
            tmp34Result2 = tmp34(ConnectPlatformButton(9432).JoinActivityButton, obj11);
          } else {
            if (tmp2(9248)(activity)) {
              const result1 = ConnectPlatformButton(9446);
              ...
              if (set.has(currentActivityGamePlatform)) {
                if (null != activity.party) {
                  ...
                  if (null != deepLinkUri) {
                    ...
                    obj12[4] = tmp5Result;
                    tmp34Result2 = tmp34(ConnectPlatformButton(9432).JoinGameActivityButton, obj12);
                  }
                }
              }
            }
            if (tmp2(8108)(activity)) {
              const obj13 = { activity: null, onAction: null };
              obj13[0] = activity;
              obj13[1] = tmp5Result;
              let tmp34Result3 = tmp34(ConnectPlatformButton(9432).WatchActivityButton, obj13);
            } else {
              tmp34Result3 = null;
      ...
      obj2[4] = obj4;
      obj2 = tmp34(ConnectPlatformButton(5068).PressableOpacity, obj2);
      obj1[1] = obj2;
      closure_21(ConnectPlatformButton(5953).AnalyticsLocationProvider, obj1);
...
function StreamActivityCard(user) {
  ...
  obj4 = { mode: "a", stream: "channel", disabled: "a", onPress: "isArray" };
  obj4[1] = stream;
...
function VoiceCallActivityCard(arg0) {
  ...
  stateFromStores(9420)(obj);
  obj1 = { style: items2, title: null, titleStyle: null, children: null };
  items2 = [tmp.card, style];
  const tmp9Result = stateFromStores(5953)(stateFromStores(5973).USER_PROFILE_VOICE_ACTIVITY_CARD);
  if (!channel.isDM()) {
    ...
    if (stateFromStores1) {
      ...
      obj9[0] = channel;
      obj8[1] = tmp18(tmp2(9259), obj9);
      const items4 = [tmp18(tmp17, obj8), tmp5];
      obj7[2] = items4;
      obj5[4] = tmp13(tmp6(4474).Text, obj7);
      let tmp13Result = tmp18(tmp6(5068).PressableOpacity, obj5);
    } else {
    ...
    if (null != stateFromStores) {
      ...
      tmp18Result = tmp18(tmp6(5068).PressableOpacity, obj13);
    }
    ...
    obj17[1] = isInChannel;
    obj17[2] = tmp11;
    items7[1] = closure_21(tmp6(9432).VoiceChannelButtons, obj17);
    obj1[3] = items7;
