...
export default function UserProfileOverflowMenu(user) {
  ...
  const stateFromStoresObject1 = user(channel[12]).useStateFromStoresObject(items1, () => ({
    selectedChannel: trackUserProfileAction.getChannel(newestAnalyticsLocation.getChannelId()),
    ...
    if (relationshipType !== tmp30.FRIEND) {
      ...
      if (isIgnored) {
        ...
        if (tmp43) {
          ...
          obj14.action = function action() {
            ...
            const tmp3 = asyncRequireImpl(11529, dependencyMap.paths);
          };
          push(obj14);
          if (result) {
      ...
      if (relationshipType !== id.BLOCKED) {
        ...
        obj18.action = function action() {
          ...
          const tmp4 = asyncRequireImpl(11530, dependencyMap.paths);
        };
        items7.push(obj18);
      }
    } else {
      ...
      const obj19 = {
        ...
        action() {
          ...
          ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(13265, dependencyMap.paths), {
            userId: id,
            showUserProfile,
          });
  ...
  if (!canRing) {
      ...
      if (tidaWebformEnabled) {
        ...
        obj21.action = function action() {
          ...
          obj.onPress = function onPress() {
            ...
            obj2.copy(
              displayProfile(channel[49]).getUserTag(bannerURL, { decoration: "never", identifiable: "always" }),
            );
          ...
          if (!hasAvatarForGuildResult) {
            ...
            hasAvatarForGuildResult = obj3.hasAvatarForGuild(guildId);
          }
          if (hasAvatarForGuildResult) {
            const obj4 = { label: null, onPress: null };
            const intl3 = user(channel[31]).intl;
            obj4.label = intl3.string(user(channel[31]).t.gERDvM);
          ...
          if (null != displayProfile) {
            ...
            if (null != bannerURL) {
              ...
              obj7.label = intl4.string(user(channel[31]).t.hsNv0R);
    ...
    if (0 !== items6.length) {
      ...
      obj26.children = function children(ref) {
        ...
        obj.icon = closure_1_19(user(channel[57]).MoreHorizontalIcon, {
          size: "sm",
          color: displayProfile(channel[58]).colors.WHITE,
        });
        return closure_1_19(user(channel[56]).IconButton, obj);
      };
      obj25.children = closure_19(tmp(tmp2[54]).ContextMenu, obj26);
      obj24.children = closure_19(displayProfile(tmp2[53]).View, obj25);
      ...
      items9[1] = closure_19(displayProfile(tmp2[59]), obj27);
  ...
  let obj6 = { disableTrack: !enabled };
  tmp30 = id;
  const tmp7Result8 = displayProfile(tmp2[28]);
}
