...
let obj2 = {
  ...
  friendRequestNote: { borderWidth: 1, borderColor: nativeDefault.colors.BORDER_STRONG },
};
...
export default function UserProfileIncomingFriendRequest(style) {
  ...
  trackUserProfileAction = isGameRelationship(trackUserProfileAction[6]).useUserProfileAnalyticsContext()
  ...
  const friendRequestActions = obj2.useFriendRequestActions({
    userId: user.id,
    applicationId,
    isGameRelationship,
  ...
  const items3 = [
    getOrFetchApplication(Text, tmp15),
    getOrFetchApplication(tmp(userResult[15]), {
      userId: user.id,
      styles: items4.friendRequestNote,
      analyticsLocation: "User Profile",
    }),
  ];
