// === Module 8930: InformationTable ===

// Module 8930 (InformationTable)
import ThemesDefault from "Themes" /* 712 */;
import registerAssetDefault from "registerAsset" /* 8933 */;
...
function InformationTable(speedBumpType) {
  ...
  obj = {
    ...
    children: items.map((item, index) => {
      const obj = { size: items(dependencyMap[15]).Icon.Sizes.MEDIUM, source: item.icon };
      obj[2] = closure_1_12(items(dependencyMap[15]).Icon, obj);
      obj[3] = item.text;
      return closure_1_12(items(dependencyMap[14]).TableRow, obj, index);
    })
  };
  return callback2(items(6286).TableRowGroup, obj);
}
let c4 = importAllResult;
({ AnalyticEvents: c10, EMPTY_STRING_SNOWFLAKE_ID: unpackModuleId } = ME);
({ jsx: closure_12, jsxs: map1 } = jsxProd);
let obj = { paddingHorizontal: ThemesDefault.space.PX_16, paddingBottom: 56 };
obj[0] = obj;
const createCacheKey = { marginBottom: ThemesDefault.space.PX_24, paddingHorizontal: ThemesDefault.space.PX_16 };
...
const memoResult = importAllResult.memo(function UserProfileSpeedBumpActionSheet(userId) {
  ...
  if (null == stateFromStores) {
    ...
    obj2[1] = items8;
    const items9 = [callback(guild_id, obj2), , ];
    ...
    obj1[1] = items9;
    const items10 = [callback(guild_id, obj1), , ];
    ...
    obj11[1] = items11;
    items10[2] = callback(guild_id, obj11);
    obj16[0] = items10;
    obj15[1] = callback(tmp2(tmp3[28]).BottomSheetView, obj16);
...
const result = require("obj132").fileFinishedImporting("modules/user_profile/native/UserProfileSpeedBumpActionSheet.tsx");
