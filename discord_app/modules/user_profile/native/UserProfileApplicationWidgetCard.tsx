...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  let id;
  let tmp7Result = tmp7(9318);
  if (data != null) {
    id = data.id;
  }
  obj = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: tmp2(9329).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  ...
  const tmp15 = surfaceConfigs[userId(undefined, 12503).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp16 = surfaceConfigs[userId(undefined, 12503).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp17 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        obj2[0] = cardStyle;
        tmp7Result = tmp7(5814);
        tmp2Result = tmp2(7125);
        ...
        const items3 = [callback(tmp2(12347).HourglassIcon, obj4), ];
  ...
  obj6[4] = userId(7125).getWidgetTitle(widget);
  const items4 = [tmp17, ];
  ...
  obj7[1] = str2;
  const tmp2Result2 = userId(7125);
  obj7[3] = userId(7125).getWidgetTitle(widget);
  items4[1] = callback(userId(4340).Text, obj7);
  obj6[5] = items4;
  ...
  if (null != tmp15) {
    if (tmp2(12418).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      ...
      obj8[2] = result;
      obj8[3] = memo;
      tmp27Result = tmp27(tmp7(12419), obj8);
    } else {
      tmp27Result = null;
      if (tmp2(12418).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        ...
        obj9[2] = result;
        obj9[3] = memo;
        tmp27Result = tmp27(tmp7(12509), obj9);
      }
    }
  }
  if (tmp16 != null) {
  ...
  if (null != tmp16) {
    if (tmp2(12418).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      ...
      obj10[1] = result;
      obj10[2] = memo;
      tmp27Result = tmp27(tmp7(12510), obj10);
      ...
      obj11[0] = tmp16;
      obj11[1] = result;
      tmp27Result = tmp27(tmp7(12511), obj11);
    } else {
      tmp27Result = null;
      if (tmp2(12418).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        ...
        obj12[0] = tmp16;
        obj12[1] = result;
        tmp27Result = tmp27(tmp7(12512), obj12);
      }
    }
  }
  tmp25Result = null;
  if (null != tmp27Result) {
    ...
    if (null != tmp27Result) {
      ...
      obj14[1] = items6;
      tmp25Result = tmp25(tmp7(5814), obj14);
      const tmp7Result1 = tmp7(5814);
    }
  }
  return tmp25Result;
};