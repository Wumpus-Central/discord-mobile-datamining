...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  let id;
  let tmp7Result = tmp7(9519);
  if (data != null) {
    id = data.id;
  }
  obj = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: tmp2(9101).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  const tmp7ResultResult = tmp7Result(obj);
  dependencyMap = tmp7ResultResult;
  ({ surfaceConfigs, resolutionContext, isLoading, hasIdentity } = stateFromStores(12283)(userId, widget.applicationId));
  const tmp15 = stateFromStores(7142)(getOrFetchApplication);
  token = tmp15.token;
  ({ fetched, canStartAuthorization } = tmp15);
  const tmp16 = surfaceConfigs[userId(undefined, 12273).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp17 = surfaceConfigs[userId(undefined, 12273).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp18 = null;
  if (null != iconURL) {
      ...
      if (!hasIdentity) {
        ...
        if (tmp21) {
          ...
          if (!someResult) {
            ...
            someResult = scopes.includes(tmp2(4375).OAuth2Scopes.SDK_SOCIAL_LAYER);
          }
          if (!someResult) {
            const scopes2 = token.scopes;
            someResult = scopes2.includes(tmp2(4375).OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE);
          }
          tmp21 = someResult;
          const arr = Array.from(tmp2(4375).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES);
        }
        if (fetched) {
        ...
        obj2[0] = cardStyle;
        tmp7Result = tmp7(7185);
        tmp2Result = tmp2(5369);
        ...
        const items3 = [callback(tmp2(11400).HourglassIcon, obj4), ];
  ...
  const tmp14 = stateFromStores(12283)(userId, widget.applicationId);
  const result = userId(12190).bindResolveFieldValue(resolutionContext);
  const obj6 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  if (null != tmp16) {
    if (tmp2(12188).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
  ...
  tmp32Result = null;
  if (null != tmp17) {
    if (tmp2(12188).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
  ...
  if (null != tmp32Result) {
    ...
    if (null != tmp32Result) {
      ...
      if (isFirstWidget) {
        ...
        isFirstWidget = layout === tmp2(12188).ApplicationWidgetLayoutName.WIDGET_TOP_HERO;
      }
      if (isFirstWidget) {
        const obj13 = { marginTop: null };
        obj13[0] = tmp2(12189).HERO_IMAGE_UNCLIPPED_TOP_BLEED;
        isFirstWidget = obj13;
      }
