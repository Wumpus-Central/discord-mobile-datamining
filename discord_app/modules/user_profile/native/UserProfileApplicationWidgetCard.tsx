// === Module 12666: UserProfileApplicationWidgetCard ===
...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  const memo = token.useMemo(() => userId(12669).createCompactNumberFormat(stateFromStores), items1);
  obj1 = userId(589);
  const items2 = [closure_8];
  const stateFromStores1 = obj1.useStateFromStores(items2, () => closure_1_8.getId() === userId);
  let obj2 = userId(5959);
  ...
  const tmp15 = surfaceConfigs[userId(undefined, 12752).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp16 = surfaceConfigs[userId(undefined, 12752).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp17 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        if (tmp20) {
          const _Array = Array;
          let someResult = Array.from(tmp2(8610).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES).some((arg0) => {
  ...
  const tmp13 = stateFromStores(12764)(userId, widget.applicationId);
  const result = userId(12669).bindResolveFieldValue(resolutionContext);
  const obj6 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  if (null != tmp15) {
    if (tmp2(12667).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      const obj8 = { header: null, topConfig: null, resolveFieldValue: null, numberFormat: null };
      obj8[0] = tmp31;
      obj8[1] = tmp15;
      obj8[2] = result;
      obj8[3] = memo;
      tmp30Result = tmp30(tmp9(12668), obj8);
    } else {
      tmp30Result = null;
      if (tmp2(12667).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        const obj9 = { header: null, topConfig: null, resolveFieldValue: null, numberFormat: null };
        obj9[0] = tmp31;
        obj9[1] = tmp15;
        obj9[2] = result;
        obj9[3] = memo;
        tmp30Result = tmp30(tmp9(12760), obj9);
      }
    }
  }
  if (tmp16 != null) {
  ...
  tmp30Result = null;
  if (null != tmp16) {
    if (tmp2(12667).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      const obj10 = { bottomConfig: null, resolveFieldValue: null, numberFormat: null };
      obj10[0] = tmp16;
      obj10[1] = result;
      obj10[2] = memo;
      tmp30Result = tmp30(tmp9(12761), obj10);
    } else if (tmp2(12667).ApplicationWidgetLayoutName.WIDGET_BOTTOM_PROGRESS === layout2) {
      const obj11 = { bottomConfig: null, resolveFieldValue: null };
      obj11[0] = tmp16;
      obj11[1] = result;
      tmp30Result = tmp30(tmp9(12762), obj11);
    } else {
      tmp30Result = null;
      if (tmp2(12667).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        const obj12 = { bottomConfig: null, resolveFieldValue: null };
        obj12[0] = tmp16;
        obj12[1] = result;
        tmp30Result = tmp30(tmp9(12763), obj12);
      }
    }
  }
