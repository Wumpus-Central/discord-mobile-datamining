// === Module 12579: UserProfileApplicationWidgetCard ===
...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  const memo = token.useMemo(() => userId(12582).createCompactNumberFormat(stateFromStores), items1);
  obj1 = userId(589);
  ...
  let id;
  let tmp7Result = tmp7(8726);
  if (data != null) {
    id = data.id;
  }
  obj = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: tmp2(8737).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  const tmp7ResultResult = tmp7Result(obj);
  dependencyMap = tmp7ResultResult;
  ({ surfaceConfigs, resolutionContext, isLoading, hasIdentity } = stateFromStores(12677)(userId, widget.applicationId));
  const tmp15 = stateFromStores(5908)(getOrFetchApplication);
  token = tmp15.token;
  ({ fetched, canStartAuthorization } = tmp15);
  const tmp16 = surfaceConfigs[userId(undefined, 12665).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp17 = surfaceConfigs[userId(undefined, 12665).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp18 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        if (tmp21) {
          const _Array = Array;
          let someResult = Array.from(tmp2(8541).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES).some((arg0) => {
            const scopes = token.scopes;
            return scopes.includes(arg0);
          });
          if (!someResult) {
            let scopes = token.scopes;
            someResult = scopes.includes(tmp2(8541).OAuth2Scopes.SDK_SOCIAL_LAYER);
          }
          if (!someResult) {
            const scopes2 = token.scopes;
            someResult = scopes2.includes(tmp2(8541).OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE);
          }
          tmp21 = someResult;
          const arr = Array.from(tmp2(8541).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES);
        }
        if (fetched) {
        ...
        obj2[0] = cardStyle;
        tmp7Result = tmp7(5952);
        tmp2Result = tmp2(7302);
        ...
        const items3 = [callback(tmp2(9317).HourglassIcon, obj4), ];
  ...
  const tmp14 = stateFromStores(12677)(userId, widget.applicationId);
  const result = userId(12582).bindResolveFieldValue(resolutionContext);
  const obj6 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  const tmp2Result1 = userId(12582);
  const tmp31 = closure_5;
  obj6[4] = userId(7302).getWidgetTitle(widget);
  const items4 = [tmp18, ];
  ...
  obj7[1] = str2;
  const tmp2Result2 = userId(7302);
  obj7[3] = userId(7302).getWidgetTitle(widget);
  items4[1] = callback(userId(4441).Text, obj7);
  obj6[5] = items4;
  ...
  if (null != tmp16) {
    if (tmp2(12580).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      const obj8 = { header: null, topConfig: null, resolveFieldValue: null, numberFormat: null };
      obj8[0] = tmp30Result;
      obj8[1] = tmp16;
      obj8[2] = result;
      obj8[3] = memo;
      tmp32Result = tmp32(tmp7(12581), obj8);
    } else {
      tmp32Result = null;
      if (tmp2(12580).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        const obj9 = { header: null, topConfig: null, resolveFieldValue: null, numberFormat: null };
        obj9[0] = tmp30Result;
        obj9[1] = tmp16;
        obj9[2] = result;
        obj9[3] = memo;
        tmp32Result = tmp32(tmp7(12673), obj9);
      }
    }
  }
  if (tmp17 != null) {
  ...
  tmp32Result = null;
  if (null != tmp17) {
    if (tmp2(12580).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      const obj10 = { bottomConfig: null, resolveFieldValue: null, numberFormat: null };
      obj10[0] = tmp17;
      obj10[1] = result;
      obj10[2] = memo;
      tmp32Result = tmp32(tmp7(12674), obj10);
    } else if (tmp2(12580).ApplicationWidgetLayoutName.WIDGET_BOTTOM_PROGRESS === layout2) {
      const obj11 = { bottomConfig: null, resolveFieldValue: null };
      obj11[0] = tmp17;
      obj11[1] = result;
      tmp32Result = tmp32(tmp7(12675), obj11);
    } else {
      tmp32Result = null;
      if (tmp2(12580).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        const obj12 = { bottomConfig: null, resolveFieldValue: null };
        obj12[0] = tmp17;
        obj12[1] = result;
        tmp32Result = tmp32(tmp7(12676), obj12);
      }
    }
  }
  tmp30Result = null;
  if (null != tmp32Result) {
    ...
    if (null != tmp32Result) {
      ...
      obj13[1] = items5;
      tmp30Result = tmp30(tmp7(5952), obj13);
      const tmp7Result1 = tmp7(5952);
    }
  }
  return tmp30Result;
};