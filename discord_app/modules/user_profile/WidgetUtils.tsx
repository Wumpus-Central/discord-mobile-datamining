// === Module 7302: findGameWidget ===

// Module 7302 (findGameWidget)
import GAME_WIDGET_LIMITS_BY_TYPE from "GAME_WIDGET_LIMITS_BY_TYPE" /* 5008 */;
import isAgeRestrictedContentClassification from "isAgeRestrictedContentClassification" /* 5010 */;
import items3 from "items" /* 7301 */;
import _modDef7306 from "module_7306" /* 7306 */;
import convertClip from "convertClip" /* 7307 */;
...
import closure_6 from "initialize" /* 7303 */;
import items from "items" /* 7304 */;
import { ContentDismissActionType } from "ContentDismissActionType" /* 1388 */;
import { USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH as closure_11 } from "USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH" /* 7305 */;
...
function findGameWidget(widgetType) {
  ...
  const found = widgets.filter(_require(7301).isGameWidget);
  ...
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === GAME_WIDGET_LIMITS_BY_TYPE.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
        ...
        obj[0] = tmp(5008).GAME_WIDGET_LIMITS_BY_TYPE[games.type];
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === _require(7300).WidgetType.PERSONAL) {
      obj = { dismissAction: null };
      obj[0] = ContentDismissActionType.INDIRECT_ACTION;
      const result = tmp15(4266).UNSAFE_markDismissibleContentAsDismissed(tmp15(1377).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_COACHMARK, obj);
      const tmp15Result = tmp15(4266);
    }
    const items = [type];
    HermesBuiltin.arraySpread(tmp7, 1);
    _modDef7306.setPendingWidgets(items);
    const obj4 = _modDef7306;
  }
};
export const removeWidgetFromPending = function removeWidgetFromPending(arg0) {
  ...
  _modDef7306.setPendingWidgets(found);
};
export const addPendingClipToClipsGalleryWidget = function addPendingClipToClipsGalleryWidget(arg0) {
  ...
  if (clips.length < closure_11) {
    ...
    _modDef7306.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = _modDef7306;
  }
};
export const updateClipTitleInClipsGalleryWidget = function updateClipTitleInClipsGalleryWidget(arg0, str) {
  ...
  if (null != found) {
    ...
    const clipsGalleryWidget = new _require(7307).ClipsGalleryWidget(obj);
    _modDef7306.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = _modDef7306;
  }
};
...
export const updatePersonalWidget = function updatePersonalWidget(arg0) {
  ...
  _modDef7306.setPendingWidgets(tmpResult);
};
export const updatePendingGameTags = function updatePendingGameTags(widgetType, arg1, tags) {
  const _require = arg1;
  if (tags.length <= Object.values(_require(7310).WidgetGameTag).length) {
    ...
    if (null != tmp5) {
      ...
      if (null != found) {
        ...
        obj.games = mapped;
        const baseGameWidget = new _require(7301).BaseGameWidget(obj);
        const tmp21 = replaceWidgetInList(baseGameWidget);
        obj(7306).setPendingWidgets(tmp21);
        const obj3 = obj(7306);
      }
    }
  }
    ...
    if (null != found) {
      if (null != found.tags) {
        if (0 !== found.tags.length) {
          ...
          if (found1.length <= Object.values(_require(7310).WidgetGameTag).length) {
            ...
            if (null != tmpResult) {
              ...
              if (null != found2) {
                ...
                obj.games = mapped;
                const baseGameWidget = new tmp4(7301).BaseGameWidget(obj);
                const tmp22 = replaceWidgetInList(baseGameWidget);
                obj(7306).setPendingWidgets(tmp22);
                const obj3 = obj(7306);
              }
            }
          }
  ...
  if (null != tmp) {
    ...
    if (null != found) {
      if (comment !== found.comment) {
        ...
        obj(7306).setPendingWidgets(tmp19);
        const obj3 = obj(7306);
      }
    }
  }
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  let num = 0;
  if (widgetType in game(5008).GAME_WIDGET_LIMITS_BY_TYPE) {
    num = tmp2(5008).GAME_WIDGET_LIMITS_BY_TYPE[widgetType];
  }
  if (null != tmp) {
    const games = tmp.games;
    let num2;
  ...
  obj.games = items1;
  const baseGameWidget = new tmp2(7301).BaseGameWidget(obj);
  const tmp7 = replaceWidgetInList;
  const tmp7Result = replaceWidgetInList(baseGameWidget);
  _modDef7306.setPendingWidgets(tmp7Result);
  const useGame = tmp2(6106).useGame;
          ...
          if (arg1 < items.length) {
            if (arg2 >= 0) {
              if (arg2 < items.length) {
                ...
                const tmp19 = replaceWidgetInList(baseGameWidget);
                _modDef7306.setPendingWidgets(tmp19);
                const obj2 = _modDef7306;
              }
            }
          }
        }
...
export const removePendingGameFromWidget = function removePendingGameFromWidget(widgetType) {
  ...
  if (null != merged) {
    ...
    _modDef7306.setPendingWidgets(tmp13);
    const obj2 = _modDef7306;
  }
};
