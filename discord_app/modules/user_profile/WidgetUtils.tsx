// === Module 7316: findGameWidget ===

// Module 7316 (findGameWidget)
import GAME_WIDGET_LIMITS_BY_TYPE from "GAME_WIDGET_LIMITS_BY_TYPE" /* 5021 */;
import isAgeRestrictedContentClassification from "isAgeRestrictedContentClassification" /* 5023 */;
import items3 from "items" /* 7315 */;
import _modDef7320 from "module_7320" /* 7320 */;
import convertClip from "convertClip" /* 7321 */;
...
import closure_6 from "initialize" /* 7317 */;
import items from "items" /* 7318 */;
import { ContentDismissActionType } from "ContentDismissActionType" /* 1388 */;
import { USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH as closure_11 } from "USER_WIDGET_CLIPS_GALLERY_MAX_LENGTH" /* 7319 */;
...
function findGameWidget(widgetType) {
  ...
  const found = widgets.filter(_require(7315).isGameWidget);
  ...
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === GAME_WIDGET_LIMITS_BY_TYPE.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
        ...
        obj[0] = tmp(5021).GAME_WIDGET_LIMITS_BY_TYPE[games.type];
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === _require(7314).WidgetType.PERSONAL) {
      let tmp13Result = tmp13(4267);
      obj = { dismissAction: null };
      obj[0] = ContentDismissActionType.INDIRECT_ACTION;
      const result = tmp13Result.UNSAFE_markDismissibleContentAsDismissed(tmp13(1377).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_COACHMARK, obj);
      tmp13Result = tmp13(4267);
      obj = { dismissAction: null };
      obj[0] = ContentDismissActionType.INDIRECT_ACTION;
      const result1 = tmp13Result.UNSAFE_markDismissibleContentAsDismissed(tmp13(1377).DismissibleContent.USER_PROFILE_PERSONAL_WIDGET_NEW_BADGE, obj);
    }
    const items = [type];
    HermesBuiltin.arraySpread(tmp7, 1);
    _modDef7320.setPendingWidgets(items);
    const obj2 = _modDef7320;
  }
};
export const removeWidgetFromPending = function removeWidgetFromPending(arg0) {
  ...
  _modDef7320.setPendingWidgets(found);
};
export const addPendingClipToClipsGalleryWidget = function addPendingClipToClipsGalleryWidget(arg0) {
  ...
  if (clips.length < closure_11) {
    ...
    _modDef7320.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = _modDef7320;
  }
};
export const updateClipTitleInClipsGalleryWidget = function updateClipTitleInClipsGalleryWidget(arg0, str) {
  ...
  if (null != found) {
    ...
    const clipsGalleryWidget = new _require(7321).ClipsGalleryWidget(obj);
    _modDef7320.setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = _modDef7320;
  }
};
...
export const updatePersonalWidget = function updatePersonalWidget(arg0) {
  ...
  _modDef7320.setPendingWidgets(tmpResult);
};
export const updatePendingGameTags = function updatePendingGameTags(widgetType, arg1, tags) {
  const _require = arg1;
  if (tags.length <= Object.values(_require(7324).WidgetGameTag).length) {
    ...
    if (null != tmp5) {
      ...
      if (null != found) {
        ...
        obj.games = mapped;
        const baseGameWidget = new _require(7315).BaseGameWidget(obj);
        const tmp21 = replaceWidgetInList(baseGameWidget);
        obj(7320).setPendingWidgets(tmp21);
        const obj3 = obj(7320);
      }
    }
  }
    ...
    if (null != found) {
      if (null != found.tags) {
        if (0 !== found.tags.length) {
          ...
          if (found1.length <= Object.values(_require(7324).WidgetGameTag).length) {
            ...
            if (null != tmpResult) {
              ...
              if (null != found2) {
                ...
                obj.games = mapped;
                const baseGameWidget = new tmp4(7315).BaseGameWidget(obj);
                const tmp22 = replaceWidgetInList(baseGameWidget);
                obj(7320).setPendingWidgets(tmp22);
                const obj3 = obj(7320);
              }
            }
          }
  ...
  if (null != tmp) {
    ...
    if (null != found) {
      if (comment !== found.comment) {
        ...
        obj(7320).setPendingWidgets(tmp19);
        const obj3 = obj(7320);
      }
    }
  }
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  let num = 0;
  if (widgetType in game(5021).GAME_WIDGET_LIMITS_BY_TYPE) {
    num = tmp2(5021).GAME_WIDGET_LIMITS_BY_TYPE[widgetType];
  }
  if (null != tmp) {
    const games = tmp.games;
    let num2;
  ...
  obj.games = items1;
  const baseGameWidget = new tmp2(7315).BaseGameWidget(obj);
  const tmp7 = replaceWidgetInList;
  const tmp7Result = replaceWidgetInList(baseGameWidget);
  _modDef7320.setPendingWidgets(tmp7Result);
  const useGame = tmp2(6119).useGame;
          ...
          if (arg1 < items.length) {
            if (arg2 >= 0) {
              if (arg2 < items.length) {
                ...
                const tmp19 = replaceWidgetInList(baseGameWidget);
                _modDef7320.setPendingWidgets(tmp19);
                const obj2 = _modDef7320;
              }
            }
          }
...
export const removePendingGameFromWidget = function removePendingGameFromWidget(widgetType) {
  ...
  if (null != merged) {
    ...
    _modDef7320.setPendingWidgets(tmp13);
    const obj2 = _modDef7320;
  }
};
