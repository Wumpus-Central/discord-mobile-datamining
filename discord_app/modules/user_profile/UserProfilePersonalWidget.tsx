...
function isSectionEmpty(type) {
  ...
  if (PersonalWidgetSectionType.PersonalWidgetSectionType.COVER === type) {
    ...
    return tmp4;
  } else if (tmp(7030).PersonalWidgetSectionType.FIELDS === type) {
      ...
      if (image === sections1.length) {
        ...
        if (image2.type === image.type) {
          ...
          if (PersonalWidgetSectionType.PersonalWidgetSectionType.COVER === type) {
            ...
            num2 = sum;
            flag = true;
          } else if (tmp16(7030).PersonalWidgetSectionType.FIELDS !== type) {
            flag = false;
          }
          tmp16 = require;
        }
...
export const parsePersonalWidgetSections = function parsePersonalWidgetSections(sections) {
  if (null == sections) {
    ...
    const mapped = sections.map((type) => {
      ...
      let title;
      type = type.type;
      if (callback(7030).PersonalWidgetSectionType.COVER === type) {
        let obj = { type: null, title: null, subtitle: null, image: null };
  ...
  if (!items.some((type) => type.type === callback(7030).PersonalWidgetSectionType.FIELDS)) {
