...
function migrateHotspotLocation(userContent, ACTIVITY_BEB_TUTORIAL, ACCOUNT_MULTIACCOUNT_TOOLTIP) {
  ...
  if (hasHiddenHotspotResult) {
    ...
    if (!tmpResult.hasBit(userContent.userContent.dismissedContents, ACCOUNT_MULTIACCOUNT_TOOLTIP)) {
      userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, ACCOUNT_MULTIACCOUNT_TOOLTIP);
      flag = true;
    ...
    run(userContent) {
      ...
      if (hasHiddenHotspotResult) {
        ...
        if (!tmpResult.hasBit(userContent.userContent.dismissedContents, HUB_WAITLIST_UPSELL)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, HUB_WAITLIST_UPSELL);
          flag2 = true;
          const tmpResult2 = Uint8ArrayUtils;
        }
        flag = flag2;
    ...
    run(userContent) {
      ...
      if (tmp4) {
        ...
        if (!tmpResult.hasBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_INVITE)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_INVITE);
          flag2 = true;
      ...
      if (tmp10) {
        ...
        if (!tmpResult7.hasBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_QUICKSWITCHER)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_QUICKSWITCHER);
          flag3 = true;
      ...
      if (tmp16) {
        const CHANNEL_NOTICE_PREMIUM_GUILD_SUBSCRIPTION = dismissible_content.DismissibleContent.CHANNEL_NOTICE_PREMIUM_GUILD_SUBSCRIPTION;
        if (null == userContent.userContent) {
        ...
        if (!tmpResult9.hasBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_PREMIUM_GUILD_SUBSCRIPTION)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, CHANNEL_NOTICE_PREMIUM_GUILD_SUBSCRIPTION);
          flag4 = true;
    ...
    run(userContent) {
      ...
      if (value) {
        ...
        if (!tmpResult.hasBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_PROMO)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_PROMO);
          flag = true;
          const tmpResult6 = Uint8ArrayUtils;
        }
        value = flag;
      ...
      if (value3) {
        ...
        if (!tmpResult7.hasBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_TIER_TWO_TRIAL_ENDING)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_TIER_TWO_TRIAL_ENDING);
          flag3 = true;
          const tmpResult8 = Uint8ArrayUtils;
        }
        value3 = flag3;
      ...
      if (value4) {
        ...
        if (!tmpResult9.hasBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_REACTIVATE)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, NAGBAR_NOTICE_PREMIUM_REACTIVATE);
          flag4 = true;
          const tmpResult10 = Uint8ArrayUtils;
        }
        value4 = flag4;
    ...
    run(userContent) {
      ...
      if (flag) {
        ...
        if (!tmpResult.hasBit(userContent.userContent.dismissedContents, FORUM_CHANNEL_HELPER_CARD)) {
          userContent.userContent.dismissedContents = Uint8ArrayUtils.addBit(userContent.userContent.dismissedContents, FORUM_CHANNEL_HELPER_CARD);
          flag3 = true;
          const tmpResult2 = Uint8ArrayUtils;
        }
        flag2 = flag3;
