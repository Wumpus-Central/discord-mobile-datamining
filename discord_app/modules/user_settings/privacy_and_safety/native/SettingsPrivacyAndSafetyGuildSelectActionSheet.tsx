// === Module 16281: SettingsPrivacyAndSafetyGuildSelectActionSheet ===
...
export default function SettingsPrivacyAndSafetyGuildSelectActionSheet() {
  ...
  if (selectedGuildId !== value) {
    ...
    callback = noop.useCallback((query) => {
      ...
      if (0 === query.length) {
        ...
        let reduced = flattenedGuildIds.reduce((arr, item) => {
          ...
          if (null != guild) {
            const obj = { type: first(5060).SelectOptionType.GUILD, value: null, label: null, guild: null };
        ...
        reduced = obj4.queryGuilds(obj5).map((record) => {
          ...
          return { type: first(5060).SelectOptionType.GUILD, value: record.id, label: record.name, guild: record };
        });
    ...
    const memo = noop.useMemo(() => callback(first), items1);
    const obj6 = {
