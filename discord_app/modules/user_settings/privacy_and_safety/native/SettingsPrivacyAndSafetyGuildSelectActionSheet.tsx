// === Module 16259: SettingsPrivacyAndSafetyGuildSelectActionSheet ===
    ...
    callback = noop.useCallback((query) => {
      ...
      if (0 === query.length) {
        ...
        let reduced = flattenedGuildIds.reduce((arr, item) => {
          ...
          if (null != guild) {
            const obj = { type: first(5058).SelectOptionType.GUILD, value: null, label: null, guild: null };
        ...
        reduced = obj4.queryGuilds(obj5).map((record) => {
          ...
          return { type: first(5058).SelectOptionType.GUILD, value: record.id, label: record.name, guild: record };
        });
