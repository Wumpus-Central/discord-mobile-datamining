...
fn(4560);
let createStyles = { flex: 1, paddingHorizontal: nativeDefault.space.PX_16, alignItems: "center", gap: nativeDefault.space.PX_24, marginBottom: null };
let PlatformUtils = fn(1115);
let num = 0;
if (!PlatformUtils.isIOS()) {
  num = nativeDefault.space.PX_16;
}
createStyles = { container: createStyles, landscapeContainer: { flexDirection: "row", gap: nativeDefault.space.PX_16 }, landscapePreview: { flex: 1 }, landscapeSelector: { flex: 1, justifyContent: "center", overflow: "hidden" }, segmentedControlContainer: null, textCentered: null, marginBottom: num };
PlatformUtils = { width: "100%", gap: nativeDefault.space.PX_16, alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, marginBottom: nativeDefault.space.PX_16 };
...
function ThemePicker(defaultIndex) {
  ...
  items = [navigation];
  stateFromStores = tmp3Result.useStateFromStores(items, () => defaultIndex(isPreview[24]).isThemeLight(navigation.systemTheme) ? _undefined.LIGHT : _undefined.DARKER);
  ...
  if ("nitro" === themeSelector) {
    obj2 = { themes: mobileThemes, currentThemeIndex: themeIndex, isPreview, isSynced, defaultIndex, deviceWidth: rounded, animatedStyles: obj1, hasMidnightNux, onThemeSelected: callback3 };
  ...
  const obj7 = { themes: mobileThemes, themeIndex, animatedStyles: obj1, data: memo3, useGradientBackground: "nitro" === themeSelector, isNitroLocked: null };
  let tmp48 = tmp14;
...
export default function ConnectedThemePicker(canGoBack) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => ({ userPreset: memo.gradientPreset, isPreview: memo.isPreview, usingSystemTheme: useSystemTheme.useSystemTheme === constants2.ON, isSynced: memo2.shouldSync("appearance"), userTheme: theme.theme, hasCustomTheme: memo1.hasCustomTheme() }));
