...
import HAPPENING_NOW_PANELS_CONTAINER_PADDING from "../../../../main_tabs_v2/native/shared_components/happening_now/HappeningNowConstants.tsx";
...
let obj = { borderRadius: ThemesDefault.radii.lg, borderWidth: StyleSheet.hairlineWidth, padding: HAPPENING_NOW_CARD_PADDING, paddingRight: HAPPENING_NOW_CARD_PADDING_RIGHT, marginRight: HAPPENING_NOW_CARD_MARGIN_RIGHT, height: HAPPENING_NOW_CARD_HEIGHT, flexDirection: "row", alignItems: "center" };
obj[0] = obj;
obj[1] = { position: "absolute", top: 0, right: 0 };
const createCacheKey = { height: HAPPENING_NOW_CONTENT_HEIGHT, minWidth: HAPPENING_NOW_CONTENT_HEIGHT, marginRight: ThemesDefault.space.PX_12, borderRadius: ThemesDefault.radii.md, position: "relative" };
...
const result = require("obj132").fileFinishedImporting("modules/user_settings/appearance/native/components/SettingsAppearanceActivityCardItem.tsx");
...
export default function ActivityCardItem(arg0) {
  ...
  if (null != avatars) {
    mapped = substr.map((item, index) => {
      ...
      let obj = { source: item, size: lib(num3[6]).AvatarSizes.XSMALL_20 };
      ...
      if (index !== diff) {
        ...
        obj1[0] = lib(num3[10]).CutoutShape.Circle;
        obj1[1] = lib(num3[6]).AVATAR_SIZE_MAP[lib(undefined, num3[6]).AvatarSizes.XSMALL_20] - 4 - 2;
        obj1[3] = lib(num3[6]).AVATAR_SIZE_MAP[lib(undefined, num3[6]).AvatarSizes.XSMALL_20] + 4;
        const items = [obj1];
        obj[0] = items;
        obj[1] = tmp5;
        let tmp2Result = closure_1_5(substr(num3[10]), obj);
        const tmp12 = substr(num3[10]);
      } else {
        tmp2Result = tmp5;
      }
      obj[1] = tmp2Result;
      return closure_1_5(closure_1_3, obj, index);
    });
  }
  ...
  if (num3 > 0) {
    ...
    obj5[4] = items6;
    obj4[1] = callback(tmp(tmp2[12]).Text, obj5);
    tmp8Result = callback(tmp7(tmp2[5]).View, obj4);
    const tmpResult = tmp(tmp2[13]);
  }
  const obj6 = { children: null };
  items4[1] = tmp8Result;
  obj3[1] = items4;
  const items7 = [callback(closure_3, obj3), callback(_require(num3[14]).HappeningNowCardHeader, { animated: true, style: animatedStyles.headerPrimary, children: title }), callback(_require(num3[14]).HappeningNowCardSubtitle, { animated: true, style: animatedStyles.headerSecondary, children: subtitle })];
  obj6[0] = items7;
  items3[1] = callback(closure_3, obj6);
  ...
  obj[1] = items3;
  return callback(substr(num3[5]).View, obj);
};