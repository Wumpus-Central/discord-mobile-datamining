...
const memoResult = importAllResult.memo(function SettingsAppearanceGradientBackground(isDimmed) {
  ...
  const memo1 = BACKGROUND_SURFACE_HIGH.useMemo(() => {
    ...
    return launchWelcomeSystemTheme(sharedValue1[6]).convertThemesToAnimatedThemes(
      sharedValue,
      prop,
      prop1,
      memo,
  ...
  const effect = obj.useEffect(() => {
    const result = sharedValue.set({
      themePrev: sharedValue.get().themeCurrent,
      themeCurrent: launchWelcomeSystemTheme,
    });
    ...
    const result2 = sharedValue1.set(
      launchWelcomeSystemTheme(sharedValue1[7]).withTiming(1, launchWelcomeSystemTheme(sharedValue1[8]).timingStandard),
    );
  }, items1);
      ...
      if (0 !== width) {
        if (0 !== height) {
          ...
          obj[0] = colors.map((arg0, arg1) => {
            ...
            let num = obj.processColor(
              launchWelcomeSystemTheme(sharedValue1[4]).interpolateColor(closure_1_6.get(), [0, 1], items),
            );
            if (num == null) {
              num = 0;
            }
  ...
  obj = {
    ...
    animatedLinearGradientLoadingProps: closure_8,
    themeState: sharedValue,
    interpolate: tmp5(tmp6[4]).interpolate,
    tweener: sharedValue1,
    getGradientStartPoint: sharedValue(tmp6[10]),
