// === Module 14376: GradientThemeBackground ===
...
function GradientThemeBackground(arg0) {
  ...
  const items1 = [closure_6(importDefault(4785), obj1), ];
  if (isThemeLocked) {
    const obj3 = { source: null, style: null };
    obj3[0] = importDefault(14377);
    obj3[1] = tmp4.lock;
...
function DefaultThemeBackground(item) {
  ...
  const token2 = require(3959) /* map */.useToken(importDefault(712).colors.ICON_STRONG, theme);
  ...
  if ("system" === item.theme) {
    ...
    obj2[0] = token2;
    obj1[1] = tmp9(importDefault(14378), obj2);
...
function CustomThemeBackground(arg0) {
  ...
  obj1[3] = item;
  const items1 = [closure_6(require(4785) /* getMixedGradientColor */.CustomThemedGradient, obj1), ];
  if (isThemeLocked) {
    const obj3 = { source: null, style: null };
    obj3[0] = importDefault(14377);
    obj3[1] = tmp4.lock;
...
export default function ThemeSelectorItem(onPress) {
  ...
  if (isPreview) {
    isPreview = themePreset.type !== require(4069) /* ClientThemeType */.ClientThemeType.STANDARD_BACKGROUND_THEME;
  }
  if (themePreset.type === require(4069) /* ClientThemeType */.ClientThemeType.STANDARD_BACKGROUND_THEME) {
    let obj = { item: null };
    obj[0] = themePreset;
  ...
  const radioA11yNative = require(3973) /* useCheckboxA11yNative */.useRadioA11yNative({ selected: isSelected, disabled: isPreview });
  ...
  return tmp9(require(4783) /* PressableBase */.PressableOpacity, obj1);
};