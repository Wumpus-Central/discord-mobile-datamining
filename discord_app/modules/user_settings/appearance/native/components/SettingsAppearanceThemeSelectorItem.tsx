...
function GradientThemeBackground(arg0) {
  ...
  const items1 = [closure_6(importDefault(4723), obj1), ];
  if (isThemeLocked) {
    const obj3 = { source: null, style: null };
    obj3[0] = importDefault(14310);
    obj3[1] = tmp4.lock;
...
function DefaultThemeBackground(item) {
  ...
  const token2 = require(3897) /* map */.useToken(importDefault(712).colors.ICON_STRONG, theme);
  ...
  if ("system" === item.theme) {
    ...
    obj2[0] = token2;
    obj1[1] = tmp9(importDefault(14311), obj2);
...
function CustomThemeBackground(arg0) {
  ...
  obj1[3] = item;
  const items1 = [closure_6(require(4723) /* getMixedGradientColor */.CustomThemedGradient, obj1), ];
  if (isThemeLocked) {
    const obj3 = { source: null, style: null };
    obj3[0] = importDefault(14310);
    obj3[1] = tmp4.lock;
...
export default function ThemeSelectorItem(onPress) {
  ...
  if (isPreview) {
    isPreview = themePreset.type !== require(4007) /* ClientThemeType */.ClientThemeType.STANDARD_BACKGROUND_THEME;
  }
  if (themePreset.type === require(4007) /* ClientThemeType */.ClientThemeType.STANDARD_BACKGROUND_THEME) {
    let obj = { item: null };
    obj[0] = themePreset;
    let tmp8 = callback(DefaultThemeBackground, obj);
    let tmp9 = callback;
  } else if (themePreset.type === tmp4(4007).ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
  ...
  const radioA11yNative = require(3911) /* useCheckboxA11yNative */.useRadioA11yNative({ selected: isSelected, disabled: isPreview });
  ...
  return tmp9(require(4721) /* PressableBase */.PressableOpacity, obj1);
};