...
const __initData = {
  code: "function SettingsAppearanceThemeCarouselTsx1(){const{withTiming,isOnyxNuxVisible,timingStandard}=this.__closure;return{opacity:withTiming(isOnyxNuxVisible.get()?1:0,timingStandard),pointerEvents:isOnyxNuxVisible.get()?'auto':'none'};}",
};
size = fn(2);
...
export default function SettingsAppearanceThemeCarousel(themes) {
  ...
  obj = {
    withTiming: themes(isPreview[12]).withTiming,
    isOnyxNuxVisible: sharedValue,
    timingStandard: themes(isPreview[13]).timingStandard,
  };
  L.__closure = obj;
  ...
  if (themes[currentThemeIndex].type !== themes(isPreview[14]).ClientThemeType.STANDARD_BACKGROUND_THEME) {
    obj1 = {
      source: currentThemeIndex(tmp3[15]),
      style: animatedStyles.iconHeaderSecondary,
      size: tmp2(tmp3[6]).IconSizes.SMALL_20,
    };
