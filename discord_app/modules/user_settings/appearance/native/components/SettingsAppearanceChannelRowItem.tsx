...
import FacepileGroupDMAvatar from "FacepileGroupDMAvatar" /* 9916 */;
...
const createCacheKey = { flexDirection: "row", gap: ThemesDefault.space.PX_4, borderRadius: ThemesDefault.radii.sm, paddingVertical: ThemesDefault.space.PX_8, paddingRight: ThemesDefault.space.PX_16, paddingLeft: ThemesDefault.space.PX_8 };
...
const result = require("obj132").fileFinishedImporting("modules/user_settings/appearance/native/components/SettingsAppearanceChannelRowItem.tsx");
...
export default function ChannelRowItem(isUnread) {
  ...
  const tmp2 = callback();
  let obj = { style: tmp2.channelItemLeft, children: null };
  ...
  if (null != avatar2) {
    ...
    let tmp5Result = callback(FacepileGroupDMAvatar.FacepileGroupDMAvatar, obj1);
    let tmp6 = require;
  } else {
    tmp6 = require;
    ...
    tmp5Result = callback(Button2.Avatar, obj2);
  }
  obj[1] = tmp5Result;
  items1[1] = callback(View, obj);
  const obj3 = { style: tmp2.channelItemContent, children: null };
  obj4 = { style: tmp2.channelItemTop, children: null };
  const items3 = [callback(tmp6(4734).Text, { animated: true, style: flag ? animatedStyles.textNormal : animatedStyles.textMuted, variant: "redesign/channel-title/semibold", children: title }), callback(tmp6(4734).Text, { animated: true, style: animatedStyles.textMuted, variant: "text-xs/medium", children: timestamp })];
  obj4[1] = items3;
  ...
  if (null == preview) {
    ...
    tmp5Result = callback(tmp6(4734).Text, animatedStyles);
  }
};