...
function UsernameStatusMessage(showHint) {
  ...
  const withResult = match.with(obj, (children) =>
    closure_1_11(showHint(4752).Text, { variant: "text-xs/medium", color: "text-feedback-critical", children }),
  );
  return match
    .with(obj, (children) =>
      closure_1_11(showHint(4752).Text, { variant: "text-xs/medium", color: "text-feedback-critical", children }),
    ...
    .with(obj2, (children) =>
      closure_1_11(showHint(4752).Text, { variant: "text-xs/medium", color: "text-feedback-positive", children }),
    )
    .otherwise(() => {
      let tmp = null;
      if (showHint) {
  ...
  onSubmitEditing = noop.useCallback(() => {
    if (isMatch) {
      ...
      navigation = closure_4(function* (arg0) {
        ...
        yield closure_2_2(7229).saveAccountChanges(user, { close: false });
        closure_129_0 = value;
        if (!closure_129_0.ok) {
