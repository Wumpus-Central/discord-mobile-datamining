...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(5213).CheckmarkSmallIcon, obj);
  }
  obj[2] = tmp2Result;
  return closure_6(code(5224).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  obj[1] = obj;
  const items2 = [headerLabel.map((children) => callback2(stateFromStores(4189).Text, { variant: "text-sm/medium", children }, arg1)), , , ];
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp3(5558).TableRowGroup, obj1);
  }
  ...
  if (tmp10Result1) {
    ...
    obj4[0] = tmp10(tmp3(4189).Text, obj5);
    obj4[1] = function onPress() {
      const verificationKey = store.getVerificationKey();
      const result = callback(13794).confirmViewBackupCodes(verificationKey, true);
    };
    obj3[1] = tmp10(tmp3(5224).TableRow, obj4);
    tmp10Result1 = tmp10(tmp3(5558).TableRowGroup, obj3);
  }
  ...
  obj[2] = items2;
  return closure_6(tmp11, { children: closure_7(stateFromStores(4602).Stack, obj) });
};