...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = closure_6(code(7243).CheckmarkSmallIcon, obj2);
  }
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  const items2 = [
    headerLabel.map((children, index) =>
      closure_1_6(stateFromStores(4634).Text, { variant: "text-sm/medium", children }, index),
    ),
    ,
    ,
  ];
  ...
  if (tmp10Result) {
    ...
    tmp10Result = closure_6(tmp3(5773).TableRowGroup, obj5);
  }
  ...
  if (tmp10Result3) {
    ...
    tmp10Result3 = closure_6(tmp3(5773).TableRowGroup, obj6);
  }
  ...
  if (tmp10Result4) {
    ...
    obj8.label = closure_6(tmp3(4634).Text, obj9);
    ...
    obj7.children = closure_6(tmp3(5691).TableRow, obj8);
    tmp10Result4 = closure_6(tmp3(5773).TableRowGroup, obj7);
  }
  const obj4 = { paddingHorizontal: token, paddingTop: nativeDefault.space.PX_16 };
  items2[3] = tmp10Result4;
  obj3.children = items2;
  return closure_6(ScrollView, { children: closure_7(stateFromStores(5058).Stack, obj3) });
}
