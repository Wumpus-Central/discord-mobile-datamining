...
const result = require("set").fileFinishedImporting(
  "modules/user_settings/account/native/mfa_modal_flow/TwoFASetupScan.tsx",
);
...
export default function TwoFASetupScan(totpSecret) {
  ...
  callback = React.useCallback(() => {
    dependencyMap(true);
    totpSecret(5981).copy(totpSecret.replace(/[^a-zA-Z0-9]/g, ""));
  }, items);
  let obj = totpSecret(14392);
  ...
  items2[2] = callback2(totpSecret(4474).Text, {
  ...
  items2[3] = callback2(totpSecret(5068).PressableOpacity, obj2);
  obj[2] = items2;
  obj4[0] = closure_5(totpSecret(5553).SafeAreaPaddingView, obj);
  return callback2(totpSecret(14388).TwoFASetupModalScreen, obj4);
}
