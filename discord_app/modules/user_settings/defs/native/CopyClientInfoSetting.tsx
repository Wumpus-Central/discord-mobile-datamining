...
function getClientInfo() {
  ...
  let tmp;
  if (overrides != null) {
    tmp = overrides[require(undefined, 11104) /* setBuildOverrideForBranch */.DEVICE_FIELD];
  }
  let str2 = "N/A";
...
function ClientClientInfoActionSheet() {
  ...
  obj[2] = function onPress() {
    callback(5677).copy(callback2().appVersion);
    const obj = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
  ...
  obj1[2] = function onPress() {
    callback(5677).copy(callback2().buildNumber);
    const obj = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
  ...
  obj2[2] = function onPress() {
    callback(5677).copy(callback2().releaseChannel);
    const obj = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
  ...
  obj3[2] = function onPress() {
    callback(5677).copy(callback2().manifest);
    const obj = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
  ...
  obj4[2] = function onPress() {
    callback(5677).copy(callback2().buildOverride);
    const obj = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
  ...
  if (obj7.isFabric()) {
    ...
    obj5[2] = function onPress() {
      callback(5677).copy("React Native New Architecture is enabled.");
      const obj = callback(5677);
      const result = callback(3984).presentCopiedToClipboard();
    };
    tmp4Result = tmp4(tmp2(5687).ActionSheetRow, obj5);
  }
  items[5] = tmp4Result;
  tmp4Result = null;
  if (tmp2Result.isFabric()) {
    let str = "Disabled";
    if (tmp2(4148).isBridgeless) {
      str = "Enabled";
    }
  ...
  obj8[2] = function onPress() {
    ...
    callback(5677).copy(combined);
    const obj3 = callback(5677);
    const result = callback(3984).presentCopiedToClipboard();
  };
