...
import closure_3 from "getCurrentBuildOverride" /* 11022 */;
...
function ClientClientInfoActionSheet() {
  ...
  obj1[2] = function onPress() {
    callback(5949).copy(callback2().appVersion);
    const obj = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
  ...
  obj2[2] = function onPress() {
    callback(5949).copy(callback2().buildNumber);
    const obj = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
  ...
  obj3[2] = function onPress() {
    callback(5949).copy(callback2().releaseChannel);
    const obj = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
  ...
  obj4[2] = function onPress() {
    callback(5949).copy(callback2().manifest);
    const obj = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
  ...
  obj5[2] = function onPress() {
    callback(5949).copy(callback2().buildOverride);
    const obj = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
  ...
  obj7[2] = function onPress() {
    ...
    const deviceInfo = callback(4427).getDeviceInfo();
    const obj = callback(4427);
    const combined = "App: " + appVersion + " (" + buildNumber + ") " + releaseChannel + "; Manifest: " + manifest + "; Build Override: " + buildOverride + "; " + "Device: " + deviceInfo + " OS " + callback(4427).getSystemVersion() + ";";
    const obj2 = callback(4427);
    callback(5949).copy(combined);
    const obj3 = callback(5949);
    const result = callback(4163).presentCopiedToClipboard();
  };
