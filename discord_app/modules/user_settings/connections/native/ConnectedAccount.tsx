...
function Integration(integration) {
  ...
  if (!tmp7) {
    ...
    if (tmp12) {
      ...
      tmp12 = callback3(tmp(4251).Text, obj);
    }
    ...
    obj2[1] = tmp(5632).GuildIconSizes.SMALL;
    obj2[2] = legacyClassComponentStyles.integrationGuildIcon;
    const items4 = [callback3(importDefault(5632), obj2), , ];
    ...
    const items5 = [callback3(tmp(4251).Text, obj4), ];
    const tmp16 = callback3;
    const tmp18 = importDefault(5632);
    const value = importDefault(4937).get(integration.type);
    let platformUserUrl;
    if (value != null) {
    ...
    items5[1] = tmp16(tmp(4251).Text, obj5);
    obj3[1] = items5;
    ...
    obj7[4] = stringResult;
    obj7 = tmp8(tmp(4666).Button, obj7);
    obj6[1] = obj7;
    callback3(View, obj6);
    const tmp9 = View;
  }
...
function RowSwitch(label) {
  ...
  return callback3(require(5621) /* TableSwitchRow */.TableSwitchRow, { label: label.label });
}
  ...
  if (account.account !== account) {
    ...
    if (account.verified) {
      ...
      if (null != inProgressMetadataVisibility) {
        ...
        const result = importDefault(5061).setMetadataVisibility(account.type, account.id, inProgressMetadataVisibility);
        const obj2 = importDefault(5061);
      }
    }
    self.setState(obj);
  }
...
prototype["renderUpsell"] = function renderUpsell() {
  ...
  if (account.twoWayLink) {
    ...
    let tmp2 = callback3(require(14082) /* XboxTwoWayLinkUpsell */.XboxTwoWayLinkUpsell, {});
  } else {
    tmp2 = null;
    if (account.type === tmp.PLAYSTATION) {
      tmp2 = callback3(require(14085) /* PlayStationTwoWayLinkUpsell */.PlayStationTwoWayLinkUpsell, {});
    }
  }
};
...
prototype["renderMetadata"] = function renderMetadata() {
    ...
    if (tmp6.BLUESKY !== type) {
      if (tmp6.TWITTER !== type) {
        if (tmp6.MASTODON !== type) {
          if (tmp6.EBAY === type) {
            redditMetadataItems = tmp2(10179).generateEbayMetadataItems(metadata);
            const tmp2Result1 = tmp2(10179);
          } else if (tmp6.PAYPAL === type) {
            redditMetadataItems = tmp2(10179).generatePaypalMetadataItems(metadata);
            const tmp2Result2 = tmp2(10179);
          } else {
            redditMetadataItems = [];
            if (tmp6.TIKTOK === type) {
              redditMetadataItems = tmp2(10179).generateTikTokMetadataItems(metadata);
              const tmp2Result3 = tmp2(10179);
            }
          }
        }
      }
    }
    redditMetadataItems = tmp2(10179).generateTwitterMetadataItems(metadata);
    const tmp2Result4 = tmp2(10179);
  }
  if (null !== createdAtDate) {
    ...
    redditMetadataItems.push(callback3(tmp2(4251).Text, obj, MetadataFields.CREATED_AT));
  }
  ...
  if (0 === redditMetadataItems.length) {
    ...
    if (self.state.metadataAlreadyRefreshed) {
      let tmp9Result = tmp9(9849);
    } else {
      tmp9Result = tmp9(14087);
    }
    obj10[2] = tmp9Result;
    ...
    const items3 = [callback3(View, obj12), callback3(tmp2(4783).PressableOpacity, obj9)];
    obj11[1] = items3;
...
prototype["renderIntegrationsRow"] = function renderIntegrationsRow() {
  ...
  if (account.integrations.length > 0) {
    ...
    if (!account.revoked) {
      ...
      const items1 = [callback3(require(4251) /* Text */.Text, obj1), ];
...
prototype["render"] = function render() {
  ...
  if (migrationExperimentEnabled) {
    ...
    obj[0] = account.name;
    let name = intl.format(tmp2(2805).Glhokn, obj);
  } else {
    name = account.name;
  }
  ...
  let obj4 = require(4101) /* AccessibilityAnnouncer */;
  ...
  obj3[1] = importDefault(7825);
  obj2[4] = closure_14(require(1297) /* Button */.Icon, obj3);
  obj1[3] = closure_14(require(4783) /* PressableBase */.PressableOpacity, obj2);
  tmp7Result = tmp7(tmp8(4664).Stack, obj1);
  ...
  obj6[1] = closure_14(require(5286) /* TableRowInner */.TableRow, { label: name, icon: tmp7Result, trailing: tmp7Result });
  ...
  items1[2] = callback4(require(5620) /* TableRowGroupTitle */.TableRowGroup, { hasIcons: false, children: items2 });
  items1[3] = result;
  obj7[1] = items1;
