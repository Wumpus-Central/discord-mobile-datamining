...
import { registerAsset } from "../../../../../_runtime/07967_registerAsset.js";
...
function Integration(integration) {
  ...
  if (!tmp7) {
    ...
    if (tmp12) {
      ...
      tmp12 = callback3(tmp(4298).Text, obj);
    }
    ...
    obj2[1] = tmp(5719).GuildIconSizes.SMALL;
    ...
    const items5 = [callback3(tmp(4298).Text, obj4), ];
    const tmp16 = callback3;
    const tmp18 = GuildIconSizes;
    const value = getPlatformUserUrl.get(integration.type);
    let platformUserUrl;
    if (value != null) {
    ...
    items5[1] = tmp16(tmp(4298).Text, obj5);
    obj3[1] = items5;
    ...
    obj7[4] = stringResult;
    obj7 = tmp8(tmp(4712).Button, obj7);
    obj6[1] = obj7;
    callback3(View, obj6);
    const tmp9 = View;
  }
  ...
  if (account.account !== account) {
    ...
    if (account.verified) {
      ...
      if (null != inProgressMetadataVisibility) {
        ...
        const result = callback.setMetadataVisibility(account.type, account.id, inProgressMetadataVisibility);
        const obj2 = callback;
      }
    }
    self.setState(obj);
...
prototype["renderMetadata"] = function renderMetadata() {
    ...
    if (tmp6.BLUESKY !== type) {
      if (tmp6.TWITTER !== type) {
        if (tmp6.MASTODON !== type) {
          if (tmp6.EBAY === type) {
            redditMetadataItems = tmp2(10981).generateEbayMetadataItems(metadata);
            const tmp2Result1 = tmp2(10981);
          } else if (tmp6.PAYPAL === type) {
            redditMetadataItems = tmp2(10981).generatePaypalMetadataItems(metadata);
            const tmp2Result2 = tmp2(10981);
          } else {
            redditMetadataItems = [];
            if (tmp6.TIKTOK === type) {
              redditMetadataItems = tmp2(10981).generateTikTokMetadataItems(metadata);
              const tmp2Result3 = tmp2(10981);
            }
          }
        }
      }
    }
    redditMetadataItems = tmp2(10981).generateTwitterMetadataItems(metadata);
    const tmp2Result4 = tmp2(10981);
  }
  if (null !== createdAtDate) {
    ...
    redditMetadataItems.push(callback3(tmp2(4298).Text, obj, MetadataFields.CREATED_AT));
  }
  ...
  if (0 === redditMetadataItems.length) {
    ...
    if (self.state.metadataAlreadyRefreshed) {
      let tmp9Result = tmp9(9958);
    } else {
      tmp9Result = tmp9(14145);
    }
    obj10[2] = tmp9Result;
    ...
    const items3 = [callback3(View, obj12), callback3(tmp2(4844).PressableOpacity, obj9)];
    obj11[1] = items3;
...
prototype["render"] = function render() {
  ...
  tmp7Result = tmp7(tmp8(4710).Stack, obj1);
