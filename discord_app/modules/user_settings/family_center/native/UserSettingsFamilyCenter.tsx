...
function FamilyCenter() {
  ...
  isLoading(5076)(() => {
    const result = DismissibleContentUnsafeUtils.UNSAFE_markDismissibleContentAsDismissed(
      ...
      { dismissAction: ContentDismissActionType.AUTO, forceTrack: true },
    );
    ...
    const obj2 = { dismissAction: ContentDismissActionType.AUTO, forceTrack: true };
  });
  if (familyCenterInitialized) {
    if (null != currentUser) {
      if (!tmp10) {
  ...
  const items3 = [
    closure_15(ref, {
      ...
      children: closure_15(familyCenterInitialized(9925).SegmentedControl, { state: segmentedControlState }),
    }),
  ];
  ...
  if (isLoading) {
    ...
    tmp14Result = closure_15(tmp4(12751).SegmentedControlPages, obj13);
  }
  obj12.children = tmp14Result;
  obj12 = closure_15(tmp23, obj12);
  items3[1] = obj12;
