...
let items1 = [
  fn(5754).GuildIconSizes.XSMALL,
  fn(5754).GuildIconSizes.SMALL_32,
  fn(5754).GuildIconSizes.NORMAL,
  fn(5754).GuildIconSizes.LARGE,
  fn(5754).GuildIconSizes.XLARGE,
];
...
export default function UserSettingsDesignSystemPile() {
  ...
  items[4] = hasOwnProperty(SampleCard, {
    ...
    children: items1.map((size) => {
      const obj = { children: null };
      items = [
        closure_5(size(4718).Text, { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() }),
      ];
      const obj2 = { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() };
      items[1] = closure_5(size(12860).GuildIconPile, {
        size,
        names: names2,
        totalCount: 128,
