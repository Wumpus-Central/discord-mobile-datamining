...
let items1 = [
  fn(5803).GuildIconSizes.XSMALL,
  fn(5803).GuildIconSizes.SMALL_32,
  fn(5803).GuildIconSizes.NORMAL,
  fn(5803).GuildIconSizes.LARGE,
  fn(5803).GuildIconSizes.XLARGE,
];
...
export default function UserSettingsDesignSystemPile() {
  ...
  items[4] = hasOwnProperty(SampleCard, {
    ...
    children: items1.map((size) => {
      const obj = { children: null };
      items = [
        closure_5(size(4753).Text, { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() }),
      ];
      const obj2 = { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() };
      items[1] = closure_5(size(12875).GuildIconPile, {
        size,
        names: names2,
        totalCount: 128,
