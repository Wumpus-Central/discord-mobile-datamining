...
let items = [
  fn(1177).AvatarSizes.XSMALL,
  fn(1177).AvatarSizes.REFRESH_MEDIUM_32,
  fn(1177).AvatarSizes.NORMAL,
  fn(1177).AvatarSizes.LARGE_48,
  fn(1177).AvatarSizes.XLARGE,
];
let items1 = [
  fn(5891).GuildIconSizes.XSMALL,
  fn(5891).GuildIconSizes.SMALL_32,
  fn(5891).GuildIconSizes.NORMAL,
  fn(5891).GuildIconSizes.LARGE,
  fn(5891).GuildIconSizes.XLARGE,
];
...
export default function UserSettingsDesignSystemPile() {
  ...
  items[4] = hasOwnProperty(SampleCard, {
    ...
    children: items1.map((size) => {
      const obj = { children: null };
      items = [
        closure_5(size(4825).Text, { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() }),
      ];
      const obj2 = { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() };
      items[1] = closure_5(size(12097).GuildIconPile, {
        size,
        names: names2,
        totalCount: 128,
  ...
  items1[1] = items2.map((item) => {
    ...
    obj2.children = DEFAULT_AVATARS.map((source, index) =>
      closure_1_5(closure_1_0(1177).Avatar, { source, size: closure_1_0(1177).AvatarSizes.LARGE_48 }, index),
    );
