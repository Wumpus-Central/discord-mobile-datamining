...
let items1 = [
  fn(5672).GuildIconSizes.XSMALL,
  fn(5672).GuildIconSizes.SMALL_32,
  fn(5672).GuildIconSizes.NORMAL,
  fn(5672).GuildIconSizes.LARGE,
  fn(5672).GuildIconSizes.XLARGE,
];
...
export default function UserSettingsDesignSystemPile() {
  ...
  items[4] = hasOwnProperty(SampleCard, {
    ...
    children: items1.map((size) => {
      const obj = { children: null };
      items = [
        closure_5(size(4636).Text, { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() }),
      ];
      const obj2 = { variant: "text-md/medium", color: "text-subtle", children: size.toLowerCase() };
      items[1] = closure_5(size(12769).GuildIconPile, {
        size,
        names: names2,
        totalCount: 128,
