...
function BackdropCard(arg0) {
  ...
  const items = [timestampProducer(Text_Text.Text, { variant: "heading-lg/bold", children: title }), timestampProducer(Text_Text.Text, { variant: "text-md/normal", color: "text-subtle", children: description }), ];
  obj = {
    text: buttonLabel,
    onPress() {
...
export default function UserSettingsDesignSystemBackdrop() {
  ...
  let obj = { withSpring: showBackdrop(5024).withSpring, showBackdrop, SUBTLE_SPRING: showBackdrop(5028).SUBTLE_SPRING };
  fn.__closure = obj;
  ...
  obj1.children = items;
  const items1 = [closure_7(showBackdrop(5023).Stack, obj1), ];
  if (showBackdrop) {
    ...
    const items2 = [closure_6(tmp8(5013).Backdrop, obj3), ];
    ...
    obj5.children = closure_7(tmp8(4601).Text, obj6);
    obj4.children = closure_6(tmp8(5657).Card, obj5);
    items2[1] = closure_6(closure_5, obj4);
    obj2.children = items2;
    showBackdrop = closure_7(tmp8(5008).Dialog, obj2);
  }
