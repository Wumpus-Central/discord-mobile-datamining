// discord_app/modules/user_settings/design_system/native/UserSettingsDesignSystemAILoader.tsx
...
let items = [
  ...
  { color: "text-subtle", label: "text-subtle" },
];
const size = fn(2);
const result = size.fileFinishedImporting(
  "modules/user_settings/design_system/native/UserSettingsDesignSystemAILoader.tsx",
);
...
export default function UserSettingsDesignSystemAILoader() {
  ...
  items = [
    ...
    React4(Text_Text.Text, {
      ...
      color: "text-subtle",
      children: "`size` is the glyph size in pixels; the gap between slots scales with it. Default 16.",
    }),
    closure_7.map((size) => {
  ...
  const items2 = [
    ...
    React4(Text_Text.Text, {
      variant: "text-md/medium",
      color: "text-subtle",
      children: "Any semantic text token. Defaults to `text-default`.",
    }),
    items.map((color) => {
