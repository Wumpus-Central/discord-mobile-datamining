...
function NitroHomeHeader(arg0) {
  ...
  const items1 = [
    closure_17(onClose(5425).PressableOpacity, {
      style: tmp.backButtonWrapper,
      onPress: callback,
      children: closure_17(onClose(5931).ArrowLargeLeftIcon, { size: "md", color: "white" }),
    }),
    ,
  ];
  const obj6 = {
  ...
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      style: tmp.backButtonWrapper,
      onPress() {
        AnalyticsUtilsDefault.track(constants.NITRO_HOME_NAVIGATION, {
  ...
  if (tmp11Result2) {
    const obj9 = {
      ...
      children: closure_17(onClose(13758).PremiumReferralTrialPill, { hasExtraMargin: true }),
    };
...
function NewTab(arg0) {
  ...
  if (isEligibleSenderForReferralProgram) {
    const obj8 = {
      ...
      children: closure_17(tmp2(13795).ReferralProgramPerkCard, {}),
    };
...
function PerksTab(extraBottomHeight) {
  ...
  _require = noop.useCallback((section_name) => {
    hasTrackedScrolledToBottom(dependencyMap[19]).track(constants.MOBILE_NITRO_HOME_PERKS_CAROUSEL_SCROLLED, {
      section_name,
    });
  }, []);
  importDefault = noop.useRef(false);
  const callback = noop.useCallback((nativeEvent) => {
    const result = PremiumNitroHomeUtils.trackIfScrolledToBottom({
      nativeEvent: nativeEvent.nativeEvent,
      trackedType: PerksTab,
      hasTrackedScrolledToBottom,
    });
  }, []);
  const obj2 = {
...
export default function PremiumNitroHome(onClose) {
  ...
  const stateFromStores1 = bottom(504).useStateFromStores(items2, () =>
  ...
  obj14.page = closure_17(PerksTab, {
    extraBottomHeight: tmp14 + 16,
    fractionalState: tmp21.fractionalState,
    isInReverseTrial,
  });
  items3[1] = obj14;
  obj11.items = items3;
  ...
  const effect2 = obj5.useEffect(() => {
    ...
    if (
      !tmpResult4.UNSAFE_isDismissibleContentDismissed(
  ...
  promotionMarketingComponent = bottom(13780).usePromotionMarketingComponent(
    tmp(11081).MarketingComponentType.PREMIUM_TAB,
  );
  ...
  const items8 = [
    closure_17(navigation(5890), obj18),
  ...
  const items11 = [
    closure_17(c5, {
      ...
      onLayout: callback,
      children: closure_17(bottom(9970).SegmentedControl, {
