...
import ReferralProgramUtils from "ReferralProgramUtils" /* 8318 */;
import PremiumPerkCardDefault from "PremiumPerkCard" /* 13674 */;
import useScrollToSectionDefault from "useScrollToSection" /* 13699 */;
import PremiumNitroHomeUtils from "PremiumNitroHomeUtils" /* 13700 */;
import MarketingPageBannerTileDefault from "MarketingPageBannerTile" /* 13701 */;
import PremiumPerkCarouselDefault from "PremiumPerkCarousel" /* 13727 */;
...
function NitroHomeHeader(arg0) {
  ...
  const mobileNitroManageSubscriptionsSettingsExperiment = onClose(13672).useMobileNitroManageSubscriptionsSettingsExperiment({ location: "NitroHomeHeader" });
  let hasActiveTrial;
  ...
  const obj2 = onClose(13672);
  const tmp8 = !hasActiveTrial;
  const items1 = [closure_17(onClose(5341).PressableOpacity, { style: tmp.backButtonWrapper, onPress: callback, children: closure_17(onClose(5847).ArrowLargeLeftIcon, { size: "md", color: "white" }) }), , ];
  ...
  items1[1] = closure_17(onClose(4753).Text, obj6);
  if (mobileNitroManageSubscriptionsSettingsExperiment) {
    ...
    const obj8 = {
      ...
      children: closure_17(onClose(7625).SettingsIcon, { size: "md", color: "white" })
    };
    tmp11Result = closure_17(onClose(5341).PressableOpacity, obj8);
  }
  ...
  if (tmp11Result2) {
    const obj9 = { style: tmp.pillParent, children: closure_17(onClose(13673).PremiumReferralTrialPill, { hasExtraMargin: true }) };
...
function NewTab(arg0) {
  ...
  if (tmp12) {
    ...
    const obj7 = { width: tmp2(13674).PERK_CARD_SIZES[tmp2(undefined, 13674).PerkCardVariant.WIDE].width };
    obj6.style = obj7;
  ...
  if (isEligibleSenderForReferralProgram) {
    const obj8 = { onLayout: useScrollToSectionDefault(ref, scrollToSectionId).createSectionLayoutHandler(NitroHomeSectionId.REFERRAL_PROGRAM), children: closure_17(tmp2(13710).ReferralProgramPerkCard, {}) };
...
function PerksTab(extraBottomHeight) {
  ...
  if (fractionalState === FractionalPremiumStates.FP_ONLY) {
    ...
    items4[2] = closure_17(tmp9(13727), obj5);
    const obj6 = { title: null, perks: null, onItemChange: null };
    const tmp9Result3 = tmp9(13727);
...
export default function PremiumNitroHome(onClose) {
  ...
  const commonTriggerPoint = bottom(13730).useCommonTriggerPoint(bottom(13731).OpenNitroTriggerPoint);
  let obj = bottom(13730);
  const giftCardMobileConsumptionHalfsheet = bottom(7630).useGiftCardMobileConsumptionHalfsheet();
  const tmp5 = closure_24();
  bottom = navigation(1612)().bottom;
  let obj2 = bottom(7630);
  const youBarSettingsCustomHeaderPaddingTop = bottom(13732).useYouBarSettingsCustomHeaderPaddingTop();
  let obj3 = bottom(13732);
  ...
  const tmp21 = navigation(7638)();
  const obj7 = bottom(504);
  const isInReverseTrial = bottom(8328).useIsInReverseTrial();
  const obj8 = bottom(8328);
  const maybeFetchTieredTenureBadgeData = bottom(13733).useMaybeFetchTieredTenureBadgeData();
  const field = PremiumNitroNavigationStore.useField("scrollToSectionId");
  const effect = stateFromStores.useEffect(() => () => {
    closure_1_9.resetState();
  }, []);
  const obj9 = bottom(13733);
  ...
  const segmentedControlState = bottom(9896).useSegmentedControlState(obj11);
  const obj10 = bottom(9896);
  ...
  sharedValue = bottom(4493).useSharedValue(num);
  const obj16 = bottom(4493);
  class Q {
    constructor() {
  ...
  const animatedStyle = bottom(4493).useAnimatedStyle(Q);
  ...
  let tmpResult = bottom(4493);
  const theme = bottom(4608).useThemeContext().theme;
  let tmpResult6 = bottom(4608);
  const isThemeDarkResult = bottom(4608).isThemeDark(theme);
  const tmpResult7 = bottom(4608);
  const tmp6Result = navigation(13734);
  const tmp6ResultResult = navigation(13734)(tmp21.endsAt, bottom(13734).CountDownMessageTypes.ENDS_IN);
  isEligibleSenderForReferralProgram = bottom(8319).useIsEligibleSenderForReferralProgram();
  ...
  const tmpResult8 = bottom(8319);
  promotionMarketingComponent = bottom(13695).usePromotionMarketingComponent(tmp(11005).MarketingComponentType.PREMIUM_TAB);
  ...
  const tmpResult9 = bottom(13695);
  obj18.source = navigation(13735);
  const items8 = [closure_17(navigation(5806), obj18), closure_17(NitroHomeHeader, { onClose: onClose.onClose, subscription: stateFromStores1 }), , , ];
  ...
  if (tmp26Result) {
    ...
    tmp26Result = closure_17(tmp6(13736), obj19);
  }
  items8[2] = tmp26Result;
  const obj20 = { style: tmp5.tabContent, children: null };
  const items9 = [closure_17(bottom(12873).SegmentedControlPages, { state: segmentedControlState }), ];
  if (tmp26Result4) {
  ...
  const tmp6Result3 = navigation(5806);
  const items11 = [closure_17(c5, { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9897).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) }), , ];
  ...
  if (!isThemeDarkResult) {
    ...
    items12[1] = obj25;
    obj24.style = items12;
    tmp26Result5 = closure_17(tmp6(5199), obj24);
  }
  items11[1] = tmp26Result5;
  const obj23 = { style: tmp5.segmentedControlActual, onLayout: callback, children: closure_17(bottom(9897).SegmentedControl, { state: segmentedControlState, variant: "experimental_Small" }) };
  if (tmpResult10.isAndroid()) {
    ...
    tmp26Result6 = closure_17(tmp6(5174), obj28);
    const tmp6Result4 = tmp6(5174);
  }
  items11[2] = tmp26Result6;
  obj22.children = items11;
  items8[4] = closure_18(navigation(4493).View, obj22);
