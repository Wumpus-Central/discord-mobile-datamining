...
export default function PremiumFeaturesWumpus(premiumType) {
  ...
  const memo = noop.useMemo(() => {
    if (premiumType === PremiumTypes.TIER_0) {
      let obj = { wumpusImageSource: _modDef9476, cloudsImageSource: importDefault(closure_1 ? 9477 : 9478) };
    } else {
      if (closure_1) {
        let tmp4 = 9479;
      } else {
      ...
      obj = { wumpusImageSource: _modDef9481, cloudsImageSource: importDefault(tmp4) };
      return obj;
    }
  }, items);
  ({ wumpusImageSource, cloudsImageSource } = memo);
  let obj = { style: tmp.clouds, resizeMode: "contain", source: cloudsImageSource };
  const items1 = [closure_4(premiumType(5638), obj)];
