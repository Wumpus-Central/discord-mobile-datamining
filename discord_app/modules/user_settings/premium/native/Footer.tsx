...
export default function Footer(showSubscribeButton) {
  ...
  const tmp = callback2();
  importDefault(12637)(importDefault(5601).PREMIUM_MARKETING_FOOTER);
  ...
  if (showSubscribeButton) {
    ...
    obj2[0] = tmp7;
    obj2[3] = tmp6;
    obj1[1] = callback(require(4666) /* Button */.Button, obj2);
  ...
  const tmp4 = importDefault(12637);
  if (!showSubscribeButton) {
    easterEggSpacing = tmp.easterEggSpacing;
  }
  const tmp2Result = importDefault(5207);
  items2[1] = tmp14(tmp2Result, { style: easterEggSpacing, source: importDefault(12638) });
  obj[1] = items2;
