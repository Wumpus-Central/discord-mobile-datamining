...
function CarouselCard(arg0) {
  ...
  const tmp = callback5();
  const obj = { variant: "heading-md/extrabold", color: "text-overlay-light", style: tmp.cardTitle, children: title };
...
function PremiumFeaturesCarousel(arg0) {
  ...
  mapped = memo.map((item, index) => {
    const obj = { width: width * closure_1_12 };
    const merged = Object.assign(item);
    return closure_1_10(CarouselCard, obj, index);
  });
  ...
  if (!width(1625).isThumbstickScrollDevice) {
    ...
    obj1[2] = first;
    items3[1] = tmp17(tmp2(1297).CarouselPagination, obj1);
    obj[1] = items3;
    return callback(closure_6, obj);
  }
  tmp19 = callback2(closure_5, { style: tmp.carousel, contentContainerStyle: { paddingHorizontal: bound, gap: PX_12 }, horizontal: true, showsHorizontalScrollIndicator: false, decelerationRate: "fast", snapToOffsets: mapped.map((item, index) => index * c4), scrollEventThrottle: 100, onScroll: callback, children: mapped });
  tmp17 = callback2;
...
let createCacheKey = { flex: 1, marginTop: ThemesDefault.space.PX_16 };
...
let result = require("obj132").fileFinishedImporting("modules/user_settings/premium/native/PremiumFeaturesCarouselSection.tsx");

export default function PremiumFeaturesCarouselSection(style) {
  let first;
  let callback;
  const tmp = callback4();
  const analyticsLocations = first(7139)().analyticsLocations;
