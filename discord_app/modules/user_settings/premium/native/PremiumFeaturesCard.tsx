...
function PriceText(fractionalPremiumInfo) {
  ...
  const tmp4 = importDefault(7571)(premiumItem.basePlanId);
  ...
  if (null != discountedPriceString) {
    if (null != discountOffer) {
      ...
      return callback2(tmp7(4281).Text, obj);
    }
  }
    ...
    if (null != premiumSubscription) {
      if (premiumSubscription.planIdFromItems === PREMIUM_YEAR_TIER_2.PREMIUM_YEAR_TIER_2) {
        ...
        if (flag) {
          ...
          obj2[3] = formatResult;
          return callback2(tmp7(4281).Text, obj2);
        }
      }
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(7966).getPriceString(premiumSubscription);
          ...
          obj3[2] = priceString1;
          formatResult = intl4.format(importDefault(2867).FwjZzr, obj3);
  ...
  if (null != subscriptionTrial) {
    if (premiumType === table[subscriptionTrial.sku_id]) {
      ...
      obj7[0] = tmp7(3931).formatIntervalDuration(obj6);
      let priceString2;
      if (tmp4 != null) {
      ...
      let tmp11Result = callback2(tmp7(4281).Text, obj5);
      const tmp16 = callback2;
      const tmp7Result2 = tmp7(3931);
    }
    return tmp11Result;
  }
  ...
  const items = [callback2(require(4281) /* Text */.Text, { variant: "text-md/bold", color: "text-overlay-light", children: priceString3 }), ];
  const obj9 = { variant: "text-md/normal", color: "text-overlay-light", children: null };
  obj9[2] = " / " + intervalStringAsNoun;
  items[1] = callback2(require(4281) /* Text */.Text, obj9);
  obj8[2] = items;
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  analyticsLocations = tmp4(5610)().analyticsLocations;
  tmp8Result = tmp8(589);
  ...
  const tmp12 = importDefault(6769)();
  ...
  const tmp23 = importDefault(8435)(premiumType, flag, UNSPECIFIED);
  ...
  if (null != first && stateFromStores && tmp19[1] && !isBoostOnly) {
    ...
    if (null != first) {
      ...
      if (tmp33) {
        tmp33 = tmp8(3931).getPremiumType(planIdFromItems) === premiumType;
        const tmp8Result6 = tmp8(3931);
      }
      flag4 = tmp33;
    }
    tmp32 = flag4;
  }
  const tmp34 = importDefault(7571)(premiumBundleWithPredicate.basePlanId);
  ...
  const tmp8Result5 = premiumType(7324);
  if (flag3) {
    let tmp37Result = tmp37(tmp4(8436), { width: 185, height: 20, alwaysWhite: true });
  } else {
  ...
  obj4[1] = tmp37Result;
  const items5 = [callback2(analyticsLocations, obj4), callback2(importDefault(8551), { premiumType }), , , ];
  if (flag3) {
  ...
  items5[3] = callback2(importDefault(8556), { style: tmp3.featureList, features: tmp23, iconStyle: tmp3.featureIcon, labelStyle: tmp3.featureLabel, rowStyle: tmp3.featureRow });
  if (hideButton) {
    items5[4] = !hideButton;
    obj3[1] = items5;
    ...
    if (tmp37Result1) {
      ...
      obj8[2] = tmp37(tmp8(4281).Text, obj9);
    ...
    if (tmp32) {
      ...
      obj11[3] = tmp37(tmp8(4281).Text, obj12);
      let obj13 = obj11;
    } else {
      ...
      if (tmp37Result1) {
        ...
        if (null != premiumDiscountOffer) {
          ...
          const tmp37Result2 = tmp37(tmp8(7625).NitroWheelIcon, obj15);
        }
