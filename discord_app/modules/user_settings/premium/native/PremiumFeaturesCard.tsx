...
function PriceText(fractionalPremiumInfo) {
  ...
  if (null != activeDiscountInfo) {
    if (null != premiumSubscription) {
      ...
      if (premiumSubscription.hasAnyPremiumGroup) {
        ...
        if (null != prop) {
          let priceString1 = tmp7(8079).getPriceString(premiumSubscription);
  ...
  if (null != subscriptionTrial) {
    if (premiumType === table[subscriptionTrial.sku_id]) {
      ...
      obj7[0] = tmp7(3930).formatIntervalDuration(obj6);
      let priceString2;
      if (tmp4 != null) {
...
export default function PremiumFeaturesCard(premiumType) {
  ...
  analyticsLocations = tmp4(5649)().analyticsLocations;
  tmp8Result = tmp8(589);
  ...
  premiumBundleWithPredicate = premiumType(6792).getPremiumBundleWithPredicate((additionalPlans) => {
    let interval;
    let numPremiumGuild;
    let premiumTier;
  ...
  const tmp8Result4 = premiumType(6792);
  ...
  if (null != first && stateFromStores && tmp19[1] && !isBoostOnly) {
    ...
    if (null != first) {
      ...
      if (tmp33) {
        tmp33 = tmp8(3930).getPremiumType(planIdFromItems) === premiumType;
        const tmp8Result6 = tmp8(3930);
      }
      flag4 = tmp33;
    }
    tmp32 = flag4;
  ...
  const items4 = [callback2(premiumType(6823).PremiumPill, obj1), ];
  ...
  if (flag3) {
    let tmp37Result = tmp37(tmp4(8549), { width: 185, height: 20, alwaysWhite: true });
  } else {
    const obj5 = { premiumType: null, style: null };
    obj5[0] = premiumType;
    obj5[1] = tmp3.logo;
    tmp37Result = tmp37(tmp4(8550), obj5);
  }
  obj4[1] = tmp37Result;
