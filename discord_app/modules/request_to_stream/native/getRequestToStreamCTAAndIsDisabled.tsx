...
import _modDef2973 from "../RequestToStream.messages.js";
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    let text = intl6.string(_modDef2973.u4QmWl);
    let isDisabled = true;
  } else if (id.author.id === id) {
    const intl5 = util.intl;
    text = intl5.string(_modDef2973["8HU1M2"]);
    isDisabled = true;
  } else {
    isDisabled = false;
    text = stringResult;
    if (!tmp3[0]) {
      if (useCanFulfillStreamRequest.StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        ...
        if (useCanFulfillStreamRequest.StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
          ...
          text = intl7.string(_modDef2973["fac+eE"]);
          isDisabled = true;
        }
      }
    }
