// === Module 13552: getRequestToStreamCTAAndIsDisabled ===

// Module 13552 (getRequestToStreamCTAAndIsDisabled)
import SnowflakeUtilsDefault from "SnowflakeUtils" /* 11 */;
import util from "util" /* 1115 */;
import _modDef2968 from "module_2968" /* 2968 */;
import isInviteActive from "isInviteActive" /* 12011 */;
import useCanFulfillStreamRequest from "useCanFulfillStreamRequest" /* 12023 */;
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    let text = intl6.string(_modDef2968.u4QmWl);
    let isDisabled = true;
  } else if (id.author.id === id) {
    const intl5 = util.intl;
    text = intl5.string(_modDef2968["8HU1M2"]);
    isDisabled = true;
  } else {
    isDisabled = false;
    text = stringResult;
    if (!tmp3[0]) {
      if (useCanFulfillStreamRequest.StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        ...
        if (useCanFulfillStreamRequest.StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
          ...
          text = intl7.string(_modDef2968["fac+eE"]);
          isDisabled = true;
        }
      }
    }
