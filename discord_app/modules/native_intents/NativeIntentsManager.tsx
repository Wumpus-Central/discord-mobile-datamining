...
function indexingEnabled() {
  let obj = importDefault(16820);
  obj = { autoTrackExposure: true, disable: null };
  obj[1] = !importDefault(16821).hasSearch();
...
function makeSearchItem(channel, guild, flag) {
  ...
  const channelName1 = require(4475) /* computeChannelName */.computeChannelName(channel, mergeGuildAvatar, upsertRelationship, false);
  ...
  const channelIconURL = require(12218) /* getChannelIconURL */.getChannelIconURL(channel, 128, false);
...
function setChannelActivity(channelId) {
  let obj = importDefault(16820);
  obj = { autoTrackExposure: true, disable: null };
  obj[1] = !importDefault(16821).hasUserActivity();
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj).activityEnabled) {
    ...
    if (null != channel) {
      ...
      const obj5 = require(4475) /* computeChannelName */;
      ...
      obj[5] = items3;
      obj[6] = sum;
      tmp(16821).setActivity(obj);
      obj6 = upsertRelationship;
      tmp10 = require;
      tmp11 = mergeGuildAvatar;
      const tmpResult1 = tmp(16821);
    } else {
      tmp(16821).resignActivity();
      const tmpResult2 = tmp(16821);
    }
  }
}
function indexChannelUpdates(items) {
  if (indexingEnabled()) {
    ...
    if (items.length > 0) {
      importDefault(16821).indexDomains(items);
      const obj3 = importDefault(16821);
    }
    if (items1.length > 0) {
      importDefault(16821).deleteSearchItems(items1);
      const obj4 = importDefault(16821);
    }
  }
}
...
prototype["handleInit"] = function handleInit() {
  ...
  if (tmp2Result.getCurrentConfig({ location: "NativeIntentsManager" }, obj).searchEnabled) {
    ...
    const tmp2Result2 = tmp2(16820);
    obj1[1] = !tmp2(16821).hasSearch();
    if (tmp2Result2.getCurrentConfig({ location: "NativeIntentsManager" }, obj1).searchEnabled) {
      ...
      obj2[1] = items;
      mapped.push(obj2);
      importDefault(16821).indexDomains(mapped);
      const obj12 = importDefault(16821);
    }
    const tmp2Result3 = tmp2(16821);
  }
};
prototype["handleLogout"] = function handleLogout() {
  ...
  obj[1] = !importDefault(16821).hasSearch();
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj).clearEnabled) {
    importDefault(16821).clearSearchIndex();
    const tmpResult = importDefault(16821);
  }
};
...
prototype["handleChannelDelete"] = function handleChannelDelete(channel) {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj).searchEnabled) {
    ...
    importDefault(16821).deleteSearchItems(items);
    const tmpResult = importDefault(16821);
  }
};
...
prototype["handleGuildDelete"] = function handleGuildDelete(guild) {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj).searchEnabled) {
    ...
    importDefault(16821).deleteSearchDomains(items);
    const tmpResult = importDefault(16821);
  }
};
