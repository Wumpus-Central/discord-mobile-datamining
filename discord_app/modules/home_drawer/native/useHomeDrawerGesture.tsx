...
export const useHomeGesture = function useHomeGesture() {
  ...
  let obj5 = panelX(updateMaxX[6]);
  class W {
    constructor() {
      return keys.get() >= outer1_9;
    }
  }
  obj = { dragOffsetX: sharedValue, INITIAL_OPEN_WIDTH: navigation };
  W.__closure = obj;
  W.__workletHash = 2525772891435;
  W.__initData = isFocused;
  class C {
  ...
  C.__initData = sharedValue2;
  const animatedReaction = obj5.useAnimatedReaction(W, C);
  let tmp9 = gestureState(updateMaxX[10])();
  obj = tmp9;
  const tmp10 = gestureState(updateMaxX[11])();
  let c12 = tmp10;
  if (enableHome) {
    enableHome = !gestureState(updateMaxX[12])().isChatBesideChannelList;
  }
  let tmpResult = tmp(tmp2[7]);
  isFocused = enableHome;
  if (enableHome) {
    isFocused = tmpResult.useIsFocused();
  }
  tmpResult = tmp(tmp2[6]);
  sharedValue2 = tmpResult.useSharedValue(0);
  let tmp13 = enableHome;
  if (enableHome) {
    tmp13 = enablePeekHint;
  }
  const homeDrawerPeekHint = panelX(updateMaxX[13]).useHomeDrawerPeekHint(tmp13, sharedValue2);
  const tmpResult1 = panelX(updateMaxX[13]);
  function ne() {
  ...
  ne.__initData = derivedValue;
  derivedValue = panelX(updateMaxX[6]).useDerivedValue(ne);
  let state = navigation.getState();
  const tmpResult2 = panelX(updateMaxX[6]);
  let tmp17;
  if (state != null) {
  ...
  let coerceGuildsRouteResult = panelX(updateMaxX[14]).coerceGuildsRoute(tmp17);
  ...
  const items3 = [enableHome, tmp19, num2, panelX];
  ...
  const tmpResult3 = panelX(updateMaxX[14]);
  class Ie {
    constructor() {
      if (closure_8.get()) {
        tmp = panelX;
  ...
  obj1 = { isDrawerInitialized: sharedValue1, initialPanelX: num2, withSpring: tmp(tmp2[15]).withSpring, panelX, gestureState, HOME_GESTURE_DRAG_PHYSICS: callback, HOME_DRAWER_FLING_PHYSICS: obj };
  Ie.__closure = obj1;
  Ie.__workletHash = 11078734703352;
  Ie.__initData = closure_17;
  const derivedValue1 = panelX(updateMaxX[6]).useDerivedValue(Ie);
  const tmpResult4 = panelX(updateMaxX[6]);
  function ge() {
  ...
  ge.__initData = num2;
  const derivedValue2 = panelX(updateMaxX[6]).useDerivedValue(ge);
  const tmpResult5 = panelX(updateMaxX[6]);
  class Se {
    constructor() {
  ...
  const animatedStyle = panelX(updateMaxX[6]).useAnimatedStyle(Se);
  const tmpResult6 = panelX(updateMaxX[6]);
  class Ae {
    constructor() {
      obj = closure_16;
      tmp = HapticFeedbackTypes;
      num = 0;
      if (closure_16.get() < HapticFeedbackTypes) {
        tmp2 = globalThis;
        _Math = Math;
        num2 = 12;
      ...
      obj4 = panelX(updateMaxX[15]);
      obj1[0] = obj4.withSpring(num, outer1_11);
      items = [, ];
      items[0] = obj1;
      obj2 = { scale: null };
      obj6 = panelX(updateMaxX[15]);
      obj2[0] = obj6.withSpring(num3, outer1_11);
      items[1] = obj2;
      obj[0] = items;
      obj7 = panelX(updateMaxX[15]);
      obj8 = panelX(updateMaxX[6]);
      obj[1] = obj7.withSpring(obj8.clamp(result, 0, 1), outer1_11);
      return obj;
    }
  }
  obj2 = { drawerRevealX: derivedValue, MAX_HOME_DRAWER_ANIMATING_WIDTH: c12, GUILD_DESCRIPTION_ANIMATION_DISTANCE: 12, MIN_HOME_DRAWER_TEXT_OPACITY: 0, MIN_HOME_DRAWER_OPEN_WIDTH: 150, GUILD_LABEL_UNSNAPPED_SCALE: 0.98, withSpring: tmp(tmp2[15]).withSpring, HOME_DRAWER_FLING_PHYSICS: obj, clamp: tmp(tmp2[6]).clamp };
  Ae.__closure = obj2;
  Ae.__workletHash = 11639414708115;
  Ae.__initData = derivedValue1;
  let closure_23 = panelX(updateMaxX[6]).useAnimatedStyle(Ae);
  const tmpResult7 = panelX(updateMaxX[6]);
  class De {
    constructor() {
      obj = closure_16;
      num = 0;
  ...
  const tmpResult8 = panelX(updateMaxX[6]);
  De.__closure = { drawerRevealX: derivedValue, MAX_HOME_DRAWER_ANIMATING_WIDTH: c12, MAX_HOME_DRAWER_ICON_DISTANCE: 8, withSpring: panelX(updateMaxX[15]).withSpring, HOME_DRAWER_FLING_PHYSICS: obj };
  De.__workletHash = 16360618332998;
  De.__initData = derivedValue2;
  let closure_24 = tmpResult8.useAnimatedStyle(De);
  const obj3 = { drawerRevealX: derivedValue, MAX_HOME_DRAWER_ANIMATING_WIDTH: c12, MAX_HOME_DRAWER_ICON_DISTANCE: 8, withSpring: panelX(updateMaxX[15]).withSpring, HOME_DRAWER_FLING_PHYSICS: obj };
  function pe() {
    let obj = derivedValue;
    let num = 0;
    if (derivedValue.get() < closure_12) {
      const _Math = Math;
      num = -Math.min(4, obj.get() / tmp * 4);
    }
    obj = { transform: null };
    obj = { translateX: null };
    obj[0] = panelX(updateMaxX[15]).withSpring(num, closure_11);
    const items = [obj];
    obj[0] = items;
    return obj;
  }
  obj4 = { drawerRevealX: derivedValue, MAX_HOME_DRAWER_ANIMATING_WIDTH: c12, UNREAD_INDICATOR_ANIMATION_DISTANCE: 4, withSpring: tmp(tmp2[15]).withSpring, HOME_DRAWER_FLING_PHYSICS: obj };
  pe.__closure = obj4;
  pe.__workletHash = 8561772429103;
  pe.__initData = animatedStyle;
  let closure_25 = panelX(updateMaxX[6]).useAnimatedStyle(pe);
  ...
  const memo = obj4.useMemo(() => {
    ...
    let obj = { panelX: settleDrawer, maxX, runOnJS: panelX(updateMaxX[6]).runOnJS, setHomeDrawerState: panelX(updateMaxX[14]).setHomeDrawerState };
    settleDrawer.__closure = obj;
    settleDrawer.__workletHash = 8226756372642;
    settleDrawer.__initData = closure_23;
    ...
    obj = { panelX: settleDrawer, FRACTION_OF_WIDTH_FOR_DRAWER_TO_REMAIN_OPEN: 0.5, maxX, INITIAL_OPEN_WIDTH: navigation, dragOffsetX: sharedValue };
    ...
    fn3.__closure = { gestureState: shouldOpenFromPosition, dragOffsetX: sharedValue, INITIAL_OPEN_WIDTH: navigation, panelX: settleDrawer, DRAWER_RESISTANCE: 3 };
    ...
    const fn4 = function t(velocityX) {
      if (velocityX.velocityX > 50) {
        ...
        panelX(updateMaxX[6]).runOnJS(panelX(updateMaxX[14]).setHomeDrawerState)(flag);
        if (flag) {
          panelX(updateMaxX[6]).runOnJS(outer1_10)(enableHome.OPEN);
          const obj5 = panelX(updateMaxX[6]);
        } else if (!tmp37) {
          panelX(updateMaxX[6]).runOnJS(outer1_10)(enableHome.PEEK);
      ...
      if (flag) {
        ...
        if (!tmp5) {
          let tmp8 = settleDrawer.get() >= navigation;
          if (tmp8) {
            tmp8 = outer1_7.get() > 0;
          }
          tmp5 = tmp8;
    ...
    fn4.__closure = { FLING_MIN_VELOCITY: 50, dragOffsetX: sharedValue, FLING_MIN_DISTANCE: 40, INITIAL_OPEN_WIDTH: navigation, runOnJS: panelX(updateMaxX[6]).runOnJS, triggerHapticFeedback: panelX(updateMaxX[9]).triggerHapticFeedback, HapticFeedbackTypes: panelX(updateMaxX[9]).HapticFeedbackTypes, shouldOpenFromPosition, settleDrawer, gestureState: shouldOpenFromPosition, trackServerDrawerInteract: callback, ServerDrawerInteractAction: enableHome };
    fn4.__workletHash = 11139594764616;
    fn4.__initData = memo;
    const obj1 = { FLING_MIN_VELOCITY: 50, dragOffsetX: sharedValue, FLING_MIN_DISTANCE: 40, INITIAL_OPEN_WIDTH: navigation, runOnJS: panelX(updateMaxX[6]).runOnJS, triggerHapticFeedback: panelX(updateMaxX[9]).triggerHapticFeedback, HapticFeedbackTypes: panelX(updateMaxX[9]).HapticFeedbackTypes, shouldOpenFromPosition, settleDrawer, gestureState: shouldOpenFromPosition, trackServerDrawerInteract: callback, ServerDrawerInteractAction: enableHome };
    const fn5 = function e() {
      ...
      if (tmp) {
        ...
        panelX(updateMaxX[6]).runOnJS(panelX(updateMaxX[14]).setHomeDrawerState)(tmp5);
        const obj2 = panelX(updateMaxX[6]);
        tmp3 = settleDrawer;
      }
    ...
    fn5.__closure = { panelX: settleDrawer, maxX, settleDrawer, shouldOpenFromPosition, isPanelTouchActive, runOnJS: panelX(updateMaxX[6]).runOnJS, noteInteraction, gestureState: shouldOpenFromPosition, dragOffsetX: sharedValue };
    fn5.__workletHash = 15210416953749;
    fn5.__initData = closure_25;
    return onEndResult.onFinalize(fn5);
  }, items5);
  obj5 = { gesture: memo, panelStyles: animatedStyle, homeDrawerState: maxX(obj4.useState(() => ({ gesture: memo, panelStyles: animatedStyle, gestureState, panelX, panelSpringTranslateX: derivedValue2, guildsBarLabelDrawerStyle: closure_23, guildsBarIconDrawerStyle: closure_24, guildsBarUnreadDrawerStyle: closure_25 })), 1)[0] };
  return obj5;
};
