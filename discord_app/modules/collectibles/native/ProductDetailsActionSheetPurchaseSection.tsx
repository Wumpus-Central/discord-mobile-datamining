...
import useIsEligibleForBogoOffer from "useIsEligibleForBogoOffer" /* 7627 */;
...
import useIsPremiumSubscriber from "useIsPremiumSubscriber" /* 9468 */;
import useVirtualCurrencyData from "useVirtualCurrencyData" /* 9583 */;
import useHandleUseNow from "useHandleUseNow" /* 10395 */;
import useHandleClaim from "useHandleClaim" /* 11993 */;
import useCanGiftProduct from "useCanGiftProduct" /* 11996 */;
import UnlockWithNitroButton from "UnlockWithNitroButton" /* 11997 */;
...
function GiftButton(onTrackPress) {
  ...
  obj[2] = callback(require(onTrackPress[13]).GiftIcon, { size: "md", color: TEXT_STRONG });
  obj[3] = function onPress() {
    if (onTrackPress != null) {
      tmp(closure_1_9.SEND_AS_GIFT);
    }
    let obj = importDefault(onTrackPress[14]);
    obj.hideActionSheet(require(onTrackPress[15]).PRODUCT_DETAILS_ACTION_SHEET_KEY);
    obj = { skuId: skuId.skuId, analyticsLocations: closure_1 };
    require(onTrackPress[16]).openShopGiftModal(obj);
    const obj2 = require(onTrackPress[16]);
  };
  const intl = require(tmp3[17]).intl;
  obj[4] = intl.string(require(onTrackPress[17]).t.PEjaCx);
  return callback(require(onTrackPress[12]).IconButton, obj);
}
  ...
  closure_6 = navigation.useCallback(() => {
    ...
    obj = {
      ...
      onCheckoutSuccess(arg0) {
        ...
        obj1 = closure_1_1(onTrackPress[23]);
        obj1.popWithKey(closure_1_20);
        if (skuId.skuId === str.ORB_PROFILE_BADGE) {
          let tmp4Result = closure_1_1(onTrackPress[23]);
          obj = { modalKey: null, onPressViewBadge: null, orbBalancePriorToPurchase: null };
          obj[0] = closure_1_19;
          obj[1] = function onPressViewBadge(dependencyMap) {
            return navigation.navigate(closure_1_13.YOU, { showOrbsBadgeCoachmark: true });
          };
          obj[2] = closure_0;
          tmp4Result.pushLazy(balance(onTrackPress[25])(onTrackPress[27], onTrackPress.paths), obj, closure_1_19);
        } else {
          const ALL = balance(onTrackPress[28]).FractionalPremiumSKUsSets.ALL;
          if (ALL.has(skuId.skuId)) {
            tmp4Result = closure_1_1(onTrackPress[14]);
            ...
            tmp4Result.openLazy(balance(onTrackPress[25])(onTrackPress[29], onTrackPress.paths), "FractionalNitroCollectedActionSheet", obj);
            const tmp9 = balance(onTrackPress[25])(onTrackPress[29], onTrackPress.paths);
          } else {
            obj1 = { product: null, useCategoryImage: true, showOrbBalancePill: true, orbBalancePriorToPurchase: null };
            obj1[0] = skuId;
            obj1[3] = closure_0;
            closure_1_1(onTrackPress[30]).open(obj1);
            const tmp4Result1 = closure_1_1(onTrackPress[30]);
          }
        }
      }
    };
    product(onTrackPress[23]).pushLazy(balance(onTrackPress[25])(onTrackPress[24], onTrackPress.paths), obj, ORB_CHECKOUT_MODAL);
  }, items);
  if (null == price) {
    return null;
  } else {
...
function PurchaseDisclaimer(arg0) {
  ...
  if (formatResult) {
    ...
    formatResult = intl.format(getSystemLocale.t.iIglwJ, obj);
  }
  obj[3] = formatResult;
...
const createCacheKey = { backgroundColor: ThemesDefault.colors.MOBILE_ACTIONSHEET_BACKGROUND, paddingHorizontal: ThemesDefault.space.PX_16, paddingTop: ThemesDefault.space.PX_8 };
...
let result = require("obj132").fileFinishedImporting("modules/collectibles/native/ProductDetailsActionSheetPurchaseSection.tsx");
...
export default function ProductDetailsActionSheetPurchaseSection(onBuy) {
  ...
  const isThemeDarkResult = obj.isThemeDark(onTrackPress(4310)());
  obj1 = map;
  ...
  let id;
  const tmp4 = onTrackPress(4310)();
  if (tmp16 != null) {
    id = tmp16.id;
  }
  obj[3] = id;
  const tmp17 = onTrackPress(11992);
  ({ handleBuyNow: c4, isBuying } = onTrackPress(11992)(obj));
  let tmp5Result = useHandleClaim;
  handleClaim = tmp5Result.useHandleClaim({ product }).handleClaim;
  const tmp20 = onTrackPress(11994)({ location: "ProductDetailsActionSheetPurchaseSection", product });
  let tmp25Result = tmp20;
  if (tmp20) {
    tmp25Result = !isPurchased;
  }
  tmp5Result = useIsEligibleForBogoOffer;
  const tmp17Result = onTrackPress(11992)(obj);
  const tmp23 = tmp5Result.useIsEligibleForBogoOffer() ? constants.MONTH : constants.YEAR;
  let canGiftProduct = useCanGiftProduct.useCanGiftProduct(product);
  let PX_16 = onTrackPress(1629)().bottom;
  ...
  if (PX_16 == null) {
    PX_16 = onTrackPress(712).space.PX_16;
  }
  ...
  if (tmp25Result) {
    ...
    let tmp2Result = onTrackPress(4756);
    ...
    obj6[7] = items5;
    obj5[0] = callback2(inlineStyles.RadialGradient, obj6);
    ...
    obj4[4] = items6;
    items4[1] = callback2(tmp2Result, obj4);
    obj3[0] = items4;
    tmp25Result = callback2(closure_18, obj3);
  }
  ...
  if (isPurchased) {
    ...
    if (tmp25Result) {
      ...
      if (canGiftProduct) {
        ...
        canGiftProduct = callback2(GiftButton, obj12);
      }
      items8[1] = canGiftProduct;
      obj9[1] = items8;
      tmp25Result = callback2(tmp26, obj9);
    }
    ...
    items9[HermesBuiltin.arraySpread(analyticsLocations, 0)] = onTrackPress(7159).PROFILE_FRAMES_EA_MARKETING;
    obj13[0] = items9;
    ...
    obj16[0] = onTrackPress(1993).getArticleURL(constants.SHOP_FRAMES_EARLY_ACCESS);
    ...
    if (!result1) {
      ...
      if (!result2) {
        ...
        if (canGiftProduct) {
          ...
          tmp38Result = callback2(GiftButton, obj25);
        }
        items12[1] = tmp38Result;
        obj23[1] = items12;
        tmp25Result1 = callback2(tmp26, obj23);
      }
      ...
      items11[3] = tmp44;
      obj21[1] = items11;
      tmp25Result2 = callback2(tmp26, obj21);
    }
  ...
  obj[1] = items7;
  return callback2(handleClaim, obj);
};