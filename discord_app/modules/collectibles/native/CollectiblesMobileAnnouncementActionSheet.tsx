...
function CatEarsBackdrop() {
  ...
  const width = top(1474)().width;
  let obj = animatedPosition(5340);
  ...
  let obj1 = animatedPosition(4146);
  const fn = function t() {
  ...
  return callback(top(4146).View, obj);
}
...
export default function CollectiblesMobileAnnouncementActionSheet(markAsDismissed) {
  ...
  const callback = React.useCallback(() => {
    ...
    let obj = markAsDismissed(6909);
    obj = { screen: outer1_6.FEATURED_PAGE, analyticsLocations: [], analyticsSource: callback(5630).ACTION_SHEET };
  ...
  const obj2 = { uri: null };
  obj2[0] = require("../../../../discord_assets/assets/collectibles/frames/ea/announcement_key_1.png.js");
  obj1[0] = obj2;
  ...
  items3[1] = callback(markAsDismissed(4281).Text, obj3);
  const obj4 = { style: tmp.featureRows, children: null };
  const obj5 = { icon: callback(markAsDismissed(5936).PaintIllocon, { size: 32 }), text: null };
  const intl4 = tmp2(1236).intl;
  obj5[1] = intl4.string(markAsDismissed(1236).t["6ZWB0C"]);
  const items4 = [callback(FeatureRow, obj5), callback(FeatureRow, { icon: callback(markAsDismissed(5936).HeartIllocon, { size: 32 }), text: stringResult }), ];
  const obj6 = { icon: callback(markAsDismissed(5936).HeartIllocon, { size: 32 }), text: stringResult };
  items4[2] = callback(FeatureRow, { icon: callback(markAsDismissed(5936).ShopIllocon, { size: 32 }), text: string2Result });
  obj4[1] = items4;
  ...
  obj8[2] = callback;
  items3[3] = callback(markAsDismissed(4695).Button, obj8);
  obj[1] = items3;
