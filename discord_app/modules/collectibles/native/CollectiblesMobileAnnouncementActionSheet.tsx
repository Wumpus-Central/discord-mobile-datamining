...
function CatEarsBackdrop() {
  ...
  let obj1 = animatedPosition(4054);
  const fn = function t() {
  ...
  return callback(top(4054).View, obj);
}
...
export default function CollectiblesMobileAnnouncementActionSheet(markAsDismissed) {
  ...
  const callback = React.useCallback(() => {
    ...
    let obj = markAsDismissed(5802);
    obj = { screen: outer1_6.FEATURED_PAGE, analyticsLocations: [], analyticsSource: callback(5539).ACTION_SHEET };
  ...
  items3[1] = callback(markAsDismissed(4189).Text, obj3);
  const obj4 = { style: tmp.featureRows, children: null };
  const obj5 = { icon: callback(markAsDismissed(6790).PaintIllocon, { size: 32 }), text: null };
  const intl4 = tmp2(1236).intl;
  obj5[1] = intl4.string(markAsDismissed(1236).t["6ZWB0C"]);
  const items4 = [callback(FeatureRow, obj5), callback(FeatureRow, { icon: callback(markAsDismissed(6790).HeartIllocon, { size: 32 }), text: stringResult }), ];
  const obj6 = { icon: callback(markAsDismissed(6790).HeartIllocon, { size: 32 }), text: stringResult };
  items4[2] = callback(FeatureRow, { icon: callback(markAsDismissed(6790).ShopIllocon, { size: 32 }), text: string2Result });
  obj4[1] = items4;
  ...
  obj8[2] = callback;
  items3[3] = callback(markAsDismissed(4604).Button, obj8);
  obj[1] = items3;
