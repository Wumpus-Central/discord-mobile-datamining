...
export const ShopCategory = function ShopCategory(category) {
  ...
  let obj6 = category(14709);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  obj = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: tmp4(14709).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  const scrollToInitialIndexOnce = obj6.useScrollToInitialIndexOnce(obj);
  collectiblesAnalyticsContext = category(9236).useCollectiblesAnalyticsContext();
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5221), obj6);
  }
  const items4 = [tmp14Result, ];
  const tmp4Result = category(9236);
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(5645).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj3[8] = items4;
  const items5 = [collectiblesAnalyticsContext(category(4797).PressableOpacity, obj3, category.storeListingId), ];
  ...
  obj9[5] = callback;
  obj9[8] = category(9233).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  obj9[10] = HeaderAndFooterSpacing;
  obj9[11] = HeaderAndFooterSpacing;
  obj9[12] = Spacing;
  obj9[13] = productIndex;
  items5[1] = ref(category(7638).FlashList, obj9);
  obj2[1] = items5;
  obj1[1] = collectiblesAnalyticsContext(filteredAndSortedProducts, obj2);
  return ref(category(9236).CollectiblesAnalyticsProvider, obj1);
};