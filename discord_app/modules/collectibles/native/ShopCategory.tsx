...
export const ShopCategory = function ShopCategory(category) {
  ...
  let obj6 = category(14729);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  obj = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: tmp4(14729).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5236), obj6);
  }
  const items4 = [tmp14Result, ];
  const tmp4Result = category(9265);
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(5660).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj3[8] = items4;
  const items5 = [collectiblesAnalyticsContext(category(4812).PressableOpacity, obj3, category.storeListingId), ];
