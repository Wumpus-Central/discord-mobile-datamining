...
function ProductPreviewInner(arg0) {
  ...
  if (product.type === cardWidth(1889).CollectiblesItemType.BUNDLE) {
    ...
    if (ALL.has(product.skuId)) {
      let size = {
        skuId: product.skuId,
        width: tmp2(8846).FRACTIONAL_NITRO_COIN_SIZE.CARD,
        height: tmp2(8846).FRACTIONAL_NITRO_COIN_SIZE.CARD,
      };
      ...
      if (tmp2(1889).CollectiblesItemType.AVATAR_DECORATION === type) {
        ...
        const obj6 = {
          profileFrame: first,
          previewWidth: tmp2(8764).COLLECTIBLES_SHOP_CARD_WIDTH - nativeDefault.space.PX_32,
          previewHeight,
