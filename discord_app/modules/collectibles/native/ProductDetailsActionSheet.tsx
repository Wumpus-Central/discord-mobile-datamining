...
function PreviewProfileTrigger(handlePreviewPress) {
  ...
  dependencyMap = tmp;
  const theme = handlePreviewPress(4535).useThemeContext().theme;
  const obj = handlePreviewPress(4535);
  const isThemeLightResult = handlePreviewPress(4680).isThemeLight(theme);
  ...
  const obj2 = handlePreviewPress(4680);
  obj3.children = closure_13(handlePreviewPress(7301).EyeIcon, {
    size: "md",
...
function ManagedProductDetailsActionSheetInner(skuId) {
  ...
  const collectiblesShopProduct = skuId(9233).useCollectiblesShopProduct(skuId, {
    needsCategory: false,
  ...
  const obj = skuId(9233);
  const getOrFetchPurchases = skuId(11082).useGetOrFetchPurchases();
  ({ hasPreviouslyFetched, fetchPurchasesError } = getOrFetchPurchases);
  const obj2 = skuId(11082);
  const ref1 = noop.useRef(null);
  ...
  if ("ready" === state) {
    if (hasPreviouslyFetched) {
      if (null != product) {
        ...
        return closure_13(tmp(7483).BottomSheet, obj4);
      }
    }
  }
  if ("error" === state) {
    const obj5 = { Illustration: tmp(8580).NoResults, body: null, children: null };
    ...
    obj6.onPress = retry;
    obj5.children = closure_13(tmp(5273).Button, obj6);
    closure_13(tmp(1177).EmptyState, obj5);
  } else {
    closure_13(initialVariantIndex(13570), {});
  }
  ref = noop.useRef(null);
}
...
let size = {
  width: fn(5278).MEDIUM_BUTTON_HEIGHT,
  height: fn(5278).MEDIUM_BUTTON_HEIGHT,
  alignItems: "center",
