...
import ShopStandalonePdpMobileExperiment from "ShopStandalonePdpMobileExperiment" /* 8237 */;
...
function PreviewProfileTrigger(handlePreviewPress) {
  ...
  dependencyMap = tmp;
  let obj = handlePreviewPress(4315);
  ...
  obj.children = closure_13(handlePreviewPress(7020).EyeIcon, obj);
...
function ManagedProductDetailsActionSheetInner(skuId) {
  ...
  if ("ready" === state) {
    if (hasPreviouslyFetched) {
      if (null != product) {
        obj = { ref: ref1, product, initialVariantIndex: tmp7, analyticsLocations, stageCollectibleChangeForEditProfile };
        obj = {
          scrollable: true,
          startExpanded: true,
          onExpand() {
        ...
        return closure_13(tmp(7202).BottomSheet, obj);
      }
    }
  }
  if ("error" === state) {
    obj1 = { Illustration: tmp(8295).NoResults, body: null, children: null };
    ...
    obj2.onPress = retry;
    obj1.children = closure_13(tmp(5025).Button, obj2);
...
let closure_19 = noop.forwardRef((product, arg1) => {
  ...
  items[HermesBuiltin.arraySpread(analyticsLocations, 0)] = analyticsLocations(ref[23]).COLLECTIBLES_SHOP_PROFILE_PREVIEW;
  ...
  const imperativeHandle = selectedProduct.useImperativeHandle(arg1, () => ({
    scrollToEnd() {
    ...
    showUserProfileActionSheetDefault({
      ...
      onClose() {
        if (null == trackPdpClick) {
          ...
          product(ref[19]);
          obj = { skuId: product(ref[20]).getSelectedProduct(product, initialVariantIndex).skuId, initialVariantIndex, analyticsLocations, shopAnalyticsContext, stageCollectibleChangeForEditProfile: tmp };
  ...
  const items8 = [tmp40, closure_13(analyticsLocations(ref[38]), { selectedProduct, size: "md", onTrackPress: trackPdpClick })];
  ...
  items9[2] = closure_13(analyticsLocations(ref[39]), { product: selectedProduct, handlePreviewPress: callback, onTrackPress: trackPdpClick, onBundleActiveItemChange: tmp39 });
  items9[3] = closure_13(analyticsLocations(ref[40]), { product: selectedProduct, onTrackPress: trackPdpClick });
  items9[4] = closure_13(analyticsLocations(ref[41]), { product, selectedVariantIndex: tmp14, disabled: tmp53, onVariantSelect: tmp15 });
