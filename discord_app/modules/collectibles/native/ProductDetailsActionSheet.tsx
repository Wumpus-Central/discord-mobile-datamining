...
function PreviewProfileTrigger(handlePreviewPress) {
  ...
  dependencyMap = tmp;
  const theme = handlePreviewPress(4540).useThemeContext().theme;
  const obj = handlePreviewPress(4540);
  const isThemeLightResult = handlePreviewPress(4685).isThemeLight(theme);
  ...
  obj3.children = closure_13(handlePreviewPress(6389).EyeIcon, {
    size: "md",
...
function ManagedProductDetailsActionSheetInner(skuId) {
  ...
  const collectiblesShopProduct = skuId(8339).useCollectiblesShopProduct(skuId, {
    needsCategory: false,
...
let size = {
  width: fn(5286).MEDIUM_BUTTON_HEIGHT,
  height: fn(5286).MEDIUM_BUTTON_HEIGHT,
  alignItems: "center",
