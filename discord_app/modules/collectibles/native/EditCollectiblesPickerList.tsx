// === Module 13487: EditCollectiblesPickerList ===
...
let obj = { list: { flex: 1, marginTop: 12 }, listContent: { paddingBottom: 88 }, loadingContainer: { paddingVertical: 80, alignItems: "center" }, header: { paddingHorizontal: fn(13482).GUTTER_SIZE, paddingTop: 10, paddingBottom: 5 } };
...
export const EditCollectiblesPickerList = function EditCollectiblesPickerList(sections) {
  ...
  const memo = obj.useMemo(() => {
    ...
    if (flag) {
      ...
      let item = sections.forEach((header) => {
        ...
        const obj2 = items(12);
        const item = items(12).chunk(header.items, items(13482).ROW_SIZE).forEach((items, index) => {
  ...
  if (flag) {
    ...
    obj2.children = jsx(sections(selectedSkuId[8]).BottomSheetFlashList, { data: memo, renderItem: callback1, getItemType: callback2, keyExtractor: tmp11, extraData: selectedSkuId, contentContainerStyle: null, onLayout: null, keyboardShouldPersistTaps: "always" });
    tmp17 = obj2;
  }
  return <num {...tmp17} />;
};