...
function CollectiblesShopInternal(analyticsSource) {
  ...
  const commonTriggerPoint = analyticsSource(screen[12]).useCommonTriggerPoint(
    analyticsSource(screen[13]).CollectiblesShopOpenTriggerPoint,
  );
  let obj = analyticsSource(screen[12]);
  let items = [isFetchingGoogleSkus];
  const first = bypassGoogleSkuSync(
    analyticsSource(screen[14]).useStateFromStoresArray(items, () => {
  ...
  const stateFromStoresObject = analyticsSource(screen[14]).useStateFromStoresObject(items1, () => ({
  ...
  const maybeFetchCollectiblesShopHome = analyticsSource(screen[18]).useMaybeFetchCollectiblesShopHome(
    HOME,
    ...
    if (screen !== tmp13.FEATURED_PAGE) {
      if (!tmp33) {
        ...
        if (screen === tmp13.SHOP_ALL) {
          ...
          let tmp39Result = closure_16(tmp7(screen[39]), obj12);
          const tmp7Result9 = tmp7(screen[39]);
          ...
          tmp39Result = closure_16(tmp7(screen[40]), obj13);
          const tmp7Result10 = tmp7(screen[40]);
        } else {
          const obj14 = { children: null };
          const obj15 = { data: shopBlocks, renderItem: callback1, getItemType: callback3 };
          obj14.children = closure_16(tmp7(screen[39]), obj15);
          tmp39Result = closure_16(tmp(screen[41]).CollectiblesCoachmarkScrollDismissProvider, obj14);
        }
        obj11.children = tmp39Result;
        obj10.children = closure_16(tmp(screen[38]).NativePaymentContextProvider, obj11);
        const items13 = [closure_16(sessionId, obj10), closure_16(tmp7(screen[42]), {})];
        obj9.children = items13;
        obj8.children = closure_17(tmp(screen[37]).CollectiblesAnalyticsProvider, obj9);
        let tmp39Result2 = closure_16(tmp(screen[25]).AnalyticsLocationProvider, obj8);
