// === Module 13552: OrbBadgeCollectedModal ===
...
import BalanceWidgetPill from "BalanceWidgetPill" /* 11438 */;
...
function OrbBadgeCollectedRootScreen(modalKey) {
  ...
  if (stateFromStores) {
    ...
    const obj4 = { uri: onPressViewBadge(12119) };
    obj3.source = obj4;
    obj3.style = tmp.background;
    let tmp9Result = closure_6(onPressViewBadge(5890), obj3);
    let tmp12 = onPressViewBadge;
    let tmp13 = closure_6;
    const tmp15 = onPressViewBadge(5890);
  } else {
    const obj5 = { source: null, poster: null, style: null, resizeMode: "contain", muted: true, pauseWhileAppInactive: true, paused: false };
    const obj6 = { uri: onPressViewBadge(12120) };
    obj5.source = obj6;
    obj5.poster = onPressViewBadge(12119);
    obj5.style = tmp.background;
    tmp9Result = closure_6(tmp2(8653).VideoComponent, obj5);
  ...
  const items4 = [tmp13(tmp12(5890), obj8), ];
  ...
  const items5 = [tmp13(modalKey(4823).Text, obj12), ];
  ...
  items5[1] = tmp13(modalKey(4823).Text, obj13);
  ...
  items3[1] = tmp13(modalKey(7454).SafeAreaPaddingView, rect);
  ...
  let obj = {
    screens: {
      [closure_9.ROOT]: {
        ...
        headerRight() {
          return timestampProducer(BalanceWidgetPill.BalanceWidgetPill, { initialRenderedBalance: orbBalancePriorToPurchase, balance: _mod9205.useFetchVirtualCurrencyBalance().balance });
        },
        title: ""
      }
    },
