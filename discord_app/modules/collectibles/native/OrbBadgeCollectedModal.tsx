// === Module 13470: OrbBadgeCollectedModal ===
...
import BalanceWidgetPill from "BalanceWidgetPill" /* 11362 */;
...
function OrbBadgeCollectedRootScreen(modalKey) {
  ...
  if (stateFromStores) {
    ...
    const obj4 = { uri: onPressViewBadge(12042) };
    obj3.source = obj4;
    obj3.style = tmp.background;
    let tmp9Result = closure_6(onPressViewBadge(5806), obj3);
    let tmp12 = onPressViewBadge;
    let tmp13 = closure_6;
    const tmp15 = onPressViewBadge(5806);
  } else {
    const obj5 = { source: null, poster: null, style: null, resizeMode: "contain", muted: true, pauseWhileAppInactive: true, paused: false };
    const obj6 = { uri: onPressViewBadge(12043) };
    obj5.source = obj6;
    obj5.poster = onPressViewBadge(12042);
    obj5.style = tmp.background;
    tmp9Result = closure_6(tmp2(8582).VideoComponent, obj5);
  ...
  const items4 = [tmp13(tmp12(5806), obj8), ];
  ...
  const items5 = [tmp13(modalKey(4753).Text, obj12), ];
  ...
  items5[1] = tmp13(modalKey(4753).Text, obj13);
  ...
  items3[1] = tmp13(modalKey(7370).SafeAreaPaddingView, rect);
  ...
  let obj = {
    screens: {
      [closure_9.ROOT]: {
        ...
        headerRight() {
          return timestampProducer(BalanceWidgetPill.BalanceWidgetPill, { initialRenderedBalance: orbBalancePriorToPurchase, balance: _mod9134.useFetchVirtualCurrencyBalance().balance });
        },
        title: ""
      }
    },
