...
let size = {
  width: fn(8971).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  height: fn(8971).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  position: "absolute",
  top: "50%",
  left: "50%",
  transform: null,
...
const obj3 = { translateX: -fn(8971).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = _modDef10852;
      } else {
        tmp8Result = _modDef10853;
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = closure_9(tmp2(7534), obj);
    let tmp15 = closure_9;
  } else {
    obj = { skuId };
  ...
  let obj2 = {
    ...
    onPress: callback,
    children: tmp15(skuId(11239).CircleQuestionIcon, { style: tmp.questionIcon, color: consumed(576).colors.WHITE }),
  };
  items2[1] = tmp15(skuId(5204).PressableOpacity, obj2);
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    items5[1] = tmp15(tmp5(5056).Button, obj8);
  ...
  return closure_10(skuId(7253).BottomSheet, obj9);
}
