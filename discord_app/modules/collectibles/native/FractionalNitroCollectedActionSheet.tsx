...
let size = {
  width: fn(9124).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  height: fn(9124).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  position: "absolute",
  top: "50%",
  left: "50%",
  transform: null,
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = _modDef10983;
      } else {
        tmp8Result = _modDef10984;
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = closure_9(tmp2(7671), obj2);
    let tmp15 = closure_9;
  } else {
    let obj3 = { skuId };
  ...
  let obj = skuId(7827);
  obj5.children = tmp15(skuId(11371).CircleQuestionIcon, {
