...
import _modDef10991 from "../../../../_runtime/metro/10991__.js";
...
function NitroAcquiredHeader(skuId) {
  ...
  const obj2 = { source: _modDef13472 };
...
let size = {
  width: fn(9130).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  height: fn(9130).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET,
  position: "absolute",
  top: "50%",
  left: "50%",
  transform: null,
...
let items = [{ translateX: -fn(9130).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 }];
const obj6 = { translateX: -fn(9130).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
items[1] = { translateY: -fn(9130).FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET / 2 };
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  const tmp = closure_12();
  dependencyMap = tmp;
  const tmp4 = consumed(4691)();
  noop = tmp4;
  const fetchFractionalPremiumInfo = skuId(7832).useFetchFractionalPremiumInfo();
  ...
  const memo = noop.useMemo(() => {
    if (consumed) {
      ...
      if (obj2.isThemeDark(closure_3)) {
        let tmp8Result = _modDef10990;
      } else {
        tmp8Result = _modDef10991;
      }
  ...
  if (consumed) {
    ...
    let tmp12Result = closure_9(tmp2(7676), obj2);
    let tmp15 = closure_9;
  } else {
    let obj3 = { skuId };
  ...
  let obj = skuId(7832);
  obj5.children = tmp15(skuId(11377).CircleQuestionIcon, {
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [tmp15(tmp5(5187).Button, tmp18)];
    ...
    items5[1] = tmp15(tmp5(5187).Button, obj11);
