...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(
    navigateToShop(7630).useSelectedVersionedDismissibleContent(
      navigateToShop(2027).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING,
      marketing.version,
      undefined,
      true,
    ),
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () =>
    marketingBySurface.getMarketingBySurface(navigateToShop(14255).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON),
  );
  ...
  if (type === navigateToShop(7807).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = {
        ref: markAsDismissed,
        IconComponent: navigateToShop(12391).ShopIcon,
