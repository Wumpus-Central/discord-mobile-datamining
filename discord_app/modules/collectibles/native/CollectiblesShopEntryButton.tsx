...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(
    navigateToShop(7718).useSelectedVersionedDismissibleContent(
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () =>
    marketingBySurface.getMarketingBySurface(navigateToShop(14354).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON),
  );
  ...
  if (type === navigateToShop(7895).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = {
        ref: markAsDismissed,
        IconComponent: navigateToShop(12480).ShopIcon,
    ...
    if (tmp4) {
      ...
      let tmp15Result = closure_6(tmp(10973).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
