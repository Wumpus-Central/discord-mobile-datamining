...
function CoachmarkVariant(shopButtonRef) {
  ...
  obj[1] = navigateToShop(11329).ShopIcon;
  ...
  obj[4] = tmp3;
  const items = [callback2(importDefault(15647), obj), callback2(importDefault(15649), { marketing, shopButtonRef, navigateToShop, visible: tmp3, onDismiss: tmp[1] })];
  obj[0] = items;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  if (type === navigateToShop(5827).CollectiblesMarketingType.COACHMARK) {
    ...
    if (tmp4) {
      ...
      let prop = null;
      if (type1 === tmp(5827).CollectiblesMarketingType.BADGE) {
      ...
      obj[1] = num;
      obj[2] = content;
      let tmp15Result = tmp15(tmp(9676).SelectedVersionedDismissibleContent, obj);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(5827).CollectiblesMarketingType.BADGE) {
