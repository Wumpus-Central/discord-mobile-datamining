...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(
    navigateToShop(7502).useSelectedVersionedDismissibleContent(
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () =>
    marketingBySurface.getMarketingBySurface(navigateToShop(14100).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON),
  );
  ...
  if (type === navigateToShop(7679).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = {
        ref: markAsDismissed,
        IconComponent: navigateToShop(12289).ShopIcon,
    ...
    if (tmp4) {
      ...
      let tmp15Result = closure_6(tmp(10774).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
