// === Module 17338: CollectiblesShopEntryButton ===

// Module 17338 (CollectiblesShopEntryButton)
import YouScreenNavIconDefault from "YouScreenNavIcon" /* 17337 */;
import MobileShopButtonCoachmarkDefault from "MobileShopButtonCoachmark" /* 17339 */;
...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(navigateToShop(7716).useSelectedVersionedDismissibleContent(navigateToShop(2028).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  const obj = navigateToShop(7716);
  obj3.IconComponent = navigateToShop(12471).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  const stateFromStores = navigateToShop(563).useStateFromStores(items, () => marketingBySurface.getMarketingBySurface(navigateToShop(14345).CollectiblesMarketingSurface.MOBILE_SHOP_BUTTON));
  ...
  if (type === navigateToShop(7893).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(12471).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1115).intl;
      ...
      return closure_1_6(shopButtonRef(17337), obj);
    }
    if (tmp4) {
      let type1;
      if (stateFromStores != null) {
      ...
      let prop = null;
      if (type1 === tmp(7893).CollectiblesMarketingType.BADGE) {
      ...
      let tmp15Result = closure_6(tmp(10967).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
        type2 = stateFromStores.type;
      }
      if (type2 === tmp(7893).CollectiblesMarketingType.BADGE) {
