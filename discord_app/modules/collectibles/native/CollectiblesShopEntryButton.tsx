...
function CoachmarkVariant(shopButtonRef) {
  ...
  const tmp = _slicedToArray(navigateToShop(6806).useSelectedVersionedDismissibleContent(navigateToShop(2029).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING, marketing.version, undefined, true), 2);
  importDefault = tmp2;
  ...
  obj3.IconComponent = navigateToShop(11620).ShopIcon;
...
export default function CollectiblesShopEntryButton(navigateToShop) {
  ...
  if (tmp4) {
    tmp4 = stateFromStores.dismissibleContent === tmp(2029).DismissibleContent.COLLECTIBLES_SHOP_ENTRY_MARKETING;
  }
  let type;
  if (stateFromStores != null) {
    type = stateFromStores.type;
  }
  if (type === navigateToShop(6985).CollectiblesMarketingType.COACHMARK) {
    ...
    function content(visibleContent) {
      ...
      const obj = { ref: markAsDismissed, IconComponent: navigateToShop(11620).ShopIcon, accessibilityLabel: null, onPress: null, showRedDot: null };
      const intl = navigateToShop(1115).intl;
    ...
    if (tmp4) {
      ...
      let tmp15Result = closure_6(tmp(10088).SelectedVersionedDismissibleContent, obj3);
    } else {
      let type2;
      if (stateFromStores != null) {
