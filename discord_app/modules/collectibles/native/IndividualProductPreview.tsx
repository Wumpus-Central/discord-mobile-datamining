...
import FractionalNitroPreview from "FractionalNitroPreview" /* 9574 */;
import OrbBadgePreview from "OrbBadgePreview" /* 9577 */;
...
function ProfilePreviewWrapper(children) {
  ...
  const obj = { style: tmp.profilePreviewGradient, start: { x: 0, y: 0.6 }, end: { x: 0, y: 1 }, colors: null };
...
const createCacheKey = { marginTop: ThemesDefault.space.PX_12, position: "relative", height: 280 };
...
const result = require("obj132").fileFinishedImporting("modules/collectibles/native/IndividualProductPreview.tsx");
...
export const IndividualProductPreview = function IndividualProductPreview(arg0) {
  ...
  if (CollectiblesItemType.CollectiblesItemType.NAMEPLATE === type) {
    ...
    if (ALL.has(product.skuId)) {
      let tmp5 = callback(FractionalNitroPreview.FractionalNitroPreview, {});
    } else {
      tmp5 = null;
      if (product.skuId === constants.ORB_PROFILE_BADGE) {
        tmp5 = callback(OrbBadgePreview.OrbBadgePreview, {});
      }
    }
    return tmp5;
  } else {
