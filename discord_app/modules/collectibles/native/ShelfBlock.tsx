...
import { CollectiblesMobileShopScreen as closure_7 } from "../CollectiblesShopConstants.tsx";
...
const createCacheKey = { width: "100%", paddingTop: ThemesDefault.space.PX_16, paddingBottom: ThemesDefault.space.PX_16 };
...
let result = require("obj132").fileFinishedImporting("modules/collectibles/native/ShelfBlock.tsx");
...
export default function _default(block) {
  ...
  const callback1 = collectiblesAnalyticsContext.useCallback((arg0) => {
    ({ item, index } = arg0);
    const obj = { product: item, preferVCPrice };
  ...
  if (0 === filteredAndSortedProducts.length) {
    ...
    if (tmp14Result) {
      ...
      obj1[1] = obj2;
      tmp14Result = callback(stateFromStores, obj1);
    }
    ...
    obj6[1] = tmp19;
    obj6[2] = block.name;
    obj4[1] = callback(tmp2(tmp3[18]).Heading, obj6);
    ...
    if (tmp14Result) {
      ...
      obj7[5] = callback;
      tmp14Result = callback(tmp2(tmp3[19]).Button, obj7);
    }
    items6[1] = tmp14Result;
    obj3[1] = items6;
    items5[1] = callback2(closure_5, obj3);
    ...
    obj8[0] = callback(tmp2(tmp3[22]).FlashList, obj9);
    items5[2] = callback(tmp2(tmp3[21]).LayerScope, obj8);
    obj[1] = items5;
    obj[1] = callback2(closure_5, obj);
    return callback(tmp2(tmp3[11]).AnalyticsLocationProvider, obj);
  }
  tmp6 = preferVCPrice;
