// === Module 16165: FeedBlock ===

// Module 16165 (FeedBlock)
import nativeDefault from "native" /* 576 */;
import ShopHomeSortType from "ShopHomeSortType" /* 16166 */;
...
export default function _default(feedBlock) {
  ...
  let tmp6 = stateFromStores1(16153)();
  dependencyMap = tmp6;
  ...
  const filteredAndSortedProducts = feedBlock(15350).useFilteredAndSortedProducts({ products: feedProducts, maxProducts: 36, screen });
  const obj3 = feedBlock(15350);
  ...
  const obj5 = { value: stateFromStores1(7409)(stateFromStores1(7429).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  const items4 = [closure_11(feedBlock(4753).Heading, { variant: "heading-lg/semibold", children: stringResult }), ];
  if (isPersonalized) {
    const obj9 = {
      onPress() {
    ...
    obj9.children = closure_11(tmp(4711).CircleInformationIcon, { size: "xs" });
    isPersonalized = closure_11(tmp(5341).PressableOpacity, obj9);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = closure_11(tmp(5187).Button, obj10);
  }
  items5[1] = tmp12Result;
  obj7.children = items5;
  const items6 = [closure_12(closure_5, obj7), closure_11(stateFromStores1(16160), { products: filteredAndSortedProducts, loadingCardsNum: 36, preferVCPrice, accessibilityLabel: stringResult, disableBundleStaticBackground }), ];
  ...
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
      ...
      if (tmpResult.isAndroid()) {
        const obj16 = { url: tmp5(16169), autoplay: true, style: feedFooterOrbImage.feedFooterOrbImage, accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants" };
        tmp12Result2 = closure_11(tmp5(9095), obj16);
        const tmp5Result3 = tmp5(9095);
      } else {
        ...
        const obj18 = { uri: tmp5(16169) };
        obj17.source = obj18;
        obj17.style = feedFooterOrbImage.feedFooterOrbImage;
        tmp12Result2 = closure_11(tmp5(5806), obj17);
        const tmp5Result4 = tmp5(5806);
      }
      tmpResult = tmp(1364);
    }
  } else {
    ...
    return closure_11(tmp(7409).AnalyticsLocationProvider, obj5);
  }
  const tmp10 = stateFromStores1(7409);
};