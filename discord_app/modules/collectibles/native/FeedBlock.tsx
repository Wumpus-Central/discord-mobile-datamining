...
export default function _default(feedBlock) {
  ...
  obj = { value: stateFromStores1(5767)(stateFromStores1(5787).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  const items4 = [callback(feedBlock(4340).Heading, { variant: "heading-lg/semibold", children: stringResult }), ];
  if (isPersonalized) {
    ...
    obj3[3] = tmp12(tmp(4300).CircleInformationIcon, { size: "xs" });
    isPersonalized = tmp12(tmp(4886).PressableOpacity, obj3);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = tmp12(tmp(4754).Button, obj4);
  }
  ...
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
      ...
      if (tmpResult.isAndroid()) {
        ...
        tmp12Result = tmp12(tmp5(5307), obj11);
        const tmp5Result1 = tmp5(5307);
      }
      tmpResult = tmp(500);
    }
  } else {
    ...
    return tmp12(tmp(5767).AnalyticsLocationProvider, obj);
  }
};