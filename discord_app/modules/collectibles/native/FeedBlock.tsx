...
export default function _default(feedBlock) {
  ...
  let tmp6 = stateFromStores1(14597)();
  dependencyMap = tmp6;
  ...
  obj = { value: stateFromStores1(5900)(stateFromStores1(5920).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  const items4 = [callback(feedBlock(4440).Heading, { variant: "heading-lg/semibold", children: stringResult }), ];
  if (isPersonalized) {
    ...
    obj3[3] = tmp12(tmp(4399).CircleInformationIcon, { size: "xs" });
    isPersonalized = tmp12(tmp(5015).PressableOpacity, obj3);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = tmp12(tmp(4879).Button, obj4);
  }
  items5[1] = tmp12Result;
  obj1[1] = items5;
  const items6 = [closure_12(closure_5, obj1), callback(stateFromStores1(15139), { products: filteredAndSortedProducts, loadingCardsNum: 36, preferVCPrice, accessibilityLabel: stringResult, disableBundleStaticBackground }), ];
  ...
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
      ...
      if (tmpResult.isAndroid()) {
        ...
        tmp12Result = tmp12(tmp5(5440), obj11);
        const tmp5Result1 = tmp5(5440);
      }
      tmpResult = tmp(500);
    }
  } else {
    ...
    return tmp12(tmp(5900).AnalyticsLocationProvider, obj);
  }
};