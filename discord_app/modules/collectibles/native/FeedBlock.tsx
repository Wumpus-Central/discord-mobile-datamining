...
export default function _default(feedBlock) {
  ...
  obj = { value: stateFromStores1(5768)(stateFromStores1(5788).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  if (isPersonalized) {
    ...
    isPersonalized = tmp12(tmp(4887).PressableOpacity, obj3);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = tmp12(tmp(4755).Button, obj4);
  }
  ...
  items7[1] = callback(feedBlock(4755).Button, obj7);
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
    ...
    return tmp12(tmp(5768).AnalyticsLocationProvider, obj);
  }
};