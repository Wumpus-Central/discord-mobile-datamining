// === Module 16240: FeedBlock ===

// Module 16240 (FeedBlock)
import nativeDefault from "native" /* 576 */;
import ShopHomeSortType from "ShopHomeSortType" /* 16241 */;
...
export default function _default(feedBlock) {
  ...
  let tmp6 = stateFromStores1(16228)();
  dependencyMap = tmp6;
  ...
  const filteredAndSortedProducts = feedBlock(15411).useFilteredAndSortedProducts({ products: feedProducts, maxProducts: 36, screen });
  const obj3 = feedBlock(15411);
  ...
  const obj5 = { value: stateFromStores1(7495)(stateFromStores1(7515).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  if (isPersonalized) {
    ...
    obj9.children = closure_11(tmp(4783).CircleInformationIcon, { size: "xs" });
    isPersonalized = closure_11(tmp(5427).PressableOpacity, obj9);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = closure_11(tmp(5273).Button, obj10);
  }
  items5[1] = tmp12Result;
  obj7.children = items5;
  const items6 = [closure_12(closure_5, obj7), closure_11(stateFromStores1(16235), { products: filteredAndSortedProducts, loadingCardsNum: 36, preferVCPrice, accessibilityLabel: stringResult, disableBundleStaticBackground }), ];
  ...
  items7[1] = closure_11(feedBlock(5273).Button, obj13);
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
      ...
      if (tmpResult.isAndroid()) {
        const obj16 = { url: tmp5(16244), autoplay: true, style: feedFooterOrbImage.feedFooterOrbImage, accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants" };
        tmp12Result2 = closure_11(tmp5(9170), obj16);
        const tmp5Result3 = tmp5(9170);
      } else {
        ...
        const obj18 = { uri: tmp5(16244) };
        obj17.source = obj18;
        obj17.style = feedFooterOrbImage.feedFooterOrbImage;
        tmp12Result2 = closure_11(tmp5(5892), obj17);
        const tmp5Result4 = tmp5(5892);
      }
      tmpResult = tmp(1364);
    }
  } else {
    ...
    return closure_11(tmp(7495).AnalyticsLocationProvider, obj5);
  }
  const tmp10 = stateFromStores1(7495);
};