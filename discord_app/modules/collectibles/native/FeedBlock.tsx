...
export default function _default(feedBlock) {
  ...
  let tmp6 = stateFromStores1(14394)();
  dependencyMap = tmp6;
  ...
  obj = { value: stateFromStores1(5789)(stateFromStores1(5809).COLLECTIBLES_SHOP_POPULAR_PICKS).analyticsLocations, children: null };
  ...
  const items4 = [callback(feedBlock(4338).Heading, { variant: "heading-lg/semibold", children: stringResult }), ];
  if (isPersonalized) {
    ...
    obj3[3] = tmp12(tmp(4298).CircleInformationIcon, { size: "xs" });
    isPersonalized = tmp12(tmp(4909).PressableOpacity, obj3);
  }
  ...
  if (screen !== constants.ORBS) {
    ...
    tmp12Result = tmp12(tmp(4777).Button, obj4);
  }
  items5[1] = tmp12Result;
  obj1[1] = items5;
  const items6 = [closure_12(closure_5, obj1), callback(stateFromStores1(14926), { products: filteredAndSortedProducts, loadingCardsNum: 36, preferVCPrice, accessibilityLabel: stringResult, disableBundleStaticBackground }), ];
  ...
  if (screen === constants.ORBS) {
    if (stateFromStores2) {
      ...
      if (tmpResult.isAndroid()) {
        ...
        tmp12Result = tmp12(tmp5(5329), obj11);
        const tmp5Result1 = tmp5(5329);
      }
      tmpResult = tmp(500);
    }
  } else {
    ...
    return tmp12(tmp(5789).AnalyticsLocationProvider, obj);
  }
};