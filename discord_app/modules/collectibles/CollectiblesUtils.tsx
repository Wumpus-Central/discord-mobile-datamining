// === Module 7246: getItemRecordsFromPurchases ===
...
import parseServerPerkConfigKind from "parseServerPerkConfigKind" /* 1943 */;
...
import fromServer2 from "fromServer" /* 7239 */;
import fromServer3 from "fromServer" /* 7240 */;
import fromServer4 from "fromServer" /* 7241 */;
import fromServer5 from "fromServer" /* 7247 */;
...
export const getCollectiblesProductPriceComparisons = function getCollectiblesProductPriceComparisons(type, hasShopDiscount) {
  ...
  if (null == tmp4) {
    ...
    let tmp12 = tmp4;
    if (type === _require(1951).CollectiblesItemType.BUNDLE) {
      tmp12 = tmp4;
      if (hasShopDiscount) {
        obj = {};
...
export const groupProfileEffects = function groupProfileEffects(arr, arr) {
  const tmp = getItemRecordsFromPurchases(arr, _require(1951).CollectiblesItemType.PROFILE_EFFECT);
  _require = tmp;
  arr = getItemRecordsFromCategories(arr, _require(1951).CollectiblesItemType.PROFILE_EFFECT);
  return {
    purchased: tmp,
    shopPreviews: getItemRecordsFromCategories(arr, _require(1951).CollectiblesItemType.PROFILE_EFFECT).filter((skuId) => {
      skuId = skuId.skuId;
