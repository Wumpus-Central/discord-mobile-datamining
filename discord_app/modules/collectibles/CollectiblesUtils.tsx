// === Module 7232: getItemRecordsFromPurchases ===
...
import fromServer2 from "fromServer" /* 7225 */;
import fromServer3 from "fromServer" /* 7226 */;
import fromServer4 from "fromServer" /* 7227 */;
import fromServer5 from "fromServer" /* 7233 */;
...
export const getCollectiblesProductPriceComparisons = function getCollectiblesProductPriceComparisons(type, hasShopDiscount) {
  ...
  if (null == tmp4) {
    ...
    let tmp12 = tmp4;
    if (type === _require(1950).CollectiblesItemType.BUNDLE) {
      tmp12 = tmp4;
      if (hasShopDiscount) {
        obj = {};
...
export const groupProfileEffects = function groupProfileEffects(arr, arr) {
  const tmp = getItemRecordsFromPurchases(arr, _require(1950).CollectiblesItemType.PROFILE_EFFECT);
  _require = tmp;
  arr = getItemRecordsFromCategories(arr, _require(1950).CollectiblesItemType.PROFILE_EFFECT);
  return {
    purchased: tmp,
    shopPreviews: getItemRecordsFromCategories(arr, _require(1950).CollectiblesItemType.PROFILE_EFFECT).filter((skuId) => {
      skuId = skuId.skuId;
