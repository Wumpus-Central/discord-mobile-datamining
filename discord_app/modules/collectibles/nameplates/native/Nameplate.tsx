...
function NameplateInner(isFocused) {
  ...
  tmpResult = tmp(1886);
  const backgroundGradientColors = tmpResult.getBackgroundGradientColors(nameplate.palette, sharedValue(4540)());
  ...
  if (null != backgroundGradientColors) {
    ...
    obj.colors = items3;
    tmp15Result = closure_5(tmp5(5037), obj);
    const tmp5Result = tmp5(5037);
  }
  const items4 = [tmp15Result, ];
  const tmpResult1 = flag6(8890);
  if (tmpResult2.isAndroid()) {
    if (tmp12) {
      obj1 = { url: str, style: tmp4.img, autoplay: true };
      let tmp17 = closure_5(tmp(8879).APNGPlayer, obj1);
    }
    items4[1] = tmp17;
    obj.children = items4;
    return closure_6(tmp5(4341).View, obj);
  }
