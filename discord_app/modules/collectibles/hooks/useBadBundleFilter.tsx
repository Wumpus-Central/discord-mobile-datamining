            ...
            if (!tmpResult.isFreeCollectiblesProduct(product)) {
              if (!obj.getProductPurchaseState(CollectiblesPurchaseStore, product).isPurchased) {
                if (tmpResult5.isOrbsExclusiveProduct(product)) {
                  ...
                  return null != canUseShopDiscountsResult(7667).getProductOrbPrice(obj3);
                } else {
                  const defaultPriceSetAssignmentPurchaseType = canUseShopDiscountsResult(7668).getDefaultPriceSetAssignmentPurchaseType(hasShopDiscount);
                  const tmpResult7 = canUseShopDiscountsResult(7668);
                  let result = canUseShopDiscountsResult(7668).extractPriceByPurchaseTypes(product, defaultPriceSetAssignmentPurchaseType);
                  if (null != result) {
                    if (0 !== result.amount) {
                      let num2 = 0;
                ...
                tmpResult5 = canUseShopDiscountsResult(7667);
              }
            }
            tmpResult = canUseShopDiscountsResult(7668);
          }
          return true;
        });
      }
