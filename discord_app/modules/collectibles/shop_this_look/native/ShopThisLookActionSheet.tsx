...
import ShopThisLookUtils from "../ShopThisLookUtils.tsx";
...
function ShopThisLookCard(skuId) {
    ...
    if (null == _undefined) {
      ...
      if (obj.getIsVariantProduct(_undefined)) {
        ...
        const bound = Math.max(
          0,
          variants.findIndex((skuId) => skuId.skuId === skuId),
        );
  ...
  const effect = stateFromStores.useEffect(() => {
    ...
    if (!current) {
      ref.current = true;
      const obj = {
        action: ShopThisLookAnalyticsUtils.ShopThisLookRowAction.ROW_VIEWED,
        skuId,
        productType: type,
        isDisabled: !memo,
        source: UserProfileThemeTypes.ACTION_SHEET,
      };
  ...
  callback = stateFromStores.useCallback(() => {
    const obj = {
      action: ShopThisLookAnalyticsUtils.ShopThisLookRowAction.ROW_CLICKED,
      skuId,
      productType: type,
      isDisabled: !memo,
      source: UserProfileThemeTypes.ACTION_SHEET,
  ...
  if ("loading" === collectiblesShopProduct.state) {
    ...
    let tmp23 = closure_9(onPress(8806), obj);
  } else {
    tmp23 = null;
    if (null != stateFromStores) {
      if (memo) {
        ...
        const items7 = [closure_9(onPress(11065), obj1)];
        ...
        if (tmp17Result) {
          ...
          tmp17Result = closure_9(onPress(8871), callback1);
        }
        ...
        const obj2 = {
          sku: stateFromStores,
          size,
          overlay: skuId(8806).WishlistItemCardOverlay.LOCKED,
          onPress: tmp10,
        };
        tmp15Result = closure_9(onPress(11065), obj2);
...
export default function ShopThisLookActionSheet(arg0) {
  ...
  ({ cardWidth: c0, rowWidth, gap } = analyticsLocations(13116)(obj));
  const tmp2 = analyticsLocations(13116)(obj);
  ...
  dependencyMap = noop.useCallback((initialProductSkuId) => {
    ...
    obj = {
      initialProductSkuId,
      analyticsLocations,
      analyticsSource: AnalyticsLocationDefault.USER_PROFILE_OVERFLOW_MENU,
    };
  ...
  obj.children = closure_9(analyticsLocations(11183), obj1);
