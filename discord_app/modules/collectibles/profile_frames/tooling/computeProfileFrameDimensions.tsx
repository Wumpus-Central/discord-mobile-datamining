...
export const computeProfileFrameDimensions = function computeProfileFrameDimensions(arr) {
  ...
  const mapped1 = found.map((dims) =>
    Math.max(0, dims.dims.height - (716 - innerWidth(8479).DefaultProfileFrameDimensions.OVERFLOW_TOP)),
  );
  let overflowTop = 0;
  if (mapped1.length > 0) {
    const _Math3 = Math;
  ...
  const mapped2 = found1.map((dims) =>
    Math.max(0, dims.dims.height - (424 - innerWidth(8479).DefaultProfileFrameDimensions.OVERFLOW_BOTTOM)),
  );
