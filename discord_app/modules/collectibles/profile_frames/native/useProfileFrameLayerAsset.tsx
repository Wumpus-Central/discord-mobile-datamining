...
let closure_15 = async function _preloadLayer(arg0) {
  closure_2 = tmp2;
...
export default function useProfileFrameLayerAsset(width) {
  ...
  ({ skuId, layer } = width);
  const obj = collectiblesItemAssetUrl(1968);
  collectiblesItemAssetUrl = obj.getCollectiblesItemAssetUrl({
    skuId,
    assetFormat: collectiblesItemAssetUrl(1968).CollectiblesItemAssetFormat.STATIC,
    assetId: layer.id,
  });
...
export const isProfileFrameLayerShown = function isProfileFrameLayerShown(anchor, profileThemeType, fn) {
  let tmp = null != fn;
  if (tmp) {
    tmp = !fn(anchor);
  }
...
export const usePreloadLayerImages = function usePreloadLayerImages(frame) {
  ...
  closure_7 = undefined;
  c8 = undefined;
  const isProfileFrameLayerPreloadEnabled = frame(filterLayer[7]).useIsProfileFrameLayerPreloadEnabled(
