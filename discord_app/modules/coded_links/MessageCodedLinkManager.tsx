// === Module 17924: MessageCodedLinkManager ===
  ...
  if (tmp2) {
    let item = arr.forEach((item) => {
      ...
      if (code(dependencyMap[5]).CodedLinkType.INVITE === type) {
        const result = code(dependencyMap[6]).queueMessageLinkFetch(closure_3(function*() {
          if (c0 === 2) {
            c0 = 3;
        ...
        const result1 = code(dependencyMap[6]).queueMessageLinkFetch(closure_3(function*() {
          if (c0 === 2) {
            c0 = 3;
            throw new TypeError("Generator functions may not be called on executing generators");
          } else if (tmp3 === 3) {
