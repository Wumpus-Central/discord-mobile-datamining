...
export const RootThemeContextProvider = function RootThemeContextProvider(children) {
  ...
  manaTypeConsolidationExperiment =
    manaTypeConsolidationExperiment(7083).useManaTypeConsolidationExperiment("RootThemeContextProvider");
  ...
  if (null == tmp4) {
    ...
    if (1 !== saturation) {
      ...
      num2 = tmpResult.setThemeFlag(0, tmp(4347).ThemeContextFlags.REDUCE_SATURATION_ENABLED);
    }
    if (1 === contrast) {
      obj = {
      ...
      obj.children = jsx(tmp(4641).PlainTextExperimentProvider, {
        enabled: plainTextExperiment,
        children: children.children,
      });
      return jsx(tmp(10204).RootThemeContextProvider, { enabled: plainTextExperiment, children: children.children });
    } else {
      tmpResult = tmp(4347);
      if (contrast > 1) {
    ...
    let setThemeFlagResult1 = tmp(4347).setThemeFlag(
      0,
      tmp(4347).ThemeContextFlags.MOBILE_LIGHT_GRADIENT_THEME_ENABLED,
    );
    const tmpResult1 = tmp(4347);
  } else {
    setThemeFlagResult1 = tmp(4347).setThemeFlag(0, tmp(4347).ThemeContextFlags.MOBILE_DARK_GRADIENT_THEME_ENABLED);
    const tmpResult2 = tmp(4347);
  }
  const obj3 = manaTypeConsolidationExperiment(14658);
};
