// === Module 17825: JankSessionManager ===
...
import NativeJankSessionModuleDefault from "NativeJankSessionModule" /* 17826 */;
import JankNavigationReporterDefault from "JankNavigationReporter" /* 17827 */;
...
prototype["deliverPendingSessions"] = function deliverPendingSessions() {
  ...
  if (!this._isDelivering) {
    ...
    const pendingReports = self(17826).getPendingReports();
    let obj = self(17826);
    const nextPromise = pendingReports.then((arr) => {
      ...
      if (0 !== arr.length) {
        const result = AnalyticsTrackingStore.submitEventsImmediately(arr.flatMap((screens) => {
          ...
          let merged = Object.assign(screens(7720).getDeviceMetadata());
          ...
          const items = [
            ...
            ...screens.map((item) => {
              ...
              const merged = Object.assign(screens(7720).getDeviceMetadata());
