// === Module 8507: showUploadFileSizeError ===
...
export default function showUploadFileSizeError(arg0) {
  ...
  const tmp2Result2 = items(8508);
  obj[1] = items(8509).getUploaderChannelId(file);
  obj[2] = baseMaxSize;
  ...
  const tmp2Result3 = items(8509);
  obj[10] = items(4881).getKestrelVariantName(kestrelConfig);
  ...
  if (!tmp22) {
    tmp22 = num > tmp2(4835).MAX_TOTAL_ATTACHMENT_SIZE;
  }
  if (!tmp22) {
    tmp22 = tmp21;
  }
  ...
  const tmp2Result5 = items(5047);
  obj[6] = items(4843).getImageCompressionQuality();
  ...
  if (tmp22) {
    if (errorReason === tmp20.ERROR_SOURCE_UNKNOWN) {
      ...
      if (kestrelConfig.enabled) {
        ...
        if (tmp21) {
          obj1 = { maxSize: null };
          obj1[0] = tmp2(4273).formatSize(tmp2(4835).MAX_TOTAL_ATTACHMENT_SIZE / tmp2(4273).BYTE_IN_KB, { useKibibytes: true });
          stringResult1 = formatToPlainString(t.tUOJdH, obj1);
          const tmp2Result7 = tmp2(4273);
        } else {
          const obj2 = { maxSize: null };
          obj2[0] = tmp2(4273).formatSize(maxSize / tmp2(4273).BYTE_IN_KB, { useKibibytes: true });
          stringResult1 = formatToPlainString(t.fxEKdS, obj2);
          const tmp2Result8 = tmp2(4273);
        }
      }
    ...
    items1(4832).show(obj3);
    const obj20 = items1(4832);
  } else {
    ...
    items4[arraySpreadResult] = items1(7197).FILE_UPLOAD_POPOUT;
    obj4[2] = items4;
    const obj6 = { type: null };
    obj6[0] = constants3.UPLOAD_ERROR_UPSELL;
    obj4[3] = obj6;
    obj4[4] = num;
    const result = items1(8510).handleShowUpsellAlert(obj4);
  }
};