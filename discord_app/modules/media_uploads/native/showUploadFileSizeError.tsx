// === Module 8468: showUploadFileSizeError ===
...
export default function showUploadFileSizeError(arg0) {
  ...
  const tmp2Result3 = items(8470);
  obj[10] = items(4876).getKestrelVariantName(kestrelConfig);
  ...
  if (tmp22) {
    if (errorReason === FileUploadErrorTypes.ERROR_SOURCE_UNKNOWN) {
      ...
      if (kestrelConfig.enabled) {
        ...
        if (tmp21) {
          ...
          obj2[0] = tmp2(4269).formatSize(maxSize / tmp2(4269).BYTE_IN_KB, { useKibibytes: true });
          stringResult1 = formatToPlainString(t.fxEKdS, obj2);
          const tmp2Result8 = tmp2(4269);
        }
      }
    ...
    items1(4827).show(obj3);
    const obj20 = items1(4827);
  } else {
    ...
    items4[arraySpreadResult] = items1(7159).FILE_UPLOAD_POPOUT;
    obj4[2] = items4;
