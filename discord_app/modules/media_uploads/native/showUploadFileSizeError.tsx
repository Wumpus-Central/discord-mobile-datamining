...
export default function showUploadFileSizeError(arg0) {
  ...
  const tmp2Result2 = items(8167);
  obj[1] = items(8168).getUploaderChannelId(file);
  obj[2] = baseMaxSize;
  ...
  const tmp2Result3 = items(8168);
  obj[10] = items(5123).getKestrelVariantName(kestrelConfig);
  ...
  if (!tmp22) {
    tmp22 = num > tmp2(5074).MAX_TOTAL_ATTACHMENT_SIZE;
  }
  if (!tmp22) {
    tmp22 = tmp21;
  }
  ...
  obj[6] = items(5082).getImageCompressionQuality();
  ...
  if (tmp22) {
    if (errorReason === tmp20.ERROR_SOURCE_UNKNOWN) {
      ...
      if (kestrelConfig.enabled) {
        ...
        if (tmp21) {
          obj1 = { maxSize: null };
          obj1[0] = tmp2(4372).formatSize(tmp2(5074).MAX_TOTAL_ATTACHMENT_SIZE / tmp2(4372).BYTE_IN_KB, {
            useKibibytes: true,
          });
    ...
    items4[arraySpreadResult] = items1(5973).FILE_UPLOAD_POPOUT;
    obj4[2] = items4;
    const obj6 = { type: null };
    obj6[0] = constants3.UPLOAD_ERROR_UPSELL;
    obj4[3] = obj6;
    obj4[4] = num;
    const result = items1(8169).handleShowUpsellAlert(obj4);
  }
}
