...
export default function showUploadFileSizeError(arg0) {
  ...
  if (!isPremiumExactlyResult) {
    tmp20 = num > NitroFileUploadExperiments.getNitroFileUploadLimitBytes({ location: "native.showUploadFileSizeError" });
  ...
  if (tmp20) {
    if (errorReason === FileUploadErrorTypes.ERROR_SOURCE_UNKNOWN) {
      ...
      const t = util.t;
      if (tmp19) {
        const obj4 = { maxSize: FileSizeUtils.formatSize(UploadUtils.MAX_TOTAL_ATTACHMENT_SIZE / FileSizeUtils.BYTE_IN_KB, { useKibibytes: true }) };
