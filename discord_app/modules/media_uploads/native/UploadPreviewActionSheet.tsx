...
createStyles.imageContainer = { overflow: "hidden", alignSelf: "center", borderRadius: nativeDefault.radii.md - nativeDefault.space.PX_4 };
...
export default function UploadPreviewActionSheet(onAdd) {
  ...
  let size = noop.useMemo(() => {
    ({ height, width } = item);
    width = Math.min(width, ACTION_SHEET_MAX_WIDTH) - 2 * closure_5.contentContainer.padding - 2 * closure_5.imageWrap.padding;
    if (null != height) {
      if (null != width) {
        if (0 !== height) {
          if (0 !== width) {
  ...
  const callback2 = noop.useCallback(() => {
    ...
    ImagePickerDefault.launchCropper(size).then((result) => {
      ...
      if ("E_PICKER_CANCELLED" !== error.code) {
        onEdit(4335);
        const obj = { key: "CROP_ERROR", IconComponent: onAdd(6711).CircleErrorIcon, content: error.message };
        obj.open(obj);
      }
    });
  }, items3);
    ...
    if (isVideo) {
      ...
      if (!tmp13) {
        if (!tmp14) {
          ...
          if (null != onRemove) {
            const obj9 = { icon: tmp22(tmp6(tmp4[39]).TrashIcon, { size: "sm", color: "control-primary-text-default" }), text: null, onPress: null, variant: "destructive" };
            ...
            tmp22Result1 = null;
            if (null != onAdd) {
              const obj10 = { icon: tmp22(tmp6(tmp4[30]).ImageFileIcon, { size: "sm", color: "control-primary-text-default" }), text: null, onPress: null };
      ...
      if (isImage) {
        const obj14 = { icon: tmp22(tmp6(tmp4[37]).PencilSparkleIcon, {}), onPress: callback2, label: null, arrow: true };
