...
import closure_12 from "initialize" /* 8562 */;
import closure_13 from "createGuildRecordFromRust" /* 1910 */;
import closure_14 from "reinjectEphemerals" /* 4701 */;
import closure_15 from "getUncachedChannelPermissions" /* 4090 */;
import closure_16 from "generateOldThreadCutoff" /* 4461 */;
import closure_17 from "markAllUserIdListsStale" /* 4099 */;
import closure_18 from "updateUserGuildSettingsInternal" /* 4667 */;
import closure_19 from "initialize" /* 8564 */;
import closure_20 from "initialize" /* 8566 */;
...
function injectRecommendedGuildsRow() {
  items1 = items1.filter((type) => type.type !== callback(8565).ICYMIItemTypes.RECOMMENDED_GUILDS);
  items = items.filter((type) => type.type !== callback(8565).ICYMIItemTypes.RECOMMENDED_GUILDS);
  if (0 !== length.length) {
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type !== MessageEmbedTypes.ICYMIItemTypes.RECOMMENDED_GUILDS) {
      ...
      if (!set.has(tmp2.id)) {
        ...
        if (tmp7) {
          let tmp8 = nextResult;
          let tmp9 = tmp2.type !== tmp3(8565).ICYMIItemTypes.MESSAGE;
          if (!tmp9) {
...
function maybeFilterChannelItems(arg0, stateFromStores1) {
  const obj = _require(8567);
  if (numberToCustomScoreResult === _require(8567).ICYMICustomScore.MUTED) {
    _require = arg0;
...
const iCYMIStore = new ICYMIStore(dispatcherDefault, {
  ...
  LOAD_ICYMI_HYDRATED: function handleLoadHydratedItems(arg0) {
    ...
    set.delete(_require(8567).generateHydrationId(startingIndex, endingIndex));
  },
