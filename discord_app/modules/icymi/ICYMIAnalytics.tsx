...
export const ICYMIAnalytics = {
  ...
  trackItemShortImpression(viewableItems, arr2, stateFromStores) {
    const obj = {
      ...
      feed_item_ids: viewableItems.map((item, index) => item.item.id),
      feed_item_types: viewableItems.map((item, index) => {
        item = item.item;
      ...
      all_feed_item_ids: arr2.map((item, index) => item.id),
      all_feed_item_types: arr2.map((item, index) => item.type),
  ...
  trackFeedLoaded(unreadFeedItems) {
    ...
    const item = unreadFeedItems.forEach((item, index) => {
      items.push(item.id);
      const type = item.type;
      if (items(items2[3]).ICYMIItemTypes.MESSAGE === type) {
        let str2 = "message";
        if (item.data.channel_type === closure_1_4.GUILD_ANNOUNCEMENT) {
          str2 = "announcement";
        }
        let str = str2;
      } else {
        str = "hotwheels_gaming_activity";
        if (items(items2[3]).ICYMIItemTypes.ACTIVITY !== type) {
          str = "hotwheels_custom_status";
          if (items(items2[3]).ICYMIItemTypes.CUSTOM_STATUS !== type) {
            str = "guild_event";
            if (items(items2[3]).ICYMIItemTypes.GUILD_EVENT !== type) {
              if (items(items2[3]).ICYMIItemTypes.RECOMMENDED_GUILDS === type) {
    ...
    const item1 = readFeedItems.forEach((item, index) => {
      items1.push(item.id);
      const type = item.type;
      if (items(items2[3]).ICYMIItemTypes.MESSAGE === type) {
        let str2 = "message";
  ...
  trackFeedShown(homeSessionId) {
    const obj = { load_id: store.getLoadId(), home_session_id: homeSessionId.homeSessionId, variant: homeSessionId.variant };
    obj.track(closure_5.FEED_SHOWN, obj);
  },
  trackFeedFirstScrollStarted() {
    const obj = { load_id: store.getLoadId(), home_session_id: "gravity" };
  ...
  trackFeedEmptyLoadingSeen() {
    const obj = { load_id: store.getLoadId(), version: store.getVersion() };
    obj.track(closure_5.ICYMI_FEED_EMPTY_LOADING_SEEN, obj);
  },
  trackFeedEmptyLoadingComplete(dwellTimeMs) {
    const obj = { load_id: store.getLoadId(), dwell_time_ms: dwellTimeMs.dwellTimeMs, version: store.getVersion() };
