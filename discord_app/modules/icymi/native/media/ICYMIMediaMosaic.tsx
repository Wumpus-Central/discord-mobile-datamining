...
import ThumbhashUtils from "../../../thumbhash/ThumbhashUtils.tsx";
...
function Media(handlePressMedia) {
  ...
  if (isSpoiler) {
    ...
    obj7.children = intl.string(initialIndex(tmp7[21]).t["F+x38C"]).toUpperCase();
    obj6.children = closure_15(initialIndex(tmp7[20]).Text, obj7);
  ...
  if (tmp15) {
    ...
    if (!isGIFV) {
      isGIFV = initialIndex(tmp7[12]).urlMatchesFileExtension(source.sourceURI, initialIndex(tmp7[13]).GIF_RE_IOS);
      const tmp6Result = initialIndex(tmp7[12]);
    }
    tmp15 = !isGIFV;
  }
  if (tmp15) {
  ...
  if (tmp15) {
    ...
    obj9.children = closure_15(initialIndex(tmp7[22]).PlayIcon, obj10);
  ...
  if (tmp6Result5.isVideo(source.uri)) {
    ...
    if (!isGIFV2) {
      ...
      const tmp6Result6 = initialIndex(tmp7[12]);
    }
    tmp19 = null;
    if (!isGIFV2) {
      ...
      if (stateFromStores) {
        ...
        let tmp20Result = closure_15(initialIndex(tmp7[26]).VoiceXIcon, obj13);
      } else {
        const obj14 = { color: handlePressMedia(tmp7[11]).colors.BLACK, size: "sm" };
        tmp20Result = closure_15(initialIndex(tmp7[27]).VoiceNormalIcon, obj14);
      }
      obj12.children = tmp20Result;
      obj12 = closure_15(initialIndex(tmp7[24]).PressableOpacity, obj12);
  ...
  if (tmp6Result7.isVideo(source.uri)) {
    ...
    if (!isGIFV3) {
      ...
      const tmp6Result8 = initialIndex(tmp7[12]);
    }
  }
  tmp26 = closure_15(MediaMosaicImage, { source, style, dimensions, isSpoiler });
  tmp6Result7 = initialIndex(ref[23]);
}
...
function TwoImagesRow(arg0) {
  ...
  return closure_15(closure_6, {
    ...
    children: sources.map((source, initialIndex) =>
      __initData(Media, { handlePressMedia, initialIndex, source, dimensions: size }, initialIndex),
    ),
  });
}
function ThreeImages(arg0) {
  ...
  const items2 = [
    __initData(Media, {
      ...
      initialIndex: 1,
      source: sources[1],
      dimensions: { width: widthOverride / 3, height: widthOverride / 3 },
    }),
    __initData(Media, {
      handlePressMedia,
      initialIndex: 2,
...
function FourImages(arg0) {
  ...
  obj2.style = items;
  const items1 = [
    __initData(Media, { handlePressMedia, initialIndex: 0, source: sources[0], dimensions: size }),
    __initData(Media, { handlePressMedia, initialIndex: 1, source: sources[1], dimensions: size }),
  ];
  ...
  obj5.style = items3;
  const items4 = [
    __initData(Media, { handlePressMedia, initialIndex: 2, source: sources[2], dimensions: size }),
    __initData(Media, { handlePressMedia, initialIndex: 3, source: sources[3], dimensions: size }),
  ];
...
const collapsedCategories = createStyles.createStyles(() => {
  ...
  obj.topRow = {
    overflow: "hidden",
    borderTopEndRadius: nativeDefault.radii.lg,
    borderTopStartRadius: nativeDefault.radii.lg,
  };
  const obj4 = {
    overflow: "hidden",
    borderTopEndRadius: nativeDefault.radii.lg,
  ...
  obj.bottomRow = {
    overflow: "hidden",
    borderBottomEndRadius: nativeDefault.radii.lg,
    borderBottomStartRadius: nativeDefault.radii.lg,
  };
  const obj5 = {
    overflow: "hidden",
    borderBottomEndRadius: nativeDefault.radii.lg,
...
export default function ICYMIMediaMosaic(message) {
  ...
  const stateFromStores = message(itemType[14]).useStateFromStores(items, () =>
  ...
  const memo = obj2.useMemo(() => {
    const result = MediaSourceUtil.extractMediaSourcesFromMessage(
      message,
      message,
      undefined,
      ICYMITypes.GRAVITY_VALID_EMBED_TYPES,
    );
    const mapped = result.map((item) => message(itemType[29]).flattenSource(item));
    const found = mapped.filter(GlobalUtils.isNotNullish);
    const tmp2 = _slicedToArray(
      _mod12.partition(found, (accessoryType) => "embed" === accessoryType.accessoryType),
      2,
    );
  ...
  handlePressMedia = obj2.useCallback((arg0) => {
    ...
    widthOverride(itemType[25]).feedItemActioned({
      itemId: items.id,
      itemType,
      actionParameters: {
    ...
    message(itemType[33]).ack(
      items.channel_id,
      { page: constants3.ICYMI, object: constants2.ACK_MEDIA_VIEWED, objectType: constants.ACK_SEMI_AUTOMATIC },
      true,
      true,
      items.id,
    );
    ...
    const obj5 = {
      ...
      object: constants2.ACK_MEDIA_VIEWED,
      objectType: constants.ACK_SEMI_AUTOMATIC,
    };
    const obj7 = {
