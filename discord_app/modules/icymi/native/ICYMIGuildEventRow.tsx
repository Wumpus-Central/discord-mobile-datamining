// === Module 15873: ICYMIGuildEventRow ===
...
function ICYMIGuildEventRow(event) {
  ...
  let React;
  const tmp = callback4();
  const tmp4 = guild(9095)(event, null);
  ...
  const callback = React.useCallback(() => {
    ...
    guild(9060).feedItemActioned(obj);
    const obj2 = guild(9060);
    obj = { eventId: event.id, event };
    const result = event(9096).openGuildEventDetails(obj);
  }, items1);
  const callback1 = React.useCallback(() => {
    ...
    guild(9060).feedItemActioned(obj);
    const obj2 = guild(9060);
    event(6066).transitionToGuild(guild.id);
  }, items2);
  ...
  obj1[1] = closure_11(event(4376).Text, {
    ...
    children: React.useMemo(() => {
      if (dependencyMap) {
        ...
        eventTimeData = tmp(9089).getEventTimeData(c3);
        const tmpResult = tmp(9089);
      }
  ...
  items3[1] = closure_11(event(4376).Text, obj2);
  ...
  const items4 = [closure_11(event(4919).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4376).Text, obj9);
  obj7[1] = items4;
  ...
  if (tmp7 == null) {
    ...
    if (null != locationFromEvent) {
      tmp8Result = tmp8(9181);
      result = tmp8Result.guildEventLocationParser(locationFromEvent, true);
    }
    tmp27 = result;
  }
  items6[1] = closure_11(event(4376).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
  obj10[1] = items6;
