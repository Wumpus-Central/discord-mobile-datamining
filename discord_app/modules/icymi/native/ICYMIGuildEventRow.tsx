// === Module 16035: ICYMIGuildEventRow ===
...
function ICYMIGuildEventRow(event) {
  ...
  let React;
  const tmp = callback4();
  const tmp4 = guild(9576)(event, null);
  ...
  const callback = React.useCallback(() => {
    ...
    guild(8568).feedItemActioned(obj);
    const obj2 = guild(8568);
    obj = { eventId: event.id, event };
    const result = event(9698).openGuildEventDetails(obj);
  }, items1);
  const callback1 = React.useCallback(() => {
    ...
    guild(8568).feedItemActioned(obj);
    const obj2 = guild(8568);
    event(6151).transitionToGuild(guild.id);
  }, items2);
  ...
  obj1[1] = closure_11(event(4442).Text, {
    ...
    children: React.useMemo(() => {
      if (dependencyMap) {
        ...
        eventTimeData = tmp(9573).getEventTimeData(c3);
        const tmpResult = tmp(9573);
      }
  ...
  items3[1] = closure_11(event(4442).Text, obj2);
  ...
  const items4 = [closure_11(event(5003).GroupIcon, { size: "xs", style: tmp.eventsChannelIcon }), ];
  ...
  items4[1] = closure_11(event(4442).Text, obj9);
  obj7[1] = items4;
  ...
  items6[1] = closure_11(event(4442).Text, { lineClamp: 2, variant: "text-xs/normal", color: "text-muted", children: tmp27 });
  obj10[1] = items6;
