...
export default function ICYMISettingsActionSheet() {
  ...
  if (ICYMIStaffDebuggingUtilityExperiment.useConfig({ location: "settings action sheet" }).enabled) {
    ...
    const items1 = [closure_10(tmp(7273).TableSwitchRow, obj), , ];
    let obj2 = {
      ...
      onPress() {
          ...
          const item = dehydratedItems.forEach((type) => {
            ...
            if (tmp3) {
              ...
              tmpResult.ack(channel_id, obj, true, true, closure_1_1(paths[20]).atPreviousMillisecond(type.data.message_id));
    ...
    items1[1] = closure_10(tmp(5656).TableRow, obj2);
    let obj3 = {
            ...
            try {
              ...
              if (0 === v1) {
                if (arg0 === 1) {
                  ...
                  v1 = 1;
                  dependencyMap = 1;
                  const obj1 = { value: obj2.regenerateFeedAndClearReadStates(constants.ACK_GRAVITY_REGENERATE_FEED_AND_CLEAR_READ_STATES_BUTTON), done: false };
                  return obj1;
                }
              } else if (arg0 === 1) {
                dependencyMap = 3;
    ...
    items1[2] = closure_10(tmp(5656).TableRow, obj3);
  ...
  items2[1] = closure_10(stateFromStoresObject(5656).TableRow, obj5);
  obj.children = items2;
  const items3 = [closure_12(stateFromStoresObject(5738).TableRowGroup, obj), ];
  const tmp4 = closure_13();
  items3[1] = closure_10(View, { style: closure_13().padding });
  obj4.children = items3;
  return closure_12(stateFromStoresObject(7270).ActionSheet, obj4);
};