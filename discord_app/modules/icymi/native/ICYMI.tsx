...
import FiltersHorizontalIcon from "FiltersHorizontalIcon" /* 14527 */;
import goBack from "goBack" /* 15837 */;
import IconWithBadge from "IconWithBadge" /* 15839 */;
import jsxDefault from "jsx" /* 15958 */;
...
import closure_10 from "initialize" /* 9125 */;
import closure_11 from "filterStaffGuild" /* 9113 */;
import { NUM_GUILDS_EXTENDED_ONBOARDING as closure_12 } from "NUM_GUILDS_EXTENDED_ONBOARDING" /* 15887 */;
...
function InfoButton() {
  return callback(IconButton.IconButton, {
    ...
    onPress() {
      callback2(9128).itemInteracted("info_button", "info_button", "press_info_button");
      const obj = callback2(9128);
      callback2(9128).feedPageActioned({ actionParameters: { actionGestureType: "press", actionTargetElement: "info_button", actionIntentType: "open", actionDestinationType: null } });
      const obj2 = callback2(9128);
      callback(15901).pushICYMIInfoModal({ extendedOnboarding: true });
    }
  });
}
...
function ICYMI(inNestedNavigator) {
  ...
  const effect3 = handleOnRefresh.useEffect(() => {
    if (stateFromStores3 === isFocused(visibleItemIds[45]).GravityICYMIDoubleTapBehavior.DEFAULT) {
      ...
      obj[0] = function scrollToTop() {
        callback(8603).showForLaterModal(callback(8604).SavedMessageSortTypes.BOOKMARK);
      };
      ref1.current = obj;
    }
  }, items8);
...
export const ICYMITab = function ICYMITab(route) {
  ...
  obj1[0] = closure_14(_require(15889).ICYMIContextProvider, obj2);
