...
function ReplyMessageContent(message) {
  ...
  const width = noop.useContext(tmp2(16880).ICYMIContext).width;
  let tmp8 = null;
...
let closure_21 = noop.memo((message) => {
  ...
  let id;
  const stateFromStores = message(guild[17]).useStateFromStores(items, () =>
    UserGuildSettingsStore.isChannelMuted(channel.getGuildId(), channel.id),
  );
  if (guild != null) {
    id = guild.id;
  ...
  if (null != iCYMIMessage) {
    ...
    tmp20Result = closure_14(ReplyMessageContent, obj7);
  }
