...
export default function NewContentPill(onPress) {
  ...
  let tmp = closure_11();
  let items = [first1];
  stateFromStoresArray = onPress(stateFromStoresArray[10]).useStateFromStoresArray(items, () =>
    first1.getNewUnreadDehydratedItems(),
  );
  let obj = onPress(stateFromStoresArray[10]);
  const items1 = [first1];
  const stateFromStores = onPress(stateFromStoresArray[10]).useStateFromStores(
    items1,
    () => first1.hasNewContent(),
    [],
  );
  ...
  closure_6 = first.useMemo(() => {
    ...
    const item = first.forEach((data) => {
      if (items.length < onPress(stateFromStoresArray[12]).MIN_ITEMS_FOR_NEW_PILL) {
        ...
        tmp4 = onPress(stateFromStoresArray[13]).isGuildItem(data) && !items.includes(data.data.guild_id);
      }
    });
    return items;
  }, items2);
  ...
  const fn = function y() {
    ...
    obj2.opacity = spring.withSpring(num2, closure_12, "respect-motion-settings");
    return obj2;
  };
  ...
  if (0 === stateFromStoresArray1.length) {
    ...
    if (isRefreshing) {
      ...
      unsafe_rawColors.color = tmp2(tmp3[21]).isThemeDark(tmp10)
      ...
      obj11.style = obj12;
      const obj13 = {
        guild: stateFromStoresArray1[stateFromStoresArray1.length - 1],
        size: tmp2(tmp3[9]).GuildIconSizes.XSMALL,
        style: tmp.guildIconBG,
      };
