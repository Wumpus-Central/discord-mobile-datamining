...
function CutoutGuildIconWithUser(guild) {
  ...
  obj = {
    animate: true,
    style: callback3().authorIcon,
    guildId: guild.id,
    user: guild.author,
    size: Button.AvatarSizes.XSMALL,
  };
...
export const navigateToPost = function navigateToPost(id, id2, id3) {
  ...
  const timerId = setTimeout(() => {
    ...
    obj = {
      page: closure_1_12.ICYMI,
      object: closure_1_10.ACK_MESSAGE_VIEWED,
      objectType: closure_1_11.ACK_SEMI_AUTOMATIC,
    };
...
export const AnnouncementContentPost = function AnnouncementContentPost(guild) {
  ...
  items2[1] = callback(guild(author[35]).AnnouncementsIcon, {
    size: "sm",
...
export const MessageContentPost = function MessageContentPost(guild) {
  ...
  obj = {
    ...
    channel,
    timestamp,
    avatar: callback(CutoutGuildIconWithUser, { guild, author }),
    title: callback(tmp2(tmp3[27]).Text, obj, channel.id),
    subtitle: null,
    onHeaderPress: null,
    onHeaderLongPress: null,
    id: null,
  ...
  obj2[3] = `${tmp10} `;
  const items3 = [
    callback(guild(author[27]).Text, obj2),
    callback(LightbulbIcon, { size: "sm", color: channel(author[21]).colors.TEXT_SUBTLE }),
  ];
  tmp2Result = tmp2(tmp3[36]);
  ...
  const effect = sharedValue.useEffect(() => {
    if (highlight) {
      ...
      const result = sharedValue.set(
        obj.withSequence(
          withTimingResult,
          obj3.withDelay(500, highlight(_undefined[45]).withTiming(0, { duration: 350 })),
        ),
      );
...
export const ThreadAsComments = function ThreadAsComments(arg0) {
  ...
  if (
    obj1.useStateFromStores(items2, () =>
      closure_1_8.canWithPartialContext(closure_1_16.VIEW_CHANNEL, { channelId: guild.id }),
    )
  ) {
    if (null != thread) {
