...
function CutoutGuildIconWithUser(guild) {
  ...
  obj.icon = collapsedCategories(native.Avatar, {
    animate: true,
...
createICYMIStyles.createICYMIStyles((paddingBottom) => {
  ...
  obj2.channelNameAndAccessoryLarge = {
...
export const navigateToPost = function navigateToPost(id, id2, id3) {
  ...
  const timerId = setTimeout(() => {
    ReadStateActionCreators.ack(
      closure_0,
      { page: constants3.ICYMI, object: constants.ACK_MESSAGE_VIEWED, objectType: constants2.ACK_SEMI_AUTOMATIC },
      true,
      true,
      closure_1,
    );
...
export const AnnouncementContentPost = function AnnouncementContentPost(guild) {
  ...
  const stateFromStores = guild(author[31]).useStateFromStores(items, () =>
    GuildMemberStore.getMember(guild.id, author.id),
  );
  let colorString;
...
export const MessageContentPost = function MessageContentPost(guild) {
  ...
  const stateFromStores = guild(author[31]).useStateFromStores(items1, () =>
    GuildMemberStore.getMember(guild.id, author.id),
  );
  let colorString;
  if (stateFromStores != null) {
  ...
  const obj4 = {
    ...
    color: "text-default",
    style: { marginRight: message.useContext(guild(author[40]).ICYMIContext).margin },
    children: null,
  };
...
export const SimplePost = function SimplePost(arg0) {
  ...
  const tmp = closure_21();
  const tmp2 = token;
  const tmp4 = token(4763)();
  token = highlight(4528).useToken(token(576).colors.MESSAGE_HIGHLIGHT_BACKGROUND_DEFAULT, tmp4);
  let obj = highlight(4528);
  const hexWithOpacityResult = highlight(4680).hexWithOpacity(token(576).unsafe_rawColors.BRAND_360, 0.25);
  dependencyMap = hexWithOpacityResult;
  let obj2 = highlight(4680);
  const sharedValue = highlight(4563).useSharedValue(0);
  let obj3 = highlight(4563);
  const fn = function c() {
  ...
  let obj4 = highlight(4563);
  fn.__closure = {
    interpolateColor: highlight(4563).interpolateColor,
  ...
  if (highlight) {
    ...
    const items2 = [closure_18(tmp2(4563).View, obj7)];
...
export const ThreadAsComments = function ThreadAsComments(arg0) {
  ...
  if (
    obj2.useStateFromStores(items2, () =>
    ...
    if (null != thread) {
      if (null != mostRecentMessage) {
        ...
        const obj4 = { user: author, guildId: thread.guild_id, size: parentMessage(1177).AvatarSizes.XSMALL };
        const items4 = [closure_18(parentMessage(1177).Avatar, obj4), ,];
    ...
    const items7 = [closure_18(parentMessage(4825).Text, obj12)];
    const obj13 = { style: tmp.commentCount, children: null };
    const obj14 = { style: tmp.commentsIcon };
    const items8 = [closure_18(parentMessage(5378).ChatIcon, obj14)];
    const obj15 = { style: tmp.chevron, size: "xxs" };
    items8[1] = closure_18(parentMessage(6625).ChevronSmallRightIcon, obj15);
    obj13.children = items8;
    items7[1] = closure_19(View, obj13);
    obj11.children = items7;
    return closure_19(parentMessage(5428).PressableHighlight, obj11);
  } else {
    return null;
  }
