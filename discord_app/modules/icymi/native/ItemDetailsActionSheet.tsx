...
export default function ItemDetailsActionSheet(arg0) {
  ...
  const tmp6 = computeChannelName(stateFromStores, true);
  if (null != stateFromStores1) {
    obj = { guild: null, size: null };
    obj[0] = stateFromStores1;
    obj[1] = tmp(5819).GuildIconSizes.LARGE;
    let tmp9 = callback(tmp5(5819), obj);
    const tmp5Result = tmp5(5819);
  ...
  const tmp8 = useDesignToggle("show_icymi_debug_scores");
  if (result) {
    result = null != stateFromStores1;
  }
  if (result) {
    result = tmp(8961).isChannelCustomScoreEligible(stateFromStores);
    const tmpResult = tmp(8961);
  }
  ...
  obj2 = { showGradient: true, startExpanded: true, header: tmp16(tmp(10243).ActionSheetIconHeader, obj1), children: null };
  obj1[2] = str;
  ...
  if (result) {
    ...
    tmp16Result = tmp16(tmp(15656).ChannelScoreSettings, obj3);
  }
  ...
  if (tmp15Result) {
    ...
    items4[1] = tmp16(tmp(15656).GuildScoreSettings, obj6);
    obj5[0] = items4;
  ...
  if (null != stateFromStores2) {
    ...
    if (tmp8) {
      ...
      obj7[2] = tmp16(tmp(5414).TableRow, obj8);
      tmp16Result = tmp16(tmp(5807).TableRowGroup, obj7);
    }
  }
