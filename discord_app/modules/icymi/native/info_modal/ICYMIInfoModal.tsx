...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  const navigatorScreens = extendedOnboarding(7240).useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() =>
        skipIntro(4959).popWithKey(extendedOnboarding(16800).ICYMI_INFO_MODAL_KEY),
      );
    } else {
  ...
  if (extendedOnboarding) {
    ...
    jsx(tmp(14448).StepModal, { screens: navigatorScreens, steps: tmp4, initialRouteName: null });
  } else {
    let obj3 = { screens: navigatorScreens, initialRouteName: tmp(16800).ICYMIInfoScreens.DEFAULT };
    return jsx(tmp(11463).Modal, { screens: navigatorScreens, initialRouteName: tmp(16800).ICYMIInfoScreens.DEFAULT });
  }
  let obj = extendedOnboarding(7240);
}
