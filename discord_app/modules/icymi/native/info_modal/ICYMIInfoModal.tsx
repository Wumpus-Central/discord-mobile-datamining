...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  let obj = extendedOnboarding(5723);
  ...
  const navigatorScreens = obj.useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() => callback2(4507).popWithKey(callback(15499).ICYMI_INFO_MODAL_KEY));
    } else {
      ...
      headerRight() {
        ...
        if (!closure_1) {
          ...
          obj[1] = function onPress() {
            const ICYMIAnalytics = callback(8861).ICYMIAnalytics;
    ...
    obj2[2] = function render() {
      return callback3(callback2(15516), {});
    };
  ...
  if (extendedOnboarding) {
    ...
    obj[1] = tmp4;
    let ICYMIInfoScreens = tmp(15499).ICYMIInfoScreens;
    obj[2] = skipIntro ? ICYMIInfoScreens.TOPICS_CLOUD : ICYMIInfoScreens.DEFAULT;
    tmp5(tmp(13355).StepModal, obj);
  } else {
    obj = { screens: null, initialRouteName: null };
    obj[0] = navigatorScreens;
    obj[1] = tmp(15499).ICYMIInfoScreens.DEFAULT;
    return tmp5(tmp(11269).Modal, obj);
  }
};