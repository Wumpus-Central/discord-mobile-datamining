...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  const navigatorScreens = extendedOnboarding(7196).useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() =>
        skipIntro(4925).popWithKey(extendedOnboarding(16724).ICYMI_INFO_MODAL_KEY),
      );
    } else {
  ...
  if (extendedOnboarding) {
    ...
    jsx(tmp(14393).StepModal, { screens: navigatorScreens, steps: tmp4, initialRouteName: null });
  } else {
    let obj3 = { screens: navigatorScreens, initialRouteName: tmp(16724).ICYMIInfoScreens.DEFAULT };
    return jsx(tmp(11451).Modal, { screens: navigatorScreens, initialRouteName: tmp(16724).ICYMIInfoScreens.DEFAULT });
  }
  let obj = extendedOnboarding(7196);
}
