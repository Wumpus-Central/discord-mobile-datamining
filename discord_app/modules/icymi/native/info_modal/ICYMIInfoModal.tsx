...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  let obj = extendedOnboarding(5955);
  ...
  const navigatorScreens = obj.useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() => callback2(4675).popWithKey(callback(15903).ICYMI_INFO_MODAL_KEY));
    } else {
    ...
    obj[extendedOnboarding(closure_1_2[3]).ICYMIInfoScreens.TOPICS_CLOUD] = {
      ...
      render() {
        return callback3(callback2(15912), {});
      },
    ...
    obj2[2] = function render() {
      return callback3(callback2(15920), {});
    };
  ...
  if (extendedOnboarding) {
    ...
    obj[1] = tmp4;
    let ICYMIInfoScreens = tmp(15903).ICYMIInfoScreens;
    obj[2] = skipIntro ? ICYMIInfoScreens.TOPICS_CLOUD : ICYMIInfoScreens.DEFAULT;
    tmp5(tmp(13704).StepModal, obj);
  } else {
    obj = { screens: null, initialRouteName: null };
    obj[0] = navigatorScreens;
    obj[1] = tmp(15903).ICYMIInfoScreens.DEFAULT;
    return tmp5(tmp(11569).Modal, obj);
  }
};