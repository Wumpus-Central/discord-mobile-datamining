...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  const navigatorScreens = extendedOnboarding(7333).useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() =>
        skipIntro(5032).popWithKey(extendedOnboarding(16894).ICYMI_INFO_MODAL_KEY),
      );
    } else {
  ...
  if (extendedOnboarding) {
    ...
    jsx(tmp(14549).StepModal, { screens: navigatorScreens, steps: tmp4, initialRouteName: null });
  } else {
    let obj3 = { screens: navigatorScreens, initialRouteName: tmp(16894).ICYMIInfoScreens.DEFAULT };
    return jsx(tmp(11551).Modal, { screens: navigatorScreens, initialRouteName: tmp(16894).ICYMIInfoScreens.DEFAULT });
  }
  let obj = extendedOnboarding(7333);
}
