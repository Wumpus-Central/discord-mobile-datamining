...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  const navigatorScreens = obj.useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() => callback2(4509).popWithKey(callback(15574).ICYMI_INFO_MODAL_KEY));
    } else {
    ...
    obj[extendedOnboarding(outer1_2[3]).ICYMIInfoScreens.TOPICS_CLOUD] = {
      ...
      render() {
        return callback3(callback2(15583), {});
      },
  ...
  if (extendedOnboarding) {
    ...
    obj[1] = tmp4;
    let ICYMIInfoScreens = tmp(15574).ICYMIInfoScreens;
    obj[2] = skipIntro ? ICYMIInfoScreens.TOPICS_CLOUD : ICYMIInfoScreens.DEFAULT;
    tmp5(tmp(13425).StepModal, obj);
  } else {
    obj = { screens: null, initialRouteName: null };
    obj[0] = navigatorScreens;
    obj[1] = tmp(15574).ICYMIInfoScreens.DEFAULT;
    return tmp5(tmp(11335).Modal, obj);
  }
};