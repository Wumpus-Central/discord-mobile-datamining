...
export default function ICYMIInfoModal(extendedOnboarding) {
  ...
  const navigatorScreens = extendedOnboarding(7114).useNavigatorScreens(() => {
    ...
    if (skipIntro) {
      let headerCloseButton = obj5.getHeaderCloseButton(() =>
        skipIntro(4843).popWithKey(extendedOnboarding(16627).ICYMI_INFO_MODAL_KEY),
      );
    } else {
  ...
  if (extendedOnboarding) {
    ...
    jsx(tmp(14298).StepModal, { screens: navigatorScreens, steps: tmp4, initialRouteName: null });
  } else {
    let obj3 = { screens: navigatorScreens, initialRouteName: tmp(16627).ICYMIInfoScreens.DEFAULT };
    return jsx(tmp(12080).Modal, { screens: navigatorScreens, initialRouteName: tmp(16627).ICYMIInfoScreens.DEFAULT });
  }
  let obj = extendedOnboarding(7114);
}
