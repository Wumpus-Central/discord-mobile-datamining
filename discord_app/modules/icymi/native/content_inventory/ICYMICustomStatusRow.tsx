...
let closure_13 = createICYMIStyles.createICYMIStyles((gap, arg1) => {
  ...
  const size2 = {
    alignItems: "center",
    justifyContent: "center",
    borderRadius: tmp3(576).radii.md,
    height: 28,
    width: 40,
  };
...
export default function GravityCustomStatusEntryRow(id) {
  ...
  if (null == stateFromStores) {
    ...
    if (renderForScreenshot) {
      ...
      size = {
        shape: tmp2(tmp3[32]).CutoutShape.RoundedRect,
        x: null,
        y: null,
        width: 46,
        height: 34,
      ...
      const size1 = {
        shape: tmp2(tmp3[32]).CutoutShape.RoundedRect,
        x: size.width - tmp14(tmp3[6]).space.PX_16 - 86 - PX_8,
        y: size.height - 14 - 3,
        width: 46,
        height: 34,
        cornerRadius: tmp14(tmp3[6]).radii.md + 3,
      };
      items12[1] = size1;
    }
