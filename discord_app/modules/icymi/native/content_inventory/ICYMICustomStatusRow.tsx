...
function UploadPlaceholder() {
  { style: callback3(false).uploadContainer, children: null };
  let obj = { source: registerAssetDefault, size: Button.IconSizes.SMALL };
...
let closure_13 = createICYMIStyles.createICYMIStyles((gap) => {
  let obj = { marginLeft: 32, borderRadius: ThemesDefault.radii.round, height: 12, width: 12, overflow: "hidden" };
  obj[1] = obj;
...
const result = require("obj132").fileFinishedImporting("modules/icymi/native/content_inventory/ICYMICustomStatusRow.tsx");
...
export default function GravityCustomStatusEntryRow(id) {
  ...
  const tmp5Result = callback3(backgroundColor);
  ...
  if (null != obj.id) {
    ...
    obj1[2] = flag;
    obj1[3] = obj;
    let tmp20Result = callback(tmp14Result, obj1);
  ...
  if (null == stateFromStores) {
    ...
    obj5[3] = tmp14(renderForScreenshot[29]).getName(stateFromStores1);
    const items6 = [callback(tmp2(renderForScreenshot[11]).Text, obj5), ];
    ...
    if (hasStatus) {
      ...
      obj7[3] = tmp2Result7.getRelativeTimestamp(tmp14(renderForScreenshot[26]).extractTimestamp(id));
      tmp31Result = callback(tmp2(renderForScreenshot[11]).Text, obj7);
      const tmp14Result3 = tmp14(renderForScreenshot[26]);
    }
    const items7 = [tmp31Result, , ];
    tmp31Result = null;
    if (tmp27) {
      tmp31Result = callback(tmp14(renderForScreenshot[31]), {});
    }
    ...
    if (tmp27) {
      ...
      obj8[3] = intl.string(tmp2(renderForScreenshot[12]).t.tWnHcL);
      tmp31Result1 = callback(tmp2(renderForScreenshot[11]).Text, obj8);
    }
    items7[2] = tmp31Result1;
    obj6[1] = items7;
    items6[1] = callback2(closure_5, obj6);
    obj3[1] = items6;
    obj2[4] = callback2(closure_5, obj3);
    ...
    if ("otherUserStatus" === variant.kind) {
      ...
      obj9[3] = intl2.string(tmp2(renderForScreenshot[12]).t.fxOLPR);
      tmp31Result2 = callback(tmp2(renderForScreenshot[11]).Text, obj9);
    }
    obj2[5] = tmp31Result2;
    ...
    const items9 = [callback(closure_5, obj11), ];
    const obj12 = { style: null };
    const items10 = [tmp.bottomBubble, tmp5Result.background];
    obj12[0] = items10;
    items9[1] = callback(closure_5, obj12);
    obj10[2] = items9;
    const items11 = [callback2(tmp14(renderForScreenshot[32]), obj10), ];
    ...
    if (renderForScreenshot) {
      ...
      obj14[0] = tmp2(renderForScreenshot[32]).CutoutShape.RoundedRect;
      obj14[1] = size.width - tmp14(renderForScreenshot[6]).space.PX_16 - 40 - 3;
      obj14[2] = size.height - 14 - 3;
      obj14[5] = tmp14(renderForScreenshot[6]).radii.md + 3;
      items12 = [obj14, ];
      const obj15 = { shape: null, x: null, y: null, width: 46, height: 34, cornerRadius: null };
      obj15[0] = tmp2(renderForScreenshot[32]).CutoutShape.RoundedRect;
      obj15[1] = size.width - tmp14(renderForScreenshot[6]).space.PX_16 - 86 - PX_8;
      obj15[2] = size.height - 14 - 3;
      obj15[5] = tmp14(renderForScreenshot[6]).radii.md + 3;
      items12[1] = obj15;
    }
    ...
    if (tmp31Result3) {
      ...
      tmp31Result3 = callback(tmp2(renderForScreenshot[11]).Text, obj19);
    }
    ...
    if (!hasStatus) {
      ...
      obj20[1] = intl3.string(tmp2(renderForScreenshot[12]).t["6ojWO0"]);
      tmp31Result4 = callback(tmp2(renderForScreenshot[11]).Text, obj20);
    }
    items15[2] = tmp31Result4;
    obj18[1] = items15;
    const items16 = [callback2(closure_5, obj18), ];
    let tmp31Result5 = !hasStatus;
    if (!hasStatus) {
      tmp31Result5 = callback(UploadPlaceholder, {});
    }
    items16[1] = tmp31Result5;
    obj17[4] = items16;
    obj16[1] = callback2(tmp2(renderForScreenshot[21]).PressableHighlight, obj17);
    const items17 = [callback(tmp14(renderForScreenshot[32]), obj16), tmp25];
    obj13[1] = items17;
    items11[1] = callback2(closure_5, obj13);
    obj2[7] = items11;
    return callback2(tmp14Result1, obj2);
  }
