...
function ContentTimestamp(entry) {
  ...
  if (isEntryActiveResult) {
    ...
    const obj3 = {
      variant: "text-sm/medium",
      color: obj[noop.useContext(noop, closure_11)].text,
      children: utils.formatEndedTimestamp(entry, tmp5),
    };
...
function BaseBadge(accessibilityLabel) {
  ...
  const items = [
    timestampProducer(Icon, { style: tmp.icon, color: iconColor }),
...
export const GameTimestampBadge = function GameTimestampBadge(entry) {
  ...
  const items = [
    timestampProducer(GameControllerIcon.GameControllerIcon, { style: tmp.icon, color: icon }),
...
export const CustomStatusTimestampBadge = function CustomStatusTimestampBadge(entry) {
  ...
  const items = [
    timestampProducer(TimerIcon.TimerIcon, { style: tmp.icon, color: nativeDefault.colors.INTERACTIVE_TEXT_DEFAULT }),
