...
function ContentTimestamp(entry) {
  ...
  if (isEntryActiveResult) {
    ...
    obj[1] = tmp.text;
    obj[2] = tmp2(8161).formatEndedTimestamp(entry, tmp6);
...
export const GameTimestampBadge = function GameTimestampBadge(entry) {
  ...
  const items = [callback(tmp2(8685).GameControllerIcon, obj), callback(ContentTimestamp, { entry })];
  obj[1] = items;
  return callback2(View, obj);
};
...
export const TrendingBadge = function TrendingBadge(entry) {
  ...
  if (null != trendingType) {
    tmp4 = null;
    if (trendingType !== tmp(12206).TrendingType.TRENDING_TYPE_UNSPECIFIED) {
      obj = { Icon: null, text: null, iconColor: null };
      obj[0] = tmp(12119).FireIcon;
      const intl = tmp(1236).intl;
