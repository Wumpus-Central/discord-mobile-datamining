// === Module 9882: SecureFramesVerificationBottomSheet ===
...
export default function SecureFramesVerificationBottomSheet(onShareClick) {
  ...
  obj1 = { title: null, leading: callback(onShareClick(5999).ActionSheetCloseButton, { onPress: callback1 }), trailing: null };
  ...
  obj2[6] = callback(onShareClick(4474).Text, obj3);
  obj1[2] = callback(onShareClick(5077).PressableOpacity, obj2);
  obj[1] = callback(onShareClick(5630).BottomSheetTitleHeader, obj1);
  const obj4 = { style: tmp.content, children: null };
  const obj5 = { spacing: 8, justify: "center", align: "center", direction: "vertical", children: null };
  const obj6 = { style: tmp.iconContainer, children: callback(onShareClick(5049).LockIcon, { style: tmp.icon, color: readableSecureFramesFingerprint(709).colors.TEXT_SUBTLE }) };
  const items1 = [callback(View, obj6), callback(onShareClick(4474).Text, { variant: "heading-xl/bold", color: "mobile-text-heading-primary", children: title }), callback(onShareClick(4474).Text, { style: tmp.subtitle, variant: "text-md/medium", color: "text-default", children: subtitle })];
  obj5[4] = items1;
  const items2 = [callback2(onShareClick(4927).Stack, obj5), , ];
  ...
  if (tmp7Result) {
    ...
    tmp7Result = tmp7(tmp10(9883), obj10);
  }
  obj9[1] = tmp7Result;
  obj9[2] = readableSecureFramesFingerprint;
  obj9[3] = closure_6;
  items2[1] = callback(readableSecureFramesFingerprint(9880), obj9);
  items2[2] = callback(tmp2(4474).Text, { style: tmp.footer, variant: "text-xs/normal", color: "text-muted", children: footer });
  obj4[1] = items2;
  obj[2] = tmp8(tmp9, obj4);
  return callback(onShareClick(5631).BottomSheet, obj);
};