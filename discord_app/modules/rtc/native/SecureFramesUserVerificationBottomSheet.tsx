...
({
  ...
  USER_VERIFIED_TOAST_KEY: closure_11,
...
export default function SecureFramesUserVerificationBottomSheet(userId) {
  ...
  const isUserSecureFramesVerified = userId(fingerprint[12]).useIsUserSecureFramesVerified({
    userId,
    channelId,
    userKey: fingerprintUserKey,
  });
  let obj3 = userId(fingerprint[12]);
  const isPersistentSecureFramesFingerprint = userId(fingerprint[13]).useIsPersistentSecureFramesFingerprint({
    userId,
    userKey: fingerprintUserKey,
  });
  ...
  const stateFromStores = userId(fingerprint[14]).useStateFromStores(items, () =>
    isUserSecureFramesVerified.isConnected(),
  );
  let obj5 = userId(fingerprint[14]);
  const items1 = [isUserSecureFramesVerified];
  const stateFromStores1 = userId(fingerprint[14]).useStateFromStores(items1, () =>
    RTCConnectionStore.isUserConnected(userId),
  );
  ...
  if (!loading) {
    ...
    items9[4] = closure_14(tmp2(tmp3[31]).Stack, obj17);
