...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  let joinButtonText = closure_11(tmp, obj.isThemeLight(voiceStates(4540)()));
  ...
  let obj3 = channel(5481);
  const stageParticipantsCount = obj3.useStageParticipantsCount(channel.id, channel(5475).StageChannelParticipantNamedIndex.AUDIENCE);
  ...
  tmp3Result = tmp3(12364);
  const connectedUserLimit = tmp3Result.useConnectedUserLimit({ channel, video: tmp12 });
  tmp8 = channel(5467).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj.children = jsx(tmp3(4601).Text, { style: null, color: "interactive-text-default", variant: "text-xs/semibold", lineClamp: 1, children: null });
  ...
  const tmp3Result1 = channel(12364);
});