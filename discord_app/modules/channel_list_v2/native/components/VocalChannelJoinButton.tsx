...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  let joinButtonText = closure_11(tmp, obj.isThemeLight(voiceStates(4571)()));
  ...
  let obj3 = channel(5512);
  const stageParticipantsCount = obj3.useStageParticipantsCount(channel.id, channel(5506).StageChannelParticipantNamedIndex.AUDIENCE);
  ...
  tmp3Result = tmp3(12423);
  const connectedUserLimit = tmp3Result.useConnectedUserLimit({ channel, video: tmp12 });
  tmp8 = channel(5498).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj = { accessibilityRole: "none", pointerEvents: "none", onPress, accessible: false, accessibilityElementsHidden: true, importantForAccessibility: "no", style: joinButtonText.joinButton, children: null };
    ...
    obj.children = jsx(tmp3(4632).Text, { style: null, color: "interactive-text-default", variant: "text-xs/semibold", lineClamp: 1, children: null });
  ...
  const tmp3Result1 = channel(12423);
});