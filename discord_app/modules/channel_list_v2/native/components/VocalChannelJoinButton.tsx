...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  let joinButtonText = closure_11(tmp, obj.isThemeLight(voiceStates(4541)()));
  ...
  let obj3 = channel(5482);
  const stageParticipantsCount = obj3.useStageParticipantsCount(channel.id, channel(5476).StageChannelParticipantNamedIndex.AUDIENCE);
  ...
  tmp3Result = tmp3(12387);
  const connectedUserLimit = tmp3Result.useConnectedUserLimit({ channel, video: tmp12 });
  tmp8 = channel(5468).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  if (isConnectedToVoiceChannel) {
    ...
    obj.children = jsx(tmp3(4602).Text, { style: null, color: "interactive-text-default", variant: "text-xs/semibold", lineClamp: 1, children: null });
  ...
  const tmp3Result1 = channel(12387);
});