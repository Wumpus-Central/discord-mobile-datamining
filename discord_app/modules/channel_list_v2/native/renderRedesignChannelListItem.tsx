...
import _modDef15784 from "items/ShowAllVoiceChannelsButton.tsx";
...
export const getFastListRecyclerKey = function getFastListRecyclerKey(guildChannels, arg1, arg2, arg3) {
  ...
  } else if (arg1 === tmp(7795).FastListItemTypes.SECTION) {
  ...
  if (guildChannels.favoritesSectionNumber === section) {
    ...
    if (obj.isNamedCategorySection(section)) {
      ...
      if (null != namedCategoryFromSection) {
        ...
        obj4[1] = arg3;
        tmp5 = callback4(tmp(15694).CategoryChannel, obj4);
      }
      return tmp5;
    } else {
      return null;
...
export const renderChannelListSectionFooter = function renderChannelListSectionFooter(
  ...
  if (computeSubtitle.SECTION_INDEX_CHANNEL_NOTICES === section) {
    return null;
  } else if (tmp5(7273).SECTION_INDEX_GUILD_ACTIONS === section) {
    const guildActionSection = guildChannels.getGuildActionSection();
    let flag = false;
      ...
      if (guildChannels.recentsSectionNumber !== section) {
        if (guildChannels.voiceChannelsSectionNumber === section) {
          ...
          if (obj.render) {
            ...
            obj3[1] = section;
            obj3[2] = ref;
            obj2[1] = callback4(_modDef15784, obj3);
...
export const getChannelListSectionFooterSize = function getChannelListSectionFooterSize(
  ...
  if (computeSubtitle.SECTION_INDEX_CHANNEL_NOTICES === section) {
    return 0;
  } else if (tmp3(7273).SECTION_INDEX_GUILD_ACTIONS === section) {
    const guildActionSection = guildChannels.getGuildActionSection();
    let flag = false;
    if (!guildActionSection.isEmpty()) {
      const rows = guildActionSection.getRows();
    ...
    let num5 = 0;
    if (flag) {
      num5 = tmp3(11862).DIVIDER_HEIGHT;
    }
    return num5;
  } else {
    if (guildChannels.favoritesSectionNumber !== section) {
    ...
    return tmp3(11862).DIVIDER_HEIGHT;
  }
};
export const renderChannelListItem = function renderChannelListItem(arg0) {
  ...
  if (channel(7273).SECTION_INDEX_CHANNEL_NOTICES === section) {
    const channelNoticeSection = guildChannels.getChannelNoticeSection();
    ...
    if (null != row) {
      if (constants.SPACER === row) {
        ...
        obj4[1] = guild;
        tmp61 = callback4(selectedChannelId(15777), obj4);
        ...
        if (null != gameClaimMarkAsDismissed) {
          ...
          obj5[1] = callback4(selectedChannelId(15779), obj6);
        ...
        if (null != applicationAccountLinkMarkAsDismissed) {
          ...
          if (null != startApplicationAccountLinkAuthorization) {
            ...
            if (null != accountLinkApplication) {
              ...
              obj7[1] = callback4(selectedChannelId(15783), obj8);
        ...
        if (tmp62.FAVORITES_SUGGESTIONS === row) {
          tmp61 = callback4(selectedChannelId(15790), {});
        }
      }
    }
    return tmp61;
  } else if (tmp(7273).SECTION_INDEX_GUILD_ACTIONS === section) {
    const guildActionSection = guildChannels.getGuildActionSection();
    ...
    if (null != row1) {
      if (constants2.GUILD_ROLE_SUBSCRIPTIONS === row1) {
        ...
        obj11[1] = tmp52;
        tmp51Result = callback4(tmp(15797).GuildRolesAndChannelsRow, obj11);
        ...
        tmp51Result = callback4(selectedChannelId(15798), obj12);
        ...
        obj15[0] = guild.id;
        obj14[1] = callback4(tmp(15799).NewMemberActionsProgress, obj15);
        const items = [callback4(View, obj14), callback4(selectedChannelId(11862), {})];
        obj13[0] = items;
        tmp51Result = callback5(View, obj13);
      } else if (tmp28.GUILD_HUB_HEADER_OPTIONS === row1) {
        const obj16 = { guild: null };
        obj16[0] = guild;
        tmp51Result = callback4(selectedChannelId(15800), obj16);
        ...
        tmp51Result = callback4(selectedChannelId(15803), obj17);
        ...
        tmp51Result = callback4(selectedChannelId(15805), obj18);
      } else if (tmp28.GUILD_BOOSTS === row1) {
        const obj19 = { guildId: null };
        obj19[0] = guild.id;
        tmp51Result = callback4(selectedChannelId(15806), obj19);
      } else if (tmp28.GUILD_PREMIUM_PROGRESS_BAR === row1) {
        const obj20 = { children: null };
        const obj21 = { guildId: null };
        obj21[0] = guild.id;
        const items1 = [callback4(selectedChannelId(15809), obj21), callback4(selectedChannelId(11862), {})];
        obj20[0] = items1;
    ...
    if (null != channelFromSectionRow) {
      ...
      if (type === type) {
        ...
        const items2 = [callback4(selectedChannelId(15813), obj22)];
        ...
        tmp13 = callback4(selectedChannelId(15823), obj24);
      } else if (tmp(692).ChannelTypes.GUILD_STAGE_VOICE === type) {
        const obj25 = { channel: null, selected: null };
        obj25[0] = record;
        obj25[1] = tmp5;
        tmp13 = callback4(selectedChannelId(15825), obj25);
      } else {
          ...
          if (tmp(692).ChannelTypes.GROUP_DM !== type) {
            if (section === guildChannels.voiceChannelsSectionNumber) {
              if (record.isCategory()) {
                ...
                obj26[0] = record;
                let tmp9 = callback4(tmp(15694).CategoryChannel, obj26);
              }
              tmp13 = tmp9;
            }
            if (tmpResult.isFavoritesGuildId(guildChannels.id)) {
              if (set2.has(record.type)) {
                ...
                tmp9 = callback4(selectedChannelId(15813), obj27);
              }
            }
            const obj28 = { channel: null, selected: null };
            obj28[0] = record;
            obj28[1] = tmp5;
            tmp9 = callback4(selectedChannelId(15828), obj28);
            tmpResult = tmp(1912);
          }
        }
        ...
        obj29[0] = record;
        obj29[1] = tmp5;
        tmp13 = callback4(selectedChannelId(15827), obj29);
      }
    }
    return tmp13;
  }
...
export const getChannelListItemSize = function getChannelListItemSize(liveChannelNoticeHeight) {
  ...
  if (computeSubtitle.SECTION_INDEX_CHANNEL_NOTICES === section) {
    ...
    if (constants.SPACER === row) {
      ...
      return tmp(15779).getScaledGameClaimNoticeHeight(fontScale);
    } else if (tmp28.APPLICATION_ACCOUNT_LINK === row) {
      return tmp(15783).getScaledAccountLinkBannerHeight(fontScale);
                ...
                if (tmp26.GUILD_BOOSTS !== row1) {
                  ...
                  if (tmp26.GUILD_VIBEGRATIONS !== row1) {
                    if (tmp26.GUILD_NEW_MEMBER_ACTIONS_PROGRESS_BAR === row1) {
                      num4 = 48 + tmp(11862).DIVIDER_HEIGHT;
                    } else {
                      ...
                      if (tmp26.GUILD_HUB_HEADER_OPTIONS !== row1) {
                        ...
                        if (tmp26.GUILD_SCHEDULED_EVENTS !== row1) {
                          if (tmp26.GUILD_PREMIUM_PROGRESS_BAR === row1) {
                            num4 = tmp(15809).BOOST_PROGRESS_BAR_HEIGHT + tmp(11862).DIVIDER_HEIGHT;
                          } else {
                            num4 = 0;
    ...
    if (null != channelFromSectionRow) {
      ...
      if (type === type) {
          ...
          if (tmp(692).ChannelTypes.GUILD_STAGE_VOICE !== type) {
            ...
            if (tmp(692).ChannelTypes.DM !== type) {
              ...
              if (tmp(692).ChannelTypes.GROUP_DM !== type) {
                ...
                let sum1 = tmp31;
                if (result > 0) {
                  sum1 = tmp31 + result * tmp(15712).getVoiceUserHeight(fontScale);
                  const tmpResult4 = tmp(15712);
                }
                num = sum1;
              }
            }
        ...
        const voiceUserHeight = tmp(15712).getVoiceUserHeight(fontScale);
        if (record.type === tmp(692).ChannelTypes.GUILD_STAGE_VOICE) {
          let participantCount = store.getParticipantCount(
            record.id,
            tmp(5368).StageChannelParticipantNamedIndex.SPEAKER,
          );
        } else {
        ...
        if (record.type === tmp(692).ChannelTypes.GUILD_STAGE_VOICE) {
          num3 = 0;
          if (store.getParticipantCount(record.id, tmp(5368).StageChannelParticipantNamedIndex.AUDIENCE) > 0) {
            num3 = tmp(15710).getAudienceItemHeight(fontScale);
            const tmpResult6 = tmp(15710);
          }
        }
        let sum4 = tmp31;
        if (participantCount > 0) {
          const sum2 = tmp31 + num2;
          const sum3 = sum2 + tmp(15823).VOICE_USERS_MARGIN_TOP + voiceUserHeight * participantCount;
          sum4 = sum3 + tmp(15823).VOICE_USERS_MARGIN_BOTTOM + num3;
        }
        num = sum4;
        const tmpResult5 = tmp(15712);
      }
    }
    return num;
  }
...
export const calculateVoiceSummary = function calculateVoiceSummary(arg0) {
  ...
  if (!obj.isVoiceChannelsSection(section, guildChannels)) {
    if (section < tmp(7273).SECTION_INDEX_FIRST_NAMED_CATEGORY) {
      return null;
    }
  }
  let tmpResult = tmp(15829);
