...
let closure_18 = noop.memo((channel) => {
  ...
  ensureSyncedChannelVoiceStates = channel(ensureSyncedChannelVoiceStates[18]).useEnsureSyncedChannelVoiceStates(
    channel.id,
    voiceStates,
  );
  ...
  const stateFromStoresObject = channel(ensureSyncedChannelVoiceStates[20]).useStateFromStoresObject(
    items,
    () => ({
  ...
  const accessibilityLabelOrCheapFallbackUnsafe = channel(
    ensureSyncedChannelVoiceStates[24],
  ...
  if (0 !== ensureSyncedChannelVoiceStates.length) {
    if (collapsed) {
      ...
      obj15.children = jsx(require("VoiceUsers"), { channel, collapsed, voiceStates: ensureSyncedChannelVoiceStates });
