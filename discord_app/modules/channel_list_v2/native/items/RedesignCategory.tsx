...
import useFavoritesGuildCategoryAddActionDefault from "useFavoritesGuildCategoryAddAction" /* 10975 */;
import CategoryCollapseActionCreators from "CategoryCollapseActionCreators" /* 11558 */;
import useFavoritesGuildCategoryFullNoticeDefault from "useFavoritesGuildCategoryFullNotice" /* 16107 */;
import useFavoritesGuildCategoryLongPressDefault from "useFavoritesGuildCategoryLongPress" /* 16108 */;
...
function renderCategoryItem(muted) {
  ...
  if (null == trailingAction) {
    ...
    if (null == note) {
      ...
      if (tmp) {
        const obj4 = { accessibilityRole: "header", accessibilityState, accessibilityActions: null, onAccessibilityAction: null, onPress: null, onLongPress: null, style: null, children: null };
        let tmp33;
        if (items2.length > 0) {
          tmp33 = items2;
        }
    ...
    let obj7 = { accessible: !tmp, accessibilityRole: "button", accessibilityLabel: null, onPress: null, hitSlop: null, androidRippleConfig: null, children: null };
    let label;
    if (!tmp) {
...
export const CategoryChannel = function CategoryChannel(channel) {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => ({ collapsed: CategoryCollapseStore.isCollapsed(channel.id), muted: UserGuildSettingsStore.isChannelMuted(channel.getGuildId(), channel.id) }));
  ...
  obj1.icon = closure_11(channel(11153).ChevronSmallDownIcon, { size: "xxs", color: merged.muted ? colors.ICON_MUTED : colors.TEXT_SUBTLE, style: collapsed2 ? createStyles : obj1 });
...
export const RecentlyActiveCategory = function RecentlyActiveCategory(guildId) {
  ...
  obj = {};
  const merged1 = Object.assign(merged);
  obj.icon = closure_11(guildId(11153).ChevronSmallDownIcon, { size: "xxs", color: merged.muted ? colors.ICON_MUTED : colors.TEXT_SUBTLE, style: collapsed ? createStyles : obj1 });
