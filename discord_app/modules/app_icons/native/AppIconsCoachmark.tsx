// === Module 16271: AppIconsCoachmarkActionSheet ===

// Module 16271 (AppIconsCoachmarkActionSheet)
import ThemesDefault from "Themes" /* 712 */;
import getPremiumPlanItemDefault from "getPremiumPlanItem" /* 4043 */;
import registerAssetDefault from "registerAsset" /* 8877 */;
import registerAssetDefault2 from "registerAsset" /* 16272 */;
...
export default function AppIconsCoachmarkActionSheet(markAsDismissed) {
  ...
  items3[1] = callback(markAsDismissed(4376).Text, obj4);
  obj2[1] = items3;
  ...
  obj5[3] = stringResult;
  items2[2] = callback(markAsDismissed(4376).Text, obj5);
  obj[1] = items2;
  ...
  const items5 = [callback(markAsDismissed(4815).Button, obj7), ];
  const obj8 = { variant: "secondary", text: null, onPress: null };
  const intl4 = tmp2(1236).intl;
  obj8[1] = intl4.string(markAsDismissed(1236).t.iSrIIZ);
  obj8[2] = callback;
  items5[1] = callback(markAsDismissed(4815).Button, obj8);
  obj6[1] = items5;
  items4[1] = callback2(closure_5, obj6);
  obj[2] = items4;
  return callback2(markAsDismissed(5503).BottomSheet, obj);
};