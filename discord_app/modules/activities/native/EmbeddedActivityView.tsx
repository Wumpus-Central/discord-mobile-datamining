...
import closure_9 from "../EmbeddedActivitiesStore.tsx";
import items3 from "../Constants.tsx";
import { OBEY_SILENT_HARDWARE_SWITCH_APP_IDS as closure_12 } from "../../applications/ApplicationConstants.tsx";
...
function useBaseActivityView(orientationLockState) {
  ...
  const layoutEffect = obj.useLayoutEffect(() => {
    showLoadingIndicator(setShowLoadingStateForLockingOrientation[11]).dispatch({
      type: "ACTIVITY_SCREEN_ORIENTATION_UPDATE",
      screenOrientation: isLandscape ? closure_1_11.LANDSCAPE : closure_1_11.PORTRAIT,
    });
  }, items);
  const items1 = [id];
  ...
  const layoutEffect2 = obj.useLayoutEffect(() => {
    if (!first1) {
      if (null == orientationLockState) {
        if (
          !showLoadingIndicator(setShowLoadingStateForLockingOrientation[12])(isLandscape, defaultOrientationLockState)
        ) {
...
const memoResult = importAllResult.memo(function EmbeddedActivityViewInner(portraitSafeAreasConfig) {
  ...
  obj1 = {
    ...
    application: tmp4,
    setOrientationLockState: tmp5(9620).setOrientationLockState,
  };
