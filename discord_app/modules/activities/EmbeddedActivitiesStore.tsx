...
import isVoiceEmbeddedActivityDefault from "isVoiceEmbeddedActivity" /* 10475 */;
import getURLForApplicationDefault from "getURLForApplication" /* 10679 */;
import getPlatformDefault from "getPlatform" /* 10691 */;
...
function updateEmbeddedActivities(content_classification) {
  ...
  if (null != tmp3) {
    const embeddedActivityLocationChannelId = application_id(4015).getEmbeddedActivityLocationChannelId(_location);
    ...
    let obj4 = map;
    const obj19 = application_id(4015);
      ...
      if (sessionId === obj3.getSessionId()) {
        ...
        if (null != tmp36) {
          if (null != obj3.getSessionId()) {
            ...
            if (id !== _location.id) {
              const embeddedActivityLocationChannelId2 = tmp79(4015).getEmbeddedActivityLocationChannelId(_location);
    ...
    const someResult1 = mapped.some((userId) => application_id(13538).isActivityParticipantCurrentUserCurrentSession(userId));
  }
}
    ...
    if ("all" !== arg0) {
      if ("visible" === arg0) {
        ...
        if (cachedVisible == null) {
          items = self.items;
          const found = items.filter((contentClassification) => !callback(10730).isAgeRestrictedClassificationReference(contentClassification.contentClassification));
        ...
        if (cachedHidden == null) {
          items1 = self.items;
          const found1 = items1.filter((contentClassification) => callback(10730).isAgeRestrictedClassificationReference(contentClassification.contentClassification));
