...
const reduced = keys.reduce((acc, item, index) => {
  ...
  obj[item] = obj[item].map((item, index) => {
    [tmp] = item;
    return tmp;
  });
  return obj;
...
const reduced1 = keys1.reduce((acc, item, index) => {
  obj = {};
  const merged = Object.assign(acc);
  obj[item] = obj[item].map((item, index) => {
    [, tmp] = item;
...
const result = obj132.fileFinishedImporting("modules/poggermode/PoggermodeConstants.tsx");
