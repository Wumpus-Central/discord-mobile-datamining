...
function ConnectedNotificationSettings(guildId) {
  ...
  obj = {
    ...
    channels: memo,
    navigation,
    shouldUseNewNotificationSystem: guildId(10179).useShouldUseNewNotificationSystem("NotificationSettingsModalNative"),
  };
  const merged = Object.assign(tmp3);
  const items2 = [closure_26(NotificationSettings, obj), closure_26(guildId(7054).NavScrim, {})];
  obj.children = items2;
  ...
  constructor() {
    ...
    applyArgumentsResult.handleMutePress = function handleMutePress() {
      ...
      if (muted) {
        ...
        obj = { guildId };
        tmpResult.openLazy(asyncRequireImpl(10176, dependencyMap.paths), "muteSettings" + guildId, obj);
...
prototype["renderMuteSection"] = function renderMuteSection() {
  ...
  if (props.shouldUseNewNotificationSystem) {
    ...
    obj1.children = dependencyMap(tmp7(5619).TableRow, obj2);
    const items = [dependencyMap(tmp7(5701).TableRowGroup, obj1, "mute")];
    let tmp10Result = null;
    if (muted) {
      const obj3 = { muteConfig, type: tmp7(10178).MuteSettingType.SERVER };
