// === Module 17137: ConnectedNotificationSettings ===
...
import Form2 from "Form" /* 8118 */;
import TableRadioRow from "TableRadioRow" /* 8135 */;
import context from "context" /* 8136 */;
import MutedUntilTextDefault from "MutedUntilText" /* 9978 */;
import NotificationSettingsMuteBanner from "NotificationSettingsMuteBanner" /* 9983 */;
import NotificationSettingsPresets from "NotificationSettingsPresets" /* 9984 */;
import NotificationSettingsMessageNotification from "NotificationSettingsMessageNotification" /* 9988 */;
import NotificationSettingsMessageUnread from "NotificationSettingsMessageUnread" /* 9995 */;
import closure_4 from "_objectWithoutProperties" /* 109 */;
import importAllResult from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import { isGuildReadableType } from "createChannelRecord" /* 1395 */;
import closure_8 from "ensureGuildLoaded" /* 1391 */;
import closure_9 from "setIndex" /* 5360 */;
import closure_10 from "handleInviteData" /* 4300 */;
import closure_11 from "createGuildRecordFromRust" /* 1910 */;
import closure_12 from "FormStates" /* 16852 */;
...
function ConnectedNotificationSettings(guildId) {
  ...
  obj = { children: null };
  obj = { guildId, channels: memo, navigation, shouldUseNewNotificationSystem: guildId(9979).useShouldUseNewNotificationSystem("NotificationSettingsModalNative") };
  const merged = Object.assign(tmp3);
  const items2 = [callback2(NotificationSettings, obj), callback2(guildId(5820).NavScrim, {})];
  obj[0] = items2;
...
prototype["renderMuteSection"] = function renderMuteSection() {
  ...
  if (props.shouldUseNewNotificationSystem) {
    ...
    obj2[2] = !muted;
    obj1[2] = callback2(tmp7(5480).TableRow, obj2);
    const items = [callback2(tmp7(5873).TableRowGroup, obj1, "mute"), ];
    let tmp10Result = null;
    if (muted) {
      const obj3 = { muteConfig: null, type: null };
      obj3[0] = muteConfig;
      obj3[1] = tmp7(9978).MuteSettingType.SERVER;
...
prototype["renderChannels"] = function renderChannels() {
  ...
  obj = { IconComponent: self(12309).PlusMediumIcon };
  obj[0] = callback2(self(5486).TableRowIcon, obj);
  const intl2 = self(1236).intl;
  obj[1] = intl2.string(self(1236).t.quib7R);
  obj[2] = this.handleAddOverride;
  obj[2] = callback2(self(5480).TableRow, obj);
  const items = [callback2(self(5873).TableRowGroup, obj, "override-header"), ];
  ...
  items[1] = callback2(self(5873).TableRowGroup, obj1, "override-channels");
  return items;
};
prototype["renderChannel"] = function renderChannel(parent_id) {
  ...
  if (null != parent_id) {
    ...
    if (self.props.shouldUseNewNotificationSystem) {
      let tmp4Result = tmp4(4592);
      tmp4Result = tmp4(4592);
    ...
    obj[0] = _require(4866).getChannelIconComponent(parent_id);
    obj[0] = callback2(_require(5486).TableRowIcon, obj);
    const tmp4Result1 = _require(4866);
    obj[1] = _require(4574).computeChannelName(parent_id, closure_15, closure_13);
    obj[2] = function onPress() {
      return self.handleChannelSelect(parent_id.id);
    };
    let channelName = null;
    if (null != channel) {
      channelName = tmp4(4574).computeChannelName(channel, closure_15, closure_13);
    ...
    if (null != stringResult) {
      obj1 = { text: null };
      obj1[0] = stringResult;
      tmp10Result = tmp10(tmp4(5480).TableRow.TrailingText, obj1);
    }
    obj[4] = tmp10Result;
    return callback2(_require(5480).TableRow, obj, parent_id.id);
  }
};
prototype["render"] = function render() {
  ...
  if (hasItem) {
    ...
    obj[1] = tmp4(tmp5(4813).Stack, obj);
    ...
    obj[2] = items;
    obj[1] = callback3(tmp5(4813).Stack, obj);
...
export default function NotificationSettingsModal() {
  ...
  const memo = importAllResult.useMemo(() => {
    let obj = {};
    obj = { headerLeft: callback(5376).getHeaderCloseButton(callback2(5368).close), title: null, render: null };
    ...
    const obj3 = callback(5376);
    obj[1] = callback(5376).getHeaderBackButton();
    obj[2] = function render(guildId, navigation) {
      return callback2(callback(17138), { guildId: guildId.guildId, navigation });
    };
    obj[constants.ADD_OVERRIDE] = obj;
    obj1 = { headerLeft: null, title: null, render: null };
    const obj5 = callback(5376);
    obj1[0] = callback(5376).getHeaderBackButton();
    const intl3 = callback(1236).intl;
    obj1[1] = intl3.string(callback(1236).t.h850Ss);
    obj1[2] = function render(channelId) {
      return callback2(callback(9973), { channelId: channelId.channelId, inGuildContext: true });
    };
