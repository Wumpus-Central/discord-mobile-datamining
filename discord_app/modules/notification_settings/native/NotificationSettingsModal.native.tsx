...
function ConnectedNotificationSettings(guildId) {
  ...
  const items2 = [
    closure_26(NotificationSettings, {
      ...
      channels: memo,
      navigation,
      shouldUseNewNotificationSystem: guildId(10492).useShouldUseNewNotificationSystem(
        "NotificationSettingsModalNative",
      ),
    }),
    closure_26(guildId(7373).NavScrim, {}),
  ];
    ...
    if (self.props.shouldUseNewNotificationSystem) {
      ...
      stringResult = tmp4Result.presetName(
        notificationSettingsPresetUtils.presetFromSettings(
          unreadSetting,
          UserGuildSettingsStore.resolvedMessageNotifications(parent_id),
        ),
      );
...
prototype["getOverriddenChannels"] = function getOverriddenChannels() {
  ...
  const set = new Set(
    notifications_NotificationUtils.filterOverrides(props.channelOverrides, {
