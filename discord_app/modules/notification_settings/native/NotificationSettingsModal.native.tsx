...
function ConnectedNotificationSettings(guildId) {
  ...
  obj = {
    ...
    channels: memo,
    navigation,
    shouldUseNewNotificationSystem: guildId(10275).useShouldUseNewNotificationSystem("NotificationSettingsModalNative"),
  };
  const merged = Object.assign(tmp3);
  const items2 = [closure_26(NotificationSettings, obj), closure_26(guildId(7143).NavScrim, {})];
  obj.children = items2;
  ...
  constructor() {
    ...
    applyArgumentsResult.handleMutePress = function handleMutePress() {
      ...
      if (muted) {
        ...
        obj = { guildId };
        tmpResult.openLazy(asyncRequireImpl(10272, dependencyMap.paths), "muteSettings" + guildId, obj);
...
prototype["renderMuteSection"] = function renderMuteSection() {
  ...
  if (props.shouldUseNewNotificationSystem) {
    ...
    obj1.children = dependencyMap(tmp7(5686).TableRow, obj2);
    const items = [dependencyMap(tmp7(5768).TableRowGroup, obj1, "mute")];
    let tmp10Result = null;
    if (muted) {
      const obj3 = { muteConfig, type: tmp7(10274).MuteSettingType.SERVER };
  ...
  if (null != parent_id) {
    ...
    if (self.props.shouldUseNewNotificationSystem) {
      ...
      stringResult = tmp4Result.presetName(
        tmp4Result.presetFromSettings(unreadSetting, UserGuildSettingsStore.resolvedMessageNotifications(parent_id)),
      );
    }
