...
const require = fn;
const result = require("obj132").fileFinishedImporting("modules/forwarding/native/ForwardMessageFooter.tsx");
...
export const ForwardMessageFooter = function ForwardMessageFooter(message) {
  ...
  const tmp5 = first(React.useState(message(trackForwardEditContextMessageOnce[6]).useStateFromStoresObject(items, () => closure_1_5.getDraft(message.channel_id, DraftType.ForwardContextMessage))), 2);
  first = tmp5[0];
  React = tmp6;
  ...
  const effect = React.useEffect(() => {
    onSend(trackForwardEditContextMessageOnce[7]).saveDraft(message.channel_id, first, DraftType.ForwardContextMessage);
  }, items2);
  ...
  const callback1 = React.useCallback(() => {
    onSend(trackForwardEditContextMessageOnce[7]).clearDraft(message.channel_id, DraftType.ForwardContextMessage);
    onSend(first);
  }, items3);
  ...
  obj[2] = jsx(onSend(trackForwardEditContextMessageOnce[13]), { inputRef: textInputRef, text: first, onChange: callback, onSelectionChange: handleSelectionChange, onFocus: handleMessageFocus, onBlur: handleMessageBlur, onPressEmoji: handlePressEmoji, onSend: callback1 });
