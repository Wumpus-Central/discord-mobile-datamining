...
export const ForwardMessageFooter = function ForwardMessageFooter(message) {
  ...
  const tmp5 = first(
    React.useState(
      message(trackForwardEditContextMessageOnce[6]).useStateFromStoresObject(items, () =>
        closure_1_5.getDraft(message.channel_id, closure_1_6.ForwardContextMessage),
      ),
    ),
    2,
  );
  ...
  const effect = React.useEffect(() => {
    onSend(trackForwardEditContextMessageOnce[7]).saveDraft(
      message.channel_id,
      first,
      closure_1_6.ForwardContextMessage,
    );
  }, items2);
  ...
  const tmp13 = onSend;
  obj[0] = jsx(message(trackForwardEditContextMessageOnce[11]).ForwardPreview, {
    message,
  ...
  obj1[4] = tmp15;
  obj1[5] = isSending;
  obj[1] = jsx(message(trackForwardEditContextMessageOnce[12]).Button, {
    variant: "primary",
    size: "md",
    text: sendLabel,
    disabled: !canSend,
  ...
  obj[2] = jsx(tmp13(trackForwardEditContextMessageOnce[13]), {
