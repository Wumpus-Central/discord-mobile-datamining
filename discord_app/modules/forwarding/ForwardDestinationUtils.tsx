...
export const useSelectedDestinationChannel = function useSelectedDestinationChannel(selectedDestinations) {
  const mapped = selectedDestinations.map(found(6162).getChannelIdFromDestinationId);
...
export const getDestinationIsUnavailable = function getDestinationIsUnavailable(components, channel, type) {
  ...
  if (null != components) {
    ...
    if (null != channel) {
      let obj = _require(4407);
      if (obj.isChannelOrGuildNSFW(channel)) {
        if (tmp) {
          let tmp11Result = tmp11(4407);
        }
      ...
      if (tmp) {
        if (callback(type.type)) {
          ...
          if (tmp5) {
            ...
            tmp11Result2 = tmp11(4526);
          }
          const items = [];
          const messageSnapshots3 = components.messageSnapshots;
          let arraySpreadResult = HermesBuiltin.arraySpread(tmp11(4528).getMessageStickers(components), 0);
...
export const isRatelimitedInChannel = function isRatelimitedInChannel(channel, outer1_6) {
  ...
  if (tmp) {
    tmp = !require(5976) /* canBypassSlowmodeHelper */.canBypassSlowmodeHelper(channel, outer1_6);
    const obj = require(5976) /* canBypassSlowmodeHelper */;
  }
  return tmp;
};
    ...
    const mapped = lib.map((id) => {
      ...
      if ("user" === id.type) {
        ...
        if (null != user) {
          ...
          if (nickname == null) {
            nickname = callback2(4032).getName(user);
            const obj2 = callback2(4032);
          }
          tmp13 = nickname;
        }
        return tmp13;
