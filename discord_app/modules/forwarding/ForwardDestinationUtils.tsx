...
export const useSelectedDestinationChannel = function useSelectedDestinationChannel(selectedDestinations) {
  const mapped = selectedDestinations.map(found(7200).getChannelIdFromDestinationId);
      ...
      if (tmp) {
        if (callback(type.type)) {
          ...
          if (tmp5) {
            ...
            tmp11Result2 = tmp11(4587);
          }
          const items = [];
          const messageSnapshots3 = components.messageSnapshots;
          let arraySpreadResult = HermesBuiltin.arraySpread(tmp11(4589).getMessageStickers(components), 0);
