...
function ReactionNotificationBody(arg0) {
  ...
  if (null != secondaryText) {
    ...
    tmp8Result = closure_1_10(Text_Text.Text, obj3);
  }
  ...
  if (hasMessageContent) {
    ...
    if (null != messagePreview) {
      ...
      tmp8Result2 = closure_1_10(ChannelRowPreview.NativeChannelRowPreview, obj4);
    }
  }
    ...
    if (null == name) {
      ...
      if (null != name1) {
        ...
        if (null == id) {
          ...
          return closure_2_10(Text_Text.Text, obj2, reaction.emoji.name);
        }
      }
      let id1;
      if (reaction != null) {
      ...
      let emojiURL;
      if (null != id1) {
        const obj4 = { id: reaction.emoji.id, animated: null, size: null };
    ...
    if (0 !== previewableMedia.length) {
      if (!closure_2) {
        if (1 === previewableMedia.length) {
          ...
          const everyResult = previewableMedia.every(
            (type) => type.type === message(closure_1_2[11]).PreviewableMediaTypes.FILE,
          );
...
export default function ReactionNotification(notification) {
  ...
  const memo = parentChannel.useMemo(
    () =>
      _mod12.sumBy(message.reactions, (count_details) => {
  ...
  const callback1 = obj2.useCallback(
    () => ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(10272, dependencyMap.paths), { channelId: id }),
    items4,
  );
  let obj4 = {
    ...
    notification,
    rightAccessory: closure_10(notification(guild[34]).MediaPreviewRightAccessory, { message }),
    children: null,
  };
  const tmp3Result2 = notification(guild[27]);
  obj4.children = closure_10(id1, {
    style: tmp.container,
    children: closure_10(ReactionNotificationBodyWrapper, {
      message,
