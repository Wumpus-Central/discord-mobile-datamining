...
import PlayIcon from "PlayIcon" /* 8546 */;
import VideoComponentDefault from "VideoComponent" /* 8570 */;
import VoiceMessageIcon from "VoiceMessageIcon" /* 10173 */;
import useEnabledHarmTypesBitmaskForChannelAndAuthorId from "useEnabledHarmTypesBitmaskForChannelAndAuthorId" /* 10221 */;
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(4998).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj2 = { style: null, children: null };
    obj2[0] = tmp.spoilerPill;
    obj2[1] = tmp14(tmp2(7711).EyeIcon, { size: "sm", color: "white" });
...
function SinglePreviewableMedia(arg0) {
  ...
  if (VoiceMessageIcon.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10173).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10173).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10173).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10173).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10173).PreviewableMediaTypes.GIF === type) {
            ...
            } else if (tmp2(10173).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8002).getAttachmentObscurityProps(obj12);
                const tmp2Result1 = tmp2(8002);
                const attachmentUrl = tmp2(8536).getAttachmentUrl(media);
                const tmp2Result2 = tmp2(8536);
                ...
                obj13[4] = 2 * size;
                let str3;
                if (type === tmp2(10173).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj13[6] = str3;
...
function MultiplePreviewableMedia(arg0) {
  ...
  const memo = React.useMemo(() => {
    ...
    const obj = { shape: callback(8066).CutoutShape.RoundedRect, x: 56 - roundToNearestPixelResult + sum, y: -sum, width: roundToNearestPixelResult, height: roundToNearestPixelResult, cornerRadius: closure_4.roundToNearestPixel(roundToNearestPixelResult / 2) };
    return obj;
  }, []);
