...
import { NOTIFICATION_PREVIEW_LINE_CLAMP as closure_5 } from "set" /* 10098 */;
...
const memoResult = importAllResult.memo(function RestrictedHoursWarningNotification(notification) {
  ...
  obj[1] = jsx(notification(10877).ThemeDarkIcon, { size: "sm", color: type(712).colors.WHITE });
  ...
  obj = { icon: <View size="sm" color={type(712).colors.WHITE} />, header: memo, children: jsx(notification(4441).Text, obj1), onPress: callback, notification };
  return jsx(notification(10175).NotificationPressable, { icon: <View size="sm" color={type(712).colors.WHITE} />, header: memo, children: jsx(notification(4441).Text, obj1), onPress: callback, notification });
});
