...
let size = {
  shape: fn(8884).CutoutShape.RoundedRect,
  x: fn(5634).ImageSizes[fn(undefined, 5634).GuildIconSizes.NORMAL] - -6 - 24,
  y: -6,
  width: 24,
  height: 24,
...
export default function AlertNotification(notification) {
  ...
  obj = { children: null };
  const obj1 = { cutouts: null, children: closure_5(guild(5634), { guild, selected: false }) };
  const items2 = [size];
  obj1.cutouts = items2;
  const items3 = [closure_5(guild(8884), obj1)];
  const obj2 = { style: tmp.warningBadge, children: null };
  const tmp8 = guild(8884);
  obj2.children = closure_5(channel(8658).WarningIcon, {
  ...
  obj.header = memo;
  obj.children = closure_5(channel(10177).SystemMessageText, { text: stringResult });
  obj.onPress = callback;
  obj.notification = notification;
  return closure_5(channel(10240).NotificationPressable, obj);
}
