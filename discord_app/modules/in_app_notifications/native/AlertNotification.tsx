...
export default function AlertNotification(notification) {
  ...
  items2 = [obj];
  obj1[1] = callback(guild(5819), { guild, selected: false });
  const items3 = [callback(guild(8341), obj1), ];
  ...
  obj2[1] = callback(channel(8007).WarningIcon, obj3);
  items3[1] = callback(View, obj2);
  obj[0] = items3;
  obj[0] = callback2(closure_6, obj);
  obj[1] = memo;
  obj[2] = callback(channel(9837).SystemMessageText, { text: stringResult });
  obj[3] = callback;
  obj[4] = notification;
  return callback(channel(9896).NotificationPressable, obj);
};