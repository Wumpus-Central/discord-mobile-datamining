...
const createCacheKey = { height: require("Button").AVATAR_SIZE_MAP[XSMALL] };
...
let result = require("obj132").fileFinishedImporting("modules/voice_calls/native/action_sheet/VoiceMemberEmbeddedActivity.tsx");
...
export default function VoiceMemberEmbeddedActivity(onItemPress) {
  ...
  const mapped = Array.from(embeddedActivity.userIds).map((item, index) => user.getUser(item));
  ...
  const stateFromStores = obj2.useStateFromStores(items2, () => {
    ...
    if (null != closure_3) {
      ...
      found = embeddedActivitiesForChannel.find((item, index) => {
        let id;
        if (id != null) {
          id = id.id;
        }
  ...
  if (null != first) {
    if (null != stateFromStores) {
      ...
      obj[0] = name;
      obj[1] = intl.formatToPlainString(channelId(tmp3[21]).t.Yw5Hr2, obj);
      obj[2] = closure_13;
      ...
      obj4[1] = iconSource;
      const items5 = [callback(stateFromStores, obj4), , ];
      ...
      obj6[3] = name;
      obj5[1] = callback(channelId(tmp3[22]).Text, obj6);
      items5[1] = callback(embeddedActivityLocationGuildId, obj5);
      ...
      obj7[1] = items6;
      obj7[2] = channelId(tmp3[9]).OverflowText;
      obj7[3] = found;
      obj7[5] = function renderItem(user) {
        let obj = { user, guildId: closure_5, size: XSMALL, cutout: null };
        let tmp5;
        if (!arg1) {
          obj = { radius: null, direction: null, inset: -6 };
          obj[0] = channelId(first[9]).AVATAR_SIZE_MAP[tmp4] / 2 + 3;
          obj[1] = channelId(first[9]).CutoutDirection.RIGHT;
          tmp5 = obj;
        }
        obj[3] = tmp5;
        return closure_1_10(channelId(first[9]).CutoutableAvatarImage, obj);
      };
      items5[2] = callback(channelId(tmp3[9]).SummarizedIconRow, obj7);
      obj3[1] = items5;
      const items7 = [callback2(embeddedActivityLocationGuildId, obj3), ];
      ...
      obj11[2] = result;
      obj10[1] = obj11;
      const items9 = [callback(tmp2(tmp3[23]), obj10), ];
      ...
      if (embeddedActivityJoinability === channelId(tmp3[16]).EmbeddedActivityJoinability.CAN_JOIN) {
        ...
        obj13[3] = intl2.string(channelId(tmp3[21]).t["4i2vj+"]);
        tmp16Result = callback(channelId(tmp3[24]).BaseTextButton, obj13);
      }
      obj12[1] = tmp16Result;
      items9[1] = callback(embeddedActivityLocationGuildId, obj12);
      obj8[1] = items9;
      items7[1] = callback2(embeddedActivityLocationGuildId, obj8);
      obj1[1] = items7;
      obj[4] = callback2(embeddedActivityLocationGuildId, obj1);
      return callback(channelId(tmp3[20]).PressableOpacity, obj);
    }
  }
  return null;
};
export const calculateActivityRowHeight = function calculateActivityRowHeight(closure_4) {
  return 40 + (closure_4 - 32) / c14 + 12 + 16;
};