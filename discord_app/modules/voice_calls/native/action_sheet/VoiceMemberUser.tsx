...
function StreamingUserRow(user) {
  ...
  if (null != stateFromStores) {
    ...
    obj.subLabel = closure_11(tmp3(7749).FormSubLabel, obj);
    ...
    if (tmp10Result) {
      ...
      obj1[1] = tmp10(channel(10805), obj2);
      tmp10Result = tmp10(closure_3, obj1);
      const tmp18 = closure_3;
      const tmp20 = channel(10805);
    }
...
function RingButton(channelId) {
  ...
  if (null != userId) {
    ...
    if (null != channelId) {
      ...
      obj[3] = tmp4Result;
      tmp4Result = tmp4(channelId(4797).PressableOpacity, obj);
    }
  }
  return tmp3;
}
...
let closure_16 = importAllResult.memo((user) => {
    ...
    if (!flag) {
      ...
      if (!tmp15) {
        ...
        if (tmp8Result.isThemeDark(stateFromStores)) {
          let tmp5Result = tmp5(12931);
        } else {
          tmp5Result = tmp5(12932);
        }
  ...
  obj9.label = closure_11(user(7749).FormRow.Label, obj12);
  ...
  return closure_11(user(7749).FormRow, obj9);
});
...
const memoResult = importAllResult.memo(function DisconnectedUserRow(user) {
  ...
  obj[1] = labelCallScreen;
  obj[1] = closure_11(id(7749).FormRow.Label, obj);
  ...
  if (!canRing) {
    ...
    return tmp7(tmp2(7749).FormRow, obj2);
  } else {
