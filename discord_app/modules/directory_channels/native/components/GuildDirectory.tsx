...
import useCanManageGuildDirectoryEntry from "useCanManageGuildDirectoryEntry" /* 12436 */;
...
function GuildDirectoryHeader(guild) {
  ...
  const obj3 = { variant: "primary-overlay", icon: closure_1_19(MagnifyingGlassIcon.MagnifyingGlassIcon, { size: "sm", color: "text-strong" }), text: null, onPress: null };
...
export default function GuildDirectory(channel) {
  ...
  if (null == onPressSearch) {
    ...
    if (!directoryIsFetching) {
      if (0 === allEntriesCount) {
        ...
        items9[1] = closure_19(tmp4(12883), obj2);
  ...
  const tmp2Result1 = channel(4469);
  tmp4 = guildId;
};