// === Module 12636: GuildDirectorySearch ===

// Module 12636 (GuildDirectorySearch)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 1177 */;
import AnalyticsUtilsDefault from "AnalyticsUtils" /* 1241 */;
import GuildDirectorySearchModalActionCreatorsDefault from "GuildDirectorySearchModalActionCreators" /* 12634 */;
...
import GuildDirectoryActionCreatorsAll from "GuildDirectoryActionCreators" /* 12650 */;
...
function DefaultState() {
  ...
  let items = [closure_12(closure_7, { style: tmp.emptyStateImage, source: _modDef12640 }), ];
...
function EmptyState(channel) {
  ...
  const canCreateOrAddGuildInDirectory = channel(12641).useCanCreateOrAddGuildInDirectory(channel);
  ...
  const obj2 = channel(12641);
  const items1 = [closure_12(closure_7, { style: tmp.emptyStateImage, source: _modDef12640 }), , ];
  ...
  items1[1] = closure_12(channel(4823).Text, obj6);
  items1[2] = closure_12(channel(4823).Text, { style: tmp.emptyStateText, variant: "text-sm/medium", color: "text-default", children: formatResult });
...
export default function GuildDirectorySearch(channel) {
  ...
  if (!tmp2[0]) {
    ...
    obj3.children = closure_12(tmp7(tmp5[21]), obj4);
    const items2 = [closure_12(tmp4(tmp5[20]).FauxHeader, obj3), tmp9];
    ...
    if (0 === searchResults.length) {
      if (!searchFetching) {
        const obj5 = { channel };
        let tmp8Result = closure_12(EmptyState, obj5);
      }
    }
    const obj6 = {
      data: memo,
