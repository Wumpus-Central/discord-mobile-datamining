...
export default function BuyNitroMarketing(onPaymentDismiss) {
  ...
  const callback1 = noop.useCallback((item) => {
    ...
    if ("section-header" === item.kind) {
      ...
      obj = { perk: item.perk, selectedTier: buyNitroPlanSelection.selection.tier, onOpenDetails, start: null, end: null };
  ...
  if (tmp19Result) {
    const obj5 = { bannerFields: promotionMarketingComponent.properties.properties.marketingPageBanner, componentId: null, promotionId: null, analyticsPage: "Marketing Page Banner Tile", style: null, onPaymentSuccess: null, onPaymentDismiss: null };
  ...
  const items5 = [ref(nativeStackNavigation, obj1), ref(onPaymentSuccess(onPaymentDismiss[23]), { planSelection: buyNitroPlanSelection, applicationId, onPaymentSuccess, onPaymentDismiss }), ];
  if (initialLoadCompleted) {
