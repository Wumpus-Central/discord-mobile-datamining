  ...
  const effect = applicationId.useEffect(() => {
    analyticsLoadId = function _runPurchase() {
      ...
      const tmp = asyncGeneratorStep(async () => {
        ...
        if (closure_1_14) {
          ...
          if (id != null) {
            ...
            if (checkoutContextRecord != null) {
              let obj4 = sku_id(7461);
              const availablePlanForItems = checkoutContextRecord.getAvailablePlanForItems(obj4.getSubscriptionItemsForProduct(_null.productId));
        ...
        obj6.track(constants.PAYMENT_FLOW_STEP, sku_id(10733).getPaymentFlowStepAnalyticsFields(basePurchaseFlowAnalyticsFields, { from_step: sku_id(10733).PaymentFlowStep.PLAN_SELECT, to_step: sku_id(10733).PaymentFlowStep.EXTERNAL_PAYMENT, subscription_plan_gateway_plan_id: _null.productId, sku_id }));
        obj4 = { productId: _null.productId, analyticsLocation: basePurchaseFlowAnalyticsFields.location, analyticsLoadId, analyticsLocations, orderId: null };
        if (id != null) {
          id = id.id;
        }
        obj4.orderId = id;
        ...
        if (1 === tmp7) {
          ...
          if (closure_3 instanceof basePurchaseFlowAnalyticsFields(10774)) {
            ...
            obj2 = basePurchaseFlowAnalyticsFields(4950);
    ...
    if (!ref.current) {
      if (!closure_14) {
        ...
        obj = { object: onOrderTrialUnavailable.BUTTON_CTA, objectType: onHostSheetClose.BUY, page: onPaymentSuccess.USER_SETTINGS, section: onPaymentDismiss.SETTINGS_PREMIUM };
...
export default function BuyNitroPurchaseFlow(item) {
  ...
  if (obj.isIOS()) {
    ...
    const obj1 = { paymentGateway: GOOGLE, orderRequired: GOOGLE === PaymentGateways.APPLE_ADVANCED_COMMERCE, skuIds: [], defaultPlans: memo, isGift: false, activeSubscription: null, initialSubscriptionFacet: tmp8, onOrderRetryCancellation: onExit, children: null };
    let obj2 = { item, analyticsLocations, analyticsLoadId, hasEmittedPaymentFlowStartedRef, applicationId, expectedPriceString, stagedTrialId, onExit, onOrderPriceMismatch, onOrderTrialUnavailable, onHostSheetClose, onPaymentSuccess, onPaymentDismiss };
    ...
    return jsx(NativeCheckoutStoreProviderDefault, { paymentGateway: GOOGLE, orderRequired: GOOGLE === PaymentGateways.APPLE_ADVANCED_COMMERCE, skuIds: [], defaultPlans: memo, isGift: false, activeSubscription: null, initialSubscriptionFacet: tmp8, onOrderRetryCancellation: onExit, children: null });
  }
};