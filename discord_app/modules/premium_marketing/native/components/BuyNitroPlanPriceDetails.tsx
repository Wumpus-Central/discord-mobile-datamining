...
export default function BuyNitroPlanPriceDetails(centered) {
  ...
  if (null != trialTier) {
    if (item.premiumTier === trialTier) {
      ...
      const items1 = [closure_7(item(4601).Text, obj)];
      if (null == value) {
        items1[1] = tmp17;
        obj.children = items1;
        ...
        const intl4 = item(1114).intl;
        if (item.interval === constants.YEAR) {
          let v9QeON = item(1114).t.ECT4A5;
        } else {
          v9QeON = item(1114).t.v9QeON;
        }
  ...
  if (item.productId === discounted.id) {
    if (null != discounted.priceString) {
      ...
      if (tmp11Result) {
        ...
        tmp11Result = closure_7(item(4601).Text, obj7);
      }
