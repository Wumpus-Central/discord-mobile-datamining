...
function UserCalloutAvatars(userIds) {
  ...
  const stateFromStoresArray = obj.useStateFromStoresArray(items, () => userIds.map((item) => user.getUser(item)), items1);
  ...
  if (1 === userIds.length) {
    ...
    return closure_11(tmp(10942).FacepileGroupDMAvatar, obj);
  }
}
...
export default function BlockedUserInGdmActionSheet(channelId) {
  ...
  const effect = noop.useEffect(() => {
    const obj = { channel_id: channelId, warning_medium: constants2.ACTION_SHEET, ignored_user_ids: ignoredUserIds, blocked_user_ids: blockedUserIds };
