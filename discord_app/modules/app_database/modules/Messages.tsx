...
import handleSelectedChannelStoreChanged from "messages/SaveableChannelsStore.tsx";
...
const prototype = function ChannelHistory(arr) {
  ...
  if (arr.length > 0) {
    ...
    if (everyResult) {
      everyResult = arr.every((item, index) => item.connectionId === connectionId);
    }
    if (everyResult) {
    ...
    obj.users = tmp6;
    obj.members = tmp7;
    obj.messages = arr.map((item, index) => item.message);
...
prototype["computeUsersAndMembers"] = function computeUsersAndMembers(arr) {
  const self = this;
  const result = obj132(5000).requireSortedDescending(arr);
  const map = new Map();
  map1 = new Map();
...
prototype2["handleMessageUpdate"] = function handleMessageUpdate(message, database) {
  ...
  if (isReadableChannelIdResult) {
    isReadableChannelIdResult = obj132(5075).isReadableChannelId(message.message.channel_id);
    const obj = obj132(5075);
  }
  if (isReadableChannelIdResult) {
...
prototype2["insertStale"] = function insertStale(guildId, channel_id, item10009, database) {
  ...
  const KvMessage = obj132(5081).KvMessage;
  const messagesTransactionResult = itemsDefault.messagesTransaction(database);
  messagesTransactionResult.put(guildId, channel_id, KvMessage.fromMessage(guildId, channel_id, item10009, result), obj132(1956).ConflictOptions.Skip);
};
prototype2["upsertOne"] = function upsertOne(guildId, channelId, message, database) {
  const messagesTransactionResult = itemsDefault.messagesTransaction(database);
  const result = closure_5.lastTimeConnectedChanged();
  const KvMessage = obj132(5081).KvMessage;
  messagesTransactionResult.put(guildId, channelId, KvMessage.fromMessage(guildId, channelId, message, result), obj132(1956).ConflictOptions.Replace);
...
prototype2["upsertMany"] = function upsertMany(guild_id, channelId, messages, database) {
  ...
  const nextResult = iter.next();
  while (iter !== undefined) {
    let KvMessage = obj132(5081).KvMessage;
...
prototype2["replaceChannel"] = function replaceChannel(arg0, channelId, arr, database) {
  ...
  messagesTransactionResult.replaceChannel(arg0, channelId, substr.map((item, index) => {
    const KvMessage = callback(table[9]).KvMessage;
    return KvMessage.fromMessage(callback, closure_1, item, table);
  }));
      ...
      try {
        ...
        if (0 === closure_2) {
          if (arg0 === 1) {
            ...
            closure_2 = undefined;
            if (null != user.id) {
              obj1 = channel_id(user[7]);
              const messagesResult = obj1.messages(database.database);
              c0 = messagesResult;
              closure_2 = 1;
              database = 1;
              obj1 = { value: null, done: false };
              obj1[0] = messagesResult.get(guildId, channel_id, user.id);
              return obj1;
            } else {
          ...
          if (null != message) {
            const KvMessage = guildId(user[9]).KvMessage;
            const obj2 = {};
...
obj132 = Object.create(Messages.prototype);
obj132.actions = {
  CHANNEL_DELETE(arg0, arg1) {
    return obj.handleChannelDelete(arg0, arg1);
  },
...
let result = obj132.fileFinishedImporting("modules/app_database/modules/Messages.tsx");
