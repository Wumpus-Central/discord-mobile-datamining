...
function LockPill(safeAreaBottom) {
  ...
  const token = safeAreaBottom(voiceMessageAnimationState[19]).useToken(
  ...
  const token1 = safeAreaBottom(voiceMessageAnimationState[19]).useToken(
  ...
  const token2 = safeAreaBottom(voiceMessageAnimationState[19]).useToken(
  ...
  const token3 = safeAreaBottom(voiceMessageAnimationState[19]).useToken(
  ...
  const token4 = safeAreaBottom(voiceMessageAnimationState[19]).useToken(
...
let closure_21 = createStyles.createStyles(() => {
  const obj = {
    ...
    contentContainer: { position: "absolute", bottom: 0, width: "100%", alignItems: "center", overflow: "hidden" },
    contentContainerFloating: { justifyContent: "flex-end", overflow: "visible" },
...
let closure_44 = noop.memo((channelId) => {
  ...
  let sharedValue;
  let ref;
  const token = channelId(sharedValue[19]).useToken(
  ...
  const effect = ref.useEffect(() => {
    ...
    const result = sharedValue.set(
      obj.withDelay(500, obj2.withTiming(1, { easing: ReanimatedRexport2.Easing.quad, duration: 250 })),
    );
  }, items);
  ...
  const effect2 = ref.useEffect(() => {
    if (channelId) {
      ...
      if (performance.now() - _undefined.current < 500) {
        const obj3 = { easing: ReanimatedRexport2.Easing.quad, duration: 250 };
        const result1 = sharedValue.set(timing.withTiming(1, obj3));
  ...
  if (!tmp7) {
    ...
    obj10.children = closure_13(voiceMessageAnimationState(sharedValue[10]).View, obj11);
    items6[2] = closure_13(closure_5, obj10);
    obj9.children = items6;
    const items7 = [closure_14(voiceMessageAnimationState(sharedValue[10]).View, obj9)];
    ...
    return closure_14(closure_15, obj16);
  }
...
const memoResult = noop.memo((opacity) => {
  ...
  let derivedValue3;
  items = [derivedValue3];
  const stateFromStores = radius(offsetThreshold[18]).useStateFromStores(
    items,
    () => derivedValue3.useReducedMotion,
    [],
  );
  ...
  obj11.children = closure_13(radius(offsetThreshold[14]).Svg, {
    children: closure_13(closure_18(), { animatedProps, opacity: opacity.opacity }),
  });
