...
let obj = {
  ...
  loading: { position: "absolute", left: 12 },
  dot: null,
  dotDismissed: null,
...
export default noop.memo((isRecording) => {
  ...
  let tmp = closure_16();
  const token = isRecording(backgroundColor[13]).useToken(
    initialAnimation(backgroundColor[9]).colors.MOBILE_VOICE_MESSAGE_RECORDING_SPINNER_COLOR,
  );
    ...
    if (!timeout) {
      ...
      timeout = setTimeout(() => {
        ...
        const result = sharedValue.set(
          obj.withTiming(1, { easing: isRecording(backgroundColor[7]).Easing.quad, duration: 200 }),
        );
      }, 1000);
      return () => {
        clearTimeout(closure_0);
