// === Module 11639: ChannelAccessInfo ===
...
let c11 = 100;
const createCacheKey = { alignItems: "center", backgroundColor: ThemesDefault.colors.BACKGROUND_SURFACE_HIGH, borderRadius: ThemesDefault.radii.sm, color: ThemesDefault.colors.TEXT_DEFAULT, flexDirection: "row", marginBottom: 8, marginTop: 8, padding: 16 };
...
let result = require("obj132").fileFinishedImporting("modules/channel_permissions/native/components/ChannelAccessInfo.tsx");
...
export default function ChannelAccessInfo(guild) {
  ...
  const existingMembers = guild(8850).getExistingMembers(memberIds, channel, guild, channel.accessPermissions);
  ...
  const items2 = [callback(guild(4734).Text, { variant: "eyebrow", children: stringResult }), ];
  ...
  if (null != first) {
    ...
    channel(11638);
    const items5 = [renderCounts(MEMBERS, existingMembers.length, 0, tmp2(6884).GroupIcon), ];
    const ROLES = constants.ROLES;
    channel(8861);
    items5[1] = renderCounts(ROLES, stateFromStoresArray.length, 0, tmp2(8859).ShieldUserIcon);
    obj6[0] = items5;
  }
  const obj7 = { children: null };
  obj[1] = callback(closure_10, obj6);
  ...
  obj[4] = items6;
  items2[1] = callback(guild(5433).PressableOpacity, obj);
  obj7[0] = items2;
  return callback(closure_10, obj7);
};