...
export default function ChannelAccessInfo(guild) {
  ...
  const existingMembers = guild(8891).getExistingMembers(memberIds, channel, guild, channel.accessPermissions);
  ...
  const items2 = [callback(guild(4251).Text, { variant: "eyebrow", children: stringResult }), ];
  obj = {
  ...
  if (null != first) {
    ...
    channel(11715);
    const items5 = [renderCounts(MEMBERS, existingMembers.length, 0, tmp2(4768).GroupIcon), ];
    const ROLES = constants.ROLES;
    channel(8902);
    items5[1] = renderCounts(ROLES, stateFromStoresArray.length, 0, tmp2(8900).ShieldUserIcon);
    obj6[0] = items5;
  }
