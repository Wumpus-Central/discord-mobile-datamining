...
function ChannelPermissionSettingsBasicView(channel) {
  ...
  closure_7 = async function _togglePrivateChannel() {
    if (dependencyMap === 2) {
      ...
      try {
        ...
        v2 = 2;
        dependencyMap = 1;
        const obj3 = { value: tmp2(9869).savePermissionUpdates(closure_129_0.id, items), done: false };
        return obj3;
      } catch (tmp16) {
        dependencyMap = tmp;
        throw tmp16;
          ...
          if (arg0 === 1) {
            ...
            if (null != guild_id.guild_id) {
              if (!privateToggleState) {
                ...
                obj1.value = obj1.checkChattableChannelThresholdMetAfterChannelPermissionDeny(
                  guild_id,
  ...
  if (null != guild) {
    if (null != sortedGuildRoles) {
      ...
      const existingMembersRows = obj3.getExistingMembersRows(
        memberIds,
        channel,
        guild,
