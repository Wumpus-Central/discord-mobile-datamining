...
function getMemberRowData(id, id2, appChannelBotUserId) {
  if (appChannelBotUserId === id.id) {
    ...
    MEMBER = RowType.OWNER;
    tmp3 = RowType;
  } else {
    MEMBER = RowType.MEMBER;
    tmp3 = RowType;
  }
  const obj = {
    rowType: MEMBER,
...
export const getRolesRows = function getRolesRows(guild, stateFromStores, channel, permission) {
    ...
    if (tmp) {
      ...
      if (null != id2) {
        if (null != permissionOverwrites) {
          let tmp8 = null == tmp7;
          if (!tmp8) {
            tmp8 = !BigFlagUtilsAll.has(tmp7.allow, closure_1);
          }
          flag = tmp8;
        }
      }
      ...
      if (null != currentUser) {
        if (null == permissionOverwrites) {
          ...
          if (dependencyMap != null) {
            tmp9 = dependencyMap[id];
          }
          if (null != tmp9) {
            tmp8 = dependencyMap[id];
          }
          let tmp10 = null == tmp8;
          if (!tmp10) {
            tmp10 = !BigFlagUtilsAll.has(tmp8.allow, closure_1);
          }
          flag = tmp10;
        }
      }
...
export const getAllExistingRolesWithPermission = function getAllExistingRolesWithPermission(
  arg0,
  arr,
  arg2,
  arg3,
  ...
  return arr.filter((id) => {
    ...
    if (!hasItem) {
      ...
      let flag = false;
      if (null != currentUser) {
        if (null == MODERATE_STAGE_CHANNEL_PERMISSIONS) {
          flag = currentUser.id !== id;
        } else {
          let tmp8 = MODERATE_STAGE_CHANNEL_PERMISSIONS.permissionOverwrites[id];
          let tmp9;
          if (dependencyMap != null) {
            tmp9 = dependencyMap[id];
          }
          ...
          let tmp10 = null == tmp8;
          if (!tmp10) {
            tmp10 = !BigFlagUtilsAll.has(tmp8.allow, importDefault);
          }
          flag = tmp10;
        }
      }
...
export const getExistingRolesRows = function getExistingRolesRows(
  guild,
  sortedGuildRoles,
  channel,
  accessPermissions,
    ...
    if (tmp2) {
      ...
      if (null != id2) {
        if (null != permissionOverwrites) {
          let tmp9 = null == tmp8;
          if (!tmp9) {
            tmp9 = !BigFlagUtilsAll.has(tmp8.allow, closure_2);
          }
          flag = tmp9;
        }
      }
  ...
  return found.filter((id) => {
    ...
    if (null != currentUser) {
      if (null == guild) {
        ...
        let tmp5 = guild.permissionOverwrites[id];
        let tmp6;
        if (dependencyMap != null) {
          tmp6 = dependencyMap[id];
        }
        if (null != tmp6) {
...
export const getExistingMembersRows = function getExistingMembersRows(
  memberIds,
  channel,
  guild,
  accessPermissions,
...
export const toggleChannelEveryonePermission = function toggleChannelEveryonePermission(
  guild_id,
  MODERATE_STAGE_CHANNEL_PERMISSIONS,
  arg2,
) {
...
export const grantUserChannelAccess = function grantUserChannelAccess(id, accessPermissions) {
  ...
  if (null != currentUser) {
    if (null == id.permissionOverwrites[currentUser.id]) {
      let obj = {
        id: currentUser.id,
        type: Server.PermissionOverwriteType.MEMBER,
        allow: BigFlagUtilsAll.add(PermissionUtilsAll.NONE, accessPermissions),
  ...
  if (first1 !== constants2.GUILD_TEXT) {
    ...
    if (first1 !== constants2.GUILD_ANNOUNCEMENT) {
      ...
      if (first1 !== constants2.GUILD_APP) {
        ...
        let tmp5 = !tmp4;
        if (!tmp4) {
          tmp5 = first1 !== constants2.GUILD_CATEGORY;
        }
        if (!tmp5) {
          let tmp6 = canResult;
          if (canResult) {
