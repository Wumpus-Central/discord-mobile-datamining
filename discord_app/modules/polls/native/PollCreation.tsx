...
import PollsIcon from "PollsIcon" /* 10980 */;
import PollCreationModalActionCreators from "PollCreationModalActionCreators" /* 12530 */;
import _modDef12532 from "module_12532" /* 12532 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12544 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12558 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12532, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12532, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12533)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12534, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4823).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5908).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7312)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(9124);
  let canUseScheduledMessages = channel(8173).useCanUseScheduledMessages();
  ...
  let obj2 = channel(8173);
  closure_18 = channel(12540).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12540);
  channel(5933).useNavigatorBackPressHandler(() => {
  ...
  obj9.source = onCancel(5984);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7705).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4823).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12542).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7705).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7705).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12556), obj15);
    let tmpResult = tmp(12556);
  }
  ...
  const items7 = [
    c9(channel(8947).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5907).TableCheckboxRow, obj22);
