...
import registerAssetDefault from "../../../../_runtime/11766_registerAsset.js";
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(11767)()[selectedDuration];
  let obj = {
    label: null,
    accessibilityLabel: null,
  ...
  obj[3] = function onPress() {
    ...
    onChange(closure_1_2[14]).openLazy(
      selectedDuration(closure_1_2[16])(closure_1_2[15], closure_1_2.paths),
      closure_1_8,
      obj,
    );
  };
  obj[4] = callback(selectedDuration(4474).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return callback(selectedDuration(5599).TableRow, obj);
}
...
let obj = {
  ...
  safeAreaContainer: null,
  header: null,
  actionButton: null,
  postButton: null,
  title: null,
  label: null,
  answerInputsContainer: null,
...
createCacheKey = {
  ...
  backgroundColor: ThemesDefault.colors.MESSAGE_BACKGROUND_HOVER,
...
export default function PollCreation(channel) {
  ...
  function handleCancelClose() {
    callback();
    channel(11764).closeCreatePollModal();
  ...
  const insets = onCancel(5594)({ includeKeyboardHeight: true }).insets;
  obj1 = channel(7589);
  ...
  const callback1 = answers.useCallback((indexToRemove) => {
    ...
    AccessibilityAnnouncer.announce(
      intl.formatToPlainString(channel(1236).t.BByGU4, { number: indexToRemove.indexToRemove + 1 }),
    );
  }, []);
  const tmp11 = onCancel(11769)(channel, callback, callback1);
  answers = tmp11.answers;
  ({ question: c4, allowMultiSelect } = tmp11);
  ({
    setAllowMultiSelect: c6,
    canAddMoreAnswers,
    canRemoveMoreAnswers: c7,
    handleAnswerTextChange: c8,
    handleEmojiSelect: c9,
    handleAddAnswer,
  } = tmp11);
  ...
  obj2[2] = onCancel(5552);
  obj2[3] = tmp7.actionButton;
  const items3 = [callback(channel(6237).HeaderActionButton, obj2), , ,];
  obj3 = {
  ...
  if (canUseScheduledMessages) {
    ...
    obj4[0] = intl3.string(tmp4(1236).t.rlf0tb);
    ...
    obj5 = { color: null };
    obj5[0] = TEXT_BRAND;
    obj4[3] = tmp14(tmp4(11776).CalendarPlusIcon, obj5);
    obj4[4] = function onPress() {
      let obj = channel(11778);
      ...
      obj[2] = fn;
      obj[3] = channel(7591).ScheduledMessageEntryPoint.POLL_CREATION;
      obj[4] = channel.id;
      return obj.pickScheduledMessageTime(obj);
    };
    canUseScheduledMessages = tmp14(tmp4(6237).HeaderActionButton, obj4);
  }
  items3[2] = canUseScheduledMessages;
  const obj6 = { text: null, style: null, disabled: null, onPress: null };
  const intl4 = tmp4(1236).intl;
  obj6[0] = intl4.string(channel(1236).t.JOj8Zk);
  const items4 = [,];
  ({ actionButton: arr6[0], postButton: arr6[1] } = tmp7);
  obj6[1] = items4;
  obj6[2] = submitting;
  obj6[3] = handleSubmitPoll;
  items3[3] = callback(channel(6237).HeaderActionButton, obj6);
  obj1[1] = items3;
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(11790), obj8);
    let tmpResult = tmp(11790);
  }
  const items6 = [tmp14Result, ,];
  const obj9 = {
    ref: obj5.refWithKey("question"),
    onChange: handleQuestionChange,
    onSubmitEditing: obj5.focusNext,
    error: null,
  };
  let question;
  if (fieldErrors != null) {
  ...
  obj11[0] = intl5.string(channel(1236).t.oMBfeS);
  obj11[2] = tmp7.label;
  const items7 = [
    callback(channel(8363).FormLabel, obj11),
    answers.map((localCreationAnswerId) => {
      const obj = {
        inputRef: obj5.refWithKey("answer-" + localCreationAnswerId.localCreationAnswerId),
        answer: localCreationAnswerId,
        index: arg1,
        channelId: channel.id,
        onSubmitEditing: obj5.focusNext,
        onAnswerTextChange: c8,
        onAnswerEmojiSelect: c9,
        onRemoveAnswer: c11,
        onRemoveAnswerImage: c12,
        canRemoveAnswer: c7,
        error: null,
      };
      let tmp4;
      ...
      obj[10] = tmp4;
      return c9(onCancel(11792), obj, localCreationAnswerId.localCreationAnswerId);
    }),
  ];
  ...
  obj15[0] = intl6.string(channel(1236).t["Ux+iQU"]);
  obj15[1] = allowMultiSelect;
  obj15[2] = function onPress() {
    return _undefined(!allowMultiSelect);
  };
  items8[1] = callback(channel(7683).TableCheckboxRow, obj15);
  obj14[1] = items8;
