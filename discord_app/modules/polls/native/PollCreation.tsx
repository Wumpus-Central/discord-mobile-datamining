...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(11388)()[selectedDuration];
  ...
  obj[4] = callback(selectedDuration(4189).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return callback(selectedDuration(5224).TableRow, obj);
}
...
let closure_14 = importAllResult.forwardRef((error, ref) => {
  ...
  return closure_9(require(6730) /* TextInput */.TextInput, obj);
});
...
export default function PollCreation(channel) {
  ...
  function handleCancelClose() {
    callback();
    channel(11385).closeCreatePollModal();
    const AccessibilityAnnouncer = channel(3906).AccessibilityAnnouncer;
  ...
  const insets = onCancel(5219)({ includeKeyboardHeight: true }).insets;
  dependencyMap = answers.useRef(false);
  callback = answers.useCallback(() => {
    dependencyMap.current = true;
    let obj = channel(11385);
    obj.closeCreatePollModal();
    obj = { key: "POLL_CREATED_SUCCESS", IconComponent: channel(9689).PollsIcon, content: null };
    const intl = channel(1236).intl;
    obj[2] = intl.string(channel(1236).t.OPsckI);
    onCancel(3894).open(obj);
  }, []);
  ...
  const tmp10 = onCancel(11390)(channel, callback, callback1);
  ...
  obj2.useNavigatorBackPressHandler(() => {
    ...
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4534).openAlert("poll-creation-unsaved-changes", _undefined2(fieldErrors, obj));
      flag = true;
  ...
  obj2[2] = onCancel(5180);
  obj2[3] = tmp7.actionButton;
  const items3 = [callback(channel(6755).HeaderActionButton, obj2), , ];
  ...
  items3[1] = callback(channel(4189).Text, obj3);
  ...
  items3[2] = callback(channel(6755).HeaderActionButton, obj4);
  obj1[1] = items3;
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(onCancel(11397), obj6);
    let tmpResult = onCancel(11397);
  }
  ...
  obj9[2] = tmp7.label;
  const items7 = [
    callback(channel(7639).FormLabel, obj9),
    answers.map((localCreationAnswerId) => {
      ...
      obj[10] = tmp4;
      return c9(onCancel(11399), obj, localCreationAnswerId.localCreationAnswerId);
    }),
  ...
  items8[1] = callback(channel(9099).TableCheckboxRow, obj13);
  obj12[1] = items8;
