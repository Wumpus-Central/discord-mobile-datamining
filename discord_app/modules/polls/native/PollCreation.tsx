...
import PollAnswerInputDefault from "PollAnswerInput" /* 11707 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef11681, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef11681, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(11682)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11683, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
  obj.trailing = closure_9(selectedDuration(4832).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return closure_9(selectedDuration(5917).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(6402)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(8230);
  let canUseScheduledMessages = channel(7265).useCanUseScheduledMessages();
  ...
  let obj2 = channel(7265);
  closure_18 = channel(11689).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(11689);
  channel(5942).useNavigatorBackPressHandler(() => {
  ...
  obj9.source = onCancel(5993);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(6795).HeaderActionButton, obj9), , , ];
  ...
  items3[1] = c9(channel(4832).Text, obj10);
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(11691).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(6795).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(6795).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(11705), obj15);
    let tmpResult = tmp(11705);
  }
  ...
  const items7 = [
    c9(channel(8053).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5916).TableCheckboxRow, obj22);
