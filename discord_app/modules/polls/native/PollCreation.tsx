...
import { registerAsset } from "../../../../_runtime/11657_registerAsset.js";
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(11658)()[selectedDuration];
  ...
  obj[4] = callback(selectedDuration(4338).Text, { variant: "text-md/normal", color: "text-muted", children: tmp });
  return callback(selectedDuration(5435).TableRow, obj);
}
...
export default function PollCreation(channel) {
  ...
  function handleCancelClose() {
    callback();
    channel(11655).closeCreatePollModal();
  ...
  const insets = onCancel(5430)({ includeKeyboardHeight: true }).insets;
  ...
  callback = answers.useCallback(() => {
    ...
    obj = { key: "POLL_CREATED_SUCCESS", IconComponent: channel(10067).PollsIcon, content: null };
    const intl = channel(1236).intl;
  ...
  const tmp10 = onCancel(11660)(channel, callback, callback1);
  ...
  obj2.useNavigatorBackPressHandler(() => {
    ...
    if (obj.isPollCreationEmpty(c4, answers)) {
      ...
      tmp(4707).openAlert("poll-creation-unsaved-changes", _undefined2(fieldErrors, obj));
      flag = true;
  ...
  obj2[2] = onCancel(5388);
  obj2[3] = tmp7.actionButton;
  const items3 = [callback(channel(6072).HeaderActionButton, obj2), , ];
  ...
  items3[1] = callback(channel(4338).Text, obj3);
  ...
  items3[2] = callback(channel(6072).HeaderActionButton, obj4);
  obj1[1] = items3;
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(onCancel(11667), obj6);
    let tmpResult = onCancel(11667);
  }
  ...
  obj9[2] = tmp7.label;
  const items7 = [
    callback(channel(8033).FormLabel, obj9),
    answers.map((localCreationAnswerId) => {
      ...
      obj[10] = tmp4;
      return c9(onCancel(11669), obj, localCreationAnswerId.localCreationAnswerId);
    }),
  ...
  items8[1] = callback(channel(8519).TableCheckboxRow, obj13);
  obj12[1] = items8;
