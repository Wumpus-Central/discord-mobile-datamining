...
import PollCreationModalActionCreators from "PollCreationModalActionCreators" /* 12450 */;
import _modDef12452 from "module_12452" /* 12452 */;
import ScheduledMessagesUtils from "ScheduledMessagesUtils" /* 12464 */;
import PollAnswerInputDefault from "PollAnswerInput" /* 12478 */;
...
function AddAnswerButton(onPress) {
  ...
  const obj = { source: _modDef12452, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color };
  const intl = util.intl;
  obj2.accessibilityLabel = intl.string(util.t.B2Uvme);
  const items = [React7(native.Icon, { source: _modDef12452, size: native.Icon.Sizes.LARGE, color: tmp.addAnswerIcon.color }), ];
...
function PollDurationSelectionRow(selectedDuration) {
  ...
  const tmp = onChange(12453)()[selectedDuration];
  ...
  obj.onPress = function onPress() {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(12454, dependencyMap.paths), React6, { selectedDuration, onChange });
  };
...
export default function PollCreation(channel) {
  ...
  const tmp7 = c12();
  const insets = onCancel(7226)({ includeKeyboardHeight: true }).insets;
  const tmp3 = onCancel(9048);
  let canUseScheduledMessages = channel(8088).useCanUseScheduledMessages();
  ...
  const tmp11 = onCancel(12455)(channel, callback, callback1);
  ...
  let obj2 = channel(8088);
  closure_18 = channel(12460).useTrackPollCreationEvents(answers, allowMultiSelect).trackPollCreationCancelled;
  let obj3 = channel(12460);
  channel(5847).useNavigatorBackPressHandler(() => {
  ...
  obj9.source = onCancel(5898);
  obj9.style = tmp7.actionButton;
  const items3 = [c9(channel(7619).HeaderActionButton, obj9), , , ];
  ...
  if (canUseScheduledMessages) {
    ...
    obj11.icon = tmp14(tmp4(12462).CalendarPlusIcon, obj12);
    ...
    canUseScheduledMessages = tmp14(tmp4(7619).HeaderActionButton, obj11);
  }
  ...
  items3[3] = c9(channel(7619).HeaderActionButton, obj13);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(12476), obj15);
    let tmpResult = tmp(12476);
  }
  ...
  const items7 = [
    c9(channel(8871).FormLabel, obj18),
  ...
  items8[1] = c9(channel(5821).TableCheckboxRow, obj22);
