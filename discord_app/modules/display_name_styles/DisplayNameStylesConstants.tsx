...
const mapped = items4.map((item, index) => ({ colors: item, name: "" }));
...
const result = obj132.fileFinishedImporting("modules/display_name_styles/DisplayNameStylesConstants.tsx");
...
export const getColorPresetsForEffect = function getColorPresetsForEffect(closure_0) {
  if (DisplayNameEffect.DisplayNameEffect.GUMMY === closure_0) {
    return items5;
  } else if (DisplayNameEffect.DisplayNameEffect.PRISM === closure_0) {
    return items6;
  } else {
    if (DisplayNameEffect.DisplayNameEffect.GRADIENT !== closure_0) {
      if (DisplayNameEffect.DisplayNameEffect.GLOW !== closure_0) {
        return items3.map((item, index) => {
          const items = [item];
          return items;
        });
      }
    }
    return mapped.map((item, index) => item.colors);
  }
};