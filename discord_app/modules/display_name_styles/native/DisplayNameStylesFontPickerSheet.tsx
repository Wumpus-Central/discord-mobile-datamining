...
const createCacheKey = { padding: ThemesDefault.space.PX_8, alignItems: "center" };
...
let result = require("obj132").fileFinishedImporting("modules/display_name_styles/native/DisplayNameStylesFontPickerSheet.tsx");
...
export default function DisplayNameStylesFontPickerSheet(displayName) {
  ...
  callback = first.useCallback(() => {
    ...
    const obj2 = callback(_undefined[13]);
  }, items);
  ...
  let items1 = [
    callback2(onSelectFont(4733).Stack, {
      direction: "horizontal",
      align: "center",
      justify: "center",
      spacing: 8,
      style: tmp.gridContainer,
      children: visibleFontOrder.map((item, index) => {
        closure_0 = item;
        ...
        let obj = {
          ...
          accessibilityLabel: intl.string(callback(_undefined[20])(item)),
        ...
        if (hasItem) {
          ...
          hasItem = closure_1_8(closure_5, obj);
        }
        items2[1] = hasItem;
        obj[1] = items2;
  ...
  if (tmp14Result) {
    ...
    let items2 = [callback2(onSelectFont(4330).CircleInformationIcon, { size: "sm" }), ];
    const obj8 = { variant: "text-xs/normal", color: "text-subtle", style: null, children: null };
    obj8[2] = tmp.disclaimerText;
    const intl3 = onSelectFont(1236).intl;
    obj8[3] = intl3.string(messagesProxyDefault["+O1xL2"]);
    items2[1] = callback2(onSelectFont(4734).Text, obj8);
    obj7[1] = items2;
    tmp14Result = callback(tmp12, obj7);
  }
  items1[1] = tmp14Result;
  obj5[1] = items1;
  obj4[1] = callback(closure_5, obj5);
