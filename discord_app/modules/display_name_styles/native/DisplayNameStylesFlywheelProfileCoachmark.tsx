...
function CoachmarkImage() {
  ...
  const items = [callback(require(5926) /* AccountAgeTier10LargeBadge */.DisplayNameLockeAbstractUI, { width: 160, height: 68, resizeMode: "contain" }), ];
...
export default function DisplayNameStylesFlywheelProfileCoachmark(visible) {
  ...
  const result = markAsDismissed(3901).canUsePremiumProfileCustomization(stateFromStores);
  const intl = visible(1236).intl;
  const string = intl.string;
  const tmp6 = markAsDismissed(2613);
  if (result) {
  ...
  const obj2 = markAsDismissed(3901);
  const coachmark = visible(8745).useCoachmark(visible.targetRef, memo);
  return null;
};