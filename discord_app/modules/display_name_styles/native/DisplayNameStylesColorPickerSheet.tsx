...
let obj2 = {
  ...
  container: { flex: 1 },
  contentContainer: { paddingHorizontal: nativeDefault.space.PX_16, paddingBottom: nativeDefault.space.PX_16 },
  presetGrid: null,
  presetRow: null,
  presetColor: null,
...
export default function DisplayNameStylesColorPickerSheet(selectedColor) {
    ...
    showCustomColorPickerActionSheetDefault({
      ...
      onSelect(arg0) {
        const result = selectedColor(onSelectColor[10]).triggerHapticFeedback(
          selectedColor(onSelectColor[10]).HapticFeedbackTypes.IMPACT_MEDIUM,
        );
        closure_1_2(arg0);
  ...
  const tmp4 = selectedEffectId;
  const obj10 = selectedEffectId(onSelectColor[19]);
  obj9.children = selectedEffectId(onSelectColor[19])
    .chunk(memo.slice(0, 12), 6)
    .map((arr, index) => {
      closure_0 = index;
      return closure_1_9(
        first,
  ...
  if (tmp12Result) {
    ...
    obj12.children = closure_9(tmp2(tmp3[18]).Button, obj13);
    tmp12Result = closure_9(tmp13, obj12);
  }
