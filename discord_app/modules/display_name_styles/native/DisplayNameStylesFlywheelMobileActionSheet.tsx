// === Module 16725: DisplayNameStylesFlywheelMobileActionSheet ===
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = noop.useContext(markAsDismissed(4547).AccessibilityPreferencesContext).reducedMotion.enabled;
  const isThemeDarkResult = markAsDismissed(4682).isThemeDark(ref(4763)());
  const tmp6 = closure_11();
  const obj2 = markAsDismissed(4682);
  const typeConsolidationTextTransform = markAsDismissed(6395).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  const obj3 = markAsDismissed(6395);
  ...
  let result = ref(4485).canUsePremiumProfileCustomization(stateFromStores);
  ...
  const callback = noop.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(6454).runAfterInteractions(() => {
        markAsDismissed(6795).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4651).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(2028).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  ...
  const items4 = [
    closure_9(markAsDismissed(6570).ActionSheetHeaderBar, {
      onPress() {
  ...
  if (enabled) {
    tmp15Result = closure_9(tmp2(16726).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    items4[2] = closure_9(tmp2(4825).Text, obj10);
    ...
    items4[3] = closure_9(tmp2(4825).Text, obj11);
    ...
    const items7 = [closure_9(tmp2(5274).Button, obj14), ];
    ...
    items7[1] = closure_9(tmp2(5274).Button, obj15);
    obj13.children = items7;
    items4[4] = closure_10(View, obj13);
    obj7.children = items4;
    obj12.children = closure_10(View, obj7);
    obj6.children = closure_9(tmp2(6539).SafeAreaPaddingView, obj12);
    return closure_9(tmp2(6566).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
      const obj16 = { source: null, style: null, resizeMode: "contain", enableAnimation: null };
      const obj17 = { uri: tmp4(16728) };
      obj16.source = obj17;
      obj16.style = tmp6.image;
      obj16.enableAnimation = !enabled;
      let tmp15Result2 = closure_9(tmp4(5894), obj16);
      const tmp4Result = tmp4(5894);
    } else {
      const obj18 = { url: tmp4(16728), style: tmp6.image, autoplay: true };
      tmp15Result2 = closure_9(tmp2(8263).APNGPlayer, obj18);
    }
