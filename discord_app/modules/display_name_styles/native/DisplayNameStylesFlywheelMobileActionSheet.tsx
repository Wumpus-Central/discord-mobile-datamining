...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  const enabled = React.useContext(markAsDismissed(4003).AccessibilityPreferencesContext).reducedMotion.enabled;
  let obj1 = markAsDismissed(4130);
  ...
  const items4 = [callback(markAsDismissed(5633).ActionSheetHeaderBar, obj1), , , , , ];
  obj2 = { style: tmp6.imageContainer, children: null };
  let tmp14Result = enabled;
  if (enabled) {
    tmp14Result = tmp14(tmp2(5948).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    obj[4] = tmp14(tmp2(5311).SafeAreaPaddingView, obj6);
    return tmp14(tmp2(5377).BottomSheet, obj);
  } else {
    if (tmp2Result.isIOS()) {
      ...
      obj10[1] = tmp6.image;
      obj10[3] = !enabled;
      tmp14Result = tmp14(tmp4(5251), obj10);
      const tmp4Result = tmp4(5251);
    } else {
