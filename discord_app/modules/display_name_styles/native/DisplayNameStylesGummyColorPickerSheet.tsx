...
import set from "../DisplayNameStylesUtils.tsx";
...
export default function DisplayNameStylesGummyColorPickerSheet(selectedColors) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  let obj = selectedColors(8868);
  let tmp5 = selectedColors.length > 0;
  const displayNameStylesEffectConfig = obj.useDisplayNameStylesEffectConfig(selectedColors(1936).DisplayNameEffect.GUMMY);
  if (tmp5) {
  ...
  first1 = tmp9[0];
  closure_7 = tmp9[1];
  const tmp12Result = onSelectColors(14875)(selectedColors(1936).DisplayNameEffect.GUMMY);
  ...
  obj1[3] = callback2;
  obj[1] = callback2(selectedColors(4893).Button, obj1);
  obj[0] = callback2(selectedColors(5586).BottomSheetTitleHeader, obj);
  ...
  if (findIndexResult >= 0) {
    ...
    obj7[0] = first1;
    tmp18Result = tmp18(tmp11(14204), obj7);
  }
  const items4 = [tmp18Result, ];
  const obj8 = { style: tmp.customIconOverlay, pointerEvents: "none", children: null };
  tmp11 = onSelectColors;
  const tmp12 = onSelectColors(14875);
  const tmp21 = first1;
  obj8[2] = callback2(closure_5, { style: tmp.customIconScrim, children: callback2(selectedColors(10264).PencilIcon, { color: "white", size: "sm" }) });
  ...
  obj10[1] = callback1;
  items6[1] = callback2(selectedColors(4893).Button, obj10);
  obj2[1] = items6;
  obj[1] = closure_10(closure_5, obj2);
  return callback2(selectedColors(5587).BottomSheet, obj);
};