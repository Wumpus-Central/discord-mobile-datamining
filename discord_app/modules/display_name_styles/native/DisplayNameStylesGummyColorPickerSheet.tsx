...
import { DISPLAY_NAME_STYLES_GUMMY_PRESETS as closure_7 } from "../DisplayNameStylesConstants.tsx";
import { AnalyticEvents } from "../../../Constants.tsx";
import jsxProd from "../../../../_runtime/react/00021_jsxProd.js";
import items from "../DisplayNameStylesUtils.tsx";
import closure_11 from "../../../index.native.tsx";
...
items[0] = items;
const createCacheKey = { paddingHorizontal: ThemesDefault.space.PX_4, alignSelf: "stretch" };
...
let result = require("obj132").fileFinishedImporting("modules/display_name_styles/native/DisplayNameStylesGummyColorPickerSheet.tsx");
...
export default function DisplayNameStylesGummyColorPickerSheet(selectedColors) {
  ...
  if (tmp5) {
    tmp5 = !closure_7.some((item, index) => selectedColors(closure_2[11]).areArraysShallowEqual(item, selectedColors));
  }
  callback = tmp5;
  ...
  const findIndexResult = tmp12Result.findIndex((item, index) => selectedColors(closure_2[11]).areArraysShallowEqual(item.colors, first1));
  c8 = findIndexResult;
  let items = [first];
  callback = first.useCallback(() => {
    ...
    obj = {
      ...
      onSelect(closure_1_11) {
        ...
        callback2(selectedColors(closure_1_2[6]).buildGummyColors(closure_1_11));
      }
    };
    onSelectColors(closure_2[14])(obj);
  }, items);
  first.useCallback((arg0) => {
    const items = [...arg0];
    dependencyMap(items);
  }, []);
  ...
  obj5[5] = items4;
  obj4[1] = callback(tmp21, obj5);
  const items5 = [
    callback2(closure_5, obj4),
    tmp12Result.map((item, index) => {
      ...
      obj = {
        ...
        children: callback(onSelectColors(closure_2[21]), { colors })
      };
      items[1] = swatchSelected;
      obj[1] = callback(first1, obj);
      return callback(closure_5, obj, index);
    })
  ];
  obj3[1] = items5;
  const items6 = [callback(closure_5, obj3), ];
