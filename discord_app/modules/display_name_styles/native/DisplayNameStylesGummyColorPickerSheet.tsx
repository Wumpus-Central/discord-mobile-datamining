...
export default function DisplayNameStylesGummyColorPickerSheet(selectedColors) {
  ...
  first1 = tmp9[0];
  closure_7 = tmp9[1];
  const tmp12Result = onSelectColors(15170)(selectedColors(1939).DisplayNameEffect.GUMMY);
  const findIndexResult = tmp12Result.findIndex((colors) =>
    selectedColors(closure_2[11]).areArraysShallowEqual(colors.colors, first1),
  );
  c8 = findIndexResult;
  let items = [first];
  ...
  obj[0] = callback2(selectedColors(5629).BottomSheetTitleHeader, obj);
  ...
  if (findIndexResult >= 0) {
    ...
    obj7[0] = first1;
    tmp18Result = tmp18(tmp11(14494), obj7);
  }
  const items4 = [tmp18Result];
  const obj8 = { style: tmp.customIconOverlay, pointerEvents: "none", children: null };
  tmp11 = onSelectColors;
  const tmp12 = onSelectColors(15170);
  const tmp21 = first1;
  obj8[2] = callback2(closure_5, {
    style: tmp.customIconScrim,
    children: callback2(selectedColors(10349).PencilIcon, { color: "white", size: "sm" }),
  });
  ...
  return callback2(selectedColors(5630).BottomSheet, obj);
}
