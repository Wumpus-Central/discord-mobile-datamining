// === Module 7961: NotificationCenterItemsStore ===
...
function handleRelationshipAddOrUpdate(relationship) {
  ...
  if (tmp14) {
    ...
    obj.notifCenterLocalItems = prop1.filter((type) => {
      ...
      if (!tmp4) {
        let tmp7 = type.type === NotificationCenterItemsTypes.NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED;
        if (tmp7) {
      ...
      if (!tmp4) {
        let tmp10 = type.type === NotificationCenterItemsTypes.NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS;
        if (tmp10) {
      ...
      if (!tmp4) {
        let tmp13 = type.type === NotificationCenterItemsTypes.NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS_ACCEPTED;
        if (tmp13) {
...
let obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: [], paginationHasMore: true, paginationCursor: "flexDirection", notifCenterActive: "H", notifCenterTabFocused: "HH" };
...
const notificationCenterItemsStore = new NotificationCenterItemsStore(DispatcherDefault, {
  ...
  LOGOUT() {
    ...
    obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: null, paginationHasMore: true, paginationCursor: "flexDirection", notifCenterActive: "H", notifCenterTabFocused: "HH" };
    if (flag) {
