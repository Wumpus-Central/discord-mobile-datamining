...
function ForYouMessagePreviewV2(item) {
  ...
  let obj = message_channel_id(5045);
  ...
  const items5 = [item(7940).renderMessageContentMarkup(notifCenterV2MessagePreviewParser, obj2, { textColor: str2 }), ];
  if (null == ATTACHMENT) {
    items5[1] = tmp26;
    obj1.children = items5;
    items3[1] = closure_22(tmp2(4601).Text, obj1);
    obj.children = items3;
...
createStyles = { color: nativeDefault.colors.MENTION_FOREGROUND, backgroundColor: nativeDefault.colors.MENTION_BACKGROUND };
...
fn(4605);
createStyles = { container: { flex: 1 }, row: null, rowCompact: null, rowActive: null, col: null, unreadIndicatorV2: null, unreadIndicatorCompactV2: null, rowText: null, rowTextV2: null, rowBody: null, rowBodyV2: null, rowBodyAcked: null, rowTime: null, rowTimeV2: null, itemV2: null, calloutContainer: null, calloutTextAcked: null, calloutTextNotAcked: null, messagePreviewContainerV2: null, messagePreviewBarV2: null, messagePreviewIconV2Container: null, messagePreviewIconV2: null, messagePreviewTextV2Acked: null, messagePreviewTextV2NotAcked: null, messagePreviewSystemTextV2: null, refreshSpinner: null, forYouDivider: null, friendRequestNoteContainer: null };
let obj6 = { color: nativeDefault.colors.MENTION_FOREGROUND, backgroundColor: "transparent" };
createStyles.row = { marginHorizontal: 4, paddingHorizontal: 12, paddingVertical: 8, marginBottom: 4, borderRadius: nativeDefault.radii.lg, flexDirection: "row", justifyContent: "space-between" };
createStyles.rowCompact = { paddingVertical: 6 };
let obj8 = { marginHorizontal: 4, paddingHorizontal: 12, paddingVertical: 8, marginBottom: 4, borderRadius: nativeDefault.radii.lg, flexDirection: "row", justifyContent: "space-between" };
...
createStyles = { marginRight: 8, borderLeftColor: nativeDefault.colors.BORDER_SUBTLE, borderLeftWidth: 3, borderRadius: 2, height: "auto" };
...
createStyles.forYouDivider = { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: nativeDefault.colors.BORDER_SUBTLE, marginTop: nativeDefault.space.PX_12, marginBottom: nativeDefault.space.PX_8 };
const obj18 = { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: nativeDefault.colors.BORDER_SUBTLE, marginTop: nativeDefault.space.PX_12, marginBottom: nativeDefault.space.PX_8 };
...
let closure_35 = noop.memo((item) => {
  ...
  const callback1 = str.useCallback(() => {
    ...
    callback();
    obj = { action_type: NotificationCenterItemsTypes.NotificationCenterActionTypes.CLICKED, notification_center_id: item.id, item_type: item.type, acked: notificationCenterItemAcked, item_index: rowIndex, deeplink: item.deeplink };
      ...
      obj.onPress = function onPress() {
        try {
          ...
          if (null == match) {
            ...
            const result = obj3.openGuildHighlightNotificationForPush(tmp21, obj, constants2.TRENDING_CONTENT_PUSH, constants.NOTIFICATION_CENTER, obj);
          }
        } catch (err) {
  ...
  if (typeof tmp10 !== "string") {
    ...
    if (tmp25Result) {
      const obj9 = { styles: tmp.friendRequestNoteContainer, backgroundColor: tmp.friendRequestNoteContainer.backgroundColor, userId: null, analyticsLocation: "Notifications Tab" };
...
export const ForYouItems = noop.memo((loadMore) => {
  ...
  const items3 = [tmp.forYouDivider, suggestedFriendAdded, onAddSuggestionAnimationFinish, stateFromStores, setting, isSoftAcked, onSoftAckItem, forceHoistItem, isForceHoisted, tmp8, onPressLoad, flag];
  const callback1 = suggestedFriendAdded.useCallback((item) => {
    ...
    switch (item.kind) {
      ...
      case "notification-center-item":
        ...
        const obj4 = { item, ackedBeforeId: setting, isSoftAcked: isSoftAcked(item.id), onSoftAckItem, forceHoistItem, isForceHoisted, rowIndex: tmp, compactMode, roleStyle: stateFromStores };
        const _HermesInternal = HermesInternal;
