// === Module 9323: useComputedImagesForActivity ===
...
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8561 */;
import unpackStageChannelParty from "unpackStageChannelParty" /* 9196 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 9324 */;
...
function useComputedImagesForActivity(activity, getIconURL) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(9196).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
      ...
      const obj2 = { largeImage: null, smallImage: "r" };
      obj2[0] = tmp14;
      obj8 = obj2;
      const tmp15Result = tmp15(9196);
    } else if (isOnXboxDefault(activity)) {
      const obj3 = { largeImage: null, smallImage: "r" };
      const obj4 = { src: null, alt: null };
      let tmp2Result = tmp2(5192);
      ...
      if (null == iconURL) {
        if (null != undefined) {
          ...
          obj8 = { largeImage: null, smallImage: "r" };
          obj8[0] = smallImage;
        }
      } else {
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  if (tmp5Result.isListenedSessionEntry(entry)) {
    ...
    if (null != largeImage) {
      ...
      obj1 = { largeImage: null, smallImage: "r" };
      obj1[0] = tmp7;
      obj4 = obj1;
    } else {
      if (null != coverURL) {
      ...
      obj4 = { largeImage: null, smallImage: "r" };
  ...
  tmp5Result = tmp5(8558);
  if (tmp5Result.isWatchedMediaEntry(entry)) {
    const obj6 = { src: null, alt: null };
    obj6[0] = tmp5(7995).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE);
    obj6[1] = entry.extra.media_title;
    tmp7 = obj6;
    const tmp5Result1 = tmp5(7995);
  } else {
