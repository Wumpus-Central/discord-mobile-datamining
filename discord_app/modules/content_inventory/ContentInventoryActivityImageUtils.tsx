...
function useComputedImagesForActivity(activity, getIconURL) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(9574).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
...
function useTrackActivityDefaultIcon(arg0) {
  ...
  const getOrFetchApplication = trackingSource(5655).useGetOrFetchApplication(application_id);
  let obj = trackingSource(5655);
  const items = [c4];
...
function useRichImageForActivity(activity, activityApplication) {
  ...
  if (null == activity) {
    return { largeImage: "Array", smallImage: "PX_8" };
  } else {
    let large_image;
    if (activity != null) {
      const assets = activity.assets;
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  tmp5Result = tmp5(8827);
  if (tmp5Result.isWatchedMediaEntry(entry)) {
    const obj6 = { src: null, alt: null };
    obj6[0] = tmp5(8164).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE);
    obj6[1] = entry.extra.media_title;
    tmp7 = obj6;
    const tmp5Result1 = tmp5(8164);
  } else {
