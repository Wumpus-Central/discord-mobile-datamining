// === Module 9345: useComputedImagesForActivity ===
...
import useGetOrFetchApplications from "useGetOrFetchApplications" /* 5927 */;
import importDefaultResult1 from "importDefaultResult1" /* 6122 */;
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8583 */;
import unpackStageChannelParty from "unpackStageChannelParty" /* 9218 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 9346 */;
...
function useComputedImagesForActivity(activity, getIconURL) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(9218).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
...
function useRichImageForActivity(activity, activityApplication) {
  ...
  if (null == activity) {
    return { largeImage: "cix", smallImage: "id" };
  } else {
    let large_image;
    if (activity != null) {
      const assets = activity.assets;
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  tmp5Result = tmp5(8580);
  if (tmp5Result.isWatchedMediaEntry(entry)) {
    const obj6 = { src: null, alt: null };
    obj6[0] = tmp5(8017).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE);
    obj6[1] = entry.extra.media_title;
    tmp7 = obj6;
    const tmp5Result1 = tmp5(8017);
  } else {
