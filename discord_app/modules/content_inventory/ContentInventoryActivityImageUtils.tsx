// === Module 9305: useComputedImagesForActivity ===
...
import isCrunchyrollActivityDefault from "isCrunchyrollActivity" /* 8546 */;
import unpackStageChannelParty from "unpackStageChannelParty" /* 9178 */;
import useEntryActivityAndApplicationDefault from "useEntryActivityAndApplication" /* 9306 */;
...
function useComputedImagesForActivity(activity, getIconURL) {
  ...
  if (null != largeImage) {
    ...
    if (obj17.isStageActivity(activity)) {
      const result = tmp15(9178).unpackStageChannelParty(activity);
      let guildIconURL;
      if (null != result) {
...
export const useImageForContentEntry = function useImageForContentEntry(trackingSource) {
  ...
  tmp5Result = tmp5(8543);
  if (tmp5Result.isWatchedMediaEntry(entry)) {
    const obj6 = { src: null, alt: null };
    obj6[0] = tmp5(7981).getAssetImage(entry.extra.application_id, entry.extra.media_assets_large_image, ImageSizes.LARGE);
    obj6[1] = entry.extra.media_title;
    tmp7 = obj6;
    const tmp5Result1 = tmp5(7981);
  } else {
