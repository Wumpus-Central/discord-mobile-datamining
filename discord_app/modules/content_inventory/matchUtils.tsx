...
import ContentInventoryListenedMediaProvider from "ContentInventoryListenedMediaProvider" /* 8693 */;
...
export const findMatchingEntry = function findMatchingEntry(entries, activity) {
  ...
  if (activity.type === ActivityTypes.PLAYING) {
    const found1 = found.filter(tmp(8691).isGamingLikeEntry);
    ...
    const found3 = found.filter(tmp(8691).isListenedSessionEntry);
    found2 = found3.find((item) => isMatchingListeningActivity(item, closure_0));
  } else if (activity.type === ActivityTypes.WATCHING) {
    const found4 = entries.filter(tmp(8691).isWatchedMediaEntry);
