...
export default function useHeroColors(pendingAvatarSrc) {
  ...
  let obj2 = first(589);
  const fallbackHeroColor = first(7975).getFallbackHeroColor(stateFromStores1, stateFromStores);
  let obj3 = first(7975);
  let tmp4 = callback(first(7973).useAvatarColors(pendingAvatarSrc, fallbackHeroColor), 2);
  first = tmp4[0];
  closure_1 = tmp6;
...
export const getHeroColors = function getHeroColors(game_name) {
  ...
  const useColorStore = tmp2(7973).useColorStore;
