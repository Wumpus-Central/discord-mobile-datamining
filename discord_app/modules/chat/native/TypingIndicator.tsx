...
function TypingIndicatorInner(channel) {
  ...
  let items = [closure_6];
  const stateFromStores = channel(cleanUp[11]).useStateFromStores(items, () =>
  ...
  const stateFromStoresObject = channel(cleanUp[11]).useStateFromStoresObject(
    items1,
    () => {
      if (null != first) {
        if (canView) {
  ...
  V.__closure = {
    ...
    withSpring: channel(cleanUp[23]).withSpring,
    springStandard: channel(cleanUp[24]).springStandard,
  };
  ...
  fn.__closure = {
    ...
    translateYValue: sharedValue1,
    transitionState,
    TransitionStates: channel(cleanUp[22]).TransitionStates,
  };
  ...
  if (null != stateFromStoresObject.config) {
    ...
    obj10.onPress = tmp27;
    let tmp21Result = closure_12(transitionState(tmp2[25]), obj10);
    const tmp6Result = transitionState(tmp2[25]);
  } else {
    let tmp20Result3 = null;
    if (null != tmp7Result) {
      tmp20Result3 = closure_12(tmp(tmp2[26]).Ellipsis, {});
    }
    ...
    items7[1] = closure_12(tmp(tmp2[27]).Text, obj12);
  ...
  if (channel.rateLimitPerUser > 0) {
    ...
    tmp20Result4 = closure_12(transitionState(tmp2[28]), obj13);
  }
