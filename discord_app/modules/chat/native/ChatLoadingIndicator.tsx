...
export const ChannelHeaderLoadingIndicator = function ChannelHeaderLoadingIndicator() {
  ...
  const fn = function t() {
    ...
    if (stateFromStores) {
      ...
      obj.scale = obj3.withRepeat(
        obj4.withSequence(withTimingResult, withTimingResult1, timing.withTiming(1, obj1)),
        -1,
      );
      const items = [obj];
      obj.transform = items;
  ...
  obj = {
    ...
    Easing: stateFromStores(4341).Easing,
  };
  fn.__closure = obj;
  ...
  items3[1] = closure_13(stateFromStores(4601).Text, obj2);
  obj.children = items3;
  const tmp8 = closure_14(View, obj);
  let tmp7Result = tmp8;
  if (stateFromStores1) {
    let obj3 = { onPress: openLoadingIndicatorDebugBody, children: tmp8 };
    tmp7Result = closure_13(stateFromStores(5173).PressableOpacity, obj3);
  }
  return tmp7Result;
};
