// === Module 17436: MuteSettingsScreen ===
...
function UnmuteOptions(channel) {
  ...
  obj2.icon = closure_10(channel(1177).Icon, { disableColor: true, source: navigation(10490) });
  const obj4 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: null };
  const intl = channel(1115).intl;
  const obj5 = { name: null };
  const obj3 = { disableColor: true, source: navigation(10490) };
  obj5.name = channel(4982).computeChannelName(channel, UserStore, RelationshipStore, true);
  obj4.children = intl.format(channel(1115).t["eC+9rj"], obj5);
  obj2.label = closure_10(channel(4825).Text, obj4);
  obj2.onPress = callback;
  const items1 = [closure_10(channel(5910).TableRow, obj2), ];
  const obj7 = { muteConfig: channel.muteConfig, type: null };
  const obj6 = channel(4982);
  const tmp6 = navigation(10491);
  const MuteSettingType = channel(10491).MuteSettingType;
      ...
      onOptionPress(arg0) {
        ...
        if (undefined !== muted) {
          if (closure_1_0.isThread()) {
            ...
            const result = applicationId(navigation[10]).setNotificationSettings(closure_1_0, obj3);
            ...
            const result1 = obj2.updateAppDMOverrideSettings(guildId, id, closure_1_1, obj5, NotificationLabel2.muted(muted));
          } else {
            const obj7 = { guildId: closure_1_0.getGuildId(), channelId: closure_1_0.id, settings: null, label: null };
...
function NotificationSettingsButton(guildMessageNotifications) {
  ...
  items3[1] = closure_10(View, { style: tmp.hint, children: closure_10(channel(navigation[23]).MuteSettingsHint, { isMuted, isGuildMuted, guildMessageNotifications: guildMessageNotifications.guildMessageNotifications }) });
...
export default noop.memo(() => {
  ...
  if (null != stateFromStores) {
    ...
    if (muted) {
      ...
      let tmp11Result = closure_10(UnmuteOptions, obj7);
      let tmp14 = closure_10;
    } else {
      const obj8 = { channel: stateFromStores, applicationId, navigation };
      tmp11Result = closure_10(MuteOptions, obj8);
      tmp14 = closure_10;
    }
