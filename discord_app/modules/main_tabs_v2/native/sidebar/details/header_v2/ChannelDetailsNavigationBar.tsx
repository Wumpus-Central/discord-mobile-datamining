...
function MuteButton(channelId) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = tmp5(stateFromStores ? 9150 : 8023);
  return closure_15(channelId(8172).IconButton, obj, constants.MUTE);
}
function SearchButton(channelId) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = _modDef7125;
  obj.disabled = shouldHideChannelContent;
  return closure_15(channelId(8172).IconButton, obj, constants.SEARCH);
}
function SettingsButton(channel) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = navigation(7454);
  return closure_15(channel(8172).IconButton, obj, constants.SETTINGS);
}
...
obj = { position: "relative", zIndex: 1, height: fn(12463).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 };
obj.container = obj;
const createStyles = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12463).SEARCH_BAR_HEIGHT };
...
let closure_24 = noop.forwardRef((cleanUp, ref) => {
  ...
  obj = { isActive: tmp2, withTiming: width(4607).withTiming, Easing: width(4343).Easing, runOnJS: width(4343).runOnJS, cleanUp, width };
  S.__closure = obj;
  ...
  obj.style = items;
  obj.children = closure_15(cleanUp(16706), { ref, channelId: channel.id, guildId: channel.guild_id, onSuggestionsLayoutMesure: onSuggestionsLayoutMeasure, suggestionsDismissed, setSuggestionsDismissed, showBackButton: true });
  return closure_15(cleanUp(4343).View, obj);
});
