...
function MuteButton(channelId) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = tmp5(stateFromStores ? 9189 : 8053);
  return closure_15(channelId(8202).IconButton, obj, constants.MUTE);
}
function SearchButton(channelId) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = _modDef7155;
  obj.disabled = shouldHideChannelContent;
  return closure_15(channelId(8202).IconButton, obj, constants.SEARCH);
}
function SettingsButton(channel) {
  ...
  obj.variant = token1;
  obj.size = token;
  obj.icon = navigation(7484);
  return closure_15(channel(8202).IconButton, obj, constants.SETTINGS);
}
...
obj = { position: "relative", zIndex: 1, height: fn(12499).SEARCH_BAR_HEIGHT, marginTop: nativeDefault.space.PX_8 };
obj.container = obj;
const createStyles = { flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_16, paddingVertical: nativeDefault.space.PX_4, position: "absolute", height: fn(12499).SEARCH_BAR_HEIGHT };
...
let closure_24 = noop.forwardRef((cleanUp, ref) => {
  ...
  obj = { isActive: tmp2, withTiming: width(4637).withTiming, Easing: width(4373).Easing, runOnJS: width(4373).runOnJS, cleanUp, width };
  S.__closure = obj;
  ...
  obj.style = items;
  obj.children = closure_15(cleanUp(16733), { ref, channelId: channel.id, guildId: channel.guild_id, onSuggestionsLayoutMesure: onSuggestionsLayoutMeasure, suggestionsDismissed, setSuggestionsDismissed, showBackButton: true });
  return closure_15(cleanUp(4373).View, obj);
});
