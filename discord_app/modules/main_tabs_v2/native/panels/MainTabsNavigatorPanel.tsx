...
export default noop.memo(function MainTabsNavigatorPanel() {
  ...
  const tmp10 = _slicedToArray(noop.useState(() => {
    const first = arr2[0];
    let type;
    if (first != null) {
      type = first.type;
  ...
  const tmp15 = _slicedToArray(noop.useState(() => useChannelScreensFromNavigation.isActiveTabsGuilds(navigation.getState())), 2);
  const first2 = tmp15[0];
  const logger = tmp15[1];
  const tmp8 = isChatLockedOpen(drawerWidth[11])();
  [isChatBesideChannelList, closure_13] = _slicedToArray(noop.useState(() => first2), 2);
  ...
  current = { canDrag: tmp25, onVisibilityChange: callback1, onDragStart: callback2, startShown: null, openWidth: null };
  const first3 = arr2[0];
  let type;
  if (first3 != null) {
    type = first3.type;
  ...
  if (isChatBesideChannelList) {
    ...
    if (arr2.length > 0) {
      const obj8 = { screens: arr2, screenStackActive: null, translateX: null, isDragging: null, maxWidth: null, highestFullyRenderedScreenIndex: null, shouldFreeze: null, focusChatPressableComponent: null, firstScreenWidth: null, firstScreenFrame: null };
