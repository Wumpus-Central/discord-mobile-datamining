...
export default function FriendRequestsScreen() {
  ...
  const effect = gameRelationshipsByType1.useEffect(() => {
    if (stateFromStoresArray.length > 0) {
      ...
      const result1 = stateFromStoresArray(16414).bulkMarkNotificationCenterItemsAcked(
  ...
  const memo1 = gameRelationshipsByType1.useMemo(
    () => stateFromStoresArray(16779).getPendingRelationshipIds(first, dependencyMap),
    items4,
  );
  ...
  if (tmp28) {
    ...
    obj6[1] = function onPress() {
      ...
      stateFromStoresArray(4906).openAlert(
