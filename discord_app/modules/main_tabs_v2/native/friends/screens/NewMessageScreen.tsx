...
function Header(recipientLimit) {
  ...
  if (recipientLimit.usePersonLimitCopy) {
    ...
    return closure_17(recipientLimit(16840), obj);
  } else {
    obj = { style: tmp.header, children: null };
    const obj1 = { title };
    const items1 = [closure_17(numInGroup(7936).GenericHeaderTitle, obj1), ];
...
function ChatPreview(channelId) {
  ...
  items2[1] = closure_17(channelId(16841).PortalKeyboardRenderer, obj1);
  obj.children = items2;
...
export default function NewMessageScreen(navigation) {
    ...
    if (null == source_page) {
      if (null == findLocalMatchingPrivateChannelId(recipients)) {
        ...
        return () => {
          ...
          obj = { id: navigation(7294).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID, guild_id: "Array", parent_id: "isArray" };
          obj.channel = obj;
          obj.dispatch(obj);
        };
      }
  ...
  const effect3 = obj.useEffect(() => {
    ...
    if (tmp2) {
      const obj = { type: "LOAD_MESSAGES_SUCCESS", channelId: insets.id, messages: [], isBefore: false, isAfter: false, hasMoreBefore: false, hasMoreAfter: false, limit: 0, jump: "PX_16", isStale: "custom" };
      obj.dispatch(obj);
    }
  }, items6);
  ...
  const memo = obj.useMemo(() => {
    ...
    if (constants) {
      if (stateFromStores1) {
        let obj = { icon: _modDef10121, IconComponent: GroupPlusIcon.GroupPlusIcon, label: null, iconVariant: "default", onPress: null };
      ...
      obj = { icon: _modDef4544, IconComponent: UserPlusIcon.UserPlusIcon, label: null, iconVariant: "default", onPress: null };
