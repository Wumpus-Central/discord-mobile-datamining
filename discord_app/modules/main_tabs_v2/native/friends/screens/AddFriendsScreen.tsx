...
function areHydratedGameFriendRequestRowStatesEqual(arr) {
  ...
  let tmp = arr === arg1;
  if (!tmp) {
    tmp = arr.length === arg1.length && arr.every((item, index) => item.user === table[index].user && item.applicationId === table[index].applicationId);
    const tmp2 = arr.length === arg1.length && arr.every((item, index) => item.user === table[index].user && item.applicationId === table[index].applicationId);
  }
  return tmp;
}
...
const createCacheKey = { backgroundColor: ThemesDefault.colors.MOBILE_ACTIONSHEET_BACKGROUND, paddingVertical: ThemesDefault.space.PX_16 };
...
let result = require("obj132").fileFinishedImporting("modules/main_tabs_v2/native/friends/screens/AddFriendsScreen.tsx");
...
export default function AddFriendsScreen(navigation) {
  ...
  importDefault(analyticsLocations[22])(() => {
    callback(analyticsLocations[23]);
    const obj = { friend_add_type: callback.FRIENDS_ADD_FRIENDS_MODAL, source_page: callback };
  ...
  const stateFromStoresArray = obj2.useStateFromStoresArray(items2, () => {
    ...
    const items1 = [];
    const item = _undefined4.forEach((item, index) => {
      const user = closure_1_10.getUser(item);
      if (null != user) {
        items1.push(user);
      }
    });
  ...
  const stateFromStores = obj3.useStateFromStores(items3, () => {
    ...
    const item = gameRelationshipsByType.forEach((item, index) => {
      ...
      if (!someResult) {
        someResult = first.some((item, index) => item.userId === id && item.applicationId === tmp);
      }
      if (!someResult) {
    ...
    const items1 = [];
    const item1 = first1.forEach((item, index) => {
      const user = authStore.getUser(item.userId);
      if (null != user) {
        const obj = { user: null, applicationId: null };
        obj[0] = user;
        obj[1] = item.applicationId;
        items1.push(obj);
      }
    });
  ...
  const effect1 = obj1.useEffect(() => {
    if (obj.isContactSyncAvailable()) {
      ...
      result.then((result) => {
        const obj = closure_1_0(analyticsLocations[19]);
        let tmp3 = result === stateFromStores.NOT_DETERMINED;
        if (!tmp3) {
          tmp3 = tmp2;
        }
        callback(tmp3);
        tmp2 = closure_1_0(analyticsLocations[19]).isAndroid() && result === stateFromStores.UNAUTHORIZED;
      });
      const tmpResult = _undefined(analyticsLocations[18]);
    }
    obj = _undefined(analyticsLocations[18]);
  }, []);
  ...
  if (0 === stateFromStoresArray.length && 0 === stateFromStores.length && 0 === friendSuggestions.length) {
    ...
    obj8[0] = memo1;
    obj8[1] = function getItemProps(flag2, arg1) {
      closure_0 = tmp;
      closure_1 = tmp2;
      if (friendSuggestions.FIND_FRIENDS === flag2) {
            ...
            obj1[3] = function component() {
              let obj = {
                onPress() {
                  ...
                  const obj = { section_id: closure_1_11.PENDING, truncated_count: 3, expanded_count: length.length, location: "AddFriends" };
        ...
        if (tmp2) {
          if (closure_21) {
            ...
            obj3[3] = function component() {
              ...
              obj[1] = substr.map((item, index) => item.user);
              obj[2] = arr2.length;
              return closure_1_17(callback2(id[37]), obj);
            };
            return obj3;
        ...
        obj4[3] = function component() {
          return callback3(callback(analyticsLocations[38]).ConnectedIncomingGameFriendRequestRow, { accepted: null != first1.find((item, index) => item.userId === id.id && item.applicationId === tmp), applicationId, user, start: closure_0, end: closure_1, onPress: closure_1_14, onDeclineIncomingRequest: closure_1_12, onAcceptIncomingRequest: closure_1_11 });
        };
        return obj4;
