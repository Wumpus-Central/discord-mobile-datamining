...
import IncomingRequestRow from "../components/IncomingRequestRow.tsx";
import ContactSuggestionRow from "../components/ContactSuggestionRow.tsx";
...
function areHydratedGameFriendRequestRowStatesEqual(arr, arg1) {
  ...
  if (!tmp) {
    tmp =
      ...
      arr.every(
        (user, index) =>
          user.user === dependencyMap[index].user && user.applicationId === dependencyMap[index].applicationId,
      );
    const tmp2 =
      arr.length === arg1.length &&
      arr.every(
...
const Sections = fn(13038).Sections;
...
let closure_19 = {
  ...
  [1]: "INCOMING_FRIEND_REQUESTS",
  INCOMING_GAME_FRIEND_REQUESTS: 2,
  [2]: "INCOMING_GAME_FRIEND_REQUESTS",
...
export default function AddFriendsScreen(navigation) {
  ...
  const tmp4 = require("useScaledRowHeight")();
  _slicedToArray = tmp4;
  ...
  const tmp10 = _slicedToArray(userRowWithSubLabelHeight1.useState([]), 2);
  ...
  const stateFromStoresArray = navigation(analyticsLocations[27]).useStateFromStoresArray(items2, () => {
    ...
    return _modDef12
      ...
      .sort((arg0, arg1) => {
        const name = source_page(4671).getName(arg0);
        const obj = source_page(4671);
        return name.localeCompare(source_page(4671).getName(arg1));
      });
  });
  let obj5 = navigation(analyticsLocations[27]);
  const items3 = [c8, c9, first];
  const items4 = [first1, first];
  const stateFromStores = navigation(analyticsLocations[27]).useStateFromStores(
    items3,
    () => {
      ...
      return source_page(analyticsLocations[28])
        ...
        .sort((user, user2) => {
          const name = items1(4671).getName(user.user);
          const obj = items1(4671);
          return name.localeCompare(items1(4671).getName(user2.user));
        });
    },
    items4,
    closure_24,
  ...
  const effect1 = obj4.useEffect(() => {
    if (obj.isContactSyncAvailable()) {
      ...
      result.then((result) => {
        ...
        closure_1_0(tmp3);
        tmp2 = navigation(analyticsLocations[19]).isAndroid() && result === callback.UNAUTHORIZED;
      });
  ...
  const obj9 = {
    ...
    children: closure_17(tmp2(obj[33]), {
      onItemPressed: friendSuggestions,
      contentContainerStyle: tmp.inviteAppsContentContainer,
    }),
  };
        ...
        if (stateFromStores.FIND_FRIENDS === flag2) {
          ...
          obj3 = {
            ...
            component() {
              ...
              obj.icon = onPress2(navigation(analyticsLocations[36]).TableRow.Icon, {
                IconComponent: navigation(analyticsLocations[38]).AtIcon,
              });
              obj.onPress = onPress;
              return onPress2(navigation(analyticsLocations[36]).TableRow, obj);
            },
          };
        } else if (stateFromStores.INCOMING_FRIEND_REQUESTS === flag2) {
          if (callback1(flag2, arg1)) {
            const obj4 = {
              ...
              component() {
                const obj = {
                  onPress() {
                    closure_1(closure_2[25]).track(constants2.FRIEND_FINDER_SECTION_EXPANDED, {
                      section_id: constants.PENDING,
                      truncated_count: 3,
            ...
            const obj5 = {
              ...
              component() {
                return closure_3_17(IncomingRequestRow.IncomingFriendRequestRow, {
                  accepted: c8.includes(user.id),
                  user,
                  start,
                  end,
                  onPress: onPress2,
