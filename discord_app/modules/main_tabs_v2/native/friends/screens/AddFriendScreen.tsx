...
export default function AddFriendScreen(navigation) {
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    ...
    result.then((result) => {
      ...
      if (!tmp5) {
        tmp5 = !navigation(12710).isContactSyncEnabled(contactSyncAccount);
        const tmp2Result = navigation(12710);
      }
      dependencyMap(tmp5);
  ...
  const items2 = [closure_10(navigation(4570).Text, obj), , ,];
  ...
  items2[1] = closure_10(navigation(4570).Text, obj1);
  items2[2] = closure_10(contactSyncAccount(13881), {
    style: tmp.input,
  ...
  const items3 = [closure_10(navigation(4570).Text, obj4)];
  let tmp10Result = null;
  if (tmp5) {
    const obj5 = { style: tmp.rowContainer, location: "Add Friend Modal" };
    tmp10Result = closure_10(contactSyncAccount(13883), obj5);
  }
