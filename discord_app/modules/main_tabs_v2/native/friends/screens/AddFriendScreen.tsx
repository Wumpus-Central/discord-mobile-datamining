...
export default function AddFriendScreen(navigation) {
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    ...
    result.then((result) => {
      ...
      if (!tmp5) {
        tmp5 = !navigation(12750).isContactSyncEnabled(contactSyncAccount);
        const tmp2Result = navigation(12750);
      }
      dependencyMap(tmp5);
  ...
  const items2 = [closure_10(navigation(4601).Text, obj), , ,];
  ...
  items2[1] = closure_10(navigation(4601).Text, obj1);
  items2[2] = closure_10(contactSyncAccount(13930), {
    style: tmp.input,
  ...
  const items3 = [closure_10(navigation(4601).Text, obj4)];
  let tmp10Result = null;
  if (tmp5) {
    const obj5 = { style: tmp.rowContainer, location: "Add Friend Modal" };
    tmp10Result = closure_10(contactSyncAccount(13932), obj5);
  }
