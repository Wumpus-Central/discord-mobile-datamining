...
function IncomingRequestRow(user) {
  ...
  const memo1 = onAcceptIncomingRequest.useMemo(() => {
    if (null != stateFromStores1) {
      ...
      let str = jsx(
        ...
        { application: stateFromStores1, textVariant: "text-xs/medium", iconSize: 12 },
        stateFromStores1.id,
      );
    } else {
      str = "";
  ...
  obj5.subLabel = userTag(user(accepted[13]).ActionStatusSubLabel, {
  ...
  obj5.trailing = userTag(user(accepted[14]).IncomingRequestRowActions, {
    user,
    pressed: sharedValue,
    applicationId,
...
export const ConnectedIncomingGameFriendRequestRow = function ConnectedIncomingGameFriendRequestRow(applicationId) {
  ...
  const stateFromStores = applicationId(563).useStateFromStores(items, () =>
    ApplicationStore.getApplication(applicationId),
  );
