...
obj.easing = Easing2.in(fn(4341).Easing.quad);
obj = { duration: 250, easing: null };
const Easing3 = fn(4341).Easing;
obj.easing = Easing3.in(fn(4341).Easing.quad);
let obj1 = { duration: 250, easing: null };
const Easing4 = fn(4341).Easing;
obj1.easing = Easing4.out(fn(4341).Easing.quad);
...
export const ContactSuggestionActions = function ContactSuggestionActions(user) {
      ...
      if (animate) {
        ...
        if (user) {
          ...
          fn = function n(arg0) {
            if (arg0) {
              let __closure = user(onAddSuggestion[3]);
              const result = sharedValue2.set(__closure.withDelay(1000, user(onAddSuggestion[7]).withTiming(0, sharedValue3)));
  ...
  __closure = { animate, runOnJS: user(onAddSuggestion[3]).runOnJS, finishAnimationCallback: SCALE_CONFIG, scale: sharedValue1, withTiming: user(onAddSuggestion[7]).withTiming, SCALE_CONFIG, opacity: sharedValue2, OPACITY_CONFIG: sharedValue2, buttonOpacity: sharedValue3, right: sharedValue, withSpring: user(onAddSuggestion[8]).withSpring, SPRING_CONFIG, withDelay: user(onAddSuggestion[3]).withDelay, OPACITY_OUT_CONFIG: sharedValue3, TRANSLATE_OUT_CONFIG: obj1 };
