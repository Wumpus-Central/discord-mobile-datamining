...
import CoachmarkImgDefault from "CoachmarkImg" /* 15921 */;
import NotificationCenterBannerImplDefault from "NotificationCenterBannerImpl" /* 15929 */;
import YouBarNavigatorScreens2 from "YouBarNavigatorScreens" /* 15930 */;
...
class Notifications {
  constructor(arg0) {
    ...
    layoutEffect = closure_3.useLayoutEffect(() => callback(7167).trackAppUIViewed(), []);
    callback = closure_3.useCallback(() => {
      const navigation = callback(4301).getRootNavigationRef();
      if (null != navigation) {
        if (navigation.canGoBack()) {
          navigation.goBack();
        } else {
...
let closure_12 = importAllResult.memo(function HeaderInner(nestedInLaunchPad) {
  ...
  if (!nestedInLaunchPad) {
    ...
    obj1[2] = goBack;
    obj1[3] = tmp12(tmp5(15922).LeftBackIconWithBadge, {});
    const items1 = [tmp12(tmp5(5033).PressableOpacity, obj1), , ];
    ...
    items1[1] = tmp12(tmp5(4442).Text, obj2);
    ...
    if (isForLaterExperimentOn) {
      ...
      obj5[0] = ref;
      let tmp2Result = tmp2(15923);
      obj5[1] = tmp5(7551).SavedMessageSortTypes.BOOKMARK;
      obj5[2] = callback;
      const items2 = [tmp12(tmp2Result, obj5), ];
      const obj6 = { type: null, onOpen: null };
      tmp2Result = tmp2(15923);
      obj6[0] = tmp5(7551).SavedMessageSortTypes.REMINDER;
      obj6[1] = callback;
      items2[1] = tmp12(tmp2Result, obj6);
      obj4[0] = items2;
      tmp10Result = tmp10(closure_8, obj4);
    }
    const items3 = [tmp10Result, tmp12(tmp2(15925), {})];
    obj3[1] = items3;
  ...
  obj[1] = tmp10Result;
  const items4 = [closure_7(_require(5518).SafeAreaPaddingView, obj), ];
