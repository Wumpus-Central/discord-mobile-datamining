...
const memoResult = importAllResult.memo(function MessagesHeader(height) {
  ...
  obj = { withSpring: height(4694).withSpring, scrollPosition };
  fn.__closure = obj;
  fn.__workletHash = 17233409273245;
  fn.__initData = closure_11;
  const animatedStyle = obj.useAnimatedStyle(fn);
  let obj2 = height(15071);
  ...
  obj[1] = callback(height(9625).PlusLargeIcon, obj1);
  ...
  const tmp13 = scrollPosition(5254)("bespoke");
  ...
  obj4[4] = stringResult;
  obj3[1] = callback(height(4281).Text, obj4);
  ...
  if (tmp4) {
    let tmp14Result = tmp2(5928);
  } else {
    tmp14Result = tmp14(tmp6(5927).MagnifyingGlassIcon, { size: "sm" });
  }
  obj6[3] = tmp14Result;
  const intl3 = tmp6(1236).intl;
  obj6[4] = intl3.string(height(1236).t["5h0QOP"]);
  const items2 = [callback(height(7672).IconButton, obj6), callback(scrollPosition(15076), { noMargin: true, onPress: callback, alternateVariant: true }), , ];
  if (tmp4) {
    let tmp2Result = tmp2(4224);
  } else {
    tmp2Result = tmp14(tmp6(4223).UserPlusIcon, { size: "sm" });
  }
  ...
  items2[2] = callback(height(4695).Button, obj7);
  items2[3] = callback(height(7672).IconButton, obj);
  obj5[1] = items2;
  items1[1] = closure_7(closure_4, obj5);
  const items3 = [tmp.headerBorder, animatedStyle];
  items1[2] = callback(scrollPosition(4145).View, { style: items3 });
  items1[3] = tmp13;
  obj2[1] = items1;
