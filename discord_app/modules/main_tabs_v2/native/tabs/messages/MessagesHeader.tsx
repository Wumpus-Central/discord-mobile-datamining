...
let obj2 = { height: fn(5061).SMALL_BUTTON_HEIGHT, gap: nativeDefault.modules.mobile.MESSAGES_HEADER_BUTTON_GAP, flexDirection: nativeDefault.modules.mobile.MESSAGES_HEADER_BUTTON_LAYOUT, alignItems: "center" };
const size = fn(2);
...
export default noop.memo(function MessagesHeader(height) {
  ...
  let obj = { withSpring: height(5055).withSpring, scrollPosition };
  fn.__closure = obj;
  fn.__workletHash = 17233409273245;
  fn.__initData = __initData;
  const animatedStyle = obj.useAnimatedStyle(fn);
  let obj2 = height(16115);
  ...
  obj.icon = closure_6(height(11082).PlusLargeIcon, { size: "sm", color: scrollPosition(576).colors.WHITE });
  ...
  obj3.children = closure_6(height(4632).Text, { color: "mobile-text-heading-primary", variant: "heading-lg/semibold", maxFontSizeMultiplier: 1.75, accessibilityRole: "header", children: stringResult });
  const items1 = [closure_6(closure_4, obj3), , , ];
  const obj4 = { style: tmp.headerPanelButtons, children: null };
  const obj5 = { onPress: callback2, variant: "secondary", size: "sm", icon: scrollPosition(7155), accessibilityLabel: null };
  const intl3 = tmp3(1114).intl;
  obj5.accessibilityLabel = intl3.string(height(1114).t["5h0QOP"]);
  const items2 = [closure_6(height(8202).IconButton, obj5), closure_6(scrollPosition(16123), { noMargin: true, onPress: callback, alternateVariant: true }), , ];
  const obj6 = { variant: "secondary", grow: true, shrink: true, size: "sm", icon: scrollPosition(4574), onPress: callback1, maxFontSizeMultiplier: 1, text: null };
  const intl4 = tmp3(1114).intl;
  obj6.text = intl4.string(height(1114).t.zIJnA6);
  items2[2] = closure_6(height(5056).Button, obj6);
  items2[3] = closure_6(height(8202).IconButton, obj);
  obj4.children = items2;
  items1[1] = closure_7(closure_4, obj4);
  const obj7 = { style: null };
  const items3 = [tmp.headerBorder, animatedStyle];
  obj7.style = items3;
  items1[2] = closure_6(scrollPosition(4373).View, obj7);
  items1[3] = tmp12;
