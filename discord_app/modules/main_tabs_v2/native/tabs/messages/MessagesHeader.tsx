...
const memoResult = importAllResult.memo(function MessagesHeader(height) {
  ...
  obj = { withSpring: height(4753).withSpring, scrollPosition };
  fn.__closure = obj;
  ...
  obj[1] = callback(height(9751).PlusLargeIcon, obj1);
  ...
  const tmp13 = scrollPosition(5310)("bespoke");
  ...
  obj4[4] = stringResult;
  obj3[1] = callback(height(4340).Text, obj4);
  ...
  if (tmp4) {
    let tmp14Result = tmp2(6048);
  } else {
    tmp14Result = tmp14(tmp6(6047).MagnifyingGlassIcon, { size: "sm" });
  }
  obj6[3] = tmp14Result;
  const intl3 = tmp6(1236).intl;
  obj6[4] = intl3.string(height(1236).t["5h0QOP"]);
  const items2 = [callback(height(7792).IconButton, obj6), callback(scrollPosition(15230), { noMargin: true, onPress: callback, alternateVariant: true }), , ];
  if (tmp4) {
    let tmp2Result = tmp2(4283);
  } else {
    tmp2Result = tmp14(tmp6(4282).UserPlusIcon, { size: "sm" });
  }
  ...
  items2[2] = callback(height(4754).Button, obj7);
  items2[3] = callback(height(7792).IconButton, obj);
  obj5[1] = items2;
  items1[1] = closure_7(closure_4, obj5);
  const items3 = [tmp.headerBorder, animatedStyle];
  items1[2] = callback(scrollPosition(4083).View, { style: items3 });
  items1[3] = tmp13;
  obj2[1] = items1;
