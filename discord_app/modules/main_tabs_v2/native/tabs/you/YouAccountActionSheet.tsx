...
function YouStatusRadioGroup() {
  const memo = noop.useMemo(() => {
    const obj = {
      icon: closure_1_19(setting(5697).TableRowIcon, { source: closure_1(14217), variant: "text-status-online" }),
  ...
  const manaTypeConsolidationExperiment = setting(7090).useManaTypeConsolidationExperiment(
...
function YouAccountRadioGroup() {
  ...
  const memo = noop.useMemo(
    () =>
      stateFromStores.map((id) => {
        ...
        const obj3 = {
          label: multiAccountUsers(4482).getUserTag(obj, { mode: "username", identifiable: str2 }),
          value: id.id,
          subLabel: combined,
          icon: null,
        };
  ...
  const manaTypeConsolidationExperiment = stateFromStores(7090).useManaTypeConsolidationExperiment(
...
function CustomStatus() {
  ...
  if (null != customStatusActivity) {
    ...
    const obj10 = { style: tmp.trailingIcon, source: _modDef7199 };
