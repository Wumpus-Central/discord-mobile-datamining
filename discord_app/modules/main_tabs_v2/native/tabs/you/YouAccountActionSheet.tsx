...
function YouStatusRadioGroup() {
  const memo = noop.useMemo(() => {
    const obj = {
      icon: closure_1_19(setting(5828).TableRowIcon, { source: closure_1(14381), variant: "text-status-online" }),
  ...
  const manaTypeConsolidationExperiment = setting(7225).useManaTypeConsolidationExperiment(
...
function YouAccountRadioGroup() {
  ...
  const memo = noop.useMemo(
    () =>
      stateFromStores.map((id) => {
        ...
        const obj3 = {
          label: multiAccountUsers(4600).getUserTag(obj, { mode: "username", identifiable: str2 }),
          value: id.id,
          subLabel: combined,
          icon: null,
        };
  ...
  const manaTypeConsolidationExperiment = stateFromStores(7225).useManaTypeConsolidationExperiment(
