...
function YouStatusRadioGroup() {
  ...
  return callback(setting(8026).TableRadioGroup, obj);
}
function ThemeRadioGroup() {
  ...
  const callback = importAllResult.useCallback((arg0) => {
    ...
    callback(11414).resetCustomTheme();
    const obj2 = callback(11414);
    callback2(8743).updateTheme(arg0);
  }, []);
  ...
  if (tmp2) {
    ...
    obj1[2] = tmp9.DARK;
    tmp6Result = tmp6(tmp3(8025).TableRadioRow, obj1);
  }
...
function YouAccountRadioGroup() {
  ...
  if (null != stateFromStores) {
    ...
    obj1[1] = closure_19(tmp4(4340).Text, obj2);
    obj[1] = closure_19(tmp4(4886).PressableOpacity, obj1);
    ...
    obj3[4] = memo.map((value) => {
      const merged = Object.assign(value);
      return callback(multiAccountUsers(8025).TableRadioRow, {}, value.value);
    });
    items4[1] = closure_19(tmp4(8026).TableRadioGroup, obj3);
    obj[1] = items4;
...
function FocusModeSetting() {
  ...
  if (!focusModeEnabled) {
    ...
    tmp5Result[2] = callback(currentLocale(9777).BellSlashIcon, obj);
    tmp5Result[3] = function onValueChange(arg0) {
      if (arg0) {
        let obj = { onSelect: null };
        obj[0] = function onSelect(quiet_mode_enabled) {
          callback(9809).setFocusMode(quiet_mode_enabled, arg1);
          const obj = callback(9809);
          callback2(4312).hideActionSheet();
          const obj2 = callback2(4312);
    ...
    if (null == setting) {
      ...
      tmp5Result = tmp5(currentLocale(5807).TableSwitchRow, tmp5Result);
      obj1[1] = tmp5Result;
      tmp5Result = tmp5(currentLocale(5806).TableRowGroup, obj1);
    }
...
function CustomStatus() {
  ...
  obj[4] = function onPress() {
    ...
    const items = [callback2(5787).YOU_ACCOUNT_ACTION_SHEET];
    obj[0] = items;
    const result = callback(9500).openEditCustomStatusModal(obj);
  };
  let emoji1;
  if (customStatusActivity != null) {
  ...
  if (null != emoji1) {
    ...
    obj2[1] = token;
    let tmp15Result = tmp15(tmp5(9569), obj2);
  } else {
    const obj3 = { size: "md", style: null };
    obj3[1] = tmp.leadingIcon;
    tmp15Result = tmp15(tmp2(7696).ReactionIcon, obj3);
  }
  ...
  if (null != customStatusActivity) {
    ...
    tmp15Result = tmp15(tmp2(4886).PressableOpacity, obj5);
  }
