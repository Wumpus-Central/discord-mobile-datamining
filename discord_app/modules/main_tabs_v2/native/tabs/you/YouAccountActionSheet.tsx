...
function YouStatusRadioGroup() {
  const memo = noop.useMemo(() => {
    const obj = {
      icon: closure_1_19(setting(5830).TableRowIcon, { source: closure_1(14387), variant: "text-status-online" }),
  ...
  const manaTypeConsolidationExperiment = setting(7227).useManaTypeConsolidationExperiment(
...
function YouAccountRadioGroup() {
  ...
  const memo = noop.useMemo(
    () =>
      stateFromStores.map((id) => {
        ...
        const obj3 = {
          label: multiAccountUsers(4601).getUserTag(obj, { mode: "username", identifiable: str2 }),
          value: id.id,
          subLabel: combined,
          icon: null,
        };
  ...
  const manaTypeConsolidationExperiment = stateFromStores(7227).useManaTypeConsolidationExperiment(
    ...
    obj2.onValueChange = function onValueChange(arg0) {
      if (arg0) {
        ...
        require("ActionSheetActionCreators").openLazy(
          require("asyncRequireImpl")(paths[48], paths.paths),
