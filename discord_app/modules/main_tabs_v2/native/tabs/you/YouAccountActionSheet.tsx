...
function YouStatusRadioGroup() {
  const memo = noop.useMemo(() => {
    const obj = {
      icon: closure_1_19(setting(5914).TableRowIcon, { source: closure_1(14467), variant: "text-status-online" }),
  ...
  const manaTypeConsolidationExperiment = setting(7311).useManaTypeConsolidationExperiment(
...
function YouAccountRadioGroup() {
  ...
  const memo = noop.useMemo(
    () =>
      stateFromStores.map((id) => {
        ...
        const obj3 = {
          label: multiAccountUsers(4671).getUserTag(obj, { mode: "username", identifiable: str2 }),
          value: id.id,
          subLabel: combined,
          icon: null,
        };
  ...
  const manaTypeConsolidationExperiment = stateFromStores(7311).useManaTypeConsolidationExperiment(
