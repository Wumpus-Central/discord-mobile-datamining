...
function YouStatusRadioGroup() {
  const memo = noop.useMemo(() => {
    const obj = {
      icon: closure_1_19(setting(5916).TableRowIcon, { source: closure_1(14476), variant: "text-status-online" }),
  ...
  const manaTypeConsolidationExperiment = setting(7313).useManaTypeConsolidationExperiment(
...
function YouAccountRadioGroup() {
  ...
  const memo = noop.useMemo(
    () =>
      stateFromStores.map((id) => {
        ...
        const obj3 = {
          label: multiAccountUsers(4673).getUserTag(obj, { mode: "username", identifiable: str2 }),
          value: id.id,
          subLabel: combined,
          icon: null,
        };
  ...
  const manaTypeConsolidationExperiment = stateFromStores(7313).useManaTypeConsolidationExperiment(
...
function CustomStatus() {
  ...
  if (null != customStatusActivity) {
    ...
    const obj10 = { style: tmp.trailingIcon, source: _modDef7422 };
