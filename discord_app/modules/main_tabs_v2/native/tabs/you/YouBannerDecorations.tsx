...
export default noop.memo((navigateToPremium) => {
  ...
  const tmp11 = closure_13(num, containerBackground.useMemo(() => {
    ...
    if (null != containerBackground) {
      ...
      hexResult1 = obj.mix(gradientSecondaryBackground, hexResult, _modDef672(containerBackground).alpha(), "rgb").hex("rgb");
      const mixResult = obj.mix(gradientSecondaryBackground, hexResult, _modDef672(containerBackground).alpha(), "rgb");
    }
    return hexResult1;
  ...
  let result = navigateToSettings(gradientSecondaryBackground[11]).useIsDismissibleContentDismissed_UNSAFE(navigateToSettings(tmp2[12]).DismissibleContent.TRIAL_FOR_ALL_2026_SETTINGS_BADGE);
  let tmp17 = !result;
  if (!result) {
    tmp17 = tmp15;
  }
  ...
  const callback = obj3.useCallback(() => {
    ...
    if (closure_7) {
      ...
      tmp5 = !tmpResult.UNSAFE_isDismissibleContentDismissed(dismissible_content.DismissibleContent.TRIAL_FOR_ALL_2026_SETTINGS_BADGE);
    }
    if (tmp5) {
      tmpResult = DismissibleContentUnsafeUtils;
      const result1 = tmpResult.UNSAFE_markDismissibleContentAsDismissed(dismissible_content.DismissibleContent.TRIAL_FOR_ALL_2026_SETTINGS_BADGE);
    }
  }, items2);
