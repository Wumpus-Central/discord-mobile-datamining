...
function UnconnectedYouScreen(arg0) {
  ...
  if (isFocused) {
    ...
    let tmp37 = null == obj3;
    const userProfileBannerBackgroundColor = navigateToSettings(8366).useUserProfileBannerBackgroundColor(obj8);
    if (!tmp37) {
      tmp37 = !obj3.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result17 = navigateToSettings(8366);
    ...
    const tmp38 = navigateToShop(7050)();
    const obj9 = { layout: "YOU_SCREEN", userId: user.id };
    const createUserProfileAnalyticsContext = navigateToSettings(8310).useCreateUserProfileAnalyticsContext(obj9);
    const tmp12Result18 = navigateToSettings(8310);
    const isScreenLandscape = navigateToSettings(5210).useIsScreenLandscape();
    let tmp43;
    const tmp12Result19 = navigateToSettings(5210);
    ...
    const tmp5Result1Result = navigateToShop(8321)(tmp43);
    let tmp47;
    const tmp5Result7 = navigateToShop(8321);
    if (!isScreenLandscape) {
      let skuId1;
      if (obj3 != null) {
    ...
    const items1 = [navigateToShop(7289).YOU_SCREEN];
    obj10.analyticsLocations = items1;
    obj10.context = createUserProfileAnalyticsContext;
    navigateToShop(8333)(obj10);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = navigateToShop(8343)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      const tmpResult4 = tmp(navigateToSettings(7495).useSelectedDismissibleContent(memo1), 2);
      closure_10 = tmp58;
      ...
      const tmp12Result21 = navigateToSettings(7495);
      const obj11 = { disabled: tmp63 };
      youSettingsCoachmark = navigateToSettings(16937).useYouSettingsCoachmark(obj11);
      let tmp65 = null != youSettingsCoachmark;
      const tmp12Result22 = navigateToSettings(16937);
      const customTypingIndicatorConfig = navigateToSettings(12101).useCustomTypingIndicatorConfig("YouScreen");
        ...
        if (customTypingIndicatorConfig.canSet) {
          if (null != obj3) {
            if (!tmp63) {
              ...
              const tmpResult5 = tmp(navigateToSettings(7495).useSelectedDismissibleContent(items5), 2);
              closure_13 = tmp68;
              ...
              const tmp12Result24 = navigateToSettings(7495);
              ...
              if (tmp70) {
                ...
                if (tmp72) {
                  if (tmp63) {
                    ...
                    tmp63 = closure_20(navigateToShop(16940), obj12);
                  }
                  const items6 = [tmp63, , ];
                  if (tmp65) {
                    const obj13 = { buttonRef: ref5 };
                    const merged = Object.assign(youSettingsCoachmark.props);
                    tmp65 = closure_20(navigateToShop(16937), obj13);
                    const tmp5Result9 = navigateToShop(16937);
                  }
                  items6[1] = tmp65;
                  let tmp80 = tmp69;
                  if (tmp69) {
                    const obj14 = { targetRef: ref5, visible: tmp69, markAsDismissed: tmp68, position: "top" };
                    tmp80 = closure_20(navigateToShop(16941), obj14);
                  }
                  const obj15 = { zIndex: 1, children: null };
                  items6[2] = tmp80;
                  obj15.children = items6;
                  tmp74Result = closure_21(navigateToSettings(7263).LayerScope, obj15);
                }
              }
              const items7 = [tmp70];
              ...
              const tmp85 = closure_20(navigateToShop(16942), obj16);
              if (tmp84Result) {
                const obj20 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: navigateToSettings(8327).ProfileFrameLayerOrder.BACK, containerWidth: bound };
                tmp84Result = closure_20(navigateToShop(8340), obj20);
                const tmp5Result11 = navigateToShop(8340);
              }
              const items10 = [tmp84Result, , , , , ];
              const obj21 = { gradientHeight: height, bannerHeight: tmp7Result, style: tmp10.background };
              items10[1] = closure_20(navigateToShop(8356), obj21);
              ...
              if (null == source) {
                ...
                if (tmp89Result) {
                  ...
                  const items18 = [closure_20(navigateToShop(8931), obj31), ];
                ...
                items16[3] = closure_20(navigateToSettings(12031).TTIFirstContentfulPaint, { label: "you_screen" });
                obj22.children = items16;
                items10[2] = closure_21(closure_25, obj22);
                let tmp84Result7 = null != tmp5Result1Result;
                if (tmp84Result7) {
                  const obj34 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: navigateToSettings(8327).ProfileFrameLayerOrder.FRONT, containerWidth: bound };
                  tmp84Result7 = closure_20(navigateToShop(8340), obj34);
                  const tmp5Result12 = navigateToShop(8340);
                }
                ...
                obj17.children = closure_20(navigateToSettings(8310).UserProfileAnalyticsProvider, obj18);
                obj35.children = closure_20(navigateToSettings(4350).ThemeContextProvider, obj17);
                return closure_20(navigateToSettings(7263).LayerScope, obj35);
                ...
                const items20 = [closure_20(navigateToShop(5672), obj37), ];
                ...
                if (!obj38) {
                  ...
                  tmp84Result8 = closure_20(navigateToSettings(10784).Caption, obj38);
                }
                items20[1] = tmp84Result8;
                obj36.children = items20;
                let tmp89Result2 = closure_21(navigateToSettings(5207).PressableOpacity, obj36);
              } else {
                ...
                tmp89Result2 = closure_20(navigateToShop(5672), obj39);
              }
      ...
      items5 = [];
      const tmp12Result23 = navigateToSettings(12101);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result8 = navigateToShop(8333);
  }
  if (!isFocused) {
    ref3.current = false;
  }
