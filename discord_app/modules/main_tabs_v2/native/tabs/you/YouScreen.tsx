...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8719)(tmp8);
  callback = tmp7Result;
  const tmp10 = callback3(tmp7Result);
  obj = { type: null, name: null };
  let tmp5Result = tmp5(9238);
  obj[0] = navigateToSettings(503).ImpressionTypes.VIEW;
  obj[1] = navigateToSettings(503).ImpressionNames.USER_YOU_SCREEN;
  tmp5Result(obj);
  let obj2 = tmp5(8089)(user.id);
  rect = tmp5(1581)();
  const tmp7 = navigateToShop(8719);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8717)({ user, displayProfile: obj2 }));
  const tmp14 = navigateToShop(8717)({ user, displayProfile: obj2 });
  ...
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8731)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  ...
  tmp12Result = tmp12(15714);
  let tmp28 = !isFocused;
  if (!isFocused) {
    tmp28 = !tmp12Result.useIsProfileModalTransitioning();
  }
  const tmp19 = navigateToShop(8731)({ scrollPosition: sharedValue, bannerHeight: tmp7Result });
  ...
  if (isFocused) {
    ...
    obj2[1] = obj2;
    let tmp37 = null == obj2;
    const userProfileBannerBackgroundColor = tmp12(8085).useUserProfileBannerBackgroundColor(obj2);
    if (!tmp37) {
      tmp37 = !obj2.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result3 = tmp12(8085);
    ...
    const tmp38 = tmp5(7534)();
    obj3 = { layout: "YOU_SCREEN", userId: null };
    obj3[1] = user.id;
    const createUserProfileAnalyticsContext = tmp12(8682).useCreateUserProfileAnalyticsContext(obj3);
    const tmp12Result4 = tmp12(8682);
    const isScreenLandscape = tmp12(4800).useIsScreenLandscape();
    let tmp43;
    ...
    obj4[0] = tmp47;
    obj4[1] = ref2.current;
    const items1 = [tmp5(5615).YOU_SCREEN];
    obj4[2] = items1;
    obj4[3] = createUserProfileAnalyticsContext;
    tmp5(8704)(obj4);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = tmp5(8714)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      tmpResult = tmp(tmp12(5916).useSelectedDismissibleContent(memo1), 2);
      c10 = tmp58;
      ...
      const tmp12Result7 = tmp12(5916);
      const obj5 = { disabled: null };
      obj5[0] = tmp63;
      youSettingsCoachmark = tmp12(15715).useYouSettingsCoachmark(obj5);
      ...
      c13 = tmp66;
      const tmp12Result8 = tmp12(15715);
      ...
      if (tmp66) {
        ...
        if (tmp68) {
          if (tmp63) {
            ...
            obj6[8] = callback1;
            tmp63 = callback2(tmp5(15718), obj6);
          }
          const items5 = [tmp63, ];
          if (tmp65) {
            ...
            tmp65 = callback2(tmp5(15715), obj7);
            const tmp5Result2 = tmp5(15715);
          }
          const obj8 = { zIndex: 1, children: null };
          items5[1] = tmp65;
          obj8[1] = items5;
          tmp70Result = closure_21(tmp12(5582).LayerScope, obj8);
      ...
      const tmp79 = callback2(tmp5(15719), obj9);
      if (tmp78Result) {
        ...
        obj13[2] = tmp12(8697).ProfileFrameLayerOrder.BACK;
        obj13[3] = bound;
        tmp78Result = tmp78(tmp5(8711), obj13);
        const tmp5Result4 = tmp5(8711);
      }
      ...
      items9[1] = callback2(tmp5(8725), obj14);
      ...
      if (null == source) {
        ...
        if (tmp78Result1) {
          ...
          tmp78Result1 = tmp78(module_8668, obj21);
        }
        ...
        if (tmp82Result) {
          ...
          obj24[0] = skuId2;
          obj24[3] = tmp28;
          const items17 = [tmp78(tmp5(9268), obj24), ];
          tmp = BackButton;
        ...
        obj26[9] = initialTab;
        obj26[10] = !tmp28;
        items15[2] = tmp78(tmp5(15729), obj26);
        items15[3] = tmp78(tmp12(11179).TTIFirstContentfulPaint, { label: "you_screen" });
        obj15[6] = items15;
        ...
        if (tmp) {
          ...
          tmp = tmp5(8711);
          obj27[2] = tmp12(8697).ProfileFrameLayerOrder.FRONT;
          obj27[3] = bound;
          tmp = tmp78(tmp, obj27);
        }
        ...
        obj10[3] = tmp78(tmp12(8682).UserProfileAnalyticsProvider, obj11);
        obj28[0] = tmp78(tmp12(3967).ThemeContextProvider, obj10);
        return tmp78(tmp12(5582).LayerScope, obj28);
        ...
        obj30[3] = source;
        obj30[4] = tmp28;
        const items19 = [tmp78(tmp5(5221), obj30), ];
        ...
        if (!obj31) {
          ...
          tmp78Result2 = tmp78(tmp12(8735).Caption, obj31);
        }
        items19[1] = tmp78Result2;
        obj29[3] = items19;
        tmp82Result = tmp82(tmp12(4797).PressableOpacity, obj29);
      } else {
        ...
        obj32[3] = source;
        obj32[4] = tmp28;
        tmp82Result = tmp78(tmp5(5221), obj32);
      }
      const tmp12Result9 = tmp12(688);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result1 = tmp5(8704);
  }
  if (!isFocused) {
    ref3.current = false;
  }
