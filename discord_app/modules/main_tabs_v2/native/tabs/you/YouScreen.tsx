...
import { CollectiblesMobileShopScreen as closure_17 } from "../../../../collectibles/CollectiblesShopConstants.tsx";
...
function BackButton(paddingTop) {
  ...
  if (stateFromStores > 0) {
    ...
    let formatToPlainStringResult = intl2.formatToPlainString(getSystemLocale.t.vxFYaM, obj);
  } else {
    const intl = getSystemLocale.intl;
    formatToPlainStringResult = intl.string(getSystemLocale.t["13/7kX"]);
  }
  obj = { style: items1, accessibilityRole: "button", accessibilityLabel: formatToPlainStringResult, onPress: handleBackButtonPress, children: callback2(IconWithBadge.CloseIconWithBadgeOnSide, { count: stateFromStores }) };
...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8972)(tmp8);
  callback = tmp7Result;
  const tmp10 = callback3(tmp7Result);
  obj = { type: null, name: null };
  let tmp5Result = navigateToShop(9177);
  obj[0] = navigateToSettings(503).ImpressionTypes.VIEW;
  obj[1] = navigateToSettings(503).ImpressionNames.USER_YOU_SCREEN;
  tmp5Result(obj);
  let obj2 = navigateToShop(8416)(user.id);
  rect = navigateToShop(1629)();
  const tmp7 = navigateToShop(8972);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8970)({ user, displayProfile: obj2 }));
  const tmp14 = navigateToShop(8970)({ user, displayProfile: obj2 });
  const ref = obj.useRef(null);
  let obj3 = navigateToSettings(4115);
  ...
  const tmp15 = navigateToShop(4310)();
  const animatedScrollHandler = obj4.useAnimatedScrollHandler(F);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8984)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  const size = navigateToShop(1494)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4232)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  const GifAutoPlay = navigateToSettings(4066).GifAutoPlay;
  const setting = GifAutoPlay.getSetting();
  let tmp12Result = navigateToSettings(1501);
  const isFocused = tmp12Result.useIsFocused();
  tmp12Result = navigateToSettings(16004);
  let tmp28 = !isFocused;
  if (!isFocused) {
    tmp28 = !tmp12Result.useIsProfileModalTransitioning();
  }
  const tmp19 = navigateToShop(8984)({ scrollPosition: sharedValue, bannerHeight: tmp7Result });
  ...
  if (isFocused) {
    ...
    let source = null;
    if (null != bannerURL) {
      source = navigateToSettings(1435).makeSource(bannerURL);
      const tmp12Result1 = navigateToSettings(1435);
    }
    const tmp12Result2 = navigateToSettings(1435);
    let intl = navigateToSettings(1236).intl;
    obj1 = { username: null };
    obj1[0] = user.username;
    const formatToPlainStringResult = intl.formatToPlainString(navigateToSettings(1236).t.gVn4uJ, obj1);
    const isAnimatedImageURLResult = navigateToSettings(1435).isAnimatedImageURL(bannerURL);
    obj2 = { user: null, displayProfile: null };
    obj2[0] = user;
    obj2[1] = obj2;
    let tmp37 = null == obj2;
    const userProfileBannerBackgroundColor = navigateToSettings(8412).useUserProfileBannerBackgroundColor(obj2);
    if (!tmp37) {
      tmp37 = !obj2.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result3 = navigateToSettings(8412);
    const memo = obj.useMemo(() => {
      const obj = { paddingBottom: rect.bottom + navigateToShop(_undefined[18]).space.PX_64 };
      obj[1] = obj;
      return obj;
    }, items);
    ({ dimensionStyle, contentContainerStyle } = memo);
    const tmp38 = navigateToShop(7925)();
    obj3 = { layout: "YOU_SCREEN", userId: null };
    obj3[1] = user.id;
    const createUserProfileAnalyticsContext = navigateToSettings(8934).useCreateUserProfileAnalyticsContext(obj3);
    const tmp12Result4 = navigateToSettings(8934);
    const isScreenLandscape = navigateToSettings(8944).useIsScreenLandscape();
    let tmp43;
    tmp5Result = navigateToShop(8945);
    ...
    let tmp47;
    const tmp12Result5 = navigateToSettings(8944);
    if (!isScreenLandscape) {
      let skuId1;
      if (obj2 != null) {
    ...
    obj4[0] = tmp47;
    obj4[1] = ref2.current;
    const items1 = [navigateToShop(7159).YOU_SCREEN];
    obj4[2] = items1;
    obj4[3] = createUserProfileAnalyticsContext;
    navigateToShop(8958)(obj4);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = navigateToShop(8967)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      tmp12Result6 = navigateToSettings(501);
      tmpResult = tmp(navigateToSettings(7366).useSelectedDismissibleContent(memo1), 2);
      closure_10 = tmp58;
      ...
      const callback1 = obj.useCallback(() => {
        navigateToShop();
        callback2(ContentDismissActionType.TAKE_ACTION);
      }, items4);
      let tmp63 = null != memo2;
      const tmp12Result7 = navigateToSettings(7366);
      const obj5 = { disabled: null };
      obj5[0] = tmp63;
      youSettingsCoachmark = navigateToSettings(16005).useYouSettingsCoachmark(obj5);
      ...
      closure_13 = tmp66;
      const tmp12Result8 = navigateToSettings(16005);
      ...
      if (tmp66) {
        ...
        if (tmp68) {
          if (tmp63) {
            ...
            obj6[8] = callback1;
            tmp63 = callback2(navigateToShop(16008), obj6);
          }
          const items5 = [tmp63, ];
          if (tmp65) {
            ...
            tmp65 = callback2(navigateToShop(16005), obj7);
            const tmp5Result2 = navigateToShop(16005);
          }
          const obj8 = { zIndex: 1, children: null };
          items5[1] = tmp65;
          obj8[1] = items5;
          tmp70Result = callback(navigateToSettings(7126).LayerScope, obj8);
        }
      }
      const items6 = [tmp66];
      ...
      const tmp79 = callback2(navigateToShop(16009), obj9);
      if (tmp78Result) {
        ...
        obj13[2] = navigateToSettings(8951).ProfileFrameLayerOrder.BACK;
        obj13[3] = bound;
        tmp78Result = tmp78(navigateToShop(8964), obj13);
        const tmp5Result4 = navigateToShop(8964);
      }
      ...
      items9[1] = callback2(navigateToShop(8978), obj14);
      ...
      const tmp5Result3 = navigateToShop(16017);
      const tmp93 = sharedValue;
      obj20[0] = navigateToSettings(688).int2hex(userProfileBannerBackgroundColor);
      items13[1] = obj20;
      obj19[0] = items13;
      const items14 = [callback2(bound, obj19), , ];
      if (null == source) {
        items14[1] = tmp94;
        let tmp78Result1 = navigateToSettings(501).isIOS() && showBlur;
        if (tmp78Result1) {
        ...
        obj18[1] = items14;
        items11[1] = callback(navigateToShop(4115).View, obj18);
        obj16[1] = items11;
        const items15 = [callback(navigateToShop(4115).View, obj16), , , ];
        if (tmp82Result) {
          ...
          obj24[0] = skuId2;
          obj24[3] = tmp28;
          const items17 = [tmp78(navigateToShop(9210), obj24), ];
          const obj25 = { paddingTop: null };
          obj25[0] = bound1;
          items17[1] = tmp78(BackButton, obj25);
          obj22[2] = items17;
          tmp82Result = callback(navigateToShop(4115).View, obj22);
        }
        ...
        obj26[9] = initialTab;
        obj26[10] = !tmp28;
        items15[2] = tmp78(navigateToShop(16019), obj26);
        items15[3] = tmp78(navigateToSettings(11075).TTIFirstContentfulPaint, { label: "you_screen" });
        obj15[6] = items15;
        items9[2] = callback(closure_25, obj15);
        let tmp78Result2 = null != tmp5Result1Result;
        if (tmp78Result2) {
          ...
          obj27[2] = navigateToSettings(8951).ProfileFrameLayerOrder.FRONT;
          obj27[3] = bound;
          tmp78Result2 = tmp78(navigateToShop(8964), obj27);
          const tmp5Result5 = navigateToShop(8964);
        }
        const obj28 = { children: null };
        items9[3] = tmp78Result2;
        items9[4] = tmp79;
        items9[5] = tmp70Result;
        obj12[2] = items9;
        obj11[5] = callback(tmp5Result3, obj12);
        obj10[3] = tmp78(navigateToSettings(8934).UserProfileAnalyticsProvider, obj11);
        obj28[0] = tmp78(navigateToSettings(4104).ThemeContextProvider, obj10);
        return tmp78(navigateToSettings(7126).LayerScope, obj28);
        ...
        let intl2 = navigateToSettings(1236).intl;
        obj29[2] = intl2.string(navigateToSettings(1236).t["3fzj/l"]);
        ...
        obj30[3] = source;
        obj30[4] = tmp28;
        const items19 = [tmp78(navigateToShop(5449), obj30), ];
        let tmp78Result3 = !obj31;
        if (!obj31) {
          obj31 = { label: null, style: null, textStyle: null };
          const intl3 = navigateToSettings(1236).intl;
          obj31[0] = intl3.string(navigateToSettings(1236).t.I5gL2H);
          ...
          tmp78Result3 = tmp78(navigateToSettings(10285).Caption, obj31);
        }
        items19[1] = tmp78Result3;
        obj29[3] = items19;
        tmp82Result = callback(navigateToSettings(5433).PressableOpacity, obj29);
      } else {
        ...
        obj32[3] = source;
        obj32[4] = tmp28;
        tmp82Result = tmp78(navigateToShop(5449), obj32);
      }
      const tmp12Result9 = navigateToSettings(688);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result1 = navigateToShop(8958);
  }
  if (!isFocused) {
    ref3.current = false;
  }
...
let closure_24 = createCacheKey.createStyles(() => {
  const obj = { position: "absolute", marginTop: ThemesDefault.space.PX_4, left: ThemesDefault.space.PX_16, zIndex: 99, alignItems: "center", justifyContent: "center" };
  obj[0] = obj;
  return obj;
});
...
let result = require("obj132").fileFinishedImporting("modules/main_tabs_v2/native/tabs/you/YouScreen.tsx");
...
export default function YouScreen(initialTab) {
  ...
  const callback1 = React.useCallback(() => {
    stateFromStores(memo[66]);
    const obj = { screen: constants.PREMIUM };
    obj.openUserSettings(obj);
  }, []);
  const callback2 = React.useCallback(() => {
    stateFromStores(memo[67]);
