...
import YouBannerDecorations from "YouBannerDecorations" /* 17356 */;
...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8578)(tmp8);
  _slicedToArray = tmp7Result;
  const tmp10 = closure_23(tmp7Result);
  let obj2 = { type: null, name: null };
  const tmp7 = navigateToShop(8578);
  obj2.type = navigateToSettings(1249).ImpressionTypes.VIEW;
  obj2.name = navigateToSettings(1249).ImpressionNames.USER_YOU_SCREEN;
  navigateToShop(9128)(obj2);
  const obj3 = navigateToShop(8534)(user.id);
  rect = navigateToShop(1612)();
  const tmp5Result = navigateToShop(9128);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8575)({ user, displayProfile: obj3 }));
  const tmp14 = navigateToShop(8575)({ user, displayProfile: obj3 });
  const ref = rect.useRef(null);
  const tmp15 = navigateToShop(4763)();
  sharedValue = navigateToSettings(4561).useSharedValue(0);
  const obj4 = navigateToSettings(4561);
  class N {
    constructor(arg0) {
  ...
  const obj5 = navigateToSettings(4561);
  const animatedScrollHandler = navigateToSettings(4561).useAnimatedScrollHandler(N);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8591)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  let size = navigateToShop(1478)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4689)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  const tmp19 = navigateToShop(8591)({ scrollPosition: sharedValue, bannerHeight: tmp7Result });
  ...
  tmp12Result14 = navigateToSettings(17355);
  ...
  if (isFocused) {
    ...
    let tmp37 = null == obj3;
    const userProfileBannerBackgroundColor = navigateToSettings(8595).useUserProfileBannerBackgroundColor(obj8);
    if (!tmp37) {
      tmp37 = !obj3.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result17 = navigateToSettings(8595);
    ...
    const tmp38 = navigateToShop(7276)();
    const obj9 = { layout: "YOU_SCREEN", userId: user.id };
    const createUserProfileAnalyticsContext = navigateToSettings(8538).useCreateUserProfileAnalyticsContext(obj9);
    const tmp12Result18 = navigateToSettings(8538);
    const isScreenLandscape = navigateToSettings(5430).useIsScreenLandscape();
    let tmp43;
    const tmp12Result19 = navigateToSettings(5430);
    ...
    const tmp5Result1Result = navigateToShop(8549)(tmp43);
    let tmp47;
    const tmp5Result7 = navigateToShop(8549);
    if (!isScreenLandscape) {
      let skuId1;
      if (obj3 != null) {
    ...
    const items1 = [navigateToShop(7515).YOU_SCREEN];
    obj10.analyticsLocations = items1;
    obj10.context = createUserProfileAnalyticsContext;
    navigateToShop(8561)(obj10);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = navigateToShop(8572)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      const tmpResult4 = tmp(navigateToSettings(7718).useSelectedDismissibleContent(memo1), 2);
      closure_10 = tmp58;
      ...
      const tmp12Result21 = navigateToSettings(7718);
      const obj11 = { disabled: tmp63 };
      youSettingsCoachmark = navigateToSettings(17364).useYouSettingsCoachmark(obj11);
      let tmp65 = null != youSettingsCoachmark;
      const tmp12Result22 = navigateToSettings(17364);
      const customTypingIndicatorConfig = navigateToSettings(12309).useCustomTypingIndicatorConfig("YouScreen");
      if ("settings" === customTypingIndicatorConfig.entryPoint) {
        if (customTypingIndicatorConfig.canSet) {
          if (null != obj3) {
            if (!tmp63) {
              if (!tmp65) {
                let items5 = [navigateToSettings(2028).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_PROFILE_COACHMARK];
              }
              const tmpResult5 = tmp(navigateToSettings(7718).useSelectedDismissibleContent(items5), 2);
              closure_13 = tmp68;
              const tmp69 = tmpResult5[0] === navigateToSettings(2028).DismissibleContent.CUSTOM_TYPING_INDICATOR_MOBILE_PROFILE_COACHMARK;
              nativeID = tmp69;
              let tmp70 = tmp63;
              if (!tmp63) {
                tmp70 = tmp65;
              ...
              const tmp12Result24 = navigateToSettings(7718);
              ...
              if (tmp70) {
                ...
                if (tmp72) {
                  if (tmp63) {
                    ...
                    tmp63 = closure_20(navigateToShop(17367), obj12);
                  }
                  const items6 = [tmp63, , ];
                  if (tmp65) {
                    const obj13 = { buttonRef: ref5 };
                    const merged = Object.assign(youSettingsCoachmark.props);
                    tmp65 = closure_20(navigateToShop(17364), obj13);
                    const tmp5Result9 = navigateToShop(17364);
                  }
                  items6[1] = tmp65;
                  let tmp80 = tmp69;
                  if (tmp69) {
                    const obj14 = { targetRef: ref5, visible: tmp69, markAsDismissed: tmp68, position: "top" };
                    tmp80 = closure_20(navigateToShop(17368), obj14);
                  }
                  const obj15 = { zIndex: 1, children: null };
                  items6[2] = tmp80;
                  obj15.children = items6;
                  tmp74Result = closure_21(navigateToSettings(7489).LayerScope, obj15);
                }
              }
              const items7 = [tmp70];
              ...
              const tmp85 = closure_20(navigateToShop(17356), obj16);
              if (tmp84Result) {
                const obj20 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: navigateToSettings(8555).ProfileFrameLayerOrder.BACK, containerWidth: bound };
                tmp84Result = closure_20(navigateToShop(8569), obj20);
                const tmp5Result11 = navigateToShop(8569);
              }
              ...
              items10[1] = closure_20(navigateToShop(8585), tmp95);
              ...
              const tmp5Result10 = navigateToShop(17369);
              ...
              if (null == source) {
                ...
                items12[1] = closure_21(navigateToShop(4561).View, obj24);
                obj22.children = items12;
                const items16 = [closure_21(navigateToShop(4561).View, obj22), , , ];
                if (tmp89Result) {
                  ...
                  const items18 = [closure_20(navigateToShop(9162), obj30), ];
                  const obj31 = { paddingTop: bound1 };
                  items18[1] = closure_20(BackButton, obj31);
                  obj29.children = items18;
                  tmp89Result = closure_21(navigateToShop(4561).View, obj29);
                }
                ...
                items16[2] = closure_20(navigateToShop(17371), obj32);
                items16[3] = closure_20(navigateToSettings(12239).TTIFirstContentfulPaint, { label: "you_screen" });
                obj21.children = items16;
                items10[2] = closure_21(closure_25, obj21);
                let tmp84Result7 = null != tmp5Result1Result;
                if (tmp84Result7) {
                  const obj33 = { frame: tmp5Result1Result, profileThemeType: UserProfileThemeTypes.YOU_SCREEN, frameOrder: navigateToSettings(8555).ProfileFrameLayerOrder.FRONT, containerWidth: bound };
                  tmp84Result7 = closure_20(navigateToShop(8569), obj33);
                  const tmp5Result12 = navigateToShop(8569);
                }
                ...
                obj17.children = closure_20(navigateToSettings(8538).UserProfileAnalyticsProvider, obj18);
                obj34.children = closure_20(navigateToSettings(4535).ThemeContextProvider, obj17);
                return closure_20(navigateToSettings(7489).LayerScope, obj34);
                ...
                const items20 = [closure_20(navigateToShop(5892), obj36), ];
                ...
                if (!obj37) {
                  ...
                  tmp84Result8 = closure_20(navigateToSettings(10999).Caption, obj37);
                }
                items20[1] = tmp84Result8;
                obj35.children = items20;
                let tmp89Result2 = closure_21(navigateToSettings(5427).PressableOpacity, obj35);
              } else {
                ...
                tmp89Result2 = closure_20(navigateToShop(5892), obj38);
              }
      ...
      items5 = [];
      const tmp12Result23 = navigateToSettings(12309);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result8 = navigateToShop(8561);
  }
  if (!isFocused) {
    ref3.current = false;
  }
