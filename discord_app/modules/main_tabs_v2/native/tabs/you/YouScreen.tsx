...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8748)(tmp8);
  callback = tmp7Result;
  ...
  tmp5Result(obj);
  let obj2 = tmp5(8230)(user.id);
  rect = tmp5(1609)();
  const tmp7 = navigateToShop(8748);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8746)({ user, displayProfile: obj2 }));
  const tmp14 = navigateToShop(8746)({ user, displayProfile: obj2 });
  const ref = obj.useRef(null);
  let obj3 = navigateToSettings(4145);
  sharedValue = obj3.useSharedValue(0);
  let obj4 = navigateToSettings(4145);
  class F {
    constructor(arg0) {
  ...
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8760)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  const size = tmp5(1474)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4138)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  if (isFocused) {
    ...
    obj2[1] = obj2;
    let tmp37 = null == obj2;
    const userProfileBannerBackgroundColor = tmp12(8226).useUserProfileBannerBackgroundColor(obj2);
    if (!tmp37) {
      tmp37 = !obj2.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result3 = tmp12(8226);
    ...
    obj3[1] = user.id;
    const createUserProfileAnalyticsContext = tmp12(8711).useCreateUserProfileAnalyticsContext(obj3);
    const tmp12Result4 = tmp12(8711);
    const isScreenLandscape = tmp12(4830).useIsScreenLandscape();
    let tmp43;
    tmp5Result = tmp5(8720);
    ...
    obj4[0] = tmp47;
    obj4[1] = ref2.current;
    const items1 = [tmp5(5669).YOU_SCREEN];
    obj4[2] = items1;
    obj4[3] = createUserProfileAnalyticsContext;
    tmp5(8733)(obj4);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = tmp5(8743)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      tmpResult = tmp(tmp12(5943).useSelectedDismissibleContent(memo1), 2);
      c10 = tmp58;
      ...
      const tmp12Result7 = tmp12(5943);
      const obj5 = { disabled: null };
      obj5[0] = tmp63;
      youSettingsCoachmark = tmp12(15755).useYouSettingsCoachmark(obj5);
      ...
      c13 = tmp66;
      const tmp12Result8 = tmp12(15755);
      ...
      if (tmp66) {
        ...
        if (tmp68) {
          if (tmp63) {
            ...
            obj6[8] = callback1;
            tmp63 = callback2(tmp5(15758), obj6);
          }
          const items5 = [tmp63, ];
          if (tmp65) {
            ...
            tmp65 = callback2(tmp5(15755), obj7);
            const tmp5Result2 = tmp5(15755);
          }
          const obj8 = { zIndex: 1, children: null };
          items5[1] = tmp65;
          obj8[1] = items5;
          tmp70Result = closure_21(tmp12(5636).LayerScope, obj8);
      ...
      const tmp79 = callback2(tmp5(15759), obj9);
      if (tmp78Result) {
        ...
        obj13[2] = tmp12(8726).ProfileFrameLayerOrder.BACK;
        obj13[3] = bound;
        tmp78Result = tmp78(tmp5(8740), obj13);
        const tmp5Result4 = tmp5(8740);
      }
      ...
      items9[1] = callback2(tmp5(8754), obj14);
      ...
      if (null == source) {
        ...
        if (tmp78Result1) {
          ...
          tmp78Result1 = tmp78(module_8697, obj21);
        }
        items14[2] = tmp78Result1;
        obj18[1] = items14;
        items11[1] = tmp82(tmp5(4145).View, obj18);
        obj16[1] = items11;
        const items15 = [tmp82(tmp5(4145).View, obj16), , , ];
        if (tmp82Result) {
          ...
          obj22[2] = items17;
          tmp82Result = tmp82(tmp5(4145).View, obj22);
        }
        ...
        obj26[9] = initialTab;
        obj26[10] = !tmp28;
        items15[2] = tmp78(tmp5(15769), obj26);
        items15[3] = tmp78(tmp12(11213).TTIFirstContentfulPaint, { label: "you_screen" });
        obj15[6] = items15;
        ...
        if (tmp) {
          ...
          tmp = tmp5(8740);
          obj27[2] = tmp12(8726).ProfileFrameLayerOrder.FRONT;
          obj27[3] = bound;
          tmp = tmp78(tmp, obj27);
        }
        ...
        obj10[3] = tmp78(tmp12(8711).UserProfileAnalyticsProvider, obj11);
        obj28[0] = tmp78(tmp12(3996).ThemeContextProvider, obj10);
        return tmp78(tmp12(5636).LayerScope, obj28);
        ...
        obj30[3] = source;
        obj30[4] = tmp28;
        const items19 = [tmp78(tmp5(5251), obj30), ];
        ...
        if (!obj31) {
          ...
          tmp78Result2 = tmp78(tmp12(8764).Caption, obj31);
        }
        items19[1] = tmp78Result2;
        obj29[3] = items19;
        tmp82Result = tmp82(tmp12(4827).PressableOpacity, obj29);
      } else {
        ...
        obj32[3] = source;
        obj32[4] = tmp28;
        tmp82Result = tmp78(tmp5(5251), obj32);
      }
      const tmp12Result9 = tmp12(688);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result1 = tmp5(8733);
  }
  if (!isFocused) {
    ref3.current = false;
  }
...
let closure_25 = require("module_4145").createAnimatedComponent(ScrollView);
