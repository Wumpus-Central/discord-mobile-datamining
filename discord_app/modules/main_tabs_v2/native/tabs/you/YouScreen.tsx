// === Module 15702: handleBackButtonPress ===
...
function handleBackButtonPress() {
  const navigation = require(4107) /* getRootNavigationRef */.getRootNavigationRef();
  if (null != navigation) {
...
function BackButton(paddingTop) {
  ...
  obj[4] = callback2(require(15362) /* IconWithBadge */.CloseIconWithBadgeOnSide, { count: stateFromStores });
...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8682)(tmp8);
  callback = tmp7Result;
  const tmp10 = callback3(tmp7Result);
  obj = { type: null, name: null };
  let tmp5Result = tmp5(8741);
  obj[0] = navigateToSettings(503).ImpressionTypes.VIEW;
  obj[1] = navigateToSettings(503).ImpressionNames.USER_YOU_SCREEN;
  tmp5Result(obj);
  let obj2 = tmp5(8105)(user.id);
  rect = tmp5(1581)();
  const tmp7 = navigateToShop(8682);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8412)({ user, displayProfile: obj2 }));
  const tmp14 = navigateToShop(8412)({ user, displayProfile: obj2 });
  const ref = obj.useRef(null);
  let obj3 = navigateToSettings(4116);
  sharedValue = obj3.useSharedValue(0);
  let obj4 = navigateToSettings(4116);
  class F {
    constructor(arg0) {
  ...
  const tmp15 = navigateToShop(4191)();
  const animatedScrollHandler = obj4.useAnimatedScrollHandler(F);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8691)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  const size = tmp5(1474)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4109)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  if (isFocused) {
    ...
    obj2[1] = obj2;
    let tmp37 = null == obj2;
    const userProfileBannerBackgroundColor = tmp12(8101).useUserProfileBannerBackgroundColor(obj2);
    if (!tmp37) {
      tmp37 = !obj2.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result3 = tmp12(8101);
    ...
    const tmp38 = tmp5(7550)();
    obj3 = { layout: "YOU_SCREEN", userId: null };
    obj3[1] = user.id;
    const createUserProfileAnalyticsContext = tmp12(8442).useCreateUserProfileAnalyticsContext(obj3);
    const tmp12Result4 = tmp12(8442);
    const isScreenLandscape = tmp12(4786).useIsScreenLandscape();
    let tmp43;
    tmp5Result = tmp5(8417);
    ...
    obj4[0] = tmp47;
    obj4[1] = ref2.current;
    const items1 = [tmp5(5601).YOU_SCREEN];
    obj4[2] = items1;
    obj4[3] = createUserProfileAnalyticsContext;
    tmp5(8675)(obj4);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = tmp5(8431)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      tmpResult = tmp(tmp12(5921).useSelectedDismissibleContent(memo1), 2);
      c10 = tmp58;
      ...
      const tmp12Result7 = tmp12(5921);
      const obj5 = { disabled: null };
      obj5[0] = tmp63;
      youSettingsCoachmark = tmp12(15704).useYouSettingsCoachmark(obj5);
      ...
      c13 = tmp66;
      const tmp12Result8 = tmp12(15704);
      ...
      if (tmp66) {
        ...
        if (tmp68) {
          if (tmp63) {
            ...
            obj6[8] = callback1;
            tmp63 = callback2(tmp5(15707), obj6);
          }
          const items5 = [tmp63, ];
          if (tmp65) {
            ...
            tmp65 = callback2(tmp5(15704), obj7);
            const tmp5Result2 = tmp5(15704);
          }
          const obj8 = { zIndex: 1, children: null };
          items5[1] = tmp65;
          obj8[1] = items5;
          tmp70Result = closure_21(tmp12(5568).LayerScope, obj8);
      ...
      const tmp79 = callback2(tmp5(15708), obj9);
      if (tmp78Result) {
        ...
        obj13[2] = tmp12(8423).ProfileFrameLayerOrder.BACK;
        obj13[3] = bound;
        tmp78Result = tmp78(tmp5(8432), obj13);
        const tmp5Result4 = tmp5(8432);
      }
      ...
      items9[1] = callback2(tmp5(8688), obj14);
      ...
      if (null == source) {
        ...
        if (tmp78Result1) {
          ...
          tmp78Result1 = tmp78(module_8693, obj21);
        }
        items14[2] = tmp78Result1;
        obj18[1] = items14;
        items11[1] = tmp82(tmp5(4116).View, obj18);
        obj16[1] = items11;
        const items15 = [tmp82(tmp5(4116).View, obj16), , , ];
        if (tmp82Result) {
          ...
          obj24[0] = skuId2;
          obj24[3] = tmp28;
          const items17 = [tmp78(tmp5(8394), obj24), ];
          tmp = BackButton;
          const obj25 = { paddingTop: null };
          obj25[0] = bound1;
          items17[1] = tmp78(BackButton, obj25);
          obj22[2] = items17;
          tmp82Result = tmp82(tmp5(4116).View, obj22);
        }
        ...
        obj26[9] = initialTab;
        obj26[10] = !tmp28;
        items15[2] = tmp78(tmp5(15718), obj26);
        items15[3] = tmp78(tmp12(10203).TTIFirstContentfulPaint, { label: "you_screen" });
        obj15[6] = items15;
        ...
        if (tmp) {
          ...
          tmp = tmp5(8432);
          obj27[2] = tmp12(8423).ProfileFrameLayerOrder.FRONT;
          obj27[3] = bound;
          tmp = tmp78(tmp, obj27);
        }
        ...
        obj10[3] = tmp78(tmp12(8442).UserProfileAnalyticsProvider, obj11);
        obj28[0] = tmp78(tmp12(3967).ThemeContextProvider, obj10);
        return tmp78(tmp12(5568).LayerScope, obj28);
        ...
        obj30[3] = source;
        obj30[4] = tmp28;
        const items19 = [tmp78(tmp5(5207), obj30), ];
        ...
        if (!obj31) {
          ...
          tmp78Result2 = tmp78(tmp12(8438).Caption, obj31);
        }
        items19[1] = tmp78Result2;
        obj29[3] = items19;
        tmp82Result = tmp82(tmp12(4783).PressableOpacity, obj29);
      } else {
        ...
        obj32[3] = source;
        obj32[4] = tmp28;
        tmp82Result = tmp78(tmp5(5207), obj32);
      }
      const tmp12Result9 = tmp12(688);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result1 = tmp5(8675);
  }
  if (!isFocused) {
    ref3.current = false;
  }
...
let closure_25 = require("module_4116").createAnimatedComponent(ScrollView);
