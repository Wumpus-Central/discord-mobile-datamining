...
import GuildsBarDefault from "GuildsBar" /* 16709 */;
...
function ContentMaskGradient(offsetX) {
  ...
  const guildsBarPullX = offsetX(16449).useHomeDrawerState().guildsBarPullX;
  let obj = offsetX(16449);
  const fn = function n() {
  ...
  let obj2 = offsetX(4561);
  fn.__closure = { roundToNearestPixel: guildsBarPullX(11347), offsetX, guildsBarPullX };
  fn.__workletHash = 7539125302557;
  fn.__initData = __initData;
  const animatedStyle = obj2.useAnimatedStyle(fn);
  const obj4 = { pointerEvents: "none", style: null, children: closure_9(guildsBarPullX(5429), { absolute: true, tall: true, wide: true, mix: true }) };
  let items = [absoluteFill.absoluteFill, animatedStyle];
  obj4.style = items;
  return closure_9(guildsBarPullX(4561).View, obj4);
}
function HomeDrawerPanelContent() {
  const tmp2 = closure_11();
  const drawerOpen = ref(16452).useDrawerOpen();
  let obj = ref(16452);
  const doesLandOnHomeDrawer = ref(16449).useDoesLandOnHomeDrawer();
  ref = isClientThemeOrCustomThemeActive.useRef(null);
  let obj2 = ref(16449);
  const homeDrawerState = ref(16449).useHomeDrawerState();
  const panelTranslateX = homeDrawerState.panelTranslateX;
  const tmp8 = HomeDrawerStore((maxX) => maxX.maxX);
  dependencyMap = tmp8;
  let obj3 = ref(16449);
  isClientThemeOrCustomThemeActive = ref(8205).useIsClientThemeOrCustomThemeActive();
  let obj4 = ref(8205);
  const token = ref(4526).useToken(panelTranslateX(576).colors.BACKGROUND_BASE_LOWEST);
  const obj5 = ref(4526);
  const token1 = ref(4526).useToken(panelTranslateX(576).colors.PANEL_BG);
  const obj6 = ref(4526);
  const fn = function n() {
  ...
  const obj7 = ref(4561);
  fn.__closure = { isGradientTheme: isClientThemeOrCustomThemeActive, maxX: tmp8, interpolateColor: ref(4561).interpolateColor, panelTranslateX, baseLowest: token, panelBg: token1 };
  ...
  const obj8 = { isGradientTheme: isClientThemeOrCustomThemeActive, maxX: tmp8, interpolateColor: ref(4561).interpolateColor, panelTranslateX, baseLowest: token, panelBg: token1 };
  ...
  const obj9 = ref(4561);
  fn2.__closure = { interpolate: ref(4561).interpolate, panelTranslateX, INITIAL_OPEN_WIDTH: ref(16449).INITIAL_OPEN_WIDTH, Extrapolation: ref(4561).Extrapolation, isGradientTheme: isClientThemeOrCustomThemeActive, interpolateColor: ref(4561).interpolateColor, maxX: tmp8, baseLowest: token, panelBg: token1 };
  ...
  obj12.style = items2;
  const items3 = [closure_9(panelTranslateX(16709), { enableHome: true }), , ];
  ...
  items3[1] = closure_9(panelTranslateX(4561).View, obj13);
  let tmp16Result2 = null;
  if (doesLandOnHomeDrawer) {
    tmp16Result2 = closure_9(tmp10(16788), {});
  }
  items3[2] = tmp16Result2;
  obj12.children = items3;
  obj11.children = closure_10(panelTranslateX(4561).View, obj12);
  return closure_9(panelTranslateX(4561).View, obj11);
}
