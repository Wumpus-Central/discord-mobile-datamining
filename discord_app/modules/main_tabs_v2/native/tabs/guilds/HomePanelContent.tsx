...
function ContentMaskGradient(offsetX) {
  ...
  const obj4 = {
    pointerEvents: "none",
    style: null,
    children: closure_9(guildsBarPullX(5343), { absolute: true, tall: true, wide: true, mix: true }),
  };
...
function HomeDrawerPanelContent() {
  ...
  fn.__closure = {
    ...
    maxX: tmp8,
    interpolateColor: ref(4493).interpolateColor,
    panelTranslateX,
    baseLowest: token,
    panelBg: token1,
  };
  ...
  const fn2 = function k() {
    ...
    const interpolateResult = ReanimatedRexport.interpolate(
      value,
      items,
      [1, 0],
      ReanimatedRexport.Extrapolation.CLAMP,
    );
    if (isClientThemeOrCustomThemeActive) {
  ...
  const obj9 = ref(4493);
  fn2.__closure = {
    interpolate: ref(4493).interpolate,
    panelTranslateX,
    INITIAL_OPEN_WIDTH: ref(16372).INITIAL_OPEN_WIDTH,
    Extrapolation: ref(4493).Extrapolation,
    isGradientTheme: isClientThemeOrCustomThemeActive,
    interpolateColor: ref(4493).interpolateColor,
    maxX: tmp8,
    baseLowest: token,
  ...
  let items2 = [
    drawerOpen ? tmp2.guildsListContainerGestured : tmp2.guildLisetContainerDefault,
...
let closure_11 = createStyles.createStyles((width) => {
  const obj = {
    ...
    guildLisetContainerDefault: { flex: 1, width },
