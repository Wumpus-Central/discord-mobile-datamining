...
const memoResult = importAllResult.memo(function PrivateChannelButtons(channelId) {
  ...
  const callback1 = obj8.useCallback(() => {
    if (closure_7) {
      ...
      obj.openAlert(
        channelId(closure_2[26]).VOICE_PANEL_VIDEO_GUARD_ERROR_KEY,
        closure_1_20(screenIndex(closure_2[26]), obj),
      );
  ...
  callback2 = obj8.useCallback(() => {
    ...
    if (null != stateFromStores) {
      ...
      const channelDetailsSearchContext = channelId(tmp2[29]).getChannelDetailsSearchContext(
        channelId,
        guildId,
        isThreadResult,
      );
  ...
  callback3 = obj8.useCallback(() => {
    ...
    if (tmp2) {
      ...
      obj[2] = application;
      screenIndex(closure_2[32]).openLazy(
        channelId(closure_2[34])(closure_2[33], closure_2.paths),
