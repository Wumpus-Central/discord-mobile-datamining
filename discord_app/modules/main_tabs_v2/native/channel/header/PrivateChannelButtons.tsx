// === Module 13666: PrivateChannelButtons ===
...
import getPrivateChannelCallDefault from "getPrivateChannelCall" /* 11219 */;
import useSearchContext from "useSearchContext" /* 12642 */;
...
import SafetyToolsButton from "SafetyToolsButton" /* 13686 */;
...
let closure_12 = fn(8207).setIsChannelDetailsSearchActive;
...
let closure_18 = fn(8208).SearchEntrypointAnalyticsLocations;
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  inappropriateConversationSafetyToolsWarningForChannel = channelId(11744).useInappropriateConversationSafetyToolsWarningForChannel(channelId);
  closure_6 = tmp7;
  const tmp2Result6 = channelId(11744);
  ...
  const VideoGuardExperiment = tmp2(13667).VideoGuardExperiment;
  ...
  const isCallActiveNullable = channelId(8329).useIsCallActiveNullable(id);
  const tmp2Result9 = channelId(8329);
  ...
  closure_11 = tmp15;
  const tmp17 = screenIndex(13668)({ context: { type: "channel", channel: stateFromStores } });
  ...
  const callback3 = obj9.useCallback(() => {
    ...
    if (tmp2) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13685, dependencyMap.paths), "AppDMOptionsBottomSheet", obj4);
    }
  }, items11);
  ...
  if (screenIndex(8724)(channelId)) {
    ...
    obj3.children = closure_20(tmp2(13345).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5427).PressableOpacity, obj3);
    ...
    if (null != application) {
      ...
      obj5.children = closure_20(tmp2(7384).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5427).PressableOpacity, obj5), ];
      ...
      obj6.children = closure_20(tmp2(7710).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5427).PressableOpacity, obj6);
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        closure_20(tmp2(5407).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = closure_20(tmp2(8213).PhoneHangUpIcon, obj11);
          ...
          tmp30Result2 = closure_20(tmp2(8211).PhoneCallIcon, obj12);
          tmp33 = closure_20;
        }
        const items15 = [
          tmp30Result2,
          visibleParticipants.map((user, index) => {
                  ...
                  obj = { style: { marginLeft: num }, user: user.user, guildId: "r", size: native.AvatarSizes.XSMALL, cutout: 1 };
                  if (index !== diff) {
                    const tmp7 = obj;
                  }
                  obj.cutout = tmp7;
        ...
        const items17 = [closure_21(tmp2(5427).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            if (videoEnabled) {
              let VideoDenyIcon = tmp2(10456).VideoIcon;
            } else {
              VideoDenyIcon = tmp2(13687).VideoDenyIcon;
            }
            obj15.children = tmp33(VideoDenyIcon, { size: "sm" });
            tmp33(tmp2(5427).PressableOpacity, obj15);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
