...
import SafetyToolsButton from "SafetyToolsButton" /* 13305 */;
...
let closure_24 = createStyles.createStyles(() => {
  button = { privateChannelButtonsWrapper: { flexDirection: "row", gap: 12, paddingEnd: 1 }, button: null, disabledButton: null, overflowBadge: null };
  button = { borderRadius: nativeDefault.modules.button.BORDER_RADIUS, minHeight: nativeDefault.space.PX_32, minWidth: nativeDefault.space.PX_32, padding: nativeDefault.space.PX_4, justifyContent: "center", flexDirection: "row", alignItems: "center" };
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  const tmp2Result3 = channelId(7995);
  ...
  const tmp17 = screenIndex(13288)(obj);
  ...
  const callback1 = obj8.useCallback(() => {
    if (closure_7) {
      ...
      obj.openAlert(VoicePanelVideoGuardErrorAlert.VOICE_PANEL_VIDEO_GUARD_ERROR_KEY, closure_2_20(VoicePanelVideoGuardErrorAlertDefault, obj));
    } else {
      callback(true);
    }
  }, items9);
  ...
  const callback3 = obj8.useCallback(() => {
    ...
    if (tmp2) {
      let obj = { settings_type: "user", destination_pane: constants2.SETTINGS_APP_DMS_MENU, source_page: "app_dm_settings", application_id: application.id };
      obj.trackWithMetadata(constants.SETTINGS_PANE_VIEWED, obj);
      obj = { userId: recipientId, channel: stateFromStores, application };
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13304, dependencyMap.paths), "AppDMOptionsBottomSheet", obj);
    }
  }, items11);
  ...
  const memo1 = obj8.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = closure_2_20(SafetyToolsButton.SafetyToolsButton, obj);
    } else {
  ...
  if (screenIndex(8402)(channelId)) {
    ...
    obj.children = closure_20(tmp2(12986).WindowLaunchIcon, { size: "sm" });
    let tmp25Result = closure_20(tmp2(5137).PressableOpacity, obj);
  } else if (tmp17.isAppDM) {
    let tmp43 = null;
    if (null != application) {
      const obj1 = { style: tmp.privateChannelButtonsWrapper, children: null };
      let obj2 = { style: tmp.button, onPress: callback2, accessibilityLabel: null, accessibilityRole: "button", children: null };
      const intl3 = tmp2(1114).intl;
      obj2.accessibilityLabel = intl3.string(tmp2(1114).t["5h0QOP"]);
      obj2.children = closure_20(tmp2(7065).MagnifyingGlassIcon, { size: "sm" });
      const items13 = [closure_20(tmp2(5137).PressableOpacity, obj2), ];
      let obj3 = { style: tmp.button, onPress: callback3, accessibilityLabel: null, accessibilityRole: "button", children: null };
      const intl4 = tmp2(1114).intl;
      obj3.accessibilityLabel = intl4.string(tmp2(1114).t["+1H47t"]);
      obj3.children = closure_20(tmp2(7394).SettingsIcon, { size: "sm" });
      items13[1] = closure_20(tmp2(5137).PressableOpacity, obj3);
    ...
    const obj5 = { accessibilityLabel: stringResult, accessibilityRole: "button", style: null, onPress: null, disabled: null, children: null };
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        let tmp30Result = closure_20(tmp2(5115).VoiceNormalIcon, unsafe_rawColors);
        ...
        if (stateFromStores1) {
          ...
          tmp30Result = closure_20(tmp2(7896).PhoneHangUpIcon, obj7);
          ...
          tmp30Result = closure_20(tmp2(7894).PhoneCallIcon, obj8);
          tmp33 = closure_20;
        }
        const items15 = [
          tmp30Result,
          visibleParticipants.map((user, index) => {
                  ...
                  const obj = { style: { marginLeft: num }, user: user.user, guildId: "r", size: native.AvatarSizes.XSMALL, cutout: "absolute" };
                  if (index !== diff) {
                    const tmp7 = obj;
                  }
                  obj.cutout = tmp7;
        ...
        if (tmp33Result) {
          ...
          obj9.children = closure_21(tmp2(4570).Text, obj10);
          tmp33Result = tmp33(tmp26, obj9);
        }
        items15[2] = tmp33Result;
        obj5.children = items15;
        const items17 = [closure_21(tmp2(5137).PressableOpacity, obj5), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
            ...
            const obj11 = { style: null, onPress: null, disabled: null, accessibilityLabel: null, accessibilityRole: "button", children: null };
            ...
            tmp33Result = tmp33(tmp2(5137).PressableOpacity, obj11);
          }
        }
        items17[1] = tmp38;
        items17[2] = memo1;
        obj4.children = items17;
        tmp25Result = closure_21(tmp26, obj4);
      }
    } else {
