...
const memoResult = importAllResult.memo((navigation) => {
  ...
  obj[2] = callback2(importDefault(15724), { goToMessageRequestPreview: callback });
  const items1 = [obj, ];
  const obj1 = { label: null, id: null, page: null };
  const intl2 = navigation(1236).intl;
  obj1[0] = intl2.string(navigation(1236).t.ulKXHp);
  obj1[1] = constants.SPAM;
  obj1[2] = callback2(importDefault(15740), { goToMessageRequestPreview: callback });
  items1[1] = obj1;
  obj[0] = items1;
  obj[1] = tmp2[0];
  const segmentedControlState = obj.useSegmentedControlState(obj);
  const obj2 = { style: tmp.container, children: null };
  const items2 = [callback2(View, { style: tmp.tabContainer, onLayout: callback1, children: callback2(navigation(8764).SegmentedControl, { state: segmentedControlState }) }), , ];
  const obj3 = { style: tmp.tabContainer, onLayout: callback1, children: callback2(navigation(8764).SegmentedControl, { state: segmentedControlState }) };
  items2[1] = callback2(View, { style: tmp.messageRequestContent, children: callback2(navigation(8762).SegmentedControlPages, { state: segmentedControlState }) });
  items2[2] = callback2(navigation(10061).TTIFirstContentfulPaint, { label: "message_requests" });
  obj2[1] = items2;
