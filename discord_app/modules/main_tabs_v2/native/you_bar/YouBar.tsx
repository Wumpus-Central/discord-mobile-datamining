...
import YouBarFloatingShadeDefault from "YouBarFloatingShade" /* 16746 */;
import ConnectionBannerDefault from "ConnectionBanner" /* 16747 */;
...
let closure_26 = noop.memo(() => {
  ...
  const mobileQuestDock = isMobileQuestDockRenderedBase(15366).useMobileQuestDock();
  let obj = isMobileQuestDockRenderedBase(15366);
  isMobileQuestDockRenderedBase = isMobileQuestDockRenderedBase(15366).useIsMobileQuestDockRenderedBase(mobileQuestDock);
  let obj2 = isMobileQuestDockRenderedBase(15366);
  const drawerWidth = isMobileQuestDockRenderedBase(11783).useDrawerWidth();
  let obj3 = isMobileQuestDockRenderedBase(11783);
  const youBarHorizontalMargin = isMobileQuestDockRenderedBase(15372).useYouBarHorizontalMargin();
  let result = 2 * youBarHorizontalMargin;
  const tmp10 = youBarHorizontalMargin(4617)().isChatBesideChannelList ? drawerWidth - result : youBarHorizontalMargin(1478)().width - result;
  dependencyMap = tmp10;
  let obj4 = isMobileQuestDockRenderedBase(15372);
  const youBarBottomMargin = isMobileQuestDockRenderedBase(15372).useYouBarBottomMargin();
  const tmp4Result = isMobileQuestDockRenderedBase(15372);
  const connectionBannerHeight = isMobileQuestDockRenderedBase(15376).useConnectionBannerHeight();
  let items = [tmp10, youBarHorizontalMargin];
  const memo = sharedValue.useMemo(() => {
    const size = { marginHorizontal: youBarHorizontalMargin, height, padding, width };
    return size;
  }, items);
  const tmp4Result11 = isMobileQuestDockRenderedBase(15376);
  sharedValue = isMobileQuestDockRenderedBase(4493).useSharedValue(false);
  const tmp4Result12 = isMobileQuestDockRenderedBase(4493);
  let fn = function r() {
  ...
  const tmp4Result13 = isMobileQuestDockRenderedBase(4493);
  fn.__closure = { withSpring: isMobileQuestDockRenderedBase(5186).withSpring, barMarginBottom: youBarBottomMargin, connectionBannerHeight, YOU_BAR_SPRING_CONFIG: callback3, isPressedValue: sharedValue, isQuestRendered: isMobileQuestDockRenderedBase };
  fn.__workletHash = 7314807713815;
  fn.__initData = __initData;
  const animatedStyle = tmp4Result13.useAnimatedStyle(fn);
  const obj5 = { withSpring: isMobileQuestDockRenderedBase(5186).withSpring, barMarginBottom: youBarBottomMargin, connectionBannerHeight, YOU_BAR_SPRING_CONFIG: callback3, isPressedValue: sharedValue, isQuestRendered: isMobileQuestDockRenderedBase };
  const iCYMIEnabled = isMobileQuestDockRenderedBase(8627).useICYMIEnabled("TabsNavigator");
  const tmp4Result14 = isMobileQuestDockRenderedBase(8627);
  const youBarCoachmark = isMobileQuestDockRenderedBase(16715).useYouBarCoachmark({ isQuestRendered: isMobileQuestDockRenderedBase });
  const visibleContent = youBarCoachmark.visibleContent;
  const markAsDismissed = youBarCoachmark.markAsDismissed;
  const tmp4Result15 = isMobileQuestDockRenderedBase(16715);
  const showTinyBroncoPromoSheet = isMobileQuestDockRenderedBase(14999).useShowTinyBroncoPromoSheet({ visibleContent, markAsDismissed });
  ...
  const tmp4Result16 = isMobileQuestDockRenderedBase(14999);
  ...
  const nameplate = isMobileQuestDockRenderedBase(8490).useNameplate({ user: stateFromStores });
  const tmp4Result18 = isMobileQuestDockRenderedBase(8490);
  const youBarAccessibilityLabel = isMobileQuestDockRenderedBase(16717).useYouBarAccessibilityLabel(stateFromStores);
  ...
  const context = sharedValue.useContext(tmp2(16352));
  const gesture = context.gesture;
  const translateX = context.translateX;
  const tmp4Result19 = isMobileQuestDockRenderedBase(16717);
  hitSlop = isMobileQuestDockRenderedBase(4493).useSharedValue(0);
  ...
  obj6.style = items9;
  const items10 = [closure_19(youBarHorizontalMargin(16734), { hasNameplate: null != nameplate, isLargeAvatar: !isMobileQuestDockRenderedBase, barWidth: tmp10, isQuestRendered: isMobileQuestDockRenderedBase, avatarSize: tmp37 }), , , ];
  let tmp40Result = tmp22;
  if (null != nameplate) {
    const obj7 = { nameplate, barWidth: tmp10, isQuestRendered: isMobileQuestDockRenderedBase, avatarSize: tmp37 };
    tmp40Result = closure_19(tmp2(16735), obj7);
  }
  ...
  obj9.children = closure_19(youBarHorizontalMargin(16736), { isQuestRendered: isMobileQuestDockRenderedBase, onAvatarPress: memo1 });
  obj8.children = closure_19(markAsDismissed, obj9);
  items10[2] = closure_19(isMobileQuestDockRenderedBase(6899).GestureDetector, obj8);
  const obj10 = { style: tmp.youRowRight, children: null };
  let tmp40Result2 = null;
  if (iCYMIEnabled) {
    const obj11 = { hasNameplate: tmp22 };
    tmp40Result2 = closure_19(tmp2(16741), obj11);
  }
  const items11 = [tmp40Result2, closure_19(youBarHorizontalMargin(16744), { hasNameplate: null != nameplate })];
  obj10.children = items11;
  items10[3] = closure_20(visibleContent, obj10);
  obj6.children = items10;
  return closure_20(youBarHorizontalMargin(4493).View, obj6);
});
