...
function YouBarAvatarLarge(transitionState) {
  ...
  class V {
    constructor() {
      ...
      obj1 = { transitionState, TransitionStates: closure_0(closure_2[14]).TransitionStates, runOnJS: closure_0(closure_2[15]).runOnJS, cleanup };
      fn.__closure = obj1;
  ...
  if (null != stateFromStores) {
    ...
    obj2 = { user: stateFromStores, guildId: "Array", size, animate: true, needsOffscreenAlphaCompositing: null, status: null, statusSizeOverride: null, cutout: null, statusStyle: "status" };
    if (OFFLINE === StatusTypes.UNKNOWN) {
      OFFLINE = StatusTypes.OFFLINE;
    }
    obj2.status = OFFLINE;
...
function YouBarAvatar(transitionState) {
  ...
  obj = { withSpring: tmp(tmp2[18]).withSpring, opacity: sharedValue, YOU_BAR_SPRING_CONFIG, transitionState, TransitionStates: tmp(tmp2[14]).TransitionStates, runOnJS: tmp(tmp2[15]).runOnJS, cleanup };
  fn.__closure = obj;
  ...
  if (null == stateFromStores) {
    ...
    obj.style = items3;
    obj1 = { user: stateFromStores, guildId: "Array", size: size2, animate: true, needsOffscreenAlphaCompositing: null, avatarDecoration, status: true, autoStatusCutout: "/assets/.cache/intl/bW9kdWxlcy9jb2xsZWN0aWJsZXMvd2Vi" };
    if (OFFLINE === StatusTypes.UNKNOWN) {
      OFFLINE = StatusTypes.OFFLINE;
    }
    obj1.status = OFFLINE;
