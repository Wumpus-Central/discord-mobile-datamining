...
function YouBarAvatarLarge(transitionState) {
  ...
  if (null != stateFromStores) {
    ...
    const obj7 = {
      ...
      cutout: null,
      statusStyle: "FRECENCY_SECTION_SET_SELECTION",
    };
    if (OFFLINE === StatusTypes.UNKNOWN) {
      OFFLINE = StatusTypes.OFFLINE;
    }
    ...
    if (tmp29Result) {
      ...
      obj10.cutout = tmp2Result23.getDecorationCutoutForAvatarCutout(
        memo,
        (tmp2(tmp3[21]).getDecorationSizeForAvatarSize(tmp33) - height) / 2,
      );
      tmp29Result = closure_22(tmp23Result4, obj10, avatarDecoration.asset);
      const tmp2Result24 = tmp2(tmp3[21]);
    }
    items6[3] = tmp29Result;
    obj4.children = items6;
    tmp28Result = closure_23(tmp23(tmp3[15]).View, obj4);
...
function YouBarAvatar(transitionState) {
  ...
  const items1 = [UserStore];
  const stateFromStores = transitionState(sharedValue[12]).useStateFromStores(items1, () =>
  ...
  fn.__closure = {
    withSpring: transitionState(sharedValue[18]).withSpring,
    opacity: sharedValue,
    YOU_BAR_SPRING_CONFIG,
    transitionState,
...
export default noop.memo(function YouBarAvatarAnimated(isLargeAvatar) {
  ...
  let callback1;
  let items = [UserStore];
  const stateFromStores = isLargeAvatar(isAvatarPressed[12]).useStateFromStores(items, () =>
    currentUser.getCurrentUser(),
  );
