...
export const useLiveStageData = function useLiveStageData(stage) {
  ...
  const stateFromStoresArray = obj.useStateFromStoresArray(
    items,
    () => {
      const mutableParticipants = memo1.getMutableParticipants(
        stage.channel_id,
        stage(stateFromStoresArray1[4]).StageChannelParticipantNamedIndex.FRIEND,
      );
      const found = mutableParticipants.filter(
        (type) => type.type === callback(table[4]).StageChannelParticipantTypes.VOICE,
      );
  ...
  stateFromStoresArray1 = _require(stateFromStoresArray1[3]).useStateFromStoresArray(
    items2,
    () => {
      const mutableParticipants = memo1.getMutableParticipants(
        stage.channel_id,
        stage(stateFromStoresArray1[4]).StageChannelParticipantNamedIndex.SPEAKER,
      );
      const found = mutableParticipants.filter(
        (type) => type.type === callback(table[4]).StageChannelParticipantTypes.VOICE,
      );
  ...
  const stateFromStoresArray2 = _require(stateFromStoresArray1[3]).useStateFromStoresArray(
    items4,
    () => {
      const mutableParticipants = memo1.getMutableParticipants(
        stage.channel_id,
        stage(stateFromStoresArray1[4]).StageChannelParticipantNamedIndex.AUDIENCE,
      );
      const found = mutableParticipants.filter(
        (type) => type.type === callback(table[4]).StageChannelParticipantTypes.VOICE,
      );
  ...
  memo1 = stateFromStoresArray2.useMemo(
    () =>
      stateFromStoresArray(stateFromStoresArray1[5]).differenceBy(stateFromStoresArray, stateFromStoresArray1, "id"),
    items7,
  );
  const items8 = [stateFromStoresArray, stateFromStoresArray2];
  const memo2 = stateFromStoresArray2.useMemo(
    () =>
      stateFromStoresArray(stateFromStoresArray1[5]).differenceBy(stateFromStoresArray2, stateFromStoresArray, "id"),
    items8,
  );
  ...
  obj = {
    ...
    channel: _require(stateFromStoresArray1[3]).useStateFromStores(
      items10,
      () => memo2.getChannel(stage.channel_id),
      items11,
    ),
  };
  return obj;
