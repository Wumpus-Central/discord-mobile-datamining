...
function IconOrPreview(arg0) {
  ...
  if (tmp9) {
    let obj = require("ApplicationAssetUtils");
    let application_id;
    if (activity != null) {
  ...
  if (null != assetImage) {
    ...
    const memo = noop.useMemo(() => {
      ...
      if (obj.isAndroid()) {
        obj = { boxShadow: null };
        obj = { offsetX: 0, offsetY: 0, blurRadius: 5, color: ColorUtils.hexWithOpacity(shadowColor, 0.32) };
        const items1 = [obj];
        obj.boxShadow = items1;
        let obj1 = obj;
    ...
    if (null != stream) {
      ...
      obj3.children = closure_11(tmp2(5668), obj4);
      obj2.children = closure_11(closure_4, obj3);
...
let obj2 = { height: "100%", backgroundColor: nativeDefault.colors.CARD_SECONDARY_BG, borderRadius: nativeDefault.radii.sm, shadowOffset: { width: 0, height: 0 }, shadowRadius: 5, shadowOpacity: 0.32 };
...
export default noop.memo((userId) => {
  ...
  const callback = activity.useCallback(() => {
    ...
    if (null != stream) {
      ...
      asyncRequireImpl(8296, dependencyMap.paths).then((result) => result.default({ userId, localUser, sourceAnalyticsLocations }));
