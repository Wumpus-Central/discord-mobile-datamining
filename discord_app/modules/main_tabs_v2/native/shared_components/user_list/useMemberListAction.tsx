...
fn(4574);
const createStyles = { wrapper: { paddingTop: fn(10245).USERS_LIST_PADDING_BETWEEN_SECTIONS } };
  ...
  return noop.useMemo(() => {
    if (null != id) {
      if (c4) {
        if (flag2) {
          let obj = {
            iconSource: closure_1(stateFromStores[15]),
            IconComponent: channel(stateFromStores[16]).ChatPlusIcon,
            label: null,
            sublabel: null,
            handlePress: null,
          };
          ...
          if (flag3) {
            ...
            obj.handlePress = function handlePress() {
              if (null != flag3.getChannel(id)) {
                const groupDMAddMembersAction = channel(stateFromStores[14]).getGroupDMAddMembersAction(
                  id,
        ...
        obj1.children = jsx(channel(stateFromStores[28]).RowButton, {
          icon: null,
          onPress: null,
          label: null,
          subLabel: null,
