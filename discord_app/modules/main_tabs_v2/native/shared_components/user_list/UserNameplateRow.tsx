...
export const UserNameplateRow = function UserNameplateRow(onPressOut) {
  ...
  let dependencyMap;
  let obj = React;
  const context = React.useContext(onPressIn(5355).TableRowGroupContext);
  let tmp6 = !context;
  if (!context) {
    tmp6 = true === end;
  }
  ...
  const token = onPressIn(3988).useToken(onPressOut(712).modules.mobile.TABLE_ROW_BORDER_RADIUS);
  ...
  if (!context) {
    if (!tmp6) {
      ...
      obj1[0] = null != icon;
      items3[1] = tmp15(tmp3(5315).TableRowDivider, obj1);
      obj[0] = items3;
