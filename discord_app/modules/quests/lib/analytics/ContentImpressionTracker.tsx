// === Module 11487: ContentImpressionTracker ===
...
import IosAttributionEligibility from "IosAttributionEligibility" /* 11488 */;
          ...
          if (null != activeIosAttributionFramework) {
            if (null != adProvenanceMetadataSealed) {
              ...
              if (tmp2Result.isCampaignIosAttributionEnabled(tmp.sourceQuestContent, item)) {
                ...
                const result = tmp2(11493).registerViewThroughImpression(obj);
                tmp.iosAttributionRegistered = true;
                const tmp2Result1 = tmp2(11493);
              } else {
                const result1 = tmp2(11492).trackIosAttributionImpression(tmp2(11492).IosAttributionImpressionResult.NOT_SKAN_ENABLED, activeIosAttributionFramework, tmp.id);
                const tmp2Result2 = tmp2(11492);
              }
            } else {
              const result2 = tmp2(11492).trackIosAttributionImpression(tmp2(11492).IosAttributionImpressionResult.NO_METADATA, activeIosAttributionFramework, tmp.id);
              const tmp2Result3 = tmp2(11492);
            }
          } else {
            const result3 = tmp2(11492).trackIosAttributionImpression(tmp2(11492).IosAttributionImpressionResult.NO_FRAMEWORK, activeIosAttributionFramework, tmp.id);
            const tmp2Result4 = tmp2(11492);
          }
        }
      }
    };
    closure_129_0 = obj;
    obj.onMinViewTimeReached = closure_3(async (arg0, value) => {
      if (c3 === 2) {
        c3 = 3;
          ...
          if (0 === dependencyMap) {
            ...
            const item = adContentIds.forEach((item, index) => {
              ...
              if (adUser.migrateQuestContentViewedToCaptureAdUserAction) {
                ...
                questLogger.info("" + item + " ad content viewed for at least " + obj1.minViewTimeSeconds + "s at " + tmp(7728).getQuestContentName(obj1.questContent), obj3);
                ...
                questLogger1.info("" + questName + " Quest viewed for at least " + obj1.minViewTimeSeconds + "s at " + tmp(7728).getQuestContentName(obj1.questContent), obj4);
                ...
                questLogger2.info("" + obj1.entity.adContentIds[index] + " ad content viewed for at least " + obj1.minViewTimeSeconds + "s at " + tmp(7728).getQuestContentName(obj1.questContent), obj7);
      ...
      if (null != flag.lastBeatTime) {
        ...
        const item = adContentIds.forEach((item, index) => {
          if (obj.entity.adCreativeType === AdCreativeType.AdCreativeType.QUEST) {
            ...
            tmp(7718).trackAdContentEvent(obj3);
    ...
    obj.start = function start() {
      ...
      const item = adContentIds.forEach((item, index) => {
        ...
        if (obj.migrateQuestContentLoadedToCaptureAdUserAction) {
          ...
          tmpResult = tmp(11193);
          obj1 = { type: tmp(11197).AdUserActionType.END_CONTENT_LOAD, surfaceId: null, sourceQuestContent: null, impressionId: null, triggeredByStatusChange: null, trackGuildAndChannelMetadata: null, questContentPosition: null, questContentRowIndex: null };
          ...
          questLogger.info("" + questName + " Quest became visible at " + tmp(7728).getQuestContentName(questContentRowIndex.questContent), obj2);
          const tmpResult1 = tmp(7728);
          const obj3 = {};
          ...
          const questLogger1 = tmp(7709).getQuestLogger();
          const tmpResult4 = tmp(7709);
          const _HermesInternal2 = HermesInternal;
          const obj5 = { impressionId: questContentRowIndex.id };
          questLogger1.info("" + questContentRowIndex.entity.adContentIds[tmpResult] + " ad content became visible at " + tmp(7728).getQuestContentName(questContentRowIndex.questContent), obj5);
          const tmpResult5 = tmp(7728);
          const obj6 = {};
