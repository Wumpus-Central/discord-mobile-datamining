...
let result = require("set").fileFinishedImporting(
  "modules/quests/native/BountiesModal/useBountyPauseAppStoreSheet.tsx",
);
...
export const useBountyPauseAppStoreSheet = function useBountyPauseAppStoreSheet(bounty) {
  ...
  const effect = isActive.useEffect(() => {
    ...
    if (isActive) {
      ...
      tmp =
        config.enabled && tmp7 === bounty(sourceQuestContent[3]).BountiesCtrExperiment1Variant.OPEN_APP_SHEET_ON_PAUSE;
      const tmp2 = bounty;
            ...
            tmp2Result.openAppStoreOrUrl(obj).then((arg0) => {
              if (arg0) {
                ...
                const subscription = ComponentDispatch.subscribe(
                  closure_1_4.QUEST_APP_STORE_OVERLAY_FINISHED,
                  handleFinished,
                );
