...
import { ACTION_SHEET_BORDER_RADIUS } from "../../../action_sheet/native/ActionSheetConstants.tsx";
...
function BountyVideoEndAppStorePanelInner(metadata) {
  ...
  const memo = onDismiss.useMemo(() => {
      ...
      if (store.get() >= 0.5) {
        if (velocityY.velocityY <= 800) {
          const result = store.set(
            closure_1_0(closure_1_2[14]).withTiming(1, closure_1_0(closure_1_2[15]).timingStandard),
          );
    ...
    fn3.__closure = {
      ...
      runOnJS: metadata(revealProgress[9]).runOnJS,
      handleDismiss: callback,
      withTiming: metadata(revealProgress[14]).withTiming,
      timingStandard: metadata(revealProgress[15]).timingStandard,
    };
  ...
  const items7 = [
    callback(onInstallPress, {
      style: tmp.scrollBody,
      contentContainerStyle: tmp.scrollContent,
      nestedScrollEnabled: true,
      showsVerticalScrollIndicator: false,
      keyboardShouldPersistTaps: "handled",
      children: callback(metadata(revealProgress[7]).AppStoreOverlayBody, {
        metadata,
  ...
  obj3[1] = callback(closure_5, {
    style: tmp.headerGestureTarget,
    children: callback(metadata(revealProgress[16]).ActionSheetHeaderBar, obj5),
  });
...
createCacheKey[5] = {
  paddingBottom: require("APP_STORE_OVERLAY_HEIGHT_RATIO").APP_STORE_OVERLAY_FOOTER_GRADIENT_HEIGHT,
};
...
let result = require("set").fileFinishedImporting(
  "modules/quests/native/BountiesModal/BountyVideoEndAppStorePanel.tsx",
);
