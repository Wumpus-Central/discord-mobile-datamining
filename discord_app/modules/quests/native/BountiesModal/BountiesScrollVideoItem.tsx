...
function BountiesScrollVideoItemInner(bounty) {
  ...
  const bountyVideoProgressPersistence = bounty(width[14]).useBountyVideoProgressPersistence({
    bountyId: bounty.id,
    endMode: tmp15,
  });
  ...
  const bountiesModalVideoAnalytics = bounty(width[15]).useBountiesModalVideoAnalytics({
    bountyId: bounty.id,
    sourceQuestContent,
  ...
  handleVideoPausedForAppStore = bounty(width[16]).useBountyPauseAppStoreSheet({
    bounty,
    sourceQuestContent,
    isActive,
    playerRef: ref,
  }).handleVideoPausedForAppStore;
