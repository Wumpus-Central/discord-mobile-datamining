...
import useBountiesAutoScroll from "useBountiesAutoScroll.tsx";
...
const __initData4 = {
  code: "function BountyVideoTsx4(){const{videoEndPeekScale,height}=this.__closure;if(videoEndPeekScale==null){return{};}const scale=videoEndPeekScale.get();if(scale>=1){return{};}const centerPivotCompensation=height*(1-scale)/2;return{transform:[{translateY:-centerPivotCompensation},{scale:scale}]};}",
};
let size = fn(2);
...
export const BountyVideo = function BountyVideo(bounty) {
  ...
  class Fe {
    constructor() {
      ...
      if (null == peekScale) {
        ...
        num2 = 2;
        obj1.translateY =
          (diff / diff1) * closure_0(closure_2[16]).AUTO_SCROLL_PEEK_TOP_OFFSET - (height * (1 - value)) / 2;
        items = [,];
        items[0] = obj1;
        obj2 = { scale: null };
        obj2.scale = value;
  ...
  const obj13 = {
    ...
    children: tmp46(handleVideoProgress(handleVideoError[23]), { bounty, visible: tmp29, sourceQuestContent }),
  };
  ...
  const obj14 = {
    style: null,
    children: tmp46(handleVideoProgress(handleVideoError[9]), {
  ...
  obj15.style = items15;
  const items16 = [
    tmp46(handleVideoProgress(handleVideoError[24]), {
      isCompleted,
      totalSeconds: rewardTotalSeconds,
      remainingSeconds: rewardRemainingSeconds,
    }),
    tmp46(bounty(handleVideoError[25]).BalanceWidgetPill, { balance: orbsBalance }, balanceWidgetPillResetKey),
  ];
