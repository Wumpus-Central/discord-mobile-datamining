...
import useBountySwipeUpNux from "useBountySwipeUpNux.tsx";
import useBountiesAutoScroll from "useBountiesAutoScroll.tsx";
import useBountiesRecapOrbCount from "useBountiesRecapOrbCount.tsx";
...
function BountiesModalContentScrollInner(arg0) {
  ...
  const stateFromStores = obj6.useStateFromStores(
    items6,
    () => BountyStore.getCompletedBountyCount(first) * orbAmount,
    items7,
  );
  const first1 = data[0];
  let obj7 = initialize;
  ...
  orbAmount = tmp5Result.useBountiesRecapOrbCount({
  ...
  obj3 = {
    VerticalScrollingDirection: AnalyticsTypes.VerticalScrollingDirection,
  ...
  obj6 = {
    ...
    scrollY: sharedValue4,
    lastBountyScrollOffset: result1,
    RECAP_SNAP_EPSILON: useBountiesRecapScroll.RECAP_SNAP_EPSILON,
  };
  jt.__closure = obj6;
  ...
  obj12 = {
    getRevealProgress: useBountiesRecapScroll.getRevealProgress,
    scrollY: sharedValue4,
    lastBountyScrollOffset: result1,
    recapRevealHeight: height3,
  };
  ...
  if (0 === data.length) {
    ...
    if (tmp55) {
      let tmp99 = c15(sourceQuestContent(15076), {});
    } else {
      tmp99 = null;
      if (tmp45) {
    ...
    let tmp2Result = sourceQuestContent(15078);
    if (peekActive) {
      peekActive = !tmp19;
    }
    ...
    if (tmp30) {
      ...
      tmp102Result = tmp102(sourceQuestContent(4310).View, obj22);
    }
    ...
    obj24.children = c15(_mod8906.AnimatedFlashList, obj25);
    items30[2] = c15(sourceQuestContent(4310).View, obj24);
    tmp102Result = null;
    if (null != tmp18) {
      const obj26 = {
        metadata: tmp18.metadata,
        sheetHeight: memo2,
        revealProgress: sharedValue1,
        onDismiss: callback2,
        onInstallPress: tmp18.onInstallPress,
      };
      tmp102Result = tmp102(sourceQuestContent(15083), obj26);
    }
    ...
    if (tmp93) {
      ...
      if (data.length > 1) {
        ...
        obj27.children = tmp102(sourceQuestContent(5001), obj28);
        tmp102Result1 = tmp102(sourceQuestContent(4310).View, obj27);
      }
    }
    ...
    if (tmp42) {
      ...
      tmp102Result2 = tmp102(sourceQuestContent(15084), obj30);
    }
    obj29.children = tmp102Result2;
    items30[5] = c15(sourceQuestContent(4310).View, obj29);
    ...
    if (visible) {
      ...
      if (!tmp19) {
        ...
        obj31.children = tmp102(sourceQuestContent(15085), obj32);
        tmp102Result3 = tmp102(tmp104, obj31);
      }
    }
    items30[6] = tmp102Result3;
    tmp2Result = sourceQuestContent(15036);
    if (!tmp55) {
      tmp55 = tmp45;
    }
    if (tmp55) {
