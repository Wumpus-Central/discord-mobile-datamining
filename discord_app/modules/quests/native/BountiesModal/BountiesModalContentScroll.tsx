...
function BountiesScrollVideoItemContainer(index) {
  ...
  const fn = function c() {
    ...
    const obj = {
      opacity: index(scrollY[11]).interpolate(absolute, [0, 0.3, 1], items, index(scrollY[11]).Extrapolation.CLAMP),
    };
    items = [1, 1, num];
    return obj;
  };
  obj = {
    ...
    isPeekEnabled,
    PEEK_OPACITY: 0.8,
    interpolate: index(scrollY[11]).interpolate,
    FADE_DEADBAND: 0.3,
    Extrapolation: index(scrollY[11]).Extrapolation,
  };
  fn.__closure = obj;
...
function BountiesModalContentScrollInner(arg0) {
  ...
  const stateFromStores1 = obj5.useStateFromStores(
    items3,
    () => {
      if (null == first1) {
        return false;
  ...
  handleRecapMomentumEnd = obj6.useBountiesRecapScroll({
    listRef: ref,
    enabled: tmp19,
    offsets: memo1,
  ...
  closure_53 = tmp5Result.useBountiesRecapOrbCount({
    scrollY: sharedValue2,
  ...
  closure_54 = obj.useCallback(() => {
    ...
    obj = {
      ...
      adCreativeType: tmp7(height2[27]).AdCreativeType.BOUNTY,
      event: rewardTimerSeconds.AD_VIDEO_MODAL_CLOSED,
      properties: null,
      sourceQuestContent: null,
    };
    obj = {
      content_name: tmp7(height2[22]).getQuestContentName(tmp7(height2[28]).QuestContent.BOUNTIES_END_INTERSTITIAL),
      content_id: tmp7(height2[28]).QuestContent.BOUNTIES_END_INTERSTITIAL,
    };
    obj[3] = obj;
    obj[4] = width;
    obj.trackAdContentEvent(obj);
  ...
  obj2 = {
    scrollY: sharedValue2,
    isDraggingSharedValue: sharedValue1,
    isScrollingInBoundsSharedValue: sharedValue,
    isScrollEventInBounds: closure_25,
  };
  Tt.__closure = obj2;
  ...
  obj3 = {
    VerticalScrollingDirection: tmp5(tmp3[22]).VerticalScrollingDirection,
    runOnJS: tmp5(tmp3[11]).runOnJS,
  ...
  obj6 = {
    ...
    scrollY: sharedValue2,
    lastBountyScrollOffset: result1,
    RECAP_SNAP_EPSILON: tmp5(tmp3[19]).RECAP_SNAP_EPSILON,
  };
  wt.__closure = obj6;
  ...
  const memo6 = obj.useMemo(() => {
    const items = [
      height2.listWrapper,
      { top: 0, left: memo.left, width: memo.width, height: memo.top + memo.height + 97 },
    ];
    return items;
  }, items15);
  const items17 = [,];
  ...
  Ft.__closure = {
    getRevealProgress: require(height2[19]).getRevealProgress,
    scrollY: sharedValue2,
    lastBountyScrollOffset: result1,
    recapRevealHeight: height3,
  };
  ...
  Yt.__closure = {
    interpolate: require(height2[11]).interpolate,
    recapPullProgress: derivedValue,
    Extrapolation: require(height2[11]).Extrapolation,
  };
  ...
  Jt.__closure = {
    ...
    FOOTER_FADE_END_PROGRESS: closure_23,
    Extrapolation: require(height2[11]).Extrapolation,
  };
  ...
  const items22 = [
    ...
    sharedValue2,
    memo8,
    "peek" === scrollAffordanceVariant && hasRecurringSwipeUpNux,
  ...
  if (0 === first.length) {
    ...
    if (tmp19) {
      ...
      obj21[5] = function children() {
        return callback(tmp7(height2[36]).BountiesScrollRecapPage, {
          orbAmount: closure_53,
          onClose: closure_54,
          style: { flex: 1 },
        });
      };
...
let closure_27 = createCacheKey.createStyles(() => {
  ...
  return {
    root: { flex: 1 },
    recapPage,
    listWrapper: { position: "absolute", zIndex: c16, overflow: "hidden" },
    closeButton: { position: "absolute", zIndex: c18 },
    peekGradient: { position: "absolute", zIndex: c17 },
    autoScrollIndicator: { position: "absolute", zIndex: c18, alignItems: "center" },
  };
});
let closure_28 = {
...
let closure_42 = {
  code: "function BountiesModalContentScrollTsx15(){const{getRevealProgress,scrollY,lastBountyScrollOffset,recapRevealHeight}=this.__closure;return getRevealProgress(scrollY.get(),lastBountyScrollOffset,recapRevealHeight);}",
};
let closure_43 = {
  code: "function BountiesModalContentScrollTsx16(){const{interpolate,recapPullProgress,Extrapolation}=this.__closure;return{opacity:interpolate(recapPullProgress.get(),[0,1],[0,1],Extrapolation.CLAMP)};}",
};
let closure_44 = {
  code: "function BountiesModalContentScrollTsx17(){const{interpolate,recapPullProgress,FOOTER_FADE_START_PROGRESS,FOOTER_FADE_END_PROGRESS,Extrapolation}=this.__closure;return{opacity:interpolate(recapPullProgress.get(),[FOOTER_FADE_START_PROGRESS,FOOTER_FADE_END_PROGRESS],[1,0],Extrapolation.CLAMP)};}",
};
let closure_45 = {
  code: "function BountiesModalContentScrollTsx18(){const{scrollY,lastBountyScrollOffset,slotHeight,recapPullProgress,getRevealProgress,recapRevealHeight,interpolate,FOOTER_FADE_START_PROGRESS,FOOTER_FADE_END_PROGRESS,Extrapolation}=this.__closure;const progress=scrollY.get()>=lastBountyScrollOffset-slotHeight/2?recapPullProgress.get():getRevealProgress(scrollY.get(),0,recapRevealHeight);return{opacity:interpolate(progress,[FOOTER_FADE_START_PROGRESS,FOOTER_FADE_END_PROGRESS],[1,0],Extrapolation.CLAMP)};}",
};
let closure_46 = {
  code: "function BountiesModalContentScrollTsx19(){const{interpolate,scrollY,slotHeight,Extrapolation}=this.__closure;return{opacity:interpolate(scrollY.get(),[0,slotHeight],[1,0],Extrapolation.CLAMP)};}",
};
let closure_47 = {
  code: "function BountiesModalContentScrollTsx20(){const{recapPullProgress,FOOTER_FADE_END_PROGRESS}=this.__closure;return recapPullProgress.get()<FOOTER_FADE_END_PROGRESS;}",
};
let closure_48 = {
  code: "function BountiesModalContentScrollTsx21(pressable,previousPressable){const{runOnJS,setIsCloseButtonPressable}=this.__closure;if(pressable!==previousPressable){runOnJS(setIsCloseButtonPressable)(pressable);}}",
};
...
export default function BountiesModalContentScroll(arg0) {
  ...
  return callback2(ThemeContextProvider.ThemeContextProvider, {
    theme: items28.ThemeTypes.DARKER,
