...
function SheetContent(arg0) {
  ...
  const items2 = [
    callback(View, { style: tmp.riveContainer, children: callback(ManaContext.NitroQuestOrbsMultiplierRive, {}) }),
    ,
    ,
  ];
  const items3 = [,];
  ({ text: arr4[0], title: arr4[1] } = tmp);
  items2[1] = callback(Text.Text, {
    style: items3,
    variant: "display-md",
...
export default function QuestOrbMultiplierPerkInfoActionSheet(multiplier) {
  ...
  obj = {
    visible:
      orbMultiplierEligibility === multiplier(11154).QuestOrbMultiplierEligibilityType.NITRO ||
      orbMultiplierEligibility === multiplier(11154).QuestOrbMultiplierEligibilityType.UPSELL,
    children: null,
  };
  const tmp4 =
    orbMultiplierEligibility === multiplier(11154).QuestOrbMultiplierEligibilityType.NITRO ||
