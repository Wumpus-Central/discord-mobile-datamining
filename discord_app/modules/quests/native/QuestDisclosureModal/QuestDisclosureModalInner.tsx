...
export default function QuestDisclosureModalInner(isTargetedDisclosure) {
  ...
  if (setting) {
    obj[0] = tmp5(tmp2(9639).ServerIcon, { size: "xs" });
    ...
    obj[0] = tmp5(tmp2(10469).GlobeEarthIcon, { size: "xs" });
    const intl = tmp2(1236).intl;
    obj[1] = intl.string(tmp2(1236).t.xQSdPv);
    items1 = [obj, , ];
    obj = { icon: null, text: null };
    obj[0] = tmp5(tmp2(11157).UserIcon, { size: "xs" });
    const intl2 = tmp2(1236).intl;
    obj[1] = intl2.string(tmp2(1236).t.mYt7hQ);
    items1[1] = obj;
    obj = { icon: null, text: null };
    obj[0] = tmp5(tmp2(8702).GameControllerIcon, { size: "xs" });
  ...
  if (isTargetedDisclosure) {
    ...
    isTargetedDisclosure = tmp6(tmp2(5375).Card, obj4);
  }
