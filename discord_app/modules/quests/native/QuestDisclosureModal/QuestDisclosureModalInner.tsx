...
let obj = {
  ...
  contentContainer: { flexGrow: 1, padding: nativeDefault.space.PX_24, gap: nativeDefault.space.PX_16 },
  illustration: null,
  closeButton: null,
  targetList: null,
  targetItem: null,
...
export default function QuestDisclosureModalInner(isTargetedDisclosure) {
  ...
  if (setting) {
    obj.icon = closure_6(tmp2(9477).ServerIcon, { size: "xs" });
    ...
    obj.icon = closure_6(tmp2(9244).GlobeEarthIcon, { size: "xs" });
    const intl = tmp2(1115).intl;
    obj.text = intl.string(tmp2(1115).t.xQSdPv);
    items1 = [obj, ,];
    const obj2 = { icon: closure_6(tmp2(12160).UserIcon, { size: "xs" }), text: null };
    const intl2 = tmp2(1115).intl;
    obj2.text = intl2.string(tmp2(1115).t.mYt7hQ);
    items1[1] = obj2;
    const obj3 = { icon: closure_6(tmp2(9425).GameControllerIcon, { size: "xs" }), text: null };
  ...
  if (isTargetedDisclosure) {
    const obj7 = {
      ...
      children: items1.map((children, index) => {
        ...
        items[1] = timestampProducer(React4, {
          style: items1,
    ...
    isTargetedDisclosure = tmp6(tmp2(5910).Card, obj7);
  }
