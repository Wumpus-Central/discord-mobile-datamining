// === Module 14716: QuestDisclosureModalInner ===
...
export default function QuestDisclosureModalInner(isTargetedDisclosure) {
  ...
  const tmp = callback();
  _require = tmp;
  const DropsOptedOut = _require(4166).DropsOptedOut;
  const setting = DropsOptedOut.useSetting();
  let obj = { icon: null, text: null };
  if (setting) {
    obj[0] = tmp5(tmp2(9355).ServerIcon, { size: "xs" });
    ...
    items1[1] = obj;
    obj = { icon: null, text: null };
    obj[0] = tmp5(tmp2(9199).GameControllerIcon, { size: "xs" });
  ...
  const items2 = [tmp6(closure_4, { style: tmp.illustration, children: tmp6(_require(6256).WumpusCouchSpotIllustration, {}) }), , , , ];
  const obj3 = { variant: "text-md/normal", color: "mobile-text-heading-primary", children: null };
  const obj2 = { style: tmp.illustration, children: tmp6(_require(6256).WumpusCouchSpotIllustration, {}) };
  const tmp7 = closure_7;
  const tmp8 = closure_3;
  const tmp9 = closure_4;
  obj3[2] = _require(10932).getDisclosureText({ gamePublisher, gameTitle, isTargetedDisclosure, isContextualDisclosure: setting, cosponsorName, isVideoQuest });
  items2[1] = tmp6(_require(4474).Text, obj3);
  if (isTargetedDisclosure) {
    ...
    isTargetedDisclosure = tmp6(tmp2(5601).Card, obj4);
  }
  ...
  obj8[4] = onClose;
  obj7[1] = tmp6(_require(4928).Button, obj8);
