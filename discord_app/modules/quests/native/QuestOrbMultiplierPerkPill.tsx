...
export const QuestOrbMultiplierPerkPill = function QuestOrbMultiplierPerkPill(questId) {
  ...
  const token1 = obj3.useToken(
    orbMultiplierEligibility(709).colors.EXPRESSIVE_GRADIENT_TENURE_BADGE_DIAMOND_END,
    "dark",
  );
  let obj4 = map;
  ...
  if (null == questOrbRewardMultiplier) {
    ...
    if (!tmp13) {
      tmp19 = questOrbRewardMultiplier(tmp2(8104).NitroWheelIcon, { size: "xs", color: "white" });
    }
    obj1 = { children: null };
    const items2 = [tmp19];
    ...
    return questOrbRewardMultiplier(tmp2(5076).PressableOpacity, obj3);
  }
  const obj10 = items5;
};
