...
function useEnrolledQuestContentProps(quest) {
  ...
  let tmp5Result = tmp5(quest(11577).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  const questLogger = quest(7781).getQuestLogger(obj);
  ...
  obj1 = {
    ...
    start: obj8.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj.icon = sourceQuestContent(5648);
        obj.open(obj);
  ...
  obj2 = {
    ...
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11286).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        tmp4Result = tmp4(11287);
        let obj = { type: tmp4(11291).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5502).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5498).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4Result.captureAdUserAction(obj);
      } else {
        obj = { questId: quest.id, questContent: tmp4(5498).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj);
      }
      closure_4(arg0);
    }
