...
function useEnrolledQuestContentProps(quest) {
  ...
  let tmp5Result = tmp5(quest(11616).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  const questLogger = quest(7811).getQuestLogger(obj);
  ...
  obj1 = {
    ...
    start: obj8.useCallback(() => {
      ...
      result.then((errorHints) => dependencyMap(errorHints.errorHints)).catch((error) => {
        ...
        obj.icon = sourceQuestContent(5678);
        obj.open(obj);
  ...
  obj2 = {
    ...
    handleTaskSelect(arg0) {
      ...
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11325).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        tmp4Result = tmp4(11326);
        let obj = { type: tmp4(11330).AdUserActionType.CLICK_INTERNAL, adCreativeType: tmp4(5532).AdCreativeType.QUEST, adCreativeId: quest.id, questContentCTA: DESELECT_PLATFORM, surfaceId: tmp4(5528).QuestContent.QUEST_BOTTOM_SHEET, sourceQuestContent, impressionId };
        tmp4Result.captureAdUserAction(obj);
      } else {
        obj = { questId: quest.id, questContent: tmp4(5528).QuestContent.QUEST_BOTTOM_SHEET, questContentCTA: DESELECT_PLATFORM, sourceQuestContent };
        closure_2(obj);
      }
      closure_4(arg0);
    }
