...
export default function QuestBottomSheetFooter(quest) {
  ...
  obj = { disabled: true, onPressDisabled: xboxAndPlaystationAccounts(14438) };
  let tmp40Result2 = null;
  if (step !== quest(14440).QuestBottomSheetStep.TASK_SELECT) {
    obj1 = { onLayout: null, ctaButton: null, backButton: null, style: null, withSafeArea: null };
    obj1[0] = quest.onLayout;
    if (tmp2(14440).QuestBottomSheetStep.CONSOLE_CONNECT === step) {
      ...
      tmp40Result = null;
      if (tmp2(14440).QuestBottomSheetStep.TASK_STATUS === step) {
        if (tmp11) {
          ...
          tmp40Result = tmp40(tmp2(4777).Button, obj5);
        } else {
          if (callback(obj2.useTaskPlatformScreen(quest, questTaskDetails), 1)[0] === tmp2(5226).TaskPlatformScreen.CONSOLE) {
    ...
    if (tmp40Result1) {
      ...
      obj8[2] = tmp40(tmp2(5335).ArrowLargeLeftIcon, {});
      obj8[3] = onBack;
      tmp40Result1 = tmp40(tmp2(7817).IconButton, obj8);
    }
    obj1[2] = tmp40Result1;
    obj1[3] = style;
    obj1[4] = withSafeArea;
