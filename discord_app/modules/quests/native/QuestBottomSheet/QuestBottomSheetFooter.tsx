// === Module 15389: QuestBottomSheetFooter ===
...
import ContentImpressionTrackerHooks from "ContentImpressionTrackerHooks" /* 11741 */;
...
import AnalyticsHooks from "AnalyticsHooks" /* 12027 */;
import QuestBottomSheetHooks from "QuestBottomSheetHooks" /* 15390 */;
import RefreshIcon from "RefreshIcon" /* 15425 */;
...
export default function QuestBottomSheetFooter(quest) {
  ...
  const mobileActivityPressHandler = quest(15390).useMobileActivityPressHandler({ questId: quest.id, sourceQuestContent, launchMobileActivity });
    ...
    if (tmp2(15387).QuestBottomSheetStep.CONSOLE_CONNECT === step) {
      ...
      if (tmp2(15387).QuestBottomSheetStep.TASK_STATUS === step) {
        if (tmp11) {
          ...
          const obj15 = { grow: true, size: "lg", onPress: mobileActivityPressHandler, text: primaryCtaCopy, icon: tmp2(12020).getPrimaryCtaIcon(quest) };
          let tmp23 = null;
          if (isQuestAccessSuspended) {
            tmp23 = obj10;
          }
          ...
          if (_slicedToArray(obj3.useTaskPlatformScreen(quest, questTaskDetails), 1)[0] === tmp2(5664).TaskPlatformScreen.CONSOLE) {
