...
export default function QuestBottomSheetHeader(arg0) {
  ...
  let obj = isScreenReaderEnabled(14248);
  ...
  const isInGameQuestResult = isScreenReaderEnabled(7104).isInGameQuest(quest);
  c5 = isInGameQuestResult;
  const tmp2Result1 = isScreenReaderEnabled(7104);
  first = gameTitle(isScreenReaderEnabled(10377).useTaskPlatformScreen(quest, questTaskDetails), 1)[0];
  ...
  const tmp2Result2 = isScreenReaderEnabled(10377);
  hasWatchVideoOnMobileTasks = isScreenReaderEnabled(14247).useHasWatchVideoOnMobileTasks(quest.config);
  const tmp2Result3 = isScreenReaderEnabled(14247);
  const items1 = [first];
  ...
  defaultRewardNameWithArticle = isScreenReaderEnabled(10394).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
  const tmp2Result5 = isScreenReaderEnabled(10394);
  const isSponsoredPlayQuestResult = isScreenReaderEnabled(7102).isSponsoredPlayQuest(quest);
  ...
  const tmp2Result6 = isScreenReaderEnabled(7102);
  isScreenReaderEnabled = isScreenReaderEnabled(4681).useIsScreenReaderEnabled();
  ...
  if (tmp23Result) {
    ...
    obj[4] = items5;
    obj[5] = memo1;
    tmp23Result = targetMinutes(tmp2(4281).Text, obj);
  ...
  if (withActionSheet) {
    ...
    obj1[4] = targetMinutes(tmp2(8527).MoreHorizontalIcon, obj2);
    withActionSheet = targetMinutes(tmp2(4827).PressableOpacity, obj1);
  }
