// === Module 12119: QuestContentImpressionTracker ===
...
function AdContentImpressionTrackerBaseNative(skipRemountKey) {
  ...
  let tmp6 = visibilityRef(8622)(tmp3);
  const adContentImpressionTrackerProps = adContentIds2(11829).useAdContentImpressionTrackerProps(skipRemountKey);
  ...
  const obj2 = adContentIds2(11829);
  let items1 = [joined];
  const tmp10 = _slicedToArray(adContentIds2(9077).useRecyclingState(false, items1), 2);
  dependencyMap = tmp11;
  ...
  const obj4 = adContentIds2(9077);
  const obj6 = {};
  const merged1 = Object.assign(skipRemountKey);
  const merged2 = Object.assign({ visible: overrideVisibility, visibleChanged: overrideVisibility !== tmp5(8622)(overrideVisibility) });
  obj6.focused = tmp3;
  ...
  if (tmp17) {
    obj7.adCreativeType = tmp(5756).AdCreativeType.QUEST;
    let tmp19 = obj7;
  } else {
    obj7.adCreativeType = skipRemountKey.adCreativeType;
    tmp19 = obj7;
  }
  return createElement(adContentIds2(11830).QuestContentImpressionTracker, tmp19);
}
let closure_6 = fn(8056).MIN_QUEST_CONTENT_VISIBILITY_PERCENTAGE;
