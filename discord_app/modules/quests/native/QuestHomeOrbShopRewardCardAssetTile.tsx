...
function ProductPreviewInner(cardHeight) {
  ...
  const tmp5 =
    cardWidth < cardWidth(8855).COLLECTIBLES_SHOP_CARD_WIDTH ||
    cardHeight < cardWidth(8855).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1889).CollectiblesItemType.BUNDLE) {
    ...
    if (ALL.has(product.skuId)) {
      size = {
        skuId: product.skuId,
        width: cardWidth(8936).FRACTIONAL_NITRO_COIN_SIZE.CARD,
        height: cardWidth(8936).FRACTIONAL_NITRO_COIN_SIZE.CARD,
      };
