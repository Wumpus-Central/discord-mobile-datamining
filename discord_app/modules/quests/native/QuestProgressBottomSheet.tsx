// === Module 16527: contextMenuButton ===

// Module 16527 (contextMenuButton)
import ThemesDefault from "Themes" /* 712 */;
import getSystemLocale from "getSystemLocale" /* 1236 */;
import IconButton from "IconButton" /* 7938 */;
import registerAssetDefault from "registerAsset" /* 8724 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_7 from "map" /* 9385 */;
import closure_8 from "initializeState" /* 7381 */;
...
function QuestProgressBottomSheet(quest) {
  ...
  const url = claim(9542)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  ...
  const tmp7 = claim(9542);
  tmpResult = tmp(11257);
  obj = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: tmp(5342).QuestContent.RUNNING_ACTIVITY };
  ...
  const isQuestAccessSuspended = quest(11003).useIsQuestAccessSuspended();
  const tmpResult1 = quest(11003);
  obj1 = { quest, questContent: tmp(5342).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: tmp(5342).QuestContent.RUNNING_ACTIVITY };
  const questRewardClaimHandler = quest(14672).useQuestRewardClaimHandler(obj1);
  ...
  const obj9 = { icon: claim(9666), onPress: callback2, variant: "secondary-overlay", size: "sm", accessibilityLabel: null };
  const intl2 = tmp(1236).intl;
  obj9[4] = intl2.string(quest(1236).t.RDE0Sc);
  const items5 = [callback2(quest(7938).IconButton, obj9), ];
  const obj10 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmpResult2 = quest(14672);
  obj10[3] = quest(5342).QuestContent.RUNNING_ACTIVITY;
  obj10[4] = contextMenuButton;
  items5[1] = callback2(claim(14702), obj10);
  obj8[1] = items5;
  ...
  const items7 = [callback2(quest(4441).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = callback2(quest(4441).Text, obj13);
  obj12[3] = items7;
  const items8 = [closure_12(quest(4878).Stack, obj12), ];
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    ...
    obj[2] = tmp(5342).QuestContent.RUNNING_ACTIVITY;
    obj[3] = tmp(5342).QuestContent.RUNNING_ACTIVITY;
    obj[4] = function children() {
      return closure_1_11(closure_1_16, { quest: stateFromStores });
    };
    tmp4 = callback2(tmp(11260).QuestContentImpressionTrackerNative, obj);
  }
  return tmp4;
};