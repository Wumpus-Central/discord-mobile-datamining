...
function QuestProgressBottomSheet(quest) {
  ...
  const url = claim(10683)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  ...
  const tmp7 = claim(10683);
  tmpResult = tmp(11106);
  obj = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: tmp(5163).QuestContent.RUNNING_ACTIVITY };
  ...
  const isQuestAccessSuspended = quest(10393).useIsQuestAccessSuspended();
  const tmpResult1 = quest(10393);
  obj1 = { quest, questContent: tmp(5163).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: tmp(5163).QuestContent.RUNNING_ACTIVITY };
  const questRewardClaimHandler = quest(14292).useQuestRewardClaimHandler(obj1);
  ...
  const items5 = [callback2(quest(7691).IconButton, obj9), ];
  const obj10 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmpResult2 = quest(14292);
  obj10[3] = quest(5163).QuestContent.RUNNING_ACTIVITY;
  obj10[4] = contextMenuButton;
  items5[1] = callback2(claim(14322), obj10);
  obj8[1] = items5;
  ...
  const items7 = [callback2(quest(4298).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = callback2(quest(4298).Text, obj13);
  obj12[3] = items7;
  const items8 = [closure_12(quest(4710).Stack, obj12), ];
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    ...
    obj[2] = tmp(5163).QuestContent.RUNNING_ACTIVITY;
    obj[3] = tmp(5163).QuestContent.RUNNING_ACTIVITY;
    obj[4] = function children() {
      return outer1_11(outer1_16, { quest: stateFromStores });
    };
    tmp4 = callback2(tmp(11109).QuestContentImpressionTrackerNative, obj);
  }
  return tmp4;
};