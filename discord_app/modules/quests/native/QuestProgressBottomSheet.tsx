...
function QuestProgressBottomSheet(quest) {
  ...
  const formatToPlainStringResult = intl.formatToPlainString(quest(1114).t.EAYZAr, {
  ...
  const questsInstructionsToWinReward = tmpResult4.useQuestsInstructionsToWinReward({
    ...
    sourceQuestContent: quest(5528).QuestContent.RUNNING_ACTIVITY,
  });
  if (userStatus != null) {
  ...
  const questRewardClaimHandler = tmpResult6.useQuestRewardClaimHandler({
    quest,
    questContent: quest(5528).QuestContent.RUNNING_ACTIVITY,
    sourceQuestContent: quest(5528).QuestContent.RUNNING_ACTIVITY,
  });
