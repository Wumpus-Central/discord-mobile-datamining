...
class QuestDockWithEntranceAnimation {
  constructor() {
    ...
    f94692 = Backdrop(useRef.useState(() => performance.now()), 1)[0];
    ...
    items = [];
    items[0] = staticUrl;
    f94693 = obj3.useStateFromStores(items, () => staticUrl.prevRestingQuestDockMode);
    ...
    fn = function n() {
      ...
      obj = { translateY: questCreative(4753).withSpring(num2, outer1_35) };
      const items = [obj];
      obj[1] = items;
      return obj;
    };
    obj = { withSpring: require("CONFIG_NEVER_ANIMATE").withSpring, isRendered, ENTRANCE_ANIMATION_SPING_CONFIG: f59622, componentDimensions: first };
    fn.__closure = obj;
    fn.__workletHash = 15545726338295;
    fn.__initData = f59622;
    ...
    effect1 = obj2.useEffect(() => {
      if (false === first1) {
        let obj = isRendered(7103);
        obj = { name: null, tags: null };
        obj[0] = questCreative(7108).MetricEvents.QUEST_CONTENT_RENDERING_FAILURE;
        const _HermesInternal = HermesInternal;
        const items = ["quest_id:" + questCreative.id, , ];
        const _HermesInternal2 = HermesInternal;
        items[1] = "quest_content:" + questCreative(7227).getQuestContentName(questCreative(5204).QuestContent.QUEST_BAR_MOBILE);
        items[2] = "reason:asset_loading_error";
        obj[1] = items;
        obj.increment(obj);
        const obj3 = questCreative(7227);
      }
    }, items2);
      ...
      obj1[2] = function onLayout(height) {
        ...
        if (!ref.current) {
          ...
          if (Math.random() < 0.1) {
            ...
            obj = { name: null, tags: null };
            obj[0] = questCreative(7108).MetricEvents.QUEST_BAR_MOBILE_TIME_TO_FIRST_PAINT;
            const _HermesInternal = HermesInternal;
            const items = ["quest_id:" + questCreative.id];
            obj[1] = items;
            isRendered(7103).distribution(obj, rounded);
            const obj2 = isRendered(7103);
          }
        }
      };
...
const memoResult = importAllResult.memo(function QuestDockWithQuestContext() {
  ...
  if (isMobileQuestDockRenderedBase(7228).AdCreativeType.NO_FILL !== type) {
