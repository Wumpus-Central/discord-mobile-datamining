...
export default noop.memo(function QuestDockVideoBackground(gradientBaseColor) {
  ...
  const setRestingQuestDockMode = windowDimensions.useContext(
    gradientBaseColor(15305).QuestDockExternalCoordinationContext,
  ...
  const obj2 = gradientBaseColor(504);
  const fn = function b() {
    let num = 0;
  ...
  fn.__closure = {
    withSpring: gradientBaseColor(5143).withSpring,
    activeQuestDockMode,
    QuestDockMode,
    QUEST_DOCK_UNENROLLED_HEADER_INSET_EXPANDED: first1,
    QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED: isRendered,
    windowDimensions,
  };
  ...
  const effect = obj.useEffect(() => {
    closure_0 = setRestingQuestDockMode.addEventListener("change", (event) => {
      ...
      const obj = gradientBaseColor(dependencyMap[21]);
    });
    return () => {
      closure_0.remove();
    };
  ...
  const tmp16Result = activeQuestDockMode(
    windowDimensions.useState("active" !== setRestingQuestDockMode.currentState),
    2,
  );
  function te() {
    let num = 1;
  ...
  if (null != imageUrl) {
    ...
    obj9.source = obj10;
    obj8.children = closure_12(expandedHeight(5757), obj9);
    tmp33 = closure_12(expandedHeight(7269), obj8);
    const tmp7Result = expandedHeight(7269);
  }
  items4[1] = tmp33;
