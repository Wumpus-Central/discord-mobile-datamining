// === Module 15513: QuestDockContentCollapsed ===
...
const QUEST_DOCK_MODE_CHANGE_PHYSICS = fn(15422).QUEST_DOCK_MODE_CHANGE_PHYSICS;
const jsx = fn(21).jsx;
...
export default noop.memo(function QuestDockContentCollapsed(children) {
  ...
  hideOnExpand = tmp;
  const activeQuestDockMode = noop.useContext(hideOnExpand(15423).QuestDockGestureContext).activeQuestDockMode;
  ...
  let obj = hideOnExpand(4559);
  fn.__closure = { withSpring: hideOnExpand(5270).withSpring, activeQuestDockMode, QuestDockMode, hideOnExpand: tmp, QUEST_DOCK_MODE_CHANGE_PHYSICS };
  fn.__workletHash = 13361221764426;
  fn.__initData = __initData;
  const animatedStyle = obj.useAnimatedStyle(fn);
  const obj2 = { withSpring: hideOnExpand(5270).withSpring, activeQuestDockMode, QuestDockMode, hideOnExpand: tmp, QUEST_DOCK_MODE_CHANGE_PHYSICS };
  class C {
    constructor() {
      pointerEvents = "auto";
      if (activeQuestDockMode.get() !== QuestDockMode.COLLAPSED) {
  ...
  const animatedProps = hideOnExpand(4559).useAnimatedProps(C);
  const obj4 = { style: null, animatedProps, children: children.children };
  const items = [tmp2.questDockContentCollapsed, animatedStyle];
  obj4.style = items;
  return jsx(activeQuestDockMode(7404), { style: null, animatedProps, children: children.children });
});