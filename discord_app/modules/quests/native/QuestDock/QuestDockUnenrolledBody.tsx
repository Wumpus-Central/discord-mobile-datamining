...
const memoResult = importAllResult.memo(function QuestDockUnenrolledBody() {
  ...
  const isRendered = setRestingQuestDockMode.useContext(
    hasWatchVideoOnMobileTasks(isMobileActivityQuest[6]),
  ).isRendered;
  ...
  hasWatchVideoOnMobileTasks = questDockQuest(isMobileActivityQuest[9]).useHasWatchVideoOnMobileTasks(
    questDockQuest.config,
  );
  ...
  const shouldShowBonusOrbsUX = questDockQuest(isMobileActivityQuest[10]).useShouldShowBonusOrbsUX(
    questDockQuest,
  ...
  const primaryCtaCopy = questDockQuest(isMobileActivityQuest[20]).usePrimaryCtaCopy({
  ...
  obj = {
    ...
    sourceQuestContent: tmp(tmp2[15]).QuestContent.QUEST_BAR_MOBILE,
  };
  ...
  const callback1 = obj1.useCallback(
    () => questDockQuest(isMobileActivityQuest[21]).getPrimaryCtaIcon(questDockQuest, true),
    items2,
  );
  ...
  obj1 = {
    rewardTile: tmp19(tmp(tmp2[22]).QuestDockBodyQuestRewardTile, obj),
  ...
  obj1[8] = tmp12;
  return jsx(tmp4(isMobileActivityQuest[22]), {
    rewardTile: tmp19(tmp(tmp2[22]).QuestDockBodyQuestRewardTile, obj),
    premiumRewardPerkPill: null,
    title: null,
    description: null,
