...
import StarfieldBackgroundDefault from "BountiesCtaHeader.tsx";
import ListEdgeSpacerDefault from "QuestHomeOrbShopCarousel.tsx";
  ...
  if (0 !== questHomeBounties.length) {
    if (!tmpResult.useStateFromStores(items4, () => closure_6.areAllBountiesCompleted())) {
      if (verticalScrollEnabled) {
        ...
        if (tmp12) {
          tmp12 =
            orbShopProducts.length >= tmp(14897).MIN_PRODUCTS_FOR_ORB_SHOP_CAROUSEL || showOrbShopPlaceholderCarousel;
          const tmp13 =
            orbShopProducts.length >= tmp(14897).MIN_PRODUCTS_FOR_ORB_SHOP_CAROUSEL || showOrbShopPlaceholderCarousel;
        }
        if (!tmp12) {
