...
function QuestHomeBountiesCarousel(bounties) {
  ...
  const callback7 = first1.useCallback((bountyId, questContentPosition) => {
    ...
    if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_home_bounties")) {
      ...
      tmpResult = tmp(7718);
      obj = { adContentId: bountyId.id, adCreativeType: tmp(5451).AdCreativeType.BOUNTY, questContent: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, questContentCTA: tmp(7728).QuestContentCTA.START_BOUNTY, sourceQuestContent: tmp(5447).QuestContent.QUEST_HOME_MOBILE_CAROUSEL, questContentPosition };
...
export default noop.memo(function QuestHomeBounties(shopCarouselConfig) {
  ...
  if (verticalScrollEnabled) {
    ...
    let tmp29Result = tmp29(BountiesCtaHeaderDefault, obj4);
  } else {
