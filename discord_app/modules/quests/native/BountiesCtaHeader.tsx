...
function BountiesCtaDescription(isEmptyOrCompleted) {
  ...
  const tmp7 = callback(tmp5(4340).Text, obj);
  ...
  if (isEmptyOrCompleted) {
    ...
    obj[1] = items;
    let tmp9Result = tmp9(tmp5(4340).Text, obj);
  } else {
    ...
    obj2[1] = formatResult;
    items1[1] = callback(tmp5(4340).Text, obj2);
    obj1[1] = items1;
...
function BountiesCtaHeaderInner(bounties) {
  ...
  let obj2 = bounties(11039);
  const QuestContent = bounties(5204).QuestContent;
  ...
  const items6 = [callback(bounties(4340).Text, obj3), callback(BountiesCtaDescription, { isEmptyOrCompleted: flag }), ];
  ...
  if (!flag) {
    ...
    obj4[4] = callback;
    tmp13Result = callback(tmp2(4754).Button, obj4);
  }
...
const memoResult = importAllResult.memo(function BountiesCtaHeader(isEmptyOrCompleted) {
  ...
  if (!tmp3) {
    ...
    if (tmp3) {
      ...
      obj1[0] = closure_8;
      obj1[1] = tmp7(7228).AdCreativeType.BOUNTY;
      obj1[2] = tmp7(5204).QuestContent.QUEST_HOME_ENTRYPOINT_MOBILE;
      obj1[4] = tmp2.isInView;
      obj1[5] = tmp7(5204).QuestContent.QUEST_HOME_ENTRYPOINT_MOBILE;
      ...
      tmp9Result = tmp9(tmp7(11234).QuestContentImpressionTrackerNative, obj1);
    }
    obj[1] = tmp9Result;
