...
function BountiesCtaDescription(isEmptyOrCompleted) {
  ...
  if (isEmptyOrCompleted) {
    ...
    obj[1] = items;
    let tmp9Result = tmp9(tmp5(4338).Text, obj);
  } else {
    ...
    obj2[1] = formatResult;
    items1[1] = callback(tmp5(4338).Text, obj2);
    obj1[1] = items1;
...
function BountiesCtaHeaderInner(bounties) {
  ...
  const items6 = [callback(bounties(4338).Text, obj3), callback(BountiesCtaDescription, { isEmptyOrCompleted: flag }), ];
...
const memoResult = importAllResult.memo(function BountiesCtaHeader(isEmptyOrCompleted) {
  ...
  if (!tmp3) {
    ...
    if (tmp3) {
      ...
      obj1[0] = closure_8;
      obj1[1] = tmp7(7233).AdCreativeType.BOUNTY;
      ...
      tmp9Result = tmp9(tmp7(11225).QuestContentImpressionTrackerNative, obj1);
    }
    obj[1] = tmp9Result;
