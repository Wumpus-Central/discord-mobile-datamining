...
const memoResult = importAllResult.memo(function VideoQuestModalContentCompleted(onRestartVideo) {
  ...
  obj = { withDelay: tmp2(4162).withDelay, ANIMATION_DELAY: 125, withSpring: tmp2(4711).withSpring, isComponentMounted: sharedValue, ANIMATED_CONTENT_SPRING_CONFIG: closure_10, interpolate: tmp2(4162).interpolate, ANIMATED_CONTENT_OFFSET_Y: 75 };
  fn.__closure = obj;
  fn.__workletHash = 2704439293952;
  fn.__initData = closure_12;
  const items1 = [sharedValue];
  const animatedStyle = quest(4162).useAnimatedStyle(fn);
  const effect = obj1.useEffect(() => {
    const result = sharedValue.set(1);
  }, items1);
  const tmp2Result1 = quest(4162);
  let isShareableQuestResult = quest(7121).isShareableQuest(quest.config);
  ...
  const windowDimensions = obj1.useContext(tmp2(14266).QuestDockGestureContext).windowDimensions;
  const tmp2Result2 = quest(7121);
  let str = "md";
  ...
  tmp17 = windowDimensions.get().height < 800;
  [tmp19, ShareIcon] = callback(obj1.useState(null), 2);
  const callback1 = obj1.useCallback((nativeEvent) => {
    callback(nativeEvent.nativeEvent.layout.height);
  }, []);
  if (tmp19 == null) {
    tmp19 = tmp2(14301).QUEST_PROGRESS_DIAMETER_BY_SIZE[str];
  }
  ...
  const items3 = [callback2(quest(4298).Heading, obj3), callback2(sharedValue(14319), { onClose, style: tmp.closeButton })];
  obj2[4] = items3;
  const items4 = [callback3(quest(4710).Stack, obj2), ];
    ...
    if (!isClaiming) {
      ...
      if (!tmp9) {
        ...
        if (isQuestAccessSuspended) {
          tmp6Result = tmp6(14288);
        }
        obj9[6] = tmp6Result;
        tmp23Result = tmp23(tmp2(4712).Button, obj9);
      }
      items7[1] = tmp23Result;
      obj8[4] = items7;
      obj7[1] = tmp22(tmp2(4710).Stack, obj8);
      ...
      const items10 = [tmp23(tmp2(4298).Heading, obj13), ];
      const obj14 = { variant: "text-sm/medium", color: "text-subtle", children: null };
      obj14[2] = quest.config.messages.gamePublisher;
      items10[1] = tmp23(tmp2(4298).Text, obj14);
      obj12[2] = items10;
      const items11 = [tmp22(tmp2(4710).Stack, obj12), ];
      ...
      obj15[3] = tmp23(tmp2(9795).RetryIcon, obj16);
      items11[1] = tmp23(tmp2(4844).PressableOpacity, obj15);
      obj11[3] = items11;
      const items12 = [tmp22(tmp2(4710).Stack, obj11), , ];
      ...
      if (tmp23Result) {
        ...
        obj20[0] = memo.url;
        obj19[1] = obj20;
        obj17[3] = tmp23(tmp6(5267), obj19);
        tmp23Result = tmp23(tmp2(4844).PressableOpacity, obj17);
      }
      ...
      obj22[3] = tmp2(10415).getExternalCtaLabel(quest);
      const items13 = [tmp23(tmp2(4712).Button, obj22), ];
      if (isShareableQuestResult) {
        ...
        obj23[3] = tmp23(tmp2(12163).ShareIcon, obj24);
        isShareableQuestResult = tmp23(tmp2(4844).PressableOpacity, obj23);
      }
      items13[1] = isShareableQuestResult;
      obj21[3] = items13;
      items12[2] = tmp22(tmp2(4710).Stack, obj21);
      obj10[1] = items12;
      items8[1] = tmp22(closure_7, obj10);
      obj6[4] = items8;
      items4[1] = tmp22(closure_6, obj6);
      obj1[2] = items4;
      return tmp22(tmp2(5327).SafeAreaPaddingView, obj1);
    }
  }
  const obj25 = { style: tmp.contentRewards, onLayout: callback1, children: null };
  const items14 = [callback2(sharedValue(14320), { withQuestName: false, withRewardAvailableCopy: false, size: str, withRewardTileAnimation: true }), ];
  ...
  obj26 = tmp23(tmp2(4298).Heading, obj26);
  items14[1] = obj26;
  obj25[2] = items14;
