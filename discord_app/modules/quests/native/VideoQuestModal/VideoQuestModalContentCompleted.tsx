...
const memoResult = importAllResult.memo(function VideoQuestModalContentCompleted(onRestartVideo) {
  ...
  let obj3 = quest(14474);
  obj = { quest, onSuccess: sharedValue(10389).close, sourceQuestContent };
  ...
  obj = { withDelay: tmp2(4115).withDelay, ANIMATION_DELAY: 125, withSpring: tmp2(4744).withSpring, isComponentMounted: sharedValue, ANIMATED_CONTENT_SPRING_CONFIG: closure_10, interpolate: tmp2(4115).interpolate, ANIMATED_CONTENT_OFFSET_Y: 75 };
  fn.__closure = obj;
  fn.__workletHash = 2704439293952;
  fn.__initData = closure_12;
  const items1 = [sharedValue];
  const animatedStyle = quest(4115).useAnimatedStyle(fn);
  ...
  const height = tmp6(8995)(obj1.useContext(tmp2(14448).QuestDockGestureContext).windowDimensions).height;
  let str = "md";
  if (height >= 760) {
    let str2 = "lg";
    if (height < 800) {
  ...
  if (tmp18 == null) {
    tmp18 = tmp2(14483).QUEST_PROGRESS_DIAMETER_BY_SIZE[str];
  }
  ...
  const items3 = [callback2(quest(4734).Heading, obj3), callback2(sharedValue(14501), { onClose, style: tmp.closeButton })];
  obj2[4] = items3;
  const items4 = [callback3(quest(4733).Stack, obj2), ];
  ...
  const items14 = [callback2(sharedValue(14502), { withQuestName: false, withRewardAvailableCopy: false, size: str, withRewardTileAnimation: true }), ];
