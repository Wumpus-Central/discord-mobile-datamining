...
export default noop.memo(function VideoQuestModalContentInProgress(arg0) {
  ...
  const callback2 = noop.useCallback(() => {
    if (isShareableQuestResult) {
      ...
      if (
        ...
        const obj2 = {
          type: getQuestUrl(7152).AdUserActionType.CLICK_INTERNAL,
          adCreativeType: getQuestUrl(5763).AdCreativeType.QUEST,
          adCreativeId: quest.id,
          questContentCTA: getQuestUrl(7141).QuestContentCTA.MOBILE_SHARESHEET,
          surfaceId: getQuestUrl(5759).QuestContent.VIDEO_MODAL_MOBILE,
          sourceQuestContent,
  ...
  const videoQuestClickCtaAndMaybeCloseModal = sourceQuestContent(14686).useVideoQuestClickCtaAndMaybeCloseModal({
    quest,
    onClose,
    sourceQuestContent,
  });
  ...
  const tmp19 =
    quest.config.taskConfigV2.tasks[sourceQuestContent(undefined, 5764).FirstPartyQuestTaskTypes.WATCH_VIDEO_ON_MOBILE];
  let tmp20 = null == tmp19;
  if (!tmp20) {
    tmp20 = "portrait" === sourceQuestContent(10735).getVideoOrientation(tmp19);
    const tmpResult = sourceQuestContent(10735);
  }
