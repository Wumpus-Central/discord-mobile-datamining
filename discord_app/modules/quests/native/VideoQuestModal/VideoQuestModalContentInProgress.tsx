...
export default noop.memo(function VideoQuestModalContentInProgress(arg0) {
  ...
  const callback2 = noop.useCallback(() => {
    if (isShareableQuestResult) {
      ...
      if (
        ...
        const obj2 = {
          type: getQuestUrl(7974).AdUserActionType.CLICK_INTERNAL,
          adCreativeType: getQuestUrl(5663).AdCreativeType.QUEST,
          adCreativeId: quest.id,
          questContentCTA: getQuestUrl(7962).QuestContentCTA.MOBILE_SHARESHEET,
          surfaceId: getQuestUrl(5659).QuestContent.VIDEO_MODAL_MOBILE,
          sourceQuestContent,
  ...
  const videoQuestClickCtaAndMaybeCloseModal = sourceQuestContent(15416).useVideoQuestClickCtaAndMaybeCloseModal({
    quest,
    onClose,
    sourceQuestContent,
  });
  ...
  const tmp19 =
    quest.config.taskConfigV2.tasks[sourceQuestContent(undefined, 5664).FirstPartyQuestTaskTypes.WATCH_VIDEO_ON_MOBILE];
  let tmp20 = null == tmp19;
  if (!tmp20) {
    tmp20 = "portrait" === sourceQuestContent(11765).getVideoOrientation(tmp19);
    const tmpResult = sourceQuestContent(11765);
  }
