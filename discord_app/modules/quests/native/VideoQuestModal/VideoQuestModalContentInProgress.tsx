...
let closure_25 = noop.memo((arg0) => {
  ...
  items3[1] = obj;
  const obj2 = { theme: ThemeTypes.DARKER, children: closure_11(contentWidth(memo[17]).VideoQuestPlayer, { captionsEnabled, onLoad: callback, externallyPaused, orientation: "portrait", contentInsets, handleOpenTranscript, handleToggleCaptions, isFullscreen, onEnd, sourceQuestContent, hasCaptionAsset, hasTranscriptAsset }) };
  const items4 = [closure_11(contentWidth(memo[16]).ThemeContextProvider, obj2), closure_11(height(memo[38]), { start, end, style: tmp18Result.gradientTop, colors: items }), closure_11(height(memo[38]), { start, end, style: tmp18Result.gradientBottom, colors: items1 }), ];
  ...
  const items7 = [closure_11(contentWidth(memo[25]).Text, { variant: "heading-lg/semibold", color: "text-overlay-light", style: tmp18Result.textShadow, accessibilityRole: "header", children: quest.config.messages.gameTitle }), ];
  ...
  const obj13 = { style: tmp18Result.questDetailsSecondary, children: closure_11(height(memo[8]), { quest, size: "x-sm", progress: questTaskDetails.percentComplete, hasConfetti: true, onPress: handleRewardDetailsPress, accessibilityLabel: videoQuestProgressRemainingAccessibilityLabel }) };
  ...
  if (null != completedAt) {
    ...
    const obj18 = { icon: closure_11(contentWidth(tmp2[40]).ArrowSmallRightIcon, { size: "sm" }), iconPosition: "end", onPress: onNavigateToPostWatchVideo, variant: "secondary", size: "sm", text: null };
...
export default noop.memo(function VideoQuestModalContentInProgress(arg0) {
  ...
  const callback1 = noop.useCallback(() => {
    const obj = { quest };
    obj.openLazy(asyncRequireImpl(15184, dependencyMap.paths), "transcript-" + quest.id, obj);
  }, items2);
  ...
  const obj2 = sourceQuestContent(11518);
  ...
  const callback2 = noop.useCallback(() => {
    if (isShareableQuestResult) {
      ...
      if (obj.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "video_quest_modal_in_progress")) {
        ...
        obj.questContent = getQuestUrl(5461).QuestContent.VIDEO_MODAL_MOBILE;
        obj.questContentCTA = getQuestUrl(7742).QuestContentCTA.MOBILE_SHARESHEET;
  ...
  const obj3 = sourceQuestContent(7736);
  ...
  const obj4 = sourceQuestContent(15185);
  ...
  if (!tmp20) {
    tmp20 = "portrait" === sourceQuestContent(11544).getVideoOrientation(tmp19);
    const tmpResult = sourceQuestContent(11544);
  }
