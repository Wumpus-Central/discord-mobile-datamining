...
export default function VideoQuestModalHeader(showCurrentVideoTime) {
  ...
  const tmp6 = useVideoQuestUIStore((arg0) => {
    ...
    if (tmp == null) {
      ...
      ({
        progressSeconds: obj.timestampSec,
        targetSeconds: obj.duration,
        progressSeconds: obj.maxTimestampSec,
      } = questTaskDetails);
      tmp = obj;
    }
  ...
  if (null == completedAt) {
    const videoQuestProgressRemainingAccessibilityLabel = tmp3(11606).getVideoQuestProgressRemainingAccessibilityLabel(
      questTaskDetails,
      tmp8,
    );
  ...
  const items1 = [closure_5(quest(4602).Text, obj1)];
  const obj3 = {
  ...
  items1[1] = closure_5(quest(4602).Text, obj3);
  obj.children = items1;
  const items2 = [
    closure_6(View, obj),
    closure_5(questTaskDetails(15206), { iconColor: closeButtonIconColor, onClose }),
  ];
  obj.children = items2;
