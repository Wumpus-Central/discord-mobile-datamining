...
const VideoQuestUIStore = fn(7118);
...
export default function VideoQuestTranscriptActionSheet(quest) {
  ...
  const effect = noop.useEffect(() => {
    const questAsset = AssetUtils.getQuestAsset(
      quest,
      AssetUtils.QuestAssetType.VIDEO_PLAYER_TRANSCRIPT,
      undefined,
      true,
    );
  ...
  obj.header = closure_7(quest(6570).BottomSheetTitleHeader, obj2);
  ...
  const items2 = [
    tmp7Result,
    memo.length > 0 &&
      memo.map((children, index) =>
        closure_1_7(
          quest(dependencyMap[14]).Text,
          { variant: "heading-md/normal", color: "text-muted", children },
          index,
        ),
      ),
  ];
  obj4.children = items2;
  obj3.children = closure_8(quest(5279).Stack, obj4);
  obj.children = closure_7(quest(6045).BottomSheetScrollView, obj3);
  return closure_7(quest(6618).ActionSheet, obj);
}
