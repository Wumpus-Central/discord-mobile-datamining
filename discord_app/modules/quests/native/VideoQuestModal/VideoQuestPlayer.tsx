...
const memoResult = importAllResult.memo((onLoad) => {
  ...
  if (null != url) {
    let VIDEO_PLAYER_VIDEO = tmp3(10477).QuestAssetType.VIDEO_PLAYER_VIDEO_HLS;
  } else {
    VIDEO_PLAYER_VIDEO = tmp3(10477).QuestAssetType.VIDEO_PLAYER_VIDEO;
  }
  ...
  const tmp16 = memo((arg0) => {
    ...
    return tmp;
  }, handleBufferAnalytics(3933).shallow);
  obj = tmp16;
  ...
  sharedValue1 = handleBufferAnalytics(4042).useSharedValue(0);
  const tmp3Result1 = handleBufferAnalytics(4042);
  class U {
    constructor() {
      obj = { width: null };
  ...
  const animatedStyle1 = handleBufferAnalytics(4042).useAnimatedStyle(U);
  ...
  const tmp3Result2 = handleBufferAnalytics(4042);
  const defaultWatchVideoTask = handleBufferAnalytics(7184).getDefaultWatchVideoTask(quest.config);
  ...
  const tmp3Result3 = handleBufferAnalytics(7184);
  ...
  if (null == memo2) {
    ...
    obj7[3] = num6;
    obj6[3] = obj7;
    items29[1] = targetSeconds(tmp3(14395).VideoQuestCaptions, obj6);
    if (!tmp72) {
      items29[2] = tmp72;
      tmp80Result = !tmp39;
      if (!tmp39) {
      ...
      if (tmp80Result4) {
        ...
        if (!hasCaptionAsset) {
          ...
          if (flag2) {
            ...
            obj11[4] = tmp80(tmp3(14281).TranscriptOutlineIcon, obj12);
          ...
          obj14[0] = color;
          obj13[4] = tmp80(tmp3(14279).ClosedCaptionsOutlineIcon, obj14);
      ...
      obj16[4] = tmp80(tmp3(14284).SkipBackwardIcon, obj17);
      const items34 = [tmp80(tmp3(14283).VideoQuestPlayerControlButton, obj16), , ];
      if (!tmp22) {
        ...
        if (!tmp22) {
          if (first1 !== tmp17.ERRORED) {
            if (tmp70) {
              let PauseIcon = tmp3(8861).PlayIcon;
            } else {
              PauseIcon = tmp3(8863).PauseIcon;
            }
            let tmp80Result2 = tmp80(PauseIcon, { size: "lg" });
          }
          obj18[3] = tmp80Result2;
          items34[1] = tmp80(tmp3(14283).VideoQuestPlayerControlButton, obj18);
          ...
          obj19[4] = tmp80(tmp3(14285).SkipForwardIcon, obj20);
          items34[2] = tmp80(tmp3(14283).VideoQuestPlayerControlButton, obj19);
          obj15[1] = items34;
          ...
          if (tmp80Result4) {
            ...
            obj24[0] = items36;
            obj23[1] = tmp80(tmp29(4042).View, obj24);
        ...
        tmp80Result2 = tmp80(tmp3(9867).RetryIcon, { size: "lg" });
      }
