...
const memoResult = importAllResult.memo((onLoad) => {
  ...
  let obj2 = handleBufferAnalytics(10506);
  ...
  if (null != url) {
    let VIDEO_PLAYER_VIDEO = tmp3(10518).QuestAssetType.VIDEO_PLAYER_VIDEO_HLS;
  } else {
    VIDEO_PLAYER_VIDEO = tmp3(10518).QuestAssetType.VIDEO_PLAYER_VIDEO;
  }
  ...
  obj = { shouldShowControls, withSpring: tmp3(4753).withSpring, SUBTLE_SPRING: tmp3(4757).SUBTLE_SPRING };
  G.__closure = obj;
  ...
  obj = { withTiming: tmp3(4345).withTiming, progress: sharedValue1, timingFast: tmp3(4348).timingFast };
  U.__closure = obj;
  ...
  const defaultWatchVideoTask = handleBufferAnalytics(7223).getDefaultWatchVideoTask(quest.config);
  ...
  const tmp3Result3 = handleBufferAnalytics(7223);
    ...
    if (!tmp72) {
      ...
      if (tmp80Result4) {
        ...
        if (!hasCaptionAsset) {
          ...
          if (flag2) {
            ...
            flag2 = tmp80(tmp3(4886).PressableOpacity, obj11);
          }
          items32[1] = flag2;
          obj10[1] = items32;
          ...
          tmp80(tmp3(4886).PressableOpacity, obj13);
        }
      }
      ...
      if (!tmp22) {
        ...
        if (!tmp22) {
          if (first1 !== tmp17.ERRORED) {
            if (tmp70) {
              let PauseIcon = tmp3(8900).PlayIcon;
            } else {
              PauseIcon = tmp3(8902).PauseIcon;
            }
          ...
          if (tmp80Result4) {
            ...
            obj21[1] = tmp80(tmp3(4886).PressableOpacity, obj22);
        ...
        tmp80Result2 = tmp80(tmp3(9908).RetryIcon, { size: "lg" });
      }
