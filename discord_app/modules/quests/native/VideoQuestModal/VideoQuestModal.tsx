...
let closure_14 = noop.memo((sourceQuestContent) => {
  ...
  if (initialStep != null) {
    ...
    const tmp17 = sourceQuestContent(tmp2[11])(
      obj1.useContext(initialStep(tmp2[10]).QuestDockGestureContext).windowDimensions,
    );
    ({ width, height } = tmp17);
    let bound = null;
    if (tmp17.landscape) {
    ...
    __closure = {
      withSpring: initialStep(tmp2[13]).withSpring,
      clamp: initialStep(tmp2[12]).clamp,
      postWatchAnimationState: sharedValue,
      BACKGROUND_ENTRANCE_ANIMATION_SPRING_CONFIG: pillarboxed,
      interpolate: initialStep(tmp2[12]).interpolate,
