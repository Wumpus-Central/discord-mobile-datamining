...
function useAllQuests(quests, sortMethod) {
  ...
  const enabled = require("RenewableEndDateSortExperiment").useConfig({
  ...
  const items8 = [
    obj2.useMemo(() => {
      ...
      const withResult = match.with(
        ...
        () => quest(5664).TaskPlatformScreen.CONSOLE,
      );
      const obj4 = { currentProgressingPlatform: null, lastPlatformProgress: constants.CONSOLE };
      const withResult1 = match
        .with({ currentProgressingPlatform: constants.CONSOLE }, () => quest(5664).TaskPlatformScreen.CONSOLE)
        .with({ currentProgressingPlatform: constants.DESKTOP }, () => quest(5664).TaskPlatformScreen.DESKTOP);
      ...
      const withResult3 = match
        ...
        .with(
          ...
          () => quest(5664).TaskPlatformScreen.DESKTOP,
        );
      const obj7 = {
        currentProgressingPlatform: null,
        lastPlatformProgress: null,
...
export const useQuestHowToHelpArticle = function useQuestHowToHelpArticle() {
  ...
  const formatResult = intl.format(accounts(1115).t.beN4DG, {
    ...
    if (tmp17) {
      if (!hasItem) {
        hasItem =
          ...
          _slicedToArray(useTaskPlatformScreen(userStatus, tmp5), 1)[0] === tmp11(5664).TaskPlatformScreen.DESKTOP;
        const tmp21 =
          0 === tmp5.percentComplete &&
          _slicedToArray(useTaskPlatformScreen(userStatus, tmp5), 1)[0] === tmp11(5664).TaskPlatformScreen.DESKTOP;
      }
...
export const useQuestHomeSortOptions = function useQuestHomeSortOptions() {
  return noop.useMemo(() => {
    ...
    return keys.map((item) => {
      const obj = {
        label: closure_1_0(closure_1_2[37]).getSortMethodText(dependencyMap[item]),
...
export const useQuestHomeSortingFilteringAnalytics = function useQuestHomeSortingFilteringAnalytics(
  ...
  const effect = noop.useEffect(() => {
    AnalyticsUtilsDefault.track(constants7.QUEST_HOME_SORT_METHOD_CHANGED, {
...
export const useFetchQuestHomeBounties = function useFetchQuestHomeBounties(arg0) {
  ...
  const stateFromStoresObject = previewAdCreativeIds(enabled2[14]).useStateFromStoresObject(items, () => ({
            ...
            if (arg0 === 1) {
              ...
              if (null != closure_0) {
                if (arr.length > 0) {
                  ...
                  c4 = 1;
                  const obj6 = {
                    value: previewAdCreativeIds(11776).fetchBountyPreview(
                      arr,
                      previewAdCreativeIds(5664).AdPlacement.VIDEO_MODAL_MOBILE,
                    ),
                    done: false,
                  };
                  return obj6;
              ...
              c4 = 1;
              const obj7 = {
                value: previewAdCreativeIds(11776).fetchQuestHomeBounties(
                  previewAdCreativeIds(5664).AdPlacement.VIDEO_MODAL_MOBILE,
                ),
                done: false,
              };
