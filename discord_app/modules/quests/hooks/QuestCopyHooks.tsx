    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5399).TaskPlatformScreen.SELECT) {
        if (tmpResult2.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(11151).getDefaultRewardNameWithArticle(
                quest.config,
                stateFromStores,
              );
              const intl5 = tmp(1233).intl;
...
export const useQuestBarTitle = function useQuestBarTitle(questDockQuest) {
  ...
  if (null != completedAt) {
    ...
    if (
      callback(obj3.useTaskPlatformScreen(questDockQuest, questTaskDetails), 1)[0] ===
      tmp(5399).TaskPlatformScreen.SELECT
    ) {
      const intl2 = tmp(1233).intl;
...
export const usePremiumExtendableCopy = function usePremiumExtendableCopy(arg0) {
  ...
  if (!memo) {
    ...
    const string = intl.string;
    hh7Rb_ = require("../../../intl/index.native.tsx").t;
    if (tmp2) {
      hh7Rb_ = hh7Rb_["hh7Rb/"];
