// === Module 11304: _getQuestsInstructionsToWinReward ===

// Module 11304 (_getQuestsInstructionsToWinReward)
import initialize from "initialize" /* 589 */;
import combinedDefault from "combined" /* 1995 */;
import isSponsoredPlayQuest from "isSponsoredPlayQuest" /* 7435 */;
...
import isActivitySupportedOnClientPlatformDefault from "isActivitySupportedOnClientPlatform" /* 9405 */;
import _getDefaultRewardName from "_getDefaultRewardName" /* 10889 */;
import useQuests from "useQuests" /* 11046 */;
import useInGameQuestConnectState from "useInGameQuestConnectState" /* 11306 */;
...
function _getQuestsInstructionsToWinReward(arg0) {
  ...
  if (isConsoleQuestResult) {
    isConsoleQuestResult = _1votF6(7437).shouldUsePlayOnDesktopTask(quest);
    const v1votF6Result = _1votF6(7437);
  }
  const v1votF6Result1 = _1votF6(7435);
  const isSponsoredPlayQuestResult = _1votF6(7435).isSponsoredPlayQuest(quest);
  const defaultRewardNameWithArticle = _1votF6(10889).getDefaultRewardNameWithArticle(quest.config, currentUser);
  if (isSponsoredPlayQuestResult) {
    ...
    if (withoutMarkdown) {
      ...
      obj = undefined;
      obj = {};
      const gameSheetHook = _1votF6(11305).createGameSheetHook(obj);
      ...
      const v1votF6Result3 = _1votF6(11305);
    }
  } else if (isConsoleQuestResult) {
    const targetMinutes3 = taskDetails.targetMinutes;
    const result = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
    const v1votF6Result4 = _1votF6(10889);
    const features4 = quest.config.features;
    const result1 = _1votF6(10889).isCollectibleQuestRewardPermanentWithPremiumSubscription(quest.config);
    ...
    const v1votF6Result5 = _1votF6(10889);
  } else {
    if (v1votF6Result6.isConsoleQuest(quest)) {
      const targetMinutes2 = taskDetails.targetMinutes;
      const result2 = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
      const v1votF6Result7 = _1votF6(10889);
      if (isPremiumResult) {
        if (result2) {
      ...
      v1votF6Result8 = _1votF6(10889);
    } else {
      if (v1votF6Result9.shouldUsePlayOnDesktopTask(quest)) {
        const result3 = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
        const v1votF6Result10 = _1votF6(10889);
        const features3 = quest.config.features;
        const result4 = _1votF6(10889).isCollectibleQuestRewardPermanentWithPremiumSubscription(quest.config);
        ...
        const v1votF6Result11 = _1votF6(10889);
      } else {
        ...
        if (v1votF6Result12.isTieredRewardCodeQuest(obj5)) {
          ...
          const v1votF6Result13 = _1votF6(7437);
          const v1votF6Result14 = _1votF6(7437);
          if (hasWatchVideoTasksResult) {
            ...
            const result5 = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
            const v1votF6Result15 = _1votF6(10889);
            if (result5) {
              if (isPremiumResult) {
            ...
            v1votF6Result17 = _1votF6(7435);
          } else {
            if (v1votF6Result18.hasAchievementInGameTask(quest)) {
              if (null != thirdPartyTaskDetails) {
                const result6 = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
                _1votF6(10889);
                if (needsToConnect) {
                ...
                const v1votF6Result19 = _1votF6(10889);
              }
            }
            if (null != thirdPartyTaskDetails) {
              let description = thirdPartyTaskDetails.description;
            } else {
              const result7 = _1votF6(10889).isCollectibleQuestRewardPremiumExtendable(quest.config);
              const v1votF6Result21 = _1votF6(10889);
              if (isPremiumResult) {
                if (result7) {
              ...
              v1votF6Result22 = _1votF6(10889);
            }
            return description;
          }
          hasWatchVideoTasksResult = _1votF6(7437).hasWatchVideoTasks(quest);
        }
        v1votF6Result12 = _1votF6(10889);
      }
      v1votF6Result9 = _1votF6(7437);
    }
    v1votF6Result6 = _1votF6(7437);
  }
}
...
export const useQuestInstructionTitle = function useQuestInstructionTitle(config, arg1, questContent, sourceQuestContent) {
  ...
  if (tmpResult.isConsoleQuest(config)) {
    ...
    if (tmpResult1.hasWatchVideoTasks(config)) {
      const defaultWatchVideoTask = tmp(7437).getDefaultWatchVideoTask(config.config);
      let videoTitle;
      if (defaultWatchVideoTask != null) {
      ...
      if (tmpResult3.hasPlayActivityTask(config)) {
        const result = tmp(7435).isPlayAnyActivityQuest(config);
...
export const useQuestBarSubtitle = function useQuestBarSubtitle(arg0) {
  ...
  if (tmp8) {
    ...
    if (isExpanded) {
      ...
      if (activeScreen !== tmp(5358).TaskPlatformScreen.SELECT) {
        if (tmpResult2.isSponsoredPlayQuest(quest)) {
          if (tmp10) {
            if (!tmp11) {
              const defaultRewardNameWithArticle = tmp(10889).getDefaultRewardNameWithArticle(quest.config, stateFromStores);
              const intl5 = tmp(1236).intl;
      ...
      if (tmpResult1.hasWatchVideoTasks(quest)) {
        ...
        if (!tmpResult4.hasAchievementInGameTask(quest)) {
          if (isQuestProgressing) {
            ...
            let contextualEntrypointHeading = tmp(10894).getContextualEntrypointHeading(obj4);
...
export const useQuestBarTitle = function useQuestBarTitle(questCreative) {
  ...
  if (null != completedAt) {
    ...
    if (null != enrolledAt) {
      if (tmp7 > 0) {
        const tmpResult1 = tmp(1897);
        const intl3 = tmp(1236).intl;
        obj = { percent: null };
        obj[0] = tmp(1897).formatPercent(stateFromStores, tmp7, { roundingMode: "floor" });
        return intl3.formatToPlainString(tmp(1236).t.lVZaXD, obj);
      }
    }
    if (callback(obj3.useTaskPlatformScreen(questCreative, questTaskDetails), 1)[0] === tmp(5358).TaskPlatformScreen.SELECT) {
      const intl2 = tmp(1236).intl;
      let stringResult = intl2.string(tmp(1236).t.EMrUHQ);
    } else {
      const tmpResult2 = tmp(7437);
      const intl = tmp(1236).intl;
      ...
      isConsoleQuestResult = tmp(7437).isConsoleQuest(questCreative);
    }
    return stringResult;
  }
...
export const getRewardCodeRedemptionInstructions = function getRewardCodeRedemptionInstructions(arg0) {
  ...
  if (platform == null) {
    platform = tmp(5358).QuestRewardCodePlatforms.CROSS_PLATFORM;
  }
  let rewardCodeQuestReward = null;
  if (result) {
    let tmpResult = tmp(10889);
...
export const useModalCtaConfig = function useModalCtaConfig(quest) {
      ...
      if (dependencyMap === 2) {
        ...
        try {
          ...
          if (0 === c1) {
            ...
            obj2[0] = c1;
            obj2[1] = closure_1_0(7441).QuestContentCTA.OPEN_GAME_LINK;
            let tmp5;
            if (callback != null) {
              tmp5 = callback();
            }
            obj2[2] = tmp5;
            obj2[3] = closure_4;
            closure_1_0(11059).openGameLinkDirectly(closure_0, obj2);
            dependencyMap = 3;
            return { value: "HermesInternal", done: "HermesInternal" };
          }
        } catch (tmp11) {
  ...
  obj[0] = quest(10894).getExternalCtaLabel(quest);
