...
prototype["queryLink"] = function queryLink(query) {
  if (this._include(AutocompleterResultTypes.LINK)) {
    ...
    if (type === CodedLink.CodedLinkType.INVITE) {
      const obj2 = {
        type: AutocompleterResultTypes.LINK,
        record: LinkRecord.fromInviteCode(findCodedLinkResult.code),
        score: AutocompleteUtils.calculateScore(11),
      };
      const items = [obj2];
      return items;
    } else {
        ...
        if (null !== pathname) {
          if (isDiscordHostnameResult) {
            if (tmp17Result2.isAppRoute(pathname)) {
              const obj3 = {
                type: AutocompleterResultTypes.LINK,
                record: LinkRecord.fromPath(pathname),
                score: AutocompleteUtils.calculateScore(11),
              };
              const items1 = [obj3];
              let items2 = items1;
