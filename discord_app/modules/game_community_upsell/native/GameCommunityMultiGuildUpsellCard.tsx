...
createStyles.content = { flex: 1, justifyContent: "space-between", marginTop: 32, marginBottom: nativeDefault.space.PX_12, marginHorizontal: nativeDefault.space.PX_12 };
...
let obj2 = { flex: 1, justifyContent: "space-between", marginTop: 32, marginBottom: nativeDefault.space.PX_12, marginHorizontal: nativeDefault.space.PX_12 };
...
createStyles.memberCount = { display: "flex", flexDirection: "row", alignItems: "center", marginBottom: nativeDefault.space.PX_12, gap: 6 };
...
export default function GameCommunityMultiGuildUpsellCard(guild) {
  ...
  const callback2 = obj.useCallback(loading(function*() {
    if (c4 === 2) {
      c4 = 3;
      throw new TypeError("Generator functions may not be called on executing generators");
    } else if (tmp6 === 3) {
  ...
  const size = { shape: guild(onDismiss[20]).CutoutShape.RoundedRect, x: 12, y: 54, width: 64, height: 64, cornerRadius: gameId(onDismiss[10]).radii.lg + 4 };
  const items9 = [size];
  obj1.cutouts = items9;
  if (null != memo1) {
  ...
  let obj4 = { style: tmp.guildIconContainer, children: closure_14(stateFromStores1, { style: tmp.guildIcon, source: memo }) };
  ...
  const items11 = [closure_14(gameId(onDismiss[21]), obj8), closure_14(guild(onDismiss[23]).Text, { variant: "heading-md/bold", color: "mobile-text-heading-primary", accessibilityRole: "header", style: tmp.guildName, lineClamp: 1, children: guild.name })];
