...
import { MAX_DISPLAYED_UPSELL_GUILDS as closure_8 } from "MAX_DISPLAYED_UPSELL_GUILDS" /* 15528 */;
...
let result = require("obj132").fileFinishedImporting("modules/game_community_upsell/native/GuildUpsellChannelList.tsx");
...
export default function GuildUpsellChannelList(style) {
  ...
  const memo = youBarTotalHeight.useMemo(() => {
    let obj = { variant: "heading-md/bold", color: "mobile-text-heading-primary", children: null };
    ...
    items[2] = closure_1_10(closure_1_5, {
      ...
      children: closure_1_13.map((item, index) => {
        let obj = { style: closure_0.templateIconWrapper, children: closure_1_10(item.Icon, { width: 114, height: 64 }) };
        ...
        const intl = closure_1_0(callback2[8]).intl;
        obj1[3] = intl.string(item.title);
        const items1 = [closure_1_10(closure_1_0(callback2[15]).Text, obj1), ];
        const obj2 = { variant: "text-sm/medium", color: "text-subtle", children: null };
        const intl2 = closure_1_0(callback2[8]).intl;
        obj2[2] = intl2.string(item.description);
        items1[1] = closure_1_10(closure_1_0(callback2[15]).Text, obj2);
        obj[0] = items1;
        items[1] = closure_1_11(closure_1_6, obj);
        obj[3] = items;
        return closure_1_11(closure_1_0(callback2[16]).Card, obj, item.id);
      })
    });
    ...
    if (tmpResult) {
      ...
      obj5[0] = lib.joinSection;
      const obj6 = { variant: "heading-md/bold", color: "mobile-text-heading-primary", children: null };
      const intl5 = lib(callback2[8]).intl;
      obj6[2] = intl5.string(lib(callback2[8]).t.rJRote);
      const items2 = [closure_1_10(lib(callback2[15]).Text, obj6), ];
      const obj7 = { variant: "text-sm/medium", color: "text-subtle", style: null, children: null };
      obj7[2] = lib.descriptionSpacing;
      const intl6 = lib(callback2[8]).intl;
      obj7[3] = intl6.string(lib(callback2[8]).t.pJT2DK);
      items2[1] = closure_1_10(lib(callback2[15]).Text, obj7);
      obj5[1] = items2;
      tmpResult = closure_1_11(closure_1_6, obj5);
    }
    items[4] = tmpResult;
    obj[1] = items;
