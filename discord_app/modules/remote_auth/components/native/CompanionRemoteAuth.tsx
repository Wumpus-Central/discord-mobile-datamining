...
const result = require("obj132").fileFinishedImporting("modules/remote_auth/components/native/CompanionRemoteAuth.tsx");
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  const effect = obj1.useEffect(() => {
    if (null != fingerprint) {
      ...
      const obj = fingerprint(dependencyMap[16]);
      fingerprint(dependencyMap[16]).sendAuthUrl("https://discord.com/ra/" + tmp).catch((error) => {
        error = new Error("Failed to initialize authentication");
        throw error;
      });
      const sendAuthUrlResult = fingerprint(dependencyMap[16]).sendAuthUrl("https://discord.com/ra/" + tmp);
    }
  }, items1);
  ...
  if (navigation(15254).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(15254).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(15254).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        obj2[3] = context;
        const items2 = [callback(tmp2(1297).Avatar, obj2), , , ];
        ...
        items2[2] = callback(tmp2(4734).Text, obj4);
  ...
  const items3 = [callback(navigation(5446).ActivityIndicator, {}), , ];
  ...
  items3[1] = callback(navigation(4734).Text, obj9);
  ...
  obj11[3] = callback1;
  obj10[1] = callback(navigation(4745).Button, obj11);
