...
function OrbCheckoutModalScreen(startTime) {
  ...
  let obj1 = onRedeemVirtualCurrency(13252);
  ...
  const items5 = [closure_11(onRedeemVirtualCurrency(8508).ModalContent, obj)];
  obj1 = {
    children: closure_11(OrbCheckoutModalFooter, {
      onPress: closure_6(() => {
        ...
        onRedeemVirtualCurrency(() => {
          ...
          let arr = orbRedemptionError(4809);
          arr = arr.pop();
        });
      }, items4),
    }),
  };
  items5[1] = closure_11(onRedeemVirtualCurrency(12020).ModalFooter, obj1);
  obj.children = items5;
  return closure_12(onRedeemVirtualCurrency(8507).ModalScreen, obj);
}
const size = fn(2);
...
export default function _default(skuId) {
  ...
  const callback = current.useCallback(() => {
    ...
    const obj = {
      load_id: current,
      application_id: VirtualCurrencyUtils.get1PShopApplicationIdForSKU(skuId),
  ...
  obj.headerLeft = skuId(analyticsLocations[19]).getHeaderTextButton(
