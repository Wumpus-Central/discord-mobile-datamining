// === Module 13468: OrbCheckoutModalComponents ===
...
import OrbCheckoutAmountTagDefault from "OrbCheckoutAmountTag" /* 11287 */;
import OrbCheckoutModalContext from "OrbCheckoutModalContext" /* 13467 */;
import useVirtualCurrencyBalance from "useVirtualCurrencyBalance" /* 13469 */;
...
export const OrbCheckoutLegalFinePrint = function OrbCheckoutLegalFinePrint() {
  const tmp = closure_8();
  skuId = skuId(13467).useOrbCheckoutModalContext().skuId;
  const items = [skuId];
  const memo = noop.useMemo(() => OrbCheckoutUtils.getOrbCheckoutDisclaimerMessage(skuId), items);
  return closure_6(skuId(4753).Text, { style: tmp.disclaimer, variant: "text-xxs/normal", color: "interactive-text-active", children: memo });
};
