...
function hydrateUserMention(everyoneOrHere, channelId) {
  ...
  if (null != str) {
    ...
    if (null != channel) {
      ...
      if (nickname == null) {
        nickname = nameFromUserDefault.getName(str);
        const tmp4Result = nameFromUserDefault;
      }
      str = nickname;
    }
    tmp2 = str;
...
obj[24] = {
  ...
  parse(arg0, arg1, returnMentionIds) {
    if (returnMentionIds.returnMentionIds) {
      ...
      const mapped = items.map((item, index) => "" + closure_12 + item);
      const _HermesInternal = HermesInternal;
...
let result = require("obj132").fileFinishedImporting("modules/markup/MarkupRules.tsx");
...
export const hydrateCommandMention = function hydrateCommandMention(commandName, commandId, channelId) {
  ...
  const mapped = items.map((item, index) => "" + closure_12 + item);
  { type: "commandMention", channelId: channelId.channelId, commandId, commandName, commandKey: "" + commandId + mapped.join(""), content: null };
  const obj = { type: "text", content: "" + commandName };
  const items1 = [obj];
  obj[5] = items1;
  return obj;
};