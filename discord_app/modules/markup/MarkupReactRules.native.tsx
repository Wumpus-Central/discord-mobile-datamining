...
import SignPostIcon2 from "SignPostIcon" /* 13916 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj.children = node(8149).smartOutput(node, output, obj);
  return closure_15(node(4601).Text, obj, state.key);
}
    ...
    fn = () => {
      if (null != node.roleId) {
        if (null != node.guildId) {
          ...
          obj5.openLazy(asyncRequireImpl(11658, dependencyMap.paths), "RoleMembersActionSheet", obj, "stack");
        }
      }
...
function MarkupInlineCode(arg0) {
  ...
  style.children = node(8149).smartOutput(node, output, state);
...
function MarkupAttachmentLink(state) {
  ...
  obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(576).colors.MENTION_FOREGROUND, source: node(13915), size: null };
  const fontScale = closure_4.getFontScale();
  if (fontScale < 1) {
  ...
  obj.size = SMALL;
  const items = [closure_15(state(1178).ThemedIcon, obj), state(8149).smartOutput(node, output, state)];
  obj.children = items;
  const tmp2Result = closure_16(state(4601).Text, obj, state.key);
...
let link = { emoji: { width: EMOJI_CHAT_SIZE, height: EMOJI_CHAT_SIZE, resizeMode: "contain" }, guildIcon: { paddingEnd: 2, paddingBottom: 1 }, list: { paddingTop: 16 }, listItem: { paddingTop: 4 }, bullet: { fontFamily: Fonts.CODE_BOLD }, strong: { fontFamily: Fonts.PRIMARY_BOLD } };
fn(4605);
link = { link: null, channelMention: null, channelMentionText: null, mention: null, inlineCode: null, codeBlock: null, blockQuote: null };
link = { fontFamily: Fonts.PRIMARY_BOLD, color: nativeDefault.colors.TEXT_LINK };
link.link = link;
const createStyles = { backgroundColor: nativeDefault.colors.MENTION_BACKGROUND, borderRadius: 3, paddingHorizontal: 2, alignItems: "center" };
...
link.inlineCode = { fontFamily: Fonts.CODE_BOLD, color: nativeDefault.colors.TEXT_DEFAULT, backgroundColor: nativeDefault.colors.BACKGROUND_CODE };
let obj4 = { fontFamily: Fonts.CODE_BOLD, color: nativeDefault.colors.TEXT_DEFAULT, backgroundColor: nativeDefault.colors.BACKGROUND_CODE };
link.codeBlock = { fontFamily: Fonts.CODE_BOLD, color: nativeDefault.colors.TEXT_DEFAULT, backgroundColor: nativeDefault.colors.BACKGROUND_CODE };
let obj5 = { fontFamily: Fonts.CODE_BOLD, color: nativeDefault.colors.TEXT_DEFAULT, backgroundColor: nativeDefault.colors.BACKGROUND_CODE };
  ...
  return {
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = obj(5048).RULES[obj(undefined, 5046).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          const obj1 = { color: textColor1, children: tmp(8149).smartOutput(icon, output, textColor) };
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: obj(576).colors.MENTION_FOREGROUND, source: obj(5078).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = closure_1_4.getFontScale();
        if (fontScale < 1) {
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, state) {
        obj = { node, state };
        return closure_1_15(obj(13920), obj, state.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8149).smartOutput(level, output, obj), " "];
          obj.children = items;
          ...
          const items1 = [obj(8149).smartOutput(level, output, obj1), "\n"];
          obj.children = items1;
