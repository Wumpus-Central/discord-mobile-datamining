...
export const useMainViewTooltipActionSheetMap = function useMainViewTooltipActionSheetMap() {
  ...
  let stateFromStores = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items, () =>
    UserSettingsProtoStore.hasLoaded(constants.PRELOADED_USER_SETTINGS),
  );
  ...
  const stateFromStores1 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items1, () =>
    UserRequiredActionStore.hasAction(),
  );
  if (stateFromStores) {
  ...
  const tmp7 = _slicedToArray(
    isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStoresArray(items2, () => {
      const items = [,];
      ({ shouldShowGooglePlayPriceChange: arr[0], priceChangeRecord: arr[1] } = GooglePlayPriceChangeStore);
      return items;
    }),
  ...
  const promotionMarketingComponent = isEligibleToShowGiftingBadgeCoachmark(13789).usePromotionMarketingComponent(
    tmp(11087).MarketingComponentType.MOBILE_BOTTOM_SHEET,
  );
  let oneofKind;
  ...
  const stateFromStores3 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items4, () => {
    const marketingComponentByType = PromotionsStore.getMarketingComponentByType(
      isEligibleToShowGiftingBadgeCoachmark(11087).MarketingComponentType.GIFT_ICON_COACHMARK,
    );
  ...
  const stateFromStores4 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(items5, () => {
    const marketingComponentByType = PromotionsStore.getMarketingComponentByType(
      isEligibleToShowGiftingBadgeCoachmark(11087).MarketingComponentType.GIFT_REMINDER_COACHMARK,
    );
    let prop = null;
    if (null != marketingComponentByType) {
      prop = null;
  ...
  const stateFromStores5 = isEligibleToShowGiftingBadgeCoachmark(504).useStateFromStores(
    items7,
    () => null != badgeById.getBadgeById(isEligibleToShowGiftingBadgeCoachmark(8532).BadgeId.GIFTING),
  );
  if (isDismissed) {
    isDismissed = tmp(4649).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(
      tmp(2028).DismissibleContent.GIFTING_PROMOTION_MOBILE_FIRST_TIME_HALFSHEET,
      stateFromStores2,
    ).isDismissed;
  ...
  if (isDismissed2) {
    isDismissed2 = tmp(4649).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(
      tmp(2028).DismissibleContent.GIFTING_PROMOTION_REMINDER,
      stateFromStores2,
    ).isDismissed;
  ...
  const nitroFileUploadAnnouncementEligible =
    isEligibleToShowGiftingBadgeCoachmark(17526).useNitroFileUploadAnnouncementEligible(MainViewTooltipActionSheets);
  const tmpResult30 = isEligibleToShowGiftingBadgeCoachmark(17526);
  const nitroFileUploadUpsellEligible =
    isEligibleToShowGiftingBadgeCoachmark(17526).useNitroFileUploadUpsellEligible(MainViewTooltipActionSheets);
  ...
  const shouldShowRobloxConnectionCoachmark =
    isEligibleToShowGiftingBadgeCoachmark(17501).useShouldShowRobloxConnectionCoachmark();
  ...
  const shouldShowConnectionDeprecationBottomSheet1 =
    isEligibleToShowGiftingBadgeCoachmark(17511).useShouldShowConnectionDeprecationBottomSheet(obj5);
  const tmpResult34 = isEligibleToShowGiftingBadgeCoachmark(17511);
  const isDisplayNameStylesFlywheelSettersEnabled =
    isEligibleToShowGiftingBadgeCoachmark(10081).useIsDisplayNameStylesFlywheelSettersEnabled(
  ...
  if (stateFromStores) {
    ...
    const obj14 = {
      isEligible: dismissibleContent === tmp(2028).DismissibleContent.PREMIUM_MARKETING_MOMENT_ANNOUNCEMENT_UPSELL,
      newSnowflakeId: null,
      actionSheetProperties: null,
    };
    let promotionId;
    if (promotionMarketingComponent != null) {
    ...
    const obj16 = {
      isEligible: dismissibleContent1 === tmp(2028).DismissibleContent.PREMIUM_MARKETING_MOMENT_REMINDER_UPSELL,
      newSnowflakeId: null,
      actionSheetProperties: null,
    };
    let promotionId2;
