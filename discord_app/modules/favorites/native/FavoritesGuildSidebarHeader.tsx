...
function EmptyBody() {
  const callback = React.useCallback(() => {
    if (obj.getFavoritesAccess().hasAccess) {
      callback(table[7])({ source: "favorites_empty_sidebar" });
    } else {
      const tmp3Result = callback(table[8]);
      tmp3Result.openLazy(callback(table[10])(table[9], table.paths), callback(table[11]).FAVORITES_UPSELL_SHEET_KEY, { source: "favorites_empty_sidebar" });
      const tmp4 = callback(table[10])(table[9], table.paths);
    }
...
function PlaceholderRows() {
  const tmp = callback4();
  const obj = { size: "sm", color: ThemesDefault.colors.ICON_MUTED };
...
const result = require("obj132").fileFinishedImporting("modules/favorites/native/FavoritesGuildSidebarHeader.tsx");
...
export default function FavoritesGuildSidebarHeader() {
  ...
  const items = [tmp7, , ];
  obj = { spacing: ThemesDefault.space.PX_8, style: tmp.copy, children: null };
  ...
  obj[2] = items1;
  items[1] = callback(Stack.Stack, obj);
  let tmp3Result = null;
  if (!tmp2) {
    const obj2 = { children: null };
    const obj3 = { style: null };
    obj3[0] = tmp.divider;
    const items2 = [callback2(View, obj3), callback2(PlaceholderRows, {})];
    obj2[0] = items2;
    tmp3Result = callback(closure_8, obj2);
  }
  items[2] = tmp3Result;
  obj[1] = items;
