// === Module 10936: FavoritesGuildAddChannelModal ===
...
import { MAX_FAVORITES_ADD_CHANNEL_COUNT as closure_7 } from "date" /* 1424 */;
...
export default function FavoritesGuildAddChannelModal(parentId) {
  ...
  height = first(1492)({ ignoreKeyboard: true }).height;
  const items1 = [height];
  let obj = {
    style: React.useMemo(() => {
      height = "100%";
  ...
  const tmp2 = first;
  const tmp4 = first(10937)();
  let intl = parentId(1233).intl;
  obj[0] = intl.string(first(3178).Rp35U1);
  obj[1] = parentId(10934).closeFavoritesGuildAddChannelModal;
  const items2 = [callback2(first(10941), obj), ];
  obj = { style: tmp.container, children: null };
  const items3 = [callback2(first(5078), { absolute: true }), , ];
  ...
  let num = 0;
  const tmp12 = first(10941);
  if (first.length > 0) {
    num = tmp2(709).space.PX_80;
  }
  obj1[4] = num;
  obj1[7] = first.length >= closure_7;
  items3[1] = callback2(first(10942), obj1);
  let obj2 = { isVisible: first.length > 0, floatingBackgroundColor: tmp.container.backgroundColor, text: null, onPress: null };
  const tmp14 = first(10942);
  obj2[2] = parentId(10955).getFavoritesAddButtonLabel(first.length);
  obj2[3] = callback1;
  items3[2] = callback2(parentId(10953).ModalFloatingAction, obj2);
  obj[1] = items3;
