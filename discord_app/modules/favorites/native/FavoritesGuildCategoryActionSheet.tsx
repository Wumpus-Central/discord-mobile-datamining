...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  let obj = {
    header: closure_5(category(7480).BottomSheetTitleHeader, { title: onClose(4980)(category, true) }),
    children: null,
  };
  let tmp7Result = null;
  if (null != tmp3) {
    ...
    obj3.icon = closure_5(tmp4(7530).ActionSheetRow.Icon, obj4);
    obj3.onPress = function onPress() {
      closure_2.perform();
      onClose();
    };
    obj2.children = closure_5(tmp4(7530).ActionSheetRow, obj3);
    tmp7Result = closure_5(tmp4(7530).ActionSheetRow.Group, obj2);
  }
  ...
  if (setting) {
    ...
    obj9.icon = closure_5(tmp4(7530).ActionSheetRow.Icon, obj10);
    obj9.onPress = function onPress() {
      ClipboardUtils.copy(category.id);
      ToastUtils.presentIdCopied();
      onClose();
    };
    obj8.children = closure_5(tmp4(7530).ActionSheetRow, obj9);
    tmp7Result2 = closure_5(tmp4(7530).ActionSheetRow.Group, obj8);
  }
  items[2] = tmp7Result2;
  obj.children = items;
  return closure_6(category(7528).ActionSheet, obj);
}
const jsxProd = fn(21);
