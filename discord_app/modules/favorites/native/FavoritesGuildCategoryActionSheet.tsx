// === Module 16449: FavoritesGuildCategoryActionSheet ===

// Module 16449 (FavoritesGuildCategoryActionSheet)
import ToastUtils from "ToastUtils" /* 4453 */;
import ClipboardUtils from "ClipboardUtils" /* 7434 */;
import openFavoritesGuildCategorySettingsModalDefault from "openFavoritesGuildCategorySettingsModal" /* 16450 */;
...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  const tmp3 = onClose(11241)(category);
  dependencyMap = tmp3;
  const DeveloperMode = category(2019).DeveloperMode;
  const setting = DeveloperMode.useSetting();
  let obj = { header: closure_5(category(7394).BottomSheetTitleHeader, { title: onClose(4909)(category, true) }), children: null };
  let tmp7Result = null;
  if (null != tmp3) {
    const obj2 = { hasIcons: true, children: null };
    const obj3 = { label: tmp3.label, icon: null, onPress: null };
    const obj4 = { IconComponent: tmp4(11216).PlusLargeIcon };
    obj3.icon = closure_5(tmp4(7444).ActionSheetRow.Icon, obj4);
    obj3.onPress = function onPress() {
      closure_2.perform();
      onClose();
    };
    obj2.children = closure_5(tmp4(7444).ActionSheetRow, obj3);
    tmp7Result = closure_5(tmp4(7444).ActionSheetRow.Group, obj2);
  }
  ...
  obj6.icon = closure_5(category(7444).ActionSheetRow.Icon, { IconComponent: category(7622).SettingsIcon });
  obj6.onPress = function onPress() {
    openFavoritesGuildCategorySettingsModalDefault(category.id);
    onClose();
  };
  obj5.children = closure_5(category(7444).ActionSheetRow, obj6);
  items[1] = closure_5(category(7444).ActionSheetRow.Group, obj5);
  ...
  if (setting) {
    ...
    const obj10 = { IconComponent: tmp4(10889).IdIcon };
    obj9.icon = closure_5(tmp4(7444).ActionSheetRow.Icon, obj10);
    obj9.onPress = function onPress() {
      ClipboardUtils.copy(category.id);
      ToastUtils.presentIdCopied();
      onClose();
    };
    obj8.children = closure_5(tmp4(7444).ActionSheetRow, obj9);
    tmp7Result2 = closure_5(tmp4(7444).ActionSheetRow.Group, obj8);
  }
  items[2] = tmp7Result2;
  obj.children = items;
  return closure_6(category(7442).ActionSheet, obj);
}
const jsxProd = fn(21);
