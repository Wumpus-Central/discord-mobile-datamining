// === Module 15546: FavoritesGuildCategoryActionSheetConnected ===
...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  const tmp3 = onClose(12407)(category);
  dependencyMap = tmp3;
  const DeveloperMode = category(4134).DeveloperMode;
  const setting = DeveloperMode.useSetting();
  let obj = { header: callback(category(5568).BottomSheetTitleHeader, { title: onClose(4638)(category, true) }), children: null };
  let tmp7Result = null;
  if (null != tmp3) {
    ...
    obj1[0] = tmp4(9923).PlusLargeIcon;
    obj[1] = tmp7(tmp4(5938).ActionSheetRow.Icon, obj1);
    obj[2] = function onPress() {
      closure_2.perform();
      onClose();
    };
    obj[1] = tmp7(tmp4(5938).ActionSheetRow, obj);
    tmp7Result = tmp7(tmp4(5938).ActionSheetRow.Group, obj);
  }
  ...
  const tmp2 = onClose(4638)(category, true);
  const tmp6 = closure_6;
  obj3[1] = callback(category(5938).ActionSheetRow.Icon, { IconComponent: category(6186).SettingsIcon });
  obj3[2] = function onPress() {
    onClose(closure_2[12])(category.id);
    onClose();
  };
  obj2[1] = callback(category(5938).ActionSheetRow, obj3);
  items[1] = callback(category(5938).ActionSheetRow.Group, obj2);
  tmp7Result = null;
  if (setting) {
    ...
    obj7[0] = tmp4(9764).IdIcon;
    obj6[1] = tmp7(tmp4(5938).ActionSheetRow.Icon, obj7);
    ...
    obj5[1] = tmp7(tmp4(5938).ActionSheetRow, obj6);
    tmp7Result = tmp7(tmp4(5938).ActionSheetRow.Group, obj5);
  }
  items[2] = tmp7Result;
  obj[1] = items;
  return tmp6(category(5936).ActionSheet, obj);
}
