...
function FavoritesGuildCategoryActionSheetConnected(category) {
  ...
  let dependencyMap;
  const tmp3 = onClose(12194)(category);
  dependencyMap = tmp3;
  const DeveloperMode = category(4034).DeveloperMode;
  const setting = DeveloperMode.useSetting();
  let obj = { header: null, children: null };
  obj[0] = callback(category(5435).BottomSheetTitleHeader, { title: onClose(4534)(category, true) });
  let tmp7Result = null;
  if (null != tmp3) {
    ...
    obj1[0] = tmp4(9751).PlusLargeIcon;
    obj[1] = tmp7(tmp4(5805).ActionSheetRow.Icon, obj1);
    obj[2] = function onPress() {
      _undefined.perform();
      onClose();
    };
    obj[1] = tmp7(tmp4(5805).ActionSheetRow, obj);
    tmp7Result = tmp7(tmp4(5805).ActionSheetRow.Group, obj);
  }
  ...
  obj4[0] = category(6052).SettingsIcon;
  obj3[1] = callback(category(5805).ActionSheetRow.Icon, obj4);
  obj3[2] = function onPress() {
    onClose(_undefined[12])(category.id);
    onClose();
  };
  obj2[1] = callback(category(5805).ActionSheetRow, obj3);
  items[1] = callback(category(5805).ActionSheetRow.Group, obj2);
  tmp7Result = null;
  if (setting) {
    ...
    obj7[0] = tmp4(9592).IdIcon;
    obj6[1] = tmp7(tmp4(5805).ActionSheetRow.Icon, obj7);
    ...
    obj5[1] = tmp7(tmp4(5805).ActionSheetRow, obj6);
    tmp7Result = tmp7(tmp4(5805).ActionSheetRow.Group, obj5);
  }
  items[2] = tmp7Result;
  obj[1] = items;
  return closure_6(category(5803).ActionSheet, obj);
}
