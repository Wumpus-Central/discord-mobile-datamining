// === Module 16551: FavoritesGuildSuggestedChannels ===
...
import SearchableDestinationListRowDefault from "SearchableDestinationListRow" /* 16553 */;
import handleFavoritesGuildAddSuggestedChannelDefault from "handleFavoritesGuildAddSuggestedChannel" /* 16554 */;
...
let closure_8 = fn(10380).getScaledCategoryRowHeight;
const jsxProd = fn(21);
...
export default function FavoritesGuildSuggestedChannels() {
  let tmp = closure_13();
  const categoryStyles = arr(16456).useCategoryStyles();
  arr = closure_5();
  ...
  if (0 !== arr.length) {
    ...
    const items2 = [tmp2(16456).renderCategoryItem(obj3), ];
    const obj4 = {
      style: tmp.rows,
      children: arr.map((item, index) => {
          ...
          obj3.onPress = function onPress() {
            const tmp = closure_1(16554);
            return tmp(arr(11253).getDestinationIdFromResult(closure_0));
          };
