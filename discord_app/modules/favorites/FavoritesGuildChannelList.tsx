...
import initializeDefault from "../threads/LazyLoadedThreadManager.tsx";
import _slicedToArray from "../../../_runtime/metro/00032__slicedToArray.js";
import noop from "../../../_runtime/00019_noop.js";
import importDefaultResult1 from "../gateway/GatewayConnectionStore.tsx";
import importDefaultResult from "../activities/EmbeddedActivitiesStore.tsx";
import importDefaultResult3 from "../threads/ActiveJoinedThreadsStore.tsx";
import importDefaultResult4 from "../threads/JoinedThreadsStore.tsx";
import createChannelRecord from "../../records/ChannelRecord.tsx";
import importDefaultResult5 from "../../stores/CategoryCollapseStore.tsx";
import importDefaultResult6 from "../../stores/ChannelStore.tsx";
import handleConnectionOpen from "../../stores/CollapsedVoiceChannelStore.tsx";
import importDefaultResult7 from "../../stores/PermissionStore.tsx";
import importDefaultResult8 from "../../stores/ReadStateStore.tsx";
import importDefaultResult9 from "../../stores/SelectedChannelStore.tsx";
import importDefaultResult10 from "../../stores/UserGuildSettingsStore.tsx";
import importDefaultResult11 from "FavoriteStore.tsx";
import { createFavoritesGuildChannelRecord as closure_19 } from "utils/createFavoritesGuildChannelRecord.tsx";
import importDefaultResult2 from "../guild_scheduled_events/GuildScheduledEventStore.tsx";
import { ChannelListChannelNoticeRow as closure_22 } from "../guild_sidebar/GuildSidebarConstants.tsx";
...
function computeFavoritesState(favoriteChannels, arg1) {
  ...
  obj = {
    ...
    getShownChannelIds() {
      return items.map((item, index) => item.id);
    },
  ...
  Object.defineProperty(obj, "channelList", {
    get: function() {
      if (null == closure_6) {
        ...
        const mapped = _undefined(12)(items).map((item, index) => {
          if (!item.isPrivate()) {
            if (!closure_2_14.can(closure_2_23.VIEW_CHANNEL, item)) {
              return null;
            }
          }
          obj = closure_1_2;
          ...
          if (tmp4) {
            let tmp5 = obj.id === item.id;
            if (!tmp5) {
              tmp5 = closure_1_3 === item.id;
            }
            tmp4 = tmp5;
          }
          const tmp7 = null != obj && obj.isThread() && obj.parent_id === item.id;
          if (!tmp4) {
            if (!tmp7) {
              if (closure_1) {
                let activeJoinedUnreadThreadsForParent = closure_2_6.getActiveJoinedUnreadThreadsForParent(item.guild_id, item.id);
              }
              if (activeJoinedUnreadThreadsForParent == null) {
                activeJoinedUnreadThreadsForParent = {};
              }
              const obj3 = items(closure_2_2[20]);
              const threadIds = obj3.computeThreadIds(item, activeJoinedUnreadThreadsForParent, obj, closure_1_3, closure_1);
              const isCollapsedResult = closure_2_13.isCollapsed(item.id);
              if (item.isThread()) {
                let isMutedResult = closure_2_7.isMuted(item.id);
              } else {
                isMutedResult = closure_2_17.isChannelMuted(item.guild_id, item.id);
              }
              ...
              if (!tmp4) {
                if (!tmp7) {
                    ...
                    if (closure_2_15.getMentionCount(item.id) <= 0) {
                      ...
                      if (closure_1) {
                        if (!isMutedResult) {
                          if (!closure_2) {
                            if (!items2(item.type)) {
                              if (closure_2_9(item.type)) {
                                if (false === closure_2_15.hasUnread(item.id)) {
                                  return null;
                                }
                              }
                            }
                  ...
                  tmp28Result = callback(closure_2_2[24]);
                }
              }
              return obj;
            }
          }
          activeJoinedUnreadThreadsForParent = closure_2_6.getActiveJoinedRelevantThreadsForParent(item.guild_id, item.id);
        });
  ...
  let found = favoritesCategories.filter((item, index) => null != item.id);
  let mapped = found.map((item, index) => {
    const id = item.id;
    items = undefined;
    c1 = undefined;
    const categoryRecord = importDefaultResult11.getCategoryRecord(id);
    if (null == categoryRecord) {
      ...
      obj[0] = importDefaultResult10.isChannelMuted(closure_1_20, id);
      ...
      Object.defineProperty(obj, "channelList", {
        get: function() {
            if (null == closure_1) {
              ...
              const mapped = _null(closure_1_2[24])(self).map((item, index) => {
                if (!item.isPrivate()) {
                  if (!closure_2_14.can(closure_2_23.VIEW_CHANNEL, item)) {
                    return null;
                  }
  ...
  return {
    ...
    isPlaceholderRow(arg0, arg1) {
      ...
      let tmp5 = !tmp3;
      if (!tmp3) {
        tmp5 = 0 === found1[arg0 - favoriteChannels(undefined, 5272).SECTION_INDEX_FIRST_NAMED_CATEGORY].channelList.length;
      }
      return tmp5;
    },
    getCategoryFromSection(arg0) {
      if (arg0 === favoriteChannels(5272).SECTION_INDEX_UNCATEGORIZED_CHANNELS) {
        ...
        tmp4 = found1[arg0 - favoriteChannels(undefined, 5272).SECTION_INDEX_FIRST_NAMED_CATEGORY];
      }
      return tmp4;
    },
      ...
      while (iter !== undefined) {
        ...
        for (const item10019 of channelList) {
          ...
          for (const item10027 of threadIds) {
            let channel = importDefaultResult6.getChannel(item10027);
            if (null != channel) {
              let tmp10 = arg0(tmp8);
            }
            continue;
...
const result = require("obj132").fileFinishedImporting("modules/favorites/FavoritesGuildChannelList.tsx");
...
export const useFavoritesGuildChannelList = function useFavoritesGuildChannelList(arg0) {
  ...
  let _require = flag;
  let callback;
  let hasAccess = require("FavoritesHooks.tsx").useFavoritesAccess("FavoritesGuildChannelList").hasAccess;
  const tmp3 = callback(React.useState(() => computeFavoritesState(undefined, { withSuggestionsNotice: c0 })), 2);
  ...
  const effect = React.useEffect(() => {
    dependencyMap(computeFavoritesState(undefined, { withSuggestionsNotice: closure_0 }));
    closure_0 = hasAccess(12).throttle(function recompute() {
      callback(closure_1_26(undefined, { withSuggestionsNotice: closure_0 }));
    }, 100);
    let item = items.forEach((item, index) => item.addChangeListener(closure_0));
    return () => {
      closure_0.cancel();
      const item = items.forEach((item, index) => item.removeChangeListener(closure_0));
    };
  }, items);
  ...
  let flag2 = false;
  if (memo.getSections().length <= require("../guild_sidebar/ChannelListState.tsx").SECTION_INDEX_FIRST_NAMED_CATEGORY) {
    _require = false;
