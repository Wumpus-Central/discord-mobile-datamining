...
export const useFavoritesGuildConfig = function useFavoritesGuildConfig(location) {
  ...
  return {
    enabled: config.enabled || closure_0.useConfig({ location: _location }).enabled,
    isFreemium: config.enabled,
  };
};
export const getFavoritesGuildConfig = function getFavoritesGuildConfig(location) {
  ...
  return {
    enabled: config.enabled || closure_0.getConfig({ location: _location }).enabled,
    isFreemium: config.enabled,
  };
};
