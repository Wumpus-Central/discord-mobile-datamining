...
import { resetFatigueCooldown } from "../dismissible_content/DismissibleContentShownStateStore.tsx";
...
import closure_10 from "FavoriteStore.tsx";
  ...
  if (obj.size(arg0) >= closure_13) {
    ...
    if (favoriteLimit > 0) {
      ...
      if (arg1 !== create.FavoriteChannelType.CATEGORY) {
        tmp6 = null;
        if (
          tmp2Result.filter(arg0, (type) => type.type !== callback(table[11]).FavoriteChannelType.CATEGORY).length >=
          favoriteLimit
        ) {
            ...
            obj3[0] = function update(favoriteChannels) {
              ...
              while (iter !== undefined) {
                ...
                let tmp11 = closure_1_21(
                  ...
                  callback(1306).FavoriteChannelType.REFERENCE_ORIGINAL,
                );
                let tmp12 = tmp11;
                if (null != tmp11) {
                  let tmp54 = flag;
                  ...
                  let FavoriteChannel = callback(1306).FavoriteChannel;
                  let obj = { nickname: "", type: null, channelType: null, position: null, parentId: null };
                  let tmp34 = callback;
                  let tmp35 = dependencyMap;
                  obj[1] = callback(1306).FavoriteChannelType.REFERENCE_ORIGINAL;
                  obj = undefined;
                  if (null != channel) {
                    let tmp37 = callback;
                    let tmp38 = dependencyMap;
                    let UInt32Value = callback(1337).UInt32Value;
                    obj = { value: null };
                    let tmp39 = channel;
                    obj[0] = tmp31.type;
                  ...
                  let result = obj3.trackFavoritesGuildAddToFavorites(
                    tmp49,
                    type,
                    closure_1_20(favoriteChannels.favoriteChannels),
                  );
                  continue;
                }
                if (flag) {
    ...
    obj[0] = function update(favoriteChannels) {
      ...
      if (flag) {
        ...
        const result = obj.trackFavoritesGuildRemoveFromFavorites(
          tmp9,
          flag(favorite[13]).filter(
            favoriteChannels.favoriteChannels,
            (type) => type.type !== callback(table[11]).FavoriteChannelType.CATEGORY,
          ).length,
        );
                ...
                obj1[0] = function update(favoriteChannels) {
                  ...
                  if (obj.size(favoriteChannels) >= closure_1_13) {
                    ...
                    if (favoriteLimit > 0) {
                      tmp6 = null;
                      if (
                        callback(tmp2[11]).FavoriteChannelType.CATEGORY !== tmp(tmp2[11]).FavoriteChannelType.CATEGORY
                      ) {
...
export const setFavoriteCategoriesCollapsed = function setFavoriteCategoriesCollapsed(arg0, id) {
  ...
  PreloadedUserSettingsActionCreators.updateAsync(
    "favorites",
    function update(favoriteChannels) {
      let tmp = closure_1;
      if (null != closure_1) {
        const items = [tmp];
        let keys = items;
    ...
    require("../user_settings/UserSettingsProtoActionCreators.tsx").UserSettingsDelay.FREQUENT_USER_ACTION,
    onSaveFailed,
  );
};
