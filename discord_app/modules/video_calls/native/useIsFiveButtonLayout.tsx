// === Module 9487: useIsFiveButtonLayout ===
...
export const useIsFiveButtonLayout = function useIsFiveButtonLayout(id) {
  ...
  const tmp = _require;
  let isConnectedToVoiceChannel = _require(9463).useIsConnectedToVoiceChannel(stateFromStores);
  let guild_id;
  ...
  const tmp6 = guild_id(9488);
  const tmp6Result = guild_id(9488)(guild_id1, id);
  ...
  const tmp5Result = guild_id(9489);
  const tmp5ResultResult = guild_id(9489)(id1);
