...
const require = fn;
noopAll;
const result = require("obj132").fileFinishedImporting("modules/video_calls/native/components/DisconnectRemoteButton.tsx");
...
export const DisconnectRemoteButton = function DisconnectRemoteButton(channel) {
  ...
  obj[3] = function onPress() {
    if (null != remoteSessionId) {
      channel(dependencyMap[8]).remoteDisconnect(tmp);
      const obj2 = channel(dependencyMap[8]);
      channel(dependencyMap[9]).handleDisconnect(channel);
      const obj3 = channel(dependencyMap[9]);
    } else {
      channel(dependencyMap[8]).disconnectRemote();
