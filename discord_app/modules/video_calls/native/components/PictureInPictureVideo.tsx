...
let closure_20 = noop.memo((channel) => {
    ...
    if (tmp8.STREAM !== type1) {
      if (tmp8.USER === type1) {
        ...
        if (tmp6(tmp2[25])(pipParticipant)) {
          tmp15 = null;
          if (!stateFromStores) {
            obj = { participant: pipParticipant, avatarSize: null, resizeMode: null, onSingleTap: null, onDoubleTap: null };
  ...
  if (pipParticipant.user.id === AuthenticationStore.getId()) {
    ...
    const obj4 = { removeEmptyStateButton: true, removeEmptyStateImage: true, resizeMode: tmp(tmp2[24]).ResizeMode.CONTAIN, participant: pipParticipant, onSingleTap: onPipTap, onDoubleTap: onPipTap };
...
let closure_22 = noop.memo((arg0) => {
  ...
  [tmp5, tmp6] = _slicedToArray(obj.useStateFromStoresArray(items, () => {
  ...
  [arr5, tmp9] = _slicedToArray(obj2.useStateFromStores(items2, () => {
