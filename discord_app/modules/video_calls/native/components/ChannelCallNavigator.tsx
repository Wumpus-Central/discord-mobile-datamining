...
function CallWithVoiceChat(channel) {
  ...
  obj = { interpolate: tmp5(4310).interpolate, translateX, width: result };
  fn2.__closure = obj;
  ...
  obj = { runOnJS: tmp5(4310).runOnJS, setShouldRenderChat: tmp12 };
  fn4.__closure = obj;
  ...
  if (tmp11[0]) {
    const obj6 = { channel };
    tmp18Result = closure_12(tmp2(10108), obj6);
  }
  obj5.children = tmp18Result;
  items3[1] = closure_12(closure_6, obj5);
  obj1.children = items3;
  return closure_13(translateX(4310).View, obj1);
}
function MainCallScreen(channel) {
  ...
  let obj1 = isConnectedToVoiceChannel(4785);
  ...
  if (isChannelContentGated) {
    obj = { onReturnToSafety: id(4777).pop, guildId: null, channelId: null };
    ({ guild_id: obj6.guildId, id: obj6.channelId } = channel);
    let tmp14Result = closure_12(id(12695), obj);
    ...
    if (channel.isGuildStageVoice()) {
      let obj2 = { channel };
      tmp14Result = closure_12(id(9691), obj2);
    }
  ...
  return tmp11(tmp22, { forceHide: isConnectedToVoiceChannel(1115).isAndroid() || !isConnectedToVoiceChannel, showWhenParticipantOnScreen: !isConnectedToVoiceChannel, children: tmp11(isConnectedToVoiceChannel(4284).ThemeContextProvider, { gradient: tmp21, children: tmp14Result }) });
}
...
export default function ChannelCallNavigator(channel) {
  ...
  obj.children = closure_12(channel(7014).Navigator, obj);
