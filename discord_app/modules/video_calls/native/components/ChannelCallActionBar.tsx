...
class DisconnectStreamButton {
  constructor(arg0) {
    ...
    items[0] = View;
    items[1] = View;
    f85938 = obj.useStateFromStores(items, () => {
...
function LeaveActivityButton(isSmallSize) {
  ...
  obj[2] = importDefault(10885);
  obj[3] = isSmallSize.isSmallSize;
  return callback(importAll(10727).PrimaryActionButton, obj);
}
function useActionBarSecondButton(channel) {
  channel = channel.channel;
  let obj = require(10392) /* useIsConnectedToVoiceChannel */;
  ...
  if (isActive) {
    isActive = importDefault(10867)(channel).isActive;
  }
...
export default function ChannelCallActionBar(arg0) {
  ...
  const tmp4 = importDefault(5771)();
  let obj = require(10730) /* useIsFiveButtonLayout */;
  const isFiveButtonLayout = obj.useIsFiveButtonLayout(channel.id);
  obj = { value: importDefault(5581)(tmp2(5601).CHANNEL_CALL_ACTION_BAR).analyticsLocations, children: null };
  ...
  if (shouldShowConnectingScreen) {
    ...
    obj1[0] = channel;
    let tmp13Result = tmp11(tmp9(10886).CallConnectingActionBar, obj1);
  } else {
    ...
    items[2] = tmp11(tmp9(10910).ChannelCallMicButton, obj5);
    if (tmp7.END_REMOTE === END_REMOTE) {
      const obj6 = { channel: null, isSmallSize: null };
      obj6[0] = channel;
      obj6[1] = isFiveButtonLayout;
      let tmp11Result2 = tmp11(tmp9(10920).DisconnectRemoteButton, obj6);
  ...
  return closure_14(require(5581) /* context */.AnalyticsLocationProvider, obj);
};
export { VideoButton };
