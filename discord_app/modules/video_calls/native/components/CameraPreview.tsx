...
function CameraPreview(arg0) {
  ...
  const tmp6 = reveal(stateFromStores(10658)(), 2);
  dependencyMap = tmp7;
  let obj1 = React;
  reveal = React.useContext(ref(10653).RevealContext).reveal;
  let tmp8 = stateFromStores(10666)();
  React = tmp8;
  let tmp9 = callback();
  c5 = tmp9;
  const tmp10 = stateFromStores(10654)(channel.id);
  c6 = tmp10;
  let obj2 = ref(4849);
  ...
  const OrientationType = ref(8895).OrientationType;
  const tmp14 = isScreenLandscape ? OrientationType.LANDSCAPE : OrientationType.PORTRAIT;
  callback = tmp14;
  const tmp15 = stateFromStores(8853)(tmp14);
  let tmp16 = tmp15;
  if (tmp15 == null) {
  ...
  const animatedStyle = ref(4036).useAnimatedStyle(ee);
  ref = obj1.useRef(null);
  const tmpResult = ref(4036);
  ...
  if (constants2.HIDE_PIP === tmp27) {
    ...
    obj2[1] = tmp(10674).openIgnoreThermalStateAlert;
    const items5 = [obj2];
    items6 = items5;
  } else {
    items6 = [];
  ...
  let tmp4Result = tmp4(10659);
  const obj9 = { ref, disabled: null, trigger: null, rows: null, onOpen: null, onClose: null };
  tmp4Result = tmp4(10676);
  if (isViewingActivity) {
    isViewingActivity = stateFromStores;
  }
  obj9[1] = !isViewingActivity;
  obj9[2] = callback2(stateFromStores(10677), { channel, selfParticipant, pipParticipant: nonSelfPipParticipant });
  obj9[3] = items6;
  ...
  obj5[2] = callback2(stateFromStores(4036).View, obj6, str);
...
export default function CameraPreviewContainer(channel) {
  ...
  const tmp19 = flag(10661)(channel.id);
  let tmp20 = null;
  if (null != tmp19) {
    tmp20 = null;
