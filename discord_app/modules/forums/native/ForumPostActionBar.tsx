...
export default function ForumPostActionBar(channel) {
  ...
  if (tmp11) {
    ...
    obj1[2] = firstMessage;
    obj1[3] = tmp7[0];
    obj[1] = callback(tmp2(10313).ForumPostActionBarReactions, obj1);
  ...
  if (stateFromStores2) {
    ...
    obj3[3] = function onPress() {
      ...
      if (null != guildId) {
        let obj = channel(8533);
        obj = { section: null };
        obj[0] = outer1_9.CHANNEL_HEADER;
        const result = obj.handleCopyLinkForumPost(guildId, channel.id, obj);
      }
    };
    obj3[4] = callback(tmp2(4229).LinkIcon, { size: "xs" });
    items7[1] = callback(tmp2(4827).PressableOpacity, obj3);
    ...
    obj4[3] = function onPress() {
      ...
      obj[3] = channel(4217).JumpType.ANIMATED;
      obj.jumpToMessage(obj);
    };
    obj4[4] = callback(tmp2(10314).ArrowLargeUpIcon, { size: "xs" });
    items7[2] = callback(tmp2(4827).PressableOpacity, obj4);
    obj2[2] = items7;
    ...
    let items9 = function handleFollow() {
      const result = channel(8533).handleToggleFollowForumPost(channel, stateFromStores1);
    };
    ...
    if (stateFromStores1) {
      ...
      obj5[3] = items9;
      intl = callback;
      items9 = [callback(tmp2(4237).CheckmarkLargeIcon, { size: "xs", color: "text-brand" }), ];
      Text = tmp2(4281).Text;
      ...
      obj5[2] = tmp.actionButton;
      obj5[3] = items9;
      const items11 = [callback(tmp2(9059).BellIcon, { size: "xs" }), ];
      ...
      items11[1] = callback(tmp2(4281).Text, obj7);
      obj5[4] = items11;
      tmp15 = obj5;
    }
    tmp9(tmp2(4827).PressableOpacity, tmp15);
  }
};