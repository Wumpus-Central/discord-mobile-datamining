...
export default function ForumPostActionBar(channel) {
  ...
  if (tmp11) {
    ...
    obj1[2] = firstMessage;
    obj1[3] = tmp7[0];
    obj[1] = callback(tmp2(10356).ForumPostActionBarReactions, obj1);
  ...
  if (stateFromStores2) {
    ...
    items7[1] = callback(tmp2(4887).PressableOpacity, obj3);
    ...
    obj4[4] = callback(tmp2(10357).ArrowLargeUpIcon, { size: "xs" });
    items7[2] = callback(tmp2(4887).PressableOpacity, obj4);
    obj2[2] = items7;
    ...
    let items9 = function handleFollow() {
      const result = channel(8659).handleToggleFollowForumPost(channel, stateFromStores1);
    };
    ...
    if (stateFromStores1) {
      ...
      obj5[2] = tmp.actionButton;
      obj5[3] = items9;
      const items11 = [callback(tmp2(9186).BellIcon, { size: "xs" }), ];
    ...
    tmp9(tmp2(4887).PressableOpacity, tmp15);
  }
};