// === Module 11757: ForumPostReactions ===

// Module 11757 (ForumPostReactions)
import ForumHooks from "ForumHooks" /* 8214 */;
import ForumPostReactionButton from "ForumPostReactionButton" /* 10561 */;
import useReactionPermissionsDefault from "useReactionPermissions" /* 11657 */;
...
export const MaxForumPostReactions = function MaxForumPostReactions(thread) {
  ...
  const tmp = closure_6();
  dependencyMap = tmp;
  const disableReactionCreates = reactionContainerStyle(11657)(thread).disableReactionCreates;
  let num = 28;
  if (disableReactionCreates) {
    num = 0;
  }
  const maxPossibleForumPostReactions = thread(8214).useMaxPossibleForumPostReactions({ containerWidth: containerWidth - num, reactionEmojiWidth: 46, digitWidth: 7.5, message: firstMessage, parentChannel });
  ...
  ,
    ...
    if (tmp8) {
      ...
      tmp8 = closure_4(tmp3(10561).AdditionalReactionCount, obj4);
    }
    items1[1] = tmp8;
    let tmp10 = !disableReactionCreates;
    if (!disableReactionCreates) {
      const obj5 = { containerStyle: reactionContainerStyle, threadId: thread.id };
      tmp10 = closure_4(tmp3(10561).AddReactionButton, obj5);
    }
    items1[2] = tmp10;
...
export const ForumPostActionBarReactions = function ForumPostActionBarReactions(thread) {
  ...
  obj2.count = num;
  const someForumPostReactions = thread(8214).useSomeForumPostReactions(obj2);
