...
export default function ForumPostListBody(arg0) {
  ...
  if (isNew) {
    ...
    isNew = callback(require("../ForumPostNewTag.tsx"), obj1);
  }
  const items1 = [isNew, callback(require("../ForumPostUsername.tsx") /* ForumPostUsername */.ForumPostAuthor, { thread, hasUnreads }), callback(require("../ForumPostTimestamp.tsx"), { thread, hasUnreads, format: ForumTimestampFormats.POSTED_DURATION_AGO })];
  obj[1] = items1;
  const items2 = [closure_6(View, obj), callback(require("../ForumPostTitle.tsx"), { title: thread.name, lineClamp: 2, ellipsizeMode: "tail", hasUnreads }), callback(require("../ForumPostMessageContent.tsx"), { messageContent, message: firstMessage, isMessageDeleted: false, messageLoaded: firstMessageLoaded, hasUnreads, senderModifier })];
  obj[1] = items2;
  ...
  if (!blocked) {
    ...
    if (null != media) {
      ...
      tmp7Result = callback(require("../ForumPostMedia.tsx") /* ForumPostMediaAndroid */.ForumPostMediaThumbnail, obj4);
    }
  }
  items3[1] = tmp7Result;
  obj[1] = items3;
