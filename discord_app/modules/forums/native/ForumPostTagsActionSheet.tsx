...
export default function ForumPostTagsActionSheet(thread) {
  ...
  let appliedTags = thread(7605).useAppliedTags(thread);
  if (null != tags) {
    appliedTags = tags;
  }
  let obj = thread(7605);
  ...
  const visibleForumTags = thread(7605).useVisibleForumTags(thread.parentChannel);
  let obj2 = {
    onDismiss() {
      let tmp;
  ...
  obj2.header = toggleTag(thread(7482).BottomSheetTitleHeader, obj3);
  const tmp4Result = thread(7605);
  let items = [
    toggleTag(closure_5, {
  ...
  obj5.children = toggleTag(thread(5273).Button, obj6);
  items[1] = toggleTag(closure_5, obj5);
  obj2.children = items;
  return closure_8(thread(7530).ActionSheet, obj2);
}
