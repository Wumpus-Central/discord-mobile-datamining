...
const prototype = function ForumChannelSeenManager(channelId) {
  ...
  const FORUM_CHANNEL = maybeMarkSeen /* maybeMarkSeen */.AnalyticsFeedTypes.FORUM_CHANNEL;
  ...
  tmp.sessionId = collectForumAnalyticsMetadata /* collectForumAnalyticsMetadata */.getForumChannelSessionId(channelId);
  return tmp;
}.prototype;
...
export const getForumPostSeenManagerId = function getForumPostSeenManagerId(arg0) {
  return "" + maybeMarkSeen /* maybeMarkSeen */.AnalyticsFeedTypes.FORUM_CHANNEL + "_" + arg0;
};
export const markForumPostItemAsSeen = function markForumPostItemAsSeen(parent_id, item, timestampMillis) {
  const result = markAnalyticsFeedItemSeen /* markAnalyticsFeedItemSeen */.markAnalyticsFeedItemSeen("" + maybeMarkSeen /* maybeMarkSeen */.AnalyticsFeedTypes.FORUM_CHANNEL + "_" + parent_id, item, timestampMillis);
};
export const markForumPostItemAsUnseen = function markForumPostItemAsUnseen(parent_id, item, timestampMillis) {
  const result = markAnalyticsFeedItemSeen /* markAnalyticsFeedItemSeen */.markAnalyticsFeedItemUnseen("" + maybeMarkSeen /* maybeMarkSeen */.AnalyticsFeedTypes.FORUM_CHANNEL + "_" + parent_id, item, timestampMillis);
};