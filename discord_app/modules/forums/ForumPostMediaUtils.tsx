...
function useForumPostEmbeds(embeds, flag) {
  const _require = flag;
  const InlineEmbedMedia = _require(4166).InlineEmbedMedia;
  const setting = InlineEmbedMedia.useSetting();
  const RenderEmbeds = _require(4166).RenderEmbeds;
  if (null == embeds) {
    return [];
  } else {
  ...
  if (null == firstResult) {
    ...
    if (tmp4) {
      if (null != components) {
        ...
        items1 = Array.from(tmp(4740).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
  ...
  if (null == components) {
    ...
    if (tmp3) {
      if (null != components) {
        ...
        const flattenComponentsResult = tmp(4740).flattenComponents(components);
        const arr = Array.from(tmp(4740).flattenComponents(components).values());
        let found = Array.from(tmp(4740).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
          if (callback(table[12]).ComponentType.THUMBNAIL === type) {
            ({ media, spoiler } = type);
            if (spoiler == null) {
  ...
  if (null == firstMessage) {
    ...
    if (tmp5) {
      if (null != components) {
        ...
        const flattenComponentsResult = tmp(4740).flattenComponents(components);
        const arr = Array.from(tmp(4740).flattenComponents(components).values());
        items = Array.from(tmp(4740).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
          if (callback(table[12]).ComponentType.THUMBNAIL === type) {
            ({ media, spoiler } = type);
            if (spoiler == null) {
      ...
      if (tmp2) {
        ...
        if (!tmp7) {
          ...
          tmp7 = null == attachments.find((filename) => {
            ...
            if (null != filename) {
              ({ height, width } = filename);
              let tmp3 = callback(4671).isImageFile(filename.filename) && null != height;
              if (tmp3) {
                tmp3 = height > 0;
              }
              if (tmp3) {
            ...
            if (!flag) {
              ...
              if (tmp4) {
                let isVideoFileResult = null != filename;
                if (isVideoFileResult) {
                  isVideoFileResult = callback(4671).isVideoFile(filename.filename);
                  const obj2 = callback(4671);
                }
                if (isVideoFileResult) {
                  isVideoFileResult = null != filename.proxy_url;
                }
