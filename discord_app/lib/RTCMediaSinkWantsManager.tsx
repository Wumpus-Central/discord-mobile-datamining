...
import WindowVisibilityVideoManager2 from "WindowVisibilityVideoManager.tsx";
...
const SimulcastOverrideQuality = fn(4854).SimulcastOverrideQuality;
let c7 = 100;
...
class RTCMediaSinkWantsManager extends tmp2 {
  constructor(arg0, arg1, arg2) {
    ...
    tmp11.updateOffscreenUsers = function updateOffscreenUsers() {
      connection = closure_0.connection;
        ...
        while (tmp14 !== undefined) {
          ...
          if (null != tmp21) {
            ...
            if (hasActiveVideoOutputSink) {
              ...
              closure_0.offscreenUsers[tmp20] = timestamp;
              let addLruResult = closure_0.addLru(tmp20, timestamp, items);
            } else if (!closure_0.offscreenDisabledUsers[tmp20]) {
              let diff = timestamp - closure_0.offscreenUsers[tmp20];
              if (diff >= closure_0.getOffscreenTimeoutMs()) {
                closure_0.offscreenDisabledUsers[tmp20] = true;
              } else {
                let addLruResult1 = closure_0.addLru(tmp20, timestamp, items);
              }
            }
          }
          continue;
    ...
    tmp11.update = function update() {
      ...
      let tmp9 = tmp5;
      if (obj2.isWeb()) {
        closure_0.invertWants(tmp5, wantsLevel);
        let tmp13 = tmp5;
        if (closure_11) {
          obj3 = {};
      ...
      if (isDesktopResult) {
        isDesktopResult = closure_0.isOneToOneCall();
      }
      if (isDesktopResult) {
        isDesktopResult = !closure_0.isStageChannel;
      }
      const tmp6Result = PlatformUtils;
      const entries = SnowflakeUtilsDefault.entries(closure_0.videoSsrcs);
      while (tmp22 !== undefined) {
        ...
        let wantsLevel1 = closure_0.getWantsLevel(num);
        let ssrc = arr2[0].ssrc;
        if (closure_0.shouldReceiveFromUser(tmp27)) {
          let tmp37 = tmp27 === closure_0.selectedParticipantId;
          if (tmp37) {
            tmp37 = wantsLevel !== c7;
          }
          if (tmp37) {
            tmp37 = !closure_0.pipOpen;
          }
          let tmp41 = tmp37;
          if (arr2.length > 1) {
            for (const item10119 of arr2) {
              if (item10119.quality === c7) {
                let ssrc2 = item10119.ssrc;
                if (tmp41) {
                  tmp5[ssrc2] = tmp53;
                  ssrc = item10119.ssrc;
                } else {
                ...
                tmp5[item10119.ssrc] = 0;
              } else {
                if (isDesktopResult) {
                  tmp5[item10119.ssrc] = wantsLevel1;
                }
                ssrc = item10119.ssrc;
              }
              continue;
            ...
            if (closure_0.supportsSeamless) {
              if (!tmp62.framesReceived[ssrc]) {
                ...
                for (const item10150 of arr2) {
                  ...
                  if (tmp70) {
                    tmp70 = closure_0.framesReceived[item10150.ssrc];
                  }
                  if (!tmp70) {
                    continue;
                  } else {
                    if (item10150.quality === c7) {
                      tmp5[item10150.ssrc] = tmp76;
                    } else {
                      let tmp78 = wantsLevel;
                      if (isDesktopResult) {
                        tmp78 = wantsLevel1;
                      }
                      tmp5[item10150.ssrc] = tmp78;
                    }
                    let arr = items1.push(item10150.ssrc);
                  }
                }
              }
            }
        ...
        let tmp96 = closure_0.supportsSeamless && flag;
        if (!tmp96) {
          let items3 = [ssrc];
          items1 = items3;
        }
      ...
      if (!isEqualResult) {
        isEqualResult = _modDef12.isEqual(closure_0.latestWants, tmp5);
      }
      if (!isEqualResult) {
        closure_0.latestWants = tmp5;
        closure_0.emit(obj3.Update, tmp5);
      }
      return tmp5;
    };
    ...
    onResult = WindowVisibilityVideoManager.on(
      tmp13(tmp14[10]).WindowVisibilityEvent.IncomingVideoEnabledChanged,
...
prototype["reset"] = function reset() {
  ...
  WindowVisibilityVideoManager.off(
    WindowVisibilityVideoManager2.WindowVisibilityEvent.IncomingVideoEnabledChanged,
