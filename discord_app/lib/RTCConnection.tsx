...
prototype["destroy"] = function destroy() {
  ...
  WindowVisibilityVideoManager2.off(require(10607) /* isIncomingVideoEnabled */.WindowVisibilityEvent.WindowVisibilityChanged, self.windowVisibilityChanged);
  self._cancelReconnect();
  self._cleanupSocket();
...
prototype["getDuration"] = function getDuration() {
  ...
  if (this._connectCompletedTime > 0) {
    num = require(4360) /* sleep */.now() - tmp._connectCompletedTime;
...
prototype["setState"] = function setState(s) {
  ...
  this.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.State, s, obj, obj);
};
...
prototype["_chooseExperiments"] = function _chooseExperiments(items) {
  ...
  const config = importDefault(5105).getConfig({ location: "_chooseExperiments" });
  if (config.enabled) {
  ...
  if (isAndroidResult) {
    isAndroidResult = tmp8(10603).isSurfaceDirectRendererExperimentEnabled();
    const tmp8Result1 = tmp8(10603);
  }
  if (isAndroidResult) {
    items.push(tmp8(10603).ANDROID_SURFACE_DIRECT_RENDERER_EXPERIMENT);
  }
...
prototype["_connectMediaEngineWithEndpoint"] = function _connectMediaEngineWithEndpoint(_sfuEndpoint, _readyData) {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.Speaking, (arg0, _lastSentSpeakingStatus, _lastSentSSRC) => {
    if (self.userId === arg0) {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.NativeMuteChanged, (arg0) => {
    if (self.context === outer1_25.DEFAULT) {
      _sfuEndpoint(_undefined[49]).nativeMuteChanged(arg0);
      const obj = _sfuEndpoint(_undefined[49]);
    }
  });
  connectResult.on(_require(4381).BaseConnectionEvent.Video, (userId, streamId, audioSsrc, arg3, arg4, arr) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.FirstFrame, (arg0, arg1, arg2) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.Silence, (arg0) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.Connected, (protocol, sdp) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.VideoEncoderFallback, (arr) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.VideoDecoderFallback, (arr) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.VideoCodecError, (codecStandard) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.Error, (error) => {
    let obj = self;
    if (socket === self._socket) {
      let preferredRegion = null;
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.ConnectionStateChange, (arg0) => {
    const logger = self.logger;
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.SecureFramesUpdate, (_secureFramesState) => {
  ...
  connectResult.on(_require(4381).BaseConnectionEvent.Ping, _handlePing.bind(self));
  const _handlePingTimeout = self._handlePingTimeout;
  connectResult.on(_require(4381).BaseConnectionEvent.PingTimeout, _handlePingTimeout.bind(self));
  const _handleOutboundLossRate = self._handleOutboundLossRate;
  connectResult.on(_require(4381).BaseConnectionEvent.OutboundLossRate, _handleOutboundLossRate.bind(self));
  const _handleLocalVideoDisabled = self._handleLocalVideoDisabled;
  connectResult.on(_require(4381).BaseConnectionEvent.LocalVideoDisabled, _handleLocalVideoDisabled.bind(self));
  tmp14 = constants5;
  const tmp2Result1 = _require(500);
  connectResult.on(_require(4381).BaseConnectionEvent.Stats, importDefault(6842).create());
  const _handleRemoteStreamsReady = self._handleRemoteStreamsReady;
  connectResult.on(_require(4381).BaseConnectionEvent.RemoteStreamsReady, _handleRemoteStreamsReady.bind(self));
  const handleUsersMerged = self.handleUsersMerged;
  connectResult.on(_require(4381).BaseConnectionEvent.UsersMerged, handleUsersMerged.bind(self));
  connectResult.on(_require(4381).BaseConnectionEvent.NoiseCancellationError, (_noiseCancellationError) => {
    self._noiseCancellationError = _noiseCancellationError;
  });
  const _handleMLSFailure = self._handleMLSFailure;
  connectResult.on(_require(4381).BaseConnectionEvent.MLSFailure, _handleMLSFailure.bind(self));
...
prototype["handleFlags"] = function handleFlags(arg0, arg1) {
  this.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.Flags, arg0, arg1);
};
prototype["handlePlatform"] = function handlePlatform(arg0, arg1) {
  this.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.Platform, arg0, arg1, this.channelId);
};
prototype["handleUsersMerged"] = function handleUsersMerged(arr) {
  this.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.UsersMerged, arr.map((id) => id.id), this.context);
};
...
prototype["_handleVideoStreamId"] = function _handleVideoStreamId(arg0) {
  ...
  this.emit(self(4380).RTCConnectionEvent.Video, this.guildId, this.channelId, userId, streamId, this.streamServerId);
  if (tmp2) {
    const logger = self.logger;
...
prototype["_handlePing"] = function _handlePing(arg0) {
  ...
  if (undefined !== arg0) {
    ...
    self.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.Ping, self._pings, self.quality);
  }
};
...
prototype["_handleOutboundLossRate"] = function _handleOutboundLossRate(_outboundLossRate) {
  ...
  this.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.OutboundLossRate, _outboundLossRate);
};
...
prototype["_handleClientConnect"] = function _handleClientConnect(arr) {
  ...
  this.emit(self(4380).RTCConnectionEvent.ClientConnect, arr);
...
prototype["_handleClientDisconnect"] = function _handleClientDisconnect(sender_user_id) {
  ...
  if (null != _videoQuality) {
    if (self.context === constants6.DEFAULT) {
      ...
      if (tmp) {
        ...
        obj = {};
        const merged = Object.assign(self._getAnalyticsProperties());
        obj.app_hardware_acceleration_enabled = importDefault(4279).getAppHardwareAccelerationEnabled();
        obj.media_session_id = self.getMediaSessionId();
        obj.sender_user_id = sender_user_id;
  ...
  self.emit(require(4380) /* RTCConnectionEvent */.RTCConnectionEvent.ClientDisconnect, sender_user_id);
  const _localMediaSinkWantsManager2 = self._localMediaSinkWantsManager;
  ...
  if (1 === self._userIds.size) {
    self._secureFramesLastBecameAloneTime = require(4360) /* sleep */.now();
    const tmp22Result = require(4360) /* sleep */;
  }
};
  ...
  if (!this._voiceConnectionSuccessTracked) {
    if (self.state === constants3.RTC_CONNECTED) {
      if (self._hasCodecs) {
        ...
        obj.connect_time = require(4360) /* sleep */.now() - (1 === self._connectCount ? self._createdTime : self._connectStartTime);
...
prototype["_handleSecureFramesInit"] = function _handleSecureFramesInit(v) {
  ...
  if (v > 0) {
    ...
    const tmpResult = require(4360) /* sleep */;
  } else {
...
prototype["_handleMLSPrepareCommitTransition"] = function _handleMLSPrepareCommitTransition(arg0, arg1) {
  ...
  const dependencyMap = _connection(4360).now();
