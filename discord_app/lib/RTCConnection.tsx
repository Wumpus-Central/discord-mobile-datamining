...
prototype["destroy"] = function destroy() {
  ...
  self.recordEvent(obj);
  const WindowVisibilityVideoManager = tmp5(10756).WindowVisibilityVideoManager;
  ...
  const WindowVisibilityVideoManager2 = tmp5(10756).WindowVisibilityVideoManager;
...
prototype["_chooseExperiments"] = function _chooseExperiments(items) {
  ...
  if (enabled) {
    const BrowserTransceiverPaddingRemovalExperiment = tmp8(13143).BrowserTransceiverPaddingRemovalExperiment;
  ...
  if (tmp8Result.isIOS()) {
    const mode = tmp3(13144).getConfig({ location: "_chooseExperiments" }).mode;
    if ("standard" === mode) {
      items.push("ios_video_stabilization_standard");
    } else if ("low_latency" === mode) {
      items.push("ios_video_stabilization_low_latency");
    }
    const tmp3Result = tmp3(13144);
  }
  tmp8Result = tmp8(500);
  let isAndroidResult = tmp8Result.isAndroid();
  if (isAndroidResult) {
    isAndroidResult = tmp8(10752).isSurfaceDirectRendererExperimentEnabled();
    const tmp8Result1 = tmp8(10752);
  }
  if (isAndroidResult) {
    items.push(tmp8(10752).ANDROID_SURFACE_DIRECT_RENDERER_EXPERIMENT);
  }
...
prototype["getOrCreateVideoQuality"] = function getOrCreateVideoQuality() {
  ...
  if (null != this._connection) {
    if (null == self._videoQuality) {
      const videoQuality = new self(13155).VideoQuality(self._connection);
      ...
      const defaultConfig = self(13156).VideoHealthManager.defaultConfig;
      ({ windowLength, allowedPoorFpsRatio, fpsThreshold, backoffTimeSec } = defaultConfig);
      if (defaultConfig.featureEnabled) {
        const videoHealthManager = new tmp10(13156).VideoHealthManager(windowLength, allowedPoorFpsRatio, fpsThreshold, backoffTimeSec);
        self._videoHealthManager = videoHealthManager;
        if (null != self._localMediaSinkWantsManager) {
          self._localMediaSinkWantsManager.videoHealthManager = self._videoHealthManager;
        }
        const _videoQuality = self._videoQuality;
        _videoQuality.on(tmp10(13155).VideoQualityEvent.FpsUpdate, (arg0, arg1, arg2) => {
          const _localMediaSinkWantsManager = self._localMediaSinkWantsManager;
          let result;
          if (_localMediaSinkWantsManager != null) {
...
prototype["_handleVideoStreamId"] = function _handleVideoStreamId(arg0) {
  ...
  this.emit(self(4439).RTCConnectionEvent.Video, this.guildId, this.channelId, userId, streamId, this.streamServerId);
  if (tmp2) {
    const logger = self.logger;
...
prototype["_handleClientConnect"] = function _handleClientConnect(arr) {
  ...
  this.emit(self(4439).RTCConnectionEvent.ClientConnect, arr);
...
prototype["_handleMLSPrepareCommitTransition"] = function _handleMLSPrepareCommitTransition(arg0, arg1) {
  ...
  const dependencyMap = _connection(4419).now();
