...
import getTimeZoneDefault from "getTimeZone" /* 17712 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17713 */;
...
const idGenerator = new fn(17710).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17711).updateDynamicSuperProperties();
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1980)(17716, dependencyMap.paths), statusCode(1980)(5083, dependencyMap.paths)];
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          const promise4 = statusCode(1980)(15958, dependencyMap.paths);
          statusCode(1980)(15958, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          }).catch(arg2);
          flag = true;
          const nextPromise2 = statusCode(1980)(15958, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          });
        }
      }
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1980)(5741, dependencyMap.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1980)(5741, dependencyMap.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1980)(14106, dependencyMap.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1980)(14106, dependencyMap.paths);
      } else {
        ...
        if (flag) {
          tmp7(1980)(17723, dependencyMap.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1980)(17723, dependencyMap.paths);
        }
      }
      tmp7Result = tmp7(17722);
    }
    obj = statusCode(10012);
  }
});
HTTPUtils = fn(1271);
