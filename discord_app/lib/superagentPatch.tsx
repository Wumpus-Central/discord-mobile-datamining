...
import getTimeZoneDefault from "getTimeZone" /* 17781 */;
import trackHttpRequestDefault from "trackHttpRequest" /* 17782 */;
...
const idGenerator = new fn(17779).IdGenerator();
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result1 = tmp(17780).updateDynamicSuperProperties();
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (400 === statusCode.statusCode) {
      ...
      if (captcha_key) {
        const items = [statusCode(1980)(17785, dependencyMap.paths), statusCode(1980)(5167, dependencyMap.paths)];
    ...
    if (401 === statusCode.statusCode) {
      ...
      if (code === AbortCodes.MFA_REQUIRED) {
        ...
        if (mfa) {
          const promise4 = statusCode(1980)(16012, dependencyMap.paths);
          statusCode(1980)(16012, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          }).catch(arg2);
          flag = true;
          const nextPromise2 = statusCode(1980)(16012, dependencyMap.paths).then((openMFAModal) => {
            openMFAModal.openMFAModal(statusCode.body.mfa, closure_1, closure_2);
          });
        }
      }
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1980)(5825, dependencyMap.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1980)(5825, dependencyMap.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1980)(14188, dependencyMap.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1980)(14188, dependencyMap.paths);
      } else {
        ...
        if (flag) {
          tmp7(1980)(17792, dependencyMap.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1980)(17792, dependencyMap.paths);
        }
      }
      tmp7Result = tmp7(17791);
    }
    obj = statusCode(10085);
  }
});
HTTPUtils = fn(1271);
