// === Module 11117: BundleUpdaterManager ===
...
prototype["emitOtaMetric"] = function emitOtaMetric(item10010) {
  const match = require(4455) /* t */.match(item10010);
  ...
  const withResult = match.with({ type: "OtaCheckAttempt" }, (result) => {
    ...
    return callback(6958).increment(closure_9.prepareOtaMetricForDatadog(result, ["result"]));
  });
  match.with({ type: "OtaCheckAttempt" }, (result) => {
    ...
    return callback(6958).increment(closure_9.prepareOtaMetricForDatadog(result, ["result", "statusCode"]));
  }).exhaustive();
};
