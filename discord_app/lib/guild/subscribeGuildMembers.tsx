      ...
      if (!discord_common_shallowEqualDefault(this.props, arg0)) {
        ...
        if (!isEqualResult) {
          if (null != self._subscriptions) {
            const item = _modDef12.forEach(self._subscriptions, (userIds, guildId) => displayName(7552).unsubscribeMembers(guildId, userIds));
            const tmpResult3 = _modDef12;
          }
          const item1 = _modDef12.forEach(tmp4, (userIds, guildId) => displayName(7552).subscribeMembers(guildId, userIds));
