...
prototype["retryOpts"] = function retryOpts() {
  if (this.item.platform === cancel /* cancel */.UploadPlatform.REACT_NATIVE) {
    let obj = { timeout: null, backoff: null, retries: 12 };
    obj[0] = set.Millis.HOUR;
    let tmp8 = fails;
    const result = 0.5 * set.Millis.SECOND;
    tmp8 = new tmp8(result, 30 * set.Millis.MINUTE);
    obj[1] = tmp8;
  } else {
    obj = { timeout: null, retries: 12, backoff: null };
    obj[0] = set.Millis.HOUR;
    const tmp5 = new fails();
    obj[2] = tmp5;
  }
  return obj;
};
prototype["createAttachmentUrlRetryOpts"] = function createAttachmentUrlRetryOpts() {
  ...
  if (this.item.platform === cancel /* cancel */.UploadPlatform.REACT_NATIVE) {
    ...
    obj[0] = 30 * set.Millis.SECOND;
    obj[1] = 30 * set.Millis.MINUTE;
    obj[0] = obj;
    let tmp4 = fails;
    const result = 0.5 * set.Millis.SECOND;
    tmp4 = new tmp4(result, 60 * set.Millis.SECOND);
    obj[1] = tmp4;
...
prototype["supportsResume"] = function supportsResume() {
  ...
  if (!_libdiscoreEnabled) {
    _libdiscoreEnabled = this.item.platform !== cancel /* cancel */.UploadPlatform.REACT_NATIVE;
  }
...
CloudUpload["isResponseUrlStale"] = function isResponseUrlStale(responseUrlSetAt) {
  if (null == responseUrlSetAt) {
    ...
    const _Date = Date;
    const result = 12 * set.Millis.HOUR;
