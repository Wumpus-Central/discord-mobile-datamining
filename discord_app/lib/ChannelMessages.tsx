...
import SortedArrayUtilsAll from "../utils/SortedArrayUtils.tsx";
...
({
  MAX_MESSAGES_PER_CHANNEL: closure_4,
  MAX_LOADED_MESSAGES: hasOwnProperty,
  MAX_MESSAGE_CACHE_SIZE: metroRequire,
  TRUNCATED_MESSAGE_VIEW_SIZE: closure_7,
    ...
    if (typeof MessageCache === "function") {
      ...
      if (typeof MessageCache === "function") {
        ...
        if (ready instanceof Function) {
          ...
          merged.jumpTargetOffset =
            undefined !== ready.jumpTargetOffset ? ready.jumpTargetOffset : self.jumpTargetOffset;
          ...
          merged.jumpReturnTargetId =
            undefined !== ready.jumpReturnTargetId ? ready.jumpReturnTargetId : self.jumpReturnTargetId;
          ...
          merged.revealedMessageId =
            undefined !== ready.revealedMessageId ? ready.revealedMessageId : self.revealedMessageId;
          ...
          merged.initialScrollSequenceId =
            undefined !== ready.initialScrollSequenceId ? ready.initialScrollSequenceId : self.initialScrollSequenceId;
          merged.suppressRowAnimationSequenceId =
            undefined !== ready.suppressRowAnimationSequenceId
              ? ready.suppressRowAnimationSequenceId
...
prototype2["mergeDelta"] = function mergeDelta() {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = items.map((item) => set(4979).createMessageRecord(item));
    const combined = found.concat(
      mapped,
      items1.map((item) => set(4979).createMessageRecord(item)),
    );
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4979).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4979).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
            ...
            if (tmp16Result.getConfig({ location: "receiveMessage" }).enabled) {
              let mutation = self.mutate((_map) => {
                ...
                if (null != _map._map[messageRecord1.id]) {
                  ...
                  SortedArrayUtilsAll.insert(_map._array, messageRecord1, (id, id2) =>
                    closure_1_1(closure_1_3[5]).compare(id.id, id2.id),
                  );
                }
              }, true);
            }
...
prototype2["loadComplete"] = function loadComplete(newMessages) {
  ...
  if (flag) {
    if (null == jump) {
      ...
      if (jumpType == null) {
        jumpType = id(4687).JumpType.ANIMATED;
      }
      let obj2 = {
        ready: true,
        loadingMore: false,
          ...
          if (null == messageId1) {
            ...
            if (null == offset) {
              ...
              if (found2.length > 0) {
                ...
                logger.info(
                  "loadComplete: merging with " +
                    found2.length +
                    " message(s) received during the fetch for channelId=" +
                    self.channelId,
                );
  ...
  found2 = [];
  const iter = reversed.map((item) => id(dependencyMap[2]).createMessageRecord(item));
};
prototype2["addCachedMessages"] = function addCachedMessages(messages, stale) {
  ...
  let item = found1.forEach((item) =>
    SortedArrayUtilsAll.insert(reversed, item, (id, id2) => self(closure_1_3[5]).compare(id.id, id2.id)),
  );
