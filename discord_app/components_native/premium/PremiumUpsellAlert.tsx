...
import registerAssetDefault3 from "../../../_runtime/08153_registerAsset.js";
import registerAssetDefault4 from "../../../_runtime/08154_registerAsset.js";
...
function GlobalEmojiUpsell(arg0) {
  ...
  if (null != skuId) {
    ...
    if (useTier0Description) {
      ...
      obj1 = { planName: null };
      obj1[0] = tmp(4108).getPremiumTypeDisplayName(closure_13.TIER_0);
...
function PremiumGuildIdentityUpsell(alertWidth) {
  ...
  if (obj3.isThemeDark(tmp5)) {
    let tmp4Result = tmp4(8151);
  } else {
    tmp4Result = tmp4(8152);
  }
...
function LongerMessageUpsell(alertWidth) {
  ...
  if (obj3.isThemeDark(tmp5)) {
    let tmp4Result = tmp4(8155);
  } else {
    tmp4Result = tmp4(8156);
  }
  obj = { image: tmp4Result, title: null, description: null };
  const intl = tmp(1236).intl;
...
function UploadUpsell(arg0) {
  ...
  if (callback(importAllResult.useState(!stateFromStores), 1)[0]) {
    ...
    tmp6Result = callback2(tmp(5958).TableSwitchRow, obj);
  }
