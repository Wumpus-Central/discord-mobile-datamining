        ...
        try {
          ...
          if (0 === c4) {
            if (arg0 === 1) {
              ...
              obj1[3] = callback(7419).STEP_ANALYTICS_NAMES[callback(undefined, 7419).CancellationFlowSteps.MOBILE_SUBSCRIPTION_MANAGE];
              const result = callback(7419).trackPremiumSubscriptionCancellationFlowStep(obj1);
              let isPurchasedViaApple;
              if (callback != null) {
                isPurchasedViaApple = tmp15.isPurchasedViaApple;
              }
              if (isPurchasedViaApple) {
                let tmp18Result = tmp18(7466);
                c4 = 1;
                c3 = 1;
...
export default function PremiumSubscriptionDetails(subscription) {
  ...
  if (null != first) {
    ...
    if (tmp9Result) {
      ...
      tmp9Result = tmp9(tmp2(7678), obj2);
    }
    items[1] = tmp9Result;
    ...
    if (tmp9Result) {
      ...
      obj3[0] = subscription;
      tmp9Result = tmp9(tmp2(7679), obj3);
    }
    items[2] = tmp9Result;
...
export const handleManageSubscription = function handleManageSubscription(subscription, registerAsset, analyticsLocations) {
  if (subscription.status === constants3.ACCOUNT_HOLD) {
    ...
    let prop;
    if (hasActiveTrial) {
      prop = tmp9(6918).excludeNitroOnlyPlansForActiveTrial;
    }
    obj[4] = prop;
