// === Module 6554: GoogleManagementLink ===
...
export default function PremiumBillingInfo(subscription) {
  ...
  obj = { subscriptionId: subscription.id, renewal: true, applyEntitlements: true, analyticsLocations: importDefault(5519)(), analyticsLocation: importDefault(5539).PREMIUM_BILLING_INFO };
  const first = callback(obj.useFetchSubscriptionInvoicePreview(obj), 1)[0];
  require(6550) /* createSubscriptionInvoicePreview */;
  if (null == first) {
    return null;
  } else {
    let tmp2Result = tmp2(6555);
    ...
    const items = [callback2(tmp2(4189).Text, obj1), , ];
    ...
    if (null != externalManagementMessage) {
      ...
      tmp11Result = callback2(tmp2(4189).Text, obj6);
    }
    items[2] = tmp11Result;
    obj[1] = items;
