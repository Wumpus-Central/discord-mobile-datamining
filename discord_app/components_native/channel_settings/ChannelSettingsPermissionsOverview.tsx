          ...
          if (0 === c2) {
            if (arg0 === 1) {
              ...
              const syncedPermissionOverwrites = obj8.getSyncedPermissionOverwrites(
                tmp5,
                tmp2(11865).getAppChannelBotUserId(tmp2),
              );
...
function RoleRow(onDelete) {
  ...
  if (isEditing) {
    ...
    if (!tmp) {
      ...
      tmp2Result = value2(CircleMinusIcon.CircleMinusIcon, obj2);
    }
  }
  obj.icon = tmp2Result;
  onDelete = onSelect;
...
function MemberRow(arg0) {
  ...
  if (isEditing) {
    ...
    const intl = util.intl;
    obj2.accessibilityLabel = intl.string(util.t.N86XcP);
    tmpResult = value2(CircleMinusIcon.CircleMinusIcon, obj2);
  }
...
function MemberOverwrites(channel) {
  ...
  const valueResult = _modDef12(useGetOrFetchChannelOverwriteUsersDefault(guild_id, channel.permissionOverwrites))
    .sortBy((username) => username.username.toLowerCase())
    .value();
  let tmp4 = null;
...
export default function ChannelSettingsPermissionsOverview(channelId) {
  ...
  const stateFromStores = channelId(stateFromStores2[29]).useStateFromStores(
    items,
