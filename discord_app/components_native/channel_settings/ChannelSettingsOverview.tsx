        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              c3 = 1;
              const obj4 = {
                value: tmp2(9880).isDefaultChannelThresholdMetAfterDelete(channel.getGuildId(), channel.id),
                done: false,
              };
              return obj4;
            }
...
prototype["renderDefaultForumLayout"] = function renderDefaultForumLayout() {
  ...
  if (!channel.isForumChannel()) {
    ...
    if (channel.defaultForumLayout === GridSquareIcon(1968).ForumLayout.GRID) {
      ...
      tmpResult = closure_47(GridSquareIcon(7203).ListViewIcon, {});
    }
    obj3.icon = tmpResult;
...
prototype["renderDeleteButton"] = function renderDeleteButton() {
  ...
  if (canManageChannels) {
    ...
    if (!hasJoinedThread) {
      ...
      const obj3 = {
        variant: "danger",
        icon: closure_47(tmp8(4595).TrashIcon, { color: "text-feedback-critical" }),
        label: stringResult,
      ...
      const obj = {
        variant: "danger",
        icon: closure_47(tmp8(4578).UserMinusIcon, { color: "text-feedback-critical" }),
        label: null,
        onPress: null,
      };
  ...
  if (channel.isForumLikeChannel()) {
    ...
    if (channel.availableTags.length > 0) {
      ...
      if (tmp3) {
        ...
        obj5.icon = closure_47(canManageChannels(9006).PlusSmallIcon, obj6);
        obj5.onPress = function onPress() {
        ...
        obj4.children = closure_47(canManageChannels(8214).IconButton, obj5);
    ...
    if (channel.availableTags.length <= 0) {
      ...
      obj8.children = closure_47(canManageChannels(4634).Text, obj9);
      tmp10Result3 = closure_47(canManageChannels(5208).PressableOpacity, obj8);
    }
    ...
    if (tmp10Result4) {
      ...
      tmp10Result4 = closure_47(canManageChannels(4634).Text, obj10);
    }
...
prototype["renderCategory"] = function renderCategory() {
  ...
  if (props.hasCategories) {
    if (props.channel.type !== constants3.GUILD_CATEGORY) {
      ...
      const obj2 = {
        icon: closure_47(tmp7(16284).FolderPlusIcon, {}),
        label: null,
        trailing: null,
        arrow: null,
        onPress: null,
