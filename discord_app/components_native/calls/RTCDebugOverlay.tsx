...
function RTCDebugGeneral() {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => ({ guildId: RTCConnectionStore.getGuildId(), channelId: RTCConnectionStore.getChannelId() }), []);
  ...
  if (null != stateFromStores) {
    ...
    if (0 !== stateFromStores.length) {
      mapped = stateFromStores.map((mediaEngineConnectionId) => {
        ...
        let tmp6Result = null;
        if (0 !== keys.length) {
          const obj2 = { title: "inbound", children: keys.map((userId) => closure_2_14(closure_2_23, { userId, data: inbound[userId] }, userId)) };
