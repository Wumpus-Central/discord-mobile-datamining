...
import useChatBottomManagerUIStore from "useChatBottomManagerUIStore" /* 9517 */;
...
export default function JumpToPresentButton(channelId) {
  ...
  obj.style = items3;
  if (tmp5) {
    obj = { accessibilityLabel: stringResult, icon: screenIndex(12299), onPress: channelId.onJumpToPresent };
    let tmp12Result = jsx(screenIndex(12298), { accessibilityLabel: stringResult, icon: screenIndex(12299), onPress: channelId.onJumpToPresent });
    const tmp16 = screenIndex(12298);
  } else {
    tmp12Result = jsx(tmp3(12300).MemoedVoicePanelDismissChatButton, {});
  }
