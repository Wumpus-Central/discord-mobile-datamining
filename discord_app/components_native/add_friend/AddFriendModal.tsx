...
function AddFriendModalScene(onSkip) {
  ...
  sourceMetadata(contactSyncAccount[11])(() => {
    ...
    result.then((result) => {
      ...
      let tmp5 =
        ...
        (onSkip(contactSyncAccount[13]).isAndroid() && result === constants2.UNAUTHORIZED);
      if (!tmp5) {
  ...
  let tmp4 = _slicedToArray(
    navigation.useState(!onSkip(contactSyncAccount[10]).isContactSyncEnabled(contactSyncAccount)),
    2,
  );
