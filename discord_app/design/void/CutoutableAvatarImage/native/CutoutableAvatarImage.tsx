...
function AnimatedNativeCutoutAvatarImage(cutout) {
  ...
  obj = { cutout, CutoutShape: cutout(8233).CutoutShape, withSpring: cutout(4711).withSpring, CHANNEL_SPRING_CONFIG };
  fn.__closure = obj;
  ...
  return callback(cutout(8233).ClipViewAnimated, obj);
}
...
function CutoutAvatarImage(arg0) {
  ...
  if (null != source) {
    if (null != tintColor) {
      ...
      let tmp8 = callback(Circle(7775).ForeignObject, obj);
    ...
    if (CIRCULAR === tmp4.CIRCULAR) {
      ...
      obj4[1] = result;
      obj4[2] = result;
      let tmp10Result = tmp10(Circle(7775).Circle, obj4);
    } else {
      const obj5 = { x: 0, y: 0, height: null, width: null, fill: "white" };
      obj5[2] = size;
      obj5[3] = size;
      tmp10Result = tmp10(Circle(7775).Rect, obj5);
    }
    ...
    obj7[1] = diff1;
    obj7[2] = radius;
    items[1] = tmp10(Circle(7775).Circle, obj7);
    obj3[3] = items;
    obj6[0] = closure_8(Circle(7775).Mask, obj3);
    const items1 = [tmp10(Circle(7775).Defs, obj6), tmp8, ];
    if (null == cutout.border) {
      ...
      tmp10Result = tmp10(Circle(7775).Rect, obj9);
    }
    tmp15 = closure_5;
    tmp9Result = tmp9(7775);
  }
  ...
  tmp8 = callback(Circle(7775).Image, obj10);
  tmp9 = importDefault;
  tmp10 = callback;
}
