...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(style) {
  ...
  const tmp2Result1 = blurTheme(4979);
  class B {
    constructor() {
  ...
  obj = { withSpring: tmp2(4988).withSpring, interpolateColor: tmp2(4310).interpolateColor, pressed, fallbackColor: token, fallbackColorPressed: token1, ON_PRESS_SPRING: tmp2(4992).ON_PRESS_SPRING };
  B.__closure = obj;
  B.__workletHash = 10497618157620;
  B.__initData = __initData;
  let animatedStyle = blurTheme(4310).useAnimatedStyle(B);
  const tmp2Result2 = blurTheme(4310);
  class C {
    constructor() {
  ...
  obj = { shouldUseFallback: isBlurDisabledResult, withSpring: tmp2(4988).withSpring, interpolateColor: tmp2(4310).interpolateColor, pressed, restingTint: tmp6, pressedTint: tmp7, ON_PRESS_SPRING: tmp2(4992).ON_PRESS_SPRING };
  C.__closure = obj;
  C.__workletHash = 11987567486157;
  C.__initData = __initData2;
  const animatedProps = blurTheme(4310).useAnimatedProps(C);
  const obj1 = { blurTheme, blurStyle: memo, style: null };
  const items1 = [token1.absoluteFill, style.style, ];
  const tmp12 = c5;
  const tmp2Result3 = blurTheme(4310);
