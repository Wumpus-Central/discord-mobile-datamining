// === Module 9959: updateContextMenuState ===
...
function updateContextMenuState(absoluteX, absoluteY, closure_17) {
  ...
  if (0 < value.length) {
    ...
    if (activeIndex.get() !== result1) {
      ...
      _mod4115.runOnJS(HapticFeedbackTypes.triggerHapticFeedback)(HapticFeedbackTypes.HapticFeedbackTypes.IMPACT_LIGHT);
      const runOnJSResult = _mod4115.runOnJS(HapticFeedbackTypes.triggerHapticFeedback);
    }
  }
  const result3 = activeIndex.set(-1);
}
keys = { INDEX_BOUNDS_WIDTH_OFFSET: 2, INDEX_BOUNDS_HEIGHT_OFFSET: 3, INDEX_BOUNDS_PAGE_Y_OFFSET: 1, INDEX_BOUNDS_PAGE_X_OFFSET: 0, INDEX_BOUNDS_OFFSET: 4, runOnJS: require("module_4115").runOnJS, triggerHapticFeedback: require("HapticFeedbackTypes").triggerHapticFeedback, HapticFeedbackTypes: require("HapticFeedbackTypes").HapticFeedbackTypes };
...
let result = require("obj132").fileFinishedImporting("design/components/ContextMenu/native/ContextMenuState.native.tsx");
...
export const showContextMenu = function showContextMenu(arg0) {
  const _require = arg0;
  _require(705).batchUpdates(() => keys.setState({ menu: closure_0 }));
};
